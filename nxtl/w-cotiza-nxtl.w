...
DEFINE NEW SHARED VARIABLE S-TPOPED     LIKE Faccpedi.tpoped INIT 'NXT'.
...
&Scoped-Define ENABLED-OBJECTS BTN-Excel B-CCTE Btn-Grupos B-CLI 
...
DEFINE VARIABLE h_b-cotiza-nxtl AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-pedido-nxtl AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-cotiza-nxtl AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotiza-nxtl2 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     Btn-Grupos AT ROW 19.58 COL 37 WIDGET-ID 2
     B-CLI AT ROW 19.58 COL 58 WIDGET-ID 4
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
ASSIGN 
       B-CLI:HIDDEN IN FRAME F-Main           = TRUE.
...
DO:
   RUN Actualiza-datos-cliente IN h_v-cotiza-nxtl2.
END.
...
DO:
  RUN Genera-Excel2 IN h_v-cotiza-nxtl2.
END.
...
DO:
  RUN vtamay/d-cotiza-3.
  RUN dispatch IN h_t-cotiza-nxtl ('open-query').
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'nxtl/v-cotiza-nxtl2.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_v-cotiza-nxtl2 ).
       RUN set-position IN h_v-cotiza-nxtl2 ( 1.00 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'nxtl/q-pedido-nxtl.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-pedido-nxtl ).
       RUN set-position IN h_q-pedido-nxtl ( 8.69 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartObject h_v-cotiza-nxtl2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-cotiza-nxtl2 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-nxtl , 'Record':U , h_v-cotiza-nxtl2 ).

       /* Links to SmartQuery h_q-pedido-nxtl. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido-nxtl ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv02 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido-nxtl ,
             B-CLI:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'nxtl/b-cotiza-nxtl.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotiza-nxtl ).
       RUN set-position IN h_b-cotiza-nxtl ( 10.42 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-cotiza-nxtl ( 10.35 , 125.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotiza-nxtl. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido-nxtl , 'Record':U , h_b-cotiza-nxtl ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotiza-nxtl ,
             B-CCTE:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'nxtl/t-cotiza-nxtl.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotiza-nxtl ).
       RUN set-position IN h_t-cotiza-nxtl ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotiza-nxtl ( 8.88 , 120.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotiza-nxtl. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotiza-nxtl ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotiza-nxtl ,
             B-CCTE:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotiza-nxtl , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ENABLE BTN-Excel B-CCTE Btn-Grupos B-CLI 
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_b-cotiza-nxtl <> ? THEN RUN dispatch IN h_b-cotiza-nxtl ('open-query':U).
          IF h_t-cotiza-nxtl <> ? THEN RUN dispatch IN h_t-cotiza-nxtl ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-cotiza-nxtl <> ? THEN RUN Recalcular-Precios IN h_t-cotiza-nxtl.
      END.
