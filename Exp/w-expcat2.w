...
&Scoped-Define ENABLED-OBJECTS RECT-1 FILL-IN-FchIng BUTTON-1 BUTTON-3 ~
BROWSE-1 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 W-Win _STRUCTURED
  QUERY BROWSE-1 DISPLAY
      t-matg.codmat
      t-matg.DesMat
      t-matg.FchAct COLUMN-LABEL "Fecha de!Actualizacion"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     FILL-IN-FchIng AT ROW 1.38 COL 19 COLON-ALIGNED
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Exportación del Catálogo de Materiales ACTUALIZADOS"
         HEIGHT             = 11.04
         WIDTH              = 64
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 80
...
/* Query rebuild information for BROWSE BROWSE-1
     ...
     _FldNameList[3]   > Temp-Tables.t-matg.FchAct
"FchAct" "Fecha de!Actualizacion" ? "date" ? ? ? ? ? ? no ?
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
ON END-ERROR OF W-Win /* Exportación del Catálogo de Materiales ACTUALIZADOS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Exportación del Catálogo de Materiales ACTUALIZADOS */
DO:
...
DO:
  ...
  THEN DO:
    ...
    FOR EACH almmmatg WHERE almmmatg.codcia = s-codcia
          AND almmmatg.fchact >= INPUT FILL-IN-FchIng 
          NO-LOCK:
      CREATE T-MATG.
  ...
  ELSE DO:
    ...
    ASSIGN
        FILL-IN-FchIng:SENSITIVE = YES
        BUTTON-2:SENSITIVE = NO
        s-Button-1 = YES
        s-Button-2 = NO.
    APPLY 'ENTRY':U TO FILL-IN-FchIng.
  END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-FchIng W-Win
ON RETURN OF FILL-IN-FchIng IN FRAME F-Main /* Nuevos a partir del dia */
DO:
    VIEW FRAME F-Mensaje.
    FOR EACH T-MATG:
        DELETE T-MATG.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 FILL-IN-FchIng BUTTON-1 BUTTON-3 BROWSE-1 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  MESSAGE "Proceso Terminado" SKIP
    "Se ha generado el archivo en c:\tmp\almmmatg.txt" 
