...
DEFINE BUTTON BUTTON-5 
     ...
     LABEL "Button 5" 
     SIZE 6 BY 1.35 TOOLTIP "Importar Cotizaciones".
...
DEFINE BUTTON BUTTON-6 
     ...
     LABEL "Button 6" 
     SIZE 6 BY 1.35 TOOLTIP "Cargar Cotizaciones".
...
ON END-ERROR OF W-Win /* IMPORTAR COTIZACIONES */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* IMPORTAR COTIZACIONES */
DO:
...
DO:
  MESSAGE 'Importamos Cotizaciones?' VIEW-AS ALERT-BOX QUESTION
    BUTTONS YES-NO UPDATE rpta-1 AS LOG.
  IF rpta-1 = NO THEN RETURN NO-APPLY.
  RUN Importar.
...
DO:
  MESSAGE 'Cargamos la información?' VIEW-AS ALERT-BOX QUESTION
    BUTTONS YES-NO UPDATE rpta-1 AS LOG.
  IF rpta-1 = NO THEN RETURN NO-APPLY.
  RUN Carga-Temporal.
END.
...
------------------------------------------------------------------------------*/
  ...
  INPUT FROM c:\importar\faccpedi.d.
  REPEAT:
    CREATE T-CPED.
    IMPORT T-CPED NO-ERROR.
  END.
  INPUT CLOSE.
  INPUT FROM c:\importar\facdpedi.d.
  REPEAT:
    CREATE T-DPED.
    IMPORT T-DPED NO-ERROR.
  END.
  INPUT CLOSE.
  /* DEPURAMOS */
  FOR EACH T-CPED WHERE T-CPED.CodCia <> s-codcia OR T-CPED.nroped = '':
    DELETE T-CPED.
  END.
  FOR EACH T-DPED WHERE T-DPED.CodCia <> s-codcia OR T-DPED.nroped = '':
    DELETE T-DPED.
  END.
  ASSIGN
    x-CodCia = 0
...
------------------------------------------------------------------------------*/
  /* CONSISTENCIA */
  DEF VAR x-NroPed AS CHAR FORMAT 'x(50)' NO-UNDO.

  FOR EACH T-CPED:
    FIND Faccpedi OF T-CPED NO-LOCK NO-ERROR.
    IF AVAILABLE Faccpedi THEN DO:
        IF x-NroPed = ''
        THEN x-NroPed = TRIM(T-CPED.nroped).
        ELSE x-NroPed = x-NroPed + ',' + TRIM(T-CPED.nroped).
    END.
  END.
  IF x-NroPed <> '' THEN DO:
    MESSAGE 'Las sigientes cotizaciones YA ESTAN registradas en el sistema:' SKIP
        TRIM(x-NroPed) SKIP
        'Si continuamos con la importación se van a recrear estas cotizaciones' SKIP
        '¿Continuamos?' VIEW-AS ALERT-BOX WARNING BUTTONS YES-NO UPDATE rpta-1 AS LOG.
    IF rpta-1 = NO THEN RETURN.
  END.
  
  FOR EACH T-CPED ON ERROR UNDO, RETURN NO-APPLY:
    FIND Faccpedi OF T-CPED EXCLUSIVE-LOCK NO-ERROR.
    IF AVAILABLE Faccpedi THEN DO:
    ...
    END.
    CREATE Faccpedi.
    BUFFER-COPY T-CPED TO Faccpedi.
    FOR EACH T-DPED OF T-CPED:
        CREATE Facdpedi.
        BUFFER-COPY T-DPED TO Facdpedi.
        DELETE T-DPED.
    END.
    DELETE T-CPED.
  END.
  MESSAGE 'Importacion completa' VIEW-AS ALERT-BOX.
  {&OPEN-QUERY-BROWSE-1}
  {&OPEN-QUERY-BROWSE-2}
