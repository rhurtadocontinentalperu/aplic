...
AND almcrepo.FlgEst = "P" AND almcrepo.FlgSit = "WL" ~
                           )
...
almcrepo.Glosa 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table almcrepo.Fecha 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      almcrepo.AlmPed COLUMN-LABEL "Despacho" FORMAT "x(3)":U
      almcrepo.Usuario COLUMN-LABEL "Usuario" FORMAT "x(8)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 129 BY 8.65
         FONT 4.
...
"almcrepo.AlmPed" "Despacho" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.almcrepo.Usuario
"almcrepo.Usuario" "Usuario" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"almcrepo.Fecha" "Entrega" ? "date" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[11]   = INTEGRAL.almcrepo.Glosa
...
------------------------------------------------------------------------------*/
    ...
    IF NOT ( {&Condicion} ) THEN DO:
        MESSAGE 'El Pedido de Reposición YA NO se encuentra pendiente de aprobación'
...
------------------------------------------------------------------------------*/
    ...
    FIND CURRENT Almcrepo NO-LOCK NO-ERROR.
    IF NOT ( {&Condicion} ) THEN DO:
        MESSAGE 'Pedido de Reposición YA NO se encuentra pendiente de aprobación'
            VIEW-AS ALERT-BOX WARNING.
        RUN dispatch IN THIS-PROCEDURE ('open-query':U).
        RETURN.
    END.
