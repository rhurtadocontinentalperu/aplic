...
"T-RutaC.FchDoc" "Fecha!Emisión" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.T-RutaC.FchSal
"T-RutaC.FchSal" "Fecha de!Salida" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"T-RutaC.CodVeh" "Placa del!Vehículo" "X(10)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/*       IF AVAILABLE Gn-vehic AND INDEX(gn-vehic.Marca, 'TAXI') > 0 THEN DI-RutaC.CtoRut = FILL-IN_CtoRut. */
      ...
      IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
          MESSAGE 'NO se pudo actualizar las ESCALAS y/o DESTINO FINAL' SKIP
              "Hoja de Ruta:" Di-RutaC.NroDoc
              VIEW-AS ALERT-BOX ERROR.
          UNDO, RETURN 'ADM-ERROR'.
      END.
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    IF fill-in-retorno < DI-RutaC.FchSal THEN DO:
        ...
        RETURN 'ADM-ERROR'.
    END.
    IF DI-RutaC.FchSal = FILL-IN-retorno THEN DO:
...
x-Hora = STRING(lHora,'99') + ':' + STRING(lMinuto,'99') + ' AM'.
IF lHora > 12 THEN x-Hora = STRING(lHora - 12,'99') + ':' + STRING(lMinuto,'99') + ' PM'.
MESSAGE 'La Fecha y Hora de Retorno es la siguiente' SKIP
    'Fecha de Retorno:' STRING(FILL-IN-retorno, '99/99/9999') SKIP
    'Hora de Retorno:' x-Hora SKIP(1)
    'Continuamos con el Cierre de Hoja de Ruta?' SKIP
    VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
    TITLE 'VERIFICACION DE FECHA Y HORA DE RETORNO'
    UPDATE rpta AS LOG.
