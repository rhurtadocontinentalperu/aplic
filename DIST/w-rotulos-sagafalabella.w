...
&Scoped-Define ENABLED-OBJECTS txtCodLPN txtBultos BUTTON-3 
&Scoped-Define DISPLAYED-OBJECTS txtCodLPN txtBultos 
...
DEFINE VARIABLE txtBultos AS INTEGER FORMAT ">,>>9":U INITIAL 0 
     LABEL "Cuantos Bultos" 
     VIEW-AS FILL-IN 
     SIZE 8 BY 1 NO-UNDO.

DEFINE VARIABLE txtCodLPN AS INT64 FORMAT ">>>>>>>>>>>>>>>>>>>9":U INITIAL 100038166022019 
     LABEL "Codigo LPN - inicio" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME F-Main
     txtCodLPN AT ROW 2.54 COL 26 COLON-ALIGNED WIDGET-ID 2
     txtBultos AT ROW 3.88 COL 26.29 COLON-ALIGNED WIDGET-ID 4
     BUTTON-3 AT ROW 6.19 COL 43 WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 62.72 BY 7.69 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Codigos LPN - Sagafalabella"
         HEIGHT             = 7.69
         WIDTH              = 62.72
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 80
...
ON END-ERROR OF W-Win /* Codigos LPN - Sagafalabella */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Codigos LPN - Sagafalabella */
DO:
...
DO:
  ASSIGN txtBultos txtCodLPN.

  IF txtBultos > 0  THEN DO:
    ...
    IF rpta2 = NO THEN RETURN NO-APPLY.
      ...
      DEFINE VAR x-barra AS CHAR.
      DEFINE VAR x-bulto AS INT.
      DEFINE VAR x-bultos AS INT.

      x-bultos = txtBultos.

      REPEAT x-bulto = 1 TO txtBultos:
          x-barra = STRING(txtCodLPN + (x-bulto - 1),"999999999999999").
          RUN imprime-rotulo(INPUT x-barra, INPUT x-bulto, x-bultos). 
      END.
  END.
...
------------------------------------------------------------------------------*/
  DISPLAY txtCodLPN txtBultos 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE txtCodLPN txtBultos BUTTON-3 
      WITH FRAME F-Main IN WINDOW W-Win.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE imprime-rotulo W-Win 
PROCEDURE imprime-rotulo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER pBarra AS CHAR.
DEFINE INPUT PARAMETER pBulto AS INT.
DEFINE INPUT PARAMETER pTotBultos AS INT.

DEFINE VAR x-bultos AS CHAR.

x-bultos = TRIM(STRING(pBulto,">,>>9")) + "  de " + TRIM(STRING(pTotBultos,">,>>9")).

    PUT STREAM REPORTE "^XA^LH010,010" SKIP.

    PUT STREAM REPORTE "^FO050,50" SKIP.
    PUT STREAM REPORTE "^A0N,30,30" SKIP.
    PUT STREAM REPORTE "^FD".
    PUT STREAM REPORTE "O/C    : __________________________________" SKIP.
    PUT STREAM REPORTE "^FS" SKIP.

    PUT STREAM REPORTE "^FO050,100" SKIP.
    PUT STREAM REPORTE "^A0N,30,30" SKIP.
    PUT STREAM REPORTE "^FD".
    PUT STREAM REPORTE "Local  : __________________________________" SKIP.
    PUT STREAM REPORTE "^FS" SKIP.

    PUT STREAM REPORTE "^FO050,150" SKIP.
    PUT STREAM REPORTE "^A0N,30,30" SKIP.
    PUT STREAM REPORTE "^FD".
    PUT STREAM REPORTE "Linea  : __________________________________" SKIP.
    PUT STREAM REPORTE "^FS" SKIP.

    PUT STREAM REPORTE "^FO050,200" SKIP.
    PUT STREAM REPORTE "^A0N,30,30" SKIP.
    PUT STREAM REPORTE "^FD".
    PUT STREAM REPORTE "Albaran : _________________________________" SKIP.
    PUT STREAM REPORTE "^FS" SKIP.

    PUT STREAM REPORTE "^FO050,250" SKIP.
    PUT STREAM REPORTE "^A0N,30,30" SKIP.
    PUT STREAM REPORTE "^FD".
    PUT STREAM REPORTE "Fecha de Entrega : _________________________" SKIP.
    PUT STREAM REPORTE "^FS" SKIP.

    PUT STREAM REPORTE "^FO050,300" SKIP.
    PUT STREAM REPORTE "^A0N,30,30" SKIP.
    PUT STREAM REPORTE "^FD".
    PUT STREAM REPORTE "Bulto   " + x-bultos FORMAT 'x(40)' SKIP.
    PUT STREAM REPORTE "^FS" SKIP.

    PUT STREAM REPORTE "^FO140,350^BY2" SKIP.
    /*PUT STREAM REPORTE "^BCN,N,100,N" SKIP.*/
    PUT STREAM REPORTE "^BCN,100,N,N,N" SKIP.
    PUT STREAM REPORTE "^FD".
    PUT STREAM REPORTE pBarra FORMAT 'x(15)' SKIP.
    PUT STREAM REPORTE "^FS".

    PUT STREAM REPORTE "^FO200,460" SKIP.
...
/*
    ...
    PUT STREAM REPORTE "^A0N,30,30" SKIP.
    PUT STREAM REPORTE "^FD".
    PUT STREAM REPORTE "Nro. PEDIDO :" + lPedido FORMAT 'x(40)' SKIP.
    PUT STREAM REPORTE "^FS".
    ...
    PUT STREAM REPORTE "^A0N,30,30" SKIP.
    PUT STREAM REPORTE "^FD".
    PUT STREAM REPORTE "CLIENTE" FORMAT 'x(25)' SKIP.
    PUT STREAM REPORTE "^FS" SKIP.
