...
&Scoped-define QUERY-STRING-br_table FOR EACH DI-RutaC WHERE ~{&KEY-PHRASE} ~
 ...
 AND DI-RutaC.CodDoc = s-coddoc NO-LOCK ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      fEstado() @ x-Estado COLUMN-LABEL "Situacion" FORMAT "x(21)":U
            WIDTH 22.86
...
"fEstado() @ x-Estado" "Situacion" "x(21)" ? ? ? ? ? ? ? no ? no no "22.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  IF Di-RutaC.FlgEst = 'C'
  THEN DO:
    MESSAGE 'La Hoja de Ruta ya está Cerrada'
        VIEW-AS ALERT-BOX WARNING.
    RETURN.
  END.
  IF Di-RutaC.FlgEst = 'A'
  THEN DO:
    MESSAGE 'La Hoja de Ruta está Anulada'
        VIEW-AS ALERT-BOX WARNING.
    RETURN.
  END.
  IF Di-RutaC.FlgEst <> 'P'
  THEN DO:
  ...
  FOR EACH di-rutad OF di-rutac NO-LOCK WHERE di-rutad.flgest = 'P':
      MESSAGE 'Antes debe actualizar el estado del documento' di-rutad.codref di-rutad.nroref
          VIEW-AS ALERT-BOX WARNING.
      RETURN.
  END.
  FOR EACH di-rutag OF di-rutac NO-LOCK WHERE di-rutag.flgest = 'P':
      MESSAGE 'Antes debe actualizar el estado del documento G/R' di-rutag.serref di-rutag.nroref
          VIEW-AS ALERT-BOX WARNING.
      RETURN.
  END.
  FOR EACH di-rutadg OF di-rutac NO-LOCK WHERE di-rutadg.flgest = 'P':
      MESSAGE 'Antes debe actualizar el estado del documento' di-rutadg.codref di-rutadg.nroref
          VIEW-AS ALERT-BOX WARNING.
      RETURN.
  END.
  ...
  FOR EACH Di-RutaD OF Di-RutaC WHERE Di-RutaD.FLgEst <> 'N' NO-LOCK:
    IF DI-RutaD.HorLle = '' THEN DO:
        MESSAGE 'Debe ingresar la hora en las Facturas y Boletas'
            VIEW-AS ALERT-BOX WARNING.
        RETURN.
    END.
  ...
  FOR EACH Di-RutaG OF Di-RutaC WHERE Di-RutaG.FlgEst <> 'N' NO-LOCK:
    IF DI-RutaG.HorLle = '' THEN DO:
        MESSAGE 'Debe ingresar la hora en las Transferencias'
            VIEW-AS ALERT-BOX WARNING.
        RETURN.
    END.
  ...
  FOR EACH Di-RutaDG OF Di-RutaC WHERE Di-RutaDG.FlgEst <> 'N' NO-LOCK:
    IF DI-RutaDG.HorLle = '' THEN DO:
        MESSAGE 'Debe ingresar la hora en las Guias Itinerantes'
            VIEW-AS ALERT-BOX WARNING.
        RETURN.
    END.
  ...
  RUN dist/d-rut002a-1 (ROWID(Di-RutaC)).
