...
DEFINE VAR cCodOrden AS CHAR.
DEFINE VAR cNroOrden AS CHAR.
DEFINE VAR iSerieOrden AS INT.
DEFINE VAR iNroOrden AS INT.
...
/* Ic - 14Mar205, verificamos si la OTR esta como no entregado en el cierre de hoja de ruta */
cCodOrden = T-RUTAD.CodRef:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.
cNroOrden = T-RUTAD.NroRef:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.

iSerieOrden = int(SUBSTR(cNroOrden,1,3)) NO-ERROR.
iNroOrden = int(SUBSTR(cNroOrden,4)) NO-ERROR.

esOTR_NO_ENTREGADA = NO.

ESTADO_HR:
FOR EACH gre_header WHERE gre_header.m_coddoc = cCodOrden AND
                        gre_header.m_nroser = iSerieOrden AND gre_header.m_nrodoc = iNroOrden NO-LOCK:
    FIND FIRST Di-RutaG WHERE Di-RutaG.codcia = s-codcia AND
                            Di-RutaG.coddoc = "H/R" AND
                            Di-RutaG.codalm = gre_header.m_codalm AND
                            Di-RutaG.tipmov = 'S' AND
                            Di-RutaG.codmov = 03 AND
                            Di-RutaG.serref = gre_header.serieGuia AND
                            Di-RutaG.nroref = gre_header.numeroGuia AND
                            DI-RutaG.coddiv = gre_header.m_divorigen  NO-LOCK NO-ERROR.
