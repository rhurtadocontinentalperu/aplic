...
DEF VAR s-Tabla AS CHAR INIT 'CFG_CAL_LOG_DIV'.
DEF VAR s-Codigo AS CHAR INIT 'BASE'.
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH FacTabla WHERE ~{&KEY-PHRASE} ~
      AND FacTabla.CodCia = s-CodCia ~
 AND FacTabla.Tabla = s-Tabla ~
 AND FacTabla.Codigo = s-Codigo NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH FacTabla WHERE ~{&KEY-PHRASE} ~
      AND FacTabla.CodCia = s-CodCia ~
 AND FacTabla.Tabla = s-Tabla ~
 AND FacTabla.Codigo = s-Codigo NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Registro-Nuevo q-tables 
PROCEDURE Registro-Nuevo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF NOT AVAILABLE FacTabla THEN DO:
    ...
    ASSIGN
        ...
        FacTabla.Valor[1] = 0   /* días */
        FacTabla.Campo-C[1] = "17:00:00"    /* hora de corte */
        .
END.
