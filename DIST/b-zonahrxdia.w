...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      T-FacTabla.Codigo FORMAT "x(8)":U WIDTH 5.43
      T-FacTabla.Nombre FORMAT "x(30)":U WIDTH 21.43
      ...
      T-FacTabla.Campo-L[1] COLUMN-LABEL "Domingo" FORMAT "yes/no":U
            WIDTH 8.72 COLUMN-FGCOLOR 15 COLUMN-BGCOLOR 12 VIEW-AS TOGGLE-BOX
  ENABLE
...
"T-FacTabla.Codigo" ? ? "character" ? ? ? ? ? ? no ? no no "5.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.T-FacTabla.Nombre
"T-FacTabla.Nombre" ? "x(30)" "character" ? ? ? ? ? ? no ? no no "21.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"T-FacTabla.Campo-L[7]" "Sábado" ? "logical" 14 0 ? ? ? ? yes ? no no "8.43" yes no no "U" "" "" "TOGGLE-BOX" "," ? ? 5 no 0 no no
     _FldNameList[9]   > Temp-Tables.T-FacTabla.Campo-L[1]
"T-FacTabla.Campo-L[1]" "Domingo" ? "logical" 12 15 ? ? ? ? yes ? no no "8.72" yes no no "U" "" "" "TOGGLE-BOX" "," ? ? 5 no 0 no no
...
FOR EACH VtaCTabla NO-LOCK WHERE VtaCTabla.CodCia = s-codcia 
    AND VtaCTabla.Tabla = 'ZGHR':   /* Zona Geografica por Hoja de Ruta */
    CREATE T-Factabla.
    ASSIGN
        T-FacTabla.CodCia = s-codcia
        T-FacTabla.Codigo = VtaCTabla.Llave
        T-FacTabla.Nombre = VtaCTabla.Descripcion.
END.
FOR EACH T-FacTabla:
    FOR EACH Vtatabla NO-LOCK WHERE VtaTabla.CodCia = s-codcia
        AND VtaTabla.Tabla = "ZGHRXDIA"
