...
&Scoped-define QUERY-STRING-br_table FOR EACH T-RutaD WHERE T-RutaD.CodCia = VtaCTabla.CodCia ~
  AND T-RutaD.Libre_c03 = VtaCTabla.Llave NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH T-RutaD WHERE T-RutaD.CodCia = VtaCTabla.CodCia ~
  AND T-RutaD.Libre_c03 = VtaCTabla.Llave NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table T-RutaD
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 66 BY 17.88
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.T-RutaD WHERE INTEGRAL.VtaCTabla <external> ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[1]      = "Temp-Tables.T-RutaD.CodCia = VtaCTabla.CodCia
  AND Temp-Tables.T-RutaD.Libre_c03 = VtaCTabla.Llave"
...
"T-RutaD.NroRef" ? "X(12)" "character" ? ? ? ? ? ? no ? no no "12.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.T-RutaD.Libre_c05
"T-RutaD.Libre_c05" "Cliente" "x(40)" "character" ? ? ? ? ? ? no ? no no "42.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
