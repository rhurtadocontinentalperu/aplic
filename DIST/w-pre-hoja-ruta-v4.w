...
DEFINE FRAME F-Main
     ...
     FILL-IN-Desde AT ROW 2.88 COL 11 COLON-ALIGNED WIDGET-ID 46
     BUTTON-1 AT ROW 2.88 COL 24 WIDGET-ID 50
     FILL-IN-Hasta AT ROW 2.88 COL 34 COLON-ALIGNED WIDGET-ID 48
     BUTTON-2 AT ROW 2.88 COL 46 WIDGET-ID 54
...
DEF VAR pCodDoc AS CHAR INIT "O/D,OTR" NO-UNDO.
DEF VAR pFlgEst AS CHAR INIT "P,C" NO-UNDO.
DEF VAR k AS INTE NO-UNDO.
DEF VAR j AS INTE NO-UNDO.

DO k = 1 TO NUM-ENTRIES(pCodDoc):
    DO j = 1 TO NUM-ENTRIES(pFlgEst):
        RLOOP:
        FOR EACH Faccpedi USE-INDEX Llave08 NO-LOCK WHERE Faccpedi.codcia = s-codcia
                AND Faccpedi.divdes = s-coddiv
                AND Faccpedi.coddoc = ENTRY(k,pCodDoc)
                AND Faccpedi.flgest = ENTRY(j,pFlgEst)
            ...
            FIRST GN-DIVI NO-LOCK WHERE GN-DIVI.CodCia = s-codcia 
                ...
                AND GN-DIVI.Campo-Char[1] <> "L":
            /* FILTROS */
            IF NOT Faccpedi.fchped <= FILL-IN-Hasta THEN LEAVE.
