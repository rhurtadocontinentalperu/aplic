...
/*     x-CodDoc = FacDocum.CodDoc.                                   */
    ...
    IF LOOKUP(x-CodDoc, 'OTR,O/D,O/M') = 0 THEN DO:
        MESSAGE 'Código de Barra errado' VIEW-AS ALERT-BOX ERROR.
        SELF:SCREEN-VALUE = ''.
        RETURN NO-APPLY.
    END.
/*     x-NroPed = SUBSTRING(ENTRY(1,SELF:SCREEN-VALUE,'-'),4). */
...
------------------------------------------------------------------------------*/
  ...
  {&OPEN-QUERY-F-Main}
  GET FIRST F-Main.
  DISPLAY FILL-IN-Barra FILL-IN_CodDoc FILL-IN_NroPed FILL-IN_NroItems 
