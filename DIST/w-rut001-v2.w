...
DEFINE VARIABLE h_v-rut001-v2 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-PreHR AT ROW 15 COL 108 WIDGET-ID 6
     BUTTON-Atencion AT ROW 16.08 COL 108 WIDGET-ID 4
     RECT-2 AT ROW 2.54 COL 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 128.86 BY 23.08
         FONT 4.
...
DO:
  RUN dispatch IN h_v-rut001-v2 ('um-imprimir-hoja-de-atencion':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'dist/v-rut001-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-rut001-v2 ).
       RUN set-position IN h_v-rut001-v2 ( 2.73 , 3.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Fac Bol|Transferencia|Itinerante' + ',
                     FOLDER-TAB-TYPE = 2':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 13.15 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-dirutac ).
       RUN set-position IN h_q-dirutac ( 1.00 , 85.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.65 , 8.29 ) */

       /* Links to SmartViewer h_v-rut001-v2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_v-rut001-v2 ).
       RUN add-link IN adm-broker-hdl ( h_q-dirutac , 'Record':U , h_v-rut001-v2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv04 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-rut001-v2 ,
             h_p-updv04 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_v-rut001-v2 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'dist/b-rut001a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-rut001a ).
       RUN set-position IN h_b-rut001a ( 14.77 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-rut001a ( 8.81 , 90.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 15.42 , 95.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 5.77 , 10.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-rut001a. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-rut001a ).
       RUN add-link IN adm-broker-hdl ( h_q-dirutac , 'Record':U , h_b-rut001a ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'dist/b-rut001b.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-rut001b ).
       RUN set-position IN h_b-rut001b ( 14.73 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-rut001b ( 8.46 , 58.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12-2 ).
       RUN set-position IN h_p-updv12-2 ( 15.46 , 73.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12-2 ( 5.19 , 11.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-rut001b. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12-2 , 'TableIO':U , h_b-rut001b ).
       RUN add-link IN adm-broker-hdl ( h_q-dirutac , 'Record':U , h_b-rut001b ).
    ...
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-rut001c ).
       RUN set-position IN h_b-rut001c ( 15.23 , 4.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12-3 ).
       RUN set-position IN h_p-updv12-3 ( 16.19 , 89.00 ) NO-ERROR.
...
CASE p-State:
    ...
    WHEN 'pinta-viewer' THEN DO:
        RUN dispatch IN h_v-rut001-v2 ('display-fields':U).
    END.
    ...
    WHEN 'disable-detail' THEN DO:
        BUTTON-Atencion:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
        BUTTON-PreHR:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
        IF h_b-rut001a <> ? THEN RUN dispatch IN h_p-updv12 ('disable':U).
        IF h_b-rut001b <> ? THEN RUN dispatch IN h_p-updv12-2 ('disable':U).
    ...
    WHEN 'enable-detail' THEN DO:
        RUN dispatch IN h_p-updv04 ('enable':U).
        RUN dispatch IN h_q-dirutac ('enable':U).
        IF h_b-rut001a <> ? THEN RUN dispatch IN h_p-updv12 ('enable':U).
        IF h_b-rut001b <> ? THEN RUN dispatch IN h_p-updv12-2 ('enable':U).
