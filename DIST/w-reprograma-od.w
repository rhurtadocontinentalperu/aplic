...
DEFINE FRAME F-Main
     ...
     BUTTON-Elimicar AT ROW 1.08 COL 43 WIDGET-ID 6
...
&Scoped-define SELF-NAME BUTTON-Elimicar
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-Elimicar W-Win
ON CHOOSE OF BUTTON-Elimicar IN FRAME F-Main /* NO REPROGRAMAR */
DO:
  MESSAGE 'Confirme la NO REPROGRAMACION' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE rpta AS LOG.
  IF rpta = NO THEN RETURN NO-APPLY.
  RUN Eliminar IN h_b-reprograma-od-cab.
  RUN dispatch IN h_b-reprograma-od-cab ('open-query':U).
END.
...
DO:
  CASE s-CodDoc:
      WHEN "O/D" THEN RUN REPROGRAMAR IN h_b-reprograma-od-cab.
      WHEN "OTR" THEN RUN REPROGRAMAR-OTR IN h_b-reprograma-od-cab.
  END CASE.
  RUN dispatch IN h_b-reprograma-od-cab ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-reprograma-od-cab ,
             BUTTON-Elimicar:HANDLE IN FRAME F-Main , 'AFTER':U ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_b-reprograma-od-cab , 'Record':U , h_b-reprograma-otr-det-2 ).
...
------------------------------------------------------------------------------*/
  ENABLE BUTTON-Actualizar BUTTON-Reprogramar BUTTON-Elimicar 
...
------------------------------------------------------------------------------*/
  ...
  CASE s-CodDoc:
      WHEN "O/D" THEN RUN select-page("1").
      WHEN "OTR" THEN RUN select-page("2").
  END CASE.
