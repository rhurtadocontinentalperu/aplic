...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      DI-RutaD.CodRef COLUMN-LABEL "Doc." FORMAT "x(3)":U WIDTH 7.43
            VIEW-AS COMBO-BOX INNER-LINES 5
                      LIST-ITEMS "O/D","OTR","O/M" 
                      DROP-DOWN-LIST 
      DI-RutaD.NroRef FORMAT "X(12)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 70 BY 18.04
...
"DI-RutaD.CodRef" "Doc." ? "character" ? ? ? ? ? ? yes ? no no "7.43" yes no no "U" "" "" "DROP-DOWN-LIST" "," "O/D,OTR,O/M" ? 5 no 0 no no
     _FldNameList[2]   > INTEGRAL.DI-RutaD.NroRef
"DI-RutaD.NroRef" ? "X(12)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  END.
  RELEASE DI-RutaD.     /* Hay un FIND en w-di-rutad.p */
...
FIND Faccpedi WHERE Faccpedi.codcia = s-codcia
    AND Faccpedi.coddoc = DI-RutaD.CodRef:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
    AND Faccpedi.nroped = DI-RutaD.NroRef:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
    AND Faccpedi.divdes = s-coddiv
    AND Faccpedi.flgest <> "A"
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE Faccpedi THEN DO:
    MESSAGE 'Documento NO válido' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO DI-RutaD.CodRef.
    RETURN 'ADM-ERROR'.
END.
