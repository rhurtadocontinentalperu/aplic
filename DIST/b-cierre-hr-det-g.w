...
"fEstadoDet() @ x-Motivo" "Motivo" "x(40)" ? ? ? ? ? ? ? no ? no no "60" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  ASSIGN
      ...
      Di-RutaG.Libre_c03 = pGlosa.

  RUN Actualiza-Similares (INPUT Di-RutaG.CodAlm,
                           Di-RutaG.TipMov,
                           Di-RutaG.CodMov,
                           Di-RutaG.SerRef,
                           Di-RutaG.NroRef,
...
DO:
  ...
  FIND CURRENT Di-RutaG EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
  IF NOT AVAILABLE Di-RutaG THEN RETURN NO-APPLY.
...
DEF INPUT PARAMETER pSerRef AS INTE.
DEF INPUT PARAMETER pNroRef AS INTE.
...
FIND B-CMOV WHERE B-CMOV.CodCia = s-CodCia
    AND B-CMOV.CodAlm = pCodAlm
    AND B-CMOV.TipMov = pTipMov
    AND B-CMOV.CodMov = pCodMov
    AND B-CMOV.NroSer = pSerRef
    AND B-CMOV.NroDoc = pNroRef
    NO-LOCK NO-ERROR.
...
ASSIGN
    x-CodDoc = B-CMOV.CodRef        /* OTR */
    x-NroDoc = B-CMOV.NroRef.
/* Buscamos todas las G/R de la misma O/D */
FOR EACH B-RUTAG OF Di-RutaC EXCLUSIVE-LOCK,
    FIRST B-CMOV WHERE B-CMOV.codcia = s-codcia
    AND B-CMOV.CodAlm = B-RUTAG.CodAlm
    AND B-CMOV.TipMov = B-RUTAG.TipMov
    AND B-CMOV.CodMov = B-RUTAG.CodMov
    AND B-CMOV.NroSer = B-RUTAG.SerRef
    AND B-CMOV.NroDoc = B-RUTAG.NroRef
    AND B-CMOV.CodRef = x-CodDoc
    AND B-CMOV.NroRef = x-NroDoc NO-LOCK:
    ASSIGN
        B-RUTAG.flgest = 'C'
        B-RUTAG.Libre_c02 = ''
        B-RUTAG.FlgEstDet = pFlgEstDet
        B-RUTAG.Libre_c03 = pGlosa.
END.
