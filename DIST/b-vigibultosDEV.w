...
/* 1ro. que estén todos los documentos de un pedido */
...
FOR EACH Detalle NO-LOCK, 
    ...
    /* acumulamos cantidad de guias por cada pedido */
    FIND T-Ped-Guias WHERE T-Ped-Guias.codped = Ccbcdocu.codped
        AND T-Ped-Guias.nroped = Ccbcdocu.nroped
        NO-ERROR.
...
FOR EACH T-Ped-Guias:
    FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia
        ...
        AND Ccbcdocu.codped = T-Ped-Guias.codped
...
FOR EACH T-Ped-Guias WHERE nroguias <> nrototal:
    MESSAGE 'No se han registrado todas la G/R' VIEW-AS ALERT-BOX WARNING.
    RETURN.
END.
/* *************************************** */
/* 2do control de bultos */
x-bultos = 0.
FOR EACH Detalle NO-LOCK,
    FIRST ccbcdocu NO-LOCK WHERE ccbcdocu.codcia = s-codcia
    AND ccbcdocu.coddoc = Detalle.coddoc
...
FOR EACH Detalle NO-LOCK:
    FIND ccbcbult WHERE ccbcbult.codcia = s-codcia
        AND ccbcbult.coddiv = s-coddiv
        AND ccbcbult.coddoc = Detalle.coddoc
        AND ccbcbult.nrodoc = Detalle.nrodoc
        NO-LOCK NO-ERROR.
    IF AVAILABLE ccbcbult THEN x-bultos = x-bultos + ccbcbult.bultos.
END.
IF FILL-IN-Bultos <> x-Bultos THEN DO:
    MESSAGE 'La cantidad de bultos no coincide' SKIP
        'Desea CERRAR de todas maneras a pesar de que falten bultos?'
        VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE rpta AS LOG.
    IF rpta = NO THEN RETURN.
    RUN dist/d-vigibultosdev (OUTPUT pFlgEstDet).
    IF pFlgEstDet = 'ADM-ERROR' THEN RETURN.
    FIND AlmTabla WHERE almtabla.Tabla = "VD"
        AND almtabla.Codigo = pFlgEstDet 
        NO-LOCK.
END.
...
DO TRANSACTION ON ERROR UNDO, RETURN ON STOP UNDO, RETURN:
    FOR EACH Detalle:
        ...
        ASSIGN
            ...
            CntDocum.NroBul = FILL-IN-Bultos
            CntDocum.Observac = (IF AVAILABLE AlmTabla THEN almtabla.Nombre ELSE '')
...
------------------------------------------------------------------------------*/
  ...
  END.
  /* NO repetir el documento */
  FIND LAST B-Docum USE-INDEX Llave02 WHERE s-codcia = s-codcia
