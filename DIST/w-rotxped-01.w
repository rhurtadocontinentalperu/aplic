...
&Scoped-Define ENABLED-OBJECTS rs-coddoc txt-nroped txt-bultos BtnDone-2 ~
BtnDone 
...
DEFINE VARIABLE rs-coddoc AS CHARACTER INITIAL "O/D" 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Orden Despacho Crédito", "O/D",
"Orden Despacho Mostrador", "O/M",
"Orden de Transferencia", "OTR",
"Transferencia", "TRA"
     SIZE 29.72 BY 4.15 NO-UNDO.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     rs-coddoc AT ROW 1.65 COL 19.29 NO-LABEL WIDGET-ID 24
     txt-nroped AT ROW 6 COL 17 COLON-ALIGNED WIDGET-ID 2
     txt-nomcli AT ROW 7.08 COL 17 COLON-ALIGNED WIDGET-ID 6
     txt-bultos AT ROW 8.15 COL 17 COLON-ALIGNED WIDGET-ID 4
     BtnDone-2 AT ROW 9.27 COL 84 WIDGET-ID 28
     BtnDone AT ROW 9.27 COL 92 WIDGET-ID 22
     "Documento:" VIEW-AS TEXT
          SIZE 11 BY .96 AT ROW 1.81 COL 8 WIDGET-ID 30
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 100 BY 9.96 WIDGET-ID 100.
...
DO:
    ...
    IF txt-nroped = '' THEN RETURN.

    FIND Ccbcbult WHERE Ccbcbult.codcia = s-codcia
        AND Ccbcbult.coddiv = s-coddiv
        AND Ccbcbult.coddoc = rs-coddoc
        AND Ccbcbult.nrodoc = txt-nroped
        NO-LOCK NO-ERROR.
    IF AVAILABLE Ccbcbult THEN DO:
        MESSAGE 'YA tiene rótulo registrado' VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    ...
    CASE rs-coddoc:
        WHEN "O/D" OR WHEN "O/M" THEN DO:
            FIND FIRST faccpedi WHERE faccpedi.codcia = s-codcia
                ...
                AND faccpedi.nroped = txt-nroped 
                AND faccpedi.flgest <> "A"      /* NO anulados */
                NO-LOCK NO-ERROR.
            IF NOT AVAIL faccpedi THEN DO:
                MESSAGE "Orden NO registrada o anulada"
                    VIEW-AS ALERT-BOX INFO BUTTONS OK.
                APPLY "entry" TO txt-nroped.
                RETURN NO-APPLY.
            END.
            IF Faccpedi.FlgSit <> "C" THEN DO:
                MESSAGE 'Orden NO ha pasado el control de barras' 
                    VIEW-AS ALERT-BOX INFO BUTTONS OK.
                APPLY "entry" TO txt-nroped.
                RETURN NO-APPLY.
            END.
        ...
        WHEN "TRA" THEN DO:
            ...
            IF Almcmov.CodRef = "OTR" THEN DO:
                MESSAGE 'Guía de Transferencia viene de una Orden de Transferencia'
                    VIEW-AS ALERT-BOX ERROR.
                APPLY "entry" TO txt-nroped.
                RETURN NO-APPLY.
            END.
            IF Almcmov.Libre_c02 <> "C" THEN DO:
                MESSAGE 'Guía de Transferencia NO ha pasado el control de barras' 
                    VIEW-AS ALERT-BOX INFO BUTTONS OK.
                APPLY "entry" TO txt-nroped.
                RETURN NO-APPLY.
            END.
    ...
    CASE rs-coddoc:
        WHEN "O/D" OR WHEN "O/M" OR WHEN "OTR" THEN DO:
            ...
            ASSIGN 
                ...
                w-report.Llave-C  = "01" + STRING (faccpedi.nroped, '999999999')
...
------------------------------------------------------------------------------*/
  ...
  ENABLE rs-coddoc txt-nroped txt-bultos BtnDone-2 BtnDone 
      WITH FRAME F-Main IN WINDOW W-Win.
