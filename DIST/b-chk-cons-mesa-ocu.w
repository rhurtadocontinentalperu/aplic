...
DEFINE VAR x-ordenes-pendientes-col AS INT.
...
fNroItems() @ x-NroItems fNroOrdenes() @ x-NroOrdenes ~
ordenes-pendientes (Chkmesas.mesa) @ x-ordenes-pendientes-col 
...
&Scoped-define QUERY-STRING-br_table FOR EACH ChkMesas WHERE ~{&KEY-PHRASE} ~
 ...
 NO-LOCK, ~
      FIRST ChkTareas WHERE ChkTareas.CodCia = ChkMesas.CodCia ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD ordenes-pendientes B-table-Win 
FUNCTION ordenes-pendientes RETURNS INTEGER
  ( INPUT pMesa AS CHAR )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      ordenes-pendientes (Chkmesas.mesa) @ x-ordenes-pendientes-col COLUMN-LABEL "Pendientes" FORMAT ">>,>>9":U
            WIDTH 7.86
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 56 BY 10
         FONT 4
         TITLE "Listado de Mesas" FIT-LAST-COLUMN.
...
"ordenes-pendientes (Chkmesas.mesa) @ x-ordenes-pendientes-col" "Pendientes" ">>,>>9" ? ? ? ? ? ? ? no ? no no "7.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Listado de Mesas */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Listado de Mesas */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Listado de Mesas */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION ordenes-pendientes B-table-Win 
FUNCTION ordenes-pendientes RETURNS INTEGER
  ( INPUT pMesa AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
DEFINE BUFFER x-chktareas FOR chktareas.

FOR EACH x-ChkTareas OF INTEGRAL.ChkMesas WHERE x-chktareas.FlgEst = "P" NO-LOCK:
    /*
      FIRST INTEGRAL.ChkControl OF INTEGRAL.ChkTareas NO-LOCK,
      FIRST INTEGRAL.FacCPedi WHERE FacCPedi.CodCia = ChkTareas.CodCia
  AND FacCPedi.CodDoc = ChkTareas.CodDoc
  AND FacCPedi.NroPed = ChkTareas.NroPed NO-LOCK
    ~{&SORTBY-PHRASE}
    */
