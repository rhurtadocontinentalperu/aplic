...
PRINCIPAL:
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    FOR EACH B-PHRC EXCLUSIVE-LOCK WHERE B-PHRC.codcia = B-RutaC.codcia
        AND B-PHRC.coddiv = B-RutaC.coddiv
        AND B-PHRC.coddoc = "PHR"
        AND B-PHRC.codref = B-RutaC.CodDoc
        AND B-PHRC.nroref = B-RutaC.NroDoc,
        EACH B-PHRD OF B-PHRC EXCLUSIVE-LOCK WHERE B-PHRD.FlgEst <> "A":
        ASSIGN 
...
FOR EACH DI-RutaD OF DI-RutaC NO-LOCK:
    ...
    FIND FACTURA WHERE FACTURA.codcia = s-codcia AND
