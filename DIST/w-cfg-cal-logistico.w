...
DEFINE VARIABLE h_b-zonahrd AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-zonahrxdia AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'LABEL-FONT = 4,
                     ...
                     LABELS = Programación Despachos|Capacidad|Zona Geográfica PRE-HOJA RUTA':U ,
             OUTPUT h_tab95 ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/dist/b-zonahrxdia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-zonahrxdia ).
       RUN set-position IN h_b-zonahrxdia ( 9.88 , 5.00 ) NO-ERROR.
       RUN set-size IN h_b-zonahrxdia ( 8.08 , 96.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv97 ).
       RUN set-position IN h_p-updv97 ( 17.96 , 5.00 ) NO-ERROR.
       RUN set-size IN h_p-updv97 ( 1.54 , 34.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-zonahrxdia. */
       RUN add-link IN adm-broker-hdl ( h_b-div-cd , 'Record':U , h_b-zonahrxdia ).
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_b-zonahrxdia ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-div-cd ,
             h_tab95 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-zonahrxdia ,
             h_b-div-cd , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv97 ,
             h_b-zonahrxdia , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
    ...
    WHEN 3 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'dist/b-zonahrd.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-zonahrd ).
       RUN set-position IN h_b-zonahrd ( 10.42 , 5.00 ) NO-ERROR.
       RUN set-size IN h_b-zonahrd ( 14.54 , 100.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-zonahrd. */
       RUN add-link IN adm-broker-hdl ( h_b-zonahrc , 'Record':U , h_b-zonahrd ).
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-zonahrd ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-zonahrd ,
             h_b-zonahrc , 'AFTER':U ).
