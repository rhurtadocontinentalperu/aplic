...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    */
    ...
    /* ********************************************************************************************** */
    FOR EACH Di-RutaD OF Di-RutaC NO-LOCK WHERE Di-RutaD.codref = "G/R" AND
            (Di-RutaD.flgest = "N" OR Di-RutaD.flgest = "T"),   /* NO Entregado o Dejado en Tienda */
        FIRST Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = Di-RutaD.codcia
            AND Ccbcdocu.coddoc = Di-RutaD.codref   /* G/R */
            AND Ccbcdocu.nrodoc = Di-RutaD.nroref
        ...
        CASE TRUE:
            WHEN Di-RutaD.FlgEst = "N" THEN DO:     /* NO ENTREGADO */
                ...
                /* BAJA EN SUNAT */
                /* ******************************************************************************************** */
                /* 12/03/2026: Susana León, la baja lo va a hacer Contabilidad */
                ...
                CASE Di-RutaD.flgest:
                    WHEN "T" THEN DO:
                        ...
                        ASSIGN
                            OD_Original.CodAlm = Almacen.codalm
                            OD_Original.DivDes = Almacen.coddiv.
                        /* 20/02/2026: Reactivamos los BULTOS */
                        REPEAT:
                            FIND FIRST Ccbcbult WHERE CcbCBult.CodCia = s-codcia 
                                ...
                                EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
                            IF NOT AVAILABLE Ccbcbult THEN LEAVE.
    ...
    */
    ...
    FOR EACH Di-RutaG OF Di-RutaC NO-LOCK WHERE 
        ...
        IF Di-RutaG.FlgEst = "N" THEN DO:
            ...
            24Feb2025 : Susana Leon indica que para OTR no dar de baja la GRE
...
DEF VAR pEstados AS CHAR NO-UNDO.
DEF VAR pEstibador AS CHAR NO-UNDO.
pEstados = "X,E,P,PS".      /* Estado válidos */
/* Estado NO válidos: 'PR,C,A,L' */
...
/*             END.                                                                                  */
        ...
        DO k = 1 TO NUM-ENTRIES(pEstibadores):
            pEstibador = ENTRY(k, pEstibadores).
            /* ************************************************ */
            ...
            DO j = 1 TO 7:
                CASE j:
                    WHEN 1 THEN DO:
                        {dist/i-hr_verifica-estibadores.i &Campo = B-RUTAC.Ayudante-1}
                    END.
                    WHEN 2 THEN DO:
                        {dist/i-hr_verifica-estibadores.i &Campo = B-RUTAC.Ayudante-2}
                    END.
                    WHEN 3 THEN DO:
                        {dist/i-hr_verifica-estibadores.i &Campo = B-RUTAC.Ayudante-3}
                    END.
                    WHEN 4 THEN DO:
                        {dist/i-hr_verifica-estibadores.i &Campo = B-RUTAC.Ayudante-4}
                    END.
                    WHEN 5 THEN DO:
                        {dist/i-hr_verifica-estibadores.i &Campo = B-RUTAC.Ayudante-5}
                    END.
                    WHEN 6 THEN DO:
                        {dist/i-hr_verifica-estibadores.i &Campo = B-RUTAC.Ayudante-6}
                    END.
                    WHEN 7 THEN DO:
                        {dist/i-hr_verifica-estibadores.i &Campo = B-RUTAC.Ayudante-7}
                    END.
                END CASE.
            END.    /* DO j = */
        END.    /* DO k = */
...
&IF DEFINED(EXCLUDE-HR_Verifica-Estibadores-OldOld) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE HR_Verifica-Estibadores-OldOld Procedure 
PROCEDURE HR_Verifica-Estibadores-OldOld :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH DI-RutaD OF DI-RutaC NO-LOCK:
    ...
    CASE DI-RutaD.FlgEst:
        WHEN "C" THEN DO:       /* Entregado: NO debe tener ingreso al almacén */
            ...
            Ic - 04Ene2026, reunion Susana Leon, Harold Segura, Pedro Tananta y Ruben Hurtado
            Suana/Harold, decidieron quitar esta validacion.
        ...
        WHEN "O/D" OR WHEN "O/M" THEN DO:
            ...
            IF lGRE_ONLINE THEN DO:
                FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia AND
                    ...
                    ccbcdocu.nroped = ORDENES.nroref:
                    IF ccbcdocu.codcli = ORDENES.codcli AND
                        LOOKUP(Ccbcdocu.coddoc, 'FAC,BOL,FAI') > 0 AND
