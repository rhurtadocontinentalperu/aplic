...
DEF TEMP-TABLE T-Ped-Guias
    ...
    FIELD TipMov LIKE Di-RutaG.Tipmov
    FIELD CodMov LIKE Di-RutaG.Codmov
...
/* ***************************  Main Block  *************************** */
  ...
  DEF VAR pMsgAcumulado AS CHAR NO-UNDO.    /* Acumula todos los mensajes parciales */

  RUN Bultos-por-GR.

  RUN Bultos-por-OTR.

  IF pError = NO THEN pMensaje = pMsgAcumulado.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH T-Ped-Guias:
        ...
        /* Procedimientos */
        RUN Grupo-reparto IN hProc (INPUT T-Ped-Guias.codped, INPUT T-Ped-Guias.nroped, 
      ...
      FOR EACH Ccbcdocu USE-INDEX llave15 NO-LOCK WHERE Ccbcdocu.codcia = B-RutaC.codcia
          ...
          IF x-DeliveryGroup = "" OR x-InvoiCustomerGroup = "" THEN DO:
              /* No tiene grupo de raparto */
              IF NOT (Ccbcdocu.flgest = "F") THEN NEXT.
          END.
          ELSE DO:
              /* Es de un grupo de reparto */
          END.
          ...
          IF NOT AVAILABLE Di-RutaD THEN DO:
              pMensaje = 'Falta Registrar la ' + Ccbcdocu.coddoc + ' ' + Ccbcdocu.nrodoc + CHR(10) +
                  'del ' + T-Ped-Guias.CodPed + T-Ped-Guias.NroPed + CHR(10) +
                  'de la ' + Ccbcdocu.Libre_c01 + ' ' + Ccbcdocu.Libre_c02.
              IF TRUE <> (pMsgAcumulado > '') THEN pMsgAcumulado = pMensaje.
              ELSE pMsgAcumulado = pMsgAcumulado + CHR(10) + pMensaje.
              IF pError = YES THEN MESSAGE pMensaje VIEW-AS ALERT-BOX WARNING.
          END.
      END.
  END.
  /* error */
...
------------------------------------------------------------------------------*/
  ...
  /* *************************************** */
  FOR EACH Di-RutaG OF B-RutaC EXCLUSIVE-LOCK,
      ...
      AND LOOKUP(TRIM(Almcmov.FlgEst), "A,R") = 0
      BREAK BY Almcmov.CodRef BY Almcmov.NroRef:
      /* *********************************************************************** */
      ...
      IF lGRE_ONLINE THEN DO:
          FIND FIRST GRE WHERE GRE.serieGuia = Almcmov.nroser AND
              ...
              GRE.m_rspta_sunat = "ACEPTADO POR SUNAT"
              NO-LOCK NO-ERROR.
      ...
      CASE TRUE:
          WHEN Almcmov.CodRef = "OTR" THEN DO:
              ...
              ASSIGN
                  ...
                  T-Ped-Guias.TipMov = Almcmov.Tipmov
                  T-Ped-Guias.CodMov = Almcmov.Codmov
                  T-Ped-Guias.nroguias = T-Ped-Guias.nroguias + 1.
          END.
          OTHERWISE DO:
              FIND FIRST CcbCBult WHERE CcbCBult.CodCia = B-RutaC.codcia
  ...
  FOR EACH T-Ped-Guias:
      FOR EACH Almcmov NO-LOCK WHERE Almcmov.codcia = B-RutaC.codcia
          ...
          AND Almcmov.tipmov = T-Ped-Guias.TipMov
          AND Almcmov.codmov = T-Ped-Guias.CodMov
          AND LOOKUP(TRIM(Almcmov.flgest), "A,R") = 0:    /* NO ANULADOS, NO REPROGRAMADOS */
          /* *********************************************************************** */
          ...
          IF NOT AVAILABLE Di-RutaG THEN DO:
              pMensaje = 'Falta Registrar la G/R ' + STRING(Almcmov.nroser, '999') + '-' + STRING(Almcmov.nrodoc) + CHR(10) +
