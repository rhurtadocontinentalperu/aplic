...
&Scoped-Define ENABLED-OBJECTS txt-CD txt-div txtDesde txtHasta btn-Ok ~
RECT-7 
&Scoped-Define DISPLAYED-OBJECTS txt-CD txt-div txt-nom-cd txtDesde ~
txtHasta txt-nom-div 
...
DEFINE FRAME F-Main
     txt-CD AT ROW 5.46 COL 17 COLON-ALIGNED WIDGET-ID 20
     txt-div AT ROW 6.85 COL 17 COLON-ALIGNED WIDGET-ID 24
     txt-nom-cd AT ROW 5.46 COL 26.72 COLON-ALIGNED WIDGET-ID 26
     ...
     txt-nom-div AT ROW 6.81 COL 26.86 COLON-ALIGNED WIDGET-ID 28
     "Cuyas salidas de vehiculos sean..." VIEW-AS TEXT
          SIZE 36 BY .62 AT ROW 2.54 COL 9.43 WIDGET-ID 10
          FONT 3
     "Detalle de hoja de Ruta (Documentos)" VIEW-AS TEXT
          SIZE 36 BY .62 AT ROW 1.38 COL 18 WIDGET-ID 2
          FGCOLOR 4 FONT 12
     "(Vacio = todo(s))" VIEW-AS TEXT
          SIZE 18.14 BY .62 AT ROW 8.5 COL 27 WIDGET-ID 22
...
DO:
  ASSIGN txtDesde txtHasta txt-CD txt-div.
...
ON LEAVE OF txt-CD IN FRAME F-Main /* De este sede */
DO:
...
ON LEAVE OF txt-div IN FRAME F-Main /* De esta division */
DO:
    DEFINE VAR lCodDiv AS CHAR.
...
------------------------------------------------------------------------------*/
  DISPLAY txt-CD txt-div txt-nom-cd txtDesde txtHasta txt-nom-div 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE txt-CD txt-div txtDesde txtHasta btn-Ok RECT-7 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
    ...
    chExcelApplication:VISIBLE = FALSE.
        ...
        chWorkSheet:Range("F1"):Value = "DOCUMENTOS HOJA DE RUTA -  DESDE :" + 
    ...
    chWorkSheet:Range("E2"):Value = "CodCliente".
    chWorkSheet:Range("F2"):Value = "Nombre del Cliente".
    chWorkSheet:Range("G2"):Value = "CodGuia".
    chWorkSheet:Range("H2"):Value = "Nro Guia".
    chWorkSheet:Range("I2"):Value = "Serie Dcto".  
    chWorkSheet:Range("J2"):Value = "Nro. Dcto".  
    chWorkSheet:Range("K2"):Value = "Div. Dcto".  
    chWorkSheet:Range("L2"):Value = "Nomb.Div. Dcto".  
...
FOR EACH tt-hojaruta NO-LOCK:
        ...
        cRange = "N" + cColumn.
        chWorkSheet:Range(cRange):Value = tt-peso.
END.
...
FOR EACH DI-RutaC WHERE DI-RutaC.codcia = s-codcia 
    ...
    AND (DI-RutaC.fchsal >= txtDesde AND DI-RutaC.fchsal <= txtHasta) 
    AND (txt-cd = "" OR di-rutaC.coddiv = txt-cd)   NO-LOCK:
