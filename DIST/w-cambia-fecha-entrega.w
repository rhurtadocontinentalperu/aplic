...
FacCPedi.Peso FacCPedi.Volumen FacCPedi.Items 
...
BtnDone BUTTON-Filtrar FILL-IN_NroPed BUTTON-Limpiar BROWSE-2 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN_FchEnt FILL-IN_NroPed 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-2 W-Win _STRUCTURED
  QUERY BROWSE-2 NO-LOCK DISPLAY
      ...
      FacCPedi.Peso COLUMN-LABEL "Peso en kg" FORMAT "->>,>>9.9999":U
            WIDTH 8.43
      FacCPedi.Volumen COLUMN-LABEL "Volumen en m3" FORMAT "->>,>>9.9999":U
      FacCPedi.Items COLUMN-LABEL "# SKU" FORMAT "->,>>>,>>9":U
            WIDTH 7.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"FacCPedi.NomCli" ? "x(60)" "character" ? ? ? ? ? ? no ? no no "44.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.FacCPedi.Peso
"FacCPedi.Peso" "Peso en kg" ? "decimal" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > INTEGRAL.FacCPedi.Volumen
"FacCPedi.Volumen" "Volumen en m3" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > INTEGRAL.FacCPedi.Items
"FacCPedi.Items" "# SKU" ? "integer" ? ? ? ? ? ? no ? no no "7.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
&ANALYZE-RESUME
        ...
        IF {&Condicion} THEN DO:
            ...
            IF B-CPEDI.CodDoc = "O/D" THEN DO:
                ...
                IF AVAILABLE PEDIDO THEN DO:
                    ...
                    END.
                    /* ***************************** */
                    /* RHC 21/09/2020 Log de control */
                    /* ***************************** */
                    RUN lib/logtabla (INPUT 'FACCPEDI',
                                      INPUT PEDIDO.coddiv + '|' + PEDIDO.coddoc + '|' + 
                                      PEDIDO.nroped + '|' + STRING(PEDIDO.FchEnt) + '|' +
                                      STRING(B-CPEDI.FchEnt),
                                      "FCHENT").
                    /* ***************************** */
                    ASSIGN
            ...
            IF B-CPEDI.CodDoc = "OTR" AND B-CPEDI.CodRef = "PED" THEN DO:
                ...
                /* ***************************** */
                RUN lib/logtabla (INPUT 'FACCPEDI',
                                  INPUT b-FacCPedi.coddiv + '|' + b-FacCPedi.coddoc + '|' + 
                                  b-FacCPedi.nroped + '|' + STRING(b-FacCPedi.FchEnt) + '|' +
                                  STRING(B-CPEDI.FchEnt),
                                  "FCHENT").
                /* ***************************** */
                ASSIGN
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN_FchEnt FILL-IN_NroPed 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-11 FILL-IN_FchEnt BUTTON-1 BUTTON-21 BtnDone BUTTON-Filtrar 
         FILL-IN_NroPed BUTTON-Limpiar BROWSE-2 
      WITH FRAME F-Main IN WINDOW W-Win.
