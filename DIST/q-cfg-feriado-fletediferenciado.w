...
DEF VAR s-Tabla AS CHAR INIT 'SZGHRFERIADO_DIV' NO-UNDO.
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH FacTabla WHERE FacTabla.CodCia = GN-DIVI.CodCia ~
  AND FacTabla.Codigo = GN-DIVI.CodDiv ~
      AND FacTabla.Tabla = s-Tabla NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH FacTabla WHERE FacTabla.CodCia = GN-DIVI.CodCia ~
  AND FacTabla.Codigo = GN-DIVI.CodDiv ~
      AND FacTabla.Tabla = s-Tabla NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Genera-Registro q-tables 
