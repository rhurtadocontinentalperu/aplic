...
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table CntDocum.NroDoc ~
CntDocum.NroBul CntDocum.Observac 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CntDocum.NroDoc FORMAT "X(11)":U WIDTH 10.14
      ...
      CntDocum.FchDoc COLUMN-LABEL "Fecha-Hora de registro" FORMAT "99/99/9999 HH:MM:SS":U
  ENABLE
      CntDocum.NroDoc
      CntDocum.NroBul
      CntDocum.Observac
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 110 BY 15.88
         FONT 4.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: B-Docum B "?" ? INTEGRAL CntDocum
   END-TABLES.
 */
...
"CntDocum.NroDoc" ? "X(11)" "character" ? ? ? ? ? ? yes ? no no "10.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CntDocum.Observac" "Observaciones" "x(40)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.CntDocum.FchDoc
"CntDocum.FchDoc" "Fecha-Hora de registro" "99/99/9999 HH:MM:SS" "datetime" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/*     END.                                                                                               */
    WHEN '9' THEN DO:           /* G/R */
        ASSIGN
            CntDocum.CodDoc:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = "G/R"
            SELF:SCREEN-VALUE = SUBSTRING(SELF:SCREEN-VALUE,2,3) +
                                SUBSTRING(SELF:SCREEN-VALUE,6,6).
    END.
/*     WHEN '3' THEN DO:           /* BOL */                                                              */
/*         ASSIGN                                                                                         */
...
ON 'RETURN':U OF CntDocum.NroBul, CntDocum.NroDoc, CntDocum.Observac
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      ...
      FIND LAST B-Docum USE-INDEX Llave02 WHERE B-Docum.codcia = s-codcia
          AND B-Docum.coddoc = Cntdocum.coddoc
          AND B-Docum.nrodoc = Cntdocum.nrodoc
          AND B-Docum.flgest <> "A"     /* NO anulados */
          AND ROWID(B-Docum) <> ROWID(CntDocum)
          NO-LOCK NO-ERROR.
      ...
      FOR LAST di-rutad NO-LOCK WHERE di-rutad.codcia = s-codcia
          ...
          AND di-rutad.codref = CntDocum.CodDoc
          AND di-rutad.nroref = CntDocum.NroDoc,
          FIRST di-rutac OF di-rutad NO-LOCK WHERE di-rutac.flgest = 'P':
          ASSIGN
              x-Ok = YES
              CntDocum.OriDoc = di-rutad.coddoc + '|' + di-rutad.nrodoc.
      END.
      IF x-Ok = NO THEN DO:
          FOR LAST di-rutag NO-LOCK WHERE di-rutag.codcia = s-codcia
              AND di-rutag.coddoc = 'H/R'
              AND di-rutag.coddiv = s-coddiv
              AND di-rutag.serref = INTEGER( SUBSTRING(CntDocum.NroDoc,1,3) )
              AND di-rutag.nroref = INTEGER( SUBSTRING(CntDocum.NroDoc,4) ),
              FIRST di-rutac OF di-rutag NO-LOCK WHERE di-rutac.flgest = 'P':
              ASSIGN
                  x-Ok = YES
                  CntDocum.OriDoc = di-rutad.coddoc + '|' + di-rutad.nrodoc.
          END.
      END.
      IF x-Ok = NO THEN DO:
          FOR LAST di-rutadg NO-LOCK WHERE di-rutadg.codcia = s-codcia
              AND di-rutadg.coddoc = 'H/R'
              AND di-rutadg.coddiv = s-coddiv
              AND di-rutadg.codref = CntDocum.CodDoc
              AND di-rutadg.nroref = CntDocum.NroDoc,
...
------------------------------------------------------------------------------*/
  ...
  */
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  FIND CURRENT CntDocum EXCLUSIVE-LOCK NO-ERROR.
  IF AVAILABLE CntDocum
      THEN ASSIGN
                CntDocum.FlgEst = 'A'
                CntDocum.Observac = 'ANULADO EL ' + STRING(DATETIME(TODAY, MTIME), '99/99/9999 HH:MM') +
                                'POR EL USUARIO ' + s-user-id.
  FIND CURRENT CntDocum NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      IF RETURN-VALUE = 'YES' THEN DO:
          ASSIGN
              CntDocum.NroDoc:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO
              CntDocum.NroBul:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO
              CntDocum.Observ:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO.
        APPLY 'ENTRY':U TO CntDocum.CodDoc.
    END.
    ELSE DO:
        ASSIGN
            CntDocum.NroDoc:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES
            CntDocum.NroBul:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES.
        APPLY 'ENTRY':U TO CntDocum.Observ.
    END.
  END.
...
IF CntDocum.FlgEst = 'A' THEN DO:
    MESSAGE 'Registro ANULADO' VIEW-AS ALERT-BOX ERROR.
    RETURN 'ADM-ERROR'.
END.
IF DATE(CntDocum.FchDoc) <> TODAY THEN DO:
    MESSAGE 'NO se puede modificar un documento de otro día'
...
FIND LAST B-Docum USE-INDEX Llave02 WHERE B-Docum.codcia = s-codcia
    ...
    AND B-Docum.nrodoc = Cntdocum.nrodoc
    NO-LOCK NO-ERROR.
IF ROWID(B-Docum) <> ROWID(Cntdocum) THEN DO:
