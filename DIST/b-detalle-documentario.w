...
DEF VAR x-FechaHora AS DATETIME NO-UNDO.
...
CcbCDocu.ImpTot CcbCDocu.usuario ~
DATETIME(STRING (CcbCDocu.FchDoc,'99/99/9999')+ ' ' + CcbCDocu.HorCie ) @ x-FechaHora 
...
&Scoped-define QUERY-STRING-br_table FOR EACH CcbCDocu WHERE CcbCDocu.CodCia = FacCPedi.CodCia ~
  ...
  OR CcbCDocu.CodDoc = "BOL") NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      DATETIME(STRING (CcbCDocu.FchDoc,'99/99/9999')+ ' ' + CcbCDocu.HorCie ) @ x-FechaHora COLUMN-LABEL "Fecha/Hora Generación"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"CcbCDocu.NroDoc" ? ? "character" ? ? ? ? ? ? no ? no no "11.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = INTEGRAL.CcbCDocu.ImpTot
     _FldNameList[4]   = INTEGRAL.CcbCDocu.usuario
     _FldNameList[5]   > "_<CALC>"
"DATETIME(STRING (CcbCDocu.FchDoc,'99/99/9999')+ ' ' + CcbCDocu.HorCie ) @ x-FechaHora" "Fecha/Hora Generación" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
