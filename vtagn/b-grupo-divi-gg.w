...
FacTabla.Nombre FacTabla.Campo-L[1] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacTabla.Nombre FORMAT "x(40)":U WIDTH 34.14
      FacTabla.Campo-L[1] COLUMN-LABEL "Parte de la escalera!de Precio" FORMAT "yes/no":U
            WIDTH 21.43 VIEW-AS TOGGLE-BOX
  ENABLE
      FacTabla.Codigo
      FacTabla.Nombre
      FacTabla.Campo-L[1]
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 70 BY 16.96
         FONT 4
         TITLE "PELDAÑOS" FIT-LAST-COLUMN.
...
"FacTabla.Nombre" ? ? "character" ? ? ? ? ? ? yes ? no no "34.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.FacTabla.Campo-L[1]
"FacTabla.Campo-L[1]" "Parte de la escalera!de Precio" ? "logical" ? ? ? ? ? ? yes ? no no "21.43" yes no no "U" "" "" "TOGGLE-BOX" "," ? ? 5 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* PELDAÑOS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* PELDAÑOS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* PELDAÑOS */
DO:
...
IF TRUE <> (FacTabla.Codigo:SCREEN-VALUE IN BROWSE {&browse-name} > '') THEN RETURN 'ADM-ERROR'.
RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
