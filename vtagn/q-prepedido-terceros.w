...
&SCOPED-DEFINE Condicion ( VtaCDocu.CodCia = s-codcia ~
    ...
    AND VtaCDocu.NroPed BEGINS STRING(S-NROSER,"999") ~
    AND VtaCDocu.FlgEst = "P" ~
    AND VtaCDocu.FchVen >= TODAY )
...
&Scoped-define INTERNAL-TABLES VtaCDocu gn-clie
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH VtaCDocu WHERE ~{&KEY-PHRASE} ~
      ...
      EACH gn-clie WHERE gn-clie.CodCli = VtaCDocu.CodCli ~
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH VtaCDocu WHERE ~{&KEY-PHRASE} ~
      ...
      AND gn-clie.CodCia = cl-codcia NO-LOCK ~
    BY VtaCDocu.NroPed DESCENDING.
&Scoped-define TABLES-IN-QUERY-Query-Main VtaCDocu gn-clie
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main VtaCDocu
&Scoped-define SECOND-TABLE-IN-QUERY-Query-Main gn-clie


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-CodCli 
...
DEFINE VARIABLE FILL-IN-CodCli AS CHARACTER FORMAT "X(11)":U 
     LABEL "Buscar Cliente por Atender" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME FRAME-A
     FILL-IN-CodCli AT ROW 1 COL 17.86 COLON-ALIGNED WIDGET-ID 4
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.VtaCDocu,INTEGRAL.gn-clie WHERE INTEGRAL.VtaCDocu ..."
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ","
     _OrdList          = "INTEGRAL.VtaCDocu.NroPed|no"
     _Where[1]         = "{&Condicion}"
     _JoinCode[2]      = "gn-clie.CodCli = VtaCDocu.CodCli"
...
ON LEAVE OF FILL-IN-CodCli IN FRAME FRAME-A /* Buscar Cliente por Atender */
DO:
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN lkup/c-prepedido-terceros.w ('BUSCAR POR CLIENTE').
...
FIND FIRST Vtacdocu WHERE {&Condicion} 
    AND Vtacdocu.codcli = FILL-IN-CodCli
    NO-LOCK NO-ERROR.
...
IF OUTPUT-VAR-1 <> ? THEN DO:
   ...
   END.
END.
ELSE MESSAGE 'Cliente no encontrado o ya atendido' VIEW-AS ALERT-BOX INFORMATION.
