...
DEFINE NEW SHARED TEMP-TABLE B-CPEDI NO-UNDO LIKE VtaCDocu
...
DEF VAR s-codped AS CHAR INIT 'PPD' NO-UNDO.
DEF VAR s-nroser AS INT NO-UNDO.
DEF VAR s-FlgIgv LIKE Vtacdocu.flgigv NO-UNDO.
DEF VAR s-PorIgv LIKE Vtacdocu.porigv NO-UNDO.

/* CORRELATIVO */
FIND FIRST Vtacordiv WHERE Vtacordiv.codcia = s-codcia
    AND Vtacordiv.coddiv = s-coddiv
    AND Vtacordiv.coddoc = s-codped
    AND VtaCorDiv.FlgEst = YES
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE Vtacordiv THEN DO:
    MESSAGE 'No se ha definido el correlativo para los PEDIDOS'
        VIEW-AS ALERT-BOX WARNING.
    RETURN.
END.
s-nroser = Vtacordiv.nroser.
...
&Scoped-Define ENABLED-OBJECTS FILL-IN-Registros BUTTON-12 BtnDone ~
FILL-IN_FchEnt BUTTON-1 RECT-3 RECT-4 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-Registros FILL-IN_FchEnt 
...
DEFINE VARIABLE h_b-pedexp-01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-pedexp-02 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE FILL-IN-Registros AS INTEGER FORMAT ">>9":U INITIAL 100 
...
DEFINE FRAME F-Main
     FILL-IN-Registros AT ROW 1.54 COL 38 COLON-ALIGNED WIDGET-ID 24
     BUTTON-12 AT ROW 1 COL 135 WIDGET-ID 22
     BtnDone AT ROW 1 COL 141 WIDGET-ID 16
     FILL-IN_FchEnt AT ROW 1.54 COL 16 COLON-ALIGNED WIDGET-ID 4
     BUTTON-1 AT ROW 1.27 COL 48 WIDGET-ID 6
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: B-CPEDI T "NEW SHARED" NO-UNDO INTEGRAL VtaCDocu
      ADDITIONAL-FIELDS:
          INDEX Llave01 AS PRIMARY Libre_d01
      END-FIELDS.
...
OR ENDKEY OF {&WINDOW-NAME} ANYWHERE DO:
  ...
  RUN Eliminar-Todo IN h_b-pedexp-01.
...
DO:
    ...
    ASSIGN FILL-IN_FchEnt FILL-IN-Registros.
    RUN Carga-Temporal IN h_b-pedexp-01 ( INPUT FILL-IN_FchEnt, INPUT FILL-IN-Registros).
    RUN dispatch IN h_b-pedexp-01 ('open-query').
...
DO:
   SESSION:SET-WAIT-STATE('GENERAL').
   RUN Proyeccion IN h_b-pedexp-01.
   button-5:SENSITIVE = YES.
   SESSION:SET-WAIT-STATE('').
   RUN dispatch IN h_b-pedexp-01 ('open-query').
   RUN dispatch IN h_b-pedexp-02 ('open-query').
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/b-pedexp-02.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedexp-02 ).
       RUN set-position IN h_b-pedexp-02 ( 16.08 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedexp-02 ( 6.69 , 119.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/b-pedexp-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedexp-01 ).
       RUN set-position IN h_b-pedexp-01 ( 2.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedexp-01 ( 13.15 , 145.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pedexp-02. */
       RUN add-link IN adm-broker-hdl ( h_b-pedexp-01 , 'Record':U , h_b-pedexp-02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedexp-02 ,
             FILL-IN-Registros:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedexp-01 ,
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-Registros FILL-IN_FchEnt 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE FILL-IN-Registros BUTTON-12 BtnDone FILL-IN_FchEnt BUTTON-1 RECT-3 
         RECT-4 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      FOR EACH B-CPEDI:
          ...
          IF VtaCDocu.FlgEst = 'T' THEN VtaCDocu.FlgEst = 'P'.
          RELEASE VtaCDocu.
          IF B-CPEDI.ImpTot <= 0 THEN NEXT.
          ...
          BUFFER-COPY B-CPEDI 
              ...
              ASSIGN 
                ...
                VtaCDocu.NroRef = B-CPEDI.NroPed
                VtaCDocu.FlgEst = 'P'
                VtaCDocu.FchCre = TODAY
                Vtacdocu.usuario = s-user-id
                Vtacdocu.codref = B-CPEDI.CodPed
                Vtacdocu.nroref = B-CPEDI.NroPed.
/*           ASSIGN                                     */
/*             Vtacordiv.NroCor = Vtacordiv.NroCor + 1. */
          ASSIGN
              Vtacdocu.FlgIgv = s-FlgIgv
          ...
          FOR EACH T-DDOCU WHERE T-DDOCU.codcia = B-CPEDI.codcia
                ...
                AND T-DDOCU.CanPed > 0:
              {vtagn/i-vtaddocu-01.i}
              I-NITEM = I-NITEM + 1.
              CREATE VtaDDocu.
              BUFFER-COPY T-DDOCU 
                  EXCEPT T-DDOCU.Libre_d01 T-DDOCU.Libre_d02 T-DDOCU.Libre_c05 T-DDOCU.CanAte
                  TO VtaDDocu
                  ASSIGN
                      ...
                      VtaDDocu.Libre_d01 = T-DDOCU.Libre_d01    /* Nuevo Saldo por Atender */
                      VtaDDocu.Libre_d02 = VtaDDocu.CanPed.     /* Nueva Cantidad Negociada */
          END.
...
RUN Pinta-Browse IN h_b-pedexp-02
    ( INPUT pRowid /* ROWID */).
