...
&Scoped-Define ENABLED-OBJECTS BUTTON-Buscar COMBO-NroSer 
...
DEFINE VARIABLE h_f-odv-gral-expo-pag1 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-orden-de-venta AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BUTTON-Buscar 
     IMAGE-UP FILE "img/b-buscar.ico":U
     LABEL "Button 1" 
     SIZE 6 BY 1.35 TOOLTIP "Buscar Pedido".
...
DEFINE FRAME F-Main
     BUTTON-Buscar AT ROW 1 COL 48 WIDGET-ID 8
     COMBO-NroSer AT ROW 1.27 COL 24.71 WIDGET-ID 4
     "Buscar:" VIEW-AS TEXT
...
ON CHOOSE OF BUTTON-Buscar IN FRAME F-Main /* Button 1 */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 1.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-navico-2 ( 1.35 , 20.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/v-orden-de-venta.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-orden-de-venta ).
       RUN set-position IN h_v-orden-de-venta ( 2.35 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-pedido ).
       RUN set-position IN h_q-pedido ( 1.00 , 37.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 9.00 ) */

       /* Links to SmartViewer h_v-orden-de-venta. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-orden-de-venta ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico-2 ,
             BUTTON-Buscar:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-orden-de-venta ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             h_v-orden-de-venta , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/f-odv-gral-expo-pag1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_f-odv-gral-expo-pag1 ).
       RUN set-position IN h_f-odv-gral-expo-pag1 ( 2.62 , 133.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-odv-gral-expo-pag1 ,
             h_v-orden-de-venta , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotizacion-general ,
             h_f-odv-gral-expo-pag1 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-Buscar COMBO-NroSer 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "Exportar-a-Pdf" THEN DO:
         RUN Exportar-a-Pdf IN h_v-orden-de-venta.
    END.
    WHEN "Exportar-a-Excel" THEN DO:
        ...
        CASE TRUE :
            WHEN INDEX(gn-divi.canalventa, "MIN") > 0
                THEN RUN Excel_Utilex IN h_v-orden-de-venta (INPUT lchoice) .
            WHEN s-CodDiv = '00101' THEN 
                RUN Genera-Excel-Delivery IN h_v-orden-de-venta (INPUT YES).   /* Siempre con IGV */
            OTHERWISE RUN Genera-Excel2 IN h_v-orden-de-venta (INPUT lchoice) .
        END CASE.
    END.
    WHEN "Transferencia-de-Saldos" THEN DO:
        DEF VAR pRowidS AS ROWID.
        RUN Transferencia-de-Saldos IN h_v-orden-de-venta ( OUTPUT pRowidS /* ROWID */).
        IF pRowidS <> ? THEN RUN Posiciona-Registro IN h_q-pedido ( INPUT pRowidS /* ROWID */).
    END.
    WHEN "Captura-Historico-Cotizaciones" THEN DO:
        RUN Captura-Historico-Cotizaciones IN h_v-orden-de-venta.
    END.
    WHEN "Importar-Excel-2015" THEN DO:
        RUN Importar-Excel-2015 IN h_v-orden-de-venta.
    END.
    WHEN "Importar-Excel-Provincias" THEN DO:
        RUN Importar-Excel-Provincias IN h_v-orden-de-venta.
    END.
    WHEN "Importar-Excel-Marco" THEN DO:
        RUN Importar-Excel-Marco IN h_v-orden-de-venta.
    END.
    WHEN "Importar-listaexpress-iversa" THEN DO:
        /*RUN Importar-listaexpress-iversa IN h_v-orden-de-venta.*/
        RUN Importar-listaexpress IN h_v-orden-de-venta.
        IF RETURN-VALUE = "ADM-ERROR" THEN RETURN NO-APPLY.
        MESSAGE 'Proceso Terminado' VIEW-AS ALERT-BOX INFORMATION.
    END.
    WHEN "Importar-tiendas-B2B" THEN DO:
        RUN Importar-tiendas-B2B IN h_v-orden-de-venta.
    END.
    WHEN "Edi-Comparativo" THEN DO:
        RUN Edi-Comparativo IN h_v-orden-de-venta.
    END.
    WHEN "Importar-Supermercados" THEN DO:
        RUN Importar-Supermercados IN h_v-orden-de-venta.
        IF RETURN-VALUE = "ADM-ERROR" THEN RETURN NO-APPLY.
        RUN dispatch IN h_t-cotizacion-unificada ('open-query':U).
    END.
    WHEN "Importar-PET" THEN DO:
        RUN Importar-PET IN h_v-orden-de-venta.
    END.
    ...
    WHEN "Actualiza-datos-cliente" THEN DO:
        RUN Actualiza-datos-cliente IN h_v-orden-de-venta.
    END.
    WHEN "Cuenta-Corriente" THEN DO:
        RUN Cuenta-Corriente IN h_v-orden-de-venta.
    END.
    WHEN "Enviar-al-Administrador" THEN DO:
        RUN dispatch IN h_v-orden-de-venta ('Enviar-al-Administrador').
    END.
    WHEN "Importar-MarketPlace" THEN DO:
        RUN Importar-MarketPlace IN h_v-orden-de-venta.
        RUN dispatch IN h_q-pedido ('open-query':U).
    END.
...
CASE L-Handle:
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
        ...
        /* BOTONERAS */
        RUN dispatch IN h_f-odv-gral-expo-pag1 ('hide':U).
        CASE s-TpoPed:
            WHEN "E" THEN DO:      /* Eventos */
                RUN dispatch IN h_f-odv-gral-expo-pag1 ('view':U).
                RUN dispatch IN h_f-cot-gral-exporta-a-excel ('hide':U).
            END.
        END CASE.
        /* Repintamos pantalla */
        RUN dispatch IN h_v-orden-de-venta ('display-fields':U).
    END.
    ...
    WHEN "Disable-Head" THEN DO:
        RUN dispatch IN h_v-orden-de-venta ('disable-fields':U).
    END.
    WHEN "Enable-Head" THEN DO:
        RUN dispatch IN h_v-orden-de-venta ('enable-fields':U).
    END.
    WHEN "browse" THEN DO:
          IF h_b-cotizacion-general <> ? THEN RUN dispatch IN h_b-cotizacion-general ('open-query':U).
          IF h_t-cotizacion-unificada <> ? THEN RUN dispatch IN h_t-cotizacion-unificada ('open-query':U). 
    END.
    WHEN "Recalculo" THEN RUN Recalcular-Precios IN h_v-orden-de-venta.
...
CASE L-Handle:
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         RUN adm-hide IN h_b-cotizacion-general.
         CASE s-TpoPed:
             WHEN "E" THEN DO:      /* Eventos */
                 RUN dispatch IN h_f-cot-gral-expo-pag1 ('view':U).
                 RUN adm-view IN h_b-cotizacion-general.
             END.
    ...
    END.
    WHEN "Recalculo" THEN RUN Recalcular-Precios IN h_v-orden-de-venta.
    WHEN 'IBC' THEN RUN IBC-Diferencias IN h_t-cotizacion-unificada.
    WHEN "Add-Record"   THEN RUN notify IN h_p-updv12   ('add-record':U).
