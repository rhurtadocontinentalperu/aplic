...
&SCOPED-DEFINE l-lookup h_l-vtacdocu-01
...
DEFINE VARIABLE h_l-vtacdocu-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 3.69 COL 81
     Btn_Cancel AT ROW 5.31 COL 81
     SPACE(5.42) SKIP(10.03)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/l-vtacdocu-01.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Codigo':U ,
             OUTPUT h_l-vtacdocu-01 ).
       RUN set-position IN h_l-vtacdocu-01 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-vtacdocu-01 ( 15.50 , 78.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 80.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 13.29 ) NO-ERROR.

       /* Links to SmartLookup h_l-vtacdocu-01. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-vtacdocu-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-vtacdocu-01 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-vtacdocu-01 , 'AFTER':U ).
    END. /* Page 0 */
