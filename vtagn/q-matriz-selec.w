...
DEF VAR x-Matriz AS CHAR NO-UNDO.
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH VtaCMatriz WHERE ~{&KEY-PHRASE} ~
      AND VtaCMatriz.CodCia = s-codcia ~
 AND VtaCMatriz.Matriz = x-Matriz NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH VtaCMatriz WHERE ~{&KEY-PHRASE} ~
      AND VtaCMatriz.CodCia = s-codcia ~
 AND VtaCMatriz.Matriz = x-Matriz NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main VtaCMatriz
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main VtaCMatriz
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "VtaCMatriz.CodCia = s-codcia
 AND VtaCMatriz.Matriz = x-Matriz"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Captura-Parametros q-tables 
...
DEF INPUT PARAMETER pMatriz AS CHAR.
