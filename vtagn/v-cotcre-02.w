...
&Scoped-Define ENABLED-FIELDS VtaCDocu.Glosa 
...
VtaCDocu.CodMon VtaCDocu.TpoCmb VtaCDocu.Glosa 
...
"US$", 2
     ...
     VtaCDocu.TpoCmb AT ROW 9.62 COL 111 COLON-ALIGNED WIDGET-ID 70
          VIEW-AS FILL-IN 
          SIZE 12.86 BY 1
     VtaCDocu.Glosa AT ROW 10.69 COL 19 COLON-ALIGNED WIDGET-ID 76
          VIEW-AS FILL-IN 
          SIZE 70 BY 1
     "Moneda:" VIEW-AS TEXT
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  IF LOOKUP (Vtacdocu.FlgEst, "P,X") = 0 THEN DO:
      MESSAGE "Acceso Denegado" VIEW-AS ALERT-BOX ERROR.
      RETURN "ADM-ERROR".
  END.
  IF VtaCDocu.CodOri = "IBC" THEN DO:
      MESSAGE "Acceso Denegado" SKIP 
          "Es un movimiento de SUPERMERCADOS"
          VIEW-AS ALERT-BOX ERROR.
      RETURN "ADM-ERROR".
  END.
  RUN Carga-Temporal.
...
IF Vtacdocu.flgest = "P" THEN DO:
    ...
    pPorDto = Vtacdocu.MrgUti.
    RUN vtagn/p-dscto-max (Vtacdocu.codcli, s-user-id, OUTPUT pDtoMax).
    IF pPorDto > pDtoMax THEN DO:
        MESSAGE 'Porcentaje de Descuento excede al permitido' SKIP                    
            ...
            VIEW-AS ALERT-BOX WARNING.
        Vtacdocu.FlgEst = "X".
        VtaCDocu.Libre_c05 = 'MARGEN DE UTILIDAD EXCEDE LO PERMITIDO'.
        RUN dispatch IN THIS-PROCEDURE ('display-fields').
    END.
    /* Margenes */
    FIND FIRST Vtaddocu OF Vtacdocu WHERE VtaDDocu.MrgUti <= 0 NO-LOCK NO-ERROR.
    IF AVAILABLE Vtaddocu THEN DO:
        MESSAGE 'Margen negativo en el producto' Vtaddocu.codmat SKIP                    
            '  La cotización pasa a estado POR APROBAR  '
            VIEW-AS ALERT-BOX WARNING.
        Vtacdocu.FlgEst = "X".
        VtaCDocu.Libre_c05 = 'MARGEN DE UTILIDAD NEGATIVO'.
        RUN dispatch IN THIS-PROCEDURE ('display-fields').
    END.
END.
