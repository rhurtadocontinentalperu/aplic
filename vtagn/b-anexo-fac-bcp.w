...
CcbCDocu.DivOri = '00519' AND ~
CcbCDocu.CodDoc = 'FAC' AND ~
(CcbCDocu.FchDoc >= x-FchDoc-1 AND CcbCDocu.FchDoc <= x-FchDoc-2) AND ~
CcbCDocu.CodCli = x-CodCli AND ~
CcbCDocu.TpoFac = 'F' AND ~
Ccbcdocu.CodRef = 'FAI' AND ~
CcbCDocu.FlgEst <> 'A' )

DEF TEMP-TABLE Detalle
    FIELD CustomerPurchaseOrder LIKE FacCPedi.CustomerPurchaseOrder LABEL 'Pedido Compra'
    FIELD NroItm AS INT FORMAT '>>>9' LABEL 'Posición Pedido'
    FIELD NomCli LIKE Ccbcdocu.CodCli LABEL 'Nombre Proveedor'
    FIELD CustomerLabel LIKE FacCPedi.CustomerLabel LABEL 'Código Centro'
    FIELD CustomerStockDepo LIKE FacCPedi.CustomerStockDepo LABEL 'Código Almacén'
    FIELD CustomerStockDepoName LIKE FacCPedi.CustomerStockDepoName LABEL 'Nombre Almacén'
    FIELD OfficeCustomer LIKE FacCPedi.OfficeCustomer LABEL 'Centro Costos'
    FIELD FchEnt AS DATE FORMAT '99/99/9999' LABEL 'Fecha Entrega'
    FIELD CustomerArtCode LIKE FacDPedi.CustomerArtCode LABEL 'Código Material'
    FIELD CustomerArtDescription LIKE FacDPedi.CustomerArtDescription LABEL 'Descripción Material'
    FIELD CanDes LIKE CcbDDocu.CanDes LABEL 'Cantidad'
    FIELD PreUni LIKE CcbDDocu.PreUni LABEL 'Precio Unitario'
    FIELD ImpLin LIKE CcbDDocu.ImpLin LABEL 'Valor Total'
    .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.NroDoc FORMAT "X(12)":U WIDTH 12.57
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 106 BY 20.46
...
DEFINE FRAME F-Main
     ...
     "Puede seleccionar uno o más registros" VIEW-AS TEXT
          SIZE 36 BY .62 AT ROW 21.46 COL 1 WIDGET-ID 2
...
"CcbCDocu.NroDoc" ? ? "character" ? ? ? ? ? ? no ? no no "12.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO x-Item = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
    IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(x-Item) THEN DO:
        /* Buscamos todas sus FAIS */
        FOR EACH FAIS NO-LOCK WHERE FAIS.codcia = Ccbcdocu.codcia AND
            FAIS.coddiv = Ccbcdocu.coddiv AND
            FAIS.coddoc = "FAI" AND
            FAIS.Libre_c03 = Ccbcdocu.CodDoc AND
            FAIS.Libre_c04 = Ccbcdocu.NroDoc,
            FIRST PEDIDO NO-LOCK WHERE PEDIDO.codcia = FAIS.codcia AND
            PEDIDO.coddoc = FAIS.codped AND
            PEDIDO.nroped = FAIS.nroped,
            FIRST COTIZACION NO-LOCK WHERE COTIZACION.codcia = PEDIDO.codcia AND
            COTIZACION.coddoc = PEDIDO.codref AND
            COTIZACION.nroped = PEDIDO.nroref,
            EACH Facdpedi OF COTIZACION NO-LOCK,
            FIRST Ccbddocu OF Ccbcdocu NO-LOCK WHERE Ccbddocu.CodMat = Facdpedi.CodMat:
            CREATE Detalle.
            BUFFER-COPY COTIZACION TO Detalle
                ASSIGN
                Detalle.NroItm = Facdpedi.NroItm
                Detalle.CustomerArtCode = FacDPedi.CustomerArtCode
                Detalle.CustomerArtDescription = FacDPedi.CustomerArtDescription
                Detalle.CanDes = CcbDDocu.CanDes
                Detalle.PreUni = CcbDDocu.PreUni
