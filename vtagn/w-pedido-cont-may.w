...
DEFINE VARIABLE h_t-pedido-cont-may AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-pedido-cont-may AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BTN-COTIZA AT ROW 7.46 COL 127 WIDGET-ID 18
     BTN-PREPEDIDO AT ROW 8.69 COL 127 WIDGET-ID 22
     Btn-Grupos-2 AT ROW 22.92 COL 37 WIDGET-ID 20
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* SETTINGS FOR BUTTON BTN-COTIZA IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       BTN-COTIZA:HIDDEN IN FRAME F-Main           = TRUE.
...
/* SETTINGS FOR BUTTON BTN-PREPEDIDO IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       BTN-PREPEDIDO:HIDDEN IN FRAME F-Main           = TRUE.
...
DO:
  RUN Asigna-Cotizacion IN h_v-pedido-cont-may.
  IF RETURN-VALUE = "ADM-ERROR" THEN RETURN NO-APPLY.
  RUN dispatch IN h_t-pedido-cont-may ('open-query':U).
...
DO:
    ...
    RUN Recalcular-Precios IN h_t-pedido-cont-may.
    RUN dispatch IN h_t-pedido-cont-may ('open-query').
END.
...
&Scoped-define SELF-NAME BTN-PREPEDIDO
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BTN-PREPEDIDO W-Win
ON CHOOSE OF BTN-PREPEDIDO IN FRAME F-Main /* PREPEDIDO */
DO:
  RUN Asigna-PrePedido IN h_v-pedido-cont-may.
  IF RETURN-VALUE = "ADM-ERROR" THEN RETURN NO-APPLY.
  RUN dispatch IN h_t-pedido-cont-may ('open-query':U).
  {&self-name}:SENSITIVE = NO.
END.
...
CASE s-CodDoc:
    WHEN "P/M" THEN W-Win:TITLE = "PEDIDOS MAYORISTAS CONTADO".
END CASE.
CASE s-TpoPed:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 2.35 , 127.00 ) NO-ERROR.
       RUN set-size IN h_p-navico-2 ( 5.12 , 10.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/v-pedido-cont-may.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedido-cont-may ).
       RUN set-position IN h_v-pedido-cont-may ( 2.62 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-pedido-cont-may. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_v-pedido-cont-may ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-pedido-cont-may ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedido-cont-may ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedido-cont ,
             BTN-PREPEDIDO:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/t-pedido-cont-may.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedido-cont-may ).
       RUN set-position IN h_t-pedido-cont-may ( 11.38 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedido-cont-may ( 11.85 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-pedido-cont-may. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedido-cont-may ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedido-cont-may ,
             BTN-PREPEDIDO:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-pedido-cont-may , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
...
CASE L-Handle:
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         ASSIGN
             ...
             Btn-PrePedido:VISIBLE = NO
             Btn-PrePedido:SENSITIVE = NO
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         ASSIGN
             ...
             Btn-PrePedido:VISIBLE = YES
             Btn-PrePedido:SENSITIVE = YES
    ...
    WHEN "Enable-Head" THEN DO:
      ...
      END.
    WHEN "Disable-btn-cotiza" THEN BTN-Cotiza:SENSITIVE = NO.
    WHEN "Disable-btn-prepedido" THEN BTN-PREPEDIDO:SENSITIVE = NO.
    WHEN "Add-Record" THEN DO:
        RUN dispatch IN h_t-pedido-cont-may ('add-record':U).
      END.
    WHEN "browse" THEN DO:
          IF h_b-pedido-cont <> ? THEN RUN dispatch IN h_b-pedido-cont ('open-query':U).
          IF h_t-pedido-cont-may <> ? THEN RUN dispatch IN h_t-pedido-cont-may ('open-query':U). 
      END.
