...
DEFINE SHARED VAR s-OrigenPedido AS CHAR.   /* CONTROL DE ORIGEN DE DATOS */
...
DEFINE TEMP-TABLE tPEDI2 LIKE PEDI2.

DEFINE TEMP-TABLE bkpw-report LIKE w-report.
...
DO:
  ...
  IF RETURN-VALUE = 'YES' AND s-OrigenPedido > "" THEN DO:
      FIND FIRST FacCVtaOnLine WHERE FacCVtaOnLine.codcia = s-codcia
          ...
          AND FacCVtaOnLine.codorigen = s-OrigenPedido
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    CASE s-OrigenPedido:
        WHEN "RIQRA" THEN DO:
            ...
            HIDE FRAME F-Proceso.
        END.
        WHEN "CONTINENTAL" THEN DO:
            /* Ya los pedidos están en Progress */
        END.
        OTHERWISE RETURN "OK".
    END CASE.
...
/*             /* *********************************************************************************************** */ */
    ...
    END.
    /* 05/08/2025: La cantidad sugerida se hace al final */
    FOR EACH PEDI2 EXCLUSIVE-LOCK:
        /* *********************************************************************************************** */
        /* Cantidad Sugerida */
        /* *********************************************************************************************** */
...
cCodCli = VtaCDocu.CodCli:SCREEN-VALUE IN FRAME {&FRAME-NAME}.

EMPTY TEMP-TABLE tw-report.

/* Excels */
RUN vtagn/d-precotizacion-unificada-excels.r(INPUT cCodCli,INPUT-OUTPUT TABLE tw-report,OUTPUT iConteo).

IF iConteo <= 0 THEN RETURN 'ADM-ERROR'.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE importar-excel-2015a V-table-Win 
PROCEDURE importar-excel-2015a :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE VAR FILLINArchivo AS CHAR.
DEFINE VAR lError AS LOG.

Fi-Mensaje = "Importando EXCEL".
DISPLAY Fi-Mensaje WITH FRAME F-Proceso.

EMPTY TEMP-TABLE tPEDI2.

FOR EACH PEDI2 NO-LOCK:
    CREATE tPEDI2.
    BUFFER-COPY PEDI2 TO tPEDI2.
END.
...
FOR EACH tw-report NO-LOCK:
    FILLINArchivo = tw-report.campo-c[2].
    
    RUN PPX_Importar-Excel IN hProc (INPUT VtaCDocu.CodCli:SCREEN-VALUE IN FRAME {&FRAME-NAME},
                                     INPUT FILLINArchivo,
                                     INPUT-OUTPUT TABLE TPEDI2,
                                     OUTPUT pMensaje).
    IF RETURN-VALUE = "ADM-ERROR" THEN DO:
        lError = YES.
        LEAVE LOOP_EXCEL.
...
FOR EACH tPEDI2 NO-LOCK:
    CREATE PEDI2.
    BUFFER-COPY tPEDI2 TO PEDI2.
END.

EMPTY TEMP-TABLE bkpw-report.

/* bkpw-report : guardar */
FOR EACH tw-report NO-LOCK:
    CREATE bkpw-report.
    BUFFER-COPY tw-report  TO bkpw-report.
END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR cCodPPedido AS CHAR.
  DEFINE VAR cNroPPedido AS CHAR.
  DEFINE VAR cCliePPedido AS CHAR.
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      ...
      IF ERROR-STATUS:ERROR = YES THEN DO:
          ...
          UNDO, RETURN 'ADM-ERROR'.
      END.
      cCodPPedido = VtaCDocu.CodPed.
      cNroPPedido = VtaCDocu.NroPed.
      cCliePPedido = VtaCDocu.Codcli.
...
/*      END.                                                                              */
  ...
  /* Grabar en una tabla los Excels procesados */
  RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
  IF RETURN-VALUE = 'YES' THEN DO:
      FOR EACH bkpw-report NO-LOCK:
          CREATE vtatabla.
          ASSIGN vtatabla.codcia = 1 
                ...
                vtatabla.llave_c4 = bkpw-report.campo-c[1]
                vtatabla.libre_C01 = USERID("DICTDB")
                vtatabla.rango_fecha[1] = TODAY
                vtatabla.rango_fecha[2] = bkpw-report.campo-d[1]
                vtatabla.rango_Valor[1] = bkpw-report.campo-i[1]
                vtatabla.libre_C02 = STRING(TIME,"hh:mm:ss").
          /*  aca grabar */
      END.
  END.
  RUN dispatch IN THIS-PROCEDURE ('display-fields':U).
...
/*              Faccpedi.FlgEst = "P".                            */
  ...
  END.
     ...
     /* PREPEDIDOS cargados con EXCEL */
    DEFINE BUFFER cvtatabla FOR vtatabla.

    FOR EACH vtatabla WHERE vtatabla.codcia = 1 AND vtatabla.tabla = 'DIGITACIONEXPO' AND
                            vtatabla.llave_c1 = VtaCDocu.CodCli AND vtatabla.llave_c2 = VtaCDocu.codped AND
                            vtatabla.llave_c3 = VtaCDocu.nroped NO-LOCK:
        FIND FIRST cvtatabla WHERE ROWID(vtatabla) = ROWID(cvtatabla) EXCLUSIVE-LOCK NO-ERROR.
        IF AVAILABLE cvtatabla THEN DO:
            DELETE cvtatabla.
            /*
            ASSIGN cvtatabla.llave_c8 = cvtatabla.llave_c8 + if( NOT( TRUE <> (cvtatabla.llave_c8 > "")) ) THEN ","  ELSE "" + cvtatabla.llave_c3
                    cvtatabla.llave_c3 = "".
            */                                                                                                                          
        END.
        RELEASE cvtatabla NO-ERROR.
    END.
