...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacTabla.Nombre FORMAT "x(40)":U WIDTH 52.14
  ENABLE
      FacTabla.Codigo
      FacTabla.Nombre
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 66 BY 14.04
...
"FacTabla.Nombre" ? ? "character" ? ? ? ? ? ? yes ? no no "52.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON 'RETURN':U OF  FacTabla.Codigo, FacTabla.Nombre
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
IF FacTabla.Codigo:SCREEN-VALUE IN BROWSE {&browse-name} = '' THEN DO:
    MESSAGE 'Debe ingresar el código' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO FacTabla.Codigo.
    RETURN 'ADM-ERROR'.
END.
RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
IF RETURN-VALUE = 'YES' AND CAN-FIND(FIRST FacTabla WHERE FacTabla.CodCia = s-codcia
