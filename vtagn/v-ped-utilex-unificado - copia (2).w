...
DEFINE TEMP-TABLE PEDI-4 NO-UNDO LIKE FacDPedi.
...
/*&SCOPED-DEFINE precio-venta-general-mayor pri/PrecioVentaMayorCreditoFlash*/
...
DEF SHARED VAR s-VentaMinorista LIKE GN-DIVI.VentaMinorista.
...
DEFINE BUFFER W-CPEDI FOR Faccpedi.
...
DEFINE VAR x-cond-vta-rappi AS CHAR INIT "603".
/*DEFINE VAR x-division-rappi AS CHAR INIT "00101".       /* """"""""" */*/
DEFINE VAR x-listaprecio-rappi AS CHAR INIT "00523".
...
FacCPedi.NroRef FacCPedi.CodOrigen FacCPedi.NroOrigen 
&Scoped-define ENABLED-TABLES FacCPedi
&Scoped-define FIRST-ENABLED-TABLE FacCPedi
&Scoped-Define ENABLED-OBJECTS BUTTON_Contrato TOGGLE-DELIVERY 
...
FacCPedi.CodOrigen FacCPedi.NroOrigen 
&Scoped-define DISPLAYED-TABLES FacCPedi
&Scoped-define FIRST-DISPLAYED-TABLE FacCPedi
&Scoped-Define DISPLAYED-OBJECTS TOGGLE-DELIVERY FILL-IN-programa F-Estado ~
F-nOMvEN COMBO-BOX-Proveedor F-CndVta F-Nomtar txtNombrePersonal 
...
DEFINE BUTTON BUTTON_Contrato 
     LABEL "REGISTRA CONTRATO" 
     SIZE 19 BY 1.12.
...
DEFINE VARIABLE TOGGLE-DELIVERY AS LOGICAL INITIAL no 
     LABEL "¿ Venta RAPPI ?" 
     VIEW-AS TOGGLE-BOX
     SIZE 26 BY .96
...
DEFINE FRAME F-Main
     BUTTON_Contrato AT ROW 5.58 COL 130 WIDGET-ID 134
     TOGGLE-DELIVERY AT ROW 1.35 COL 50 WIDGET-ID 130
...
DEFINE FRAME F-Main
     ...
     FacCPedi.Libre_c01 AT ROW 3.54 COL 118 COLON-ALIGNED WIDGET-ID 132
          LABEL "Delivery"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     FacCPedi.CodOrigen AT ROW 4.62 COL 118 COLON-ALIGNED WIDGET-ID 136
          LABEL "No. Doc PPO" FORMAT "x(25)"
          VIEW-AS FILL-IN 
          SIZE 4 BY .81
     FacCPedi.NroOrigen AT ROW 4.62 COL 122.57 COLON-ALIGNED NO-LABEL WIDGET-ID 138
          VIEW-AS FILL-IN 
          SIZE 12.14 BY .81
     "Moneda:" VIEW-AS TEXT
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: PEDI-4 T "?" NO-UNDO INTEGRAL FacDPedi
...
&Scoped-define SELF-NAME BUTTON_Contrato
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON_Contrato V-table-Win
ON CHOOSE OF BUTTON_Contrato IN FRAME F-Main /* REGISTRA CONTRATO */
DO:

    DEFINE VAR x-coddoc AS CHAR.
    DEFINE VAR x-nrodoc AS CHAR.

    RUN vtagn/d-operador-contrato (INPUT Faccpedi.FlgSit,       /* Tipo DOc */
                                   INPUT Faccpedi.Libre_c02,    /* Operador */
                                   INPUT Faccpedi.CodCli,       /* Codigo del cliente */
    ...
    IF pMensaje = "OK" THEN DO:
        ASSIGN 
            faccpedi.codorigen:SCREEN-VALUE = x-coddoc
            faccpedi.nroorigen:SCREEN-VALUE = x-nrodoc.

        FIND FIRST w-cpedi WHERE ROWID(W-CPEDI) = ROWID(Faccpedi) EXCLUSIVE-LOCK NO-ERROR.
        IF AVAILABLE w-cpedi THEN DO:
            ASSIGN 
                w-cpedi.codorigen = x-coddoc
                w-cpedi.nroorigen = x-nrodoc.
            RELEASE w-cpedi NO-ERROR.
        END.
        ELSE DO:
            RELEASE w-cpedi NO-ERROR.
            MESSAGE "No se pudo grabar el CONTRATO" SKIP
                    "Vuelva intentar...por favor" VIEW-AS ALERT-BOX INFORMATION.
        END.
    END.
END.
...
DO:
  ...
  IF NOT AVAILABLE gn-clie THEN DO:      /* CREA EL CLIENTE NUEVO */
      RUN vtamay/d-regcli-utilex (INPUT-OUTPUT S-CODCLI).
  ...
  /* 23/09/2022: Coordinación GLL y JH */
  /* **************************************** */
...
DO:
    ...
    /* Valores por Defecto */
    BUTTON_Contrato:VISIBLE = NO.
    /*
    BUTTON_Contrato:SENSITIVE = NO.
    */
    S-FMAPGO = "000".   /* Valor por defecto */
    IF toggle-delivery:SCREEN-VALUE = "YES" THEN DO:
        S-FMAPGO = x-cond-vta-rappi.
    END.

    COMBO-BOX-Proveedor:LABEL = "Proveedor Vales".
    COMBO-BOX-Proveedor:DELETE(COMBO-BOX-Proveedor:LIST-ITEMS).
    ...
    CASE s-FlgSit:
        WHEN "KC" THEN DO:
            ...
            ENABLE faccpedi.libre_c05 WITH FRAME {&FRAME-NAME}.
            faccpedi.libre_c05:LABEL IN FRAME {&FRAME-NAME} = "INGRESE BARRAS".
            faccpedi.codorigen:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ''.
            faccpedi.nroorigen:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ''.
            FIND FIRST gn-prov WHERE gn-prov.codcia = pv-codcia AND gn-prov.codpro = '10003814'
                NO-LOCK NO-ERROR.
        ...
        WHEN "PPO" THEN DO:
            ...
            /*
            COMBO-BOX-Proveedor:DELETE(COMBO-BOX-Proveedor:LIST-ITEMS).
            x-filer = COMBO-BOX-Proveedor:LABEL.
            */
            IF NOT CAN-FIND(FIRST VtaCTabla WHERE VtaCTabla.codcia = s-codcia AND
                            VtaCTabla.tabla = "PAGOXOPERADOR" /*s-flgsit*/ NO-LOCK)
                THEN DO:
                MESSAGE 'NO hay ningún operador configurado' VIEW-AS ALERT-BOX ERROR.
                /*RETURN NO-APPLY.*/
            END.
            COMBO-BOX-Proveedor:LABEL = "OPERADOR".
            COMBO-BOX-Proveedor:DELETE(COMBO-BOX-Proveedor:LIST-ITEMS).
            FOR EACH VtaCTabla WHERE VtaCTabla.codcia = s-codcia AND
                VtaCTabla.tabla = "PAGOXOPERADOR",
                FIRST gn-prov NO-LOCK WHERE gn-prov.codcia = pv-codcia
                AND gn-prov.codpro = VtaCTabla.llave
                BREAK BY VtaCTabla.llave:
                COMBO-BOX-Proveedor:ADD-LAST(gn-prov.codpro + ' - ' + gn-prov.nompro).
                IF FIRST-OF(VtaCTabla.llave) THEN DO:                    
                    COMBO-BOX-Proveedor:SCREEN-VALUE = (gn-prov.codpro + ' - ' + gn-prov.nompro).
                    s-codpro = gn-prov.codpro.
                END.
            END.
            COMBO-BOX-Proveedor:SENSITIVE = YES.
            toggle-delivery:SCREEN-VALUE = 'NO'.
            S-FMAPGO = "800".       /* Valor Sugerido */

            RUN GET-ATTRIBUTE("ADM-NEW-RECORD").
            IF RETURN-VALUE = "NO" AND Faccpedi.FlgSit:SCREEN-VALUE = "PPO" THEN DO:
                BUTTON_contrato:VISIBLE = YES.
            END.
...
/*     IF s-FlgSit <> 'PPO' THEN DO:                                 */
/*         DISPLAY "" @ FacCPedi.codorigen WITH FRAME {&FRAME-NAME}. */
/*     END.                                                          */

    DISPLAY S-FMAPGO @ FacCPedi.FmaPgo WITH FRAME {&FRAME-NAME}.
...
&Scoped-define SELF-NAME TOGGLE-DELIVERY
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL TOGGLE-DELIVERY V-table-Win
ON VALUE-CHANGED OF TOGGLE-DELIVERY IN FRAME F-Main /* ¿ Venta RAPPI ? */
DO:
    DO WITH FRAME {&FRAME-NAME}:        
        faccpedi.fmapgo:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "000".
        IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'YES' THEN DO:

            FIND FIRST gn-convt WHERE gn-convt.codig = x-cond-vta-rappi NO-LOCK NO-ERROR.

            IF AVAILABLE gn-convt THEN DO:
                faccpedi.flgsit:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
                faccpedi.libre_c01:SCREEN-VALUE = x-listaprecio-rappi. /* Lista de Precios RAPPI */
                faccpedi.fmapgo:SCREEN-VALUE IN FRAME {&FRAME-NAME} = x-cond-vta-rappi.
            END.
            ELSE DO:
                MESSAGE "La condicion de venta (" + x-cond-vta-rappi + ") No existe"
                    VIEW-AS ALERT-BOX INFORMATION.
            END.

        END.
        ELSE DO:
            faccpedi.flgsit:SCREEN-VALUE  = "".
            faccpedi.libre_c01:SCREEN-VALUE = "".
            COMBO-BOX-Proveedor:DELETE(COMBO-BOX-Proveedor:LIST-ITEMS).
        END.

        APPLY 'LEAVE':U TO faccpedi.fmapgo.

        APPLY 'VALUE-CHANGED':U TO faccpedi.flgsit.
    END.
END.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE faccpedi THEN DO:          
      IF faccpedi.codcli = '11111111119' THEN DO:        
          MESSAGE "Codigo de cliente esta considerado como PRUEBA " SKIP.
                    " Imposible generar PAGO".
          RETURN "ADM-ERROR".
      END.
  END.      

  /* Verificar si es PAGO POR PROVEEDOR (PPO) */
  IF faccpedi.flgsit = 'PPO' THEN DO:

    IF TRUE <> (faccpedi.codorigen > "") THEN DO:
        MESSAGE "Ingrese el documento Pago por Operador (" + faccpedi.flgsit + ")" SKIP.
                  " Imposible generar PAGO".
        RETURN "ADM-ERROR".
    END.

    FIND FIRST x-ccbcdocu WHERE x-ccbcdocu.codcia = s-codcia AND 
        x-ccbcdocu.coddoc = faccpedi.codorigen AND
        x-ccbcdocu.nrodoc = faccpedi.nroorigen NO-LOCK NO-ERROR.
    IF NOT AVAILABLE x-ccbcdocu THEN DO:
        MESSAGE "El documento " + faccpedi.codorigen + ' ' + faccpedi.nroorigen + " NO EXISTE" SKIP.
                  " Imposible generar PAGO".
        RETURN "ADM-ERROR".
    END.

    IF x-ccbcdocu.flgest = 'A' THEN DO:
        MESSAGE "El documento " + faccpedi.flgsit + " " + faccpedi.nroorigen + " ESTA ANULADO" SKIP.
                  " Imposible generar PAGO".
        RETURN "ADM-ERROR".
    END.
    IF x-ccbcdocu.imptot <> x-ccbcdocu.sdoact THEN DO:
        MESSAGE "El documento " + faccpedi.flgsit + " " + faccpedi.nroorigen + " YA FUE CONSUMIDO" SKIP.
                  " Imposible generar PAGO".
        RETURN "ADM-ERROR".
    END.
    IF faccpedi.imptot < x-ccbcdocu.sdoact THEN DO:
        MESSAGE "El importe de la venta debe ser mayor/igual al importe del documento " + faccpedi.flgsit + " " + faccpedi.nroorigen SKIP.
                  " Imposible generar PAGO".
        RETURN "ADM-ERROR".
    END.

    IF x-ccbcdocu.codcob <> faccpedi.libre_c02 THEN DO:
        MESSAGE "El OPERADOR del pedido es diferente al del contrato" SKIP.
                  " Imposible generar PAGO".
        RETURN "ADM-ERROR".
    END.
    IF x-ccbcdocu.codant <> faccpedi.atencion THEN DO:
        MESSAGE "El DNI del pedido es diferente al del contrato" SKIP.
                  " Imposible generar PAGO".
        RETURN "ADM-ERROR".
    END.
    IF x-ccbcdocu.codcli <> faccpedi.codcli THEN DO:
        MESSAGE "El codigo de CLIENTE del pedido es diferente al del contrato" SKIP.
                  " Imposible generar PAGO".
        RETURN "ADM-ERROR".
    END.
  ...
  &IF {&ARITMETICA-SUNAT} &THEN
    RUN sunat\cancelar-pedido-minorista-sunat-v31.r (ROWID(FacCPedi)).
  &ELSE
    RUN sunat\cancelar-pedido-minorista-sunat-v3.r (ROWID(FacCPedi)).
  &ENDIF
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = "ADM-ERROR" THEN DO:
      pMensaje = 'No se pudo armar la promoción'.
      RETURN 'ADM-ERROR'.
  END.
  /* ************************************************************************************** */
...
------------------------------------------------------------------------------*/
  ...
  txtNombrePersonal:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ''.
  toggle-delivery:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "no".
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' AND s-FlgEnv = YES THEN DO:
      ...
      IF NOT AVAILABLE PEDI THEN DO:
          pMensaje = "NO hay registros que grabar".
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      ...
      ASSIGN 
        ...
        FacCPedi.Lista_de_Precios = s-CodDiv.
      ASSIGN
          s-PorIgv = FacCfgGn.PorIgv .
  ...
  /* Caso ventas RAPPI */
  ASSIGN faccpedi.lista_de_precios = Faccpedi.Libre_c01:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
  
  /* Detalle del Pedido */
  /*RUN Procesa-Handle IN lh_Handle ('Recalculo').    /* NO afecta venta por CD */*/

  /* 31/10/2022 Optimización de la grabación */
  DEF VAR x-Cuenta AS INTE NO-UNDO.
  DEF VAR x-Cuenta-2 AS INTE NO-UNDO.

  x-Cuenta = 0.
  x-Cuenta-2 = 0.
  EMPTY TEMP-TABLE PEDI-4.
  FOR EACH PEDI NO-LOCK:
      x-Cuenta = x-Cuenta + 1.
      CREATE PEDI-4.
      BUFFER-COPY PEDI TO PEDI-4.
  END.
  RUN Resumen-Pedido.   /* Puede que sean menos registros debido a este proceso */
  FOR EACH PEDI NO-LOCK:
      x-Cuenta-2 = x-Cuenta-2 + 1.
  END.
  IF x-Cuenta <> x-Cuenta-2 THEN DO:    /* Solo si hay diferencia */
      RUN Recalcular-Precios-Update.
  END.

  RUN Genera-Pedido.    /* Detalle del pedido */

  IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
      pMensaje = 'NO se pudo generar el pedido' + CHR(10) +
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      END.
      /* Anulamos PPO */
      IF FacCPedi.CodOrigen = "PPO" THEN DO:
          FIND FIRST Ccbcdocu WHERE Ccbcdocu.codcia = s-codcia
              AND Ccbcdocu.coddoc = Faccpedi.CodOrigen
              AND Ccbcdocu.nrodoc = Faccpedi.NroOrigen
...
------------------------------------------------------------------------------*/
  ...
  BUTTON_contrato:VISIBLE IN FRAME {&FRAME-NAME} = NO.
  ...
  IF AVAILABLE Faccpedi THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      IF NOT (TRUE <> (faccpedi.libre_c01 > ""))  THEN DO:
            toggle-delivery:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "yes".
      END.
      
      BUTTON_Contrato:VISIBLE = NO.
      BUTTON_Contrato:SENSITIVE = NO.
      IF Faccpedi.FlgEst = 'P'  AND FacCPedi.FlgSit = "PPO" AND TRUE <> (faccpedi.codorigen > "") THEN DO:
          BUTTON_Contrato:VISIBLE = YES.
          BUTTON_Contrato:SENSITIVE = YES.
      END.
      
END.
toggle-delivery:SENSITIVE = NO.
    ...
    CASE FacCPedi.FlgSit:
        WHEN "PPO" THEN DO:
            ...
            FOR EACH VtaCTabla WHERE VtaCTabla.codcia = s-codcia AND
                ...
                BREAK BY VtaCTabla.llave:
                IF FIRST-OF(VtaCTabla.llave) THEN DO:
                    IF faccpedi.libre_c02 = gn-prov.codpro THEN DO:
                        COMBO-BOX-Proveedor:ADD-LAST(gn-prov.codpro + ' - ' + gn-prov.nompro).
                        COMBO-BOX-Proveedor:SCREEN-VALUE = (gn-prov.codpro + ' - ' + gn-prov.nompro).
                    END.
                END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:    
    ...
    IF RETURN-VALUE = "NO" AND Faccpedi.FlgSit = "PPO" THEN DO:
        ASSIGN Faccpedi.codorigen:SENSITIVE = NO.
        ASSIGN Faccpedi.nroorigen:SENSITIVE = NO.
        /*BUTTON_contrato:VISIBLE = YES.*/
    END.
...
/**/
RUN venta-delivery-cab(OUTPUT x-lista-precio).

FOR EACH PEDI EXCLUSIVE-LOCK WHERE PEDI.Libre_c05 <> "OF", FIRST Almmmatg OF PEDI NO-LOCK:
    ...
    ELSE DO:
        /* Ventas DELIVERY */
        RUN {&precio-venta-general-mayor} (
            "N",
            x-lista-precio,
            s-CodCli,
            s-CodMon,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Recalcular-Precios-Update V-table-Win 
PROCEDURE Recalcular-Precios-Update :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* SOLO LOS DIFERENTES RECALCULAMOS PRECIOS */
FOR EACH PEDI EXCLUSIVE-LOCK WHERE PEDI.Libre_c05 <> "OF", FIRST Almmmatg OF PEDI NO-LOCK,
    FIRST PEDI-4 NO-LOCK WHERE PEDI-4.CodMat = PEDI.CodMat AND PEDI-4.CanPed <> PEDI.CanPed:
...
-------------------------------------------------------------*/
  ...
  IF p-state = 'update-begin':U THEN DO WITH FRAME {&FRAME-NAME}:
     RUN Actualiza-Item.
     RUN Procesa-Handle IN lh_Handle ('Pagina2').
     /* 31/10/2022 Acordado con Daniel Llican */
     /*RUN Procesa-Handle IN lh_Handle ('Recalculo').*/
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME} :
    ...
    /* RHC 22/07/2020 Nuevo bloqueo de clientes */
    /* **************************************** */
    RUN pri/p-verifica-cliente-utilex (INPUT Faccpedi.CodCli:SCREEN-VALUE,
                                       INPUT s-CodDoc,
                                       INPUT s-CodDiv).
    IF RETURN-VALUE = "ADM-ERROR" THEN DO:
        APPLY "ENTRY" TO FacCPedi.CodCli.
...
/*     END.                                                              */
    ...
    FIND gn-clie WHERE gn-clie.codcia = cl-codcia AND
        gn-clie.codcli = Faccpedi.codcli:SCREEN-VALUE NO-LOCK.
    IF gn-clie.libre_c01 = "N" AND Faccpedi.Atencion:SCREEN-VALUE > '' THEN DO:
...
IF NOT (TRUE <> (Faccpedi.nroorigen > "")) THEN DO:
    MESSAGE 'Pedido ya tiene CONTRATO asignado imposible modificar' VIEW-AS ALERT-BOX ERROR.
    RETURN "ADM-ERROR".
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE venta-delivery-cab V-table-Win 
PROCEDURE venta-delivery-cab :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO WITH FRAME {&FRAME-NAME}:
    pListaPrecio = TRIM(faccpedi.libre_c01:SCREEN-VALUE).
END.
