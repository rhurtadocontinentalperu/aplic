...
DEFINE SHARED VAR s-ControlPromotor AS LOG.     /* CONTROL DE PROMOTOR */
...
&Scoped-define QUERY-STRING-br_table FOR EACH PEDI2 WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      FIRST Almmmatg OF PEDI2 NO-LOCK ~
    BY PEDI2.NroItm
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH PEDI2 WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      FIRST Almmmatg OF PEDI2 NO-LOCK ~
    BY PEDI2.NroItm.
...
&Scoped-Define ENABLED-OBJECTS br_table BUTTON-7 BUTTON-9 btn-elimina 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almmmatg.DesMat FORMAT "X(80)":U WIDTH 78.57
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(25)":U WIDTH 8.43
      PEDI2.UndVta FORMAT "x(6)":U WIDTH 5.43
      PEDI2.CanPed FORMAT "->>>,>>9.9999":U WIDTH 8.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 121 BY 14
...
DEFINE FRAME F-Main
     ...
     BUTTON-9 AT ROW 4.5 COL 123 WIDGET-ID 40
     btn-elimina AT ROW 8.27 COL 123 WIDGET-ID 6
...
"Temp-Tables.PEDI2.Libre_c01" "Pag" "x(4)" "character" ? ? ? ? ? ? no ? no no "3.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.PEDI2.Libre_c02
"Temp-Tables.PEDI2.Libre_c02" "Secu" "x(6)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"integral.Almmmatg.DesMat" ? "X(80)" "character" ? ? ? ? ? ? no ? no no "78.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > integral.Almmmatg.DesMar
"integral.Almmmatg.DesMar" "Marca" "X(25)" "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.PEDI2.UndVta
"Temp-Tables.PEDI2.UndVta" ? "x(6)" "character" ? ? ? ? ? ? no ? no no "5.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.PEDI2.CanPed
"Temp-Tables.PEDI2.CanPed" ? ? "decimal" ? ? ? ? ? ? no ? no no "8.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON CHOOSE OF BUTTON-7 IN FRAME F-Main /* TERCEROS */
DO:
    IF TRUE <> (s-codcli > '') THEN DO:
        ...
        RETURN NO-APPLY.
    END.
    RUN vtagn/d-precot-terceros-v02.w.
    /*RUN vtagn/d-precot-terceros-v03.w.      /* Solo para probar */*/
    RUN dispatch IN THIS-PROCEDURE ('open-query':U).
END.
...
ON CHOOSE OF BUTTON-9 IN FRAME F-Main /* PROPIOS */
DO:
    IF TRUE <> (s-codcli > '') THEN DO:
        MESSAGE 'Debe ingresar Código Cliente'
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        RETURN NO-APPLY.
    END.
    RUN vtagn/d-precot-propios-v01.w.
    RUN dispatch IN THIS-PROCEDURE ('open-query':U).
END.
