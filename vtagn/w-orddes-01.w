...
DEFINE NEW SHARED VARIABLE S-CODPED   AS CHAR INIT 'O/D'.
...
DEFINE NEW SHARED VARIABLE s-codref AS CHAR INIT "PED".
...
DEFINE VARIABLE h_t-orddes-01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-orddes-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 146.72 BY 26.62 WIDGET-ID 100.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/v-orddes-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-orddes-01 ).
       RUN set-position IN h_v-orddes-01 ( 2.62 , 2.14 ) NO-ERROR.
       /* Size in UIB:  ( 10.69 , 144.00 ) */
       ...
       /* Links to SmartViewer h_v-orddes-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_v-orddes-01 ).
       RUN add-link IN adm-broker-hdl ( h_q-vtacdocu-01 , 'Record':U , h_v-orddes-01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv04 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-orddes-01 ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedcre-01 ,
             h_v-orddes-01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-totcotcre-01 ,
             h_b-pedcre-01 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/t-orddes-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-orddes-01 ).
       RUN set-position IN h_t-orddes-01 ( 13.92 , 3.00 ) NO-ERROR.
       RUN set-size IN h_t-orddes-01 ( 13.12 , 143.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-orddes-01 ,
             h_v-orddes-01 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
           ...
           IF h_t-orddes-01 <> ? THEN RUN dispatch IN h_t-orddes-01 ('open-query':U). 
      END.
