...
DEFINE VARIABLE h_t-ped-utilex-unificado AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-ped-utilex-unificado AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-Canc AT ROW 2.62 COL 176 WIDGET-ID 12
     RECT-24 AT ROW 2.54 COL 2 WIDGET-ID 16
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 183.86 BY 25.38
         FONT 4.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "PEDIDOS CAJA RAPIDA - SUNAT"
         HEIGHT             = 25.38
         WIDTH              = 183.86
         MAX-HEIGHT         = 39.12
         MAX-WIDTH          = 274.29
...
ON END-ERROR OF W-Win /* PEDIDOS CAJA RAPIDA - SUNAT */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* PEDIDOS CAJA RAPIDA - SUNAT */
DO:
...
DO:
  ...
  RUN Cancelar-Pedido IN h_v-ped-utilex-unificado.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv08 ).
       RUN set-position IN h_p-updv08 ( 1.00 , 36.00 ) NO-ERROR.
       RUN set-size IN h_p-updv08 ( 1.54 , 61.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/v-ped-utilex-unificado.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ped-utilex-unificado ).
       RUN set-position IN h_v-ped-utilex-unificado ( 2.62 , 2.86 ) NO-ERROR.
       /* Size in UIB:  ( 7.73 , 148.00 ) */
       ...
       /* Links to SmartViewer h_v-ped-utilex-unificado. */
       RUN add-link IN adm-broker-hdl ( h_p-updv08 , 'TableIO':U , h_v-ped-utilex-unificado ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_v-ped-utilex-unificado ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv08 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ped-utilex-unificado ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/b-ped-utilex-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedmos-utilex ).
       RUN set-position IN h_b-pedmos-utilex ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedmos-utilex ( 15.88 , 182.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/t-ped-utilex-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-ped-utilex-unificado ).
       RUN set-position IN h_t-ped-utilex-unificado ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-ped-utilex-unificado ( 15.85 , 182.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 24.96 , 3.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.35 , 51.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-ped-utilex-unificado. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-ped-utilex-unificado ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-ped-utilex-unificado ,
             BUTTON-Canc:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-ped-utilex-unificado , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-ped-utilex-unificado <> ? THEN RUN dispatch IN h_t-ped-utilex-unificado ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_v-ped-utilex-unificado <> ? THEN RUN Recalcular-Precios IN h_v-ped-utilex-unificado.
      END.
...
RUN venta-delivery-cab IN h_v-ped-utilex-unificado(OUTPUT pListaPrecio).
