...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      T-DDocu.AlmDes COLUMN-LABEL "Almacén" FORMAT "x(3)":U
...
"Temp-Tables.T-DDocu.AlmDes" "Almacén" ? "character" ? ? ? ? ? ? yes "F8 solicita ayuda" no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    IF NOT AVAILABLE Vtaddocu THEN DO:
        MESSAGE "Producto no registrado en la cotización"
...
ON F8 OF T-DDocu.AlmDes IN BROWSE br_table /* Almacén */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL T-DDocu.AlmDes br_table _BROWSE-COLUMN B-table-Win
ON LEAVE OF T-DDocu.AlmDes IN BROWSE br_table /* Almacén */
DO:
  IF SELF:SCREEN-VALUE = "" THEN RETURN.
END.
...
IF NOT AVAILABLE VtaAlmDiv THEN DO:
    MESSAGE "Almacén no válido para descargar" VIEW-AS ALERT-BOX ERROR.
    APPLY "ENTRY":U TO T-DDocu.AlmDes IN BROWSE {&browse-name}.
    RETURN "ADM-ERROR".
END.
...
IF NOT AVAILABLE Almmmate THEN DO:
    MESSAGE 'Producto no asignado al almacén' T-DDOCU.AlmDes:SCREEN-VALUE IN BROWSE {&Browse-name}
        VIEW-AS ALERT-BOX ERROR.
    APPLY "ENTRY":U TO T-DDocu.AlmDes IN BROWSE {&browse-name}.
    RETURN "ADM-ERROR".
END.
