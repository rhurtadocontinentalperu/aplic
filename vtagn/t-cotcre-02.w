...
T-DDocu.ImpLin 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table T-DDocu.PorDto2 ~
T-DDocu.PorDto3 
...
&Scoped-Define ENABLED-OBJECTS BUTTON-DESCUENTO br_table 
...
DEFINE BUTTON BUTTON-DESCUENTO 
     LABEL "APLICAR DESCUENTO GENERAL" 
     SIZE 35 BY 1.12.
...
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 135 BY 9.42
         FONT 4
         TITLE "DESCENTO POR PORCENTAJE".


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     BUTTON-DESCUENTO AT ROW 1 COL 101 WIDGET-ID 16
...
/* BROWSE-TAB br_table BUTTON-DESCUENTO F-Main */
ASSIGN 
...
"T-DDocu.PreUni" ? ">,>>9.999999" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"T-DDocu.PorDto2" "%Dto2" "->>9.9999" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > Temp-Tables.T-DDocu.PorDto3
"T-DDocu.PorDto3" "%Dto3" "->>9.9999" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* DESCENTO POR PORCENTAJE */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* DESCENTO POR PORCENTAJE */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* DESCENTO POR PORCENTAJE */
DO:
...
DO:
    IF DECIMAL (T-DDocu.PreBas:SCREEN-VALUE IN BROWSE {&browse-name}) > DEC(T-DDOCU.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) THEN DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-DESCUENTO B-table-Win
ON CHOOSE OF BUTTON-DESCUENTO IN FRAME F-Main /* APLICAR DESCUENTO GENERAL */
DO:
  RUN vtagn/d-vtaddocu-02.
...
ON "RETURN":U OF T-DDocu.PorDto2, T-DDocu.PorDto1, T-DDocu.PorDto3
DO:
    APPLY "TAB":U.
    RETURN NO-APPLY.
END.
...
/* Productos repetidos */
    ...
    pPorDto = ( 1 - (1 - DEC (T-DDocu.PorDto1:SCREEN-VALUE IN BROWSE {&Browse-name}) / 100) * 
                (1 - DEC (T-DDocu.PorDto2:SCREEN-VALUE IN BROWSE {&Browse-name}) / 100) * 
                (1 - DEC (T-DDocu.PorDto3:SCREEN-VALUE IN BROWSE {&Browse-name}) / 100) ) * 100.
    ...
    IF pPorDto > pDtoMax THEN DO:
        MESSAGE "Máximo Descuento permitido" SKIP
            ...
            VIEW-AS ALERT-BOX WARNING.
/*         APPLY "ENTRY":U TO T-DDocu.PorDto1 IN BROWSE {&Browse-name}. */
/*         RETURN "ADM-ERROR".                                          */
    END.
