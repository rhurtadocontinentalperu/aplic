...
DEF NEW SHARED VARIABLE S-CMPBNTE  AS CHAR.
...
END CASE.
s-TpoPed = "CO".    /* COntado */
IF NUM-ENTRIES(pCodDoc) > 1 AND ENTRY(2, pCodDoc) > '' THEN s-TpoPed = ENTRY(2, pCodDoc).
...
DEFINE VARIABLE h_tpedidocontadomay AS HANDLE NO-UNDO.
...
DO:
    ...
    RUN Recalcular-Precios IN h_tpedidocontadomay.
    RUN dispatch IN h_tpedidocontadomay ('open-query').
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/v-ped-mostrador-gral.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/t-ped-mostrador-gral.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tpedidocontadomay ).
       RUN set-position IN h_tpedidocontadomay ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tpedidocontadomay ( 11.85 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_tpedidocontadomay. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tpedidocontadomay ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tpedidocontadomay ,
             h_vpedidocontadomay , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tpedidocontadomay , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         RUN dispatch IN h_tpedidocontadomay ('open-query':U).
      END.
    ...
    WHEN "Add-Record" THEN DO:
        RUN dispatch IN h_tpedidocontadomay ('add-record':U).
      END.
    WHEN "browse" THEN DO:
          IF h_b-pedido-cont <> ? THEN RUN dispatch IN h_b-pedido-cont ('open-query':U).
          IF h_tpedidocontadomay <> ? THEN RUN dispatch IN h_tpedidocontadomay ('open-query':U). 
      END.
    WHEN "browse2" THEN DO:
          RUN dispatch IN h_tpedidocontadomay ('open-query':U). 
      END.
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        RUN Recalcular-Precio IN h_vpedidocontadomay.
        IF h_tpedidocontadomay <> ? THEN RUN dispatch IN h_tpedidocontadomay ('open-query':U). 
      END.
END CASE.
