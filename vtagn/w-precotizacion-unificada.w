...
DEFINE VARIABLE h_t-precotizacion-unificada AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-precotizacion-unificada AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/v-precotizacion-unificada.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-precotizacion-unificada ).
       RUN set-position IN h_v-precotizacion-unificada ( 2.92 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-precotizacion-unificada. */
       RUN add-link IN adm-broker-hdl ( h_p-updv06 , 'TableIO':U , h_v-precotizacion-unificada ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido2x , 'Record':U , h_v-precotizacion-unificada ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_p-updv06 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-precotizacion-unificada ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido2x ,
             h_v-precotizacion-unificada , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
    ...
    WHEN 2 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/t-precotizacion-unificada.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-precotizacion-unificada ).
       RUN set-position IN h_t-precotizacion-unificada ( 10.69 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-precotizacion-unificada ( 15.38 , 132.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-precot-gral-cotmovil ,
             h_v-precotizacion-unificada , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-precotizacion-unificada ,
...
CASE pParam:
    WHEN "Asigna-Cotizacion" THEN DO:
        RUN Asigna-Cotizacion IN h_v-precotizacion-unificada.
    END.
    WHEN "Asigna-PrePedido" THEN DO:
        RUN Asigna-PrePedido IN h_v-precotizacion-unificada.
