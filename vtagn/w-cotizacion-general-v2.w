...
IF NUM-ENTRIES(pParametro) > 1 AND ENTRY(2, pParametro) > ''
...
&Scoped-Define ENABLED-OBJECTS BTN-Excel BUTTON-Excel-Trabajo COMBO-NroSer 
...
DEFINE VARIABLE h_f-imp-le-2016 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BTN-Excel AT ROW 1 COL 120 WIDGET-ID 16
     BUTTON-Excel-Trabajo AT ROW 1 COL 129 WIDGET-ID 58
     btnExcVend AT ROW 1 COL 145 WIDGET-ID 40
...
ON END-ERROR OF W-Win /* COTIZACIONES UNIFICADA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* COTIZACIONES UNIFICADA */
DO:
    ...
    *******/
    ...
    CASE TRUE :
        ...
        WHEN s-CodDiv = '00101' THEN 
            RUN Genera-Excel-Delivery IN h_v-cotizacion-general (INPUT YES).   /* Siempre con IGV */
...
END CASE.
CASE s-TpoPed:
    WHEN "LE" THEN W-Win:TITLE = W-Win:TITLE + "- LISTA EXPRESS".
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/v-cotizacion-general-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotizacion-general ).
       RUN set-position IN h_v-cotizacion-general ( 2.35 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.77 , 128.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             h_v-cotizacion-general , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_f-cot-gral-expo-pag1 ( 2.35 , 131.00 ) NO-ERROR.
       ...
       RUN set-position IN h_f-imp-excel-plantilla ( 4.50 , 131.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 25.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtagn/f-cot-gral-imp-le-2016.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_f-imp-le-2016 ).
       RUN set-position IN h_f-imp-le-2016 ( 5.58 , 131.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 25.00 ) */
       ...
       RUN set-position IN h_f-cot-gral-super ( 6.65 , 131.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.19 , 25.00 ) */
       ...
       RUN set-position IN h_f-cot-gral-expo-import ( 8.81 , 131.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 25.00 ) */
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_f-cot-gral-pag2 ( 2.35 , 131.00 ) NO-ERROR.
       ...
       RUN set-position IN h_f-cot-gral-imp-ped-excel ( 4.50 , 131.00 ) NO-ERROR.
       ...
       RUN set-position IN h_f-cot-gral-imp-excel-provi ( 5.58 , 131.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 25.00 ) */
       ...
       RUN set-position IN h_f-cot-gral-super-pag2 ( 6.65 , 131.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 25.00 ) */
       ...
       RUN set-position IN h_f-cot-gral-cotrecu ( 7.73 , 131.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.12 , 25.00 ) */
       ...
       RUN set-position IN h_f-cot-gral-sede-cliente-2 ( 8.81 , 131.00 ) NO-ERROR.
       ...
       RUN set-position IN h_f-cot-gral-matriz-de-ventas ( 12.04 , 139.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 19.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/t-cotizacion-general-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotizacion-general ).
       RUN set-position IN h_t-cotizacion-general ( 13.12 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotizacion-general ( 12.96 , 156.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BTN-Excel BUTTON-Excel-Trabajo COMBO-NroSer 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    ...
    WHEN "Importar-PET" THEN DO:
        RUN Importar-PET IN h_v-cotizacion-general.
    END.
    ...
    WHEN "Actualiza-datos-cliente" THEN DO:
        RUN Actualiza-datos-cliente IN h_v-cotizacion-general.
    END.
    OTHERWISE DO:
...
CASE L-Handle:
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
        ...
        /* BOTONERAS */
        RUN dispatch IN h_f-cot-gral-imp-ped-excel ('hide':U).
