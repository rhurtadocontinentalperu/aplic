...
/* Ic - 2025, Para generar las O/D, HPK y PHR, pedidos de RIQRA HORIZONTAL */
lProcesoBatch = NO.
cMsgProcesoBatch = "".
...
DEF OUTPUT PARAMETER TABLE FOR ITEM-1.
...
DO TRANSACTION:
    ...
    REPEAT iColumn = 9 TO 65000:
        ...
        /* 23/01/2025: NO duplicados */
        IF CAN-FIND(FIRST ITEM-1 WHERE ITEM-1.codmat = x-codmat NO-LOCK) THEN NEXT.
        /* ************************* */
...
DO TRANSACTION:
    FOR EACH ITEM-1 EXCLUSIVE-LOCK BY ITEM-1.NroItm:
        FIND FIRST ITEM WHERE ITEM.codmat = ITEM-1.codmat NO-LOCK NO-ERROR.
        IF AVAILABLE ITEM THEN DO:
            /* Solo nos quedamos con los nuevos */
            DELETE ITEM-1.
            NEXT.
        END.
        CREATE ITEM.
...
&IF DEFINED(EXCLUDE-COT_Importar-Excel-Old) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE COT_Importar-Excel-Old Procedure 
PROCEDURE COT_Importar-Excel-Old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO TRANSACTION:
    FOR EACH ITEM-1 NO-LOCK BY ITEM-1.NroItm:
        FIND FIRST ITEM WHERE ITEM.codmat = ITEM-1.codmat NO-LOCK NO-ERROR.
...
FOR EACH ResumenxLinea:
    ...
    x-CanPed = ResumenxLinea.CanPed * ResumenxLinea.Factor.
    /* Determinamos cuantos packs podemos armar */
    x-Oferta = (PriDMasxMenos.Llave_d1 + PriDMasxMenos.Llave_d2). /* Ej. 12 + 1 */
    x-Ciclos = TRUNCATE(x-CanPed / PriDMasxMenos.Llave_d1,0).
    /* Ajustamos a packs completos */
    IF x-Ciclos * x-Oferta > x-CanPed THEN x-Ciclos = x-Ciclos - 1.
    IF x-Ciclos <= 0 THEN DO: DELETE ResumenxLinea. NEXT. END.
    /* Precio unitario unidad base */
    x-PreUni = ResumenxLinea.PreUni / ResumenxLinea.Factor.
    /* Cantidad sobrante */
    x-Sobrante = x-CanPed - (x-Ciclos * x-Oferta).
    /* Importe Teórico */
    x-ImpTot = ( x-Sobrante + (x-Ciclos * PriDMasxMenos.Llave_d1)) * x-PreUni.
    /* Recalculamos el % de descuento */
    ResumenxLinea.Por_Dsctos[3] = ROUND( ( 1 - ( x-ImpTot / x-ImpLin ) ) * 100 , 4 ).
/*     MESSAGE ResumenxLinea.codmat ResumenxLinea.Por_Dsctos[3] SKIP */
/*         x-ImpTot x-ImpLin.                                        */
END.
...
&IF DEFINED(EXCLUDE-DCTO_MAS_POR_MENOS-OLD) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE DCTO_MAS_POR_MENOS-OLD Procedure 
PROCEDURE DCTO_MAS_POR_MENOS-OLD :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&IF DEFINED(EXCLUDE-PED_Despachar-Pedido-v2) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PED_Despachar-Pedido-v2 Procedure 
PROCEDURE PED_Despachar-Pedido-v2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    */
    ...
    CASE TRUE:
        WHEN Faccpedi.CrossDocking = NO THEN DO:
            RUN VTA_Genera-OD-v2 ( pRowid, OUTPUT pComprobante, OUTPUT pMensaje ).
            IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO RLOOP, RETURN 'ADM-ERROR'.
        END.
        WHEN Faccpedi.CrossDocking = YES THEN DO:
            RUN VTA_Genera-OTR ( pRowid, OUTPUT pMensaje).
    ...
    IF AVAILABLE PEDIDO THEN DO:
        ...
        CASE TRUE:
            WHEN PEDIDO.CrossDocking = NO THEN DO:
                RUN VTA_Genera-OD-v2 (ROWID(PEDIDO), OUTPUT pComprobante, OUTPUT pMensaje).
                IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO RLOOP, RETURN 'ADM-ERROR'.
            END.
            WHEN PEDIDO.CrossDocking = YES THEN DO:
                RUN VTA_Genera-OTR (ROWID(PEDIDO), OUTPUT pMensaje).
...
&IF DEFINED(EXCLUDE-VTA_Genera-OD-v2) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE VTA_Genera-OD-v2 Procedure 
PROCEDURE VTA_Genera-OD-v2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE :
    ...
    */
    ...
    ELSE DO:
        /* 03/02/2022 Generación de PHR */
        IF lProcesoBatch = NO THEN DO:      /* Para caso de Riqra la generacion de PHR/HPK se realizara en otro proceso */
            /* 10/04/2025 Felix Perez: Se van a genera la PHR y HPK en forma independiente */
            IF FacCPedi.FlgSit = "T" THEN DO:
                RUN VTA_Genera-PHR_y_HPK-v2 (INPUT ROWID(Faccpedi), OUTPUT pMensaje).     /* Dato: O/D */
                IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                    IF TRUE <> (pMensaje > '') THEN pMensaje = "ERROR: No se pudo generar la PHR ni la HPK".
                    UNDO LoopGrabarData, LEAVE.        
                END.
...
&IF DEFINED(EXCLUDE-VTA_Genera-PHR_y_HPK-v2) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE VTA_Genera-PHR_y_HPK-v2 Procedure 
PROCEDURE VTA_Genera-PHR_y_HPK-v2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       La PHR y la HPK se generar en forma independiente
------------------------------------------------------------------------------*/
...
FIND FIRST ORDENES WHERE ROWID(ORDENES) = pRowid NO-LOCK NO-ERROR.
IF NOT AVAILABLE ORDENES THEN DO:
    pMensaje = "NO se pudo ubicar el registro de la O/D u OTR".
    RETURN 'ADM-ERROR'.
END.
...
/* ****************************************** */
/* 1ro Se genera la HPK: Con la nueva versión */
/* ****************************************** */
DEFINE VAR hLibLogis AS HANDLE NO-UNDO.
RUN logis/p-genera-hpk-library.p PERSISTENT SET hLibLogis.

RUN HPK_Genera-HPK-Master-v2 IN hLibLogis (INPUT ROWID(ORDENES), OUTPUT pMensaje) NO-ERROR.    
IF ERROR-STATUS:ERROR THEN  pMensaje = pMensaje + CHR(10) + ERROR-STATUS:GET-MESSAGE(1).
IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.    
...
/* ******************************************************************************** */
/* 2do Generamos la PHR siempre y cuando se despache en un CD por Picking por rutas */
/* ******************************************************************************** */
DEFINE VAR xCodDoc AS CHAR.
DEFINE VAR xNroDoc AS CHAR.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* 05/06/2024: Actualizar el estado de la O/D */
    /* ****************************************** */
    FIND FIRST ORDENES WHERE ROWID(ORDENES) = pRowid EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
    IF ERROR-STATUS:ERROR = YES THEN DO:
        {lib/mensaje-de-error.i &CuentaError="pCuenta" &MensajeError="pMensaje"}
        UNDO, RETURN 'ADM-ERROR'.
    END.
    ...
    IF s-adm-new-record = YES THEN DO:
        ...
        ASSIGN
            ...
            DI-RutaC.FchDoc = TODAY
            DI-RutaC.NroDoc = STRING(FacCorre.NroSer, '999') + STRING(FacCorre.Correlativo, '999999')
            DI-RutaC.flgest  = "PK"     /* Con PRE Hoja de Ruta */
            DI-RutaC.Libre_c05 = "AUTOMATICO"   
            DI-RutaC.Observ = pObserv
            DI-RutaC.usuario = s-user-id
            NO-ERROR.
