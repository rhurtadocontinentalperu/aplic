...
&Scoped-define INTERNAL-TABLES Almmmatg Almmmate Almtfami VtaListaMay
...
F-PRESOLA @ VtaListaMay.PreOfi VtaListaMay.Chr__01 
...
&Scoped-define QUERY-STRING-br_table FOR EACH Almmmatg WHERE ~{&KEY-PHRASE} ~
 ...
 AND (RADIO-SET-Stock = 'T' OR Almmmate.StkAct > 0) NO-LOCK, ~
      ...
      FIRST VtaListaMay OF Almmmatg ~
      WHERE VtaListaMay.CodDiv = s-coddiv NO-LOCK ~
...
&Scoped-define FOURTH-TABLE-IN-QUERY-br_table VtaListaMay
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almmmatg.UndStk COLUMN-LABEL "Unid Base" FORMAT "X(6)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 119 BY 10.38
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Almmmatg,INTEGRAL.Almmmate OF INTEGRAL.Almmmatg,INTEGRAL.Almtfami OF INTEGRAL.Almmmatg,INTEGRAL.VtaListaMay OF INTEGRAL.Almmmatg"
 ...
 AND (RADIO-SET-Stock = 'T' OR Almmmate.StkAct > 0)"
     ...
     _Where[4]         = "INTEGRAL.VtaListaMay.CodDiv = s-coddiv"
...
"Almmmatg.UndStk" "Unid Base" "X(6)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"_StkAct() @ F-STKACT" "Stock Actual" "->>>>,>>9.99" ? 10 ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
