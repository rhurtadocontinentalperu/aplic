...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 1 COL 144
     COMBO-NroSer AT ROW 1.27 COL 44.71 WIDGET-ID 4
     B-MARGEN AT ROW 6.65 COL 153
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 2.35 , 143.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 5.65 , 9.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-cotizacion-cred-dcto-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-cotizacion-cred ( 8.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotizacion-cred ( 15.08 , 182.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/t-cotizacion-dcto-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotizacion-cred-dcto ).
       RUN set-position IN h_t-cotizacion-cred-dcto ( 8.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotizacion-cred-dcto ( 15.62 , 181.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv96 ).
       RUN set-position IN h_p-updv96 ( 22.00 , 3.00 ) NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE usuario-que-autoriza W-Win 
PROCEDURE usuario-que-autoriza :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE OUTPUT PARAMETER pUsuario AS CHAR.

RUN usuario-que-autoriza IN h_v-cotizacion-cred-dcto(OUTPUT pUsuario).
