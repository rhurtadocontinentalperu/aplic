...
DEF INPUT PARAMETER pStock AS CHAR NO-UNDO.
...
FOR EACH t-Almmmatg EXCLUSIVE-LOCK:
    ...
    IF (pStock = 'C' AND t-Almmmatg.StkMax <= 0) OR t-Almmmatg.StkMax < 0 THEN DO:
        DELETE t-Almmmatg.
        NEXT.
    END.
    /* En unidades A */
    FIND Almtconv WHERE Almtconv.CodUnid = t-Almmmatg.UndBas AND
        Almtconv.Codalter = t-Almmmatg.UndA NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almtconv THEN DO:
        DELETE t-Almmmatg.
        NEXT.
    END.
    ASSIGN
        t-Almmmatg.StkMax = t-Almmmatg.StkMax / Almtconv.Equival.
    /* Precio A B y C */
    ASSIGN
...
FOR EACH t-Almmmatg EXCLUSIVE-LOCK WHERE t-Almmmatg.Prevta[1] <= 0 OR (TRUE <> (t-Almmmatg.UndA > '')):
    DELETE t-Almmmatg.
END.
...
CASE TRUE:
    ...
    WHEN pMarca > '' THEN DO:
        ...
        END.
    END.
    WHEN CMB-filtro <> 'Todos' AND pFiltro > '' THEN DO:
        CASE CMB-filtro:
