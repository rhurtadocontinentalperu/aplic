...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Captura-No-Grabados V-table-Win 
PROCEDURE Captura-No-Grabados :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       En caso hubiera habido una caida del sistema se puede
                recuperar los digitado
------------------------------------------------------------------------------*/

IF NOT CAN-FIND(FIRST w-report WHERE w-report.Task-No = 0
                AND w-report.Llave-C = s-Llave-Control NO-LOCK)
    THEN RETURN.
/* Disparamos pantalla de control */
MESSAGE 'Se han detectado artículos que no han podidos ser grabados' SKIP
    'en una transacción anterior' SKIP
    'Se requiere una decisión del usuario'
    VIEW-AS ALERT-BOX INFORMATION.
RUN vtagn/d-no-grabados-flash (INPUT s-Llave-Control,
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      /* Capturamos no grabados */
      RUN Captura-No-Grabados.
...
------------------------------------------------------------------------------*/
  ...
  /* 20/05/2022: Actualiza precios en el detalle */
  RUN Procesa-Handle IN lh_handle ('Actualiza-Precios-Items').
...
/*      END.                                                                */
    ...
    /* VALIDACION DE LA CONDICION DE VENTA */    
     ...
     END.
     /* VALIDACION DE ITEMS */
     IF CAN-FIND(FIRST ITEM WHERE ITEM.ImpLin <= 0 NO-LOCK) THEN DO:
         MESSAGE 'Debe ACTUALIZAR PRECIOS (F5) antes de grabar' VIEW-AS ALERT-BOX INFORMATION.
         RETURN "ADM-ERROR".   
     END.
     FOR EACH ITEM NO-LOCK:
