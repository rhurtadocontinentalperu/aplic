...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    {vta2/icorrelativosecuencial.i &Codigo = s-coddoc &Serie = s-nroser}
        MESSAGE 'despues del correlativo'.
    ASSIGN 
...
chWorkSheet:Range(cRange):Value = "Direccion :" + gn-clie.dircli. 
cRange = "F" + cColumn.
chWorkSheet:Range(cRange):Value = "Teléfono : ".
/* Referencia */
t-Column = t-Column + 1.
cColumn = STRING(t-Column).
cRange = "A" + cColumn.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard beavior. */
  CASE s-TpoPed:
      WHEN "LF" THEN DO:
...
/*   RUN vtagn/ventas-library PERSISTENT SET hProc. */
  ...
  IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
  /* ****************************************************************************************** */
  /* RHC 10/02/2016 Cualquier variación en la Lista Express requiere aprobación */
  /* ****************************************************************************************** */
