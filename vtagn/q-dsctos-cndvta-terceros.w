...
&Scoped-define QUERY-STRING-Query-Main FOR EACH Dsctos WHERE Dsctos.CndVta = gn-ConVt.Codig ~
      AND Dsctos.clfCli = "T" NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Dsctos WHERE Dsctos.CndVta = gn-ConVt.Codig ~
      AND Dsctos.clfCli = "T" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main Dsctos
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main Dsctos
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.Dsctos WHERE INTEGRAL.gn-ConVt <external> ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[1]      = "Dsctos.CndVta = gn-ConVt.Codig"
     _Where[1]         = "Dsctos.clfCli = ""T"""
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE gn-convt THEN DO:
      IF NOT CAN-FIND(FIRST dsctos WHERE dsctos.cndvta = gn-convt.codig
                      AND dsctos.clfcli = 'T' NO-LOCK)
          THEN DO:
          CREATE dsctos.
          ASSIGN
