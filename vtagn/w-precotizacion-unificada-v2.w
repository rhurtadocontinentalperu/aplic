...
/* Sintaxis : x,ddddd,[YES|NO],[YES|NO],[RIQRA|CONTINENTAL]
    x: Tipo de Pedido
        N: Venta normal
        S: Canal Moderno
    ...
    [RIQRA|CONTINENTAL]: Origen de información
...
IF NUM-ENTRIES(pParametro) > 3 THEN DO:
    ...
    END.
END.
/* 05/08/2025: CONTROL DE ORIGEN DE PEDIDOS */
DEFINE NEW GLOBAL SHARED VAR s-OrigenPedido AS CHAR INIT "".
IF NUM-ENTRIES(pParametro) > 4 THEN DO:
    ASSIGN s-OrigenPedido = ENTRY(5, pParametro) NO-ERROR.
    IF ERROR-STATUS:ERROR THEN DO:
        MESSAGE 'Error en el parámetro 5: ORIGEN DE PEDIDO'
            VIEW-AS ALERT-BOX WARNING.
        RETURN ERROR.
    END.
END.
...
DEFINE VARIABLE h_f-precot-import-continental AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 2 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtagn/f-precot-import-continental.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_f-precot-import-continental ).
       RUN set-position IN h_f-precot-import-continental ( 3.69 , 124.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.12 , 22.00 ) */
       ...
       RUN set-position IN h_f-cot-gral-imp-ped-excel ( 5.04 , 124.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-precot-gral-cotmovil ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-precot-import-continental ,
             h_v-precotizacion-unificada-v2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-cot-gral-imp-ped-excel ,
             h_f-precot-import-continental , 'AFTER':U ).
...
CASE pParam:
    ...
    END.
    WHEN "Asigna-PrePedido-Continental" THEN DO:
        RUN Asigna-PrePedido IN h_v-precotizacion-unificada-v2.
        IF RETURN-VALUE = "ADM-ERROR" THEN RETURN 'ADM-ERROR'.
        RUN dispatch IN h_t-precotizacion-unificada-v2 ('open-query':U).
    END.
...
CASE L-Handle:
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         RUN dispatch IN h_f-precot-import-continental ('hide':U).
         CASE s-OrigenPedido:
             WHEN "RIQRA" THEN DO:
                 RUN dispatch IN h_f-precot-gral-cotmovil ('view':U).
             END.
             WHEN "CONTINENTAL" THEN DO:
                 RUN dispatch IN h_f-precot-import-continental ('view':U).
             END.
         END CASE.
      END.
