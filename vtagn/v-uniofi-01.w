...
F-totstk FILL-IN-flgcomercial F-comprometido F-dispo 
...
DEFINE FRAME F-Main
     ...
     F-totstk AT ROW 4.27 COL 14.43 COLON-ALIGNED
     FILL-IN-flgcomercial AT ROW 5.19 COL 26.29 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     F-comprometido AT ROW 5.23 COL 14.43 COLON-ALIGNED
     F-dispo AT ROW 6.15 COL 14.43 COLON-ALIGNED
     "Indicador comercial" VIEW-AS TEXT
          SIZE 16.14 BY .5 AT ROW 4.62 COL 28.57 WIDGET-ID 6
          FGCOLOR 4 FONT 6
...
ON F9 ANYWHERE
DO:
    IF s-TpoPed = "M" THEN RUN vtagn/d-dtovolmarco (trim(s-codmat) ).
        ELSE RUN Vta/D-Dtovol2.r(trim(s-codmat) ).
END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
        /*RUN vta2/stock-comprometido (s-codmat, x-CodAlm, OUTPUT x-canped).*/
      RUN gn/Stock-Comprometido-v2 (s-CodMat,
                                    x-CodAlm,
                                    YES,
                                    OUTPUT x-CanPed).
        /*****/
