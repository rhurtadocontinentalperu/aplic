...
DEFINE VARIABLE F-PorImp AS DEC DECIMALS 6 NO-UNDO.

DEFINE VARIABLE s-local-new-record AS CHAR.

DEF VAR s-ModPreUni AS LOG NO-UNDO.

FIND gn-divi WHERE gn-divi.codcia = s-codcia
    AND gn-divi.coddiv = s-coddiv NO-LOCK.
s-ModPreUni = GN-DIVI.ModPreUni.
...
Almmmatg.DesMat T-DDocu.UndVta T-DDocu.CanPed T-DDocu.PreBas T-DDocu.PorDto ~
T-DDocu.PreUni T-DDocu.PorDto1 T-DDocu.PorDto2 T-DDocu.PorDto3 ~
T-DDocu.ImpLin 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table T-DDocu.CodMat ~
T-DDocu.CanPed T-DDocu.PreUni 
...
FILL-IN_ImpVta FILL-IN_ImpDto FILL-IN_ImpIgv FILL-IN_ImpTot 
...
DEFINE FRAME F-Main
     ...
     FILL-IN_ImpVta AT ROW 10.69 COL 115 COLON-ALIGNED WIDGET-ID 14
     FILL-IN_ImpDto AT ROW 11.77 COL 19 COLON-ALIGNED WIDGET-ID 6
     FILL-IN_ImpIgv AT ROW 11.77 COL 59 COLON-ALIGNED WIDGET-ID 10
     FILL-IN_ImpTot AT ROW 11.77 COL 115 COLON-ALIGNED WIDGET-ID 12
...
"T-DDocu.CodMat" "Producto" "X(13)" "character" ? ? ? ? ? ? yes ? no no "8.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"T-DDocu.CanPed" ? ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"T-DDocu.PorDto" "% Dscto!Base" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.T-DDocu.PreUni
...
"T-DDocu.PorDto2" "%Dto2" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > Temp-Tables.T-DDocu.PorDto3
"T-DDocu.PorDto3" "%Dto3" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    IF s-local-new-record = "NO" AND SELF:SCREEN-VALUE = T-DDOCU.CodMat THEN RETURN.
    ...
    RUN vtagn/PrecioVenta (s-codcia,
                           ...
                           Almmmatg.codmat,
                           s-cndvta,
                           DECIMAL( T-DDocu.CanPed:SCREEN-VALUE IN BROWSE {&browse-name} ),
                           6,
                           OUTPUT f-PreBas,
                           OUTPUT f-PreVta,
                           OUTPUT f-Dsctos,
...
DO:
    ...
    IF Almmmatg.Dec__03 > 0 AND DECIMAL(T-DDOCU.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > 0 
    THEN DO:
        IF DECIMAL(T-DDOCU.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) < Almmmatg.Dec__03 
        THEN DO:
...
DO:
    ...
    DEF VAR f-PreUni AS DEC NO-UNDO.

    f-PreBas = DEC(T-DDocu.PreBas:SCREEN-VALUE IN BROWSE {&browse-name}) *
        ( 1 - DEC(T-DDOCU.PorDto:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) / 100 ).
    f-PreUni = DEC(T-DDOCU.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
    IF f-PreUni - f-PreBas < -0.001 THEN DO:
...
ON "RETURN":U OF T-DDocu.CodMat, T-DDocu.CanPed, T-DDOCU.PreUni
DO:
    APPLY "TAB":U.
    RETURN NO-APPLY.
END.
...
FOR EACH BT-DDOCU:
    ...
    FIND Almmmatg WHERE 
         ...
         Almmmatg.codmat = BT-DDOCU.codmat
         USE-INDEX matg01
         NO-LOCK NO-ERROR.
    ...
    IF ABSOLUTE(T-DDOCU.PreUni - f-PreVta) / f-PreVta * 100 > 0.25 THEN DO:
        CREATE T-DPEDI.
        BUFFER-COPY T-DDOCU TO T-DPEDI
            ASSIGN
...
------------------------------------------------------------------------------*/
  ...
  IF s-import-ibc = YES OR s-import-cia = YES THEN DO:
  ...
  END.
  IF s-cndvta = '' THEN DO:
      MESSAGE 'Ingrese la condición de venta' VIEW-AS ALERT-BOX WARNING.
      RETURN "ADM-ERROR".
  END.
...
------------------------------------------------------------------------------*/
  ...
  /* AJUSTAMOS EL DESCUENTO BASE SIEMPRE Y CUANDO NO HAY OTROS DESCUENTOS APLICADOS */
  T-DDOCU.PorDto = ( 1 - ( T-DDOCU.PreUni / T-DDOCU.PreBas ) ) * 100.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  IF s-ModPreUni = YES 
  THEN T-DDocu.PreUni:READ-ONLY IN BROWSE {&browse-name} = NO.
  ELSE T-DDocu.PreUni:READ-ONLY IN BROWSE {&browse-name} = YES.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Recalcula-Precios B-table-Win 
...
IF T-DDocu.CodMat:SCREEN-VALUE IN BROWSE {&browse-name} = "" THEN DO:
    MESSAGE 'Producto no debe estar en blanco' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO T-DDOCU.CodMat IN BROWSE {&browse-name}.
    RETURN "ADM-ERROR".
END.
/* Valida Maestro Productos */
FIND Almmmatg WHERE Almmmatg.CodCia = S-CODCIA 
...
ELSE DO:
    IF CAN-FIND(FIRST BT-DDOCU WHERE BT-DDOCU.codmat = T-DDocu.CodMat:SCREEN-VALUE IN BROWSE {&browse-name}
                AND ROWID(BT-DDOCU) <> ROWID(T-DDOCU) )
        THEN DO:
        MESSAGE 'Producto duplicado' VIEW-AS ALERT-BOX ERROR.
        APPLY 'ENTRY':U TO T-DDOCU.CodMat IN BROWSE {&browse-name}.
        RETURN "ADM-ERROR".
    END.
END.
IF DECIMAL(T-DDocu.CanPed:SCREEN-VALUE IN BROWSE {&Browse-name}) = 0 THEN DO:
...
END.
/* RHC 02.01.06 AHORA RESULTA QUE NO SE DEBE VENDER MENOS QUE Almmmatg.Dec__03 */
IF Almmmatg.Dec__03 > 0 AND DECIMAL(T-DDOCU.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > 0 THEN DO:
    IF DECIMAL(T-DDOCU.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) < Almmmatg.Dec__03 THEN DO:
        MESSAGE " La Cantidad debe ser como mínimo" STRING(Almmmatg.Dec__03, '>>>,>>9.99')
            VIEW-AS ALERT-BOX WARNING.
        APPLY 'ENTRY':U TO T-DDOCU.CanPed.
        RETURN NO-APPLY.
