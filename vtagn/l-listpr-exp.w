...
DEFINE VARIABLE f-StkDis AS DEC NO-UNDO.
...
Almmmatg.codmat Almmmatg.DesMat Almmmatg.UndStk _StkAct() @ F-STKACT ~
_Comprometido(Almmmatg.codmat, x-codalm) @ f-StockComprometido ~
_StkDis() @ f-StkDis F-PRESOLA @ F-PRESOLA Almmmatg.Chr__01 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD _StkDis L-table-Win 
FUNCTION _StkDis RETURNS DECIMAL
  ( /* parameter-definitions */ )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      _Comprometido(Almmmatg.codmat, x-codalm) @ f-StockComprometido COLUMN-LABEL "Stock!Comprometido" FORMAT "->>>>,>>9.99":U
            WIDTH 10.14
      _StkDis() @ f-StkDis COLUMN-LABEL "Stock!Disponible" FORMAT "->>>>,>>9.99":U
            COLUMN-FGCOLOR 12 COLUMN-BGCOLOR 10
      F-PRESOLA @ F-PRESOLA COLUMN-LABEL "Precio Vta!Soles" FORMAT ">>,>>>,>>9.9999":U
            WIDTH 9.14
      Almmmatg.Chr__01 COLUMN-LABEL "UM." FORMAT "X(8)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"_Comprometido(Almmmatg.codmat, x-codalm) @ f-StockComprometido" "Stock!Comprometido" "->>>>,>>9.99" ? ? ? ? ? ? ? no ? no no "10.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > "_<CALC>"
"_StkDis() @ f-StkDis" "Stock!Disponible" "->>>>,>>9.99" ? 10 12 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > "_<CALC>"
"F-PRESOLA @ F-PRESOLA" "Precio Vta!Soles" ">>,>>>,>>9.9999" ? ? ? ? ? ? ? no ? no no "9.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > INTEGRAL.Almmmatg.Chr__01
"Almmmatg.Chr__01" "UM." ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  IF Almmmatg.MonVta = 1 THEN DO:
    F-PRESOLA = Almmmatg.PreAlt[4].

    F-PREUSSA = Almmmatg.PreAlt[4] / Almmmatg.TpoCmb.
  END.
  ELSE DO:
    F-PREUSSA = Almmmatg.PreAlt[4].

    F-PRESOLA = Almmmatg.PreAlt[4] * Almmmatg.TpoCmb.
  END.
...
RUN vtagn/stock-comprometido-expo (pCodMat, pCodAlm, OUTPUT pComprometido).
f-StockComprometido = pComprometido.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION _StkDis L-table-Win 
FUNCTION _StkDis RETURNS DECIMAL
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
f-StkDis = f-StkAct - f-StockComprometido .
RETURN f-StkDis.   /* Function return value. */
