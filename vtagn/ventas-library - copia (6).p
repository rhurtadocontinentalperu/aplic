...
FOR EACH PriCDctoxImport NO-LOCK WHERE PriCDctoxImport.CodCia = s-codcia AND
    ...
    x-PorDto = 0.
    DETALLE:
    FOR EACH B-DPEDI OF B-CPEDI NO-LOCK WHERE B-DPEDI.Libre_c05 <> "OF", FIRST Almmmatg OF B-DPEDI NO-LOCK:
        x-hay-dsctos = NO.
        /* ***************************************************************************************** */
        /* Excepciones */
        /* ***************************************************************************************** */
...
/*                 IF x-PuntoControl = NO AND COTIZACION.CodDiv <> T-VtamDctoVolTime.CodDiv THEN NEXT RLOOP.   /* División Hija */ */
/*                 IF COTIZACION.CodDiv = T-VtamDctoVolTime.CodDiv THEN x-PuntoControl = YES.              /* División Padre */    */
...
/* *********************************** */
    ...
    DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
        FOR EACH T-DPEDI NO-LOCK WHERE T-DPEDI.Libre_c01 = "*", 
            FIRST B-DPEDI OF B-CPEDI WHERE B-DPEDI.codmat = T-DPEDI.codmat,
            FIRST Almmmatg OF B-DPEDI NO-LOCK
            ON ERROR UNDO, THROW:
            FIND Facdpedi WHERE ROWID(Facdpedi) = ROWID(B-DPEDI) EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
            IF ERROR-STATUS:ERROR = YES THEN DO:
