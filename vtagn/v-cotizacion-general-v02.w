...
DEFINE FRAME F-Main
     ...
     FacCPedi.Libre_c04 AT ROW 1.77 COL 76 NO-LABEL WIDGET-ID 144
...
"5", 5
     ...
     "Lista MARCO?" VIEW-AS TEXT
          SIZE 11 BY .5 AT ROW 1.96 COL 65 WIDGET-ID 148
          BGCOLOR 15 FGCOLOR 1 
     "Moneda:" VIEW-AS TEXT
          SIZE 6 BY .5 AT ROW 5.77 COL 94 WIDGET-ID 84
     "Stock de Letras" VIEW-AS TEXT
          SIZE 15.14 BY .69 AT ROW 9.65 COL 84.86 WIDGET-ID 150
          BGCOLOR 15 FGCOLOR 9 FONT 10
     "Comprobante:" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 5.04 COL 90 WIDGET-ID 106
     "Redondedo del P.U.:" VIEW-AS TEXT
          SIZE 15 BY .5 AT ROW 8.15 COL 85 WIDGET-ID 100
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1 SCROLLABLE 
         FONT 4 WIDGET-ID 100.
...
FOR EACH Facdpedi OF Faccpedi NO-LOCK WHERE Facdpedi.Libre_c05 <> "OF" :
    ...
    /* ************************* */
    IF Facdpedi.Libre_c04 = 'DVXDSF'    THEN ITEM.Por_Dsctos[3] = 0.
    IF Facdpedi.Libre_c04 = 'EDVXSALDOC' THEN ITEM.Por_Dsctos[3] = 0.
...
------------------------------------------------------------------------------*/
  ...
  END.
  ASSIGN
      FacCPedi.Hora   = STRING(TIME,"HH:MM:SS").    /* Ultima grabación */
  /* ************************************************************************ */
  /* *************************** POR APROBAR ******************************** */
  /* ************************************************************************ */
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE FacCPedi THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      IF AVAILABLE ccbstklet THEN txtStkLetras:SCREEN-VALUE = STRING(ccbstklet.qstklet,"->,>>99").
      /* Postal */
      FILL-IN-Postal:SCREEN-VALUE = ''.
      FIND almtabla WHERE almtabla.tabla = 'CP' AND 
          almtabla.codigo = FacCPedi.CodPos NO-LOCK NO-ERROR.
      IF AVAILABLE almtabla THEN FILL-IN-Postal:SCREEN-VALUE = almtabla.nombre.
  END.
