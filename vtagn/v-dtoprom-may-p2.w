...
DEFINE FRAME F-Main
     ...
     VtaListaMay.PromFchH AT ROW 2.35 COL 26 COLON-ALIGNED NO-LABEL WIDGET-ID 6
          VIEW-AS FILL-IN 
          SIZE 12.86 BY .81
     VtaListaMay.PromDto AT ROW 2.35 COL 43 COLON-ALIGNED NO-LABEL WIDGET-ID 2
          VIEW-AS FILL-IN 
          SIZE 8.29 BY .81
...
DO:
    IF DECIMAL(SELF:SCREEN-VALUE) = 0 THEN DO:
        ...
        RETURN.
    END.
    PromDto:SCREEN-VALUE = STRING ( ROUND ( ( 1 -  DECIMAL(SELF:SCREEN-VALUE) / VtaListaMay.PreOfi  ) * 100, 4 ) ).
END.
...
DO:
    F-PRECIO = ROUND(VtaListaMay.PreOfi * ( 1 - ( DECI(SELF:SCREEN-VALUE) / 100 ) ),4).
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE VtaListaMay THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      F-PRECIO = ROUND(VtaListaMay.PreOfi * ( 1 - ( VtaListaMay.PromDto / 100 ) ),4).
