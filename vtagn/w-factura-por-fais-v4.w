...
&Scoped-define INTERNAL-TABLES T-DESTINO CcbCDocu PEDIDO COTIZACION GUIA ~
T-ORIGEN
...
COTIZACION.CustomerPurchaseOrder GUIA.NroDoc CcbCDocu.ImpTot 
...
&Scoped-define QUERY-STRING-BROWSE-DESTINO FOR EACH T-DESTINO NO-LOCK, ~
  ...
  AND COTIZACION.NroPed = PEDIDO.NroRef NO-LOCK, ~
      FIRST GUIA WHERE GUIA.CodCia = CcbCDocu.CodCia ~
  AND GUIA.CodDoc = "G/R" ~
  AND GUIA.CodRef = CcbCDocu.CodDoc ~
  AND GUIA.NroRef = CcbCDocu.NroDoc NO-LOCK ~
...
&Scoped-define FIFTH-TABLE-IN-QUERY-BROWSE-DESTINO GUIA
...
&Scoped-define FIFTH-TABLE-IN-QUERY-BROWSE-ORIGEN GUIA
...
BUTTON-Exportar Btn_OK BtnDone BUTTON-2 RECT-2 RECT-3 BROWSE-DESTINO 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-DESTINO W-Win _STRUCTURED
  QUERY BROWSE-DESTINO NO-LOCK DISPLAY
      ...
      GUIA.NroDoc COLUMN-LABEL "G/R" FORMAT "X(12)":U
...
DEFINE FRAME F-Main
     ...
     FILL-IN-FchSal-1 AT ROW 4.23 COL 41 COLON-ALIGNED WIDGET-ID 42
     ...
     BUTTON-Filtrar AT ROW 3.96 COL 81 WIDGET-ID 38
     BUTTON-Exportar AT ROW 5.04 COL 81 WIDGET-ID 48
...
/* Query rebuild information for BROWSE BROWSE-DESTINO
     _TblList          = "Temp-Tables.T-DESTINO,INTEGRAL.CcbCDocu OF Temp-Tables.T-DESTINO,PEDIDO WHERE INTEGRAL.CcbCDocu ...,COTIZACION WHERE PEDIDO ...,GUIA WHERE INTEGRAL.CcbCDocu ..."
  ...
  AND COTIZACION.NroPed = PEDIDO.NroRef"
     _JoinCode[5]      = "GUIA.CodCia = INTEGRAL.CcbCDocu.CodCia
  AND GUIA.CodDoc = ""G/R""
  AND GUIA.CodRef = INTEGRAL.CcbCDocu.CodDoc
  AND GUIA.NroRef = INTEGRAL.CcbCDocu.NroDoc"
...
"COTIZACION.CustomerPurchaseOrder" ? "x(15)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.GUIA.NroDoc
"GUIA.NroDoc" "G/R" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/* Query rebuild information for BROWSE BROWSE-ORIGEN
     _TblList          = "Temp-Tables.T-ORIGEN,INTEGRAL.CcbCDocu OF Temp-Tables.T-ORIGEN,PEDIDO WHERE INTEGRAL.CcbCDocu ...,COTIZACION WHERE PEDIDO ...,GUIA WHERE INTEGRAL.CcbCDocu ..."
     _Options          = "NO-LOCK"
     _TblOptList       = ", FIRST, FIRST, FIRST, FIRST"
     _OrdList          = "INTEGRAL.COTIZACION.DeliveryGroup|yes,INTEGRAL.COTIZACION.CustomerPurchaseOrder|yes"
     _JoinCode[3]      = "PEDIDO.CodCia = INTEGRAL.CcbCDocu.CodCia
...
FOR EACH DI-RutaC NO-LOCK WHERE DI-RutaC.CodCia = s-CodCia AND 
    ...
    FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-CodCia AND
        ...
        /* Ic - 05Oct2020 - Que NO sean de Grupo de Raparto */
        RUN Grupo-reparto IN hProc (INPUT B-CDOCU.libre_c01, INPUT B-CDOCU.libre_c02, 
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BROWSE-ORIGEN BUTTON-1 COMBO-BOX-Clientes FILL-IN-FchSal-1 
         ...
         BtnDone BUTTON-2 RECT-2 RECT-3 BROWSE-DESTINO 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "GUIA"}
