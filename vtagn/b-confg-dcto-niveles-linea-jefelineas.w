...
Almtfami.Libre_c05 VtaTabla.Valor[1] VtaTabla.Valor[2] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c2 COLUMN-LABEL "Línea" FORMAT "x(8)":U WIDTH 5.14
      Almtfami.desfam FORMAT "X(30)":U WIDTH 32.57
      Almtfami.Libre_c05 COLUMN-LABEL "Propia!Terceros" FORMAT "x(3)":U
            WIDTH 6.29
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 73.72 BY 9.62
         FONT 4
         TITLE "EXCEPCIONES A LA REGLA GENERAL A LINEA DE PRODUCTO".
...
"VtaTabla.Llave_c2" "Línea" ? "character" ? ? ? ? ? ? yes ? no no "5.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almtfami.desfam
"Almtfami.desfam" ? ? "character" ? ? ? ? ? ? no ? no no "32.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.Almtfami.Libre_c05
"Almtfami.Libre_c05" "Propia!Terceros" "x(3)" "character" ? ? ? ? ? ? no ? no no "6.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* EXCEPCIONES A LA REGLA GENERAL A LINEA DE PRODUCTO */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* EXCEPCIONES A LA REGLA GENERAL A LINEA DE PRODUCTO */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* EXCEPCIONES A LA REGLA GENERAL A LINEA DE PRODUCTO */
DO:
...
ON 'return':U OF vtatabla.llave_c2, vtatabla.valor[1]
DO:
    APPLY 'TAB'.
    RETURN NO-APPLY.
END.
...
ON FIND OF vtatabla DO:
    ...
    If lookup(x-user,'ADMIN,MASTER') = 0 Then do:
        FIND FIRST x-vtatabla WHERE x-vtatabla.codcia = s-codcia AND
                                x-vtatabla.tabla = 'LP' AND
                                x-vtatabla.llave_c1 = x-user AND
                                x-vtatabla.llave_c2 = vtatabla.llave_c2 NO-LOCK NO-ERROR.
...
FIND FIRST Almtfami WHERE Almtfami.CodCia = s-codcia AND
                ...
                NO-LOCK NO-ERROR. /*OR
    LOOKUP(Almtfami.codfam, '010,011,012,013') > 0*/ 
IF NOT AVAILABLE almtfami THEN DO:
    MESSAGE 'Línea NO esta registrada' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO VtaTabla.Llave_c2.
    RETURN 'ADM-ERROR'.
END.
RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
IF RETURN-VALUE = 'YES' THEN DO:
    IF CAN-FIND(FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia AND
                ...
                NO-LOCK)
        THEN DO:
        MESSAGE 'Código repetido' VIEW-AS ALERT-BOX INFORMATION.
        APPLY 'ENTRY':U TO VtaTabla.Llave_c2.
        RETURN 'ADM-ERROR'.
    END.
END.
...
If lookup(x-user,'ADMIN,MASTER') = 0 Then do:
    ...
    IF NOT AVAILABLE x-vtatabla THEN DO:
        MESSAGE 'Linea no pertenece al jefe de linea' VIEW-AS ALERT-BOX INFORMATION.
        APPLY 'ENTRY':U TO VtaTabla.Llave_c2.
        RETURN 'ADM-ERROR'.        
    END.
end.
...
IF NOT (x-valor >= 0 AND x-valor <= 100) THEN DO:
    MESSAGE '%Descuento TERCEROS debe estar entre 0% y 100%' VIEW-AS ALERT-BOX ERROR.
...
IF NOT (x-valor >= 0 AND x-valor <= 100) THEN DO:
    MESSAGE '%Descuento PROPIOS debe estar entre 0% y 100%' VIEW-AS ALERT-BOX ERROR.
...
/**/
DEFINE VAR x-prop-terc AS CHAR.

x-prop-terc = almtfami.libre_c05.

IF LOOKUP(x-prop-terc,"P,T") = 0 THEN DO:
    /*
    MESSAGE 'La linea no esta definida como Propios o Terceros' VIEW-AS ALERT-BOX INFORMATION.
    RETURN 'ADM-ERROR'.
    */
END.

/*
IF x-prop-terc = 'T' THEN DO:
    IF x-valor > DECIMAL(almtabla.nomant) THEN DO:
        MESSAGE '% Max de Dscto no debe ser superior al de la politica General' VIEW-AS ALERT-BOX INFORMATION.
        RETURN 'ADM-ERROR'.
    END.
END.
ELSE DO:
    IF x-valor > DECIMAL(almtabla.codcta1) THEN DO:
        MESSAGE '% Max de Dscto no debe ser superior al de la politica General' VIEW-AS ALERT-BOX INFORMATION.
        RETURN 'ADM-ERROR'.
    END.
END.
