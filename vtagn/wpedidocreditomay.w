...
IF NOT AVAILABLE FacCorre THEN DO:
   MESSAGE "Codigo de Documento no configurado" VIEW-AS ALERT-BOX INFORMATION.
   RETURN ERROR.
END.
...
&Scoped-Define ENABLED-OBJECTS BUTTON-Despachar COMBO-NroSer Btn-Excel ~
btn-margen BTN-CCTE B-AGTRANS BUTTON-REDONDEO 
...
DEFINE VARIABLE h_t-pedido-credito AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vpedidocreditomay AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     btn-margen AT ROW 7.54 COL 127.29 WIDGET-ID 2
     BTN-CCTE AT ROW 8.5 COL 127.29 WIDGET-ID 22
     B-AGTRANS AT ROW 9.46 COL 127.29 WIDGET-ID 24
     BUTTON-REDONDEO AT ROW 10.42 COL 127.29 WIDGET-ID 26
     "Buscar el número:" VIEW-AS TEXT
          SIZE 13 BY .5 AT ROW 1.54 COL 92 WIDGET-ID 8
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 143.43 BY 25.35
         FONT 4.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "VENTAS MAYORISTAS - PEDIDOS AL CREDITO"
         HEIGHT             = 25.35
         WIDTH              = 143.43
         MAX-HEIGHT         = 31.35
         MAX-WIDTH          = 167.29
...
ON END-ERROR OF W-Win /* VENTAS MAYORISTAS - PEDIDOS AL CREDITO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* VENTAS MAYORISTAS - PEDIDOS AL CREDITO */
DO:
...
DO:
   RUN Asigna-datos IN h_vpedidocreditomay.
END.
...
DO:
  ...
  RUN Excel IN h_vpedidocreditomay.
...
DO:
  RUN Margen IN h_vpedidocreditomay.
END.
...
DO:
  RUN Despachar-Pedido IN h_vpedidocreditomay.
END.
...
DO:
  ...
  RETURN NO-APPLY.
  RUN Redondeo IN h_vpedidocreditomay.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/vpedidocreditomay.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vpedidocreditomay ).
       RUN set-position IN h_vpedidocreditomay ( 2.35 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.96 , 126.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 2.35 , 126.57 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_vpedidocreditomay. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_vpedidocreditomay ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_vpedidocreditomay ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_vpedidocreditomay ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_vpedidocreditomay , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-pedido-cred ( 12.31 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-pedido-credito.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedido-credito ).
       RUN set-position IN h_t-pedido-credito ( 12.58 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedido-credito ( 12.12 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-pedido-credito. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_t-pedido-credito ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedido-credito ,
             BUTTON-REDONDEO:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv95 ,
             h_t-pedido-credito , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-pedido-credito.w':U ,
       ...
       RUN set-position IN h_t-pedido-cred-2 ( 12.85 , 2.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-Despachar COMBO-NroSer Btn-Excel btn-margen BTN-CCTE B-AGTRANS 
         BUTTON-REDONDEO 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
   ...
   WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
       ...
       IF h_t-pedido-credito <> ? THEN RUN dispatch IN h_t-pedido-credito ('open-query':U).
 ...
 WHEN "Disable-Head" THEN DO:
     ...
     RUN dispatch IN h_vpedidocreditomay ('disable-fields').
   END.
 WHEN "Enable-Head" THEN DO:
     RUN dispatch IN h_p-updv04 ('enable':U).
     RUN dispatch IN h_vpedidocreditomay ('enable-fields').
   END.
    WHEN "browse" THEN DO:
          IF h_b-pedido-cred <> ? THEN RUN dispatch IN h_b-pedido-cred ('open-query':U).
          IF h_t-pedido-credito <> ? THEN RUN dispatch IN h_t-pedido-credito ('open-query':U).
