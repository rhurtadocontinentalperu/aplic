...
DEF NEW SHARED VAR s-NroRef AS CHAR.
DEF NEW SHARED VAR s-DiasVtoPed AS INT.
...
DEF NEW SHARED VAR s-Genera-Orden AS LOG NO-UNDO.
/* Códigos:
   PM:  Pedido Mostrador
...
*/
CASE s-CodDoc:
    WHEN "PPV" THEN s-Genera-Orden = YES.
    OTHERWISE s-Genera-Orden = NO.
END CASE.
...
CASE s-TpoPed:
    ...
    END.
    OTHERWISE DO:
        /* TODOS los Almacenes (no Remates) */
        FOR EACH VtaAlmDiv NO-LOCK WHERE Vtaalmdiv.codcia = s-codcia
            AND Vtaalmdiv.coddiv = s-coddiv,
            FIRST Almacen OF Vtaalmdiv NO-LOCK WHERE Almacen.Campo-C[3] <> 'Si'
            BY VtaAlmDiv.Orden:
...
ASSIGN
    s-import-ibc = NO
    s-import-cissac = NO
    pCodDiv = s-CodDiv.     /* Valor por defecto*/
...
DEFINE VARIABLE h_b-ped-market-unificado AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-pedido AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-ped-market-unificado AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-ped-market-unificado AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BUTTON-Genera-Orden 
     IMAGE-UP FILE "img/alm.ico":U
     LABEL "Button1" 
     SIZE 15 BY 1.92 TOOLTIP "Generar Orden de Despacho al ALMACEN".

DEFINE BUTTON BUTTON-Verifica-Orden 
     IMAGE-UP FILE "img/ayuda.ico":U
     LABEL "Button 2" 
     SIZE 15 BY 1.73 TOOLTIP "Verifica el Estado de la Orden de Despacho".
...
DEFINE FRAME F-Main
     COMBO-NroSer AT ROW 1.54 COL 2.71 WIDGET-ID 14
     BUTTON-Genera-Orden AT ROW 2.62 COL 167 WIDGET-ID 10
     BUTTON-Verifica-Orden AT ROW 4.73 COL 167 WIDGET-ID 12
     RECT-1 AT ROW 2.62 COL 2 WIDGET-ID 16
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 183.14 BY 27.81
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* SETTINGS FOR BUTTON BUTTON-Genera-Orden IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       BUTTON-Genera-Orden:HIDDEN IN FRAME F-Main           = TRUE.

/* SETTINGS FOR BUTTON BUTTON-Verifica-Orden IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       BUTTON-Verifica-Orden:HIDDEN IN FRAME F-Main           = TRUE.
...
&Scoped-define SELF-NAME BUTTON-Genera-Orden
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-Genera-Orden W-Win
ON CHOOSE OF BUTTON-Genera-Orden IN FRAME F-Main /* Button1 */
DO:
  RUN Genera-Orden-Despacho IN h_v-ped-market-unificado.
END.
...
&Scoped-define SELF-NAME BUTTON-Verifica-Orden
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-Verifica-Orden W-Win
ON CHOOSE OF BUTTON-Verifica-Orden IN FRAME F-Main /* Button 2 */
DO:
  RUN Verifica-Pedido IN h_v-ped-market-unificado.
END.
...
CASE s-CodDoc:
    ...
    WHEN "PPV" THEN W-Win:TITLE = "PRE-PEDIDOS MAYORISTAS".
END CASE.
CASE s-TpoPed:
    WHEN "CO" THEN W-Win:TITLE = W-Win:TITLE + " - MOSTRADOR".
END CASE.

W-Win:TITLE = ">>> PEDIDO MOSTRADOR RETAIL".
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 1.00 , 99.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv01 ).
       RUN set-position IN h_p-updv01 ( 1.27 , 25.00 ) NO-ERROR.
       RUN set-size IN h_p-updv01 ( 1.42 , 73.57 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtagn/v-ped-market-unificado-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ped-market-unificado ).
       RUN set-position IN h_v-ped-market-unificado ( 2.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.42 , 128.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-pedido ).
       RUN set-position IN h_q-pedido ( 1.27 , 16.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 9.00 ) */

       /* Links to SmartViewer h_v-ped-market-unificado. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_v-ped-market-unificado ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-ped-market-unificado ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv01 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ped-market-unificado ,
             BUTTON-Genera-Orden:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             BUTTON-Verifica-Orden:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ped-market-unificado ).
       RUN set-position IN h_b-ped-market-unificado ( 13.65 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-ped-market-unificado ( 15.08 , 182.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-ped-market-unificado. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_b-ped-market-unificado ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-ped-market-unificado ,
             BUTTON-Verifica-Orden:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtagn/t-ped-market-sunat-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-ped-market-unificado ).
       RUN set-position IN h_t-ped-market-unificado ( 13.65 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-ped-market-unificado ( 15.08 , 181.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 27.12 , 3.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.42 , 34.14 ) NO-ERROR.

       /* Links to SmartBrowser h_t-ped-market-unificado. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-ped-market-unificado ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-ped-market-unificado ,
             BUTTON-Verifica-Orden:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-ped-market-unificado , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         ASSIGN
             ...
             BUTTON-Genera-Orden:SENSITIVE = s-Genera-Orden
             BUTTON-Genera-Orden:VISIBLE   = s-Genera-Orden
             BUTTON-Verifica-Orden:SENSITIVE = NOT s-Genera-Orden
             BUTTON-Verifica-Orden:VISIBLE = NOT s-Genera-Orden.
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         ASSIGN
             ...
             BUTTON-Genera-Orden:SENSITIVE = NO
             BUTTON-Verifica-Orden:SENSITIVE = NO.
         RUN dispatch IN h_q-pedido ('disable':U).
         RUN dispatch IN h_t-ped-market-unificado ('open-query':U).
      END.
    ...
    WHEN "Add-Record" THEN DO:
        RUN dispatch IN h_t-ped-market-unificado ('add-record':U).
      END.
    WHEN "browse" THEN DO:
          IF h_b-ped-market-unificado <> ? THEN RUN dispatch IN h_b-ped-market-unificado ('open-query':U).
          IF h_t-ped-market-unificado <> ? THEN RUN dispatch IN h_t-ped-market-unificado ('open-query':U).
      END.
    WHEN "browse2" THEN DO:
          RUN dispatch IN h_t-ped-market-unificado ('open-query':U).
      END.
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN Recalcular-Precios IN h_v-ped-market-unificado.
         IF h_t-ped-market-unificado <> ? THEN RUN dispatch IN h_t-ped-market-unificado ('open-query':U).
      END.
END CASE.
