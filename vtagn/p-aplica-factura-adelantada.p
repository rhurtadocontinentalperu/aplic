...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    FIND FIRST Ccbcdocu WHERE ROWID(Ccbcdocu) = pRowid EXCLUSIVE-LOCK NO-ERROR.
    ...
    /* *************************** */
    /* *************************** */
    /* 05/08/2022 Aritmética Sunat */
    /* *************************** */
    &IF {&ARITMETICA-SUNAT} &THEN
        x-Saldo-Actual = CcbCDocu.TotalVenta.
    &ELSE
    ...
    FOR EACH  F-CDOCU USE-INDEX Llave06 WHERE F-CDOCU.codcia = CcbCDocu.codcia
        ...
        AND F-CDOCU.CodMon = CcbCDocu.CodMon:   /* RHC 16/12/2021, SLM solo a la misma moneda */
      IF CcbCDocu.codmon = F-CDOCU.codmon
      THEN ASSIGN x-Monto-Aplicar  = MINIMUM(x-Saldo-Actual, F-CDOCU.SdoAct).
      ELSE IF CcbCDocu.codmon = 1
          THEN ASSIGN x-Monto-Aplicar  = ROUND(MINIMUM(F-CDOCU.SdoAct * x-TpoCmb-Compra, x-Saldo-Actual),2).
          ELSE ASSIGN x-Monto-Aplicar  = ROUND(MINIMUM(F-CDOCU.SdoAct / x-TpoCmb-Venta , x-Saldo-Actual),2).
    ...
    /* RHC 09/12/2020 Cerramos la Factura */
    IF Ccbcdocu.SdoAct <= 0 THEN ASSIGN Ccbcdocu.FlgEst = "C" CcbCDocu.FchCan = TODAY.
END.
RELEASE F-CDOCU.
RELEASE Ccbdcaja.
...
------------------------------------------------------------------------------*/
    ...
    /* Tipo de Documento */
    FIND FIRST FacDoc WHERE FacDoc.CodCia = s-CodCia AND FacDoc.CodDoc = para_CodDoc
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE FacDoc THEN DO:
        MESSAGE para_CodDoc 'NO CONFIGURADO' VIEW-AS ALERT-BOX ERROR.
        RETURN "ADM-ERROR".
    END.
