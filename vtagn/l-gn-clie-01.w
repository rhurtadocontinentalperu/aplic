...
AND (TRUE <> (input-var-1 > "") OR  gn-clie.canal = input-var-1) ~
    )

&SCOPED-DEFINE CODIGO gn-clie.Codcli

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( gn-clie.NomCli BEGINS TRIM(FILL-IN-filtro) )
...
DO:
    ...
    CASE TRUE:
        WHEN CMB-filtro = "Nombres que contengan" THEN DO:
            /* Una sola palabra */
            DEF VAR cTexto AS CHAR NO-UNDO.
            cTexto = REPLACE(TRIM(FILL-IN-filtro),"*","").
            IF SUBSTRING(cTexto,LENGTH(cTexto),1) = "z" THEN cTexto = SUBSTRING(cTexto,1,LENGTH(cTexto) - 1).
            cTexto = cTexto + "*".
            hQueryHandle:QUERY-CLOSE().
            x-sql = 'FOR EACH gn-clie NO-LOCK WHERE '.
            x-sql = x-sql + "gn-clie.nomcli CONTAINS '" + cTexto + "'".
            x-sql = x-sql + " AND gn-clie.codcia = " + STRING(cl-codcia).
            x-sql = x-sql + " AND gn-clie.flgsit = 'A'".
            x-sql = x-sql + " AND ( TRUE <> ('" + input-var-1 + "'" + " > '')" +
                            " OR gn-clie.canal = '" + input-var-1 + "' )".
            hQueryHandle:QUERY-PREPARE(x-sql).
            hQueryHandle:QUERY-OPEN().
            /*
            /* Armamos el query */
            IF NUM-ENTRIES(FILL-IN-filtro, " ") = 1 THEN DO:
                /* Una sola palabra */
                hQueryHandle:QUERY-CLOSE().
                x-sql = 'FOR EACH gn-clie NO-LOCK WHERE '.
                x-sql = x-sql + "gn-clie.nomcli CONTAINS '" + FILL-IN-filtro + "'".
                x-sql = x-sql + " AND gn-clie.codcia = " + STRING(cl-codcia).
                x-sql = x-sql + " AND gn-clie.flgsit = 'A'".
                x-sql = x-sql + " AND ( TRUE <> ('" + input-var-1 + "'" + " > '')" + 
                                " OR gn-clie.canal = '" + input-var-1 + "' )".
            ...
            ELSE DO:
                ...
                x-sql = 'FOR EACH gn-clie NO-LOCK WHERE '.
                x-sql = x-sql + "( gn-clie.nomcli CONTAINS '" + FILL-IN-filtro + "'" +
                                " OR INDEX(gn-clie.nomcli, '" + FILL-IN-filtro + "') > 0" + 
                                " OR gn-clie.nomcli BEGINS '" + FILL-IN-filtro + "' )".
                x-sql = x-sql + " AND gn-clie.codcia = " + STRING(cl-codcia).
                x-sql = x-sql + " AND gn-clie.flgsit = 'A'".
                x-sql = x-sql + " AND ( TRUE <> ('" + input-var-1 + "'" + " > '')" + 
                                " OR gn-clie.canal = '" + input-var-1 + "' )".
