...
/* Definimos un llave única por usuario */
DEF NEW SHARED VAR s-Llave-Control AS CHAR.
...
s-Llave-Control = s-user-id.

DEF VAR lClientComputerName AS CHAR.
DEF VAR lClientName AS CHAR.
DEF VAR lComputerName AS CHAR.
DEF VAR lxClientName AS CHAR.
...
IF lxClientName > '' THEN s-Llave-Control = s-user-id + "-" + lxClientName.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/v-cotizacion-unificada-flash.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
    ...
    WHEN 2 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtagn/t-cotizacion-unificada-flash.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
...
CASE L-Handle:
    ...
    END.
    WHEN "Actualiza-Precios-Items" THEN DO:
        RUN Actualiza-Precios-Items IN h_t-cotizacion-unificada.
    END.
/*     OTHERWISE DO:                                                                */
/*         MESSAGE 'ERROR, Rutina NO definida:' L-Handle VIEW-AS ALERT-BOX WARNING. */
/*     END.                                                                         */
