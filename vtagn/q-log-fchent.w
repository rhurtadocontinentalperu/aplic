...
&Scoped-define INTERNAL-TABLES Log-Edi-Trans

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for QUERY Query-Main                                     */
&Scoped-define QUERY-STRING-Query-Main FOR EACH Log-Edi-Trans WHERE ~{&KEY-PHRASE} ~
      AND Log-Edi-Trans.Codcia = s-codcia ~
 AND (Log-Edi-Trans.nOrdenCompra BEGINS 'PED' ~
OR Log-Edi-Trans.nOrdenCompra BEGINS 'OTR' ~
OR Log-Edi-Trans.nOrdenCompra BEGINS 'O/D' ~
OR Log-Edi-Trans.nOrdenCompra BEGINS 'O/M') NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Log-Edi-Trans WHERE ~{&KEY-PHRASE} ~
      AND Log-Edi-Trans.Codcia = s-codcia ~
 AND (Log-Edi-Trans.nOrdenCompra BEGINS 'PED' ~
OR Log-Edi-Trans.nOrdenCompra BEGINS 'OTR' ~
OR Log-Edi-Trans.nOrdenCompra BEGINS 'O/D' ~
OR Log-Edi-Trans.nOrdenCompra BEGINS 'O/M') NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main Log-Edi-Trans
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main Log-Edi-Trans
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.Log-Edi-Trans"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "Log-Edi-Trans.Codcia = s-codcia
 AND (Log-Edi-Trans.nOrdenCompra BEGINS 'PED'
OR Log-Edi-Trans.nOrdenCompra BEGINS 'OTR'
OR Log-Edi-Trans.nOrdenCompra BEGINS 'O/D'
OR Log-Edi-Trans.nOrdenCompra BEGINS 'O/M')"
...
FIND FIRST Log-Edi-Trans WHERE Log-Edi-Trans.Codcia = s-CodCia
    AND Log-Edi-Trans.nOrdenCompra = pCodDoc + ',' + pNroPed
     NO-LOCK NO-ERROR.
IF AVAILABLE Log-Edi-Trans THEN output-var-1 = ROWID(Log-Edi-Trans).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
