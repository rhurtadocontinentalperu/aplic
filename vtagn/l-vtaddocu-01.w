...
&SCOPED-DEFINE CONDICION ( vtaddocu.codcia = s-codcia  AND ~
vtaddocu.coddiv = s-coddiv AND ~
vtaddocu.codped = input-var-1 AND ~
vtaddocu.nroped = input-var-2 AND ~
(vtaddocu.canped - vtaddocu.canate) > 0 )

&SCOPED-DEFINE CODIGO Vtacdocu.NroPed
...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaDDocu WHERE ~{&KEY-PHRASE} ~
      AND {&condicion} NO-LOCK, ~
      EACH Almmmatg OF VtaDDocu NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH VtaDDocu WHERE ~{&KEY-PHRASE} ~
      AND {&condicion} NO-LOCK, ~
      EACH Almmmatg OF VtaDDocu NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
...
<SORTBY-OPTIONS>
Codigo|y||INTEGRAL.VtaDDocu.CodMat|yes
Descripcion|||INTEGRAL.Almmmatg.DesMat|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fCanPed L-table-Win 
FUNCTION fCanPed RETURNS DECIMAL
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaDDocu.CodMat COLUMN-LABEL "Producto" FORMAT "X(6)":U WIDTH 8.57
      Almmmatg.DesMat FORMAT "X(60)":U
      fCanPed() @ x-CanPed COLUMN-LABEL "Por Atender" FORMAT "(ZZZ,ZZZ,ZZ9.99)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 74.14 BY 13.19
...
"VtaDDocu.CodMat" "Producto" ? "character" ? ? ? ? ? ? no ? no no "8.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almmmatg.DesMat
"Almmmatg.DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > "_<CALC>"
"fCanPed() @ x-CanPed" "Por Atender" "(ZZZ,ZZZ,ZZ9.99)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Codigo':U THEN DO:
           &Scope SORTBY-PHRASE BY VtaDDocu.CodMat
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fCanPed L-table-Win 
FUNCTION fCanPed RETURNS DECIMAL
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  RETURN ( Vtaddocu.CanPed - Vtaddocu.CanAte ).
