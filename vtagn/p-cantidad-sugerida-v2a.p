...
/* VERSION QUE SE USA EN PRE-COTIZACION */
/* DOS PARAMETROS MAS */
...
DEF INPUT PARAMETER pNroPag AS INT.
DEF INPUT PARAMETER pNroSec AS INT.
            ...
            CASE TRUE:
                WHEN GN-DIVI.CanalVenta = "FER" AND GN-DIVI.VentaMayorista = 2 THEN DO:
                    ...
                    FIND AlmCatVtaD WHERE AlmCatVtaD.CodCia = s-CodCia AND
                        AlmCatVtaD.CodDiv = pCodDiv AND     /*pListaPrecios AND */
                        AlmCatVtaD.CodPro = pCodPro AND
                        AlmCatVtaD.NroPag = pNroPag AND
                        AlmCatVtaD.NroSec = pNroSec AND
                        ...
                        NO-LOCK NO-ERROR.
                    IF AVAILABLE AlmCatVtaD AND AlmCatVtaD.Libre_d02 > 0 THEN f-MinimoVentas = AlmCatVtaD.Libre_d02.
                    IF f-MinimoVentas > 0 THEN DO:
...
/* Aproximamos al 80% del empaque Master */
/* RHC 27/12/2019 Correo de Cesar Camus */

DEF VAR x-factor AS DEC INIT 80 NO-UNDO.
...
DO:
    ...
    /* ********************************************************************** */
    pSugerido = 0.
    IF Almmmatg.Libre_d03 > 0 THEN DO:
        pEmpaque = Almmmatg.Libre_d03.      /* Empaque Master Expolibreria */
    END.
    IF pEmpaque <= 0 AND Almmmatg.CanEmp > 0 OR Almmmatg.Libre_d03 > 0 THEN DO:
        pEmpaque = Almmmatg.CanEmp.         /* Empaque Master Empresa */
    END.
    IF AVAILABLE AlmCatVtaD AND AlmCatVtaD.Libre_d03 > 0 THEN DO:
        pEmpaque = AlmCatVtaD.Libre_d03.    /* Empaque Master Expolibreria */
    END.
    IF pEmpaque > 0 THEN DO:
        /* Aproximamos al 90% del empaque Master */
        ...
        END.
        ELSE DO:
            IF (pCanPed / pEmpaque * 100) >= x-factor THEN DO:
                f-CanFinal = pEmpaque.
                pSugerido = pSugerido + f-CanFinal.
    ...
    /* *************************************************************************************************** */
    IF AVAILABLE AlmCatVtaD AND AlmCatVtaD.Libre_d02 > 0 
        THEN f-CanFinal = TRUNCATE(pCanPed / AlmCatVtaD.Libre_d02, 0) * AlmCatVtaD.Libre_d02.
    ELSE IF Almmmatg.StkMax > 0 THEN f-CanFinal = TRUNCATE(pCanPed / Almmmatg.StkMax, 0) * Almmmatg.StkMax.
