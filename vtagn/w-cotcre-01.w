...
DEFINE NEW SHARED TEMP-TABLE T-DPEDI NO-UNDO LIKE VtaDDocu.
...
/* CORRELATIVO */
FIND FIRST Vtacordiv WHERE Vtacordiv.codcia = s-codcia
...
&Scoped-Define ENABLED-OBJECTS RECT-2 RECT-3 C-NroSer 
...
DEFINE VARIABLE h_q-vtacdocu-01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-cotcre-01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotcre-01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-totcotcre-01 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BUTTON-Empresas 
     LABEL "IMPORTAR O/COMPRA ENTRE EMPRESAS" 
     SIZE 43 BY 1.12.
...
DEFINE FRAME F-Main
     C-NroSer AT ROW 1.27 COL 2 NO-LABEL WIDGET-ID 2
     BUTTON-Supermercados AT ROW 12.85 COL 39 WIDGET-ID 8
     BUTTON-Empresas AT ROW 12.85 COL 72 WIDGET-ID 10
     RECT-2 AT ROW 2.35 COL 2 WIDGET-ID 4
     RECT-3 AT ROW 12.58 COL 2 WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 141.57 BY 25.46 WIDGET-ID 100.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: T-DPEDI T "NEW SHARED" NO-UNDO INTEGRAL VtaDDocu
   END-TABLES.
 */
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* SETTINGS FOR BUTTON BUTTON-Empresas IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       BUTTON-Empresas:HIDDEN IN FRAME F-Main           = TRUE.
...
&Scoped-define SELF-NAME BUTTON-Empresas
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-Empresas W-Win
ON CHOOSE OF BUTTON-Empresas IN FRAME F-Main /* IMPORTAR O/COMPRA ENTRE EMPRESAS */
DO:
  RUN Importar-entre-companias IN h_v-cotcre-01.
END.
...
DO:
  RUN Supermercados IN h_v-cotcre-01.
END.
...
DO:
  ...
  RUN dispatch IN h_q-vtacdocu-01 ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 1.00 , 9.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv01 ).
       RUN set-position IN h_p-updv01 ( 1.00 , 27.00 ) NO-ERROR.
       RUN set-size IN h_p-updv01 ( 1.35 , 73.57 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtagn/v-cotcre-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotcre-01 ).
       RUN set-position IN h_v-cotcre-01 ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.62 , 130.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtagn/q-vtacdocu-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'SortBy-Case = NroPed':U ,
             OUTPUT h_q-vtacdocu-01 ).
       RUN set-position IN h_q-vtacdocu-01 ( 1.00 , 124.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.50 , 7.72 ) */

       /* Links to SmartViewer h_v-cotcre-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_v-cotcre-01 ).
       RUN add-link IN adm-broker-hdl ( h_q-vtacdocu-01 , 'Record':U , h_v-cotcre-01 ).

       /* Links to SmartQuery h_q-vtacdocu-01. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-vtacdocu-01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv01 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotcre-01 ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-cotcre-01 ( 12.85 , 3.00 ) NO-ERROR.
       ...
       RUN set-position IN h_v-totcotcre-01 ( 23.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.08 , 132.57 ) */

       /* Links to SmartBrowser h_b-cotcre-01. */
       RUN add-link IN adm-broker-hdl ( h_q-vtacdocu-01 , 'Record':U , h_b-cotcre-01 ).

       /* Links to SmartViewer h_v-totcotcre-01. */
       RUN add-link IN adm-broker-hdl ( h_q-vtacdocu-01 , 'Record':U , h_v-totcotcre-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotcre-01 ,
             h_v-cotcre-01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-totcotcre-01 ,
             BUTTON-Empresas:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 12.85 , 3.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.35 , 34.14 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtagn/t-cotcre-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotcre-01 ).
       RUN set-position IN h_t-cotcre-01 ( 14.19 , 3.00 ) NO-ERROR.
       RUN set-size IN h_t-cotcre-01 ( 11.77 , 135.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-cotcre-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotcre-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_v-cotcre-01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotcre-01 ,
             BUTTON-Empresas:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN 'Recalcula' THEN DO:
        RUN Recalcula-Precios IN h_t-cotcre-01.
    END.
    WHEN 'IBC' THEN DO:
        RUN IBC-Diferencias IN h_t-cotcre-01.
    END.
    WHEN "browse" THEN DO:
           IF h_b-cotcre-01 <> ? THEN RUN dispatch IN h_b-cotcre-01 ('open-query':U). 
           IF h_t-cotcre-01 <> ? THEN RUN dispatch IN h_t-cotcre-01 ('open-query':U). 
      END.
