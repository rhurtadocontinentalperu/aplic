    ...
    DO J = 1 TO 10:
        IF VtaListaMinGn.PromDivi[J] = S-CODDIV 
            ...
            IF VtaListaMinGn.Monvta = 1 THEN 
              ASSIGN X-PREVTA1 = F-PREVTA
                     X-PREVTA2 = ROUND(F-PREVTA / VtaListaMinGn.TpoCmb,6).
            ELSE
              ASSIGN X-PREVTA2 = F-PREVTA
                     X-PREVTA1 = ROUND(F-PREVTA * VtaListaMinGn.TpoCmb,6).
            X-PREVTA1 = X-PREVTA1.
            X-PREVTA2 = X-PREVTA2.
        END.
    END.
...
END.    
/************************************************/
IF s-FlgSit = "C" AND (TODAY >= 10/01/2012 AND TODAY <= 12/31/2012) 
    AND LOOKUP(s-CodMat, '035387,040158,040153,037923,023931,035709,035711,035712,042740,044720') > 0
    THEN DO:
    /* DESCUENTOS ESPECIALES POR CUPONES  
        SE TOMA AL MAYOR */
    IF Y-DSCTOS < 5 THEN Y-DSCTOS = 5.       /* 5% */
END.
/* *********************************** */
RUN BIN/_ROUND1(F-PREVTA,X-NRODEC,OUTPUT F-PREVTA).
/* *********************************** */
