        ...
        IF NOT CAN-FIND(FIRST PriDDctoxImport WHERE PriDDctoxImport.CodCia = PriCDctoxImport.codcia AND
            ...
            FOR EACH PriDDctoxImport NO-LOCK WHERE PriDDctoxImport.CodCia = PriCDctoxImport.codcia AND
                ...
                IF PriDDctoxImport.Tipo = "LINEA" THEN DO:
                    IF Almmmatg.CodFam <> PriDDctoxImport.Llave_c1 THEN NEXT DETALLE.
                    IF PriDDctoxImport.Llave_c2 > '' AND Almmmatg.SubFam <> PriDDctoxImport.Llave_c2 
                        THEN NEXT DETALLE.
                END.
                IF PriDDctoxImport.Tipo = "PROVEEDOR" AND Almmmatg.CodPr1 <> PriDDctoxImport.Llave_c1 
                    THEN NEXT DETALLE.
                IF PriDDctoxImport.Tipo = "MARCA" AND Almmmatg.CodMar <> PriDDctoxImport.Llave_c1 
                    THEN NEXT DETALLE.
            END.
        END.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    FOR EACH ResumenxLinea NO-LOCK,
        ...
        BUFFER-COPY Facdpedi TO logdsctosped
            ...
            logdsctosped.PorDto = Facdpedi.Por_Dsctos[1]
...
FOR EACH PriCDctoxImport NO-LOCK WHERE PriCDctoxImport.CodCia = s-codcia AND
    ...
    DO x-Item = 1 TO 5:
        ...
        END.
    END.
    IF x-PorDto = 0 THEN NEXT.      /* OJO: se pasa a la siguiente promoción activa */
    LEAVE.      /* OJO: Solo el primero que cumpla */
END.
IF x-PorDto = 0 THEN RETURN.    /* OJO: NO hay descuento por monto acumulado */
