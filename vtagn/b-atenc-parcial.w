...
DEF VAR s-Tabla AS CHAR INIT 'ATENCION_PARCIAL' NO-UNDO.
...
&SCOPED-DEFINE Condicion ( VtaCTabla.CodCia = s-CodCia AND ~
...
&Scoped-define INTERNAL-TABLES VtaCTabla FacCPedi GN-DIVI
...
FacCPedi.CodDoc FacCPedi.NroPed FacCPedi.FchPed FacCPedi.CodCli ~
FacCPedi.NomCli 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH VtaCTabla WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK, ~
      FIRST FacCPedi WHERE FacCPedi.CodCia = VtaCTabla.CodCia ~
      AND FacCPedi.CodDiv = ENTRY(1,Vtactabla.Llave) ~
 AND FacCPedi.CodDoc = ENTRY(2,Vtactabla.Llave) ~
 AND FacCPedi.NroPed = ENTRY(3,Vtactabla.Llave) NO-LOCK, ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH VtaCTabla WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacCPedi.NroPed = ENTRY(3,Vtactabla.Llave) NO-LOCK, ~
      FIRST GN-DIVI OF FacCPedi NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table VtaCTabla FacCPedi GN-DIVI
...
DEFINE QUERY br_table FOR 
      ...
      FacCPedi, 
      GN-DIVI
    FIELDS(GN-DIVI.CodDiv
      GN-DIVI.DesDiv) SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.FchPed COLUMN-LABEL "Fch.Documento" FORMAT "99/99/9999":U
      FacCPedi.CodCli COLUMN-LABEL "Cliente" FORMAT "x(11)":U WIDTH 11.43
      FacCPedi.NomCli FORMAT "x(60)":U WIDTH 47.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 139 BY 22.62
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaCTabla,INTEGRAL.FacCPedi WHERE INTEGRAL.VtaCTabla ...,INTEGRAL.GN-DIVI OF INTEGRAL.FacCPedi"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST, FIRST USED"
     _Where[1]         = "{&Condicion}"
     _JoinCode[2]      = "FacCPedi.CodCia = VtaCTabla.CodCia"
     _Where[2]         = "FacCPedi.CodDiv = ENTRY(1,Vtactabla.Llave)
 AND FacCPedi.CodDoc = ENTRY(2,Vtactabla.Llave)
 AND FacCPedi.NroPed = ENTRY(3,Vtactabla.Llave)"
...
"FacCPedi.FchPed" "Fch.Documento" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.NomCli" ? "x(60)" "character" ? ? ? ? ? ? no ? no no "47.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/* ***************************  Main Block  *************************** */
/* ON FIND OF Vtactabla DO:                     */
/*     ASSIGN                                   */
/*         x-CodDiv = ENTRY(1,Vtactabla.Llave)  */
/*         x-CodDoc = ENTRY(2,Vtactabla.Llave)  */
/*         x-NroDoc = ENTRY(3,Vtactabla.Llave). */
/*                                              */
/* END.                                         */
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    FIND CURRENT Vtactabla EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
    IF ERROR-STATUS:ERROR = YES THEN DO:
        {lib/mensaje-de-error.i &MensajeError="pMensaje" &CuentaError="pCuenta"}
        UNDO, LEAVE.
    END.
    ASSIGN
        VtaCTabla.FchAnulacion = NOW
        VtaCTabla.UsrAnulacion = s-user-id
        VtaCTabla.Estado = "A".
END.
IF pMensaje > '' THEN MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.
ELSE RUN dispatch IN THIS-PROCEDURE ('open-query':U).
...
DEF OUTPUT PARAMETER pRowidReturn AS ROWID.

DEF VAR pMensaje AS CHAR NO-UNDO.
DEF VAR pCuenta AS INT NO-UNDO.

pRowidReturn = ?.
IF pRowid = ? THEN RETURN 'OK'.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    FIND Vtactabla WHERE VtaCTabla.CodCia = s-CodCia AND
        VtaCTabla.Tabla = s-Tabla AND
        VtaCTabla.Llave = Faccpedi.coddiv + ',' + Faccpedi.coddoc + ',' + Faccpedi.nroped
        NO-LOCK NO-ERROR.
    ...
    ELSE DO:
        ...
        END.
        ASSIGN
            VtaCTabla.FchModificacion = NOW
            VtaCTabla.UsrModificacion = s-user-id
            VtaCTabla.Estado = "P".
    END.
    pRowidReturn = ROWID(VtaCTabla).
END.
IF pMensaje > '' THEN MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.
