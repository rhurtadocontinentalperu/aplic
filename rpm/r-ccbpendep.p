...
DISABLE TRIGGERS FOR LOAD OF ccbpendep.
...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND FIRST ccbpendep WHERE ccbpendep.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) AND
         ccbpendep.coddoc = SUBSTRING(pkeyvalue,4,3) AND
         ccbpendep.coddiv = SUBSTRING(pkeyvalue,7,5) AND
         ccbpendep.codref = SUBSTRING(pkeyvalue,12,3) AND
         ccbpendep.nroref = SUBSTRING(pkeyvalue,15)
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE ccbpendep THEN CREATE ccbpendep.
            RAW-TRANSFER pDATARECORD TO ccbpendep.
            L-FLG = YES.
        END.
    END CASE.
