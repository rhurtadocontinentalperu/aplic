...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND ccbdmov WHERE ccbdmov.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) AND
         ccbdmov.coddoc = SUBSTRING(pkeyvalue,4,3) AND
         ccbdmov.nrodoc = SUBSTRING(pkeyvalue,7,15) AND
         ccbdmov.codref = SUBSTRING(pkeyvalue,22,3) AND
         ccbdmov.nroref = SUBSTRING(pkeyvalue,25)
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE ccbdmov THEN CREATE ccbdmov.
            RAW-TRANSFER pDATARECORD TO ccbdmov.
            L-FLG = YES.
        END.
    END CASE.
