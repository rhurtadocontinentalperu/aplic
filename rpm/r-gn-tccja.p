...
DISABLE TRIGGERS FOR LOAD OF gn-tccja.
...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND gn-tccja WHERE gn-tccja.coddiv = SUBSTRING(pkeyvalue,1,5) AND
         gn-tccja.fecha = DATE (SUBSTRING(pkeyvalue,6))
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE gn-tccja THEN CREATE gn-tccja.
            RAW-TRANSFER pDATARECORD TO gn-tccja.
            L-FLG = YES.
        END.
    END CASE.
    ...
    END CATCH.
END.
IF AVAILABLE(gn-tccja) THEN RELEASE gn-tccja.
