...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND ccbddocu WHERE ccbddocu.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) AND
         ccbddocu.Coddiv = SUBSTRING(pkeyvalue,4,5) AND
         ccbddocu.coddoc = SUBSTRING(pkeyvalue,9,3) AND
         ccbddocu.nrodoc = SUBSTRING(pkeyvalue,12,15) AND
         ccbddocu.codmat = SUBSTRING(pkeyvalue,27,6)
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE ccbddocu THEN CREATE ccbddocu.
            RAW-TRANSFER pDATARECORD TO ccbddocu.
            L-FLG = YES.
        END.
    END CASE.
