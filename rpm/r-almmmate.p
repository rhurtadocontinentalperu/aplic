...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND almmmate WHERE almmmate.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) AND
        almmmate.codalm = SUBSTRING(pkeyvalue,4,3) AND
        almmmate.codmat = SUBSTRING(pkeyvalue,7) 
    ...
    CASE pEvent:
        ...
        WHEN 'WRITE' THEN DO:
            ...
            RAW-TRANSFER pDATARECORD TO almmmate.
            L-FLG = YES.
        END.
    END CASE.
