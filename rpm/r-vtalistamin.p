...
DISABLE TRIGGERS FOR LOAD OF vtalistamin.
...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND vtalistamin WHERE vtalistamin.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) 
        AND vtalistamin.coddiv = SUBSTRING(pkeyvalue,4,5) 
        AND vtalistamin.codmat = SUBSTRING(pkeyvalue,9)
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE vtalistamin THEN CREATE vtalistamin.
            RAW-TRANSFER pDATARECORD TO vtalistamin.
            L-FLG = YES.
        END.
    END CASE.
