...
DISABLE TRIGGERS FOR LOAD OF vtactickets.
...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND vtactickets WHERE vtactickets.codcia = INTEGER (SUBSTRING(pkeyvalue,1,3)) 
        AND vtactickets.codpro = SUBSTRING(pkeyvalue,4,11)
        AND vtactickets.producto = SUBSTRING(pkeyvalue,15)
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE vtactickets THEN CREATE vtactickets.
            RAW-TRANSFER pDATARECORD TO vtactickets.
            L-FLG = YES.
        END.
    END CASE.
    ...
    END CATCH.
END.
IF AVAILABLE(vtactickets) THEN RELEASE vtactickets.
