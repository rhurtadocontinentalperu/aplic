...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND ccbdecl WHERE ccbdecl.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) AND
        ccbdecl.usuario = SUBSTRING(pkeyvalue,4,15) AND
        ccbdecl.fchcie = DATE(SUBSTRING(pkeyvalue,19,8)) AND
        ccbdecl.horcie = SUBSTRING(pkeyvalue,27)
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE ccbdecl THEN CREATE ccbdecl.
            RAW-TRANSFER pDATARECORD TO ccbdecl.
            L-FLG = YES.
        END.
    END CASE.
