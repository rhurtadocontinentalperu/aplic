...
DISABLE TRIGGERS FOR LOAD OF vtadprom.
...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND vtadprom WHERE vtadprom.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) 
        AND vtadprom.codmat = SUBSTRING(Pkeyvalue,4) 
        EXCLUSIVE-LOCK NO-ERROR.
    CASE pEvent:
        WHEN 'DELETE' THEN DO:
            IF AVAILABLE vtadprom THEN DO:
                DELETE vtadprom.
            END.
            L-FLG = YES.    
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE vtadprom THEN CREATE vtadprom.
            RAW-TRANSFER pDATARECORD TO vtadprom.
            L-FLG = YES.
        END.
    END CASE.
    ...
    END CATCH.
END.
IF AVAILABLE(vtadprom) THEN RELEASE vtadprom.
