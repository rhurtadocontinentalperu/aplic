...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND FIRST vtatabla WHERE vtatabla.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) 
        AND vtatabla.tabla = TRIM(SUBSTRING(pkeyvalue,4,20))
        AND vtatabla.llave_c1 = TRIM(SUBSTRING(pkeyvalue,24,30))
        AND vtatabla.llave_c2 = TRIM(SUBSTRING(pkeyvalue,54,20))
        AND vtatabla.llave_c3 = TRIM(SUBSTRING(pkeyvalue,74))
    ...
    CASE pEvent:
        ...
        WHEN 'WRITE' THEN DO:
            ...
            RAW-TRANSFER pDATARECORD TO vtatabla.
            L-FLG = YES.
        END.
    END CASE.
