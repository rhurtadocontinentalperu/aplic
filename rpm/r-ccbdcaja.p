...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND ccbdcaja WHERE ccbdcaja.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) AND
         ccbdcaja.coddoc = SUBSTRING(pkeyvalue,4,3) AND
         ccbdcaja.nrodoc = SUBSTRING(pkeyvalue,7,15) AND
         ccbdcaja.codref = SUBSTRING(pkeyvalue,22,3) AND
         ccbdcaja.nroref = SUBSTRING(pkeyvalue,25)
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE ccbdcaja THEN CREATE ccbdcaja.
            RAW-TRANSFER pDATARECORD TO ccbdcaja.
            L-FLG = YES.
        END.
    END CASE.
