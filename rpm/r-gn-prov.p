...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND gn-prov WHERE gn-prov.codcia = INTEGER (SUBSTRING(pkeyvalue,1,3)) AND
        gn-prov.codpro = SUBSTRING(pkeyvalue,4)
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE gn-prov THEN CREATE gn-prov.
            RAW-TRANSFER pDATARECORD TO gn-prov.
            L-FLG = YES.
        END.
    END CASE.
    ...
    END CATCH.
END.
IF AVAILABLE(gn-prov) THEN RELEASE gn-prov.
