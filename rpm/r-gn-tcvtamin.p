...
DISABLE TRIGGERS FOR LOAD OF gn-tcvtamin.
...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND gn-tcvtamin WHERE gn-tcvtamin.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3))
        AND gn-tcvtamin.fechaini = DATE (SUBSTRING(pkeyvalue,4))
        EXCLUSIVE-LOCK NO-ERROR.
    CASE pEvent:
        WHEN 'DELETE' THEN DO:
            IF AVAILABLE gn-tcvtamin THEN DO:
                DELETE gn-tcvtamin.
            END.
            L-FLG = YES.    
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE gn-tcvtamin THEN CREATE gn-tcvtamin.
            RAW-TRANSFER pDATARECORD TO gn-tcvtamin.
            L-FLG = YES.
        END.
    END CASE.
    ...
    END CATCH.
END.
IF AVAILABLE(gn-tcvtamin) THEN RELEASE gn-tcvtamin.
