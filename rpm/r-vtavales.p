...
DISABLE TRIGGERS FOR LOAD OF vtavales.
...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND vtavales WHERE vtavales.codcia = INTEGER (SUBSTRING(pkeyvalue,1,3)) 
        AND vtavales.nrodoc = SUBSTRING(pkeyvalue,4)
        EXCLUSIVE-LOCK NO-ERROR.
    CASE pEvent:
        WHEN 'DELETE' THEN DO:
            IF AVAILABLE vtavales THEN DO:
                DELETE vtavales.
            END.
            L-FLG = YES.    
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE vtavales THEN CREATE vtavales.
            RAW-TRANSFER pDATARECORD TO vtavales.
            L-FLG = YES.
        END.
    END CASE.
    ...
    END CATCH.
END.
IF AVAILABLE(vtavales) THEN RELEASE vtavales.
