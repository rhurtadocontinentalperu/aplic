...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND vtacprom WHERE vtacprom.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) AND
         vtacprom.Coddiv = SUBSTRING(pkeyvalue,4,5) AND
         vtacprom.coddoc = SUBSTRING(pkeyvalue,9,3) AND
         vtacprom.nrodoc = INTEGER(SUBSTRING(pkeyvalue,12))
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE almmmatg THEN CREATE almmmatg.
            RAW-TRANSFER pDATARECORD TO almmmatg.
            L-FLG = YES.
        END.
    END CASE.
