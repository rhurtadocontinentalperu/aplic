...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND faccorre WHERE faccorre.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) AND
         faccorre.coddoc = SUBSTRING(pkeyvalue,4,3) AND
         faccorre.nroser = INTEGER (SUBSTRING(pkeyvalue,7))
    ...
    CASE pEvent:
        ...
        WHEN 'WRITE' THEN DO:
            ...
            RAW-TRANSFER pDATARECORD TO faccorre.
            L-FLG = YES.
        END.
    END CASE.
