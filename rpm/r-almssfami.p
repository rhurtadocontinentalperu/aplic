...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND almssfami WHERE almssfami.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) 
        AND almssfami.codfam = SUBSTRING(pkeyvalue,4,3) 
        AND almssfami.subfam = SUBSTRING(pkeyvalue,7,3)
        AND almssfami.codssfam = SUBSTRING(pkeyvalue,10,3)
        EXCLUSIVE-LOCK NO-ERROR.
    CASE pEvent:
        WHEN 'DELETE' THEN DO:
            IF AVAILABLE almssfami THEN DO:
                DELETE almssfami.
            END.
            L-FLG = YES.    
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE almssfami THEN CREATE almssfami.
            RAW-TRANSFER pDATARECORD TO almssfami.
            L-FLG = YES.
        END.
    END CASE.
    ...
    END CATCH.
END.
IF AVAILABLE(almssfami) THEN RELEASE almssfami.
