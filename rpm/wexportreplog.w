...
COMBO-BOX-CodDiv FILL-IN-Mensaje 
...
DO:
  ...
  IF FILL-IN-Fecha-1 = ? OR FILL-IN-Fecha-2 = ?
      ...
      RETURN NO-APPLY.
  END.
  IF pParametro = "CENTRAL" AND COMBO-BOX-CodDiv BEGINS 'Seleccione' THEN DO:
...
DEF VAR iErrores AS INT NO-UNDO.
DEF VAR i AS INT NO-UNDO.
/* 1ro. Cargamos el temporal y actualizamos el control de réplicas */
RLOOP:
DO TRANSACTION ON ERROR UNDO, RETURN ON STOP UNDO, RETURN:
    EMPTY TEMP-TABLE t-replog.
    CASE pParametro:    /* Sede donde se origina la información */
        WHEN "REMOTA" THEN DO:
            {rpm\iexportreplog.i &pTabla=replog &pIndice=Idx0 &pCampo=FlgDb0 &pBuffer=b-replog &pTemporal=t-replog}
        END.
        WHEN "CENTRAL" THEN DO:
            CASE COMBO-BOX-CodDiv:
                WHEN "00023" THEN DO:
                    {rpm\iexportreplog.i &pTabla=replog &pIndice=Idx02 &pCampo=FlgDb2 &pBuffer=b-replog &pTemporal=t-replog}
                END.
                WHEN "00027" THEN DO:
                    {rpm\iexportreplog.i &pTabla=replog &pIndice=Idx03 &pCampo=FlgDb3 &pBuffer=b-replog &pTemporal=t-replog}
                END.
            END CASE.
        END.
    END CASE.
END.
/* 2do. Creamos el archivo de transfererencia (replog.d) */
OUTPUT STREAM Reporte TO VALUE(pArchivo).
FOR EACH t-replog NO-LOCK:
    EXPORT STREAM Reporte t-replog.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-Fecha-1 FILL-IN-Fecha-2 COMBO-BOX-CodDiv FILL-IN-Mensaje 
