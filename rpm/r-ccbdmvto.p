...
DO TRANSACTION ON STOP UNDO, RETRY:
    ...
    FIND FIRST ccbdmvto WHERE ccbdmvto.codcia = INTEGER(SUBSTRING(pkeyvalue,1,3)) AND
         ccbdmvto.coddoc = SUBSTRING(pkeyvalue,4,3) AND
         ccbdmvto.nrodoc = SUBSTRING(pkeyvalue,7,15) AND
         ccbdmvto.codref = SUBSTRING(pkeyvalue,22,3) AND
         ccbdmvto.nroref = SUBSTRING(pkeyvalue,25)
    ...
    CASE pEvent:
        ...
        END.
        WHEN 'WRITE' THEN DO:
            IF NOT AVAILABLE ccbdmvto THEN CREATE ccbdmvto.
            RAW-TRANSFER pDATARECORD TO ccbdmvto.
            L-FLG = YES.
        END.
    END CASE.
