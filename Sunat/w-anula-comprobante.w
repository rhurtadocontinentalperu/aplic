...
/* CONTADO, CREDITO Y ADELANTADAS Y MANUALES */
/* RHC 10/01/2020 se agregó SERVICIOS */
/* 12/10/23 Se amplía "V" por Venta Anticipada */
DEFINE NEW SHARED VARIABLE S-TPOFAC   AS CHAR INITIAL "C,CR,A,F,M,S,V".     
...
DEFINE VARIABLE h_q-comprobantes-divi AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_v-factura-cred ( 10.42 , 3.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/sunat/q-comprobantes-divi.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-comprobantes-divi ).
       RUN set-position IN h_q-comprobantes-divi ( 8.54 , 54.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.19 , 10.72 ) */

       /* Links to SmartViewer h_v-factura-cred. */
       RUN add-link IN adm-broker-hdl ( h_p-updv98 , 'TableIO':U , h_v-factura-cred ).
       RUN add-link IN adm-broker-hdl ( h_q-comprobantes-divi , 'Record':U , h_v-factura-cred ).

       /* Links to SmartViewer h_v-totales. */
       RUN add-link IN adm-broker-hdl ( h_q-comprobantes-divi , 'Record':U , h_v-totales ).

       /* Links to SmartQuery h_q-comprobantes-divi. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-comprobantes-divi ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-comprobantes-divi ,
             h_v-totales , 'AFTER':U ).
    END. /* Page 0 */
