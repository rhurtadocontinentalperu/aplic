...
cUrbanizacion = Substring(gn-cias.libre-c[16],1,25).     /* Urb */
...
&IF DEFINED(EXCLUDE-fget-glosa-detraccion-old) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fget-glosa-detraccion-old Procedure 
FUNCTION fget-glosa-detraccion-old RETURNS CHARACTER
  ( INPUT pTipoDocumento AS CHAR )  FORWARD.
...
&IF DEFINED(EXCLUDE-fget-glosa-dsctos-globales) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fget-glosa-dsctos-globales Procedure 
FUNCTION fget-glosa-dsctos-globales RETURNS CHARACTER
...
IF AVAILABLE factabla THEN DO:
    ...
    nVecesTimeOut = factabla.valor[10].
END.
*/
...
/*
    x-retval = - Se supero el tiempo de de espera para la operacion
                - IN PROCESS                    
*/
...
DEFINE VAR lTimeOutOn AS LOG.     /* Para verificar si se activo el TIMEOUT */
DEFINE VAR cTimeOutText AS CHAR.
DEFINE VAR cMsgRsptaServer AS CHAR.
DEFINE VAR iConteo AS INT.

cTimeOutText = "se supero el tiempo de espera de la operacion".

p-msg = "OK".
/*
MESSAGE "x-XML-cabecera  " SKIP 
        STRING(x-XML-cabecera).
MESSAGE "x-XML-adicionales   " SKIP 
        STRING(x-XML-adicionales).
MESSAGE "x-XML-detalle   " SKIP 
        STRING(x-XML-detalle).
MESSAGE "x-XML-factoring   " SKIP 
        STRING(x-XML-factoring).
MESSAGE "x-XML-anticipos  " SKIP .
    STRING(x-XML-anticipos).
*/
...
IF ERROR-STATUS:GET-NUMBER(1) > 0 THEN DO:    
    ...
    cMsgRsptaServer = ERROR-STATUS:GET-MESSAGE(1).
    ...
    p-msg = "ERROR en el evento SEND : Al momento de enviar la trama genero un ERROR, acontinuacion el numero de error y mensaje" + gcCRLF + " (" + 
        ...
        "MENSAJE   : " + cMsgRsptaServer.

    x-log-envio = flog-envio-xml("2.- ERROR en el evento SEND : Al momento de enviar la trama genero un ERROR, acontinuacion el numero de error y mensaje" ).
    x-log-envio = flog-envio-xml("2a.- No ERROR :" + STRING(ERROR-STATUS:GET-NUMBER(1))).
    x-log-envio = flog-envio-xml("2b.- MENSAJE :" + cMsgRsptaServer).

    cMsgRsptaServer = LOWER(cMsgRsptaServer).
    cMsgRsptaServer = REPLACE(cMsgRsptaServer,"ó","o").

    RELEASE OBJECT x-oXmlHttp NO-ERROR.
    RELEASE OBJECT x-oXMLBody NO-ERROR.

    IF INDEX(cMsgRsptaServer,cTimeOutText ) > 0 THEN DO:
        /* Se activo TimeOut */
        lTimeOutOn = YES.
        x-log-envio = flog-envio-xml("2c.- se activo TimeOut").
    END.
    ELSE DO:
        RETURN "ADM-ERROR".
    END.
...
END.
ELSE DO:
    x-log-envio = flog-envio-xml("4.- X-RSPTA - TIMEOUT ").
    /* Ir  a consultar el estado del documento, 'n' veces como este configurado */
    CONSULTACMPTE:
    REPEAT iConteo = 1 TO nVecesTimeOut:
        x-status = "".
        x-xml-hashcode = "".
        x-hashcode = "".

        RUN estado-documento-sunat(pTipoDocmto, pNroDocmto, pCodDiv, OUTPUT x-status, OUTPUT  x-hashcode).

        IF x-status = ? THEN x-status = "".
        IF x-hashcode = ? THEN x-hashcode = "".

        IF NOT (TRUE <> (x-status > ""))  THEN DO:            
           LEAVE CONSULTACMPTE.               
        END.        
    END.
    x-log-envio = flog-envio-xml("4a.- X-RSPTA - TIMEOUT : " + string(iConteo) + " veces,  Status -> " + x-status + "  Hash ->" + x-hashcode ).

    IF TRUE <> (x-status > "")  THEN DO:
        p-msg = "Comprobante no fue enviado a SUNAT".
        RETURN "ADM-ERROR".
    END.
END.
...
IF NOT (TRUE <> (x-status > "")) THEN DO:
    ...
    IF CAPS(x-status) = "DUPLICATED" OR CAPS(x-status) = "SIGNED" OR  CAPS(x-status) = "IN PROCESS" THEN DO:
        /* 
            PROCESO OK 
            28May2025 - para IN PROCESS se da por firmado el comprobante
         */
...
&IF DEFINED(EXCLUDE-estado-documento-sunat) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE estado-documento-sunat Procedure 
PROCEDURE estado-documento-sunat :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE INPUT PARAMETER p2CodDoc AS CHAR.
DEFINE INPUT PARAMETER p2NroDoc AS CHAR.
DEFINE INPUT PARAMETER p2CodDiv AS CHAR.
DEFINE OUTPUT PARAMETER pStatus AS CHAR.
DEFINE OUTPUT PARAMETER pHashCode AS CHAR.

EMPTY TEMP-TABLE tTagsEstadoDoc.

RUN gn/p-estado-documento-electronico-v3.r(INPUT p2CodDoc,
                    INPUT p2NroDoc,
                    INPUT p2Coddiv,
...
IF AVAILABLE tTagsEstadoDoc THEN pStatus = tTagsEstadoDoc.cValue.

FIND FIRST tTagsEstadoDoc WHERE tTagsEstadoDoc.cTag = "hashCode" NO-LOCK NO-ERROR.
IF AVAILABLE tTagsEstadoDoc THEN pHashCode = tTagsEstadoDoc.cValue.
...
DEFINE VAR cArticuloSunat AS CHAR.
...
FOR EACH b-ccbddocu OF b-ccbcdocu NO-LOCK BY b-ccbddocu.nroitm :
    ...
    */    
    ...
    cArticuloSunat = "".
    IF AVAILABLE almmmatg THEN cArticuloSunat = almmmatg.codnew.

    IF b-ccbcdocu.coddoc = 'N/D' OR 
        (b-ccbcdocu.coddoc = 'N/C' AND b-ccbcdocu.cndcre = 'N' AND b-ccbcdocu.tpofac <> 'otros') THEN DO:
        /* Si es N/D or N/C por otros descuentos que no involucra el maestro de items */
        cArticuloSunat = "".
    END.
    ELSE DO:
        IF TRUE <> (cArticuloSunat > "") THEN DO:
            cArticuloSunat = "88888888".    /* Debe existir un producto de Sunat */
        END.
    END.
    ...
    IF cArticuloSunat <> "" THEN DO:
        pXML-DETAIL = pXML-DETAIL + "<codigoProductoSUNAT>" + TRIM(cArticuloSunat) + "</codigoProductoSUNAT>".
    END.    
...
DEFINE VAR yTotalImpuestos AS DEC.
...
END.
/* Descuentos Globales */
IF b-ccbcdocu.descuentosGlobales > 0 THEN DO:
    p-xml-header = p-xml-header + "<descuentosGlobales>" + TRIM(STRING(b-ccbcdocu.descuentosGlobales,">>>>>>>9.99")) + "</descuentosGlobales>".
    p-xml-header = p-xml-header + "<porcentajeDsctoGlobal>" + TRIM(STRING(b-ccbcdocu.porcentajeDsctoGlobal,">>>>>>>9.9999")) + "</porcentajeDsctoGlobal>".
    p-xml-header = p-xml-header + "<montoBaseDescuentoGlobal>" + TRIM(STRING(b-ccbcdocu.montoBaseDescuentoGlobal,">>>>>>>9.99")) + "</montoBaseDescuentoGlobal>".
END.
...
/* Lugar de emision, nombre de la tienda, Nro Pedido, Forma Pago, TipoVta, Cajera, % Igv, Orden Compra */
...
/* Glosa descuentos Globales */
p-xml-header = p-xml-header + fget-glosa-dsctos-globales().
...
DEFINE OUTPUT PARAMETER pRetVal AS CHAR NO-UNDO.
    ...
    IF AVAILABLE gn-convt THEN DO:
        IF gn-convt.tipvta = '1' OR gn-convt.tipvta = '2' THEN DO:
            IF LOOKUP(b-ccbcdocu.fmapgo,"899,900") > 0 OR gn-convt.tipvta = '1' THEN DO:
...
FUNCTION fget-glosa-detraccion RETURNS CHARACTER
  ( INPUT pTipoDocumento AS CHAR ) :
    ...
    DEFINE VAR lRetVal AS CHAR INIT "".
    DEFINE VAR cTexto AS CHAR INIT "".
    DEFINE VAR cDetraccion AS CHAR INIT "".
    DEFINE VAR cCtaBancaria AS CHAR INIT "".
    DEFINE VAR cFormaPago AS CHAR INIT "005".

    DEFINE VAR cTabla AS CHAR INIT "CONFIG-CMPTE-ELECTRONICO".
    DEFINE VAR cLlave_C1 AS CHAR INIT "GLOSA".
    DEFINE VAR cLlave_C2 AS CHAR INIT "DETRACCION".
    DEFINE VAR cLlave_C3 AS CHAR INIT "CTA.BANCARIA".

    IF NOT AVAILABLE b-ccbcdocu THEN RETURN lRetVal.

    /* Verificamos si tiene detraccion */
    FIND FIRST vtacdocu WHERE vtacdocu.codcia = 1 AND vtacdocu.codped = b-ccbcdocu.coddoc AND
                                vtacdocu.nroped = b-ccbcdocu.nrodoc NO-LOCK NO-ERROR.

    IF NOT AVAILABLE vtacdocu THEN RETURN lRetVal.
    ...
    IF AVAILABLE x-vtatabla THEN DO:
        cCtaBancaria = trim(x-vtatabla.llave_c5).
    END.

    /* Forma Pago */
    cLlave_C3 = "FORMA-PAGO".
    FIND FIRST x-vtatabla WHERE x-vtatabla.codcia = 1 AND x-vtatabla.tabla = cTabla AND x-vtatabla.llave_c1 = cLlave_C1 
                                AND x-vtatabla.llave_c2 = cLlave_C2 AND 
                                x-vtatabla.llave_c3 = cLlave_C3 NO-LOCK NO-ERROR.
    IF AVAILABLE x-vtatabla THEN DO:
        cFormaPago = trim(x-vtatabla.llave_c5).
    END.

    cDetraccion = "AFECTO A DETRACCION - " + trim(vtacdocu.libre_c01).
    FIND FIRST factabla WHERE factabla.codcia = 1 AND 
                                factabla.tabla = 'SUNAT_CFG_DETRACCION' AND
                                factabla.codigo = vtacdocu.libre_c01 NO-LOCK.
    IF AVAILABLE factabla THEN DO:
        cDetraccion = cDetraccion + " " + trim(factabla.nombre).
    END.

    lRetVal = lRetVal + fgenera-tag("codigoDetraccion",TRIM(vtacdocu.libre_c01)).
    lRetVal = lRetVal + fgenera-tag("porcentajeDetraccion",TRIM(STRING(vtacdocu.impfle,">>>>>>>9.99"))).
    lRetVal = lRetVal + fgenera-tag("totalDetraccion",TRIM(STRING(vtacdocu.libre_d01,">>>>>>>9.99"))).
    lRetVal = lRetVal + fgenera-tag("numeroCtaBancoNacion",TRIM(cCtaBancaria)).
    lRetVal = lRetVal + fgenera-tag("formaPago",TRIM(cFormaPago)).
    lRetVal = lRetVal + "<codigoLeyenda_2>2006</codigoLeyenda_2>".
    lRetVal = lRetVal + "<textoLeyenda_2>Operacion sujeta a detraccion</textoLeyenda_2>".
/*
    cTexto = "Cta.Bco de la Nacion: " + cCtaBancaria + "   %Tasa: " + string(vtacdocu.impfle) + "   Monto detraccion: " + STRING(vtacdocu.libre_d01,"->,>>>,>>9.99") + "   Neto pagar: " + STRING(vtacdocu.libre_d02,"->,>>>,>>9.99").
    lRetVal = lRetVal + "<codigoAuxiliar250_5>9673</codigoAuxiliar250_5>".
    lRetVal = lRetVal + "<textoAuxiliar250_5>" + cDetraccion + "</textoAuxiliar250_5>".
    lRetVal = lRetVal + "<codigoAuxiliar250_6>9674</codigoAuxiliar250_6>".
    lRetVal = lRetVal + "<textoAuxiliar250_6>" + cTexto + "</textoAuxiliar250_6>".    
*/
    RETURN lRetVal.   /* Function return value. */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fget-glosa-detraccion-old Procedure 
FUNCTION fget-glosa-detraccion-old RETURNS CHARACTER
  ( INPUT pTipoDocumento AS CHAR ) :
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fget-glosa-dsctos-globales Procedure 
FUNCTION fget-glosa-dsctos-globales RETURNS CHARACTER
...
DEFINE VAR cMoneda AS CHAR.

/* Descuentos Globales */
IF b-ccbcdocu.descuentosGlobales > 0 THEN DO:

    cMoneda = IF (b-ccbcdocu.codmon = 2) THEN "$" ELSE "S/".

    cTexto = "DESCUENTO GLOBAL:   % : " + TRIM(STRING(b-ccbcdocu.mrguti)) +  
                                "        IMPORTE : " + cMoneda + "    " +  TRIM(STRING(b-ccbcdocu.impint,">>>>>>>9.99")).
    lRetVal = lRetVal + "<codigoAuxiliar250_7>9675</codigoAuxiliar250_7>".
    lRetVal = lRetVal + "<textoAuxiliar250_7>" + cTexto + "</textoAuxiliar250_7>".
END.
  ...
  IF AVAILABLE zy-ccbcdocu THEN DO:
      ...
      */
      ...
      IF zy-ccbcdocu.tipo <> ? AND zy-ccbcdocu.tipo <> '' THEN DO:
          ...
          lRetval = lRetval + "<textoAuxiliar40_3> - </textoAuxiliar40_3>" .
          /* Ic - 22Ago2025 - correo de Susana en coordinacion con Juan Hermoza, no enviar tipo de venta
...
IF AVAILABLE xy-ccbcdocu THEN DO:
    ...
    /* 03Ene2025 - Detracciones */
    /* Verificamos si tiene detraccion */
    FIND FIRST vtacdocu WHERE vtacdocu.codcia = 1 AND vtacdocu.codped = b-ccbcdocu.coddoc AND
                                vtacdocu.nroped = b-ccbcdocu.nrodoc NO-LOCK NO-ERROR.

    IF AVAILABLE vtacdocu THEN x-retval = "1001".
END.
