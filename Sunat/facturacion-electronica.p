...
DEFINE VARIABLE mDataServer AS CHAR.
...
FUNCTION fget-personalizados RETURNS CHARACTER
  ( INPUT pTipoVenta AS CHAR,INPUT pRowId AS ROWID )  FORWARD.
...
/* Personalizados */
lPersonalizados = fget-personalizados(b-ccbcdocu.tipo, ROWID(b-ccbcdocu)).
...
&IF DEFINED(EXCLUDE-enviar-archivo-al-servidor) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE enviar-archivo-al-servidor Procedure 
PROCEDURE enviar-archivo-al-servidor :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT PARAMETER pFileTxt AS CHAR NO-UNDO.
    ...
    pRetVal = "999|No se pudo enviar el archivo al e-POS".

    FILE-INFO:FILE-NAME = pFileTxt.
    SET-SIZE(mFileData) = FILE-INFO:FILE-SIZE.
    
    INPUT FROM pFileTxt BINARY NO-MAP NO-CONVERT.
    ...
    DO:
        pRetVal = "000|Archivo enviado correctamente".
    END.
...
&IF DEFINED(EXCLUDE-pconectar-epos) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pconectar-epos Procedure 
PROCEDURE pconectar-epos :
/*------------------------------------------------------------------------------
...
IF AVAILABLE FECFGEPOS THEN DO:
    ...
    FOR EACH FECFGEPOS NO-LOCK WHERE FECFGEPOS.codcia = s-codcia AND 
        ...
        mID_ePOS = TRIM(FECFGEPOS.ID_POS).     
        ...
        IF lHost <> '' AND lPort <> '' THEN DO:
            ...
            IF hSocket:CONNECTED() = YES THEN DO:
                ...
                IF SUBSTRING(lRetVal,1,3)="000"  THEN DO:
                    lRetVal = lRetVal + "|" + lHost + "|" + mID_ePOS.
                    LEAVE.
                END.                
                ELSE DO:
            ...
            ELSE lRetVal = "999|Imposible conectarse al e-Pos -  Host(" + lHost + 
                            ") Port(" + lPort + ") ID (" + mID_ePOS + ") - Divi:" + pDivision .
        END.
...
IF AVAILABLE ccbcterm THEN DO:
    IF NOT (TRUE <> (ccbcterm.id_pos > ""))  THEN DO:
        ...
        FIND FIRST FECFGEPOS WHERE FECFGEPOS.codcia = s-codcia AND
                                    FECFGEPOS.coddiv = pDivision AND
                                    FECFGEPOS.ID_POS = ccbcterm.id_pos
                                    NO-LOCK NO-ERROR.
        IF AVAILABLE FECFGEPOS THEN DO:
            CREATE tt-ePos.
                ASSIGN  tt-sec  = "00"
...
IF AVAILABLE tt-ePos THEN DO:
    ...
    FOR EACH tt-epos NO-LOCK BY tt-sec:
        ...
        /* el ID de la caja para el ePOS, va para la configuracion */
        mID_ePOS = TRIM(tt-epos.tt-id).
        mIP_ePOS = lHost.
        lRetVal = "002|EROR Conexion - IP(" + lHost + ") , ID(" + mID_ePOS + ")".

        IF lHost <> '' AND lPort <> '' THEN DO:
            /* Conectar el e-POS */
            lRetVal = "000|Conexion Ok - IP(" + lHost + ") , ID(" + mID_ePOS + ")".
            lStrConn = "-H " + lHost + " -S " + lPort.
            hSocket:CONNECT(lStrConn) NO-ERROR.        
            IF hSocket:CONNECTED() = YES THEN DO:
                /* Configuracion del e-POS */            
...
DEFINE OUTPUT PARAMETER pRetval AS CHAR.
  ...
  lDataEnvio = mControlIniMsg + "1" + mSeparador + mModoPrueba + mSeparador + cRucEmpresa
            + mSeparador + mID_ePOS + mSeparador + cDivision + mControlFinMsg .
...
IF AVAILABLE b-ccbcdocu THEN DO:   
    ...
    lID_POS = mID_ePOS. /*fGet_id_pos(pCodDiv).*/    
...
&IF DEFINED(EXCLUDE-pconfirmar-documento-preanulado) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pconfirmar-documento-preanulado Procedure 
PROCEDURE pconfirmar-documento-preanulado :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF AVAILABLE b-ccbcdocu THEN DO:    
    ...
    IF AVAILABLE FELogComprobantes THEN DO:
        ...
        IF FELogComprobantes.flagPPLL = 2 THEN DO:
            ...
            lID_POS = FELogComprobantes.ID_POS.
            ...
            IF SUBSTRING(pRetVal,1,3) = '000' THEN DO:
                ...
                IF SUBSTRING(pRetVal,1,3) = '000' THEN DO:
                    ...
                    pRetVal = "000|Confirmacion enviado correctamente".
                    IF lRC = FALSE OR ERROR-STATUS:GET-MESSAGE(1) <> '' THEN DO:
                        pRetVal = "004|No se pudo enviar la CONFIRMACION al e-POS(" + lIP_POS + ") ID(" + lID_POS + ")".
                        SET-SIZE(mData) = 0.
                    END.
                    IF SUBSTRING(pRetVal,1,3)="000"  THEN DO:
                        /* A esperar la respuesta del Server */          
...
DEFINE INPUT PARAMETER pIP_ePOS AS CHAR.
DEFINE INPUT PARAMETER pID_ePOS AS CHAR.
DEFINE INPUT PARAMETER pTipoDocto AS CHAR.
DEFINE INPUT PARAMETER pNroDocto AS CHAR.
DEFINE INPUT PARAMETER pCodDiv AS CHAR.
DEFINE OUTPUT PARAMETER pRetVal AS CHAR.
...
IF AVAILABLE b-ccbcdocu THEN DO:   
    ...
    lID_POS = pID_ePOS.    /*mID_ePOS*/ /*fGet_id_pos(pCodDiv).*/    
    ...
    REPEAT lIntento = 1 TO lReintentosConfirmar:
        ...
        IF lID_POS <> "" THEN DO:
            ...
            pRetVal = '999|Imposible conectarse al ePOS(' + pIP_ePOS + ') ID (' + pID_ePOS + ')de la tienda(' + pCodDiv + ")".
            /* Conectarse */
            ...
            IF SUBSTRING(pRetVal,1,3) = "000" THEN DO:
                ...
                pRetVal = "000|Confirmacion enviado correctamente".
                IF lRC = FALSE OR ERROR-STATUS:GET-MESSAGE(1) <> '' THEN DO:
                    pRetVal = "004|No se pudo enviar la CONFIRMACION al e-POS(" + pIP_ePOS + "), ID(" + pID_ePOS + ")".
                    SET-SIZE(mData) = 0.
                END.
                ...
                IF SUBSTRING(pRetVal,1,3)="000"  THEN DO:
                  ...
                  END.        
                  ELSE DO:
                      pRetVal = "999|Se desconecto del servidor(" + pIP_ePOS + ") ID(" + pID_ePOS + "), cuando esperaba respuesta de este".
                  END.
                END.
            END.
        END.
...
&IF DEFINED(EXCLUDE-penviar-documento) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE penviar-documento Procedure 
PROCEDURE penviar-documento :
/*------------------------------------------------------------------------------
...
IF AVAILABLE b-ccbcdocu THEN DO:
    ...
    REPEAT lIntento = 1 TO lReintentos:
        ...
        /* Conectar al ePOS */
        RUN pconexion-epos(INPUT pIP_EPOS, INPUT "5500", OUTPUT lRetVal).
...
/* WS del eServer */
lURL_wdsl = 'http://asp4demos.paperless.com.pe/axis2/services/Online?wsdl'.
/* WS del eGateWay */
lURL_wdsl = 'http://192.168.100.249:8060/axis2/services/Online?wsdl'.
...
&IF DEFINED(EXCLUDE-pgenerar_factura) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pgenerar_factura Procedure 
PROCEDURE pgenerar_factura :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FUNCTION fget-como-pago RETURNS CHARACTER
    ...
    FIND FIRST zz-ccbcdocu WHERE ROWID(zz-ccbcdocu) = pRowId NO-LOCK NO-ERROR.
...
FUNCTION fget-personalizados RETURNS CHARACTER
  ( INPUT pTipoVenta AS CHAR,INPUT pRowId AS ROWID ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
    ...
    */
    ...
    lRetVal = fget-personalizados-pre-impreso(pRowId).
...
FUNCTION fget-personalizados-pre-impreso RETURNS CHARACTER
  ...
  FIND FIRST zy-ccbcdocu WHERE ROWID(zy-ccbcdocu) = pRowId NO-LOCK NO-ERROR.
...
FUNCTION fget-personalizados-ticket RETURNS CHARACTER
    ...
    /* PAGO */
    lRetVal = lRetVal + fget-como-pago(pRowId).
    /* */
    lRetVal = lRetVal + "PES|MensajesAt" + gcCRLF.
