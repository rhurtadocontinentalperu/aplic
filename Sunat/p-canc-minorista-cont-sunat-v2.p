...
DO:     /* Solo para control de flujo */
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        RUN THIRD-TRANSACTION.  /* ANULA I/C FAC Y KARDEX */
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        ...
        UNDO RLOOP, RETURN 'ADM-ERROR'.
    END.
    IF RETURN-VALUE = "ERROR-EPOS" THEN RETURN "ADM-ERROR".  /* Cortamos el proceso */
END.
RETURN 'OK'.
...
/* EN ESTE PUNTO NO SE PUEDE ANULAR LA VENTA, YA ESTA CONFIRMADA EN EL E-POS */
pMensaje = "".
RLOOP:
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    /* VOLVEMOS A FIJAR EL PEDIDO EN UN PUNTERO DEL BUFFER */
    {lib\lock-genericov21.i &Tabla="B-CPEDM" ~
        &Condicion="ROWID(B-CPEDM) = d_rowid" ~
        &Bloqueo="EXCLUSIVE-LOCK" ~
        &Accion="RETRY" ~
    ...
    RUN Graba-Ordenes.
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        IF pMensaje = "" THEN pMensaje = "NO se pudo crear las Ordenes de Despacho".
        UNDO RLOOP, RETURN 'ADM-ERROR'.
    END.
...
------------------------------------------------------------------------------*/
    FOR EACH T-CDOCU NO-LOCK:
        FIND Ccbcdocu OF T-CDOCU EXCLUSIVE-LOCK NO-ERROR.
        IF AVAILABLE Ccbcdocu THEN DO:
            ASSIGN
                Ccbcdocu.FlgEst = "A".
    ...
    FIND Ccbccaja WHERE CcbCCaja.CodCia = s-CodCia
        ...
        EXCLUSIVE-LOCK NO-ERROR.
    IF AVAILABLE CcbCCaja THEN ASSIGN CcbCCaja.FlgEst = "A".   /* ANULADO */
