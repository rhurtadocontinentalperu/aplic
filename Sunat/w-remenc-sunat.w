...
DEFINE VARIABLE h_v-remenc-sunat AS HANDLE NO-UNDO.
...
DO:
    RUN dispatch IN h_v-remenc-sunat('imprime':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'Sunat/v-remenc-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-remenc-sunat ).
       RUN set-position IN h_v-remenc-sunat ( 2.15 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.65 , 76.57 ) */

       /* Links to SmartViewer h_v-remenc-sunat. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-remenc-sunat ).
       RUN add-link IN adm-broker-hdl ( h_q-remesa , 'Record':U , h_v-remenc-sunat ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-remenc-sunat ,
             FILL-IN-desc:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
