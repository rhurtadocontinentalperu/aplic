...
IF LOOKUP(s-CodDoc, 'NC,ND') = 0 THEN RETURN ERROR.
CASE s-CodDoc:
...
DEF NEW SHARED VAR s-Tpofac AS CHAR INIT "REBATE".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-5 W-Win _STRUCTURED
  QUERY BROWSE-5 NO-LOCK DISPLAY
      ...
      Docu.NroDoc FORMAT "X(12)":U WIDTH 9.14
...
ON CHOOSE OF BUTTON-NC IN FRAME F-Main /* GENERAR COMPROBANTE */
DO:
  ASSIGN
...
{src/adm/template/windowmn.i}
    ...
    CASE s-tpofac:
        WHEN "REBATE" THEN  {&WINDOW-NAME}:TITLE = {&WINDOW-NAME}:TITLE + " - REBATE".
...
x-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** GENERANDO COMPROBANTE **'.
...
FOR EACH DOCU:
    ...
    ASSIGN
        ...
        t-cdocu.cndcre = s-CndCre
        t-cdocu.fmapgo = ccbcdocu.fmapgo
        t-cdocu.tpofac = s-Tpofac.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      CASE s-CodDoc:
          WHEN 'N/C' THEN c-NroSer:LABEL = 'N° Serie N/C'.
          WHEN 'N/D' THEN c-NroSer:LABEL = 'N° Serie N/D'.
      END CASE.
      RUN Carga-Series.
      APPLY 'VALUE-CHANGED' TO c-NroSer.
  END.
