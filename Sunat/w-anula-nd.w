...
DEFINE NEW SHARED VARIABLE S-CODDOC   AS CHAR INITIAL "N/D".
DEFINE NEW SHARED VARIABLE s-CndCre AS CHAR INIT "N".
...
IF NUM-ENTRIES(pParametros,'|') > 3 THEN ASSIGN s-permiso-anulacion = LOGICAL(ENTRY(4,pParametros,'|')) NO-ERROR.
IF s-permiso-anulacion = ? THEN DO:
    MESSAGE 'El cuarto parámetro debe ser un YES o NO' VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.

DEFINE NEW SHARED VAR x-es-find AS LOG.
x-es-find = NO.
...
&Scoped-Define ENABLED-OBJECTS RECT-73 C-NroSer 
...
DEFINE VARIABLE h_q-ndnc-divi AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vnotacreditodebito AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE VARIABLE C-CodDoc AS CHARACTER FORMAT "X(256)":U INITIAL "N/D" 
     LABEL "Comprobante" 
...
DEFINE FRAME F-Main
     C-CodDoc AT ROW 8.54 COL 6.14 WIDGET-ID 6
     C-NroSer AT ROW 8.54 COL 23.71 WIDGET-ID 8
     "Buscar el número:" VIEW-AS TEXT
          SIZE 13 BY .5 AT ROW 8.81 COL 36 WIDGET-ID 10
     RECT-73 AT ROW 8 COL 2 WIDGET-ID 22
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 117.14 BY 18.54
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "ANULACION DE NOTAS DE DEBITO DE CREDITOS Y COBRANZAS"
         HEIGHT             = 18.54
         WIDTH              = 117.14
         MAX-HEIGHT         = 29.54
         MAX-WIDTH          = 144.29
...
ON END-ERROR OF W-Win /* ANULACION DE NOTAS DE DEBITO DE CREDITOS Y COBRANZAS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ANULACION DE NOTAS DE DEBITO DE CREDITOS Y COBRANZAS */
DO:
...
DO:
   ...
   CASE s-coddoc:
       ...
       WHEN 'N/C' OR WHEN 'NCI' OR WHEN 'PNC' THEN s-tabla = 'N/C'.
       OTHERWISE s-tabla = s-coddoc.
   END CASE.
   RUN Carga-Series.
END.
...
DO:
  ...
  RUN dispatch IN h_q-ndnc-divi ('open-query':U).
END.
...
/* ***************************  Main Block  *************************** */
  ...
  CASE s-CodDoc:
      WHEN "N/C" THEN {&WINDOW-NAME}:TITLE = "ANULACION NOTAS DE CREDITO POR OTROS CONCEPTOS".
      WHEN "N/D" THEN {&WINDOW-NAME}:TITLE = "ANULACION NOTAS DE DEBITO".
  END CASE.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 8.27 , 61.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv98 ).
       RUN set-position IN h_p-updv98 ( 8.27 , 81.00 ) NO-ERROR.
       RUN set-size IN h_p-updv98 ( 1.54 , 34.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'CCB/vnotacreditodebito.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vnotacreditodebito ).
       RUN set-position IN h_vnotacreditodebito ( 10.12 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.92 , 115.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'Sunat/q-ndnc-divi.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-ndnc-divi ).
       RUN set-position IN h_q-ndnc-divi ( 8.54 , 49.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 11.00 ) */

       /* Links to SmartViewer h_vnotacreditodebito. */
       RUN add-link IN adm-broker-hdl ( h_p-updv98 , 'TableIO':U , h_vnotacreditodebito ).
       RUN add-link IN adm-broker-hdl ( h_q-ndnc-divi , 'Record':U , h_vnotacreditodebito ).

       /* Links to SmartQuery h_q-ndnc-divi. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-ndnc-divi ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-ndnc-divi ,
