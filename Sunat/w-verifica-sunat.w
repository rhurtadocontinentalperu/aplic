...
fchdoc @ fchdoc T-Comprobantes.CodHash T-Comprobantes.LogDate ~
T-Comprobantes.FlagPPLL T-Comprobantes.LogEstado T-Comprobantes.FlagSunat ~
T-Comprobantes.EstadoSunat T-Comprobantes.FlagEstado 
...
&Scoped-Define ENABLED-OBJECTS chbReprocesa rbReferencia rdFiltro BUTTON-21 ~
BUTTON-22 BUTTON-Division txtDesde txtHasta ChkFac ChkBol ChkN_C ChkN_D ~
BUTTON-19 BUTTON-20 txtEmpieze BROWSE-4 BUTTON-24 BUTTON-25 
&Scoped-Define DISPLAYED-OBJECTS chbReprocesa rbReferencia rdFiltro ~
x-CodDiv txtDesde txtHasta ChkFac ChkBol ChkN_C ChkN_D txtEmpieze ~
FILL-IN-Mensaje 
...
DEFINE VARIABLE rbReferencia AS INTEGER INITIAL 3 
...
DEFINE VARIABLE rdFiltro AS INTEGER INITIAL 1 
     ...
     RADIO-BUTTONS 
          "Todos", 1,
"Sin Validacion SUNAT", 2,
"No enviados a SUNAT", 3,
"Rechazados por SUNAT", 4
...
DEFINE VARIABLE ChkBol AS LOGICAL INITIAL no 
     LABEL "Boletas" 
     VIEW-AS TOGGLE-BOX
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-4 W-Win _STRUCTURED
  QUERY BROWSE-4 NO-LOCK DISPLAY
      ...
      fchdoc @ fchdoc COLUMN-LABEL "Emitido" FORMAT "99/99/9999":U
            WIDTH 10.43
      T-Comprobantes.CodHash FORMAT "x(20)":U WIDTH 21.43
      T-Comprobantes.LogDate FORMAT "99/99/9999 HH:MM:SS.SSS":U
      T-Comprobantes.FlagPPLL FORMAT "->,>>>,>>9":U
      T-Comprobantes.LogEstado COLUMN-LABEL "Log PPLL" FORMAT "x(60)":U
            WIDTH 14.43
      T-Comprobantes.FlagSunat FORMAT "->,>>>,>>9":U WIDTH 7.43
      T-Comprobantes.EstadoSunat FORMAT "->,>>>,>>9":U WIDTH 9.29
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 152.14 BY 19.81
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     rbReferencia AT ROW 4.23 COL 57.72 NO-LABEL WIDGET-ID 128
     rdFiltro AT ROW 1.19 COL 124 NO-LABEL WIDGET-ID 118
     ...
     BUTTON-19 AT ROW 3.85 COL 127.43 WIDGET-ID 86
     BUTTON-20 AT ROW 25.96 COL 103.29 WIDGET-ID 88
     txtEmpieze AT ROW 3.38 COL 79.14 COLON-ALIGNED WIDGET-ID 110
     ...
     "Caso de N/C y N/D, mostrar solo aquellos que hacer referencia a" VIEW-AS TEXT
          SIZE 54.43 BY .5 AT ROW 4.27 COL 3 WIDGET-ID 126
          FGCOLOR 9 FONT 6
     "v1.0" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 4.04 COL 145.43 WIDGET-ID 136
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 153.43 BY 26.08
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "SUNAT - ESTADO DE LOS COMPROBANTES"
         HEIGHT             = 26.08
         WIDTH              = 153.43
         MAX-HEIGHT         = 29.54
         MAX-WIDTH          = 195.14
...
"T-Comprobantes.CodDiv" ? ? "character" ? ? ? ? ? ? no ? no no "6.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"fchdoc @ fchdoc" "Emitido" "99/99/9999" ? ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.T-Comprobantes.CodHash
"T-Comprobantes.CodHash" ? ? "character" ? ? ? ? ? ? no ? no no "21.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   = Temp-Tables.T-Comprobantes.LogDate
     _FldNameList[8]   = Temp-Tables.T-Comprobantes.FlagPPLL
     _FldNameList[9]   > Temp-Tables.T-Comprobantes.LogEstado
"T-Comprobantes.LogEstado" "Log PPLL" "x(60)" "character" ? ? ? ? ? ? no ? no no "14.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > Temp-Tables.T-Comprobantes.FlagSunat
"T-Comprobantes.FlagSunat" ? ? "integer" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > Temp-Tables.T-Comprobantes.EstadoSunat
"T-Comprobantes.EstadoSunat" ? ? "integer" ? ? ? ? ? ? no ? no no "9.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > Temp-Tables.T-Comprobantes.FlagEstado
"T-Comprobantes.FlagEstado" "Log SUNAT (Doble Click para ver mas detalle)" "x(60)" "character" 11 0 ? 9 15 ? no ? no no "30.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-4 */
&ANALYZE-RESUME
...
ON END-ERROR OF W-Win /* SUNAT - ESTADO DE LOS COMPROBANTES */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* SUNAT - ESTADO DE LOS COMPROBANTES */
DO:
            ...
            IF t-comprobantes.sactualizar = 'X' THEN DO:
                ...
                IF rpta = YES THEN DO:
                   IF s-user-id = 'ADMIN' THEN s-user-id = 'PRUEBAS'.
                    ...
                    RUN envio-a-ppl.
                    x-dobleclick = NO.
                    IF s-user-id = 'PRUEBAS' THEN s-user-id = 'ADMIN'.
                    /*s-user-id = 'ADMIN'.*/
                END.
            END.
        END.
...
------------------------------------------------------------------------------*/
  DISPLAY chbReprocesa rbReferencia rdFiltro x-CodDiv txtDesde txtHasta ChkFac 
          ChkBol ChkN_C ChkN_D txtEmpieze FILL-IN-Mensaje 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE chbReprocesa rbReferencia rdFiltro BUTTON-21 BUTTON-22 BUTTON-Division 
         txtDesde txtHasta ChkFac ChkBol ChkN_C ChkN_D BUTTON-19 BUTTON-20 
         txtEmpieze BROWSE-4 BUTTON-24 BUTTON-25 
      WITH FRAME F-Main IN WINDOW W-Win.
...
DEFINE VAR x-actualizar AS CHAR.
...
DO WITH FRAME {&FRAME-NAME}.
    ...
    DO lRowSele = 1 TO lRowSeleTot:
        IF BROWSE-4:FETCH-SELECTED-ROW(lRowSele) THEN DO:
            ...
            x-actualizar = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.sactualizar.
