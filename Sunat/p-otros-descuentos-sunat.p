...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH Ccbddocu OF B-CDOCU NO-LOCK, FIRST Almmmatg OF Ccbddocu NO-LOCK BREAK BY Ccbddocu.codcia BY Ccbddocu.nroitm:
        ...
        /* ********************************** */
        CASE pMotivo:
            WHEN "DSCTO_DESPACHO" THEN DO:
                FIND FIRST VtadDctoLogis WHERE VtadDctoLogis.IdMaster = VtamDctoLogis.IdMaster
                    AND VtadDctoLogis.CodFam = Almmmatg.CodFam NO-LOCK NO-ERROR.
                IF NOT AVAILABLE VtadDctoLogis THEN NEXT.   /* Inclusiones */
            END.
            WHEN "DSCTO_RECOJO" THEN DO:
                FIND FIRST VtaDTabla OF VtaCTabla WHERE VtaDTabla.Tipo = Almmmatg.CodFam NO-LOCK NO-ERROR.
                IF AVAILABLE VtaDTabla THEN NEXT.           /* Excepciones */
            END.
        END CASE.
        /* ********************************** */
        ...
        {sunat/sunat-calculo-importes-sku.i &Cabecera="B-CDOCU" &Detalle="B-DDOCU"}
        ...
        /* *************************************** */
        /* 03/10/2023 Log de control de descuentos */
        /* *************************************** */
        /* Acumulativo */
        CREATE logdsctosped.
        BUFFER-COPY Ccbddocu TO logdsctosped
            ASSIGN
            logdsctosped.CodPed = Ccbddocu.coddoc
            logdsctosped.NroPed = Ccbddocu.nrodoc
            logdsctosped.CodCli = B-CDOCU.CodCli
            logdsctosped.CodMon = B-CDOCU.CodMon
            logdsctosped.CanPed = CcbDDocu.CanDes
            logdsctosped.Fecha = TODAY
            logdsctosped.Hora = STRING(TIME,'HH:MM:SS')
            logdsctosped.TipDto = pMotivo
            logdsctosped.PorDto = pPorDto
            logdsctosped.Usuario = s-user-id.
        RELEASE logdsctosped.
        /* *************************************** */
    END.
    {vtagn/i-total-factura-sunat.i &Cabecera="B-CDOCU" &Detalle="B-DDOCU"}
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH Ccbddocu OF Ccbcdocu, FIRST Almmmatg OF Ccbddocu NO-LOCK 
        ...
        IF AVAILABLE ttDsctosArtProntoPago THEN DO:
            ...
            {sunat/sunat-calculo-importes-sku.i &Cabecera="Ccbcdocu" &Detalle="Ccbddocu"}

            IF Ccbddocu.AftIgv THEN DO:
                ASSIGN CcbDDocu.Dcto_Otros_VV = CcbDDocu.Dcto_Otros_PV / ( 1 + (Ccbcdocu.PorIgv / 100) ).
            END.
            ELSE DO:
...
FIND VtaCTabla WHERE VtaCTabla.CodCia = s-CodCia AND 
    ...
    VtaCTabla.Llave = COTIZACION.Lista_de_Precios
    NO-LOCK NO-ERROR.
...
IF AVAILABLE ORDEN THEN DO:
    ...
    CASE TRUE:
        ...
        OTHERWISE DO:
            ...
            FIND FIRST VtamDctoLogis WHERE VtamDctoLogis.TipDto = "DSCTO_DESPACHO" AND
                VtamDctoLogis.CodDiv = COTIZACION.Libre_c01 AND
                ORDEN.FchEnt >= VtamDctoLogis.FchIni AND
                ORDEN.FchEnt <= VtamDctoLogis.FchFin AND
                VtamDctoLogis.FlgEst = "Activo" NO-LOCK NO-ERROR.
            CASE TRUE:
                WHEN AVAILABLE VtamDctoLogis THEN DO:
                    RUN Pronto-Despacho.
                    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Puntero VtamDctoLogis en posición
------------------------------------------------------------------------------*/

DEF VAR s-Tabla AS CHAR INIT "DSCTO_DESPACHO" NO-UNDO.

/* Acumulamos Importes por cada Item */
DEF VAR x-Importe AS DEC INIT 0 NO-UNDO.
DEF VAR pPorDto AS DEC NO-UNDO.
DEF VAR pMotivo AS CHAR NO-UNDO.

ASSIGN pMotivo = s-Tabla.

DEF BUFFER B-CDOCU FOR Ccbcdocu.
DEF BUFFER B-DDOCU FOR Ccbddocu.

DEF VAR LocalControlUpdate AS LOG NO-UNDO.

LocalControlUpdate = NO.
RLOOP:
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    {lib/lock-genericov3.i ~
        &Tabla="B-CDOCU" ~
    ...
    ASSIGN
        B-CDOCU.Dcto_Otros_VV = 0
        B-CDOCU.Dcto_Otros_PV = 0
        B-CDOCU.Dcto_Otros_Mot = ""
        B-CDOCU.Dcto_Otros_Factor = 0.

    FOR EACH Ccbddocu OF Ccbcdocu NO-LOCK, FIRST Almmmatg OF Ccbddocu NO-LOCK,
        FIRST VtadDctoLogis NO-LOCK WHERE VtadDctoLogis.IdMaster = VtamDctoLogis.IdMaster AND VtadDctoLogis.CodFam = Almmmatg.CodFam:
        pPorDto = VtadDctoLogis.PorDto.
        IF pPorDto <= 0 THEN NEXT.

        x-Importe = (CcbDDocu.ImpLin - CcbDDocu.ImpDto2).   /* Importe neto */
        IF x-Importe > 0 THEN DO:
            FIND B-DDOCU WHERE ROWID(B-DDOCU) = ROWID(Ccbddocu) EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
            IF ERROR-STATUS:ERROR = YES THEN DO:
                {lib/mensaje-de-error.i &MensajeError="x-Mensaje"}
                UNDO RLOOP, RETURN 'ADM-ERROR'.
            ...
            ASSIGN
                B-CDOCU.Dcto_Otros_PV = B-CDOCU.Dcto_Otros_PV + B-DDOCU.Dcto_Otros_PV   /* OJO */
                B-CDOCU.Dcto_Otros_Mot = pMotivo.
            ...
            RELEASE logdsctosped.
            /* *************************************** */
            LocalControlUpdate = YES.   /* Al menos 1 registro actualizado */
        END.
    END.
    IF LocalControlUpdate = YES THEN DO:
        {vtagn/i-total-factura-sunat.i &Cabecera="B-CDOCU" &Detalle="B-DDOCU"}
    END.
END.
