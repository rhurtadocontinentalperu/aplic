...
FOR EACH T-CDOCU NO-LOCK BY T-CDOCU.NroPed:         
    ...
    */
    ...
    END.
    ELSE DO:
        MESSAGE "El Documento : " + T-CDOCU.CodDoc + " " + T-CDOCU.NroDoc + " de la division :" + S-CodDiv + " " SKIP 
                "NO EXISTE, por favor avisar a SISTEMAS con los datos mostrados".
    END.
...
/* ******************************************************************************* */
/*{ccb\i-canc-mayorista-cont-v2.i}*/
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH T-DDOCU OF T-CDOCU NO-LOCK:        
        ...
        IF LOOKUP(T-DDOCU.CodMat, '098878,098879,098880,098881,098882,098883') > 0
            THEN T-CDOCU.AcuBon[10] = T-CDOCU.AcuBon[10] + (T-DDOCU.CanDes * T-DDOCU.Factor * 0.10).
    END.
    ASSIGN
    ...
    /* ******************************************** */
    ASSIGN
        T-CDOCU.ImpTot = T-CDOCU.ImpTot + T-CDOCU.AcuBon[10]
        T-CDOCU.SdoAct = T-CDOCU.SdoAct + T-CDOCU.AcuBon[10].
...
FOR EACH T-CDOCU NO-LOCK, FIRST Ccbcdocu OF T-CDOCU NO-LOCK:
    ...
    RUN sunat\progress-to-ppll-v3  ( INPUT Ccbcdocu.coddiv,
                                     ...
                                     OUTPUT pMensaje ).
    /* RHC 16/04/2018 En todos los casos de error ANULAMOS los movimientos */
