...
DEFINE VAR lDirEmpresa3 AS CHAR.
...
DEFINE OUTPUT PARAMETER pExisteMsg AS LOG.
    ...
    FIND FIRST vtacdocu WHERE vtacdocu.codcia = 1 AND vtacdocu.codped = ccbcdocu.coddoc AND
                                vtacdocu.nroped = ccbcdocu.nrodoc NO-LOCK NO-ERROR.
    ...
    IF AVAILABLE x-vtatabla THEN DO:
        cCtaBancaria = x-vtatabla.llave_c5.
    END.
    ...
    FIND FIRST factabla WHERE factabla.codcia = 1 AND 
                                ...
                                factabla.codigo = vtacdocu.libre_c01 NO-LOCK NO-ERROR.
    ...
    ASSIGN w-report.Campo-c[25] = cDetraccion
            w-report.Campo-c[26] = cTexto.
...
&IF DEFINED(EXCLUDE-PRINT_carga-detraccion_old) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PRINT_carga-detraccion_old Procedure 
PROCEDURE PRINT_carga-detraccion_old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* 28/01/2026: tomamos el s-dircia */
DEF VAR s-dircia AS CHAR NO-UNDO.
FIND gn-cias WHERE gn-cias.codcia = s-codcia NO-LOCK NO-ERROR.
IF AVAILABLE gn-cias THEN DO:
    lDirEmpresa = gn-cias.libre-c[15].
    lDirEmpresa2 = gn-cias.libre-c[16].
    lDirEmpresa3 = gn-cias.libre-c[17] + "-" + gn-cias.libre-c[18] + "-" + gn-cias.libre-c[19].
END.
/* ******************************** */
...
FOR EACH ccbddocu OF ccbcdocu NO-LOCK BREAK BY Ccbddocu.NroItm:
    ...
    END.
    RUN PRINT_carga_dsctos_globales.
...
&IF DEFINED(EXCLUDE-PRINT_carga_dsctos_globales) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PRINT_carga_dsctos_globales Procedure 
PROCEDURE PRINT_carga_dsctos_globales :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* Descuentos Globales */
IF ccbcdocu.descuentosGlobales > 0 THEN DO:
    /*MESSAGE "ccbcdocu.impint " b-ccbcdocu.impint.*/
    cTexto = "          DESCUENTO GLOBAL:   % : " + TRIM(STRING(ccbcdocu.mrguti)) +  "    IMPORTE : " + lSignoMoneda + "   " +
        TRIM(STRING(ccbcdocu.impint,">>>>>>>9.99")).

    ASSIGN w-report.Campo-c[27] = cTexto.
...
ASSIGN
    ...
    pWeb = "https://sfe.bizlinks.com.pe".
...
/* Ic - 22Ago2025
   Correo de Susana Leon en coordinacion con Juan Hermoza, Tipo de Venta dejar en blanco

"~npTipoVta = " + trim(ccbcdocu.tipo) +

*/

DEFINE VAR pTipoVta AS CHAR.
...
DEFINE VAR cDirecEmisor AS CHAR NO-UNDO.
DEFINE VAR cUrbanizacion AS CHAR NO-UNDO.
DEFINE VAR cDepartamento AS CHAR NO-UNDO.
DEFINE VAR cProvincia AS CHAR NO-UNDO.
DEFINE VAR cDistrito AS CHAR NO-UNDO.
DEFINE VAR cUBIGEO AS CHAR NO-UNDO.
DEFINE VAR cUBIGEONombres AS CHAR NO-UNDO.

FIND FIRST gn-cias NO-LOCK NO-ERROR.

cDirecEmisor = gn-cias.libre-c[15].
...
cUBIGEONombres = cDepartamento + " - " + cProvincia + " - " + cDistrito.
...
REPEAT iCopia = 1 TO iCopias:
    ...
    ASSIGN RB-OTHER-PARAMETERS = "cDataQR = " + cPathGraficoQR +
                                    ...
                                    "~ncDirecEmisor= " + cDirecEmisor +
                                    "~ncUrbanizacion= " + cUrbanizacion +
                                    "~ncUBIGEONombres= " + cUBIGEONombres.
