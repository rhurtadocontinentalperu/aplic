...
ON LEAVE OF wrk_dcaja.NroRef DO:
    ...
    FIND ccbcdocu WHERE ccbcdocu.codcia = s-codcia 
        AND ccbcdocu.coddiv = S-CODDIV      /* N/C De la misma tienda */
        AND ccbcdocu.coddoc = wrk_dcaja.codref:SCREEN-VALUE 
    ...
    ELSE DO:
        ...
        MESSAGE
            "La N/C NO encontrada o NO registrada en esta tienda" VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
END.
    ...
    IF AVAILABLE ccbcdocu THEN DO:
        ...
        IF AVAILABLE gn-clie AND gn-clie.rucold = "Si" THEN DO:
            ...
            FIND FIRST CcbCmov WHERE
                ...
                NO-LOCK NO-ERROR.
            IF NOT AVAILABLE CCBCMOV AND
                NOT CAN-FIND(FIRST wrk_ret WHERE
                wrk_ret.CodDoc = ccbcdocu.coddoc AND
                wrk_ret.NroDoc = ccbcdocu.nrodoc) THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    FIND ccbcdocu WHERE ccbcdocu.codcia = s-codcia 
        AND ccbcdocu.coddiv = S-CODDIV      /* N/C De la misma división */
        AND ccbcdocu.coddoc = L-CODREF 
        AND ccbcdocu.nrodoc = L-NROREF NO-LOCK NO-ERROR.
    IF NOT AVAILABLE ccbcdocu THEN DO:
        MESSAGE "Documento de referencia no registrado o no fue generado en esta tienda" 
    ...
    IF ccbcdocu.flgest NE "P" THEN DO:
        MESSAGE "El documento de referencia no esta pendiente de pago"
            VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY":U TO wrk_dcaja.nroref.
        RETURN "ADM-ERROR".
    END.
    ...
    IF Ccbcdocu.fchvto < TODAY THEN DO:
        ...
        RETURN "ADM-ERROR".
    END.
    /* CANCELACION DE PEDIDOS MOSTRADOR */
    CASE TRUE:
        WHEN s-Tipo = 'MOSTRADOR' THEN DO:
            IF Ccbcdocu.Tipo <> 'MOSTRADOR' THEN DO:
                MESSAGE 'La Nota de Crédito NO se originó en Mostrador' VIEW-AS ALERT-BOX ERROR.
                RETURN 'ADM-ERROR'.
            END.
        END.
        OTHERWISE DO:
            IF Ccbcdocu.Tipo = 'MOSTRADOR' THEN DO:
                MESSAGE 'La Nota de Crédito NO se originó en Créditos' VIEW-AS ALERT-BOX ERROR.
                RETURN 'ADM-ERROR'.
            END.
        END.
    END CASE.
