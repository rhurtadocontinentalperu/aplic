...
DEFINE VARIABLE h_v-remenc-utilex AS HANDLE NO-UNDO.
...
DEFINE VARIABLE CB-Tipo AS CHARACTER FORMAT "X(256)":U INITIAL "DEVONC" 
     LABEL "Tipo" 
...
ON END-ERROR OF W-Win /* Devolucion de Efectivo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Devolucion de Efectivo */
DO:
...
DO:
    RUN dispatch IN h_v-remenc-utilex('imprime':U).
END.
...
DO:
    ...
    CASE s-Tipo:
        ...
        WHEN 'DEVOAC'
            THEN ASSIGN
                FILL-IN-desc = "Anticipos de Campaña"
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'Sunat/v-remenc-utilex.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-remenc-utilex ).
       RUN set-position IN h_v-remenc-utilex ( 2.15 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.65 , 76.57 ) */

       /* Links to SmartViewer h_v-remenc-utilex. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-remenc-utilex ).
       RUN add-link IN adm-broker-hdl ( h_q-remesa , 'Record':U , h_v-remenc-utilex ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-remenc-utilex ,
             FILL-IN-desc:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
...
/*   END.                                                  */
  ...
  APPLY 'VALUE-CHANGED':U TO CB-Tipo IN FRAME {&FRAME-NAME}.
