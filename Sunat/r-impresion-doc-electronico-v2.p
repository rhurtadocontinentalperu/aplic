...
/* ***************************  Definitions  ************************** */
/* Por ahora solicitamos el proveedor de facturacion electrónica */
DEFINE INPUT PARAMETER pProvSunat AS CHAR.
...
FIND FacTabla WHERE FacTabla.codcia = s-codcia AND
    FacTabla.Tabla = "SUNATPRV" AND
    FacTabla.Codigo = pProvSunat
    NO-LOCK NO-ERROR.
MESSAGE 'Proveedor SUNAT =' pProvSunat 'NO registrado' VIEW-AS ALERT-BOX ERROR.
RETURN 'ADM-ERROR'.
...
ELSE DO:
    ...
    IF LOOKUP(ccbcdocu.divori,"00024,00030,00070") > 0 THEN DO:
...
FIND VtaCTabla WHERE VtaCTabla.CodCia = s-codcia AND
    VtaCTabla.Tabla = 'SUNATPRV' AND 
    CAN-FIND(FIRST VtaDTabla OF VtaCTabla WHERE VtaDTabla.Tipo = pCodDiv NO-LOCK)
    NO-LOCK NO-ERROR.
IF AVAILABLE VtaCTabla THEN DO:
    IF VtaDTabla.Libre_c01 > '' THEN pResolucion = VtaDTabla.Libre_c01.
    IF VtaDTabla.Libre_c02 > '' THEN pWeb        = VtaDTabla.Libre_c02.
END.
MESSAGE pweb presolucion. RETURN.
/* ****************************************************************************** */
/* ****************************************************************************** */
