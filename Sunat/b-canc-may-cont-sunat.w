...
/*                     pMensaje = 'Error an la generación del archivo para SUNAT' + CHR(10) + */
/*                         'Proceso abortado'.                                                */
...
------------------------------------------------------------------------------*/
    ...
    DEF VAR x-Format AS CHAR INIT '999-999999' NO-UNDO.

    FIND FacDocum  WHERE FacDocum.CodCia = s-codcia
        AND FacDocum.CodDoc = s-coddoc NO-LOCK NO-ERROR.
    IF AVAILABLE FacDocum AND FacDocum.codcta[10] <> '' AND INDEX(FacDocum.codcta[10],'-') > 1 
        THEN x-Format = FacDocum.codcta[10].
    CREATE CcbCDocu.
    ASSIGN
        CcbCDocu.CodCia = s-CodCia
        CcbCDocu.CodDiv = s-CodDiv   
        ...
        CcbCDocu.NroDoc = STRING(FacCorre.NroSer,ENTRY(1,x-Format,'-')) + STRING(FacCorre.Correlativo,ENTRY(2,x-Format,'-')) 
        CcbCDocu.FchDoc = TODAY
