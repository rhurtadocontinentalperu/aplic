...
&Scoped-define FIELDS-IN-QUERY-BROWSE-10 tt-FELogComprobantes.Campo-Log[1] ~
tt-FELogComprobantes.libre_c02 tt-FELogComprobantes.CodDiv ~
tt-FELogComprobantes.CodDoc tt-FELogComprobantes.NroDoc ~
tt-FELogComprobantes.LogDate CcbCDocu.FchDoc CcbCDocu.CodCli ~
CcbCDocu.NomCli if(CcbCDocu.CodMon = 2) then "USD" else "S/" @ x-col-moneda ~
CcbCDocu.ImpTot 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-10 ~
tt-FELogComprobantes.Campo-Log[1] 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-10 tt-FELogComprobantes
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-10 tt-FELogComprobantes
&Scoped-define QUERY-STRING-BROWSE-10 FOR EACH tt-FELogComprobantes NO-LOCK, ~
...
&Scoped-define OPEN-QUERY-BROWSE-10 OPEN QUERY BROWSE-10 FOR EACH tt-FELogComprobantes NO-LOCK, ~
      EACH CcbCDocu WHERE ccbcdocu.codcia = tt-felogcomprobantes.codcia and ~
ccbcdocu.coddoc = tt-felogcomprobantes.coddoc and ~
ccbcdocu.nrodoc = tt-felogcomprobantes.nrodoc ~
      AND ccbcdocu.fchdoc = x-emitidos NO-LOCK.
&Scoped-define TABLES-IN-QUERY-BROWSE-10 tt-FELogComprobantes CcbCDocu
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-10 tt-FELogComprobantes
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-10 CcbCDocu
...
&Scoped-Define ENABLED-OBJECTS BROWSE-10 BUTTON-29 FILL-IN-desde ~
FILL-IN-hasta BUTTON-1 BROWSE-6 TOGGLE-fac TOGGLE-nc TOGGLE-nd ~
FILL-IN-coddiv RECT-1 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-nrodoc FILL-IN-2 FILL-IN-desde ~
...
DEFINE BUTTON BUTTON-29 
     LABEL "Enviar BOLETAS BAJA" 
     SIZE 23 BY 1.12
     BGCOLOR 12 FGCOLOR 12 .

DEFINE VARIABLE FILL-IN-2 AS CHARACTER FORMAT "X(100)":U INITIAL "GENERACION DE BAJAS DE BOLETAS" 
     VIEW-AS FILL-IN 
     SIZE 65 BY 1.35
...
DEFINE VARIABLE FILL-IN-nrodoc AS CHARACTER FORMAT "X(25)":U 
     VIEW-AS FILL-IN 
     SIZE 30 BY 1
     FONT 7 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-10 W-Win _STRUCTURED
  QUERY BROWSE-10 NO-LOCK DISPLAY
      ...
      tt-FELogComprobantes.libre_c02 COLUMN-LABEL "Estado" FORMAT "x(15)":U
            WIDTH 10.86
      tt-FELogComprobantes.CodDiv FORMAT "x(5)":U WIDTH 5.43
      tt-FELogComprobantes.CodDoc FORMAT "x(3)":U
      tt-FELogComprobantes.NroDoc FORMAT "X(12)":U WIDTH 9.57
      ...
      CcbCDocu.NomCli FORMAT "x(50)":U WIDTH 27.29
      if(CcbCDocu.CodMon = 2) then "USD" else "S/" @ x-col-moneda COLUMN-LABEL "Moneda" FORMAT "x(5)":U
      CcbCDocu.ImpTot FORMAT "->>,>>>,>>9.99":U WIDTH 12.14
  ENABLE
      tt-FELogComprobantes.Campo-Log[1]
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 129.43 BY 19.85
         FONT 4
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 32 BY 16.88
...
DEFINE FRAME F-Main
     BROWSE-10 AT ROW 4.04 COL 34.57 WIDGET-ID 400
     FILL-IN-nrodoc AT ROW 22.88 COL 1 COLON-ALIGNED NO-LABEL WIDGET-ID 30
     FILL-IN-2 AT ROW 1.19 COL 96.86 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     BUTTON-29 AT ROW 21.5 COL 5.29 WIDGET-ID 10
     FILL-IN-desde AT ROW 1.31 COL 16 COLON-ALIGNED WIDGET-ID 2
     FILL-IN-hasta AT ROW 1.31 COL 39.72 COLON-ALIGNED WIDGET-ID 4
     BUTTON-1 AT ROW 1.31 COL 82.57 WIDGET-ID 6
     BROWSE-6 AT ROW 4.12 COL 2 WIDGET-ID 300
     TOGGLE-fac AT ROW 2.69 COL 3.14 WIDGET-ID 18
     TOGGLE-nc AT ROW 2.65 COL 11.57 WIDGET-ID 20
     TOGGLE-nd AT ROW 2.65 COL 19.86 WIDGET-ID 22
     FILL-IN-coddiv AT ROW 2.54 COL 33 COLON-ALIGNED WIDGET-ID 26
     FILL-IN-desdiv AT ROW 2.54 COL 42 COLON-ALIGNED NO-LABEL WIDGET-ID 28
     "v1" VIEW-AS TEXT
          SIZE 8 BY .62 AT ROW 1.77 COL 65 WIDGET-ID 32
     RECT-1 AT ROW 3.77 COL 1 WIDGET-ID 8
...
"tt-FELogComprobantes.Campo-Log[1]" "Sele" ? "logical" ? ? ? ? ? ? yes ? no no "3.43" yes no no "U" "" "" "TOGGLE-BOX" "," ? ? 5 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-FELogComprobantes.libre_c02
"tt-FELogComprobantes.libre_c02" "Estado" "x(15)" "character" ? ? ? ? ? ? no ? no no "10.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-FELogComprobantes.CodDiv
"tt-FELogComprobantes.CodDiv" ? ? "character" ? ? ? ? ? ? no ? no no "5.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   = Temp-Tables.tt-FELogComprobantes.CodDoc
     _FldNameList[5]   > Temp-Tables.tt-FELogComprobantes.NroDoc
"tt-FELogComprobantes.NroDoc" ? "X(12)" "character" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.tt-FELogComprobantes.LogDate
"tt-FELogComprobantes.LogDate" ? ? "datetime" ? ? ? ? ? ? no ? no no "17.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.NomCli" ? ? "character" ? ? ? ? ? ? no ? no no "27.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.ImpTot" ? ? "decimal" ? ? ? ? ? ? no ? no no "12.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-10 */
&ANALYZE-RESUME
...
ON CHOOSE OF BUTTON-29 IN FRAME F-Main /* Enviar BOLETAS BAJA */
DO:

    IF NOT AVAILABLE tt-w-report THEN RETURN NO-APPLY.

        MESSAGE 'Seguro de enviar la BAJA del dia ' + STRING(tt-w-report.llave-d,"99/99/9999") + '?' VIEW-AS ALERT-BOX QUESTION
...
IF toggle-fac = YES THEN x-documentos = 'BOL'.
...
FOR EACH ccbcdocu WHERE ccbcdocu.codcia = s-codcia AND 
                        (ccbcdocu.fchdoc >= x-emitidos-desde AND ccbcdocu.fchdoc <= x-emitidos-hasta) AND
                        LOOKUP(ccbcdocu.coddoc,x-documentos) > 0 NO-LOCK:
    IF fill-in-coddiv = "" OR fill-in-coddiv = ccbcdocu.coddiv THEN DO:
        ...
        IF AVAILABLE FELogcomprobantes THEN DO:
            ...
            fill-in-nrodoc:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ccbcdocu.coddoc + " " + ccbcdocu.nrodoc.
            ...
            IF FELogcomprobantes.coddoc = x-doc-principal OR 
                ...
                ASSIGN tt-felogcomprobantes.libre_c02 = x-registro-valido.
                              
                IF x-registro-valido = "NO ENVIADO" THEN DO:

                    IF NOT (TRUE > (FELogcomprobantes.libre_c02 <> "")) THEN DO:
                        /* Las anulaciones de Boletas se hace por el Resumen (RC) */
                        x-registro-valido = "".

                        RUN validar-documento-v2(INPUT FELogcomprobantes.libre_c02, OUTPUT x-registro-valido).
                        ASSIGN tt-felogcomprobantes.libre_c02 =  x-registro-valido.
        ...
        ELSE DO:
            FIND FIRST fecomprocab WHERE fecomprocab.codcia = s-codcia AND
                                            fecomprocab.coddoc = ccbcdocu.coddoc AND
                                            fecomprocab.nrodoc = ccbcdocu.nrodoc NO-LOCK NO-ERROR.
            IF AVAILABLE fecomprocab THEN DO:
                FIND FIRST tt-w-report WHERE tt-w-report.llave-d = ccbcdocu.fchdoc NO-ERROR.
                IF NOT AVAILABLE tt-w-report THEN DO:
                    CREATE tt-w-report.
                        ASSIGN tt-w-report.llave-d = ccbcdocu.fchdoc
                ...
                CREATE tt-felogcomprobantes.
                    ASSIGN tt-felogcomprobantes.codcia = s-codcia
                            tt-felogcomprobantes.coddiv = ccbcdocu.coddiv
                            tt-felogcomprobantes.coddoc = ccbcdocu.coddoc
                            tt-felogcomprobantes.nrodoc = ccbcdocu.nrodoc
                            tt-felogcomprobantes.logdate = ?
                            tt-felogcomprobantes.tipodocsunat = fecomprocab.tipodocumento
                            tt-felogcomprobantes.vvtagravada = fecomprocab.totvvtaopgravadas
                            tt-felogcomprobantes.totalisc = 0
                            tt-felogcomprobantes.totaligv = fecomprocab.totigv
                            tt-felogcomprobantes.imptetotal = fecomprocab.totalventa
                            tt-felogcomprobantes.nrodocadq = fecomprocab.nrodocadquiriente
                            tt-felogcomprobantes.tipodocadq = fecomprocab.tipodocadquiriente
                            tt-felogcomprobantes.nrodocrefpri = fecomprocab.nrodocrefprincipal
                            tt-felogcomprobantes.tipodocrefpri = fecomprocab.tipodocrefprincipal
                            tt-felogcomprobantes.libre_c02 = "NO REGISTRO"
                            .


                IF ccbcdocu.flgest = 'A' THEN tt-felogcomprobantes.libre_c02 =  "ANULADO". 

                FIND LAST fersmcomprobdtl /*USE-INDEX idx03*/ WHERE fersmcomprobdtl.codcia = s-codcia AND
                                                    fersmcomprobdtl.coddoc = ccbcdocu.coddoc AND
                                                    fersmcomprobdtl.nrodoc = ccbcdocu.nrodoc AND
                                                    fersmcomprobdtl.tiporsm = 'RC' NO-LOCK NO-ERROR.
                IF AVAILABLE fersmcomprobdtl THEN DO:
                    x-registro-valido = "".

                    RUN validar-documento-v2(INPUT fersmcomprobdtl.nrorsm, OUTPUT x-registro-valido).
                    ASSIGN tt-felogcomprobantes.libre_c02 =  x-registro-valido.

                END.
                ELSE DO:
                    IF ccbcdocu.nrodoc = "25400001092" THEN DO:
                /*        MESSAGE "NO ESTAAA".*/
                    END.
                END.
            END.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-nrodoc FILL-IN-2 FILL-IN-desde FILL-IN-hasta TOGGLE-fac 
          TOGGLE-nc TOGGLE-nd FILL-IN-coddiv FILL-IN-desdiv 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE BROWSE-10 BUTTON-29 FILL-IN-desde FILL-IN-hasta BUTTON-1 BROWSE-6 
         TOGGLE-fac TOGGLE-nc TOGGLE-nd FILL-IN-coddiv RECT-1 
      WITH FRAME F-Main IN WINDOW W-Win.
...
/*IF USERID("DICTDB") = "MASTER" THEN DO:*/
    COPY-LOB x-xml-data TO FILE x-tmp + "RX-" + x-fecha-envio + "-" + STRING(x-correla,"999") + ".xml" NO-ERROR.
/*END.*/
        ...
        DO:
            ...
            FOR EACH t-fersmcomprobdtl NO-LOCK:
                ...
                IF NOT AVAILABLE b-felogcomprobantes THEN DO:
                    ...
                    UNDO GRABAR_DATOS, LEAVE GRABAR_DATOS.
                END.
                ASSIGN b-felogcomprobantes.libre_c02 = x-nro-envio.
            END.
        END.
        p-msg = "OK".
    END. 
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    DO  WHILE AVAILABLE tt-FELogcomprobantes:
        IF tt-FELogcomprobantes.campo-log[1] = YES THEN DO:
            ...
            x-xml-detail = x-xml-detail + "<tipoDocumento>" + {&FIRST-TABLE-IN-QUERY-BROWSE-10}.tipodocsunat + "</tipoDocumento>".
            ...
            END.
            x-xml-detail = x-xml-detail + "<estadoItem>3</estadoItem>".         /* estado 3 : Dar de baja */
            x-xml-detail = x-xml-detail + "</item>".
        END.
        GET NEXT BROWSE-10.
    END.
...
IF TRUE > (FELogcomprobantes.libre_c01 <> "") THEN DO:
    /* Aun NO a sido enviado a SUNAT */
    RETURN.
END.

/* Tiene envio de anulacion */
IF NOT (TRUE > (FELogcomprobantes.libre_c02 <> "")) THEN DO:
    /* Las anulaciones de Boletas se hace por el Resumen (RC) */
    x-estado-bizlinks = "".
    x-estado-sunat = "".
    /**/
    RUN gn/p-estado-documento-electronico.r(INPUT "RC",
                        INPUT FELogcomprobantes.libre_c02,
                        INPUT "",
                        OUTPUT x-estado-bizlinks,
                        OUTPUT x-estado-sunat,
                        OUTPUT x-estado-doc).
    /* Ya fue ANULADO, el resumen de ANULACION fue aceptado por SUNAT */
...
/* Verificar si el envio del RESUMEN esta aceptado por SUNAT */
IF NOT (TRUE > (FELogcomprobantes.libre_c01 <> "")) THEN DO:
    x-estado-bizlinks = "".
    x-estado-sunat = "".
    /**/
    RUN gn/p-estado-documento-electronico.r(INPUT "RC",
                        INPUT FELogcomprobantes.libre_c01,
                        INPUT "",
                        OUTPUT x-estado-bizlinks,
                        OUTPUT x-estado-sunat,
                        OUTPUT x-estado-doc).
    /* El estado del envio del RESUMEN  */    
    IF ENTRY(1,x-estado-bizlinks,"|") <> "SIGNED" THEN DO:
        /* El envio esta en camino */
        RETURN.
    END.
    IF ENTRY(1,x-estado-sunat,"|") <> "AC_03" THEN DO:
        /* El envio no esta aceptado por SUNAT */
        RETURN. /*???????????????????????????????????????????????????????????????*/
    END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE validar-documento-v2 W-Win 
PROCEDURE validar-documento-v2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER pNroResumen AS CHAR.
DEFINE OUTPUT PARAMETER pMsg AS CHAR.
...
*/
/* Tiene envio de anulacion */
IF NOT (TRUE > (pNroResumen <> "")) THEN DO:
    /* Las anulaciones de Boletas se hace por el Resumen (RC) */
    x-estado-bizlinks = "".
    x-estado-sunat = "".
    pMsg = "ENVIO BAJA".
