...
DEFINE VAR lxCodigoEstablecimiento AS CHAR.
...
&IF DEFINED(EXCLUDE-fGet-Establecimiento) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fGet-Establecimiento Procedure 
FUNCTION fGet-Establecimiento RETURNS CHARACTER
...
IF x-ruta-file-contingencia <> "" THEN DO:
    cSerieSunat = "0"  +  SUBSTRING(b-ccbcdocu.nrodoc,1,3).
END.
ELSE DO:
...
lxTipoOperacion = "".
lxTipoOperacion = fGetTipoOperacion(b-ccbcdocu.coddoc, b-ccbcdocu.nrodoc).
...
mDataClient = mDataClient + "EN|" +
        ...
        "|" +    /* Codigo de pais emisor */
        "|" +    /* Urbanizacion Emisor */
        "|" +    /* Direccion del punto de partida, Ubigeo */
        "|" +    /* Direccion del punto de partida completa y detallada */
        "|" +    /* Direccion del punto de partida, urbanizacion */
        "|" +    /* Direccion del punto de partida, provincia */
        "|" +    /* Direccion del punto de partida, departamento */
        "|" +    /* Direccion del punto de partida, distrito */
        "|" +    /* Direccion del punto de partida, codigo pais */
        "|" +    /* Direccion del punto de llegada, ubigeo */
        "|" +    /* Direccion del punto de llegada, completa y detallada */
        "|" +    /* Direccion del punto de llegada, urbanizacion */
        "|" +    /* Direccion del punto de llegada, provincia */
        "|" +    /* Direccion del punto de llegada, departamento */
        "|" +    /* Direccion del punto de llegada, distrito */
        "|" +    /* Direccion del punto de llegada, codigo pais */
        "|" +    /* Placa vehiculo */
        "|" +    /* habilitacion vehicular*/
        "|" +    /* Marca vehiculo*/
        "|" +    /* Nro de licencia conducir */
        "|" +    /* RUC trasnportista*/
        ...
        lxTipoOperacion +    /* Tipo de Operacion*/
        gcCRLF.
...
DEFINE VAR lPorIgv AS DEC.
DEFINE VAR lPrecioUnit2 AS DEC.
DEFINE VAR lImpItmVVta2 AS DEC.
...
FOR EACH b-ccbddocu OF b-ccbcdocu NO-LOCK:
    ...
    lPorIGV = b-ccbcdocu.porigv / 100.
    lPorIgv = IF(b-ccbddocu.aftigv = NO) THEN 0 ELSE lPorIgv.
    
    /* a) */
    lPrecioUnit = ROUND((b-ccbddocu.implin - b-ccbddocu.ImpDto2) / b-ccbddocu.candes , 4).
    /* b) */
    lImpItmVVta = (b-ccbddocu.implin - b-ccbddocu.impigv).
    /* c) */    
    lPrecioUnit2 = (b-ccbddocu.implin - b-ccbddocu.ImpIgv).
    lPrecioUnit2 = lPrecioUnit2 - (b-ccbddocu.ImpDto2 - ((b-ccbddocu.ImpDto2 / (1 + lPorIGV)) * lPorIGV)).
    lPrecioUnit2 = ROUND(lPrecioUnit2 / b-ccbddocu.candes , 4).
    /* d */
    lImpItmVVta2 = lPrecioUnit2 * b-ccbddocu.candes.
    ...
    */
    ...
    mDataClient = mDataClient + "DE|" +
        ...
        b-ccbddocu.codmat + "|" +
        lFPago + "|" +
        STRING(lPrecioUnit2,lFmtoImpte) + "|" +
        STRING(lImpItmVVta2,lFmtoImpte) + "|" + 
        "|" +
        lMarca + 
        gcCRLF.
    ...
    */
    ...
    IF b-ccbddocu.impigv > 0 THEN DO:
        ...
        lIgvImpDto2 = ROUND(b-ccbddocu.impdto2 / ( 1 + lPorIGV) * lPorIGV, 4).
...
IF x-ruta-file-contingencia <> "" THEN DO:
    COPY-LOB mDataClient TO FILE x-ruta-file-contingencia + "\" + cSerieSunat + "-" + cCorrelativoSunat + 
...
IF NOT USERID("integral") = "MASTER" THEN DO:
    ...
    IF SUBSTRING(pReturn,1,3)="000"  THEN DO:
        ...
        pReturn = "789|Esperando respuesta del e-POS".
...
lxTipoOperacion = "".
lxTipoOperacion = fGetTipoOperacion(INPUT b-ccbcdocu.coddoc, INPUT b-ccbcdocu.nrodoc).
...
mDataClient = mDataClient + "EN|" +
        ...
        "|" +   /*Razón social del transportista*/
        "|" +    /*Condiciones de pago*/
        lOrdenCompra + "|" +     /* OrdenCompra - Order Reference*/
        "|" +    /* Numero de contrato */
        "|" +    /* Nota general del documento */
        "|" +    /* Valor del seguro */
        "|" +    /* Valor del flete */
...
FOR EACH b-ccbddocu OF b-ccbcdocu NO-LOCK:
    ...
    */
    ...
    mDataClient = mDataClient + "DE|" +
        ...
        STRING(lPrecioUnit2,lFmtoImpte) + "|" +                                                    /* PriceAmount */
        STRING(lImpItmVVta2,lFmtoImpte) + "|" +                                                    /* LineExtensionAmount */
        "|" +
        lMarca +
        gcCRLF.
...
/* Ic - 27Nov2018, si solo contingencia */
IF x-ruta-file-contingencia <> "" THEN DO:
    COPY-LOB mDataClient TO FILE x-ruta-file-contingencia + "\" + cSerieSunat + "-" + cCorrelativoSunat + 
        "-" + cTipoDoctoSunat + ".txt" NO-ERROR.
    pReturn = "OK".
...
IF NOT USERID("integral") = "MASTER" THEN DO:
    ...
    pReturn = "000|Escritura OK en el e-POS".
    IF lRC = FALSE OR ERROR-STATUS:ERROR = YES OR ERROR-STATUS:GET-MESSAGE(1) <> '' THEN DO:
      pReturn = "999|Imposible escribir en el e-POS(" + ERROR-STATUS:GET-MESSAGE(1) + ")". 
      SET-SIZE(mData) = 0.
    END.
...
/*
    codcia = 1 and codalm = '21' and tipmov = 'I' and codmov = 9 and nrodoc = 5819

      CcbCDocu.CodAlm = Almcmov.CodAlm = 21
      CcbCDocu.CodMov = Almcmov.CodMov = 9
      CcbCDocu.CodVen = Almcmov.CodVen
      CcbCDocu.NroPed = STRING(Almcmov.NroDoc) = 5819
*/
...
/* Ic-27Nov2018, si solo es Contigencia   */
IF x-ruta-file-contingencia <> "" THEN DO:
    COPY-LOB mDataClient TO FILE x-ruta-file-contingencia + "\" + cSerieSunat + "-" + cCorrelativoSunat + 
        "-" + cTipoDoctoSunat + ".txt" NO-ERROR.
    pReturn = "OK".
...
IF AVAILABLE tt-ePos THEN DO:
    ...
    FOR EACH tt-epos NO-LOCK BY tt-sec:
        ...
        */
        ...
        /*MESSAGE mIP_ePOS mNO_PORT mID_caja .*/
        ...
        IF lHost <> '' AND lPort <> '' THEN DO:
            ...
            IF hSocket:CONNECTED() = YES THEN DO:
                ...
                x-logtxt = flog-epos-txt("CONECTO EPOS (Conexion OK) : " + x-logmsg).
                /* -- LOG ---- */
...
DEFINE VAR z-filer AS CHAR.
  ...
  pRetVal = "998|Configuracion error en escritura del SOCKET".
  ...
  z-filer = flog-epos-txt("CONECTO EPOS (Configurar) :   Paso 1.5 - Data para el SOCKET DIV(" + cDivision + ")  Caja(" + mID_caja + ") DATA:" + lDataEnvio).
  ...
  IF lRC = FALSE OR ERROR-STATUS:ERROR = YES THEN DO:
      ...
      pRetVal = "001|No se pudo enviar la configuracion al e-POS (Revise el eGateway)".

      /*IF USERID("DICTDB") = 'MASTER' THEN pRetVal = pRetVal + " - " + STRING(ERROR-STATUS:GET-NUMBER(1)).   /*GET-MESSAGE(1).*/*/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fGet-Establecimiento Procedure 
FUNCTION fGet-Establecimiento RETURNS CHARACTER
...
FIND FIRST xy-gn-divi WHERE xy-gn-divi.codcia = s-codcia AND 
                            xy-gn-divi.coddiv = pCodDiv NO-LOCK NO-ERROR.
IF AVAILABLE xy-gn-divi THEN DO:
    ...
    if TRUE <> (x-codestable > "") THEN x-codestable = "0000".
...
RETURN x-codestable.   /* Function return value. */
...
FUNCTION fget-personalizados-pre-impreso RETURNS CHARACTER
  ...
  IF AVAILABLE zy-ccbcdocu THEN DO:
      ...
      /* Ic - 15Jun2018, Codigo de establecimiento */
      lRetVal = lRetVal + "PE|CodEstSUNAT|" + fGet-Establecimiento(zy-ccbcdocu.coddiv) + gcCRLF.
      ...
      ELSE DO:
          IF zy-ccbcdocu.coddoc = 'N/C' AND zy-ccbcdocu.codcli = "20100070970" THEN DO:
              ...
              lRetVal = lRetVal + "PESD|1|INCORP. AL REGIMEN DE AGENTE DE RETENCION DE IGV (RS: 265-2009) A PARTIR DEL 01/10/10"  + gcCRLF.
              IF lOrdenCompra <> "" THEN DO:
                  lRetVal = lRetVal + "PESD|2|RTV : " + lOrdenCompra  + gcCRLF.
              END.              
          END.
      END.
  END.
...
IF AVAILABLE xy-ccbcdocu THEN DO:
    ...
    FIND FIRST xy-gn-clie WHERE xy-gn-clie.codcia = s-codcia AND 
                                    ...
                                    NO-LOCK NO-ERROR.
    IF AVAILABLE xy-gn-clie AND xy-gn-clie.libre_c01 = 'E' THEN x-retval = '03'.
