...
&IF INDEX("{&CabeceraTempo}", "Faccpedi") > 0 &THEN
    IF {&CabeceraTempo}.CodDiv = "00506" /*OR cPlataforma = "RIQRA"*/ THEN DO:
        importeDescuentoGlobal = {&CabeceraTempo}.porcent[1].
        /* OJO: esto pasa al momento que migran los datos de Open a Progress y
                ANTES de registrar el FLETE en Progress */
        IF {&CabeceraTempo}.CodDiv = "00506" THEN Local_Delta = {&CabeceraTempo}.ImpTot - importeDescuentoGlobal - {&CabeceraTempo}.TotalVenta.
...
&IF INDEX("{&CabeceraTempo}", "Ccbcdocu") > 0 &THEN
    IF ({&CabeceraTempo}.DivOri = "00506" /*OR cPlataforma = "RIQRA"*/) 
        ...
        THEN DO:
        importeDescuentoGlobal = {&CabeceraTempo}.impint.
        IF {&CabeceraTempo}.CodDiv = "00506" THEN Local_Delta = {&CabeceraTempo}.ImpTot - importeDescuentoGlobal - {&CabeceraTempo}.TotalVenta.
...
/* ************************************************************************* */
    ...
    &IF INDEX("{&CabeceraTempo}", "Ccbcdocu") > 0  &THEN
        ...
        IF LOOKUP({&CabeceraTempo}.FmaPgo, "899,900") > 0 OR {&CabeceraTempo}.FlgEst = "C" THEN
            ASSIGN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE descuentos-globales Include 
PROCEDURE descuentos-globales :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE VAR importeDescuento AS DEC INIT 0.

/* Enero2025 - Descuentos Globales */
&IF INDEX("{&CabeceraTempo}", "Faccpedi") > 0  &THEN
    importeDescuento = {&CabeceraTempo}.porcent[1].
&ENDIF  

&IF INDEX("{&CabeceraTempo}", "Ccbcdocu") > 0  &THEN
    importeDescuento = {&CabeceraTempo}.impint.
&ENDIF

IF importeDescuento <= 0 THEN RETURN "OK".

x-montoBaseDsctoGlobal = {&CabeceraTempo}.totalValorVentaNetoOpGravadas.
x-descuentosGlobales = importeDescuento / (1 + x-TasaIGV ).
x-porcentajeDescuentoGlobal = x-descuentosGlobales / x-montoBaseDsctoGlobal.

{&CabeceraTempo}.montoBaseDescuentoGlobal = x-montoBaseDsctoGlobal.
{&CabeceraTempo}.porcentajeDsctoGlobal = x-porcentajeDescuentoGlobal.
{&CabeceraTempo}.descuentosGlobales = x-descuentosGlobales.
{&CabeceraTempo}.totalValorVentaNetoOpGravadas = x-montoBaseDsctoGlobal - x-descuentosGlobales.
{&CabeceraTempo}.totalIgv = {&CabeceraTempo}.totalValorVentaNetoOpGravadas * x-TasaIGV.
{&CabeceraTempo}.totalImpuestos = {&CabeceraTempo}.totalIgv.                        /* Cuando haya ICBPER - bolsas plasticas corroborar */
{&CabeceraTempo}.totalValorVenta = {&CabeceraTempo}.totalValorVentaNetoOpGravadas.
{&CabeceraTempo}.totalPrecioVenta = {&CabeceraTempo}.totalValorVentaNetoOpGravadas + {&CabeceraTempo}.totalIgv.
