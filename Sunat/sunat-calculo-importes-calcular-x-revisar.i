...
/* Contamos registros */
DEF VAR LocalContador AS INTE INIT 0 NO-UNDO.
FOR EACH {&DetalleX} OF {&CabeceraX} NO-LOCK:
    LocalContador = LocalContador + 1.
    IF LocalContador > 1 THEN LEAVE.
END.
FOR EACH {&DetalleX} OF {&CabeceraX} NO-LOCK:
    ...
    END.
    ELSE DO:
        ASSIGN {&DetalleTempo}.cPreUniSinImpuesto = ROUND({&DetalleTempo}.preuni / (1 + x-TasaIGV),4).
    END.
    ...
    IF {&DetalleTempo}.cTipoAfectacion <> "GRATUITA" THEN DO:
        ASSIGN {&DetalleTempo}.cImporteTotalConImpuesto = {&DetalleTempo}.MontoBaseIGV + {&DetalleTempo}.ImporteIGV.
    END.
    /* Bolsa Plastica */
    ...
    END.
    IF LocalContador = 1 THEN DO:
        IF {&DetalleTempo}.cTipoAfectacion = "GRAVADA" AND /*{&DetalleTempo}.CanDes = 1 AND*/
            {&DetalleTempo}.FactorDescuento = 0 AND {&DetalleTempo}.ImporteReferencial = 0 AND
            {&DetalleTempo}.ImporteBaseDescuento = 0 AND {&DetalleTempo}.ImporteDescuento = 0 AND
            {&DetalleTempo}.cImporteVentaExonerado = 0 AND {&DetalleTempo}.cImporteVentaGratuito = 0 AND
            {&DetalleTempo}.cImporteTotalConImpuesto <> {&DetalleTempo}.ImpLin AND 
            ABS( {&DetalleTempo}.cImporteTotalConImpuesto - {&DetalleTempo}.ImpLin ) <= 0.01
            THEN DO:
            {&DetalleTempo}.cPreUniSinImpuesto = {&DetalleTempo}.ImpLin - {&DetalleTempo}.ImpIgv.
            {&DetalleTempo}.ImporteUnitarioSinImpuesto = {&DetalleTempo}.cPreUniSinImpuesto.
            {&DetalleTempo}.ImporteTotalSinImpuesto = {&DetalleTempo}.cPreUniSinImpuesto.
            {&DetalleTempo}.MontoBaseIGV = {&DetalleTempo}.ImporteTotalSinImpuesto.
            {&DetalleTempo}.ImporteIGV = {&DetalleTempo}.ImpIgv.
            {&DetalleTempo}.ImporteTotalImpuestos = {&DetalleTempo}.ImpIgv.
            /*
            {&DetalleTempo}.ImporteUnitarioConImpuesto = {&DetalleTempo}.ImpLin.
            {&DetalleTempo}.cSumaImpteTotalSinImpuesto = {&DetalleTempo}.cPreUniSinImpuesto.
            */
            {&DetalleTempo}.cMontoBaseIGV = {&DetalleTempo}.cSumaImpteTotalSinImpuesto.
            {&DetalleTempo}.cSumaIGV = {&DetalleTempo}.ImpIgv.
            {&DetalleTempo}.cImporteTotalConImpuesto = {&DetalleTempo}.ImpLin.
        END.
    END.
    IF LOOKUP({&DetalleTempo}.cTipoAfectacion,"EXONERADA,GRATUITA") = 0 THEN DO:
...
/* ************************************************************************* */
/* &IF INDEX("{&CabeceraTempo}", "Ccbcdocu") > 0 &THEN                                  */
/*     /* Caso de aplicación de A/C a la factura en forma automática: NO CONTEMPLADO */ */
/*     IF {&CabeceraTempo}.ImpTot2 > 0 THEN RETURN "OK".                                */
/* &ENDIF                                                                               */
/* ************************************************************************* */
/* ************************************************************************* */
...
/* ************************************************************************* */
/*
    TEMPORALMENTE COMENTADO HASTA CUANDO OFICIALMENTE SE PONGA EN PRODUCTIVO LA 
    ARIMETICA DE SUNAT
    */
    ASSIGN
