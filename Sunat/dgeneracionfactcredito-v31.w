...
/* Lógica Principal:
1ra. Nota de Crédito:
    Se genera por cada A/C por el monto que se ha aplicado a la factura
    La N/C hace referencia a la FAC del A/C
    La N/C disminuirá el saldo del A/C
    La N/C amortiza la factura inicial
    Concepto 00014
    
2da. Nota de Crédito:    
    Se genera por Descuento Pronto Pago
    Se basa en la Fecha de la BD el cual generó el A/C
    Se hace por cada N/C que se generado en el paso 1
    Concepto 00001
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    FOR EACH ANTICIPOS EXCLUSIVE-LOCK WHERE ANTICIPOS.codcia = Ccbcdocu.codcia
        ...
        /* ***************************************************************************************** */
        /* Buscamos la fecha de pago */
        DEF VAR x-FchPago AS DATE NO-UNDO.
        DEF BUFFER B-CDOCU FOR Ccbcdocu.
        x-FchPago = ANTICIPOS.FchDoc.    /* Valor por defecto */

        FIND B-CDOCU WHERE B-CDOCU.codcia = s-codcia
            AND B-CDOCU.coddoc = ANTICIPOS.codref
            AND B-CDOCU.nrodoc = ANTICIPOS.nroref
            NO-LOCK NO-ERROR.
        IF AVAILABLE B-CDOCU THEN DO:
            ...
            IF AVAILABLE Ccbdcaja THEN DO:
                ...
                IF AVAILABLE Ccbccaja AND Ccbccaja.Voucher[5] > '' THEN DO:
                    x-FchPago = Ccbccaja.FchDoc.
                    FIND B-CDOCU WHERE B-CDOCU.codcia = s-codcia
                        AND B-CDOCU.coddoc = 'BD'
                        AND B-CDOCU.nrodoc = Ccbccaja.Voucher[5]
                        NO-LOCK NO-ERROR.
                    IF AVAILABLE B-CDOCU THEN x-FchPago = B-CDOCU.FchAte.
                END.
            END.
        END.
        /*MESSAGE 'fecha de pago' x-fchpago SKIP 'lista precios' COTIZACION.Libre_c01.*/
        DESCUENTO:
        FOR EACH CcbTabla NO-LOCK WHERE CcbTabla.CodCia = s-codcia
            AND CcbTabla.Tabla = "%AC"
            AND CcbTabla.Codigo = COTIZACION.Libre_c01      /* Lista de Precios */
            AND CcbTabla.Libre_d01 > 0:
            IF (CcbTabla.Libre_f01 = ? AND CcbTabla.Libre_f02 = ?) OR
                (CcbTabla.Libre_f01 = ? AND CcbTabla.Libre_f02 <> ? AND x-FchPago <= CcbTabla.Libre_f02) OR
                (CcbTabla.Libre_f01 <> ? AND CcbTabla.Libre_f02 = ? AND x-FchPago >= CcbTabla.Libre_f01) OR
                (CcbTabla.Libre_f01 <> ? AND CcbTabla.Libre_f02 <> ? AND x-FchPago >= CcbTabla.Libre_f01 AND x-FchPago <= CcbTabla.Libre_f02)
                THEN DO:
                RUN Genera-NC (
                    "00001",        /* Descuento pronto pago */
                    ROUND(x-Monto-Aplicar * CcbTabla.Libre_d01 / 100, 2),
                    "DESCUENTO",
                    x-TpoCmb-Compra,
                    x-TpoCmb-Venta,
                    OUTPUT pMensaje
                ...
                IF RETURN-VALUE = "ADM-ERROR" THEN DO:
                    IF TRUE <> (pMensaje > '') THEN pMensaje = "NO se pudieron generar las N/C x Descuento Pronto Pago".
                    UNDO RLOOP, LEAVE RLOOP.
                END.
                LEAVE DESCUENTO.
            END.
...
------------------------------------------------------------------------------*/
    ...
    DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
        ...
        IF AVAILABLE b-faccpedi THEN DO:
            ...
            /* RHC 20/02/2017 Que genere guia si desean */
            /*
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    {lib\lock-genericov21.i &Tabla="FacCPedi" ~
        ...
        &Mensaje="YES" ~
        &TipoError="DO: ~
        pMensaje = 'O/D en uso por otro usuario' + CHR(10) + 'Proceso Abortado'. ~
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        IF pMensaje = "" THEN pMensaje = "NO se pudo generar la Devolución Automática Lista Express".
        UNDO RLOOP, RETURN 'ADM-ERROR'.
    END.
...
/* MESSAGE 'nota de credito' SKIP   */
/*     'concepto' pconcepto SKIP    */
/*     'monto' x-monto-aplicar SKIP */
/*     'tipo' ptpofac               */
/*     VIEW-AS ALERT-BOX WARNING.   */
RLOOP:
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    BUFFER-COPY NCREDITO
        ...
        NO-ERROR.
    IF ERROR-STATUS:ERROR THEN DO:
        pMensaje = "NO se pudo generar el detalle de la N/C por Anticipos".
        UNDO, LEAVE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    DO TRANSACTION ON ERROR UNDO trloop, RETURN 'ADM-ERROR' ON STOP UNDO trloop, RETURN 'ADM-ERROR':
        ...
        FOR EACH T-CDOCU WHERE LOOKUP(T-CDOCU.CodDoc, 'FAC,BOL') > 0, 
            FIRST B-CDOCU OF T-CDOCU:
            CASE cboFormaPago :
                WHEN "Efectivo" THEN Ccbccaja.ImpNac[1] = Ccbccaja.ImpNac[1] + B-CDOCU.ImpTot.
                WHEN "Tarjeta"  THEN Ccbccaja.ImpNac[4] = Ccbccaja.ImpNac[4] + B-CDOCU.ImpTot.
                WHEN "Deposito Bancario" THEN Ccbccaja.ImpNac[5] = Ccbccaja.ImpNac[5] + B-CDOCU.ImpTot.
            END CASE.
            CREATE Ccbdcaja.
            ASSIGN
...
IF s-user-id = 'ADMIN' THEN RETURN 'OK'.    /* SISTEMAS no genera comprobantes */
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH Reporte NO-LOCK, FIRST CcbCDocu OF Reporte NO-LOCK:
      ...
      IF Ccbcdocu.CodDoc = "N/C" THEN LEAVE.
      /* ************************************************************** */
