...
CREATE FELogErrores.
ASSIGN
    FELogErrores.CodCia = B-CDOCU.codcia
    FELogErrores.CodDiv = B-CDOCU.coddiv
    FELogErrores.CodDoc = B-CDOCU.coddoc
    FELogErrores.NroDoc = B-CDOCU.nrodoc
    FELogErrores.CodHash = cCodHash
    FELogErrores.ErrorDate = NOW
    FELogErrores.EstadoPPLL = iEstadoPPLL
    FELogErrores.FlagPPLL = (IF SUBSTRING(cRetVal,1,3) = "000" THEN 1 ELSE iFlagPPLL)
    FELogErrores.ID_Pos = cID_Pos
    FELogErrores.IP_ePos = cIP_ePos
    FELogErrores.LogDate = NOW
    FELogErrores.LogEstado = pCodError      /*cRetVal*/
    FELogErrores.LogUser = s-user-id
    FELogErrores.NumDocumento = cNumDocumento
    NO-ERROR.
