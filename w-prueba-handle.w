...
DEFINE VARIABLE h_b-prueba-handle AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 3.69 COL 73 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 92.14 BY 18.92 WIDGET-ID 100.
...
&ANALYZE-RESUME
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _XFTR "SmartWindowCues" sW-Win _INLINE
...
ON END-ERROR OF sW-Win /* <insert SmartWindow title> */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL sW-Win sW-Win
ON WINDOW-CLOSE OF sW-Win /* <insert SmartWindow title> */
DO:
...
DO:
  DEFINE VAR htAlmacen AS HANDLE.

  EMPTY TEMP-TABLE ttAlmacen.
  FOR EACH Almacen NO-LOCK WHERE Almacen.codcia = 1 AND Almacen.codalm BEGINS '2':
      CREATE ttAlmacen.
      BUFFER-COPY Almacen TO ttAlmacen.
  END.
  
  htAlmacen = TEMP-TABLE ttAlmacen:HANDLE.
  RUN Captura-Temporal IN h_b-prueba-handle ( INPUT TABLE-HANDLE htAlmacen).
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/b-prueba-handle.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-prueba-handle ).
       RUN set-position IN h_b-prueba-handle ( 6.65 , 10.00 ) NO-ERROR.
       RUN set-size IN h_b-prueba-handle ( 6.69 , 66.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-prueba-handle ,
