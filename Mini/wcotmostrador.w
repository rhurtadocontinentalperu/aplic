...
DEFINE VARIABLE h_tcotmostrador AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-BUSCAR AT ROW 1 COL 32 WIDGET-ID 4
     BtnDone AT ROW 1 COL 52 WIDGET-ID 8
     FILL-IN-NroPed AT ROW 19.27 COL 36 COLON-ALIGNED NO-LABEL WIDGET-ID 10
...
DO:
    ...
    END.
    RUN dispatch IN h_tcotmostrador ('open-query':U).
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/mini/tcotmostrador.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tcotmostrador ).
       RUN set-position IN h_tcotmostrador ( 2.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tcotmostrador ( 16.58 , 53.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_tcotmostrador. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tcotmostrador ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tcotmostrador ,
             BtnDone:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tcotmostrador , 'AFTER':U ).
    END. /* Page 1 */
...
CASE l-Handle:
    WHEN "Add-Record" THEN DO:
        RUN dispatch IN h_tcotmostrador ('add-record':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         IF h_tcotmostrador <> ? THEN RUN Recalcular-Precios IN h_tcotmostrador.
         IF h_tcotmostrador <> ? THEN RUN dispatch IN h_tcotmostrador ('open-query':U). 
         /* Cambia color */
         ASSIGN
             FILL-IN-NroPed:BGCOLOR = 10 
             FILL-IN-NroPed:FGCOLOR = 1.
