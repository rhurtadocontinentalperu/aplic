...
IF NOT AVAILABLE Almcinv THEN DO:
    MESSAGE 'No se puede hacer el 2do. conteo' SKIP
        'No se ha configurado el inventario, no se ha hecho el primer conteo o ya se hizo el conteo'
...
FOR EACH T-DINV BREAK BY T-DINV.CodMat
    ...
    END.
    ASSIGN
        Almdinv.QtyReconteo = Almdinv.QtyReconteo + T-DINV.QtyReconteo
        Almdinv.Libre_d01 = Almdinv.QtyReconteo
        Almdinv.CodUserRec = s-user-id
        Almdinv.FecRec = DATETIME(TODAY,MTIME).
    IF LAST-OF(T-DINV.CodMat) THEN DO:
        IF (Almdinv.QtyReconteo - AlmDInv.QtyFisico) <> 0 THEN lDif = YES.
        FIND FIRST AlmCInv WHERE AlmCInv.CodCia = s-CodCia
            AND AlmCInv.CodAlm  = s-codalm
            AND AlmcInv.NomCia  = "CONTI"
