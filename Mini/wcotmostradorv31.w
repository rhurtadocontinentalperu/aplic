...
BtnDone FILL-IN-NroPed BUTTON-1 BUTTON-3 BUTTON-4 
...
DEFINE BUTTON BUTTON-1 
     IMAGE-UP FILE "img/insert.ico":U
     LABEL "ADICIONAR" 
     SIZE 11 BY 2.69.
...
DEFINE FRAME F-Main
     ...
     FILL-IN-NroPed AT ROW 1 COL 57 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     BUTTON-1 AT ROW 4.65 COL 72 WIDGET-ID 12
     BUTTON-3 AT ROW 7.35 COL 72 WIDGET-ID 14
     BUTTON-4 AT ROW 9.65 COL 72 WIDGET-ID 16
     BUTTON-5 AT ROW 12.15 COL 72 WIDGET-ID 18
...
ON CHOOSE OF BUTTON-1 IN FRAME F-Main /* ADICIONAR */
DO:
...
DO:
   RUN dispatch IN h_tcotcredito ('cancel-record':U).
   RUN dispatch IN h_tcotcredito ('disable-fields':U).
END.
...
ON CHOOSE OF BUTTON-4 IN FRAME F-Main /* BORRAR */
DO:
   RUN dispatch IN h_tcotcredito ('delete-record':U).
END.
...
DO:
  RUN Apply-Tab IN h_tcotcredito. 
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'mini/tcotmostradorv31.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tcotcredito ).
       RUN set-position IN h_tcotcredito ( 2.54 , 1.57 ) NO-ERROR.
       RUN set-size IN h_tcotcredito ( 20.19 , 70.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  /*RUN Cambia-Font IN h_p-updv12 ( INPUT 10).*/
  RUN dispatch IN h_tcotcredito ('disable-fields':U).
