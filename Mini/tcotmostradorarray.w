...
FOR EACH VtaDTabla NO-LOCK WHERE VtaDTabla.CodCia = s-codcia
    ...
    AND VtaDTabla.Llave = pGrupo,
    FIRST Almmmatg NO-LOCK WHERE Almmmatg.CodCia =  VtaDTabla.CodCia 
    AND Almmmatg.codmat = VtaDTabla.Tipo:
    CREATE PEDI.
    ASSIGN
        PEDI.codcia = s-codcia
        PEDI.CodMatWeb = VtaDTabla.Tipo
        PEDI.NroItm = VtaDTabla.Libre_d01
        PEDI.DesMatWeb = VtaDTabla.Libre_c01
    ...
    FIND FIRST ITEM WHERE ITEM.codmat = PEDI.CodMatWeb
        ...
        NO-LOCK NO-ERROR.
    IF AVAILABLE ITEM THEN ASSIGN PEDI.CanPed = ITEM.CanPed PEDI.Libre_c03 = ITEM.Libre_c03.
    ASSIGN 
        F-FACTOR = 1
  ...
  IF s-FlgMinVenta = YES THEN DO:
      ...
      IF s-VentaMayorista = 2 THEN DO:  /* LISTA POR DIVISION */
          IF Almmmatg.StkMax > 0 THEN DO:
              ...
              IF f-CanPed <> DECIMAL(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&browse-name}) * f-Factor THEN DO:
                  MESSAGE 'Solo puede vender en múltiplos de' Almmmatg.StkMax Almmmatg.UndBas
                      VIEW-AS ALERT-BOX ERROR.
                  APPLY 'ENTRY':U TO PEDI.CanPed.
                  RETURN "ADM-ERROR".
              END.
