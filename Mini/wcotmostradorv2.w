...
DEFINE VARIABLE h_tcotmostradorv2 AS HANDLE NO-UNDO.
...
DO:
    ...
    END.
    RUN dispatch IN h_tcotmostradorv2 ('open-query':U).
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'mini/tcotmostradorv2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tcotmostradorv2 ).
       RUN set-position IN h_tcotmostradorv2 ( 2.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tcotmostradorv2 ( 16.73 , 63.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_tcotmostradorv2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tcotmostradorv2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tcotmostradorv2 ,
             BtnDone:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tcotmostradorv2 , 'AFTER':U ).
    END. /* Page 1 */
...
CASE l-Handle:
    WHEN "Add-Record" THEN DO:
        RUN dispatch IN h_tcotmostradorv2 ('add-record':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         IF h_tcotmostradorv2 <> ? THEN RUN Recalcular-Precios IN h_tcotmostradorv2.
         IF h_tcotmostradorv2 <> ? THEN RUN dispatch IN h_tcotmostradorv2 ('open-query':U). 
         /* Cambia color */
         ASSIGN
             FILL-IN-NroPed:BGCOLOR = 10 
             FILL-IN-NroPed:FGCOLOR = 1.
