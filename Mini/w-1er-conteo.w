...
DEFINE FRAME F-Main
     COMBO-BOX-CodUbi AT ROW 1.19 COL 21 COLON-ALIGNED WIDGET-ID 8
     FILL-IN-CodMat AT ROW 2.73 COL 21 COLON-ALIGNED WIDGET-ID 12 AUTO-RETURN 
     FILL-IN-UndBas AT ROW 2.73 COL 35 COLON-ALIGNED NO-LABEL WIDGET-ID 26
     EDITOR-DesMat AT ROW 4.27 COL 3 NO-LABEL WIDGET-ID 10
     FILL-IN-Factor AT ROW 8.31 COL 28 COLON-ALIGNED WIDGET-ID 22
     FILL-IN-Qty AT ROW 9.65 COL 28 COLON-ALIGNED WIDGET-ID 24
     FILL-IN-Cantidad AT ROW 11 COL 28 COLON-ALIGNED WIDGET-ID 20
...
FOR EACH T-DINV BREAK BY T-DINV.CodMat
    ...
    END.
    ASSIGN
        Almdinv.QtyConteo = Almdinv.QtyConteo + T-DINV.QtyConteo
        Almdinv.Libre_d01 = Almdinv.QtyConteo
        Almdinv.CodUserCon = s-user-id
        Almdinv.FecCon = DATETIME(TODAY,MTIME).
    IF LAST-OF(T-DINV.CodMat) THEN DO:
        IF (AlmDInv.QtyConteo - AlmDInv.QtyFisico) <> 0 THEN lDif = YES.
        FIND FIRST AlmCInv WHERE AlmCInv.CodCia = s-CodCia
            AND AlmCInv.CodAlm  = s-codalm
            AND AlmcInv.NomCia  = "CONTI"
