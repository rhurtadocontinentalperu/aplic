...
IF NOT AVAILABLE InvZonas THEN DO:
    MESSAGE 'No se puede hacer el 1er. conteo' SKIP
...
IF RETURN-VALUE = 'ADM-ERROR' OR pCodInventariador = "" THEN RETURN ERROR.
...
btnOk ChkbxAutinc BtnDone btnCancelar btnBrowse RECT-1 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-CodUbi FILL-IN-CodMat ~
FILL-IN-Qty FILL-IN-Cantidad ChkbxAutinc txtOldUbi txtOldDigitado txtOldArt ~
txtOldFactor txtOldCantidad txtOldTotal FILL-IN-UndBas EDITOR-DesMat ~
FILL-IN-Marca FILL-IN-Factor FILL-IN-UbiAlm 
...
DEFINE VARIABLE ChkbxAutinc AS LOGICAL INITIAL no 
     LABEL ">>" 
     VIEW-AS TOGGLE-BOX
     SIZE 7.14 BY .77
     FGCOLOR 12  NO-UNDO.
...
DEFINE FRAME F-Main
     COMBO-BOX-CodUbi AT ROW 4.69 COL 3.86 WIDGET-ID 28
     FILL-IN-CodMat AT ROW 6 COL 19.43 COLON-ALIGNED WIDGET-ID 12 AUTO-RETURN 
     FILL-IN-Qty AT ROW 15.27 COL 25 COLON-ALIGNED WIDGET-ID 24
     FILL-IN-Cantidad AT ROW 16.62 COL 25 COLON-ALIGNED WIDGET-ID 20
     btnOk AT ROW 19.12 COL 2 WIDGET-ID 76
     ChkbxAutinc AT ROW 15.65 COL 2.14 WIDGET-ID 96
     txtOldUbi AT ROW 1.27 COL 7.29 COLON-ALIGNED NO-LABEL WIDGET-ID 52
     txtOldDigitado AT ROW 1.27 COL 24.86 COLON-ALIGNED NO-LABEL WIDGET-ID 74
     txtOldArt AT ROW 2.27 COL 4.86 COLON-ALIGNED NO-LABEL WIDGET-ID 56
     txtOldFactor AT ROW 3.42 COL 6.72 COLON-ALIGNED NO-LABEL WIDGET-ID 62
     txtOldCantidad AT ROW 3.42 COL 23.57 COLON-ALIGNED NO-LABEL WIDGET-ID 66
     txtOldTotal AT ROW 3.42 COL 40.29 COLON-ALIGNED NO-LABEL WIDGET-ID 70
     FILL-IN-UndBas AT ROW 6.04 COL 46.57 COLON-ALIGNED NO-LABEL WIDGET-ID 26
     EDITOR-DesMat AT ROW 7.46 COL 2 NO-LABEL WIDGET-ID 10
     BUTTON-1 AT ROW 9.08 COL 40 WIDGET-ID 36
     FILL-IN-Marca AT ROW 11 COL 17.29 COLON-ALIGNED WIDGET-ID 30
     FILL-IN-Factor AT ROW 13.88 COL 25 COLON-ALIGNED WIDGET-ID 22
     BUTTON-Grabar AT ROW 18.73 COL 35.57 WIDGET-ID 16
     BtnDone AT ROW 18.73 COL 46.72 WIDGET-ID 18
     btnCancelar AT ROW 19.12 COL 9.57 WIDGET-ID 78
     FILL-IN-UbiAlm AT ROW 12.38 COL 4.43 WIDGET-ID 34
     btnBrowse AT ROW 19.08 COL 23 WIDGET-ID 94
     "Dig." VIEW-AS TEXT
          SIZE 5.72 BY .88 AT ROW 1.27 COL 21.29 WIDGET-ID 72
          FONT 11
     "Ubic" VIEW-AS TEXT
          SIZE 7.14 BY .77 AT ROW 1.31 COL 2 WIDGET-ID 54
          FONT 11
     "Art.:" VIEW-AS TEXT
          SIZE 4 BY .77 AT ROW 2.35 COL 1.72 WIDGET-ID 58
          FONT 11
     "Fact." VIEW-AS TEXT
          SIZE 7.14 BY .77 AT ROW 3.54 COL 1.72 WIDGET-ID 60
          FONT 11
     "Cant.:" VIEW-AS TEXT
          SIZE 7 BY .77 AT ROW 3.54 COL 19 WIDGET-ID 64
          FONT 11
     "Tot.:" VIEW-AS TEXT
          SIZE 5.43 BY .77 AT ROW 3.54 COL 36.57 WIDGET-ID 68
          FONT 11
     RECT-1 AT ROW 1.04 COL 1.14 WIDGET-ID 38
...
DO:
    ...
    IF chkbxAutinc = NO THEN DO:
        FILL-IN-cantidad:SCREEN-VALUE = "0.00" .
    END.
      
    APPLY 'ENTRY':U TO FILL-IN-CodMat.
    IF chkbxAutinc = YES THEN DO:
        APPLY 'GO':U TO FILL-IN-CodMat.
    END.
    /*RETURN NO-APPLY.*/
...
&Scoped-define SELF-NAME ChkbxAutinc
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ChkbxAutinc W-Win
ON VALUE-CHANGED OF ChkbxAutinc IN FRAME F-Main /* >> */
DO:

    ASSIGN ChkbxAutinc.
        IF ChkbxAutinc = YES THEN DO:
          FILL-IN-qty:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "1.00".
          FILL-IN-qty:SENSITIVE = NO.
        END.
        ELSE DO: 
          FILL-IN-qty:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "0.00".
          FILL-IN-qty:SENSITIVE = YES.
        END.
END.
...
DO:
  ...
  FIND InvZonas WHERE InvZonas.CodCia = s-codcia
      ...
      NO-LOCK NO-ERROR.
  IF NOT AVAILABLE InvZonas THEN DO:
      MESSAGE 'Zona YA esta procesada' VIEW-AS ALERT-BOX ERROR.
      SELF:SCREEN-VALUE = ''.
      RETURN NO-APPLY.
  END.
  ...
  IF AVAILABLE b-invzonas THEN DO:
      ASSIGN b-invzonas.FHIniConteo = NOW
            b-invzonas.UsrConteo = pCodInventariador.
      pSeleUbi = YES.
  END.
...
DO:
    ...
    IF AVAILABLE Almmmate THEN FILL-IN-UbiAlm:SCREEN-VALUE = Almmmate.CodUbi.
      ...
      FILL-IN-cantidad:SCREEN-VALUE = 
      STRING(DECIMAL(FILL-IN-Factor:SCREEN-VALUE) * DECIMAL(FILL-IN-Qty:SCREEN-VALUE),">>>,>>9.99") .
...
ON 'RETURN':U OF COMBO-BOX-codubi, FILL-IN-codmat, FILL-IN-qty IN FRAME {&FRAME-NAME} DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY COMBO-BOX-CodUbi FILL-IN-CodMat FILL-IN-Qty FILL-IN-Cantidad 
          ChkbxAutinc txtOldUbi txtOldDigitado txtOldArt txtOldFactor 
          txtOldCantidad txtOldTotal FILL-IN-UndBas EDITOR-DesMat FILL-IN-Marca 
          FILL-IN-Factor FILL-IN-UbiAlm 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE COMBO-BOX-CodUbi FILL-IN-CodMat FILL-IN-Qty btnOk ChkbxAutinc BtnDone 
         btnCancelar btnBrowse RECT-1 
      WITH FRAME F-Main IN WINDOW W-Win.
...
DEFINE VARIABLE iSConteo AS INTE INIT 1 NO-UNDO.    /* 2do Conteo */
...
DO TRANSACTION ON ERROR UNDO, RETURN ERROR ON STOP UNDO, RETURN ERROR:
    ...
    FOR EACH InvdPDA NO-LOCK WHERE InvdPDA.CodCia = s-codcia
        ...
        IF invCargaInicial.usrInventario[1] = ? OR 
                invCargaInicial.usrInventario[1] = ''  
                THEN ASSIGN invCargaInicial.usrInventario[1] = pCodInventariador.
    ...
    FIND InvZonas WHERE InvZonas.CodCia = s-codcia
        ...
        EXCLUSIVE-LOCK NO-ERROR.
    IF AVAILABLE InvZonas THEN DO:
        ASSIGN InvZonas.SCont1 = YES
                invZonas.FHFINConteo = NOW.
    END.
...
------------------------------------------------------------------------------*/
  ...
  /*txtAutoinc:VISIBLE IN FRAME {&FRAME-NAME} = NO.*/
