...
DEF NEW SHARED VARIABLE pCodDiv   AS CHAR.       /* División de Precios */
DEF NEW SHARED VAR s-tpoped AS CHAR INIT 'N'.
...
ASSIGN
    ...
    s-FlgSit = ''
    s-CodMon = 1        /* Soles */
    s-CodDoc = "PPX"    /* Pre-Cotización Móvil */
    s-NroDec = 4
...
/* RHC 28/12/2016 Solicitamos el Promotor */
...
ASSIGN
    ...
    input-var-2 = "51135890"        /* Standford */
    input-var-3 = ""
    output-var-1 = ?.
RUN lkup/c-promotor ("Seleccione el Promotor").
IF output-var-1 = ? THEN RETURN ERROR.

DEF NEW SHARED VAR s-promotor AS CHAR.
s-Promotor = output-var-3.    /* Proveedor|Promotor|Equipo (al grabar completamos |Familia) */
...
DEFINE VARIABLE h_tcotmostradorv3 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE FILL-IN-NroPed AS CHARACTER FORMAT "X(256)":U INITIAL "NUEVO" 
     VIEW-AS FILL-IN 
     SIZE 21 BY 1.35
     BGCOLOR 10 FGCOLOR 1 FONT 11 NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     FILL-IN-NroPed AT ROW 1 COL 49 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     BtnDone AT ROW 1 COL 72 WIDGET-ID 8
...
DO:
    ...
    IF NOT AVAILABLE Faccpedi THEN DO:
        ...
        RETURN NO-APPLY.
    END.
    IF LOOKUP(Faccpedi.flgest, 'I,P') = 0 THEN DO:
        MESSAGE 'El pedido se encuentra en digitación' VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    ...
    END.
    RUN dispatch IN h_tcotmostradorv3 ('open-query':U).
...
DO:
  ...
  IF AVAILABLE ITEM THEN DO:
      MESSAGE 'Existe información digitada' SKIP
          'Desea perder todo lo ingresado?'
...
ASSIGN {&WINDOW-NAME}:TITLE = "VENTAS - " + s-Promotor.
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'mini/tcotmostradorv3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tcotmostradorv3 ).
       RUN set-position IN h_tcotmostradorv3 ( 2.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tcotmostradorv3 ( 16.73 , 76.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm-vm/objects/p-updv12g.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
       ...
       RUN set-size IN h_p-updv12 ( 1.42 , 70.00 ) NO-ERROR.

       /* Links to SmartBrowser h_tcotmostradorv3. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tcotmostradorv3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tcotmostradorv3 ,
             BtnDone:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tcotmostradorv3 , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-NUEVA BUTTON-GRABAR BUTTON-BUSCAR FILL-IN-NroPed BtnDone 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ITEM WHERE ITEM.CanPed > 0, FIRST Almmmatg OF ITEM NO-LOCK BY ITEM.NroItm:
      I-NITEM = I-NITEM + 1.
      CREATE Facdpedi.
      BUFFER-COPY ITEM 
          EXCEPT /*ITEM.Libre_c05*/ ITEM.CodMatWeb ITEM.DesMatWeb
          TO Facdpedi
          ASSIGN
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
  ...
  IF s-adm-new-record = 'YES' THEN DO:
      ...
      ASSIGN
          FacCPedi.CodCli = s-NroPed    /*s-CodCli*/
...
CASE l-Handle:
    WHEN "Add-Record" THEN DO:
        RUN dispatch IN h_tcotmostradorv3 ('add-record':U).
      END.
    WHEN "Update-Record" THEN DO:
        /*RUN dispatch IN h_tcotmostradorv3 ('update-record':U).*/
        RUN new-state IN h_p-updv12 ('update-begin':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         IF h_tcotmostradorv3 <> ? THEN RUN Recalcular-Precios IN h_tcotmostradorv3.
         IF h_tcotmostradorv3 <> ? THEN RUN dispatch IN h_tcotmostradorv3 ('open-query':U). 
         /* Cambia color */
         ASSIGN
             FILL-IN-NroPed:BGCOLOR = 10 
             FILL-IN-NroPed:FGCOLOR = 1.
