...
DEFINE VARIABLE h_blistacontrmarco AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-option AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_qlistacontrmarco AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vlistacontrmarco AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "LISTA DE PRECIOS CONTRATO MARCO"
         COLUMN             = 16.43
         ROW                = 7.15
         HEIGHT             = 17.54
         WIDTH              = 112.57
...
ON END-ERROR OF W-Win /* LISTA DE PRECIOS CONTRATO MARCO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* LISTA DE PRECIOS CONTRATO MARCO */
DO:
    ...
    CASE choice:
      WHEN TRUE THEN /* Yes */
         DO:
            ...
            RUN Activar-lista IN h_blistacontrmarco.
            RUN dispatch IN h_vlistacontrmarco ('display-fields':U).
         END.
      WHEN FALSE THEN /* No */
       DO:
          RETURN.
...
DO:
  ...
  RUN Excel IN h_blistacontrmarco.
END.
...
DO:
  RUN Importar-Excel IN h_vlistacontrmarco.
END.
...
DO:
  ASSIGN CB-TpoBie.
  RUN Tipo-Bien IN h_blistacontrmarco (LOOKUP(CB-TpoBie,CB-TpoBie:LIST-ITEMS)).
  RUN dispatch IN h_blistacontrmarco ('open-query':U).
END.
...
DO:
  ...
  END.
  END.
  RUN Busca_Codigo IN h_blistacontrmarco (SELF:SCREEN-VALUE).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LGC/vlistacontrmarco.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vlistacontrmarco ).
       RUN set-position IN h_vlistacontrmarco ( 1.27 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LGC/blistacontrmarco.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_blistacontrmarco ).
       RUN set-position IN h_blistacontrmarco ( 6.58 , 1.72 ) NO-ERROR.
       RUN set-size IN h_blistacontrmarco ( 9.77 , 110.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LGC/qlistacontrmarco.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_qlistacontrmarco ).
       RUN set-position IN h_qlistacontrmarco ( 5.19 , 82.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.27 , 15.14 ) */

       /* Links to SmartViewer h_vlistacontrmarco. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_vlistacontrmarco ).
       RUN add-link IN adm-broker-hdl ( h_qlistacontrmarco , 'Record':U , h_vlistacontrmarco ).

       /* Links to SmartBrowser h_blistacontrmarco. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_blistacontrmarco ).
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_blistacontrmarco ).
       RUN add-link IN adm-broker-hdl ( h_qlistacontrmarco , 'Record':U , h_blistacontrmarco ).
       RUN add-link IN adm-broker-hdl ( h_vlistacontrmarco , 'State':U , h_blistacontrmarco ).

       /* Links to SmartQuery h_qlistacontrmarco. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_qlistacontrmarco ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_vlistacontrmarco ,
             F-Codigo:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_vlistacontrmarco , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv02 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_blistacontrmarco ,
             h_p-updv02 , 'AFTER':U ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_qlistacontrmarco ,
