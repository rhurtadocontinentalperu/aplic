...
DEFINE VAR x-col-peso-x-atender2 AS DEC.
DEFINE VAR x-col-importe-x-atender2 AS DEC.
DEFINE VAR x-col-peso-x-atender2_col AS DEC.
DEFINE VAR x-col-importe-x-atender2_col AS DEC.
...
x-col-importe-x-atender2 @ x-col-Importe-x-atender2_col FacCPedi.Libre_d02 ~
FacCPedi.ImpTot FacCPedi.CodDiv FacCPedi.Libre_c01 
...
txtDesde FILL-IN-contenga BUTTON-1 BUTTON-2 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-cotizacion txtDesde txtHasta ~
FILL-IN-contenga txtFechaEntrega txtUltFechaTope 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-4 W-Win _STRUCTURED
  QUERY BROWSE-4 NO-LOCK DISPLAY
      ...
      calcula-peso-x-atender(faccpedi.coddoc, faccpedi.nroped)  @ x-col-peso-x-atender COLUMN-LABEL "Peso!Atender" FORMAT "->>,>>>,>>9.99":U
            COLUMN-FONT 0
      x-col-importe-x-atender2 @ x-col-Importe-x-atender2_col COLUMN-LABEL "Peso!Atender" FORMAT "->>,>>>,>>9.99":U
            COLUMN-FONT 0
...
DEFINE FRAME F-Main
     ...
     FILL-IN-contenga AT ROW 1.92 COL 24.72 COLON-ALIGNED NO-LABEL WIDGET-ID 104
     txtFechaEntrega AT ROW 1.15 COL 96.57 COLON-ALIGNED WIDGET-ID 98
     BUTTON-1 AT ROW 2.73 COL 66 WIDGET-ID 100
     BUTTON-2 AT ROW 24.65 COL 83.57 WIDGET-ID 116
...
"x-col-importe-x-atender2 @ x-col-Importe-x-atender2_col" "Peso!Atender" "->>,>>>,>>9.99" ? ? ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    IF lColumName = 'x-col-peso-x-atender' THEN lColumName = 'csubzona'.
    IF lColumName = ' x-col-Importe-x-atender2_col' THEN lColumName = 'czona'.
    ...
    hQueryHandle:QUERY-PREPARE("FOR EACH FacCPedi WHERE " + "{&" + "CONDICION" + "} NO-LOCK, FIRST FacDPedi OF FacCPedi WHERE (facdpedi.canped - facdpedi.canate) > 0 NO-LOCK " + x-sort-command).
    ...
    IF x-sort-color-reset = YES THEN DO:
        ...
        DO n_cols_browse = 1 TO browse-4:NUM-COLUMNS.
            n_celda = browse-4:GET-BROWSE-COLUMN(n_cols_browse).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BROWSE-4 FILL-IN-cotizacion BUTTON-4 txtDesde FILL-IN-contenga 
         BUTTON-1 BUTTON-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
FUNCTION calcula-peso-x-atender RETURNS DECIMAL
  ...
  ------------------------------------------------------------------------------*/
    ...
    x-col-peso-x-atender2 = 0.
    x-col-importe-x-atender2 = 0.
    ...
    IF AVAILABLE x-faccpedi THEN DO:
        ...
        FOR EACH x-facdpedi WHERE x-facdpedi.codcia = s-codcia AND
            ...
            IF AVAILABLE x-almmmatg THEN DO:
                ...
                x-col-peso-x-atender2 = x-col-peso-x-atender2 + ((x-facdpedi.canped - x-facdpedi.canate) * x-almmmatg.pesmat).
            END.
            x-precio = x-facdpedi.implin / x-facdpedi.canped .
            x-col-importe-x-atender2 = x-col-importe-x-atender2 + ((x-facdpedi.canped - x-facdpedi.canate) * x-precio).
