...
DEFINE TEMP-TABLE ttFacCPedi NO-UNDO LIKE FacCPedi.
...
&Scoped-define INTERNAL-TABLES ttFacCPedi FacDPedi

/* Definitions for BROWSE BROWSE-6                                      */
&Scoped-define FIELDS-IN-QUERY-BROWSE-6 ttFacCPedi.NroPed ttFacCPedi.CodCli ~
ttFacCPedi.NomCli ttFacCPedi.FchPed ttFacCPedi.FchEnt ~
calcula-peso-x-atender(ttfaccpedi.coddoc, ttfaccpedi.nroped)  @ x-col-peso-x-atender ~
calcula-importe-x-atender(ttfaccpedi.coddoc, ttfaccpedi.nroped)  @ x-col-importe-x-atender ~
ttFacCPedi.Libre_d02 ttFacCPedi.ImpTot ttFacCPedi.CodDiv ~
ttFacCPedi.Libre_c01 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-6 
&Scoped-define QUERY-STRING-BROWSE-6 FOR EACH ttFacCPedi NO-LOCK, ~
      FIRST FacDPedi OF ttFacCPedi NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-6 OPEN QUERY BROWSE-6 FOR EACH ttFacCPedi NO-LOCK, ~
      FIRST FacDPedi OF ttFacCPedi NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-6 ttFacCPedi FacDPedi
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-6 ttFacCPedi
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-6 FacDPedi
...
&Scoped-Define ENABLED-OBJECTS BROWSE-6 FILL-IN-cotizacion BUTTON-4 ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-6 W-Win _STRUCTURED
  QUERY BROWSE-6 NO-LOCK DISPLAY
      ttFacCPedi.NroPed COLUMN-LABEL "Cotizacion" FORMAT "X(12)":U
            WIDTH 10.43 COLUMN-FONT 0
      ttFacCPedi.CodCli FORMAT "x(11)":U COLUMN-FONT 0
      ttFacCPedi.NomCli COLUMN-LABEL "Nombre del Cliente" FORMAT "x(100)":U
            WIDTH 39.29 COLUMN-FONT 0
      ttFacCPedi.FchPed COLUMN-LABEL "Emision" FORMAT "99/99/9999":U
            COLUMN-FONT 0
      ttFacCPedi.FchEnt COLUMN-LABEL "Entrega" FORMAT "99/99/9999":U
            COLUMN-BGCOLOR 11 COLUMN-FONT 0
      calcula-peso-x-atender(ttfaccpedi.coddoc, ttfaccpedi.nroped)  @ x-col-peso-x-atender COLUMN-LABEL "Peso!Atender" FORMAT "->>,>>>,>>9.99":U
            COLUMN-FONT 0
      calcula-importe-x-atender(ttfaccpedi.coddoc, ttfaccpedi.nroped)  @ x-col-importe-x-atender COLUMN-LABEL "Importe!Atender" FORMAT "->>,>>>,>>9.99":U
            WIDTH 13.72 COLUMN-FONT 0
      ttFacCPedi.Libre_d02 COLUMN-LABEL "Peso Total" FORMAT "->>>,>>>,>>9.99":U
            COLUMN-FONT 0
      ttFacCPedi.ImpTot FORMAT "->>,>>>,>>9.99":U COLUMN-FONT 0
      ttFacCPedi.CodDiv FORMAT "x(5)":U COLUMN-FONT 0
      ttFacCPedi.Libre_c01 COLUMN-LABEL "Lista" FORMAT "x(6)":U
            WIDTH 8.14 COLUMN-FONT 0
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 129 BY 19.65 FIT-LAST-COLUMN.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     BROWSE-6 AT ROW 4.5 COL 2 WIDGET-ID 300
     ...
     txtFechaEntrega AT ROW 1.15 COL 103.86 COLON-ALIGNED WIDGET-ID 98
     BUTTON-1 AT ROW 3.15 COL 66.86 WIDGET-ID 100
     BUTTON-2 AT ROW 24.65 COL 79 WIDGET-ID 116
     txtUltFechaTope AT ROW 2.08 COL 103.86 COLON-ALIGNED WIDGET-ID 120
     ...
     "Cotizaciones emitidas" VIEW-AS TEXT
          ...
          FGCOLOR 4 FONT 2
     "v2" VIEW-AS TEXT
          SIZE 4.72 BY .62 AT ROW 2.08 COL 125.29 WIDGET-ID 128
     "Cliente" VIEW-AS TEXT
          SIZE 14.86 BY .62 AT ROW 1.19 COL 27.14 WIDGET-ID 106
          FGCOLOR 9 FONT 6
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: ttFacCPedi T "?" NO-UNDO INTEGRAL FacCPedi
   END-TABLES.
 */
...
/* Query rebuild information for BROWSE BROWSE-6
     _TblList          = "Temp-Tables.ttFacCPedi,INTEGRAL.FacDPedi OF Temp-Tables.ttFacCPedi"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _TblOptList       = ", FIRST"
     _FldNameList[1]   > Temp-Tables.ttFacCPedi.NroPed
"ttFacCPedi.NroPed" "Cotizacion" ? "character" ? ? 0 ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.ttFacCPedi.CodCli
"ttFacCPedi.CodCli" ? ? "character" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.ttFacCPedi.NomCli
"ttFacCPedi.NomCli" "Nombre del Cliente" ? "character" ? ? 0 ? ? ? no ? no no "39.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.ttFacCPedi.FchPed
"ttFacCPedi.FchPed" "Emision" ? "date" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.ttFacCPedi.FchEnt
"ttFacCPedi.FchEnt" "Entrega" ? "date" 11 ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"calcula-peso-x-atender(ttfaccpedi.coddoc, ttfaccpedi.nroped)  @ x-col-peso-x-atender" "Peso!Atender" "->>,>>>,>>9.99" ? ? ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > "_<CALC>"
"calcula-importe-x-atender(ttfaccpedi.coddoc, ttfaccpedi.nroped)  @ x-col-importe-x-atender" "Importe!Atender" "->>,>>>,>>9.99" ? ? ? 0 ? ? ? no ? no no "13.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.ttFacCPedi.Libre_d02
"ttFacCPedi.Libre_d02" "Peso Total" "->>>,>>>,>>9.99" "decimal" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.ttFacCPedi.ImpTot
"ttFacCPedi.ImpTot" ? ? "decimal" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > Temp-Tables.ttFacCPedi.CodDiv
"ttFacCPedi.CodDiv" ? ? "character" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > Temp-Tables.ttFacCPedi.Libre_c01
"ttFacCPedi.Libre_c01" "Lista" "x(6)" "character" ? ? 0 ? ? ? no ? no no "8.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-6 */
&ANALYZE-RESUME
...
ON ENTRY OF BROWSE-6 IN FRAME F-Main
DO:
    x-coddoc = "".
    x-nroped = "".
    IF AVAILABLE ttfaccpedi THEN DO:
          x-coddoc = ttfaccpedi.coddoc.
          x-nroped = ttfaccpedi.nroped.
    END.
...
DO:
  ...
  IF AVAILABLE ttfaccpedi THEN DO:
      RUN lgc/w-generacion-pco-grabar.r(INPUT ttfaccpedi.coddoc, INPUT ttfaccpedi.nroped, INPUT txtFechaEntrega, INPUT txtUltFechaTope).      
...
DO:
    ...
    DEFINE VAR x1 AS INT.

    ASSIGN fill-in-contenga fill-in-cotizacion fill-in-cliente.
  ...
  /*{&OPEN-QUERY-BROWSE-4}*/
  EMPTY TEMP-TABLE ttfaccpedi.
  IF fill-in-cliente <> "" THEN DO:

      FOR EACH faccpedi WHERE faccpedi.codcia = 1 AND faccpedi.codcli = fill-in-cliente AND
            faccpedi.coddoc = 'cot' AND 
            (faccpedi.fchped >= x-emision-desde and faccpedi.fchped <= x-emision-hasta) AND 
            faccpedi.flgest = 'P' NO-LOCK:
            IF (x-todos = YES AND lookup(faccpedi.libre_c01,x-lista-precios) > 0) 
                OR (faccpedi.coddiv = s-coddiv AND lookup(faccpedi.libre_c01,x-lista-precios) > 0 ) THEN DO:
                CREATE ttfaccpedi.
                BUFFER-COPY faccpedi TO ttfaccpedi.
            END.          
      END.
  END.
  ELSE DO:
      IF x-cotizacion <> "" THEN DO:

          FOR EACH faccpedi WHERE faccpedi.codcia = 1 AND faccpedi.coddoc = 'cot' AND faccpedi.nroped = x-cotizacion 
              AND faccpedi.flgest = 'P' NO-LOCK:
              IF (x-todos = YES AND lookup(faccpedi.libre_c01,x-lista-precios) > 0) OR 
                  (faccpedi.coddiv = s-coddiv AND lookup(faccpedi.libre_c01,x-lista-precios) > 0 ) THEN DO:
                  CREATE ttfaccpedi.
                  BUFFER-COPY faccpedi TO ttfaccpedi.
              END.
      ...
      ELSE DO:
            x1 = 0.
          FOR EACH faccpedi WHERE faccpedi.codcia = 1 AND faccpedi.coddoc = 'cot' AND faccpedi.flgest = 'P' AND
              (faccpedi.fchped >= x-emision-desde and faccpedi.fchped <= x-emision-hasta) NO-LOCK:
              IF (x-todos = YES AND lookup(faccpedi.libre_c01,x-lista-precios) > 0)
               OR (faccpedi.coddiv = s-coddiv AND lookup(faccpedi.libre_c01,x-lista-precios) > 0 ) THEN DO:
                  CREATE ttfaccpedi.
                  BUFFER-COPY faccpedi TO ttfaccpedi.
...
ON LEAVE OF FILL-IN-cliente IN FRAME F-Main
DO:
    DEFINE VAR cCod AS CHAR.
  fill-in-contenga:SCREEN-VALUE = "".
    cCod = SELF:SCREEN-VALUE.
    FIND FIRST gn-clie WHERE gn-clie.codcia = 0 AND gn-clie.codcli = cCod NO-LOCK NO-ERROR.
    fill-in-contenga:SCREEN-VALUE = gn-clie.nomcli.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BROWSE-6 FILL-IN-cotizacion BUTTON-4 txtDesde BUTTON-1 BUTTON-2 
         FILL-IN-cliente 
      WITH FRAME F-Main IN WINDOW W-Win.
...
DO WITH FRAME {&FRAME-NAME}:
    x-cotizaciones = browse-6:NUM-SELECTED-ROWS.
    ...
    DO x-sec = 1 TO x-cotizaciones:
        IF browse-6:FETCH-SELECTED-ROW(x-sec) THEN DO:
            x-coddoc = {&FIRST-TABLE-IN-QUERY-browse-6}.coddoc.
            x-nroped = {&FIRST-TABLE-IN-QUERY-browse-6}.nroped.
            x-msgerror = "".
...
FOR EACH b-faccpedi WHERE b-faccpedi.codcia = s-codcia AND                             
                            ...
                            b-faccpedi.nroref = x-faccpedi.nroped AND
                            b-faccpedi.coddoc = 'PCO' NO-LOCK:
    x-corre = x-corre + 1.
END.
x-corre = x-corre + 1.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "ttFacCPedi"}
