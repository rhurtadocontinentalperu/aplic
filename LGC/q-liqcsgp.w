...
DEFINE VARIABLE L-NROSER AS CHAR.
...
&Scoped-define INTERNAL-TABLES lg-liqcsg

/* Definitions for QUERY Query-Main                                     */
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH lg-liqcsg ~
      WHERE lg-liqcsg.CodCia = S-CODCIA ~
 AND lg-liqcsg.NroSer = S-NROSER NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main lg-liqcsg
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main lg-liqcsg


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS RECT-2 C-NroSer F-Numero 
...
/* SETTINGS FOR BUTTON BUTTON-1 IN FRAME FRAME-A
   NO-ENABLE                                                            */
ASSIGN 
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.lg-liqcsg"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _Where[1]         = "INTEGRAL.lg-liqcsg.CodCia = S-CODCIA
 AND INTEGRAL.lg-liqcsg.NroSer = S-NROSER"
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND lg-liqcsg WHERE lg-liqcsg.CodCia = S-CODCIA
     AND lg-liqcsg.NroSer = S-NROSER 
     AND lg-liqcsg.NroDoc = F-Numero NO-LOCK NO-ERROR.
IF AVAILABLE lg-liqcsg THEN output-var-1 = ROWID(lg-liqcsg).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
