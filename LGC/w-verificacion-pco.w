...
&Scoped-Define ENABLED-OBJECTS BROWSE-4 FILL-IN-cotizacion BUTTON-8 ~
RADIO-SET-estado BUTTON-4 FILL-IN-contenga BUTTON-1 BUTTON-2 BUTTON-3 ~
BUTTON-5 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-cotizacion RADIO-SET-estado ~
FILL-IN-contenga txtFechaAbastecimiento txtFechaTope 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 138 BY 20.85 ROW-HEIGHT-CHARS .62 FIT-LAST-COLUMN.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     BROWSE-4 AT ROW 3.77 COL 2.14 WIDGET-ID 200
     FILL-IN-cotizacion AT ROW 2.69 COL 11 COLON-ALIGNED WIDGET-ID 134
     BUTTON-8 AT ROW 24.85 COL 107 WIDGET-ID 132
     RADIO-SET-estado AT ROW 1.04 COL 41.86 NO-LABEL WIDGET-ID 124
     BUTTON-4 AT ROW 24.85 COL 71.29 WIDGET-ID 122
     FILL-IN-contenga AT ROW 1.65 COL 2 COLON-ALIGNED NO-LABEL WIDGET-ID 104
     txtFechaAbastecimiento AT ROW 1.08 COL 110.14 COLON-ALIGNED WIDGET-ID 98
     BUTTON-1 AT ROW 1.19 COL 128 WIDGET-ID 100
     BUTTON-2 AT ROW 24.77 COL 2.29 WIDGET-ID 116
     BUTTON-3 AT ROW 24.81 COL 22.14 WIDGET-ID 118
     txtFechaTope AT ROW 2.08 COL 110.14 COLON-ALIGNED WIDGET-ID 128
     BUTTON-5 AT ROW 24.85 COL 49 WIDGET-ID 130
     "Que contengan" VIEW-AS TEXT
          SIZE 14.86 BY .62 AT ROW 1.04 COL 4.43 WIDGET-ID 106
          FGCOLOR 9 FONT 6
     "DobleClick en Nro PCO para autorizar." VIEW-AS TEXT
          SIZE 32 BY .62 AT ROW 2.31 COL 48.86 WIDGET-ID 120
          FGCOLOR 9 FONT 6
...
"FacCPedi.FchEnt" "Entrega" ? "date" 11 ? 0 ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > INTEGRAL.FacCPedi.Libre_f01
"FacCPedi.Libre_f01" "Abastecimiento" "99/99/9999" "date" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > INTEGRAL.FacCPedi.Libre_f02
"FacCPedi.Libre_f02" "Fecha Tope" "99/99/9999" "date" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > INTEGRAL.FacCPedi.Libre_c01
...
DO:
    ...
    ASSIGN fill-in-contenga RADIO-SET-estado fill-in-cotizacion.
    ...
    /*
    IF fill-in-cotizacion <> "" THEN DO:
        x-cotizacion = "*" + fill-in-contenga + "*".
    END.
    */
...
DO TRANSACTION ON ERROR UNDO, LEAVE:
  ...
  FOR EACH x-facdpedi OF x-faccpedi NO-LOCK ON ERROR UNDO, THROW:
    /* OrderLine update block */
      ...
      END.
      ELSE DO:
          pMsgError = "Este articulo " + x-facdpedi.codmat + " NO existe en VTADDOCU ".
          UNDO GRABAR_DATOS, LEAVE GRABAR_DATOS.
      END.
...
RLOOP:
DO TRANSACTION ON ERROR UNDO, LEAVE:
    FIND COTIZACION WHERE COTIZACION.codcia = x-faccpedi.codcia
        AND COTIZACION.coddoc = x-faccpedi.codref
        AND COTIZACION.nroped = x-faccpedi.nroref
        EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
    IF ERROR-STATUS:ERROR = YES THEN DO:
        {lib/mensaje-de-error.i &CuentaError="pCuenta" &MensajeError="pMsgError" }
        UNDO RLOOP, LEAVE RLOOP.
    END.
    ASSIGN 
        COTIZACION.FlgEst = "P".  /* OJO */
    FOR EACH x-facdpedi OF x-faccpedi WHERE x-facdpedi.canped > 0 ON ERROR UNDO, THROW:
        ...
        END.
        ASSIGN 
            facdpedi.canate = facdpedi.canate - x-facdpedi.canped.
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-cotizacion RADIO-SET-estado FILL-IN-contenga 
          txtFechaAbastecimiento txtFechaTope 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE BROWSE-4 FILL-IN-cotizacion BUTTON-8 RADIO-SET-estado BUTTON-4 
         FILL-IN-contenga BUTTON-1 BUTTON-2 BUTTON-3 BUTTON-5 
      WITH FRAME F-Main IN WINDOW W-Win.
        ...
        IF browse-4:FETCH-SELECTED-ROW(x-sec) THEN DO:
            ...
            IF s-user-id = 'ADMIN' THEN DO:
                IF x-msgerror <> 'OK' THEN DO:
                    MESSAGE x-msgerror VIEW-AS ALERT-BOX WARNING.
                END.
            END.
        END.
    END.
