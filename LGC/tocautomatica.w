...
&Scoped-define INTERNAL-TABLES DCMP INTEGRAL.Almmmatg

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table DCMP.Codmat ~
INTEGRAL.Almmmatg.DesMat INTEGRAL.Almmmatg.DesMar INTEGRAL.Almmmatg.TipArt ~
DCMP.CanPedi DCMP.UndCmp DCMP.PreUni DCMP.Dsctos[1] DCMP.Dsctos[2] ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH DCMP WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH INTEGRAL.Almmmatg OF DCMP NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH DCMP WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH INTEGRAL.Almmmatg OF DCMP NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table DCMP INTEGRAL.Almmmatg
...
DEFINE BUTTON BUTTON-1 
     LABEL "IMPORTAR SALDOS DE CISSAC" 
     SIZE 25 BY 1.12.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      INTEGRAL.Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "x(15)":U
      INTEGRAL.Almmmatg.TipArt COLUMN-LABEL "Rotación" FORMAT "X(1)":U
...
DEFINE FRAME F-Main
     ...
     BUTTON-1 AT ROW 8.69 COL 87 WIDGET-ID 68
...
"DCMP.PreUni" "Unitario" ">>>>9.9999" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.DCMP.Dsctos[1]
"DCMP.Dsctos[1]" "%Dcto1" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.DCMP.Dsctos[2]
"DCMP.Dsctos[2]" "%Dcto2" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > Temp-Tables.DCMP.Dsctos[3]
"DCMP.Dsctos[3]" "%Dcto3" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON CHOOSE OF BUTTON-1 IN FRAME F-Main /* IMPORTAR SALDOS DE CISSAC */
DO:
  DEF VAR pOk AS LOG.
  DEF VAR pCodAlm AS CHAR.
  DEF VAR pCodFam AS CHAR.

  RUN lgc/pgocautomatica ( OUTPUT pOk, OUTPUT pCodAlm, OUTPUT pCodFam).
  IF pOk = NO THEN RETURN NO-APPLY.
  RUN Actualiza-Cabecera IN lh_handle (pCodAlm, pCodFam).
  RUN Recalcular.
...
FOR EACH DCMP:
    FIND Almmmatg WHERE Almmmatg.CodCia = S-CODCIA 
        AND Almmmatg.CodMat = DCMP.CodMat
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almmmatg THEN DO:
        MESSAGE "Articulo" DCMP.codmat "NO registrado en el catálogo" VIEW-AS ALERT-BOX WARNING.
        DELETE DCMP.
        NEXT.
    END.
    IF Almmmatg.Tpoart <> "A" THEN DO:
        MESSAGE "Articulo" DCMP.codmat "Desactivado" VIEW-AS ALERT-BOX WARNING.
        DELETE DCMP.
        NEXT.
    END.
    F-FACTOR = 1.
    ...
    IF NOT AVAILABLE Almtconv THEN DO:
        MESSAGE "Tabla de equivalencias NO definida para:" SKIP
            "Artículo:" Almmmatg.codmat SKIP
            "Unidad base:" Almmmatg.undbas SKIP
            "Unidad de compra:" Almmmatg.undcmp
    ...
    FIND lg-dmatpr WHERE lg-dmatpr.codcia = s-codcia
        AND lg-dmatpr.codmat = DCMP.codmat
        AND CAN-FIND(FIRST lg-cmatpr WHERE lg-cmatpr.codcia = s-codcia
                     AND lg-cmatpr.nrolis = lg-dmatpr.nrolis
                     AND lg-cmatpr.codpro = s-provee
                     AND lg-cmatpr.flgest = 'A' NO-LOCK)
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE LG-dmatpr THEN DO:
        MESSAGE "Articulo" DCMP.codmat "NO está asignado al proveedor" VIEW-AS ALERT-BOX WARNING.
        DELETE DCMP.
        NEXT.
    END.
    IF AVAILABLE LG-dmatpr THEN DO:
        ASSIGN
            ...
            DCMP.IgvMat    = LG-dmatpr.IgvMat.
        IF S-CODMON = 1 THEN DO:
            IF LG-dmatpr.CodMon = 1 THEN DCMP.PreUni = LG-dmatpr.PreAct.
            ELSE DCMP.PreUni = ROUND(LG-dmatpr.PreAct * S-TPOCMB,4).
        END.
        ELSE DO:
            IF LG-dmatpr.CodMon = 2 THEN DCMP.PreUni = LG-dmatpr.PreAct.
            ELSE DCMP.PreUni = ROUND(LG-dmatpr.PreAct / S-TPOCMB,4).
        END.
    END.
