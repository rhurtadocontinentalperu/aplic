...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "RESUMEN ANUAL DE COMPRAS"
         HEIGHT             = 11.81
         WIDTH              = 71.86
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 80
...
ON END-ERROR OF W-Win /* RESUMEN ANUAL DE COMPRAS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* RESUMEN ANUAL DE COMPRAS */
DO:
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH LG-COCMP WHERE LG-COCMP.codcia = s-codcia 
        ...
        NO-LOCK:
    IF LOOKUP(TRIM(Lg-cocmp.flgsit), 'X,G,A,V') > 0 THEN NEXT.
    x-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'PROCESANDO: ' + STRING(Lg-cocmp.nrodoc) + ' ' + STRING(Lg-cocmp.fchdoc).
    ASSIGN
        x-NroMes = MONTH(Lg-cocmp.fchdoc)
        x-TpoCmb = 1.
    FIND FIRST Gn-tcmb WHERE Gn-tcmb.fecha >= Lg-cocmp.fchdoc NO-LOCK NO-ERROR.
    ...
    FOR EACH LG-DOCMP OF LG-COCMP NO-LOCK, FIRST Almmmatg OF LG-docmp NO-LOCK:
        FIND w-report WHERE w-report.Llave-I = s-codcia
            AND w-report.Llave-C = Lg-docmp.codmat
        ...
        IF NOT AVAILABLE w-report THEN DO:
            ...
            ASSIGN
                ...
                w-report.llave-c = lg-docmp.codmat
                w-report.llave-i = s-codcia
                w-report.campo-c[20] = Almmmatg.desmat
                w-report.campo-c[21] = Almmmatg.undbas.
        END.
        IF x-Tipo = 1 THEN DO:
            ASSIGN
                w-report.campo-f[x-NroMes] = w-report.campo-f[x-NroMes] + Lg-docmp.canpedi.
        END.
        ELSE DO:
            IF x-CodMon = Lg-cocmp.codmon
            THEN w-report.campo-f[x-NroMes] = w-report.campo-f[x-NroMes] + LG-DOCmp.ImpTot.
            ELSE IF x-CodMon = 1
                    THEN w-report.campo-f[x-NroMes] = w-report.campo-f[x-NroMes] + LG-DOCmp.ImpTot * x-TpoCmb.
                    ELSE w-report.campo-f[x-NroMes] = w-report.campo-f[x-NroMes] + LG-DOCmp.ImpTot / x-TpoCmb.
        END.
    END.
  END.
