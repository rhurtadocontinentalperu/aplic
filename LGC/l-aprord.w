...
DEFINE VARIABLE f-tippro AS CHAR INIT "".
DEFINE VARIABLE X-FLGSIT AS CHAR INIT "G".
DEFINE VARIABLE X-MON    AS CHAR INIT "".
DEFINE VARIABLE X-EST    AS CHAR INIT "".

/* Preprocesadores para condiciones */
&SCOPED-DEFINE CONDICION ( LG-Cocmp.codcia = s-codcia AND LG-Cocmp.coddiv = s-coddiv ~
AND LG-COcmp.TpoDoc BEGINS F-TIPPRO AND LG-COcmp.FlgSit = X-flgsit)
&SCOPED-DEFINE CONDICFEC ( LG-COcmp.FchDoc >= f-desde AND LG-COcmp.FchDoc <= f-hasta )
&SCOPED-DEFINE CONDICIMP ( (Lg-cocmp.codmon = 1 AND Lg-cocmp.imptot < 250000) OR ~
(Lg-cocmp.codmon = 2 AND Lg-cocmp.imptot * Lg-cocmp.tpocmb < 250000) )

&SCOPED-DEFINE CODIGO LG-COcmp.NroDoc

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( LG-COcmp.NomPro BEGINS FILL-IN-filtro AND {&CONDICFEC} AND {&CONDICION} ) 
&SCOPED-DEFINE FILTRO2 ( INDEX ( LG-COcmp.Nompro , FILL-IN-filtro ) <> 0  AND {&CONDICFEC} AND {&CONDICION} )
...
DEF VAR x-Usuario AS CHAR NO-UNDO.      /* Ultimo usuario */
...
&Scoped-define FIELDS-IN-QUERY-br_table LG-COCmp.NroDoc LG-COCmp.NomPro ~
LG-COCmp.Fchdoc LG-COCmp.FchVto LG-COCmp.CodAlm X-MON @ X-MON ~
LG-COCmp.ImpTot fFlgSit() @ X-EST fUsuario() @ x-Usuario 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH LG-COCmp WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} AND {&CONDICFEC} ~
 AND {&CONDICIMP}  NO-LOCK ~
...
FILL-IN-filtro C-Estado C-TIPPRO f-hasta f-desde br_table 
&Scoped-Define DISPLAYED-OBJECTS CMB-filtro FILL-IN-codigo FILL-IN-filtro ~
C-Estado C-TIPPRO f-hasta f-desde 
...
<FOREIGN-KEYS>
Nombres que inicien con|y||integral.LG-COCmp.NomPro
Nombres que contengan|y||integral.LG-COCmp.NomPro
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Código|y||integral.LG-COCmp.NroDoc|yes
Descripción|||integral.LG-COCmp.NomPro|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fFlgSit L-table-Win 
...
DEFINE MENU POPUP-MENU-br_table 
       MENU-ITEM m_Observaciones LABEL "Observaciones" .


/* Definitions of the field level widgets                               */
DEFINE VARIABLE C-Estado AS CHARACTER FORMAT "X(256)":U INITIAL "Emitido" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "Emitido","Aprobado","Rechazado" 
     DROP-DOWN-LIST
     SIZE 12.14 BY 1
     BGCOLOR 15  NO-UNDO.

DEFINE VARIABLE C-TIPPRO AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "","Nacional","Extranjero" 
     DROP-DOWN-LIST
     SIZE 12.14 BY 1
     BGCOLOR 15  NO-UNDO.
...
DEFINE VARIABLE FILL-IN-codigo AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "Código" 
     VIEW-AS FILL-IN 
     SIZE 10.29 BY .81
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      LG-COCmp.NroDoc COLUMN-LABEL "Numero!O/C." FORMAT "999999":U
      LG-COCmp.NomPro COLUMN-LABEL "Proveedor" FORMAT "X(43)":U
      LG-COCmp.Fchdoc COLUMN-LABEL "     Fecha    !    Emision" FORMAT "99/99/9999":U
      LG-COCmp.FchVto COLUMN-LABEL "     Fecha    !Vencimiento" FORMAT "99/99/9999":U
      LG-COCmp.CodAlm COLUMN-LABEL "Almacen ! Ingreso" FORMAT "x(3)":U
      X-MON @ X-MON COLUMN-LABEL "Mon." FORMAT "XXXX":U
      LG-COCmp.ImpTot FORMAT "ZZ,ZZZ,ZZ9.99":U
      fFlgSit() @ X-EST COLUMN-LABEL "Estado" FORMAT "XXX":U
      fUsuario() @ x-Usuario COLUMN-LABEL "Usuario" FORMAT "x(10)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 93.14 BY 12.42
...
DEFINE FRAME F-Main
     ...
     C-Estado AT ROW 2.31 COL 26.86 COLON-ALIGNED NO-LABEL
     C-TIPPRO AT ROW 2.35 COL 1 COLON-ALIGNED NO-LABEL
     f-hasta AT ROW 2.35 COL 75.29 COLON-ALIGNED
     f-desde AT ROW 2.38 COL 58.72 COLON-ALIGNED
     br_table AT ROW 3.23 COL 1.86
     "Estado" VIEW-AS TEXT
          SIZE 6.72 BY .65 AT ROW 2.38 COL 41.14
          FONT 6
     "Buscar x" VIEW-AS TEXT
          SIZE 7.86 BY .62 AT ROW 1.46 COL 2.14
          FONT 6
     "Filtrar x" VIEW-AS TEXT
          SIZE 6.72 BY .65 AT ROW 2.42 COL 15.29
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "{&CONDICION} AND {&CONDICFEC}
 AND {&CONDICIMP} "
     _FldNameList[1]   > integral.LG-COCmp.NroDoc
"LG-COCmp.NroDoc" "Numero!O/C." ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.LG-COCmp.NomPro
"LG-COCmp.NomPro" "Proveedor" "X(43)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.LG-COCmp.Fchdoc
"LG-COCmp.Fchdoc" "     Fecha    !    Emision" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"LG-COCmp.CodAlm" "Almacen ! Ingreso" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"X-MON @ X-MON" "Mon." "XXXX" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   = integral.LG-COCmp.ImpTot
     _FldNameList[8]   > "_<CALC>"
"fFlgSit() @ X-EST" "Estado" "XXX" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > "_<CALC>"
"fUsuario() @ x-Usuario" "Usuario" "x(10)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
   RUN LGC\D-aprord.r(LG-COcmp.TpoDoc,LG-COcmp.NroDoc,LG-COcmp.CodPro).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Estado L-table-Win
ON VALUE-CHANGED OF C-Estado IN FRAME F-Main
DO:
...
&Scoped-define SELF-NAME C-TIPPRO
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-TIPPRO L-table-Win
ON VALUE-CHANGED OF C-TIPPRO IN FRAME F-Main
DO:

  CASE C-TIPPRO:SCREEN-VALUE:
       WHEN "Nacional"  THEN 
          ASSIGN 
             F-TIPPRO = "N".
       WHEN "Extranjero" THEN 
          ASSIGN 
             F-TIPPRO = "I".
       OTHERWISE
          ASSIGN 
...
DO:
    IF CMB-filtro = CMB-filtro:SCREEN-VALUE AND
       ...
       C-TIPPRO = C-TIPPRO:SCREEN-VALUE AND
       C-ESTADO = C-ESTADO:SCREEN-VALUE THEN RETURN.
    ASSIGN
       FILL-IN-filtro
       CMB-filtro
       C-TIPPRO
    ...
    IF CMB-filtro = "Todos" THEN DO:
       IF C-TIPPRO <> "" THEN RUN set-attribute-list('Key-Name=Nombres que inicien con').
...
&Scoped-define SELF-NAME m_Observaciones
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL m_Observaciones L-table-Win
ON CHOOSE OF MENU-ITEM m_Observaciones /* Observaciones */
DO:
  IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
     RUN LGC\D-Notas.r(LG-COcmp.TpoDoc,LG-COcmp.NroDoc,LG-COcmp.CodPro).
...
/*            
    CASE LG-COcmp.FlgSit :
         WHEN "X" THEN X-EST = "REC".
         WHEN "G" THEN X-EST = "EMI".
         WHEN "P" THEN X-EST = "APR".
         WHEN "T" THEN X-EST = "ATE".
         WHEN "A" THEN X-EST = "ANU".
         WHEN "V" THEN X-EST = "VEN".
    END.
*/    
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY LG-COCmp.NroDoc
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY LG-COCmp.NomPro
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fFlgSit L-table-Win 
FUNCTION fFlgSit RETURNS CHARACTER
...
------------------------------------------------------------------------------*/
    CASE LG-COcmp.FlgSit :
         WHEN "X" THEN RETURN "REC".
         WHEN "G" THEN RETURN "EMI".
         WHEN "P" THEN RETURN "APR".
         WHEN "T" THEN RETURN "ATE".
         WHEN "A" THEN RETURN "ANU".
         WHEN "V" THEN RETURN "VEN".
    END.
...
------------------------------------------------------------------------------*/
  ...
  x-Llave = STRING(LG-COCMP.CodDiv, 'x(5)') +
            STRING(LG-COCMP.TpoDoc, 'X') +
            STRING(LG-COCMP.NroDoc, '999999') +
            STRING(LG-COCMP.flgsit, 'X').    

  FIND LAST LogTabla WHERE LogTabla.codcia = s-codcia
    AND LogTabla.Tabla = 'LG-COCMP'
    AND LogTabla.ValorLlave = x-Llave
    NO-LOCK NO-ERROR.
  IF AVAILABLE LogTabla
  THEN RETURN LogTabla.Usuario.
  ELSE RETURN LG-COCMP.Userid-com.    
