...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 18.5 COL 3 WIDGET-ID 2
...
ON CHOOSE OF BUTTON-1 IN FRAME F-Main /* INGRESOS */
DO:
  RUN Ingresos-Parciales IN h_b-ordcmp.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lgc/v-ordcmp.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ordcmp ).
       RUN set-position IN h_v-ordcmp ( 1.27 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.08 , 92.72 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lgc/b-ordcmp.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ordcmp ).
       RUN set-position IN h_b-ordcmp ( 9.35 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-ordcmp ( 9.12 , 91.57 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lgc/q-conord.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-conord ).
       RUN set-position IN h_q-conord ( 18.23 , 81.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.15 , 14.86 ) */

       /* Links to SmartViewer h_v-ordcmp. */
       RUN add-link IN adm-broker-hdl ( h_q-conord , 'Record':U , h_v-ordcmp ).

       /* Links to SmartBrowser h_b-ordcmp. */
       RUN add-link IN adm-broker-hdl ( h_q-conord , 'Record':U , h_b-ordcmp ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ordcmp ,
