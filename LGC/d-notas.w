...
DEFINE FRAME D-Dialog
     EDITOR-1 AT ROW 1.15 COL 1.57 NO-LABEL
     Btn_OK AT ROW 1.54 COL 49
     Btn_Cancel AT ROW 2.77 COL 49
     SPACE(0.85) SKIP(0.38)
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Registrar Observaciones */
DO:  
...
FIND CCMP WHERE CCMP.CODCIA = S-CODCIA AND
                CCMP.TPODOC = C-TIPDOC AND
                CCMP.NRODOC = C-NRODOC NO-ERROR.
     IF AVAILABLE CCMP THEN DO:
        ASSIGN
          CCMP.ObsInt[1] = F-Notas.       
     END.                
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL EDITOR-1 D-Dialog
ON LEAVE OF EDITOR-1 IN FRAME D-Dialog
DO:
  ASSIGN editor-1. 
  F-Notas = EDITOR-1.
END.
