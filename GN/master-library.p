...
&IF DEFINED(EXCLUDE-ALM_Genera-PHR-ING-XD) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ALM_Genera-PHR-ING-XD Procedure 
PROCEDURE ALM_Genera-PHR-ING-XD :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FIND B-CREPO WHERE B-CREPO.codcia = B-CPEDI.codcia AND 
    B-CREPO.tipmov = "A" AND
    B-CREPO.nroser = INTEGER(SUBSTRING(B-CPEDI.nroref,1,3)) AND
    B-CREPO.nrodoc = INTEGER(SUBSTRING(B-CPEDI.nroref,4))
    NO-LOCK NO-ERROR.
IF AVAILABLE B-CREPO THEN DO:
    RUN ALM_Genera-PHR_y_HPK (INPUT pRowid, OUTPUT pMensaje).
...
FIND FIRST Almacen WHERE Almacen.codcia = s-codcia AND Almacen.codalm = x-AlmOrigen NO-LOCK NO-ERROR.
IF NOT AVAILABLE Almacen THEN RETURN 'ADM-ERROR'.
x-DivOrigen = Almacen.coddiv.
FIND FIRST Almacen WHERE Almacen.codcia = s-codcia AND Almacen.codalm = x-AlmDestino NO-LOCK NO-ERROR.
IF NOT AVAILABLE Almacen THEN RETURN 'ADM-ERROR'.
