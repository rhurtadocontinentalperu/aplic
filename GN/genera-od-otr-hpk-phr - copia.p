...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    RUN OTR_Generacion_por_RA (INPUT pCrossDocking,
                               INPUT pAlmacenXD,
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH Almcrepo NO-LOCK WHERE almcrepo.CodCia = AlmCIncidencia.CodCia AND
        ...
        */
        ...
        /* ********************************************************* */
        RUN OTR_Generacion_por_RA (INPUT NO,
                                   INPUT "",
                                   OUTPUT pFechaEntrega,
                                   OUTPUT pMensaje).
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
...
PROCEDURE CCB_Verifica_Cliente :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FIND FIRST gn-clied WHERE gn-clied.codcia = cl-codcia
   AND gn-clied.codcli = PEDIDO.codcli
   AND gn-clied.sede = PEDIDO.sede NO-LOCK NO-ERROR.
...
PROCEDURE GEN_Consistencia_Cruzada :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
PROCEDURE GEN_Datos_del_Transportista :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE GEN_Detalle_de_la_orden :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
RUN HPK_Genera-HPK-Master-v2 IN hLibLogis (INPUT ROWID(Faccpedi), OUTPUT pMensaje) NO-ERROR.    
...
CASE Faccpedi.CodDoc:
    WHEN "O/D" THEN DO:
        ...
        FIND VtaTabla WHERE VtaTabla.CodCia = s-CodCia AND
            ...
            VtaTabla.Llave_c1 = Faccpedi.DivDes AND
        ...
        CASE VtaTabla.Libre_c03:
            WHEN "CR" THEN DO:
                IF FacCPedi.Cliente_Recoge = NO THEN RETURN 'OK'.
                pObserv = "CL RECOGE".
            END.
            WHEN "CNR" THEN DO:
                IF FacCPedi.Cliente_Recoge = YES THEN RETURN 'OK'.
                pObserv = "CL NO RECOGE".
            END.
            WHEN "CRNR" THEN DO:
                CASE FacCPedi.Cliente_Recoge:
        ...
        IF TRUE <> (pObserv > '') THEN DO:
            ...
            RETURN 'ADM-ERROR'.
        END.
        pObserv = TRIM(pObserv) + " - " + FacCPedi.CodDiv.       /* OJO */
    END.
    OTHERWISE DO:
    ...
    FOR EACH Facdpedi OF FacCPedi NO-LOCK,
        ...
        IF FIRST-OF(SUBSTRING(Almmmate.CodUbi,1,2)) THEN DO:
            ...
            IF lSectorOK = YES OR lSectorG0 = NO THEN DO:
                ...
                END.
                ASSIGN
                    Vtacdocu.Glosa = FacCPedi.Glosa.    /* OJO */
...
PROCEDURE OD_Generacion_Rutina_Master :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* Detalle de la OD */
    /* ******************************************************************** */
    RUN GEN_Detalle_de_la_orden (OUTPUT pMensaje).
...
PROCEDURE OTR_Generacion_por_CrossDocking :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* DETALLE DE LA OTR */
    /* ***************************************************************** */
    RUN GEN_Detalle_de_la_OTR (INPUT lDivDespacho,
                               INPUT lAlmDespacho,
                               OUTPUT pMensaje).
    IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
...
PROCEDURE OTR_Generacion_por_RA :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Almcrepo ya debe estar en el registro correcto y bloqueado
------------------------------------------------------------------------------*/
...
/* ***************************************************************** */
IF Almcrepo.TipMov = "INC" AND (Almcrepo.Incidencia = "F" OR Almcrepo.Incidencia = "S") THEN DO:
    RETURN "OK".
END.
...
DEF VAR lAlmDespacho LIKE Almcrepo.almped NO-UNDO.
DEF VAR s-NroSer AS INTE NO-UNDO.
DEF VAR s-DiasVtoPed LIKE GN-DIVI.DiasVtoPed.
DEF VAR s-FlgEmpaque LIKE GN-DIVI.FlgEmpaque.
DEF VAR s-VentaMayorista LIKE GN-DIVI.VentaMayorista.
...
/* Almacén Destino */
/* ************************************************************************************** */
FIND FIRST Almacen WHERE Almacen.codcia = s-codcia AND Almacen.codalm = Almcrepo.CodAlm NO-LOCK NO-ERROR.
IF NOT AVAILABLE almacen THEN DO:
    pMensaje = 'Almacén ' + Almcrepo.CodAlm + ' NO existe'.
    RETURN "ADM-ERROR".
END.
...
/* ************************************************************************************** */
FIND FIRST Almacen WHERE Almacen.codcia = s-codcia AND Almacen.codalm = Almcrepo.almped NO-LOCK NO-ERROR.
IF NOT AVAILABLE almacen THEN DO:
    pMensaje = 'Almacen de despacho ' + Almcrepo.almped + ' No existe'.
    RETURN "ADM-ERROR".
END.
lDivDespacho = Almacen.CodDiv.
...
IF NOT AVAILABLE FacCorre THEN DO:
    ...
    RETURN "ADM-ERROR".
END.
/* La serie segun el almacén de donde se desea despachar(Almcrepo.almped) segun la R/A */
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FIND Almacen WHERE Almacen.codcia = s-codcia AND 
        Almacen.codalm = Almcrepo.codalm NO-LOCK NO-ERROR.
    /* **************************************************************************** */
    CREATE Faccpedi.
    ASSIGN 
        Faccpedi.CodCia = S-CODCIA
    ...
    /* *************************************************************************** */
    ASSIGN
        Faccpedi.Libre_c03 = Almcrepo.Libre_c03.
    /* *************************************************************************** */
    /* RHC 21/03/2019 Motivo de la INCIDENCIA */
    /* *************************************************************************** */
    IF Almcrepo.TipMov = "INC" THEN DO:
        ...
        Faccpedi.CodOrigen = Almcrepo.CodRef.    /* INC */
        Faccpedi.NroOrigen = Almcrepo.NroRef.    /* AlmCIncidencia.NroControl */
        Faccpedi.TpoPed = "INC".
    END.
    /* *************************************************************************** */
    /* 20/12/17 Cross Docking */
    ...
    /* Detalle de la OTR */
    /* ******************************************************************** */
    RUN GEN_Detalle_OTR_por_RA (OUTPUT pMensaje).
    IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
    ...
    */
    ...
    /* ***************************************************************** */
/*     IF Almcrepo.TipMov = "INC" AND (Almcrepo.Incidencia = "F" OR Almcrepo.Incidencia = "R") THEN DO: */
/*         ASSIGN                                                                                       */
