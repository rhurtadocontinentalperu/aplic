...
DEF VAR s-Tabla AS CHAR INIT 'CFG__CC_DCT_100' NO-UNDO.
...
&Scoped-define INTERNAL-TABLES FacTabla almtabla

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table FacTabla.Codigo almtabla.Nombre ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH FacTabla WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacTabla.Tabla = s-tabla NO-LOCK, ~
      FIRST almtabla WHERE almtabla.Codigo = FacTabla.Codigo ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH FacTabla WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacTabla.Tabla = s-tabla NO-LOCK, ~
      ...
      AND almtabla.Tabla = "CC" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table FacTabla almtabla
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacTabla.Codigo COLUMN-LABEL "Cat. Contable" FORMAT "x(8)":U
      almtabla.Nombre FORMAT "x(40)":U WIDTH 48.72
      FacTabla.Campo-C[1] COLUMN-LABEL "Línea" FORMAT "x(8)":U
      FacTabla.Campo-C[2] COLUMN-LABEL "Sub-Línea" FORMAT "x(8)":U
  ENABLE
      FacTabla.Codigo
      FacTabla.Campo-C[1]
      FacTabla.Campo-C[2]
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 115 BY 14
         FONT 4.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.FacTabla,INTEGRAL.almtabla WHERE INTEGRAL.FacTabla  ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST"
     _Where[1]         = "FacTabla.CodCia = s-codcia
 AND FacTabla.Tabla = s-tabla"
     _JoinCode[2]      = "almtabla.Codigo = FacTabla.Codigo"
     _Where[2]         = "almtabla.Tabla = ""CC"""
     _FldNameList[1]   > INTEGRAL.FacTabla.Codigo
"FacTabla.Codigo" "Cat. Contable" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacTabla.Campo-C[1]" "Línea" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacTabla.Campo-C[2]
"FacTabla.Campo-C[2]" "Sub-Línea" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FacTabla.Codigo IN BROWSE br_table /* Cat. Contable */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FacTabla.Codigo br_table _BROWSE-COLUMN B-table-Win
ON LEFT-MOUSE-DBLCLICK OF FacTabla.Codigo IN BROWSE br_table /* Cat. Contable */
OR F8 OF FacTabla.Codigo
    DO:
        input-var-1 = 'CC'.
        input-var-2 = ''.
        input-var-3 = ?.
...
IF RETURN-VALUE = 'YES' THEN DO:
    IF CAN-FIND(FacTabla WHERE FacTabla.CodCia = s-codcia AND
                ...
                FacTabla.codigo =  FacTabla.Codigo:SCREEN-VALUE IN BROWSE {&browse-name}
                NO-LOCK)
        THEN DO:
        MESSAGE 'Cat. Contable ya registrada' VIEW-AS ALERT-BOX WARNING.
...
IF NOT CAN-FIND(Almtabla WHERE Almtabla.tabla = 'CC' AND
                Almtabla.codigo = FacTabla.Codigo:SCREEN-VALUE IN BROWSE {&browse-name}
                NO-LOCK)
    THEN DO:
