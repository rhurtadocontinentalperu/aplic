...
DEF VAR s-Tabla AS CHAR INIT 'FLETE' NO-UNDO.
...
GN-DIVI.Campo-Log[4] FacTabla.Valor[1] FacTabla.Valor[2] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacTabla.Codigo COLUMN-LABEL "División/!Lista Precio" FORMAT "x(8)":U
      ...
      FacTabla.Valor[2] COLUMN-LABEL "Flete Unitario x m3!S/ con IGV" FORMAT ">>>,>>9.9999":U
            WIDTH 2.72 COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 11
  ENABLE
      FacTabla.Valor[1]
      FacTabla.Valor[2]
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 75 BY 9.15
...
/* Query rebuild information for BROWSE br_table
  ...
  AND GN-DIVI.CodDiv = FacTabla.Codigo"
     _Where[2]         = "GN-DIVI.Campo-Log[4] = TRUE"
     _FldNameList[1]   > INTEGRAL.FacTabla.Codigo
"FacTabla.Codigo" "División/!Lista Precio" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.GN-DIVI.DesDiv
     _FldNameList[3]   > INTEGRAL.GN-DIVI.Campo-Log[4]
"GN-DIVI.Campo-Log[4]" "FLETE?" "SI/NO" "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "TOGGLE-BOX" "," ? ? 5 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacTabla.Valor[1]
"FacTabla.Valor[1]" "Flete Unitario x kg.!S/ con IGV" ">>>,>>9.9999" "decimal" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.FacTabla.Valor[2]
"FacTabla.Valor[2]" "Flete Unitario x m3!S/ con IGV" ">>>,>>9.9999" "decimal" 11 0 ? ? ? ? yes ? no no "2.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
FOR EACH GN-DIVI NO-LOCK WHERE GN-DIVI.CodCia = s-CodCia AND GN-DIVI.Campo-Log[1] = NO:
    FIND FIRST FacTabla WHERE {&Condicion} AND FacTabla.Codigo = GN-DIVI.CodDiv NO-LOCK NO-ERROR.
...
DO iCounter = 1 TO hBrowse:NUM-COLUMNS:
    ...
    IF hColumn:NAME = "Valor" THEN hColumn:READ-ONLY = TRUE.
END.
...
------------------------------------------------------------------------------*/
  ...
  ELSE pEvento = 'UPDATE'.
  RUN lib/logtabla ('FLETE_COMERCIAL',
                    FacTabla.Codigo + '|' + 
                    STRING(FacTabla.Valor[1]) + '|' + 
                    STRING(FacTabla.Valor[2]),
                    pEvento).
