...
pMensaje = ''.
/* Buscamos la OTR Origen */
FIND ORDEN WHERE ORDEN.codcia = B-CPEDI.codcia 
    AND ORDEN.codref = B-CPEDI.codref       /* R/A */
    AND ORDEN.nroref = B-CPEDI.nroref 
    AND ORDEN.coddoc = B-CPEDI.coddoc       /* OTR */
    AND ORDEN.crossdocking = YES
    AND ORDEN.flgest <> "A"
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE ORDEN THEN DO:
    pMensaje = "NO se pudo ubicar la OTR Origen".
