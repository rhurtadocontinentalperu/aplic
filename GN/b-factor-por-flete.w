...
DEF VAR s-Tabla AS CHAR INIT 'FACTORPORFLETE' NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almtfami.desfam FORMAT "X(30)":U WIDTH 42.43
      VtaTabla.Valor[1] COLUMN-LABEL "Factor!en %" FORMAT ">9.99":U
            WIDTH 3.43
  ENABLE
      VtaTabla.Llave_c2
      VtaTabla.Valor[1]
...
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 57 BY 6.69
         FONT 4
         TITLE "% MINIMO A COBRAR POR FLETE (si es menor NO cobrar)" FIT-LAST-COLUMN.
...
"Almtfami.desfam" ? ? "character" ? ? ? ? ? ? no ? no no "42.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaTabla.Valor[1]
"VtaTabla.Valor[1]" "Factor!en %" ">9.99" "decimal" ? ? ? ? ? ? yes ? no no "3.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* % MINIMO A COBRAR POR FLETE (si es menor NO cobrar) */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* % MINIMO A COBRAR POR FLETE (si es menor NO cobrar) */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* % MINIMO A COBRAR POR FLETE (si es menor NO cobrar) */
DO:
