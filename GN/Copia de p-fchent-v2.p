...
IF CAN-FIND(FIRST FacTabla WHERE FacTabla.CodCia = s-codcia
    ...
    THEN DO:
    /* Como mínimo un día */
    IF pFchEnt - pFechaBase < 1 THEN pFchEnt = pFechaBase + 1.
...
IF CAN-FIND(FIRST FacTabla WHERE FacTabla.CodCia = s-codcia
            ...
            AND FacTabla.Codigo = pDivOri NO-LOCK)
    THEN DO:
    /* Como mínimo un día */
    IF pFchEnt - pFechaBase < 1 THEN pFchEnt = pFechaBase + 1.
    IF WEEKDAY(pFchEnt) = 1 THEN pFchEnt = pFchEnt + 1.
...
FIND Gn-divi WHERE Gn-divi.codcia = s-codcia
    ...
    NO-LOCK NO-ERROR.
CASE TRUE:
    WHEN GN-DIVI.CanalVenta = "TDA" OR GN-DIVI.CodDiv = "00024"
        THEN DO:    /* Tiendas Mayoristas  o Institucionales */
...
CASE pUbigeo:
    WHEN "P0" THEN cZonaGHR = "02".     /* LIMA CENTRO (AGENCIAS) */
    WHEN "CR" THEN cZonaGHR = "CR".     /* CLIENTE RECOGE */
    OTHERWISE DO:
...
REPEAT:
    ...
    IF B-TABLA.Valor[1] = 0                 /* NO tiene mínimos definidos, fecha OK */
        AND B-TABLA.Valor[2] = 0 AND B-TABLA.Valor[3] = 0 AND B-TABLA.Valor[4] = 0
        THEN LEAVE. 
    /* ************************************************************* */
