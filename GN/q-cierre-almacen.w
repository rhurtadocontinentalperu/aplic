...
&Scoped-define QUERY-STRING-Query-Main FOR EACH TabGener WHERE ~{&KEY-PHRASE} ~
 ...
 AND TabGener.Codigo = s-codigo NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Valor-Inicial q-tables 
PROCEDURE Valor-Inicial :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF NOT CAN-FIND(FIRST TabGener WHERE TabGener.CodCia = s-codcia
                AND TabGener.Clave = s-tabla
                AND TabGener.Codigo= s-codigo NO-LOCK)
    THEN DO:
    CREATE TabGener.
