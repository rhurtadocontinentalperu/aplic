...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 D-Dialog _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      GN-DIVI.CodDiv COLUMN-LABEL "Código" FORMAT "x(5)":U
      GN-DIVI.DesDiv COLUMN-LABEL "Descripción" FORMAT "x(60)":U
            WIDTH 43.86
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME D-Dialog
     ...
     BUTTON-1 AT ROW 9.54 COL 57.29 WIDGET-ID 2
     BUTTON-2 AT ROW 10.69 COL 57.29 WIDGET-ID 4
     SPACE(1.70) SKIP(7.83)
...
"GN-DIVI.CodDiv" "Código" "x(5)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.GN-DIVI.DesDiv
"GN-DIVI.DesDiv" "Descripción" "x(60)" "character" ? ? ? ? ? ? no ? no no "43.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
DO:
  ...
  DO i = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS:
    IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(i) THEN DO:
        IF i = 1 
        THEN pCodigos = TRIM(GN-DIVI.CodDiv).
        ELSE pCodigos = pCodigos + ',' + TRIM(GN-DIVI.CodDiv).
    END.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF NUM-ENTRIES(pCodigos) > 0 THEN DO WITH FRAME {&FRAME-NAME}:
      DO iItem = 1 TO NUM-ENTRIES(pCodigos):
          ...
          REPEAT WHILE AVAILABLE Gn-Divi:
              IF Gn-Divi.CodDiv = ENTRY(iItem,pCodigos) THEN DO:
                  REPOSITION {&browse-name} TO ROWID ROWID(Gn-Divi).
