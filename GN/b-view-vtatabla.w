...
DEFINE TEMP-TABLE t-VtaTabla NO-UNDO LIKE VtaTabla
       ...
       INDEX Llave01 CodCia ttLogDate.
...
&Scoped-define FIELDS-IN-QUERY-br_table t-VtaTabla.Llave_c1 ~
t-vtatabla.ttlogdate @ xtLogDate t-vtatabla.ttevent @ xtEvent ~
t-vtatabla.ttusuario @ xtUsuario t-VtaTabla.Llave_c2 ~
t-VtaTabla.Rango_fecha[1] t-VtaTabla.Rango_fecha[2] ~
t-VtaTabla.Rango_valor[1] t-VtaTabla.Rango_valor[2] 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH t-VtaTabla WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH t-VtaTabla WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table t-VtaTabla
&Scoped-define FIRST-TABLE-IN-QUERY-br_table t-VtaTabla
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table SmartBrowser _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      t-VtaTabla.Llave_c1 COLUMN-LABEL "Codigo" FORMAT "x(8)":U
      t-vtatabla.ttlogdate @ xtLogDate COLUMN-LABEL "Fecha" FORMAT "99/99/9999 HH:MM:SS":U
      t-vtatabla.ttevent @ xtEvent COLUMN-LABEL "Evento" FORMAT "x(8)":U
      t-vtatabla.ttusuario @ xtUsuario COLUMN-LABEL "Usuario" FORMAT "x(10)":U
      t-VtaTabla.Llave_c2 COLUMN-LABEL "División" FORMAT "x(8)":U
      t-VtaTabla.Rango_fecha[1] COLUMN-LABEL "Inicio" FORMAT "99/99/9999":U
      t-VtaTabla.Rango_fecha[2] COLUMN-LABEL "Fin" FORMAT "99/99/9999":U
      t-VtaTabla.Rango_valor[1] COLUMN-LABEL "Descuento" FORMAT "->>9.9999":U
      t-VtaTabla.Rango_valor[2] COLUMN-LABEL "Precio" FORMAT "->>>,>>9.9999":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"t-VtaTabla.Llave_c1" "Codigo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > "_<CALC>"
"t-vtatabla.ttlogdate @ xtLogDate" "Fecha" "99/99/9999 HH:MM:SS" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > "_<CALC>"
"t-vtatabla.ttevent @ xtEvent" "Evento" "x(8)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > "_<CALC>"
"t-vtatabla.ttusuario @ xtUsuario" "Usuario" "x(10)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.t-VtaTabla.Llave_c2
"t-VtaTabla.Llave_c2" "División" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.t-VtaTabla.Rango_fecha[1]
"t-VtaTabla.Rango_fecha[1]" "Inicio" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.t-VtaTabla.Rango_fecha[2]
"t-VtaTabla.Rango_fecha[2]" "Fin" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.t-VtaTabla.Rango_valor[1]
"t-VtaTabla.Rango_valor[1]" "Descuento" "->>9.9999" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.t-VtaTabla.Rango_valor[2]
"t-VtaTabla.Rango_valor[2]" "Precio" "->>>,>>9.9999" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
FOR EACH logtransactions NO-LOCK WHERE
    ...
    ASSIGN
        t-vtatabla.ttlogdate = logtransactions.logdate
        t-vtatabla.ttevent = logtransactions.event
        t-vtatabla.ttusuario = logtransactions.Usuario.
END.
