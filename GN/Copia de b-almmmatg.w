...
&Scoped-Define ENABLED-OBJECTS BUTTON-11 RADIO-SET-TpoArt COMBO-BOX-CodFam ~
FILL-IN-CodMat COMBO-BOX-SubFam FILL-IN-DesMat FILL-IN-DesMar BUTTON-10 ~
BUTTON-6 br_table RECT-1 
&Scoped-Define DISPLAYED-OBJECTS EDITOR-1 FILL-IN-Temporada ~
RADIO-SET-TpoArt COMBO-BOX-CodFam FILL-IN-CodMat COMBO-BOX-SubFam ~
FILL-IN-DesMat FILL-IN-DesMar FILL-IN-Mensaje 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 106 BY 6.69
...
DEFINE FRAME F-Main
     ...
     FILL-IN-Mensaje AT ROW 12.15 COL 1 NO-LABEL WIDGET-ID 28
     BUTTON-10 AT ROW 1 COL 60 WIDGET-ID 32
     BUTTON-6 AT ROW 1 COL 75 WIDGET-ID 24
     br_table AT ROW 5.42 COL 1
     "Estado:" VIEW-AS TEXT
...
DO TRANSACTION ON ERROR UNDO, RETURN ERROR ON STOP UNDO, RETURN ERROR:
    ...
    FOR EACH Almmmate WHERE Almmmate.codcia = Almmmatg.codcia 
...
IF NOT AVAILABLE Almcfggn OR LOOKUP(AlmCfgGn.Temporada, 'C,NC') = 0 THEN DO:
    ...
    RETURN.
END.
IF COMBO-BOX-CodFam = 'Todas' AND FILL-IN-CodMat = '' THEN DO:
    MESSAGE 'NO se puede procesar TODAS als líneas' SKIP 'Proceso abortado'
        VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.
...
DO WHILE AVAILABLE Almmmatg TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    /*MESSAGE 'Inicio:' almmmatg.codmat.*/
