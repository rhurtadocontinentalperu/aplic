...
&Scoped-Define ENABLED-OBJECTS RECT-29 RECT-30 RECT-1 br_table 
&Scoped-Define DISPLAYED-OBJECTS F-descuento F-CODIGO F-ubase F-descrip ~
F-marca F-totstk F-comprometido F-dispo 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 31.86 BY 3.23
...
DEFINE FRAME F-Main
     br_table AT ROW 1.23 COL 11.14
     F-descuento AT ROW 4.85 COL 1.29 NO-LABEL
     F-CODIGO AT ROW 5.85 COL 9 COLON-ALIGNED
     F-ubase AT ROW 5.85 COL 32.86 COLON-ALIGNED
     F-descrip AT ROW 6.77 COL 9 COLON-ALIGNED
     F-marca AT ROW 7.73 COL 9 COLON-ALIGNED
     F-totstk AT ROW 9 COL 17.29 COLON-ALIGNED
     F-comprometido AT ROW 9.96 COL 17.29 COLON-ALIGNED
     F-dispo AT ROW 10.88 COL 17.29 COLON-ALIGNED
     RECT-29 AT ROW 5.73 COL 1.29
     RECT-30 AT ROW 8.77 COL 1.29
...
DO:
  IF whpadre <> ? THEN RUN p-aceptar IN whpadre.
END.
 ...
 *                    AND  LOOKUP(TRIM(FacDPedi.CodDoc),'PED') > 0 
...
IF AVAIL B-Almmmatg THEN DO:
    FIND almmmate WHERE Almmmate.CodCia = B-Almmmatg.CodCia
                   ...
                   AND  Almmmate.codmat = trim(B-Almmmatg.codmat)
                  NO-LOCK.
    IF AVAILABLE almmmate THEN 
        ASSIGN
            F-totstk    = Almmmate.StkAct
            F-CODIGO    = trim(B-Almmmatg.codmat)  
            F-descrip   = B-Almmmatg.DesMat
            F-marca     = B-Almmmatg.DesMar
            F-ubase     = trim(B-Almmmatg.UndBas)
            F-comprometido = X-CanPed
            F-dispo     = F-totstk - X-CanPed
            .

    /****   PRECIO A    ****/
    IF trim(B-Almmmatg.UndA) <> "" THEN DO:
        CREATE tmp-tabla.
        ASSIGN tmp-tabla.UM = trim(B-Almmmatg.UndA)
    ...
    /****   PRECIO B    ****/
    IF trim(B-Almmmatg.UndB) <> "" THEN DO:
        CREATE tmp-tabla.
        ASSIGN tmp-tabla.UM = trim(B-Almmmatg.UndB)
    ...
    /****   PRECIO C    ****/
    IF trim(B-Almmmatg.UndC) <> "" THEN DO:
        CREATE tmp-tabla.
        ASSIGN tmp-tabla.UM     = trim(B-Almmmatg.UndC)
