...
DEF VAR s-Tabla AS CHAR INIT "FLETEXLINEA" NO-UNDO.

DEFINE BUFFER x-vtatabla FOR vtatabla.


/* Ic - 23Dic2019, Ticket 68824 */
...
/*IF s-user-id <> 'ADMIN' THEN DO:*/
    FOR EACH x-vtatabla WHERE x-vtatabla.codcia = s-codcia AND
                                x-vtatabla.tabla = 'LP' AND 
                                x-vtatabla.llave_c1 = s-user-id NO-LOCK:
        CREATE ttLineasAutorizadas.
            ASSIGN ttLineasAutorizadas.tlinea = x-vtatabla.llave_c2.
    END.
/*END.*/
...
VtaTabla.LLave_c3 AlmSFami.dessub 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c2 COLUMN-LABEL "Linea" FORMAT "x(3)":U
      Almtfami.desfam FORMAT "X(40)":U
      VtaTabla.LLave_c3 COLUMN-LABEL "SubLinea" FORMAT "x(3)":U
      AlmSFami.dessub FORMAT "X(40)":U WIDTH 16.14
  ENABLE
      VtaTabla.Llave_c2
      VtaTabla.LLave_c3
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 76 BY 11.31
         FONT 4
         TITLE "LINEAS (Y SUBLINEAS) AFECTAS A FLETE" FIT-LAST-COLUMN.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaTabla WHERE INTEGRAL.GN-DIVI <external> ...,INTEGRAL.Almtfami WHERE INTEGRAL.VtaTabla ...,INTEGRAL.AlmSFami WHERE INTEGRAL.VtaTabla ..."
...
"VtaTabla.Llave_c2" "Linea" "x(3)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaTabla.LLave_c3" "SubLinea" "x(3)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.AlmSFami.dessub
"AlmSFami.dessub" ? "X(40)" "character" ? ? ? ? ? ? no ? no no "16.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* LINEAS (Y SUBLINEAS) AFECTAS A FLETE */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* LINEAS (Y SUBLINEAS) AFECTAS A FLETE */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* LINEAS (Y SUBLINEAS) AFECTAS A FLETE */
DO:
...
ON LEAVE OF VtaTabla.Llave_c2 IN BROWSE br_table /* Linea */
DO:
  FIND Almtfami WHERE Almtfami.CodCia = s-CodCia AND Almtfami.codfam = SELF:SCREEN-VALUE
      NO-LOCK NO-ERROR.
...
/* Ic - 23Dic2019, Ticket 68824 */
ON FIND OF vtatabla DO:
        
    FIND FIRST ttLineasAutorizadas WHERE tLinea = vtatabla.llave_c2 NO-LOCK NO-ERROR.
    IF NOT AVAILABLE ttLineasAutorizadas THEN DO:
        /* Linea NO esta asignado al usuario */
        RETURN ERROR.
    END.
    RETURN.
END.
...
IF TRUE <> (VtaTabla.Llave_c2:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > '')  OR
    NOT AVAILABLE Almtfami THEN DO:
...
IF VtaTabla.LLave_c3:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > '' THEN DO:
    FIND Almsfami WHERE AlmSFami.CodCia = s-CodCia AND 
        AlmSFami.codfam = VtaTabla.Llave_c2:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} AND
        AlmSFami.subfam = VtaTabla.LLave_c3:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
        NO-LOCK NO-ERROR.
...
FIND FIRST ttLineasAutorizadas WHERE tLinea = VtaTabla.Llave_c2:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} NO-LOCK NO-ERROR.
IF NOT AVAILABLE ttLineasAutorizadas THEN DO:
    /* Linea NO esta asignado al usuario */
    MESSAGE 'Línea NO esta asignado al usuario' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO VtaTabla.Llave_c2.
    RETURN 'ADM-ERROR'.
