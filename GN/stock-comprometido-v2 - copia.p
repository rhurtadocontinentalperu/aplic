    ...
    FOR EACH B-DPEDI USE-INDEX Llave04 NO-LOCK WHERE B-DPEDI.codcia = s-CodCia
        ...
        IF TimeOut > 0 THEN DO:
            ...
            THEN DO:
                ...
                /*MESSAGE b-cpedi.coddoc b-cpedi.nroped.*/
...
DEF VAR LocalDocu AS CHAR NO-UNDO.
...
END.
/* DO LocalItem = 1 TO 3:                                                                           */
/*     LocalDocu = ENTRY(LocalItem, LocalCodDoc).                                                   */
/*     FOR EACH B-DPEDI NO-LOCK WHERE B-DPEDI.codcia = s-CodCia                                     */
/*         AND B-DPEDI.almdes = pCodAlm                                                             */
/*         AND B-DPEDI.codmat = pCodMat                                                             */
/*         AND B-DPEDI.coddoc = LocalDocu                                                           */
/*         AND B-DPEDI.flgest = 'P',                                                                */
/*         FIRST B-CPEDI OF B-DPEDI NO-LOCK WHERE LOOKUP(B-CPEDI.FlgEst, "G,X,P,W,WX,WL") > 0:      */
...
/*     IF NOT LOOKUP(B-CREPO.TipMov, 'A,M,RAN,INC') > 0 THEN NEXT.            */
/*     /*MESSAGE b-crepo.nrodoc.*/                                            */
/*     pComprometido = pComprometido + (B-DREPO.CanApro - B-DREPO.CanAten).   */
/* END.                                                                       */
