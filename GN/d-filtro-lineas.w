...
&Scoped-define QUERY-STRING-BROWSE-3 FOR EACH Almtfami ~
      WHERE Almtfami.CodCia = s-codcia NO-LOCK
&Scoped-define OPEN-QUERY-BROWSE-3 OPEN QUERY BROWSE-3 FOR EACH Almtfami ~
      WHERE Almtfami.CodCia = s-codcia NO-LOCK.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-3 D-Dialog _STRUCTURED
  QUERY BROWSE-3 NO-LOCK DISPLAY
      ...
      Almtfami.desfam FORMAT "X(30)":U WIDTH 59.86
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME D-Dialog
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "SELECCIONE UNA O VARIAS LINEAS"
...
/* Query rebuild information for BROWSE BROWSE-3
     ...
     _Options          = "NO-LOCK"
     _Where[1]         = "Almtfami.CodCia = s-codcia"
...
"Almtfami.desfam" ? ? "character" ? ? ? ? ? ? no ? no no "59.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-3 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* SELECCIONE UNA O VARIAS LINEAS */
DO:  
...
DO:
    ...
    DO i = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS:
      IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(i) THEN DO:
          IF i = 1 
          THEN pCodigos = TRIM(Almtfami.CodFam).
          ELSE pCodigos = pCodigos + ',' + TRIM(Almtfami.CodFam).
      END.
    END.
END.
  ...
  IF NUM-ENTRIES(pCodigos) > 0 THEN DO WITH FRAME {&FRAME-NAME}:
      DO iItem = 1 TO NUM-ENTRIES(pCodigos):
          ...
          REPEAT WHILE AVAILABLE Almtfami:
              IF Almtfami.CodFam = ENTRY(iItem,pCodigos) THEN DO:
                  REPOSITION {&browse-name} TO ROWID ROWID(Almtfami).
