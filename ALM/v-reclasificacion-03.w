...
------------------------------------------------------------------------------*/
  ...
  DEF VAR f-Factor-1 LIKE INTEGRAL.AlmMatEqu.Factor.

  DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
      /* SALIDAS */
      /* RHC 28.01.10 El codigo se puede repetir => acumular por codigo */
      N-Itm = 0.
      FOR EACH ITEM WHERE ITEM.codmat <> "" BY ITEM.NroItm:
          FIND Almdmov WHERE Almdmov.codcia = Almcmov.codcia
              ...
              AND Almdmov.codmat = ITEM.codmat
              NO-ERROR.
          ...
          ASSIGN
              ...
              Almdmov.CanDev = Almdmov.CanDev + ITEM.CanDev.
      ...
      FOR EACH ITEM WHERE ITEM.codant <> "" BY ITEM.NroItm:
          ...
          ASSIGN 
              ...
              R-ROWID = ROWID(Almdmov).
          ASSIGN 
              Almdmov.CanDes = Almdmov.candes + ITEM.CanDev.
          /* Valorizacion */
          FIND AlmMatEqu WHERE AlmMatEqu.CodCia = s-codcia
              AND AlmMatEqu.codmat = ITEM.CodAnt
              AND AlmMatEqu.codmat2 = ITEM.CodMat
              NO-LOCK.
          f-Factor-1 =  AlmMatEqu.Factor.
          ...
          THEN ASSIGN
              Almdmov.PreUni = AlmStkge.CtoUni * f-Factor-1.
