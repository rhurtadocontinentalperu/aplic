...
FOR EACH T-MATG NO-LOCK,
    ...
    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} =
    "*** CALCULANDO Código: " + T-MATG.codmat.
    ...
    ASSIGN
        ...
        T-GENER.libre_d01 = Almmmate.StkAct - Almmmate.StkComprometido.
    ASSIGN
...
IF rsCuales = 2 THEN DO:
    ...
    /**/
    FOR EACH T-MATG :
        FIND FIRST T-GENER NO-LOCK WHERE T-GENER.CodCia = T-MATG.CodCia
                                            AND T-GENER.Clave = "ZG"
                                            AND T-GENER.Libre_c03 = T-MATG.CodMat NO-ERROR.
        /**/
...
FOR EACH T-MATG NO-LOCK,
    ...
    IF AVAILABLE B-AlmTabla THEN DO:
        ...
        /* Datos Resumen */
        ASSIGN
            t-report.Campo-F[2] = t-report.Campo-F[2] + Almmmate.VInMn1     /* Maximo */
            t-report.Campo-F[3] = t-report.Campo-F[3] + Almmmate.VInMn2     /* Seguridad */
            ...
            t-report.Campo-F[6] = t-report.Campo-F[6] + T-GENER.Libre_d02   /* Tránsito */
            t-report.Campo-F[7] = t-report.Campo-F[7] + T-GENER.Libre_d04   /* Compra Tránsito */
            .
    END.
END.
FOR EACH t-report :
...
FOR EACH bt-gener BREAK BY libre_c03 BY codigo BY libre_c01 :
    ...
    cRange = ENTRY(iColumn,cColList,',') + cColumn.
    chWorkSheet:Range(cRange):VALUE = if( AVAILABLE b-almmmate) THEN  b-almmmate.vinmn1 ELSE 0.
    /* Seguridad */
    iColumn = fPosCelda(bt-gener.codigo,bt-gener.libre_c01,3).
    cRange = ENTRY(iColumn,cColList,',') + cColumn.
    chWorkSheet:Range(cRange):VALUE = if(AVAILABLE b-almmmate) THEN b-almmmate.vinmn2 ELSE 0.
    /* Transito Transito */
    ...
    lTotal = if( AVAILABLE b-almmmate) THEN b-almmmate.vinmn1 ELSE 0.
    lTotal = lTotal * -1.
    lTotal = lTotal - (if(AVAILABLE b-almmmate) THEN b-almmmate.vinmn2 ELSE 0).
...
/*         /* Entero superior */                                                                              */
/*         IF TRUNCATE(T-MATE.VInMn2,0) <> T-MATE.VInMn2 THEN DO:                                             */
/*             ASSIGN T-MATE.VInMn2 = TRUNCATE(T-MATE.VInMn2,0) + 1.                                          */
/*         END.                                                                                               */
/*     END.                                                                                                   */
/*     /* Stock Maximo */                                                                                     */
/*     FOR EACH T-MATE:                                                                                       */
