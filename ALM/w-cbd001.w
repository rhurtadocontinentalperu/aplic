...
DEF VAR x-FchAst  AS DATE.

DEF VAR x-Mensaje-1 AS CHAR FORMAT "x(30)" LABEL "Almacen".
DEF VAR x-Mensaje-2 AS CHAR FORMAT "x(30)" LABEL "Detalle".
DEF FRAME f-Mensaje 
    SPACE(1) SKIP
    x-Mensaje-1 SKIP
    x-Mensaje-2 SKIP
    SPACE(1) SKIP
    WITH SIDE-LABELS OVERLAY CENTERED VIEW-AS DIALOG-BOX TITLE "Procesando Informacion ....".

DEF TEMP-TABLE REPORTE LIKE T-DMOV.
...
&Scoped-Define ENABLED-OBJECTS RECT-25 COMBO-BOX-Periodo COMBO-BOX-NroMes ~
COMBO-BOX-CodDiv BUTTON-1 BUTTON-3 BROWSE-1 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-Periodo COMBO-BOX-NroMes ~
COMBO-BOX-CodDiv FILL-IN-Debe-1 FILL-IN-Debe-2 FILL-IN-Haber-1 ~
FILL-IN-Haber-2 
...
DEFINE FRAME F-Main
     COMBO-BOX-Periodo AT ROW 1.38 COL 11 COLON-ALIGNED
     COMBO-BOX-NroMes AT ROW 1.38 COL 28 COLON-ALIGNED
     COMBO-BOX-CodDiv AT ROW 1.38 COL 44 COLON-ALIGNED
     BUTTON-1 AT ROW 2.54 COL 5
     BUTTON-2 AT ROW 2.54 COL 18
     BUTTON-3 AT ROW 2.54 COL 33
     BROWSE-1 AT ROW 4.46 COL 3
     FILL-IN-Debe-1 AT ROW 13.88 COL 73 COLON-ALIGNED
     FILL-IN-Debe-2 AT ROW 13.88 COL 92 COLON-ALIGNED
     FILL-IN-Haber-1 AT ROW 14.85 COL 73 COLON-ALIGNED
     FILL-IN-Haber-2 AT ROW 14.85 COL 92 COLON-ALIGNED
     "DEBE:" VIEW-AS TEXT
          SIZE 5 BY .5 AT ROW 14.08 COL 65
     "HABER:" VIEW-AS TEXT
          SIZE 6 BY .5 AT ROW 15.04 COL 65
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "ASIENTO CONTABLE COSTOS DE VENTAS"
         HEIGHT             = 15.23
         WIDTH              = 108.43
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 108.43
...
/* Query rebuild information for BROWSE BROWSE-1
     ...
     _OrdList          = "Temp-Tables.T-DMOV.CodOpe|yes"
...
ON END-ERROR OF W-Win /* ASIENTO CONTABLE COSTOS DE VENTAS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ASIENTO CONTABLE COSTOS DE VENTAS */
DO:
...
DO:
  ...
  THEN DO:
    ...
    FIND cb-peri WHERE cb-peri.CodCia  = s-codcia  
      AND cb-peri.Periodo = COMBO-BOX-Periodo NO-LOCK NO-ERROR.
    IF AVAILABLE cb-peri 
    THEN s-NroMesCie = cb-peri.MesCie[COMBO-BOX-NroMes + 1].
...
*MLR* ***/
    ASSIGN
        ...
        L-BUTTON-1 = NO.
    RUN bin/_dateif (COMBO-BOX-NroMes, COMBO-BOX-Periodo, OUTPUT x-Fecha-1, OUTPUT x-Fecha-2).
    RUN Carga-Temporal.
...
/*
  FOR EACH CcbDCaja WHERE ccbdcaja.codcia = s-codcia
        AND ccbdcaja.coddoc = "N/C"
        AND ccbdcaja.fchdoc = FILL-IN-FchDoc,
...
DEFINE VAR x-ImpMn1  AS DECI NO-UNDO.
DEFINE VAR x-ImpMn2  AS DECI NO-UNDO.
...
FIND FIRST cb-control WHERE cb-control.CodCia  = s-codcia 
    ...
    AND cb-control.tipo    = '@CVT' 
    AND cb-control.fchpro  = x-FchAst 
    NO-LOCK NO-ERROR.
...
FOR EACH cb-control WHERE cb-control.CodCia  = s-codcia 
    ...
    END.   
END.
/* SE VA A RESUMIR POR CUENTA Y CENTRO DE COSTO */
FOR EACH T-DMOV BREAK BY T-DMOV.codope BY T-DMOV.coddiv BY T-DMOV.codcta BY T-DMOV.cco BY T-DMOV.TpoMov:
    IF FIRST-OF(T-DMOV.codope) THEN DO:
        p-codope = T-DMOV.codope.
        /* Verifico si el movimiento se realiz¢ anteriormente */
        FIND cb-control WHERE cb-control.codcia = p-codcia
            AND cb-control.periodo = p-periodo
            AND cb-control.nromes = p-nromes
            AND cb-control.coddiv = T-DMOV.coddiv
            AND cb-control.codope = p-codope
            AND cb-control.tipo   = '@CVT'
            AND cb-control.fchpro = x-FchAst
            EXCLUSIVE-LOCK NO-ERROR.
        IF AVAILABLE cb-control
        THEN p-NroAst = cb-control.nroast.
        ELSE DO:
            CREATE cb-control.
            ASSIGN
        ...
        RELEASE cb-control.
    END.
    IF FIRST-OF(T-DMOV.codcta) OR FIRST-OF(T-DMOV.coddiv) OR FIRST-OF(T-DMOV.cco) OR FIRST-OF(T-DMOV.tpomov)
    THEN DO:
        ASSIGN
            x-ImpMn1 = 0
            x-ImpMn2 = 0.
    END.
    ASSIGN
        x-ImpMn1 = x-ImpMn1 + T-DMOV.impmn1
        x-ImpMn2 = x-ImpMn2 + T-DMOV.impmn2.
    IF LAST-OF(T-DMOV.codcta) OR LAST-OF(T-DMOV.coddiv) OR LAST-OF(T-DMOV.cco) OR LAST-OF(T-DMOV.tpomov)
    THEN DO:
        /* DETALLE */
        FIND cb-ctas WHERE
        ...
        ASSIGN
            ...
            CB-DMOV.impmn1  = x-impmn1
            CB-DMOV.impmn2  = x-impmn2
        ...
        /* ******************************** */
    END.
    IF LAST-OF(T-DMOV.codope) THEN DO:
        FIND cb-cmov WHERE
        ...
        ASSIGN
            ...
            cb-cmov.NOTAST = 'Costo de Ventas al ' + STRING(x-FchAst)
            cb-cmov.GLOAST = 'Costo de Ventas al ' + STRING(x-FchAst).
    END.
END.
RUN Actualiza-Flag.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  /* IMPRESION SIMILAR AL REGISTRO DE VENTAS GENERADO */
  DEFINE VAR x-clfaux  AS CHAR NO-UNDO.
  DEFINE VAR x-genaut  AS INTE NO-UNDO.
  DEFINE VAR I         AS INTE NO-UNDO.
  DEFINE VAR J         AS INTE NO-UNDO.
  ...
  FOR EACH REPORTE:
    DELETE REPORTE.
  END.
  FOR EACH T-DMOV BREAK BY T-DMOV.codope 
                        BY T-DMOV.coddiv 
                        BY T-DMOV.codcta 
                        BY T-DMOV.cco 
                        BY T-DMOV.TpoMov:
    IF FIRST-OF(T-DMOV.codope) THEN j  = 0.
    IF FIRST-OF(T-DMOV.codcta) 
        OR FIRST-OF(T-DMOV.coddiv) 
        OR FIRST-OF(T-DMOV.cco) 
        OR FIRST-OF(T-DMOV.tpomov)
    THEN DO:
        ASSIGN
            x-ImpMn1 = 0
            x-ImpMn2 = 0.
    ...
    ASSIGN
        x-ImpMn1 = x-ImpMn1 + T-DMOV.impmn1
        x-ImpMn2 = x-ImpMn2 + T-DMOV.impmn2.
    IF LAST-OF(T-DMOV.codcta) 
        OR LAST-OF(T-DMOV.coddiv) 
        OR LAST-OF(T-DMOV.cco) 
        OR LAST-OF(T-DMOV.tpomov)
    THEN DO:
        /* DETALLE */
        FIND cb-ctas WHERE
            cb-ctas.codcia = cb-codcia AND
        ...
        J = J + 1.
        CREATE REPORTE.
        BUFFER-COPY T-DMOV TO REPORTE.
        ASSIGN
            REPORTE.NROITM  = J
            REPORTE.codcta  = T-DMOV.codcta
            REPORTE.coddiv  = T-DMOV.coddiv
            REPORTE.cco     = T-DMOV.cco
            REPORTE.Coddoc  = IF x-coddoc THEN T-DMOV.coddoc ELSE ''
            REPORTE.Nrodoc  = IF x-coddoc THEN T-DMOV.nrodoc ELSE ''
            REPORTE.clfaux  = IF x-clfaux <> '' THEN x-clfaux ELSE ''
            REPORTE.codaux  = IF x-clfaux <> '' THEN T-DMOV.codaux ELSE ''
            REPORTE.Nroruc  = IF x-clfaux <> '' THEN T-DMOV.nroruc ELSE ''
            REPORTE.GLODOC  = T-DMOV.glodoc
            REPORTE.tpomov  = T-DMOV.tpomov
            REPORTE.impmn1  = x-impmn1
            REPORTE.impmn2  = x-impmn2
            REPORTE.FCHDOC  = T-DMOV.Fchdoc
            REPORTE.FCHVTO  = T-DMOV.Fchvto
            REPORTE.FLGACT  = TRUE
            REPORTE.RELACION = 0
            REPORTE.TM      = T-DMOV.tm
            REPORTE.codmon  = T-DMOV.codmon
            REPORTE.tpocmb  = T-DMOV.tpocmb
            REPORTE.c-fcaja  = T-DMOV.c-fcaja.
        /* ******************************** */
    END.
END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH T-DMOV:
    DELETE T-DMOV.
  END.
  /* SOLO ALMACENES PROPIOS */
  FOR EACH Almacen WHERE Almacen.codcia = s-codcia
      AND (COMBO-BOX-CodDiv = 'Todas' OR Almacen.coddiv = COMBO-BOX-CodDiv)
      AND Almacen.FlgRep = YES NO-LOCK:
    /* DIVISION REAL */
    x-CodDiv = Almacen.CodDiv.
    /* ************* */
    FOR EACH AlmDMov USE-INDEX ALMD04 NO-LOCK WHERE almdmov.codcia = s-codcia
            ...
            FIRST Cb-CfgAlm WHERE cb-cfgalm.codcia = s-codcia
                AND cb-cfgalm.catconta = almmmatg.catconta[1] 
                AND cb-cfgalm.tipmov   = 'X' NO-LOCK:
        /* SOLO TOMAMOS LOS MOVIMIENTOS Y CATEGORIAS CONFIGURADOS CONTABLEMENTE */
        IF cb-cfgalm.CodctaD[1] = '' AND cb-cfgalm.CodctaH[1] = '' THEN NEXT.
        IF Almtmovm.Codope = '' AND (Almtmovm.Ctadbe1 = '' AND Almtmovm.Ctahbe1 = '') THEN NEXT.
        /* ******************************************************************** */
        ASSIGN
            x-Mensaje-1 = almdmov.codalm
            x-Mensaje-2 = almdmov.codmat + ' - ' + 
                                almdmov.tipmov + ' - ' +
        ...
        ASSIGN
            x-CodOpe = Almtmovm.codope
            x-ClfAux = ''
            x-CodAux = ''
            x-Cco    = almcmov.Cco.
        IF Almtmovm.PidPro = YES
        THEN ASSIGN
                x-ClfAux = '@PV'
                x-CodAux = almcmov.codpro.
        IF Almtmovm.PidCli = YES
        THEN ASSIGN
                x-ClfAux = '@CL'
                x-CodAux = almcmov.codcli.
        /* SALIDA POR VENTA */
        IF Almtmovm.TipMov = 'S' AND Almtmovm.CodMov = 02
        THEN DO:
            /* Buscamos el documento de origen */
            CASE almcmov.codref:
                WHEN 'G/R' THEN DO:
                    FIND FIRST ccbcdocu WHERE ccbcdocu.codcia = s-codcia
                        ...
                        NO-LOCK NO-ERROR.
                    IF AVAILABLE ccbcdocu THEN x-Cco = ccbcdocu.cco.
                END.
                ...
                END.
            END CASE.
            IF x-Cco = '' AND AVAILABLE CcbCDocu
            THEN DO:
        ...
        END.
        IF x-Cco = ''
        THEN CASE Almacen.CodDiv:
                WHEN '00012' THEN x-Cco = '25'.
             END CASE.

        /* CONTRACUENTA */
        IF almtmovm.ctadbe1 <> ''
        THEN ASSIGN
                x-CodCta = almtmovm.ctadbe1
                x-TpoMov = NO.
        ELSE ASSIGN
                x-CodCta = almtmovm.ctahbe1
                x-TpoMov = YES.
        
        x-Importe = almdmov.candes * almdmov.factor * almdmov.vctomn1.
        RUN Graba-Detalle (x-codope,
                            ...
                            x-Importe,
                            almdmov.FchDoc,
                            STRING(almdmov.nroser, '999') + STRING(almdmov.nrodoc, '999999'),
                            '',
                            almdmov.codmat).

        /* CUENTA DE MATERIALES 2xx */
        x-TpoMov = NOT x-TpoMov.
        IF cb-cfgalm.codctad[1] <> ''
        THEN ASSIGN
                x-CodCta = cb-cfgalm.codctad[1].
        ELSE ASSIGN
                x-CodCta = cb-cfgalm.codctah[1].
        x-Importe = almdmov.candes * almdmov.factor * almdmov.vctomn1.
        RUN Graba-Detalle (x-codope,
                            x-CodDiv,
                            '',
...
------------------------------------------------------------------------------*/
  DISPLAY COMBO-BOX-Periodo COMBO-BOX-NroMes COMBO-BOX-CodDiv FILL-IN-Debe-1 
          FILL-IN-Debe-2 FILL-IN-Haber-1 FILL-IN-Haber-2 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-25 COMBO-BOX-Periodo COMBO-BOX-NroMes COMBO-BOX-CodDiv BUTTON-1 
         BUTTON-3 BROWSE-1 
      WITH FRAME F-Main IN WINDOW W-Win.
...
ASSIGN
    p-periodo = COMBO-BOX-Periodo
    p-nromes  = COMBO-BOX-NroMes.
...
ASSIGN
    ...
    t-dmov.tm      = IF x-tm = 0 THEN cb-ctas.TM ELSE x-tm
    t-dmov.cco     = x-cco
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE cTitulo AS CHARACTER FORMAT "X(100)" NO-UNDO.

  DEFINE FRAME F-Header
    HEADER
    S-NOMCIA FORMAT "X(45)" 
    "Fecha  : " AT 110 TODAY SKIP(1)
    cTitulo AT 20 SKIP(1)
  ...
  DEFINE FRAME F-Detalle
    ...
    REPORTE.coddiv   COLUMN-LABEL "Division"
    REPORTE.codcta   COLUMN-LABEL "Cuenta"
    REPORTE.cco      COLUMN-LABEL "Cco"
    REPORTE.clfaux   COLUMN-LABEL "Clf"
    REPORTE.codaux   COLUMN-LABEL "Auxiliar"
    REPORTE.coddoc   COLUMN-LABEL "Doc"
    REPORTE.nrodoc   COLUMN-LABEL "Numero"
    REPORTE.glodoc   COLUMN-LABEL " Concepto"
    REPORTE.tpomov   COLUMN-LABEL "T"
    x-debe          COLUMN-LABEL "Debe" FORMAT ">>>>,>>>,>>9.99" 
    x-haber         COLUMN-LABEL "Haber" FORMAT ">>>>,>>>,>>9.99" 
    WITH NO-BOX WIDTH 200 STREAM-IO DOWN. 

  I-NroItm = 0.
  cTitulo = "COSTO DE VENTAS" + 
      " PERIODO: " + STRING(COMBO-BOX-Periodo,"9999") +
      " MES: " + STRING(COMBO-BOX-NroMes,"99") +
      " DIVISION: " + COMBO-BOX-CodDiv.
  FOR EACH REPORTE BREAK BY REPORTE.coddiv:
    VIEW FRAME F-Header.
    IF FIRST-OF(REPORTE.coddiv) THEN  I-Nroitm = 0.
    I-Nroitm = I-Nroitm + 1.
    x-debe   = 0.
    x-haber  = 0.
    IF REPORTE.tpomov THEN x-haber = REPORTE.impmn1.
    ELSE x-debe = REPORTE.impmn1.
    ACCUMULATE x-debe (TOTAL BY REPORTE.coddiv).
    ACCUMULATE x-haber (TOTAL BY REPORTE.coddiv).
    DISPLAY 
    ...
    IF LAST-OF(REPORTE.coddiv) THEN DO:
        UNDERLINE 
            x-debe
            x-haber
            WITH FRAME F-Detalle.
        DISPLAY 
            "        TOTAL "    @ REPORTE.glodoc
            ACCUM TOTAL BY REPORTE.coddiv x-debe  @ x-debe
            ACCUM TOTAL BY REPORTE.coddiv x-haber @ x-haber
            WITH FRAME F-Detalle.
        PAGE.
    END.
  END.    
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR x-Periodo AS CHAR INIT ''.
  
  FOR EACH CB-PERI NO-LOCK WHERE cb-peri.codcia = s-codcia BREAK BY cb-peri.codcia:
        IF FIRST-OF(cb-peri.codcia) THEN x-Periodo = STRING(cb-peri.periodo, '9999').
        ELSE x-Periodo = x-Periodo + ',' + STRING(cb-peri.periodo, '9999').
  END.
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    COMBO-BOX-Periodo:SCREEN-VALUE = STRING(YEAR(TODAY), '9999').
    COMBO-BOX-NroMes:SCREEN-VALUE = STRING(MONTH(TODAY), '99').
  END.
