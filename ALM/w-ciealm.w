...
FOR EACH AlmCierr WHERE 
    ...
    IF month(AlmCierr.FchCie) = 01 AND AlmCierr.FlgCie THEN DIA-1 = YES.
    IF month(AlmCierr.FchCie) = 02 AND AlmCierr.FlgCie THEN DIA-2 = YES.
    IF month(AlmCierr.FchCie) = 03 AND AlmCierr.FlgCie THEN DIA-3 = YES.
    IF month(AlmCierr.FchCie) = 04 AND AlmCierr.FlgCie THEN DIA-4 = YES.
    IF month(AlmCierr.FchCie) = 05 AND AlmCierr.FlgCie THEN DIA-5 = YES.
    IF month(AlmCierr.FchCie) = 06 AND AlmCierr.FlgCie THEN DIA-6 = YES.
    IF month(AlmCierr.FchCie) = 07 AND AlmCierr.FlgCie THEN DIA-7 = YES.
    IF month(AlmCierr.FchCie) = 08 AND AlmCierr.FlgCie THEN DIA-8 = YES.
    IF month(AlmCierr.FchCie) = 09 AND AlmCierr.FlgCie THEN DIA-9 = YES.
    IF month(AlmCierr.FchCie) = 10 AND AlmCierr.FlgCie THEN DIA-10 = YES.
    IF month(AlmCierr.FchCie) = 11 AND AlmCierr.FlgCie THEN DIA-11 = YES.
    IF month(AlmCierr.FchCie) = 12 AND AlmCierr.FlgCie THEN DIA-12 = YES.
...
DO D-FchCie = D-INIMES TO D-FINMES:
   ...
   IF NOT AVAILABLE AlmCierr THEN DO:
      ...
      ASSIGN AlmCierr.CodCia = S-CODCIA
             AlmCierr.FchCie = D-FchCie.
   END.
   if AlmCierr.FlgCie   = YES then 
     ASSIGN AlmCierr.FlgCie   = NO
...
RUN abrir-dat.
/*
/* MUEVE LOS INGRESOS POR CONSIGNACION */
RUN Mueve-Consignacion('I',90).
/* MUEVE LA DEVOLUCION DE CONSIGNACION */
RUN Mueve-Consignacion('S',75).
*/
RUN ABRIR-DAT.
END PROCEDURE.
