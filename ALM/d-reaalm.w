...
DEFINE FRAME D-Dialog
     ...
     FILL-IN-DiaCie AT ROW 2.65 COL 13.29 COLON-ALIGNED
...
DEFINE FRAME D-Dialog
     ...
     SPACE(2.57) SKIP(6.52)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 4
         TITLE "Reapertura de Almacen".
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Reapertura de Almacen */
DO:  
...
ON LEAVE OF FILL-IN-DiaCie IN FRAME D-Dialog /* Dia a Reaperturar */
DO:
   ASSIGN FILL-IN-DiaCie l-meses w-ano.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Generar-Reapertura D-Dialog 
PROCEDURE Generar-Reapertura :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FIND AlmCierr WHERE 
     AlmCierr.CodCia = S-CODCIA   AND
     AlmCierr.FchCie = D-FchCie NO-LOCK NO-ERROR.
IF NOT AVAILABLE AlmCierr THEN DO:
   MESSAGE "Dia no se encuentra cerrado" VIEW-AS ALERT-BOX ERROR.
   RETURN "ADM-ERROR".
END.
...
IF AVAILABLE AlmCierr THEN DO:
   ASSIGN AlmCierr.FlgCie  = NO
          AlmCierr.UsuAper = S-USER-ID.
END.
RELEASE AlmCierr.       
