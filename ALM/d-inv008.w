...
DEF TEMP-TABLE T-Ajuste
    ...
    INDEX Idx01 IS UNIQUE PRIMARY CodMat.
...
&Scoped-Define ENABLED-OBJECTS x-FchInv FILL-IN-CodUbi Btn_OK Btn_Cancel ~
Btn_Cancel-2 
&Scoped-Define DISPLAYED-OBJECTS x-FchInv FILL-IN-CodUbi 
...
DEFINE VARIABLE x-FchInv AS DATE FORMAT "99/99/99":U 
...
DEFINE FRAME F-Main
     x-FchInv AT ROW 2.73 COL 25 COLON-ALIGNED
     FILL-IN-CodUbi AT ROW 3.88 COL 25 COLON-ALIGNED
     Btn_OK AT ROW 10.35 COL 36.86
     Btn_Cancel AT ROW 10.35 COL 48.14
     Btn_Cancel-2 AT ROW 10.35 COL 59.43 WIDGET-ID 2
...
ON END-ERROR OF W-Win /* DIFERENCIA RAPIDA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* DIFERENCIA RAPIDA */
DO:
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.CodCia = s-CodCia USE-INDEX Matg01,
    ...
    FIND LAST InvRecont WHERE InvRecont.CodCia =  InvConfig.CodCia
        ...
        AND   InvRecont.CodMat =  Almmmatg.CodMat
...
------------------------------------------------------------------------------*/
  DISPLAY x-FchInv FILL-IN-CodUbi 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE x-FchInv FILL-IN-CodUbi Btn_OK Btn_Cancel Btn_Cancel-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
s-SubTit = 'Almacen: ' + s-codalm + ' Fecha de Inventario: ' + STRING(x-FchInv).
...
cRange = "B" + '2'.
chWorkSheet:Range(cRange):Value = "DIFERENCIA RAPIDA DE INVENTARIO ALMACEN".
cRange = "B" + '3'.
...
FOR EACH T-Ajuste BY T-Ajuste.desmat:
    t-Column = t-Column + 1.
    cColumn = STRING(t-Column).
    cRange = "A" + cColumn.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-DETALLE
    ...
    T-Ajuste.dife       COLUMN-LABEL "Diferencia"       FORMAT '->>>>,>>9.99'
    T-Ajuste.CodUbi     COLUMN-LABEL "Zona"             FORMAT 'x(20)'
  WITH WIDTH 250 NO-BOX STREAM-IO DOWN. 

  DEFINE FRAME F-HEADER
    HEADER
    S-NOMCIA FORMAT "X(50)" AT 1 SKIP
    "DIFERENCIA RAPIDA DE INVENTARIO ALMACEN" AT 20 s-codalm
