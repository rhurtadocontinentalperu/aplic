...
Almcmov.FchDoc Almcmov.Observ CMOV.AlmDes CMOV.NroRf1 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table CMOV.NroRf1 
&Scoped-define ENABLED-TABLES-IN-QUERY-br_table CMOV
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br_table CMOV
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CMOV.AlmDes COLUMN-LABEL "Almacén Origen" FORMAT "x(6)":U
      CMOV.NroRf1 COLUMN-LABEL "No. Guia" FORMAT "x(12)":U
  ENABLE
      CMOV.NroRf1
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 83 BY 6.69
         FONT 4
         TITLE "INGRESOS POR TRANSFERENCIAS" FIT-LAST-COLUMN.
...
"Almcmov.FchDoc" "Fecha" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[5]   > Temp-Tables.CMOV.AlmDes
"CMOV.AlmDes" "Almacén Origen" "x(6)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.CMOV.NroRf1
"CMOV.NroRf1" "No. Guia" "x(12)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
END.
/* ************************************************************************************************* */
/* Transformamos los kits */
/* ************************************************************************************************* */
DEF VAR x-Cuenta AS INT NO-UNDO.
FOR EACH CMOV NO-LOCK:
    /* Barremos el detalle de kits */
    FOR EACH Almdmov OF CMOV NO-LOCK, 
        FIRST vtactabla NO-LOCK WHERE vtactabla.codcia = s-codcia
            AND vtactabla.tabla = 'PACKS-ECOMERCE'
            AND vtactabla.llave = Almdmov.codmat:
        /* ****************************************************************************************** */
        /* Cada kit lo disgregamos */
        /* ****************************************************************************************** */
        x-Cuenta = 0.
        ...
        IF (Almdmov.CanDes * Almdmov.Factor) MODULO x-Cuenta > 0 THEN NEXT.     /* Solo múltiplos exactos */
        /* ****************************************************************************************** */
        ...
        FIND LAST Almstkge WHERE Almstkge.codcia = s-codcia
            AND Almstkge.codmat = T-SALKIT.CodMat
            AND Almstkge.fecha <= TODAY
            NO-LOCK NO-ERROR.
        IF AVAILABLE Almstkge 
            THEN x-ImpCto = AlmStkGe.CtoUni * (T-SALKIT.CanDes * T-SALKIT.Factor).
        ELSE x-ImpCto = 0.
        x-CanDes = Almdmov.CanDes * Almdmov.Factor.
        ASSIGN
            T-SALKIT.ImpCTo = x-ImpCto.
        /* ****************************************************************************************** */
        /* Movimiento de INGRESO de los HIJOS */
        /* ****************************************************************************************** */
        FOR EACH vtadtabla WHERE vtadtabla.codcia = vtactabla.codcia AND 
                vtadtabla.tabla = vtactabla.tabla AND
                vtadtabla.llave = vtactabla.llave NO-LOCK,
            ...
            END.
            ASSIGN
                DMOV.CanDes = DMOV.CanDes + ((x-CanDes / x-Cuenta) * vtadtabla.libre_d01).
            /* Valorización: El VALOR TOTAL del PADRE de distribuye a los HIJOS */
            ASSIGN 
                DMOV.ImpCto = x-ImpCto * (DMOV.CanDes / x-CanDes).
            /*MESSAGE t-salkit.codmat dmov.codmat x-impcto dmov.candes x-candes.*/
        END.
    END.
END.
FOR EACH CMOV:
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
  FOR EACH T-SALKIT OF Almcmov EXCLUSIVE-LOCK:
      DELETE T-SALKIT.
  END.
  FOR EACH DMOV OF INTEGRAL.Almcmov EXCLUSIVE-LOCK:
      DELETE DMOV.
  END.
