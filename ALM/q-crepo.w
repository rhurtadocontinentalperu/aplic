...
/* RHC 14/05/2015 Ahora puede ser "A" (automatico) "M" (manual) */
&SCOPED-DEFINE CONDICION almcrepo.CodCia = s-codcia ~
AND almcrepo.CodAlm = s-codalm ~
AND almcrepo.NroSer = s-nroser 
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH almcrepo WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    BY almcrepo.NroSer DESCENDING ~
       BY almcrepo.NroDoc DESCENDING
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH almcrepo WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    BY almcrepo.NroSer DESCENDING ~
       BY almcrepo.NroDoc DESCENDING.
&Scoped-define TABLES-IN-QUERY-Query-Main almcrepo
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main almcrepo
...
/* Query rebuild information for QUERY Query-Main
     ...
     _OrdList          = "INTEGRAL.almcrepo.NroSer|no,INTEGRAL.almcrepo.NroDoc|no"
     _Where[1]         = "{&CONDICION}"
...
/*       NO-LOCK NO-ERROR.                          */
  ASSIGN  input-var-1 = s-codalm
          input-var-2 = ''  /*s-tipmov*/
          input-var-3 = STRING(s-nroser, '999')    /*STRING(Faccorre.nroser)*/
          output-var-1 = ?
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN lkup/c-crepo ("Pedidos por Reposicion Automatica").
