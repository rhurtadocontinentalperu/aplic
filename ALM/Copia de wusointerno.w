...
&Scoped-Define ENABLED-OBJECTS COMBO-NroSer RECT-1 
...
DEFINE VARIABLE h_qusointerno AS HANDLE NO-UNDO.
DEFINE VARIABLE h_tusointerno AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vusointerno AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     COMBO-NroSer AT ROW 1.08 COL 69.71 WIDGET-ID 4
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW sW-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "SOLICITUD DE ARTICULOS PARA USO INTERNO"
         HEIGHT             = 14.65
         WIDTH              = 89.72
         MAX-HEIGHT         = 26
         MAX-WIDTH          = 143.43
...
ON END-ERROR OF sW-Win /* SOLICITUD DE ARTICULOS PARA USO INTERNO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL sW-Win sW-Win
ON WINDOW-CLOSE OF sW-Win /* SOLICITUD DE ARTICULOS PARA USO INTERNO */
DO:
...
DO:
    ...
    RUN dispatch IN h_qusointerno ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/vusointerno.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vusointerno ).
       RUN set-position IN h_vusointerno ( 2.73 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.08 , 77.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/qusointerno.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_qusointerno ).
       RUN set-position IN h_qusointerno ( 1.00 , 81.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 9.00 ) */

       /* Links to SmartViewer h_vusointerno. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_vusointerno ).
       RUN add-link IN adm-broker-hdl ( h_qusointerno , 'Record':U , h_vusointerno ).

       /* Links to SmartQuery h_qusointerno. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_qusointerno ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_vusointerno ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_qusointerno ,
             h_vusointerno , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-size IN h_busointerno ( 8.08 , 84.00 ) NO-ERROR.

       /* Links to SmartBrowser h_busointerno. */
       RUN add-link IN adm-broker-hdl ( h_qusointerno , 'Record':U , h_busointerno ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-size IN h_tusointerno ( 6.69 , 86.00 ) NO-ERROR.
