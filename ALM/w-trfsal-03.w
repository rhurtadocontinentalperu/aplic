...
/* RHC 31/07/2015 Control de Pre-Picking y Picking x División */
...
FIND FIRST Almtdocm WHERE Almtdocm.CodAlm = s-codalm
...
IF NOT AVAILABLE Almtdocm THEN DO:
    MESSAGE 'NO está configurado el movimiento en este almacén'
...
DEFINE FRAME F-Main
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 103.14 BY 20.69 WIDGET-ID 100.
...
DO:
  RUN Captura-Pedido IN h_v-trfsal-03.
...
DO:
    RUN Captura-OD IN h_v-trfsal-03.
    IF RETURN-VALUE <> "ADM-ERROR" THEN DO:
        ASSIGN
            BUTTON-1:VISIBLE IN FRAME {&FRAME-NAME} = NO
            BUTTON-1:SENSITIVE IN FRAME {&FRAME-NAME} = NO
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/v-trfsal-03.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-trfsal-03 ).
       RUN set-position IN h_v-trfsal-03 ( 4.50 , 3.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_v-trfsal-03 ).
       RUN add-link IN adm-broker-hdl ( h_q-trfsal-01 , 'Record':U , h_v-trfsal-03 ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-size IN h_t-trfsal-02 ( 9.96 , 88.00 ) NO-ERROR.
...
CASE L-Handle:
    ...
    END.
    WHEN 'disable-headers' THEN RUN dispatch IN h_p-updv13 ('disable':U).
    WHEN 'enable-headers'  THEN RUN dispatch IN h_p-updv13 ('enable':U).
END CASE.
