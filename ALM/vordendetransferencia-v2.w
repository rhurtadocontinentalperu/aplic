...
DEFINE FRAME F-Main
     ...
     FacCPedi.CodAlm AT ROW 1.81 COL 16 COLON-ALIGNED WIDGET-ID 110
          LABEL "Almacén Despacho" FORMAT "x(8)"
          VIEW-AS FILL-IN 
          SIZE 6 BY .81
          BGCOLOR 8 FGCOLOR 0 
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      IF x-Tipo = -1 THEN DO:
          IF NOT CAN-FIND(FIRST B-DPEDI OF B-CPEDI WHERE B-DPEDI.CanAte <> 0 NO-LOCK)
              THEN B-CPedi.FlgEst = "P".     /* OJO >>> APROBADO */
      END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Extorna_R_A V-table-Win 
PROCEDURE Extorna_R_A :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* RHC 28/11/17 Esta rutina ya NO cambia el estado de la R/A */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Extorna_R_A_CrossDocking V-table-Win 
PROCEDURE Extorna_R_A_CrossDocking :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    ASSIGN            
        Almcrepo.FlgEst = "P"       /* PENDIENTE */
        Almcrepo.FlgSit = "G"       /* Por Aprobar RHC 20/02/17 Max Ramos */
        almcrepo.FchApr = TODAY
        Almcrepo.HorApr = STRING(TIME, 'HH:MM')
        Almcrepo.UsrApr = s-user-id
        Almcrepo.Libre_c02 = pMotivo + '|' + pGlosa + '|' + STRING(NOW) + '|' + s-user-id.
...
/* RHC 24/04/2019 */
/* RHC 14/08/19 Solicitado por MR */
/* IF AVAILABLE Faccpedi AND Faccpedi.TpoPed = "INC"            */
/*     THEN DO:                                                 */
...
/* END.                                                         */
    ...
    /* NOTA: 19/041/2018
        Existen 3 tipos de OTR:                 Faccpedi.CodRef Faccpedi.CrossDocking Faccpedi.TpoPed
        - Generada a partir de una R/A                  R/A                 No              -
        - Generada a partir de una R/A con XD           R/A                 Yes             -
        - Generada a partir del I-03 de la OTR          R/A                 No              XD
    */        
    ...
    FOR EACH DI-RutaD NO-LOCK WHERE DI-RutaD.CodCia = s-codcia
        ...
        MESSAGE 'No se puede eliminar si ya está en una Pre-Hoja de Ruta' SKIP
            DI-RutaC.CodDoc DI-RutaC.NroDoc VIEW-AS ALERT-BOX ERROR.
        RETURN 'ADM-ERROR'.
    END.
    /* Si es la segunda OTR del Cross Docking */
    DEF VAR x-LocalXD AS LOG INIT NO NO-UNDO.
    DEF VAR x-CrossDocking AS LOG INIT NO NO-UNDO.
    IF Faccpedi.CodRef = "R/A" AND Faccpedi.CrossDocking = NO AND Faccpedi.TpoPed = "XD" THEN DO:
    ...
    END.
    /* *********************************************************************** */
    /* 7-8-2023: Ciclo DESPACHO POR SECTORES 
                 NO debe haber pasado de ASIGNACION DE TAREA */
    /* *********************************************************************** */
    FOR EACH Vtacdocu NO-LOCK WHERE VtaCDocu.CodCia = s-codcia AND 
        VtaCDocu.CodPed = Faccpedi.CodDoc AND 
        VtaCDocu.NroPed BEGINS Faccpedi.NroPed:
        IF NOT (VtaCDocu.FlgEst = "P" AND LOOKUP(VtaCDocu.FlgSit, "T,P") > 0 )
            THEN DO:
            MESSAGE 'La OTR ya inició el ciclo de DESPACHO POR SECTORES' SKIP
                'No se puede eliminar' VIEW-AS ALERT-BOX ERROR.
            RETURN 'ADM-ERROR'.
        END.
    END.
    /* *********************************************************************** */
    ...
    DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
        ...
        /* RHC 16/03/17 Motivo de anulación */
        ASSIGN
            Faccpedi.Libre_c05 = pMotivo + '|' + pGlosa + '|' + STRING(NOW) + '|' + s-user-id.
        /* Marcamos como ANULADO antes de correr los procesos */
        ASSIGN
        ...
        FOR EACH Vtacdocu NO-LOCK WHERE VtaCDocu.CodCia = s-codcia AND 
            ...
            IF NOT (VtaCDocu.FlgEst = "P" AND LOOKUP(VtaCDocu.FlgSit, "T,P") > 0 )
                THEN DO:
                pMensaje = 'La OTR ya inició el ciclo de DESPACHO POR SECTORES' + CHR(10) +
                    'No se puede eliminar'.
                UNDO RLOOP, LEAVE RLOOP.
            END.
            /*MESSAGE vtacdocu.codped vtacdocu.nroped vtacdocu.coddiv vtacdocu.usrsac.*/
            FIND PikSacadores WHERE PikSacadores.CodCia = s-codcia AND
                PikSacadores.CodDiv = VtaCDocu.CodDiv AND
                PikSacadores.FlgEst = "A" AND
                PikSacadores.FlgTarea = "O" AND
                PikSacadores.CodPer = VtaCDocu.UsrSac 
                NO-LOCK NO-ERROR.
            IF AVAILABLE PikSacadores THEN DO:
                FIND b-PikSacadores WHERE ROWID(b-PikSacadores) = ROWID(PikSacadores) EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
                IF NOT AVAILABLE b-PikSacadores THEN DO:
                    pMensaje = "NO se pudo actualizar el control de sacadores" .
                    UNDO RLOOP, LEAVE RLOOP.
                END.
                ASSIGN
                    b-PikSacadores.FlgTarea = "L".
                RELEASE b-PikSacadores.
            END.
        END.
        /* *********************************************************************** */
        CASE TRUE:
            WHEN x-CrossDocking = YES THEN DO:
                ...
                */
                RUN gn/p-anula-otr-xd.p (INPUT ROWID(Faccpedi),
                                         INPUT x-LocalXD,
                                         OUTPUT pMensaje).
                IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                ...
                END.
            END.
            WHEN Faccpedi.codref = "PED" THEN DO:   /* Por Cross Docking por Ventas */
...
IF Faccpedi.CodRef = "R/A" AND Faccpedi.CrossDocking = NO AND Faccpedi.TpoPed = "XD" THEN DO:
    MESSAGE "Acceso Denegado" SKIP
        "Esta Orden ha sido generada automáticamente en el ciclo de Cross Docking"
        VIEW-AS ALERT-BOX ERROR.
    RETURN "ADM-ERROR".
END.
FIND FIRST FacDPedi OF FacCPedi WHERE FacDPedi.CanAte > 0 NO-LOCK NO-ERROR.
