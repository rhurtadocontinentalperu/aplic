...
DEFINE        VAR S-TPOMOV AS CHAR INIT 'I' NO-UNDO.
DEFINE        VAR S-CODMOV AS INTEGER NO-UNDO.

FIND FIRST LG-Cfgcpr WHERE LG-Cfgcpr.Codcia = S-CODCIA 
     NO-LOCK NO-ERROR.

IF NOT AVAILABLE LG-Cfgcpr THEN DO:
   MESSAGE "No existe Configuracion Consignacion Proveedores" VIEW-AS ALERT-BOX ERROR.
   RETURN ERROR.
END.
      
IF LG-Cfgcpr.TipMov[1] <> S-TPOMOV THEN DO:
   MESSAGE "Ingreso de Consignaciones no configurado" VIEW-AS ALERT-BOX ERROR.
   RETURN ERROR.

END.

FIND FIRST Almtmovm WHERE Almtmovm.CodCia = S-CODCIA AND
     Almtmovm.Tipmov = LG-Cfgcpr.TipMov[1] AND
     Almtmovm.CodMov = LG-Cfgcpr.CodMov[1] NO-LOCK NO-ERROR.

IF AVAILABLE Almtmovm THEN S-CODMOV = Almtmovm.Codmov.
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Almtdocm WHERE ~{&KEY-PHRASE} ~
 ...
 AND Almtdocm.CodMov = S-CODMOV NO-LOCK ~
...
/* Query rebuild information for QUERY Query-Main
 ...
 AND Almtdocm.CodMov = S-CODMOV"
     _Design-Parent    is WINDOW q-tables @ ( 1 , 1.57 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
