...
/* ***************************  Main Block  *************************** */
  ...
  CASE TRUE:
      ...
      OTHERWISE DO:
          ...
          IF LENGTH(pCodMat) < 6 THEN DO:
            pCodMat1 = fill("0", 6 - LENGTH(pCodMat) ) +  pCodMat.
          END.
          FIND FIRST Almmmatg WHERE Almmmatg.codcia = pcodcia
              AND Almmmatg.codmat = pCodMat1
              NO-LOCK NO-ERROR.
          IF AVAILABLE Almmmatg THEN DO:
          ...
          END.      
          ASSIGN
              pCodMat = STRING(INTEGER(pCodMat), '999999') NO-ERROR.
          FIND Almmmatg WHERE Almmmatg.codcia = pcodcia
...
/*       IF NOT CAN-FIND(FIRST Almmmatg OF Almmmat1 WHERE Almmmatg.TpoArt = 'A' NO-LOCK)   */
/*           THEN NEXT.                                                                    */
/*       DO x-Item = 1 TO 6:                                                               */
