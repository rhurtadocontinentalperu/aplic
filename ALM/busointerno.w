...
&Scoped-define FIELDS-IN-QUERY-br_table AlmDDocu.Codigo Almmmatg.DesMat ~
Almmmatg.DesMar AlmDDocu.Libre_d01 AlmDDocu.Libre_d02 Almmmatg.UndBas 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH AlmDDocu OF AlmCDocu WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Almmmatg WHERE Almmmatg.CodCia = AlmDDocu.CodCia ~
  AND Almmmatg.codmat = AlmDDocu.Codigo NO-LOCK ~
    BY AlmDDocu.NroItm
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH AlmDDocu OF AlmCDocu WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Almmmatg WHERE Almmmatg.CodCia = AlmDDocu.CodCia ~
  AND Almmmatg.codmat = AlmDDocu.Codigo NO-LOCK ~
    BY AlmDDocu.NroItm.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      AlmDDocu.Codigo FORMAT "x(6)":U WIDTH 6.43
      Almmmatg.DesMat FORMAT "X(60)":U WIDTH 59.57
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(20)":U
      AlmDDocu.Libre_d01 COLUMN-LABEL "Solicitado" FORMAT ">>>,>>9.99":U
      AlmDDocu.Libre_d02 COLUMN-LABEL "Aprobado" FORMAT ">>>,>>9.99":U
      Almmmatg.UndBas COLUMN-LABEL "Unidad" FORMAT "X(6)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 107 BY 8.08
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.AlmDDocu OF INTEGRAL.AlmCDocu,INTEGRAL.Almmmatg WHERE INTEGRAL.AlmDDocu ..."
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ","
     _OrdList          = "INTEGRAL.AlmDDocu.NroItm|yes"
     _JoinCode[2]      = "Almmmatg.CodCia = AlmDDocu.CodCia
  AND Almmmatg.codmat = AlmDDocu.Codigo"
     _FldNameList[1]   > INTEGRAL.AlmDDocu.Codigo
"AlmDDocu.Codigo" ? "x(6)" "character" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"AlmDDocu.Libre_d01" "Solicitado" ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.AlmDDocu.Libre_d02
"AlmDDocu.Libre_d02" "Aprobado" ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
