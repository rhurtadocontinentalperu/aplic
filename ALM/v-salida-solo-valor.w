...
DEFINE SHARED VARIABLE S-TPOMOV AS CHAR INIT 'S'.   /* Salidas */
...
Almcmov.CodPro Almcmov.NroRf1 Almcmov.Libre_c01 Almcmov.NroRf2 ~
Almcmov.AlmDes Almcmov.NroRf3 Almcmov.Observ Almcmov.cco 
&Scoped-define ENABLED-TABLES Almcmov
&Scoped-define FIRST-ENABLED-TABLE Almcmov
&Scoped-Define ENABLED-OBJECTS FILL-IN-serie FILL-IN-nro RECT-25 
...
FILL-IN-NomCli FILL-IN-NomPro FILL-IN-serie FILL-IN-nro FILL-IN-NomSede ~
F-AlmRef 
...
DEFINE VARIABLE FILL-IN-nro AS INTEGER FORMAT ">>>>>>>>9":U INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .81 NO-UNDO.

DEFINE VARIABLE FILL-IN-serie AS INTEGER FORMAT ">>9":U INITIAL 0 
...
DEFINE FRAME F-Main
     FILL-IN_NroDoc AT ROW 1.27 COL 13 COLON-ALIGNED
     F-Estado AT ROW 1.27 COL 30 COLON-ALIGNED NO-LABEL
     F-Situacion AT ROW 1.27 COL 48 COLON-ALIGNED NO-LABEL
     Almcmov.FchDoc AT ROW 1.27 COL 73 COLON-ALIGNED FORMAT "99/99/9999"
          VIEW-AS FILL-IN 
          SIZE 10.14 BY .69
     Almcmov.CodCli AT ROW 2.08 COL 13 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 11 BY .69
     FILL-IN-NomCli AT ROW 2.08 COL 24.14 COLON-ALIGNED NO-LABEL
     Almcmov.CodMon AT ROW 2.15 COL 75.57 NO-LABEL
          VIEW-AS RADIO-SET HORIZONTAL
          RADIO-BUTTONS 
                    "S/.", 1,
"US$", 2
          SIZE 12 BY .5
     Almcmov.CodPro AT ROW 2.88 COL 13 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 11 BY .69
     FILL-IN-NomPro AT ROW 2.88 COL 24.14 COLON-ALIGNED NO-LABEL
     Almcmov.NroRf1 AT ROW 2.92 COL 73 COLON-ALIGNED
          LABEL "Referencia 1" FORMAT "x(15)"
          VIEW-AS FILL-IN 
          SIZE 13 BY .69
     FILL-IN-serie AT ROW 4.73 COL 86 COLON-ALIGNED WIDGET-ID 8
     FILL-IN-nro AT ROW 4.73 COL 90 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     Almcmov.Libre_c01 AT ROW 3.69 COL 13 COLON-ALIGNED WIDGET-ID 2
          LABEL "Sede" FORMAT "x(8)"
          VIEW-AS FILL-IN 
          SIZE 11 BY .69
     FILL-IN-NomSede AT ROW 3.69 COL 24 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     Almcmov.NroRf2 AT ROW 3.69 COL 73 COLON-ALIGNED
          LABEL "Referencia 2" FORMAT "x(12)"
          VIEW-AS FILL-IN 
          SIZE 13 BY .69
     Almcmov.AlmDes AT ROW 4.5 COL 13 COLON-ALIGNED
          LABEL "Almacen Destino"
          VIEW-AS FILL-IN 
          SIZE 5.57 BY .69
     F-AlmRef AT ROW 4.5 COL 20.43 COLON-ALIGNED NO-LABEL
     Almcmov.NroRf3 AT ROW 4.5 COL 73 COLON-ALIGNED
          LABEL "O/T" FORMAT "xxx-xxxxxx"
          VIEW-AS FILL-IN 
          SIZE 10 BY .69
     Almcmov.Observ AT ROW 5.31 COL 13 COLON-ALIGNED FORMAT "X(60)"
          VIEW-AS FILL-IN 
          SIZE 49.72 BY .69
     Almcmov.usuario AT ROW 5.31 COL 73 COLON-ALIGNED
          LABEL "Usuario"
          VIEW-AS FILL-IN 
          SIZE 10 BY .69
     Almcmov.cco AT ROW 6.12 COL 13 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 5 BY .69
     Almcmov.CodTra AT ROW 6.12 COL 73 COLON-ALIGNED
          LABEL "Transportista" FORMAT "X(11)"
          VIEW-AS FILL-IN 
          SIZE 11 BY .69
     "Moneda :" VIEW-AS TEXT
          SIZE 6.57 BY .69 AT ROW 2.15 COL 68.43
     RECT-25 AT ROW 1 COL 1 WIDGET-ID 12
...
&Scoped-define SELF-NAME FILL-IN-nro
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-nro V-table-Win
ON LEAVE OF FILL-IN-nro IN FRAME F-Main
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-serie V-table-Win
ON LEAVE OF FILL-IN-serie IN FRAME F-Main /* Parte de ingreso */
DO:
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ITEM WHERE ITEM.codmat <> "" ON ERROR UNDO, RETURN "ADM-ERROR":
      ...
      RUN gn/stock-comprometido-v2 (ITEM.codmat, s-codalm, TRUE, OUTPUT pComprometido).
...
/*   END.                                                                                                 */
  ...
  IF Almtmovm.ReqGuia THEN DO:
      ...
      END.
      ELSE DO:
          MESSAGE 'Error en la serie' s-NroSer VIEW-AS ALERT-BOX ERROR.
          RETURN 'ADM-ERROR'.
      END.
  END.
  ELSE DO:
  ...
  L-CREA = YES.
  DO WITH FRAME {&FRAME-NAME}:
     DISPLAY TODAY @ Almcmov.FchDoc FILL-IN_NroDoc.
     FIND LAST gn-tcmb NO-LOCK NO-ERROR.
     IF AVAILABLE gn-tcmb THEN F-TPOCMB = gn-tcmb.compra.
     Almcmov.Nrorf3:SCREEN-VALUE = "".
     FILL-in-serie:SCREEN-VALUE = "0".
     FILL-in-nro:SCREEN-VALUE = "0".
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ASSIGN fill-in-serie fill-in-nro.
  END.
  ...
  IF L-CREA THEN DO:
      ...
      IF Almtdocm.CodMov = 30 AND Almtdocm.TipMov = "S" THEN DO:
            ASSIGN Almcmov.nrorf1 = STRING(fill-in-serie,"999") + "-" + 
                                    STRING(fill-in-nro,"999999999").
      END.
...
/*         MESSAGE 'Imposible eliminar, Guia de remision electronica esta activo' VIEW-AS ALERT-BOX ERROR. */
/*         RETURN 'ADM-ERROR'.                                                                             */
/*     END.                                                                                                */
/*   END.                                                                                                  */
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN almcmov.NroRf1:VISIBLE IN FRAME {&FRAME-NAME} = YES.
  ASSIGN almcmov.NroRf1:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
  ASSIGN fill-in-serie:VISIBLE IN FRAME {&FRAME-NAME} = NO.
  ASSIGN fill-in-nro:VISIBLE IN FRAME {&FRAME-NAME} = NO.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME} :
      ASSIGN almcmov.NroRf1:VISIBLE = YES.
      /*IF S-TPOMOV = 'S' AND INTEGER(C-CODMOV) = 30 THEN DO:*/
      IF Almtdocm.CodMov = 30 AND Almtdocm.TipMov = "S" THEN DO:
            ASSIGN almcmov.NroRf1:VISIBLE = NO.
            ASSIGN fill-in-serie:VISIBLE = YES.
            ASSIGN fill-in-nro:VISIBLE = YES.

            DEFINE VAR hNroRef1 AS HANDLE.
            DEFINE VAR hSerie AS HANDLE.
            DEFINE VAR hNumero AS HANDLE.
            DEFINE VAR hLabelSerie AS HANDLE.

            hNroRef1 = almcmov.NroRf1:HANDLE.
            hSerie = fill-in-serie:HANDLE.
            hNumero = fill-in-nro:HANDLE.

            hSerie:X = hNroRef1:X.
            hSerie:Y = hNroRef1:Y.

            hNumero:Y = hSerie:Y.
            hNumero:X = hNroRef1:X + hSerie:WIDTH + 25.

            ASSIGN hLabelSerie = hSerie:SIDE-LABEL-HANDLE.
            IF VALID-HANDLE(hLabelSerie) THEN DO:               
                hLabelSerie:Y = hNroRef1:Y.
                hLabelSerie:X = hNroRef1:X - hLabelSerie:WIDTH-PIXELS - 2.
                hLabelSerie:VISIBLE =  YES.
            END.
      END.
    RUN Procesa-Handle IN lh_handle ('pagina2').
  END.
...
------------------------------------------------------------------------------*/
  ...
  fill-in-serie:VISIBLE IN FRAME  {&FRAME-NAME} = NO.
  fill-in-nro:VISIBLE IN FRAME  {&FRAME-NAME} = NO.
  /*
  IF S-TPOMOV = 'S' AND INTEGER(C-CODMOV) = 30 THEN DO:
        button-pingreso:VISIBLE IN FRAME {&FRAME-NAME} = YES.
  END.
  */
END PROCEDURE.
...
DO WITH FRAME {&FRAME-NAME} :
    ...
    IF INPUT Almcmov.fchdoc < dFchCie THEN DO:
        MESSAGE 'NO se puede ingresar ningún documento antes del' (dFchCie)
            VIEW-AS ALERT-BOX WARNING.
        RETURN 'ADM-ERROR'.
    END.
    IF INPUT Almcmov.fchdoc > TODAY THEN DO: 
        MESSAGE 'NO se puede ingresar ningún documento después del' TODAY
            VIEW-AS ALERT-BOX WARNING.
        RETURN 'ADM-ERROR'.
    END.
    /* *********************************************************** */
   ...
   IF N-ITEM > pItemsGuias THEN DO:
       MESSAGE "Numero de Items Mayor al Configurado para el Tipo de Documento " 
           SKIP "Items Por Guia : " +  STRING(pItemsGuias,"999") 
...
/*    END.                                                                              */
   ...
   IF Almtdocm.CodMov = 30 AND Almtdocm.TipMov = "S" THEN DO:
       ...
       ASSIGN fill-in-serie FILL-IN-nro.
       x-nro-nd = almcmov.NroRf2:SCREEN-VALUE.
       ...
       FIND FIRST x-almcmov WHERE x-almcmov.codcia = s-codcia AND
                               ...
                               x-almcmov.codmov = 09 AND
                               x-almcmov.nroser = fill-in-serie AND
                               x-almcmov.nrodoc = fill-in-nro AND
...
------------------------------------------------------------------------------*/
  ...
  IF x-eliminando = NO THEN DO:
      IF S-TPOMOV = 'S' AND almtdocm.codmov = 30 THEN DO:
          MESSAGE 'Tipo/Codigo de Movimiento no es modificable (' + S-TPOMOV + "/" + STRING(almtdocm.codmov) + ")" VIEW-AS ALERT-BOX ERROR.
          RETURN 'ADM-ERROR'.
      END.
  END.
