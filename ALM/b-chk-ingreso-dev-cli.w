...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Solo el saldo en caso ya tenga una deocluión parcial anterior
------------------------------------------------------------------------------*/
...
FOR EACH Ccbddocu OF Ccbcdocu NO-LOCK WHERE Ccbddocu.CanDes > Ccbddocu.CanDev, 
    FIRST Almmmatg OF Ccbddocu NO-LOCK:
    FIND FIRST T-DPEDI WHERE T-DPEDI.codmat = Ccbddocu.codmat NO-ERROR.
    IF NOT AVAILABLE T-DPEDI THEN DO:
        CREATE T-DPEDI.
        ASSIGN
            T-DPEDI.CodCia = Ccbddocu.codcia
            T-DPEDI.CodMat = Ccbddocu.codmat
            T-DPEDI.UndVta = Almmmatg.UndStk
            T-DPEDI.CanPed = (Ccbddocu.CanDes - Ccbddocu.CanDev) * Ccbddocu.factor
            T-DPEDI.Factor = 1.     
    END.
    ELSE T-DPEDI.canped = T-DPEDI.canped + ((Ccbddocu.CanDes - Ccbddocu.CanDev) * Ccbddocu.factor).
...
------------------------------------------------------------------------------*/
  ...
  CASE TRUE:
      WHEN TabGener.Libre_L03 = NO THEN DO:
          /* Ingreso directo */
          RUN alm/p-genera-ing-dev-cli.p (INPUT x-CodDoc,
                                            INPUT x-NroDoc,
                                            INPUT x-UsrChq,
                                            INPUT pMotivo,
                                            INPUT pObserv,
      ...
      WHEN TabGener.Libre_L03 = YES THEN DO:
          /* Ingreso Sloting */
          RUN alm/p-genera-ing-dev-cli-v2.p (INPUT x-CodDoc,
                                             INPUT x-NroDoc,
                                             INPUT x-UsrChq,
                                             INPUT pMotivo,
                                             INPUT pObserv,
