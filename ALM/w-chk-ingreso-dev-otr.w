...
IF TabGener.Libre_l03 = YES THEN DO:
    MESSAGE 'Está configurado como SLOTING' SKIP
        'Proceso Abortado' VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
CASE TRUE:
...
DEFINE VARIABLE h_b-chk-ing-dev-otr AS HANDLE NO-UNDO.
...
DO:
  ...
  RUN Cerrar-Orden IN h_b-chk-ing-dev-otr (INPUT ENTRY(1,COMBO-BOX-1:SCREEN-VALUE,' - '),
...
OR RETURN OF {&SELF-NAME} DO:
    IF SELF:SCREEN-VALUE = '' THEN DO:
        RUN Captura-Parametros IN h_b-chk-ing-dev-otr (INPUT pTipo,
                                                       INPUT "" /* CHARACTER */,
                                                       INPUT "" /* CHARACTER */).
        RETURN.
    END.
    ...
    IF x-NroDoc <> SELF:SCREEN-VALUE THEN DO:
        ...
        RUN Captura-Parametros IN h_b-chk-ing-dev-otr
            ( INPUT pTipo,
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/b-chk-ing-dev-otr.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-chk-ing-dev-otr ).
       RUN set-position IN h_b-chk-ing-dev-otr ( 5.04 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-chk-ing-dev-otr ( 18.31 , 143.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 23.35 , 1.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.42 , 34.14 ) NO-ERROR.

       /* Links to SmartBrowser h_b-chk-ing-dev-otr. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-chk-ing-dev-otr ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-chk-ing-dev-otr ,
             FILL-IN_Observ:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_b-chk-ing-dev-otr , 'AFTER':U ).
    END. /* Page 0 */
...
OCXFile = SEARCH( "w-chk-ingreso-dev-otr.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-chk-ingreso-dev-otr.wrx":U SKIP(1)
