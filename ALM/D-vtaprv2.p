...
FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = S-CODCIA
                          ...
                          USE-INDEX matg09,
   EACH Almdmov NO-LOCK WHERE Almdmov.CodCia = Almmmatg.codcia 
                         AND  LOOKUP(Almdmov.CodAlm, "89") > 0
                         AND  Almdmov.codmat = Almmmatg.codmat
                         AND  (Almdmov.FchDoc >= DesdeF
                         AND   Almdmov.FchDoc <= HastaF)
                         AND  Almdmov.TipMov = "S" 
...
*/
      ...
      END.
      ASSIGN t-tot_uca = t-tot_uca + IF Almdmov.CodAlm = "89" THEN F-Salida ELSE 0
 ...
 *          t-tot_par            FORMAT "->>>>,>>9.99"*/
  ...
  DEFINE FRAME F-HEADER
         ...
         SKIP(1) 
        "-----------------------------------------------------------------------------------------------------------------------------------------" SKIP
        "                                                           V E N T A                    STOSCK    NUMERO  NRO.DE" SKIP
        "CODIGO DESCRIPCION                         MARCA U.B       LAUCHUN         TOTAL         TOTAL  PERIODOS    DIAS" SKIP
        "-----------------------------------------------------------------------------------------------------------------------------------------" SKIP
