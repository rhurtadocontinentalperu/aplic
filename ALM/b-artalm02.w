...
&SCOPED-DEFINE CONDICION ( Almmmatg.CodCia = S-CODCIA  AND LOOKUP(TRIM(S-CODALM),Almmmatg.almacenes) > 0 AND Almmmatg.TpoArt BEGINS R-Tipo and Almmmatg.TipArt BEGINS RADIO-SET_TipArt )
                           
&SCOPED-DEFINE CODIGO Almmmatg.codmat

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( (Almmmatg.DesMat BEGINS FILL-IN-filtro) AND (Almmmatg.Codfam BEGINS F-Codfam) AND (Almmmatg.CodPr1 BEGINS F-PROVEE) )
&SCOPED-DEFINE FILTRO2 ( (INDEX ( Almmmatg.DesMat , FILL-IN-filtro ) <> 0) AND (Almmmatg.Codfam BEGINS F-Codfam) AND (Almmmatg.CodPr1 BEGINS F-PROVEE) )
...
FILL-IN-filtro F-CodFam F-PROVEE R-Tipo RADIO-SET_TipArt BUTTON-2 RECT-1 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-codigo CMB-filtro FILL-IN-filtro ~
F-CodFam F-PROVEE R-Tipo RADIO-SET_TipArt EDITOR_Detalle FILL-IN_FchIng ~
FILL-IN_UndBas F-XUndM2 FILL-IN_Pesmat 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      fStkAct() @ f-StkAct COLUMN-LABEL "Stock Actual" FORMAT "->>>,>>>,>>9.99<<<":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 88 BY 11.69
...
DEFINE FRAME F-Main
     br_table AT ROW 4.27 COL 90 RIGHT-ALIGNED
     ...
     R-Tipo AT ROW 3.5 COL 9 NO-LABEL
     RADIO-SET_TipArt AT ROW 3.5 COL 64 NO-LABEL
     EDITOR_Detalle AT ROW 16.08 COL 55 NO-LABEL
     FILL-IN_FchIng AT ROW 16.19 COL 11 COLON-ALIGNED
     BUTTON-2 AT ROW 16.27 COL 49
     ...
     "Estado" VIEW-AS TEXT
          ...
          FONT 4
     "Tipo de Rotación" VIEW-AS TEXT
          SIZE 12 BY .5 AT ROW 3.5 COL 51
     "F8 : Consulta de Stocks Almacenes        F7 : Consulta de Pedidos" VIEW-AS TEXT
...
"fStkAct() @ f-StkAct" "Stock Actual" "->>>,>>>,>>9.99<<<" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    IF CMB-filtro = CMB-filtro:SCREEN-VALUE AND
        ...
        F-Provee = F-Provee:SCREEN-VALUE AND
        R-Tipo = R-Tipo:SCREEN-VALUE AND
        RADIO-SET_TipArt = RADIO-SET_TipArt:SCREEN-VALUE THEN RETURN.
...
&Scoped-define SELF-NAME RADIO-SET_TipArt
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL RADIO-SET_TipArt L-table-Win
ON VALUE-CHANGED OF RADIO-SET_TipArt IN FRAME F-Main
DO:
  APPLY "VALUE-CHANGED" TO CMB-filtro.
END.
...
DEFINE VARIABLE dCtoUni                 AS DECIMAL     NO-UNDO.
...
REPEAT WHILE AVAILABLE Almmmatg:
    ...
    FIND LAST AlmStkGe WHERE AlmStkGe.codcia = s-codcia
        ...
        AND AlmStkGe.fecha <= 12/05/2010 NO-LOCK NO-ERROR.
    IF AVAILABLE AlmStkGe THEN dCtoUni = AlmStkge.CtoUni. 
    ...
    cRange = "G" + cColumn.
    chWorkSheet:Range(cRange):Value = dCtoUni.
