...
&Scoped-define INTERNAL-TABLES Almmmatg almubimat
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almmmatg WHERE ~{&KEY-PHRASE} ~
      ...
      WHERE {&Condicion} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table Almmmatg almubimat
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Almmmatg
&Scoped-define SECOND-TABLE-IN-QUERY-br_table almubimat
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 103 BY 19.12
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Almmmatg,INTEGRAL.almubimat OF INTEGRAL.Almmmatg"
...
"almubimat.CodUbi" "Ubicación" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    FIND FIRST Almmmatg WHERE Almmmatg.codcia = s-codcia
        ...
        NO-LOCK.
    DISPLAY Almmmatg.DesMar Almmmatg.DesMat WITH BROWSE {&browse-name}.
END.
...
x-CodAlm = pCodAlm.
x-CodFam = pCodFam.
IF pCodFam = 'Todas' THEN x-CodFam = ''.
x-CodMat = pCodMat.
IF x-CodMat > '' THEN x-CodFam = ''.
