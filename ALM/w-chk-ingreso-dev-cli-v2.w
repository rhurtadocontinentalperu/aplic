...
DEFINE VARIABLE h_b-chk-ingreso-dev-cli-v2 AS HANDLE NO-UNDO.
...
DO:
  ...
  MESSAGE 'Cerramos la Factura?' VIEW-AS ALERT-BOX QUESTION
      BUTTONS YES-NO UPDATE rpta AS LOG.
  IF rpta = NO THEN RETURN NO-APPLY.
  RUN Cerrar-Orden IN h_b-chk-ingreso-dev-cli-v2 (INPUT ENTRY(1,COMBO-BOX-1:SCREEN-VALUE,' - '),
...
OR RETURN OF {&SELF-NAME} DO:
    IF SELF:SCREEN-VALUE = '' THEN DO:
        RUN Captura-Parametros IN h_b-chk-ingreso-dev-cli-v2 (INPUT pTipo,
                                                       INPUT "" /* CHARACTER */,
                                                       INPUT "" /* CHARACTER */).
        RETURN.
    END.
    ...
    IF x-NroDoc <> SELF:SCREEN-VALUE THEN DO:
        ...
        RUN Captura-Parametros IN h_b-chk-ingreso-dev-cli-v2
            ( INPUT pTipo,
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ALM/b-chk-ingreso-dev-cli-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-chk-ingreso-dev-cli-v2 ).
       RUN set-position IN h_b-chk-ingreso-dev-cli-v2 ( 5.31 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-chk-ingreso-dev-cli-v2 ( 18.31 , 143.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-chk-ingreso-dev-cli-v2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-chk-ingreso-dev-cli-v2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-chk-ingreso-dev-cli-v2 ,
             FILL-IN_Observ:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_b-chk-ingreso-dev-cli-v2 , 'AFTER':U ).
    END. /* Page 0 */
...
OCXFile = SEARCH( "w-chk-ingreso-dev-cli-v2.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-chk-ingreso-dev-cli-v2.wrx":U SKIP(1)
