...
&Scoped-define INTERNAL-TABLES PEDI Almmmatg FacDPedi
...
Almmmatg.DesMat Almmmatg.DesMar PEDI.UndVta PEDI.CanPed PEDI.Libre_d01 
...
&Scoped-define QUERY-STRING-br_table FOR EACH PEDI WHERE TRUE /* Join to FacCPedi incomplete */ NO-LOCK, ~
      ...
      FIRST FacDPedi WHERE FacDPedi.CodCia = PEDI.CodCia ~
  AND FacDPedi.CodDiv = PEDI.CodDiv ~
  AND FacDPedi.CodDoc = PEDI.CodDoc ~
  AND FacDPedi.NroPed = PEDI.NroPed ~
  AND FacDPedi.codmat = PEDI.codmat NO-LOCK ~
    BY PEDI.NroItm
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH PEDI WHERE TRUE /* Join to FacCPedi incomplete */ NO-LOCK, ~
  ...
  AND FacDPedi.codmat = PEDI.codmat NO-LOCK ~
    BY PEDI.NroItm.
&Scoped-define TABLES-IN-QUERY-br_table PEDI Almmmatg FacDPedi
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      PEDI.CanPed COLUMN-LABEL "Cantidad!Aprobada" FORMAT ">>>,>>9.99":U
            WIDTH 14.86 COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 11
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.PEDI Where INTEGRAL.FacCPedi ...,INTEGRAL.Almmmatg OF Temp-Tables.PEDI,INTEGRAL.FacDPedi WHERE Temp-Tables.PEDI ..."
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ", FIRST, FIRST"
     _OrdList          = "Temp-Tables.PEDI.NroItm|yes"
     _JoinCode[3]      = "INTEGRAL.FacDPedi.CodCia = Temp-Tables.PEDI.CodCia
  AND INTEGRAL.FacDPedi.CodDiv = Temp-Tables.PEDI.CodDiv
  AND INTEGRAL.FacDPedi.CodDoc = Temp-Tables.PEDI.CodDoc
  AND INTEGRAL.FacDPedi.NroPed = Temp-Tables.PEDI.NroPed
  AND INTEGRAL.FacDPedi.codmat = Temp-Tables.PEDI.codmat"
...
"PEDI.CanPed" "Cantidad!Aprobada" ">>>,>>9.99" "decimal" 11 0 ? ? ? ? yes ? no no "14.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.PEDI.Libre_d01
"PEDI.Libre_d01" "Cantidad!Pedida" ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "14.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON 'RETURN':U OF PEDI.CanPed, PEDI.codmat, PEDI.UndVta 
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  RUN gn/Stock-Comprometido-v2 (PEDI.CodMat, 
                                PEDI.AlmDes, 
                                YES,
  ...
  ASSIGN
      ...
      /*s-StkComprometido = s-StkComprometido - (PEDI.CanPed * PEDI.Factor).*/
