...
------------------------------------------------------------------------------*/
  ...
  END.
  ELSE DO:
      /* 31/10/2025: En caso de transferencias internas verificamos si debe pasar ppor control de barras */
      FIND FIRST TabGener WHERE TabGener.CodCia = s-CodCia
          AND TabGener.Clave = 'CFGINC'
          AND TabGener.Codigo = s-CodDiv
          NO-LOCK NO-ERROR.
      IF AVAILABLE TabGener AND TabGener.Libre_l01 = YES THEN DO:
          MESSAGE 'Esta sede está configurada para que las transferencias internas pase por control de barras'
...
------------------------------------------------------------------------------*/
  ...
  FIND CURRENT CMOV EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
  ...
  IF CMOV.CrossDocking = YES THEN DO:
      ...
      RUN ALM_Migrar_OTR_CrossDocking IN hProc (INPUT CMOV.CodRef,
                                                INPUT CMOV.NroRef,
                                                INPUT Almcmov.CodAlm,        /* Almacén de Salida */
                                                INPUT Almcmov.AlmacenXD,     /* Almacén de Destino */
                                                INPUT pZona,
                                                INPUT pBultos,
                                                OUTPUT pComprobante,
                                                OUTPUT pMensaje).
...
------------------------------------------------------------------------------*/
  ...
  IF x-CrossDocking = YES THEN DO:
      IF x-MsgCrossDocking > "" THEN MESSAGE x-MsgCrossDocking VIEW-AS ALERT-BOX INFORMATION.
...
/*        END.                                                                                   */
   ...
   IF CMOV.CrossDocking = YES THEN DO:
       ...
       RUN alm/d-ingreso-por-transferencia.w (INPUT ROWID(CMOV),
                                              OUTPUT pZona,
                                              OUTPUT pBultos,
                                              OUTPUT pOk).
