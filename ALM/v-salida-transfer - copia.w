...
Almcmov.Observ Almcmov.CrossDocking Almcmov.AlmacenXD 
...
Almcmov.HorSal Almcmov.Observ Almcmov.FchAnu Almcmov.CrossDocking ~
Almcmov.AlmacenXD 
...
DEFINE FRAME F-Main
     ...
     Almcmov.FchDoc AT ROW 1 COL 78 COLON-ALIGNED WIDGET-ID 12 FORMAT "99/99/9999"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     Almcmov.AlmDes AT ROW 1.81 COL 13 COLON-ALIGNED WIDGET-ID 2
          LABEL "Almacen  Destino" FORMAT "x(5)"
          VIEW-AS FILL-IN 
          SIZE 5 BY .81
     F-NomDes AT ROW 1.81 COL 18 COLON-ALIGNED NO-LABEL WIDGET-ID 8
     Almcmov.usuario AT ROW 1.81 COL 78 COLON-ALIGNED WIDGET-ID 38
          LABEL "Digitado por"
          VIEW-AS FILL-IN 
          SIZE 7.14 BY .81
     Almcmov.CodRef AT ROW 2.62 COL 13 COLON-ALIGNED WIDGET-ID 40
          LABEL "Reposición" FORMAT "x(3)"
          VIEW-AS FILL-IN 
          SIZE 5 BY .81
     Almcmov.NroRef AT ROW 2.62 COL 18 COLON-ALIGNED NO-LABEL WIDGET-ID 42 FORMAT "X(12)"
          VIEW-AS FILL-IN 
          SIZE 11 BY .81
     Almcmov.HorSal AT ROW 2.62 COL 78 COLON-ALIGNED WIDGET-ID 34 FORMAT "X(5)"
          VIEW-AS FILL-IN 
          SIZE 7.14 BY .81
     Almcmov.Observ AT ROW 3.42 COL 13 COLON-ALIGNED WIDGET-ID 22
     ...
     Almcmov.FchAnu AT ROW 3.42 COL 78 COLON-ALIGNED WIDGET-ID 32
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     FILL-IN-Motivo AT ROW 4.23 COL 13 COLON-ALIGNED WIDGET-ID 44
     FILL-IN-Urgente AT ROW 4.23 COL 78 COLON-ALIGNED NO-LABEL WIDGET-ID 46
     Almcmov.CrossDocking AT ROW 5.04 COL 15 WIDGET-ID 48
          LABEL "Cross Docking"
          VIEW-AS TOGGLE-BOX
          SIZE 14 BY .77
...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    ...
    END.
    FIND FIRST Facdpedi OF Faccpedi WHERE Facdpedi.CanPed > Facdpedi.CanAte
        NO-LOCK NO-ERROR.
...
/*         pMensaje = 'NO se pudo reasigar los pedidos expo'. */
/*         UNDO, RETURN 'ADM-ERROR'.                          */
/*     END.                                                   */
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      ...
      output-var-1 = ?.
  RUN lkup/c-otr ("Ordenes de Transferencia").
...
/*        IF almcmov.fchdoc <= dFchCie THEN DO:                                          */
/*            MESSAGE 'NO se puede anular ningun documento antes del' (dFchCie + 1) SKIP */
...
------------------------------------------------------------------------------*/
  ...
  ELSE IF AlmCmov.FlgEst <> "A" THEN RUN ALM\R-ImpGui-v2.R (ROWID(Almcmov)).
...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    FIND Faccpedi WHERE Faccpedi.codcia = s-codcia
        ...
        EXCLUSIVE-LOCK NO-ERROR.
    IF NOT AVAILABLE Faccpedi THEN DO:
        MESSAGE 'No se pudo bloquear el Pedido por Reposición Automática'
            VIEW-AS ALERT-BOX ERROR.
        RETURN "ADM-ERROR".
    END.
    FOR EACH almdmov OF almcmov NO-LOCK:
        FIND Facdpedi OF Faccpedi WHERE Facdpedi.codmat = Almdmov.codmat EXCLUSIVE-LOCK NO-ERROR.
        IF NOT AVAILABLE Facdpedi THEN DO:
