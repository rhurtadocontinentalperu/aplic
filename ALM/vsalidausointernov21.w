...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    FIND AlmCDocu WHERE AlmCDocu.codcia = s-codcia
        AND AlmCDocu.Libre_c05 = Almcmov.cco
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      input-var-1 = "SUI"
      input-var-2 = "P"
      input-var-3 = s-codalm + '|' + STRING(Almtdocm.CodMov, '99')
      output-var-1 = ?.
  RUN lkup/c-usointernoxalmv21 ("Solicitudes para Uso Interno").
  ...
  DO WITH FRAME {&FRAME-NAME}:
     DISPLAY 
         ...
         Almcdocu.Libre_c05 @ Almcmov.cco
         Almcdocu.libre_c01 @ Almcmov.Observ.
     FIND LAST gn-tcmb NO-LOCK NO-ERROR.
     IF AVAILABLE gn-tcmb THEN F-TPOCMB = gn-tcmb.compra.
     FIND cb-auxi WHERE cb-auxi.codcia = cb-codcia
         AND cb-auxi.clfaux = "CCO"
         AND cb-auxi.codaux = Almcdocu.Libre_c05
         NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE Almcmov THEN DO WITH FRAME {&FRAME-NAME}:
     ...
     FIND cb-auxi WHERE cb-auxi.codcia = cb-codcia
         AND cb-auxi.clfaux = "CCO"
         AND cb-auxi.codaux = Almcmov.cco
         NO-LOCK NO-ERROR.
     IF AVAILABLE cb-auxi THEN DISPLAY cb-auxi.nomaux @  FILL-IN-NomAux.
