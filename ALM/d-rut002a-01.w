...
DEFINE INPUT PARAMETER pDivOri AS CHAR.
...
DEFINE FRAME gDialog
     ...
     RADIO-SET-1 AT ROW 4.77 COL 75 NO-LABEL WIDGET-ID 2
     SPACE(0.85) SKIP(1.14)
...
/* SETTINGS FOR RADIO-SET RADIO-SET-1 IN FRAME gDialog
   NO-ENABLE                                                            */
...
"CodCta2" "Area Responsable" ? "character" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
  ...
  IF LOOKUP(pDivOri,"00024,00030,00070") > 0 THEN DO:
      radio-set-1:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'A'.
  END.
