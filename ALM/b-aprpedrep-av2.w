...
DEF VAR s-TipMov AS CHAR INIT 'A,M'.
...
almcrepo.AlmPed = s-codalm AND ~
almcrepo.FlgEst = s-flgest AND ~
(s-FlgSit = "" OR almcrepo.FlgSit = s-flgsit) AND ~
LOOKUP(almcrepo.tipmov,s-tipmov) > 0 )
...
&Scoped-define FIELDS-IN-QUERY-br_table almcrepo.TipMov almcrepo.CodAlm ~
Almacen.Descripcion almcrepo.FchDoc almcrepo.Fecha almcrepo.FchVto ~
almcrepo.NroSer almcrepo.NroDoc 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH almcrepo WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK, ~
      FIRST Almacen OF almcrepo NO-LOCK ~
    BY almcrepo.FchDoc DESCENDING INDEXED-REPOSITION
...
DEFINE VARIABLE x-FlgSit AS CHARACTER INITIAL "P" 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Por Aprobar x CD", "G",
"Por aprobar x Otros", "P"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      almcrepo.TipMov COLUMN-LABEL "Tipo" FORMAT "X":U
      ...
      almcrepo.NroDoc FORMAT "999999":U WIDTH 6.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     ...
     x-FlgEst AT ROW 1.19 COL 87 NO-LABEL WIDGET-ID 10
     BUTTON-11 AT ROW 1.27 COL 99 WIDGET-ID 18
     x-FlgSit AT ROW 5.31 COL 87 NO-LABEL WIDGET-ID 6
...
"almcrepo.TipMov" "Tipo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"almcrepo.FchDoc" "Emision" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.almcrepo.Fecha
"almcrepo.Fecha" "Entrega" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   = INTEGRAL.almcrepo.FchVto
     _FldNameList[7]   = INTEGRAL.almcrepo.NroSer
     _FldNameList[8]   > INTEGRAL.almcrepo.NroDoc
"almcrepo.NroDoc" ? ? "integer" ? ? ? ? ? ? no ? no no "6.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    FOR EACH Almcrepo NO-LOCK WHERE {&Condicion},
        ...
        FOR EACH almdrepo OF almcrepo NO-LOCK:
            ...
            ASSIGN
                ...
                w-report.Campo-C[3] = s-FlgEst
                w-report.Campo-C[4] = s-FlgSit
