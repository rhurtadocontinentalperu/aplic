...
Almcmov.Observ Almcmov.TpoCmb Almcmov.NroRf2 Almcmov.FchCbd 
...
Almcmov.TpoCmb Almcmov.NroRf2 Almcmov.FchCbd 
...
"US$", 2
     ...
     Almcmov.TpoCmb AT ROW 3.35 COL 73.86 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 8.57 BY .69
     Almcmov.NroRf2 AT ROW 4.08 COL 13 COLON-ALIGNED
          LABEL "Referencia 2"
          VIEW-AS FILL-IN 
          SIZE 10 BY .69
     Almcmov.FchCbd AT ROW 4.08 COL 32 COLON-ALIGNED
          LABEL "Fecha G/R"
          VIEW-AS FILL-IN 
          SIZE 8.57 BY .69
     "Moneda :" VIEW-AS TEXT
...
/* SETTINGS FOR FILL-IN F-STATUS IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN Almcmov.FchCbd IN FRAME F-Main
   EXP-LABEL                                                            */
...
input-var-1 = "A,T,G".  /* que LG-COCmp.FlgSit sea diferente de 'T' 0 'A' */

RUN LKUP\C-OCSGPEN("Consignaciones Pendientes").
...
IF AVAILABLE LG-COCmp THEN DO WITH FRAME {&FRAME-NAME}:
   ...
   DISPLAY LG-COCmp.NroDoc @ Almcmov.NroRf1 
   ...
   FOR EACH LG-DOCmp NO-LOCK WHERE LG-DOCmp.CodCia = LG-COCmp.CodCia 
                              ...
                              AND  (LG-DOCmp.CanPedi - LG-DOCmp.CanAten) > 0:
            /*  FIND ITEM WHERE ITEM.Codcia  = S-CODCIA AND
                     ITEM.CodAlm = S-CODALM AND
                     ITEM.CodMat = LG-DOCmp.Codmat EXCLUSIVE-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  FOR EACH Almdmov OF Almcmov EXCLUSIVE-LOCK ON ERROR UNDO, RETURN "ADM-ERROR":
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
    /* RHC 05.04.04 BLOQUEADO, SE ACTUALIZA EN LA NOCHE 
...
------------------------------------------------------------------------------*/
   ...
   FOR EACH LG-DOCmp NO-LOCK WHERE 
            LG-DOCmp.CodCia = Almcmov.CodCia AND
            LG-DOCmp.CodDiv = s-CodDiv AND
            LG-DOCmp.TpoDoc = "N" AND
  ...
  IF AVAILABLE LG-COCmp THEN DO ON ERROR UNDO, RETURN "ADM-ERROR": 
...
/*
       ...
       FIND Almmmatg WHERE Almmmatg.codcia = s-codcia AND
            Almmmatg.codmat = ITEM.Codmat NO-ERROR.
...
DO WITH FRAME {&FRAME-NAME} :
   ...
   IF Almcmov.NroRf2:SCREEN-VALUE = "" THEN DO:
      ...
      RETURN "ADM-ERROR".
   END.
   IF Almcmov.FchCbd:SCREEN-VALUE = "" THEN DO:
      MESSAGE "Digite la Fecha de la Guia" VIEW-AS ALERT-BOX ERROR.
      APPLY "ENTRY" TO Almcmov.FchCbd.
      RETURN "ADM-ERROR".
   END.
