...
DEF OUTPUT PARAMETER pParam AS LOG.
DEF OUTPUT PARAMETER pFiltro AS CHAR.   /* Valores separados por '|', Ej. 'UHU|FABER' */
...
&Scoped-Define ENABLED-OBJECTS RECT-10 BROWSE-2 Btn_OK Btn_Cancel 
      ...
      LT-DREPO.TipMov COLUMN-LABEL "Estado" FORMAT "X(10)":U WIDTH 12.43
            VIEW-AS COMBO-BOX INNER-LINES 5
                      LIST-ITEM-PAIRS " ","",
                                      ...
                                      "FILTRAR","FILTRAR"
                      DROP-DOWN-LIST 
      LT-DREPO.AlmPed COLUMN-LABEL "Marca" FORMAT "x(30)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 105 BY 9.62
...
DEFINE FRAME D-Dialog
     ...
     FILL-IN-Costo AT ROW 10.96 COL 52 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     FILL-IN-Peso AT ROW 10.96 COL 69 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     FILL-IN-Volumen AT ROW 10.96 COL 86 COLON-ALIGNED NO-LABEL WIDGET-ID 8
     Btn_OK AT ROW 12.58 COL 2
     Btn_Cancel AT ROW 12.58 COL 21 WIDGET-ID 16
     "TOTALES" VIEW-AS TEXT
          SIZE 8 BY .62 AT ROW 11.23 COL 43 WIDGET-ID 14
          FONT 6
     RECT-10 AT ROW 10.69 COL 2 WIDGET-ID 12
     SPACE(2.28) SKIP(2.30)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "RESUMEN POR MARCA" WIDGET-ID 100.
...
/* SETTINGS FOR DIALOG-BOX D-Dialog
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-2 RECT-10 D-Dialog */
ASSIGN 
...
"LT-DREPO.TipMov" "Estado" "X(10)" "character" ? ? ? ? ? ? yes ? no no "12.43" yes no no "U" "" "" "DROP-DOWN-LIST" "," ? " ,,ELIMINAR,ELIMINAR,FILTRAR,FILTRAR" 5 no 0 no no
     _FldNameList[2]   > Temp-Tables.LT-DREPO.AlmPed
"LT-DREPO.AlmPed" "Marca" "x(30)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"LT-DREPO.CanTran" "Volumen (m3)" "ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? no ? no no "16.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* RESUMEN POR MARCA */
DO:  
...
DO:
    /* Procedemos a barrer el temporal */
    pParam = YES.
    pFiltro = ''.
    FOR EACH LT-DREPO NO-LOCK:
        CASE LT-DREPO.TipMov:
            WHEN 'ELIMINAR' THEN DO:
                FOR EACH T-DREPO, 
                    FIRST Almmmatg OF T-DREPO NO-LOCK WHERE Almmmatg.desmar = LT-DREPO.AlmPed:
                    DELETE T-DREPO.
                END.
            END.
            WHEN 'FILTRAR' THEN DO:
                pFiltro = pFiltro +
                    (IF TRUE <> (pFiltro > '') THEN '' ELSE '|') +
                    LT-DREPO.AlmPed.
            END.
        END CASE.
    END.
...
FOR EACH T-DREPO, FIRST Almmmatg OF T-DREPO NO-LOCK:
    FIND LT-DREPO WHERE LT-DREPO.AlmPed = Almmmatg.DesMar NO-ERROR.
    IF NOT AVAILABLE LT-DREPO THEN CREATE LT-DREPO.
    ASSIGN
        LT-DREPO.AlmPed = Almmmatg.DesMar
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-Costo FILL-IN-Peso FILL-IN-Volumen 
      WITH FRAME D-Dialog.
  ENABLE RECT-10 BROWSE-2 Btn_OK Btn_Cancel 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
