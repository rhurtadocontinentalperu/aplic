...
DEFINE SHARED VAR s-codalm AS CHARACTER.
  ...
  DEF VAR x-alminv AS CHAR.
  DEF VAR X-alminvcia AS CHAR.

ASSIGN X-alminv = s-codalm.
ASSIGN X-alminvcia = 'CONTI'.
...
/* SETTINGS FOR FILL-IN InvConfig.FchInv IN FRAME F-Main
   EXP-LABEL                                                            */
...
*/    
  ...
  FOR EACH T-Ajuste USE-INDEX Idx02 NO-LOCK 
    ...
    ASSIGN
        ...
        /* RDP Cambio pedido en la señor Enrique ** 
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH AlmDInv WHERE AlmDInv.CodCia = s-codcia
      AND AlmDInv.CodAlm = X-alminv
      AND AlmDInv.NomCia = X-alminvcia NO-LOCK,
      ...
      IF X-Dife <> 0 THEN DO:
          ...
          ASSIGN 
              ...
              T-Ajuste.DesMat = Almmmatg.DesMat
              T-Ajuste.UndStk = Almmmatg.UndStk
              T-Ajuste.CodAlm = X-alminv   /*InvConfig.CodAlm*/
              T-Ajuste.StkSub = X-StkSub
              T-Ajuste.Dife   = X-Dife
              T-Ajuste.TipMov = X-TipMov
              T-Ajuste.TipInv = X-TipInv.
  ...
  FOR EACH T-Ajuste USE-INDEX Idx02 NO-LOCK 
    ...
    ASSIGN
        ITEM.CodCia = InvConfig.codcia
        ITEM.CodAlm = x-alminv  /*InvConfig.CodAlm*/
        ITEM.TipMov = T-Ajuste.TipMov
        /*ITEM.CodMov = 01*/
        ITEM.CodMov = x-CodMov
        /*RDP - Cambio de fecha pedido señor Enrique ***
        ITEM.FchDoc = InvConfig.FchInv 
        *** */
        ITEM.FchDoc = TODAY
        ITEM.CodMon = 1
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE crea-excel-data V-table-Win 
PROCEDURE crea-excel-data :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH ITEM NO-LOCK :
     ...
     chWorkSheet:Range(cCelda):Value = ITEM.codmat.
     cCelda = "B" + STRING(lLinea).
     chWorkSheet:Range(cCelda):Value = ITEM.candes.
     cCelda = "C" + STRING(lLinea).
     chWorkSheet:Range(cCelda):Value = ITEM.codund.
     cCelda = "D" + STRING(lLinea).
     chWorkSheet:Range(cCelda):Value = ITEM.tipmov.

    lLinea = lLinea + 1.
END.

/*chWorkSheet:SaveAs("d:\ciman\file.xls",6,,,,0,,,).*/
chWorkSheet:SaveAs("d:\ciman\tempoAlm45.xls").
        ...
        IF FIRST-OF(ITEM.TipMov) OR FIRST-OF(ITEM.Flg_Factor) THEN DO:
            ...
            */
            ...
            ASSIGN 
                ...
                /*RDP - Cambio a pedido del señor Enrique
...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    ...
    FOR EACH ITEM WHERE ITEM.codalm = X-alminv  AND ITEM.tipmov = 'I':
        ASSIGN
            ITEM.codalm = '11T'
            ITEM.tipmov = 'S'.        
    END.
    ...
    FOR EACH ITEM NO-LOCK WHERE ITEM.CodMat <> '' AND ITEM.TipMov = 'S' BREAK BY ITEM.Flg_Factor BY ITEM.CodMat:
        ...
        IF FIRST-OF(ITEM.Flg_Factor) THEN DO:
           ...
           ASSIGN 
               ...
               Almcmov.CodAlm = ITEM.CodAlm                        
               /*Almcmov.AlmDes = (IF ITEM.CodAlm = '11T' THEN InvConfig.CodAlm ELSE '11T') */
               Almcmov.AlmDes = (IF ITEM.CodAlm = '11T' THEN X-alminv ELSE '11T')
               Almcmov.TipMov = ITEM.TipMov
               Almcmov.CodMov = ITEM.CodMov
               Almcmov.NroSer = 000
               Almcmov.NroDoc = Almacen.CorrSal
