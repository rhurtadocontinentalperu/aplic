...
DEFINE VARIABLE RADIO-SET-Costo AS INTEGER INITIAL 2 
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "DIFERENCIA CONSOLIDADA VALORIZADA"
         HEIGHT             = 10.96
         WIDTH              = 67.14
         MAX-HEIGHT         = 11.5
         MAX-WIDTH          = 80
...
ON END-ERROR OF W-Win /* DIFERENCIA CONSOLIDADA VALORIZADA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* DIFERENCIA CONSOLIDADA VALORIZADA */
DO:
...
------------------------------------------------------------------------------*/
  DO WITH FRAME {&FRAME-NAME}:
    ASSIGN DesdeC HastaC DesdeP HastaP DFecha HFecha RADIO-SET-Costo RADIO-SET-1.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
    ...
    END.
  END.
  /* Buscamos los ajustes posteriores al inventario */
  FOR EACH Almdmov NO-LOCK WHERE Almdmov.codcia = s-codcia
...
------------------------------------------------------------------------------*/
  ...
  ENABLE DFecha HFecha DesdeC HastaC RADIO-SET-Costo RADIO-SET-1 Btn_OK 
         Btn_Cancel 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  x-SubTit = IF RADIO-SET-Costo = 1 
            THEN 'VALORIZADO AL COSTO PROMEDIO'
            ELSE 'VALORIZADO AL COSTO DE REPOSICION'.
  DEFINE FRAME F-DETALLE
    ...
    DETALLE.CtoUni      COLUMN-LABEL "Unitario"         FORMAT '->,>>9.9999'
    DETALLE.Cto03       COLUMN-LABEL "Dif 03"           FORMAT '->>>,>>9.99'
    DETALLE.Cto04       COLUMN-LABEL "Dif 04"           FORMAT '->>>,>>9.99'
    DETALLE.Cto05       COLUMN-LABEL "Dif 05"           FORMAT '->>>,>>9.99'
    DETALLE.Cto11       COLUMN-LABEL "Dif 11"           FORMAT '->>>,>>9.99'
    DETALLE.Cto16       COLUMN-LABEL "Dif 16"           FORMAT '->>>,>>9.99'
    DETALLE.Cto17       COLUMN-LABEL "Dif 17"           FORMAT '->>>,>>9.99'
    DETALLE.Cto18       COLUMN-LABEL "Dif 18"           FORMAT '->>>,>>9.99'
    DETALLE.Cto48       COLUMN-LABEL "Dif 48"           FORMAT '->>>,>>9.99'
    DETALLE.Cto83       COLUMN-LABEL "Dif 83"           FORMAT '->>>,>>9.99'
    DETALLE.CtoOtr      COLUMN-LABEL "Dif Otros"        FORMAT '->>>>>>,>>9.99'
    x-DifPos            COLUMN-LABEL "Total (+)"        FORMAT '->>>,>>9.99'
    x-DifNeg            COLUMN-LABEL "Total (-)"        FORMAT '->,>>>,>>9.99'
/*    x-CtoTot            COLUMN-LABEL "Total"            FORMAT '->,>>>,>>9.99'*/
  ...
  DEFINE FRAME F-HEADER
    ...
    "DIFERENCIA CONSOLIDADA VALORIZADA POR ARTICULO" AT 20 
  ...
  FOR EACH DETALLE BREAK BY DETALLE.codcia BY DETALLE.catconta[1] BY DETALLE.desmat:
    ...
    x-CanInv = stk03 + stk04 + stk05 + stk11 + stk16 + stk17 + stk18 + stk48 + stk83 + stkotr.
    x-CtoTot = Cto03 + Cto04 + Cto05 + Cto11 + Cto16 + Cto17 + Cto18 + Cto48 + Cto83 + Ctootr.
    x-CtoUni = x-CtoTot / x-CanInv.
    x-DifPos = 0.
    x-DifNeg = 0.
    IF x-CtoTot >= 0
    THEN x-DifPos = x-CtoTot.
    ELSE x-DifNeg = x-CtoTot.
 ...
 *         x-CtoTot*/
    ...
    ACCUMULATE DETALLE.Cto17 (TOTAL BY DETALLE.codcia).
    ACCUMULATE DETALLE.Cto18 (TOTAL BY DETALLE.codcia).
    ACCUMULATE DETALLE.Cto48 (TOTAL BY DETALLE.codcia).
    ACCUMULATE DETALLE.Cto83 (TOTAL BY DETALLE.codcia).
    ACCUMULATE DETALLE.CtoOtr (TOTAL BY DETALLE.codcia).
    ACCUMULATE x-CtoTot (TOTAL BY DETALLE.codcia).
    ACCUMULATE x-DifPos (TOTAL BY DETALLE.codcia).
    ACCUMULATE x-DifNeg (TOTAL BY DETALLE.codcia).
    IF LAST-OF(DETALLE.codcia) THEN DO:
        ...
        DISPLAY STREAM REPORT
            ...
            ACCUM TOTAL BY DETALLE.codcia DETALLE.Cto17 @ DETALLE.Cto17
            ACCUM TOTAL BY DETALLE.codcia DETALLE.Cto18 @ DETALLE.Cto18
            ACCUM TOTAL BY DETALLE.codcia DETALLE.Cto48 @ DETALLE.Cto48
            ACCUM TOTAL BY DETALLE.codcia DETALLE.Cto83 @ DETALLE.Cto83
            ACCUM TOTAL BY DETALLE.codcia DETALLE.CtoOtr @ DETALLE.CtoOtr
            ACCUM TOTAL BY DETALLE.codcia x-DifPos @ x-DifPos
            ACCUM TOTAL BY DETALLE.codcia x-DifNeg @ x-DifNeg
            /*ACCUM TOTAL BY DETALLE.codcia x-CtoTot @ x-CtoTot*/
            WITH FRAME F-DETALLE.
    END.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-DETALLE
    DETALLE.codmat      COLUMN-LABEL "Codigo"
    DETALLE.desmat      COLUMN-LABEL "Descripcion"      FORMAT 'X(35)'
    DETALLE.desmar      COLUMN-LABEL "Marca"            FORMAT 'X(8)'
    DETALLE.undbas      COLUMN-LABEL "Unidad"           FORMAT 'X(5)'
    DETALLE.catconta[1] COLUMN-LABEL "CC"               FORMAT 'X(2)'
    DETALLE.CtoUni      COLUMN-LABEL "Unitario"         FORMAT '->,>>9.9999'
    DETALLE.Cto03       COLUMN-LABEL "Dif 03"           FORMAT '->>>,>>9.99'
  ...
  FOR EACH DETALLE,
    ...
    ACCUMULATE x-DifPos (TOTAL BY DETALLE.codcia).
    ACCUMULATE x-DifNeg (TOTAL BY DETALLE.codcia).
    ACCUMULATE DETALLE.Cto03 (TOTAL BY DETALLE.codfam).
    ACCUMULATE DETALLE.Cto04 (TOTAL BY DETALLE.codfam).
    ACCUMULATE DETALLE.Cto05 (TOTAL BY DETALLE.codfam).
    ACCUMULATE DETALLE.Cto11 (TOTAL BY DETALLE.codfam).
    ACCUMULATE DETALLE.Cto16 (TOTAL BY DETALLE.codfam).
    ACCUMULATE DETALLE.Cto17 (TOTAL BY DETALLE.codfam).
    ACCUMULATE DETALLE.Cto18 (TOTAL BY DETALLE.codfam).
    ACCUMULATE DETALLE.Cto48 (TOTAL BY DETALLE.codfam).
    ACCUMULATE DETALLE.Cto83 (TOTAL BY DETALLE.codfam).
    ACCUMULATE DETALLE.CtoOtr (TOTAL BY DETALLE.codfam).
    ACCUMULATE x-CtoTot (TOTAL BY DETALLE.codfam).
    ACCUMULATE x-DifPos (TOTAL BY DETALLE.codfam).
    ACCUMULATE x-DifNeg (TOTAL BY DETALLE.codfam).
    ACCUMULATE DETALLE.Cto03 (TOTAL BY DETALLE.subfam).
    ACCUMULATE DETALLE.Cto04 (TOTAL BY DETALLE.subfam).
    ACCUMULATE DETALLE.Cto05 (TOTAL BY DETALLE.subfam).
    ACCUMULATE DETALLE.Cto11 (TOTAL BY DETALLE.subfam).
    ACCUMULATE DETALLE.Cto16 (TOTAL BY DETALLE.subfam).
    ACCUMULATE DETALLE.Cto17 (TOTAL BY DETALLE.subfam).
    ACCUMULATE DETALLE.Cto18 (TOTAL BY DETALLE.subfam).
    ACCUMULATE DETALLE.Cto48 (TOTAL BY DETALLE.subfam).
    ACCUMULATE DETALLE.Cto83 (TOTAL BY DETALLE.subfam).
    ACCUMULATE DETALLE.CtoOtr (TOTAL BY DETALLE.subfam).
    ACCUMULATE x-CtoTot (TOTAL BY DETALLE.subfam).
    ACCUMULATE x-DifPos (TOTAL BY DETALLE.subfam).
    ACCUMULATE x-DifNeg (TOTAL BY DETALLE.subfam).
    IF LAST-OF(DETALLE.subfam) THEN DO:
        UNDERLINE STREAM REPORT
            ...
            WITH FRAME F-DETALLE.
        DISPLAY STREAM REPORT
            'TOTAL SUB-FAMILIA  >>>' @ DETALLE.desmat
            ACCUM TOTAL BY DETALLE.subfam DETALLE.Cto03 @ DETALLE.Cto03
            ACCUM TOTAL BY DETALLE.subfam DETALLE.Cto04 @ DETALLE.Cto04
            ACCUM TOTAL BY DETALLE.subfam DETALLE.Cto05 @ DETALLE.Cto05
            ACCUM TOTAL BY DETALLE.subfam DETALLE.Cto11 @ DETALLE.Cto11
            ACCUM TOTAL BY DETALLE.subfam DETALLE.Cto16 @ DETALLE.Cto16
            ACCUM TOTAL BY DETALLE.subfam DETALLE.Cto17 @ DETALLE.Cto17
            ACCUM TOTAL BY DETALLE.subfam DETALLE.Cto18 @ DETALLE.Cto18
            ACCUM TOTAL BY DETALLE.subfam DETALLE.Cto48 @ DETALLE.Cto48
            ACCUM TOTAL BY DETALLE.subfam DETALLE.Cto83 @ DETALLE.Cto83
            ACCUM TOTAL BY DETALLE.subfam DETALLE.CtoOtr @ DETALLE.CtoOtr
            ACCUM TOTAL BY DETALLE.subfam x-DifPos @ x-DifPos
            ACCUM TOTAL BY DETALLE.subfam x-DifNeg @ x-DifNeg
            /*ACCUM TOTAL BY DETALLE.subfam x-CtoTot @ x-CtoTot*/
            WITH FRAME F-DETALLE.
    END.
    IF LAST-OF(DETALLE.codfam) THEN DO:
        UNDERLINE STREAM REPORT
            ...
            WITH FRAME F-DETALLE.
        DISPLAY STREAM REPORT
            'TOTAL FAMILIA  >>>' @ DETALLE.desmat
            ACCUM TOTAL BY DETALLE.codfam DETALLE.Cto03 @ DETALLE.Cto03
            ACCUM TOTAL BY DETALLE.codfam DETALLE.Cto04 @ DETALLE.Cto04
            ACCUM TOTAL BY DETALLE.codfam DETALLE.Cto05 @ DETALLE.Cto05
            ACCUM TOTAL BY DETALLE.codfam DETALLE.Cto11 @ DETALLE.Cto11
            ACCUM TOTAL BY DETALLE.codfam DETALLE.Cto16 @ DETALLE.Cto16
            ACCUM TOTAL BY DETALLE.codfam DETALLE.Cto17 @ DETALLE.Cto17
            ACCUM TOTAL BY DETALLE.codfam DETALLE.Cto18 @ DETALLE.Cto18
            ACCUM TOTAL BY DETALLE.codfam DETALLE.Cto48 @ DETALLE.Cto48
            ACCUM TOTAL BY DETALLE.codfam DETALLE.Cto83 @ DETALLE.Cto83
            ACCUM TOTAL BY DETALLE.codfam DETALLE.CtoOtr @ DETALLE.CtoOtr
            ACCUM TOTAL BY DETALLE.codfam x-DifPos @ x-DifPos
            ACCUM TOTAL BY DETALLE.codfam x-DifNeg @ x-DifNeg
            /*ACCUM TOTAL BY DETALLE.codfam x-CtoTot @ x-CtoTot*/
            WITH FRAME F-DETALLE.
    END.
    IF LAST-OF(DETALLE.codcia) THEN DO:
        UNDERLINE STREAM REPORT
