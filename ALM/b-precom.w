...
&SCOPED-DEFINE FILTRO1 ( (Almmmatg.DesMat BEGINS FILL-IN-filtro) AND ~
                         (Almmmatg.DesMar BEGINS F-Marca) AND (Almmmatg.CodPr1 BEGINS F-PROVEE) )
...
&Scoped-Define ENABLED-OBJECTS RECT-1 F-Codfam F-SubFam FILL-IN-codigo ~
F-PROVEE F-Marca CMB-filtro FILL-IN-filtro br_table 
&Scoped-Define DISPLAYED-OBJECTS F-Codfam F-SubFam FILL-IN-codigo F-PROVEE ~
F-Marca CMB-filtro FILL-IN-filtro 
...
Marca|||integral.Almmmatg.CodCia|yes,integral.Almmmatg.CodMar|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "Codigo,Descripcion,Marca",
     Sort-Case = Codigo':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      X-MON @ X-MON COLUMN-LABEL "Moneda"
      Almmmatg.TpoCmb
      Almmmatg.CtoTot
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 81.86 BY 6.19
...
DEFINE FRAME F-Main
     F-Codfam AT ROW 1.27 COL 8.29 COLON-ALIGNED
     F-SubFam AT ROW 2.12 COL 8.29 COLON-ALIGNED
     FILL-IN-codigo AT ROW 2.92 COL 8.29 COLON-ALIGNED
     F-PROVEE AT ROW 2.27 COL 27.14 COLON-ALIGNED
     F-Marca AT ROW 3 COL 27.14 COLON-ALIGNED
     CMB-filtro AT ROW 1.31 COL 54.57 NO-LABEL
     FILL-IN-filtro AT ROW 2.23 COL 54.43 NO-LABEL
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "integral.Almmmatg.CodCia|yes,integral.Almmmatg.codfam|yes"
     _Where[1]         = "{&CONDICION}"
     _FldNameList[1]   > integral.Almmmatg.codmat
"codmat" "Codigo     .!Articulo" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   > integral.Almmmatg.DesMat
"DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   > integral.Almmmatg.DesMar
"DesMar" "Marca" "X(20)" "character" ? ? ? ? ? ? no ?
     _FldNameList[4]   > "_<CALC>"
"X-MON @ X-MON" "Moneda" ? ? ? ? ? ? ? ? no ?
...
DO:
    IF CMB-filtro = CMB-filtro:SCREEN-VALUE AND
        ...
        F-Provee = F-Provee:SCREEN-VALUE THEN RETURN.
    ASSIGN
        FILL-IN-filtro
        CMB-filtro
        F-Marca
...
DO:
  IF F-CodFam = F-CodFam:SCREEN-VALUE THEN RETURN.
  ASSIGN F-CodFam.
  RUN dispatch IN THIS-PROCEDURE ('open-query-cases':U).
END.
...
DO:
  IF F-SubFam = F-SubFam:SCREEN-VALUE THEN RETURN.
  ASSIGN  F-CodFam F-SubFam.
  RUN dispatch IN THIS-PROCEDURE ('open-query-cases':U).
END.
...
DO:
    ...
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        ...
        /****   para ubicar el registro en el browse    ****/   
         {src/adm/template/brschnge.i}
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         ...
         WHEN 'Marca':U THEN DO:
           &Scope SORTBY-PHRASE BY Almmmatg.CodCia BY Almmmatg.CodMar
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE local-assign-statement L-table-Win 
...
/*  FIND AlmsFami WHERE AlmsFami.Codcia = s-codcia AND
  ...
  IF AVAILABLE AlmsFami THEN 
      ASSIGN
         Almmmatg.Ordlis = (AlmsFami.Orden * 10000) + Almmmatg.Orden. */
      ASSIGN
         Almmmatg.Ordlis = Almmmatg.Orden.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Reordenar L-table-Win 
PROCEDURE Reordenar :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE VAR X-Orden  AS INTEGER NO-UNDO.
  DEFINE VAR X-OrdMat AS INTEGER NO-UNDO.

  /* Reordenar de 3 en 3 */  
  FOR EACH B-MATG WHERE B-MATG.Codcia = s-codcia 
      BREAK BY B-MATG.CodFam BY B-MATG.SubFam BY B-MATG.Orden:
      IF FIRST-OF(B-MATG.Codfam) THEN DO:
         X-OrdMat = 1.
      END.
      ASSIGN
         B-MATG.OrdTmp = X-OrdMat.
      X-OrdMat = X-OrdMat + 3.
  END.

  /* Orden de presentacion de acuerdo a lista */
  FOR EACH B-MATG WHERE B-MATG.Codcia = s-codcia 
      BREAK BY B-MATG.CodFam BY B-MATG.OrdTmp:
      IF FIRST-OF(B-MATG.Codfam) THEN DO:
         X-Orden = INTEGER(B-MATG.Codfam).
      END.
      ASSIGN
         B-MATG.Orden  = B-MATG.OrdTmp
         B-MATG.Ordlis = B-MATG.Orden. /*(X-Orden * 100000) + B-MATG.OrdTmp.*/
  END.
