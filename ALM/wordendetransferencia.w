...
DEFINE VARIABLE h_vordendetransferencia AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/vordendetransferencia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vordendetransferencia ).
       RUN set-position IN h_vordendetransferencia ( 2.62 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_vordendetransferencia. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_vordendetransferencia ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_vordendetransferencia ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_vordendetransferencia ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             h_vordendetransferencia , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
...
CASE L-Handle:
 ...
 WHEN "Disable-Head" THEN DO:
     ...
     RUN dispatch IN h_vordendetransferencia ('disable-fields').
   END.
 WHEN "Enable-Head" THEN DO:
     RUN dispatch IN h_p-updv04 ('enable':U).
     RUN dispatch IN h_vordendetransferencia ('enable-fields').
   END.
END CASE.
