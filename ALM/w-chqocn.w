...
&Scoped-Define ENABLED-OBJECTS Btn_Done BUTTON-9 BUTTON-10 
...
DEFINE VARIABLE h_b-chqocn AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-9 AT ROW 1.19 COL 56
     BUTTON-10 AT ROW 1.19 COL 71 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 109.43 BY 18.81
         FONT 4.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CHEQUEO DE INGRESOS POR  ORDEN DE COMPRA"
         HEIGHT             = 18.81
         WIDTH              = 109.43
         MAX-HEIGHT         = 32.69
         MAX-WIDTH          = 205.29
         VIRTUAL-HEIGHT     = 32.69
         VIRTUAL-WIDTH      = 205.29
         RESIZE             = no
         SCROLL-BARS        = no
         STATUS-AREA        = no
         BGCOLOR            = ?
...
ON END-ERROR OF W-Win /* CHEQUEO DE INGRESOS POR  ORDEN DE COMPRA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* CHEQUEO DE INGRESOS POR  ORDEN DE COMPRA */
DO:
...
ON CHOOSE OF BUTTON-10 IN FRAME F-Main /* Nueva Orden */
DO:
  RUN Nueva-Orden IN h_b-chqocn.
END.
...
ON CHOOSE OF BUTTON-9 IN FRAME F-Main /* Cerra Orden */
DO:
  MESSAGE 'Continuamos con el proceso?' VIEW-AS ALERT-BOX QUESTION
      BUTTONS YES-NO UPDATE rpta AS LOG.
  IF rpta = NO THEN RETURN NO-APPLY.
  RUN Cierre-de-guia IN h_b-chqocn.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/b-chqocn.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-chqocn ).
       RUN set-position IN h_b-chqocn ( 2.54 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-chqocn ( 16.19 , 108.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-chqocn. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-chqocn ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             Btn_Done:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-chqocn ,
...
------------------------------------------------------------------------------*/
  ENABLE Btn_Done BUTTON-9 BUTTON-10 
