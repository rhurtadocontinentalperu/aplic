...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      /* RHC 21/10/2016 BLOQUEAMOS LA REPOSICION DE MERCADERIA */
      FIND Almcrepo WHERE Almcrepo.codcia = s-codcia
      ...
      IF ERROR-STATUS:ERROR THEN DO:
          pMensaje = "NO se pudo bloquear el pedido por reposición: " + Faccpedi.nroref.
          UNDO, RETURN 'ADM-ERROR'.
      END.
      IF NOT (Almcrepo.FlgEst = "P" AND Almcrepo.flgsit = "A") THEN DO:
          pMensaje = "La reposición " + Faccpedi.nroref + " ya ha sido procesada por otro usuario".
          UNDO, RETURN 'ADM-ERROR'.
      END.
      ASSIGN
          Almcrepo.FlgEst = "C".    /* OJO: CON OTR */
      /* ***************************************************** */
      ...
      /* *************************************** */
      /* RHC 27/07/2016 Aprobación por Logística */
      /* *************************************** */
/*       ASSIGN                      */
/*           FacCPedi.FlgEst = "WL". */
      /* *************************************** */
      ...
      /* ***************************************************** */
      /* Recalculamos la Fecha de Entrega */
      /* ***************************************************** */
      DEF VAR pNroSKU AS INT NO-UNDO.
      DEF VAR pPeso AS de NO-UNDO.
      ...
      RUN gn/p-fchent-v3 (Faccpedi.CodAlm,
                          ...
                          Faccpedi.CodCli,
                          Faccpedi.CodDiv,
                          gn-divi.campo-char[3] + gn-divi.campo-char[4] + gn-divi.campo-char[5],
                          Faccpedi.coddoc,
                          Faccpedi.nroped,
                          pNroSKU,
                          pPeso,
                          INPUT-OUTPUT d-FechaEntrega,
                          OUTPUT pMensaje).
      IF pMensaje > '' THEN UNDO, RETURN 'ADM-ERROR'.
      ASSIGN
          Faccpedi.FchEnt = d-FechaEntrega.
      /* ***************************************************** */
      /* TRACKING */
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
  /* RHC 09/04/2015 VERIFICAMOS QUE EL PED. DE REPOSIC. NO HAYA
                    SIDO ACTUALIZADO POR OTRO USUARIO
  */
  FIND Almcrepo WHERE almcrepo.CodCia = s-codcia
      AND almcrepo.AlmPed = s-codalm
      AND almcrepo.NroSer = INTEGER(SUBSTRING(s-nrocot,1,3))
      AND almcrepo.NroDoc = INTEGER(SUBSTRING(s-nrocot,4))
      NO-LOCK.
  IF NOT AVAILABLE Almcrepo
      OR NOT (Almcrepo.almped = s-codalm 
              AND Almcrepo.flgest = "P"
              AND Almcrepo.flgsit = "A")
      THEN DO:
      MESSAGE "El PEDIDO PARA REPOSICION AUTOMATICA:" s-NroCot SKIP
          "ha sido modificado por otro usuario" SKIP
          "Proceso Abortado"
          VIEW-AS ALERT-BOX WARNING.
      UNDO, RETURN "ADM-ERROR".
  END.
