...
&SCOPED-DEFINE CONDICION ( ~
            INTEGRAL.vtactabla.CodCia = s-codcia AND ~
            INTEGRAL.vtactabla.tabla = x-tabla )
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH VtaCTabla WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH VtaCTabla WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main VtaCTabla
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main VtaCTabla
...
DEFINE FRAME FRAME-D
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 17.57 BY .88 WIDGET-ID 100.
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "{&CONDICION}"
     _Design-Parent    is WINDOW q-tables @ ( 1.04 , 9.86 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
      RUN lkup/c-pack-ec ('Packs e-Commerce').
    /*
