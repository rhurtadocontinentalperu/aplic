...
DEFINE VARIABLE h_ttransfxotr AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-movmto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vtransfxotr AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 103.14 BY 20.35 WIDGET-ID 100.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/vtransfxotr.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vtransfxotr ).
       RUN set-position IN h_vtransfxotr ( 4.65 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.08 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/q-movsaltrf.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/qtransfxotr.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_qtransfxotr ).
       RUN set-position IN h_qtransfxotr ( 2.73 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 16.14 ) */

       /* Links to SmartViewer h_v-movmto. */
       RUN add-link IN adm-broker-hdl ( h_q-movsaltrf , 'Record':U , h_v-movmto ).

       /* Links to SmartViewer h_vtransfxotr. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_vtransfxotr ).
       RUN add-link IN adm-broker-hdl ( h_qtransfxotr , 'Record':U , h_vtransfxotr ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_vtransfxotr ,
             h_p-updv13 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_qtransfxotr ,
             h_vtransfxotr , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/ttransfxotr.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_ttransfxotr ).
       RUN set-position IN h_ttransfxotr ( 9.08 , 2.00 ) NO-ERROR.
       RUN set-size IN h_ttransfxotr ( 11.92 , 85.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_ttransfxotr ,
             h_vtransfxotr , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    ...
    WHEN "Pagina2"  THEN DO:
         ...
         RUN adm-open-query IN h_ttransfxotr.
    END.
END CASE.
