...
DEFINE VARIABLE h_b-pedrepautv4 AS HANDLE NO-UNDO.
...
DO:
  ...
  RUN Eliminar IN h_b-pedrepautv4.
END.
...
DO:
   RUN Importar-Excel IN h_b-pedrepautv4.
   /* Renumeramos */
   ...
   FOR EACH T-DREPO BY T-DREPO.CodMat:
       ...
       x-Item = x-Item + 1.
   END.
   RUN dispatch IN h_b-pedrepautv4 ('open-query':U).
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ALM/b-pedrepautv4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Codigo':U ,
             OUTPUT h_b-pedrepautv4 ).
       RUN set-position IN h_b-pedrepautv4 ( 10.69 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedrepautv4 ( 14.77 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-pedrepautv4. */
       RUN add-link IN adm-broker-hdl ( h_p-updv08 , 'TableIO':U , h_b-pedrepautv4 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedrepautv4 ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv08 ,
             h_b-pedrepautv4 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
        ...
        ASSIGN
            ...
            T-DREPO.CanGen = 100
            T-DREPO.StkAct = 100.
        RUN adm-open-query-cases IN h_b-pedrepautv4.
...
/*IF T-MATE.CodMat = '067794' THEN MESSAGE 'LLEGA:' pReposicion.*/
...
FOR EACH Almrepos NO-LOCK WHERE Almrepos.CodCia = s-codcia
    ...
    REPEAT WHILE (x-CanReq + x-StkAct) > x-Tolerancia:
...
RUN adm-open-query IN h_b-pedrepautv4.
...
CASE TRUE:
    WHEN pOkSolicitante = YES AND pOk = YES THEN DO:   /* ES UN CD */
        ...
        IF AVAILABLE T-MATE-2 THEN DO:
            ...
            ELSE DO:
                ...
                IF (T-MATE-2.DesStkDis - T-MATE-2.DesStkMax + T-MATE-2.DesStkTra) > 
                    ( Almmmate.StkAct - Almmmate.StkComprometido - 
                      (IF AVAILABLE TabGener AND TabGener.Libre_l02 = NO THEN 0 ELSE Almmmate.StkMin) + 
                      fStockTransito(Almmmate.CodAlm,Almmmate.CodMat)
                      ) THEN DO:
                    ASSIGN
                        /* Stock Disponible Despachante */
    ...
    OTHERWISE DO:    /* ES UNA TIENDA */
        ...
        /* Datos adicionales */
        ASSIGN
            T-MATE-2.DesStkMax = Almmmate.VInMn1
