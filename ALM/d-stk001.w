...
DEF TEMP-TABLE DETALLE LIKE Almstkge
    FIELD Valor AS DEC
    FIELD DesMat LIKE Almmmatg.desmat
    FIELD DesMar LIKE Almmmatg.desmar
    FIELD CatConta LIKE Almmmatg.CatConta[1]
...
&Scoped-Define ENABLED-OBJECTS RECT-57 DFecha DesdeC HastaC c-Categoria ~
Btn_OK Btn_Cancel 
&Scoped-Define DISPLAYED-OBJECTS DFecha DesdeC HastaC c-Categoria 
...
DEFINE VARIABLE c-Categoria AS CHARACTER FORMAT "X(256)":U INITIAL "Todos" 
     LABEL "Categoria" 
...
DEFINE FRAME F-Main
     ...
     c-Categoria AT ROW 4.08 COL 18 COLON-ALIGNED
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "ARTICULOS CON COSTOS CERO NEGATIVO"
         HEIGHT             = 10.92
         WIDTH              = 80
         MAX-HEIGHT         = 11.5
         MAX-WIDTH          = 80
...
ON END-ERROR OF W-Win /* ARTICULOS CON COSTOS CERO NEGATIVO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ARTICULOS CON COSTOS CERO NEGATIVO */
DO:
...
DO WITH FRAME {&FRAME-NAME}:
    ASSIGN DesdeC DFecha HastaC c-Categoria.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Almmmatg WHERE Almmmatg.codcia = s-codcia 
        ...
        AND (c-Categoria = 'Todos' OR Almmmatg.CatConta[1] = c-Categoria)
        NO-LOCK:
    FIND LAST Almstkge OF Almmmatg WHERE Almstkge.fecha <= DFecha
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almstkge 
        OR (Almstkge.stkact * Almstkge.ctouni > 0)
    THEN NEXT.
    IF Almstkge.stkact > 0 AND Almstkge.ctouni <= 0 
    THEN DO:
        CREATE DETALLE.
        BUFFER-COPY Almstkge TO DETALLE
            ASSIGN
                DETALLE.desmat = Almmmatg.desmat
                DETALLE.desmar = Almmmatg.desmar
                DETALLE.Valor  = (Almstkge.stkact * Almstkge.ctouni)
...
------------------------------------------------------------------------------*/
  DISPLAY DFecha DesdeC HastaC c-Categoria 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-57 DFecha DesdeC HastaC c-Categoria Btn_OK Btn_Cancel 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  DEFINE FRAME F-REPORTE
    ...
    DETALLE.StkAct COLUMN-LABEL "Stock"       FORMAT '->>>,>>>,>>9.99'
    DETALLE.Valor  COLUMN-LABEL "Costo Total" FORMAT '->>>,>>>,>>9.99'
    DETALLE.CatConta COLUMN-LABEL "Categoria"
  WITH WIDTH 132 NO-BOX STREAM-IO DOWN. 

  DEFINE FRAME F-HEADER
         HEADER
         {&Prn2} + {&Prn7A} + {&Prn6A} + S-NOMCIA + {&Prn7B} + {&Prn6A} + {&Prn3} FORMAT "X(50)" AT 1 SKIP
         "MATERIALES CON COSTO CERO O NEGATIVO" AT 30 
         "Pagina :" TO 85 PAGE-NUMBER(REPORT) FORMAT "ZZZZZ9" SKIP
         "Al " AT 37 DFecha FORMAT '99/99/9999' 
         "Fecha :" TO 85 TODAY FORMAT "99/99/9999" SKIP
         "Hora :"  TO 85 STRING(TIME,"HH:MM") SKIP(1)
    WITH PAGE-TOP WIDTH 132 NO-BOX NO-LABELS NO-UNDERLINE STREAM-IO DOWN. 

  FOR EACH DETALLE BY DETALLE.CatConta BY DETALLE.CodMat:
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH AlmTabla WHERE Tabla = 'CC':
    c-Categoria:ADD-LAST(Almtabla.codigo) IN FRAME {&FRAME-NAME}.
  END.
