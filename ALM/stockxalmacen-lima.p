...
for each almacen where almacen.codcia = s-codcia and
                   lookup(almacen.codalm,x-codalm) > 0:
                   
find last almsub where almsub.codcia = s-codcia and
                       almsub.codmat = entry(i,x-codmat) and
                       almsub.codalm = almacen.codalm and
                       almsub.fchdoc  < 01/01/2003
                       no-lock no-error.
  if available almsub and almsub.stksub <> 0 then do:    
     x-tot = x-tot + almsub.stksub.
    display 
     ...
     almsub.stksub  column-label "Stock"
