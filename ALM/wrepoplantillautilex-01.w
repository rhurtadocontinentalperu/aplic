...
T-MATE.StkMin T-MATE.StkMax Almmmatg.UndStk Almmmatg.DesMat Almmmatg.DesMar 
...
T-MATE-2.StkMin T-MATE-2.StkMax 
...
&Scoped-Define ENABLED-OBJECTS RECT-26 BROWSE-5 BUTTON-1 BUTTON-2 TOGGLE-1 ~
BROWSE-7 
&Scoped-Define DISPLAYED-OBJECTS TOGGLE-1 FILL-IN-Mensaje 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-5 W-Win _STRUCTURED
  QUERY BROWSE-5 NO-LOCK DISPLAY
      ...
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(20)":U WIDTH 12.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-7 W-Win _STRUCTURED
  QUERY BROWSE-7 NO-LOCK DISPLAY
      T-MATE-2.CodAlm COLUMN-LABEL "ALMACEN" FORMAT "x(3)":U WIDTH 9.43
      T-MATE-2.codmat COLUMN-LABEL "ARTICULO" FORMAT "X(6)":U WIDTH 13.43
      T-MATE-2.StkMin COLUMN-LABEL "MINIMO" FORMAT "Z,ZZZ,ZZ9.99":U
            WIDTH 13.43
      T-MATE-2.StkMax COLUMN-LABEL "EMPAQUE" FORMAT "Z,ZZZ,ZZZ,ZZ9.99":U
            WIDTH 10.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 52 BY 4.5
         FONT 4
         TITLE "  COLUM A | COLUMNA B   | COLUMNA C   | COLUMNA D |" FIT-LAST-COLUMN.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     BROWSE-5 AT ROW 1.19 COL 2 WIDGET-ID 200
     BUTTON-1 AT ROW 1.19 COL 110 WIDGET-ID 2
     BUTTON-2 AT ROW 2.35 COL 110 WIDGET-ID 4
     TOGGLE-1 AT ROW 3.88 COL 110 WIDGET-ID 38
     FILL-IN-Mensaje AT ROW 5.23 COL 108 COLON-ALIGNED NO-LABEL WIDGET-ID 44
     BROWSE-7 AT ROW 19.08 COL 7 WIDGET-ID 300
     "Poner en cero todos los mínimos" VIEW-AS TEXT
          SIZE 22 BY .5 AT ROW 3.69 COL 113 WIDGET-ID 40
     "de los almacenes involucrados" VIEW-AS TEXT
          SIZE 22 BY .5 AT ROW 4.27 COL 113 WIDGET-ID 42
     "FORMATO PLANTILLA EXCEL" VIEW-AS TEXT
          SIZE 23 BY .5 AT ROW 18.31 COL 4 WIDGET-ID 36
          BGCOLOR 1 FGCOLOR 15 
     RECT-26 AT ROW 18.31 COL 2 WIDGET-ID 34
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 134.86 BY 23.27
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-5 RECT-26 F-Main */
/* BROWSE-TAB BROWSE-7 FILL-IN-Mensaje F-Main */
...
"T-MATE.codmat" "Artículo" ? "character" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.T-MATE.StkMin
"T-MATE.StkMin" "Stock Minimo" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.T-MATE.StkMax
"T-MATE.StkMax" "Empaque" ? "decimal" ? ? ? ? ? ? no ? no no "10.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Almmmatg.DesMar" "Marca" "X(20)" "character" ? ? ? ? ? ? no ? no no "12.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-5 */
&ANALYZE-RESUME
...
"T-MATE-2.CodAlm" "ALMACEN" ? "character" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.T-MATE-2.codmat
"T-MATE-2.codmat" "ARTICULO" ? "character" ? ? ? ? ? ? no ? no no "13.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.T-MATE-2.StkMin
"T-MATE-2.StkMin" "MINIMO" ? "decimal" ? ? ? ? ? ? no ? no no "13.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.T-MATE-2.StkMax
"T-MATE-2.StkMax" "EMPAQUE" ? "decimal" ? ? ? ? ? ? no ? no no "10.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-7 */
&ANALYZE-RESUME
...
IF cValue = "" OR cValue = ? OR LOOKUP(cValue, "ALMACEN,ALMACÉN,Almacén") = 0 THEN DO:
    MESSAGE '1 Formato del archivo Excel errado' cValue VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.
t-column = t-column + 1.
cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
IF cValue = "" OR cValue = ? OR LOOKUP(cValue, "ARTICULO,ARTÍCULO,Artículo") = 0 THEN DO:
    MESSAGE '2 Formato del archivo Excel errado' cValue VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.
t-column = t-column + 1.
cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
IF cValue = "" OR cValue = ? OR LOOKUP(cValue, "MINIMO,MÍNIMO,Mínimo") = 0 THEN DO:
    MESSAGE '3 Formato del archivo Excel errado' cValue VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.
...
REPEAT:
    ...
    END.
/*     ASSIGN                                                         */
/*         cValue = STRING(INTEGER(cValue), '99') NO-ERROR.           */
...
/*     IF cValue = "" OR cValue = ? THEN LEAVE.    /* FIN DE DATOS */ */
    ...
    ASSIGN
        T-MATE.StkMin = DECIMAL(cValue) NO-ERROR.
    /* STKMAX */
    t-Column = t-Column + 1.
    ASSIGN
        cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE NO-ERROR.
    ...
    IF T-MATE.StkMax = 0 OR T-MATE.StkMax = ? THEN T-MATE.StkMax = 1.
END.
/* DEPURAMOS EL EXCEL */
FOR EACH T-MATE WHERE T-MATE.StkMin < 0:
    DELETE T-MATE.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY TOGGLE-1 FILL-IN-Mensaje 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-26 BROWSE-5 BUTTON-1 BUTTON-2 TOGGLE-1 BROWSE-7 
      WITH FRAME F-Main IN WINDOW W-Win.
...
FOR EACH T-MATE BREAK BY T-MATE.CodAlm ON STOP UNDO, NEXT:
    FIND Almmmate WHERE Almmmate.codcia = T-MATE.codcia
    ...
    END.
    /* 1ro borramos todo */
    IF FIRST-OF(T-MATE.CodAlm) AND TOGGLE-1 = YES THEN DO:
        FOR EACH B-MATE WHERE B-MATE.codcia = T-MATE.codcia
            AND B-MATE.codalm = T-MATE.codalm 
            AND (B-MATE.StkMin <> 0 /*OR B-MATE.StkMax <> 0*/):
            ASSIGN
                B-MATE.StkMin = 0
                /*B-MATE.StkMax = 0*/.
        END.
