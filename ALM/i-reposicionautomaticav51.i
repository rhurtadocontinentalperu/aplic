...
PROCEDURE ARTICULOS-A-SOLICITAR PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF pTipoCalculo = "AUT" THEN DO:
    IF (B-MATE.StkMin + B-MATE.VCtMn1 + B-MATE.VCtMn2) = 0 THEN RETURN 'OK'.
END.
/* ************************ */
...
CASE pOk:
    WHEN YES THEN DO:   /* ES UN CD */
        FIND FIRST TabGener WHERE TabGener.CodCia = s-codcia
            ...
            NO-LOCK NO-ERROR.
        IF AVAILABLE TabGener THEN pCodigo = TabGener.Codigo.  /* Zona Geografica que nos interesa Ej. ATE */
        ...
        FOR EACH TabGener NO-LOCK WHERE TabGener.CodCia = s-CodCia
            ...
            FIND FIRST T-MATE WHERE T-MATE.CodCia = s-CodCia AND
                T-MATE.CodMat = pCodMat EXCLUSIVE-LOCK NO-ERROR.
        ...
        FIND FIRST T-MATE WHERE T-MATE.codcia = s-codcia
            ...
            AND T-MATE.codmat = pCodMat
            EXCLUSIVE-LOCK NO-ERROR.
        IF AVAILABLE B-MATE AND AVAILABLE T-MATE THEN DO:
        ...
        END.
        IF AVAILABLE(T-MATE) THEN RELEASE T-MATE.
    END.
...
PROCEDURE DATOS-FINALES PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE B-MATE THEN DO:
        ...
        IF AVAILABLE Almtubic THEN
            ASSIGN
            T-DREPO.CodUbi = TRIM(Almtubic.CodUbi)
            T-DREPO.CodZona = TRIM(Almtubic.CodZona).
    END.
    /* Clasificacion */
...
PROCEDURE RESUMEN-POR-DESPACHO PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
CASE TRUE:
    WHEN pOkSolicitante = YES AND pOk = YES THEN DO:   /* ES UN CD */
        FIND FIRST TabGener WHERE TabGener.CodCia = s-codcia
            ...
            AND TabGener.Libre_l01 = YES
            NO-LOCK NO-ERROR.
        IF AVAILABLE TabGener THEN pCodigo = TabGener.Codigo.  /* Zona Geografica que nos interesa */
        lReservaMaximo = YES.
        FOR EACH TabGener NO-LOCK WHERE TabGener.CodCia = s-CodCia
            AND TabGener.Clave = "ZG"
            AND TabGener.Codigo = pCodigo,
        ...
        FIND FIRST T-MATE-2 OF B-MATE EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
        IF AVAILABLE T-MATE-2 THEN DO:
            ...
            END.
        END.
        IF AVAILABLE(T-MATE-2) THEN RELEASE T-MATE-2.
    END.
...
------------------------------------------------------------------------------*/
    ...
    DEF VAR pCmpTra AS DECI INIT 0 NO-UNDO.

    FOR EACH OOComPend NO-LOCK WHERE OOComPend.CodAlm = pCodAlm
        AND OOComPend.CodMat = pCodMat:
        pCmpTra = pCmpTra + (OOComPend.CanPed - OOComPend.CanAte).
    END.
    RETURN pCmpTra.
