...
&Scoped-Define ENABLED-OBJECTS txt-date txt-almvirtual cb-almacen ~
x-almcissac Btn_OK btn-ok-2 b-busca rs-inv RECT-57 
&Scoped-Define DISPLAYED-OBJECTS txt-date txt-almvirtual cb-almacen ~
x-almcissac rs-inv x-mensaje 
...
DEFINE BUTTON Btn_OK 
     ...
     LABEL "Aceptar" 
     SIZE 9 BY 1.5 TOOLTIP "Procesar"
     BGCOLOR 8 .
...
DEFINE FRAME F-Main
     txt-date AT ROW 1.46 COL 10 COLON-ALIGNED WIDGET-ID 26
     txt-almvirtual AT ROW 1.58 COL 44 COLON-ALIGNED WIDGET-ID 48
     cb-almacen AT ROW 2.88 COL 22 COLON-ALIGNED NO-LABEL WIDGET-ID 8
     x-almcissac AT ROW 3.96 COL 22 COLON-ALIGNED NO-LABEL WIDGET-ID 46
     Btn_OK AT ROW 3.15 COL 74
     btn-ok-2 AT ROW 3.15 COL 84 WIDGET-ID 36
     b-busca AT ROW 2.88 COL 68 WIDGET-ID 28
     rs-inv AT ROW 5.58 COL 4 NO-LABEL WIDGET-ID 30
     x-mensaje AT ROW 5.58 COL 29 COLON-ALIGNED NO-LABEL WIDGET-ID 34
     "Almacenes Standford" VIEW-AS TEXT
          SIZE 19 BY .5 AT ROW 4.04 COL 4 WIDGET-ID 42
          FONT 6
     "Almacenes Continental" VIEW-AS TEXT
          SIZE 19 BY .5 AT ROW 2.96 COL 4 WIDGET-ID 38
          FONT 6
...
DO:
    ...
    ASSIGN txt-date rs-inv cb-almacen x-almcissac txt-almvirtual.
    cAlmacen = cb-almacen .
    
    /* Valido el Almacen Virtual */
    IF txt-almvirtual = ? OR txt-almvirtual = "" THEN DO:
        MESSAGE 'Ingrese el Almacen Virtual'
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        RETURN.
    END.
    FIND FIRST integral.almacen WHERE codcia =s-codcia AND codalm = txt-almvirtual NO-LOCK NO-ERROR.
    IF NOT AVAILABLE integral.almacen THEN DO:
        MESSAGE 'Codigo de Almacen Virtual NO EXISTE'
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        RETURN.
    END.

    FIND FIRST integral.almdinv WHERE almdinv.codcia = s-codcia AND almdinv.codalm = txt-almvirtual NO-LOCK NO-ERROR.
    IF AVAILABLE integral.almdinv THEN DO:
        MESSAGE
        "   Ya existen movimiento en el ALMACEN VIRTUAL."     SKIP
        "Si continua con el proceso se BORRARA todo lo anterior"  SKIP
        "      Esta seguro de Continuar?       "  
        VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
        UPDATE choice1 AS LOGICAL.
        IF choice1 = FALSE THEN RETURN.
    END.

    MESSAGE
    "   Este proceso se realiza solo una vez."     SKIP
    "Si la información ingresada es la correcta"  SKIP
    "      ingrese la clave y de Aceptar       "  
    VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       18Mar2013, comentario nomcia = 'CONTISTAND'
                          no eliminaba los ingresos manuales (pag 8000 y 9000)
------------------------------------------------------------------------------*/

    /*Borra Cabecera de integral.almdinv*/
    FOR EACH integral.almdinv WHERE integral.almdinv.CodCia = s-CodCia 
        AND integral.almdinv.CodAlm = txt-almvirtual /*"11x"        */
    ...
    FOR EACH integral.almcinv WHERE integral.almcinv.CodCia = s-CodCia 
        AND integral.almcinv.CodAlm = txt-almvirtual  /*"11x" */
...
------------------------------------------------------------------------------*/
    ...
    IF cAlmacen = ? THEN cAlmacen = "".
    
    /*Utilizar el Carga-Cissac-a*/
    /*RUN alm\p-carga-cissac.p (cAlmacen, txt-date, OUTPUT x-task-no). */

    DISPLAY "Procesando Cissac" @ x-mensaje WITH FRAME {&FRAME-NAME}.

    RUN alm\carga-cissac-a (cAlmacen, dFecha, OUTPUT x-task-no).                    
    /*RUN alm\carga-cissac-a (cAlmacen, OUTPUT x-task-no).                    */
    ...
    FOR EACH integral.w-report WHERE integral.w-report.task-no = x-task-no NO-LOCK:
        FIND FIRST tmp-tempo WHERE tmp-tempo.tt-codcia  = s-codcia 
            AND tmp-tempo.tt-almacen = txt-almvirtual   /*"11x"*/
        ...
        IF NOT AVAILABLE tmp-tempo THEN DO:
            ...
            ASSIGN
                ...
                tmp-tempo.tt-almacen = txt-almvirtual  /*"11x"*/
        ...
        DISPLAY "Cargando Temporal: " + tmp-tempo.tt-codmat  @ x-mensaje
...
------------------------------------------------------------------------------*/
    ...
    FIND integral.InvConfig WHERE integral.InvConfig.CodCia = s-CodCia
        AND integral.InvConfig.CodAlm = txt-almvirtual /*"11x"*/
    ...
    IF NOT AVAIL integral.InvConfig THEN DO:
        ...
        ASSIGN
            ...
            integral.InvConfig.CodAlm  =  txt-almvirtual /*"11x"*/
...
------------------------------------------------------------------------------*/
  DISPLAY txt-date txt-almvirtual cb-almacen x-almcissac rs-inv x-mensaje 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE txt-date txt-almvirtual cb-almacen x-almcissac Btn_OK btn-ok-2 b-busca 
         rs-inv RECT-57 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
    ...
    /*Comprueba si la informacion ha sido cargada para los almacenes ingresados CONTINENTAL */
    ...
    DO iInt = 1 TO NUM-ENTRIES(cAlm):
        FOR EACH integral.almcinv WHERE integral.almcinv.codcia = s-codcia
            ...
            FIND FIRST tmp-tempo WHERE tt-codcia  = s-codcia 
                AND tt-almacen = txt-almvirtual  /*"11x"*/
