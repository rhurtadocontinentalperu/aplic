...
Almmmate.StkMin Almmmate.StkMax Almmmate.StkRep 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table Almmmate.CodUbi 
...
&Scoped-Define ENABLED-OBJECTS F-CodMat F-Filtro R-Tipo RECT-18 br_table 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almmmate.desmat COLUMN-LABEL "Descripcion" FORMAT "x(48)":U
            WIDTH 40
      S-DesMar @ S-DesMar COLUMN-LABEL "Marca" FORMAT "X(15)":U
      Almmmatg.TpoArt FORMAT "X(1)":U
      Almmmate.StkAct COLUMN-LABEL "Stock!Actual" FORMAT "(>,>>,>>9.99<<<)":U
            COLUMN-FONT 4
      Almmmate.CodUbi COLUMN-LABEL "Zonas o!Ubicación" FORMAT "x(8)":U
      Almmmate.StkMin COLUMN-LABEL "Stock!Minimo" FORMAT "ZZZ,ZZ9.99":U
      Almmmate.StkMax COLUMN-LABEL "Stock!Maximo" FORMAT "ZZZ,ZZ9.99":U
      Almmmate.StkRep COLUMN-LABEL "Promedio!Venta Diaria" FORMAT "ZZZ,ZZ9.99":U
  ENABLE
      Almmmate.CodUbi
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     ...
     R-Tipo AT ROW 2.35 COL 13 NO-LABEL
     br_table AT ROW 3.81 COL 1.57
     "Estado:" VIEW-AS TEXT
          SIZE 6 BY .5 AT ROW 2.46 COL 5.43
          FONT 4
     "Zonificacion" VIEW-AS TEXT
          SIZE 12 BY .92 AT ROW 2.35 COL 62 WIDGET-ID 2
          FGCOLOR 4 FONT 6
...
"Almmmate.desmat" "Descripcion" "x(48)" "character" ? ? ? ? ? ? no ? no no "40" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Almmmate.StkAct" "Stock!Actual" "(>,>>,>>9.99<<<)" "decimal" ? ? 4 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > integral.Almmmate.CodUbi
"Almmmate.CodUbi" "Zonas o!Ubicación" "x(8)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > integral.Almmmate.StkMin
"Almmmate.StkMin" "Stock!Minimo" "ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > integral.Almmmate.StkMax
"Almmmate.StkMax" "Stock!Maximo" "ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > integral.Almmmate.StkRep
"Almmmate.StkRep" "Promedio!Venta Diaria" "ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&Scoped-define SELF-NAME Almmmate.CodUbi
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Almmmate.CodUbi br_table _BROWSE-COLUMN B-table-Win
ON LEAVE OF Almmmate.CodUbi IN BROWSE br_table /* Zonas o!Ubicación */
DO:
...
/*   IF INTEGER(SELF:SCREEN-VALUE) <> 0 THEN SELF:SCREEN-VALUE = STRING(INTEGER(SELF:SCREEN-VALUE),"999999"). */
  ASSIGN F-CodMat. 
...
ON "RETURN":U OF Almmmate.CodUbi
DO:
   APPLY "TAB":U.
   RETURN NO-APPLY.
END.
...
/*     END.                                                                         */
    ...
    /*ASSIGN Almmmate.CodUbi = Almmmate.CodUbi SELF:SCREEN-VALUE.*/
  ...
  APPLY 'ENTRY' TO f-codmat IN FRAME {&FRAME-NAME}.
...
/* Consistencia de ubicaciones */
IF Almmmate.CodUbi:SCREEN-VALUE IN BROWSE {&browse-name} > '' THEN DO:
    FIND Almtubic WHERE Almtubic.codcia = s-codcia
        AND Almtubic.codalm = s-codalm
        AND Almtubic.codubi = Almmmate.CodUbi:SCREEN-VALUE IN BROWSE {&browse-name}
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almtubic THEN DO:
        MESSAGE 'Código de ubicación NO registrado' VIEW-AS ALERT-BOX ERROR.
        Almmmate.CodUbi:SCREEN-VALUE IN BROWSE {&browse-name} = Almmmate.CodUbi.
        APPLY 'ENTRY' TO Almmmate.CodUbi IN BROWSE {&browse-name}.
        RETURN 'ADM-ERROR'.
    END.
END.
