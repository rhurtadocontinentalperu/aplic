...
BUTTON-Generar 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN_NroDoc FILL-IN_AlmOri ~
FILL-IN_ChkAlmOri FILL-IN_NomChkAlmOri FILL-IN_FchChkAlmOri ~
FILL-IN_HorChkAlmOri FILL-IN_AlmDes FILL-IN_ChkAlmDes FILL-IN_NomChkAlmDes ~
FILL-IN_FchChkAlmDes FILL-IN_HorChkAlmDes FILL-IN_CrossDocking ~
FILL-IN_ResponAlmacen FILL-IN_NomResponAlmacen COMBO-BOX-ResponDistribucion 
...
DEFINE VARIABLE FILL-IN_AlmDes AS CHARACTER FORMAT "x(8)" 
     VIEW-AS FILL-IN 
     SIZE 7.14 BY .81.

DEFINE VARIABLE FILL-IN_AlmOri AS CHARACTER FORMAT "x(8)" 
     VIEW-AS FILL-IN 
     SIZE 7.14 BY .81.
...
DEFINE FRAME F-Main
     ...
     FILL-IN_AlmOri AT ROW 2.35 COL 1 COLON-ALIGNED NO-LABEL WIDGET-ID 90
     ...
     FILL-IN_AlmDes AT ROW 3.42 COL 1 COLON-ALIGNED NO-LABEL WIDGET-ID 88
...
/* SETTINGS FOR FILL-IN FILL-IN_AlmDes IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FILL-IN_AlmOri IN FRAME F-Main
   NO-ENABLE                                                            */
...
DO:
  ...
  FIND FIRST Almcmov WHERE Almcmov.codcia = s-codcia AND
      ...
      Almcmov.FlgSit = "R" NO-LOCK NO-ERROR.
  ...
  FIND TabGener WHERE TabGener.CodCia = s-CodCia AND
      ...
      NO-LOCK NO-ERROR.
  IF AVAILABLE TabGener AND TabGener.Libre_d01 > 0 THEN DO:
      IF (TODAY - Almcmov.FchDoc) > TabGener.Libre_d01 THEN DO:
          MESSAGE 'Se excedió de su límite de días para registrar la incidencia'  SKIP(2)
              'Comuníquese con operaciones' VIEW-AS ALERT-BOX ERROR.
          FILL-IN_NroDoc:SCREEN-VALUE = ''.
          APPLY 'ENTRY':U TO FILL-IN_NroDoc.
          RETURN NO-APPLY.
      END.
  ...
  APPLY 'LEAVE':U TO FILL-IN_ChkAlmDes.
  ...
  RLOOP:
  FOR EACH Almcmov NO-LOCK WHERE Almcmov.codcia = s-codcia AND
      Almcmov.CodRef = x-CodRef AND
      Almcmov.NroRef = x-NroRef AND
      Almcmov.TipMov = "S" AND
      Almcmov.FlgEst <> 'A' AND 
      Almcmov.FlgSit = "R" WITH FRAME {&FRAME-NAME}:
...
------------------------------------------------------------------------------*/
  ...
  {&OPEN-QUERY-F-Main}
  GET FIRST F-Main.
  DISPLAY FILL-IN_NroDoc FILL-IN_AlmOri FILL-IN_ChkAlmOri FILL-IN_NomChkAlmOri 
          FILL-IN_FchChkAlmOri FILL-IN_HorChkAlmOri FILL-IN_AlmDes 
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    IF NOT AVAILABLE Almacen THEN DO:
        pMensaje = "Almacén ORIGEN no válido".
        UNDO, RETURN 'ADM-ERROR'.
    END.
    ASSIGN
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        IF TRUE <> (pMensaje > '') THEN pMensaje = 'NO se pudieron ganarar las R/As'.
        UNDO, RETURN 'ADM-ERROR'.
    END.
...
/*                                                                                                 */
/*     FOR EACH AlmDIncidencia OF AlmCIncidencia NO-LOCK BREAK BY AlmDIncidencia.Incidencia:       */
/*         IF FIRST-OF(AlmDIncidencia.Incidencia) THEN DO:                                         */
/*             pIncidencia = AlmDIncidencia.Incidencia.                                            */
/*             CASE pIncidencia:                                                                   */
/*                 WHEN 'S' THEN DO:   /* Sobrante */                                              */
/*                     pCodAlm = AlmCIncidencia.AlmDes.                                            */
/*                     pAlmPed = AlmCIncidencia.AlmOri.                                            */
/*                     FIND Almacen WHERE Almacen.codcia = s-CodCia                                */
...
/*                     pCodAlm = AlmCIncidencia.AlmOri.                                            */
/*                     pAlmPed = AlmCIncidencia.AlmDes.                                            */
/*                     FIND Almacen WHERE Almacen.codcia = s-CodCia                                */
/*                         AND Almacen.CodAlm = pCodAlm                                            */
/*                         NO-LOCK.                                                                */
/*                     pCodDiv = Almacen.CodDiv.                                                   */
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    pMensaje = "Correlativo para " + s-coddoc + " en uso por otro usuario".
    {lib/lock-genericov3.i ~
        &Tabla="FacCorre" ~
        &Alcance="FIRST" ~
    ...
    pMensaje = ''.
    /* Una incidencia y varios R/A */
    RUN Genera-Incidencia.
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
