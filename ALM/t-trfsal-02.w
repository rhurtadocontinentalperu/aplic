...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ITEM.codmat COLUMN-LABEL "<Codigo>" FORMAT "X(14)":U WIDTH 6.43
      Almmmatg.DesMat FORMAT "X(60)":U WIDTH 42
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(20)":U
      Almmmatg.UndBas COLUMN-LABEL "Unidad" FORMAT "X(7)":U WIDTH 8.57
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 88 BY 9.96
         FONT 4.
...
"ITEM.codmat" "<Codigo>" "X(14)" "character" ? ? ? ? ? ? yes ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almmmatg.DesMat
"Almmmatg.DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ? no no "42" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.Almmmatg.DesMar
"Almmmatg.DesMar" "Marca" "X(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.Almmmatg.UndBas
"Almmmatg.UndBas" "Unidad" "X(7)" "character" ? ? ? ? ? ? no ? no no "8.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON 'return':U OF ITEM.codmat, ITEM.candes
DO:
    APPLY 'tab':U.
    RETURN NO-APPLY.
END.
...
/* Limpieza final */
FOR EACH ITEM, FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND
    Almmmatg.codmat = ITEM.codmat:
    IF ITEM.CanDes <= 0 OR ITEM.CanDes = ? THEN DO:
        DELETE ITEM.
        NEXT.
    END.
    ...
    IF AVAILABLE Unidades AND Unidades.Libre_l02 = YES THEN DO:
        IF ITEM.CanDes <> INTEGER(ITEM.CanDes) THEN DO:
            DELETE ITEM.
            NEXT.
        END.
    END.
...
/*
    Ic 11Set2019
    Pedido de Jose Nieto y autorizado por Daniel Llican, por configuracion de Almacen
    NO controlar STOCK
*/
