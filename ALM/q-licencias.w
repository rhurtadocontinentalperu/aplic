...
&Scoped-define INTERNAL-TABLES almtblas

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for QUERY Query-Main                                     */
&Scoped-define QUERY-STRING-Query-Main FOR EACH almtblas WHERE ~{&KEY-PHRASE} ~
      AND almtblas.Tabla = "LC" NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH almtblas WHERE ~{&KEY-PHRASE} ~
      AND almtblas.Tabla = "LC" NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main almtblas
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main almtblas
...
DEFINE VARIABLE F-Tabla AS CHARACTER FORMAT "X(2)":U 
     VIEW-AS FILL-IN 
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "integral.almtblas"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "integral.almtblas.Tabla = ""LC"""
     _Design-Parent    is WINDOW q-tables @ ( 1.04 , 8 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND almtblas WHERE almtblas.Tabla = F-Tabla NO-LOCK NO-ERROR.
IF AVAILABLE almtblas THEN output-var-1 = ROWID(almtblas).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
