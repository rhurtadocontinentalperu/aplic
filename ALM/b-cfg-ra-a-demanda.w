...
&Scoped-define QUERY-STRING-br_table FOR EACH FacTabla WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacTabla.Tabla = s-tabla NO-LOCK, ~
      FIRST Almtfami WHERE Almtfami.CodCia = FacTabla.CodCia ~
  AND Almtfami.codfam = FacTabla.Codigo OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
/* Query rebuild information for BROWSE br_table
 ...
 AND FacTabla.Tabla = s-tabla"
     _JoinCode[2]      = "Almtfami.CodCia = FacTabla.CodCia
  AND Almtfami.codfam = FacTabla.Codigo"
     _FldNameList[1]   > INTEGRAL.FacTabla.Codigo
"FacTabla.Codigo" "Línea" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
