...
DEFINE FRAME F-Main
     x-codalm AT ROW 2.88 COL 12 COLON-ALIGNED WIDGET-ID 30
     BUTTON-1 AT ROW 2.88 COL 61.86 WIDGET-ID 32
     x-FchInv AT ROW 4.77 COL 27 COLON-ALIGNED WIDGET-ID 36
     x-mensaje AT ROW 7.62 COL 4.14 COLON-ALIGNED WIDGET-ID 4
...
ON END-ERROR OF W-Win /* STOCK FINAL VALORIZADO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* STOCK FINAL VALORIZADO */
DO:
...
------------------------------------------------------------------------------*/
  ...
  cCodAlm = x-codalm:SCREEN-VALUE IN FRAME {&FRAME-NAME}.

  FOR EACH AlmCInv WHERE AlmCInv.CodCia = s-codcia
      AND LOOKUP(AlmCInv.CodAlm,cCodAlm) > 0
      ...
      FOR EACH AlmDInv OF AlmCInv NO-LOCK,
          ...
          ASSIGN 
              ...
              T-Ajuste.CodAlm = AlmDInv.CodAlm
            ...
            DISPLAY "PROCESANDO: " + Almmmatg.CodMat + "-" + 
                Almmmatg.DesMat @ x-mensaje WITH  FRAME {&FRAME-NAME}.
      END.
  END.
...
/*
  ...
  FIND InvConfig WHERE InvConfig.CodCia = s-codcia
    AND InvConfig.CodAlm = s-codalm
    AND InvConfig.FchInv = x-FchInv
    NO-LOCK NO-ERROR.
  
  FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.CodCia = s-CodCia USE-INDEX Matg01,
      FIRST Almmmate OF Almmmatg NO-LOCK WHERE Almmmate.codalm = Invconfig.CodAlm
      AND Almmmate.codubi BEGINS FILL-IN-CodUbi:
      ...
      X-CanInv = 0.
      X-StkSub = 0.  
      FIND AlmDInv WHERE AlmDInv.CodCia = s-CodCia
          AND AlmDInv.CodAlm = s-CodAlm
          AND AlmDInv.NomCia = "CONTI"
          AND AlmDInv.CodMat = Almmmatg.CodMat NO-LOCK NO-ERROR.
      ...
      */
      ...
      ASSIGN 
          ...
          T-Ajuste.CodUbi = Almmmate.CodUbi
          T-Ajuste.StkAct = x-StkSub
          T-Ajuste.StkSub = x-CanInv
          /*T-Ajuste.Dife   = X-Dife*/ .
        FIND LAST AlmStkGe WHERE AlmStkGe.codcia = s-codcia
...
DEF VAR x-CtoVta AS DEC NO-UNDO.
DEF VAR x-totfam AS DEC NO-UNDO.
DEF VAR x-totalm AS DEC NO-UNDO.
...
cRange = "C" + '2'.
chWorkSheet:Range(cRange):Value = "STOCK FINAL VALORIZADO".
...
/* set the column names for the Worksheet */
 ...
 cRange = "G" + cColumn.
 chWorkSheet:Range(cRange):Value = "Stock Fisico    ".
 cRange = "H" + cColumn.
...
FOR EACH T-Ajuste, FIRST Almtfami NO-LOCK WHERE Almtfami.codcia = s-codcia
    ...
    /*
    x-CtoPos = T-Ajuste.StkSub * T-Ajuste.CtoUni.
    */
    x-CtoPos = T-Ajuste.StkAct * T-Ajuste.CtoUni.
    x-CtoVta = T-Ajuste.StkAct * T-Ajuste.CtoUni.

    IF x-CtoPos <> ? THEN DO:
        x-totfam = x-totfam + x-CtoPos.
        x-totalm = x-CtoVta + x-totalm.
        x-total  = x-total  + x-CtoVta.
    END.    
    ...
    IF LAST-OF(T-Ajuste.CodFam) THEN DO:
        ...
        cRange = "I" + cColumn.
        chWorkSheet:Range(cRange):Value = x-totfam.
    END.
    IF LAST-OF(T-Ajuste.CodAlm) THEN DO:
        ...
        cRange = "I" + cColumn.
        chWorkSheet:Range(cRange):Value = x-totalm.
    END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-DETALLE
    T-Ajuste.CodAlm     COLUMN-LABEL "Almacen"          
    ...
    T-Ajuste.stksub     COLUMN-LABEL "Stock Fisico"     FORMAT '->>>>,>>9.99'
/*
...
*/    
  ...
  FOR EACH T-Ajuste, FIRST Almtfami NO-LOCK WHERE Almtfami.codcia = s-codcia
        AND Almtfami.codfam = T-Ajuste.codfam
        BREAK BY T-Ajuste.codcia BY T-Ajuste.codalm 
            BY T-Ajuste.CodFam BY T-Ajuste.desmat:
    ...
    */
    ...
    IF LAST-OF(T-Ajuste.CodFam) THEN DO:
        ...
        DISPLAY STREAM REPORT
            "SUBTOTAL FAM: " + T-Ajuste.CodFam      @ T-Ajuste.CodUbi  
    ...
    IF LAST-OF(T-Ajuste.CodAlm) THEN DO:
        UNDERLINE STREAM REPORT x-CtoPos WITH FRAME F-DETALLE.
        DISPLAY STREAM REPORT
            "SUBTOTAL ALM: " + T-Ajuste.CodAlm      @ T-Ajuste.CodUbi  
