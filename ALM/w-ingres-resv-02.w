...
DEFINE VARIABLE h_t-ingres-resv-02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-ingres-resv-02 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn-Pendientes AT ROW 6 COL 73
...
ON END-ERROR OF W-Win /* Ingreso de Productos Terminados */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Ingreso de Productos Terminados */
DO:
...
DO:
  RUN Asigna-Documento IN h_v-ingres-resv-02.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-ingres-resv-02.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ingres-resv-02 ).
       RUN set-position IN h_v-ingres-resv-02 ( 1.19 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.62 , 92.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 6.00 , 13.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv10 ).
       RUN set-position IN h_p-updv10 ( 6.00 , 30.00 ) NO-ERROR.
       RUN set-size IN h_p-updv10 ( 1.35 , 41.72 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'pro/q-ingpro.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-devfac ).
       RUN set-position IN h_q-devfac ( 6.00 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 8.29 ) */

       /* Links to SmartViewer h_v-ingres-resv-02. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-ingres-resv-02 ).
       RUN add-link IN adm-broker-hdl ( h_q-devfac , 'Record':U , h_v-ingres-resv-02 ).

       /* Links to SmartQuery h_q-devfac. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-devfac ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ingres-resv-02 ,
             Btn-Pendientes:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-ingres-resv-02 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv10 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-devfac ,
             Btn-Pendientes:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-devolu ).
       RUN set-position IN h_b-devolu ( 7.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-devolu ( 7.88 , 85.29 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Layout = ,
                     Create-On-Add = ?':U ,
             OUTPUT h_t-ingres-resv-02 ).
       RUN set-position IN h_t-ingres-resv-02 ( 7.54 , 4.00 ) NO-ERROR.
       RUN set-size IN h_t-ingres-resv-02 ( 8.08 , 75.14 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv95 ).
       RUN set-position IN h_p-updv95 ( 9.08 , 81.00 ) NO-ERROR.
       RUN set-size IN h_p-updv95 ( 3.65 , 10.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-ingres-resv-02. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_t-ingres-resv-02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-ingres-resv-02 ,
             Btn-Pendientes:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv95 ,
             h_t-ingres-resv-02 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
         ...
         RUN dispatch IN h_t-ingres-resv-02 ('open-query':U).
      END.
