...
&SCOPED-DEFINE CONDICION ( (cb-auxi.CodCia = cb-CodCia OR cb-auxi.CodCia = S-CODCIA) AND cb-auxi.Clfaux = input-var-1 )
&SCOPED-DEFINE CODIGO cb-auxi.Codaux

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( cb-auxi.Nomaux BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( cb-auxi.Nomaux , FILL-IN-filtro ) <> 0 )
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH cb-auxi WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table cb-auxi
...
<FOREIGN-KEYS>
Nombres que inicien con|y||integral.cb-auxi.Nomaux
Nombres que contengan|y||integral.cb-auxi.Nomaux
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.cb-auxi.CodCia|yes,INTEGRAL.cb-auxi.Clfaux|yes,INTEGRAL.cb-auxi.Libre_c01|yes
Descripción|||integral.cb-auxi.Nomaux|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      cb-auxi.Codaux COLUMN-LABEL "<<Codigo>>" FORMAT "x(8)":U
      cb-auxi.Nomaux COLUMN-LABEL "Descripción" FORMAT "x(60)":U
      cb-auxi.Libre_c01 COLUMN-LABEL "Equivalencia" FORMAT "x(10)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 77.14 BY 16
...
DEFINE FRAME F-Main
     FILL-IN-codigo AT ROW 1.19 COL 35.72 COLON-ALIGNED
     CMB-filtro AT ROW 1.15 COL 1.57 NO-LABEL
     FILL-IN-filtro AT ROW 2.12 COL 2 NO-LABEL
...
"Codaux" "<<Codigo>>" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.cb-auxi.Nomaux
"Nomaux" "Descripción" "x(60)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.cb-auxi.Libre_c01
"Libre_c01" "Equivalencia" "x(10)" "character" ? ? ? ? ? ? no ? no no "4.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY cb-auxi.CodCia BY cb-auxi.Clfaux BY cb-auxi.Libre_c01
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY cb-auxi.Nomaux
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    DO k = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS:
        ...
        ASSIGN
            ...
            output-var-2 = output-var-2 + (IF output-var-2 = '' THEN '' ELSE ',') +
            {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.Codaux
            output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.Nomaux.
    END.
END.
