...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fMovSal B-table-Win 
FUNCTION fMovSal RETURNS LOGICAL
...
------------------------------------------------------------------------------*/
  ...
  /*Verifica si existe salidas en otros almacens*/
  IF fMovSal(CntDocum.CodDoc:SCREEN-VALUE IN BROWSE {&BROWSE-name}, CntDocum.NroDoc:SCREEN-VALUE IN BROWSE {&BROWSE-name}) = FALSE 
      THEN DO:
      MESSAGE '¡¡NO PUEDE GENERAR INGRESO PARA ESTE DOCUMENTO¡¡' SKIP
              '    Documento no registra Salidas o ya fue ingresado'
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fMovSal B-table-Win 
FUNCTION fMovSal RETURNS LOGICAL
  ( INPUT cCodDoc AS CHAR,
...
------------------------------------------------------------------------------*/
  FIND LAST CntDocum WHERE CntDocum.CodCia = s-codcia
      ...
      AND CntDocum.NroDoc  = cNroDoc NO-LOCK NO-ERROR.
  IF AVAIL CntDocum AND CntDocum.TipMov  = 'S' THEN RETURN TRUE.
  ELSE RETURN FALSE.
