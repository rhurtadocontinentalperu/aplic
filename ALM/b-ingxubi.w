...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 85 BY 17.23
         FONT 4
...
DO:
   ...
   FIND FIRST b-dinv WHERE b-dinv.CodCia = s-codcia
       AND b-dinv.CodAlm = s-codalm
       AND b-dinv.NomCia = "CONTI"
       /*AND AlmDInv.CodUbi = tmp-AlmDInv.CodUbi:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}*/
       AND b-dinv.CodMat = SELF:SCREEN-VALUE NO-LOCK NO-ERROR.
   IF AVAILABLE b-dinv THEN DO:
       MESSAGE "Articulo ya registrado en página: " b-dinv.NroPagina
...
------------------------------------------------------------------------------*/
  ...
  /* Si ya existe la pagina 8000, ubico cual es el ultimo correlativo y lo incremento en 1 */
  ...
  FIND FIRST almcinv WHERE almcinv.codcia = s-codcia
      ...
      AND almcinv.nomcia = 'conti' EXCLUSIVE-LOCK NO-ERROR.
  IF NOT AVAIL almcinv THEN DO:
      CREATE almcinv.
      ASSIGN 
          AlmCInv.Codcia     = s-codcia
          AlmCInv.CodAlm     = s-codalm
