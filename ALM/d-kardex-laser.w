...
DEFINE TEMP-TABLE Detalle NO-UNDO LIKE w-report.
...
DEFINE FRAME F-Main
     ...
     Btn_OK AT ROW 10.69 COL 57
     Btn_Cancel AT ROW 10.69 COL 68.43
     "Moneda:" VIEW-AS TEXT
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: Detalle T "?" NO-UNDO INTEGRAL w-report
   END-TABLES.
 */
...
------------------------------------------------------------------------------*/
    ...
    DEFINE OUTPUT PARAMETER s-Task-No AS INT.
    s-Task-No = 0.
    ...
    DEFINE VARIABLE iOrden AS INT NO-UNDO.
...
/*         ACCUMULATE F-Salida (TOTAL BY Almmmatg.CodMat).  */
        ...
        ASSIGN
            ...
            w-report.Llave-F    = iOrden
            w-report.Llave-C    = Almmmatg.CodMat
            w-report.Campo-D[1] = Almdmov.FchDoc
            w-report.Campo-C[1] = cTpoDoc
            w-report.Campo-I[1] = iSerDoc
            w-report.Campo-C[2] = cNroDoc
            w-report.Campo-C[3] = cTpoOpe
            w-report.Campo-F[1] = F-Ingreso
            w-report.Campo-F[2] = F-PreIng
            w-report.Campo-F[3] = dTotIng
            w-report.Campo-F[4] = F-Salida
            w-report.Campo-F[5] = F-PRECIO
            w-report.Campo-F[6] = dTotSal
            w-report.Campo-F[7] = F-SALDO
            w-report.Campo-F[8] = f-CtoUni
            w-report.Campo-F[9] = F-VALCTO
            w-report.Campo-C[10] = x-Tipo
            .
...
RUN Carga-Temporal (OUTPUT s-Task-No).
SESSION:SET-WAIT-STATE('').
IF s-Task-No = 0 THEN RETURN.

GET-KEY-VALUE SECTION "Startup" KEY "BASE" VALUE RB-REPORT-LIBRARY.
RB-REPORT-LIBRARY = RB-REPORT-LIBRARY + "/alm/rbalm.prl".
RB-REPORT-NAME = "Kardex Laser".
RB-INCLUDE-RECORDS = "O".
RB-FILTER = "w-report.task-no = " + STRING(s-task-no).
RB-OTHER-PARAMETERS = "s-moneda = " + (IF nCodmon = 1 THEN "EN NUEVOS SOLES" ELSE "EN DOLARES").
RB-OTHER-PARAMETERS = RB-OTHER-PARAMETERS + 
                        "~ns-nomcia = " + s-nomcia.
RB-OTHER-PARAMETERS = RB-OTHER-PARAMETERS + 
                        "~ns-periodo = " + STRING(YEAR(DesdeF)).
