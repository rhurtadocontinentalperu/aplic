...
x-NroItem FILL-IN-tiempo FILL-IN_Observ 
...
DEFINE FRAME F-Main
     BUTTON-13 AT ROW 1.27 COL 69 WIDGET-ID 24
     BUTTON-15 AT ROW 1.27 COL 82 WIDGET-ID 26
     ...
     x-NroItem AT ROW 2.62 COL 88 COLON-ALIGNED WIDGET-ID 38
...
OR RETURN OF {&SELF-NAME} DO:
    ...
    FOR EACH B-CMOV NO-LOCK WHERE B-CMOV.codcia = Faccpedi.codcia AND
        ...
        IF B-CMOV.FlgSit <> "T" THEN DO:
            MESSAGE 'La G/R' B-CMOV.NroSer '-' B-CMOV.NroDoc 'NO está pendiente de recepcionar'
...
OCXFile = SEARCH( "w-chk-ingtra-otr-man.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-chk-ingtra-otr-man.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  DISPLAY x-FchDoc x-NroDoc FILL-IN-Almacen x-NroItem FILL-IN-tiempo 
          FILL-IN_Observ 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-65 BUTTON-13 BUTTON-15 x-NroDoc FILL-IN_Observ 
      WITH FRAME F-Main IN WINDOW W-Win.
