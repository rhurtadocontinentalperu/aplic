...
&Scoped-define QUERY-STRING-Query-Main FOR EACH almtmatg WHERE ~{&KEY-PHRASE} ~
      AND almtmatg.CodCia = s-codcia NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH almtmatg WHERE ~{&KEY-PHRASE} ~
      AND almtmatg.CodCia = s-codcia NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main almtmatg
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main almtmatg


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS BUTTON-1 FILL-IN-CodMat 
...
DEFINE FRAME FRAME-A
     BUTTON-1 AT ROW 1.12 COL 9
     FILL-IN-CodMat AT ROW 1.19 COL 1 NO-LABEL
...
/* SETTINGS FOR FILL-IN FILL-IN-CodMat IN FRAME FRAME-A
   ALIGN-L                                                              */
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Design-Parent    is WINDOW q-tables @ ( 1.08 , 13.14 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-CodMat q-tables
ON LEAVE OF FILL-IN-CodMat IN FRAME FRAME-A
DO:
  IF FILL-IN-CodMat:SCREEN-VALUE = "" THEN RETURN.
  ASSIGN FILL-IN-CodMat.
  FILL-IN-CodMat = STRING(INTEGER(FILL-IN-CodMat),"999999").
  RUN Ubica-Registro.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN lkup\c-catart-p ("Catalogo de Articulos").
...
------------------------------------------------------------------------------*/
  ...
  FIND MATG WHERE MATG.CodCia = S-CODCIA AND
                  MATG.CodMat = FILL-IN-CodMat NO-LOCK NO-ERROR.
