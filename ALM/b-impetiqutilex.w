...
&SCOPED-DEFINE precio-venta-mayorista-contado web/PrecioFinalContadoMayorista.p
...
DEFINE FRAME F-Main
     ...
     TOGGLE-nuevoformato AT ROW 2.92 COL 35 WIDGET-ID 8
...
------------------------------------------------------------------------------*/
    ...
    IF gn-divi.CanalVenta = "MIN" OR gn-divi.CanalVenta = "TDA" THEN DO:
        ...
        IF toggle-nuevoformato = YES THEN DO:
            ...
            IF dDstoVo1 <= 0 THEN DO:
                FOR EACH VtaDctoProm NO-LOCK WHERE VtaDctoProm.CodCia = s-CodCia AND 
                    ...
                    dDstoVo1 = VtaDctoProm.Descuento. 
                    dDstoVo1 = MAXIMUM(dDstoVo1, x-Old-Descuento). 
                    x-Old-Descuento = dDstoVo1.
                END.
            END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Imprime-Etiqueta-Precio-mayorista B-table-Win 
PROCEDURE Imprime-Etiqueta-Precio-mayorista :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
    ...
    x-papel = YES.
    IF USERID("DICTDB") = "ADMIN" OR USERID("DICTDB") = "MASTER" THEN x-papel = NO.
