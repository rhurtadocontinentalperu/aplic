...
&Scoped-Define ENABLED-OBJECTS RECT-57 F-CodFam F-CodAlm F-AlmDes R-Tipo ~
Btn_OK F-DiaRep Btn_Cancel 
...
&Scoped-define SELF-NAME F-DiaRep
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-DiaRep W-Win
ON LEAVE OF F-DiaRep IN FRAME F-Main /* Dias de Reposicion */
DO:
   IF SELF:SCREEN-VALUE = "" THEN RETURN.
    DO I = 1 To NUM-ENTRIES(SELF:SCREEN-VALUE):
     FIND Almacen WHERE Almacen.CodCia = S-CODCIA AND  
            ...
            do ii = 1 to num-entries(f-codalm):
               ...
               if available almmmate and almmmate.stkact < almmmate.stkmin * x-factor then do:
                     ...
                     if not available repo then do:
                        ...
                        repo.candes = (almmmate.stkmin * x-factor) - almmmate.stkact
.

                     end.

                     x-candes = (almmmate.stkmin * x-factor) - almmmate.stkact .
...
------------------------------------------------------------------------------*/
  DISPLAY F-CodFam F-CodAlm F-AlmDes R-Tipo F-DiaRep 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-57 F-CodFam F-CodAlm F-AlmDes R-Tipo Btn_OK F-DiaRep Btn_Cancel 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-HEADER
         ...
         "Alm       Aritculo                                       UM  Cat.    Cantidad            Minimo  Actual ." SKIP
         "---------------------------------------------------------------------------------------------------------" SKIP
...
------------------------------------------------------------------------------*/
DO WITH FRAME {&FRAME-NAME}:
  ASSIGN F-CodFam F-Codalm F-Almdes R-Tipo F-Diarep.  
END.
