...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 Btn_Excel BtnDone x-CodDiv x-CodDoc ~
x-FmaPgo x-FchDoc-1 x-FchDoc-2 x-FlgEst 
&Scoped-Define DISPLAYED-OBJECTS x-CodDiv x-CodDoc x-FmaPgo x-FchDoc-1 ~
x-FchDoc-2 x-FlgEst f-Mensaje 
...
DEFINE VARIABLE x-CodDoc AS CHARACTER FORMAT "X(256)":U INITIAL "Todos" 
     LABEL "Comprobante" 
     VIEW-AS COMBO-BOX INNER-LINES 5
...
DEFINE FRAME fMain
     ...
     x-CodDoc AT ROW 2.62 COL 22 COLON-ALIGNED WIDGET-ID 4
     x-FmaPgo AT ROW 3.69 COL 22 COLON-ALIGNED WIDGET-ID 16
     x-FchDoc-1 AT ROW 4.77 COL 22 COLON-ALIGNED WIDGET-ID 8
     x-FchDoc-2 AT ROW 4.77 COL 42 COLON-ALIGNED WIDGET-ID 10
     x-FlgEst AT ROW 5.85 COL 24 NO-LABEL WIDGET-ID 24
...
DO:
  ASSIGN
    x-CodDiv x-CodDoc x-FchDoc-1 x-FchDoc-2 x-FmaPgo x-FlgEst.
   RUN Imprimir.
END.
...
DO WITH FRAME {&FRAME-NAME}:
  ASSIGN x-CodDiv x-CodDoc x-FmaPgo x-FchDoc-1 x-FchDoc-2 x-FlgEst.
END.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ccbcdocu NO-LOCK WHERE codcia = s-codcia
      AND coddiv = x-coddiv
      AND (x-coddoc = 'Todos' OR coddoc = x-coddoc)
      AND LOOKUP(coddoc, 'FAC,BOL') > 0
      AND fchdoc >= x-fchdoc-1
      AND fchdoc <= x-fchdoc-2
      AND flgest <> 'A':
      IF x-FlgEst <> '' THEN DO:
          IF Ccbcdocu.flgest <> x-flgest THEN NEXT COMPROBANTES.
      END.
      IF  x-fmapgo <> 'Todos' THEN DO: 
          IF fmapgo <> SUBSTRING(x-fmapgo,1,3) THEN NEXT COMPROBANTES.
      END.
      f-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'Procesando ' + Ccbcdocu.coddoc + ' ' + Ccbcdocu.nrodoc + ' ' + STRING(Ccbcdocu.fchdoc).
      FIND FIRST di-rutad WHERE di-rutad.codcia = ccbcdocu.codcia
          AND di-rutad.coddoc = 'H/R'
/*           AND di-rutad.coddiv = ccbcdocu.coddiv */
          AND di-rutad.codref = ccbcdocu.codref
          AND di-rutad.nroref = ccbcdocu.nroref
          AND di-rutad.flgest = 'C'     /* Entregado */
          NO-LOCK NO-ERROR.
      IF NOT AVAILABLE di-rutad THEN NEXT.
      FIND FIRST di-rutac OF di-rutad NO-LOCK NO-ERROR.
      IF NOT AVAILABLE di-rutac THEN NEXT.
      ...
      ASSIGN
          Task-No    = s-task-no
          campo-i[1] = ccbcdocu.codcia
          campo-c[1] = ccbcdocu.coddiv
          campo-c[2] = ccbcdocu.coddoc
          campo-c[3] = ccbcdocu.nrodoc
          campo-c[4] = di-rutad.nrodoc
          campo-d[1] = di-rutac.fchdoc.
  END.
...
------------------------------------------------------------------------------*/
  DISPLAY x-CodDiv x-CodDoc x-FmaPgo x-FchDoc-1 x-FchDoc-2 x-FlgEst f-Mensaje 
      WITH FRAME fMain IN WINDOW NoiNwWin.
  ENABLE BUTTON-1 Btn_Excel BtnDone x-CodDiv x-CodDoc x-FmaPgo x-FchDoc-1 
         x-FchDoc-2 x-FlgEst 
      WITH FRAME fMain IN WINDOW NoiNwWin.
...
FOR EACH b-ccbcdocu NO-LOCK WHERE b-ccbcdocu.codcia = s-codcia
      ...
      FIND FIRST b-di-rutad WHERE b-di-rutad.codcia = b-ccbcdocu.codcia
          ...
          AND b-di-rutad.codref = b-ccbcdocu.codref
          AND b-di-rutad.nroref = b-ccbcdocu.nroref
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      FOR EACH gn-convt NO-LOCK:
          x-FmaPgo:add-last(gn-ConVt.Codig + ' ' + gn-ConVt.Nombr).
      END.
  END.
