...
CcbCDocu.FchDoc CcbCDocu.CodCli CcbCDocu.NomCli CcbCDocu.FchCbd ~
CcbCDocu.FchVto CcbCDocu.FchCre 
...
&Scoped-define QUERY-STRING-br_table FOR EACH CcbCDocu WHERE ~{&KEY-PHRASE} ~
 ...
 AND ( TOGGLE-1 = NO OR CcbCDocu.FchCbd <> ? ) ~
 AND ( TOGGLE-2 = NO OR CcbCDocu.FchCre <> ? ) NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&Scoped-Define ENABLED-OBJECTS x-CodDoc x-FchDoc TOGGLE-1 TOGGLE-2 br_table 
&Scoped-Define DISPLAYED-OBJECTS x-CodDoc x-FchDoc TOGGLE-1 TOGGLE-2 
...
DEFINE VARIABLE x-CodDoc AS CHARACTER INITIAL "FAC|BOL" 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.FchCre COLUMN-LABEL "Fecha!Credito" FORMAT "99/99/9999":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 103 BY 12.38
         FONT 4.
...
DEFINE FRAME F-Main
     x-CodDoc AT ROW 1.27 COL 21 NO-LABEL WIDGET-ID 8
     x-FchDoc AT ROW 2.08 COL 19 COLON-ALIGNED WIDGET-ID 14
     TOGGLE-1 AT ROW 3.15 COL 21 WIDGET-ID 16
     TOGGLE-2 AT ROW 4.23 COL 21 WIDGET-ID 18
     br_table AT ROW 5.31 COL 2
     "Documento:" VIEW-AS TEXT
          SIZE 11 BY .62 AT ROW 1.27 COL 10 WIDGET-ID 12
...
/* BROWSE-TAB br_table TOGGLE-2 F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br_table
 ...
 AND CcbCDocu.FchDoc >= x-FchDoc
 AND ( TOGGLE-1 = NO OR CcbCDocu.FchCbd <> ? )
 AND ( TOGGLE-2 = NO OR CcbCDocu.FchCre <> ? )"
...
"CcbCDocu.FchDoc" "Fecha!Emision" ? "date" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.CodCli" ? ? "character" ? ? ? ? ? ? no ? no no "11.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[6]   > INTEGRAL.CcbCDocu.FchCbd
...
"CcbCDocu.FchVto" "Fecha!Vencimiento" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > INTEGRAL.CcbCDocu.FchCre
"CcbCDocu.FchCre" "Fecha!Credito" "99/99/9999" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON VALUE-CHANGED OF TOGGLE-1 IN FRAME F-Main /* Solo con fecha de entrega */
DO:
    ASSIGN {&self-name}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL TOGGLE-2 B-table-Win
ON VALUE-CHANGED OF TOGGLE-2 IN FRAME F-Main /* Solo con fecha de crédito */
DO:
    ASSIGN {&self-name}.
    RUN dispatch IN THIS-PROCEDURE ('open-query').
...
ON VALUE-CHANGED OF x-CodDoc IN FRAME F-Main
DO:
  ASSIGN {&self-name}.
  RUN dispatch IN THIS-PROCEDURE ('open-query').
END.
...
&Scoped-define SELF-NAME x-FchDoc
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL x-FchDoc B-table-Win
ON LEAVE OF x-FchDoc IN FRAME F-Main /* Emitidos desde el */
DO:
    ASSIGN {&self-name}.
    RUN dispatch IN THIS-PROCEDURE ('open-query').
