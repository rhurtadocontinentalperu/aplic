...
/*             almmmatg.codpr2 = ttCarga.tcodpr2 */
    ...
    END.
    ASSIGN 
        /*almmmatgext.codintpr1 = ttCarga.tcodpr1*/
        almmmatgext.codintpr1 = ttCarga.tcodintpr1
    ...
    ASSIGN 
        almmmatgext.largoinner2 = ttCarga.tlargoinner2
    ...
    ASSIGN 
        almmmatgext.largomaster = ttCarga.tlargomaster
    ...
    ASSIGN 
        almmmatgext.largopallet = ttCarga.tlargopallet
...
/*     END.                                                                        */
    ...
    /* Validamos que no se repita el proveedor y su código interno en el sistema */
    IF ttCarga.tCodPr1 > '' AND ttCarga.tCodIntPr1 > '' THEN DO:
        FIND FIRST Almmmatg WHERE Almmmatg.codcia = s-CodCia 
            AND Almmmatg.CodPr1 = ttCarga.tCodPr1
            AND Almmmatg.TpoArt <> "D"
            AND CAN-FIND(FIRST Almmmatgext OF Almmmatg WHERE AlmmmatgExt.codintpr1 = ttCarga.tCodIntPr1 NO-LOCK)
            NO-LOCK NO-ERROR.
        IF AVAILABLE Almmmatg THEN DO:
            ASSIGN ttCarga.tmsgerr = "COD.INT. DEL PROVEE. YA REGIS. " + Almmmatg.CodMat + ' ' + ttCarga.tCodIntPr1.
            NEXT.
        END.
    END.

    /* Carge de Pesos y Volumen en este orden: Master, Inner y Unidad */
    ...
    ASSIGN
        x-Peso = tPesMat
        x-Volumen = tLibre_d02.
    CASE TRUE:
        WHEN tPesoMaster > 0 AND tCanEmp > 0 THEN x-Peso = tPesoMaster / tCanEmp.
        WHEN tPesoInner2 > 0 AND tequival_2 > 0 THEN x-Peso = tPesoInner2 / tequival_2.
        WHEN tPesoInner > 0 AND tequival_1 > 0 THEN x-Peso = tPesoInner / tequival_1.
    END CASE.
    CASE TRUE:
        WHEN tVolMaster > 0 AND tCanEmp > 0 THEN x-Volumen = tVolMaster / tCanEmp.
        WHEN tVolInner2 > 0 AND tequival_2 > 0 THEN x-Volumen = tVolInner2 / tequival_2.
        WHEN tVolInner > 0 AND tequival_1 > 0 THEN x-Volumen = tVolInner / tequival_1.
    END CASE.
    ASSIGN
        tPesMat = x-Peso
        tLibre_d02 = x-Volumen.
    /* **************************************************************************** */
