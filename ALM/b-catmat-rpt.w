...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      almmmatg.codmat COLUMN-LABEL "Codigo" FORMAT "X(6)":U
      almmmatg.DesMat FORMAT "X(45)":U WIDTH 34.14
      almmmatg.UndBas COLUMN-LABEL "Unidad" FORMAT "X(8)":U
      almmmatg.CodBrr COLUMN-LABEL "EAN 13" FORMAT "X(13)":U
      almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(30)":U WIDTH 24.29
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 81 BY 15.96
         FONT 2.
...
"DesMat" ? ? "character" ? ? ? ? ? ? no ? no no "34.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.almmmatg.UndBas
"UndBas" "Unidad" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.almmmatg.CodBrr
"CodBrr" "EAN 13" "X(13)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.almmmatg.DesMar
"DesMar" "Marca" ? "character" ? ? ? ? ? ? no ? no no "24.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE um-refresh B-table-Win 
PROCEDURE um-refresh :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
x-Familias = s-parametro.

IF s-parametro = '' THEN DO:
    x-Familias = ''.
    FOR EACH Almtfami NO-LOCK WHERE Almtfami.codcia = s-codcia:
        CASE s-Parametro:
            WHEN '+' THEN DO:
...
/*         MESSAGE 'El código de barra debe tener 13 caracteres'                       */
...
/*
    X = SUBSTRING(almmmatg.CodBrr
    RUN lib/_dig_verif-ean13 (SUBSTRING(
    */
END.
RETURN "OK".
