...
/*DEF VAR s-FlgSit AS CHAR INIT 'P'.      /* Por Aprobar */*/
DEF VAR s-FlgSit AS CHAR INIT 'P,G'.      /* Por Aprobar, No Autorizado */
...
AND LOOKUP(almcrepo.FlgSit,s-FlgSit) > 0 ~
...
AND B-CREPO.AlmPed = s-CodAlm ~
AND B-CREPO.FlgEst = s-FlgEst ~
AND LOOKUP(B-CREPO.FlgSit,s-FlgSit) > 0 ~
...
&Scoped-Define ENABLED-OBJECTS RECT-1 BUTTON-2 BUTTON-4 COMBO-BOX-CodAlm ~
BUTTON-1 COMBO-BOX-Motivo BUTTON-15 FILL-IN-FchDoc-1 FILL-IN-FchDoc-2 ~
FILL-IN-Usuario FILL-IN-Fecha-1 FILL-IN-Fecha-2 br_table 
...
/* SETTINGS FOR COMBO-BOX COMBO-BOX-AlmPed IN FRAME F-Main
   NO-ENABLE                                                            */
...
DO:
  MESSAGE 'Está seguro de APROBAR TODO?' VIEW-AS ALERT-BOX QUESTION
      BUTTONS YES-NO UPDATE rpta AS LOG.
  IF rpta = NO THEN RETURN NO-APPLY.
  RUN Aprobar-Todo.
END.
...
DO k = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}
    ...
    IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(k) THEN DO:
        IF NOT ( almcrepo.flgest = s-FlgEst AND LOOKUP(almcrepo.flgsit,s-FlgSit) > 0 ) THEN NEXT.
        ...
        ASSIGN
            ...
            B-CREPO.HorApr = STRING(TIME, 'HH:MM:SS')
...
FOR EACH facdpedi OF faccpedi NO-LOCK,
    ...
    IF FIRST-OF(SUBSTRING(Almmmate.CodUbi,1,2)) THEN DO:
        ...
        /* 18May2017, Felix Perez creo nueva ZONA (07) */
...
/*       END.                                                                                           */
      COMBO-BOX-AlmPed:DELETE(1).
      FIND Almacen WHERE Almacen.codcia = s-codcia
          AND Almacen.codalm = s-codalm
          NO-LOCK NO-ERROR.
      COMBO-BOX-AlmPed:ADD-LAST( Almacen.CodAlm + ' - ' + Almacen.Descripcion, Almacen.CodAlm ).
      COMBO-BOX-AlmPed = Almacen.CodAlm.
      ...
      ASSIGN
          ...
          FILL-IN-FchDoc-2 = TODAY.
      ASSIGN
          FILL-IN-FchDoc-1 = ADD-INTERVAL (FILL-IN-FchDoc-1, -1, 'month').
...
DO k = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
    IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(k) THEN DO:
        ...
        ASSIGN
            ...
            almcrepo.HorApr = STRING(TIME, 'HH:MM:SS')
