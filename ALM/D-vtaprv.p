...
FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = S-CODCIA
                          ...
                          USE-INDEX matg09,
   EACH Almdmov NO-LOCK WHERE Almdmov.CodCia = Almmmatg.codcia 
                         AND  LOOKUP(trim(Almdmov.CodAlm), "03,04,05") > 0
                         AND  Almdmov.codmat = Almmmatg.codmat
                         AND  (Almdmov.FchDoc >= DesdeF
                         AND   Almdmov.FchDoc <= HastaF)
                         AND  Almdmov.TipMov = "S" 
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-HEADER
         ...
         "Pagina :" TO 125 PAGE-NUMBER(REPORT) TO 137 FORMAT "ZZZZZ9" 
         SKIP
         S-DESALM  FORMAT "X(30)" AT 56
         "Fecha  :" TO 125 TODAY TO 137 FORMAT "99/99/9999" 
         SKIP
         "PERIODO DE VENTA : " AT 61 FORMAT "X(19)" STRING(x-dia, ">9") " DIAS" FORMAT "X(5)"
         "Hora   :" TO 125 STRING(TIME,"HH:MM:SS") TO 137 
         SKIP
         S-SUBTIT AT 1 SKIP
         "COSTOS EXPRESADOS EN " + IF NCODMON = 1 THEN "NUEVOS SOLES" ELSE "DOLARES AMERICANOS" + " INCLUIDO IGV" AT 1 FORMAT "X(100)"  
          SKIP(1) 
        "-----------------------------------------------------------------------------------------------------------------------------------------" SKIP
        "                                                               V    E    N    T    A    S                         STOCK   NRO     COSTO  " SKIP
        "CODIGO DESCRIPCION                         MARCA U.B       UCAYALI   ANDAHUAYLA       PARURO        TOTAL         TOTAL   DIAS    TOTAL  " SKIP
        "-----------------------------------------------------------------------------------------------------------------------------------------" SKIP
