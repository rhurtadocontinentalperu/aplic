...
DEFINE NEW SHARED VARIABLE s-codref   AS CHAR INITIAL "STR".
...
DEFINE VARIABLE h_vpedidodetransferencia AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vpedidodetransferencia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vpedidodetransferencia ).
       RUN set-position IN h_vpedidodetransferencia ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.88 , 112.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'qpedidodetransferencia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-pedido ).
       ...
       /* Links to SmartViewer h_vpedidodetransferencia. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_vpedidodetransferencia ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_vpedidodetransferencia ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_vpedidodetransferencia ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             h_vpedidodetransferencia , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'bpedidodetransferencia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bpedidodetransferencia ).
       RUN set-position IN h_bpedidodetransferencia ( 6.96 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'tpedidodetransferencia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tpedidodetransferencia ).
       RUN set-position IN h_tpedidodetransferencia ( 6.96 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv95 ).
       RUN set-position IN h_p-updv95 ( 18.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv95 ( 1.42 , 34.00 ) NO-ERROR.

       /* Links to SmartBrowser h_tpedidodetransferencia. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_tpedidodetransferencia ).
...
CASE L-Handle:
 ...
 WHEN "Disable-Head" THEN DO:
     ...
     RUN dispatch IN h_vpedidodetransferencia ('disable-fields').
   END.
 WHEN "Enable-Head" THEN DO:
     RUN dispatch IN h_p-updv04 ('enable':U).
     RUN dispatch IN h_vpedidodetransferencia ('enable-fields').
   END.
END CASE.
