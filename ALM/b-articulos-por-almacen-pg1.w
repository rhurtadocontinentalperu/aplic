...
DEF VAR x-TpoArt AS CHAR INIT 'A' NO-UNDO.

&SCOPED-DEFINE Condicion ( ~
Almmmate.codcia = s-codcia AND ~
Almmmate.codalm = s-codalm AND ~
(TRUE <> (x-CodMat > '') OR Almmmate.codmat = x-CodMat) ~
                           )
...
&Scoped-define QUERY-STRING-br_table FOR EACH Almmmate WHERE ~{&KEY-PHRASE} ~
      ...
      WHERE (TRUE <> (x-TpoArt > '') OR Almmmatg.TpoArt = x-TpoArt) NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almmmatg.TpoArt FORMAT "X(5)":U
      Almmmate.StkAct FORMAT "(ZZZ,ZZZ,ZZ9.99)":U
      Almmmate.CodUbi FORMAT "x(7)":U
      Almmmate.StkRep FORMAT "ZZ,ZZZ,ZZ9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 132 BY 22.35
         FONT 4.
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "{&Condicion}"
     _Where[2]         = "(TRUE <> (x-TpoArt > '') OR Almmmatg.TpoArt = x-TpoArt)"
...
"Almmmatg.TpoArt" ? "X(5)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
