...
------------------------------------------------------------------------------*/
  ...
  IF x-CodPro > '' THEN DO:
      ...
      END.
  END.
  IF x-FiltroOK = YES AND x-ItemOK = NO THEN RETURN.
  IF cCategoria > '' THEN DO:
      x-FiltroOK = YES.
      IF x-ItemOK = YES THEN DO:
          FOR EACH T-MATG:
              IF T-MATG.catconta[1] <> cCategoria THEN DELETE T-MATG.
          END.
          IF NOT CAN-FIND(FIRST T-MATG NO-LOCK) THEN x-ItemOK = NO.
      END.
      ELSE DO:
          FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
              AND Almmmatg.codmat >= DesdeC
              AND Almmmatg.codmat <= HastaC
              AND Almmmatg.catconta[1] = cCategoria:
              CREATE T-MATG.
              BUFFER-COPY Almmmatg TO T-MATG.
              x-ItemOK = YES.
          END.
  ...
  END.
  IF x-FiltroOK = YES AND x-ItemOK = NO THEN RETURN.
  IF x-FiltroOK = NO THEN DO:   /* Toda la Tabla */
      FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
          AND Almmmatg.codmat >= DesdeC
          AND Almmmatg.codmat <= HastaC:
          CREATE T-MATG.
  ...
  IF x-CodAlm = 'Todos' THEN DO:
      FOR EACH Almacen WHERE almacen.codcia = s-codcia
          ...
          x-Almacen = x-Almacen + (IF TRUE <> (x-Almacen > '') THEN '' ELSE ',') +
              Almacen.CodAlm.
      END.
  END.
  ...
  DO x-Item = 1 TO NUM-ENTRIES(x-Almacen):
      ...
      FOR EACH T-MATG NO-LOCK WHERE T-MATG.codcia = s-codcia,
          ...
          IF NOT (TRUE <> (c-CodFam > '') OR Almmmatg.codfam = c-CodFam) THEN NEXT.
/*       FOR EACH Almmmate NO-LOCK WHERE Almmmate.codcia = s-codcia                             */
/*           AND Almmmate.CodAlm = ENTRY(x-Item, x-Almacen)                                     */
/*           AND Almmmate.codmat >= DesdeC                                                      */
/*           AND Almmmate.codmat <= HastaC,                                                     */
/*           FIRST T-MATG OF Almmmate, FIRST Almmmatg OF T-MATG NO-LOCK,                        */
/*           FIRST Almtfami OF T-MATG NO-LOCK,                                                  */
/*           FIRST Almsfami OF T-MATG NO-LOCK:                                                  */
/*           IF NOT (TRUE <> (c-CodFam > '') OR Almmmatg.codfam = c-CodFam) THEN NEXT.          */
/*           IF NOT (TRUE <> (cCategoria > '') OR Almmmatg.catconta[1] = cCategoria) THEN NEXT. */
/*           IF NOT (TRUE <> (x-CodPro > '') OR Almmmatg.codpr1 = x-CodPro) THEN NEXT.          */
          x-Cuenta = x-Cuenta + 1.
          ASSIGN F-Saldo  = 0.
