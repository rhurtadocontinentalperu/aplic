...
FILL-IN-CodPro COMBO-BOX-Tipo SELECT-Clasificacion FILL-IN-PorRep ~
FILL-IN-PorStkMax FILL-IN-ImpMin BUTTON-7 BUTTON-20 COMBO-BOX-Motivo ~
FILL-IN-Glosa TOGGLE-VtaPuntual txtFechaEntrega FILL-IN-Cotizacion 
...
FILL-IN-CodPro FILL-IN-NomPro COMBO-BOX-Tipo SELECT-Clasificacion ~
FILL-IN-PorRep FILL-IN-PorStkMax FILL-IN-ImpMin FILL-IN-Texto ~
...
DEFINE FRAME F-Main
     ...
     FILL-IN-CodFam AT ROW 3.15 COL 11 COLON-ALIGNED WIDGET-ID 14
     BUTTON-4 AT ROW 3.15 COL 44 WIDGET-ID 12
     FILL-IN-SubFam AT ROW 4.23 COL 11 COLON-ALIGNED WIDGET-ID 54
     BUTTON-6 AT ROW 4.23 COL 44 WIDGET-ID 52
     FILL-IN-Marca AT ROW 5.31 COL 11 COLON-ALIGNED WIDGET-ID 28
     FILL-IN-CodPro AT ROW 6.38 COL 11 COLON-ALIGNED WIDGET-ID 22
     FILL-IN-NomPro AT ROW 6.38 COL 23 COLON-ALIGNED NO-LABEL WIDGET-ID 24
     COMBO-BOX-Tipo AT ROW 7.46 COL 18 COLON-ALIGNED WIDGET-ID 88
     SELECT-Clasificacion AT ROW 7.46 COL 38 NO-LABEL WIDGET-ID 114
     FILL-IN-PorRep AT ROW 8.54 COL 18 COLON-ALIGNED WIDGET-ID 42
     FILL-IN-PorStkMax AT ROW 9.62 COL 18 COLON-ALIGNED WIDGET-ID 84
     FILL-IN-ImpMin AT ROW 10.69 COL 18 COLON-ALIGNED WIDGET-ID 44
     BUTTON-7 AT ROW 11.23 COL 29 WIDGET-ID 58
     FILL-IN-Texto AT ROW 13.12 COL 11 COLON-ALIGNED WIDGET-ID 64
     BUTTON-20 AT ROW 13.12 COL 44 WIDGET-ID 66
     COMBO-BOX-Motivo AT ROW 15.54 COL 15 COLON-ALIGNED WIDGET-ID 70
     FILL-IN-Glosa AT ROW 16.35 COL 15 COLON-ALIGNED WIDGET-ID 32
     TOGGLE-VtaPuntual AT ROW 17.15 COL 17 WIDGET-ID 68
     txtFechaEntrega AT ROW 17.96 COL 15 COLON-ALIGNED WIDGET-ID 56
     FILL-IN-Cotizacion AT ROW 19.04 COL 15 COLON-ALIGNED WIDGET-ID 138
     FILL-IN-Mensaje AT ROW 20.38 COL 2 NO-LABEL WIDGET-ID 60
     BUTTON-Stop-It AT ROW 20.38 COL 47 WIDGET-ID 134
     "Datos Cabecera" VIEW-AS TEXT
          SIZE 18 BY .5 AT ROW 14.73 COL 3 WIDGET-ID 128
     ...
     RECT-27 AT ROW 15 COL 2 WIDGET-ID 130
     RECT-28 AT ROW 12.85 COL 2 WIDGET-ID 132
...
/*   DISPLAY FILL-IN-CodRef FILL-IN-NroRef WITH FRAME {&FRAME-NAME}. */
  ...
  DO k = 1 TO NUM-ENTRIES(SELECT-Clasificacion:LIST-ITEM-PAIRS):
      ...
      END.
  END.
  /* En caso NO solicite información de un archivo texto */
  IF TRUE <> (FILL-IN-Texto > '') THEN DO:
      IF j = 0 THEN DO:
          MESSAGE 'Debe al menos seleccionar una clasificación' VIEW-AS ALERT-BOX WARNING.
          APPLY 'ENTRY':U TO SELECT-Clasificacion.
          RETURN NO-APPLY.
      END.
      IF TRUE <> (FILL-IN-CodFam > '') THEN DO:
          MESSAGE 'Debe seleccionar al menos una línea' VIEW-AS ALERT-BOX WARNING.
          APPLY 'ENTRY':U TO FILL-IN-CodFam.
          RETURN NO-APPLY.
      END.
  END.
  ASSIGN
...
DO:
  ...
  RUN alm/d-ra-generacion-v2.w (OUTPUT x-Archivo, OUTPUT TABLE T-DATOS).
  IF TRUE <> (x-Archivo > '') THEN DO:
      RETURN NO-APPLY.
  END.
  FILL-IN-Texto:SCREEN-VALUE = x-Archivo.
  /*{&OPEN-QUERY-BROWSE-5}*/

  /* Bloqueamos los FILTROS */
  DISABLE BUTTON-4 BUTTON-6 FILL-IN-CodPro FILL-IN-Marca SELECT-Clasificacion COMBO-BOX-Tipo
...
ON LEAVE OF FILL-IN-CodFam IN FRAME F-Main /* Lineas */
DO:
  ASSIGN {&self-name}.
END.
...
ON LEAVE OF FILL-IN-SubFam IN FRAME F-Main /* SubLíneas */
DO:
    ASSIGN {&self-name}.
...
FOR EACH Almrepos NO-LOCK WHERE Almrepos.CodCia = s-codcia
    ...
    BY Almrepos.Orden:
    FIND Almacen WHERE Almacen.codcia = Almrepos.codcia AND Almacen.codalm = Almrepos.almped NO-LOCK NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-TDREPO W-Win 
PROCEDURE Carga-TDREPO :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH T-MATG NO-LOCK, EACH T-MATE OF T-MATG NO-LOCK BY T-MATG.CodMat:
    /* Pintamos cada 100 registros */
    x-Contador = x-Contador + 1.
    IF (x-Contador MODULO 100) = 0 THEN FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "PROCESANDO: " + T-MATG.codmat + " " + T-MATG.desmat.
...
x-Contador = 0.
FOR EACH T-DREPO, FIRST B-MATG OF T-DREPO NO-LOCK BY T-DREPO.CodMat:
    /* Pintamos cada 100 registros */
    x-Contador = x-Contador + 1.
    IF (x-Contador MODULO 100) = 0 THEN FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "DATOS ADICIONALES: " + T-DREPO.codmat.
...
IF NOT TRUE <> (FILL-IN-Texto > '') THEN DO:
    FOR EACH T-DATOS NO-LOCK, 
        ...
        Almmmatg.codmat = T-DATOS.Llave-C:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-TMATE W-Win 
PROCEDURE Carga-TMATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
EMPTY TEMP-TABLE T-MATE.
/* Barremos producto por producto */
FOR EACH T-MATG NO-LOCK BY T-MATG.CodMat:
    /* Pintamos cada 100 registros */
    x-Contador = x-Contador + 1.
    IF (x-Contador MODULO 100) = 0 THEN FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "PROCESANDO CD: " + T-MATG.codmat + " " + T-MATG.desmat.
    /* ******************************************************************************** */
    /* RHC 08/05/2017 SE VA A GENERAR UNA TABLA CON LA INFORMACION DE ALMMMATE PERO CON 
        LOS VALORES DEL CD (EJ.ATE) SI FUERA EL CASO */
    /* ******************************************************************************** */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-TMATG W-Win 
PROCEDURE Carga-TMATG :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
ELSE DO:
    ...
    FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
        AND Almmmatg.tpoart <> "D"
        AND Almmmatg.codfam = FILL-IN-CodFam
        AND (FILL-IN-SubFam = "" OR LOOKUP(Almmmatg.subfam, FILL-IN-SubFam) > 0)
        AND (FILL-IN-CodPro = "" OR Almmmatg.codpr1 = FILL-IN-CodPro)
        AND (FILL-IN-Marca = "" OR Almmmatg.desmar BEGINS FILL-IN-Marca):
        /* Pintamos cada 100 registros */
        x-Contador = x-Contador + 1.
        IF (x-Contador MODULO 100) = 0 THEN FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "SELECCIONANDO: " + Almmmatg.codmat + " " + Almmmatg.desmat.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-Almacen FILL-IN-Tipo FILL-IN-Temporada FILL-IN-CodFam 
          ...
          FILL-IN-ImpMin FILL-IN-Texto COMBO-BOX-Motivo FILL-IN-Glosa 
          TOGGLE-VtaPuntual txtFechaEntrega FILL-IN-Cotizacion FILL-IN-Mensaje 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-9 RECT-1 RECT-27 RECT-28 BUTTON-1 BUTTON-5 BUTTON-IMPORTAR-EXCEL 
         BUTTON-2 BUTTON-4 FILL-IN-Marca FILL-IN-CodPro COMBO-BOX-Tipo 
         SELECT-Clasificacion FILL-IN-PorRep FILL-IN-PorStkMax FILL-IN-ImpMin 
         BUTTON-7 BUTTON-20 COMBO-BOX-Motivo FILL-IN-Glosa TOGGLE-VtaPuntual 
         txtFechaEntrega FILL-IN-Cotizacion 
      WITH FRAME F-Main IN WINDOW W-Win.
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    ENABLE BUTTON-4 BUTTON-6 FILL-IN-CodPro FILL-IN-Marca SELECT-Clasificacion COMBO-BOX-Tipo
      WITH FRAME {&FRAME-NAME}.
END.
...
/* *********************************************************************** */
FILL-IN-Mensaje:VISIBLE IN FRAME {&FRAME-NAME} = YES.
FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "INICIADO: Espere un momento por favor...".
RUN Carga-TMATG.
