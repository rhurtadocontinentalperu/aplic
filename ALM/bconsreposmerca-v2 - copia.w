...
almcrepo.AlmPed REPOSICIONES.AlmacenXD almcrepo.CodAlm almcrepo.FchDoc ~
...
"almcrepo.AlmPed" "Almacén!Despacho" ? "character" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"almcrepo.CodAlm" "Almacén!Destino" ? "character" ? ? ? ? ? ? no ? no no "6.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"reposiciones.obsrechazo @ reposiciones.libre_c03" "Observacion Rechazo" "x(60)" ? ? ? ? ? ? ? no ? no no "22.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[33]   > INTEGRAL.almcrepo.VtaPuntual
...
/* Seguimiento */
FOR EACH REPOSICIONES WHERE LOOKUP(REPOSICIONES.FlgEst, "A,R") = 0 AND REPOSICIONES.OT > '':
    /* ******************************************************************************** */
    ...
    FOR EACH faccpedi NO-LOCK WHERE faccpedi.codcia = s-codcia
        ...
        AND faccpedi.nroped = REPOSICIONES.OT:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Otros-Old B-table-Win 
PROCEDURE Carga-Otros-Old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH REPOSICIONES:
    ...
    /* DOS CASOS:
        ...
        */
    CASE TRUE:
        WHEN TRUE <> (REPOSICIONES.OT > '') THEN DO:
            lCantAtendido = 0.
        ...
        OTHERWISE DO:
            ...
            FOR EACH almdrepo NO-LOCK WHERE almdrepo.codcia = REPOSICIONES.codcia
                ...
                AND almdrepo.nroser = REPOSICIONES.nroser
                AND almdrepo.nrodoc = REPOSICIONES.nrodoc,
                FIRST Facdpedi OF Faccpedi WHERE Facdpedi.codmat = Almdrepo.codmat NO-LOCK,
                FIRST almcrepo OF almdrepo NO-LOCK,
                FIRST Almmmatg OF almdrepo NO-LOCK:
                FIND LAST almstkge WHERE almstkge.codcia = almmmatg.codcia 
                    AND almstkge.codmat = almmmatg.codmat 
                ...
                ASSIGN
                    ...
                    REPOSICIONES.Libre_d02 = REPOSICIONES.Libre_d02 + (facdpedi.canped * almmmatg.pesmat).
                /* Costos */
                IF AVAILABLE Almstkge THEN
                    ASSIGN
                        ...
                        REPOSICIONES.TotCosAte = REPOSICIONES.TotCosAte + (facdpedi.canped * almstkge.CtoUni).
                /* Volumen */
                ASSIGN
                    ...
                    REPOSICIONES.TotVolAte = REPOSICIONES.TotVolAte + (facdpedi.canped * almmmatg.libre_d02 / 1000000).
            END.
...
FOR EACH almacen NO-LOCK WHERE almacen.codcia = s-codcia
    ...
    EACH almcrepo NO-LOCK WHERE {&Condicion},
    EACH Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-CodCia AND
        Faccpedi.coddoc = 'OTR' AND
        Faccpedi.codref = 'R/A' AND
        Faccpedi.nroref = STRING(Almcrepo.NroSer,'999') + STRING(Almcrepo.NroDoc, '999999') AND
        Faccpedi.FlgEst <> 'A':
    CREATE REPOSICIONES.
    ...
    IF Faccpedi.crossdocking = YES THEN
        ASSIGN
...
FOR EACH almacen NO-LOCK WHERE almacen.codcia = s-codcia
        AND (FILL-IN-CodAlm = '' OR LOOKUP(almacen.codalm, FILL-IN-CodAlm) > 0),
    EACH almcrepo NO-LOCK WHERE {&Condicion}:
    IF CAN-FIND(FIRST Faccpedi WHERE Faccpedi.codcia = s-CodCia AND
                Faccpedi.coddoc = 'OTR' AND
                Faccpedi.codref = 'R/A' AND
                Faccpedi.nroref = STRING(Almcrepo.NroSer,'999') + STRING(Almcrepo.NroDoc, '999999') AND
    ...
    BUFFER-COPY Almcrepo TO REPOSICIONES
        ...
        REPOSICIONES.libre_c04 = almcrepo.horapr.
END.
/* LIMPIAMOS REPOSICIONES */
FOR EACH REPOSICIONES, FIRST Almcrepo OF REPOSICIONES:
    ASSIGN
    ...
    IF NOT (txtUsuario = '' OR almcrepo.usuario = txtUsuario) THEN DO:
        DELETE REPOSICIONES.
        NEXT.
    END.
    IF NOT (COMBO-BOX-Motivo = 'Seleccione' OR almcrepo.MotReposicion = COMBO-BOX-Motivo) THEN DO:
        DELETE REPOSICIONES.
        NEXT.
    END.
    ...
    IF TOGGLE-CrossDocking = YES THEN DO:
        ...
        END.
    END.
    IF x-Estado = 'Rechazado' AND almcrepo.flgest <> 'R' THEN DO:
        DELETE REPOSICIONES.
        NEXT.
    END.
    IF TOGGLE-VtaPuntual = YES AND almcrepo.VtaPuntual = NO THEN DO:
        DELETE REPOSICIONES.
        NEXT.
    END.
    /* En caso del producto */
    IF FILL-IN-CodMat > '' THEN DO:
        FIND FIRST Almdrepo OF Almcrepo WHERE Almdrepo.codmat = FILL-IN-CodMat NO-LOCK NO-ERROR.
    ...
    IF almcrepo.libre_c02 <> ? THEN DO:
        IF NUM-ENTRIES(almcrepo.libre_c02,"|") > 1 THEN DO:
            ...
            ASSIGN 
                REPOSICIONES.codrechazo = cFiler1
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Tracking-Old B-table-Win 
PROCEDURE Carga-Tracking-Old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Dame-OT B-table-Win 
PROCEDURE Dame-OT :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
pNroPed = ''.
IF AVAILABLE REPOSICIONES THEN pNroPed = REPOSICIONES.OT.
    ...
    REPEAT WHILE AVAILABLE REPOSICIONES:
        FOR EACH almdrepo OF almcrepo NO-LOCK, FIRST Almmmatg OF Almdrepo NO-LOCK:
            ...
            BUFFER-COPY Almcrepo
                ...
                T-Detalle-Item.Glosa     = almcrepo.Glosa
...
FOR EACH B-REPOSICIONES NO-LOCK WHERE (x-Estado = 'Todos' OR LOOKUP(B-REPOSICIONES.Libre_c05, x-Estado) > 0)
    ...
    ASSIGN
        ...
        .
      /* Ic - Correo Max Ramos - 20Dic2016 */
      IF LOOKUP(B-REPOSICIONES.Libre_c05,cEstados) > 0 AND TODAY > almcrepo.fecha 
