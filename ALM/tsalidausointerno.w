...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ITEM.codmat COLUMN-LABEL "Codigo" FORMAT "X(6)":U WIDTH 6.43
      Almmmatg.DesMat FORMAT "X(60)":U
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(15)":U
      ITEM.CodUnd COLUMN-LABEL "Unidad" FORMAT "X(7)":U
      ITEM.CanDes FORMAT "ZZZ,ZZ9.99":U COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 11
      ITEM.StkAct COLUMN-LABEL "Solicitado" FORMAT "ZZZ,ZZ9.99":U
            COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 14
  ENABLE
      ITEM.CanDes
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 88 BY 12.5
         FONT 4.
...
"Temp-Tables.ITEM.codmat" "Codigo" ? "character" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Temp-Tables.ITEM.CodUnd" "Unidad" "X(7)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.ITEM.CanDes
"Temp-Tables.ITEM.CanDes" ? "ZZZ,ZZ9.99" "decimal" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.ITEM.StkAct
"Temp-Tables.ITEM.StkAct" "Solicitado" "ZZZ,ZZ9.99" "decimal" 14 0 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/* CONSISTENCIA NORMAL */
IF DECIMAL(ITEM.CanDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) *  ITEM.Factor > (Almmmate.stkact - pComprometido)
    THEN DO:
    ...
    RETURN "ADM-ERROR".
END.
IF DECIMAL(ITEM.CanDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) *  ITEM.Factor > ITEM.StkAct
    THEN DO:
    MESSAGE 'NO se puede sacar mas de' ITEM.StkAct
        VIEW-AS ALERT-BOX ERROR.
    APPLY "ENTRY" TO ITEM.CanDes.
    RETURN "ADM-ERROR".
END.
