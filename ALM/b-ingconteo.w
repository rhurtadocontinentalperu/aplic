...
DEF VAR lVal    AS LOGICAL   NO-UNDO.
...
&Scoped-define INTERNAL-TABLES tt-AlmDInv INTEGRAL.Almmmatg
...
tt-AlmDInv.codmat INTEGRAL.Almmmatg.DesMat INTEGRAL.Almmmatg.UndBas ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH tt-AlmDInv WHERE ~{&KEY-PHRASE} ~
      ...
      FIRST INTEGRAL.Almmmatg OF tt-AlmDInv NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH tt-AlmDInv WHERE ~{&KEY-PHRASE} ~
      AND tt-AlmDInv.Codcia = s-codcia NO-LOCK, ~
      FIRST INTEGRAL.Almmmatg OF tt-AlmDInv NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table tt-AlmDInv INTEGRAL.Almmmatg
...
&Scoped-Define ENABLED-OBJECTS txtZona txt-page btn-consulta txt-codalmac ~
txt-codinv br_table btn_ingresa btn-dif btn-lista btn-grabar btn-cancelar ~
btn-exit BUTTON-2 RECT-67 RECT-68 
&Scoped-Define DISPLAYED-OBJECTS txt-codalm txt-desalm txtZona txt-page ~
txt-codalmac txt-nombre1 txt-codinv txt-totpag 
...
DEFINE VARIABLE txtZona AS CHARACTER FORMAT "X(256)":U 
     LABEL "Zona" 
     VIEW-AS FILL-IN 
     SIZE 9 BY .81 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      INTEGRAL.Almmmatg.DesMat FORMAT "X(50)":U WIDTH 40
      INTEGRAL.Almmmatg.UndBas COLUMN-LABEL "UndBas" FORMAT "X(4)":U
            WIDTH 6
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 82 BY 21.92
         FONT 4
...
DEFINE FRAME F-Main
     txt-codalm AT ROW 1.5 COL 13.86 COLON-ALIGNED WIDGET-ID 4
     txt-desalm AT ROW 1.5 COL 19.29 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     txtZona AT ROW 1.58 COL 76 COLON-ALIGNED WIDGET-ID 54
     txt-page AT ROW 2.54 COL 79 COLON-ALIGNED WIDGET-ID 36
     btn-consulta AT ROW 1.5 COL 91 WIDGET-ID 8
     txt-codalmac AT ROW 2.46 COL 13.86 COLON-ALIGNED WIDGET-ID 24
     txt-nombre1 AT ROW 2.46 COL 23.57 COLON-ALIGNED NO-LABEL WIDGET-ID 26
     txt-codinv AT ROW 3.42 COL 13.86 COLON-ALIGNED WIDGET-ID 28
     txt-totpag AT ROW 3.58 COL 95 COLON-ALIGNED NO-LABEL WIDGET-ID 50
     br_table AT ROW 5.04 COL 4
     btn_ingresa AT ROW 6.38 COL 87 WIDGET-ID 12
     btn-dif AT ROW 7.85 COL 87 WIDGET-ID 42
     btn-lista AT ROW 9.35 COL 87.14 WIDGET-ID 46
     btn-grabar AT ROW 10.88 COL 87.14 WIDGET-ID 44
     btn-cancelar AT ROW 12.42 COL 87.14 WIDGET-ID 40
     btn-exit AT ROW 14.08 COL 90 WIDGET-ID 38
     BUTTON-2 AT ROW 18.5 COL 88 WIDGET-ID 52
     "Total Pág:" VIEW-AS TEXT
          SIZE 8.86 BY .5 AT ROW 3.69 COL 87 WIDGET-ID 48
          FONT 6
...
DO:
    ...
    ASSIGN
        ...
        txtZona.
/*   Ic - Abr/2013
    IF LOOKUP(s-codalm,"11,22,85") > 0 THEN DO:
        MESSAGE "No se puede ingresar datos a este almacen"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        RETURN "ADM-ERROR".
    END.
  */

    IF txtZona = "" AND txt-page <= 0 THEN DO:
       MESSAGE "Ingrese Zona ó Pagina ó Ambas".
    END.
    ELSE DO:
        /*Carga Temporal*/
        RUN Borra-Temporal. 
        ...
        IF lVal THEN DO:
            IF LRecon THEN DO:
                ...
                lVal = NO.
            END.
            ELSE IF lState THEN DO:
                MESSAGE ' Ya se registro CONTEO para' SKIP
...
ON CHOOSE OF BUTTON-2 IN FRAME F-Main /* Hojas Reconteo */
DO:
    ASSIGN txt-page txt-totpag.
    RUN Carga-Impresion.
    RUN Imprime. 
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL txtZona B-table-Win
ON LEAVE OF txtZona IN FRAME F-Main /* Zona */
DO:
    txtzona:SCREEN-VALUE = REPLACE(txtzona:SCREEN-VALUE,"'","-").  
END.
    ...
    FOR EACH tt-AlmDInv NO-LOCK:
        ...
        IF AVAILABLE AlmDInv THEN DO:
            ASSIGN
                ...
                AlmDInv.CodUserCon = txt-codinv  /*Contado por */
...
/*
    FIND FIRST AlmCInv WHERE AlmCInv.CodCia = s-CodCia
        ...
        AND (txtZona = "" OR AlmCInv.CodAlma  = txtZona:SCREEN-VALUE IN FRAME {&FRAME-NAME} )
        AND (txt-page = 0 OR AlmCInv.Nropagina = INT(txt-page:SCREEN-VALUE IN FRAME {&FRAME-NAME}))
...
*/
    FOR FIRST AlmCInv WHERE AlmCInv.CodCia = s-CodCia
        ...
        RUN lib/logtabla ('almcinv',
                          almcinv.codalm + '|' + STRING(almcinv.nropag) + '|' + 'CONTEO' ,
                          'CREATE').

        ASSIGN
            AlmCInv.SwConteo     = YES
            AlmCInv.CodUser      = s-user-id
            /*AlmCInv.CodAlma      = txt-codalmac /*supervisado por*/*/
            AlmCInv.CodUserCon   = txt-codinv   /*contado por */         
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH AlmDInv USE-INDEX Llave01
        ...
        AND AlmDInv.NomCia    =  cNomCia NO-LOCK:
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH AlmDInv OF AlmCInv NO-LOCK:
        FIND FIRST tt-AlmDInv WHERE tt-AlmDInv.CodCia = AlmDInv.CodCia
            ...
            AND tt-AlmDInv.Codubi    = AlmDInv.Codubi
...
------------------------------------------------------------------------------*/
  ...
  RB-REPORT-NAME = 'Listado de Reconteo_2a Draft'.
  
  RB-INCLUDE-RECORDS = 'O'.
  RB-FILTER = "w-report.task-no = " + STRING(S-TASK-NO).

  RB-OTHER-PARAMETERS = "s-nomcia=" + s-nomcia +
                        "~ncNropage = " + STRING(txt-totpag).
