...
DEF TEMP-TABLE Detalle
    ...
    FIELD DesFam LIKE Almtfami.desfam LABEL 'NOM. FAM.'
    FIELD SubFam LIKE Almmmatg.codfam LABEL 'SUB-FAMILIA'
    FIELD DesSub LIKE Almsfami.dessub LABEL 'NOM. SUB-FAM.'
    FIELD CodPro AS CHAR FORMAT 'x(11)' LABEL 'PROVEEDOR' INIT ""
    FIELD NomPro AS CHAR FORMAT 'x(30)' LABEL 'NOM. PROVEE.' INIT ""
    FIELD CodMat LIKE Almmmatg.codmat LABEL 'CODIGO'
    FIELD DesMat LIKE Almmmatg.desmat LABEL 'DESCRIPCION'
    FIELD DesMar LIKE Almmmatg.desmar LABEL 'MARCA'
    FIELD UndStk LIKE Almmmatg.undstk LABEL 'UNIDAD'
    FIELD CodAlm LIKE Almacen.codalm LABEL 'ALMACEN'
    FIELD NomAlm LIKE Almacen.Descripcion LABEL 'NOM. ALM.'
    FIELD StkAct LIKE Almmmate.stkact LABEL 'STOCK'
    FIELD StkCom AS DEC LABEL 'STOCK COMPROMETIDO'   INIT 0
    FIELD StkDis AS DEC LABEL "Stock Disponible" INIT 0
    FIELD StkTra AS DEC LABEL 'Transito por Transferencia'   INIT 0
    FIELD StkCmp AS DEC LABEL 'Transito por Compras' INIT 0
    FIELD MaxCampana   LIKE Almmmate.VCtMn1 LABEL 'MAX. CAMPAÑA'
    FIELD MaxNoCampana LIKE Almmmate.VCtMn2 LABEL 'MAX.NO CAMPAÑA'
    FIELD EmpRepo   AS DEC  LABEL "Empaque Reposicion" INIT 0
    FIELD EmpInn    AS DEC  LABEL "Empaque Inner" INIT 0
    FIELD EmpMas    AS DEC  LABEL "Empaque Master" INIT 0
    FIELD flgcomercial    AS CHAR   FORMAT 'x(30)'  LABEL "Indice Comercial" INIT ""
    FIELD cmpClfGrl AS CHAR FORMAT 'x(3)'  LABEL "Campaña-Clasf Gral"   INIT ""
    FIELD cmpClfUti AS CHAR FORMAT 'x(3)'  LABEL "Campaña-Clasf Utilex" INIT ""
    FIELD cmpClfMay AS CHAR FORMAT 'x(3)'  LABEL "Campaña-Clasf Mayorista"  INIT ""    
    FIELD nocmpClfGrl AS CHAR FORMAT 'x(3)'  LABEL "NO Campaña-Clasf Gral"   INIT ""
    FIELD nocmpClfUti AS CHAR FORMAT 'x(3)'  LABEL "NO Campaña-Clasf Utilex" INIT ""
    FIELD nocmpClfMay AS CHAR FORMAT 'x(3)'  LABEL "NO Campaña-Clasf Mayorista"  INIT ""    
    FIELD CostoKardex    AS DEC  LABEL "Costo Kardex" INIT 0
    FIELD PesMat AS DEC LABEL 'Peso kg' INIT 0
    FIELD Libre_d02 AS DEC LABEL 'Volumen cm3' INIT 0
    .
...
DEFINE TEMP-TABLE tmp-tabla
    ...
    FIELD t-NomCli LIKE FacCPedi.NomCli LABEL "Cliente" FORMAT "x(35)"
...
&Scoped-Define ENABLED-OBJECTS R-Tipo EDITOR-CodFam BUTTON-1 DesdeC ~
F-marca1 F-provee1 EDITOR-CodAlm BUTTON-3 BUTTON-5 TipoStk BUTTON-4 ~
Btn_Cancel 
&Scoped-Define DISPLAYED-OBJECTS R-Tipo EDITOR-CodFam DesdeC F-marca1 ~
F-provee1 EDITOR-CodAlm FILL-IN-Archivo TipoStk 
...
DEFINE VARIABLE EDITOR-CodFam AS CHARACTER 
     VIEW-AS EDITOR SCROLLBAR-VERTICAL
     SIZE 49 BY 2.42 NO-UNDO.
...
DEFINE FRAME F-Main
     R-Tipo AT ROW 1 COL 82 NO-LABEL
     EDITOR-CodFam AT ROW 1.27 COL 21 NO-LABEL WIDGET-ID 18
     BUTTON-1 AT ROW 1.27 COL 71 WIDGET-ID 22
     DesdeC AT ROW 3.96 COL 19 COLON-ALIGNED
     F-marca1 AT ROW 4.92 COL 19 COLON-ALIGNED
     F-provee1 AT ROW 5.88 COL 19 COLON-ALIGNED
     EDITOR-CodAlm AT ROW 6.77 COL 21 NO-LABEL WIDGET-ID 14
     BUTTON-3 AT ROW 6.77 COL 70
     FILL-IN-Archivo AT ROW 9.19 COL 19 COLON-ALIGNED WIDGET-ID 8
     BUTTON-5 AT ROW 9.19 COL 70 WIDGET-ID 10
     TipoStk AT ROW 10.81 COL 21 NO-LABEL
     BUTTON-4 AT ROW 12.04 COL 2 WIDGET-ID 6
     Btn_Cancel AT ROW 12.04 COL 19
     "Familias:" VIEW-AS TEXT
          SIZE 9 BY .5 AT ROW 1.27 COL 11 WIDGET-ID 20
     "Almacenes:" VIEW-AS TEXT
          SIZE 9 BY .5 AT ROW 6.77 COL 12 WIDGET-ID 16
...
DO:
    ...
    x-Familias = EDITOR-CodFam:SCREEN-VALUE.
    RUN alm/d-tfamilias (INPUT TABLE T-LINEA, INPUT-OUTPUT x-Familias).
    EDITOR-CodFam:SCREEN-VALUE = x-Familias.
END.
...
ON CHOOSE OF BUTTON-4 IN FRAME F-Main /* TEXTO */
DO:
  RUN Asigna-Variables.
  /* CONSISTENCIA */
  IF NUM-ENTRIES(EDITOR-CodFam) = 0 THEN DO:
    MESSAGE 'Debe seleccionar al menos 1 familia de producto'
  ...
  DEF VAR c-xls-file AS CHAR INIT 'Archivo_Texto' NO-UNDO.
  ...
  IF rpta = YES THEN DO:
      ...
      RUN lib/tt-file (TEMP-TABLE Detalle:HANDLE, c-xls-file, cOptions).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Familias W-Win 
PROCEDURE Carga-Familias :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
  ...
  ELSE DO:
      FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
          ...
          AND ((TRUE <> (f-provee1 > "")) OR Almmmatg.CodPr1 = F-provee1)
          AND ((TRUE <> (R-Tipo > '')) OR Almmmatg.TpoArt = R-Tipo)
          AND LOOKUP(almmmatg.codfam, EDITOR-CodFam) > 0
...
------------------------------------------------------------------------------*/
  DISPLAY R-Tipo EDITOR-CodFam DesdeC F-marca1 F-provee1 EDITOR-CodAlm 
          FILL-IN-Archivo TipoStk 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE R-Tipo EDITOR-CodFam BUTTON-1 DesdeC F-marca1 F-provee1 EDITOR-CodAlm 
         BUTTON-3 BUTTON-5 TipoStk BUTTON-4 Btn_Cancel 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
DO WITH FRAME {&FRAME-NAME}:
    ENABLE ALL EXCEPT FILL-IN-Archivo.
END.
...
DO WITH FRAME {&FRAME-NAME}:
  ASSIGN DesdeC 
         TipoStk R-Tipo
         F-marca1 F-provee1 FILL-IN-Archivo.
  
  FILL-IN-Archivo = ''.
  DISPLAY FILL-IN-Archivo.
END.
