...
Almmmatg.DesMat Almmmatg.DesMar Almmmatg.UndStk ITEM.CanDes 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almmmatg.UndStk COLUMN-LABEL "Unidad" FORMAT "X(4)":U WIDTH 6.72
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 102 BY 5.38
         FONT 4
         TITLE "KITS".
...
"Almmmatg.UndStk" "Unidad" ? "character" ? ? ? ? ? ? no ? no no "6.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON 'RETURN':U OF ITEM.CanDes, ITEM.codmat
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Componentes B-table-Win 
PROCEDURE Carga-Componentes :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH B-ITEM:
    FIND Almckits OF B-ITEM NO-LOCK.
    FOR EACH Almdkit OF Almckit NO-LOCK WHERE Almdkit.Cantidad > 0,
        FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
        AND Almmmatg.codmat = AlmDKits.codmat2:
        FIND ITEM-1 WHERE ITEM-1.codmat = AlmDKits.codmat2 NO-ERROR.
        IF NOT AVAILABLE ITEM-1 THEN CREATE ITEM-1.
        ASSIGN
            ITEM-1.codcia = s-codcia
            ITEM-1.codmat = AlmDKits.codmat2
            ITEM-1.candes = ITEM-1.candes + B-ITEM.CanDes * AlmDKits.Cantidad
...
------------------------------------------------------------------------------*/
    ...
    IF NOT AVAILABLE Almckits THEN DO:
        MESSAGE 'NO es un código de KIT' VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO ITEM.CodMat.
        RETURN 'ADM-ERROR'.
    END.
    FIND FIRST Almdkits OF Almckits WHERE AlmDKits.Cantidad > 0 NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almdkits THEN DO:
        MESSAGE 'KIT mal configurado' VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO ITEM.CodMat.
        RETURN 'ADM-ERROR'.
    END.
    FOR EACH Almdkit OF Almckit NO-LOCK WHERE Almdkit.Cantidad > 0:
        FIND Almmmate WHERE Almmmate.CodCia = S-CodCia AND
             Almmmate.CodAlm = S-CodAlm AND
             Almmmate.CodMat = AlmDKits.codmat2
             NO-LOCK NO-ERROR.
        IF NOT AVAILABLE Almmmate THEN DO:
            MESSAGE "Componente del KIT:" AlmDKits.codmat2
                "no está asignado al almacén " s-CodAlm VIEW-AS ALERT-BOX ERROR.
            APPLY "ENTRY" TO ITEM.CodMat.
            RETURN "ADM-ERROR".   
        END. 
    END.
    ...
    FOR EACH Almdkit OF Almckit NO-LOCK WHERE Almdkit.Cantidad > 0:
        ...
        FIND B-MATG WHERE B-MATG.codcia = s-codcia
            AND B-MATG.codmat = AlmDKits.codmat2
            NO-LOCK NO-ERROR.
        IF B-MATG.MonVta = 1 THEN x-CtoLis = B-MATG.CtoLis.
        ELSE x-CtoLis = B-MATG.CtoLis * B-MATG.TpoCmb.
        x-CtoAnt = x-CtoAnt + x-CtoLis * AlmDKits.Cantidad * f-Factor-1.
    END.
    x-pordif = ((x-ctoant - x-ctomat) / x-ctoant ) * 100.
