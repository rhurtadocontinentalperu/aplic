...
DEFINE TEMP-TABLE tt-datos LIKE FacDPedi.
...
DEFINE VARIABLE cCodDoc AS CHARACTER   NO-UNDO INIT "O/D".
...
Almmmatg.DesMar tt-datos.CanPed tt-datos.Libre_c01 
&Scoped-define ENABLED-FIELDS-IN-QUERY-r_table tt-datos.CanPed 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS r_table B-table-Win _STRUCTURED
  QUERY r_table NO-LOCK DISPLAY
      ...
      tt-datos.CanPed FORMAT ">,>>>,>>9.9999":U
      tt-datos.Libre_c01 COLUMN-LABEL "Bultos" FORMAT "x(60)":U
            WIDTH 20
  ENABLE
      tt-datos.CanPed
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 96 BY 15.62
         FONT 4.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tt-datos T "?" ? INTEGRAL FacDPedi
   END-TABLES.
 */
...
ASSIGN 
       tt-datos.Libre_c01:VISIBLE IN BROWSE r_table = FALSE.
...
"Temp-Tables.tt-datos.codmat" "Articulo" ? "character" ? ? ? ? ? ? no ? no no "7" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almmmatg.DesMat
"INTEGRAL.Almmmatg.DesMat" ? ? "character" ? ? ? ? ? ? no ? no no "50" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.Almmmatg.DesMar
"INTEGRAL.Almmmatg.DesMar" "Marca" ? "character" ? ? ? ? ? ? no ? no no "20" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-datos.CanPed
"Temp-Tables.tt-datos.CanPed" ? ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.tt-datos.Libre_c01
"Temp-Tables.tt-datos.Libre_c01" "Bultos" ? "character" ? ? ? ? ? ? no ? no no "20" no no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is NOT OPENED
*/  /* BROWSE r_table */
&ANALYZE-RESUME
...
&Scoped-define SELF-NAME tt-datos.Libre_c01
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tt-datos.Libre_c01 r_table _BROWSE-COLUMN B-table-Win
ON F10 OF tt-datos.Libre_c01 IN BROWSE r_table /* Bultos */
DO:
    DEFINE VARIABLE c-lisempaque    AS CHAR.
    DEFINE VARIABLE c-liscantidad   AS CHAR.
  
    MESSAGE "Varios Bultos jejeje" SKIP tt-datos.canped.
    RUN alm\w-detbultos.w(tt-datos.canped, OUTPUT c-lisempaque, OUTPUT c-liscantidad).
END.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH faccpedi WHERE faccpedi.codcia = s-codcia
        ...
        AND faccpedi.coddoc = cCodDoc 
        AND faccpedi.nroped = s-nroref NO-LOCK,
        EACH facdpedi OF faccpedi NO-LOCK:
            
        CREATE tt-datos.
        BUFFER-COPY facdpedi TO tt-datos
            ASSIGN 
                tt-datos.libre_d01 = 0
                tt-datos.libre_c01 = "".
        ...
        ***/
        ...
        DO iInt = 1 TO tt-datos.numepq:
            IF tt-datos.candes > d-undemp THEN 
                x-undemp = STRING(tt-datos.candev) + " " + VtaProxCli.UndStk. /*Cantidad Empaque*/
            ELSE 
                x-undemp = STRING(tt-datos.candes - d-undemp) + " " + VtaProxCli.UndStk. /*Cantidad Empaque*/
...
------------------------------------------------------------------------------*/
    ...
    IF INTEGER(tt-datos.libre_c01:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > s-numero THEN DO:
        MESSAGE "Excede el número de bultos"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        APPLY "ENTRY" TO tt-datos.libre_c01 IN BROWSE {&browse-NAME}.
        RETURN "ADM-ERROR".
    END.
