...
DEFINE VARIABLE h_qtransfxreposicion AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-trfsal-02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-movmto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vtransfxreposicion AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     RECT-1 AT ROW 4.27 COL 2 WIDGET-ID 12
     RECT-28 AT ROW 1 COL 1 WIDGET-ID 14
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 95 BY 19.69 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "TRANSFERENCIA POR REPOSICION"
         HEIGHT             = 19.69
         WIDTH              = 95
         MAX-HEIGHT         = 21.81
         MAX-WIDTH          = 115.57
...
ON END-ERROR OF W-Win /* TRANSFERENCIA POR REPOSICION */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* TRANSFERENCIA POR REPOSICION */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-movmto ).
       RUN set-position IN h_v-movmto ( 1.19 , 20.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 2.73 , 19.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv13 ).
       RUN set-position IN h_p-updv13 ( 2.73 , 37.00 ) NO-ERROR.
       RUN set-size IN h_p-updv13 ( 1.62 , 58.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtransfxreposicion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vtransfxreposicion ).
       RUN set-position IN h_vtransfxreposicion ( 4.65 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.31 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'q-movsaltrf.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-movsaltrf ).
       RUN set-position IN h_q-movsaltrf ( 1.00 , 84.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.50 , 7.72 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'qtransfxreposicion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_qtransfxreposicion ).
       RUN set-position IN h_qtransfxreposicion ( 2.73 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 16.14 ) */

       /* Links to SmartViewer h_v-movmto. */
       RUN add-link IN adm-broker-hdl ( h_q-movsaltrf , 'Record':U , h_v-movmto ).

       /* Links to SmartViewer h_vtransfxreposicion. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_vtransfxreposicion ).
       RUN add-link IN adm-broker-hdl ( h_qtransfxreposicion , 'Record':U , h_vtransfxreposicion ).

       /* Links to SmartQuery h_q-movsaltrf. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-movsaltrf ).

       /* Links to SmartQuery h_qtransfxreposicion. */
       RUN add-link IN adm-broker-hdl ( h_p-navico-2 , 'Navigation':U , h_qtransfxreposicion ).
       RUN add-link IN adm-broker-hdl ( h_q-movsaltrf , 'Record':U , h_qtransfxreposicion ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_vtransfxreposicion ,
             h_p-updv13 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_qtransfxreposicion ,
             h_vtransfxreposicion , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_b-trfsal-01 ( 8.31 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-trfsal-01 ( 12.12 , 85.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-trfsal-01. */
       RUN add-link IN adm-broker-hdl ( h_qtransfxreposicion , 'Record':U , h_b-trfsal-01 ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_t-trfsal-02 ( 8.31 , 2.00 ) NO-ERROR.
...
CASE L-Handle:
    WHEN "Pagina1"  THEN DO:
         RUN select-page(1).
         RUN adm-enable IN h_qtransfxreposicion.
         RUN adm-open-query IN h_b-trfsal-01.
    END.
    WHEN "Pagina2"  THEN DO:
         RUN select-page(2).
         RUN adm-disable IN h_qtransfxreposicion.
