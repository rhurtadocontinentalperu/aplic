...
AlmDIncidencia.CanInc AlmDIncidencia.Incidencia fNroRA() @ x-NroRA 
  ...
  QUERY br_table NO-LOCK DISPLAY
      ...
      AlmDIncidencia.Incidencia FORMAT "x(12)":U VIEW-AS COMBO-BOX INNER-LINES 5
                      LIST-ITEM-PAIRS "Mal Estado","M",
                                      ...
                                      "Sobrante","S"
                      DROP-DOWN-LIST 
      fNroRA() @ x-NroRA COLUMN-LABEL "R/A" FORMAT "x(12)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 126 BY 16.96
...
"fNroRA() @ x-NroRA" "R/A" "x(12)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Almcrepo NO-LOCK WHERE almcrepo.CodCia = AlmCIncidencia.CodCia AND 
      ...
      RETURN STRING(almdrepo.NroSer,'999') + STRING(almdrepo.NroDoc, '99999999').
  END.
