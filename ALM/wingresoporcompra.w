...
DEFINE NEW SHARED VARIABLE S-NROSER  AS INTEGER INIT 0.
...
&Scoped-Define ENABLED-OBJECTS RECT-6 COMBO-NroSer 
...
DEFINE VARIABLE h_b-tmpcmp AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv05 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-ingcmp AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-movalm AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-movmto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vingresoporcompra AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BtnImp 
     LABEL "Imprimir para Contabilidad/Finanzas" 
     SIZE 30 BY .96.

DEFINE VARIABLE COMBO-NroSer AS CHARACTER FORMAT "X(3)":U INITIAL "000" 
     LABEL "Serie" 
...
DEFINE FRAME F-Main
     COMBO-NroSer AT ROW 7.35 COL 69.71 WIDGET-ID 4
     BtnImp AT ROW 17.88 COL 63 WIDGET-ID 8
     RECT-6 AT ROW 7.15 COL 69 WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 108.29 BY 18.12
         FONT 4.
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* SETTINGS FOR BUTTON BtnImp IN FRAME F-Main
   NO-ENABLE                                                            */
...
ON END-ERROR OF sW-Win /* Ingresos de Almacen */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL sW-Win sW-Win
ON WINDOW-CLOSE OF sW-Win /* Ingresos de Almacen */
DO:
...
&Scoped-define SELF-NAME BtnImp
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BtnImp sW-Win
ON CHOOSE OF BtnImp IN FRAME F-Main /* Imprimir para Contabilidad/Finanzas */
DO:
  RUN ue_imp_contafinan IN h_vingresoporcompra.
END.
...
DO:
    ...
    RUN dispatch IN h_q-movalm ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/vingresoporcompra.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vingresoporcompra ).
       RUN set-position IN h_vingresoporcompra ( 2.31 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.81 , 89.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv05 ).
       RUN set-position IN h_p-updv05 ( 7.15 , 1.00 ) NO-ERROR.
       RUN set-size IN h_p-updv05 ( 1.42 , 49.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 7.15 , 50.00 ) NO-ERROR.
       RUN set-size IN h_p-navico-2 ( 1.42 , 19.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/q-ingcmp.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-ingcmp ).
       RUN set-position IN h_q-ingcmp ( 1.00 , 60.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.00 , 15.86 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/q-movalm.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-movalm ).
       RUN set-position IN h_q-movalm ( 7.35 , 82.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 12.00 ) */

       /* Links to SmartViewer h_v-movmto. */
       RUN add-link IN adm-broker-hdl ( h_q-ingcmp , 'Record':U , h_v-movmto ).

       /* Links to SmartViewer h_vingresoporcompra. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_vingresoporcompra ).
       RUN add-link IN adm-broker-hdl ( h_q-movalm , 'Record':U , h_vingresoporcompra ).

       /* Links to SmartQuery h_q-movalm. */
       RUN add-link IN adm-broker-hdl ( h_p-navico-2 , 'Navigation':U , h_q-movalm ).
       RUN add-link IN adm-broker-hdl ( h_q-ingcmp , 'Record':U , h_q-movalm ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-movmto ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vingresoporcompra ,
             h_v-movmto , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_vingresoporcompra , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico-2 ,
             h_p-updv05 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-ingcmp ,
             BtnImp:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-movalm ,
             h_q-ingcmp , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ingrso ).
       RUN set-position IN h_b-ingrso ( 8.69 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-ingrso ( 8.62 , 106.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-ingrso. */
       RUN add-link IN adm-broker-hdl ( h_q-movalm , 'Record':U , h_b-ingrso ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/b-tmpcmp.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpcmp ).
       RUN set-position IN h_b-tmpcmp ( 8.62 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpcmp ( 7.27 , 97.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 16.12 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.35 , 33.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-tmpcmp. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-tmpcmp ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tmpcmp ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
...
CASE L-Handle:
    WHEN "browse" THEN DO:
         ...
         ELSE RUN dispatch IN h_b-tmpcmp ('open-query':U).
      END.
    WHEN "Pagina1" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN select-page(1).
         COMBO-NroSer:SENSITIVE = YES.
         RUN dispatch IN h_q-movalm ('enable':U).
      END.
    WHEN "Pagina2" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN select-page(2).
         COMBO-NroSer:SENSITIVE = NO.
         RUN dispatch IN h_q-movalm ('disable':U).
      END.
END CASE.
