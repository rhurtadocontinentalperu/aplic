...
"T-MoviAlmacen.Peso @ x-Peso" "Peso kg" ">>>,>>9.99" ? ? ? ? ? ? ? no ? no no "2.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH b-ooMoviAlmacen WHERE b-ooMoviAlmacen.CodCia = ooMoviAlmacen.CodCia
        ...
        AND b-ooMoviAlmacen.NroSer = ooMoviAlmacen.NroSer
        AND b-ooMoviAlmacen.NroDoc = ooMoviAlmacen.NroDoc 
        AND b-ooMoviAlmacen.FlagMigracion = "N" EXCLUSIVE-LOCK ON ERROR UNDO, THROW:
        FIND Almdmov OF Almcmov WHERE Almdmov.CodMat = b-ooMoviAlmacen.CodMat NO-LOCK NO-ERROR.
        IF AVAILABLE Almdmov THEN DO:
            MESSAGE "ERROR:Detalle ya registrado (almacen" ooMoviAlmacen.CodAlm "mov" 
                ooMoviAlmacen.TipMov ooMoviAlmacen.CodMov "numero" ooMoviAlmacen.NroSer
                ooMoviAlmacen.NroDoc "producto" b-ooMoviAlmacen.CodMat ")" SKIP
                'Proceso abortado' VIEW-AS ALERT-BOX ERROR.
        ...
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            MESSAGE "ERROR:Stock (almacen" ooMoviAlmacen.CodAlm "mov" 
                ooMoviAlmacen.TipMov ooMoviAlmacen.CodMov "numero" ooMoviAlmacen.NroSer
                ooMoviAlmacen.NroDoc "producto" b-ooMoviAlmacen.CodMat ")" SKIP
                'Proceso Abortado' VIEW-AS ALERT-BOX ERROR.
            UNDO CICLO, RETURN 'ADM-ERROR'.
        ...
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            MESSAGE "ERROR:Kardex (almacen" ooMoviAlmacen.CodAlm "mov" 
                ooMoviAlmacen.TipMov ooMoviAlmacen.CodMov "numero" ooMoviAlmacen.NroSer
                ooMoviAlmacen.NroDoc "producto" b-ooMoviAlmacen.CodMat ")" SKIP
                'Proceso Abortado' VIEW-AS ALERT-BOX ERROR.
            UNDO CICLO, RETURN 'ADM-ERROR'.
