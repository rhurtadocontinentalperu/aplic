...
&Scoped-Define ENABLED-OBJECTS RECT-60 RECT-56 RECT-58 RECT-59 RECT-61 ~
F-Catconta Btn_OK DesdeC HastaC F-marca1 F-provee1 Btn_Cancel D-Corte ~
R-Tipo I-CodMon T-Resumen 
&Scoped-Define DISPLAYED-OBJECTS F-Desconta F-Catconta DesdeC HastaC ~
F-marca1 F-provee1 D-Corte R-Tipo I-CodMon T-Resumen 
...
DEFINE FRAME F-Main
     ...
     R-Tipo AT ROW 6.19 COL 16.86 NO-LABEL
     I-CodMon AT ROW 7.54 COL 17 NO-LABEL
     T-Resumen AT ROW 8.85 COL 17.29
     "Criterio de Seleccion" VIEW-AS TEXT
          SIZE 17.72 BY .5 AT ROW 1 COL 5.43
          FONT 6
     "Moneda" VIEW-AS TEXT
          SIZE 6.14 BY .5 AT ROW 7.96 COL 8.57
     "Estado" VIEW-AS TEXT
          SIZE 5.57 BY .5 AT ROW 6.46 COL 9.72
     RECT-60 AT ROW 1.19 COL 1.57
     RECT-56 AT ROW 6 COL 16
     RECT-58 AT ROW 7.42 COL 16
     RECT-59 AT ROW 8.62 COL 16.14
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.CodCia = S-CODCIA 
      ...
      FIND LAST Almstk WHERE Almstk.Codcia = S-CODCIA AND
                        Almstk.Codmat = Almmmatg.Codmat AND
                        ALmstk.FchDoc <= D-Corte
                        NO-LOCK NO-ERROR.
                        
      IF NOT AVAILABLE Almstk OR (AVAILABLE Almstk AND Almstk.Stkactcb = 0 ) THEN NEXT. 
      ...
      IF AVAILABLE Almstk THEN DO:
         ...
         IF I-CodMon = 1 THEN F-VALCTO = Almstk.VctoMn1Cbd.
         IF I-CodMon = 2 THEN F-VALCTO = Almstk.VctoMn2Cbd. 

      END.         
      ELSE F-STKGEN = 0.
      IF F-STKGEN = 0 THEN F-VALCTO = 0.
      IF F-STKGEN = 0 THEN NEXT.
      
      F-VALCTO = IF F-STKGEN <> 0 THEN F-VALCTO ELSE 0.
      F-PRECTO = IF F-STKGEN <> 0 THEN F-VALCTO / F-STKGEN ELSE 0.
      F-PESGEN = F-STKGEN * Almmmatg.Pesmat.

      /*
      IF ( F-PRECTO = 0 OR F-PRECTO = ? ) THEN DO:
          F-PRECTO = ALMMMATG.CTOLIS.
          IF AVAILABLE gn-tcmb THEN DO:
             IF I-CodMon <> Almmmatg.MonVta THEN DO:
                IF I-CodMon = 1 THEN F-PRECTO = F-PRECTO * gn-tcmb.venta. 
                IF I-CodMon = 2 THEN F-PRECTO = F-PRECTO / gn-tcmb.venta. 
             END.     
             F-VALCTO = F-PRECTO * F-STKGEN.
          END.          
      END.
      */
      ...
      /***************************************************************/
      IF Almmmatg.Catconta[1] = "CR" OR
            Almmmatg.Catconta[1] = "SP" THEN DO:
            F-PRECTO = 0. 
            F-VALCTO = 0. 
            F-COSTO  = 0.
            FT-COSTO = 0.          
...
------------------------------------------------------------------------------*/
  DISPLAY F-Desconta F-Catconta DesdeC HastaC F-marca1 F-provee1 D-Corte R-Tipo 
          I-CodMon T-Resumen 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-60 RECT-56 RECT-58 RECT-59 RECT-61 F-Catconta Btn_OK DesdeC 
         HastaC F-marca1 F-provee1 Btn_Cancel D-Corte R-Tipo I-CodMon T-Resumen 
      WITH FRAME F-Main IN WINDOW W-Win.
