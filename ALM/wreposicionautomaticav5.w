...
DO:
  ...
  /*RUN Generar-Pedidos.*/
  RUN Generar-Pedidos IN h_b-pedrepautv5
...
CASE pOk:
    WHEN YES THEN DO:   /* ES UN CD */
        ...
        FOR EACH T-MATG NO-LOCK:
            ...
            FOR EACH TabGener NO-LOCK WHERE TabGener.CodCia = s-CodCia
                ...
                pStockTransito = fStockTransito(Almmmate.CodAlm, Almmmate.CodMat).
                pCompraTransito = fCompraTransito(Almmmate.CodAlm, Almmmate.CodMat).
                ASSIGN
    ...
    WHEN NO THEN DO:   /* ES UNA TIENDA */
        FOR EACH T-MATG NO-LOCK,
            ...
            ASSIGN
                /* Stock Disponible Solicitante */
                T-MATE.StkAct = (Almmmate.StkAct - pStkComprometido)
...
FOR EACH T-DREPO, FIRST Almmmatg OF T-DREPO NO-LOCK:
    ASSIGN
        T-DREPO.DesMat = Almmmatg.DesMat.
    FIND FIRST Almmmate WHERE Almmmate.CodCia = s-codcia
...
EMPTY TEMP-TABLE T-DREPO-3.     /* Acumula los que #Items >= 10 */
EMPTY TEMP-TABLE T-DREPO-4.     /* Acumula los que #Items < 10  */
EMPTY TEMP-TABLE T-DREPO-5.     /* Lo que no ha generado R/A  */
DEF BUFFER B-MATG FOR Almmmatg.
...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    ...
    /* RHC 12/05/2017 SI UNA MARCA ESPECIFICA TIENE IGUAL O MAS DE 10 ITEMS => GENERA UN REPOSICION */
/*     RUN Resumimos-por-Marca IN h_b-pedrepautv5                 */
/*     ( INPUT s-acceso-total /* LOGICAL */,                      */
/*       INPUT-OUTPUT TABLE T-DREPO-2,                            */
/*       INPUT-OUTPUT TABLE T-DREPO-3,                            */
/*       INPUT-OUTPUT TABLE T-DREPO-4).                           */
/*                                                                */
/*     FOR EACH T-DREPO:                                          */
/*         CREATE T-DREPO-5.                                      */
/*         BUFFER-COPY T-DREPO TO T-DREPO-5.                      */
/*     END.                                                       */
/*                                                                */
/*     /* Parte 1: Los resumidos por MARCA */                     */
/*     EMPTY TEMP-TABLE T-DREPO.                                  */
...
/* FOR EACH T-DREPO-5:                   */
/*     CREATE T-DREPO.                   */
/*     BUFFER-COPY T-DREPO-5 TO T-DREPO. */
/* END.                                  */
RELEASE Faccorre.
RELEASE almcrepo.
RELEASE almdrepo.

RUN Limpiar-Filtros IN h_b-pedrepautv5.
RUN adm-open-query IN h_b-pedrepautv5.
...
CASE TRUE:
    WHEN pOkSolicitante = YES AND pOk = YES THEN DO:   /* ES UN CD */
        ...
        FOR EACH TabGener NO-LOCK WHERE TabGener.CodCia = s-CodCia
            ...
            ASSIGN
                ...
                T-MATE-2.DesStkDis = T-MATE-2.DesStkDis + (Almmmate.StkAct - pStkComprometido)
        ...
        IF AVAILABLE T-MATE-2 THEN DO:
            ...
            ELSE DO:
                ...
                IF ((T-MATE-2.StkAct + T-MATE-2.StkRep + T-MATE-2.StkActCbd) - T-MATE-2.StkMin) > 
                    ( Almmmate.StkAct - pStkComprometido ) THEN DO:
                    ASSIGN
                        /* Stock Disponible Despachante */
                        T-MATE-2.StkAct = (Almmmate.StkAct - pStkComprometido)
    ...
    OTHERWISE DO:    /* ES UNA TIENDA */
        ...
        ASSIGN
            ...
            T-MATE-2.DesStkDis = (Almmmate.StkAct - pStkComprometido)
