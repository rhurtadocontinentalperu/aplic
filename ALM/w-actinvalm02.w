...
DEFINE VARIABLE x-codalm AS CHARACTER FORMAT "X(256)":U INITIAL "85,22,11" 
     VIEW-AS FILL-IN 
     SIZE 24 BY 1
     BGCOLOR 15  NO-UNDO.

DEFINE VARIABLE x-codalm-2 AS CHARACTER FORMAT "X(256)":U INITIAL "85,22,11" 
     VIEW-AS FILL-IN 
     SIZE 24 BY 1
     BGCOLOR 15  NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "Actualizacion de Stocks CONTINENTAL - STANDFORD" VIEW-AS TEXT
          SIZE 71 BY 1.35 AT ROW 2.08 COL 2 WIDGET-ID 6
          FONT 9
    ...
    FOR EACH tmp-almdinv NO-LOCK
        ...
        ELSE DO:
            ASSIGN
                ...
                /*integral.almdinv.libre_d02   = tmp-almdinv.libre_d02.*/
        END.
    END.
END PROCEDURE.
    ...
    DO iInt = 1 TO NUM-ENTRIES(cAlmacen):
        FOR EACH cissac.almcinv WHERE cissac.almcinv.codcia = s-codcia
            ...
            FOR EACH cissac.almdinv OF cissac.almcinv EXCLUSIVE-LOCK:
                ...
                DISPLAY "PROCESANDO CISSAC " + cissac.almdinv.codmat  @ x-mensaje
                        WITH FRAME {&FRAME-NAME}.

                IF cissac.almdinv.qtyfisico < 0 THEN NEXT Detalle.               
                FIND FIRST integral.almdinv WHERE integral.almdinv.codcia = s-codcia
                    AND integral.almdinv.codalm    = "11x"
                    ...
                    /*AND integral.almdinv.libre_d02 <> 0 NO-ERROR.   Ic */
...
/*                     IF integral.almcinv.swconteo = NO THEN RETURN "adm-error". */
                    ...
                    /* Ic
                    IF integral.almdinv.libre_d02 > 0 THEN DO:
                        IF cissac.almdinv.qtyfisico <= integral.almdinv.libre_d02 THEN
                            ASSIGN
                                cissac.almdinv.qtyconteo   = cissac.almdinv.qtyfisico
                                cissac.almdinv.qtyreconteo = cissac.almdinv.qtyfisico
                                cissac.almdinv.libre_d02   = cissac.almdinv.qtyfisico
                                lPag = YES.
                        ELSE 
                            ASSIGN
                                cissac.almdinv.qtyconteo   = integral.almdinv.libre_d02
                                cissac.almdinv.qtyreconteo = integral.almdinv.libre_d02
                                cissac.almdinv.libre_d02   = integral.almdinv.libre_d02
                                lPag = YES.

                                integral.almdinv.libre_d02 = 
                                    (integral.almdinv.libre_d02 - cissac.almdinv.libre_d02).
                        END.
                    ELSE DO:
                         ASSIGN
                            cissac.almdinv.qtyconteo   = cissac.almdinv.qtyfisico
...
/*                     IF b-almcinv.swconteo = NO THEN RETURN "adm-error". */
                    ...
                    /*
                    IF b-almdinv.libre_d02 > 0 THEN DO:
                        IF integral.almdinv.qtyfisico <= b-almdinv.libre_d02 THEN
                            ASSIGN
                                integral.almdinv.qtyconteo   = integral.almdinv.qtyfisico
                                integral.almdinv.qtyreconteo = integral.almdinv.qtyfisico
                                integral.almdinv.libre_d02   = integral.almdinv.qtyfisico
                                lPag = YES.
                        ELSE 
                            ASSIGN
                                integral.almdinv.qtyconteo   = b-almdinv.libre_d02
                                integral.almdinv.qtyreconteo = b-almdinv.libre_d02
                                integral.almdinv.libre_d02   = b-almdinv.libre_d02
                                lPag = YES.
                        b-almdinv.libre_d02 = 
                            (b-almdinv.libre_d02 - integral.almdinv.libre_d02).
                    END.
                    ELSE DO:
                        ASSIGN
                            integral.almdinv.qtyconteo   = integral.almdinv.qtyfisico
...
***********/    
    MESSAGE "Pasa a Stock Sobrante".
    /*Completa Stocks Sobrantes*/
    Two:
    FOR EACH integral.almdinv WHERE integral.almdinv.codcia = s-codcia
        ...
        /*AND integral.almdinv.libre_d02 <> 0:   Ic*/  

        FIND FIRST b-almdinv WHERE b-almdinv.codcia = integral.almdinv.codcia
            AND LOOKUP(b-almdinv.codalm,"85,22") > 0
            /*AND LOOKUP(b-almdinv.codalm,"11,85,22") > 0*/
        ...
        IF AVAIL b-almdinv THEN DO:
            ASSIGN
                /*  Ic
                b-almdinv.qtyconteo   = b-almdinv.qtyconteo   + integral.almdinv.libre_d02
                b-almdinv.qtyreconteo = b-almdinv.qtyreconteo + integral.almdinv.libre_d02
                b-almdinv.libre_d02   = b-almdinv.libre_d02   + integral.almdinv.libre_d02.
                integral.almdinv.libre_d02 = 0.
                */
    ...
    FOR EACH integral.almdinv WHERE integral.almdinv.codcia = s-codcia
        AND integral.almdinv.codalm    = "11x"
        /*AND integral.almdinv.nomcia    = "CONTISTAND"  */
        AND integral.almdinv.nomcia    = "CONTI"  
        AND integral.almdinv.libre_f01 = x-fecha 
        /*AND integral.almdinv.nropagina >= 9000:*/
        AND integral.almdinv.nropagina >= 8000:
        CREATE tmp-almdinv.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Completa_Stocks_Sobrantes W-Win 
PROCEDURE Completa_Stocks_Sobrantes :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE cLisAlmConti AS CHARACTER   NO-UNDO.
    DEFINE VARIABLE iInt         AS INTEGER     NO-UNDO.

    cLisAlmConti = x-codalm.                                           
    Two:
    FOR EACH integral.almdinv WHERE integral.almdinv.codcia = s-codcia
        AND integral.almdinv.codalm = "11x"
        AND integral.almdinv.nomcia = "contistand"
        AND integral.almdinv.libre_d01 <> 0:                /*Articulos que conservan Stock*/

        /* AND integral.almdinv.libre_d02 <> 0: */

        DO iInt = 1 TO NUM-ENTRIES(cLisAlmConti):
            FIND FIRST b-almdinv WHERE b-almdinv.codcia = integral.almdinv.codcia
                AND b-almdinv.codalm = ENTRY(iInt,cLisAlmConti,",")
                AND b-almdinv.nomcia    = "CONTI"
                AND b-almdinv.codmat    = integral.almdinv.codmat
                /**** AND b-almdinv.qtyfisico < 0 ****/ NO-ERROR.
            IF AVAIL b-almdinv THEN DO:
