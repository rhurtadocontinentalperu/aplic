...
&SCOPED-DEFINE Condicion Almcmov.CodCia = Almtdocm.CodCia ~
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH Almcmov WHERE {&Condicion} NO-LOCK ~
    BY Almcmov.NroDoc DESCENDING INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Almcmov WHERE {&Condicion} NO-LOCK ~
    BY Almcmov.NroDoc DESCENDING INDEXED-REPOSITION.
...
DEFINE FRAME FRAME-A
     BUTTON-1 AT ROW 1 COL 9
     F-Numero AT ROW 1.19 COL 1.57 NO-LABEL
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND FIRST Almcmov WHERE {&Condicion} 
...
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND Almcmov WHERE
        ROWID(Almcmov) = OUTPUT-VAR-1 NO-LOCK NO-ERROR.
