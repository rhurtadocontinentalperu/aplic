...
RB-NUMBER-COPIES desdeF B-impresoras RB-BEGIN-PAGE R-Codmon hastaF ~
RB-END-PAGE Btn_OK Btn_Cancel Btn_Help 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-1 FILL-IN-3 RB-NUMBER-COPIES ~
desdeF RB-BEGIN-PAGE R-Codmon hastaF RB-END-PAGE 
...
DEFINE FRAME D-Dialog
     RADIO-SET-1 AT ROW 5.65 COL 3 NO-LABEL
     FILL-IN-3 AT ROW 1.38 COL 4.85
     RB-NUMBER-COPIES AT ROW 9 COL 10.72 COLON-ALIGNED
     desdeF AT ROW 3.31 COL 17.72 COLON-ALIGNED
     B-impresoras AT ROW 6.42 COL 16.14
     b-archivo AT ROW 7.54 COL 16.14
     RB-OUTPUT-FILE AT ROW 7.38 COL 20 COLON-ALIGNED NO-LABEL
     RB-BEGIN-PAGE AT ROW 9 COL 25.43 COLON-ALIGNED
     R-Codmon AT ROW 1.85 COL 40 NO-LABEL
     hastaF AT ROW 3.31 COL 36.29 COLON-ALIGNED
     RB-END-PAGE AT ROW 9 COL 39.57 COLON-ALIGNED
     Btn_OK AT ROW 4.85 COL 50.86
     Btn_Cancel AT ROW 6.69 COL 51
     Btn_Help AT ROW 8.5 COL 51
     RECT-49 AT ROW 1.04 COL 1.29
     RECT-5 AT ROW 4.5 COL 1.14
     " Configuracion de Impresion" VIEW-AS TEXT
          SIZE 47.86 BY .62 AT ROW 4.73 COL 1.72
          BGCOLOR 1 FGCOLOR 15 FONT 6
     "Rango de Fechas :" VIEW-AS TEXT
          SIZE 17.14 BY .54 AT ROW 2.58 COL 7.86
          FONT 6
     "Moneda :" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 1.96 COL 29.29
          FONT 6
     "Paginas" VIEW-AS TEXT
          SIZE 7.72 BY .54 AT ROW 8.35 COL 34.14
          FONT 6
     RECT-50 AT ROW 1.62 COL 38
     SPACE(5.29) SKIP(7.56)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 4
...
FOR EACH Almcmov NO-LOCK WHERE Almcmov.codcia = s-codcia 
    ...
    FIND T-MOVI WHERE T-MOVI.codcia = s-codcia 
                 ...
                 AND  T-MOVI.Codpro = Almcmov.codpro 
                NO-LOCK NO-ERROR.
 ...
 * END.
 * FIND FacDocum WHERE FacDocum.codcia = s-codcia 
 *                AND  FacDocum.coddoc = 'FAC' 
 *               NO-LOCK NO-ERROR.
 * IF AVAILABLE FacDocum THEN DO:
 *    RUN Carga-Ventas('S',FacDocum.codmov,1).
 * END.
 * FIND FacDocum WHERE FacDocum.codcia = s-codcia 
 *                AND  FacDocum.coddoc = 'BOL' 
 *               NO-LOCK NO-ERROR.
 * IF AVAILABLE FacDocum THEN DO:
 *    RUN Carga-Ventas('S',FacDocum.codmov,1).
 * END. */
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-1 FILL-IN-3 RB-NUMBER-COPIES desdeF RB-BEGIN-PAGE R-Codmon 
          hastaF RB-END-PAGE 
      WITH FRAME D-Dialog.
  ENABLE RECT-49 RECT-5 RECT-50 RADIO-SET-1 RB-NUMBER-COPIES desdeF 
         B-impresoras RB-BEGIN-PAGE R-Codmon hastaF RB-END-PAGE Btn_OK 
         Btn_Cancel Btn_Help 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
