...
&Scoped-Define ENABLED-OBJECTS BUTTON-3 BUTTON-1 BUTTON-2 BUTTON-5 
...
DEFINE VARIABLE h_v-invcfg-b AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-1 AT ROW 4.15 COL 14.29
     BUTTON-2 AT ROW 4.15 COL 37.29
     BUTTON-5 AT ROW 4.15 COL 61.29 WIDGET-ID 2
...
ON END-ERROR OF W-Win /* Ajuste de Inventario */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Ajuste de Inventario */
DO:
...
DO:
  MESSAGE 'Este proceso genera los movimientos de entrada y salida' SKIP
        ...
        UPDATE rpta AS LOG.
  IF rpta = YES THEN DO:
    RUN Carga-Data IN h_v-invcfg-b (01).
...
DO:
  ...
  END.
  MESSAGE 'Este proceso genera los movimientos de entrada y salida' SKIP
        'en el almacen considerando la fecha de hoy' SKIP
        '¿Continuamos?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
        UPDATE rpta AS LOG.
  IF rpta = YES THEN DO:
    RUN Genera-Movimiento IN h_v-invcfg-b.
...
ON CHOOSE OF BUTTON-5 IN FRAME F-Main /* (3) EXCEL */
DO:
  RUN Excel IN h_v-invcfg-b.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/v-invcfg-b.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-invcfg-b ).
       RUN set-position IN h_v-invcfg-b ( 1.27 , 4.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Entrada|Salida' + ',
                     FOLDER-TAB-TYPE = 2':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 5.38 , 2.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 10.73 , 73.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-invcfg-b. */
       RUN add-link IN adm-broker-hdl ( h_q-invcfg , 'Record':U , h_v-invcfg-b ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-invcfg-b ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ajting ).
       RUN set-position IN h_b-ajting ( 7.19 , 4.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ajtsal ).
       RUN set-position IN h_b-ajtsal ( 7.19 , 4.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ENABLE BUTTON-3 BUTTON-1 BUTTON-2 BUTTON-5 
