...
DEF VAR x-Letra AS CHAR NO-UNDO.
...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH Almacen ~
 ...
 AND (RADIO-SET-Tipo = 'Todos' OR  ~
INTEGRAL.Almacen.Campo-C[6] = RADIO-SET-Tipo) ~
 AND (RADIO-SET-Estado = 'Todos' OR  ~
(IF RADIO-SET-Estado = "Inactivos" THEN Almacen.Campo-C[9] = "I" ~
ELSE Almacen.Campo-C[9] <> "I")) ~
 AND (TRUE <> (x-Letra > '') OR INDEX(CodAlm, x-Letra) > 0)  NO-LOCK
...
&Scoped-Define ENABLED-OBJECTS RECT-58 RADIO-SET-Tipo RADIO-SET-Estado ~
FILL-IN_Letra BROWSE-1 Btn_OK Btn_Cancel BUTTON-1 BUTTON-2 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-Tipo RADIO-SET-Estado ~
FILL-IN_Letra 
...
DEFINE VARIABLE FILL-IN_Letra AS CHARACTER FORMAT "X(6)":U 
     LABEL "Que contenga" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .81 NO-UNDO.

DEFINE VARIABLE RADIO-SET-Estado AS CHARACTER INITIAL "Activos" 
...
DEFINE VARIABLE RADIO-SET-Tipo AS CHARACTER INITIAL "Todos" 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "Todos los Almacenes", "Todos",
"Solo Almacenes Comerciales", "Si"
     SIZE 43 BY .77 NO-UNDO.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS MULTIPLE SIZE 52 BY 16.15
...
DEFINE FRAME D-Dialog
     RADIO-SET-Tipo AT ROW 1.27 COL 14 NO-LABEL WIDGET-ID 8
     RADIO-SET-Estado AT ROW 2.15 COL 14 NO-LABEL WIDGET-ID 14
     FILL-IN_Letra AT ROW 2.88 COL 12 COLON-ALIGNED WIDGET-ID 20
     BROWSE-1 AT ROW 3.96 COL 5
     Btn_OK AT ROW 3.96 COL 59
     Btn_Cancel AT ROW 5.15 COL 59
     BUTTON-1 AT ROW 9.54 COL 57 WIDGET-ID 2
     BUTTON-2 AT ROW 10.69 COL 57 WIDGET-ID 4
     "Filtrar:" VIEW-AS TEXT
          SIZE 5 BY .5 AT ROW 1 COL 3 WIDGET-ID 12
          BGCOLOR 9 FGCOLOR 15 
     RECT-58 AT ROW 1 COL 2 WIDGET-ID 18
     SPACE(3.00) SKIP(16.57)
...
/* SETTINGS FOR DIALOG-BOX D-Dialog
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-1 FILL-IN_Letra D-Dialog */
ASSIGN 
...
(IF RADIO-SET-Estado = ""Inactivos"" THEN Almacen.Campo-C[9] = ""I""
ELSE Almacen.Campo-C[9] <> ""I""))
 AND (TRUE <> (x-Letra > '') OR INDEX(CodAlm, x-Letra) > 0) "
...
&Scoped-define SELF-NAME FILL-IN_Letra
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN_Letra D-Dialog
ON LEAVE OF FILL-IN_Letra IN FRAME D-Dialog /* Que contenga */
OR ANY-PRINTABLE OF FILL-IN_Letra DO:
    x-Letra = TRIM(SELF:SCREEN-VALUE).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL RADIO-SET-Estado D-Dialog
ON VALUE-CHANGED OF RADIO-SET-Estado IN FRAME D-Dialog
DO:
  ASSIGN {&self-name}.
   {&OPEN-QUERY-{&BROWSE-NAME}}
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-Tipo RADIO-SET-Estado FILL-IN_Letra 
      WITH FRAME D-Dialog.
  ENABLE RECT-58 RADIO-SET-Tipo RADIO-SET-Estado FILL-IN_Letra BROWSE-1 Btn_OK 
...
------------------------------------------------------------------------------*/
  ...
  IF NUM-ENTRIES(x-Almacenes) > 0 THEN DO WITH FRAME {&FRAME-NAME}:
      DO iItem = 1 TO NUM-ENTRIES(x-Almacenes):
          ...
          REPEAT WHILE AVAILABLE Almacen:
              IF Almacen.codalm = ENTRY(iItem,x-Almacenes) THEN DO:
                  REPOSITION {&browse-name} TO ROWID ROWID(Almacen).
