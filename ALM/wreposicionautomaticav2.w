...
FILL-IN-CodFam FILL-IN-PorRep FILL-IN-SubFam FILL-IN-PorStkMax ~
COMBO-BOX-Motivo FILL-IN-ImpMin FILL-IN-CodPro FILL-IN-NomPro ~
SELECT-Clasificacion FILL-IN-Marca FILL-IN-Glosa txtFechaEntrega ~
FILL-IN-Texto TOGGLE-VtaPuntual 
...
DEFINE FRAME F-Main
     ...
     FILL-IN-Mensaje AT ROW 24.65 COL 94 COLON-ALIGNED NO-LABEL WIDGET-ID 60
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-998 sW-Win
...
DO:
  ...
  END.
  RUN dispatch IN h_b-pedrepaut ('open-query-cases':U).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-Resumen sW-Win
ON CHOOSE OF BUTTON-Resumen IN FRAME F-Main /* RESUMEN POR DESPACHO */
DO:
  RUN alm\d-reposicion-resum-desp.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL COMBO-BOX-Motivo sW-Win
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-CodFam sW-Win
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-Glosa sW-Win
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-ImpMin sW-Win
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-PorRep sW-Win
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-PorStkMax sW-Win
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-SubFam sW-Win
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL SELECT-Clasificacion sW-Win
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL TOGGLE-VtaPuntual sW-Win
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL txtFechaEntrega sW-Win
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_b-pedrepaut ( 15.08 , 141.00 ) NO-ERROR.
...
/* RHC 22/08/2016: Dos formas, por los filtros o de un archivo texto */
...
IF NOT TRUE <> (FILL-IN-Texto > '') THEN DO:
    ...
    REPEAT:
        ...
        FIND Almmmatg WHERE Almmmatg.codcia = s-codcia
            ...
            NO-LOCK NO-ERROR.
        FIND FIRST Almmmate OF Almmmatg WHERE Almmmate.codalm = s-codalm
            AND Almmmate.VInMn1 > 0
            AND Almmmate.StkAct < Almmmate.VInMn1
            NO-LOCK NO-ERROR.
        IF AVAILABLE Almmmatg AND AVAILABLE Almmmate THEN DO:
            RUN Por-Utilex.
        END.
    END.
    INPUT CLOSE.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-Temporada FILL-IN-Almacen FILL-IN-CodFam FILL-IN-PorRep 
          FILL-IN-SubFam FILL-IN-PorStkMax COMBO-BOX-Motivo FILL-IN-ImpMin 
          FILL-IN-CodPro FILL-IN-NomPro SELECT-Clasificacion FILL-IN-Marca 
          FILL-IN-Glosa txtFechaEntrega FILL-IN-Texto TOGGLE-VtaPuntual 
...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    ...
    FOR EACH T-DREPO WHERE T-DREPO.AlmPed <> '998',
        ...
        BREAK BY T-DREPO.AlmPed BY T-DREPO.Origen BY Almmmatg.DesMar BY Almmmatg.DesMat:
...
ASSIGN
    ...
    x-StockMinimo = (Almmmate.StkMin * FILL-IN-PorStkMax / 100)
...
/* ************************************************** */
IF x-StkAct >= x-StockMinimo THEN RETURN.
/* ********************* Cantidad de Reposicion ******************* */
/* Definimos el stock maximo */
/* RHC 05/06/2014 Cambio de filosofía, ahora es el % del stock mínimo */

IF x-StkAct >= (x-StockMinimo * FILL-IN-PorRep / 100) THEN RETURN.
...
*/
/* *************************************************** */
IF Almmmatg.CanEmp > 0 AND (pReposicion MODULO Almmmatg.CanEmp) > 0 THEN DO:
...
FOR EACH Almrepos NO-LOCK WHERE  almrepos.CodCia = s-codcia
    ...
    x-StkMax = Almmmate.StkMax.
    IF x-StkMax <= 0 THEN x-StkMax = 1.
    /* No debe superar el stock disponible */
    REPEAT WHILE x-CanReq > x-StockDisponible:
        x-CanReq = x-CanReq - x-StkMax.
    END.
    IF x-CanReq <= 0 THEN NEXT.    /* Menos que la cantidad por empaque */
