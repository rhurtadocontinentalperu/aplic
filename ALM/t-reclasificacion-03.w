...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      ITEM.codmat COLUMN-LABEL "Codigo!Salida" FORMAT "X(6)":U
            WIDTH 8
      Almmmatg.DesMat FORMAT "X(50)":U
      Almmmatg.UndStk COLUMN-LABEL "Unidad" FORMAT "X(4)":U
      ITEM.CanDes FORMAT "(ZZZ,ZZ9.9999)":U WIDTH 6.86
      ...
      ITEM.CanDev FORMAT "(ZZZ,ZZ9.9999)":U WIDTH 7.43
  ENABLE
      ITEM.codmat
      ITEM.CanDes
      ITEM.CodAnt
...
"ITEM.codmat" "Codigo!Salida" ? "character" ? ? ? ? ? ? yes ? no no "8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"ITEM.CanDes" ? "(ZZZ,ZZ9.9999)" "decimal" ? ? ? ? ? ? yes ? no no "6.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.ITEM.CodAnt
"ITEM.CodAnt" "Codigo!Entrada" "X(6)" "character" ? ? ? ? ? ? yes ? no no "8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"B-MATG.UndStk" "Unidad" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.ITEM.CanDev
"ITEM.CanDev" ? "(ZZZ,ZZ9.9999)" "decimal" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
    ...
    END.
    FIND Almmmatg WHERE Almmmatg.CodCia = S-CodCia AND
         Almmmatg.CodMat = ITEM.CodAnt:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
         NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almmmatg THEN DO:
        ...
        RETURN "ADM-ERROR".   
    END. 
    x-CodFam-2 = Almmmatg.codfam + ALmmmatg.subfam.
    FIND Almmmate WHERE Almmmate.CodCia = S-CodCia AND
         Almmmate.CodAlm = S-CodAlm AND
    ...
    THEN DO:
        ...
        FIND LAST Almstkge WHERE Almstkge.codcia = s-codcia
            ...
            AND Almstkge.fecha <= s-FchDoc NO-LOCK NO-ERROR.
        IF AVAILABLE Almstkge THEN x-ctomat = AlmStkge.CtoUni * AlmDRecl.Factor.
        */
        FIND Almmmatg WHERE Almmmatg.CodCia = S-CodCia AND
             Almmmatg.CodMat = ITEM.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
             NO-LOCK NO-ERROR.
        ...
        /*
        FIND LAST Almstkge WHERE Almstkge.codcia = s-codcia
            AND Almstkge.codmat = ITEM.CodAnt:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
            AND Almstkge.fecha <= s-FchDoc NO-LOCK NO-ERROR.
        IF AVAILABLE Almstkge THEN x-ctoant = AlmStkge.CtoUni.
        */
        FIND B-MATG WHERE B-MATG.codcia = s-codcia
    ...
    ELSE DO:
        ...
        FIND LAST Almstkge WHERE Almstkge.codcia = s-codcia
            AND Almstkge.codmat = ITEM.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
            AND Almstkge.fecha <= s-FchDoc NO-LOCK NO-ERROR.
        IF AVAILABLE Almstkge THEN x-ctomat = AlmStkge.CtoUni * f-Factor-1.
        */
        FIND Almmmatg WHERE Almmmatg.CodCia = S-CodCia AND
             Almmmatg.CodMat = ITEM.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
             NO-LOCK NO-ERROR.
        ...
        FIND LAST Almstkge WHERE Almstkge.codcia = s-codcia
            AND Almstkge.codmat = ITEM.CodAnt:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
            AND Almstkge.fecha <= s-FchDoc NO-LOCK NO-ERROR.
        IF AVAILABLE Almstkge THEN x-ctoant = AlmStkge.CtoUni * f-Factor-1.
        */
        FIND B-MATG WHERE B-MATG.codcia = s-codcia
            AND B-MATG.codmat = ITEM.CodAnt:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
            NO-LOCK NO-ERROR.
