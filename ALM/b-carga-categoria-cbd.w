...
T-MATG.UndBas T-MATG.DesMar T-MATG.catconta[1] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      T-MATG.DesMat FORMAT "X(60)":U WIDTH 60.57
      T-MATG.UndBas COLUMN-LABEL "Unidad" FORMAT "X(4)":U
      T-MATG.DesMar COLUMN-LABEL "Marca" FORMAT "X(20)":U WIDTH 20.43
      T-MATG.catconta[1] COLUMN-LABEL "Categoria!Contable" FORMAT "x(2)":U
            WIDTH 17.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 118 BY 20.96
...
DEFINE FRAME F-Main
     ...
     BUTTON-2 AT ROW 1 COL 27 WIDGET-ID 4
...
"T-MATG.codmat" "Articulo" ? "character" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.T-MATG.DesMat
"T-MATG.DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ? no no "60.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.T-MATG.UndBas
"T-MATG.UndBas" "Unidad" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.T-MATG.DesMar
"T-MATG.DesMar" "Marca" "X(20)" "character" ? ? ? ? ? ? no ? no no "20.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.T-MATG.catconta[1]
"T-MATG.catconta[1]" "Categoria!Contable" ? "character" ? ? ? ? ? ? no ? no no "17.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON CHOOSE OF BUTTON-2 IN FRAME F-Main /* GRABAR CATEGORIAS */
DO:

    RUN Grabar-Categorias.
    ASSIGN
...
FOR EACH T-MATG:
    ...
    IF NOT AVAILABLE Almmmatg THEN DO:
        ...
        NEXT.
    END.
    BUFFER-COPY Almmmatg EXCEPT CatConta TO T-MATG.
END.
IF pMensaje <> "" THEN DO:
    EMPTY TEMP-TABLE T-MATG.
    RETURN.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Excel-de-Errores B-table-Win 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Grabar-Categorias B-table-Win 
PROCEDURE Grabar-Categorias :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
MESSAGE 'Procedemos a actualizar las CATEGORIAS CONTABLES?'
...
FOR EACH T-MATG WHERE T-MATG.CatConta[1] <> "":
    FIND Almtabla WHERE Almtabla.Tabla  = "CC" 
        AND Almtabla.codigo = T-MATG.Catconta[1]
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE AlmTabla THEN DO:
        CREATE E-MATG.
        BUFFER-COPY T-MATG 
            TO E-MATG
            ASSIGN E-MATG.Libre_c01 = 'Artículo ' +  TRIM(T-MATG.codmat) + ' Error en la categoria contable: ' +
            T-MATG.CatConta[1].
        DELETE T-MATG.
        NEXT.
    END.
...
FOR EACH T-MATG ON STOP UNDO, RETURN ON ERROR UNDO, RETURN:
    ...
    END.
    ASSIGN
        Almmmatg.CatConta[1] = T-MATG.catconta[1].
END.
EMPTY TEMP-TABLE T-MATG.
