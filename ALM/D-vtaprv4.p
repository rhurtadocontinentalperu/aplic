...
DEFINE TEMP-TABLE tmp-tempo 
    ...
    FIELD t-tot_nor AS DEC           FORMAT "->>>>,>>9.99"
    FIELD t-tot_ate AS DEC           FORMAT "->>>>,>>9.99"
...
FOR EACH Ccbcdocu NO-LOCK WHERE ccbcdocu.CodCia = S-CODCIA 
   ...
   EACH Ccbddocu NO-LOCK WHERE ccbddocu.CodCia = Ccbcdocu.codcia 
                           ...
                           FIND Almmmatg NO-LOCK  WHERE Almmmatg.Codcia = Ccbddocu.codcia 
                                                    AND Almmmatg.Codmat = Ccbddocu.Codmat.
                           IF AVAIL Almmmatg  AND  Almmmatg.codfam BEGINS F-CodFam
                                              AND  Almmmatg.subfam BEGINS F-Subfam
                                              AND  (Almmmatg.codmat >= DesdeC
                                              AND  Almmmatg.CodMat <= HastaC)
                                              AND  (Almmmatg.CodPr1 >= F-prov1
                                              AND  Almmmatg.CodPr1 <= F-prov2)
                                              AND  Almmmatg.TpoArt BEGINS R-Tipo THEN
                           DO:
                                             
                           /* DISPLAY ccbddocu.CodMat @ Fi-Mensaje LABEL "Codigo de Articulo "
 *                             FORMAT "X(11)" WITH FRAME F-Proceso.*/
                            
                            F-Salida  = ccbddocu.CanDes - CCbddocu.Candev .
                            FIND tmp-tempo WHERE t-codfam  = Almmmatg.codfam
                              ...
                              IF NOT AVAIL tmp-tempo THEN DO:
                               message 'no encontro en temporal'.
                                CREATE tmp-tempo.
                              ...
                              IF Ccbcdocu.codmon = 1 THEN DO: 
                                 t-totals = (((ccbddocu.CanDes - CCbddocu.Candev ) * Ccbddocu.Preuni ) / 1.18 ) / (If ncodmon = 1 THEN 1 ELSE Ccbcdocu.Tpocmb).
                              END.
                              ELSE DO:
                                 t-totals = (((ccbddocu.CanDes - CCbddocu.Candev ) * Ccbddocu.Preuni ) / 1.18 ) * (If ncodmon = 2 THEN 1 ELSE Ccbcdocu.Tpocmb).
                              END.
                        
                              ASSIGN t-tot_uca = t-tot_uca + IF ccbcdocu.CodAlm = "03" THEN F-Salida ELSE 0
                                     t-tot_and = t-tot_and + IF ccbcdocu.CodAlm = "04" THEN F-Salida ELSE 0
                                     t-tot_par = t-tot_par + IF ccbcdocu.CodAlm = "05" THEN F-Salida ELSE 0
                                     t-tot_lau = t-tot_lau + IF ccbcdocu.CodAlm = "89" THEN F-Salida ELSE 0
                                     t-tot_ate = t-tot_ate + IF ccbcdocu.CodAlm = "11" THEN F-Salida ELSE 0
                                     t-tot_nor = t-tot_nor + IF ccbcdocu.CodAlm = "18" THEN F-Salida ELSE 0
                                     t-tot_vta = t-tot_vta + F-Salida.
                                     t-totald  = t-totald + t-totals.
...
display ccbcdocu.coddoc ccbcdocu.nrodoc ccbddocu.codmat.
 ...
 *                          AND  B-Almmmate.CodAlm = "03"
 *                          AND  B-Almmmate.codmat = x-codmat
 *                         NO-LOCK NO-ERROR.
 *         IF AVAIL b-almmmate THEN x-stkact = x-stkact + b-Almmmate.StkAct.
 *         FIND b-almmmate WHERE B-Almmmate.CodCia = S-CODCIA
 *                          AND  B-Almmmate.CodAlm = "04"
