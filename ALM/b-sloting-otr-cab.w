...
DEF VAR dFchDoc AS DATE NO-UNDO.
...
&SCOPED-DEFINE Condicion ( OOMoviAlmacen.codcia = s-codcia ~
    ...
    AND OOMoviALmacen.FchDoc >= dFchDoc )
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Captura_Filtro B-table-Win 
PROCEDURE Captura_Filtro :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAMETER pdFchDoc AS DATE NO-UNDO.
...
FOR EACH T-MoviAlmacen NO-LOCK,
    ...
    IF pImprimirDetalle = NO THEN DO:
        ...
        ASSIGN
            ...
            w-report.Campo-D[1] = OOMoviAlmacen.fchdoc      /*FlagFecha */
            w-report.Campo-C[5] = OOMoviAlmacen.hradoc.    /*FlagHora.*/
...
FOR EACH TI-MoviAlmacen NO-LOCK,
    ...
    AND OOMoviAlmacen.FlagMigracion = "S" NO-LOCK,
    FIRST Almacen WHERE Almacen.CodCia = OOMoviAlmacen.CodCia
    AND Almacen.CodAlm = OOMoviAlmacen.AlmOri NO-LOCK:
...
FOR EACH OOMoviAlmacen WHERE OOMoviAlmacen.codcia = s-codcia
                        ...
                        AND OOMoviAlmacen.codalm = s-codalm
                        AND OOMoviAlmacen.TipMov = "I" 
                        AND OOMoviAlmacen.CodMov = 03 NO-LOCK
    BREAK BY OOMoviAlmace.CodCia
    BY OOMoviAlmacen.CodAlm 
    BY OOMoviAlmacen.TipMov
    BY OOMoviAlmacen.CodMov
    ...
    IF FIRST-OF(OOMoviAlmacen.CodAlm)
        ...
        END.
        TI-MoviAlmacen.Volumen = TI-MoviAlmacen.Volumen / 1000000.

/*         TI-MoviAlmacen.Chequeador = ''.                                                            */
/*         FIND FIRST Faccpedi WHERE Faccpedi.codcia = s-codcia AND                                  */
/*             Faccpedi.coddoc = OOMoviAlmacen.CodRef AND                                            */
/*             Faccpedi.nroped = OOMoviAlmacen.NroRef                                                */
/*             NO-LOCK NO-ERROR.                                                                     */
...
/*                 NO-LOCK NO-ERROR.                                                                 */
/*             IF AVAILABLE LogTabla THEN DO:                                                        */
/*                 TI-MoviAlmacen.Chequeador = ENTRY(6,LogTabla.ValorLlave,'|').                      */
/*             END.                                                                                  */
/*         END.                                                                                      */
/*         RUN gn/nombre-personal (INPUT s-codcia, INPUT TI-MoviAlmacen.Chequeador, OUTPUT x-NomPer). */
/*         TI-MoviAlmacen.Chequeador = TI-MoviAlmacen.Chequeador + ' ' + x-NomPer.                     */
    END.
END.
