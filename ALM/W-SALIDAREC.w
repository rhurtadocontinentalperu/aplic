...
/*DEFINE NEW SHARED VARIABLE S-MOVCMP AS INTEGER.
 * DEFINE NEW SHARED VARIABLE S-MOVTRF AS INTEGER.*/
...
DEFINE VARIABLE h_q-salidarec AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-movmto ).
       RUN set-position IN h_v-movmto ( 1.08 , 1.57 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-salida ).
       RUN set-position IN h_v-salida ( 2.38 , 1.72 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 7.15 , 22.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv05 ).
       RUN set-position IN h_p-updv05 ( 7.19 , 40.72 ) NO-ERROR.
       RUN set-size IN h_p-updv05 ( 1.31 , 49.43 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/q-salidarec.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-salidarec ).
       RUN set-position IN h_q-salidarec ( 1.12 , 60.72 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_q-salidarec , 'Record':U , h_v-movmto ).
       ...
       RUN add-link IN adm-broker-hdl ( h_q-salidarec , 'Record':U , h_q-alcmov ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-salidarec ,
             h_p-updv05 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-alcmov ,
             h_q-salidarec , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-salida ).
       RUN set-position IN h_b-salida ( 8.62 , 1.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpsal ).
       RUN set-position IN h_b-tmpsal ( 8.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpsal ( 12.50 , 86.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 9.62 , 89.00 ) NO-ERROR.
...
CASE L-Handle:
    ...
    WHEN "Pagina2"  THEN DO:
          ...
          RUN dispatch IN h_b-tmpsal ('open-query':U).
      END.
    WHEN "Carga-Series" THEN RUN Carga-Series IN h_q-salidarec.
END CASE.
