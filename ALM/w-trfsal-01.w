...
DEFINE VARIABLE h_t-trfsal-01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-movmto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-trfsal-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 9.35 COL 38 WIDGET-ID 2
     BUTTON-2 AT ROW 9.35 COL 66 WIDGET-ID 4
     BUTTON-EXCEL AT ROW 9.35 COL 66 WIDGET-ID 6
     RECT-1 AT ROW 4.23 COL 2 WIDGET-ID 8
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 98.29 BY 20.77 WIDGET-ID 100.
...
DO:
  RUN Captura-Pedido IN h_v-trfsal-01.
END.
...
ON CHOOSE OF BUTTON-2 IN FRAME F-Main /* REPOSICION MANUAL */
DO:
    SESSION:SET-WAIT-STATE('GENERAL').
  RUN Captura-Pedido-Manual IN h_v-trfsal-01.
...
ON CHOOSE OF BUTTON-EXCEL IN FRAME F-Main /* IMPORTAR DESDE EXCEL */
DO:
  RUN Importar-desde-Excel IN h_t-trfsal-01.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/v-trfsal-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-trfsal-01 ).
       RUN set-position IN h_v-trfsal-01 ( 4.50 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-trfsal-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_v-trfsal-01 ).
       RUN add-link IN adm-broker-hdl ( h_q-trfsal-01 , 'Record':U , h_v-trfsal-01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-trfsal-01 ,
             h_p-updv13 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-trfsal-01 ( 9.35 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/t-trfsal-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-trfsal-01 ).
       RUN set-position IN h_t-trfsal-01 ( 10.96 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-trfsal-01 ( 9.96 , 85.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-trfsal-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-trfsal-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_v-trfsal-01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-trfsal-01 ,
             BUTTON-EXCEL:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
/*          BUTTON-2:SENSITIVE IN FRAME {&FRAME-NAME} = NO. */
    ...
    WHEN "Pagina2"  THEN DO:
         ...
         RUN adm-open-query IN h_t-trfsal-01.
