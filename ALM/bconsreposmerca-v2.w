...
DEFINE TEMP-TABLE Almacen-Despacho NO-UNDO LIKE Almacen.
DEFINE TEMP-TABLE Almacen-Destino NO-UNDO LIKE Almacen.
...
DEFINE TEMP-TABLE T-Detalle
    ...
    FIELD AlmacenXD AS CHAR LABEL "Alm. XD"
    ...
    FIELD NewOT LIKE Reposiciones.nrorf1 LABEL 'Nva O/T'
    ...
    FIELD MotReposicion LIKE Reposiciones.MotReposicion LABEL 'Motivo Reposicion'
...
DEFINE TEMP-TABLE T-Detalle-Item LIKE T-Detalle
    ...
    FIELD ClfGral AS CHAR             LABEL 'Clf. Gral.'
    FIELD ClfUtilex AS CHAR           LABEL 'Clf. Utilex'
    FIELD ClfMay AS CHAR              LABEL 'Clf. May.'
    ...
    FIELD Volumen LIKE almmmatg.libre_d02 LABEL 'Volumen (m3)'
    FIELD Origen LIKE almdrepo.origen LABEL 'Origen'
    FIELD NroGuia AS CHAR FORMAT 'x(60)' LABEL 'Guia de Remision'
.

DEFIN TEMP-TABLE t-Track-Text
    FIELD AlmOri AS CHAR LABEL 'Alm. Despacho'
    FIELD AlmDes AS CHAR LABEL 'Alm. Destino'
    FIELD FchEmi AS DATE LABEL 'Emision'
    FIELD Usuario AS CHAR LABEL 'Usuario'
    FIELD Serie AS INTE LABEL 'Serie'
    FIELD NroDoc AS INTE LABEL 'Numero'
    FIELD FchEnt AS DATE LABEL 'Entrega'
    FIELD NroOT AS CHAR LABEL 'O/T'
    FIELD FlgEst AS CHAR LABEL 'Estado'
    FIELD FchEst AS DATE LABEL 'Fecha Estado'
    FIELD Bultos AS CHAR LABEL 'Bultos'
    FIELD Items AS INTE LABEL 'Total Items'
    FIELD Peso AS DECI LABEL 'Peso Solicitado Kgrs'
    FIELD Peso2 AS DECI LABEL 'Peso Atendido Kgrs'
    FIELD Vol AS DECI LABEL 'Total m3 Solicitad'
    FIELD Vol2 AS DECI LABEL 'Total m3 Atendido'
    FIELD Glosa AS CHAR LABEL 'Observaciones'
    FIELD Nombre AS CHAR LABEL 'Nombre Rechazo'
    FIELD FchTrk AS DATE LABEL 'Fecha Track'
    FIELD Estado AS CHAR LABEL 'Estado'
    FIELD Docum AS CHAR LABEL 'Documento'
    FIELD Hora AS CHAR LABEL 'Hora'
    FIELD Usuario2 AS CHAR LABEL 'Usuario'
    FIELD NomUsu AS CHAR LABEL 'Nombre Usuario'
    .
...
&Scoped-Define ENABLED-OBJECTS FILL-IN-FchDoc-1 FILL-IN-FchDoc-2 BUTTON-19 ~
BUTTON-3 FILL-IN-FchEnt-1 FILL-IN-FchEnt-2 COMBO-BOX-Motivo FILL-IN-NroSer ~
FILL-IN-NroDoc txtUsuario txtSerOtr txtNroOtr TOGGLE-CrossDocking ~
FILL-IN-CodMat TOGGLE-VtaPuntual COMBO-BOX-PreHoja SELECT-FlgEst BUTTON-BD ~
BUTTON-Limpiar BUTTON-Excel RADIO-SET-Excel br_table RECT-20 RECT-22 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-FchDoc-1 FILL-IN-FchDoc-2 ~
FILL-IN-AlmPed FILL-IN-CodAlm FILL-IN-FchEnt-1 FILL-IN-FchEnt-2 ~
COMBO-BOX-Motivo FILL-IN-NroSer FILL-IN-NroDoc txtUsuario txtSerOtr ~
txtNroOtr TOGGLE-CrossDocking FILL-IN-CodMat TOGGLE-VtaPuntual ~
COMBO-BOX-PreHoja FILL-IN-TotVolSol FILL-IN-TotVolAte txtPedAtrazados ~
FILL-IN-TotPedidos FILL-IN-TotPesSol FILL-IN-TotPesAte FILL-IN-TotBultos ~
FILL-IN-TotItm SELECT-FlgEst RADIO-SET-Excel 
...
DEFINE BUTTON BUTTON-BD 
     LABEL "Consultar BD" 
     SIZE 15 BY 1.12.

DEFINE BUTTON BUTTON-Excel 
     IMAGE-UP FILE "img\tbldat.ico":U
     LABEL "Button 7" 
     SIZE 6 BY 1.54 TOOLTIP "Exportar a Texto".
...
DEFINE VARIABLE SELECT-FlgEst AS CHARACTER INITIAL "Todos" 
     ...
     LIST-ITEMS "Todos","Emitido","Aprobado","Impreso","Picking","Chequeado","Documentado","En Ruta","Recepcionado","Anulado","Rechazado" 
     SIZE 20 BY 5.92 TOOLTIP "Selecciona Estados" NO-UNDO.
...
DEFINE FRAME F-Main
     FILL-IN-FchDoc-1 AT ROW 1.27 COL 19 COLON-ALIGNED WIDGET-ID 26
     FILL-IN-FchDoc-2 AT ROW 1.27 COL 43 COLON-ALIGNED WIDGET-ID 28
     FILL-IN-AlmPed AT ROW 2.08 COL 19 COLON-ALIGNED HELP
          "Separados por comas. Ejemplo: 11,10,10a,21" WIDGET-ID 14
     BUTTON-19 AT ROW 2.08 COL 75 WIDGET-ID 12
     FILL-IN-CodAlm AT ROW 2.88 COL 19 COLON-ALIGNED HELP
          "Separados por comas. Ejemplo: 11,10,10a,21" WIDGET-ID 4
     BUTTON-3 AT ROW 2.88 COL 75 WIDGET-ID 6
     ...
     BUTTON-BD AT ROW 1.27 COL 110 WIDGET-ID 62
     BUTTON-Limpiar AT ROW 2.42 COL 110 WIDGET-ID 50
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: Almacen-Despacho T "?" NO-UNDO INTEGRAL Almacen
      TABLE: Almacen-Destino T "?" NO-UNDO INTEGRAL Almacen
...
/* BROWSE-TAB br_table RADIO-SET-Excel F-Main */
ASSIGN 
...
/* SETTINGS FOR FILL-IN FILL-IN-AlmPed IN FRAME F-Main
   NO-ENABLE                                                            */
...
DO:
    ...
    RUN gn/d-filtra-almacen-tmp (INPUT TABLE Almacen-Despacho, INPUT-OUTPUT x-almacenes).
...
DO:
    ...
    RUN gn/d-filtra-almacen-tmp (INPUT TABLE Almacen-Destino, INPUT-OUTPUT x-almacenes).
...
&Scoped-define SELF-NAME BUTTON-BD
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-BD B-table-Win
ON CHOOSE OF BUTTON-BD IN FRAME F-Main /* Consultar BD */
DO:
  ASSIGN
  ...
  IF FILL-IN-FchDoc-1 = ? OR FILL-IN-FchDoc-2 = ? THEN DO:
      MESSAGE 'Debe ingresar ambas fechas: EMITIDOS DESDE y EMITIDOS HASTA' VIEW-AS ALERT-BOX INFORMATION.
  ...
  CASE s-Nivel-Acceso:
      WHEN 1 THEN DO:
          IF TRUE <> (FILL-IN-AlmPed > '') OR TRUE <> (FILL-IN-CodAlm > '') THEN DO:
              MESSAGE 'Sería bueno ingresar ambos almacenes, el Almacén Despacho y el Almacén Destino' SKIP
...
DO:
    ...
    CASE RADIO-SET-Excel:
        ...
        WHEN 2 THEN DO:
            ...
            cArchivo = LC(pArchivo).
            RUN lib/tt-filev2 (TEMP-TABLE t-Detalle-Item:HANDLE, cArchivo, pOptions).
        END.
    END CASE.
...
DO:
    ASSIGN {&self-name}.
    IF SELF = ? THEN DO:
        MESSAGE 'NO se puede dejar la fecha en blanco' VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    IF s-nivel-acceso = 0 AND ABS(FILL-IN-FchDoc-1 - FILL-IN-FchDoc-2) > 15 THEN DO:
        MESSAGE 'NO se aceptan fechas con más de 15 días de diferencia' VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Captura-Motivo B-table-Win 
PROCEDURE Captura-Motivo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
CASE s-nivel-acceso:
    WHEN 0 THEN DO:     /* Administradores */
        IF ABS(FILL-IN-FchDoc-1 - FILL-IN-FchDoc-2) > 7 THEN DO:
            /* Grabamos info */
            CREATE LogTabla.
            ASSIGN
                logtabla.codcia = s-codcia
                logtabla.Dia = TODAY
                logtabla.Evento = 'CONSULTA'
                logtabla.Hora = STRING(TIME, 'HH:MM:SS')
                logtabla.Tabla = 'ALMCREPO'
                logtabla.Usuario = s-user-id
                logtabla.ValorLlave = "Glosa: " + pGlosa + " Emitidos Desde: " + STRING(FILL-IN-FchDoc-1) + " Hasta: " + STRING(FILL-IN-FchDoc-2).
            RELEASE LogTabla.
        END.
    END.
    WHEN 1 THEN DO:     /* Abasteciientos */
        IF ABS(FILL-IN-FchDoc-1 - FILL-IN-FchDoc-2) > 15 THEN DO:
            MESSAGE 'Usted ha ingresado una rango de fechas de Emisión mayor a 15 días' SKIP
                'Necesito que ingrese el motivo' VIEW-AS ALERT-BOX WARNING.
            RUN gn/d-glosa (OUTPUT pGlosa).
            IF TRUE <> (pGlosa > '') THEN RETURN 'ADM-ERROR'.
            /* Grabamos info */
            CREATE LogTabla.
            ASSIGN
                logtabla.codcia = s-codcia
...
/* Se va a cargar el dominio de valores para el almacén despacho y destino */
DO WITH FRAME {&FRAME-NAME}:
    ASSIGN
        FILL-IN-FchDoc-1 FILL-IN-FchDoc-2.
    IF ABS(FILL-IN-FchDoc-1 - FILL-IN-FchDoc-2) > 15 THEN DO:
        MESSAGE 'Usted ha seleccionado un rango de fechas de más de 15 días' SKIP
            'Continuamos?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
            UPDATE rpta AS LOG.
        IF rpta = NO THEN RETURN 'ADM-ERROR'.
    END.
    SESSION:SET-WAIT-STATE('GENERAL').
    EMPTY TEMP-TABLE Almacen-Despacho.
    EMPTY TEMP-TABLE Almacen-Destino.
    ...
    IF AVAILABLE Almacen THEN x-CodDiv = Almacen.CodDiv.

    DO k = 1 TO 2:
        /* ********* */
        FOR EACH almacen NO-LOCK WHERE almacen.codcia = s-codcia:
            IF s-nivel-acceso = 0 AND Almacen.coddiv <> x-coddiv THEN NEXT.
            FOR EACH almcrepo NO-LOCK WHERE almcrepo.codcia = s-codcia
                ...
                FIND FIRST Almacen-Despacho WHERE Almacen-Despacho.codcia = s-codcia AND
                    Almacen-Despacho.codalm = Almcrepo.AlmPed
                    NO-LOCK NO-ERROR.
                IF NOT AVAILABLE Almacen-Despacho THEN DO:
                    CREATE Almacen-Despacho.
                    Almacen-Despacho.codcia = s-codcia.
                    Almacen-Despacho.codalm = Almcrepo.AlmPed.
                END.
                FIND FIRST Almacen-Destino WHERE Almacen-Destino.codcia = s-codcia AND
                    Almacen-Destino.codalm = Almcrepo.CodAlm
                    NO-LOCK NO-ERROR.
                IF NOT AVAILABLE Almacen-Destino THEN DO:
                    CREATE Almacen-Destino.
                    Almacen-Destino.codcia = s-codcia.
                    Almacen-Destino.codalm = Almcrepo.CodAlm.
                END.
            END.
        END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      CASE s-Nivel-Acceso:
          WHEN 0 THEN DO:
              /*ASSIGN FILL-IN-CodAlm:SENSITIVE = NO FILL-IN-FchDoc-2:SENSITIVE = NO.*/
              DISABLE FILL-IN-CodAlm FILL-IN-FchDoc-2 BUTTON-Excel RADIO-SET-Excel.
          END.
      END CASE.
      /* Motivos */
...
EMPTY TEMP-TABLE t-Track-Text.
FOR EACH b-REPO NO-LOCK, FIRST b-almcrepo OF b-REPO NO-LOCK :
    FOR EACH b-TRACKING NO-LOCK WHERE b-TRACKING.CodCia = b-almcrepo.CodCia
            AND b-TRACKING.CodAlm = b-almcrepo.CodAlm
            AND b-TRACKING.Libre_d01 = b-almcrepo.NroSer
            AND b-TRACKING.Libre_d02 = b-almcrepo.NroDoc:
        CREATE t-Track-Text.
        ASSIGN
            t-Track-Text.AlmOri = b-almcrepo.almped
            t-Track-Text.AlmDes = b-almcrepo.codalm
            t-Track-Text.FchEmi = b-almcrepo.fchdoc
            t-Track-Text.Usuario = b-repo.usuario
            t-Track-Text.Serie = b-almcrepo.nroser
            t-Track-Text.NroDoc = b-almcrepo.nrodoc
            t-Track-Text.FchEnt = b-almcrepo.fecha
            t-Track-Text.NroOT = b-repo.ot
            t-Track-Text.FlgEst = b-repo.libre_c05
            t-Track-Text.FchEst = b-repo.libre_f02
            t-Track-Text.Bultos = b-repo.flgcbd
            t-Track-Text.Items = b-repo.totitm
            t-Track-Text.Peso = b-repo.libre_d01
            t-Track-Text.Peso2 = b-repo.libre_d02
            t-Track-Text.Vol = b-repo.totvolsol
            t-Track-Text.Vol2 = b-repo.totvolate
            t-Track-Text.Glosa = b-almcrepo.glosa
            t-Track-Text.Nombre = b-repo.nomrechazo
            t-Track-Text.FchTrk = b-tracking.fechai
            t-Track-Text.Estado = b-tracking.libre_c01
            t-Track-Text.Docum = b-tracking.nroped
            t-Track-Text.Hora = b-tracking.libre_c04
            t-Track-Text.Usuario2 = b-tracking.usuario
            t-Track-Text.NomUsu = b-tracking.libre_c05
            .
    END.
END.
SESSION:SET-WAIT-STATE('').
...
cArchivo = LC(pArchivo).
RUN lib/tt-filev2 (TEMP-TABLE t-Track-Text:HANDLE, cArchivo, pOptions).
