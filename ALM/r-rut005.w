...
------------------------------------------------------------------------------*/
  ...
  DEF VAR x-ImpEfe AS DEC NO-UNDO.      /* Importe efectivamente entregado */
  ...
  FOR EACH Di-RutaC NO-LOCK WHERE di-rutac.codcia = s-codcia
    ...
    FOR EACH Di-RutaD OF Di-RutaC NO-LOCK WHERE LOOKUP(di-rutad.flgest, 'C,X,D,N') > 0,
        ...
        IF LOOKUP (Di-RutaD.FlgEst, 'C,D') > 0 THEN x-ImpEFe = x-ImpTot.
        ...
        CASE di-rutad.flgest:
            ...
            THEN DO:
                FOR EACH Di-RutaDv WHERE di-rutadv.codcia = di-rutad.codcia
                    ...
                    ASSIGN
                        x-ImpEfe = x-ImpEfe - di-rutadv.candev * (x-impdev / di-rutadv.candes)
                        x-DevTot = x-DevTot + di-rutadv.candev * (x-impdev / di-rutadv.candes).
                END.
            END.
        END CASE.
        ASSIGN
            DETALLE.MtoDes = DETALLE.MtoDes + (x-ImpTot - x-DevTot)
            DETALLE.MtoEfe = DETALLE.MtoEFe + x-ImpEFe.
    END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME FC-REP
    ...
    DETALLE.mtoefe      COLUMN-LABEL "Monto!Efectivo"               FORMAT ">>>,>>>,>>9.99"
    DETALLE.mnoobr      COLUMN-LABEL "Costo!Mano de Obra"           FORMAT ">>>,>>>,>>9.99"
    WITH WIDTH 265 NO-BOX STREAM-IO DOWN. 
  ...
  FOR EACH DETALLE BREAK BY DETALLE.codcia BY DETALLE.FchDoc BY DETALLE.CodVeh:
    ...
    ACCUMULATE DETALLE.mtoefe (TOTAL BY DETALLE.codcia).
    ACCUMULATE DETALLE.mtoefe (SUB-TOTAL BY DETALLE.fchdoc).
    ...
    THEN DO:
        ...
        DISPLAY STREAM REPORT
            ...
            ACCUM SUB-TOTAL BY DETALLE.FchDoc DETALLE.mtoefe  @ DETALLE.mtoefe
