...
DEFINE VARIABLE h_b-matalm-v2 AS HANDLE NO-UNDO.
...
DO:
   RUN Asigna-por-Articulos IN h_b-matalm-v2.
END.
...
DO:
  RUN Asigna-por-Familia IN h_b-matalm-v2.
END.
...
DO:
  RUN Asigna-por-Marca IN h_b-matalm-v2.
END.
...
DO:
  RUN Asigna-por-Proveedor IN h_b-matalm-v2.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/b-matalm-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Codigo':U ,
             OUTPUT h_b-matalm-v2 ).
       RUN set-position IN h_b-matalm-v2 ( 1.04 , 1.72 ) NO-ERROR.
       RUN set-size IN h_b-matalm-v2 ( 15.88 , 111.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-matalm-v2. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_b-matalm-v2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_b-matalm-v2 ).

       /* Links to SmartViewer h_v-matalm. */
       RUN add-link IN adm-broker-hdl ( h_b-matalm-v2 , 'Record':U , h_v-matalm ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-matalm-v2 ,
             Btn-Asigna:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_b-matalm-v2 , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ASSIGN 
          {&WINDOW-NAME}:TITLE = {&WINDOW-NAME}:TITLE + " [ " + S-CODALM + " - " + S-DESALM + " ]".
      IF s-acceso-total = NO THEN DO:
          ASSIGN Btn-Asigna:VISIBLE = NO.
          RUN dispatch IN h_p-updv05 ('hide':U).
      END.
  END.
