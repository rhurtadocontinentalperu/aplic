...
/*             CREATE AlmCtrlArti.                                                          */
/*             ASSIGN                                                                       */
/*                 almctrlarti.Codcia = b-ooMoviAlmacen.CodCia                              */
/*                 almctrlarti.codmat = b-ooMoviAlmacen.CodMat                              */
/*                 almctrlarti.CodAlm = b-ooMoviAlmacen.CodAlm                              */
/*                 almctrlarti.SerialNumber = b-ooMoviAlmacen.SerialNumber                  */
/*                 almctrlarti.Cantidad = (b-ooMoviAlmacen.CanDes * b-ooMoviAlmacen.Factor) */
/*                 almctrlarti.Saldo = (b-ooMoviAlmacen.CanDes * b-ooMoviAlmacen.Factor)    */
/*                 almctrlarti.AssetId = b-ooMoviAlmacen.AssetId                            */
/*                 almctrlarti.Fecha = TODAY                                                */
/*                 almctrlarti.Hora = STRING(TIME, 'HH:MM:SS')                              */
/*                 almctrlarti.Usuario = s-user-id.                                         */
/*                 /*almctrlarti.Vencimiento*/                                              */
        END.
        /* ********************************************************* */
        ...
        /* Verificación Final */
        /* **************************************************************************************** */
/*         FIND CURRENT B-MATE EXCLUSIVE-LOCK NO-ERROR NO-WAIT.                                                             */
/*         FIND CURRENT Almdmov EXCLUSIVE-LOCK NO-ERROR NO-WAIT.                                                            */
/*         IF AVAILABLE B-MATE AND AVAILABLE Almdmov AND (B-MATE.StkAct <> Almdmov.StkSub) THEN DO:                         */
/*             MESSAGE b-mate.codmat b-mate.codalm b-mate.stkact almdmov.stksub.                                            */
/*             pMensaje = "ERROR: Diferencia kardex y stock almacén (almacen " + ooMoviAlmacen.CodAlm + " mov " +           */
/*                 ooMoviAlmacen.TipMov + STRING(ooMoviAlmacen.CodMov) + " numero " + STRING(ooMoviAlmacen.NroSer, '999') + */
/*                 STRING(ooMoviAlmacen.NroDoc, '999999999') + " producto " + b-ooMoviAlmacen.CodMat + ")" + CHR(10) +      */
/*                 'Proceso Abortado'.                                                                                      */
/*             UNDO CICLO, RETURN 'ADM-ERROR'.                                                                              */
