...
DEFINE FRAME F-Main
     ...
     Btn_OK AT ROW 10.31 COL 33.86
     Btn_Cancel AT ROW 10.35 COL 45.43
     Btn_Cancel-2 AT ROW 10.38 COL 57 WIDGET-ID 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "DIFERENCIA CONSOLIDADA MENOS SUSTENTO"
         HEIGHT             = 10.96
         WIDTH              = 68.14
         MAX-HEIGHT         = 10.96
         MAX-WIDTH          = 68.14
...
ON END-ERROR OF W-Win /* DIFERENCIA CONSOLIDADA MENOS SUSTENTO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* DIFERENCIA CONSOLIDADA MENOS SUSTENTO */
DO:
...
------------------------------------------------------------------------------*/
  ...
  ENABLE DFecha HFecha DesdeC HastaC RADIO-SET-1 Btn_OK Btn_Cancel Btn_Cancel-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
/*Header*/
cRange = "B" + '2'.
chWorkSheet:Range(cRange):Value = "DIFERENCIA CONSOLIDADA POR ARTICULO".
cRange = "O" + '2'.
...
cRange = "F" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif 03".
cRange = "G" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif 04".
cRange = "H" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif 05".
cRange = "I" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif 11".
cRange = "J" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif 16".
cRange = "K" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif 17".
cRange = "L" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif 18".
cRange = "M" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif 48".
cRange = "N" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif 83".
cRange = "O" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif Otros".
cRange = "P" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif Total (+)".
cRange = "Q" + cColumn.
chWorkSheet:Range(cRange):Value = "Dif Total (-)".
cRange = "R" + cColumn.
...
FOR EACH DETALLE BREAK BY DETALLE.codcia BY DETALLE.catconta[1] BY DETALLE.desmat:
    ASSIGN
        ...
        x-CtoPos = 0
        x-CtoNeg = 0.
    IF x-CtoTot >= 0 THEN x-CtoPos = x-CtoTot. ELSE x-CtoNeg = x-CtoTot.    
    ...
    cRange = "K" + cColumn.
    chWorkSheet:Range(cRange):Value = DETALLE.stk17.
    cRange = "L" + cColumn.
    chWorkSheet:Range(cRange):Value = DETALLE.stk18.
    cRange = "M" + cColumn.
    chWorkSheet:Range(cRange):Value = DETALLE.stk48.
    cRange = "N" + cColumn.
    chWorkSheet:Range(cRange):Value = DETALLE.stk83. 
    cRange = "O" + cColumn.
    chWorkSheet:Range(cRange):Value = DETALLE.stkOtr.
    IF x-CanInv >= 0 THEN DO:
        cRange = "P" + cColumn.
    ...
    ACCUMULATE x-CtoPos (TOTAL BY DETALLE.codcia).
    ACCUMULATE x-CtoNeg (TOTAL BY DETALLE.codcia).

    IF LAST-OF(Detalle.CodCia) THEN DO:
        cRange = "R" + cColumn.
        chWorkSheet:Range(cRange):Value = ACCUM TOTAL BY DETALLE.codcia x-CtoPos.
        cRange = "S" + cColumn.
        chWorkSheet:Range(cRange):Value = ACCUM TOTAL BY DETALLE.codcia x-CtoNeg.
    END.
END.
...
FOR EACH DETALLE,
    ...
    IF FIRST-OF(DETALLE.codfam)THEN DO:
        ...
        cColumn = STRING(t-Column).
        cRange = "A" + cColumn.
        chWorkSheet:Range(cRange):Value = Almtfami.codfam.
        cRange = "B" + cColumn.
        chWorkSheet:Range(cRange):Value = Almtfami.desfam.
    END.

    IF FIRST-OF(DETALLE.subfam) THEN DO:
        t-Column = t-Column + 1.
        cColumn = STRING(t-Column).
        cRange = "A" + cColumn.
        chWorkSheet:Range(cRange):Value = AlmSFami.subfam.
        cRange = "B" + cColumn.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-DETALLE
    ...
    x-CtoPos            COLUMN-LABEL "Costo Total (+)"  FORMAT '->>,>>>,>>>,>>9.99'
    x-CtoNeg            COLUMN-LABEL "Costo Total (-)"  FORMAT '->>,>>>,>>>,>>9.99'
  ...
  FOR EACH DETALLE BREAK BY DETALLE.codcia BY DETALLE.catconta[1] BY DETALLE.desmat:
    ...
    IF LAST-OF(DETALLE.codcia) THEN DO: 
        UNDERLINE STREAM REPORT
            ...
            WITH FRAME F-DETALLE.
        DISPLAY STREAM REPORT
            ACCUM TOTAL BY DETALLE.codcia x-CtoPos @ x-CtoPos
            ACCUM TOTAL BY DETALLE.codcia x-CtoNeg @ x-CtoNeg
            WITH FRAME F-DETALLE.
    END.
  END.
