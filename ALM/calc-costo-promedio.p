...
DEF VAR x-ctomed AS DECI DECIMALS 4 NO-UNDO.    /* 08/02/2024:J.Hermoza */
DEF VAR x-stkgen AS DECI NO-UNDO.
DEF VAR x-cto    AS DECI DECIMALS 4 NO-UNDO.    /* 08/02/2024:J.Hermoza */
        ...
        IF AVAILABLE AlmTmov THEN DO:
           IF almtmov.TipMov = "I" AND x-Factor <> 0 THEN DO:
               ...
               IF Almtmov.TpoCto = 0 OR Almtmov.TpoCto = 1 THEN DO:
                   ...
                   END.
                   ELSE DO:
                       IF AlmStkGe.StkAct > 0 THEN x-ctomed = ( (x-ctomed * AlmStkGe.StkAct) + x-cto) / AlmStkGe.StkAct.
                       /* PARCHE 12.01.10 */
           ...
           IF Almtmov.TipMov = "S" AND x-Factor <> 0 THEN DO:
               IF ( Almtmov.MovCmp = YES AND Almtmov.CodMov = 09 ) OR 
                   ( Almtmov.MovCmp = YES AND Almtmov.CodMov <> 09 ) THEN DO:
                   x-cto = 0.
                   IF almdmov.codmon = 1 THEN x-cto = Almdmov.Preuni / Almdmov.factor.
                   IF almdmov.codmon = 2 THEN x-cto = Almdmov.Preuni / Almdmov.factor * Almdmov.TpoCmb.
                   /* RHC  22/01/2013 CASO DE "SOLO VALORES" */
