...
DEFINE NEW SHARED VARIABLE s-codref   AS CHAR INITIAL "".       /* Reposiciones Automáticas */
...
DEF NEW SHARED VAR s-AlmSalida  AS CHAR.
DEF NEW SHARED VAR s-AlmIngreso AS CHAR.
...
DEFINE VARIABLE h_tordendetransfautom AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vordendetransfautom AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "ORDEN DE TRANSFERENCIA RAPIDA"
         HEIGHT             = 20
         WIDTH              = 130.72
         MAX-HEIGHT         = 27.38
         MAX-WIDTH          = 167.29
...
ON END-ERROR OF W-Win /* ORDEN DE TRANSFERENCIA RAPIDA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ORDEN DE TRANSFERENCIA RAPIDA */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ALM/vordendetransfautom.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vordendetransfautom ).
       RUN set-position IN h_vordendetransfautom ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.88 , 110.00 ) */
       ...
       /* Links to SmartViewer h_vordendetransfautom. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_vordendetransfautom ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_vordendetransfautom ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_vordendetransfautom ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             h_vordendetransfautom , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_bpedidodetransferencia ( 7.73 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/tordendetransfautom.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tordendetransfautom ).
       RUN set-position IN h_tordendetransfautom ( 7.73 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tordendetransfautom ( 11.31 , 119.00 ) NO-ERROR.
       ...
       RUN set-size IN h_p-updv12 ( 1.42 , 34.00 ) NO-ERROR.

       /* Links to SmartBrowser h_tordendetransfautom. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tordendetransfautom ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tordendetransfautom ,
             h_vordendetransfautom , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tordendetransfautom , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_tordendetransfautom <> ? THEN RUN dispatch IN h_tordendetransfautom ('open-query':U).
      END.
 ...
 WHEN "Disable-Head" THEN DO:
     ...
     RUN dispatch IN h_vordendetransfautom ('disable-fields').
   END.
 WHEN "Enable-Head" THEN DO:
     RUN dispatch IN h_p-updv04 ('enable':U).
     RUN dispatch IN h_vordendetransfautom ('enable-fields').
   END.
    WHEN 'Add-Record' THEN RUN dispatch IN h_tordendetransfautom ('add-record':U).
END CASE.
