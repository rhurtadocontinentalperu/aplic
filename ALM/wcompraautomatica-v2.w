...
FILL-IN-CodPro FILL-IN-PorStkMax FILL-IN-Marca BUTTON-7 BUTTON-20 BUTTON-1 ~
...
ELSE DO:
    ...
    REPEAT x-sec = 1 TO NUM-ENTRIES(pCodFam,","):
        ...
        FOR EACH Almmmatg USE-INDEX matg09 NO-LOCK WHERE Almmmatg.codcia = s-codcia
            ...
            /*IF LOOKUP(Almmmatg.codmat,"600234,101121") = 0 THEN NEXT.*/
...
FOR EACH T-MATG /*NO-LOCK*/ :
    ...
    FOR EACH TabGener NO-LOCK WHERE TabGener.CodCia = s-CodCia
        ...
        END.
        /*pStkComprometido = fStockComprometido(Almmmate.CodMat, Almmmate.CodAlm).*/
        RUN gn/stock-comprometido-v2.p (
            Almmmate.CodMat, 
            Almmmate.CodAlm, 
            NO, 
        ...
        ASSIGN T-MATG.stktrans = pStockTransito.
        ...
        IF AVAILABLE OOComPend THEN
            ...
            fCompras = fCompras + (OOComPend.CanPed - OOComPend.CanAte ).
        /*
        MESSAGE " fDisponible " fDisponible SKIP
                "fComprometido " fComprometido SKIP
                "fMaximo " fMaximo SKIP
                "fTransito " fTransito SKIP
                "fCompras " fCompras SKIP
                Almmmate.codmat.
        */
        IF LAST-OF(TabGener.Codigo) THEN DO:    /* FIN DEL GRUPO */
            FIND FIRST T-MATE USE-INDEX Mate03 WHERE T-MATE.codcia = s-CodCia AND
            ...
            fSaldoAlmacen = fDisponible + fTransito + fCompras.
            /*
            MESSAGE "fSaldoAlmacen " fSaldoAlmacen SKIP
                    "fMaximo " fMaximo SKIP
                    T-MATE.codmat.
            */
            /* Si es SOLO FALTANTES */
...
FOR EACH T-MATE:
    ...
    T-MATE.StkMin = T-MATE.StkMin * FILL-IN-PorStkMax / 100.
    /*
    MESSAGE "T-MATE.StkMin " T-MATE.StkMin SKIP
            "FILL-IN-PorStkMax " FILL-IN-PorStkMax SKIP
            "T-MATE.StkAct " T-MATE.StkAct SKIP
            "T-MATE.StkRep " T-MATE.StkRep SKIP
            "T-MATE.CmpTra " T-MATE.CmpTra SKIP
            T-MATE.codmat SKIP
...
FOR EACH TabGener NO-LOCK WHERE TabGener.CodCia = s-codcia
    ...
    END.
END.
IF toggle-todos = NO AND pReposicion <= 0 THEN DO:
    RETURN.    /* No hay nada que comprar */
END. 
...
REPEAT x-dia = x-fechaX TO dToday:
    REPEAT x-sec = 1 TO 2:
        /* Ingresos devolucion de clientes */
        IF x-sec = 1 THEN x-tipmov = 'I'.
        IF x-sec = 1 THEN x-codmov = 09.

        /* Salida x ventas */
        IF x-sec = 2 THEN x-tipmov = 'S'.        
        IF x-sec = 2 THEN x-codmov = 02.

        FOR EACH Almdmov USE-INDEX Almd02 NO-LOCK WHERE almdmov.codcia = s-codcia
            AND Almdmov.codmat = T-DREPO.CodMat
            AND Almdmov.fchdoc = x-dia  
            AND Almdmov.tipmov = x-tipmov
            AND Almdmov.codmov = x-codmov
            /*
            AND Almdmov.fchdoc >= x-fechaX  /*ADD-INTERVAL(dToday,  -3, 'months')*/
            AND Almdmov.fchdoc <= dToday*/ :

            FIND FIRST TabGener NO-LOCK WHERE TabGener.CodCia = s-codcia
                            AND TabGener.Clave = "ZG"
                            AND CAN-DO(FILL-IN_Grupos,TabGener.Codigo) /*LOOKUP(TabGener.Codigo, FILL-IN_Grupos) > 0*/
                            AND TabGener.Libre_c01= Almdmov.codalm NO-ERROR.
            ...
            */
            /* 
                Ic - 11Oct2019, correo Cesar Camus del dia 02 de Octubre

                Daniel / César
                    Necesitamos que por favor en el módulo de generación de compras al cálculo se retire las 
                    ventas generadas por las divisiones 070 y 030 en las columnas de información de ventas en 
                    todos los rangos expuestos 
                    Muchas gracias        
             */

            IF NOT (TRUE <> (x-coddiv > "")) THEN DO:
                FIND FIRST almcmov WHERE almcmov.codcia = almdmov.codcia AND
                ...
                IF AVAILABLE almcmov THEN DO:
                    ...
                    IF AVAILABLE ccbcdocu THEN DO:
                        IF LOOKUP(ccbcdocu.divori,x-coddiv) > 0 THEN DO:
                            NEXT.
                        END.
                    END.
                END.
...
/* Cargamos Ventas Año anterior */
/*  
    Correo de Cesar Camus del 01Dic2020 cuyo asunto dice : MÓDULO DE COMPRAS
    Los tres meses del año pasado
*/

/*
dToday = ADD-INTERVAL(TODAY,-1,'year').
x-fechaX = ADD-INTERVAL(dToday,3,'month').      /* Asi es la fecha limite de los 3 meses */
*/

x-fechaX = ADD-INTERVAL(TODAY,-1,'year').      /* Asi es la fecha limite de los 3 meses */
dToday = ADD-INTERVAL(x-fechaX,3,'month').
    ...
    REPEAT x-sec = 1 TO 2:
        ...
        FOR EACH Almdmov USE-INDEX Almd02 NO-LOCK WHERE almdmov.codcia = s-codcia
            ...
            FOR EACH Almdmov USE-INDEX Almd02 NO-LOCK WHERE almdmov.codcia = s-codcia
                AND Almdmov.codmat = T-DREPO.CodMat
                AND Almdmov.fchdoc >= dToday
                AND Almdmov.fchdoc <= x-fechaX  /*ADD-INTERVAL(dToday,3,'month') */ :
            */
            ...
            */
            /* 
                ...
                Daniel / César
                    Necesitamos que por favor en el módulo de generación de compras al cálculo se retire las ventas generadas por las divisiones 070 y 030 en las columnas de información de ventas en todos los rangos expuestos
                    Muchas gracias
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-9 RECT-1 RECT-11 BUTTON-Grupos COMBO-BOX-Tipo BUTTON-4 
         ...
         FILL-IN-PorStkMax FILL-IN-Marca BUTTON-7 BUTTON-20 BUTTON-1 BUTTON-5 
         BUTTON-IMPORTAR-EXCEL TOGGLE-todos BUTTON-21 BUTTON-22 
      WITH FRAME F-Main IN WINDOW W-Win.
