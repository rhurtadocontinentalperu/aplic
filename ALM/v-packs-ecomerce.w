...
------------------------------------------------------------------------------*/
  ...
  APPLY 'ENTRY':U TO vtactabla.llave IN FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = "NO" THEN DO:
      ASSIGN    
          ...
          vtactabla.usrmodificacion = s-user-id.
      FOR EACH Vtadtabla OF Vtactabla EXCLUSIVE-LOCK ON ERROR UNDO, RETURN 'ADM-ERROR':
          DELETE Vtadtabla.
      END.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
      IF RETURN-VALUE = 'NO' THEN DISABLE INTEGRAL.VtaCTabla.Llave.
/*       ENABLE                    */
/*           vtactabla.llave       */
/*           vtactabla.descripcion */
/*           vtactabla.libre_d01   */
...
DO WITH FRAME {&FRAME-NAME} :
    IF TRUE <> (vtactabla.llave:SCREEN-VALUE > '') THEN DO:
        MESSAGE 'Artículo en blanco' VIEW-AS ALERT-BOX ERROR.
    ...
    IF x-art-ok = NO THEN DO:
        MESSAGE "Este articulo(" + x-codmat + ") esta vacio no existe en el almacén" 
