...
&Scoped-Define ENABLED-OBJECTS RECT-16 RECT-18 RECT-19 F-Codfam DesdeC ~
HastaC Btn_OK Btn_Cancel I-TpoCto ICodMon 
&Scoped-Define DISPLAYED-OBJECTS C-ALMCEN D-FCHINV F-Codfam DesdeC HastaC ~
C-DESALM I-TpoCto ICodMon 
...
DEFINE FRAME D-Dialog
     ...
     F-Codfam AT ROW 3.73 COL 16.14 COLON-ALIGNED
     DesdeC AT ROW 4.65 COL 16.14 COLON-ALIGNED
     HastaC AT ROW 5.42 COL 16.14 COLON-ALIGNED
     C-DESALM AT ROW 1.58 COL 21.86 COLON-ALIGNED NO-LABEL
     Btn_OK AT ROW 6.73 COL 23
     Btn_Cancel AT ROW 6.73 COL 40
     I-TpoCto AT ROW 5 COL 44.57 NO-LABEL
     ICodMon AT ROW 3.73 COL 52.86 NO-LABEL
     RECT-16 AT ROW 3.54 COL 51.14
     RECT-18 AT ROW 1.23 COL 1
     "Moneda :" VIEW-AS TEXT
          SIZE 6.86 BY .62 AT ROW 3.73 COL 42.14
     RECT-19 AT ROW 4.69 COL 42.29
     SPACE(3.71) SKIP(2.41)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 4
...
/* SETTINGS FOR FILL-IN C-DESALM IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN D-FCHINV IN FRAME D-Dialog
   NO-ENABLE                                                            */
...
DO:
  ASSIGN DesdeC HastaC  ICodMon F-Codfam I-TpoCto.
...
DO:
  ...
  IF F-Codfam <> '' THEN DO:
     ...
     IF NOT AVAILABLE Almtfami THEN DO:
        MESSAGE 'Familia no existe.... ' VIEW-AS ALERT-BOX ERROR.
...
FOR EACH Almmmate WHERE 
         Almmmate.CodCia = S-CODCIA AND 
         Almmmate.CodAlm = S-CODALM AND 
         Almmmate.CodMat >= DesdeC  AND 
         Almmmate.CodMat <= HastaC NO-LOCK USE-INDEX Mate01,
         FIRST Almmmatg OF Almmmate WHERE 
               Almmmatg.codfam BEGINS F-Codfam NO-LOCK:
    DISPLAY Almmmate.CodMat @ Fi-Mensaje LABEL "Codigo de Articulo "
            FORMAT "X(8)" WITH FRAME F-Proceso.
    /* Calculamos el stock por entregar */
    F-StkFac = STOCKFACTURA(ALMMMATE.CodMat). 
    
    /* UBICAMOS EL STOCK GENERAL */
    FIND LAST Almdmov WHERE 
              Almdmov.CodCia = Almmmate.CodCia AND
    ...
    FIND InvRecont WHERE
         InvRecont.CodCia = Almmmate.CodCia AND
         InvRecont.CodAlm = Almmmate.CodAlm AND
         InvRecont.FchInv = D-FCHINV        AND
         InvRecont.CodMat = Almmmate.CodMat NO-LOCK NO-ERROR.
    IF AVAILABLE InvRecont THEN
       FOR EACH InvRecont WHERE 
                InvRecont.CodCia = Almmmate.CodCia AND
                InvRecont.CodAlm = Almmmate.CodAlm AND
                InvRecont.FchInv = D-FCHINV        AND
                InvRecont.CodMat = Almmmate.CodMat NO-LOCK:
           F-InvFis = F-InvFis + INVRECONT.CanInv.      
       END.
    ELSE
       FOR EACH InvConteo WHERE 
                InvConteo.CodCia = Almmmate.CodCia AND
                InvConteo.CodAlm = Almmmate.CodAlm AND
                InvConteo.FchInv = D-FCHINV        AND
                InvConteo.CodMat = Almmmate.CodMat NO-LOCK:
           F-InvFis = F-InvFis + INVCONTEO.CanInv.      
       END.
        
    /*    
    FIND InvConteo WHERE 
         InvConteo.CodCia = Almmmate.CodCia AND
         InvConteo.CodAlm = Almmmate.CodAlm AND
         InvConteo.CodMat = Almmmate.CodMat NO-LOCK NO-ERROR.
    FIND InvRecont WHERE 
         InvRecont.CodCia = Almmmate.CodCia AND
         InvRecont.CodAlm = Almmmate.CodAlm AND
         InvRecont.CodMat = Almmmate.CodMat NO-LOCK NO-ERROR.
    */   
    ...
    IF (C-TpoInv = "P" AND (AVAILABLE InvRecont OR AVAILABLE InvConteo)) OR 
       ...
       CREATE T-report.
       ASSIGN T-report.Llave-C    = Almmmate.CodMat
              T-report.Campo-c[1] = Almmmatg.DesMat
              T-report.Campo-c[2] = Almmmatg.UndStk
              T-report.Campo-c[3] = Almmmate.CodAlm
       ...
       IF F-STKALM = 0 AND T-report.Campo-f[6] < 0 THEN T-report.Campo-f[6] = 0.
...
------------------------------------------------------------------------------*/
  DISPLAY C-ALMCEN D-FCHINV F-Codfam DesdeC HastaC C-DESALM I-TpoCto ICodMon 
      WITH FRAME D-Dialog.
  ENABLE RECT-16 RECT-18 RECT-19 F-Codfam DesdeC HastaC Btn_OK Btn_Cancel 
         I-TpoCto ICodMon 
      WITH FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
 ...
 ASSIGN Almcmov.CodCia  = Almtdocm.CodCia 
        ...
        Almcmov.Observ  = 'AJUSTE POR DIFERENCIA DE INVENTARIO'
 ...
 FOR EACH T-REPORT WHERE T-report.Campo-f[6] > 0 :
     CREATE almdmov.
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST Almmmate WHERE 
               Almmmate.CodCia = S-CODCIA AND 
               Almmmate.CodAlm = S-CODALM AND 
               Almmmate.CodMat >= DesdeC  NO-LOCK NO-ERROR.
    DesdeC = Almmmate.Codmat.
    
    FIND LAST  InvConfig WHERE 
    ...
    OK-S = FALSE.
    FOR EACH T-REPORT:
        IF T-report.Campo-f[6] > 0 THEN OK-I = TRUE.
        IF T-report.Campo-f[6] < 0 THEN OK-S = TRUE.
    END.
    
    IF OK-I THEN RUN Genera-Ajuste-Ingreso.
    IF OK-S THEN RUN Genera-Ajuste-Salida.
