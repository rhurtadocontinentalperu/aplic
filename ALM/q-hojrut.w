...
&Scoped-define Condicion DI-rutac.Codcia = S-CODCIA AND DI-RutaC.CodDiv = S-CODDIV AND DI-RutaC.CodDoc = S-CODDOC
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH DI-RutaC WHERE ~{&KEY-PHRASE} ~
...
&Scoped-Define ENABLED-OBJECTS f-nrodoc 
&Scoped-Define DISPLAYED-OBJECTS f-nrodoc 
...
DEFINE VARIABLE f-nrodoc AS CHARACTER FORMAT "X(9)":U 
     VIEW-AS FILL-IN 
     SIZE 10 BY .73 NO-UNDO.
...
DEFINE FRAME FRAME-C
     f-nrodoc AT ROW 1.15 COL 2.86 NO-LABEL
...
/* SETTINGS FOR FRAME FRAME-C
                                                                        */
/* SETTINGS FOR FILL-IN f-nrodoc IN FRAME FRAME-C
   ALIGN-L                                                              */
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "{&Condicion}"
     _Design-Parent    is WINDOW q-tables @ ( 1.08 , 12.14 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
ON LEAVE OF f-nrodoc IN FRAME FRAME-C
DO:
  ASSIGN SELF:SCREEN-VALUE = STRING(INTEGER(SELF:SCREEN-VALUE), "999999999") NO-ERROR.
  FIND DI-RutaC WHERE {&Condicion} AND nrodoc = SELF:SCREEN-VALUE NO-LOCK NO-ERROR.
  IF AVAILABLE DI-RutaC
  THEN output-var-1 = ROWID(Di-RutaC).
  ELSE output-var-1 = ?.
