...
&Scoped-Define ENABLED-OBJECTS RECT-1 BUTTON-2 BUTTON-4 x-Barras BUTTON-3 ~
x-Tipo x-Copias x-size BUTTON-7 
...
DEFINE FRAME F-Main
     ...
     BUTTON-7 AT ROW 17.92 COL 101 WIDGET-ID 14
     RECT-1 AT ROW 10.81 COL 89.57
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 150.43 BY 18.42
         FONT 4.
...
ON CHOOSE OF BUTTON-7 IN FRAME F-Main /* Imprimir BARRA para el RACK */
DO:
    ...
    RUN Imprime-Barras-rack IN h_b-catmat
      (INPUT x-Tipo,
       INPUT x-Barras,
       INPUT x-Copias,
       INPUT x-size).
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv97 ).
       RUN set-position IN h_p-updv97 ( 1.19 , 3.00 ) NO-ERROR.
       RUN set-size IN h_p-updv97 ( 1.54 , 34.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/b-catmat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-catmat ).
       RUN set-position IN h_b-catmat ( 2.88 , 1.86 ) NO-ERROR.
       RUN set-size IN h_b-catmat ( 15.96 , 87.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 2.88 , 90.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-almtg1 ).
       RUN set-position IN h_v-almtg1 ( 4.50 , 89.57 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-almtg1 ).
       RUN set-position IN h_q-almtg1 ( 1.58 , 128.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.65 , 8.29 ) */

       /* Links to SmartBrowser h_b-catmat. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_b-catmat ).

       /* Links to SmartViewer h_v-almtg1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_v-almtg1 ).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 BUTTON-2 BUTTON-4 x-Barras BUTTON-3 x-Tipo x-Copias x-size 
         BUTTON-7 
      WITH FRAME F-Main IN WINDOW W-Win.
