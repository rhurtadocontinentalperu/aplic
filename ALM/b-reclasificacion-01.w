...
&Scoped-define QUERY-STRING-br_table FOR EACH ITEM WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      ...
      FIRST B-MATG WHERE B-MATG.CodCia = ITEM.CodCia ~
  AND B-MATG.codmat = ITEM.CodAnt OUTER-JOIN NO-LOCK ~
    BY ITEM.NroItm
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 130 BY 8.08
...
"ITEM.codmat" "Codigo Salida" "X(14)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"ITEM.CodAnt" "Codigo Entrada" "X(6)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
