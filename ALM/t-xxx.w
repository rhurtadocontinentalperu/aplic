...
DEF TEMP-TABLE TT-DMOV LIKE Almdmov
    INDEX LLAVE01 CodCia NroItm.
...
Almmmatg.DesMat Almmmatg.DesMar T-DMOV.CodUnd T-DMOV.CanDes 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 93 BY 10.77
         FONT 4.
...
"T-DMOV.CodUnd" "Unidad" "X(6)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.T-DMOV.CanDes
...
------------------------------------------------------------------------------*/
  ...
  x-NroItm = 1.
  FOR EACH TT-DMOV:
      MESSAGE tt-dmov.nroitm tt-dmov.codmat.
      x-NroItm = TT-DMOV.NroItm + 1.
  END.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
  FIND FIRST TT-DMOV NO-ERROR.
  IF AVAILABLE TT-DMOV THEN DELETE TT-DMOV.
