...
------------------------------------------------------------------------------*/
    ...
    IF TRUE <> (ITEM.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > "") THEN DO:
       MESSAGE "Código de articulo en blanco" VIEW-AS ALERT-BOX ERROR.
       APPLY "ENTRY" TO ITEM.CodMat.
       RETURN "ADM-ERROR".   
    END.
    ...
    IF NOT AVAILABLE Almmmate OR 
       ...
       IF NOT AVAILABLE Almmmate THEN 
          MESSAGE "Artículo no esta asignado al almacén " S-CodAlm VIEW-AS ALERT-BOX ERROR.
   ...
   IF TRUE <> (ITEM.CodAnt:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > "") THEN DO:
       MESSAGE "Código de artículo en blanco" VIEW-AS ALERT-BOX ERROR.
       APPLY "ENTRY" TO ITEM.CodAnt.
       RETURN "ADM-ERROR".   
    END.
    FIND B-MATG WHERE B-MATG.CodCia = S-CodCia AND
         ...
         NO-LOCK NO-ERROR.
    IF NOT AVAILABLE B-MATG THEN DO:
        MESSAGE "Artículo" ITEM.CodAnt:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
            "NO registrado" VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO ITEM.CodAnt.
        RETURN "ADM-ERROR".   
    END. 
    ...
    IF NOT AVAILABLE Almmmate THEN DO:
        MESSAGE "Artículo"  ITEM.CodAnt:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
            "no esta asignado al almacén " S-CodAlm VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO ITEM.CodAnt.
       RETURN "ADM-ERROR".   
    END. 
    /* ******************************************** */
    ...
    IF Almmmatg.UndStk <> B-MATG.UndStk THEN DO:
        /* Verificamos que pertenece al grupo reclasificador */
        ...
        IF NOT AVAILABLE Almdrecl THEN DO:
            MESSAGE 'Las unidades de medida SON diferentes entre los dos códigos' SKIP
                'Se requiere configurar en Grupo Clasificador' VIEW-AS ALERT-BOX ERROR.
            APPLY "ENTRY" TO ITEM.CodMat.
            RETURN "ADM-ERROR".   
        END.
    END.
    ...
    /* ******************************************** */
    /* VALORIZACION DEL INGRESO */
    /* 21/07/2022 : SLM en base al costo promedio */
    /* ******************************************** */
    DEF BUFFER b-Almstkge FOR Almstkge.
    ...
    IF NOT AVAILABLE Almstkge OR AlmStkge.CtoUni <= 0 THEN DO:
        MESSAGE 'Error en el costo prometido artículo:' ITEM.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO ITEM.CodMat.
        RETURN "ADM-ERROR".   
    END.
    FIND LAST b-Almstkge WHERE b-Almstkge.codcia = s-codcia
        AND b-Almstkge.codmat = ITEM.CodAnt:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
        AND b-Almstkge.fecha <= s-FchDoc
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE b-Almstkge OR b-AlmStkge.CtoUni <= 0 THEN DO:
        MESSAGE 'Error en el costo prometido artículo:' ITEM.CodAnt:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO ITEM.CodMat.
        RETURN "ADM-ERROR".   
    END.
    ...
    IF AVAILABLE Almdrecl THEN DO:
        x-CtoMat = AlmStkge.CtoUni * Almdrecl.Factor.
        x-CtoAnt = b-AlmStkge.CtoUni.
    END.
    ELSE DO:
        x-CtoMat = AlmStkge.CtoUni.
        x-CtoAnt = b-AlmStkge.CtoUni.
    END.
    /* ******************************************** */
    /* RHC 02/06/2015 */
    ...
    x-PorDif = ABS(x-PorDif).   /* Porcentual */
    
    /* 21/07/2022: SLM y DLL se establece el límite en 0.0001% */
    /* 27/07/2022: SLM 10% o no mas de 5 (S/ o US$) */
/*     IF x-pordif > 0.0001 THEN DO:                                                                          */
/*         MESSAGE 'La Diferencia de Costos Supera a la Permitida' '(' x-PorDif '%)' VIEW-AS ALERT-BOX ERROR. */
/*         APPLY "ENTRY" TO ITEM.CodMat.                                                                      */
/*         RETURN 'ADM-ERROR'.                                                                                */
/*     END.                                                                                                   */
    IF x-PorDif > 10 OR ABS(x-ctoant - x-ctomat) > 0.50 THEN DO:
        MESSAGE 'La Diferencia de Costos supera a la permitida' VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO ITEM.CodMat.
        RETURN 'ADM-ERROR'.
    END.
