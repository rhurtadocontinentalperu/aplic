...
DEFINE NEW SHARED TEMP-TABLE T-DDESP LIKE FacDdesp.
...
DEFINE VARIABLE h_b-reclasificacion-autom AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_t-reclasificacion-autom AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-movmto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-reclasificacion-autom AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 143.14 BY 25.12 WIDGET-ID 100.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: T-DDESP T "NEW SHARED" ? INTEGRAL FacDdesp
   END-TABLES.
 */
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ALM/v-reclasificacion-autom.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-reclasificacion-autom ).
       RUN set-position IN h_v-reclasificacion-autom ( 4.23 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.42 , 130.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/q-alcmov-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
       ...
       /* Links to SmartViewer h_v-reclasificacion-autom. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-reclasificacion-autom ).
       RUN add-link IN adm-broker-hdl ( h_q-alcmov-01 , 'Record':U , h_v-reclasificacion-autom ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-reclasificacion-autom ,
             h_p-updv05 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-alcmov-01 ,
             h_v-reclasificacion-autom , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ALM/b-reclasificacion-autom.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-reclasificacion-autom ).
       RUN set-position IN h_b-reclasificacion-autom ( 7.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-reclasificacion-autom ( 18.31 , 141.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-reclasificacion-autom. */
       RUN add-link IN adm-broker-hdl ( h_q-alcmov-01 , 'Record':U , h_b-reclasificacion-autom ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-reclasificacion-autom ,
             h_v-reclasificacion-autom , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/t-reclasificacion-autom.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-reclasificacion-autom ).
       RUN set-position IN h_t-reclasificacion-autom ( 7.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-reclasificacion-autom ( 17.23 , 136.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 24.69 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.42 , 34.14 ) NO-ERROR.

       /* Links to SmartBrowser h_t-reclasificacion-autom. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-reclasificacion-autom ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-reclasificacion-autom ,
             h_v-reclasificacion-autom , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-reclasificacion-autom , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
        IF h_b-reclasificacion-autom <> ? THEN  RUN dispatch IN h_b-reclasificacion-autom ('open-query').
        IF h_t-reclasificacion-autom <> ? THEN RUN dispatch IN h_t-reclasificacion-autom ('open-query').
    END.
    WHEN "Carga-Series" THEN RUN Carga-Series IN h_q-salida-01.
    WHEN "Pagina1"  THEN DO:
          RUN select-page(1).
          RUN dispatch IN h_b-reclasificacion-autom ('open-query').
      END.
    WHEN "Pagina2"  THEN DO:
          RUN select-page(2).
          RUN dispatch IN h_t-reclasificacion-autom ('open-query').
      END.
