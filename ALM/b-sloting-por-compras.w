...
DEFINE TEMP-TABLE T-MoviAlmacen LIKE OOMoviAlmacen
       ...
       FIELD Items AS INT
       .
DEFINE TEMP-TABLE TI-MoviAlmacen NO-UNDO LIKE OOMoviAlmacen
       FIELD Peso AS DEC
       FIELD Volumen AS DEC
       FIELD Items AS INT.
DEFINE TEMP-TABLE trf-OOMoviAlmacen NO-UNDO LIKE OOMoviAlmacen.
...
&SCOPED-DEFINE Condicion ( OOMoviAlmacen.codcia = s-codcia ~
    ...
    AND OOMoviAlmacen.FchDoc >= ADD-INTERVAL(TODAY,-2,'months') ~
...
DEFINE        VAR pZona    AS INTEGER NO-UNDO.
DEFINE        VAR pBultos  AS INTEGER NO-UNDO.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      END-FIELDS.
      TABLE: TI-MoviAlmacen T "?" NO-UNDO INTEGRAL OOMoviAlmacen
      ADDITIONAL-FIELDS:
          FIELD Peso AS DEC
          FIELD Volumen AS DEC
          FIELD Items AS INT
      END-FIELDS.
      TABLE: trf-OOMoviAlmacen T "?" NO-UNDO INTEGRAL OOMoviAlmacen
   END-TABLES.
 */
...
DEFINE INPUT PARAMETER pImprimirDetalle AS LOG.
...
IF ptoggle-rango = NO THEN DO:
    RUN Carga-Temporal.
END.
ELSE DO:
    RUN Carga-Temporal-impresion(pfill-in-desde,pfill-in-hasta).
    RUN carga-impresion-rango(s-Task-No,pHoraInicio,pImprimirDetalle).
    RETURN.
END.
...
FOR EACH T-MoviAlmacen NO-LOCK,
    ...
    IF pImprimirDetalle = NO THEN DO:
        CREATE w-report.
        ASSIGN
    ...
    END.
    ELSE DO:
        RUN lib/_time-passed (DATETIME(STRING(OOMoviAlmacen.fchdoc,'99/99/9999') + ' ' + OOMoviAlmacen.hradoc), 
                              pHoraInicio, 
                              OUTPUT pLeadTime).
    END.
    ...
    FOR EACH b-OOMoviAlmacen WHERE b-OOMoviAlmacen.CodCia = OOMoviAlmacen.CodCia
        ...
        IF pImprimirDetalle = NO THEN DO:
            ASSIGN w-report.Campo-I[1] = w-report.Campo-I[1] + 1.
        END.
        ELSE DO:
            CREATE w-report.
            ASSIGN
            ...
            ASSIGN
                ...
                w-report.Campo-D[1] = OOMoviAlmacen.MigFecha 
                w-report.Campo-C[5] = OOMoviAlmacen.MigHora
                w-report.Campo-C[9] = b-OOMoviAlmacen.codmat
                w-report.Campo-C[10] = almmmatg.desmat
                w-report.Campo-C[11] = almmmatg.desmar
                w-report.Campo-C[6] = pLeadTime
                w-report.Campo-C[12] = oomovialmacen.lpn
                w-report.Campo-D[2] = OOMoviAlmacen.ExpiryDate.

            FIND FIRST Almmmate WHERE Almmmate.codcia = s-codcia AND
                            Almmmate.codalm = b-OOMoviAlmacen.CodAlm AND
                            almmmate.codmat = b-OOMoviAlmacen.codmat NO-LOCK NO-ERROR.
    
            ASSIGN w-report.Campo-f[1] = w-report.Campo-f[1] + (b-OOMoviAlmacen.candes * b-OOMoviAlmacen.factor)
                     w-report.Campo-C[8] = b-OOMoviAlmacen.codund
                    w-report.Campo-C[7] = IF( AVAILABLE Almmmate) THEN Almmmate.codubi ELSE 'NE'.
        END.
    END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE carga-impresion-rango B-table-Win 
PROCEDURE carga-impresion-rango :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH TI-MoviAlmacen NO-LOCK,
    FIRST OOMoviAlmacen WHERE OOMoviAlmacen.CodCia = TI-MoviAlmacen.CodCia
    AND OOMoviAlmacen.CodAlm = TI-MoviAlmacen.CodAlm
    AND OOMoviAlmacen.TipMov = TI-MoviAlmacen.TipMov
    AND OOMoviAlmacen.CodMov = TI-MoviAlmacen.CodMov
    AND OOMoviAlmacen.NroSer = TI-MoviAlmacen.NroSer
    AND OOMoviAlmacen.NroDoc = TI-MoviAlmacen.NroDoc 
    /*AND OOMoviAlmacen.FlagMigracion = "N"*/ NO-LOCK:
    ...
    FOR EACH b-OOMoviAlmacen WHERE b-OOMoviAlmacen.CodCia = OOMoviAlmacen.CodCia
        ...
        AND b-OOMoviAlmacen.NroSer = OOMoviAlmacen.NroSer
        AND b-OOMoviAlmacen.NroDoc = OOMoviAlmacen.NroDoc 
        AND b-OOMoviAlmacen.FlagMigracion = "S" NO-LOCK,
        FIRST INTEGRAL.Almmmatg WHERE Almmmatg.CodCia = b-OOMoviAlmacen.CodCia
        AND Almmmatg.codmat = b-OOMoviAlmacen.codmat NO-LOCK:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE carga-temporal-impresion B-table-Win 
PROCEDURE carga-temporal-impresion :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
EMPTY TEMP-TABLE TI-MoviAlmacen.

FOR EACH OOMoviAlmacen WHERE OOMoviAlmacen.codcia = s-codcia
                            AND OOMoviAlmacen.FlagMigracion = "S"
                            AND (OOMoviAlmacen.flagfecha >= pfill-in-desde AND OOMoviAlmacen.flagfecha <= pfill-in-hasta)
                            AND OOMoviAlmacen.codalm = s-codalm
                            AND OOMoviAlmacen.TipMov = "I" 
                            AND OOMoviAlmacen.CodMov = 90
                            AND OOMoviAlmacen.UseInDropShipment = "no" NO-LOCK BREAK BY OOMoviAlmacen.CodAlm 
    BY OOMoviAlmacen.TipMov
    BY OOMoviAlmacen.CodMov
    BY OOMoviAlmacen.NroSer 
    BY OOMoviAlmacen.NroDoc:
    IF FIRST-OF(OOMoviAlmacen.CodAlm) 
        ...
        OR FIRST-OF(OOMoviAlmacen.NroDoc)
        THEN DO:
        CREATE TI-MoviAlmacen.
        BUFFER-COPY OOMoviAlmacen TO TI-MoviAlmacen.
        TI-MoviAlmacen.Peso = 0.
        TI-MoviAlmacen.Volumen = 0.
        TI-MoviAlmacen.Items = 0.
        FIND FIRST Faccpedi WHERE Faccpedi.codcia = s-codcia AND
            Faccpedi.coddoc = OOMoviAlmacen.CodRef AND
            Faccpedi.nroped = OOMoviAlmacen.NroRef
            NO-LOCK NO-ERROR.
        FOR EACH b-ooMoviAlmacen NO-LOCK WHERE b-ooMoviAlmacen.CodCia = ooMoviAlmacen.CodCia AND
            ...
            FIRST Almmmatg OF b-ooMoviAlmacen NO-LOCK:
            TI-MoviAlmacen.Items = TI-MoviAlmacen.Items + 1.
            TI-MoviAlmacen.Peso = TI-MoviAlmacen.Peso + (b-ooMoviAlmacen.CanDes * b-ooMoviAlmacen.Factor * Almmmatg.Pesmat).
            TI-MoviAlmacen.Volumen = TI-MoviAlmacen.Volumen + (b-ooMoviAlmacen.CanDes * b-ooMoviAlmacen.Factor * Almmmatg.Libre_d02).
        END.
        ASSIGN TI-MoviAlmacen.Volumen = TI-MoviAlmacen.Volumen / 1000000.
    END.
END.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* 19/06/2025 SOLO CrossDocking */
    IF (Almcmov.codmov = 90 OR Almcmov.codmov = 91) AND t-MoviAlmacen.almfinal > "" THEN DO:
        ...
        END.
    END.
    /* 10/07/2020 Los códigos pueden ir repetidos, los vamos acumulando */
    /* Vamos cargando el temporal también */
    EMPTY TEMP-TABLE trf-OOMoviAlmacen.
    DETALLE:
    REPEAT:
        ...
        CREATE trf-OOMoviAlmacen.
        BUFFER-COPY b-ooMoviAlmacen TO trf-ooMoviAlmacen.
        ...
        /* 
            Ic - 15Abr2025, Felix Perez, desde la recepcion de mercaderia x compras(OpenOrange) 
            viene el Lote y Fecha Vcto
         */
        IF t-MoviAlmacen.tipmov = 'i' AND t-MoviAlmacen.codmov = 90 THEN DO:
            IF NOT (TRUE <> (t-MoviAlmacen.lpn > '')) AND t-MoviAlmacen.ExpiryDate <> ? THEN DO:
                FIND FIRST maeLote WHERE maeLote.codmat = t-MoviAlmacen.codmat AND
                                            maeLote.clote = t-MoviAlmacen.lpn NO-LOCK NO-ERROR.
                IF NOT AVAILABLE maelote THEN DO:
                    CREATE maeLote.
                    ASSIGN maeLote.codmat = t-MoviAlmacen.codmat
                            maeLote.clote = t-MoviAlmacen.lpn 
                            maeLote.fregistro = TODAY
                            maeLote.hregistro = STRING(TIME,"hh:mm:ss").
                END.
                ELSE DO:
                    FIND CURRENT maeLote EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
                    IF ERROR-STATUS:ERROR = YES THEN DO:
                        DISPLAY "ERROR en MAELOTE: Registro en uso por otro usuario (almacen " t-MoviAlmacen.CodAlm  " mov " 
                            t-MoviAlmacen.TipMov  STRING(t-MoviAlmacen.CodMov)  " numero "  STRING(t-MoviAlmacen.NroSer, '999') 
                            STRING(t-MoviAlmacen.NroDoc, '999999999')  " producto "  t-MoviAlmacen.CodMat " lote " t-MoviAlmacen.lpn  ")"
                            WITH STREAM-IO NO-BOX WIDTH 320.
                        PAUSE 0.
                        UNDO CICLO, RETURN.
                    END.
                END.
                ASSIGN maeLote.fingresoalm = t-MoviAlmacen.fchdoc
                        maeLote.fvencimiento = t-MoviAlmacen.ExpiryDate.
    
            END.        
        END.
        /* **************************************************************************************** */
        /* Transferido y Cerrado */
        /* **************************************************************************************** */
        ASSIGN
            ...
            b-ooMoviAlmacen.FlagMigracion = "S"
            b-ooMoviAlmacen.FlgSit = "S"
            .
        RELEASE b-ooMoviAlmacen.
    END.
    /* **************************************************************************************** */
    /* GENERACION DE OTR POR COMPRAS CROSSDOCKING */
    /* **************************************************************************************** */
    IF t-MoviAlmacen.CodMov = 90 
        AND t-MoviAlmacen.almfinal > ""                         /* Si tiene almacen final hay que generar OTR */
        AND (TRUE <> (t-MoviAlmacen.flgsit > ""))               /* Indica generar OTR */
        AND t-MoviAlmacen.UseInDropShipment = "no"              /* NO DropShipping */
        THEN DO:
        DEFINE VAR hProc AS HANDLE.
        RUN gn/genera-od-otr-hpk-phr.p PERSISTENT SET hProc.
        RUN OTR_Generacion_por_Compras IN hProc (INPUT TABLE trf-OOMoviAlmacen,
                                                 INPUT pZona,
                                                 INPUT pBultos,
                                                 OUTPUT pMensaje).
        DELETE PROCEDURE hProc.
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            pMensaje = "ERROR : " + pMensaje.
            UNDO CICLO, RETURN "ADM-ERROR".
        END.
    END.
END.
...
/* ********************************************************************* */
/* MIGRACION DE ORDENES DE DESPACHO DEL ORIGEN AL DESTINO */
/* ********************************************************************* */
ASSIGN
    pZona = 0
    pBultos = 0.
IF AVAILABLE t-MoviAlmacen AND t-MoviAlmacen.CodMov = 90 
    AND t-MoviAlmacen.almfinal > ""                         /* Si tiene almacen final hay que generar OTR */
    AND (TRUE <> (t-MoviAlmacen.flgsit > ""))               /* Indica generar OTR */
    AND t-MoviAlmacen.UseInDropShipment = "no"              /* NO DropShipping */
    THEN DO:
    DEF VAR pOk AS LOG NO-UNDO.
    /* Solicitamos zona y bulto, similar a RECEPCION DE BULTOS DISTRIBUCION */
    FIND gn-divi WHERE gn-divi.codcia = s-codcia 
        AND gn-divi.coddiv = s-coddiv NO-LOCK NO-ERROR.
    RUN alm/d-ingreso-por-compras.w (INPUT ROWID(OOMoviAlmacen),
                                     INPUT gn-divi.Campo-Char[6],       /* PR SE */
                                     OUTPUT pZona,
                                     OUTPUT pBultos,
                                     OUTPUT pOk).
