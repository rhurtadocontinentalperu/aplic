...
&Scoped-define INTERNAL-TABLES AlmRCDoc AlmDCdoc
...
(IF NUM-ENTRIES(AlmDCdoc.CodCli,'|')>= 2 THEN ENTRY(2,AlmDCdoc.CodCli,'|')  ELSE '') @ x-NomCli ~
(IF NUM-ENTRIES(AlmDCdoc.CodCli,'|') >= 3 THEN  ENTRY(3,AlmDCdoc.CodCli,'|') ELSE '') @ x-CodAlm ~
AlmRCDoc.Bultos AlmRCDoc.Observ AlmRCDoc.Fecha AlmRCDoc.Hora 
...
&Scoped-define QUERY-STRING-br_table FOR EACH AlmRCDoc WHERE ~{&KEY-PHRASE} ~
 ...
 AND AlmRCDoc.usuario = fill-in-usuario NO-LOCK, ~
      FIRST AlmDCdoc WHERE AlmDCdoc.CodCia = AlmRCDoc.CodCia ~
  AND AlmDCdoc.CodDoc = AlmRCDoc.CodDoc ~
  AND AlmDCdoc.NroDoc = AlmRCDoc.NroDoc ~
      AND AlmDCdoc.FlgEst = "R" NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH AlmRCDoc WHERE ~{&KEY-PHRASE} ~
 ...
 AND AlmRCDoc.usuario = fill-in-usuario NO-LOCK, ~
  ...
  AND AlmDCdoc.NroDoc = AlmRCDoc.NroDoc ~
      AND AlmDCdoc.FlgEst = "R" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table AlmRCDoc AlmDCdoc
&Scoped-define FIRST-TABLE-IN-QUERY-br_table AlmRCDoc
&Scoped-define SECOND-TABLE-IN-QUERY-br_table AlmDCdoc
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      AlmRCDoc.NroDoc COLUMN-LABEL "<<Numero>>" FORMAT "X(12)":U
            WIDTH 12.57
      (IF NUM-ENTRIES(AlmDCdoc.CodCli,'|')>= 2 THEN ENTRY(2,AlmDCdoc.CodCli,'|')  ELSE '') @ x-NomCli COLUMN-LABEL "Nombre" FORMAT "x(45)":U
      (IF NUM-ENTRIES(AlmDCdoc.CodCli,'|') >= 3 THEN 
ENTRY(3,AlmDCdoc.CodCli,'|') ELSE '') @ x-CodAlm COLUMN-LABEL "Alm." FORMAT "x(3)":U
      AlmRCDoc.Bultos FORMAT "->,>>>,>>9":U
      AlmRCDoc.Observ FORMAT "x(40)":U
      AlmRCDoc.Fecha FORMAT "99/99/99":U
      AlmRCDoc.Hora FORMAT "X(5)":U WIDTH .43
  ENABLE
      AlmRCDoc.CodDoc
      AlmRCDoc.NroDoc
      AlmRCDoc.Bultos
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 107 BY 12.69
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.AlmRCDoc,INTEGRAL.AlmDCdoc WHERE INTEGRAL.AlmRCDoc ..."
 ...
 AND INTEGRAL.AlmRCDoc.usuario = fill-in-usuario"
     _JoinCode[2]      = "AlmDCdoc.CodCia = AlmRCDoc.CodCia
  AND AlmDCdoc.CodDoc = AlmRCDoc.CodDoc
  AND AlmDCdoc.NroDoc = AlmRCDoc.NroDoc"
     _Where[2]         = "AlmDCdoc.FlgEst = ""R"""
...
"AlmRCDoc.NroDoc" "<<Numero>>" "X(12)" "character" ? ? ? ? ? ? yes ? no no "12.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > "_<CALC>"
"(IF NUM-ENTRIES(AlmDCdoc.CodCli,'|')>= 2 THEN ENTRY(2,AlmDCdoc.CodCli,'|')  ELSE '') @ x-NomCli" "Nombre" "x(45)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > "_<CALC>"
"(IF NUM-ENTRIES(AlmDCdoc.CodCli,'|') >= 3 THEN 
ENTRY(3,AlmDCdoc.CodCli,'|') ELSE '') @ x-CodAlm" "Alm." "x(3)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"AlmRCDoc.Hora" ? ? "character" ? ? ? ? ? ? no ? no no ".43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  FIND B-DCDOC WHERE B-DCDOC.codcia = AlmRCdoc.codcia
    AND B-DCDOC.coddoc = AlmRCdoc.CodDoc
    AND B-DCDOC.nrodoc = AlmRCdoc.NroDoc
    AND B-DCDOC.flgest = 'R' 
    AND B-DCDOC.fchret = almrcdoc.fecha
    AND B-DCDOC.horret = almrcdoc.hora
    EXCLUSIVE-LOCK NO-ERROR.
  IF NOT AVAILABLE B-DCDOC 
  THEN DO:
