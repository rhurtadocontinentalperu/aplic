...
logisdchequeo.SerialNumber Almmmatg.DesMat Almmmatg.DesMar ~
logisdchequeo.UndVta logisdchequeo.CanChk logisdchequeo.Factor ~
logisdchequeo.CanPed logisdchequeo.FechaHora 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table logisdchequeo.CodMat ~
logisdchequeo.SerialNumber logisdchequeo.UndVta logisdchequeo.CanChk ~
logisdchequeo.Factor logisdchequeo.CanPed 
...
&Scoped-define QUERY-STRING-br_table FOR EACH logisdchequeo WHERE ~{&KEY-PHRASE} ~
 ...
 AND logisdchequeo.NroPed = x-NroDoc NO-LOCK, ~
    ...
    BY logisdchequeo.Etiqueta ~
       BY logisdchequeo.NroItm INDEXED-REPOSITION
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almmmatg.DesMat FORMAT "X(80)":U WIDTH 49
      ...
      logisdchequeo.CanChk COLUMN-LABEL "Cantidad" FORMAT ">,>>>,>>9.9999":U
            WIDTH 8.72
      logisdchequeo.Factor FORMAT ">>,>>9.9999":U WIDTH 6.43
      logisdchequeo.CanPed COLUMN-LABEL "Cantidad!Calculada" FORMAT ">,>>>,>>9.9999":U
      logisdchequeo.FechaHora FORMAT "99/99/9999 HH:MM:SS":U WIDTH 13.86
  ENABLE
...
"logisdchequeo.CodMat" "Articulo" "X(14)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.logisdchequeo.SerialNumber
"logisdchequeo.SerialNumber" "# de Serie" "x(25)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.Almmmatg.DesMat
"Almmmatg.DesMat" ? "X(80)" "character" ? ? ? ? ? ? no ? no no "49" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.Almmmatg.DesMar
"Almmmatg.DesMar" "!Marca" "X(20)" "character" ? ? ? ? ? ? no ? no no "11.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.logisdchequeo.UndVta
"logisdchequeo.UndVta" "Unidad" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.logisdchequeo.CanChk
"logisdchequeo.CanChk" "Cantidad" ? "decimal" ? ? ? ? ? ? yes ? no no "8.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.logisdchequeo.Factor
"logisdchequeo.Factor" ? ? "decimal" ? ? ? ? ? ? yes ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"logisdchequeo.CanPed" "Cantidad!Calculada" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > INTEGRAL.logisdchequeo.FechaHora
"logisdchequeo.FechaHora" ? "99/99/9999 HH:MM:SS" "datetime" ? ? ? ? ? ? no ? no no "13.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Solo el saldo en caso ya tenga una devolución parcial anterior
  
  OJO: Se establece el campo Libre_d05 como acumulador de devoluciones de la OTR
------------------------------------------------------------------------------*/
...
FOR EACH Facdpedi OF Faccpedi NO-LOCK WHERE Facdpedi.CanPed > Facdpedi.Libre_d05, 
    ...
    IF NOT AVAILABLE T-DPEDI THEN DO:
        ...
        ASSIGN
            ...
            T-DPEDI.CanPed = (Facdpedi.CanPed - Facdpedi.Libre_d05) * Facdpedi.Factor
            T-DPEDI.Factor = 1.     
    END.
    ELSE T-DPEDI.canped = T-DPEDI.canped + ((Facdpedi.CanPed - Facdpedi.Libre_d05) * Facdpedi.factor).
...
------------------------------------------------------------------------------*/
  ...
  /* Así no sea igual al total de la factura igual pasa */
  /* ***************************************************************************************** */
  ...
  IF CAN-FIND(FIRST t-Detalle WHERE t-Detalle.CanPed <> t-Detalle.CanChk) THEN DO:
      ...
      pEstado = "".
      RUN alm/d-cierre-chk-ingreso-dev-cli (INPUT-OUTPUT TABLE t-Detalle, OUTPUT pEstado).
  ...
  pMensaje = "".
  /* De acuerdo a la configuración de la sede se hace por sloting o ingreso directo */
  FIND FIRST TabGener WHERE TabGener.CodCia = s-CodCia
      AND TabGener.Clave = 'CFGINC'
      AND TabGener.Codigo = s-CodDiv
      NO-LOCK NO-ERROR.
  CASE TRUE:
      WHEN TabGener.Libre_L03 = NO THEN DO:
          /* Ingreso directo */
          MESSAGE 'Ingreso Directo'.
          RUN alm/p-genera-ing-dev-otr.p (INPUT x-CodDoc,
                                            INPUT x-NroDoc,
                                            INPUT x-UsrChq,
                                            INPUT pMotivo,
                                            INPUT pObserv,
          ...
          END.
      END.
      WHEN TabGener.Libre_L03 = YES THEN DO:
          /* Ingreso Sloting */
          MESSAGE 'Ingreso por Sloting'.
          RUN alm/p-genera-ing-dev-otr-v2.p (INPUT x-CodDoc,
                                             INPUT x-NroDoc,
                                             INPUT x-UsrChq,
                                             INPUT pMotivo,
                                             INPUT pObserv,
...
------------------------------------------------------------------------------*/
  ...
  FIND T-DPEDI WHERE T-DPEDI.codmat = logisdchequeo.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
      NO-LOCK NO-ERROR.
  IF NOT AVAILABLE T-DPEDI THEN DO:
      MESSAGE 'Artículo NO registrado en el comprobante' VIEW-AS ALERT-BOX ERROR.
      APPLY 'ENTRY':U TO logisdchequeo.CodMat.
      RETURN 'ADM-ERROR'.
  END.
  ...
  FIND Almmmatg WHERE Almmmatg.codcia = s-CodCia
      ...
      NO-LOCK.
  IF Almmmatg.RequiereSerialNr = "si" THEN DO:
      IF TRUE <> (logisdchequeo.SerialNumber:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > '')
          THEN DO:
      ...
      FIND fifommatg WHERE fifommatg.CodCia = Almmmatg.CodCia
          ...
          AND fifommatg.SerialNumber = logisdchequeo.SerialNumber:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
          NO-LOCK NO-ERROR.
      IF NOT AVAILABLE fifommatg THEN DO:
          ...
          RETURN 'ADM-ERROR'.
      END.
      /* Que haya salido en ese comprobante */
      DEF VAR LocalSerieOk AS LOG NO-UNDO.
      LocalSerieOk = NO.
      FOR EACH Almcmov NO-LOCK WHERE Almcmov.CodCia = s-CodCia
          ...
          EACH fifodmov NO-LOCK WHERE fifodmov.CodCia = Almcmov.CodCia
          AND fifodmov.CodAlm = Almcmov.CodAlm
          AND fifodmov.TipMov = Almcmov.TipMov
          AND fifodmov.CodMov = Almcmov.CodMov
          AND fifodmov.NroSer = Almcmov.NroSer
          AND fifodmov.NroDoc = Almcmov.NroDoc
          AND fifodmov.CodMat = Almmmatg.CodMat
          AND fifodmov.SerialNumber = logisdchequeo.SerialNumber:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}:
          LocalSerieOk = NO.
          LEAVE.
      END.
      IF LocalSerieOk = NO THEN DO:
          MESSAGE 'La serie del artículo no corresponde a este comprobante'
  ...
  FOR EACH b-Detalle NO-LOCK WHERE b-Detalle.CodCia = s-CodCia
      ...
      AND b-Detalle.CodPed = x-CodDoc
      AND b-Detalle.NroPed = x-NroDoc:
      IF b-Detalle.codmat = logisdchequeo.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
          THEN x-CanPed = x-CanPed + b-Detalle.canped.
  END.
  ...
  IF AVAILABLE T-DPEDI AND x-CanPed > T-DPEDI.canped THEN DO:
      MESSAGE 'CANTIDAD ingresada SUPERA la del comprobante' VIEW-AS ALERT-BOX ERROR.
      APPLY 'ENTRY':U TO logisdchequeo.CanChk.
      RETURN 'ADM-ERROR'.
  END.
