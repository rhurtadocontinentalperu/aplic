...
DEFINE TEMP-TABLE PEDI2 LIKE facdpedi.
...
IF NOT CAN-FIND(FIRST PEDI NO-LOCK) THEN RETURN 'ADM-ERROR'.
/* 2do. Dividimos PEDI en 2: PEDI2 va a contener los saldos */
/* RHC 26/10/18 Por ahora se bloquea hasta que se pongan de acuerdo */
/*RUN divide-temporal.*/

/* 3ro. Dividimos en dos procesos */
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        ...
        UNDO, LEAVE.
    END.
    /* RHC 25/02/2019 UNA sola OTR sin límite de items */
/*     EMPTY TEMP-TABLE PEDI.                              */
/*     FOR EACH PEDI2:                                     */
/*         CREATE PEDI.                                    */
/*         BUFFER-COPY PEDI2 TO PEDI.                      */
/*     END.                                                */
/*     IF CAN-FIND(FIRST PEDI NO-LOCK) THEN DO:            */
/*         RUN Rutina-Master.                              */
...
&IF DEFINED(EXCLUDE-divide-temporal) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE divide-temporal Procedure 
PROCEDURE divide-temporal :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Se supone que siempre trabajamos en unidades de stock
------------------------------------------------------------------------------*/

DEF VAR x-CanPed AS DEC NO-UNDO.        
EMPTY TEMP-TABLE PEDI2.    
FOR EACH PEDI, FIRST Almmmatg OF PEDI NO-LOCK WHERE Almmmatg.CanEmp > 0:
    IF PEDI.CanPed > Almmmatg.CanEmp THEN DO:
        x-CanPed = (PEDI.CanPed MODULO Almmmatg.CanEmp).
        /* Si hay un saldo sobrante */
        IF x-CanPed > 0 THEN DO:
            CREATE PEDI2.
            BUFFER-COPY PEDI TO PEDI2 ASSIGN PEDI2.CanPed = x-CanPed.
            PEDI.CanPed = PEDI.CanPed - PEDI2.CanPed.   /* Solo queda empaques master */
        END.
    END.
END.
...
&IF DEFINED(EXCLUDE-Rutina-Master) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Rutina-Master Procedure 
PROCEDURE Rutina-Master :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    IF B-CREPO.TipMov = "INC" THEN DO:
        CASE B-CREPO.Incidencia:
            WHEN "F" THEN ASSIGN Faccpedi.Glosa  = "INCIDENCIA: FALTANTE".
            WHEN "S" THEN ASSIGN Faccpedi.Glosa  = "INCIDENCIA: SOBRANTE".
            WHEN "M" THEN ASSIGN Faccpedi.Glosa  = "INCIDENCIA: MAL ESTADO".
        END CASE.
    END.
    /* *************************************************************************** */
    /* *************************************************************************** */
...
/*                         OUTPUT pMensaje).          */
    ...
    ASSIGN
        pMensaje2 = "ORDEN DE TRANSFERENCIA N° " + Faccpedi.NroPed + CHR(10) +
                    "Ffecha de entrega: " + STRING(lFechaPedido).
    /* *********************************************************** */
    /* *********************************************************** */
