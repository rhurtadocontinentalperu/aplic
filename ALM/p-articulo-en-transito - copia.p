...
*/
/*IF s-codalm = '31' THEN MESSAGE 'inicio'.*/
FOR EACH Almdrepo USE-INDEX Llave03 NO-LOCK WHERE Almdrepo.codcia = s-CodCia
    AND Almdrepo.codalm = s-CodAlm
    ...
    /*IF s-codalm = '31' THEN MESSAGE 'uno' almcrepo.nroser almcrepo.nrodoc.*/
    ...
    END.
    /*IF s-codalm = '31' THEN MESSAGE 'dos' almcrepo.nroser almcrepo.nrodoc.*/
END.
...
FOR EACH Facdpedi NO-LOCK WHERE Facdpedi.codcia = s-codcia
    ...
    IF NOT AVAIL tmp-tabla THEN DO:
        ...
        x-Total = x-Total + t-CanPed.
    END.
/*     MESSAGE facdpedi.codmat faccpedi.crossdocking faccpedi.almacenxd faccpedi.codcli */
...
/* Cuando la orden de compra aun no lo atiende el proveedor */
/* FOR EACH OOComPend WHERE OOComPend.CodAlm = s-CodAlm AND         */
/*                             OOComPend.CodMat = s-CodMat NO-LOCK: */
/*     x-Total = x-Total + (OOComPend.CanPed - OOComPend.CanAte).   */
/* END.                                                             */
...
/* Cuando la compra lo atendio el proveedor llego a CONTINENTAL y aun no afecta al KARDEX */
FOR EACH OOMoviAlmacen NO-LOCK WHERE OOMoviAlmacen.codcia = s-codcia        
    AND OOMoviAlmacen.CodAlm = s-CodAlm    
    AND OOMoviAlmacen.TipMov = "I" 
    AND OOMoviAlmacen.CodMov = 90    
    AND OOMoviAlmacen.FlagMigracion = "N"       /* aun NO afecta al kardex */
    AND OOMoviAlmacen.CodMat = s-CodMat    
    AND OOMoviAlmacen.FchDoc >= DATE(06,01,2019) 
    AND OOMoviAlmacen.UseInDropShipment = "NO",
    FIRST Almacen NO-LOCK WHERE Almacen.codcia = s-CodCia AND
