...
DEFINE VARIABLE h_b-reclasificacion-01 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_t-reclasificacion-01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-movmto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-reclasificacion-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 5.31 COL 92 WIDGET-ID 4
     RECT-4 AT ROW 3.96 COL 2 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 140.86 BY 15.69 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "RECLASIFICACION AUTOMATICA"
         HEIGHT             = 15.69
         WIDTH              = 140.86
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 140.86
...
ON END-ERROR OF W-Win /* RECLASIFICACION AUTOMATICA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* RECLASIFICACION AUTOMATICA */
DO:
...
DO:
  RUN alm/d-reclasificacion-01.
  RUN dispatch IN h_t-reclasificacion-01('open-query').
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/v-reclasificacion-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-reclasificacion-01 ).
       RUN set-position IN h_v-reclasificacion-01 ( 4.23 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-reclasificacion-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-reclasificacion-01 ).
       RUN add-link IN adm-broker-hdl ( h_q-alcmov-01 , 'Record':U , h_v-reclasificacion-01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-reclasificacion-01 ,
             h_p-updv05 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/b-reclasificacion-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-reclasificacion-01 ).
       RUN set-position IN h_b-reclasificacion-01 ( 7.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-reclasificacion-01 ( 8.08 , 130.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-reclasificacion-01 ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/t-reclasificacion-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-reclasificacion-01 ).
       RUN set-position IN h_t-reclasificacion-01 ( 7.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-reclasificacion-01 ( 8.08 , 136.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 15.27 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.42 , 34.14 ) NO-ERROR.

       /* Links to SmartBrowser h_t-reclasificacion-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-reclasificacion-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-reclasificacion-01 ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-reclasificacion-01 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
        IF h_b-reclasificacion-01 <> ? THEN  RUN dispatch IN h_b-reclasificacion-01 ('open-query').
        IF h_t-reclasificacion-01 <> ? THEN RUN dispatch IN h_t-reclasificacion-01 ('open-query').
    END.
    WHEN "Carga-Series" THEN RUN Carga-Series IN h_q-salida-01.
    WHEN "Pagina1"  THEN DO:
          RUN select-page(1).
          RUN dispatch IN h_b-reclasificacion-01 ('open-query').
    ...
    WHEN 'Activa-Boton' THEN DO:
        ...
        /* 21/07/2022: SLM desactivado hasta confirmar con K.Mujica */
