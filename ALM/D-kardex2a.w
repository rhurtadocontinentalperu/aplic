...
DEFINE TEMP-TABLE tt-datos
    FIELDS t-codcia AS INT
    FIELDS t-codmat AS CHAR
    FIELDS t-desmat AS CHAR
    FIELDS t-desmar AS CHAR
    FIELDS t-undbas AS CHAR
    FIELDS t-FchDoc AS DATE
    FIELDS t-TpoDoc AS CHAR
    FIELDS t-SerDoc AS INT
    FIELDS t-NroDoc AS CHAR
    FIELDS t-TpoOpe AS CHAR
    FIELDS t-Ingreso AS DECIMAL
    FIELDS t-PreIng AS DECIMAL
    FIELDS t-TotIng AS DECIMAL
    FIELDS t-Salida AS DECIMAL
    FIELDS t-Precio AS DECIMAL
    FIELDS t-TotSal AS DECIMAL
    FIELDS t-SALDO  AS DECIMAL
    FIELDS t-VALCTO AS DECIMAL
    FIELDS t-codalm AS CHAR
    FIELDS t-codmov AS CHAR
    FIELDS t-docnro AS INT
    FIELDS t-almori AS CHAR
    FIELDS t-codpro AS CHAR
    FIELDS t-codcli AS CHAR
    FIELDS t-codref AS CHAR
    FIELDS t-ref002 AS CHAR.    
...
DEFINE FRAME F-Main
     ...
     x-mensaje AT ROW 9.35 COL 6 COLON-ALIGNED NO-LABEL WIDGET-ID 8
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE cMovCod AS CHARACTER   NO-UNDO.
    DEFINE VARIABLE xCodpro LIKE Almcmov.codpro NO-UNDO.
    DEFINE VARIABLE xCodcli LIKE Almcmov.codcli NO-UNDO.
    DEFINE VARIABLE xNrorf1 LIKE Almcmov.nrorf1 NO-UNDO.
    DEFINE VARIABLE xNrorf2 LIKE Almcmov.nrorf2 NO-UNDO.
    ...
    FOR EACH Almmmatg NO-LOCK
        ...
        FOR EACH Almdmov NO-LOCK USE-INDEX ALMD02
            ...
            FIND Almcmov NO-LOCK
                ...
                AND Almcmov.NroDoc = Almdmov.nrodoc NO-ERROR.
            IF AVAILABLE Almcmov THEN DO:
                ...
                END.
                ASSIGN
                    xCodPro = Almcmov.codpro
                    xCodCli = Almcmov.codcli
                    xNroRf1 = Almcmov.nrorf1
                    xNroRf2 = Almcmov.nrorf2.
    
            END. /*If avail Almcmov*/
            ...
            cMovCod  = Almdmov.TipMov + STRING(Almdmov.CodMov,"99"). 
            ...
            /*Buscan Temporal*/
            FIND FIRST tt-datos WHERE t-codcia = almmmatg.codcia
                AND t-codmat = almmmatg.codmat
                AND t-fchdoc = Almdmov.FchDoc NO-ERROR.
            IF NOT AVAIL tt-datos THEN DO:
                CREATE tt-datos.
                ASSIGN
                    t-codcia  = almmmatg.codcia
                    t-codmat  = almmmatg.codmat  
                    t-desmat  = almmmatg.desmat
                    t-desmar  = almmmatg.desmar
                    t-undbas  = almmmatg.undbas.
            END.   
            ASSIGN
                t-FchDoc  = Almdmov.FchDoc         
                t-TpoDoc  = cTpoDoc                
                t-SerDoc  = iSerDoc                 
                t-NroDoc  = cNroDoc                
                t-TpoOpe  = cTpoOpe                
                ...
                t-codmov  = cMovCod
                t-docnro  = almdmov.nrodoc
                t-codpro = xCodpro
                t-codcli = xCodcli
                t-codref = xNrorf1
                t-ref002 = xNrorf2.        
            IF almdmov.codmov = 03 THEN ASSIGN t-almori = almdmov.almori.

        END. /*For each almdmov....*/
        DISPLAY "Código de Artículo: " + Almmmatg.CodMat @ x-mensaje
            WITH FRAME {&FRAME-NAME}.  
        PAUSE 0.
    END. /*For each almmmatg...*/

    /*Busca Stock a codigos s/n mov*/
    FOR EACH Almmmatg NO-LOCK
        WHERE Almmmatg.CodCia = S-CODCIA
        AND Almmmatg.CodMat >= DesdeC
        AND Almmmatg.CodMat <= HastaC
        ...
        IF NOT AVAIL tt-datos THEN DO:
            ...
            IF AVAILABLE AlmStkGe THEN DO:
                ...
                ASSIGN
                    ...
                    t-desmat = almmmatg.desmat
                    t-FchDoc = (DesdeF - 1)
                    t-NroDoc = "Saldo Inicial"
                    t-Precio = AlmStkGe.CtoUni
                    t-SALDO  = AlmStkGe.StkAct
                    t-VALCTO = (AlmStkGe.StkAct * AlmStkGe.CtoUni).
            END.
        END.
        PAUSE 0.
    END.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH tt-datos 
        BREAK BY tt-datos.t-codcia 
            BY tt-datos.t-codmat 
            BY tt-datos.t-fchdoc:

        DISPLAY "Código de Artículo: " + t-CodMat @ x-mensaje
            WITH FRAME {&FRAME-NAME}.            

        IF FIRST-OF(tt-datos.t-codmat) THEN DO:
            iCount = iCount + 1.
            cColumn = STRING(iCount).
            cRange = "A" + cColumn.
            ...
            cRange = "M" + cColumn.
            chWorkSheet:Range(cRange):VALUE = t-precio.
            cRange = "N" + cColumn.
            chWorkSheet:Range(cRange):VALUE = t-saldo.
            cRange = "O" + cColumn.
            chWorkSheet:Range(cRange):VALUE = t-valcto.
        END.

        ACCUMULATE t-Ingreso (TOTAL BY t-CodMat).
        ACCUMULATE t-Salida (TOTAL BY t-CodMat).

        iCount = iCount + 1.
        cColumn = STRING(iCount).
        cRange = "A" + cColumn.
        chWorkSheet:Range(cRange):VALUE = "'" + t-codalm.
        cRange = "B" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-codmov.
        cRange = "C" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-nrodoc.
        cRange = "D" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-almori.       
        cRange = "E" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-codpro.
        cRange = "F" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-codcli.
        cRange = "G" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-codref.
        cRange = "H" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-ref002.
        cRange = "I" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-fchdoc.
        cRange = "J" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-ingreso.
        cRange = "K" + cColumn.
        chWorkSheet:Range(cRange):VALUE = f-salida.
        /*
        IF  Almdmov.TipMov = "I" AND (ALmtmovm.TpoCto = 0 OR ALmtmovm.TpoCto = 1) THEN DO:
        ...
        */
        ...
        cRange = "O" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-valcto.
        IF LAST-OF(t-CodMat) THEN DO:
            iCount = iCount + 1.
            cColumn = STRING(iCount).
            cRange = "J" + cColumn.
            chWorkSheet:Range(cRange):VALUE = (ACCUM TOTAL BY t-CodMat t-Ingreso).
            cRange = "K" + cColumn.
            chWorkSheet:Range(cRange):VALUE = (ACCUM TOTAL BY t-CodMat t-Salida).
            x-total = x-total + t-VALCTO.
        END.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE FRAME f-header
        ...
        "RAZON SOCIAL        :" S-NOMCIA FORMAT "X(50)" SKIP
        "ESTABLECIMIENTO     :" SKIP
        "CODIGO              :" tt-datos.t-codmat SKIP
        "TIPO                :" SKIP
        "DESCRIPCION         :" tt-datos.t-desmat SKIP
        "UNIDAD DE MEDIDA    :" SKIP
        "METODO DE VALUACION : PROMEDIO PONDERADO"
        WITH PAGE-TOP WIDTH 250 NO-LABELS NO-UNDERLINE STREAM-IO DOWN.

    DEFINE FRAME f-report
        t-FchDoc  COLUMN-LABEL "FECHA DE!DOCUMENT" FORMAT "99/99/99"
        t-TpoDoc  COLUMN-LABEL "TP!DC"
        t-SerDoc  COLUMN-LABEL "SE-!RIE"
        t-NroDoc  COLUMN-LABEL "NUMERO!DOCUMEN"
        t-TpoOpe  COLUMN-LABEL "TP!OP"
        t-Ingreso COLUMN-LABEL "INGRESOS"     FORMAT '->>>,>>9.99'
        t-PreIng  COLUMN-LABEL "COSTO"        FORMAT '->>>,>>9.9999'
        t-TotIng  COLUMN-LABEL "TOTAL"        FORMAT '->>>>>,>>9.99'
        t-Salida  COLUMN-LABEL "SALIDAS"      FORMAT '->>>>,>>9.99'
        t-PRECIO  COLUMN-LABEL "COSTO"        FORMAT '->>>>9.9999'
        t-TotSal  COLUMN-LABEL "TOTAL"        FORMAT '->>>>>,>>9.99'
        t-SALDO   COLUMN-LABEL "SALDO"        FORMAT '->>>>,>>9.99'
        t-VALCTO  COLUMN-LABEL "COSTO!TOTAL"  FORMAT '->>>>>,>>9.99'
        WITH WIDTH 250 STREAM-IO DOWN.
    ...
    FOR EACH tt-datos 
        ...
        IF LAST-OF(t-CodMat) THEN DO:
            UNDERLINE STREAM REPORT
                ...
                WITH FRAME f-report.
            DISPLAY STREAM REPORT
                ACCUM TOTAL BY t-CodMat t-Ingreso
                    WHEN (ACCUM TOTAL BY t-CodMat t-Ingreso) > 0 @ t-Ingreso
                ACCUM TOTAL BY t-CodMat t-Salida  @ t-Salida
                WITH FRAME f-report. 
            x-total = x-total + t-VALCTO.
            PAGE STREAM Report.
        END.
