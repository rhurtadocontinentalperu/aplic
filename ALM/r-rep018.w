...
&Scoped-Define ENABLED-OBJECTS TOGGLE-Almacenes x-CodFam x-CodPro ~
TOGGLE-Resumen BUTTON-6 BUTTON-7 
&Scoped-Define DISPLAYED-OBJECTS TOGGLE-Almacenes txt-codalm x-CodFam ~
x-desFam x-CodPro x-NomPro FILL-IN-DiasMinimo FILL-IN-DiasMaximo ~
TOGGLE-Resumen x-mensaje 
...
DEFINE VARIABLE TOGGLE-Almacenes AS LOGICAL INITIAL yes 
...
DEFINE FRAME F-Main
     TOGGLE-Almacenes AT ROW 1.81 COL 13 WIDGET-ID 90
     txt-codalm AT ROW 2.88 COL 11 COLON-ALIGNED WIDGET-ID 30
     BUTTON-1 AT ROW 2.88 COL 71 WIDGET-ID 62
     x-CodFam AT ROW 3.96 COL 11 COLON-ALIGNED WIDGET-ID 70
     x-desFam AT ROW 3.96 COL 19 COLON-ALIGNED NO-LABEL WIDGET-ID 72
     x-CodPro AT ROW 5.04 COL 11 COLON-ALIGNED WIDGET-ID 74
     x-NomPro AT ROW 5.04 COL 26 COLON-ALIGNED NO-LABEL WIDGET-ID 76
     FILL-IN-DiasMinimo AT ROW 6.38 COL 19 COLON-ALIGNED WIDGET-ID 86
     FILL-IN-DiasMaximo AT ROW 7.46 COL 19 COLON-ALIGNED WIDGET-ID 88
     TOGGLE-Resumen AT ROW 8.81 COL 12 WIDGET-ID 82
     BUTTON-6 AT ROW 10.15 COL 2 WIDGET-ID 64
     BUTTON-7 AT ROW 10.15 COL 9 WIDGET-ID 66
     x-mensaje AT ROW 10.69 COL 17 COLON-ALIGNED NO-LABEL WIDGET-ID 68
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 82.86 BY 11.27 WIDGET-ID 100.
...
DO:
    ASSIGN TOGGLE-Almacenes
        txt-codalm x-codfam x-codpro FILL-IN-DiasMaximo FILL-IN-DiasMinimo TOGGLE-Resumen.
    IF txt-codalm = "" AND TOGGLE-Almacenes = NO THEN DO:
...
&Scoped-define SELF-NAME TOGGLE-Almacenes
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL TOGGLE-Almacenes W-Win
ON VALUE-CHANGED OF TOGGLE-Almacenes IN FRAME F-Main /* Todos los almacenes */
DO:
  ASSIGN {&self-name}.
  IF INPUT {&self-name} = NO
      THEN ASSIGN  BUTTON-1:SENSITIVE = YES txt-codalm:SENSITIVE = YES.
  ELSE ASSIGN  BUTTON-1:SENSITIVE = NO txt-codalm:SENSITIVE = NO.
...
IF TOGGLE-Almacenes = YES THEN DO:
    txt-codalm = ''.
    FOR EACH almacen NO-LOCK WHERE almacen.codcia = s-codcia:
        IF txt-codalm = '' THEN txt-codalm = TRIM(almacen.codalm).
        ELSE txt-codalm = txt-codalm + ',' + TRIM(almacen.codalm).
    END.
END.
/* Cargamos stocks */
        ...
        IF AVAILABLE Almmmate THEN DO:
            ...
            BUFFER-COPY Almmmatg TO T-MATG
                ASSIGN
                    ...
                    T-MATG.StkMin = DECIMAL(Almmmate.Libre_c02)
                    /*T-MATG.StkMax = Almmmate.StkMax*/
                    T-MATG.StkMax = DECIMAL(Almmmate.Libre_c03)
                    /*T-MATG.StkRep = Almmmate.StkRep*/
                    T-MATG.StkRep = DECIMAL(Almmmate.Libre_c04)
                    T-MATG.Libre_d01 = Almmmate.Libre_d01.
...
FOR EACH T-MATG:
    ...
    T-MATG.Cantidad = ROUND(T-MATG.Cantidad / T-MATG.CanEmp, 0).
        ...
        IF AVAILABLE Almmmate THEN DO:
            ...
            BUFFER-COPY Almmmatg 
                ...
                ASSIGN
                    ...
                    T-MATG.StkMin = T-MATG.StkMin + DECIMAL(Almmmate.Libre_c02)
                    /*T-MATG.StkMax = T-MATG.StkMax + Almmmate.StkMax*/
                    T-MATG.StkMax = T-MATG.StkMax + DECIMAL(Almmmate.Libre_c03)
                    /*T-MATG.StkRep = T-MATG.StkRep + Almmmate.StkRep*/
                    T-MATG.StkRep = T-MATG.StkRep + DECIMAL(Almmmate.Libre_c04)
                    /*x-Factor = x-Factor + ( Almmmate.libre_d01 * Almmmate.StkRep )*/
                    x-Factor = x-Factor + ( Almmmate.libre_d01 * DECIMAL(Almmmate.Libre_c04) ).
            IF T-MATG.StkRep <> 0 THEN T-MATG.Libre_d01 = x-Factor / T-MATG.StkRep.    /* Ponderado */
            IF Almmmatg.CanEmp = 0 THEN T-MATG.CanEmp = 1.
            IF Almmmatg.MonVta = 2 THEN T-MATG.CtoTot = T-MATG.CtoTot * T-MATG.TpoCmb.
        END.
    END.
...
FOR EACH T-MATG:
    ...
    IF T-MATG.StkAct < T-MATG.Minimo AND T-MATG.CanEmp > 0 THEN DO:
        T-MATG.Compras = ROUND ( ABS(T-MATG.Cantidad) / T-MATG.CanEmp , 0) * T-MATG.CanEmp.
...
------------------------------------------------------------------------------*/
  DISPLAY TOGGLE-Almacenes txt-codalm x-CodFam x-desFam x-CodPro x-NomPro 
          FILL-IN-DiasMinimo FILL-IN-DiasMaximo TOGGLE-Resumen x-mensaje 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE TOGGLE-Almacenes x-CodFam x-CodPro TOGGLE-Resumen BUTTON-6 BUTTON-7 
      WITH FRAME F-Main IN WINDOW W-Win.
...
x-Llave = x-Llave + "Producto" + CHR(9).
...
x-Llave = x-Llave + "Clase" + CHR(9).
x-Llave = x-Llave + "Ranking" + CHR(9).
x-Llave = x-Llave + "".
PUT STREAM REPORTE x-LLave SKIP.
FOR EACH T-MATG:
    x-Llave = "".
    x-Llave = x-Llave + STRING(T-MATG.codalm, "999") + CHR(9).
    x-Llave = x-Llave + STRING(T-MATG.codmat, "999999").
...
/*     x-Llave = x-Llave + STRING(T-MATG.stkmax) + CHR(9). */
    ...
    x-Llave = x-Llave + T-MATG.TipRot[1] + CHR(9).
    x-Llave = x-Llave + STRING(T-MATG.OrdTmp, '>>>>>9') + CHR(9).
    x-Llave = x-Llave + "".
    PUT STREAM REPORTE x-LLave SKIP.
END.            
