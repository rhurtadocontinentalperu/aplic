...
DEFINE NEW SHARED VAR L-NROSER AS CHAR.    /* Nº Serie Válidos */
DEFINE NEW SHARED VAR S-MOVVAL  AS LOGICAL.
DEFINE NEW SHARED VAR S-CODMOV  AS INTEGER.
DEFINE NEW SHARED VAR ORDTRB    AS CHAR.
...
FIND FIRST Almtmovm WHERE Almtmovm.CodCia = S-CODCIA 
    ...
    AND Almtmovm.Indicador[1] = YES      /* RESERVADO */
    NO-LOCK NO-ERROR.
...
&Scoped-Define ENABLED-OBJECTS RECT-1 BUTTON-Excel 
...
DEFINE VARIABLE h_b-ingreso-transfer AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_t-ingreso-transfer AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-ingreso-transfer AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-Excel AT ROW 1.54 COL 98 WIDGET-ID 2
     RECT-1 AT ROW 3.96 COL 2 WIDGET-ID 4
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "INGRESO POR TRANSFERENCIA INTERNAS Y CROSSDOCKING"
         HEIGHT             = 24.54
         WIDTH              = 118.29
         MAX-HEIGHT         = 25.73
         MAX-WIDTH          = 143.86
...
ON END-ERROR OF W-Win /* INGRESO POR TRANSFERENCIA INTERNAS Y CROSSDOCKING */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* INGRESO POR TRANSFERENCIA INTERNAS Y CROSSDOCKING */
DO:
...
DO:
  RUN To-Excel IN h_v-ingreso-transfer.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-movmto ).
       RUN set-position IN h_v-movmto ( 1.00 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 2.35 , 22.72 ) NO-ERROR.
       ...
       RUN set-size IN h_p-updv05 ( 1.42 , 54.43 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ALM/v-ingreso-por-transferencia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ingreso-transfer ).
       RUN set-position IN h_v-ingreso-transfer ( 4.23 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.65 , 111.72 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-trfing ).
       RUN set-position IN h_q-trfing ( 1.15 , 72.86 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-alctrf ).
       RUN set-position IN h_q-alctrf ( 2.62 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.12 , 20.29 ) */

       /* Links to SmartViewer h_v-movmto. */
       RUN add-link IN adm-broker-hdl ( h_q-trfing , 'Record':U , h_v-movmto ).

       /* Links to SmartViewer h_v-ingreso-transfer. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-ingreso-transfer ).
       RUN add-link IN adm-broker-hdl ( h_q-alctrf , 'Record':U , h_v-ingreso-transfer ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ingreso-transfer ,
             h_p-updv05 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-trfing ,
             h_v-ingreso-transfer , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-alctrf ,
             h_q-trfing , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ALM/b-ingreso-transfer.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ingreso-transfer ).
       RUN set-position IN h_b-ingreso-transfer ( 10.15 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-ingreso-transfer ( 15.08 , 116.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-ingreso-transfer. */
       RUN add-link IN adm-broker-hdl ( h_q-alctrf , 'Record':U , h_b-ingreso-transfer ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-ingreso-transfer ,
             h_v-ingreso-transfer , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ALM/t-ingreso-transfer.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-ingreso-transfer ).
       RUN set-position IN h_t-ingreso-transfer ( 10.15 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-ingreso-transfer ( 15.08 , 116.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-ingreso-transfer ,
             h_v-ingreso-transfer , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
         IF h_b-ingreso-transfer <> ?     THEN RUN dispatch IN h_b-ingreso-transfer ('open-query':U).
         IF h_t-ingreso-transfer <> ?  THEN RUN dispatch IN h_t-ingreso-transfer ('open-query':U).
      END.
    ...
    WHEN "Pagina2" THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         IF h_t-ingreso-transfer <> ? THEN RUN dispatch IN h_t-ingreso-transfer ('open-query':U).
      END.
    WHEN "Last-Record" THEN DO:
         IF h_q-alctrf <> ? THEN RUN dispatch IN h_q-alctrf ('get-last':U).
    END.
END CASE.
