...
DEF TEMP-TABLE Reporte
    FIELD NroSer LIKE Almdmov.NroSer LABEL 'Serie'
    FIELD NroDoc LIKE Almdmov.NroDoc LABEL 'Numero'
    FIELD FchDoc LIKE Almdmov.FchDoc LABEL 'Fecha' FORMAT '99/99/9999'
    FIELD TipMov LIKE Almdmov.TipMov LABEL 'Tip Mov'
    FIELD CodMov LIKE Almdmov.CodMov LABEL 'Cod Mov' FORMAT '99'
    FIELD AlmOri LIKE Almdmov.AlmOri LABEL 'Alm Ref'
    FIELD CodUnd LIKE Almdmov.CodUnd LABEL 'Unidad'
    FIELD Ingreso LIKE Almdmov.CanDes LABEL 'Ingresos'
    FIELD Salida LIKE Almdmov.CanDes LABEL 'Salidas'
    FIELD StkSub LIKE Almdmov.StkSub LABEL 'Stock Acumulado'
    .
...
&Scoped-Define ENABLED-OBJECTS BUTTON-2 IMAGE-3 BROWSE-1 
...
DEFINE QUERY BROWSE-1 FOR 
    ...
    FIELDS(Almdmov.NroSer
      ...
      Almdmov.StkSub), 
      Almcmov
    FIELDS(Almcmov.CodRef
      Almcmov.NroRef
      Almcmov.DateUpdate) SCROLLING.
&ANALYZE-RESUME
...
ON CHOOSE OF BUTTON-2 IN FRAME D-Dialog /* A TEXTO */
DO:
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-CodMat FILL-IN-DesMat F-DESMOV FILL-IN-Mensaje 
      WITH FRAME D-Dialog.
  ENABLE BUTTON-2 IMAGE-3 BROWSE-1 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
IF AVAILABLE almdmov THEN DO:
    FIND ALMTMOVM WHERE Almtmovm.CodCia = S-CODCIA AND 
        Almtmovm.Codmov = Almdmov.CodMov AND 
        Almtmovm.Tipmov = Almdmov.TipMov
        NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH Almdmov NO-LOCK USE-INDEX ALMD03 WHERE Almdmov.codcia = s-codcia
            ...
            BY Almdmov.nrodoc DESC:
        CREATE Reporte.
        BUFFER-COPY Almdmov TO Reporte.
        IF LOOKUP(Almdmov.TipMov, "I,R,U" ) = 0  
        THEN Reporte.Ingreso = Almdmov.CanDes.
        ELSE Reporte.Salida = Almdmov.CanDes.
    END.
    SESSION:SET-WAIT-STATE('').
