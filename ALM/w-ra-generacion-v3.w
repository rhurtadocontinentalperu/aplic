...
PROCEDURE Carga-Cantidad-Reponer-B PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE Carga-Productos PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
ELSE DO:
    EMPTY TEMP-TABLE T-MATG.
    /* 19/3/25: Optimizando la carga de acuerdo al filtro activo */
    CASE TRUE:
        WHEN FILL-IN-CodFam > "" THEN DO:
            DEF VAR k AS INTE NO-UNDO.
            DO k = 1 TO NUM-ENTRIES(FILL-IN-CodFam):
                FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia 
                    AND Almmmatg.codfam = ENTRY(k, FILL-IN-CodFam)
                    AND Almmmatg.tpoart <> "D":
                    IF FILL-IN-SubFam > "" AND LOOKUP(Almmmatg.subfam, FILL-IN-SubFam) = 0 THEN NEXT.
                    IF FILL-IN-CodPro > "" AND Almmmatg.codpr1 <> FILL-IN-CodPro THEN NEXT.
                    IF FILL-IN-Marca > "" AND NOT Almmmatg.desmar BEGINS FILL-IN-Marca THEN NEXT.
                    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "SELECCIONANDO: " + Almmmatg.codmat + " " + Almmmatg.desmat.
                    {alm/i-ra-gen-carga-prod.i}
                END.
            END.
        END.
        WHEN FILL-IN-Marca > "" THEN DO:
            FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia 
            ...
            END.
        END.
        WHEN FILL-IN-CodPro > "" THEN DO:
            FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND
                Almmmatg.codpr1 = FILL-IN-CodPro AND Almmmatg.tpoart <> "D":
                FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "SELECCIONANDO: " + Almmmatg.codmat + " " + Almmmatg.desmat.
                {alm/i-ra-gen-carga-prod.i}
            END.
        END.
...
PROCEDURE CARGA-REPOSICION PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
