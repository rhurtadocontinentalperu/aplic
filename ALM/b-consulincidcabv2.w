...
DEF TEMP-TABLE Cabecera
    ...
    FIELD FchReg        AS DATE                 LABEL 'Fecha de Regularización de Inc'
...
DEF TEMP-TABLE Detalle LIKE Cabecera
    ...
    FIELD CtoKardex     AS DECI FORMAT '->>>,>>>,>>9.99'    LABEL 'Costo Kardex'
    .
************** */
...
FILL-IN-AlmOri FILL-IN-FchDoc-1 FILL-IN-FchDoc-2 FILL-IN-CodMat BUTTON-2 ~
BUTTON-3 br_table 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-FlgEst FILL-IN-AlmDes ~
FILL-IN-AlmOri FILL-IN-FchDoc-1 FILL-IN-FchDoc-2 FILL-IN-CodMat ~
FILL-IN-DesMat 
...
DEF VAR dFchReg AS DATE NO-UNDO.

GET FIRST {&BROWSE-NAME}.
REPEAT WHILE AVAILABLE AlmCIncidencia:
    CREATE Cabecera.

    {alm/i-consul-incid-cab.i &Tabla="Cabecera"}
...
/*     IF AlmcIncidencia.FlgEst = "C" THEN ASSIGN Cabecera.FchReg = AlmCIncidencia.FechaAprobacion Cabecera.UsrReceptor = AlmCIncidencia.UsrAprobacion. */
/*     IF AlmcIncidencia.FlgEst = "A" THEN ASSIGN Cabecera.FchReg = AlmCIncidencia.FechaAnulacion Cabecera.UsrReceptor = AlmCIncidencia.UsrAnulacion.   */
/*                                                                                                                                                      */
/*     FIND gn-user WHERE gn-user.codcia = s-codcia AND gn-user.USER-ID = Cabecera.UsrReceptor NO-LOCK NO-ERROR.                                        */
/*     IF AVAILABLE gn-user THEN Cabecera.UsrReceptor = gn-users.User-Name.                                                                             */
/*                                                                                                                                                      */
/*     FIND gn-user WHERE gn-user.codcia = s-codcia AND gn-user.USER-ID = AlmCIncidencia.Usuario NO-LOCK NO-ERROR.                                      */
/*     IF AVAILABLE gn-user THEN Cabecera.UsrEmisor = gn-users.User-Name.                                                                               */
...
REPEAT WHILE AVAILABLE AlmCIncidencia:
    FOR EACH AlmDIncidencia OF AlmCIncidencia NO-LOCK, FIRST Almmmatg OF AlmDIncidencia NO-LOCK:
        ...
        {alm/i-consul-incid-cab.i &Tabla="Detalle"}
        ...
        CASE Detalle.Indicencia:
            WHEN "F" THEN Detalle.Indicencia = "FALTANTE".
            WHEN "S" THEN Detalle.Indicencia = "SOBRANTE".
            WHEN "M" THEN Detalle.Indicencia = "MAL ESTADO".
        END CASE.
        /* 07/04/2026 Michael Poma, datos adicionales */
        FIND LAST Almstkge USE-INDEX Llave01 WHERE AlmStkge.CodCia = s-codcia AND 
            AlmStkge.codmat = Detalle.codmat AND 
            AlmStkge.Fecha <= AlmCIncidencia.Fecha NO-LOCK NO-ERROR.
        IF AVAILABLE Almstkge THEN Detalle.CtoKardex = AlmDIncidencia.CanInc * AlmDIncidencia.Factor * AlmStkge.CtoUni.
        /* 16/07/2026: Sebastian Munive */
        ASSIGN
            Detalle.Familia = Almmmatg.codfam
            Detalle.SubFamilia = Almmmatg.subfam.
        FIND Almtfami OF ALmmmatg NO-LOCK NO-ERROR.
        IF AVAILABLE Almtfami THEN Detalle.Familia = Almtfami.codfam + ' ' + Almtfami.desfam.
        FIND Almsfami OF Almmmatg NO-LOCK NO-ERROR.
        IF AVAILABLE Almsfami THEN Detalle.SubFamilia = AlmSFami.subfam + ' ' + AlmSFami.dessub.
        ASSIGN
...
IF TRUE <> (pNomPer > '') THEN DO:
    FIND gn-user WHERE gn-user.codcia = s-codcia AND gn-user.USER-ID = pCodPeR NO-LOCK NO-ERROR.
    IF AVAILABLE gn-user THEN pNomPer = gn-user.user-name.
END.
