...
Almmmatg.DesMat VtaDTabla.Libre_d01 
...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-Cantidad 
...
DEFINE VARIABLE FILL-IN-Cantidad AS DECIMAL FORMAT "ZZZ,ZZ9.99":U INITIAL 0 
     LABEL "TOTAL" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1
     FONT 6 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaDTabla.LlaveDetalle COLUMN-LABEL "Codigo" FORMAT "x(8)":U
...
DEFINE FRAME F-Main
     ...
     FILL-IN-Cantidad AT ROW 7.73 COL 57 COLON-ALIGNED WIDGET-ID 2
...
"VtaDTabla.LlaveDetalle" "Codigo" "x(8)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almmmatg.DesMat
"Almmmatg.DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ? no no "52.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaDTabla.Libre_d01
"VtaDTabla.Libre_d01" "Cantidad" ">>,>>9.99" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
FOR EACH B-DTABLA OF VtaCTabla NO-LOCK:
    FILL-IN-Cantidad = FILL-IN-Cantidad + B-DTABLA.Libre_d01.
END.
...
IF NOT CAN-FIND(FIRST Almmmatg WHERE Almmmatg.CodCia = s-CodCia AND
                ...
                NO-LOCK)
    THEN DO:
    MESSAGE 'Código material NO registrado' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO VtaDTabla.LlaveDetalle.
    RETURN 'ADM-ERROR'.
END.
RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
IF RETURN-VALUE = 'YES' THEN DO:
    IF CAN-FIND(FIRST VtadTabla OF VtacTabla WHERE 
                VtaDTabla.LlaveDetalle = VtaDTabla.LlaveDetalle:SCREEN-VALUE IN BROWSE {&browse-name}
                NO-LOCK)
        THEN DO:
        MESSAGE 'Código material repetido' VIEW-AS ALERT-BOX ERROR.
        APPLY 'ENTRY':U TO VtaDTabla.LlaveDetalle.
        RETURN 'ADM-ERROR'.
    END.
END.
IF DECIMAL(VtaDTabla.Libre_d01:SCREEN-VALUE IN BROWSE {&browse-name}) = 0
    THEN DO:
    MESSAGE 'Cantidad en cero' VIEW-AS ALERT-BOX ERROR.
