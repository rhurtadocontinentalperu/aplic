...
&Scoped-Define ENABLED-OBJECTS FILL-IN-CodPer 
...
DEFINE VARIABLE h_b-reg-pickeadores AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     FILL-IN-CodPer AT ROW 1.38 COL 9 COLON-ALIGNED WIDGET-ID 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "REGISTRO Y SECTORIZACION DE LOS PICKEADORES"
         HEIGHT             = 25.85
         WIDTH              = 144.29
         MAX-HEIGHT         = 29.54
         MAX-WIDTH          = 144.29
...
ON END-ERROR OF W-Win /* REGISTRO Y SECTORIZACION DE LOS PICKEADORES */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* REGISTRO Y SECTORIZACION DE LOS PICKEADORES */
DO:
...
DO:
  ...
  IF NOT AVAILABLE pl-pers THEN DO:
      MESSAGE 'Código del pickeador NO registrado' VIEW-AS ALERT-BOX ERROR.
      SELF:SCREEN-VALUE = ''.
      RETURN NO-APPLY.
  END.
  RUN alm\d-reg-pickeadores (FILL-IN-CodPer, OUTPUT pSector).
  IF pSector = '' THEN RETURN NO-APPLY.
  RUN Graba-Registro.
  ...
  RUN dispatch IN h_b-reg-pickeadores ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/alm/b-reg-pickeadores.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-reg-pickeadores ).
       RUN set-position IN h_b-reg-pickeadores ( 2.73 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-reg-pickeadores ( 21.15 , 141.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv97 ).
       RUN set-position IN h_p-updv97 ( 24.08 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv97 ( 1.54 , 34.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-reg-pickeadores. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_b-reg-pickeadores ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-reg-pickeadores ,
             FILL-IN-CodPer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv97 ,
             h_b-reg-pickeadores , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      FIND PikSacadores WHERE PikSacadores.codcia = s-codcia
          AND PikSacadores.coddiv = s-coddiv
          AND PikSacadores.codper = FILL-IN-CodPer
          AND PikSacadores.flgest = "A"     /* Activo */
          EXCLUSIVE-LOCK NO-ERROR.
      IF AVAILABLE PikSacadores THEN DO:
          MESSAGE 'El Pickeador YA fue registrado' VIEW-AS ALERT-BOX ERROR.
          UNDO, RETURN 'ADM-ERROR'.
      END.
      CREATE PikSacadores.
      ASSIGN 
          PikSacadores.CodCia = s-codcia
          PikSacadores.CodDiv = s-coddiv
          PikSacadores.CodPer = FILL-IN-CodPer
          PikSacadores.Fecha  = DATETIME(TODAY, MTIME)
          PikSacadores.Sector = pSector
          PikSacadores.Usuario= s-user-id.
  END.
