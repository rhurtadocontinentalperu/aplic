...
DEFINE VARIABLE h_t-trfing-03 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-movmto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-trfing-03 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-Excel AT ROW 4.08 COL 98 WIDGET-ID 2
     RECT-1 AT ROW 3.88 COL 2 WIDGET-ID 4
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/v-trfing-03.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-trfing-03 ).
       RUN set-position IN h_v-trfing-03 ( 4.08 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-trfing-03. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-trfing-03 ).
       RUN add-link IN adm-broker-hdl ( h_q-alctrf , 'Record':U , h_v-trfing-03 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-trfing-03 ,
             h_p-updv05 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ingrso ).
       RUN set-position IN h_b-ingrso ( 8.69 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'alm/t-trfing-03.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-trfing-03 ).
       RUN set-position IN h_t-trfing-03 ( 8.50 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-trfing-03 ( 8.88 , 93.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-trfing-03 ,
             BUTTON-Excel:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
         ...
         RUN dispatch IN h_t-trfing-03 ('open-query':U).
      END.
