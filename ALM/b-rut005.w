...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Apertura-Hoja B-table-Win 
PROCEDURE Apertura-Hoja :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      /* Se blanquea el flag de cierre */
      ASSIGN
          DI-RutaC.FlgCie = "".
      /* Se reinicia el flag de estado */
      ASSIGN
          DI-RutaC.FlgEst = 'P'.    /* Valor por defecto: Para Despacho por Sectores */

      FIND gn-divi WHERE gn-divi.codcia = s-codcia AND gn-divi.coddiv = s-coddiv NO-LOCK.
/*       CASE gn-divi.Campo-Log[5]:                                         */
/*           WHEN YES THEN DI-RutaC.FlgEst = "PR".      /* Es un CD */      */
/*           OTHERWISE DI-RutaC.FlgEst = "P".           /* Es una Tienda */ */
/*       END CASE.                                                          */
      CASE gn-divi.Campo-Char[6]:
          WHEN "PR" THEN DI-RutaC.FlgEst = "PR".     /* Despacho por Rutas */
          OTHERWISE DI-RutaC.FlgEst = "P".          /* Despacho por Sectores */
      END CASE.
      /* Log de control */
      RUN lib/logtabla ( "DI-RUTAC",
                         Di-Rutac.CodDiv + '|' + Di-RutaC.CodDoc + '|' + Di-RutaC.NroDoc,
                         "REINICIADA" ).
/*       RUN lib/logtabla ( "DI-RUTAC",                                                      */
/*                          Di-Rutac.CodDiv + '|' + Di-RutaC.CodDoc + '|' + Di-RutaC.NroDoc, */
/*                          "APERTURA" ).                                                    */
      ...
      FOR EACH Di-RutaD OF Di-RutaC NO-LOCK WHERE Di-RutaD.FlgEst = 'C'
            AND LOOKUP(DI-RutaD.CodRef, 'FAC,BOL') > 0,
            FIRST Ccbcdocu WHERE Ccbcdocu.codcia = Di-RutaD.codcia
                ...
                NO-LOCK:
          /* TRACKING SOLO PARA COMPROBANTES FAC y BOL */
          FIND Almacen OF Ccbcdocu NO-LOCK.
/*           RUN gn/pTracking-01 (Di-RutaD.CodCia,                         */
/*                             Almacen.CodDiv,                             */
/*                             Ccbcdocu.CodPed,                            */
/*                             Ccbcdocu.NroPed,                            */
/*                             s-User-Id,                                  */
