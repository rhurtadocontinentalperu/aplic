...
DEFINE TEMP-TABLE Detalle NO-UNDO
...
------------------------------------------------------------------------------*/
    ...
    RUN aplic/gn/genera-od-otr-hpk-phr PERSISTENT SET hProc.
    ...
    DO k = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
        IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(k) THEN DO:
            ...
            END.
            /* ******************************************************************************************* */
            RUN ABAS_Aprobar_RA IN hProc (INPUT ROWID(Almcrepo),
                                          INPUT pCrossDocking,
                                          INPUT pAlmacenXD,
                                          INPUT-OUTPUT TABLE Detalle,
                                          OUTPUT pMensaje2,
                                          OUTPUT pMensaje).
            IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                pMensaje = pMensaje + CHR(10) + "Pasamos al siguiente registro".
...
FOR EACH B-ALM NO-LOCK WHERE B-ALM.codcia = s-codcia AND LOOKUP(B-ALM.codalm, EDITOR-AlmPed) > 0:
