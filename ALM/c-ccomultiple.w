...
&SCOPED-DEFINE l-lookup h_l-ccomultiple
...
DEFINE VARIABLE h_l-ccomultiple AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 3.31 COL 82
     Btn_Cancel AT ROW 4.85 COL 82
     SPACE(2.56) SKIP(13.26)
...
DO:
    RUN captura-datos IN {&l-lookup} NO-ERROR.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'l-ccomultiple.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-ccomultiple ).
       RUN set-position IN h_l-ccomultiple ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-ccomultiple ( 18.08 , 78.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.19 , 81.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 14.72 ) NO-ERROR.

       /* Links to SmartLookup h_l-ccomultiple. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-ccomultiple ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-ccomultiple ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-ccomultiple , 'AFTER':U ).
    END. /* Page 0 */
