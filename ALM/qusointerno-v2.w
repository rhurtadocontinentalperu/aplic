...
DEFINE SHARED VARIABLE S-CCO     AS CHAR.
DEFINE SHARED VARIABLE s-user-id AS CHAR.

&SCOPED-DEFINE CONDICION ( AlmCDocu.CodCia = S-CODCIA AND ~
                           AlmCDocu.CodLlave = s-user-id AND ~
                           AlmCDocu.CodDoc = S-CODDOC )


/*AlmCDocu.CodLlave = S-CCO AND ~*/
...
&Scoped-define INTERNAL-TABLES AlmCDocu

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for QUERY Query-Main                                     */
&Scoped-define QUERY-STRING-Query-Main FOR EACH AlmCDocu WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    BY AlmCDocu.FchDoc INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH AlmCDocu WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    BY AlmCDocu.FchDoc INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main AlmCDocu
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main AlmCDocu
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.AlmCDocu"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE"
     _OrdList          = "INTEGRAL.AlmCDocu.FchDoc|yes"
     _Where[1]         = "{&Condicion}"
     _Design-Parent    is WINDOW q-tables @ ( 1 , 1.29 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN lkup/c-almcdocu-v2 ('Solicitud para Uso Interno').
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
/* FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE {&CONDICION} AND                                                    */
/*      {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}}.NroDoc = STRING(S-NROSER,"999") + STRING(F-Numero,"999999")               */
/*      NO-LOCK NO-ERROR.                                                                                               */
