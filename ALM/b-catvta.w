...
&Scoped-define INTERNAL-TABLES AlmCatVtaD Almmmatg
...
&Scoped-define QUERY-STRING-br_table FOR EACH AlmCatVtaD WHERE AlmCatVtaD.CodCia = AlmCatVtaC.CodCia ~
  ...
  AND AlmCatVtaD.NroPag = AlmCatVtaC.NroPag NO-LOCK, ~
      EACH Almmmatg OF AlmCatVtaD NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH AlmCatVtaD WHERE AlmCatVtaD.CodCia = AlmCatVtaC.CodCia ~
  ...
  AND AlmCatVtaD.NroPag = AlmCatVtaC.NroPag NO-LOCK, ~
      EACH Almmmatg OF AlmCatVtaD NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table AlmCatVtaD Almmmatg
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      AlmCatVtaD.NroSec COLUMN-LABEL "Sec" FORMAT "9999":U WIDTH 5
      AlmCatVtaD.codmat FORMAT "X(6)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 72 BY 10.23
         FONT 4
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.AlmCatVtaD WHERE INTEGRAL.AlmCatVtaC <external> ...,INTEGRAL.Almmmatg OF INTEGRAL.AlmCatVtaD"
...
"AlmCatVtaD.NroSec" "Sec" ? "integer" ? ? ? ? ? ? no ? no no "5" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.AlmCatVtaD.codmat
"AlmCatVtaD.codmat" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Lista de Articulos */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Lista de Articulos */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Lista de Articulos */
DO:
...
------------------------------------------------------------------------------*/
  ...
  FIND LAST AlmCatVtaD WHERE AlmCatVtaD.CodCia = AlmCatVtaC.CodCia
      AND AlmCatVtaD.CodDiv = AlmCatVtaC.CodDiv
      AND AlmCatVtaD.NroPag = AlmCatVtaC.NroPag NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME {&FRAME-NAME}:
        FIND FIRST b-almcatvtad WHERE b-almcatvtad.codcia = almcatvtac.codcia
            ...
            AND b-almcatvtad.codmat = almcatvtad.codmat:SCREEN-VALUE IN BROWSE {&browse-name} NO-LOCK NO-ERROR.
        IF AVAIL b-almcatvtad THEN DO:
            MESSAGE "Articulo ya registrado en la página " + STRING(b-almcatvtad.nropag)
