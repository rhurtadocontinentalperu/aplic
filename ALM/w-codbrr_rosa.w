...
&Scoped-Define ENABLED-OBJECTS x-stand x-CodBrr BUTTON-3 BROWSE-1 RECT-1 
&Scoped-Define DISPLAYED-OBJECTS x-stand x-CodBrr x-DesMat x-CodMat ~
x-UndBas x-Cantidad 
...
DEFINE VARIABLE x-stand AS CHARACTER FORMAT "X(3)":U 
     LABEL "Stand" 
     VIEW-AS FILL-IN 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 W-Win _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      ...
      DETALLE.DesMat FORMAT "X(45)":U
      DETALLE.UndBas FORMAT "X(4)":U
      DETALLE.TpoArt FORMAT "X(1)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 53 BY 4.5
...
DEFINE FRAME F-Main
     x-stand AT ROW 3.15 COL 51 COLON-ALIGNED WIDGET-ID 6
     x-CodBrr AT ROW 1.38 COL 15 COLON-ALIGNED
     BUTTON-3 AT ROW 10.42 COL 41
     x-DesMat AT ROW 2.19 COL 15 COLON-ALIGNED
     x-CodMat AT ROW 3 COL 15 COLON-ALIGNED
     x-UndBas AT ROW 3.77 COL 15 COLON-ALIGNED
     x-Cantidad AT ROW 4.65 COL 15 COLON-ALIGNED
     BROWSE-1 AT ROW 5.62 COL 5
...
"DETALLE.codmat" "Código!Artículo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[3]   = Temp-Tables.DETALLE.UndBas
     _FldNameList[4]   = Temp-Tables.DETALLE.TpoArt
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
DO:
  IF x-stand:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '' THEN DO:
      MESSAGE 'ingrese zona'
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      APPLY 'entry' TO x-stand.
      RETURN 'adm-error'.
  END.
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST w-report WHERE w-report.task-no = 123450
        AND w-report.llave-c = x-codbrr:SCREEN-VALUE IN FRAME {&FRAME-NAME}
        AND w-report.campo-c[1] = x-stand:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
    IF NOT AVAIL w-report THEN DO:
        CREATE w-report.
        ASSIGN
            w-report.task-no = 123450
            w-report.llave-c = x-codbrr:SCREEN-VALUE IN FRAME {&FRAME-NAME}
            w-report.campo-c[1] = x-stand:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY x-stand x-CodBrr x-DesMat x-CodMat x-UndBas x-Cantidad 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE x-stand x-CodBrr BUTTON-3 BROWSE-1 RECT-1 
      WITH FRAME F-Main IN WINDOW W-Win.
