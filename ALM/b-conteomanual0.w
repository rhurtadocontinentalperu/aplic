...
DEFINE TEMP-TABLE tmp-AlmDInv NO-UNDO LIKE INTEGRAL.AlmDInv.
...
&Scoped-define INTERNAL-TABLES tmp-AlmDInv INTEGRAL.Almmmatg
...
tmp-AlmDInv.codmat INTEGRAL.Almmmatg.DesMat INTEGRAL.Almmmatg.DesMar ~
INTEGRAL.Almmmatg.UndBas tmp-AlmDInv.QtyConteo tmp-AlmDInv.CodUserCon 
...
&Scoped-define QUERY-STRING-br_table FOR EACH tmp-AlmDInv WHERE ~{&KEY-PHRASE} ~
 ...
 AND tmp-AlmDInv.CodAlm = s-codalm NO-LOCK, ~
      EACH INTEGRAL.Almmmatg OF tmp-AlmDInv NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH tmp-AlmDInv WHERE ~{&KEY-PHRASE} ~
 ...
 AND tmp-AlmDInv.CodAlm = s-codalm NO-LOCK, ~
      EACH INTEGRAL.Almmmatg OF tmp-AlmDInv NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table tmp-AlmDInv INTEGRAL.Almmmatg
...
&Scoped-Define ENABLED-OBJECTS br_table btn-consulta btn-save btn-exit 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      INTEGRAL.Almmmatg.DesMat FORMAT "X(45)":U WIDTH 40
...
/* SETTINGS FOR FILL-IN txt-codmat IN FRAME F-Main
   NO-ENABLE                                                            */
...
/* Query rebuild information for BROWSE br_table
 ...
 AND Temp-Tables.tmp-AlmDInv.CodAlm = s-codalm"
...
------------------------------------------------------------------------------*/
   ...
   /* RUN Borra-Datos.*/
    ...
    FIND FIRST almcinv WHERE almcinv.codcia = s-codcia
        ...
        AND AlmCInv.NomCia = "CONTI" NO-LOCK NO-ERROR.
    IF AVAIL almcinv THEN dFchInv = DATE(almcinv.fecupdate).
    ELSE DO:
        MESSAGE "No se ha cargado el inventario" SKIP
                "    del almacen " + s-codalm 
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        RETURN "ADM-ERROR".
    END.

    FOR EACH almdinv WHERE almdinv.codcia = s-codcia AND 
        AlmDInv.CodAlm  = s-codalm AND AlmDInv.NroPagina = 9000 AND
        AlmDInv.NomCia  = "CONTI"  NO-LOCK :
        iNroSec = iNroSec + 1.
    END.
...
------------------------------------------------------------------------------*/
  ...
  FIND LAST tmp-AlmDInv WHERE tmp-AlmDInv.CodCia = s-codcia
      AND tmp-AlmDInv.NomCia = "CONTI"
      AND tmp-AlmDInv.CodAlm = s-codalm NO-LOCK NO-ERROR.
