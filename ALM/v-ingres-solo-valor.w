...
Almcmov.AlmDes Almcmov.NroRf1 Almcmov.Observ Almcmov.NroRf2 Almcmov.NroRf3 
...
"US$", 2
     ...
     Almcmov.NroRf2 AT ROW 3.96 COL 72 COLON-ALIGNED
          ...
          VIEW-AS FILL-IN 
          SIZE 10 BY .69
     Almcmov.NroRf3 AT ROW 4.77 COL 71.86 COLON-ALIGNED
...
DO:
  ...
  THEN DO:
    FIND CMOV WHERE CMOV.codcia = s-codcia
        ...
        AND CMOV.tipmov = 'S'
        AND CMOV.codmov = 10
        AND CMOV.nroser = INTEGER(SUBSTRING(SELF:SCREEN-VALUE,1,3))
        AND CMOV.nrodoc = INTEGER(SUBSTRING(SELF:SCREEN-VALUE,4,6))
        NO-LOCK NO-ERROR.
    IF AVAILABLE CMOV
    THEN DO:
        Almcmov.CodCli:SCREEN-VALUE = CMOV.codcli.
        DISPLAY CMOV.codcli @ almcmov.codcli WITH FRAME {&FRAME-NAME}.
        APPLY 'LEAVE':U TO Almcmov.codcli.
        /* Cargamos el detalle */
        FOR EACH ITEM:
            DELETE ITEM.
        END.
        FOR EACH DMOV OF CMOV WHERE DMOV.CanDes > DMOV.CanDev NO-LOCK:
            CREATE ITEM.
            BUFFER-COPY DMOV TO ITEM
                ASSIGN ITEM.CanDes = DMOV.CanDes - DMOV.CanDev.
        END.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE Almcmov THEN RUN ALM\R-IMPFMT-1.R(ROWID(almcmov), YES).
...
DO WITH FRAME {&FRAME-NAME} :
   ...
   /* INGRESO MUESTRAS RETORNABLES */
   IF Almtdocm.tipmov = 'I' AND Almtdocm.codmov = 10
   THEN DO:
        FIND CMOV WHERE CMOV.codcia = s-codcia
            AND CMOV.codalm = s-codalm
            AND CMOV.tipmov = 'S'
            ...
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE CMOV OR CMOV.flgest = 'A'
        THEN DO:
            MESSAGE 'No se encontro la guia de salida de Muestras Retornables'
                VIEW-AS ALERT-BOX ERROR.
   ...
   FOR EACH ITEM NO-LOCK:
       N-ITEM = N-ITEM + (ITEM.CanDes + ITEM.ImpCto).
   END.
   IF N-ITEM = 0 THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  IF almcmov.fchdoc < dFchCie THEN DO:
      MESSAGE 'NO se puede anular/modificar ningún documento antes del' (dFchCie)
          VIEW-AS ALERT-BOX WARNING.
      RETURN 'ADM-ERROR'.
  END.
  /* fin de consistencia */
