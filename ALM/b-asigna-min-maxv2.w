...
&Scoped-define INTERNAL-TABLES Almmmate Almacen TabGener almtabla
...
(IF TabGener.Libre_l01 = YES THEN '*' ELSE '') @ TabGener.Libre_c05 ~
TabGener.Codigo Almmmate.CodAlm Almacen.Descripcion Almmmate.VInMn1 ~
Almmmate.VInMn2 Almmmate.StkMin Almmmate.VCtMn1 Almmmate.VCtMn2 ~
Almmmate.StkMax fStockDisponible() @ x-StockDisponible ~
x-StockTransito @ x-StockTransito fSaldo() @ x-Saldo 
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almmmate OF Almmmatg WHERE ~{&KEY-PHRASE} NO-LOCK, ~
  ...
  AND almtabla.Codigo = TabGener.Codigo NO-LOCK ~
    BY TabGener.Codigo ~
       BY Almmmate.CodAlm.
&Scoped-define TABLES-IN-QUERY-br_table Almmmate Almacen TabGener almtabla
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Almmmate
&Scoped-define SECOND-TABLE-IN-QUERY-br_table Almacen
&Scoped-define THIRD-TABLE-IN-QUERY-br_table TabGener
&Scoped-define FOURTH-TABLE-IN-QUERY-br_table almtabla
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      (IF TabGener.Libre_l01 = YES THEN '*' ELSE '') @ TabGener.Libre_c05 COLUMN-LABEL "T" FORMAT "X":U
      TabGener.Codigo COLUMN-LABEL "Zona!Geografica" FORMAT "x(8)":U
            WIDTH 9.57
      Almmmate.CodAlm FORMAT "x(3)":U
      ...
      Almmmate.StkMin COLUMN-LABEL "Máximo + !Seguridad" FORMAT "ZZZ,ZZ9.99":U
            WIDTH 8.86 COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 14
      ...
      Almmmate.StkMax COLUMN-LABEL "Empaque" FORMAT "Z,ZZZ,ZZZ,ZZ9.99":U
            WIDTH 8.57
      fStockDisponible() @ x-StockDisponible COLUMN-LABEL "Stock Disponible" FORMAT "-ZZZ,ZZZ,ZZ9.99":U
            WIDTH 14.43
      x-StockTransito @ x-StockTransito COLUMN-LABEL "Stock en!Tránsito Transferencias" FORMAT "ZZZ,ZZ9.99":U
            WIDTH 17.72
      fSaldo() @ x-Saldo COLUMN-LABEL "Faltante/Sobrante" FORMAT "-ZZZ,ZZ9.99":U
  ENABLE
      Almmmate.VCtMn1
      Almmmate.VCtMn2
      Almmmate.StkMax
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 127 BY 12.69
         FONT 4.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Almmmate OF INTEGRAL.Almmmatg,INTEGRAL.Almacen OF INTEGRAL.Almmmate,INTEGRAL.TabGener WHERE INTEGRAL.Almmmate ...,INTEGRAL.almtabla WHERE INTEGRAL.TabGener ..."
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ", FIRST,, FIRST"
     _OrdList          = "INTEGRAL.TabGener.Codigo|yes,INTEGRAL.Almmmate.CodAlm|yes"
     _JoinCode[3]      = "TabGener.CodCia = Almmmate.CodCia
  AND TabGener.Libre_c01 = Almmmate.CodAlm"
     _Where[3]         = "TabGener.Clave = ""ZG"""
     _JoinCode[4]      = "INTEGRAL.almtabla.Tabla = TabGener.Clave
  AND INTEGRAL.almtabla.Codigo = TabGener.Codigo"
     _FldNameList[1]   > "_<CALC>"
"(IF TabGener.Libre_l01 = YES THEN '*' ELSE '') @ TabGener.Libre_c05" "T" "X" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.TabGener.Codigo
"TabGener.Codigo" "Zona!Geografica" ? "character" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Almmmate.VInMn1" "Máximo" "ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.Almmmate.VInMn2
"Almmmate.VInMn2" "Seguridad" "ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.Almmmate.StkMin
"Almmmate.StkMin" "Máximo + !Seguridad" "ZZZ,ZZ9.99" "decimal" 14 0 ? ? ? ? no ? no no "8.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > INTEGRAL.Almmmate.VCtMn1
"Almmmate.VCtMn1" "Campaña" "ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > INTEGRAL.Almmmate.VCtMn2
"Almmmate.VCtMn2" "No Campaña" "ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > INTEGRAL.Almmmate.StkMax
"Almmmate.StkMax" "Empaque" ? "decimal" ? ? ? ? ? ? yes ? no no "8.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > "_<CALC>"
"fStockDisponible() @ x-StockDisponible" "Stock Disponible" "-ZZZ,ZZZ,ZZ9.99" ? ? ? ? ? ? ? no ? no no "14.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > "_<CALC>"
"x-StockTransito @ x-StockTransito" "Stock en!Tránsito Transferencias" "ZZZ,ZZ9.99" ? ? ? ? ? ? ? no ? no no "17.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > "_<CALC>"
"fSaldo() @ x-Saldo" "Faltante/Sobrante" "-ZZZ,ZZ9.99" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON FIND OF Almmmate DO:
    ...
    */
    ...
    x-Saldo = x-StockDisponible + x-StockTransito + x-StockCompras - Almmmate.StkMin.
END.
...
FOR FIRST B-MATE OF Almmmatg NO-LOCK,
