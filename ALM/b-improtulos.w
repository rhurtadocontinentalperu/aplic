...
&Scoped-define FIELDS-IN-QUERY-r_table FacDPedi.codmat Almmmatg.DesMat ~
Almmmatg.DesMar FacDPedi.CanPed 
&Scoped-define ENABLED-FIELDS-IN-QUERY-r_table 
&Scoped-define QUERY-STRING-r_table FOR EACH FacDPedi WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacDPedi.NroPed = s-nroref NO-LOCK, ~
    ...
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-r_table OPEN QUERY r_table FOR EACH FacDPedi WHERE ~{&KEY-PHRASE} ~
      AND FacDPedi.CodCia = s-codcia ~
 AND FacDPedi.CodDiv = s-coddiv ~
 AND FacDPedi.CodDoc = s-coddoc ~
 AND FacDPedi.NroPed = s-nroref NO-LOCK, ~
      EACH Almmmatg OF FacDPedi NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-r_table FacDPedi Almmmatg
&Scoped-define FIRST-TABLE-IN-QUERY-r_table FacDPedi
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS r_table B-table-Win _STRUCTURED
  QUERY r_table NO-LOCK DISPLAY
      FacDPedi.codmat COLUMN-LABEL " Articulo" FORMAT "X(6)":U
            WIDTH 9
      Almmmatg.DesMat FORMAT "X(45)":U WIDTH 48
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(30)":U
      FacDPedi.CanPed FORMAT ">,>>>,>>9.9999":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 96 BY 15.88
         FONT 4.
...
/* Query rebuild information for BROWSE r_table
 ...
 AND FacDPedi.CodDiv = s-coddiv
 AND FacDPedi.CodDoc = s-coddoc
 AND FacDPedi.NroPed = s-nroref"
     _FldNameList[1]   > INTEGRAL.FacDPedi.codmat
"FacDPedi.codmat" " Articulo" ? "character" ? ? ? ? ? ? no ? no no "9" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almmmatg.DesMat
"Almmmatg.DesMat" ? ? "character" ? ? ? ? ? ? no ? no no "48" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
