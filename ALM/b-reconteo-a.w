...
tt-AlmDInv.CodUbi tt-AlmDInv.codmat INTEGRAL.Almmmatg.DesMat ~
INTEGRAL.Almmmatg.DesMar INTEGRAL.Almmmatg.UndBas tt-AlmDInv.QtyConteo ~
...
&Scoped-Define ENABLED-OBJECTS txt-zona btn-consulta txt-user-reconteo ~
txt-page btn-ingresa btn-grabar btn-cancelar btn-exit-2 RECT-67 RECT-68 ~
RECT-69 RECT-70 br_table 
&Scoped-Define DISPLAYED-OBJECTS txt-codalm txt-desalm txt-totpag txt-zona ~
txt-user-reconteo txt-page txt-user-supervisor txt-user-conteo 
...
DEFINE VARIABLE txt-zona AS CHARACTER FORMAT "X(10)":U 
     LABEL "Zona" 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      INTEGRAL.Almmmatg.DesMat FORMAT "X(40)":U WIDTH 35
      INTEGRAL.Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(20)":U
            WIDTH 18
      INTEGRAL.Almmmatg.UndBas COLUMN-LABEL "Unidad" FORMAT "X(4)":U
            WIDTH 3.57
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 105.14 BY 18.62
         FONT 4
...
DEFINE FRAME F-Main
     ...
     txt-totpag AT ROW 1.23 COL 72.43 COLON-ALIGNED NO-LABEL WIDGET-ID 62
     txt-zona AT ROW 1.31 COL 89 COLON-ALIGNED WIDGET-ID 78
     btn-consulta AT ROW 1.35 COL 99.72 WIDGET-ID 8
     txt-user-reconteo AT ROW 2.23 COL 12.86 COLON-ALIGNED WIDGET-ID 24
     txt-page AT ROW 2.35 COL 89 COLON-ALIGNED WIDGET-ID 36
     ...
     btn-ingresa AT ROW 24.65 COL 3 WIDGET-ID 50
     btn-grabar AT ROW 24.69 COL 18 WIDGET-ID 54
     btn-cancelar AT ROW 24.69 COL 34 WIDGET-ID 48
     btn-exit-2 AT ROW 24.69 COL 50 WIDGET-ID 52
     br_table AT ROW 5.46 COL 3.14
     "   Reconteo = ? :  Indica que no tuvo reconteo" VIEW-AS TEXT
          SIZE 43 BY 1.54 AT ROW 24.73 COL 64 WIDGET-ID 76
          FGCOLOR 12 FONT 16
     "Supervisado por:" VIEW-AS TEXT
          SIZE 12 BY .62 AT ROW 4.12 COL 5.57 WIDGET-ID 72
          BGCOLOR 8 
     "Total Pág:" VIEW-AS TEXT
          SIZE 8.72 BY .5 AT ROW 1.46 COL 65.14 WIDGET-ID 60
          FONT 6
     ...
     RECT-69 AT ROW 24.38 COL 2 WIDGET-ID 58
...
/* BROWSE-TAB br_table RECT-70 F-Main */
ASSIGN 
...
/*
    ...
    AND (txt-Zona = "" OR AlmCInv.CodAlma    = txt-zona:SCREEN-VALUE IN FRAME {&FRAME-NAME} )
*/
/*    
...
*/        
    FOR EACH AlmCInv WHERE AlmCInv.CodCia = s-CodCia
        ...
        RUN lib/logtabla ('almcinv',
                          almcinv.codalm + '|' + STRING(almcinv.nropag) + '|' + 'RECONTEO',
                          'CREATE').
    END. 
        ...
        FOR EACH TT-ALMDINV:
            ...
            IF AVAIL AlmDInv THEN DO:
                ...
                ASSIGN 
                    ...
                    AlmDInv.CodUserRec   = txt-user-reconteo
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST AlmCInv WHERE AlmCInv.CodCia = s-CodCia
        ...
        AND AlmCInv.Sw3Conteo  = NO  NO-LOCK NO-ERROR.
    IF NOT AVAILABLE AlmCInv THEN DO: 
        MESSAGE 'Este Almacén y Número de Página' SKIP
                '  aún no han registrado Conteo ó Tiene 3er Conteo.'
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
    ...
    FOR EACH AlmDInv WHERE AlmDInv.CodCia = s-CodCia
        ...
        IF almDInv.libre_c04 = ? OR almDInv.libre_c04 = "" THEN NEXT.

        CREATE tt-AlmDInv.
        BUFFER-COPY AlmDInv TO tt-AlmDInv.
        IF tt-AlmDInv.QtyReconteo = 0  THEN DO:
