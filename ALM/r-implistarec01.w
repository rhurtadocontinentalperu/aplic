...
&Scoped-Define ENABLED-OBJECTS x-zona x-zona-2 x-sup x-sup-2 r-pages ~
x-nropages r-imp btn-Excel Btn_Ok Btn_Done RECT-71 RECT-75 RECT-72 RECT-73 
&Scoped-Define DISPLAYED-OBJECTS x-codalm txt-desalm x-zona x-zona-2 x-sup ~
x-sup-2 r-pages x-nropages r-imp x-mensaje 
...
DEFINE FRAME F-Main
     ...
     txt-desalm AT ROW 1.96 COL 18.43 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     ...
     r-pages AT ROW 7.35 COL 4 NO-LABEL WIDGET-ID 52
     x-nropages AT ROW 8.5 COL 11 COLON-ALIGNED NO-LABEL WIDGET-ID 62
     r-imp AT ROW 10.88 COL 6 NO-LABEL WIDGET-ID 16
     x-mensaje AT ROW 12.04 COL 3.29 COLON-ALIGNED NO-LABEL WIDGET-ID 36
     btn-Excel AT ROW 13.08 COL 35.43 WIDGET-ID 68
     Btn_Ok AT ROW 13.08 COL 47.43 WIDGET-ID 64
     Btn_Done AT ROW 13.08 COL 59.57
     "Ingrese número de páginas o rango de páginas separadas" VIEW-AS TEXT
          SIZE 41 BY .5 AT ROW 9.31 COL 13 WIDGET-ID 60
     "por coma. Por ejemplo:1,2,5-10." VIEW-AS TEXT
          SIZE 41 BY .5 AT ROW 9.92 COL 13 WIDGET-ID 48
     "Rango Páginas" VIEW-AS TEXT
          SIZE 11.72 BY .5 AT ROW 6.65 COL 3.57 WIDGET-ID 58
          FGCOLOR 1 
     ...
     RECT-71 AT ROW 12.88 COL 2.86 WIDGET-ID 24
     RECT-75 AT ROW 10.69 COL 2 WIDGET-ID 46
     RECT-72 AT ROW 6.88 COL 2 WIDGET-ID 50
...
------------------------------------------------------------------------------*/
    ...
    IF x-zona   = '' THEN cZonaD = '0A-00'. ELSE cZonaD = x-zona.
    IF x-zona-2 = '' THEN cZonaH = '9Z-99'. ELSE cZonaH = x-zona-2.
    
    IF x-sup    = '' AND x-zona   = '' THEN cZonaD = '0A-00'.
    IF x-sup-2  = '' AND x-zona-2 = '' THEN cZonaH = '9Z-99'.

    /**
    IF x-sup    = '' THEN cSupeD = ''. ELSE cSupeD = x-sup.
    IF x-sup-2  = '' THEN cSupeH = 'ZZZZZZZZZZZZ'. ELSE cSupeH = x-sup-2.
    ***/
    /*Zonas por Supervisor*/
    IF x-sup <> '' AND x-zona = '' THEN DO:
        FIND FIRST TabSupAlm WHERE TabSupAlm.CodCia = s-codcia
            AND TabSupAlm.CodAlm = "11"
...
------------------------------------------------------------------------------*/
  DISPLAY x-codalm txt-desalm x-zona x-zona-2 x-sup x-sup-2 r-pages x-nropages 
          r-imp x-mensaje 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE x-zona x-zona-2 x-sup x-sup-2 r-pages x-nropages r-imp btn-Excel 
         Btn_Ok Btn_Done RECT-71 RECT-75 RECT-72 RECT-73 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  IF r-pages = 1 THEN DO:
      /* RB-REPORT-NAME = 'Listado de Reconteos' */
