...
&SCOPED-DEFINE CONDICION ( ~
    ...
    ( FacCPedi.CodDiv = s-CodDiv OR FacCPedi.CodAlm = s-CodAlm ) AND ~
...
&Scoped-define INTERNAL-TABLES FacCPedi Almacen

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table FacCPedi.CodAlm FacCPedi.CodDoc ~
FacCPedi.NroPed FacCPedi.CodRef FacCPedi.NroRef FacCPedi.NomCli ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
    ...
    BY FacCPedi.FchPed DESCENDING ~
       BY FacCPedi.NroPed DESCENDING.
&Scoped-define TABLES-IN-QUERY-br_table FacCPedi Almacen
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.NroPed COLUMN-LABEL "Orden!Despacho" FORMAT "X(9)":U
            WIDTH 9.57 COLUMN-FONT 0
      FacCPedi.CodRef COLUMN-LABEL "Refer." FORMAT "x(3)":U
      FacCPedi.NroRef COLUMN-LABEL "N° Referencia" FORMAT "X(9)":U
      FacCPedi.NomCli COLUMN-LABEL "Nombre del Cliente" FORMAT "x(50)":U
            WIDTH 44
      FacCPedi.FchPed COLUMN-LABEL "Fecha!Emisión" FORMAT "99/99/99":U
      fGetCurr(FacCPedi.CodMon) @ cCurrency COLUMN-LABEL "Mon" FORMAT "x(3)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 95 BY 7.81
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     " Orden de Despacho" VIEW-AS TEXT
          SIZE 15 BY .62 AT ROW 1 COL 5 WIDGET-ID 4
          FGCOLOR 9 
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.FacCPedi,INTEGRAL.Almacen OF INTEGRAL.FacCPedi"
     _Options          = "NO-LOCK KEY-PHRASE"
     _OrdList          = "INTEGRAL.FacCPedi.FchPed|no,INTEGRAL.FacCPedi.NroPed|no"
     _Where[1]         = "{&CONDICION}"
     _Where[2]         = "Almacen.CodDiv = s-coddiv"
     _FldNameList[1]   = INTEGRAL.FacCPedi.CodAlm
...
"FacCPedi.NroPed" "Orden!Despacho" ? "character" ? ? 0 ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacCPedi.CodRef
"FacCPedi.CodRef" "Refer." ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.FacCPedi.NroRef
"FacCPedi.NroRef" "N° Referencia" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.FchPed" "Fecha!Emisión" "99/99/99" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > "_<CALC>"
"fGetCurr(FacCPedi.CodMon) @ cCurrency" "Mon" "x(3)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
    ...
    IF NOT AVAILABLE FacCPedi OR
        ...
        RETURN.
    END.
    RUN vta\gGenGuia2a (ROWID(FacCPedi), 'A', 'G/R').       /* GUIA AUTOMATICA */
...
------------------------------------------------------------------------------*/
    IF NOT AVAILABLE FacCPedi OR
        ...
        RETURN.
    END.
    RUN vta/w-guias-02-1 (ROWID(FacCPedi)).
    IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN.
    RUN vta\gGenGuia2a (ROWID(FacCPedi), 'M', 'G/R').       /* GUIA MANUAL */
    RUN dispatch IN THIS-PROCEDURE ('open-query').
