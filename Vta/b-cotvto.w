...
FacCPedi.ImpTot 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table FacCPedi.fchven 
...
&Scoped-define QUERY-STRING-br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
 ...
 AND LOOKUP(FacCPedi.FlgEst, "A,C,S,X,R") = 0 NO-LOCK ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.fchven COLUMN-LABEL "    Fecha    !    Vcmto." FORMAT "99/99/99":U
            COLUMN-BGCOLOR 11
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 83 BY 8.73
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     wclient AT ROW 1.23 COL 37.14 COLON-ALIGNED
     wcotiza AT ROW 1.27 COL 37.29 COLON-ALIGNED
...
/* Query rebuild information for BROWSE br_table
 ...
 AND LOOKUP(FacCPedi.FlgEst, ""A,C,S,X,R"") = 0"
...
"FacCPedi.fchven" "    Fecha    !    Vcmto." ? "date" 11 ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  /*IF Faccpedi.Fchven:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = "" THEN DO:
 *    MESSAGE "Fecha No Valida" VIEW-AS ALERT-BOX ERROR.
 *    RETURN "ADM-ERROR".   
 *   END.*/
/*  Faccpedi.Fchven:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} =*/
    /*RUN vta\d-pedapr.r(Faccpedi.nroped,"PED").*/
...
DO:
  ...
  find first FacCpedi where FacCpedi.NroPed = substring(wcotiza:screen-value,1,3) +
             ...
             integral.FacCPedi.NomCli BEGINS wclient NO-LOCK NO-ERROR.
...
/*   IF Faccpedi.flgest = 'X' AND FacCPedi.fchven >= TODAY */
  IF Faccpedi.flgest = 'V' AND FacCPedi.fchven >= TODAY
      THEN ASSIGN
            Faccpedi.flgest = 'P'
            Faccpedi.libre_f01 = TODAY
            FacCPedi.UsrApr = s-user-id.
  ASSIGN
      COMBO-BOX-3:SENSITIVE IN FRAME {&FRAME-NAME} = YES
      wclient:SENSITIVE IN FRAME {&FRAME-NAME} = YES
      wcotiza:SENSITIVE IN FRAME {&FRAME-NAME} = YES.
...
------------------------------------------------------------------------------*/
ASSIGN
    COMBO-BOX-3:SENSITIVE IN FRAME {&FRAME-NAME} = NO
    wclient:SENSITIVE IN FRAME {&FRAME-NAME} = NO
    wcotiza:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
RETURN "OK".
