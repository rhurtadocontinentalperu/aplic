...
"VtaTabla.Rango_fecha[2]" "Término" ? "date" ? ? ? ? ? ? no ? no no "13" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
DO:
    ...
    IF {&BROWSE-NAME}:NUM-SELECTED-ROWS = 0 THEN DO:
        MESSAGE 'Debe seleccionar al menos un registro de las CAMPAÑAS'
...
FOR EACH estavtas.evtarti NO-LOCK WHERE estavtas.evtarti.codcia = s-codcia
    ...
    T-StkAct  = 0.
    F-Salida  = 0.
    x-Llave = VtaTabla.Llave_c1 + '|'.       /* CAMPAÑA */
