...
&Scoped-define QUERY-STRING-br_table FOR EACH CcbDDocu OF CcbCDocu WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Almmmatg OF CcbDDocu OUTER-JOIN NO-LOCK, ~
      EACH Almmmate WHERE Almmmate.CodCia = CcbDDocu.CodCia ~
  AND Almmmate.codmat = CcbDDocu.codmat ~
      AND Almmmate.CodAlm = CcbCDocu.CodAlm OUTER-JOIN NO-LOCK ~
    BY Almmmate.CodUbi ~
       BY CcbDDocu.codmat
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbDDocu.Por_Dsctos[1] FORMAT "->>9.99":U
      CcbDDocu.ImpLin FORMAT "->>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 89 BY 7.65
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     F-TotBrt AT ROW 9.38 COL 2 NO-LABEL
...
"Almmmatg.DesMat" ? "X(38)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
