...
DEFINE TEMP-TABLE ttod_det NO-UNDO
    FIELD  ttod_codalm LIKE almmmate.codalm COLUMN-LABEL "Alm"
...
"FacCPedi.FchPed" "Fecha!Pedido" ? "date" ? ? ? ? ? ? no ? no no "4.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-od */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH ttod_det NO-LOCK
        BREAK BY ttod_codalm BY ttod_codubi BY ttod_codmat:
        ACCUMULATE ttod_canped (SUB-TOTAL BY ttod_codmat).
        IF LAST-OF(ttod_codmat) THEN DO:
            iCount = iCount + 1.
            cColumn = STRING(iCount).
            cRange = "A" + cColumn.
            chWorkSheet:Range(cRange):VALUE = ttod_codalm.
            cRange = "B" + cColumn.
