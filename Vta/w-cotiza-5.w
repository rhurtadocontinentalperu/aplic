...
DEFINE NEW SHARED TEMP-TABLE T-DPEDI2   NO-UNDO LIKE VtaDDocu.

DEFINE NEW SHARED VARIABLE s-coddoc   AS CHAR INITIAL "PNX".
...
DEFINE VARIABLE h_b-cotiza-5 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-tmpcot-5 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv03 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-pedido-5 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotiza-5 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-8 AT ROW 20.65 COL 39
     BUTTON-11 AT ROW 20.65 COL 67.72 WIDGET-ID 8
     Btn-Grupos AT ROW 20.65 COL 115 WIDGET-ID 4
...
DO:
  RUN vta/d-cotiza-5.
  RUN dispatch IN h_b-tmpcot-5 ('open-query').
END.
...
DO:
  RUN dispatch IN h_v-cotiza-5 ('imprime'). 
END.
...
ON CHOOSE OF BUTTON-11 IN FRAME F-Main /* IMPORTAR DETALLE */
DO:
  RUN Asigna-Texto02 IN h_v-cotiza-5.
  RUN dispatch IN h_b-tmpcot-5 ('open-query':U).
END.
...
DO:
    ...
    CASE s-coddiv :
        WHEN '00023' OR WHEN '00024' THEN RUN Excel_Utilex IN h_v-cotiza-5 (INPUT lchoice) .
        OTHERWISE RUN Excel2 IN h_v-cotiza-5 (INPUT lchoice) .
    END CASE.
...
ON CHOOSE OF BUTTON-8 IN FRAME F-Main /* IMPORTAR CABECERA */
DO:
  RUN Asigna-Texto IN h_v-cotiza-5.
  RUN dispatch IN h_b-tmpcot-5 ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-cotiza-5.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotiza-5 ).
       RUN set-position IN h_v-cotiza-5 ( 1.00 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'q-pedido-5.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-pedido-5 ).
       RUN set-position IN h_q-pedido-5 ( 9.62 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-cotiza-5. */
       RUN add-link IN adm-broker-hdl ( h_p-updv03 , 'TableIO':U , h_v-cotiza-5 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-5 , 'Record':U , h_v-cotiza-5 ).

       /* Links to SmartQuery h_q-pedido-5. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido-5 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotiza-5 ,
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv03 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido-5 ,
             Btn-Grupos:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-cotiza-5.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotiza-5 ).
       RUN set-position IN h_b-cotiza-5 ( 10.96 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotiza-5 ( 10.77 , 116.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotiza-5. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido-5 , 'Record':U , h_b-cotiza-5 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotiza-5 ,
             h_p-updv03 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-tmpcot-5.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpcot-5 ).
       RUN set-position IN h_b-tmpcot-5 ( 10.96 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpcot-5 ( 9.42 , 117.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-tmpcot-5. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-tmpcot-5 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tmpcot-5 ,
             h_p-updv03 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_b-tmpcot-5 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ENABLE B-CCTE BUTTON-COT BUTTON-4 BUTTON-10 BUTTON-8 BUTTON-11 Btn-Grupos 
...
CASE L-Handle:
    WHEN 'IBC' THEN DO:
        RUN IBC-Diferencias IN h_b-tmpcot-5.
    END.
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-tmpcot-5 ('open-query':U).
      END.
    WHEN "browse1" THEN DO:
         RUN dispatch IN h_b-tmpcot-5 ('open-query':U).
         RUN dispatch IN h_b-tmpcot-5 ('apply-entry':U).
         RETURN NO-APPLY.
       END.
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
         RUN select-page(1).
         RUN dispatch IN h_b-cotiza-5 ('open-query':U).
         B-CCTE:SENSITIVE = NO.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN Recalcular-Precios IN h_b-tmpcot-5.
      END.
END CASE.
