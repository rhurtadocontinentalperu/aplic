...
/* Detraccion */
DEFINE TEMP-TABLE ttTempoDtl
    FIELD   codmat      AS  CHAR    FORMAT 'x(8)'
    FIELD   codsunat    AS  CHAR    FORMAT 'x(8)'
    FIELD   topeminimo  AS  DEC     FORMAT '->,>>>,>>9.99' INIT 0
    FIELD   factor      AS  DEC     FORMAT '->>,>>9.9999' INIT 0
    FIELD   imptelin    AS  DEC     FORMAT '->,>>>,>>9.99' INIT 0
    field   impteSunat  AS  DEC     FORMAT '->,>>>,>>9.99' INIT 0.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE genera-detraccion V-table-Win 
PROCEDURE genera-detraccion :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE VAR codSunat AS CHAR.
DEFINE VAR dTopeMinimo AS DEC INIT 0.
DEFINE VAR dFactorSunat AS DEC INIT 0.
DEFINE VAR dImpteSunat AS DEC INIT 0.
DEFINE VAR dImpteTotalSunat AS DEC INIT 0.
DEFINE VAR dImpLin AS DEC INIT 0.
DEFINE VAR dSumaImpLin AS DEC INIT 0.
DEFINE VAR dImpteTotalSunatOriginal AS DEC INIT 0.
DEFINE VAR iCorrelativo AS INT.

EMPTY TEMP-TABLE ttTempoDtl.

/* 
    Segun correo de Juan Hermoza del dia 20Ene2025.
    Basta que un item del comprobante este afecto a detraccion y 
    el monto total de la factura supere el minimo
    *-
    Se asume que el pedido esta validado (xRuben) que:
    Los items afectos a detraccion tengan el mismo topeminimo, el mismo % y el mismo codigo de Sunat
*/
    
BUSCAMOSITEMSCONDETRACCION:
FOR EACH ccbddocu WHERE ccbddocu.codcia = ccbcdocu.codcia AND 
                        ccbddocu.coddoc = ccbcdocu.coddoc AND
                        ccbddocu.nrodoc = ccbcdocu.nrodoc NO-LOCK:
    FIND FIRST almmserv WHERE almmserv.CodCia = Ccbddocu.codcia AND
                            almmserv.codmat = Ccbddocu.codmat NO-LOCK NO-ERROR.
    IF NOT AVAILABLE almmserv THEN NEXT.
    IF almmserv.AftDetraccion = NO THEN NEXT.
                
    codSunat = almmserv.codfam.

    /* Buscamos en Sunat */
    FIND FIRST factabla WHERE factabla.codcia = 1 AND 
                                factabla.tabla = 'SUNAT_CFG_DETRACCION' AND
                                factabla.codigo = codSunat NO-LOCK.
    IF NOT AVAILABLE factabla THEN NEXT.

    /* Calculamos */
    IF factabla.valor[2] > dTopeMinimo THEN dTopeMinimo = factabla.valor[2].
    dFactorSunat = factabla.valor[1].   
    dImpLin = ccbddocu.implin.
    /*
    IF Ccbcdocu.CodMon = 2 THEN dImpLin = dImpLin * Ccbcdocu.TpoCmb.

    dSumaImpLin = dSumaImpLin + dImpLin.

    dImpteSunat = ROUND(dImpLin * (dFactorSunat / 100),4).
    
    dImpteTotalSunat = dImpteTotalSunat + dImpteSunat.
    */
    /* Cargamos */
    CREATE ttTempoDtl.
        ASSIGN ttTempoDtl.codmat = Ccbddocu.codmat
                ttTempoDtl.codsunat = codSunat
                ttTempoDtl.topeminimo = dTopeMinimo
                ttTempoDtl.factor = dFactorSunat
                ttTempoDtl.imptelin = dImpLin
                ttTempoDtl.impteSunat = dImpteSunat.
END.

/* Si la venta no llega al tope minimo retornamos */
dSumaImpLin = ccbcdocu.TotalPrecioVenta.
/* Convertir a soles el total del comprobante */
IF Ccbcdocu.CodMon = 2 THEN dSumaImpLin = dSumaImpLin * Ccbcdocu.TpoCmb.
IF dTopeMinimo <= 0 OR dSumaImpLin <= 0 OR dSumaImpLin < dTopeMinimo THEN RETURN "OK".

/* Calculamos la detraccion*/
dImpteTotalSunat = dSumaImpLin * (dFactorSunat / 100) .
dImpteTotalSunatOriginal = dImpteTotalSunat.              
/* Convertimos la detraccion a la moneda original del comprobante */
IF Ccbcdocu.CodMon = 2 THEN dImpteTotalSunatOriginal = dImpteTotalSunat / Ccbcdocu.TpoCmb.

/* Creamos la detraccion */
CREATE vtacdocu.
    ASSIGN vtacdocu.codcia = 1
            vtacdocu.codped = ccbcdocu.coddoc
            vtacdocu.nroped = ccbcdocu.nrodoc
            vtacdocu.fchped = ccbcdocu.fchdoc
            vtacdocu.codcli = ccbcdocu.codcli
            vtacdocu.nomcli = ccbcdocu.nomcli
            vtacdocu.coddiv = ccbcdocu.coddiv
            vtacdocu.usuario = USERID("DICTDB")
            vtacdocu.fchinicio = NOW
            vtacdocu.impcto = dTopeMinimo                                               /* Tope minimo */
            vtacdocu.imptot = ccbcdocu.TotalPrecioVenta                                 /* Importe total del comprobante moneda original */
            vtacdocu.impfle = dFactorSunat                                              /* % detraccion aplicado */
            vtacdocu.libre_d01 = dImpteTotalSunat                                       /* Importe de la Detraccion en soles */
            vtacdocu.libre_d02 = ccbcdocu.TotalPrecioVenta - dImpteTotalSunatOriginal    /* Neto a pagar en la moneda del comprobante */
            vtacdocu.libre_c01 = codSunat.

iCorrelativo = 1.

FOR EACH ttTempoDtl NO-LOCK:
    CREATE vtaddocu.
    ASSIGN vtaddocu.codcia = 1
            vtaddocu.codped = vtacdocu.codped
            vtaddocu.nroped = vtacdocu.nroped
            vtaddocu.coddiv = vtacdocu.coddiv
            vtaddocu.nroitm = iCorrelativo
            vtaddocu.codmat = ttTempoDtl.codmat             /* Articulo de conti */
            vtaddocu.libre_c01 = ttTempoDtl.codsunat        /* codigo de sunat */
            vtaddocu.canped = 1                             /* siempre vas ser 1 */
            vtaddocu.factor = 1
            vtaddocu.implin = ttTempoDtl.impteSunat         /* Importe de a detraer */
            vtaddocu.canpick = ttTempoDtl.factor            /* % aplicado */
            vtaddocu.libre_d01 = ttTempoDtl.topeminimo      /* Tope minimo */
            vtaddocu.libre_d02 = ttTempoDtl.imptelin        /* Implin del comprobante */
        .
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR x-tipocambio AS DEC.
...
FIND Faccpedi WHERE Faccpedi.codcia = s-codcia
   ...
   IF AVAILABLE gn-tcmb THEN x-tipocambio = gn-tcmb.venta.
...
------------------------------------------------------------------------------*/
  ...
  IF Faccorre.flgest = NO THEN DO:
      MESSAGE 'Correlativo no esta Activo' VIEW-AS ALERT-BOX ERROR.
      RETURN 'ADM-ERROR'.
  END.
...
/*     Ccbcdocu.nrodoc = STRING(Faccorre.nroser, '999') + STRING(Faccorre.correlativo, '999999') */
  ...
  /* 27Dic2024 / Ic - Generar registro para la detraccion */
  RUN genera-detraccion.
  IF RETURN-VALUE = "ADM-ERROR" THEN UNDO, RETURN 'ADM-ERROR'.
