...
&Scoped-Define ENABLED-OBJECTS RECT-20 RECT-64 C-NroSer Btn-Print BUTTON-1 ~
BUTTON-6 BUTTON-7 
...
DEFINE VARIABLE h_b-guias-05 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-tmpgui AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-guias-05 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     C-NroSer AT ROW 7.69 COL 1.14 NO-LABEL
     Btn-Print AT ROW 7.81 COL 80.72
     BUTTON-1 AT ROW 12.85 COL 72
     BUTTON-6 AT ROW 13.96 COL 72
     BUTTON-7 AT ROW 15.04 COL 72
     RECT-20 AT ROW 7.54 COL 79.86
     RECT-64 AT ROW 12.85 COL 75
...
DO:
  RUN dispatch IN h_v-guias-05 ('imprime':U).
END.
...
DO:
  RUN Seleccionar-Items IN h_b-tmpgui.  
END.
...
DO:
  RUN Eliminar-Items IN h_b-tmpgui.  
END.
...
DO:
  RUN Desmarcar-Items IN h_b-tmpgui.  
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta/v-guias-05.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-guias-05 ).
       RUN set-position IN h_v-guias-05 ( 1.00 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.54 , 88.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv10 ).
       RUN set-position IN h_p-updv10 ( 7.54 , 39.72 ) NO-ERROR.
       RUN set-size IN h_p-updv10 ( 1.31 , 40.29 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-guias ).
       RUN set-position IN h_q-guias ( 7.62 , 8.86 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_v-guias-05. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-guias-05 ).
       RUN add-link IN adm-broker-hdl ( h_q-guias , 'Record':U , h_v-guias-05 ).

       /* Links to SmartQuery h_q-guias. */
       RUN add-link IN adm-broker-hdl ( h_p-navico-2 , 'Navigation':U , h_q-guias ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-guias-05 ,
             C-NroSer:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv10 ,
             h_v-guias-05 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 7.54 , 19.00 ) NO-ERROR.
       RUN set-size IN h_p-navico-2 ( 1.35 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta/b-guias-05.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-guias-05 ).
       RUN set-position IN h_b-guias-05 ( 9.08 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-guias-05 ( 9.42 , 83.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-guias-05. */
       RUN add-link IN adm-broker-hdl ( h_q-guias , 'Record':U , h_b-guias-05 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico-2 ,
             h_v-guias-05 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-guias-05 ,
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv95 ).
       RUN set-position IN h_p-updv95 ( 8.88 , 78.00 ) NO-ERROR.
       RUN set-size IN h_p-updv95 ( 3.77 , 10.72 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/b-tmpgui.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpgui ).
       RUN set-position IN h_b-tmpgui ( 8.92 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpgui ( 8.31 , 76.43 ) NO-ERROR.

       /* Links to SmartBrowser h_b-tmpgui. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_b-tmpgui ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv95 ,
             Btn-Print:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tmpgui ,
             h_p-updv95 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-20 RECT-64 C-NroSer Btn-Print BUTTON-1 BUTTON-6 BUTTON-7 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
         RUN dispatch IN h_b-guias-05 ('open-query':U).
         RUN dispatch IN h_b-tmpgui ('open-query':U).
      END.
