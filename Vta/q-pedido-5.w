...
/* Query rebuild information for QUERY Query-Main
 ...
 AND integral.VtaCDocu.usuario = S-USER-ID
 AND integral.VtaCDocu.CodPed  = S-CODDOC
"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-Numero q-tables
ON RETURN OF F-Numero IN FRAME FRAME-A
DO:
  IF SELF:SCREEN-VALUE = "" THEN RETURN.
  ASSIGN F-Numero.
  RUN Ubica-Registro.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN  input-var-1 = s-coddoc
          ...
          input-var-3 = s-tpoped
          output-var-1 = ?
          OK-WAIT-STATE = SESSION:SET-WAIT-STATE("GENERAL").

  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
      RUN lkup/c-pedido-3 ('Pedidos').
...
FIND VtaCDocu WHERE VtaCDocu.CodCia = S-CODCIA 
    ...
    AND VtaCDocu.Usuario = s-User-Id
    AND VtaCDocu.CodPed = s-coddoc 
    /*AND VtaCDocu.TpoPed = s-TpoPed*/
    AND VtaCDocu.NroPed = STRING(I-NroSer,"999") + STRING(F-Numero,"999999") 
     NO-LOCK NO-ERROR.
IF AVAILABLE VtaCDocu 
THEN output-var-1 = ROWID(VtaCDocu).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
    REPOSITION Query-Main TO ROWID OUTPUT-VAR-1 NO-ERROR.
    IF ERROR-STATUS:ERROR = NO
    THEN RUN dispatch IN THIS-PROCEDURE ('get-next':U).
   /* RHC 28-01-04
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
