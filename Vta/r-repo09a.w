...
------------------------------------------------------------------------------*/
      ...
      FOR EACH ccbcdocu NO-LOCK WHERE ccbcdocu.codcia = s-codcia
        ...
        IF NOT AVAILABLE t-cdoc THEN DO:
            ...
            BUFFER-COPY ccbcdocu 
                TO t-cdoc
                ASSIGN t-cdoc.coddiv = ccbcdocu.divori.     /* OJO */
  ...
  FOR EACH GN-DIVI NO-LOCK WHERE GN-DIVI.codcia = s-codcia
          AND (x-CodDiv = 'Todas' OR GN-DIVI.coddiv = x-CodDiv):
      FOR EACH CcbCdocu NO-LOCK WHERE CcbCdocu.CodCia = S-CODCIA 
            AND ccbcdocu.coddiv = GN-DIVI.coddiv    /* OJO */
            AND CCbCdocu.FchDoc >= x-FchDoc-1
            AND CcbCdocu.FchDoc <= x-FchDoc-2
        ...
        BUFFER-COPY Ccbcdocu TO t-cdoc
            ASSIGN
                t-cdoc.coddiv = B-CDOCU.divori  /* OJO */
  ...
  FOR EACH t-cdoc:
    ...
    FIND LAST VtaTabla WHERE VtaTabla.codcia = s-codcia
        ...
        NO-LOCK NO-ERROR.
    DETALLE:
    FOR EACH t-ddoc WHERE t-ddoc.codcia = t-cdoc.codcia
        AND t-ddoc.coddoc = t-cdoc.coddoc
        AND t-ddoc.nrodoc = t-cdoc.nrodoc,
        FIRST Almmmatg OF t-ddoc NO-LOCK WHERE Almmmatg.codfam <> '009':
  ...
  FOR EACH t-cdoc NO-LOCK, 
      EACH t-ddoc WHERE t-ddoc.codcia = t-cdoc.codcia
      AND t-ddoc.coddoc = t-cdoc.coddoc
      AND t-ddoc.nrodoc = t-cdoc.nrodoc NO-LOCK, 
