...
DEFINE BUFFER B-PedM  FOR FacCPedi.
...
&SCOPED-DEFINE CONDICION ( FacCPedi.CodCia = S-CODCIA AND FacCPedi.CodDoc = "PED" ~
                           AND FacCPedi.FlgEst = "P" AND FacCPedi.CodDiv = S-CODDIV AND FacCPedi.FchPed = TODAY )

/*&SCOPED-DEFINE CONDICION ( FacCPedi.CodCia = S-CODCIA AND FacCPedi.CodDoc = "PED" ~
 *                            AND FacCPedi.FlgEst = "P" AND FacCPedi.CodDiv = S-CODDIV  )*/
...
C-MON @ C-MON FacCPedi.ImpTot FacCPedi.FchPed FacCPedi.Hora ~
FacCPedi.Cmpbnte FacCPedi.usuario 
...
&Scoped-Define ENABLED-OBJECTS br_table FILL-IN-filtro CMB-filtro RECT-1 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.NomCli COLUMN-LABEL "Nombre o Razon Social" FORMAT "x(31)":U
      ...
      FacCPedi.usuario FORMAT "x(10)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"FacCPedi.NomCli" "Nombre o Razon Social" "x(31)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR x-codalm LIKE s-codalm NO-UNDO.

  x-codalm = s-codalm.  /* OJO */
  D-ROWID = ROWID(faccpedi).
  
  FIND B-PedM WHERE ROWID(B-PedM) = D-ROWID NO-LOCK NO-ERROR. 
  
  RUN vta\D-Nroser (OUTPUT L-OK).
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN ERROR ON STOP UNDO, RETURN 'ADM-ERROR':
     ...
     FOR EACH T-CPEDM:
         ...
         ASSIGN T-CPEDM.CodRef    = S-CodDoc
                ...
                CcbCDocu.CodCia   = S-CodCia    /**** Esta  ****/
                CcbCDocu.CodDoc   = S-CodDoc    /**** Esta  ****/
                CcbCDocu.FchDoc   = TODAY   /**** Esta  ****/
                CcbCDocu.usuario  = S-User-Id
                CcbCDocu.usrdscto = T-CPEDM.usrdscto 
                CcbCDocu.Tipo     = S-Tipo  /**** Esta "OFICINA" ****/
                CcbCDocu.CodAlm   = S-CodAlm    /**** Esta  ****/
                CcbCDocu.CodDiv   = S-CodDiv    /**** Esta  ****/
                CcbCDocu.CodCli   = T-CPEDM.Codcli  /****   Esta    ****/
                CcbCDocu.RucCli   = T-CPEDM.RucCli  /****   Esta    ****/
                CcbCDocu.NomCli   = T-CPEDM.Nomcli  /****   Esta    ****/
                CcbCDocu.DirCli   = T-CPEDM.DirCli  /****   Esta    ****/
                CcbCDocu.CodMon   = T-CPEDM.codmon  /****   Esta    ****/
                CcbCDocu.CodMov   = S-CodMov    /**** Esta  ****/
                CcbCDocu.CodPed   = T-CPEDM.coddoc  /**** Esta  ****/
                CcbCDocu.CodVen   = T-CPEDM.codven  /**** Esta  ****/
                CcbCDocu.NroOrd   = T-CPEDM.OrdCmp  /**** Esta  ****/
/*                CcbCDocu.FchCan   = TODAY*/
/*                CcbCDocu.FchVto   = T-CPEDM.fchven   /****   Actualizar D-FCHVTO ****/*/
                ...
                CcbCDocu.TipVta   = "2" /****   Esta    ****/
                CcbCDocu.TpoFac   = "R"
                CcbCDocu.FlgEst   = "P" /**** Esta  ****/
                CcbCDocu.FlgAte   = "P" /**** Nuevo ****/
                CcbCDocu.FmaPgo   = T-CPEDM.FmaPgo
                CcbCDocu.NroPed   = B-PedM.NroPed   /**** Esta  ****/
                CcbCDocu.CodRef   = "G/R"   /**** Nuevo ****/
/*                CcbCDocu.NroRef   = C-NROGUI  /**** Nuevo ****/*/
                CcbCDocu.PorIgv   = T-CPEDM.porigv  /****   FacCfgGn.PorIgv *****/
                CcbCDocu.PorDto   = T-CPEDM.PorDto
                CcbCDocu.SdoAct   = T-CPEDM.imptot
                CcbCDocu.TpoCmb   = T-CPEDM.tpocmb  /****   Esta FacCfgGn.TpoCmb[1] ****/
...
/*                CcbCDocu.FlgCon   = IF T-CPEDM.DocDesp = "GUIA" THEN "G" ELSE "".*/
         ...
         IF AVAILABLE gn-convt THEN
            CcbCDocu.FchVto = TODAY + gn-convt.totdias.
         /*********************************/

         /* actualizamos el detalle */
         FOR EACH T-DPEDM OF T-CPEDM BY nroitm:
             CREATE ccbDDocu.
             ASSIGN CcbDDocu.CodCia = ccbcdocu.codcia   /****   Esta    ****/
                    CcbDDocu.CodDoc = ccbcdocu.coddoc   /****   Esta    ****/
                    CcbDDocu.NroDoc = ccbcdocu.nrodoc   /****   Esta    ****/
                    CcbDDocu.codmat = T-DPEDM.codmat   /****   Esta    ****/
                    CcbDDocu.Factor = T-DPEDM.factor   /****   Esta    ****/
                    CcbDDocu.ImpDto = T-DPEDM.impdto   /****   Esta    ****/
                    CcbDDocu.ImpIgv = T-DPEDM.impigv   /****   Esta    ****/
                    CcbDDocu.ImpIsc = T-DPEDM.impisc   /****   Esta    ****/
                    CcbDDocu.ImpLin = T-DPEDM.implin   /****   Esta    ****/
                    CcbDDocu.AftIgv = T-DPEDM.aftigv   /****   Esta    ****/
                    CcbDDocu.AftIsc = T-DPEDM.aftisc   /****   Esta    ****/
                    CcbDDocu.CanDes = T-DPEDM.canped   /****   Esta    ****/
                    CcbDDocu.NroItm = i
                    CcbDDocu.PorDto = T-DPEDM.pordto   /****   Esta    ****/
                    CcbDDocu.PreBas = T-DPEDM.prebas   /****   Esta    ****/
                    ...
                    CcbDDocu.UndVta = T-DPEDM.undvta   /****   Esta    ****/
                    CcbDDocu.AlmDes = T-DPEDM.AlmDes   /****   Esta    ****/
             i = i + 1.
         END.
     END.
     /**** ACTUALIZAMOS FLAG DEL PEDIDO  DE MOSTRADOR COMO ATENDIDO ****/
     ASSIGN  B-PedM.flgest = "F".
 ...
 *      END.*/
  ...
  FOR EACH T-CPEDM NO-LOCK 
      ...
      IF AVAILABLE CcbCDocu THEN  DO:
         IF Ccbcdocu.CodDoc = "FAC" THEN RUN VTA\R-IMPFAC (ROWID(CcbCDocu)).
         IF Ccbcdocu.CodDoc = "BOL" THEN RUN VTA\R-IMPBOL (ROWID(CcbCDocu)).
         FIND FIRST Ccbddocu OF Ccbcdocu NO-LOCK NO-ERROR.
         IF AVAIL CcbDdocu THEN
            RUN VTA\r-odesp.R(ROWID(ccbcdocu), CcbDDocu.AlmDes).
      END.
  END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH FacDPedi OF FacCPedi 
      ...
      IF FIRST-OF(FacDPedi.AlmDes) OR L-NewPed THEN DO:
         I-NPED = I-NPED + 1.
         CREATE T-CPEDM.
         ASSIGN T-CPEDM.NroPed   = SUBSTRING(FacCPedi.NroPed,1,3) + STRING(I-NPED,"999999")
                T-CPEDM.CodAlm   = FacDPedi.AlmDes
                T-CPEDM.Cmpbnte  = FacCPedi.Cmpbnte 
/*                T-CPEDM.CodAlm   = FacCPedi.CodAlm*/      /**** MOD BY C.Q 03/03/2000 ****/
                T-CPEDM.CodCia   = FacCPedi.CodCia 
...
/*                T-CPEDM.DocDesp  = FacCPedi.DocDesp */
                ...
                T-CPEDM.OrdCmp   = FacCPedi.OrdCmp 
...
------------------------------------------------------------------------------*/
   ...
   FOR EACH T-CPEDM NO-LOCK BY T-CPEDM.NroPed ON ERROR UNDO, RETURN "ADM-ERROR":
       FIND CcbCDocu WHERE CcbCDocu.CodCia = S-CodCia 
                      AND  CcbCDocu.CodDoc = T-CPEDM.CodRef 
                      AND  CcbCDocu.NroDoc = T-CPEDM.NroRef 
                      AND  CcbCDocu.CodAlm = T-CPEDM.CodAlm        /****   ADD BY C.Q. 03/03/2000  ****/
                     EXCLUSIVE-LOCK NO-ERROR. 
       IF AVAILABLE CcbCDocu THEN  DO:
