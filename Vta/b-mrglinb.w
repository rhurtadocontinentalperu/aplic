...
&Scoped-define INTERNAL-TABLES VtaTabla almtabla

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table VtaTabla.Llave_c2 almtabla.Nombre ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaTabla WHERE VtaTabla.CodCia = TabGener.CodCia ~
  ...
  AND VtaTabla.Llave_c1 = TabGener.Codigo NO-LOCK, ~
      EACH almtabla WHERE almtabla.Codigo = VtaTabla.Llave_c2 ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH VtaTabla WHERE VtaTabla.CodCia = TabGener.CodCia ~
  ...
  AND VtaTabla.Llave_c1 = TabGener.Codigo NO-LOCK, ~
      ...
      AND almtabla.Tabla = "MK" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table VtaTabla almtabla
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c2 COLUMN-LABEL "Marca" FORMAT "x(4)":U
      almtabla.Nombre COLUMN-LABEL "Descripción" FORMAT "x(40)":U
      VtaTabla.Valor[1] COLUMN-LABEL "Productos!de Terceros" FORMAT "->>>,>>>,>>9.9999":U
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaTabla WHERE INTEGRAL.TabGener <external> ...,INTEGRAL.almtabla WHERE INTEGRAL.VtaTabla ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[1]      = "INTEGRAL.VtaTabla.CodCia = INTEGRAL.TabGener.CodCia
  AND INTEGRAL.VtaTabla.Tabla = INTEGRAL.TabGener.Clave
  AND INTEGRAL.VtaTabla.Llave_c1 = INTEGRAL.TabGener.Codigo"
     _JoinCode[2]      = "INTEGRAL.almtabla.Codigo = INTEGRAL.VtaTabla.Llave_c2"
     _Where[2]         = "INTEGRAL.almtabla.Tabla = ""MK"""
     _FldNameList[1]   > INTEGRAL.VtaTabla.Llave_c2
"VtaTabla.Llave_c2" "Marca" "x(4)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaTabla.Valor[1]" "Productos!de Terceros" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.VtaTabla.Valor[2]
"VtaTabla.Valor[2]" "Linea Propia" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF VtaTabla.Llave_c2 IN BROWSE br_table /* Marca */
DO:
...
------------------------------------------------------------------------------*/
    ...
    */
    ...
    DO WITH FRAME {&FRAME-NAME}:
        CASE HANDLE-CAMPO:name:
            WHEN "Llave_c2" THEN ASSIGN input-var-1 = "MK".
        END CASE.
    END.
...
FIND Almtabla WHERE Almtabla.Tabla = "MK"
    AND Almtabla.codigo = VtaTabla.Llave_c2:SCREEN-VALUE IN BROWSE {&browse-name}
    NO-LOCK NO-ERROR.
