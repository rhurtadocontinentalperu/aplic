...
&Scoped-define FIELDS-IN-QUERY-br_table T-CodAlm T-Desc t-StkAct t-StkComprometido   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define SELF-NAME br_table
&Scoped-define QUERY-STRING-br_table FOR EACH T-Almacen WHERE T-CodCia = S-CODCIA  AND T-StkAct > 0  NO-LOCK
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH T-Almacen WHERE T-CodCia = S-CODCIA  AND T-StkAct > 0  NO-LOCK.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 64 BY 6.69
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     FILL-IN-2 AT ROW 7.73 COL 47 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     FILL-IN-1 AT ROW 7.81 COL 33 COLON-ALIGNED
...
FOR EACH almrepos WHERE almrepos.codcia = s-codcia
    ...
    FIRST almacen NO-LOCK WHERE almacen.codcia = almrepos.codcia
        AND almacen.codalm = almrepos.almped
        AND almacen.autmov:

    CREATE t-Almacen.
    ASSIGN
          t-CodCia = Almacen.CodCia 
          t-CodAlm = Almacen.CodAlm 
          t-Desc   = Almacen.Descripcion.

    FIND almmmate WHERE Almmmate.CodCia = t-CodCia
                   AND  Almmmate.CodAlm = almrepos.almped
                   AND  Almmmate.codmat = S-CODMAT
                  NO-LOCK NO-ERROR.
    IF AVAILABLE almmmate THEN t-StkAct = Almmmate.StkAct.
    stock-tot = stock-tot + t-StkAct.

    /* STOCK comprometido */
    RUN gn/stock-comprometido (s-CodMat, t-CodAlm, OUTPUT x-StockComprometido).
