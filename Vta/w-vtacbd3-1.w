...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-CodOpe x-Periodo x-NroMes ~
f-Division FILL-IN-fchast-1 FILL-IN-fchast-2 FILL-IN-Tpocmb ~
FILL-IN-TcCompra 
...
DEFINE FRAME F-Main
     FILL-IN-CodOpe AT ROW 4.08 COL 9 COLON-ALIGNED
     ...
     FILL-IN-fchast-1 AT ROW 1.96 COL 20 COLON-ALIGNED NO-LABEL
     FILL-IN-fchast-2 AT ROW 1.96 COL 32 COLON-ALIGNED NO-LABEL
     FILL-IN-Tpocmb AT ROW 3.38 COL 20 COLON-ALIGNED NO-LABEL
     FILL-IN-TcCompra AT ROW 3.38 COL 32.43 COLON-ALIGNED NO-LABEL
     ...
     " REGISTRO  DE  VENTAS MENSUAL TRANSF GRAT" VIEW-AS TEXT
          SIZE 48 BY .85 AT ROW 1.19 COL 45
          FONT 12
     "Fecha de Proceso" VIEW-AS TEXT
          SIZE 13.43 BY .5 AT ROW 1.38 COL 26
     "T.C.Venta" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 2.92 COL 22.57
     "T.C.Compra" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 2.92 COL 34.72
 ...
 *      MESSAGE 'Fecha no asignada' VIEW-AS ALERT-BOX ERROR.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-vtacbd ).
       RUN set-position IN h_b-vtacbd ( 5.04 , 1.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv10 ,
             FILL-IN-CodOpe:HANDLE IN FRAME F-Main , 'AFTER':U ).
...
/* Limpio la informaci¢n de los movimientos transferidos anteriormente
 ...
 FOR EACH cb-control WHERE 
          ...
          cb-control.tipo    = '@PVT' AND           /* Extorno Ventas por Transf Gratuita */
...
FOR EACH CcbCDocu WHERE CcbCDocu.codcia = s-codcia AND
    ...
    THEN x-Cco = '25'.
    /* RHC 15/03/18 Solicitado por Luis Urbano Fung */
    x-CCO = "03".   /* COMERCIAL */
    /* ******************************************************* */
    CREATE T-CDOC.
...
FOR EACH T-CDOC WHERE LOOKUP(T-CDOC.CodDoc, 'FAC,BOL') > 0 
        AND LOOKUP(T-CDOC.FmaPgo, '899,900') > 0
...
FOR EACH T-CDOC BREAK BY T-CDOC.CodDiv BY T-CDOC.Cco BY T-CDOC.NroDoc:
    ...
    IF NOT AVAILABLE Cb-cfgrv OR Cb-cfgrv.Codcta = ""  THEN DO:
      MESSAGE 'Cuenta No Configurada Para ' + T-CDOC.Coddoc + ' ' + T-CDOC.Nrodoc  VIEW-AS ALERT-BOX.
      NEXT .
    END.
    X-codcta = Cb-cfgrv.Codcta .       
    RUN Graba-Documentos (x-codcbd, x-codcta, NOT x-tpodoc, T-CDOC.ImpTot, x-coddoc, 08, T-CDOC.Codmon,x-detalle, x-cco).
...
DO FILL-IN-FchAst = FILL-IN-fchast-1 TO FILL-IN-fchast-2:
    ...
    FOR EACH FacDocum WHERE FacDocum.CodCia = s-codcia NO-LOCK:
        IF LOOKUP(FacDocum.CodDoc, 'FAC,BOL') > 0 THEN DO:
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-CodOpe x-Periodo x-NroMes f-Division FILL-IN-fchast-1 
          FILL-IN-fchast-2 FILL-IN-Tpocmb FILL-IN-TcCompra 
...
------------------------------------------------------------------------------*/
 ...
 X-NRODOC = IF X-DETALLE THEN T-CDOC.Nrodoc ELSE "111111111".       /* UN SOLO NUMERO DE DOCUMENTO (SIN DETALLE) */
 ...
 *  END.*/
 ...
 IF NOT AVAILABLE t-prev THEN DO:
   ...
   ASSIGN
      ...
      t-prev.tpomov  = x-tpomov
      t-prev.clfaux  = '@CL'
/*ML01*/ t-prev.nroruc  = IF cRuc <> "" THEN cRuc ELSE x-codcli /*Gn-Clie.Ruc*/
...
/*      t-prev.impmn1  = t-prev.impmn1 + ROUND(( x-importe * FILL-IN-Tpocmb ) ,2).  */
        t-prev.impmn1  = t-prev.impmn1 + ROUND(( x-importe * x-TCVenta ) ,2).  
 ASSIGN
/*    t-prev.Tpocmb  = IF x-codmon = 2 THEN FILL-IN-Tpocmb ELSE 0*/
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE cb-Cfgg THEN DO:
     ASSIGN
        ...
        x-ctaigv = cb-cfgg.codaux[9]
        x-ctadto = cb-cfgg.codcta[10]
        x-codope = '066'.               /* cb-cfgg.Codope. */
     END.
  ELSE DO:
