...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      almmserv.codmat COLUMN-LABEL "Codigo" FORMAT "X(6)":U WIDTH 8
      almmserv.DesMat FORMAT "X(45)":U
      almmserv.CatConta COLUMN-LABEL "Cuenta" FORMAT "x(8)":U WIDTH 8
  ENABLE
      almmserv.CatConta
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 54 BY 13.85
         FONT 4
         TITLE "Cuentas por Servicios" FIT-LAST-COLUMN.
...
"codmat" "Codigo" ? "character" ? ? ? ? ? ? no ? no no "8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.almmserv.DesMat
     _FldNameList[3]   > INTEGRAL.almmserv.CatConta
"CatConta" "Cuenta" "x(8)" "character" ? ? ? ? ? ? yes ? no no "8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Cuentas por Servicios */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Cuentas por Servicios */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Cuentas por Servicios */
DO:
...
ASSIGN
    chWorkSheet:Range("A1"):Value = "CUENTAS POR SERVICIOS"
    chWorkSheet:Range("A2"):Value = "CODIGO"
    chWorkSheet:Columns("A"):NumberFormat = "@"
    chWorkSheet:Range("B2"):Value = "DESCRIPCION"
    chWorkSheet:Range("C2"):Value = "CUENTA"
    chWorkSheet:Columns("C"):NumberFormat = "@".
ASSIGN
    t-Row = 2.
FOR EACH Almmserv NO-LOCK WHERE Almmser.codcia = s-codcia:
    ASSIGN
        t-Column = 0
        t-Row    = t-Row + 1.
    ASSIGN
        t-Column = t-Column + 1
        chWorkSheet:Cells(t-Row, t-Column):VALUE = almmserv.codmat.
    ASSIGN
        t-Column = t-Column + 1
        chWorkSheet:Cells(t-Row, t-Column):VALUE = almmserv.DesMat.
    ASSIGN
        t-Column = t-Column + 1
        chWorkSheet:Cells(t-Row, t-Column):VALUE = almmserv.CatConta.
END.
...
------------------------------------------------------------------------------*/
    ...
    IF Almmserv.CatConta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} <> "" THEN DO:
        FIND cb-ctas WHERE
            cb-ctas.CodCia = cb-codcia AND
            cb-ctas.CodCta = Almmserv.CatConta:SCREEN-VALUE
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE cb-ctas THEN DO:
            MESSAGE
                "Cuenta Contable no Existe"
                VIEW-AS ALERT-BOX ERROR.
            APPLY "ENTRY" TO Almmserv.CatConta.
            RETURN "ADM-ERROR".   
        END.
    END.
