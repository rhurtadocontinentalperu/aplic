...
DEFINE TEMP-TABLE tmp-tempo 
    FIELD t-codcia  LIKE evtprovf.Codcia 
    FIELD t-CodProv LIKE evtprovf.CodProv     
    FIELD t-fmapgo  LIKE evtprovf.fmapgo     
    FIELD t-Codano  LIKE evtprovf.Codano
    FIELD t-Codmes  LIKE evtprovf.Codmes
    FIELD t-Coddia  LIKE evtprovf.Codano
...
&Scoped-Define ENABLED-OBJECTS RECT-58 RECT-62 RECT-63 C-tipo C-tipo-2 ~
F-DIVISION BUTTON-5 F-prov1 BUTTON-4 F-pago BUTTON-6 F-DIVISION-1 ~
F-DIVISION-2 DesdeF HastaF F-DIVISION-3 F-DIVISION-4 F-DIVISION-5 ~
F-DIVISION-6 nCodMon Btn_OK Btn_Cancel Btn_Help 
&Scoped-Define DISPLAYED-OBJECTS C-tipo C-tipo-2 F-DIVISION F-prov1 F-pago ~
F-DIVISION-1 F-NOMDIV-1 F-DIVISION-2 F-NOMDIV-2 DesdeF F-NOMDIV-3 HastaF ~
...
DEFINE VARIABLE C-tipo-2 AS CHARACTER FORMAT "X(20)":U INITIAL "Cantidad" 
     ...
     LIST-ITEMS "Cantidad","Venta","Costo","Margen","Utilidad","ooooo" 
     DROP-DOWN-LIST
     SIZE 16 BY 1 NO-UNDO.
...
DEFINE VARIABLE F-pago AS CHARACTER FORMAT "X(11)":U 
     LABEL "Forma de Pago" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .69 NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     F-prov1 AT ROW 3.96 COL 11.72 COLON-ALIGNED
     BUTTON-4 AT ROW 3.96 COL 25.29
     F-pago AT ROW 4.92 COL 11.72 COLON-ALIGNED
     BUTTON-6 AT ROW 4.92 COL 25.29
     ...
     DesdeF AT ROW 6.38 COL 11.86 COLON-ALIGNED
     F-NOMDIV-3 AT ROW 6.42 COL 55.86 COLON-ALIGNED NO-LABEL
     HastaF AT ROW 6.46 COL 33.57 COLON-ALIGNED
...
DO:
  DO WITH FRAME {&FRAME-NAME}:
    input-var-1 = "".
    output-var-2 = "".
    RUN lkup\C-condvt.r("Forma de Pago").
    IF output-var-2 <> ? THEN DO:
        F-PAGO = output-var-2.
        DISPLAY F-pago.
        APPLY "ENTRY" TO F-PAGO.
        RETURN NO-APPLY.
...
FOR EACH evtprovf NO-LOCK WHERE evtprovf.CodCia = S-CODCIA
                         AND   evtprovf.CodDiv = Gn-Divi.Coddiv
                         AND   evtprovf.CodProv BEGINS F-Prov1
                         AND   (evtprovf.Nrofch >= INTEGER(STRING(YEAR(DesdeF),"9999") + STRING(MONTH(DesdeF),"99"))
                         AND   evtprovf.Nrofch <= INTEGER(STRING(YEAR(HastaF),"9999") + STRING(MONTH(HastaF),"99")) ):
                         

      DISPLAY evtprovf.Coddiv @ Fi-Mensaje LABEL "Codigo de Articulo "
      ...
      IF evtprovf.Codmes < 12 THEN DO:
        ASSIGN
        X-CODMES = evtprovf.Codmes + 1
        X-CODANO = evtprovf.Codano .
      END.
      ELSE DO: 
        ASSIGN
        X-CODMES = 01
        X-CODANO = evtprovf.Codano + 1 .
      END.
      /**********************************************************************/
      ...
      /*********************** Calculo Para Obtener los datos diarios ************/
       DO I = 1 TO DAY( DATE(STRING(X-CODDIA,"99") + "/" + STRING(X-CODMES,"99") + "/" + STRING(X-CODANO,"9999")) - 1 ) :
            ...
            X-FECHA = DATE(STRING(I,"99") + "/" + STRING(evtprovf.Codmes,"99") + "/" + STRING(evtprovf.Codano,"9999")).
            IF X-FECHA >= DesdeF AND X-FECHA <= HastaF THEN DO:
                FIND Gn-tcmb WHERE Gn-tcmb.Fecha = DATE(STRING(I,"99") + "/" + STRING(evtprovf.Codmes,"99") + "/" + STRING(evtprovf.Codano,"9999")) NO-LOCK NO-ERROR.
                IF AVAILABLE Gn-tcmb THEN DO: 
                    T-Vtamn   = evtprovf.Vtaxdiamn[I] + evtprovf.Vtaxdiame[I] * Gn-Tcmb.Venta.
                    T-Vtame   = evtprovf.Vtaxdiame[I] + evtprovf.Vtaxdiamn[I] / Gn-Tcmb.Compra.
                    T-Ctomn   = evtprovf.Ctoxdiamn[I] + evtprovf.Ctoxdiame[I] * Gn-Tcmb.Venta.
                    T-Ctome   = evtprovf.Ctoxdiame[I] + evtprovf.Ctoxdiamn[I] / Gn-Tcmb.Compra.
                    FIND tmp-tempo WHERE T-Codcia  = S-CODCIA AND
                                         T-CodProv  = evtprovf.CodProv AND
                                         T-Codano  = evtprovf.Codano AND
                                         T-Codmes  = evtprovf.Codmes AND
                                         T-Coddia  = I NO-ERROR.
                    IF NOT AVAIL tmp-tempo THEN DO:
                      CREATE tmp-tempo.
                      ASSIGN T-codcia  = S-CODCIA
                             T-CodProv  = evtprovf.CodProv
                             T-Codano  = evtprovf.Codano 
                             T-Codmes  = evtprovf.Codmes
                             T-Coddia  = I 
                             T-Fchdoc  = X-FECHA.                         
                    END.
                    ...
                    X-ENTRA = FALSE.
                    IF F-DIVISION-1 <> "" AND evtprovf.Coddiv = F-DIVISION-1 THEN DO:
                       ASSIGN T-Venta[1] = T-venta[1] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-2 <> "" AND evtprovf.Coddiv = F-DIVISION-2 THEN DO:
                       ASSIGN T-Venta[2] = T-venta[2] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-3 <> "" AND evtprovf.Coddiv = F-DIVISION-3 THEN DO:
                       ASSIGN T-Venta[3] = T-venta[3] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-4 <> "" AND evtprovf.Coddiv = F-DIVISION-4 THEN DO:
                       ASSIGN T-Venta[4] = T-venta[4] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-5 <> "" AND evtprovf.Coddiv = F-DIVISION-5 THEN DO:
                       ASSIGN T-Venta[5] = T-venta[5] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-6 <> "" AND evtprovf.Coddiv = F-DIVISION-6 THEN DO:
...
FOR EACH evtprovf NO-LOCK WHERE evtprovf.CodCia = S-CODCIA
                         AND   evtprovf.CodDiv  = Gn-Divi.Coddiv
                         AND   evtprovf.CodProv BEGINS F-Prov1
                         AND   evtprovf.FmaPgo  BEGINS F-pago
                         AND   (evtprovf.Nrofch >= INTEGER(STRING(YEAR(DesdeF),"9999") + STRING(MONTH(DesdeF),"99"))
                         AND   evtprovf.Nrofch  <= INTEGER(STRING(YEAR(HastaF),"9999") + STRING(MONTH(HastaF),"99")) ):
                         

      DISPLAY evtprovf.Coddiv @ Fi-Mensaje LABEL "Division" SKIP
              evtprovf.Codprov @ Fi-Mensaje LABEL "Proveedor"   
              FORMAT "X(11)" WITH FRAME F-Proceso.
      ...
      /*********************** Calculo Para Obtener los datos diarios ************/
       DO I = 1 TO DAY( DATE(STRING(X-CODDIA,"99") + "/" + STRING(X-CODMES,"99") + "/" + STRING(X-CODANO,"9999")) - 1 ) :        
            ...
            IF X-FECHA >= DesdeF AND X-FECHA <= HastaF THEN DO:
                FIND Gn-tcmb WHERE Gn-tcmb.Fecha = DATE(STRING(I,"99") + "/" + STRING(evtprovf.Codmes,"99") + "/" + STRING(evtprovf.Codano,"9999")) NO-LOCK NO-ERROR.
                IF AVAILABLE Gn-tcmb THEN DO: 
                 T-Vtamn   = T-Vtamn   + evtprovf.Vtaxdiamn[I] + evtprovf.Vtaxdiame[I] * Gn-Tcmb.Venta.
                 T-Vtame   = T-Vtame   + evtprovf.Vtaxdiame[I] + evtprovf.Vtaxdiamn[I] / Gn-Tcmb.Compra.
                 T-Ctomn   = T-Ctomn   + evtprovf.Ctoxdiamn[I] + evtprovf.Ctoxdiame[I] * Gn-Tcmb.Venta.
                 T-Ctome   = T-Ctome   + evtprovf.Ctoxdiame[I] + evtprovf.Ctoxdiamn[I] / Gn-Tcmb.Compra.
                END.
            END.
       END.         
      ...
      FIND tmp-tempo WHERE t-codcia  = S-CODCIA AND
                           T-CodProv = evtprovf.CodProv AND
                           T-fmapgo  = evtprovf.FmaPgo AND 
                           T-Codano  = evtprovf.Codano AND
                           T-Codmes  = evtprovf.Codmes NO-ERROR.
      IF NOT AVAIL tmp-tempo THEN DO:
        CREATE tmp-tempo.
        ASSIGN t-codcia  = S-CODCIA
               T-CodProv = evtprovf.CodProv
               T-fmapgo  = evtprovf.FmaPgo
               T-Codano  = evtprovf.Codano 
               T-Codmes  = evtprovf.Codmes .
      END.
      ASSIGN T-Venta[10] = T-venta[10] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
...
------------------------------------------------------------------------------*/
  DISPLAY C-tipo C-tipo-2 F-DIVISION F-prov1 F-pago F-DIVISION-1 F-NOMDIV-1 
          F-DIVISION-2 F-NOMDIV-2 DesdeF F-NOMDIV-3 HastaF F-DIVISION-3 
          F-DIVISION-4 F-NOMDIV-4 F-DIVISION-5 F-NOMDIV-5 F-DIVISION-6 
          F-NOMDIV-6 nCodMon 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-58 RECT-62 RECT-63 C-tipo C-tipo-2 F-DIVISION BUTTON-5 F-prov1 
         BUTTON-4 F-pago BUTTON-6 F-DIVISION-1 F-DIVISION-2 DesdeF HastaF 
  ...
  FOR EACH tmp-tempo  /*WHERE t-venta[10] <> 0 */
      ...
      IF FIRST-OF(t-CodProv) THEN DO:
         ...
         IF FIRST-OF(T-FmaPgo) THEN DO:
            FIND Gn-Convt WHERE Gn-Convt.Codig = T-FmaPgo
                            NO-LOCK NO-ERROR.
                            
            PUT STREAM REPORT T-FmaPgo AT 1 .
            IF AVAILABLE Gn-Convt THEN DO:
               PUT STREAM REPORT Gn-Convt.NomBr AT 14 FORMAT "X(45)" SKIP.
            END.
...
------------------------------------------------------------------------------*/
 DEFINE FRAME F-REPORTE
  ...
  DEFINE FRAME F-HEADER
         ...
         "DIVISIONES EVALUADAS  : " + X-CODDIV  At 1 FORMAT "X(150)" SKIP
         "LOS IMPORTE INCLUYEN I.G.V. "  At 1 FORMAT "X(150)" SKIP
         "PROVEEDORES           : " + F-PROV1  At 1 FORMAT "X(150)" SKIP
         "CONDICION DE PAGOS    : " + F-PAGO   At 1 FORMAT "X(150)" SKIP
        "------------------------------------------------------------------------------------------------------------------------------------------------------------" SKIP
        F-DIVISION-1 FORMAT "X(15)" AT  32 
        F-DIVISION-2 FORMAT "X(15)" AT  48 
        F-DIVISION-3 FORMAT "X(15)" AT  63 
        F-DIVISION-4 FORMAT "X(15)" AT  78 
        F-DIVISION-5 FORMAT "X(15)" AT  93
        F-DIVISION-6 FORMAT "X(15)" AT  108
        "O T R O S"  AT  120
        "T O T A L"  AT  140 SKIP
        "          AÑO MES              Venta         Venta          Venta          Venta           Venta          Venta           Venta             Venta    " SKIP
        "------------------------------------------------------------------------------------------------------------------------------------------------------------" SKIP
  ...
  FOR EACH tmp-tempo  
      ...
      IF FIRST-OF(t-CodProv) THEN DO:
         FIND Gn-Prov WHERE Gn-prov.Codcia = pv-codcia AND
                            Gn-prov.CodPro = T-CodProv
                            NO-LOCK NO-ERROR.
         PUT STREAM REPORT T-CodProv AT 5 .
         IF AVAILABLE Gn-Prov THEN DO:
            PUT STREAM REPORT Gn-Prov.NomPro AT 17 FORMAT "X(45)" SKIP.
         END.
