...
DEFINE VARIABLE h_b-orddes-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-tmpdes-4 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     B-AGTRANS AT ROW 11.23 COL 91 WIDGET-ID 4
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 108.86 BY 23.04
         FONT 4.
...
DO:
   RUN Asigna_datos IN h_v-orddes-4.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta/v-orddes-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-orddes-4 ).
       RUN set-position IN h_v-orddes-4 ( 1.00 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.23 , 98.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm-vm/objects/p-updv13.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
                     SmartPanelType = Update,
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv13 ).
       RUN set-position IN h_p-updv13 ( 11.23 , 30.00 ) NO-ERROR.
       RUN set-size IN h_p-updv13 ( 1.31 , 61.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-pedido ).
       RUN set-position IN h_q-pedido ( 11.23 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-orddes-4. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_v-orddes-4 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-orddes-4 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-orddes-4 ,
             B-AGTRANS:HANDLE IN FRAME F-Main , 'BEFORE':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta/b-orddes-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-orddes-4 ).
       RUN set-position IN h_b-orddes-4 ( 12.58 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-orddes-4 ( 9.69 , 96.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-orddes-4. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_b-orddes-4 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-orddes-4 ,
             B-AGTRANS:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta/b-tmpdes-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpdes-4 ).
       RUN set-position IN h_b-tmpdes-4 ( 12.58 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpdes-4 ( 9.42 , 100.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tmpdes-4 ,
             B-AGTRANS:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-orddes-4 ('open-query':U).
          RUN dispatch IN h_b-tmpdes-4 ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN Recalcular-Precios IN h_b-tmpdes-4.
      END.
END CASE.
