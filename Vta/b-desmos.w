...
&Scoped-define OPEN-QUERY-br_table CASE COMBO-BOX-8:SCREEN-VALUE IN FRAME {&FRAME-NAME} :      WHEN "Fechas" THEN DO :           OPEN QUERY {&SELF-NAME} FOR EACH Faccpedm WHERE {&KEY-PHRASE}           AND Faccpedm.codcia = s-codcia           AND Faccpedm.codalm = S-CODALM           AND FacCPedm.CodDiv BEGINS S-CODDIV           AND FacCPedm.CodDoc BEGINS S-CODDOC           AND FacCpedm.Nomcli BEGINS wclient           AND FacCPedm.FlgEst BEGINS f-estado           AND FacCPedm.FchPed >= F-DesFch           AND FacCPedm.FchPed <= F-HasFch NO-LOCK           {&SORTBY-PHRASE}.      END.      OTHERWISE  DO :         OPEN QUERY {&SELF-NAME} FOR EACH Faccpedm WHERE {&KEY-PHRASE}         AND FacCPedm.CodCia = S-CODCIA         AND Faccpedm.codalm = S-CODALM         AND FacCPedm.CodDiv BEGINS S-CODDIV         AND FacCPedm.CodDoc BEGINS S-CODDOC         AND FacCpedm.Nomcli BEGINS wclient         AND FacCPedm.FlgEst BEGINS f-estado NO-LOCK         {&SORTBY-PHRASE}.      END. END CASE.
...
DEFINE VARIABLE COMBO-BOX-8 AS CHARACTER FORMAT "X(256)":U INITIAL "Pedido" 
     ...
     LIST-ITEMS "Pedido","Cliente","Fechas" 
     SIZE 12 BY 1 NO-UNDO.

DEFINE VARIABLE COMBO-BOX-9 AS CHARACTER FORMAT "X(256)":U INITIAL "Todos" 
     LABEL "Condición" 
     VIEW-AS COMBO-BOX INNER-LINES 5
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 85.86 BY 9.77
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     wclient AT ROW 1.31 COL 31.57 COLON-ALIGNED
     F-DesFch AT ROW 1.31 COL 32.29 COLON-ALIGNED
     F-HasFch AT ROW 1.31 COL 50.57 COLON-ALIGNED
     COMBO-BOX-9 AT ROW 1.23 COL 71.29 COLON-ALIGNED
...
CASE COMBO-BOX-8:SCREEN-VALUE IN FRAME {&FRAME-NAME} :
     WHEN "Fechas" THEN DO :
          ...
          AND FacCPedm.FlgEst BEGINS f-estado
...
&Scoped-define SELF-NAME COMBO-BOX-9
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL COMBO-BOX-9 B-table-Win
ON VALUE-CHANGED OF COMBO-BOX-9 IN FRAME F-Main /* Condición */
DO:
  ASSIGN COMBO-BOX-9.
  CASE COMBO-BOX-9:
       WHEN "Pendiente" THEN 
