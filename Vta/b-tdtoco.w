...
ON "RETURN":U OF PEDI.Preuni, PEDI.Por_DSCTOS[1]
DO:
   APPLY "TAB":U.
   RETURN NO-APPLY.
END.
...
DO:
  ...
  DEFINE VARIABLE x-des     AS decimal init 0.
  ...
  DEFINE VARIABLE F-FACTOR  AS DECIMAL.
  ...
  x-des = ( 1 - ( x-newpre / PEDI.preuni )) * 100.
  ...
  FIND Almtabla WHERE 
       ...
       NO-LOCK NO-ERROR.
/******************  descuento permitido por usuario ********************/
  
  /*CASE almtabla.Codigo:
 *        WHEN "D1" THEN DO:
 *             IF x-des > FacCfgGn.DtoMax THEN DO:
 *                MESSAGE "Maximo DSCTO.  permitido" SKIP
 *                        "para el nivel de Usuario" SKIP
 *                        TRIM(s-nivel) " : " almtabla.Nombre " es : " FacCfgGn.DtoMax " %" SKIP
 *                        VIEW-AS ALERT-BOX ERROR TITLE "Descuento por PEDI".
 *                APPLY "ENTRY" TO PEDI.Preuni.
 *                RETURN NO-APPLY.
 *             END.
 *        END.
 *        WHEN "D2" THEN
 *             IF x-des > FacCfgGn.DtoDis THEN DO:
 *                MESSAGE "Maximo DSCTO.  permitido" SKIP
 *                        "para el nivel de Usuario" SKIP
 ...
 *             END.
 *        WHEN "D3" THEN
 *             IF x-des > FacCfgGn.DtoMay THEN DO:
 *                MESSAGE "Maximo DSCTO.  permitido" SKIP
 *                        "para el nivel de Usuario" SKIP
 ...
 *   END CASE.*/
  ...
  dsct_disp = ROUND(((1 - ((1 - Almmmatg.PorMax / 100) / (1 - PEDI.PorDto / 100))) * 100), 2).
  ...
  X-NEWDSC = ROUND(((1 - (1 - PEDI.PorDto / 100) * (1 - F-DSCTO / 100))) * 100 ,2).
  /*  IF X-NEWDSC > X-MAXDSC THEN DO:*/

/****************************  VALIDACIÓN DSCTO POR ARTICULO*****/

/*  IF x-des > dsct_disp THEN DO:
 *     MESSAGE "Se ha excedido DSCTO. del Articulo :" Almmmatg.PorMax SKIP
 *             "Dscto. posible : " dsct_disp " %" SKIP
 *             VIEW-AS ALERT-BOX ERROR.
 *             X-NEWDSC = X-NEWDSC - x-des.
 *             APPLY "ENTRY" TO PEDI.Preuni.
 *             RETURN NO-APPLY.
 *   END.*/
    
  X-NEWPRE = ((Almmmatg.PreOfi - (Almmmatg.PreOfi * (X-NEWDSC / 100) ))).
  
  /****   Convierte al tipo de moneda ****/.
  IF Almmmatg.MonVta = 1 THEN
      ASSIGN Pre1 = X-NEWPRE
             Pre2 = ROUND(X-NEWPRE / S-TPOCMB,6).
  ELSE
      ASSIGN Pre2 = X-NEWPRE
             Pre1 = ROUND(X-NEWPRE * S-TPOCMB,6).
             
   X-NEWPRE = IF S-CODMON = 1 THEN Pre1 ELSE Pre2.
   
   PEDI.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = STRING(X-NEWPRE).
   
   PEDI.Por_DSCTOS[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = string(x-des). 
...
/*    x-cto1 = 0.
    ...
    x-cto  = 0.
    x-uni  = 0.
    x-uni     = DEC(PEDI.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
    MESSAGE X-UNI.
    x-margen  = Almmmatg.CtoTot.
    IF Almmmatg.MonVta = 1 THEN
        ASSIGN x-cto1 = x-margen
               x-cto2 = ROUND(x-margen / S-TPOCMB,4).
    ELSE
        ASSIGN x-cto2 = x-margen
...
/*  CASE almtabla.Codigo:
 *        WHEN "D1" THEN DO:
 *             IF DEC(PEDI.Por_DSCTOS[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > FacCfgGn.DtoMax THEN DO:
 *                MESSAGE "Maximo DSCTO.  permitido" SKIP
 *                        "para el nivel de Usuario" SKIP
 *                        TRIM(s-nivel) " : " almtabla.Nombre " es : " FacCfgGn.DtoMax " %" SKIP
 *                        VIEW-AS ALERT-BOX ERROR TITLE "Descuento por PEDI".
 ...
 *        END.
 *        WHEN "D2" THEN
 *             IF DEC(PEDI.Por_DSCTOS[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > FacCfgGn.DtoDis THEN DO:
 *                MESSAGE "Maximo DSCTO.  permitido" SKIP
 *                        "para el nivel de Usuario" SKIP
 *                        TRIM(s-nivel) " : " almtabla.Nombre " es : " FacCfgGn.DtoDis " %" SKIP
 *                        VIEW-AS ALERT-BOX ERROR TITLE "Descuento por PEDI".
 ...
 *             END.
 *        WHEN "D3" THEN
 *             IF DEC(PEDI.Por_DSCTOS[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > FacCfgGn.DtoMay THEN DO:
 *                MESSAGE "Maximo DSCTO.  permitido" SKIP
 *                        "para el nivel de Usuario" SKIP
 *                        TRIM(s-nivel) " : " almtabla.Nombre " es : " FacCfgGn.DtoMay " %"  SKIP
 *                        VIEW-AS ALERT-BOX ERROR TITLE "Descuento por PEDI".
 ...
 *   END CASE.*/ 
  ...
  /*dsct_disp = ROUND((1 - (1 - PEDI.Por_Dsctos[2] / 100) / (1 - PEDI.PorDto / 100)), 2) * 100.*/
...
/*  IF DEC(PEDI.Por_DSCTOS[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > dsct_disp THEN DO:
 *      MESSAGE "Se ha excedido DSCTO. del Articulo :" PEDI.Por_Dsctos[2]  SKIP
 *              "Dscto. posible : " dsct_disp  " %" SKIP
 *              VIEW-AS ALERT-BOX ERROR.
 *              X-NEWDSC = X-NEWDSC - DEC(PEDI.Por_DSCTOS[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
 *              APPLY "ENTRY" TO PEDI.Por_DSCTOS[1] IN BROWSE {&BROWSE-NAME}.
 *              RETURN NO-APPLY.
 *   END.*/
  ...
  /*********/ 
    ...
    ELSE
     ...
     IF x-Margen <= 0 AND s-Nivel <> 'D3' THEN DO:     /* NO Gerente General */
         MESSAGE 'NO está permitido margen negativo'
             VIEW-AS ALERT-BOX ERROR.
         RETURN NO-APPLY.
     END.
   /*********/   
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE Almmmatg THEN DO:
     ASSIGN PEDI.PreUni = DEC(PEDI.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME})
