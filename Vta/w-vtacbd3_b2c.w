...
DEFINE TEMP-TABLE T-CDOC LIKE CCBCDOCU      /* ARCHIVO DE TRABAJO */
    FIELD   b2c-web AS CHAR   FORMAT 'x(50)'
    FIELD   torigen-tarjeta AS    CHAR    FORMAT 'x(50)'
    FIELD   ttipo-tarjeta   AS  CHAR    FORMAT 'x(50)'
...
DEFINE FRAME F-Main
     x-Periodo AT ROW 1.58 COL 17.43 COLON-ALIGNED
     x-NroMes AT ROW 2.54 COL 17.29 COLON-ALIGNED
     f-Division AT ROW 3.5 COL 17 COLON-ALIGNED
     FILL-IN-fchast-1 AT ROW 2.54 COL 27.72 COLON-ALIGNED NO-LABEL
     FILL-IN-fchast-2 AT ROW 2.54 COL 39.72 COLON-ALIGNED NO-LABEL
     FILL-IN-Tpocmb AT ROW 2.35 COL 65.14 COLON-ALIGNED NO-LABEL
     FILL-IN-TcCompra AT ROW 2.42 COL 84.57 COLON-ALIGNED NO-LABEL
     B-filtro AT ROW 1.19 COL 116
     B-Transferir AT ROW 2.92 COL 116
     B-Imprimir AT ROW 1.19 COL 130
     "Tranferir" VIEW-AS TEXT
          SIZE 6.43 BY .5 AT ROW 3.12 COL 109
     "Pre-asiento" VIEW-AS TEXT
          SIZE 8.14 BY .65 AT ROW 1.58 COL 107
     "Asiento" VIEW-AS TEXT
          SIZE 5.57 BY .5 AT ROW 3.65 COL 109.57
     "Imprimir" VIEW-AS TEXT
          SIZE 5.43 BY .5 AT ROW 2.85 COL 130.57
     "Generacio de Asientos contables B2C  (Niubiz, PagoEfectivo)" VIEW-AS TEXT
          SIZE 58 BY .85 AT ROW 3.69 COL 35
          BGCOLOR 15 FGCOLOR 9 FONT 12
     "Fecha de Proceso" VIEW-AS TEXT
          SIZE 13.43 BY .5 AT ROW 1.96 COL 33.72
     "T.C.Venta" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 2.54 COL 59
     "T.C.Compra" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 2.54 COL 77
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 136.57 BY 23.62
         FONT 4.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "e-Commerce - Generacion de Asiento Contable Mensual"
         HEIGHT             = 23.62
         WIDTH              = 136.57
         MAX-HEIGHT         = 24.77
         MAX-WIDTH          = 149.72
...
ON END-ERROR OF W-Win /* e-Commerce - Generacion de Asiento Contable Mensual */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* e-Commerce - Generacion de Asiento Contable Mensual */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-vtacbd3 ).
       RUN set-position IN h_b-vtacbd3 ( 5.27 , 1.14 ) NO-ERROR.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER X-coddoc  AS CHAR.           /* FAC,BOL,... */
DEFINE INPUT PARAMETER X-codcta1 AS CHAR.
DEFINE INPUT PARAMETER X-codcta2 AS CHAR.
DEFINE INPUT PARAMETER X-codcbd  AS CHAR.           /* TIPO DOCUMENTO SUNAT - 01,03 - FAC,BOL*/
DEFINE INPUT PARAMETER X-tpodoc  AS LOGICAL.        /* CARGO / ABONO */

DEFINE VAR x-codcta  AS CHAR    NO-UNDO.
DEFINE VAR x-detalle AS LOGICAL NO-UNDO.
DEFINE VAR x-cco     AS CHAR    NO-UNDO.

DEFINE VAR x-vendedor-web AS CHAR.

X-nrodoc1 = ''.
X-nrodoc2 = ''.

FIND FIRST cb-cfgg WHERE cb-cfgg.CodCia = cb-codcia AND
     cb-cfgg.Codcfg = 'R02' NO-LOCK NO-ERROR.
...
FOR EACH CcbCDocu WHERE CcbCDocu.codcia = s-codcia AND
        ...
        Ccbcdocu.tpofac <> "B" NO-LOCK:
    
    /* PARCHE: anuladas NO deben pasar */
    IF Ccbcdocu.flgest = 'A' THEN NEXT.
    /* *************************************** */

    /* Verificamos si es NIUBIZ o PAGOEFECTIVO */
    FIND FIRST pago_efectivo WHERE pago_efectivo.codcia = s-codcia AND
                                    pago_efectivo.orden_comercio = ccbcdocu.nroord NO-LOCK NO-ERROR.
    IF NOT AVAILABLE pago_efectivo THEN DO:
        FIND FIRST b2c-interfase WHERE b2c-interfase.codcia = s-codcia AND
                                        b2c-interfase.pedido_cliente = ccbcdocu.nroord NO-LOCK NO-ERROR.
        
        x-vendedor-web = "NIUBIZ".
        IF NOT AVAILABLE b2c-interfase THEN NEXT.
    END.
    ELSE DO:        
        x-vendedor-web = "PAGOEFECTIVO".
    END.

   /* QUE PASA SI NO TIENE CENTRO DE COSTO */
    x-Cco = CcbCDocu.Cco.
    IF x-Cco = '' THEN DO:        /* Buscamos por el vendedor */
        FIND FIRST GN-VEN OF CcbCDocu NO-LOCK NO-ERROR.
    ...
    BUFFER-COPY ccbcdocu to t-cdoc
        ...
        */
        T-CDOC.b2c-web = x-vendedor-web.
    IF x-vendedor-web = 'NIUBIZ' THEN DO:
        ASSIGN t-cdoc.torigen-tarjeta = b2c-interfase.origen-tarjeta
                t-cdoc.ttipo-tarjeta = b2c-interfase.tipo_tarjeta.
    END.
    ELSE DO:
        ASSIGN t-cdoc.torigen-tarjeta = ""
                t-cdoc.ttipo-tarjeta = "pago_efectivo.".
    END.
...
/*  */
DEFINE VAR y-imptot AS DEC.
DEFINE VAR y-impcom AS DEC.
DEFINE VAR y-factor AS DEC.
DEFINE VAR y-importe AS DEC.
...
FOR EACH T-CDOC BREAK BY T-CDOC.CodDiv BY T-CDOC.Cco BY T-CDOC.NroDoc:
    IF FIRST-OF(T-CDOC.Cco) THEN ASSIGN
            x-cco     = T-CDOC.Cco
    ...
    y-imptot = T-CDOC.ImpTot.
    y-impcom = 0.
    y-factor = 0.

    /* Procedimientos */
    RUN COMISION-B2C-WEB IN hProc (INPUT  T-CDOC.b2c-web,
                                INPUT T-CDOC.ImpTot, 
                                INPUT torigen-tarjeta, 
                                INPUT ttipo-tarjeta, 
                                OUTPUT y-impcom /*x-comision-calculada*/,
                                OUTPUT y-factor /*x-factor-comision*/).

    FOR EACH vtadtabla WHERE vtadtabla.codcia = vtactabla.codcia AND
                                vtadtabla.tabla = vtactabla.tabla AND 
                                vtadtabla.llave = vtactabla.llave AND 
                                vtadtabla.tipo = T-CDOC.coddoc NO-LOCK:

        /* x-coddoc = FAC,BOL */
        /* x-tpodoc =  Debe, Haber */
        x-detalle = YES.
        x-tpodoc = YES.
        IF vtadtabla.libre_c01 = "HABER" THEN x-tpodoc = NO.
        x-codcta = vtadtabla.llavedetalle.      /* Soles */
        IF T-CDOC.codmon = 2 THEN x-codcta = vtadtabla.libre_c03.   /* Dolares */
        
        y-importe = y-imptot.
        IF vtadtabla.libre_c02 = "IMP-TOTAL-MENOS-COMI" THEN y-importe = y-imptot - y-impcom.
        IF vtadtabla.libre_c02 = "IMP-COMISION" THEN y-importe = y-impcom.

        RUN Graba-Documentos (x-codcbd, x-codcta, NOT x-tpodoc, y-importe /*T-CDOC.ImpTot*/, x-coddoc, 01, T-CDOC.codmon,x-detalle, x-cco).
...
DEFINE VAR z-documentos AS CHAR.
DEFINE VAR z-coddoc AS CHAR.
DEFINE VAR x-sec AS INT.

SESSION:SET-WAIT-STATE("GENERAL").

z-documentos = "FAC,BOL".

DO FILL-IN-FchAst = FILL-IN-fchast-1 TO FILL-IN-fchast-2:
    /* TIPO DE CAMBIO DEL DIA */
    FIND FIRST gn-tcmb WHERE gn-tcmb.fecha = FILL-IN-fchast NO-LOCK NO-ERROR.
    ...
    FOR EACH vtactabla WHERE vtactabla.codcia = s-codcia AND
                                ...
                                vtactabla.estado = 'A' NO-LOCK:

        x-codope = vtactabla.libre_c01.

        REPEAT x-sec = 1 TO NUM-ENTRIES(z-documentos,","):
            z-coddoc = ENTRY(x-sec,z-documentos,",").

            FIND FIRST facdocum WHERE facdocum.codcia = s-codcia AND
                                        facdocum.coddoc = z-coddoc NO-LOCK NO-ERROR.

            IF AVAILABLE facdocum THEN DO:

                RUN Carga-FAC-BOL (z-coddoc,FacDocum.CodCta[1],FacDocum.CodCta[2],FacDocum.Codcbd,FacDocum.TpoDoc).
            END.
...
IF x-codcta = "" THEN DO:
 ...
 FIND FIRST CB-CTAS WHERE CB-CTAS.codcia = cb-codcia AND CB-CTAS.codcta = x-codcta NO-LOCK NO-ERROR.
 IF AVAILABLE CB-CTAS THEN DO:
    x-Cco    = IF CB-CTAS.PidCco THEN x-Cco    ELSE ''.
    x-CodAux = IF CB-CTAS.PidAux THEN x-CodAux ELSE ''.
 END.    
        
 FIND FIRST t-prev WHERE t-prev.coddiv = T-CDOC.Coddiv AND
