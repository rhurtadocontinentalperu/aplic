...
DEFINE VAR X-TITU    AS CHAR INIT "ESTADISTICA   DE    VENTAS  X   CLIENTES   EXCLUSIVOS   (TARJETAS)".
...
DEFINE TEMP-TABLE tmp-tempo 
    ...
    FIELD t-codcli  LIKE evtcard.Codcli
...
&Scoped-Define ENABLED-OBJECTS RECT-66 RECT-63 RECT-61 RECT-65 w-tiprep ~
F-DIVISION BUTTON-5 DesdeF HastaF nCodMon Btn_OK Btn_Cancel Btn_Help 
&Scoped-Define DISPLAYED-OBJECTS w-tiprep F-DIVISION DesdeF HastaF nCodMon ~
txt-msj 
...
DEFINE VARIABLE w-tiprep AS INTEGER 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Venta", 1,
"Nombre", 2
     SIZE 11.14 BY 2.23 NO-UNDO.
...
DEFINE FRAME F-Main
     w-tiprep AT ROW 2.54 COL 36.86 NO-LABEL
     F-DIVISION AT ROW 2.58 COL 9 COLON-ALIGNED
     BUTTON-5 AT ROW 2.58 COL 22.72
     DesdeF AT ROW 5 COL 11.57 COLON-ALIGNED
     HastaF AT ROW 6.23 COL 11.57 COLON-ALIGNED
     nCodMon AT ROW 7.15 COL 33.72 NO-LABEL
     Btn_OK AT ROW 8.54 COL 20.86
     Btn_Cancel AT ROW 8.54 COL 32.14
     Btn_Help AT ROW 8.54 COL 43.57
     txt-msj AT ROW 8.88 COL 2.14 NO-LABEL WIDGET-ID 2
     " Ordenado por:" VIEW-AS TEXT
          SIZE 12.43 BY .5 AT ROW 1.96 COL 34.57
          FONT 6
     " Criterio de Selección" VIEW-AS TEXT
          SIZE 18.43 BY .5 AT ROW 1.35 COL 4.43
          FONT 6
     " Moneda" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 6.62 COL 32.86
          FONT 6
     "Fechas" VIEW-AS TEXT
          SIZE 9.43 BY .5 AT ROW 4.27 COL 5.57
          FONT 0
     RECT-66 AT ROW 2.19 COL 34
     RECT-46 AT ROW 8.38 COL 1.86
     RECT-63 AT ROW 6.73 COL 31
     RECT-61 AT ROW 1.54 COL 1.86
     RECT-65 AT ROW 4.42 COL 4.72
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Ventas Totales X Clientes Exclusivos"
         HEIGHT             = 9.27
         WIDTH              = 55
         MAX-HEIGHT         = 9.27
         MAX-WIDTH          = 55
...
ON END-ERROR OF W-Win /* Ventas Totales X Clientes Exclusivos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Ventas Totales X Clientes Exclusivos */
DO:
...
FOR EACH evtcard NO-LOCK WHERE evtcard.CodCia = S-CODCIA
    AND   evtcard.CodDiv = Gn-Divi.Coddiv
    AND   (evtcard.Nrofch >= INTEGER(STRING(YEAR(DesdeF),"9999") + STRING(MONTH(DesdeF),"99"))
    AND   evtcard.Nrofch <= INTEGER(STRING(YEAR(HastaF),"9999") + STRING(MONTH(HastaF),"99")) ):

    /*
    DISPLAY evtcard.Codcli @ Fi-Mensaje LABEL "Cliente"
    ...
    */
    ...
    IF evtcard.Codmes < 12 THEN DO:
      ASSIGN
      X-CODMES = evtcard.Codmes + 1
      X-CODANO = evtcard.Codano .
    END.
    ELSE DO: 
      ASSIGN
      X-CODMES = 01
      X-CODANO = evtcard.Codano + 1 .
    END.
    /**********************************************************************/
    ...
    /*********************** Calculo Para Obtener los datos diarios ************/
     DO I = 1 TO DAY( DATE(STRING(X-CODDIA,"99") + "/" + STRING(X-CODMES,"99") + "/" + STRING(X-CODANO,"9999")) - 1 ) :        
          ...
          X-FECHA = DATE(STRING(I,"99") + "/" + STRING(evtcard.Codmes,"99") + "/" + STRING(evtcard.Codano,"9999")).
          IF X-FECHA >= DesdeF AND X-FECHA <= HastaF THEN DO:
              FIND Gn-tcmb WHERE Gn-tcmb.Fecha = DATE(STRING(I,"99") + "/" + STRING(evtcard.Codmes,"99") + "/" + STRING(evtcard.Codano,"9999")) NO-LOCK NO-ERROR.
              IF AVAILABLE Gn-tcmb THEN DO: 
               T-Vtamn   = T-Vtamn   + evtcard.Vtaxdiamn[I] + evtcard.Vtaxdiame[I] * Gn-Tcmb.Venta.
               T-Vtame   = T-Vtame   + evtcard.Vtaxdiame[I] + evtcard.Vtaxdiamn[I] / Gn-Tcmb.Compra.
              END.
          END.
     END.         
    ...
    IF NOT AVAIL tmp-tempo THEN DO:
      ...
      FIND gn-clie WHERE gn-clie.CodCia = cl-codcia AND 
                         gn-clie.Codcli = evtcard.Codcli  NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  DISPLAY w-tiprep F-DIVISION DesdeF HastaF nCodMon txt-msj 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-66 RECT-63 RECT-61 RECT-65 w-tiprep F-DIVISION BUTTON-5 DesdeF 
...
------------------------------------------------------------------------------*/
    ...
    DO c-Copias = 1 TO s-nro-copias ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
        ...
        END CASE.
        IF w-tiprep = 1 THEN RUN reporte.
        ELSE RUN reporte2.
        PAGE STREAM REPORT.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-HEADER
         ...
         {&PRN2} + {&PRN6A} + X-TITU + {&PRN6B} + {&PRN3} AT 40 FORMAT "X(70)" 
         {&PRN3} + {&PRN6A} + "(" + C-TIPO + ")" + {&PRN6B} + {&PRN3} AT 130 FORMAT "X(25)" SKIP(1)
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE reporte2 W-Win 
PROCEDURE reporte2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
