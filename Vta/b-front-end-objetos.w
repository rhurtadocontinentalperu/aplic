...
gn-divi.coddiv = vtatabla.llave_c2 NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c2 COLUMN-LABEL "Division" FORMAT "x(5)":U
            WIDTH 6.72
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 60.72 BY 12.69
         FONT 4.
...
/* Query rebuild information for BROWSE br_table
     ...
     _TblOptList       = ", FIRST"
     _Where[1]         = "{&Condicion}"
     _JoinCode[2]      = "gn-divi.codcia = s-codcia and
gn-divi.coddiv = vtatabla.llave_c2"
     _FldNameList[1]   > INTEGRAL.VtaTabla.Llave_c2
"VtaTabla.Llave_c2" "Division" "x(5)" "character" ? ? ? ? ? ? yes ? no no "6.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.GN-DIVI.DesDiv
     _FldNameList[3]   > INTEGRAL.VtaTabla.Rango_fecha[1]
"VtaTabla.Rango_fecha[1]" "Desde" ? "date" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.VtaTabla.Rango_fecha[2]
"VtaTabla.Rango_fecha[2]" "Hasta" ? "date" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF VtaTabla.Llave_c2 IN BROWSE br_table /* Division */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL VtaTabla.Llave_c2 br_table _BROWSE-COLUMN B-table-Win
ON MOUSE-SELECT-DBLCLICK OF VtaTabla.Llave_c2 IN BROWSE br_table /* Division */
DO:
    ASSIGN
    input-var-1 = ''
    input-var-2 = ''
...
------------------------------------------------------------------------------*/
    ...
    IF RETURN-VALUE = 'YES' THEN DO:
        DO WITH FRAME {&FRAME-NAME} :
            ASSIGN vtatabla.llave_c2:SCREEN-VALUE IN BROWSE {&browse-name} = '00000'.
        END.        
    END.
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    FIND FIRST b-gn-divi WHERE b-gn-divi.codcia = s-codcia AND
                                b-gn-divi.coddiv = vtatabla.llave_c2:SCREEN-VALUE NO-LOCK NO-ERROR.
    IF NOT AVAILABLE b-gn-divi THEN DO:
        MESSAGE "Codigo de division no existe"
            VIEW-AS ALERT-BOX INFORMATION.
        RETURN "ADM-ERROR".
    END.

    IF NOT (b-gn-divi.campo-char[1] = 'D' OR b-gn-divi.campo-char[1] = 'A') THEN DO:
        MESSAGE "Codigo debe pertenecer a DIVISION y no a lista de precios"
            VIEW-AS ALERT-BOX INFORMATION.
        RETURN "ADM-ERROR".
    END.
    ...
    FIND FIRST b-vtatabla WHERE b-vtatabla.codcia = s-codcia AND
                                ...
                                b-vtatabla.llave_c1 = x-ventana AND
