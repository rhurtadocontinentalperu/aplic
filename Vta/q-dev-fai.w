...
&Scoped-define QUERY-STRING-Query-Main FOR EACH Almcmov WHERE ~{&KEY-PHRASE} ~
      AND Almcmov.CodCia = S-CODCIA ~
 AND Almcmov.CodAlm = S-CODALM ~
 AND Almcmov.TipMov = "I" ~
 AND Almcmov.CodMov = S-CODMOV ~
 AND Almcmov.NroSer = 0 NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Almcmov WHERE ~{&KEY-PHRASE} ~
      AND Almcmov.CodCia = S-CODCIA ~
 AND Almcmov.CodAlm = S-CODALM ~
 AND Almcmov.TipMov = "I" ~
 AND Almcmov.CodMov = S-CODMOV ~
...
DEFINE VARIABLE F-Numero AS INTEGER FORMAT "999999999":U INITIAL 0 
     VIEW-AS FILL-IN 
...
/* Query rebuild information for QUERY Query-Main
 ...
 AND Almcmov.NroSer = 0"
     _Design-Parent    is WINDOW q-tables @ ( 1.23 , 16.72 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN  input-var-1 = S-CODALM
          input-var-2 = "I"
          input-var-3 = STRING(S-CODMOV)
          output-var-1 = ?
          OK-WAIT-STATE = SESSION:SET-WAIT-STATE("GENERAL").

  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:

    RUN LKUP\C-MovAlm ("Devoluciones").
...
FIND AlmCMov WHERE AlmCMov.CodCia = S-CODCIA AND
    ...
    AlmCMov.NroSer = 000      AND
    Almcmov.NroDoc = F-Numero
    NO-LOCK NO-ERROR.
