...
DEF VAR s-Tabla LIKE VtaTabla.Tabla NO-UNDO.
...
VtaTabla.Llave_c2 VtaTabla.LLave_c3 
...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaTabla WHERE ~{&KEY-PHRASE} ~
 ...
 AND VtaTabla.Tabla = s-Tabla NO-LOCK, ~
      EACH gn-clie WHERE gn-clie.CodCli = VtaTabla.Llave_c1 ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      VtaTabla.Llave_c2 COLUMN-LABEL "Productos Propios" FORMAT "x(2)":U
            VIEW-AS COMBO-BOX INNER-LINES 5
                      LIST-ITEMS "Si","No" 
                      DROP-DOWN-LIST 
      VtaTabla.LLave_c3 COLUMN-LABEL "Productos de Terceros" FORMAT "x(2)":U
            VIEW-AS COMBO-BOX INNER-LINES 5
                      LIST-ITEMS "Si","No" 
                      DROP-DOWN-LIST 
  ENABLE
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 84 BY 15.35
         FONT 4.
...
/* Query rebuild information for BROWSE br_table
 ...
 AND INTEGRAL.VtaTabla.Tabla = s-Tabla"
     _JoinCode[2]      = "gn-clie.CodCli = VtaTabla.Llave_c1"
     _Where[2]         = "gn-clie.CodCia = cl-codcia"
     _FldNameList[1]   > INTEGRAL.VtaTabla.Llave_c1
"VtaTabla.Llave_c1" "Cliente" "x(11)" "character" ? ? ? ? ? ? yes ? no no "12.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.gn-clie.NomCli
     _FldNameList[3]   > INTEGRAL.VtaTabla.Llave_c2
"VtaTabla.Llave_c2" "Productos Propios" "x(2)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," "Si,No" ? 5 no 0 no no
     _FldNameList[4]   > INTEGRAL.VtaTabla.LLave_c3
"VtaTabla.LLave_c3" "Productos de Terceros" "x(2)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," "Si,No" ? 5 no 0 no no
...
DO:
  ...
  IF RETURN-VALUE = 'YES' THEN
      DISPLAY
          'Si' @ VtaTabla.Llave_c2
          'Si' @ VtaTabla.Llave_c3
...
IF RETURN-VALUE = 'YES' THEN DO:
    FIND gn-clie WHERE gn-clie.codcia = cl-codcia
        AND gn-clie.codcli = VtaTabla.Llave_c1:SCREEN-VALUE IN BROWSE {&browse-name}
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE gn-clie THEN DO:
        ...
        RETURN 'ADM-ERROR'.
    END.
    IF CAN-FIND( Vtatabla WHERE Vtatabla.codcia = s-codcia
                 AND VTatabla.tabla = s-tabla
                 AND Vtatabla.llave_c1 =  VtaTabla.Llave_c1:SCREEN-VALUE IN BROWSE {&browse-name}
                 NO-LOCK)
        THEN DO:
