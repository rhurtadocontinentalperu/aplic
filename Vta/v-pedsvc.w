...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    /* 27/12/2024 J.Hermoza validar el tope mínimo de detracción */
    DEF VAR x-Minimo        AS DECI INIT 0 NO-UNDO.
    DEF VAR x-Codigo        AS CHAR INIT "" NO-UNDO.
    DEF VAR x-Valor         AS DECI INIT 0 NO-UNDO.

    FOR EACH PEDI NO-LOCK:
        FIND Almmserv WHERE almmserv.CodCia = s-codcia AND 
            almmserv.codmat = PEDI.codmat NO-LOCK NO-ERROR.
        IF AVAILABLE Almmserv AND Almmserv.codfam > '' THEN DO:
            FIND Factabla WHERE Factabla.codcia = s-codcia AND
                Factabla.tabla = 'SUNAT_CFG_DETRACCION' AND
                Factabla.codigo = Almmserv.codfam
                NO-LOCK NO-ERROR.
            IF AVAILABLE Factabla AND Factabla.valor[1] > 0 AND Factabla.valor[2] > 0 
                THEN DO:
                /* Control */
                IF x-Minimo = 0 THEN DO:
                    x-Minimo = Factabla.valor[2].
                    x-Codigo = FacTabla.codigo.
                    x-Valor  = Factabla.Valor[1].
                END.
                ELSE DO:
                    IF (x-Minimo <> Factabla.valor[2] AND
                        x-Valor <> Factabla.Valor[1] AND
                        x-Codigo <> FacTabla.codigo) THEN DO:
                        MESSAGE 'No se puede ofrecer servicios con diferente importe mínimo, % de aplicación y código de SUNAT' SKIP(1)
                            'Base código detracción SUNAT:' x-Codigo SKIP(2)
                            'Grabación abortada' VIEW-AS ALERT-BOX WARNING.
                        RETURN 'ADM-ERROR'.
                    END.
                END.
            END.
