...
DEFINE VARIABLE h_b-docmto-3-1 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-faccre-02-1 AS HANDLE NO-UNDO.
...
DO:
  RUN Asigna-Guias IN h_v-faccre-02-1.
END.
...
DO:
    RUN Asigna-Pedidos IN h_v-faccre-02-1.
END.
...
DO:
  RUN dispatch IN h_v-faccre-02-1 ('imprime':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-faccre-02-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-faccre-02-1 ).
       RUN set-position IN h_v-faccre-02-1 ( 1.00 , 1.72 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-faccre-02-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-faccre-02-1 ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto-2 , 'Record':U , h_v-faccre-02-1 ).

       /* Links to SmartQuery h_q-docmto-2. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-docmto-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-faccre-02-1 ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-docmto-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-docmto-3-1 ).
       RUN set-position IN h_b-docmto-3-1 ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-docmto-3-1 ( 9.42 , 108.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-docmto-3-1. */
       RUN add-link IN adm-broker-hdl ( h_q-docmto-2 , 'Record':U , h_b-docmto-3-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-docmto-3-1 ,
             Btn-Print:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-tmpdoc-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpdoc-3-1 ).
       RUN set-position IN h_b-tmpdoc-3-1 ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpdoc-3-1 ( 9.42 , 105.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  END.
  IF s-user-id BEGINS 'jcvp' OR
      s-user-id BEGINS 'admin' THEN L-NROSER = L-NROSER + "," + '056'.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-docmto-3-1 ('open-query':U).
