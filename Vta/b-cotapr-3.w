...
FacCPedi.Libre_c05 FacCPedi.CodVen FacCPedi.FchPed FacCPedi.fchven ~
X-VTA @ X-VTA X-MON @ X-MON FacCPedi.ImpTot 
...
&Scoped-define QUERY-STRING-br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
 ...
 AND lookup(integral.FacCPedi.FlgEst,f-estado,'|') > 0  NO-LOCK, ~
...
&Scoped-Define ENABLED-OBJECTS COMBO-BOX-3 F-Estado wclient wcotiza ~
br_table 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-3 F-Estado wclient wcotiza 
...
DEFINE VARIABLE F-Estado AS CHARACTER INITIAL "X" 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.Libre_c05 COLUMN-LABEL "Motivo" FORMAT "x(30)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 105 BY 8.54
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     F-Estado AT ROW 1.27 COL 71 NO-LABEL WIDGET-ID 12
...
/* Query rebuild information for BROWSE br_table
 ...
 AND lookup(integral.FacCPedi.FlgEst,f-estado,'|') > 0 "
...
"FacCPedi.NomCli" ? "x(40)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.FacCPedi.Libre_c05
"FacCPedi.Libre_c05" "Motivo" "x(30)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > integral.FacCPedi.CodVen
...
DO:
    RUN vta\d-pedapr-3 (Faccpedi.nroped,"PED").
...
ON VALUE-CHANGED OF F-Estado IN FRAME F-Main
DO:
  ASSIGN {&self-name}.
  RUN abrir-query.
END.
