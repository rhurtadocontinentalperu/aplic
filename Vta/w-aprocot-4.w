...
DEFINE VARIABLE h_b-aprobcot-4 AS HANDLE NO-UNDO.
...
DO:
    ...
    IF choice THEN RUN Aprobar-Cotizaciones IN h_b-aprobcot-4.
...
DO:
    MESSAGE 'Esta Seguro de Rechazar Cotización'
        VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
        TITLE '' UPDATE choice AS LOGICAL.
    IF choice THEN RUN Rechaza-Cotizacion IN h_b-aprobcot-4.
    ELSE RETURN 'ADM-ERROR'.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-aprobcot-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-aprobcot-4 ).
       RUN set-position IN h_b-aprobcot-4 ( 1.15 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-aprobcot-4 ( 9.42 , 83.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-aprobcot-4 , 'Record':U , h_v-aprob ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-aprobcot-4 ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_b-aprobcot-4 , 'Record':U , h_b-cotdet-4 ).
