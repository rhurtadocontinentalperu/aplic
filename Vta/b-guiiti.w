...
DEF VAR s-tpofac AS CHAR INIT 'R'.
...
&Scoped-Define ENABLED-OBJECTS x-CodDiv x-FChDoc-1 x-FChDoc-2 x-NroSer ~
br_table 
&Scoped-Define DISPLAYED-OBJECTS x-CodDiv x-FChDoc-1 x-FChDoc-2 x-NroSer 
...
DEFINE FRAME F-Main
     x-CodDiv AT ROW 1.19 COL 6 COLON-ALIGNED
     x-FChDoc-1 AT ROW 2.15 COL 6 COLON-ALIGNED
     x-FChDoc-2 AT ROW 2.15 COL 25 COLON-ALIGNED
     x-NroSer AT ROW 3.12 COL 6 COLON-ALIGNED
...
/* BROWSE-TAB br_table x-NroSer F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br_table
 ...
 AND INTEGRAL.CcbCDocu.CodDiv = SUBSTRING(x-coddiv,1,5)
 AND INTEGRAL.CcbCDocu.TpoFac = s-tpofac
 AND INTEGRAL.CcbCDocu.NroDoc BEGINS x-nroser
 AND INTEGRAL.CcbCDocu.FchDoc >= x-fchdoc-1
 AND INTEGRAL.CcbCDocu.FchDoc <= x-fchdoc-2"
...
&Scoped-define SELF-NAME x-NroSer
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL x-NroSer B-table-Win
ON LEAVE OF x-NroSer IN FRAME F-Main /* Serie Nº */
DO:
  ASSIGN {&SELF-NAME}.
  RUN dispatch IN THIS-PROCEDURE ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR x-Orden AS INT INIT 0 NO-UNDO.
  
  DO WITH FRAME {&FRAME-NAME}:
    x-NroSer = STRING(s-nroser, '999').
    x-FchDoc-1 = TODAY - DAY(TODAY) + 1.
    x-FchDoc-2 = TODAY.
    FOR EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-codcia:
        x-Cuenta = x-Cuenta + 1.
        IF gn-divi.coddiv = s-coddiv THEN x-orden = x-cuenta.
        x-CodDiv:ADD-LAST(TRIM(gn-divi.coddiv) + ' ' + gn-divi.desdiv).
    END.
    x-CodDiv = x-CodDiv:ENTRY(x-Orden).
  END.
