...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 95 BY 10.5
         FONT 2.
...
DO:
  ...
  FIND T-DDOCU WHERE T-DDOCU.codmat = SELF:SCREEN-VALUE 
    NO-LOCK NO-ERROR.
...
FOR EACH Ccbddocu NO-LOCK WHERE Ccbddocu.codcia = s-codcia
    AND Ccbddocu.coddoc = s-coddoc
    AND Ccbddocu.nrodoc = x-nrodoc:
/*     FIND Almckits OF Ccbddocu NO-LOCK NO-ERROR. */
/*     IF NOT AVAILABLE Almckits THEN DO:          */
        FIND T-DDOCU WHERE T-DDOCU.codmat = Ccbddocu.codmat NO-ERROR.
        IF NOT AVAILABLE T-DDOCU THEN DO:
            CREATE T-DDOCU.
            ASSIGN
                T-DDOCU.codmat = Ccbddocu.codmat
                T-DDOCU.candes = Ccbddocu.candes
                T-DDOCU.undvta = Ccbddocu.undvta.
        END.
        ELSE T-DDOCU.candes = T-DDOCU.candes + Ccbddocu.candes.
/*     END.                                                                                                  */
/*     ELSE DO:                                                                                              */
...
/*             FIND T-DDOCU WHERE T-DDOCU.codmat = Almdkits.codmat2 NO-ERROR.                                */
/*             IF NOT AVAILABLE T-DDOCU THEN DO:                                                             */
/*                 CREATE T-DDOCU.                                                                           */
/*                 ASSIGN                                                                                    */
/*                     T-DDOCU.codmat = AlmDKits.codmat2                                                     */
/*                     T-DDOCU.candes = AlmDKits.Cantidad * Ccbddocu.candes * Ccbddocu.factor                */
/*                     T-DDOCU.undvta = ALmmmatg.undbas.                                                     */
/*             END.                                                                                          */
/*             ELSE T-DDOCU.candes = T-DDOCU.candes + AlmDKits.Cantidad * Ccbddocu.candes * Ccbddocu.factor. */
/*         END.                                                                                              */
/*     END.                                                                                                  */
END.
...
DEF VAR x-UsrChq LIKE FacCPedi.UsrChq NO-UNDO.
  ...
  FOR EACH T-DDOCU:
    FIND C-Detalle WHERE C-Detalle.codmat = T-DDOCU.codmat
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE C-Detalle THEN DO:
        MESSAGE 'No se ha registrado el artículo' T-DDOCU.codmat
            VIEW-AS ALERT-BOX ERROR.
        RETURN 'ADM-ERROR'.
    END.
    IF C-Detalle.CanDes <> T-DDOCU.CanDes THEN DO:
        MESSAGE 'Diferencia de cantidades en el artículo' T-DDOCU.codmat
            VIEW-AS ALERT-BOX ERROR.
        RETURN 'ADM-ERROR'.
    END.
  END.        
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    THEN DO:
        ...
        ASSIGN 
            CcbCDocu.Libre_c05 = x-UsrChq
            CcbCDocu.Libre_f02 = TODAY
            Ccbcdocu.flgest = 'P'.
        /* RHC 28.06.10 Caso de guias manuales */
        IF Ccbcdocu.TpoFac = 'M' THEN Ccbcdocu.FlgEst = 'F'.
        /* *********************************** */
        /* TRACKING */
...
------------------------------------------------------------------------------*/
  ...
  END.
  FIND T-DDOCU WHERE T-DDOCU.codmat = Detalle.codmat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
    NO-LOCK NO-ERROR.
  IF AVAILABLE T-DDOCU AND x-CanPed > T-DDOCU.candes THEN DO:
