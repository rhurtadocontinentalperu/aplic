...
&Scoped-Define ENABLED-OBJECTS br_table COMBO-BOX-division 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c2 COLUMN-LABEL "Cod!Doc" FORMAT "x(5)":U
      VtaTabla.Rango_fecha[1] COLUMN-LABEL "Desde" FORMAT "99/99/9999":U
      VtaTabla.Rango_fecha[2] COLUMN-LABEL "Hasta" FORMAT "99/99/9999":U
      VtaTabla.Valor[1] COLUMN-LABEL "Importe!Minimo" FORMAT "->>>,>>>,>>9.999999":U
      VtaTabla.Libre_c01 COLUMN-LABEL "Activo" FORMAT "x(8)":U
  ENABLE
      VtaTabla.Llave_c2
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 72.72 BY 12.69
         FONT 4.
...
DEFINE FRAME F-Main
     br_table AT ROW 2.54 COL 1.29
     COMBO-BOX-division AT ROW 1.38 COL 20 COLON-ALIGNED WIDGET-ID 2
...
"Llave_c2" "Cod!Doc" "x(5)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.VtaTabla.Rango_fecha[1]
"Rango_fecha[1]" "Desde" ? "date" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaTabla.Rango_fecha[2]
"Rango_fecha[2]" "Hasta" ? "date" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.VtaTabla.Valor[1]
"Valor[1]" "Importe!Minimo" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF VtaTabla.Llave_c2 IN BROWSE br_table /* Cod!Doc */
DO:
  SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} = CAPS(SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
END.
...
ON VALUE-CHANGED OF COMBO-BOX-division IN FRAME F-Main /* Seleccione el Evento */
DO:
  x-division = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
    ...
    IF RETURN-VALUE = 'YES' THEN DO:
        DO WITH FRAME {&FRAME-NAME} :
            ASSIGN vtatabla.llave_c2:SCREEN-VALUE IN BROWSE {&browse-name} = 'COT'.
        END.        
    END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
    ...
    logtabla.ValorLlave = x-tabla + "|" + vtatabla.llave_c1 + "|" + vtatabla.llave_c2 + "|" + 
                            STRING(vtatabla.rango_fecha[1], '99/99/9999') + '|' +
                            STRING(vtatabla.rango_fecha[2], '99/99/9999') + '|' + 
                            STRING(vtatabla.valor[1],"->>,>>>,>>9,99")
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR x-valorllave AS CHAR.

  x-valorllave = x-tabla + "|" + vtatabla.llave_c1 + "|" + vtatabla.llave_c2 + "|" + 
                          STRING(vtatabla.rango_fecha[1], '99/99/9999') + '|' +
                          STRING(vtatabla.rango_fecha[2], '99/99/9999') + '|' + 
                          STRING(vtatabla.valor[1],"->>,>>>,>>9,99").
...
ASSIGN
  ...
  logtabla.Tabla = 'vtatabla'
  logtabla.Usuario =  USERID("DICTDB")
  logtabla.ValorLlave = x-valorllave
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
          ...
          /* En el INITIALIZE */
  COMBO-BOX-division:DELETE(COMBO-BOX-division:NUM-ITEMS) IN FRAME {&FRAME-NAME}.
	...
	DO WITH FRAME {&FRAME-NAME} :
        ...
        FOR EACH gn-divi WHERE gn-divi.codcia = s-codcia AND 
            ...
            COMBO-BOX-division:ADD-LAST(TRIM(gn-divi.desdiv) + " ( " + TRIM(gn-divi.coddiv) + ")", gn-divi.coddiv).
            IF TRUE <> (COMBO-BOX-division:SCREEN-VALUE > "") THEN DO:                    
                    ASSIGN COMBO-BOX-division:SCREEN-VALUE = gn-divi.coddiv.
            END.
                                
        END.

		APPLY 'VALUE-CHANGED':U TO COMBO-BOX-division.
  	END.
...
DO WITH FRAME {&FRAME-NAME}:
    x-desde = DATE(vtatabla.rango_fecha[1]:SCREEN-VALUE IN BROWSE {&browse-name}).
    x-hasta = DATE(vtatabla.rango_fecha[2]:SCREEN-VALUE IN BROWSE {&browse-name}).

    IF x-desde = ? OR x-hasta = ? THEN DO:
        MESSAGE "Fechas estan mal registradas"
    ...
    IF x-desde > x-hasta THEN DO:
        ...
        RETURN "ADM-ERROR".
    END.
    IF DEC(vtatabla.valor[1]:SCREEN-VALUE) < 0 THEN DO:
    ...
    FIND FIRST facdocum WHERE facdocum.codcia = s-codcia AND
                                facdocum.coddoc = vtatabla.llave_c2:SCREEN-VALUE NO-LOCK NO-ERROR.
    IF NOT AVAILABLE facdocum THEN DO:
        MESSAGE "Codigo de documento no existe"
            VIEW-AS ALERT-BOX INFORMATION.
        RETURN "ADM-ERROR".
    END.

    FIND FIRST b-vtatabla WHERE b-vtatabla.codcia = s-codcia AND
                                b-vtatabla.tabla = x-tabla AND
                                b-vtatabla.llave_c1 = x-division AND
                                b-vtatabla.llave_c2 = vtatabla.llave_c2:SCREEN-VALUE AND
                                b-vtatabla.rango_fecha[1] = x-desde AND
                                b-vtatabla.rango_fecha[2] = x-hasta
                                NO-LOCK NO-ERROR.
