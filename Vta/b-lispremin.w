...
&SCOPED-DEFINE FILTRO1 (Almmmatg.DesMat = F-Filtro) 
...
Almmmatg.CtoTot Almmmatg.MrgAlt[1] Almmmatg.UndAlt[1] Almmmatg.PreAlt[1] ~
Almmmatg.Dec__01 Almmmatg.PreOfi Almmmatg.Chr__01 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table Almmmatg.PreAlt[1] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almmmatg.CtoLis COLUMN-LABEL "Costo!S/IGV" FORMAT "->,>>9.9999"
      Almmmatg.CtoTot COLUMN-LABEL "Costo!Lista Total" FORMAT "->,>>9.9999"
      Almmmatg.MrgAlt[1] COLUMN-LABEL "% Margen!Utilidad" COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 14
      Almmmatg.UndAlt[1] COLUMN-LABEL "U.M." FORMAT "x(6)" COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 14
      Almmmatg.PreAlt[1] COLUMN-LABEL "Precio!x Menor" COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 14
      Almmmatg.Dec__01 COLUMN-LABEL "%Marg Utilid!Pre.Ofi." FORMAT "->>9.99"
            COLUMN-FGCOLOR 15 COLUMN-BGCOLOR 12
      Almmmatg.PreOfi FORMAT ">>>,>>9.9999" COLUMN-FGCOLOR 15 COLUMN-BGCOLOR 12
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 112.86 BY 10.58
         FONT 4.
...
"Almmmatg.DesMar" ? "X(14)" "character" ? ? ? ? ? ? no ?
     _FldNameList[4]   > integral.Almmmatg.clase
"Almmmatg.clase" "Com.!Dif." "!" "character" ? ? ? ? ? ? no ?
...
"Almmmatg.PorMax" "%Max!Dscto" ">9.99" "decimal" ? ? ? ? ? ? no ?
     _FldNameList[9]   > integral.Almmmatg.UndBas
"Almmmatg.UndBas" "Und!Base" "X(6)" "character" ? ? ? ? ? ? no ?
     _FldNameList[10]   > integral.Almmmatg.CtoLis
"Almmmatg.CtoLis" "Costo!S/IGV" "->,>>9.9999" "decimal" ? ? ? ? ? ? no ?
     _FldNameList[11]   > integral.Almmmatg.CtoTot
"Almmmatg.CtoTot" "Costo!Lista Total" "->,>>9.9999" "decimal" ? ? ? ? ? ? no ?
     _FldNameList[12]   > integral.Almmmatg.MrgAlt[1]
"Almmmatg.MrgAlt[1]" "% Margen!Utilidad" ? "decimal" 14 0 ? ? ? ? no ?
     _FldNameList[13]   > integral.Almmmatg.UndAlt[1]
"Almmmatg.UndAlt[1]" "U.M." "x(6)" "character" 14 0 ? ? ? ? no ?
     _FldNameList[14]   > integral.Almmmatg.PreAlt[1]
"Almmmatg.PreAlt[1]" "Precio!x Menor" ? "decimal" 14 0 ? ? ? ? yes ?
     _FldNameList[15]   > integral.Almmmatg.Dec__01
"Almmmatg.Dec__01" "%Marg Utilid!Pre.Ofi." "->>9.99" "decimal" 12 15 ? ? ? ? no ?
     _FldNameList[16]   > integral.Almmmatg.PreOfi
"Almmmatg.PreOfi" ? ">>>,>>9.9999" "decimal" 12 15 ? ? ? ? no ?
     _FldNameList[17]   > integral.Almmmatg.Chr__01
"Almmmatg.Chr__01" "UM.!Pre.Ofi." "X(6)" "character" 12 15 ? ? ? ? no ?
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Almmmatg.UndAlt[1] br_table _BROWSE-COLUMN B-table-Win
ON LEAVE OF Almmmatg.UndAlt[1] IN BROWSE br_table /* U.M. */
DO:
...
&Scoped-define SELF-NAME Almmmatg.PreAlt[1]
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Almmmatg.PreAlt[1] br_table _BROWSE-COLUMN B-table-Win
ON LEAVE OF Almmmatg.PreAlt[1] IN BROWSE br_table /* Precio!x Menor */
DO:
   ASSIGN
       F-MrgUti-A = DECIMAL(Almmmatg.MrgAlt[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME})
       F-PreVta-A = DECIMAL(SELF:SCREEN-VALUE).
    F-FACTOR = 1.
    /****   Busca el Factor de conversion   ****/
    IF UndAlt[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} <> "" 
    THEN DO:
        FIND Almtconv WHERE Almtconv.CodUnid = Almmmatg.UndBas
            AND  Almtconv.Codalter = UndAlt[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} 
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE Almtconv THEN DO:
           MESSAGE "Codigo de unidad no existe" VIEW-AS ALERT-BOX ERROR.
           RETURN NO-APPLY.
    ...
    DISPLAY F-MrgUti-A @ Almmmatg.MrgAlt[1]
...
DO:
  IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
     RUN Vta/D-Dtopromin ({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codmat).
     RUN dispatch IN THIS-PROCEDURE ('display-fields':U).
END.
...
ON "RETURN":U OF Almmmatg.PreAlt[1],Almmmatg.MrgAlt[1]
DO:
   APPLY "TAB":U.
   RETURN NO-APPLY.
END.
