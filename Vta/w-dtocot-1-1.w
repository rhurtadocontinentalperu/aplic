...
DEFINE VARIABLE h_b-dtocot AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-tdtoCo-1 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     B-MARGEN AT ROW 4.62 COL 75.43
     BUTTON-1 AT ROW 4.62 COL 85.29
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 128.29 BY 15.81
         FONT 4.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Descuentos de Cotizaciones"
         HEIGHT             = 15.81
         WIDTH              = 128.29
         MAX-HEIGHT         = 15.81
         MAX-WIDTH          = 146.72
         VIRTUAL-HEIGHT     = 15.81
         VIRTUAL-WIDTH      = 146.72
         RESIZE             = no
         SCROLL-BARS        = no
         STATUS-AREA        = no
         BGCOLOR            = ?
...
ON END-ERROR OF W-Win /* Descuentos de Cotizaciones */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Descuentos de Cotizaciones */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-dtocot-1-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-dtocot-1 ).
       RUN set-position IN h_v-dtocot-1 ( 1.00 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 4.54 , 11.57 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv97 ).
       RUN set-position IN h_p-updv97 ( 4.58 , 38.72 ) NO-ERROR.
       RUN set-size IN h_p-updv97 ( 1.35 , 35.86 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta/q-dtoped.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-dtoped ).
       RUN set-position IN h_q-dtoped ( 4.69 , 2.29 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-dtocot-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-dtocot ).
       RUN set-position IN h_b-dtocot ( 6.12 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-dtocot ( 10.50 , 127.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-dtocot. */
       RUN add-link IN adm-broker-hdl ( h_q-dtoped , 'Record':U , h_b-dtocot ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dtocot ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-tdtoco-1-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tdtoCo-1 ).
       RUN set-position IN h_b-tdtoCo-1 ( 6.12 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tdtoCo-1 ( 8.65 , 126.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv96 ).
       RUN set-position IN h_p-updv96 ( 14.73 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv96 ( 1.35 , 34.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-tdtoCo-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv96 , 'TableIO':U , h_b-tdtoCo-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tdtoCo-1 ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv96 ,
             h_b-tdtoCo-1 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-dtocot ('open-query':U).
          RUN dispatch IN h_b-tdtoco-1 ('open-query':U).
      END.
