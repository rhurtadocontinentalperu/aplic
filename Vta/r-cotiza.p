...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-od W-Win _STRUCTURED
  QUERY BROWSE-od NO-LOCK DISPLAY
      FacCPedi.NroPed COLUMN-LABEL "Cotización" FORMAT "X(11)":U
...
"NroPed" "Cotización" "X(11)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FILL-IN-CodAlm IN FRAME F-Main /* Almacén */
DO:
    ASSIGN FILL-IN-CodAlm.
    ...
    FOR almacen FIELDS
        ...
        almacen.codalm = FILL-IN-codalm NO-LOCK:
    END.
    IF NOT AVAILABLE almacen THEN FILL-IN-desalm = "ALMACÉN NO VÁLIDO!!!".
