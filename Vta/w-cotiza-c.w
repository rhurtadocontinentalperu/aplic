...
&Scoped-Define ENABLED-OBJECTS B-CCTE BUTTON-4 B-TXT BUTTON-8 BUTTON-FILE 
...
DEFINE FRAME F-Main
     B-CCTE AT ROW 6.96 COL 76
     BUTTON-4 AT ROW 6.96 COL 82.72
     B-TXT AT ROW 7.73 COL 75.72
     BUTTON-8 AT ROW 17.88 COL 3
     BUTTON-FILE AT ROW 17.88 COL 84
...
&Scoped-define SELF-NAME B-TXT
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-TXT W-Win
ON CHOOSE OF B-TXT IN FRAME F-Main /* TXT */
DO:
  RUN Imprimir-txt IN h_v-cotiza-c.
END.
...
ON CHOOSE OF BUTTON-8 IN FRAME F-Main /* Texto Plano */
DO:
  RUN Asigna-Texto IN h_v-cotiza-c.
  RUN dispatch IN h_b-tmpcot-c ('open-query':U).
END.
...
/*        procname:SCREEN-VALUE = FILE-INFO:FULL-PATHNAME.*/
    ...
    END.
    if procname <> "" THEN DO:
        RUN Importar-Cabecera  IN h_v-cotiza-c(procname).
        RUN Importar-Articulos IN h_b-tmpcot-c(procname).
        RUN dispatch IN h_b-tmpcot-c ('open-query':U).
    end.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/v-cotiza-c.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotiza-c ).
       RUN set-position IN h_v-cotiza-c ( 1.00 , 1.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 7.08 , 10.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 1.31 , 12.43 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv02 ).
       RUN set-position IN h_p-updv02 ( 7.08 , 22.57 ) NO-ERROR.
       RUN set-size IN h_p-updv02 ( 1.31 , 53.14 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-pedido ).
       RUN set-position IN h_q-pedido ( 7.15 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-cotiza-c. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-cotiza-c ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-cotiza-c ).

       /* Links to SmartQuery h_q-pedido. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotiza-c ,
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             BUTTON-FILE:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedido ).
       RUN set-position IN h_b-pedido ( 8.50 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-pedido ( 10.77 , 91.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedido ,
             B-TXT:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/b-tmpcot-c.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpcot-c ).
       RUN set-position IN h_b-tmpcot-c ( 8.50 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpcot-c ( 9.23 , 88.29 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 17.73 , 48.43 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.19 , 35.14 ) NO-ERROR.

       /* Links to SmartBrowser h_b-tmpcot-c. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-tmpcot-c ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tmpcot-c ,
             B-TXT:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_b-tmpcot-c , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ENABLE B-CCTE BUTTON-4 B-TXT BUTTON-8 BUTTON-FILE 
...
CASE L-Handle:
    ...
    WHEN "browse1" THEN DO:
         RUN dispatch IN h_b-tmpcot-c ('open-query':U).
         RUN dispatch IN h_b-tmpcot-c ('apply-entry':U).
         RETURN NO-APPLY.
       END.
...
/*         Btn-Asigna:VISIBLE = YES.*/
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN Recalcular-Precios IN h_b-tmpcot-c.
      END.
END CASE.
