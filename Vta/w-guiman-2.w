...
DEFINE NEW SHARED VARIABLE s-TpoFac   AS CHAR INIT 'I'.
...
&Scoped-Define ENABLED-OBJECTS RECT-65 C-NroSer 
...
DEFINE VARIABLE C-NroSer AS CHARACTER FORMAT "X(3)":U INITIAL "018" 
     ...
     LIST-ITEMS "018","102","237","761","251" 
     DROP-DOWN-LIST
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'src/adm-vm/objects/p-updv02.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-guiman-2 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/b-guiman-2b.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-guiman-2b ).
       RUN set-position IN h_b-guiman-2b ( 10.42 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-guiman-2b ( 6.69 , 94.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 17.15 , 3.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.42 , 34.14 ) NO-ERROR.

       /* Links to SmartBrowser h_b-guiman-2b. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-guiman-2b ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-guiman-2b ,
             h_v-guiman-2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_b-guiman-2b , 'AFTER':U ).
    END. /* Page 2 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ue-desde-fac-fai W-Win 
PROCEDURE ue-desde-fac-fai :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
RUN ue-gen-desde-fac IN h_b-guiman-2b(INPUT pNroFac, INPUT pCodCli, OUTPUT pRetVal).
