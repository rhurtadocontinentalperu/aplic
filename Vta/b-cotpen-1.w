...
/* AND FacCpedi.Nomcli BEGINS wclient*/ ~
 ...
 AND ( wclient = '' OR INDEX(FacCPedi.NomCli, wclient) > 0 ) NO-LOCK ~
...
&Scoped-Define ENABLED-OBJECTS COMBO-BOX-3 wclient wcotiza COMBO-BOX-5 ~
x-FchPed-1 x-FchPed-2 br_table 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-3 wclient wcotiza COMBO-BOX-5 ~
x-FchPed-1 x-FchPed-2 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 88.14 BY 12.23
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     wclient AT ROW 1.27 COL 33 COLON-ALIGNED
     wcotiza AT ROW 1.27 COL 33 COLON-ALIGNED
     COMBO-BOX-5 AT ROW 1.27 COL 72.43 COLON-ALIGNED
     x-FchPed-1 AT ROW 2.08 COL 33 COLON-ALIGNED WIDGET-ID 2
     x-FchPed-2 AT ROW 2.08 COL 48 COLON-ALIGNED WIDGET-ID 4
...
/* AND integral.FacCpedi.Nomcli BEGINS wclient*/
 ...
 AND FacCPedi.FchPed >= x-FchPed-1
 AND FacCPedi.FchPed <= x-FChPed-2
 AND ( wclient = '' OR INDEX(FacCPedi.NomCli, wclient) > 0 )"
...
ON LEAVE OF x-FchPed-1 IN FRAME F-Main /* Emitidos desde */
DO:
    ASSIGN {&self-name}.
    run abrir-query.
...
/*  IF Faccpedi.tipvta = "1" THEN
    ...
    CASE FacCPedi.FlgEst:
        WHEN 'E' THEN X-STA = "EMI".
        WHEN 'P' THEN X-STA = "PEN".
        WHEN 'C' THEN X-STA = "ATE".
        OTHERWISE X-STA = "ANU". 
    END CASE.
