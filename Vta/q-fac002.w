...
&Scoped-Define ENABLED-OBJECTS RECT-22 s-CodDoc s-NroSer FILL-IN-NroDoc 
&Scoped-Define DISPLAYED-OBJECTS s-CodDoc s-NroSer FILL-IN-NroDoc 
...
DEFINE {&NEW} SHARED VARIABLE s-CodDoc AS CHARACTER FORMAT "X(256)":U INITIAL ? 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "BOL","FAC" 
     SIZE 7 BY 1.

DEFINE {&NEW} SHARED VARIABLE s-NroSer AS INTEGER FORMAT "999":U INITIAL ? 
     LABEL "Serie" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "0" 
     SIZE 6 BY 1.
...
DEFINE FRAME FRAME-A
     s-CodDoc AT ROW 1.38 COL 2 NO-LABEL
     s-NroSer AT ROW 1.38 COL 9.71
     FILL-IN-NroDoc AT ROW 1.38 COL 24 COLON-ALIGNED
...
/* SETTINGS FOR FRAME FRAME-A
                                                                        */
/* SETTINGS FOR COMBO-BOX s-CodDoc IN FRAME FRAME-A
   SHARED ALIGN-L                                                       */
/* SETTINGS FOR COMBO-BOX s-NroSer IN FRAME FRAME-A
   SHARED ALIGN-L                                                       */
...
/* Query rebuild information for QUERY Query-Main
 ...
 AND INTEGRAL.CcbCDocu.NroDoc BEGINS STRING(s-nroser, '999')
 AND INTEGRAL.CcbCDocu.TpoFac = ""I""
...
ON LEAVE OF FILL-IN-NroDoc IN FRAME FRAME-A /* Buscar */
OR RETURN OF FILL-IN-NroDoc
DO:
  ASSIGN FILL-IN-NroDoc.
  FIND ccbcdocu WHERE ccbcdocu.codcia = s-codcia
    AND ccbcdocu.coddoc = s-coddoc
    AND ccbcdocu.coddiv = s-coddiv
    AND ccbcdocu.nrodoc = STRING(s-nroser, '999') + STRING(FILL-IN-NroDoc, '999999')
    NO-LOCK NO-ERROR.
  IF AVAILABLE ccbcdocu
  THEN DO:
    REPOSITION {&QUERY-NAME} TO ROWID ROWID(ccbcdocu) NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL s-CodDoc q-tables
ON VALUE-CHANGED OF s-CodDoc IN FRAME FRAME-A
DO:
  FIND FIRST FacCorre WHERE faccorre.codcia = s-codcia 
        AND faccorre.coddiv = s-coddiv 
        AND faccorre.codalm = s-codalm
        AND faccorre.coddoc = s-coddoc:SCREEN-VALUE
        NO-LOCK NO-ERROR.
  IF NOT AVAILABLE faccorre
  THEN DO:
    MESSAGE 'NO existen series definidas para este documento'
        VIEW-AS ALERT-BOX ERROR.
    RETURN NO-APPLY.
  END.
  x-nroser = faccorre.nroser.
  
  ASSIGN s-coddoc.
  REPEAT:
    IF s-nroser:DELETE(1) = NO THEN LEAVE.
  END.
  FOR EACH FacCorre WHERE faccorre.codcia = s-codcia 
        ...
        NO-LOCK:
    s-nroser:ADD-LAST(STRING(faccorre.nroser, '999')) IN FRAME {&FRAME-NAME}.
  END.
  s-nroser = x-nroser.
  DISPLAY s-nroser WITH FRAME {&FRAME-NAME}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL s-NroSer q-tables
ON VALUE-CHANGED OF s-NroSer IN FRAME FRAME-A /* Serie */
DO:
  ASSIGN s-nroser.
  RUN dispatch IN THIS-PROCEDURE ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  DISPLAY s-coddoc s-nroser WITH FRAME {&FRAME-NAME}.
