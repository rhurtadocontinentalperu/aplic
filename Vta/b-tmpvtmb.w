...
ON "RETURN":U OF ITEM.codmat, ITEM.CanPed, ITEM.ImpLin
DO:
   APPLY "TAB":U.
   RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH B-ITEM:
      ...
      I-NroItm = B-ITEM.NroItm.
  END.
  IF (N-ITMS + 1) > FacCfgGn.Items_Factura THEN DO:
...
/*              ITEM.PreUni = DEC(ITEM.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) /*F-PREVTA*/*/
              ITEM.PreUni = ROUND(DEC(ITEM.ImpLin:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) / (IF SW-LOG1 THEN ITEM.CanPed ELSE (ITEM.CanPed * F-FACTOR)) , 4)
...
/*              ITEM.ImpLin = ROUND( ITEM.PreUni * (IF SW-LOG1 THEN ITEM.CanPed ELSE (ITEM.CanPed * F-FACTOR)) , 2 ) - ITEM.ImpDto.*/
              ITEM.ImpLin = DEC(ITEM.ImpLin:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
       ...
       IF ITEM.AftIgv THEN
        ...
        /***********Grabando Costos **********************/
        ITEM.Mrguti = ROUND( Almmmatg.Ctotot * ITEM.CanPed , 2 ) . 
        IF S-CODMON <> Almmmatg.Monvta THEN DO:
           ITEM.Mrguti = IF S-CODMON = 1 THEN ROUND( Almmmatg.Ctotot * ITEM.Canped * Almmmatg.Tpocmb , 2 )
                                         ELSE ROUND(( Almmmatg.Ctotot * ITEM.Canped ) /  Almmmatg.Tpocmb , 2 ).
        END.
        /*************************************************/
...
END.
*/
IF DECIMAL(ITEM.ImpLin:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) = 0 THEN DO:
