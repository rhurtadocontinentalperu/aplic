...
/*                DMOV.PorDto = CcbDDocu.PorDto */
                DMOV.ImpLin = CcbDDocu.ImpLin 
                DMOV.ImpIsc = CcbDDocu.ImpIsc 
                DMOV.ImpIgv = CcbDDocu.ImpIgv 
/*                DMOV.ImpDto = CcbDDocu.ImpDto */
                DMOV.AftIsc = CcbDDocu.AftIsc 
                DMOV.AftIgv = CcbDDocu.AftIgv
/*                DMOV.Por_Dsctos[1] = CcbDDocu.Por_Dsctos[1]
 *                 DMOV.Por_Dsctos[2] = CcbDDocu.Por_Dsctos[2]
 *                 DMOV.Por_Dsctos[3] = CcbDDocu.Por_Dsctos[3]*/
                DMOV.Flg_factor = CcbDDocu.Flg_factor.
            IF Ccbddocu.candev <> 0 THEN DO:
                ASSIGN
            ...
            END.
/*            ASSIGN
 *                 DMOV.ImpDto = ROUND( DMOV.PreUni * DMOV.CanDes * (DMOV.Por_Dsctos[1] / 100),4 )
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    FOR EACH CcbDDocu NO-LOCK WHERE CcbDDocu.CodCia = CcbCDocu.CodCia 
        ...
        IF (CcbDDocu.CanDes - CcbDDocu.CanDev) > 0 THEN DO:
           CREATE DMOV.
           ASSIGN DMOV.CodCia = CcbDDocu.CodCia
/*                  DMOV.CodAlm = X-ALMDES /*S-CODALM*/*/
                  DMOV.CodAlm = s-CodAlm
                  DMOV.TipMov = "S"
                  DMOV.codmat = CcbDDocu.CodMat
                  DMOV.CanDes = (CcbDDocu.CanDes - CcbDDocu.CanDev)
           ...
           IF AVAILABLE Almmmatg THEN DO:
              IF DMOV.Flg_factor = "" THEN DO:
                    /****   Add By C.Q. para validar las cantidades ****/
                      /****   PRECIO C    ****/
                      SW-LOG1 = FALSE.
                      X-FACTOR = 1.
                      IF Almmmatg.UndC <> "" AND NOT SW-LOG1 THEN DO:
                          ...
                          IF (CcbDDocu.CanDes * DMOV.Factor) >= 
                             (0.25 * X-FACTOR) THEN DO:
                              SW-LOG1 = TRUE.
                          END.
                      END.
                      /****   PRECIO B    ****/
                    ...
                    ASSIGN
/*                        DMOV.ImpDto = ROUND(DMOV.PreUni * (DMOV.PorDto / 100) * (IF SW-LOG1 THEN (DMOV.CanDes) ELSE (DMOV.CanDes * DMOV.Factor)), 2)
 *                         DMOV.ImpLin = ROUND(DMOV.PreUni * (IF SW-LOG1 THEN (DMOV.CanDes) ELSE (DMOV.CanDes * DMOV.Factor)) , 2 ) .*/
                        DMOV.ImpDto = ROUND(DMOV.PreUni * (DMOV.Por_Dsctos[1] / 100) * (IF SW-LOG1 THEN (DMOV.CanDes) ELSE (DMOV.CanDes * DMOV.Factor)), 2)
                        DMOV.ImpLin = ROUND(DMOV.PreUni * (IF SW-LOG1 THEN (DMOV.CanDes) ELSE (DMOV.CanDes * DMOV.Factor)) , 2 ) - DMOV.ImpDto. .
                    IF DMOV.AftIsc THEN 
                       ASSIGN
                          DMOV.ImpIsc = ROUND(DMOV.PreBas * (IF SW-LOG1 THEN (DMOV.CanDes) ELSE (DMOV.CanDes * DMOV.Factor)) * (Almmmatg.PorIsc / 100),4).
                    IF DMOV.AftIgv THEN 
                       ASSIGN  
                          DMOV.ImpIgv = DMOV.ImpLin - ROUND(DMOV.ImpLin  / (1 + (FacCfgGn.PorIgv / 100)),4).
      
                    /**** Se puso para poder diferenciar cuando se tiene que aplicar el factor de conversion  
                          , Add by C.Q. 09/03/2000    ****/
                    DMOV.CodAnt = IF SW-LOG1 THEN "YES" ELSE "NO".
                    /***************************************************************************************/
              END.
              ELSE DO:
                  ASSIGN
                      DMOV.ImpDto = ROUND(DMOV.PreUni * (DMOV.Por_Dsctos[1] / 100) * (IF DMOV.Flg_factor = "1" THEN DMOV.CanDes ELSE (DMOV.CanDes * DMOV.Factor)), 2)
                      DMOV.ImpLin = ROUND(DMOV.PreUni * (IF DMOV.Flg_factor = "1" THEN DMOV.CanDes ELSE (DMOV.CanDes * DMOV.Factor)) , 2 ) - DMOV.ImpDto.
                  IF DMOV.AftIsc THEN 
                     ASSIGN
                        DMOV.ImpIsc = ROUND(DMOV.PreBas * (IF DMOV.Flg_factor = "1" THEN DMOV.CanDes ELSE (DMOV.CanDes * DMOV.Factor)) * (Almmmatg.PorIsc / 100),4).
                  IF DMOV.AftIgv THEN 
                     ASSIGN  
                    DMOV.ImpIgv = DMOV.ImpLin - ROUND(DMOV.ImpLin  / (1 + (FacCfgGn.PorIgv / 100)),4).
              END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    Almcmov.CodRef:SCREEN-VALUE = "FAC".
  END.
  C-NROFAC = "".
