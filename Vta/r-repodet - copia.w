...
&Scoped-Define ENABLED-OBJECTS RECT-1 FILL-IN-CodAlm BUTTON-3 rs-tipo ~
...
DEFINE FRAME F-Main
     ...
     F-mnje AT ROW 9.88 COL 69 NO-LABEL WIDGET-ID 24
     RECT-1 AT ROW 1.08 COL 1.57 WIDGET-ID 26
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 78.43 BY 10.77 WIDGET-ID 100.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH Almmmate NO-LOCK WHERE Almmmate.codcia = s-codcia
        AND Almmmate.codmat >= c-ArtDesde
        AND Almmmate.codmat <= c-ArtHasta
        AND LOOKUP(Almmmate.codalm, c-almacenes) > 0,
        FIRST Almmmatg OF Almmmate NO-LOCK WHERE (txt-proveedor = '' OR almmmatg.CodPr1 = txt-proveedor)
        AND (txt-codfam = '' OR almmmatg.codfam = txt-codfam)
        AND (txt-subfam = '' OR almmmatg.subfam = txt-subfam)
        ...
        BUFFER-COPY Almmmate 
            ...
            Detalle.CtoPro        = Almmmatg.CtoTot * dTpoCmb
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 FILL-IN-CodAlm BUTTON-3 rs-tipo txt-Desde txt-Hasta 
         txt-fchcorte tg-stock rs-estado txt-codfam txt-subfam txt-Proveedor 
         Btn-Excel BUTTON-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
chWorkSheet:Range("R2"):Value = "Proveedor".
chWorkSheet:Range("S2"):Value = "Categoria".
...
chWorkSheet:Range("P2"):Value = "Proveedor".
chWorkSheet:Range("Q2"):Value = "Categoria".
