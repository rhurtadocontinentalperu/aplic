...
DEF INPUT PARAMETER p-nroped AS CHAR.
DEF INPUT PARAMETER p-coddoc AS CHAR.

/* Local Variable Definitions ---                                       */

DEFINE NEW SHARED TEMP-TABLE PEDI LIKE FacDPedi.
DEFINE NEW SHARED VARIABLE s-coddoc   AS CHAR INITIAL "COT".
DEFINE NEW SHARED VARIABLE s-nroped   AS CHAR INIT '011073297'.
ASSIGN
    s-coddoc = p-coddoc
    s-nroped = p-nroped.
...
DEFINE VARIABLE h_q-cotcon AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotiza AS HANDLE NO-UNDO.

/* ************************  Frame Definitions  *********************** */

DEFINE FRAME D-Dialog
     SPACE(96.31) SKIP(17.40)
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* COTIZACION */
DO:  
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotiza ).
       RUN set-position IN h_v-cotiza ( 1.38 , 5.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedido ).
       RUN set-position IN h_b-pedido ( 7.62 , 5.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.77 , 88.29 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/q-cotcon.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-cotcon ).
       RUN set-position IN h_q-cotcon ( 3.88 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.65 , 8.29 ) */

       /* Links to SmartViewer h_v-cotiza. */
       RUN add-link IN adm-broker-hdl ( h_q-cotcon , 'Record':U , h_v-cotiza ).

       /* Links to SmartBrowser h_b-pedido. */
       RUN add-link IN adm-broker-hdl ( h_q-cotcon , 'Record':U , h_b-pedido ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedido ,
             h_v-cotiza , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-cotcon ,
             h_b-pedido , 'AFTER':U ).
    END. /* Page 0 */
