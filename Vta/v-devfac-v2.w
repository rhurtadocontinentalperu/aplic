...
Almcmov.NroDoc Almcmov.usuario Almcmov.LPN Almcmov.CodRef Almcmov.NroRf1 ~
...
DEFINE FRAME F-Main
     ...
     Almcmov.NroDoc AT ROW 1.77 COL 13 COLON-ALIGNED
          ...
          SIZE 12 BY .81
          FONT 0
     Almcmov.usuario AT ROW 2.35 COL 72 COLON-ALIGNED WIDGET-ID 8 FORMAT "x(15)"
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
      ...
      FOR EACH Almdmov OF Almcmov NO-LOCK:
          ...
          /* 08/04/2026: Validación adicional */
          IF Ccbddocu.CanDev > Ccbddocu.CanDes THEN DO:
              pMensaje = "ERROR:" + CHR(10) + "Artículo " + Almdmov.codmat + " la devolución sobrepasa la venta" + CHR(10) +
                  "Proceso abortado".
              UNDO RLOOP, RETURN 'ADM-ERROR'.
          END.
          RELEASE CcbDDocu.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR iLineaFacturado AS DEC.
  DEFINE VAR qFacturado AS DEC.
  DEFINE VAR iPrecioUnitario AS DEC.
    ...
    FOR EACH CcbDDocu NO-LOCK WHERE CcbDDocu.CodCia = CcbCDocu.CodCia 
        ...
        IF x-Cantidad > 0 THEN DO:
            ...
            &IF {&ARITMETICA-SUNAT} &THEN
                iLineaFacturado = CcbDDocu.MontoBaseIGV + CcbDDocu.ImporteIGV.
                qFacturado = Ccbddocu.CanDes.
                iPrecioUnitario = ROUND(iLineaFacturado / qFacturado,4).

                IF DMOV.CanDes = Ccbddocu.CanDes THEN DO:   /* DEVOLUCION TOTAL */
                    ASSIGN
                        DMOV.ImpLin = iLineaFacturado.
                        DMOV.PreUni = iPrecioUnitario.
                END.
            ...
            &ELSE
                ...
                END.
            &ENDIF
            IF Ccbddocu.cTipoAfectacion = "GRATUITA" THEN ASSIGN DMOV.PorDto = 100 DMOV.ImpLin = 0.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN Almcmov.NroRf3 = cReturnValue.
...
DEFINE VAR iCmpteAntiguedaLimite AS INT.
DEFINE VAR iCmpteAntigueda AS INT.
...
/*    END.                                                                                */
   ...
   IF CcbCDocu.FlgSit = "D" THEN DO:
      ...
      RETURN "ADM-ERROR".   
   END.
    /* Validamos la antiguedad del comprobante referenciado */
   cLlave_c1 = 'ALMACEN'.
   cLlave_c2 = 'PI'.
   cLlave_c3 = 'CMPTE-ANTIGUEDAD'.
   iCmpteAntiguedaLimite = 360.
   FIND FIRST vtatabla WHERE vtatabla.codcia = s-codcia AND vtatabla.tabla = x-tabla AND
                                vtatabla.llave_C1 = cLlave_c1 AND vtatabla.llave_c2 = cLlave_C2 AND
                                vtatabla.llave_c3 = cLlave_c3 NO-LOCK NO-ERROR.
   IF AVAILABLE vtatabla THEN iCmpteAntiguedaLimite = vtatabla.rango_valor[1].

   iCmpteAntigueda = TODAY - CcbCDocu.fchdoc.
   IF iCmpteAntigueda > iCmpteAntiguedaLimite THEN DO:
       MESSAGE "El comprobante referenciado " Almcmov.CodRef:SCREEN-VALUE " " Almcmov.NroRf1:SCREEN-VALUE SKIP
            "es demasiado antiguo, como maximo se aceptan comprobantes con " 
            iCmpteAntiguedaLimite " dias de antiguedad " VIEW-AS ALERT-BOX WARNING.
       APPLY "ENTRY" TO Almcmov.NroRf1.
       RETURN "ADM-ERROR".   
   END.
