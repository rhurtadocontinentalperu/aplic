...
DesdeF HastaF C-tipo F-DIVISION-1 F-DIVISION-2 F-DIVISION-3 F-DIVISION-4 ~
F-DIVISION-5 F-DIVISION-6 nCodMon Btn_OK Btn_Cancel Btn_Help BUTTON-5 
&Scoped-Define DISPLAYED-OBJECTS F-DIVISION DesdeF HastaF C-tipo ~
...
DEFINE FRAME F-Main
     ...
     DesdeF AT ROW 4 COL 11.43 COLON-ALIGNED
     HastaF AT ROW 4.08 COL 33.14 COLON-ALIGNED
...
FOR EACH Evtdivi NO-LOCK WHERE Evtdivi.CodCia = S-CODCIA
                         ...
                         AND   (EvtDivi.Nrofch >= INTEGER(STRING(YEAR(DesdeF),"9999") + STRING(MONTH(DesdeF),"99"))
                         AND   EvtDivi.Nrofch <= INTEGER(STRING(YEAR(HastaF),"9999") + STRING(MONTH(HastaF),"99")) ):
                         

      DISPLAY Evtdivi.Coddiv @ Fi-Mensaje LABEL "Codigo de Articulo "
            ...
            IF X-FECHA >= DesdeF AND X-FECHA <= HastaF THEN DO:
                ...
                IF AVAILABLE Gn-tcmb THEN DO: 
                    ...
                    FIND tmp-tempo WHERE T-Codcia  = S-CODCIA AND
                                         T-Codano  = Evtdivi.Codano AND
                                         T-Codmes  = Evtdivi.Codmes AND
                                         T-Coddia  = I NO-ERROR.
                    IF NOT AVAIL tmp-tempo THEN DO:
                      CREATE tmp-tempo.
                      ASSIGN T-codcia  = S-CODCIA
                             T-Codano  = Evtdivi.Codano 
                    ...
                    X-ENTRA = FALSE.
                    IF F-DIVISION-1 <> "" AND Evtdivi.Coddiv = F-DIVISION-1 THEN DO:
                       ASSIGN T-Venta[1] = T-venta[1] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-2 <> "" AND Evtdivi.Coddiv = F-DIVISION-2 THEN DO:
                       ASSIGN T-Venta[2] = T-venta[2] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-3 <> "" AND Evtdivi.Coddiv = F-DIVISION-3 THEN DO:
                       ASSIGN T-Venta[3] = T-venta[3] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-4 <> "" AND Evtdivi.Coddiv = F-DIVISION-4 THEN DO:
                       ASSIGN T-Venta[4] = T-venta[4] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-5 <> "" AND Evtdivi.Coddiv = F-DIVISION-5 THEN DO:
                       ASSIGN T-Venta[5] = T-venta[5] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-6 <> "" AND Evtdivi.Coddiv = F-DIVISION-6 THEN DO:
...
FOR EACH Evtdivi NO-LOCK WHERE Evtdivi.CodCia = S-CODCIA
      ...
      DISPLAY Evtdivi.Coddiv @ Fi-Mensaje LABEL "Division"
              FORMAT "X(11)" WITH FRAME F-Proceso.
      ...
      FIND tmp-tempo WHERE t-codcia  = S-CODCIA AND
                           T-Codano  = Evtdivi.Codano AND
                           T-Codmes  = Evtdivi.Codmes NO-ERROR.
      IF NOT AVAIL tmp-tempo THEN DO:
        CREATE tmp-tempo.
        ASSIGN t-codcia  = S-CODCIA
               T-Codano  = Evtdivi.Codano 
...
------------------------------------------------------------------------------*/
  DISPLAY F-DIVISION DesdeF HastaF C-tipo F-DIVISION-1 F-DIVISION-2 F-DIVISION-3 
          F-DIVISION-4 F-DIVISION-5 F-DIVISION-6 nCodMon F-NOMDIV-1 F-NOMDIV-2 
          F-NOMDIV-3 F-NOMDIV-4 F-NOMDIV-5 F-NOMDIV-6 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-58 RECT-61 RECT-62 RECT-63 F-DIVISION DesdeF HastaF C-tipo 
         F-DIVISION-1 F-DIVISION-2 F-DIVISION-3 F-DIVISION-4 F-DIVISION-5 
         F-DIVISION-6 nCodMon Btn_OK Btn_Cancel Btn_Help BUTTON-5 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-HEADER
        ...
        "------------------------------------------------------------------------------------------------------------------------------------------------------------" SKIP
        SUBSTRING(F-NOMDIV-1,1,15) FORMAT "X(15)" AT  30 
        SUBSTRING(F-NOMDIV-2,1,15) FORMAT "X(15)" AT  45 
        SUBSTRING(F-NOMDIV-3,1,15) FORMAT "X(15)" AT  60 
        SUBSTRING(F-NOMDIV-4,1,15) FORMAT "X(15)" AT  75 
        SUBSTRING(F-NOMDIV-5,1,15) FORMAT "X(15)" AT  90
        SUBSTRING(F-NOMDIV-6,1,15) FORMAT "X(15)" AT  105
        "O T R O S"  AT  120
        "T O T A L"  AT  140 SKIP
        "    FECHA                     Importe         Importe        Importe        Importe         Importe        Importe         Importe           IMPORTE    " SKIP
        "------------------------------------------------------------------------------------------------------------------------------------------------------------" SKIP
  ...
  FOR EACH tmp-tempo  WHERE t-venta[10] <> 0 
      ...
      IF FIRST-OF(t-codano) THEN DO:
        PUT STREAM REPORT {&PRN6A} + ("PERIODO  : " + STRING(T-Codano,"9999") ) + {&PRN6B} FORMAT "X(30)" SKIP.
      END.

      IF FIRST-OF(t-codmes) THEN DO:
        PUT STREAM REPORT {&PRN6A} + ENTRY(T-codmes,X-NOMMES) + {&PRN6B} FORMAT "X(20)" AT 5 SKIP.    
      END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-HEADER
        ...
        "O T R O S"  AT  120
        "T O T A L"  AT  140 SKIP
        "    PERIODO     MES           Importe         Importe        Importe        Importe         Importe        Importe         Importe           IMPORTE    " SKIP
        "------------------------------------------------------------------------------------------------------------------------------------------------------------" SKIP
