...
DEFINE TEMP-TABLE ttrep_det NO-UNDO
    FIELDS ttrep_codmat LIKE FacDPedi.codmat COLUMN-LABEL "Código!Artículo"
...
FILL-IN-codalm BROWSE-od BUTTON-ok BUTTON-cancel BUTTON-excel 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-fecha FILL-IN-fecha-2 ~
FILL-IN-codcli FILL-IN-nomcli FILL-IN-codalm FILL-IN-desalm 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 68 BY 15
...
DEFINE FRAME F-Main
     ...
     FILL-IN-codcli AT ROW 2.62 COL 16 COLON-ALIGNED WIDGET-ID 26
     FILL-IN-nomcli AT ROW 2.62 COL 28 COLON-ALIGNED NO-LABEL WIDGET-ID 28
     FILL-IN-codalm AT ROW 3.69 COL 16 COLON-ALIGNED WIDGET-ID 30
     FILL-IN-desalm AT ROW 3.69 COL 28 COLON-ALIGNED NO-LABEL WIDGET-ID 34
     BROWSE-od AT ROW 4.77 COL 3 WIDGET-ID 100
     BUTTON-ok AT ROW 20.12 COL 45
     BUTTON-cancel AT ROW 20.12 COL 53
     BUTTON-excel AT ROW 20.12 COL 61
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Reporte Consolidado O/D Pendientes - Autoservicios"
         HEIGHT             = 21.5
         WIDTH              = 71.86
         MAX-HEIGHT         = 21.5
         MAX-WIDTH          = 71.86
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-od FILL-IN-desalm F-Main */
...
"FacCPedi.NroPed" ? "X(11)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.FacCPedi.CodCli
"FacCPedi.CodCli" "Cliente" "x(14)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = INTEGRAL.FacCPedi.NomCli
     _FldNameList[4]   > INTEGRAL.FacCPedi.FchPed
"FacCPedi.FchPed" "Fecha!Pedido" ? "date" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-od */
&ANALYZE-RESUME
...
ON END-ERROR OF W-Win /* Reporte Consolidado O/D Pendientes - Autoservicios */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Reporte Consolidado O/D Pendientes - Autoservicios */
DO:
...
DO:
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        IF NOT AVAILABLE almacen THEN DO:
            MESSAGE "Almacén de reposición NO válido"
...
DO:
    ...
    IF CAN-FIND (FIRST ttrep_det) THEN DO:
        MESSAGE "¿Desea generar registro de reposición?"
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH ttrep_det:
        ...
        /* Busca Cantidad Disponible en Almacén de Despacho */
        FIND almmmate WHERE
            almmmate.codcia = s-codcia AND
            almmmate.codmat = ttrep_codmat AND
            almmmate.codalm = s-codalm NO-LOCK NO-ERROR.
        ...
        IF dStockAct <= 0 THEN NEXT.
        IF ttrep_canped > dStockAct THEN DO:
            dCantRep = ttrep_canped - dStockAct.
            /* Busca Cantidad Disponible en Almacén de Reposición */
            FIND almmmate WHERE
                almmmate.codcia = s-codcia AND
                almmmate.codmat = ttrep_codmat AND
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-fecha FILL-IN-fecha-2 FILL-IN-codcli FILL-IN-nomcli 
          FILL-IN-codalm FILL-IN-desalm 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE FILL-IN-fecha FILL-IN-fecha-2 FILL-IN-codcli FILL-IN-codalm BROWSE-od 
         BUTTON-ok BUTTON-cancel BUTTON-excel 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
    ...
    cTitle = "ARTICULOS POR REPONER - ALM REPOSICION " + FILL-IN-codalm.
    PAGE STREAM report.
...
------------------------------------------------------------------------------*/
    ...
    DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
        ...
        CREATE Almcrepo.
        ASSIGN
            almcrepo.AlmPed = s-codalm
            almcrepo.CodAlm = FILL-IN-codalm
        ...
        FOR EACH ttrep_det:
            ...
            ASSIGN
                ...
                almdrepo.ITEM = iItem
                almdrepo.AlmPed = Almrepos.almped
