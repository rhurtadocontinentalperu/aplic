...
DEFINE VARIABLE X-PESMAT AS DECIMAL NO-UNDO.
...
&Scoped-define FIELDS-IN-QUERY-br_table PEDI.NroItm PEDI.codmat Almmmatg.DesMat Almmmatg.DesMar PEDI.UndVta PEDI.PesMat PEDI.CanAte PEDI.CanPed   
...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-Total F-dispo F-totstk 
...
DEFINE VARIABLE F-dispo AS DECIMAL FORMAT "->>>,>>9.99":U INITIAL 0 
     LABEL "Stock Disp." 
     VIEW-AS FILL-IN 
     SIZE 11.14 BY .81
     BGCOLOR 1 FGCOLOR 15 FONT 6 NO-UNDO.

DEFINE VARIABLE F-totstk AS DECIMAL FORMAT "->>>,>>9.99":U INITIAL 0 
     LABEL "Stock Actual" 
     VIEW-AS FILL-IN 
     SIZE 11.14 BY .81
     BGCOLOR 1 FGCOLOR 15 FONT 6 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _FREEFORM
  QUERY br_table NO-LOCK DISPLAY
      ...
      PEDI.PesMat COLUMN-LABEL "Solicitado" FORMAT ">>>,>>>.99"
      PEDI.CanAte COLUMN-LABEL "Atendido"   FORMAT ">>>,>>>.99"
      PEDI.CanPed COLUMN-LABEL "Cantidad"   FORMAT ">>>,>>>.99"
  ENABLE
      PEDI.CanPed
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 88 BY 6.73
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     FILL-IN-Total AT ROW 8.19 COL 9.29 COLON-ALIGNED
     F-dispo AT ROW 8.19 COL 38.86 COLON-ALIGNED
     F-totstk AT ROW 8.19 COL 59.86 COLON-ALIGNED
...
DO:
  /* This ADM trigger code must be preserved in order to notify other
    ...
    RUN vta/stkdispo (s-codcia, 
                      ...
                      PEDI.codmat, 
                      0 /*FacDPedi.Factor * F-CANPED*/ ,
                      OUTPUT S-OK, 
                      OUTPUT S-STKDIS
                      ).

    FIND almmmate WHERE Almmmate.CodCia = s-codcia 
                   AND  Almmmate.CodAlm = PEDI.AlmDes  /*s-codalm */
                   AND  Almmmate.codmat = PEDI.codmat 
                  NO-LOCK.
    IF AVAILABLE almmmate THEN DO:
        DISPLAY Almmmate.StkAct @ F-totstk
                S-STKDIS @ F-dispo
...
ON "LEAVE":U OF PEDI.Canped DO:
  ...
  X-PESMAT = PEDI.canped * Almmmatg.pesmat.

/*  DEF VAR f-CanPed AS DEC.
 *   F-CANPED = DEC(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
 *   RUN vta/stkdispo (s-codcia, 
 *                     s-codalm, 
 *                     Pedi.codmat, 
 *                     Pedi.Factor * F-CANPED ,
 *                     OUTPUT S-OK, 
 *                     OUTPUT S-STKDIS).
 *   IF NOT S-OK THEN DO:
 *     MESSAGE 'No puede superar el stock disponible' VIEW-AS ALERT-BOX WARNING.
 *     F-CANPED = S-STKDIS.
 *     DISPLAY f-CanPed @ PEDI.CanPed WITH BROWSE {&BROWSE-NAME}.
 *     RETURN NO-APPLY.
 *   END.*/
      ...
      DO I = 1 TO I-NITM:
         ...
         IF NOT AVAILABLE B-PEDI THEN DO:
            ...
            IF B-PEDI.AftIgv THEN  
               B-PEDI.ImpIgv = B-PEDI.ImpLin - ROUND(B-PEDI.ImpLin  / (1 + (s-PorIgv / 100)),4).
         END.
      END.
...
------------------------------------------------------------------------------*/
   FILL-IN-Total  = 0.
   FOR EACH B-PEDI,
       EACH Almmmatg OF B-PEDI:
       FILL-IN-Total  = FILL-IN-Total + B-PEDI.ImpLin.
   END.
   IF S-TIPVTA = 'SIN IGV' THEN
      FILL-IN-Total = FILL-IN-Total * ( 1 + s-PorIgv / 100).
...
/* ****************** RHC 23.08.04 Rutina anterior
  ASSIGN PEDI.CodCia = S-CODCIA 
         PEDI.Factor = F-FACTOR 
         PEDI.NroItm = I-NroItm
         PEDI.Pesmat = (PEDI.canped * PEDI.Factor) * Almmmatg.pesmat.

  FIND Almmmatg WHERE Almmmatg.CodCia = S-CODCIA AND
       Almmmatg.codmat = PEDI.codmat NO-LOCK NO-ERROR.
  IF AVAILABLE Almmmatg THEN DO:
     ASSIGN PEDI.PreBas = F-PreBas 
            PEDI.Pesmat = Almmmatg.pesmat * PEDI.Canped * F-Factor
            PEDI.AftIgv = Almmmatg.AftIgv 
            PEDI.AftIsc = Almmmatg.AftIsc 
            PEDI.PreUni = F-PREVTA 
            PEDI.Implin = ROUND(PEDI.PreUni * (1 - PEDI.Pordto / 100) * (1 - PEDI.Pordto2 / 100), 2) * PEDI.Canped * PEDI.Factor
            PEDI.ImpDto = ROUND(PEDI.PreUni * PEDI.Canped * PEDI.Factor, 2) - PEDI.Implin
            PEDI.Impigv = 0.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
  RUN valida-update.
  /*message "s-tpoped: "s-tpoped view-as alert-box.*/

/*  IF S-TPOPED = "2" THEN DO:*/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Tipo-Lista B-table-Win 
PROCEDURE Tipo-Lista :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  IF S-TIPVTA = 'Margen Utilidad' THEN DO:
...
IF PEDI.UndVta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = "" THEN DO:
   ...
   RETURN "ADM-ERROR".
END.
/* usamos canate para controlar lo maximo que podemos despachar segun el saldo del pedido */
IF DECIMAL(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > (PEDI.PesMat - PEDI.CanAte) THEN DO:
   MESSAGE "Cantidad no debe ser mayor a " (PEDI.PesMat - PEDI.CanAte) VIEW-AS ALERT-BOX ERROR.
...
FIND Almmmate WHERE 
     ...
     Almmmate.CodAlm = PEDI.AlmDes   /*S-CODALM*/  AND
...
/* VERIFICAMOS SI ES UNA MODIFICACION */
x-CanPed = F-FACTOR * DECIMAL(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
FIND Facdpedi WHERE Facdpedi.codcia = PEDI.codcia
    AND Facdpedi.coddoc = PEDI.coddoc
    AND Facdpedi.nroped = PEDI.nroped
    AND Facdpedi.codmat = PEDI.codmat NO-LOCK NO-ERROR.
IF AVAILABLE Facdpedi THEN x-CanPed = x-CanPed - Facdpedi.canped * Facdpedi.factor.
RUN vta/stkdispo (s-codcia, Pedi.AlmDes, PEDI.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME},
        x-CanPed, 
...
IF S-OK = NO THEN DO:
    MESSAGE "No hay STOCK disponible" SKIP
            ...
            "       DISPONIBLE : " S-STKDIS VIEW-AS ALERT-BOX ERROR.
    RETURN "ADM-ERROR".
END.
FIND Almtconv WHERE 
     Almtconv.CodUnid  = Almmmatg.UndBas AND  
...
/*IF S-TPOPED = "1" THEN RETURN "ADM-ERROR".*/
/*IF PEDI.CanAte > 0 THEN DO:
 *     MESSAGE 'Item con atención parcial' VIEW-AS ALERT-BOX ERROR.
 *     RETURN 'ADM-ERROR'.
 * END.*/
