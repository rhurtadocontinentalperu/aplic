...
DEFINE VARIABLE h_v-totfac AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/v-facsvc.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-facsvc ).
       RUN set-position IN h_v-facsvc ( 2.54 , 2.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv04 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-facsvc ,
             h_p-updv04 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-facsvc ).
       RUN set-position IN h_b-facsvc ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-facsvc ( 6.69 , 95.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/v-totfac.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-totfac ).
       RUN set-position IN h_v-totfac ( 17.35 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.12 , 97.00 ) */

       /* Links to SmartBrowser h_b-facsvc. */
       RUN add-link IN adm-broker-hdl ( h_q-cdocu , 'Record':U , h_b-facsvc ).

       /* Links to SmartViewer h_v-totfac. */
       RUN add-link IN adm-broker-hdl ( h_q-cdocu , 'Record':U , h_v-totfac ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-facsvc ,
             h_v-facsvc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-totfac ,
             h_b-facsvc , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
