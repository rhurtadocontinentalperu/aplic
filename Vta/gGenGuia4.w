...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* PEDIDOS MOSTRADOR */
    IF CcbCDocu.CodPed = "P/M" THEN DO:
        CcbCDocu.FlgEst = "F".  /* FACTURADA */
        FIND FIRST B-CDOCU WHERE B-CDOCU.codcia = s-codcia
            AND B-CDOCU.codped = Ccbcdocu.codped
            AND B-CDOCU.nroped = Ccbcdocu.nroped
            AND LOOKUP(B-CDOCU.coddoc, 'FAC,BOL,TCK') > 0
...
/*             MESSAGE 'Error en el correlativo del documento' cCodDoc */
/*                 VIEW-AS ALERT-BOX ERROR.                            */
...
/*         IF ERROR-STATUS:ERROR THEN UNDO trloop, RETURN 'ADM-ERROR'. */
        ...
        FOR EACH PEDI,
            ...
            IF PEDI.AftIgv THEN  PEDI.ImpIgv = PEDI.ImpLin - ROUND(PEDI.ImpLin  / (1 + (FacCPedi.PorIgv / 100)),4).
            /* Crea Detalle */
            CREATE CcbDDocu.
            BUFFER-COPY PEDI TO CcbDDocu
                ASSIGN
                    ...
                    CcbDDocu.AlmDes = PEDI.AlmDes        /* OJO */
