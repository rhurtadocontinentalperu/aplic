...
DEFINE VARIABLE h_t-precotexp_prov AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-precotexp_01 ( 11.62 , 2.72 ) NO-ERROR.
       RUN set-size IN h_b-precotexp_01 ( 13.35 , 133.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  't-precotexp_prov.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-precotexp_prov ).
       RUN set-position IN h_t-precotexp_prov ( 11.65 , 1.00 ) NO-ERROR.
       RUN set-size IN h_t-precotexp_prov ( 13.35 , 137.86 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-precotexp_prov ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_t-precotexp_prov <> ? THEN RUN dispatch IN h_t-precotexp_prov ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        RUN Recalcular-Precios IN h_t-precotexp_prov.
      END.
