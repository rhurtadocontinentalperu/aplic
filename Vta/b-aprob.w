...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 57 BY 6.69
         FONT 4
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE VARIABLE X-APROB LIKE CCBCDOCU.GLOSA FORMAT "X(20)". 
  ...
  DO WITH FRAME {&FRAME-NAME}:
    DO i = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS:
        IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(i) THEN DO:
            ...
            X-APROB = TRIM( S-USER-ID + ' ' + STRING(TODAY) + ' ' + STRING(TIME, 'HH:MM') ).
            ASSIGN
                CcbCDocu.FlgEst = 'P'
                CcbCDocu.FlgCon = X-APROB.
