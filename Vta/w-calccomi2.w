...
/* SETTINGS FOR BUTTON B-GRABA-COMISION IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       B-GRABA-COMISION:HIDDEN IN FRAME F-Main           = TRUE.
...
------------------------------------------------------------------------------*/
  ...
  END.
  /**********
  /* ELIMINAMOS LAS PENDIENTES */
  FOR EACH DETALLE:
    IF DETALLE.SdoAct > 0
    THEN DO:
        x-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'ELIMINANDO DOCUMENTOS: ' + DETALLE.coddoc + ' ' +
            DETALLE.nrodoc + ' ' + STRING(DETALLE.fchdoc).
/*        FOR EACH t-dcaja WHERE t-dcaja.codcia = DETALLE.codcia
 *                 AND t-dcaja.codref = DETALLE.coddoc
 *                 AND t-dcaja.nroref = DETALLE.nrodoc:
 *             DELETE t-dcaja.
 *         END.*/
        DELETE DETALLE.
    END. 
    ...
    IF AVAILABLE Ccbcmvto THEN DO:      /* CANJE POR LETRA UBICADO */
        ...
        FOR EACH Ccbdmvto NO-LOCK WHERE Ccbdmvto.codcia = Ccbcmvto.codcia
            ...
            BUFFER-COPY Ccbcdocu TO t-cdoc
                ASSIGN
                    /*
                    t-cdoc.ImpIgv = Ccbcdocu.impigv * x-FactorCje
                    t-cdoc.ImpTot = Ccbcdocu.imptot * x-FactorCje
                    t-cdoc.ImpVta = Ccbcdocu.impvta * x-FactorCje
                    */
  ...
  FOR EACH Ccbcdocu NO-LOCK USE-INDEX LLave13 WHERE Ccbcdocu.codcia = s-codcia
    ...
    END.        
  END.
/*  FOR EACH t-dcaja NO-LOCK WHERE t-dcaja.codcia = s-codcia
 *         AND t-dcaja.coddoc = 'I/C'
 *         BREAK BY t-dcaja.coddoc BY t-dcaja.nrodoc:
 *     IF FIRST-OF(t-dcaja.coddoc) AND FIRST-OF(t-dcaja.nrodoc)
 *     THEN DO:
 *         FOR EACH Ccbdmov NO-LOCK WHERE Ccbdmov.codcia = t-dcaja.codcia
 *                 AND Ccbdmov.coddoc = 'N/C'
 *                 AND Ccbdmov.codref = t-dcaja.coddoc
 *                 AND Ccbdmov.nroref = t-dcaja.nrodoc,
 ...
 *                     AND Ccbcdocu.cndcre <> 'N':     /* NO por OTRAS NOTAS DE CREDITO */
 *             x-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'CARGANDO N/C: ' + Ccbcdocu.coddoc + ' ' +
 *                 Ccbcdocu.nrodoc.
 *             FIND FIRST t-cdoc OF Ccbcdocu EXCLUSIVE-LOCK NO-ERROR.
...
FOR EACH t-dcomi NO-LOCK,
        ...
        FIRST Ccbcdocu WHERE Ccbcdocu.codcia = t-dcomi.codcia
            ...
            AND Ccbcdocu.nrodoc = t-dcomi.nrodoc NO-LOCK,
            EACH CcbdDocu NO-LOCK WHERE CcbdDocu.CodCia = CcbcDocu.CodCia
    ...
    FIND FIRST Almmmatg WHERE Almmmatg.CodCia = CcbcDocu.CodCia 
        AND Almmmatg.CodMat = CcbdDocu.CodMat NO-LOCK NO-ERROR.
    FIND FIRST t-cdoc WHERE t-cdoc.CodCia = t-cdoc.codcia
        AND t-cdoc.coddoc = t-dcomi.coddoc
        AND t-cdoc.nrodoc = t-dcomi.nrodoc NO-LOCK NO-ERROR.
    ...
    cRange = "N" + cColumn.
    chWorkSheet:Range(cRange):Value = STRING(ccbcdocu.imptot - CcbcDocu.SdoAct).
    cRange = "O" + cColumn.
    chWorkSheet:Range(cRange):Value = STRING(CcbdDocu.CodMat).   
    cRange = "P" + cColumn.
    chWorkSheet:Range(cRange):Value = Almmmatg.Clase.
    cRange = "Q" + cColumn.
    chWorkSheet:Range(cRange):Value = CcbDDocu.ImpLin.
    cRange = "R" + cColumn.
    chWorkSheet:Range(cRange):Value = STRING(Almmmatg.codfam).    
    cRange = "S" + cColumn.
