...
DEF VAR x-Descrip AS CHAR NO-UNDO.
...
FacTabla.Valor[1] FacTabla.Campo-C[2] ~
fNomCta(FacTabla.Campo-C[2]) @ x-Descrip 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fNomCta B-table-Win 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacTabla.Valor[1] COLUMN-LABEL "Proveedor" FORMAT "99999999":U
            WIDTH 9.43
      FacTabla.Campo-C[2] COLUMN-LABEL "Cuenta" FORMAT "x(10)":U
            WIDTH 9.29
      fNomCta(FacTabla.Campo-C[2]) @ x-Descrip COLUMN-LABEL "Descripción" FORMAT "x(40)":U
  ENABLE
      FacTabla.Campo-C[2]
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 101 BY 8.08
...
"Valor[1]" "Proveedor" "99999999" "decimal" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacTabla.Campo-C[2]
"Campo-C[2]" "Cuenta" "x(10)" "character" ? ? ? ? ? ? yes ? no no "9.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"fNomCta(FacTabla.Campo-C[2]) @ x-Descrip" "Descripción" "x(40)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEFT-MOUSE-DBLCLICK OF FacTabla.Campo-C[2] IN BROWSE br_table /* Cuenta */
OR F8 OF FacTabla.Campo-C[2]
DO:
    DEF VAR ROWID-RETURN AS ROWID.

    RUN cbd/h-ctas02 (s-codcia, OUTPUT ROWID-RETURN).
    IF ROWID-RETURN <> ? THEN DO:
        FIND cb-ctas WHERE ROWID(cb-ctas) = ROWID-RETURN NO-LOCK.
...
ON 'RETURN':U OF FacTabla.Campo-C[2]
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
IF FacTabla.Campo-C[2]:SCREEN-VALUE IN BROWSE {&browse-name} <> '' THEN DO:
    FIND cb-ctas WHERE cb-ctas.codcia = cb-codcia
        AND cb-ctas.codcta = FacTabla.Campo-C[2]:SCREEN-VALUE IN BROWSE {&browse-name}
        NO-LOCK NO-ERROR.
    ...
    IF LENGTH(TRIM(cb-ctas.CodCta)) <>  INTEGER(ENTRY(NUM-ENTRIES(cb-niveles),cb-niveles))
    THEN DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fNomCta B-table-Win 
FUNCTION fNomCta RETURNS CHARACTER
