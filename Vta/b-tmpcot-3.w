...
DEFINE FRAME F-Main
     ...
     RECT-11 AT ROW 7.92 COL 1.57 WIDGET-ID 18
...
DO:
    ...
    F-FACTOR = 1.
    IF s-import-ibc = NO 
        THEN RUN vta/PrecioVenta-2 (s-codcia,
                         s-codcli,
                         s-codmon,
                         f-factor,
                         Almmmatg.codmat,
...
/*       IF AVAILABLE PEDI THEN APPLY 'entry':U TO PEDI.CanPed IN BROWSE {&BROWSE-NAME}. */
/*   END.                                                                                */
/*   ELSE DO:                                                                            */
/*       PEDI.codmat:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO.                            */
/*       IF AVAILABLE PEDI THEN APPLY 'entry':U TO PEDI.Codmat IN BROWSE {&BROWSE-NAME}. */
/*   END.                           */
  IF s-import-ibc = YES THEN DO:
      PEDI.codmat:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES. 
...
FOR EACH B-PEDI:
    ASSIGN
        ...
        F-DSCTOS = PEDI.PorDto.
    FIND Almmmatg WHERE Almmmatg.CodCia = S-CODCIA 
        AND Almmmatg.codmat = B-PEDI.codmat 
        NO-LOCK NO-ERROR.
...
/*     /*MESSAGE 'costo' x-costo SKIP 'margen' x-margen SKIP 'limite' x-limite.*/                        */
