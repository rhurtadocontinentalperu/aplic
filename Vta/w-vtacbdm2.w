...
DEF TEMP-TABLE T-CDOC LIKE CCBCDOCU
    INDEX LLAVE01 Cco FchDoc NroDoc.
...
FOR EACH CcbCDocu WHERE ccbcdocu.codcia = s-codcia
        ...
        AND ccbcdocu.fchdoc >= x-fecini
        AND ccbcdocu.fchdoc <= x-fecfin
        AND ccbcdocu.coddoc = x-coddoc NO-LOCK:
...
FOR EACH T-CDOC BREAK BY T-CDOC.Cco BY T-CDOC.FchDoc BY T-CDOC.NroDoc:
   ...
   FOR EACH CcbDDocu WHERE CcbDDocu.CodCia = s-codcia AND
        CcbDDocu.CodDoc = T-CDOC.coddoc  AND CcbDDocu.NroDoc = T-CDOC.nrodoc NO-LOCK:
        FIND CcbTabla WHERE CcbTabla.CodCia = s-codcia AND
