...
DEFINE TEMP-TABLE T-MATG LIKE INTEGRAL.Almmmatg.
...
T-MATG.DesMar T-MATG.UndBas T-MATG.codfam T-MATG.subfam 
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH T-MATG WHERE TRUE /* Join to almtabla incomplete */ NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      T-MATG.codmat COLUMN-LABEL "Material"
      T-MATG.DesMat
      T-MATG.DesMar COLUMN-LABEL "Marca"
      T-MATG.UndBas COLUMN-LABEL "Unidad"
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.T-MATG WHERE INTEGRAL.almtabla <external> ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > Temp-Tables.T-MATG.codmat
"T-MATG.codmat" "Material" ? "character" ? ? ? ? ? ? yes ?
     _FldNameList[2]   = Temp-Tables.T-MATG.DesMat
     _FldNameList[3]   > Temp-Tables.T-MATG.DesMar
"T-MATG.DesMar" "Marca" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[4]   > Temp-Tables.T-MATG.UndBas
"T-MATG.UndBas" "Unidad" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[5]   > Temp-Tables.T-MATG.codfam
"T-MATG.codfam" "Familia" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[6]   > Temp-Tables.T-MATG.subfam
"T-MATG.subfam" "Sub-Familia" ? "character" ? ? ? ? ? ? no ?
...
ON LEAVE OF T-MATG.codmat IN BROWSE br_table /* Material */
DO:
...
------------------------------------------------------------------------------*/
  FOR EACH T-MATG:
    DELETE T-MATG.
  END.
  IF NOT AVAILABLE Almtabla OR Almtabla.codigo = '' THEN RETURN.
  FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
        AND Almmmatg.codant = almtabla.Codigo:
    CREATE T-MATG.
...
------------------------------------------------------------------------------*/
  ...
  BUFFER-COPY Almmmatg TO T-MATG.
  ASSIGN
    Almmmatg.codant = almtabla.Codigo.
  RELEASE Almmmatg.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAILABLE Almmmatg THEN DO:
    ...
    RETURN 'ADM-ERROR'.
  END.
  IF Almmmatg.codant <> '' AND Almmmatg.codant <> T-MATG.codmat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
  THEN DO:
    MESSAGE 'El material YA está asignado al equivalencias' Almmmatg.codant SKIP
