...
DEFINE VAR F-ESTADO AS CHAR INIT 'P,C,A,J'.
...
AND (x-FchCan-desde = ? OR CcbCDocu.FchCan >= x-FchCan-desde) ~
AND (x-FchCan-hasta = ? OR CcbCDocu.FchCan <= x-FchCan-hasta) ~
...
AND (c-Serie = '' OR CcbcDocu.NroDoc BEGINS c-Serie) ~
...
AND ( TOGGLE-1 = NO OR ccbcdocu.acubon[5] > 0 ) ~
)
...
&Scoped-define FIELDS-IN-QUERY-br_table fEstado() @ X-EST CcbCDocu.CodDoc ~
CcbCDocu.NroDoc CcbCDocu.CodRef CcbCDocu.NroRef CcbCDocu.NomCli ~
CcbCDocu.FmaPgo CcbCDocu.CodVen CcbCDocu.FchDoc CcbCDocu.FchCbd ~
CcbCDocu.FchCan CcbCDocu.FchVto X-MON @ X-MON ~
CcbCDocu.ImpTot - CcbCDocu.AcuBon[5] @ x-ImpSinPer CcbCDocu.AcuBon[5] ~
CcbCDocu.ImpTot CcbCDocu.SdoAct CcbCDocu.ImpDto ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH CcbCDocu WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} USE-INDEX LLAVE10 NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&Scoped-Define ENABLED-OBJECTS RECT-55 RECT-57 FILL-IN-codigo BUTTON-4 ~
B-Imprimir FILL-IN-CodCli CMB-filtro FILL-IN-filtro BUTTON-5 C-CMPBTE ~
x-FchCan-desde x-CodDiv c-Serie F-FMAPGO x-Fchcan-hasta COMBO-BOX-5 f-desde ~
f-hasta xcodmod f-codven x-FchVto-1 x-FchVto-2 TOGGLE-1 br_table BUTTON-1 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-codigo FILL-IN-CodCli CMB-filtro ~
FILL-IN-filtro C-CMPBTE x-CndCre x-FchCan-desde x-CodDiv c-Serie F-FMAPGO ~
F-CndVta x-Fchcan-hasta COMBO-BOX-5 f-desde f-hasta xcodmod f-codven ~
F-nomven x-FchVto-1 x-FchVto-2 TOGGLE-1 f-totsol f-totdol f-sdosol f-sdodol 
...
DEFINE VARIABLE C-CMPBTE AS CHARACTER FORMAT "X(256)":U INITIAL "Todos" 
     LABEL "Tipo Cmpbte." 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "Todos" 
     DROP-DOWN-LIST
...
DEFINE VARIABLE TOGGLE-1 AS LOGICAL INITIAL no 
     LABEL "Solamente con Percepción" 
     VIEW-AS TOGGLE-BOX
     SIZE 22 BY .77 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.NroRef FORMAT "X(12)":U WIDTH 9.72
...
DEFINE FRAME F-Main
     ...
     BUTTON-5 AT ROW 2.62 COL 96 WIDGET-ID 8
     C-CMPBTE AT ROW 3.12 COL 17 COLON-ALIGNED
     x-CndCre AT ROW 3.12 COL 31 NO-LABEL
     x-FchCan-desde AT ROW 3.5 COL 125 COLON-ALIGNED WIDGET-ID 10
     ...
     x-Fchcan-hasta AT ROW 4.46 COL 125 COLON-ALIGNED WIDGET-ID 12
     ...
     TOGGLE-1 AT ROW 6.19 COL 109 WIDGET-ID 6
     br_table AT ROW 7.54 COL 2
     BUTTON-1 AT ROW 17.58 COL 100
     f-totsol AT ROW 17.65 COL 82 RIGHT-ALIGNED NO-LABEL
     f-totdol AT ROW 17.65 COL 84 NO-LABEL
     f-sdosol AT ROW 18.42 COL 68 NO-LABEL
     f-sdodol AT ROW 18.42 COL 84 NO-LABEL
     "Buscar x" VIEW-AS TEXT
          SIZE 7.86 BY .62 AT ROW 1.38 COL 5
          FONT 6
     "Documentos en S/." VIEW-AS TEXT
          SIZE 14 BY .5 AT ROW 17.08 COL 68
     "(*)  Doble Click - Visualiza Detalle" VIEW-AS TEXT
          SIZE 29 BY .73 AT ROW 17.85 COL 1.43
          FONT 6
     "Saldo Total" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 18.42 COL 58
     "Documentos en US$/" VIEW-AS TEXT
          SIZE 15 BY .5 AT ROW 17.08 COL 84
     ...
     "Importe Total" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 17.65 COL 58
     RECT-55 AT ROW 16.88 COL 57
     RECT-57 AT ROW 1 COL 1 WIDGET-ID 4
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "{&CONDICION} USE-INDEX LLAVE10"
...
"CcbCDocu.NroDoc" ? "x(12)" "character" ? ? ? ? ? ? no ? no no "10.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.NroRef" ? ? "character" ? ? ? ? ? ? no ? no no "9.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL TOGGLE-1 L-table-Win
ON VALUE-CHANGED OF TOGGLE-1 IN FRAME F-Main /* Solamente con Percepción */
DO:
    APPLY "VALUE-CHANGED" TO CMB-filtro.
END.
...
&Scoped-define SELF-NAME x-FchCan-desde
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL x-FchCan-desde L-table-Win
ON LEAVE OF x-FchCan-desde IN FRAME F-Main /* Cancelados desde */
DO:
  ASSIGN {&SELF-NAME}.
  APPLY "VALUE-CHANGED" TO CMB-filtro.
END.
...
&Scoped-define SELF-NAME x-Fchcan-hasta
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL x-Fchcan-hasta L-table-Win
ON LEAVE OF x-Fchcan-hasta IN FRAME F-Main /* Cancelados Hasta */
DO:
  ASSIGN {&SELF-NAME}.
  APPLY "VALUE-CHANGED" TO CMB-filtro.
END.
...
FOR EACH CcbCDocu WHERE {&KEY-PHRASE} AND {&CONDICION} USE-INDEX LLAVE10 NO-LOCK:
     x-Factor = 1.
...
chWorkSheet:Range("J2"):Value = "Recepción".
chWorkSheet:Range("K2"):Value = "Vencimiento".
chWorkSheet:Range("L2"):Value = "Cancelación".
chWorkSheet:Range("M2"):Value = "Nro Unico".

CASE xcodmod:
    WHEN 1 THEN DO:
        chWorkSheet:Range("N2"):Value = "Imp. Sin Perc. S/.".
        chWorkSheet:Range("O2"):Value = "Percepción S/.".
    ...
    END.
    WHEN 2 THEN DO:
        chWorkSheet:Range("N2"):Value = "Imp. Sin Perc. US$".
        chWorkSheet:Range("O2"):Value = "Percepción US$".
    ...
    WHEN 3 THEN DO:
        ...
        chWorkSheet:Range("Q2"):Value = "Importe S/. SIN Adelanto".
        chWorkSheet:Range("R2"):Value = "Saldo S/.".
        chWorkSheet:Range("S2"):Value = "Imp. Sin Perc. US$".
        chWorkSheet:Range("T2"):Value = "Percepción US$".
...
FOR EACH t-ccbcdocu:
    ...
    cRange = "M" + cColumn.
    chWorkSheet:Range(cRange):Value = t-ccbcdocu.nrosal.
