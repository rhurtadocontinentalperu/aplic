...
DEFINE TEMP-TABLE tmp-tempo 
    FIELD t-codcia  LIKE EvtLine.Codcia 
    FIELD t-codfam  LIKE EvtLine.CodFam     
    FIELD t-Codano  LIKE EvtLine.Codano
    FIELD t-Codmes  LIKE EvtLine.Codmes
    FIELD t-Coddia  LIKE EvtLine.Codano
...
&Scoped-Define ENABLED-OBJECTS RECT-58 RECT-63 RECT-62 RECT-61 C-tipo ~
C-tipo-2 F-DIVISION BUTTON-5 F-CodFam BUTTON-1 DesdeF HastaF F-DIVISION-1 ~
F-DIVISION-2 F-DIVISION-3 F-DIVISION-4 F-DIVISION-5 F-DIVISION-6 nCodMon ~
Btn_OK Btn_Cancel Btn_Help 
&Scoped-Define DISPLAYED-OBJECTS C-tipo C-tipo-2 F-DIVISION F-CodFam DesdeF ~
HastaF F-DIVISION-1 F-NOMDIV-1 F-DIVISION-2 F-NOMDIV-2 F-NOMDIV-3 ~
...
DEFINE FRAME F-Main
     ...
     F-CodFam AT ROW 3.96 COL 11.57 COLON-ALIGNED
     BUTTON-1 AT ROW 3.96 COL 25.14
     DesdeF AT ROW 4.77 COL 11.72 COLON-ALIGNED
     HastaF AT ROW 4.85 COL 33.43 COLON-ALIGNED
...
FOR EACH EvtLine NO-LOCK WHERE EvtLine.CodCia = S-CODCIA
                         AND   EvtLine.CodDiv = Gn-Divi.Coddiv
                         AND   EvtLine.CodFam BEGINS F-CodFam
                         AND   (EvtLine.Nrofch >= INTEGER(STRING(YEAR(DesdeF),"9999") + STRING(MONTH(DesdeF),"99"))
                         AND   EvtLine.Nrofch <= INTEGER(STRING(YEAR(HastaF),"9999") + STRING(MONTH(HastaF),"99")) ):
                         
      /*
      DISPLAY EvtLine.Coddiv @ Fi-Mensaje LABEL "Codigo de Articulo "
      ...
      */
      ...
      IF EvtLine.Codmes < 12 THEN DO:
        ASSIGN
        X-CODMES = EvtLine.Codmes + 1
        X-CODANO = EvtLine.Codano .
      END.
      ELSE DO: 
        ASSIGN
        X-CODMES = 01
        X-CODANO = EvtLine.Codano + 1 .
      END.
      /**********************************************************************/
      ...
      /*********************** Calculo Para Obtener los datos diarios ************/
       DO I = 1 TO DAY( DATE(STRING(X-CODDIA,"99") + "/" + STRING(X-CODMES,"99") + "/" + STRING(X-CODANO,"9999")) - 1 ) :
            ...
            X-FECHA = DATE(STRING(I,"99") + "/" + STRING(EvtLine.Codmes,"99") + "/" + STRING(EvtLine.Codano,"9999")).
            IF X-FECHA >= DesdeF AND X-FECHA <= HastaF THEN DO:
                FIND Gn-tcmb WHERE Gn-tcmb.Fecha = DATE(STRING(I,"99") + "/" + STRING(EvtLine.Codmes,"99") + "/" + STRING(EvtLine.Codano,"9999")) NO-LOCK NO-ERROR.
                IF AVAILABLE Gn-tcmb THEN DO: 
                    T-Vtamn   = EvtLine.Vtaxdiamn[I] + EvtLine.Vtaxdiame[I] * Gn-Tcmb.Venta.
                    T-Vtame   = EvtLine.Vtaxdiame[I] + EvtLine.Vtaxdiamn[I] / Gn-Tcmb.Compra.
                    T-Ctomn   = EvtLine.Ctoxdiamn[I] + EvtLine.Ctoxdiame[I] * Gn-Tcmb.Venta.
                    T-Ctome   = EvtLine.Ctoxdiame[I] + EvtLine.Ctoxdiamn[I] / Gn-Tcmb.Compra.
                    FIND tmp-tempo WHERE T-Codcia  = S-CODCIA AND
                                         T-CodFam  = EvtLine.CodFam AND
                                         T-Codano  = EvtLine.Codano AND
                                         T-Codmes  = EvtLine.Codmes AND
                                         T-Coddia  = I NO-ERROR.
                    IF NOT AVAIL tmp-tempo THEN DO:
                      CREATE tmp-tempo.
                      ASSIGN T-codcia  = S-CODCIA
                             T-CodFam  = EvtLine.CodFam
                             T-Codano  = EvtLine.Codano 
                             T-Codmes  = EvtLine.Codmes
                             T-Coddia  = I 
                             T-Fchdoc  = X-FECHA.                         
                    END.
                    ...
                    X-ENTRA = FALSE.
                    IF F-DIVISION-1 <> "" AND EvtLine.Coddiv = F-DIVISION-1 THEN DO:
                       ASSIGN T-Venta[1] = T-venta[1] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-2 <> "" AND EvtLine.Coddiv = F-DIVISION-2 THEN DO:
                       ASSIGN T-Venta[2] = T-venta[2] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-3 <> "" AND EvtLine.Coddiv = F-DIVISION-3 THEN DO:
                       ASSIGN T-Venta[3] = T-venta[3] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-4 <> "" AND EvtLine.Coddiv = F-DIVISION-4 THEN DO:
                       ASSIGN T-Venta[4] = T-venta[4] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-5 <> "" AND EvtLine.Coddiv = F-DIVISION-5 THEN DO:
                       ASSIGN T-Venta[5] = T-venta[5] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                              ...
                              X-ENTRA = TRUE.
                    END.       
                    IF F-DIVISION-6 <> "" AND EvtLine.Coddiv = F-DIVISION-6 THEN DO:
...
FOR EACH EvtLine NO-LOCK WHERE EvtLine.CodCia = S-CODCIA
      ...
      /*
      DISPLAY EvtLine.Coddiv @ Fi-Mensaje LABEL "Division"
              FORMAT "X(11)" WITH FRAME F-Proceso.
      */
      T-Vtamn   = 0.
      T-Vtame   = 0.
      ...
      /*********************** Calculo Para Obtener los datos diarios ************/
       DO I = 1 TO DAY( DATE(STRING(X-CODDIA,"99") + "/" + STRING(X-CODMES,"99") + "/" + STRING(X-CODANO,"9999")) - 1 ) :        
            ...
            IF X-FECHA >= DesdeF AND X-FECHA <= HastaF THEN DO:
                FIND Gn-tcmb WHERE Gn-tcmb.Fecha = DATE(STRING(I,"99") + "/" + STRING(EvtLine.Codmes,"99") + "/" + STRING(EvtLine.Codano,"9999")) NO-LOCK NO-ERROR.
                IF AVAILABLE Gn-tcmb THEN DO: 
                 T-Vtamn   = T-Vtamn   + EvtLine.Vtaxdiamn[I] + EvtLine.Vtaxdiame[I] * Gn-Tcmb.Venta.
                 T-Vtame   = T-Vtame   + EvtLine.Vtaxdiame[I] + EvtLine.Vtaxdiamn[I] / Gn-Tcmb.Compra.
                 T-Ctomn   = T-Ctomn   + EvtLine.Ctoxdiamn[I] + EvtLine.Ctoxdiame[I] * Gn-Tcmb.Venta.
                 T-Ctome   = T-Ctome   + EvtLine.Ctoxdiame[I] + EvtLine.Ctoxdiamn[I] / Gn-Tcmb.Compra.
                END.
            END.
       END.         
      ...
      FIND tmp-tempo WHERE t-codcia  = S-CODCIA AND
                           T-CodFam  = EvtLine.CodFam AND
                           T-Codano  = EvtLine.Codano AND
                           T-Codmes  = EvtLine.Codmes NO-ERROR.
      IF NOT AVAIL tmp-tempo THEN DO:
        CREATE tmp-tempo.
        ASSIGN t-codcia  = S-CODCIA
               T-CodFam  = EvtLine.CodFam
...
------------------------------------------------------------------------------*/
  DISPLAY C-tipo C-tipo-2 F-DIVISION F-CodFam DesdeF HastaF F-DIVISION-1 
          F-NOMDIV-1 F-DIVISION-2 F-NOMDIV-2 F-NOMDIV-3 F-DIVISION-3 
          F-DIVISION-4 F-NOMDIV-4 F-DIVISION-5 F-NOMDIV-5 F-DIVISION-6 
          F-NOMDIV-6 nCodMon txt-msj 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-58 RECT-63 RECT-62 RECT-61 C-tipo C-tipo-2 F-DIVISION BUTTON-5 
         F-CodFam BUTTON-1 DesdeF HastaF F-DIVISION-1 F-DIVISION-2 F-DIVISION-3 
  ...
  FOR EACH tmp-tempo  /*WHERE t-venta[10] <> 0 */
      ...
      IF FIRST-OF(t-codfam) THEN DO:
         FIND AlmTfami WHERE AlmTfami.Codcia = S-CODCIA AND
                             AlmTfami.CodFam = T-CodFam
                             NO-LOCK NO-ERROR.
                      
         PUT STREAM REPORT T-CodFam AT 1 .
         IF AVAILABLE AlmTfami THEN DO:
            PUT STREAM REPORT AlmTfami.DesFam AT 5 FORMAT "X(45)" SKIP.
         END.
      ...
      IF LAST-OF(t-codfam) THEN DO:
        ...
        WITH FRAME F-REPORTE.
        DISPLAY STREAM REPORT 
            ("TOTAL : " + STRING(T-CodFam,"999") ) @ t-fchdoc
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH tmp-tempo  /*WHERE t-Marge[10] <> 0 */
      ...
      IF LAST-OF(t-codfam) THEN DO:
        ...
        DISPLAY STREAM REPORT 
            ("TOTAL : " + STRING(T-Codfam,"999") ) @ t-fchdoc
            (ACCUM SUB-TOTAL BY t-codfam t-Marge[1]) @ t-Marge[1]
            (ACCUM SUB-TOTAL BY t-codfam t-Marge[2]) @ t-Marge[2]
            (ACCUM SUB-TOTAL BY t-codfam t-Marge[3]) @ t-Marge[3]
            (ACCUM SUB-TOTAL BY t-codfam t-Marge[4]) @ t-Marge[4]
            (ACCUM SUB-TOTAL BY t-codfam t-Marge[5]) @ t-Marge[5]
            (ACCUM SUB-TOTAL BY t-codfam t-Marge[6]) @ t-Marge[6]
            (ACCUM SUB-TOTAL BY t-codfam t-Marge[7]) @ t-Marge[7]
            (ACCUM SUB-TOTAL BY t-codfam t-Marge[10]) @ t-Marge[10]
            WITH FRAME F-REPORTE.
