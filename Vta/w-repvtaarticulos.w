...
&Scoped-Define ENABLED-OBJECTS BUTTON-6 f-codfam f-subfam BUTTON-7 ~
x-codmat1 x-codmat2 BUTTON-1 BUTTON-2 
&Scoped-Define DISPLAYED-OBJECTS f-codfam f-subfam x-codmat1 x-codmat2 ~
x-mensaje 
...
DEFINE VARIABLE x-codmat1 AS CHARACTER FORMAT "X(6)":U 
     LABEL "Articulo" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.

DEFINE VARIABLE x-codmat2 AS CHARACTER FORMAT "X(6)":U 
     LABEL "Articulo" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     x-codmat1 AT ROW 6.38 COL 19 COLON-ALIGNED WIDGET-ID 10
     x-codmat2 AT ROW 6.38 COL 43 COLON-ALIGNED WIDGET-ID 12
...
DO:
    ASSIGN x-coddiv f-codfam f-subfam x-codmat1 x-codmat2.
    RUN Carga-Datos.
    RUN Excel.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE VARIABLE cCodMat1 AS CHARACTER   NO-UNDO.
    DEFINE VARIABLE cCodMat2 AS CHARACTER   NO-UNDO.
    DEFINE VARIABLE iFecha   AS INTEGER     NO-UNDO.
    
    IF x-codmat1 = "" THEN cCodMat1 = "000000". ELSE cCodMat1 = x-codmat1.
    IF x-codmat2 = "" THEN cCodMat2 = "999999". ELSE cCodMat2 = x-codmat2.
    ...
    FOR EACH evtarti WHERE evtarti.codcia = s-codcia
        ...
        AND evtarti.nrofch >= iFecha         
        AND evtarti.codmat >= cCodMat1 
        AND evtarti.codmat <= cCodMat2  NO-LOCK,
        ...
        FIND FIRST tt-tempo WHERE tt-coddiv = evtarti.coddiv
            ...
            AND tt-codmat = almmmatg.codmat
            /*
            AND tt-codfam = almmmatg.codfam 
            AND tt-subfam = almmmatg.subfam
            AND tt-codmar = almmmatg.codmar
            */  NO-ERROR.
...
------------------------------------------------------------------------------*/
  DISPLAY f-codfam f-subfam x-codmat1 x-codmat2 x-mensaje 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE BUTTON-6 f-codfam f-subfam BUTTON-7 x-codmat1 x-codmat2 BUTTON-1 
         BUTTON-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
chWorkSheet:Range("D2"):Value = "Articulo".
chWorkSheet:Range("E2"):Value = "Descripcion".
chWorkSheet:Range("F2"):Value = "Marca".
chWorkSheet:Range("G2"):Value = "Unidad Base".
chWorkSheet:Range("H2"):Value = "Familia".
chWorkSheet:Range("I2"):Value = "Sub-Familia".
...
FOR EACH tt-tempo NO-LOCK,
    FIRST almmmatg WHERE almmmatg.codcia = s-codcia
        AND almmmatg.codmat = tt-codmat NO-LOCK
    BREAK BY tt-coddiv BY tt-nromes BY tt-codmat:
    t-column = t-column + 1.
