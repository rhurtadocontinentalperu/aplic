...
&Scoped-define FIELDS-IN-QUERY-br_table PEDI2.NroItm PEDI2.codmat ~
Almmmatg.DesMat Almmmatg.DesMar PEDI2.UndVta PEDI2.CanPed PEDI2.PreUni ~
PEDI2.PorDto PEDI2.Por_Dsctos[1] PEDI2.Por_Dsctos[3] PEDI2.ImpLin 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table PEDI2.codmat PEDI2.CanPed ~
PEDI2.PreUni 
&Scoped-define ENABLED-TABLES-IN-QUERY-br_table PEDI2
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br_table PEDI2
&Scoped-define QUERY-STRING-br_table FOR EACH PEDI2 WHERE TRUE /* Join to VtaCDocu incomplete */ NO-LOCK, ~
      EACH Almmmatg OF PEDI2 NO-LOCK ~
    BY PEDI2.NroItm
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH PEDI2 WHERE TRUE /* Join to VtaCDocu incomplete */ NO-LOCK, ~
      EACH Almmmatg OF PEDI2 NO-LOCK ~
    BY PEDI2.NroItm.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      PEDI2.NroItm COLUMN-LABEL "No" FORMAT "ZZ9":U
      PEDI2.codmat COLUMN-LABEL "Articulo" FORMAT "X(6)":U
      Almmmatg.DesMat FORMAT "X(50)":U
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(20)":U
      PEDI2.UndVta COLUMN-LABEL "Unidad" FORMAT "x(10)":U
      PEDI2.CanPed FORMAT ">>>,>>9.99":U
      PEDI2.PreUni COLUMN-LABEL "Precio!Unitario" FORMAT ">>,>>9.9999":U
      PEDI2.PorDto COLUMN-LABEL "% Dscto.!Incluido" FORMAT ">>9.9999":U
      PEDI2.Por_Dsctos[1] COLUMN-LABEL "% Dscto!Manual" FORMAT ">>9.99":U
      PEDI2.Por_Dsctos[3] COLUMN-LABEL "% Dscto!Prom." FORMAT ">>9.99":U
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.PEDI2 Where INTEGRAL.VtaCDocu ...,INTEGRAL.Almmmatg OF Temp-Tables.PEDI2"
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ","
     _OrdList          = "Temp-Tables.PEDI2.NroItm|yes"
     _FldNameList[1]   > Temp-Tables.PEDI2.NroItm
"PEDI2.NroItm" "No" "ZZ9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.PEDI2.codmat
"PEDI2.codmat" "Articulo" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Almmmatg.DesMar" "Marca" "X(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.PEDI2.UndVta
"PEDI2.UndVta" "Unidad" "x(10)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.PEDI2.CanPed
"PEDI2.CanPed" ? ">>>,>>9.99" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.PEDI2.PreUni
"PEDI2.PreUni" "Precio!Unitario" ">>,>>9.9999" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.PEDI2.PorDto
"PEDI2.PorDto" "% Dscto.!Incluido" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.PEDI2.Por_Dsctos[1]
"PEDI2.Por_Dsctos[1]" "% Dscto!Manual" ">>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > Temp-Tables.PEDI2.Por_Dsctos[3]
"PEDI2.Por_Dsctos[3]" "% Dscto!Prom." ">>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > Temp-Tables.PEDI2.ImpLin
"PEDI2.ImpLin" ? ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  DISPLAY I-NroItm @ PEDI2.NroItm WITH BROWSE {&BROWSE-NAME}.
END.
...
&Scoped-define SELF-NAME PEDI2.CanPed
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL PEDI2.CanPed br_table _BROWSE-COLUMN B-table-Win
ON ENTRY OF PEDI2.CanPed IN BROWSE br_table /* Cantidad */
DO:
    IF s-registro-activo = NO THEN RETURN.
    IF PEDI2.codmat:SCREEN-VALUE IN BROWSE {&browse-name} = '' THEN RETURN.
    FIND Almmmate WHERE Almmmate.CodCia = S-CODCIA 
        AND Almmmate.CodAlm = S-CODALM 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL PEDI2.CanPed br_table _BROWSE-COLUMN B-table-Win
ON LEAVE OF PEDI2.CanPed IN BROWSE br_table /* Cantidad */
DO:
    IF s-registro-activo = NO THEN RETURN.
    ...
    IF Almtconv.Multiplos <> 0 THEN DO:
        IF DEC(PEDI2.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) / Almtconv.Multiplos <> 
            INT(DEC(PEDI2.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) / Almtconv.Multiplos) THEN DO:
...
&Scoped-define SELF-NAME PEDI2.PreUni
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL PEDI2.PreUni br_table _BROWSE-COLUMN B-table-Win
ON LEAVE OF PEDI2.PreUni IN BROWSE br_table /* Precio!Unitario */
DO:
    IF s-registro-activo = NO THEN RETURN.
    IF F-PREVTA > DEC(PEDI2.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) THEN DO:
        MESSAGE "No Autorizado para modificar Precio " VIEW-AS ALERT-BOX ERROR.
        DISPLAY F-PREVTA @ PEDI2.PreUni WITH BROWSE {&BROWSE-NAME}.
        RETURN NO-APPLY.   
    END.
END.
...
ON "RETURN":U OF PEDI2.codmat, PEDI2.UndVta ,PEDI2.CanPed ,PEDI2.PorDto ,PEDI2.PreUni
DO:
  IF PEDI2.Codmat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = "" THEN DO:
       APPLY "ENTRY" TO PEDI2.CodMat IN BROWSE {&BROWSE-NAME}.
       RETURN NO-APPLY.
  END.
...
FOR EACH PEDI2:
    ...
    IF ABSOLUTE(PEDI2.PreUni - f-PreVta) / f-PreVta * 100 > 0.25 THEN DO:
        CREATE T-DPEDI2.
        BUFFER-COPY PEDI2 TO T-DPEDI2
            ASSIGN
                T-DPEDI2.Libre_d01 = f-PreVta.
    END.
...
------------------------------------------------------------------------------*/
  ...
  FIND Almmmatg WHERE Almmmatg.CodCia = S-CODCIA 
      AND Almmmatg.codmat = PEDI2.codmat 
      NO-LOCK NO-ERROR.
  ASSIGN 
    PEDI2.UndVta = PEDI2.UndVta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
    PEDI2.PreBas = F-PreBas 
    PEDI2.AftIgv = Almmmatg.AftIgv 
    PEDI2.AftIsc = Almmmatg.AftIsc 
    PEDI2.PreUni = DEC(PEDI2.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME})
    PEDI2.PorDto = DEC(PEDI2.PorDto:SCREEN-VALUE IN BROWSE {&BROWSE-NAME})
  ...
  IF PEDI2.AftIgv 
  THEN PEDI2.ImpIgv = PEDI2.ImpLin - ROUND( PEDI2.ImpLin  / ( 1 + (s-PorIgv / 100) ), 4 ).
...
/*       PEDI2.codmat:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES.                           */
/*       IF AVAILABLE PEDI2 THEN APPLY 'entry':U TO PEDI2.CanPed IN BROWSE {&BROWSE-NAME}. */
/*   END.                                                                                */
/*   ELSE DO:                                                                            */
/*       PEDI2.codmat:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO.                            */
/*       IF AVAILABLE PEDI2 THEN APPLY 'entry':U TO PEDI2.Codmat IN BROWSE {&BROWSE-NAME}. */
/*   END.                           */
  IF s-import-ibc = YES THEN DO:
      PEDI2.codmat:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES. 
      PEDI2.PreUni:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES.
  END.
  ELSE DO:
      PEDI2.codmat:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO. 
      PEDI2.PreUni:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO.
  END.
...
-------------------------------------------------------------*/
  ...
  END CASE.
  IF p-state = 'update-begin':U THEN APPLY 'ENTRY':U TO PEDI2.codmat IN BROWSE {&browse-name}.
END PROCEDURE.
...
IF INTEGER(PEDI2.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) = 0 THEN DO:
...
END.
  ...
  x-PreUni = DECIMAL ( PEDI2.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} ) *
      ( 1 - DECIMAL (PEDI2.Por_Dsctos[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} ) / 100 ) *
      /*( 1 - PEDI2.Por_Dsctos[2] / 100 ) **/
      ( 1 - DECIMAL (PEDI2.Por_Dsctos[3]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} ) / 100 ) .
  RUN vtagn/p-margen-utilidad (
      PEDI2.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME},      /* Producto */
      x-PreUni,  /* Precio de venta unitario */
      PEDI2.UndVta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME},
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
IF NOT AVAILABLE PEDI2 THEN RETURN "ADM-ERROR".
