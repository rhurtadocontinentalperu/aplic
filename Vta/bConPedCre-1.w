...
_FlgEst(CcbCDocu.FlgEst) @ x-Estado 
...
&Scoped-define QUERY-STRING-br_table FOR EACH CcbCDocu WHERE CcbCDocu.CodCia = FacCPedi.CodCia ~
  ...
  OR CcbCDocu.CodDoc = "BOL" OR CcbCDocu.CodDoc = "G/R") ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.FchDoc FORMAT "99/99/99":U WIDTH 6.43
      CcbCDocu.ImpTot FORMAT "->>,>>>,>>9.99":U WIDTH 10.43
      CcbCDocu.CodRef COLUMN-LABEL "Ref." FORMAT "x(3)":U WIDTH 4
      CcbCDocu.NroRef FORMAT "X(12)":U WIDTH 8.43
      _FlgEst(CcbCDocu.FlgEst) @ x-Estado COLUMN-LABEL "Estado" FORMAT "x(10)":U
            WIDTH 10.43
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 62 BY 5.65
         FONT 4
         TITLE "COMPROBANTES POR PEDIDO".
...
"CcbCDocu.FchDoc" ? "99/99/99" "date" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.CcbCDocu.ImpTot
"CcbCDocu.ImpTot" ? ? "decimal" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.CcbCDocu.CodRef
"CcbCDocu.CodRef" "Ref." ? "character" ? ? ? ? ? ? no ? no no "4" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.CcbCDocu.NroRef
"CcbCDocu.NroRef" ? ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > "_<CALC>"
"_FlgEst(CcbCDocu.FlgEst) @ x-Estado" "Estado" "x(10)" ? ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEFT-MOUSE-DBLCLICK OF br_table IN FRAME F-Main /* COMPROBANTES POR PEDIDO */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-ENTRY OF br_table IN FRAME F-Main /* COMPROBANTES POR PEDIDO */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* COMPROBANTES POR PEDIDO */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* COMPROBANTES POR PEDIDO */
DO:
...
RUN gn/fFlgEstCCB(pFlgEst, OUTPUT pEstado).
