...
DEFINE VAR X-VALSOL  AS DECIMAL.
DEFINE VAR X-VALDOL  AS DECIMAL.
...
&Scoped-Define ENABLED-OBJECTS RECT-28 F-PREUNI 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-2 FILL-IN-3 F-PREUNI 
...
DEFINE VARIABLE F-PREUNI AS DECIMAL FORMAT "->>,>>>,>>9.9999":U INITIAL 0 
     LABEL "Precio" 
     VIEW-AS FILL-IN 
     SIZE 12.29 BY .81
     BGCOLOR 15  NO-UNDO.
...
DEFINE FRAME D-Dialog
     ...
     F-PREUNI AT ROW 2.88 COL 11.14 COLON-ALIGNED
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Precios */
DO:  
...
&Scoped-define SELF-NAME F-PREUNI
...
/***************/
    ...
    IF S-CODMON = 1 THEN
      ASSIGN Pre1 = X-NEWPRE
             ...
             X-VALSOL = X-VALVTA
             X-VALDOL = X-VALVTA / X-TpoCmb.
    ELSE
      ASSIGN Pre2 = X-NEWPRE
             Pre1 = ROUND(X-NEWPRE * X-TpoCmb,6)
             X-VALDOL = X-VALVTA
             X-VALSOL = X-VALVTA * X-TpoCmb.

    X-NEWPRE = IF X-MonVta = 1 THEN Pre1
                               ELSE Pre2.

    IF S-CODMON = 1 THEN
        X-NEWDSC = ROUND((1 - (X-NEWPRE / F-FACTOR) / X-VALSOL) * 100, 2).
    ELSE
        X-NEWDSC = ROUND((1 - (X-NEWPRE / F-FACTOR) / X-VALDOL) * 100, 2).
