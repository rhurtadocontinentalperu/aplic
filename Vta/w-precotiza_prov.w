...
DEFINE VARIABLE h_b-precotiza_prov AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv06 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-pedido2x AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-precotiza_prov AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-precotiza AS HANDLE NO-UNDO.
...
DO:
    RUN Asigna-Cotizacion IN h_v-precotiza.  
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-precotiza.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-precotiza ).
       RUN set-position IN h_v-precotiza ( 1.27 , 2.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-precotiza. */
       RUN add-link IN adm-broker-hdl ( h_p-updv06 , 'TableIO':U , h_v-precotiza ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido2x , 'Record':U , h_v-precotiza ).

       /* Links to SmartQuery h_q-pedido2x. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido2x ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-precotiza ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-precotiza_prov.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-precotiza_prov ).
       RUN set-position IN h_b-precotiza_prov ( 11.62 , 2.72 ) NO-ERROR.
       RUN set-size IN h_b-precotiza_prov ( 13.35 , 133.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-precotiza_prov. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido2x , 'Record':U , h_b-precotiza_prov ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-precotiza_prov ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  't-precotiza_prov.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-precotiza_prov ).
       RUN set-position IN h_t-precotiza_prov ( 11.65 , 1.00 ) NO-ERROR.
       RUN set-size IN h_t-precotiza_prov ( 13.35 , 137.86 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-precotiza_prov ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_t-precotiza_prov <> ? THEN RUN dispatch IN h_t-precotiza_prov ('open-query':U).
      END.
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
         RUN select-page(1).
         BUTTON-2:SENSITIVE = YES.
         BUTTON-6:SENSITIVE = YES.
         RUN adm-open-query IN h_b-precotiza_prov.
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        RUN Recalcular-Precios IN h_t-precotiza_prov.
      END.
