...
ON LEAVE OF F-DSCTO IN FRAME D-Dialog /* % Dscto. */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-DSCTO D-Dialog
ON RETURN OF F-DSCTO IN FRAME D-Dialog /* % Dscto. */
DO:
  ...
  CASE X-NIV:
    WHEN "D1" THEN DO:
        IF F-DSCTO > FacCfgGn.DtoMax THEN DO:
            MESSAGE "Maximo DSCTO.  permitido" SKIP
                    "para el nivle de Usuario" SKIP
                    TRIM(X-NIV) " : " almtabla.Nombre " es : " FacCfgGn.DtoMax " %" SKIP
    ...
    WHEN "D2" THEN
        IF F-DSCTO > FacCfgGn.DtoDis THEN DO:
            MESSAGE "Maximo DSCTO.  permitido" SKIP
                    "para el nivle de Usuario" SKIP
                    TRIM(X-NIV) " : " almtabla.Nombre " es : " FacCfgGn.DtoDis " %" SKIP
                    VIEW-AS ALERT-BOX ERROR TITLE "Descuento por Item".
            APPLY "ENTRY" TO F-DSCTO IN FRAME {&FRAME-NAME}.
            RETURN NO-APPLY.
        END.
    WHEN "D3" THEN
        IF F-DSCTO > FacCfgGn.DtoMay THEN DO:
            MESSAGE "Maximo DSCTO.  permitido" SKIP
                    "para el nivle de Usuario" SKIP
                    TRIM(X-NIV) " : " almtabla.Nombre " es : " FacCfgGn.DtoMay " %"  SKIP
                    VIEW-AS ALERT-BOX ERROR TITLE "Descuento por Item".
            APPLY "ENTRY" TO F-DSCTO IN FRAME {&FRAME-NAME}.
            RETURN NO-APPLY.
        END.
    OTHERWISE DO:
            MESSAGE "Nivel de Usuario no existe : " X-NIV
  ...
  dsct_disp = ROUND(((1 - ((1 - X-MAXDSC / 100) / (1 - DEC(FILL-IN-3:SCREEN-VALUE) / 100))) * 100), 2).

  X-NEWDSC = X-NEWDSC + F-DSCTO.
  IF X-NEWDSC > dsct_disp /*X-MAXDSC*/ THEN DO:
    MESSAGE "Se ha excedido DSCTO. del Articulo" SKIP
            "Dscto. posible : " dsct_disp /*X-MAXDSC*/ " %" SKIP
...
/*********/
    ...
    X-NEWPRE = ((X-VALVTA - (X-VALVTA * (X-NEWDSC / 100) )) * F-FACTOR).
    ...
    X-NEWPRE = IF S-CODMON = 1 THEN Pre1
                               ELSE Pre2.
/*********/
  APPLY "END-ERROR":U TO SELF.
  RETURN NO-APPLY.
