...
IF NOT AVAILABLE Almtdocm THEN DO:
   MESSAGE "No existen movimientos asignados al almacén" VIEW-AS ALERT-BOX ERROR.
   RETURN ERROR.
END.

ASSIGN
    S-CODMOV = INTEGER(c-CodMov).
...
DEFINE VARIABLE h_q-dev-fai AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-dev-fai AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-dev-fai AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Devolucion Mercaderia por FAI (Cliente)"
         HEIGHT             = 18.19
         WIDTH              = 88.14
         MAX-HEIGHT         = 20.35
         MAX-WIDTH          = 99
...
ON END-ERROR OF W-Win /* Devolucion Mercaderia por FAI (Cliente) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Devolucion Mercaderia por FAI (Cliente) */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'Vta/v-dev-fai.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-dev-fai ).
       RUN set-position IN h_v-dev-fai ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.85 , 84.43 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'Vta/q-dev-fai.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-dev-fai ).
       RUN set-position IN h_q-dev-fai ( 1.00 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 15.00 ) */

       /* Links to SmartViewer h_v-dev-fai. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-dev-fai ).
       RUN add-link IN adm-broker-hdl ( h_q-dev-fai , 'Record':U , h_v-dev-fai ).

       /* Links to SmartQuery h_q-dev-fai. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-dev-fai ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_p-updv05 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-dev-fai ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-dev-fai ,
             h_v-dev-fai , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-devolu ).
       RUN set-position IN h_b-devolu ( 7.73 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-devolu ( 11.42 , 85.72 ) NO-ERROR.

       /* Links to SmartBrowser h_b-devolu. */
       RUN add-link IN adm-broker-hdl ( h_q-dev-fai , 'Record':U , h_b-devolu ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'Vta/t-dev-fai.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-dev-fai ).
       RUN set-position IN h_t-dev-fai ( 7.73 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-dev-fai ( 11.31 , 86.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-dev-fai ,
             h_v-dev-fai , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
         ...
         RUN dispatch IN h_t-dev-fai ('open-query':U).
      END.
