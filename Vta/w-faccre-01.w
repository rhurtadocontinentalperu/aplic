...
DEFINE VARIABLE h_v-faccre-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     C-CodDoc AT ROW 7.08 COL 2 NO-LABEL
     C-NroSer AT ROW 7.08 COL 9.57 NO-LABEL
     Btn-Print AT ROW 7.23 COL 80.86
     Btn-Guias AT ROW 8.54 COL 81
     Btn-Pedido AT ROW 9.62 COL 81
     BUTTON-1 AT ROW 17.54 COL 80
     RECT-20 AT ROW 6.96 COL 80.29
...
DO:
  RUN Asigna-Guias IN h_v-faccre-01.
END.
...
DO:
    RUN Asigna-Pedidos IN h_v-faccre-01.
END.
...
DO:
  RUN dispatch IN h_v-faccre-01 ('imprime':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta/v-faccre-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-faccre-01 ).
       RUN set-position IN h_v-faccre-01 ( 1.00 , 1.72 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 6.96 , 26.14 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv10 ).
       RUN set-position IN h_p-updv10 ( 6.96 , 42.72 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-docmto ).
       RUN set-position IN h_q-docmto ( 6.96 , 16.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-faccre-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-faccre-01 ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-faccre-01 ).

       /* Links to SmartQuery h_q-docmto. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-docmto ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-faccre-01 ,
             C-CodDoc:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-faccre-01 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-docmto ).
       RUN set-position IN h_b-docmto ( 8.35 , 1.43 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpdoc ).
       RUN set-position IN h_b-tmpdoc ( 8.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpdoc ( 9.31 , 78.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-20 C-CodDoc C-NroSer Btn-Print Btn-Guias Btn-Pedido BUTTON-1 
      WITH FRAME F-Main IN WINDOW W-Win.
