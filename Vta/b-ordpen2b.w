...
&Scoped-define INTERNAL-TABLES Ccbcdocu Almacen

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table Ccbcdocu.CodAlm Ccbcdocu.CodDoc ~
CcbCDocu.NroDoc Ccbcdocu.NroPed Ccbcdocu.NomCli Ccbcdocu.FchDoc ~
fGetCurr(Ccbcdocu.CodMon) @ cCurrency 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH Ccbcdocu WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK, ~
      EACH Almacen OF Ccbcdocu ~
      WHERE Almacen.CodDiv = s-coddiv NO-LOCK ~
    BY Ccbcdocu.FchDoc DESCENDING ~
       BY Ccbcdocu.NroPed DESCENDING
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Ccbcdocu WHERE ~{&KEY-PHRASE} ~
    ...
    BY Ccbcdocu.FchDoc DESCENDING ~
       BY Ccbcdocu.NroPed DESCENDING.
&Scoped-define TABLES-IN-QUERY-br_table Ccbcdocu Almacen
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Ccbcdocu.CodAlm FORMAT "x(3)":U
      Ccbcdocu.CodDoc COLUMN-LABEL "Doc" FORMAT "x(3)":U WIDTH 4
      CcbCDocu.NroDoc FORMAT "X(9)":U WIDTH 9.57
      Ccbcdocu.NroPed FORMAT "X(9)":U WIDTH 9.57 COLUMN-FONT 0
      Ccbcdocu.NomCli COLUMN-LABEL "Nombre del Cliente" FORMAT "x(45)":U
            WIDTH 44
      Ccbcdocu.FchDoc COLUMN-LABEL "Fecha!Emisión" FORMAT "99/99/99":U
      fGetCurr(Ccbcdocu.CodMon) @ cCurrency COLUMN-LABEL "Mon" FORMAT "x(3)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 90 BY 7.81
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     "Comprobantes" VIEW-AS TEXT
          SIZE 11 BY .62 AT ROW 1 COL 5 WIDGET-ID 4
          FGCOLOR 9 
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Ccbcdocu,INTEGRAL.Almacen OF INTEGRAL.Ccbcdocu"
     _Options          = "NO-LOCK KEY-PHRASE"
     _OrdList          = "INTEGRAL.Ccbcdocu.FchDoc|no,INTEGRAL.Ccbcdocu.NroPed|no"
     _Where[1]         = "{&CONDICION}"
     _Where[2]         = "Almacen.CodDiv = s-coddiv"
     _FldNameList[1]   = INTEGRAL.Ccbcdocu.CodAlm
     _FldNameList[2]   > INTEGRAL.Ccbcdocu.CodDoc
"Ccbcdocu.CodDoc" "Doc" ? "character" ? ? ? ? ? ? no ? no no "4" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.CcbCDocu.NroDoc
"CcbCDocu.NroDoc" ? ? "character" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.Ccbcdocu.NroPed
"Ccbcdocu.NroPed" ? ? "character" ? ? 0 ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.Ccbcdocu.NomCli
"Ccbcdocu.NomCli" "Nombre del Cliente" "x(45)" "character" ? ? ? ? ? ? no ? no no "44" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.Ccbcdocu.FchDoc
"Ccbcdocu.FchDoc" "Fecha!Emisión" "99/99/99" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
    ...
    IF NOT AVAILABLE Ccbcdocu OR
        ...
        RETURN.
    END.
    RUN vta\gGenGuia2b (ROWID(Ccbcdocu), 'A', 'G/R').       /* GUIA AUTOMATICA */
