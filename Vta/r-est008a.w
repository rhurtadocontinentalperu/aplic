...
DEF BUFFER B-CVTA  FOR dwh_ventas_cab.
DEF BUFFER B-DVTA  FOR dwh_ventas_det.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga_Tempo W-Win 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE VARIABLE dFechaI AS INTEGER     NO-UNDO.
DEFINE VARIABLE dFechaF AS INTEGER     NO-UNDO.
DEFINE VARIABLE fFecha  AS DATE        NO-UNDO.

dFechaI = (YEAR(x-CodFchI) * 10000) + (MONTH(x-CodFchI) * 100) + DAY(x-CodFchI).
dFechaF = (YEAR(x-CodFchF) * 10000) + (MONTH(x-CodFchF) * 100) + DAY(x-CodFchF).
...
FOR EACH Gn-Divi WHERE Gn-Divi.Codcia = S-CodCia NO-LOCK :
    FOR EACH dwh_ventas_cab WHERE dwh_ventas_cab.CodCia = s-codcia
        ...
        AND dwh_ventas_cab.Fecha  >= dFechaI
        AND dwh_ventas_cab.Fecha  <= dFechaF NO-LOCK
        BREAK BY dwh_ventas_cab.CodCia 
              BY dwh_ventas_cab.CodDiv 
              BY dwh_ventas_cab.Fecha :

        ASSIGN 
            pCodDiv = dwh_ventas_cab.CodDiv
            pCanalVenta = GN-DIVI.CanalVenta.
        
        IF dwh_ventas_cab.coddiv <> '00017' 
            AND dwh_ventas_cab.codven = '151' THEN pCodDiv = '00017'.   /* Supermercados */
        IF dwh_ventas_cab.coddiv <> '00018' 
            AND LOOKUP(dwh_ventas_cab.codven, '015,173,900,901,902,017') > 0 THEN pCodDiv = '00018'.    /* Provincias */
        IF dwh_ventas_cab.codcli = '20511358907' THEN pCodDiv = '00022'.  /* STANDFORD */
        IF dwh_ventas_cab.coddiv <> '00019' 
            AND dwh_ventas_cab.codven = '081' THEN pCodDiv = '00019'.   /* Mesa redonda */

        IF LOOKUP(dwh_ventas_cab.CodDoc,"TCK,FAC,BOL,N/C,N/D") = 0 THEN NEXT.

        fill-in-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "** PROCESANDO  " + STRING(dwh_ventas_cab.fecha) + ' ' +
        ...
        ASSIGN
            x-signo1 = ( IF dwh_ventas_cab.Coddoc = "N/C" THEN -1 ELSE 1 )
            x-Day   = INT(SUBSTRING(STRING(dwh_ventas_cab.fecha),7))
            x-Month = INT(SUBSTRING(STRING(dwh_ventas_cab.fecha),5,6))
            x-Year  = INT(SUBSTRING(STRING(dwh_ventas_cab.fecha),1,4))
            x-ImpTot = dwh_ventas_cab.ImpNacCIGV.     /* <<< OJO <<< */

        /* buscamos si hay una aplicación de fact adelantada */
        FIND FIRST dwh_ventas_det OF dwh_ventas_cab WHERE dwh_ventas_det.ImpNacCIGV < 0 NO-LOCK NO-ERROR.
        IF AVAILABLE dwh_ventas_det THEN x-ImpTot = x-ImpTot + ABSOLUTE(dwh_ventas_det.ImpNacCIGV).
        /* ************************************************* */

        /* VARIABLES DE VENTAS */
        ASSIGN
            x-codven   = dwh_ventas_cab.CodVen
            X-FMAPGO   = dwh_ventas_cab.FmaPgo
            /*
            x-NroCard  = dwh_ventas_cab.nrocard
            x-Sede     = dwh_ventas_cab.Sede 
            */
            x-Canal    = ''
            x-CodCli   = dwh_ventas_cab.CodCli
            x-CodUnico = dwh_ventas_cab.codcli
            x-Zona     = ''.
        ...
        /* EN CASO DE SER UN CLIENTE VARIOS */
        IF dwh_ventas_cab.codcli = s-CliVar THEN DO:
            ASSIGN
                x-CodCli = s-CliVar
                x-CodUnico = s-CliVar.
            IF x-NroCard <> '' THEN DO:
        ...
        FOR EACH dwh_ventas_det OF dwh_ventas_cab NO-LOCK WHERE dwh_ventas_det.CodMat = x-CodMat:
            /* ****************** Filtros ************************* */
            FIND Almmmatg WHERE Almmmatg.Codcia = dwh_ventas_det.Codcia 
                AND Almmmatg.CodMat = dwh_ventas_det.CodMat NO-LOCK NO-ERROR.
            IF NOT AVAILABLE Almmmatg THEN NEXT.
/*             IF dwh_ventas_det.ImpNacCIGV <= 0 THEN NEXT.       /* <<< OJO <<< */ */
            /* **************************************************** */
            FIND Almtconv WHERE Almtconv.CodUnid  = Almmmatg.UndBas /* 
                AND Almtconv.Codalter = dwh_ventas_det.UndVta*/  NO-LOCK NO-ERROR.
            F-FACTOR  = 1. 
            IF AVAILABLE Almtconv THEN DO:
                F-FACTOR = Almtconv.Equival.
                IF Almmmatg.FacEqu <> 0 THEN F-FACTOR = Almtconv.Equival / Almmmatg.FacEqu.
            ...
            CREATE detalle.
            ASSIGN
                detalle.coddiv = estavtas.dwh_ventas_cab.CodDiv
                detalle.coddoc = estavtas.dwh_ventas_cab.CodDoc
                detalle.nrodoc = estavtas.dwh_ventas_cab.NroDoc
                detalle.fchdoc = estavtas.dwh_ventas_cab.Fecha
/*                 detalle.codref = Ccbcdocu.CodRef */
/*                 detalle.nroref = Ccbcdocu.NroRef */
                detalle.codcli = x-CodCli
                detalle.codven = x-CodVen
                detalle.VtaMn  = estavtas.dwh_ventas_det.ImpNacCIGV
                detalle.VtaMe  = estavtas.dwh_ventas_det.ImpExtCIGV.
                detalle.Cantidad =  x-signo1 * estavtas.dwh_ventas_det.Cantidad * F-FACTOR * x-can.
...
/*         STRING(detalle.fchdoc, '99/99/9999') + '|' + */
        STRING(detalle.fchdoc, '99999999') + '|' +
        detalle.codref + '|' + 
        detalle.nroref + '|'.
