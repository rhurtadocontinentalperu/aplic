  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    FOR EACH CcbDDocu NO-LOCK WHERE CcbDDocu.CodCia = CcbCDocu.CodCia 
        ...
        IF (CcbDDocu.CanDes - CcbDDocu.CanDev) > 0 THEN DO:
            ...
            x-PreUni = CcbDDocu.cImporteTotalConImpuesto / CcbDDocu.CanDes.
            ASSIGN
                DMOV.PreUni = x-PreUni
                DMOV.ImpLin = x-PreUni * DMOV.CanDes.
            IF DMOV.AftIgv = YES THEN DMOV.ImpIgv = Ccbddocu.ImporteIGV / Ccbddocu.CanDes * DMOV.CanDes.
/*             IF DMOV.CanDes = Ccbddocu.CanDes THEN DO:   /* DEVOLUCION TOTAL */                                                    */
/*                 ASSIGN                                                                                                            */
/*                     DMOV.ImpLin = x-ImpLin      /*Ccbddocu.ImpLin - Ccbddocu.ImpDto2 */                                           */
/*                     DMOV.PreUni = x-ImpLin / DMOV.CanDes.                                                                         */
/*             END.                                                                                                                  */
/*             ELSE DO:    /* DEVOLUCION PARCIAL */                                                                                  */
/*                 ASSIGN                                                                                                            */
/*                     DMOV.PreUni = x-ImpLin / Ccbddocu.CanDes                                                                      */
...
/*    END.                                                                                */
   ...
   END.
   IF C-NROFAC = "" THEN DO:
      MESSAGE "No asignó ninguna Factura" VIEW-AS ALERT-BOX ERROR.
      APPLY "ENTRY" TO Almcmov.CodCli.
      RETURN "ADM-ERROR".   
   END.
...
/*                 "Imposible realizar la devolución"                                           */
/*            VIEW-AS ALERT-BOX INFORMATION.                                                    */
/*        RETURN "ADM-ERROR".                                                                   */
/*    END.                                                                                      */
