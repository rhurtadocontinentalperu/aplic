...
DEFINE VARIABLE h_bconprecre-3 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_bconcotcre ( 11.00 , 124.00 ) NO-ERROR.
       ...
       RUN set-position IN h_bconcotcre-1 ( 12.04 , 7.00 ) NO-ERROR.
       RUN set-size IN h_bconcotcre-1 ( 5.65 , 79.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'src/adm/objects/folder.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Comprobantes|Tracking|Hojas de Ruta' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 17.96 , 3.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 9.96 , 109.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_bconpedcre-1-2 ( 19.58 , 8.00 ) NO-ERROR.
       RUN set-size IN h_bconpedcre-1-2 ( 7.00 , 76.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_bconpedcre-2-2 ( 19.31 , 7.00 ) NO-ERROR.
       RUN set-size IN h_bconpedcre-2-2 ( 8.08 , 100.00 ) NO-ERROR.
    ...
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/bconprecre-3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bconprecre-3 ).
       RUN set-position IN h_bconprecre-3 ( 19.85 , 6.00 ) NO-ERROR.
       RUN set-size IN h_bconprecre-3 ( 6.69 , 66.00 ) NO-ERROR.

       /* Links to SmartBrowser h_bconprecre-3. */
       RUN add-link IN adm-broker-hdl ( h_bconcotcre , 'Record':U , h_bconprecre-3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bconprecre-3 ,
             h_folder , 'AFTER':U ).
    END. /* Page 3 */
