...
&Scoped-define FIELDS-IN-QUERY-br_table DMOV.codmat Almmmatg.DesMat DMOV.CodUnd Almmmatg.DesMar DMOV.CanDes   
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 75.14 BY 7.42
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     FILL-IN-Total AT ROW 8.5 COL 46 COLON-ALIGNED
...
------------------------------------------------------------------------------*/
FILL-IN-Total = 0.
FOR EACH B-DMOV NO-LOCK:
    FILL-IN-Total = FILL-IN-Total + B-DMOV.ImpLin.
END.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  ASSIGN DMOV.Factor = F-Equivale
         DMOV.CodCia = S-CODCIA
         DMOV.CODALM = S-CODALM.
  FIND Almmmatg WHERE 
       Almmmatg.CodCia = S-CODCIA AND
       Almmmatg.codmat = DMOV.codmat NO-LOCK NO-ERROR.
  IF AVAILABLE Almmmatg THEN DO:
     ASSIGN DMOV.Pesmat = Almmmatg.Pesmat * DMOV.Candes * DMOV.Factor
...
------------------------------------------------------------------------------*/
IF DMOV.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = "" THEN DO:
...
IF DECIMAL(DMOV.CanDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > DMOV.CanDev THEN DO:
   MESSAGE "Cantidad no debe ser mayor a " DMOV.CanDev VIEW-AS ALERT-BOX ERROR.
   APPLY "ENTRY":U TO DMOV.CanDes.
   RETURN "ADM-ERROR".   
END.
FIND B-DMOV WHERE B-DMOV.Codcia  = S-CODCIA AND
     B-DMOV.CodAlm = S-CODALM AND
     B-DMOV.CodMat = DMOV.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
     NO-LOCK NO-ERROR.
IF AVAILABLE B-DMOV AND ROWID(B-DMOV) <> ROWID(DMOV) THEN DO:
...
FIND Almmmate WHERE 
     ...
     Almmmate.CodMat = DMOV.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
     NO-LOCK NO-ERROR.
IF NOT AVAILABLE Almmmate THEN DO:
   MESSAGE "Articulo no asignado al almacen " S-CodAlm VIEW-AS ALERT-BOX ERROR.
   APPLY "ENTRY":U TO DMOV.CodMat.
   RETURN "ADM-ERROR".   
END.
IF (Almmmate.StkAct - DECIMAL(DMOV.CanDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) ) < 0 THEN DO:
