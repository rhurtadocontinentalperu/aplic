...
ON "ANY-PRINTABLE":U OF PEDI.PorDto
DO:
  IF NOT x-password THEN RETURN NO-APPLY.
END.

ON "F3":U OF PEDI.PorDto
DO:
  ...
  IF x-password = TRUE THEN DO:
    RUN VTA/D-DSCTO1.R(DEC(PEDI.PorDto:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}),
                      DEC(PEDI.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}),
                      Almmmatg.Chr__02,
                      Almmmatg.UndBas,
                      Almmmatg.Chr__01,
                      Almmmatg.PorMax,
     ...
     PEDI.PorDto:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = STRING(X-NEWDSC).
     PEDI.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = STRING(X-NEWPRE).
  END.
  x-password = FALSE.
  APPLY "ENTRY" TO PEDI.PorDto IN BROWSE {&BROWSE-NAME}.
  RETURN NO-APPLY.
END.
...
ON "ANY-PRINTABLE":U OF PEDI.PreUni
DO:
  IF NOT x-password THEN RETURN NO-APPLY.
END.

ON "F3":U OF PEDI.PreUni
DO:
  DEFINE VAR RPTA AS CHAR.
  DEFINE VAR NIV  AS CHAR.
  DEFINE VAR X-MAXDSC AS DEC.
  ...
  END.
  IF x-password = TRUE THEN DO:
    RUN VTA/D-PREUNI.R(DEC(PEDI.PorDto:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}),
                      DEC(PEDI.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}),
                      Almmmatg.UndBas,
                      output-var-2,
                      Almmmatg.PorMax,
  ...
  END.
  x-password = FALSE.
  APPLY "ENTRY" TO PEDI.PreUni IN BROWSE {&BROWSE-NAME}.
  RETURN NO-APPLY.
END.
...
/*  *******************************************
   ...
   IF Almmmatg.AftIgv THEN F-PorImp = IF S-CODIGV = 1 THEN 1 ELSE (1 / (F-PorImp * (1 + FacCfgGn.PorIgv / 100))).

/*   IF S-CODMON = 1 THEN DO:
 *       IF Almmmatg.MonVta = 1 THEN 
 *         ASSIGN F-PREBAS = Almmmatg.PreBas * F-PorImp * F-FACTOR.
 *       ELSE 
 *         ASSIGN F-PREBAS = ROUND(Almmmatg.PreBas * F-PorImp,2) * FacCfgGn.Tpocmb[1] * F-FACTOR.
 *    END.
 *    IF S-CODMON = 2 THEN DO:
 *       IF Almmmatg.MonVta = 2 THEN 
 *         ASSIGN F-PREBAS = Almmmatg.PreBas * F-PorImp * F-FACTOR.
 *       ELSE 
 *         ASSIGN F-PREBAS = (Almmmatg.PreBas * F-PorImp / FacCfgGn.Tpocmb[1]) * F-FACTOR.
 *    END.
 *    CASE I-ListPr:
 ...
 *    F-PREVTA = ROUND(F-PREBAS,2).*/
  ...
  /* MESSAGE S-TpoCmb VIEW-AS ALERT-BOX. */
    ...
    /****   Halla el Dscto. segun Campaña o Normal  ****/
...
------------------------------------------------------------------------------*/
   ...
   IF DEC(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) <> 0 THEN DO:
      ...
      DO J = 1 TO 10 :
         IF DEC(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) >= Almmmatg.DtoVolR[J]
                                           AND Almmmatg.DtoVolR[J] > 0 THEN DO:
