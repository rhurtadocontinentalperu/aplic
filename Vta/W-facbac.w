...
&Scoped-Define ENABLED-OBJECTS C-CodDoc RECT-20 Btn-Print 
...
DEFINE VARIABLE h_b-tmpvtmb AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Registro facturas BATCH (Detalle)"
         HEIGHT             = 17.15
         WIDTH              = 89.57
         MAX-HEIGHT         = 17.73
         MAX-WIDTH          = 91.43
...
ON END-ERROR OF W-Win /* Registro facturas BATCH (Detalle) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Registro facturas BATCH (Detalle) */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-faccre ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-docmto ).
       RUN set-position IN h_b-docmto ( 7.77 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 9.15 , 85.86 ) */
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/b-tmpvtmb.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpvtmb ).
       RUN set-position IN h_b-tmpvtmb ( 7.46 , 1.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 16.77 , 51.14 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.31 , 38.43 ) NO-ERROR.

       /* Links to SmartBrowser h_b-tmpvtmb. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-tmpvtmb ).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE C-CodDoc RECT-20 Btn-Print 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          RUN dispatch IN h_b-tmpvtmb ('open-query':U).
      END.
