...
DEFINE SHARED VAR S-CODALM AS CHAR NO-UNDO.
DEFINE SHARED VAR S-NOMALM AS CHAR NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS COMBO-BOX-6 wclient F-DesFch F-HasFch ~
COMBO-BOX-7 wpedido br_table 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-6 wclient F-DesFch F-HasFch ~
COMBO-BOX-7 wpedido 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 88.86 BY 8.58
         FONT 4.
...
ELSE DO :
    OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE {&KEY-PHRASE}
         ...
         AND FacCPedi.CodDoc BEGINS S-CODDOC
         AND FacCpedi.Nomcli BEGINS wclient
         AND FacCPedi.FlgEst BEGINS f-estado NO-LOCK,
         FIRST gn-clie WHERE gn-clie.CodCia = cl-codcia
         AND gn-clie.CodCli = FacCPedi.CodCli NO-LOCK,
             FIRST gn-ConVt WHERE gn-ConVt.Codig = FacCPedi.FmaPgo NO-LOCK
            {&SORTBY-PHRASE}.
...
ON VALUE-CHANGED OF COMBO-BOX-7 IN FRAME F-Main /* Condición */
DO:
  ASSIGN COMBO-BOX-7.
  CASE COMBO-BOX-7:
       WHEN "Pendiente" THEN 
