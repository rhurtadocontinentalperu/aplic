...
&SCOPED-DEFINE CONDICION ( FacCLiqu.CodCia = S-CODCIA AND ~
                           FacCLiqu.CodDiv = S-CODDIV AND ~
                           FacCLiqu.CodDoc = S-CODDOC AND ~
                           FacCLiqu.NroDoc BEGINS STRING(S-NROSER,"999") )
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH FacCLiqu WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main FacCLiqu
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main FacCLiqu
...
DEFINE FRAME FRAME-A
     F-Numero AT ROW 1.15 COL 2.29 NO-LABEL
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "{&CONDICION}"
     _Design-Parent    is WINDOW q-tables @ ( 1 , 11.14 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
FIND FacCLiqu WHERE 
     ...
     FacCLiqu.CodDoc = s-coddoc AND
     FacCLiqu.NroDoc = STRING(S-NroSer,"999") + STRING(F-Numero,"999999") 
     NO-LOCK NO-ERROR.
IF AVAILABLE FacCLiqu THEN output-var-1 = ROWID(FacCLiqu).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
