...
DEFINE TEMP-TABLE tmp-tempo2
    FIELD t-codcia2  LIKE Almdmov.Codcia 
    FIELD t-nrodoc2  LIKE CcbCDocu.NroDoc
    FIELD t-coddoc2  LIKE CcbCDocu.CodDoc
    FIELD t-nroref2  LIKE CcbCDocu.NroRef
    FIELD t-codref2  LIKE CcbCDocu.CodRef
    FIELD t-nroped2  LIKE CcbCDocu.NroPed
    FIELD t-codped2  LIKE CcbCDocu.CodPed
    FIELD t-fchdoc2  LIKE CcbCDocu.FchDoc
    FIELD t-codcli2  LIKE CcbCDocu.CodCli
    FIELD t-NomCli2  LIKE CcbCDocu.NomCli
    /*FIELD t-ventamn1 AS DEC            FORMAT "->>>,>>>,>>9.99" 
    FIELD t-ventame1 AS DEC            FORMAT "->>>,>>>,>>9.99"
    FIELD t-devolme1 AS DEC            FORMAT "->>>,>>>,>>9.99"
    FIELD t-devolmn1 AS DEC            FORMAT "->>>,>>>,>>9.99"
...
DEFINE FRAME F-Main
     ...
     Btn_OK AT ROW 7.73 COL 44
     Btn_Cancel AT ROW 7.73 COL 60
     " Criterio de Selección" VIEW-AS TEXT
          SIZE 18.57 BY .5 AT ROW 1.35 COL 4.43
          FONT 6
     " Moneda:" VIEW-AS TEXT
          SIZE 7 BY .5 AT ROW 5.85 COL 5
     RECT-61 AT ROW 1 COL 3
     RECT-46 AT ROW 7.46 COL 3
...
/*******Inicializa la Tabla Temporal ******/
    ...
    FOR EACH tmp-tempo WHERE tmp-tempo.t-codcia  = S-CODCIA ,
        EACH CcbCdocu NO-LOCK WHERE CcbCdocu.CodCia = S-CODCIA
        AND Ccbcdocu.fchdoc >= DesdeF
        AND Ccbcdocu.fchdoc <= HastaF
        AND CcbCdocu.CodCli = tmp-tempo.t-CodCli
        AND CcbCdocu.CodDoc = "G/R"
        AND CcbCdocu.NroRef = tmp-tempo.t-nrodoc
        AND CcbCdocu.CodRef = tmp-tempo.t-coddoc:
        IF CcbCDocu.FlgEst = "A" THEN NEXT.
        MESSAGE "Nrodoc G/R: " CcbCdocu.NroDoc SKIP
                "Coddoc G/R: " CcbCdocu.CodDoc SKIP
                "Nroped O/D: " CcbCdocu.Nroped SKIP 
                "Codped O/D: " CcbCdocu.Codped. 
        CREATE  tmp-tempo2.
            ASSIGN  
                tmp-tempo2.t-codcia2  = S-CODCIA
                tmp-tempo2.t-codcli2  = CcbCDocu.CodCli
                tmp-tempo2.t-NomCli2  = CcbCDocu.NomCli
                tmp-tempo2.t-nrodoc2  = CcbCDocu.NroDoc
                tmp-tempo2.t-coddoc2  = CcbCDocu.CodDoc
                tmp-tempo2.t-nroped2  = CcbCDocu.NroPed
                tmp-tempo2.t-codped2  = CcbCDocu.CodPed
                tmp-tempo2.t-fchdoc2  = CcbCDocu.FchDoc.
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY F-CodCli x-NomCli DesdeF HastaF nCodMon 
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH tmp-tempo BREAK BY tmp-tempo.t-codcli BY tmp-tempo.t-fchdoc BY tmp-tempo.t-nrodoc :
     ...
     IF LAST-OF(tmp-tempo.t-codcli) THEN DO:
      ...
      END.
     END.
     FOR EACH tmp-tempo2 WHERE tmp-tempo2.t-codcli2 = tmp-tempo.t-codcli BREAK BY tmp-tempo2.t-codcli2: 
