...
&Scoped-Define ENABLED-OBJECTS RECT-4 x-usuario x-password 
...
DEFINE VARIABLE x-password AS CHARACTER FORMAT "X(30)":U 
     LABEL "Clave" 
     VIEW-AS FILL-IN 
     SIZE 15.72 BY .81
...
/*    IF LOOKUP(X-NIVEL, FacUsers.Niveles) = 0 THEN DO:
 *         MESSAGE "Usuario no Autorizado" SKIP
 *                 "a realizar " x-mens
 *                 VIEW-AS ALERT-BOX ERROR.
 *         APPLY "ENTRY" TO x-usuario IN FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
  DISPLAY x-usuario x-password 
      WITH FRAME D-Dialog.
  ENABLE RECT-4 x-usuario x-password 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
