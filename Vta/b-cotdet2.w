...
FacDPedi.UndVta FacDPedi.CanPed FacDPedi.PorDto FacDPedi.Por_Dsctos[1] ~
Price @ Price dMargen @ dMargen dCto @ dCto 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 103 BY 9.42
         FONT 4.
...
"FacDPedi.codmat" "Codigo" ? "character" ? ? ? ? ? ? no ? no no "10.57" yes yes no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[5]   > INTEGRAL.FacDPedi.PorDto
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH almdmov USE-INDEX Almd07
        WHERE almdmov.codcia = s-CodCia
        AND almdmov.TipMov = "I"
        AND almdmov.CodMov = 02
        AND almdmov.CodMat = FacDPEdi.CodMat NO-LOCK
