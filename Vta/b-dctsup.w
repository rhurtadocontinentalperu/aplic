...
DEF VAR s-Tabla AS CHAR INIT 'AU'.      /* Autoservicios */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacTabla.Codigo COLUMN-LABEL "<<<Cliente>>>" FORMAT "x(11)":U
      gn-clie.NomCli FORMAT "x(50)":U
      FacTabla.Valor[1] COLUMN-LABEL "%Descuento" FORMAT ">>9.99":U
      FacTabla.Valor[2] COLUMN-LABEL "%Incremento" FORMAT ">,>>9.99":U
  ENABLE
      FacTabla.Codigo
      FacTabla.Valor[1]
      FacTabla.Valor[2]
...
/* Query rebuild information for BROWSE br_table
 ...
 AND INTEGRAL.FacTabla.Tabla = s-tabla"
     _JoinCode[2]      = "INTEGRAL.gn-clie.CodCli = INTEGRAL.FacTabla.Codigo"
     _Where[2]         = "INTEGRAL.gn-clie.CodCia = cl-codcia"
     _FldNameList[1]   > INTEGRAL.FacTabla.Codigo
"FacTabla.Codigo" "<<<Cliente>>>" "x(11)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.gn-clie.NomCli
     _FldNameList[3]   > INTEGRAL.FacTabla.Valor[1]
"FacTabla.Valor[1]" "%Descuento" ">>9.99" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacTabla.Valor[2]
"FacTabla.Valor[2]" "%Incremento" ">,>>9.99" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON 'return':U OF FacTabla.Codigo, FacTabla.Valor[1]
DO:
    APPLY 'tab':u.
    RETURN NO-APPLY.
END.
