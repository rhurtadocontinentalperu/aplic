...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-5 W-Win _STRUCTURED
  QUERY BROWSE-5 NO-LOCK DISPLAY
      FacCPedi.NroPed COLUMN-LABEL "N° Orden" FORMAT "X(9)":U WIDTH 8
...
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 91 BY 8.62
         FONT 1
         TITLE "Ordenes de Despacho Pendientes" FIT-LAST-COLUMN.
...
DEFINE FRAME F-Main
     ...
     b-print AT ROW 10.15 COL 63 WIDGET-ID 2
     b-cancel AT ROW 10.15 COL 78 WIDGET-ID 4
...
"NroPed" "N° Orden" ? "character" ? ? ? ? ? ? no ? no no "8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.FacCPedi.NomCli
"NomCli" ? ? "character" ? ? ? ? ? ? no ? no no "47" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.FacCPedi.FchPed
"FchPed" ? ? "date" ? ? ? ? ? ? no ? no no "12.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"NroRef" "Numero Referencia" ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-5 */
&ANALYZE-RESUME
...
ON END-ERROR OF W-Win /* O/D por Ubicación */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* O/D por Ubicación */
DO:
