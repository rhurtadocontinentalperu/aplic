...
DEFINE VARIABLE cb-tipo AS CHARACTER FORMAT "X(256)":U INITIAL "Orden" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "Orden","Cliente" 
     DROP-DOWN-LIST
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 72 BY 9.42
         FONT 1
...
DEFINE FRAME F-Main
     ...
     b-print AT ROW 12.31 COL 44 WIDGET-ID 2
     b-cancel AT ROW 12.31 COL 59 WIDGET-ID 4
     rs-alm AT ROW 12.42 COL 19.86 NO-LABEL WIDGET-ID 8
...
ON END-ERROR OF W-Win /* Impresion de Orden de Despacho */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Impresion de Orden de Despacho */
DO:
...
ON LEAVE OF txt-pedido IN FRAME F-Main /* NroOrden */
DO:
  ASSIGN 
      s-registro-actual = ?
      txt-pedido.
 ...
 */
 FOR EACH Reporte WHERE  
     NOT ( Reporte.CodFam = '010' OR Reporte.CodFam = '011'  
           OR (Reporte.CodFam = '002' AND Reporte.DesMar BEGINS 'TILIBRA') )
...
/*      IF Reporte.CodFam = '011' THEN NEXT. */
 ...
 PAGE STREAM Report.
 FOR EACH Reporte 
     WHERE ( Reporte.CodFam = '010' OR Reporte.CodFam = '011'  
             OR (Reporte.CodFam = '002' AND Reporte.DesMar BEGINS 'TILIBRA') )
