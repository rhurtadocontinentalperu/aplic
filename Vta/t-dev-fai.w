...
Almmmatg.DesMar DMOV.CodUnd DMOV.CanDes 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      DMOV.codmat COLUMN-LABEL "Articulo" FORMAT "X(6)":U WIDTH 8.43
      Almmmatg.DesMat FORMAT "X(60)":U
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(20)":U
      DMOV.CodUnd COLUMN-LABEL "Unidad" FORMAT "X(10)":U
      DMOV.CanDes FORMAT "(ZZZ,ZZZ,ZZ9.9999)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 86 BY 11.31
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.DMOV,INTEGRAL.Almmmatg OF Temp-Tables.DMOV"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST USED"
     _FldNameList[1]   > Temp-Tables.DMOV.codmat
"Temp-Tables.DMOV.codmat" "Articulo" ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"INTEGRAL.Almmmatg.DesMar" "Marca" "X(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.DMOV.CodUnd
"Temp-Tables.DMOV.CodUnd" "Unidad" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   = Temp-Tables.DMOV.CanDes
