...
DEFINE VARIABLE h_b-cotiz2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-tmpco2 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn-Asigna AT ROW 16.46 COL 79.29
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Cotizaciones por Precio de Venta"
         HEIGHT             = 16.73
         WIDTH              = 88.14
         MAX-HEIGHT         = 18.31
         MAX-WIDTH          = 91.43
         VIRTUAL-HEIGHT     = 18.31
         VIRTUAL-WIDTH      = 91.43
         RESIZE             = no
         SCROLL-BARS        = no
         STATUS-AREA        = no
...
ON END-ERROR OF W-Win /* Cotizaciones por Precio de Venta */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Cotizaciones por Precio de Venta */
DO:
...
DO:
  RUN Asignar-Articulos IN h_b-tmpco2.
  RUN dispatch IN h_b-tmpco2 ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 5.69 , 10.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 1.31 , 20.29 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv05 ).
       RUN set-position IN h_p-updv05 ( 5.69 , 31.29 ) NO-ERROR.
       RUN set-size IN h_p-updv05 ( 1.31 , 57.43 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-pedido ).
       RUN set-position IN h_q-pedido ( 5.73 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.12 , 14.86 ) */

       /* Links to SmartViewer h_v-cotiza. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-cotiza ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/b-cotiz2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotiz2 ).
       RUN set-position IN h_b-cotiz2 ( 7.08 , 1.86 ) NO-ERROR.
       /* Size in UIB:  ( 10.08 , 87.72 ) */

       /* Links to SmartBrowser h_b-cotiz2. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_b-cotiz2 ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/b-tmpco2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpco2 ).
       RUN set-position IN h_b-tmpco2 ( 7.12 , 1.43 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 16.42 , 43.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.19 , 35.14 ) NO-ERROR.

       /* Links to SmartBrowser h_b-tmpco2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-tmpco2 ).
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-cotiz2 ('open-query':U).
          RUN dispatch IN h_b-tmpco2 ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN Recalcular-Precios IN h_b-tmpco2.
      END.
END CASE.
