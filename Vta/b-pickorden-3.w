...
/*           WHEN '00015' THEN RUN Graba-Totales-Expo. */
          WHEN '00000' OR WHEN '00015' THEN RUN Graba-Totales-Ate.
          OTHERWISE RUN Graba-Totales-Lima.
      END CASE.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':  
      ...
      /* 3ra. PARTE: Regeneramos el Pedido */
      RUN Regenera-Pedido.
      IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      FIND B-CCOTI WHERE B-CCOTI.codcia = B-CPEDI.codcia
          ...
          NO-LOCK NO-ERROR.
      IF NOT AVAILABLE B-CCOTI THEN DO:
          MESSAGE 'NO se encontró la COTIZACION' B-CPEDI.nroref
              VIEW-AS ALERT-BOX ERROR.
          RETURN 'ADM-ERROR'.
      END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE x-Item AS INT INIT 1 NO-UNDO.
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      /* Renumeramos la Orden de Despacho */
      FOR EACH PEDI:
          DELETE PEDI.
      END.
