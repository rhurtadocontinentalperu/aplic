...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE Almmmatg THEN DO:
    IF DMOV.candev <> DMOV.candes THEN DO:
        ASSIGN
...
/* ******************************** CALCULO ANTERIOR 08.07.08
  FIND Almmmatg WHERE Almmmatg.CodCia = S-CODCIA 
                 AND  Almmmatg.codmat = DMOV.codmat 
                NO-LOCK NO-ERROR.
  IF AVAILABLE Almmmatg THEN DO:
     /****  Se puso para no calcular el Dscto. pero en caso que tenga Dscto. adicional 
            registrar el Dscto. para poder diferenciar del Dscto. de linea,
            Add by C.Q. 09/03/2000  ****/
     IF DMOV.Flg_factor = "" THEN DO:
         ASSIGN
            DMOV.ImpDto = ROUND(DMOV.PreUni * (DMOV.PorDto / 100) * (IF TRIM(DMOV.CodAnt) = "YES" THEN (DMOV.CanDes) ELSE (DMOV.CanDes * DMOV.Factor)), 2)
            DMOV.ImpLin = ROUND(DMOV.PreUni * (IF TRIM(DMOV.CodAnt) = "YES" THEN (DMOV.CanDes) ELSE (DMOV.CanDes * DMOV.Factor)) , 2 ) .
         IF DMOV.AftIsc THEN 
            DMOV.ImpIsc = ROUND(DMOV.PreBas * (IF TRIM(DMOV.CodAnt) = "YES" THEN (DMOV.CanDes) ELSE (DMOV.CanDes * DMOV.Factor)) * (Almmmatg.PorIsc / 100),4).
         IF DMOV.AftIgv THEN 
            DMOV.ImpIgv = DMOV.ImpLin - ROUND(DMOV.ImpLin  / (1 + (FacCfgGn.PorIgv / 100)),4).
     END.
     ELSE DO:
         ASSIGN
            DMOV.ImpDto = ROUND(DMOV.PreUni * (DMOV.Por_Dsctos[1] / 100) * (IF TRIM(DMOV.Flg_factor) = "1" THEN DMOV.CanDes ELSE (DMOV.CanDes * DMOV.Factor)), 2)
            DMOV.ImpLin = ROUND(DMOV.PreUni * (IF TRIM(DMOV.Flg_factor) = "1" THEN DMOV.CanDes ELSE (DMOV.CanDes * DMOV.Factor)) , 2 ) - DMOV.ImpDto.
         IF DMOV.AftIsc THEN
            DMOV.ImpIsc = ROUND(DMOV.PreBas * (IF TRIM(DMOV.Flg_factor) = "1" THEN DMOV.CanDes ELSE (DMOV.CanDes * DMOV.Factor)) * (Almmmatg.PorIsc / 100),4).
         IF DMOV.AftIgv THEN 
            DMOV.ImpIgv = DMOV.ImpLin - ROUND(DMOV.ImpLin  / (1 + (FacCfgGn.PorIgv / 100)),4).
     END.
     /*********************************************************************************/
...
END.
   ...
   /* IF Almmmatg.FlgInt THEN DO:
 *            IF DECIMAL(DMOV.Candes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) <> INT(DMOV.Candes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) THEN DO:
 *               MESSAGE " La Cantidad de la Venta, " SKIP
 ...
 *            IF DEC(DMOV.Candes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) / Almtconv.Multiplos <> 
 *               INT(DEC(DMOV.Candes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) / Almtconv.Multiplos) THEN DO:
