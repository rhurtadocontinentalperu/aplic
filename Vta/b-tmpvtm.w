...
DO:
/*  &IF DEFINED(EXCLUDE-adm-cancel-record) = 0 &THEN MESSAGE "HOLA" VIEW-AS ALERT-BOX.
 *   &ENDIF.*/
/*   IF "{&adm-cancel-record}" = "" THEN DO:
 *    MESSAGE "ESTOY PROBANDO, HAGAN CLICK EN ACEPTAR" SKIP
 *            "GRACIAS ,     CARLOS" SKIP
 *    VIEW-AS ALERT-BOX.
...
/*   IF ITEM.PorDto:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} <> "" OR
 *       ITEM.PreUni:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} <> "" THEN DO:
...
DO:
   ...
   IF SELF:SCREEN-VALUE <> S-CODALM THEN DO:
    IF SELF:SCREEN-VALUE <> "83" THEN DO:
      MESSAGE "No puede despachar de otro almacen que no sea el 83 " VIEW-AS ALERT-BOX ERROR.
      APPLY "ENTRY" TO ITEM.AlmDes IN BROWSE {&BROWSE-NAME}.
      RETURN NO-APPLY.
    END.
   END.
 ...
 * /*        IF Almtconv.CodUnid = "UNI" AND Almtconv.Equival = 1 THEN DO:*/
 *            IF DECIMAL(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) <> INT(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) THEN DO:
...
/*    IF Almmmatg.FlgPre THEN DO:*/
        ...
        IF Almmmatg.UndC <> "" AND NOT SW-LOG1 THEN DO:
            ...
            IF (DEC(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) * F-FACTOR) >= 
               (0.25 * X-FACTOR) THEN  
            DO:
    /*        MESSAGE "INGREESA 1" DEC(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) * F-FACTOR  
     *            (0.25 * X-FACTOR) VIEW-AS ALERT-BOX.*/
                SW-LOG1 = TRUE.
        ...
        IF Almmmatg.UndB <> "" AND NOT SW-LOG1 THEN DO:
            ...
            IF (DEC(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) * F-FACTOR) >= 
               (0.25 * X-FACTOR) THEN
            DO:
    /*        MESSAGE "INGREESA 2" DEC(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) * F-FACTOR  
     *            (0.25 * X-FACTOR) VIEW-AS ALERT-BOX.*/
                SW-LOG1 = TRUE.
        ...
        IF Almmmatg.UndA <> "" AND NOT SW-LOG1 THEN DO:
            ...
            IF (DEC(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) * F-FACTOR) >= 
               (0.25 * X-FACTOR) THEN
            DO:
/*            MESSAGE X-FACTOR "INGREESA 3" DEC(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) * F-FACTOR  
 *                 (0.25 * X-FACTOR) VIEW-AS ALERT-BOX.*/
                SW-LOG1 = TRUE.
