...
DEFINE VAR lban      AS LOGICAL     NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS TOGGLE-CodDiv TOGGLE-CodCli BUTTON-5 ~
TOGGLE-CodMat BUTTON-6 TOGGLE-CodPro TOGGLE-CodVen TOGGLE-FmaPgo ~
TOGGLE-NroCard DesdeF HastaF RADIO-SET-Tipo BUTTON-1 BtnDone 
&Scoped-Define DISPLAYED-OBJECTS TOGGLE-CodDiv COMBO-BOX-CodDiv ~
TOGGLE-CodCli FILL-IN-CodCli FILL-IN-NomCli FILL-IN-file ~
TOGGLE-Resumen-Depto FILL-IN-CodMat FILL-IN-DesMat TOGGLE-CodMat ~
FILL-IN-file-2 COMBO-BOX-CodFam TOGGLE-Resumen-Linea COMBO-BOX-SubFam ~
...
DEFINE FRAME F-Main
     ...
     BUTTON-5 AT ROW 3.5 COL 105 WIDGET-ID 66
     FILL-IN-file AT ROW 3.54 COL 26 COLON-ALIGNED NO-LABEL WIDGET-ID 68
     TOGGLE-Resumen-Depto AT ROW 4.65 COL 28 WIDGET-ID 64
     FILL-IN-CodMat AT ROW 5.54 COL 26 COLON-ALIGNED WIDGET-ID 70
     FILL-IN-DesMat AT ROW 5.54 COL 41 COLON-ALIGNED NO-LABEL WIDGET-ID 72
     TOGGLE-CodMat AT ROW 5.65 COL 4 WIDGET-ID 14
     BUTTON-6 AT ROW 6.58 COL 104.86 WIDGET-ID 74
     FILL-IN-file-2 AT ROW 6.62 COL 25.86 COLON-ALIGNED NO-LABEL WIDGET-ID 76
     ...
     DesdeF AT ROW 14.19 COL 26 COLON-ALIGNED WIDGET-ID 10
     HastaF AT ROW 14.19 COL 47 COLON-ALIGNED WIDGET-ID 12
     RADIO-SET-Tipo AT ROW 15.27 COL 28 NO-LABEL WIDGET-ID 56
     BUTTON-1 AT ROW 16.08 COL 103 WIDGET-ID 24
     BtnDone AT ROW 16.08 COL 109 WIDGET-ID 28
     FILL-IN-Mensaje AT ROW 16.62 COL 2 COLON-ALIGNED NO-LABEL WIDGET-ID 22
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 129.72 BY 17.31 WIDGET-ID 100.
...
/* SETTINGS FOR FILL-IN FILL-IN-file IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FILL-IN-file-2 IN FRAME F-Main
   NO-ENABLE                                                            */
...
DO:
    ...
    ASSIGN
        ...
        FILL-IN-File FILL-IN-CodMat FILL-IN-File-2.
    /* CONSISTENCIA */
    IF (TOGGLE-CodCli OR 
        TOGGLE-CodDiv OR
        TOGGLE-CodMat OR
...
ON LEAVE OF FILL-IN-CodMat IN FRAME F-Main /* Codigo */
DO:
  IF SELF:SCREEN-VALUE = '' THEN RETURN.
...
DO:
    ...
    FILL-IN-File:SENSITIVE = NOT FILL-IN-File:SENSITIVE.
...
DO:
    ...
    FILL-IN-File-2:SENSITIVE = NOT FILL-IN-File-2:SENSITIVE.
...
FOR EACH estavtas.evtarti NO-LOCK WHERE estavtas.evtarti.codcia = s-codcia
    ...
    AND estavtas.evtarti.codmat BEGINS fill-in-CodMat,                                                  /*RDP01 - Filtro por Articulo*/
...
FOR EACH estavtas.evtartdv NO-LOCK WHERE estavtas.evtartdv.codcia = s-codcia
    ...
    AND estavtas.evtartdv.codmat BEGINS fill-in-CodMat,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Articulos-Divisiones2 W-Win 
PROCEDURE Carga-Articulos-Divisiones2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH tt-articulo NO-LOCK:    
    FOR EACH estavtas.evtartdv NO-LOCK WHERE estavtas.evtartdv.codcia = s-codcia
        AND estavtas.evtartdv.nrofch >= INTEGER(STRING(YEAR(DesdeF),"9999") + STRING(MONTH(DesdeF),"99"))
        AND estavtas.evtartdv.nrofch <= INTEGER(STRING(YEAR(HastaF),"9999") + STRING(MONTH(HastaF),"99"))
        AND estavtas.evtartdv.coddiv BEGINS x-CodDiv
        /*AND estavtas.evtartdv.codmat BEGINS fill-in-CodMat,*/
        AND estavtas.evtartdv.codmat BEGINS tt-articulo.tt-codmat,
        FIRST Almmmatg OF estavtas.evtartdv NO-LOCK WHERE Almmmatg.codfam BEGINS x-CodFam
        AND Almmmatg.subfam BEGINS x-SubFam:
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** PROCESANDO ' + 
            'MES ' + STRING(codmes, '99') + ' AÑO ' + STRING(codano, '9999') +
        ...
        FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
            ...
            AND estavtas.evtartdv.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
            AND estavtas.evtartdv.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
            NO-LOCK NO-ERROR.
        IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
        /**RD01 - Fin  ********/
...
FOR EACH tt-articulo NO-LOCK:
    FOR EACH estavtas.EvtProvArti NO-LOCK WHERE estavtas.EvtProvArti.codcia = s-codcia
        ...
        AND estavtas.EvtProvArti.codmat BEGINS tt-articulo.tt-codmat,
        ...
        FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
            AND VtaTabla.Tabla = 'CAMPAÑAS' 
            AND estavtas.EvtProvArti.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
            AND estavtas.EvtProvArti.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
            NO-LOCK NO-ERROR.
        IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
        /**RD01 - Fin  ********/
...
FOR EACH estavtas.evtvenarti NO-LOCK WHERE estavtas.evtvenarti.codcia = s-codcia
    ...
    AND estavtas.evtvenarti.codmat BEGINS fill-in-codmat,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Articulos-Vendedores2 W-Win 
PROCEDURE Carga-Articulos-Vendedores2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH tt-articulo NO-LOCK:
    FOR EACH estavtas.evtvenarti NO-LOCK WHERE estavtas.evtvenarti.codcia = s-codcia
        ...
        AND estavtas.evtvenarti.codven BEGINS x-CodVen
        /*AND estavtas.evtvenarti.codmat BEGINS fill-in-codmat*/
        AND estavtas.evtvenarti.codmat BEGINS tt-articulo.tt-codmat,
        FIRST Almmmatg OF estavtas.evtvenarti NO-LOCK WHERE Almmmatg.codfam BEGINS x-CodFam
        AND Almmmatg.subfam BEGINS x-SubFam:
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** PROCESANDO ' + 
            'MES ' + STRING(codmes, '99') + ' AÑO ' + STRING(codano, '9999') +
        ...
        FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
            AND VtaTabla.Tabla = 'CAMPAÑAS' 
            AND estavtas.evtvenarti.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
            AND estavtas.evtvenarti.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
            NO-LOCK NO-ERROR.
        IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
        /**RD01 - Fin  ********/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Articulos2 W-Win 
PROCEDURE Carga-Articulos2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH tt-articulo NO-LOCK:
    FOR EACH estavtas.evtarti NO-LOCK WHERE estavtas.evtarti.codcia = s-codcia
        ...
        AND estavtas.evtarti.nrofch <= INTEGER(STRING(YEAR(HastaF),"9999") + STRING(MONTH(HastaF),"99"))
        /*AND estavtas.evtarti.codmat BEGINS fill-in-CodMat,      RDP01 - Filtro por Articulo*/
        AND estavtas.evtarti.codmat BEGINS tt-articulo.tt-codmat,
        FIRST Almmmatg OF estavtas.evtarti NO-LOCK WHERE Almmmatg.codfam BEGINS x-CodFam
        AND Almmmatg.subfam BEGINS x-SubFam:
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** PROCESANDO ' + 
            'MES ' + STRING(codmes, '99') + ' AÑO ' + STRING(codano, '9999') +
        ...
        FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
            AND VtaTabla.Tabla = 'CAMPAÑAS' 
            AND estavtas.evtarti.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
            AND estavtas.evtarti.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
            NO-LOCK NO-ERROR.
        IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
        /**RD01 - Fin  ********/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Cli-Art01 W-Win 
PROCEDURE Carga-Cli-Art01 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH tt-cliente NO-LOCK:
    FOR EACH estavtas.evtclarti USE-INDEX Indice01
        NO-LOCK WHERE estavtas.evtclarti.codcia = s-codcia
        ...
        AND estavtas.evtclarti.codcli BEGINS tt-cliente.tt-codcli
        AND estavtas.evtclarti.codmat BEGINS fill-in-codmat,
        ...
        FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
            AND VtaTabla.Tabla = 'CAMPAÑAS' 
            AND estavtas.evtclarti.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
            AND estavtas.evtclarti.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
            NO-LOCK NO-ERROR.
        IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
        /**RD01 - Fin  ********/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Cli-Art01-b W-Win 
PROCEDURE Carga-Cli-Art01-b :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/    
    ...
    IF NOT AVAIL tt-cliente THEN lBan = NO.
    ELSE lBan = YES.
    ...
    FOR EACH tt-cliente NO-LOCK:    
        ...
        FOR EACH estavtas.evtclarti USE-INDEX Indice01
            ...
            IF lBan THEN DO:
                FIND FIRST tt-articulo WHERE tt-articulo.tt-codmat = estavtas.evtclarti.codmat                
                    NO-LOCK NO-ERROR.
                IF NOT AVAIL tt-articulo THEN NEXT Estavtas.            
            END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Cli-Art02 W-Win 
PROCEDURE Carga-Cli-Art02 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
FOR EACH tt-cliente NO-LOCK:
    FOR EACH estavtas.evtclarti USE-INDEX Indice03
        NO-LOCK WHERE estavtas.evtclarti.codcia = s-codcia
        AND estavtas.evtclarti.nrofch >= INTEGER(STRING(YEAR(DesdeF),"9999") + STRING(MONTH(DesdeF),"99"))
        AND estavtas.evtclarti.nrofch <= INTEGER(STRING(YEAR(HastaF),"9999") + STRING(MONTH(HastaF),"99"))
        AND estavtas.evtclarti.coddiv BEGINS x-CodDiv
        ...
        /*Verifica articulo*/
        IF NOT estavtas.evtclarti.codmat BEGINS fill-in-codmat THEN NEXT.
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** PROCESANDO ' + 
            'MES ' + STRING(codmes, '99') + ' AÑO ' + STRING(codano, '9999') +
            ' DIVISION ' + coddiv + ' CLIENTE ' + codcli + ' **'.
        T-Vtamn   = 0.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Cli-Art03 W-Win 
PROCEDURE Carga-Cli-Art03 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Clientes-Articulos2 W-Win 
PROCEDURE Carga-Clientes-Articulos2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    /*RD01- Modifica opciones*** 
    IF fill-in-codcli = '' AND fill-in-codmat  <> '' then RUN Carga-Cli-Art01.
    IF fill-in-codcli <> '' THEN RUN Carga-Cli-Art02.
    IF fill-in-codcli = '' AND fill-in-codmat = '' THEN RUN Carga-Cli-Art03.
    ****/
    IF fill-in-codcli = '' AND fill-in-codmat  <> '' THEN RUN Carga-Cli-Art01.
    IF fill-in-codcli <> '' AND fill-in-file-2 = '' THEN RUN Carga-Cli-Art02.    
    IF fill-in-codcli = '' AND fill-in-codmat = '' AND  fill-in-file-2 = '' THEN RUN Carga-Cli-Art03.
    IF fill-in-codcli = '' AND fill-in-file-2 <> '' THEN RUN Carga-Cli-Art01-b.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Clientes2 W-Win 
PROCEDURE Carga-Clientes2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH tt-cliente NO-LOCK:
    FOR EACH estavtas.evtclie NO-LOCK WHERE estavtas.evtclie.codcia = s-codcia
        ...
        AND estavtas.evtclie.nrofch <= INTEGER(STRING(YEAR(HastaF),"9999") + STRING(MONTH(HastaF),"99"))
        AND estavtas.evtclie.coddiv BEGINS x-CodDiv
        AND estavtas.evtclie.codcli BEGINS tt-cliente.tt-CodCli:
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** PROCESANDO ' + 
            'MES ' + STRING(codmes, '99') + ' AÑO ' + STRING(codano, '9999') +
            ' DIVISION ' + coddiv + ' CLIENTE ' + codcli + ' **'.
        T-Vtamn   = 0.
        ...
        FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
            AND VtaTabla.Tabla = 'CAMPAÑAS' 
            AND estavtas.evtclie.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
            AND estavtas.evtclie.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
            NO-LOCK NO-ERROR.
        IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
        /**RD01 - Fin  ********/
...
FOR EACH estavtas.evtdivi NO-LOCK WHERE estavtas.evtdivi.codcia = s-codcia
    ...
    FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
        AND VtaTabla.Tabla = 'CAMPAÑAS' 
        AND estavtas.evtdivi.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
        AND estavtas.evtdivi.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
        NO-LOCK NO-ERROR.
    IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
    /**RD01 - Fin  ********/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Lista-Articulos W-Win 
...
------------------------------------------------------------------------------*/
    ...
    IF SEARCH(FILL-IN-file-2) <> ? THEN DO:
        OUTPUT TO VALUE(FILL-IN-file-2) APPEND.
        ...
        REPEAT:
            CREATE tt-datos.
            IMPORT tt-codigo.
            IF tt-codigo = '' THEN ASSIGN tt-codigo = "?".        
        END.
        INPUT CLOSE.
    END.
...
/*             CREATE tt-articulo.                            */
/*             IMPORT tt-codmat.                              */
/*             IF tt-codmat = '' THEN ASSIGN tt-codmat = "?". */
/*         END.                                               */
/*         INPUT CLOSE.                                       */
/*     END.                                                   */
    
    FOR EACH tt-datos:
        IF tt-codigo = '' THEN DELETE tt-datos.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Lista-Clientes W-Win 
...
------------------------------------------------------------------------------*/    
    ...
    IF SEARCH(FILL-IN-file) <> ? THEN DO:
        ...
        REPEAT:
            CREATE tt-cliente.
            IMPORT tt-codcli.
            IF tt-codcli = '' THEN ASSIGN tt-codcli = "?".        
        END.
        INPUT CLOSE.
    END.
    
    FOR EACH tt-cliente:
        IF tt-codcli = '' THEN DELETE tt-cliente.
    END.
...
FOR EACH estavtas.evtfpgo NO-LOCK WHERE estavtas.evtfpgo.codcia = s-codcia
    ...
    FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
        AND VtaTabla.Tabla = 'CAMPAÑAS' 
        AND estavtas.evtfpgo.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
        AND estavtas.evtfpgo.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
        NO-LOCK NO-ERROR.
    IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
    /**RD01 - Fin  ********/
...
FOR EACH estavtas.evtprov NO-LOCK WHERE estavtas.evtprov.codcia = s-codcia
    ...
    FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
        AND VtaTabla.Tabla = 'CAMPAÑAS' 
        AND estavtas.evtprov.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
        AND estavtas.evtprov.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
        NO-LOCK NO-ERROR.
    IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
    /**RD01 - Fin  ********/
...
FOR EACH tt-cliente NO-LOCK:
    FOR EACH tt-articulo NO-LOCK:
        FOR EACH estavtas.evtall04 NO-LOCK WHERE estavtas.evtall04.codcia = s-codcia
            ...
            AND estavtas.evtall04.codcli BEGINS tt-cliente.tt-CodCli
            AND estavtas.evtall04.codmat BEGINS tt-articulo.tt-CodMat
            ...
            FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
                AND VtaTabla.Tabla = 'CAMPAÑAS' 
                AND estavtas.evtall04.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
                AND estavtas.evtall04.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
                NO-LOCK NO-ERROR.
            IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
            /**RD01 - Fin  ********/
...
FOR EACH estavtas.evtcard NO-LOCK WHERE estavtas.evtcard.codcia = s-codcia
    ...
    FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
        AND VtaTabla.Tabla = 'CAMPAÑAS' 
        AND estavtas.evtcard.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
        AND estavtas.evtcard.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
        NO-LOCK NO-ERROR.
    IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
    /**RD01 - Fin  ********/
...
IF TOGGLE-CodCli THEN /*x-CodCli = FILL-IN-CodCli*/ RUN Carga-Lista-Clientes.
/*RD01 - Carga Clientes ***/
IF TOGGLE-CodMat AND (FILL-IN-CodMat <> '' OR FILL-IN-file-2 <> '') THEN RUN Carga-Lista-Articulos.
/*RD01 - Fin ****/
...
IF TOGGLE-CodMat = YES 
    AND (TOGGLE-CodCli OR 
         ...
         TOGGLE-FmaPgo OR 
         TOGGLE-NroCard) = NO THEN DO:
    RUN Carga-Articulos2. /*RDP01 - 15.02.11*/
    RETURN.
END.
IF (TOGGLE-CodMat AND TOGGLE-CodDiv) = YES 
    AND (TOGGLE-CodCli OR 
         ...
         TOGGLE-FmaPgo OR 
         TOGGLE-NroCard) = NO THEN DO:
    RUN Carga-Articulos-Divisiones2. /*RDP01 - 15.02.11*/
    RETURN.
END.
...
IF (TOGGLE-CodMat AND TOGGLE-CodVen) = YES 
    AND (TOGGLE-CodDiv OR 
         ...
         TOGGLE-FmaPgo OR 
         TOGGLE-NroCard) = NO THEN DO:
    RUN Carga-Articulos-Vendedores2.
    RETURN.
END.
...
IF (TOGGLE-CodCli AND TOGGLE-CodVen) = YES 
    AND (TOGGLE-CodDiv OR 
         ...
         TOGGLE-FmaPgo OR 
         TOGGLE-NroCard) = NO THEN DO:
    RUN Carga-Vendedores-Clientes2.
    RETURN.
END.
...
IF (TOGGLE-CodMat AND TOGGLE-CodPro) = YES 
    ...
    RETURN.
END.
IF (TOGGLE-CodMat AND TOGGLE-CodPro AND TOGGLE-CodDiv) = YES 
    AND (TOGGLE-CodCli OR 
         TOGGLE-CodVen OR
         TOGGLE-FmaPgo OR 
         TOGGLE-NroCard) = NO THEN DO:
...
FIND FIRST tt-articulo 
    WHERE tt-articulo.tt-codmat <> '' NO-LOCK NO-ERROR.
IF NOT AVAIL tt-articulo THEN lBan = NO.
ELSE lBan = YES.
...
FOR EACH tt-cliente NO-LOCK:  
    ...
    FOR EACH estavtas.evtall01 NO-LOCK WHERE estavtas.evtall01.codcia = s-codcia
        ...
        AND estavtas.evtall01.codcli BEGINS tt-cliente.tt-CodCli
        /*
        AND estavtas.evtall01.codmat BEGINS fill-in-codmat  /*Filtrar por codigo mat*/
        */
        ...
        /*RD01 - Filtra por articulos*/
        IF lBan THEN DO:
            FIND FIRST tt-articulo WHERE tt-articulo.tt-codmat = estavtas.evtall01.codmat
                NO-LOCK NO-ERROR.
            IF NOT AVAIL tt-articulo THEN NEXT Estavtas.            
        END.
        ...
        FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
            AND VtaTabla.Tabla = 'CAMPAÑAS' 
            AND estavtas.evtall01.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
            AND estavtas.evtall01.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
            NO-LOCK NO-ERROR.
        IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
        /**RD01 - Fin  ********/
...
FOR EACH estavtas.evtven NO-LOCK WHERE estavtas.evtven.codcia = s-codcia
    ...
    FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
        AND VtaTabla.Tabla = 'CAMPAÑAS' 
        AND estavtas.evtven.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
        AND estavtas.evtven.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
        NO-LOCK NO-ERROR.
    IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
    /**RD01 - Fin  ********/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Vendedores-Clientes2 W-Win 
PROCEDURE Carga-Vendedores-Clientes2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH tt-cliente NO-LOCK:
    FOR EACH estavtas.evtvencli NO-LOCK WHERE estavtas.evtvencli.codcia = s-codcia
        ...
        AND estavtas.evtvencli.nrofch <= INTEGER(STRING(YEAR(HastaF),"9999") + STRING(MONTH(HastaF),"99"))
        AND estavtas.evtvencli.coddiv BEGINS x-CodDiv
        AND estavtas.evtvencli.codcli BEGINS tt-cliente.tt-codcli
        AND estavtas.evtvencli.codven BEGINS x-CodVen:
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** PROCESANDO ' + 
            'MES ' + STRING(codmes, '99') + ' AÑO ' + STRING(codano, '9999') +
            ' DIVISION ' + coddiv + ' VENDEDOR ' + codven + ' **'.
        ...
        FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
            AND VtaTabla.Tabla = 'CAMPAÑAS' 
            AND estavtas.evtvencli.nrofch >= ((YEAR(VtaTabla.Rango_fecha[1]) * 100) + MONTH(VtaTabla.Rango_fecha[1]))
            AND estavtas.evtvencli.nrofch <= ((YEAR(VtaTabla.Rango_fecha[2]) * 100) + MONTH(VtaTabla.Rango_fecha[2]))
            NO-LOCK NO-ERROR.
        IF AVAIL VtaTabla THEN x-Llave = VtaTabla.Llave_c1 + "|".
        /**RD01 - Fin  ********/
...
------------------------------------------------------------------------------*/
  DISPLAY TOGGLE-CodDiv COMBO-BOX-CodDiv TOGGLE-CodCli FILL-IN-CodCli 
          FILL-IN-NomCli FILL-IN-file TOGGLE-Resumen-Depto FILL-IN-CodMat 
          FILL-IN-DesMat TOGGLE-CodMat FILL-IN-file-2 COMBO-BOX-CodFam 
  ...
  ENABLE TOGGLE-CodDiv TOGGLE-CodCli BUTTON-5 TOGGLE-CodMat BUTTON-6 
         TOGGLE-CodPro TOGGLE-CodVen TOGGLE-FmaPgo TOGGLE-NroCard DesdeF HastaF 
         RADIO-SET-Tipo BUTTON-1 BtnDone 
      WITH FRAME F-Main IN WINDOW W-Win.
...
DEF VAR x-campania AS CHAR NO-UNDO .
...
FOR EACH tmp-detalle:
    ...
    i-Campo = i-Campo + 1.
    x-Titulo = x-Titulo + 'CAMPAÑA' + CHR(9).
    ...
    IF TOGGLE-CodMat = YES THEN DO:
        ...
        IF (TOGGLE-Resumen-Linea = NO AND TOGGLE-Resumen-Marca = NO) THEN DO:
            ...
            x-Llave = x-LLave + CHR(9).
            i-Campo = i-Campo + 1.
            IF x-Titulo = '' THEN x-Titulo = 'UNIDAD' + CHR(9). ELSE x-Titulo = x-Titulo + 'UNIDAD' + CHR(9).
            /* LICENCIA */
            x-Campo = ENTRY(i-Campo, tmp-detalle.Llave, '|').
            x-Llave = x-Llave + x-Campo.
    ...
    END.
    x-Titulo = x-Titulo + 'CANTIDAD' + CHR(9) + 'VENTA-DOLARES' + CHR(9) + 'VENTA-SOLES' + CHR(9)+ ' '.
    x-Llave = x-Llave + STRING(tmp-detalle.CanxMes, '->>>,>>>,>>>,>>9.99') + CHR(9).
    x-Llave = x-Llave + STRING(tmp-detalle.VtaxMesMe, '->>>,>>>,>>>,>>9.99') + CHR(9).
    x-Llave = x-Llave + STRING(tmp-detalle.VtaxMesMn, '->>>,>>>,>>>,>>9.99') + CHR(9).
    IF pParametro = '+COSTO' THEN DO:
        x-Titulo = x-Titulo + 'COSTO-DOLARES' + CHR(9) + 'COSTO-SOLES' + CHR(9).
        x-Llave = x-Llave + STRING(tmp-detalle.CtoxMesMe, '->>>,>>>,>>>,>>9.99') + CHR(9).
        x-Llave = x-Llave + STRING(tmp-detalle.CtoxMesMn, '->>>,>>>,>>>,>>9.99') + CHR(9).
        x-Titulo = x-Titulo + 'PROMEDIO-DOLARES' + CHR(9) + 'PROMEDIO-SOLES' + CHR(9).
        x-Llave = x-Llave + STRING(tmp-detalle.ProxMesMe, '->>>,>>>,>>>,>>9.99') + CHR(9).
        x-Llave = x-Llave + STRING(tmp-detalle.ProxMesMn, '->>>,>>>,>>>,>>9.99') + CHR(9).
    END.
    IF TOGGLE-CodMat = YES AND TOGGLE-Resumen-Linea = NO AND TOGGLE-Resumen-Marca = NO THEN DO:
        x-Titulo = x-Titulo + 'STOCK-CONTABLE' + CHR(9).
        x-Llave = x-Llave + STRING(tmp-detalle.StkAct, '->>>,>>>,>>>,>>9.99') + CHR(9).
    END.
    x-Llave = x-Llave + ' ' .
    x-Titulo = x-Titulo + ' '.
