...
/*------------------------------------------------------------------------
  ...
  History: /*ML01*/ Habilita boton salida a Excel.
...
DEFINE NEW SHARED VAR s-TpoPed AS CHAR INIT "".
...
&Scoped-Define ENABLED-OBJECTS RECT-22 btn-margen BTN-COTIZA BTN-CCTE ~
Btn-Excel 
...
DEFINE VARIABLE h_q-pedido-p AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-pedido-3-1 AS HANDLE NO-UNDO.
...
DEFINE BUTTON Btn-Excel 
     ...
     LABEL "Excel" 
     SIZE 8 BY 1.62 TOOLTIP "Salida a excel".
...
DEFINE FRAME F-Main
     btn-margen AT ROW 2.08 COL 105 WIDGET-ID 2
     BTN-COTIZA AT ROW 2.88 COL 105
     BTN-CCTE AT ROW 3.69 COL 105 WIDGET-ID 4
     Btn-Excel AT ROW 5.04 COL 107 WIDGET-ID 20
     RECT-22 AT ROW 1 COL 2 WIDGET-ID 22
...
DO:
  RUN Asigna-Cotizacion IN h_v-pedido-3.
  IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN NO-APPLY.
  RUN dispatch IN h_t-pedido-3-1 ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-pedido-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedido-3 ).
       RUN set-position IN h_v-pedido-3 ( 1.27 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.69 , 98.57 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 11.23 , 11.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv13 ).
       RUN set-position IN h_p-updv13 ( 11.23 , 29.00 ) NO-ERROR.
       RUN set-size IN h_p-updv13 ( 1.54 , 61.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'q-pedido-p.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-pedido-p ).
       RUN set-position IN h_q-pedido-p ( 11.23 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-pedido-3. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_v-pedido-3 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-p , 'Record':U , h_v-pedido-3 ).

       /* Links to SmartQuery h_q-pedido-p. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido-p ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv13 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido-p ,
             h_p-updv13 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-pedido-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedido-3 ).
       RUN set-position IN h_b-pedido-3 ( 12.85 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedido-3 ( 12.31 , 120.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pedido-3. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido-p , 'Record':U , h_b-pedido-3 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/t-pedido-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedido-3-1 ).
       RUN set-position IN h_t-pedido-3-1 ( 12.85 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedido-3-1 ( 10.69 , 121.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-pedido-3-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedido-3-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedido-3-1 ,
             h_p-updv13 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-pedido-3-1 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ENABLE RECT-22 btn-margen BTN-COTIZA BTN-CCTE Btn-Excel 
...
------------------------------------------------------------------------------*/
  ...
  CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-pedido-3-1 <> ? THEN RUN dispatch IN h_t-pedido-3-1 ('open-query':U).
      END.
...
/*ML01*/ Btn-Excel:SENSITIVE = FALSE.
      END.
