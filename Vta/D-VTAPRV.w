...
DEFINE VARIABLE F-Almacen AS CHARACTER FORMAT "X(60)":U 
     LABEL "Stock Almacén" 
     VIEW-AS FILL-IN 
...
------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE VAR X-ENTRA AS LOGICAL INIT FALSE NO-UNDO.
...
FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = S-CODCIA
  ...
  FOR EACH Ccbddocu NO-LOCK WHERE Ccbddocu.Codcia = Almmmatg.Codcia
      ...
      IF LOOKUP(Ccbddocu.CodDoc,"FAC,BOL,N/C,N/D,TCK") = 0 THEN NEXT.
      ...
      X-SIGNO   = IF Ccbddocu.Coddoc = "N/C" THEN -1 ELSE 1.
      F-FACTOR  = 1.
      T-Vtamn   = 0.
      T-Vtame   = 0.                 
      F-Salida  = 0.
      ...
      FIND Gn-tcmb WHERE Gn-tcmb.Fecha = Ccbddocu.fchdoc.

      IF AVAILABLE Gn-tcmb THEN DO: 
         F-Salida = F-salida + X-SIGNO * (Ccbddocu.Candes * F-FACTOR).
         IF Ccbcdocu.Codmon = 1 THEN DO:
            T-Vtamn  = T-Vtamn + X-SIGNO * (Ccbddocu.ImpLin) .
            T-Vtame  = T-Vtame + X-SIGNO * (Ccbddocu.Implin / Gn-Tcmb.Compra).
         END.   
         IF Ccbcdocu.Codmon = 2 THEN DO:
            T-Vtamn  = T-Vtamn + X-SIGNO * (Ccbddocu.ImpLin * Gn-Tcmb.Venta).
            T-Vtame  = T-Vtame + X-SIGNO * (Ccbddocu.Implin ).
         END.   
      END.
      ...
      IF NOT AVAIL tmp-tempo THEN DO:
        ...
        IF Almmmatg.MonVta = 1 THEN DO:
           t-costome = Almmmatg.CtoLis / Almmmatg.Tpocmb.
           t-costomn = Almmmatg.CtoLis .
        END.
        IF Almmmatg.MonVta = 2 THEN DO:
           t-costome = Almmmatg.CtoLis . 
           t-costomn = Almmmatg.CtoLis * Almmmatg.Tpocmb.
        END.
        t-costo[1] = IF ncodmon = 1 THEN t-costomn ELSE t-costome.
      ...
      X-ENTRA = FALSE.
      IF F-DIVISION-1 <> "" AND Ccbddocu.Coddiv = F-DIVISION-1 THEN DO:
         ASSIGN T-Canti[1] = T-Canti[1] + F-Salida 
                T-Venta[1] = T-venta[1] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                X-ENTRA = TRUE.
      END.       
      IF F-DIVISION-2 <> "" AND Ccbddocu.Coddiv = F-DIVISION-2 THEN DO:
         ASSIGN T-Canti[2] = T-Canti[2] + F-Salida 
                T-Venta[2] = T-venta[2] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                X-ENTRA = TRUE.
      END.       
      IF F-DIVISION-3 <> "" AND Ccbddocu.Coddiv = F-DIVISION-3 THEN DO:
         ASSIGN T-Canti[3] = T-Canti[3] + F-Salida 
                T-Venta[3] = T-venta[3] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                X-ENTRA = TRUE.
      END.       
      IF F-DIVISION-4 <> "" AND Ccbddocu.Coddiv = F-DIVISION-4 THEN DO:
         ASSIGN T-Canti[4] = T-Canti[4] + F-Salida  
                T-Venta[4] = T-venta[4] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                X-ENTRA = TRUE.
      END.       
      IF F-DIVISION-5 <> "" AND Ccbddocu.Coddiv = F-DIVISION-5 THEN DO:
         ASSIGN T-Canti[5] = T-Canti[5] + F-Salida 
                T-Venta[5] = T-venta[5] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                X-ENTRA = TRUE.
      END.       
      IF F-DIVISION-6 <> "" AND Ccbddocu.Coddiv = F-DIVISION-6 THEN DO:
         ASSIGN T-Canti[6] = T-Canti[6] + F-Salida 
                T-Venta[6] = T-venta[6] + ( IF ncodmon = 1 THEN T-Vtamn ELSE T-Vtame )
                X-ENTRA = TRUE.     
      END.       
...
FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = S-CODCIA
  ...
  DO x-Item = 1 TO NUM-ENTRIES(x-CodDiv):
    FIND Gn-divi WHERE Gn-divi.codcia = s-codcia
        AND Gn-divi.coddiv = ENTRY(x-Item, x-CodDiv)
        NO-LOCK NO-ERROR.
