...
&Scoped-Define ENABLED-FIELDS gn-clie.Flgsit gn-clie.Ruc gn-clie.ApePat ~
gn-clie.ApeMat gn-clie.Nombre gn-clie.NomCli gn-clie.E-Mail gn-clie.DirCli ~
gn-clie.FaxCli gn-clie.DirEnt gn-clie.FlagAut gn-clie.ImpLC gn-clie.FchVLC ~
gn-clie.CndVta 
...
gn-clie.CndVta gn-clie.clfCli gn-clie.ClfCli2 
...
"Rechazado", "R":U
     ...
     gn-clie.ImpLC AT ROW 9.54 COL 16 COLON-ALIGNED
          LABEL "Importe" FORMAT "Z,ZZZ,ZZZ,ZZ9.99"
          VIEW-AS FILL-IN 
          SIZE 17 BY .81
...
DEFINE FRAME F-Main
     ...
     gn-clie.clfCli AT ROW 12.85 COL 28.72 COLON-ALIGNED
          LABEL "Clasificacion productos PROPIOS"
          VIEW-AS FILL-IN 
          SIZE 5 BY .81
     f-ClfCli AT ROW 12.85 COL 35 COLON-ALIGNED NO-LABEL
     gn-clie.ClfCli2 AT ROW 13.92 COL 31 NO-LABEL WIDGET-ID 12
          VIEW-AS RADIO-SET HORIZONTAL
          RADIO-BUTTONS 
                    "A++", "A++":U,
"A+", "A+":U,
...
"Sin clasificacion", ""
     ...
     "Clasificacion productos de TERCEROS:" VIEW-AS TEXT
          SIZE 27 BY .5 AT ROW 14.19 COL 4 WIDGET-ID 18
 ...
 *     gn-clie.fchact = TODAY.*/
  ...
  ASSIGN
    ...
    logtabla.ValorLlave = STRING(gn-clie.codcli, 'x(11)') + '|' +
                            ...
                            STRING(gn-clie.MonLC, '9') + '|' + 
                            STRING(gn-clie.ImpLC, '->>>>>>>>9.99') + '|' +
...
------------------------------------------------------------------------------*/
  DO WITH FRAME {&FRAME-NAME} :
    IF gn-clie.FlagAut:SCREEN-VALUE = "" AND DECIMAL(gn-clie.ImpLC:SCREEN-VALUE) > 0 
    THEN DO:
        DISPLAY 0 @ gn-clie.impLC 
                ? @ gn-clie.fchvLC.
       MESSAGE "Cliente no Tiene Autorizacion Financiera " SKIP
               "      Solicitar su Autorizacion          " VIEW-AS ALERT-BOX ERROR.
       APPLY "ENTRY" TO gn-clie.impLC. 
       RETURN 'ADM-ERROR'.
    END.
    IF gn-clie.FlagAut:SCREEN-VALUE = "R" AND DECIMAL(gn-clie.ImpLC:SCREEN-VALUE) > 0 
    THEN DO:
        DISPLAY 0 @ gn-clie.impLC 
                ? @ gn-clie.fchvLC.
       MESSAGE "Cliente esta rechazado no se puede " SKIP
               "otorgar Linea Credito.             " SKIP 
               "Consulte con el Area Financiera.   " VIEW-AS ALERT-BOX ERROR.
       APPLY "ENTRY" TO gn-clie.impLC. 
       RETURN 'ADM-ERROR'.
    END.
    
    /* 03.08.08 SOLO controla en caso de modificarlo */
    IF Gn-clie.ImpLC <> DECIMAL(Gn-clie.ImpLC:SCREEN-VALUE) THEN DO:
        IF INTEGER(gn-clie.MonLC:SCREEN-VALUE) = 1 THEN DO:
           IF DECIMAL(gn-clie.ImpLC:SCREEN-VALUE) > ROUND( gn-LinUsr.MaxDol * wcambio, 2)  
           THEN DO:
           ...
           END.
        END.
        IF INTEGER(gn-clie.MonLC:SCREEN-VALUE) = 2 THEN DO:
           IF DECIMAL(gn-clie.ImpLC:SCREEN-VALUE) > gn-LinUsr.MaxDol 
           THEN DO:
              MESSAGE "El Maximo Credito que puede otorgar es US$. " gn-LinUsr.MaxDol
                      VIEW-AS ALERT-BOX WARNING.
              RETURN 'ADM-ERROR'.
           END.
        END.
