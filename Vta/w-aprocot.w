...
/* Parameters Definitions ---                                           */
 ...
 DEFINE NEW GLOBAL SHARED VARIABLE x-lista AS LOGICAL.
...
DEFINE VARIABLE h_b-cotdet AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     "Ingrese Descuento" VIEW-AS TEXT
          ...
          FGCOLOR 7 FONT 14
     "**** Seleccione con doble-click el articulo para poder visualizar sus Ordenes de Compra****" VIEW-AS TEXT
...
DO:
    MESSAGE 'Usted aprobara la cotización seleccionadas' SKIP
            '         ¿Esta usted seguro?             '
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/b-aprobcot.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-aprobcot ).
       RUN set-position IN h_b-aprobcot ( 1.15 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-aprobcot ( 9.42 , 83.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotdet ).
       RUN set-position IN h_b-cotdet ( 20.69 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-cotdet ( 8.62 , 124.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotdet. */
       RUN add-link IN adm-broker-hdl ( h_b-aprobcot , 'Record':U , h_b-cotdet ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotdet ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_b-detordcmp ( 20.62 , 4.57 ) NO-ERROR.
       RUN set-size IN h_b-detordcmp ( 8.69 , 101.43 ) NO-ERROR.
