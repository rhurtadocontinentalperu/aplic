...
&Scoped-define FIELDS-IN-QUERY-br_table vtadtrkped.CodDiv vtadtrkped.FechaI ~
vtadtrkped.FechaT TabGener.Descripcion ~
fFlgSit(vtadtrkped.FlgSit) @ x-Situacion DI-RutaC.NroDoc DI-RutaC.FchDoc ~
fEstado() @ x-Estado fEstadoDet() @ x-Motivo 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH vtadtrkped WHERE vtadtrkped.CodCia = CcbCDocu.CodCia ~
  AND vtadtrkped.CodDoc = CcbCDocu.CodPed ~
  AND vtadtrkped.NroPed = CcbCDocu.NroPed ~
  AND vtadtrkped.CodRef = CcbCDocu.CodDoc ~
  AND vtadtrkped.NroRef = CcbCDocu.NroDoc NO-LOCK, ~
  ...
  AND DI-RutaC.NroDoc = vtadtrkped.Libre_c02 OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH vtadtrkped WHERE vtadtrkped.CodCia = CcbCDocu.CodCia ~
  AND vtadtrkped.CodDoc = CcbCDocu.CodPed ~
  AND vtadtrkped.NroPed = CcbCDocu.NroPed ~
  AND vtadtrkped.CodRef = CcbCDocu.CodDoc ~
  AND vtadtrkped.NroRef = CcbCDocu.NroDoc NO-LOCK, ~
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 125 BY 8.08
         FONT 4
         TITLE "TRACKING POR COMPROBANTE".
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.vtadtrkped WHERE INTEGRAL.CcbCDocu ...,INTEGRAL.TabGener WHERE INTEGRAL.vtadtrkped ...,INTEGRAL.DI-RutaC WHERE INTEGRAL.vtadtrkped ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST OUTER, FIRST OUTER"
     _JoinCode[1]      = "INTEGRAL.vtadtrkped.CodCia = INTEGRAL.CcbCDocu.CodCia
  AND INTEGRAL.vtadtrkped.CodDoc = INTEGRAL.CcbCDocu.CodPed
  AND INTEGRAL.vtadtrkped.NroPed = INTEGRAL.CcbCDocu.NroPed
  AND INTEGRAL.vtadtrkped.CodRef = INTEGRAL.CcbCDocu.CodDoc
  AND INTEGRAL.vtadtrkped.NroRef = INTEGRAL.CcbCDocu.NroDoc"
...
"vtadtrkped.FechaT" "Término" "99/99/9999 HH:MM" "datetime" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.TabGener.Descripcion
...
"fFlgSit(vtadtrkped.FlgSit) @ x-Situacion" "Situación" "x(20)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.DI-RutaC.NroDoc
"DI-RutaC.NroDoc" "Hoja de Ruta" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.DI-RutaC.FchDoc
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* TRACKING POR COMPROBANTE */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* TRACKING POR COMPROBANTE */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* TRACKING POR COMPROBANTE */
DO:
