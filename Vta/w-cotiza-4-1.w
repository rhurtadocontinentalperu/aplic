...
DEFINE FRAME F-Main
     ...
     BUTTON-9 AT ROW 20.65 COL 67
     Btn-Grupos AT ROW 20.65 COL 86 WIDGET-ID 4
...
DO:
  RUN dispatch IN h_v-cotiza-4 ('imprime'). 
END.
...
DO:
    ...
    CASE s-coddiv :
        WHEN '00023' OR WHEN '00024' THEN RUN Excel_Utilex IN h_v-cotiza-4 (INPUT lchoice) .
        OTHERWISE RUN Excel2 IN h_v-cotiza-4 (INPUT lchoice) .
    END CASE.
...
DO:
  RUN Asigna-Texto IN h_v-cotiza-4.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-cotiza-4-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotiza-4 ).
       RUN set-position IN h_v-cotiza-4 ( 1.00 , 2.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv03 , 'TableIO':U , h_v-cotiza-4 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-cotiza-4 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-tmpcot-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpcot-3 ).
       RUN set-position IN h_b-tmpcot-3 ( 10.96 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpcot-3 ( 9.42 , 117.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ENABLE B-CCTE BUTTON-COT BUTTON-4 BUTTON-10 BUTTON-8 BUTTON-9 Btn-Grupos 
