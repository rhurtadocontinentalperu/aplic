...
&SCOPED-DEFINE l-lookup h_l-pedpen-4
...
DEFINE VARIABLE h_l-pedpen-4 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-pedpen-4.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Numero':U ,
             OUTPUT h_l-pedpen-4 ).
       RUN set-position IN h_l-pedpen-4 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-pedpen-4 ( 10.92 , 89.00 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-pedpen-4. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-pedpen-4 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-pedpen-4 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-pedpen-4 , 'AFTER':U ).
    END. /* Page 0 */
