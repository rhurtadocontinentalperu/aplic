...
&SCOPED-DEFINE CONDICION ( FacCLiqu.CodCia = S-CODCIA AND ~
                           FacCLiqu.CodDoc = "LIQ" AND ~
                           FacCLiqu.NroDoc BEGINS C-NROSER AND ~
                           FacCLiqu.FlgEst = "P" AND ~
                           FacCLiqu.CodCli BEGINS input-var-1 )
                           
&SCOPED-DEFINE CODIGO FacCLiqu.NroDoc 

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( FacCLiqu.NomCli BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( FacCLiqu.NomCli , FILL-IN-filtro ) <> 0 )
...
&Scoped-define INTERNAL-TABLES FacCLiqu

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table FacCLiqu.NroDoc FacCLiqu.NomCli ~
FacCLiqu.FchDoc FacCLiqu.NroPed ~
IF FacCLiqu.CodMon = 1 THEN 'S/.' ELSE  'US$' @ FacCLiqu.CodDiv ~
FacCLiqu.ImpTot 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH FacCLiqu WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table FacCLiqu
&Scoped-define FIRST-TABLE-IN-QUERY-br_table FacCLiqu
...
<FOREIGN-KEYS>
Nombres que inicien con|y||integral.FacCLiqu.NomCli
Nombres que contengan|y||integral.FacCLiqu.NomCli
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Liquidacion|y||integral.FacCLiqu.CodCia|yes,integral.FacCLiqu.CodDoc|yes,integral.FacCLiqu.NroDoc|yes
Nombre|||integral.FacCLiqu.NomCli|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "Liquidacion,Nombre",
     SortBy-Case = Liquidacion':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacCLiqu.NroDoc FORMAT "XXX-XXXXXXXX"
      FacCLiqu.NomCli FORMAT "x(40)"
      FacCLiqu.FchDoc COLUMN-LABEL "Fecha Emision"
      FacCLiqu.NroPed FORMAT "XXX-XXXXXXXX"
      IF FacCLiqu.CodMon = 1 THEN 'S/.' ELSE  'US$' @ FacCLiqu.CodDiv COLUMN-LABEL "Mon." FORMAT "XXXX"
      FacCLiqu.ImpTot
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 84.57 BY 8.62
...
DEFINE FRAME F-Main
     ...
     "Buscar x" VIEW-AS TEXT
          SIZE 8.43 BY .54 AT ROW 1.35 COL 12.57
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "integral.FacCLiqu"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST"
     _Where[1]         = "{&CONDICION}"
     _FldNameList[1]   > integral.FacCLiqu.NroDoc
"FacCLiqu.NroDoc" ? "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   > integral.FacCLiqu.NomCli
"FacCLiqu.NomCli" ? "x(40)" "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   > integral.FacCLiqu.FchDoc
"FacCLiqu.FchDoc" "Fecha Emision" ? "date" ? ? ? ? ? ? no ?
     _FldNameList[4]   > integral.FacCLiqu.NroPed
"FacCLiqu.NroPed" ? "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ?
     _FldNameList[5]   > "_<CALC>"
"IF FacCLiqu.CodMon = 1 THEN 'S/.' ELSE  'US$' @ FacCLiqu.CodDiv" "Mon." "XXXX" ? ? ? ? ? ? ? no ?
     _FldNameList[6]   = integral.FacCLiqu.ImpTot
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* Guia */
DO:
    IF INPUT FILL-IN-codigo = 0 THEN RETURN.
    C-NROGUI = C-NROSER + STRING(INPUT FILL-IN-codigo,"999999").
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Liquidacion':U THEN DO:
           &Scope SORTBY-PHRASE BY FacCLiqu.CodCia BY FacCLiqu.CodDoc BY FacCLiqu.NroDoc
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Nombre':U THEN DO:
           &Scope SORTBY-PHRASE BY FacCLiqu.NomCli
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
