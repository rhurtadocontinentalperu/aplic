...
&SCOPED-DEFINE l-lookup h_l-pncndxotros
...
DEFINE VARIABLE h_l-pncndxotros AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.58 COL 67.43
     Btn_Cancel AT ROW 8.77 COL 67.43
     Btn_Help AT ROW 10.58 COL 67.43
     SPACE(3.13) SKIP(0.71)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LKUP/l-pncndxotros.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-pncndxotros ).
       RUN set-position IN h_l-pncndxotros ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-pncndxotros ( 10.92 , 63.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.38 , 65.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-pncndxotros. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-pncndxotros ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-pncndxotros ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-pncndxotros , 'AFTER':U ).
    END. /* Page 0 */
