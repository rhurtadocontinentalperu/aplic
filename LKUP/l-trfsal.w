...
DEFINE VARIABLE I-NROSER   AS CHAR .
DEFINE VARIABLE X-CODALM AS CHARACTER.
X-CODALM = S-CODALM.
S-CODMOV = 03 .
/* Preprocesadores para condiciones */
&SCOPED-DEFINE CONDICION (Almcmov.CodCia = S-CODCIA AND ~
Almcmov.CodAlm = X-CODALM AND ~
...
&SCOPED-DEFINE CODIGO ( STRING(Almcmov.Nroser,"999") + STRING(Almcmov.NroDoc,"999999") )

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( ALMCMOV.observ BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX (  ALMCMOV.observ , FILL-IN-filtro ) <> 0 )
...
&Scoped-define FIELDS-IN-QUERY-br_table Almcmov.CodAlm Almcmov.NroSer ~
Almcmov.NroDoc Almcmov.FchDoc Almcmov.HorSal Almcmov.Observ Almcmov.usuario ~
Almcmov.NroRf2 
...
&Scoped-Define ENABLED-OBJECTS F-Serie F-CodAlm FILL-IN-codigo br_table ~
RECT-1 
&Scoped-Define DISPLAYED-OBJECTS F-Serie F-CodAlm FILL-IN-codigo CMB-filtro ~
FILL-IN-filtro 
...
<SORTBY-OPTIONS>
Código|y||integral.Almcmov.CodCia|yes,integral.Almcmov.CodAlm|yes,integral.Almcmov.TipMov|yes,integral.Almcmov.CodMov|yes,integral.Almcmov.NroDoc|no
Descripción|||integral.Almcmov.CodCia|yes,integral.Almcmov.CodAlm|yes,integral.Almcmov.TipMov|yes,integral.Almcmov.CodMov|yes,integral.Almcmov.NroDoc|no
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
DEFINE VARIABLE F-CodAlm AS CHARACTER FORMAT "x(8)":U 
     LABEL "Almacen Origen" 
     VIEW-AS FILL-IN 
     SIZE 6.14 BY .81
     BGCOLOR 15  NO-UNDO.

DEFINE VARIABLE F-Serie AS INTEGER FORMAT "999":U INITIAL 0 
     LABEL "Transferencia" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .81
     BGCOLOR 15  NO-UNDO.

DEFINE VARIABLE FILL-IN-codigo AS INTEGER FORMAT "9999999":U INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 7.29 BY .81
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Almcmov.CodAlm FORMAT "x(5)":U
      Almcmov.NroSer COLUMN-LABEL " Serie" FORMAT "9999":U
      Almcmov.NroDoc COLUMN-LABEL "Numero" FORMAT "9999999":U
      Almcmov.FchDoc COLUMN-LABEL "Fecha!Transferencia" FORMAT "99/99/9999":U
      Almcmov.HorSal COLUMN-LABEL "Hora de!Transferencia" FORMAT "X(8)":U
      Almcmov.Observ FORMAT "X(50)":U
      Almcmov.usuario FORMAT "x(8)":U
      Almcmov.NroRf2 FORMAT "x(10)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 75 BY 8.62
...
DEFINE FRAME F-Main
     F-Serie AT ROW 1.19 COL 23.43 COLON-ALIGNED
     F-CodAlm AT ROW 1.19 COL 46.57 COLON-ALIGNED
     FILL-IN-codigo AT ROW 1.19 COL 27.43 COLON-ALIGNED NO-LABEL
     ...
     "Buscar x" VIEW-AS TEXT
          SIZE 8.14 BY .54 AT ROW 1.42 COL 6.72
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[2]   > integral.Almcmov.NroSer
"Almcmov.NroSer" " Serie" "9999" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.Almcmov.NroDoc
"Almcmov.NroDoc" "Numero" "9999999" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > integral.Almcmov.FchDoc
"Almcmov.FchDoc" "Fecha!Transferencia" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > integral.Almcmov.HorSal
"Almcmov.HorSal" "Hora de!Transferencia" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   = integral.Almcmov.Observ
     _FldNameList[7]   = integral.Almcmov.usuario
     _FldNameList[8]   = integral.Almcmov.NroRf2
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-CodAlm L-table-Win
ON LEAVE OF F-CodAlm IN FRAME F-Main /* Almacen Origen */
DO:
  ASSIGN
    F-CodAlm.
...
&Scoped-define SELF-NAME F-Serie
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-Serie L-table-Win
ON LEAVE OF F-Serie IN FRAME F-Main /* Transferencia */
DO:
  ASSIGN F-serie.
END.
...
DO:
    ...
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE
            {&CONDICION} AND
            ( {&CODIGO} = STRING(INPUT F-Serie,"999")  + STRING(INPUT FILL-IN-codigo,"999999") )
            NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Código':U THEN DO:
      &Scope SORTBY-PHRASE BY Almcmov.CodCia BY Almcmov.CodAlm BY Almcmov.TipMov BY Almcmov.CodMov BY Almcmov.NroDoc DESCENDING
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = STRING( {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nrodoc )
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME {&FRAME-NAME}:
        /*F-CodAlm:SCREEN-VALUE = X-CODALM.*/
        DISPLAY F-SERIE.
    END.
