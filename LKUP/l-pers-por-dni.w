...
&SCOPED-DEFINE CONDICION ( PL-PERS.CodCia = s-codcia AND pl-pers.NroDocId > '')
&SCOPED-DEFINE CODIGO pl-pers.NroDocId
...
&Scoped-define FIELDS-IN-QUERY-br_table PL-PERS.NroDocId PL-PERS.patper ~
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.PL-PERS.NroDocId|yes
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      PL-PERS.NroDocId FORMAT "x(15)":U
      PL-PERS.patper FORMAT "X(40)":U WIDTH 25
      PL-PERS.matper FORMAT "X(40)":U WIDTH 26.43
      PL-PERS.nomper FORMAT "X(40)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 98.14 BY 8.62
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "{&Condicion}"
     _FldNameList[1]   = INTEGRAL.PL-PERS.NroDocId
     _FldNameList[2]   > INTEGRAL.PL-PERS.patper
"PL-PERS.patper" ? ? "character" ? ? ? ? ? ? no ? no no "25" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.PL-PERS.matper
"PL-PERS.matper" ? ? "character" ? ? ? ? ? ? no ? no no "26.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON FIND OF pl-pers DO:
    IF NOT CAN-FIND(FIRST pl-flg-mes WHERE PL-FLG-MES.CodCia = s-codcia 
                    AND PL-FLG-MES.codper = pl-pers.codper no-lock)
        THEN RETURN ERROR.
END.
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY PL-PERS.NroDocId
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nrodocid
