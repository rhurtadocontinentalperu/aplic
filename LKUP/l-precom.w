...
&SCOPED-DEFINE CONDICION ( Almmmatc.CodCia = S-CODCIA AND Almmmatc.CodMat = input-var-1)
&SCOPED-DEFINE CODIGO Almmmatc.codmat

/* Preprocesadores para cada campo filtro */
/*
&SCOPED-DEFINE FILTRO1 ( (Almmmatg.DesMat BEGINS FILL-IN-filtro) AND (Almmmatg.CodFam BEGINS F-CodFam) )
&SCOPED-DEFINE FILTRO2 ( (INDEX ( Almmmatg.DesMat , FILL-IN-filtro ) <> 0) AND (Almmmatg.CodFam BEGINS F-CodFam)  )
*/
...
Almmmatc.Prevta[1] Almmmatc.FchIng[1] 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almmmatc WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK, ~
      EACH almtabla WHERE TRUE /* Join to Almmmatc incomplete */ ~
      AND almtabla.Tabla = "co" ~
 AND almtabla.Codigo = almmmatc.Codcom NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&Scoped-Define ENABLED-OBJECTS RECT-1 CMB-filtro br_table FILL-IN-codigo ~
FILL-IN-filtro F-CodFam 
...
<SORTBY-OPTIONS>
Codigo|y||integral.Almmmatc.CodCia|yes,integral.Almmmatc.codmat|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
DEFINE FRAME F-Main
     ...
     FILL-IN-filtro AT ROW 2.19 COL 23 NO-LABEL
     F-CodFam AT ROW 1.23 COL 26.43 COLON-ALIGNED
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "integral.Almmmatc,integral.almtabla WHERE integral.Almmmatc ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "{&CONDICION}"
     _Where[2]         = "integral.almtabla.Tabla = ""co""
 AND integral.almtabla.Codigo = integral.almmmatc.Codcom"
...
DO:
    IF CMB-filtro = CMB-filtro:SCREEN-VALUE AND
        ...
        F-CodFam = F-CodFam:SCREEN-VALUE THEN RETURN.
    ASSIGN
        FILL-IN-filtro
        CMB-filtro
        F-CodFam.
    CASE CMB-filtro:
         WHEN 'Todos':U THEN DO:
              IF F-CodFam <> "" THEN RUN set-attribute-list('Key-Name=Nombres que inicien con').
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Codigo':U THEN DO:
      &Scope SORTBY-PHRASE BY Almmmatc.CodCia BY Almmmatc.codmat
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = STRING({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.Prevta[1]) /* codmat*/
