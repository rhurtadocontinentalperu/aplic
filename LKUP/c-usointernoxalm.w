...
&SCOPED-DEFINE l-lookup h_l-usointernoxalm
...
&Scoped-Define ENABLED-OBJECTS RECT-27 Btn_OK Btn_Cancel 
...
DEFINE VARIABLE h_l-usointernoxalm AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.69 COL 105.29
     Btn_Cancel AT ROW 7.27 COL 105.29
     RECT-27 AT ROW 3.81 COL 103.86
     SPACE(1.27) SKIP(0.07)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LKUP/l-usointernoxalm.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Almacen':U ,
             OUTPUT h_l-usointernoxalm ).
       RUN set-position IN h_l-usointernoxalm ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-usointernoxalm ( 10.96 , 101.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.38 , 103.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-usointernoxalm. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-usointernoxalm ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-usointernoxalm ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-usointernoxalm , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE RECT-27 Btn_OK Btn_Cancel 
      WITH FRAME C-Dialog.
  VIEW FRAME C-Dialog.
