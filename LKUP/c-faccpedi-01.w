...
&SCOPED-DEFINE l-lookup h_l-faccpedi-01
...
&Scoped-Define ENABLED-OBJECTS RECT-26 Btn_OK Btn_Cancel 
...
DEFINE VARIABLE h_l-faccpedi-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 5.04 COL 93.72
     Btn_Cancel AT ROW 7.58 COL 93.72
     RECT-26 AT ROW 4.19 COL 92.14
     SPACE(1.85) SKIP(2.23)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-faccpedi-01.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Numero':U ,
             OUTPUT h_l-faccpedi-01 ).
       RUN set-position IN h_l-faccpedi-01 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-faccpedi-01 ( 10.92 , 89.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.81 , 91.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-faccpedi-01. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-faccpedi-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-faccpedi-01 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-faccpedi-01 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE RECT-26 Btn_OK Btn_Cancel 
      WITH FRAME C-Dialog.
  VIEW FRAME C-Dialog.
