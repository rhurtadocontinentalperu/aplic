...
&SCOPED-DEFINE l-lookup h_l-canal
...
DEFINE VARIABLE h_l-canal AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4 COL 54.72
     Btn_Cancel AT ROW 6.31 COL 55
     SPACE(3.42) SKIP(4.56)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LKUP/l-canalvta.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-canal ).
       RUN set-position IN h_l-canal ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-canal ( 10.92 , 50.14 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.42 , 52.43 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-canal. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-canal ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-canal ,
