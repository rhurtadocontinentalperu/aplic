...
&SCOPED-DEFINE l-lookup h_l-pedpen-2
...
DEFINE VARIABLE h_l-pedpen-2 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.5 COL 78.72
     Btn_Cancel AT ROW 7.04 COL 78.72
     RECT-26 AT ROW 3.65 COL 77.14
     SPACE(2.28) SKIP(2.77)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-pedpen-2.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Numero':U ,
             OUTPUT h_l-pedpen-2 ).
       RUN set-position IN h_l-pedpen-2 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-pedpen-2 ( 10.92 , 74.00 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-pedpen-2. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-pedpen-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-pedpen-2 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-pedpen-2 , 'AFTER':U ).
    END. /* Page 0 */
