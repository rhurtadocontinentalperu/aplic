...
&SCOPED-DEFINE l-lookup h_l-cuenta-01
...
DEFINE VARIABLE h_l-cuenta-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 5.54 COL 57.29
     Btn_Cancel AT ROW 7.92 COL 57.29
     SPACE(3.27) SKIP(2.72)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-cuenta-01.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-cuenta-01 ).
       RUN set-position IN h_l-cuenta-01 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-cuenta-01 ( 10.92 , 51.72 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.46 , 55.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-cuenta-01. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-cuenta-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-cuenta-01 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-cuenta-01 , 'AFTER':U ).
    END. /* Page 0 */
