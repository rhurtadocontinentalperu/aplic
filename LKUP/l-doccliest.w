...
&SCOPED-DEFINE CONDICION ( CcbCDocu.CodCia = S-CODCIA ~
                            AND LOOKUP(CcbCDocu.CodDoc, input-var-1) > 0 ~
                            AND CcbCDocu.CodCli = input-var-2 ~
                            AND CcbCDocu.FlgEst = input-var-3 )
...
CcbCDocu.NomCli CcbCDocu.FchDoc CcbCDocu.FchVto ~
(IF CcbcDocu.CodMon = 1 THEN 'S/.' ELSE 'US$') @ CcbCDocu.FlgUbi ~
CcbCDocu.SdoAct 
...
Cliente|||integral.CcbCDocu.NomCli|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.NroDoc FORMAT "XXX-XXXXXXXX":U WIDTH 9.72
      CcbCDocu.NomCli FORMAT "x(40)":U
      CcbCDocu.FchDoc COLUMN-LABEL "Fecha Emision" FORMAT "99/99/9999":U
      CcbCDocu.FchVto COLUMN-LABEL "F.Vencimiento" FORMAT "99/99/9999":U
      (IF CcbcDocu.CodMon = 1 THEN 'S/.' ELSE 'US$') @ CcbCDocu.FlgUbi COLUMN-LABEL "Mon" FORMAT "XXXX":U
      CcbCDocu.SdoAct COLUMN-LABEL "Saldo Actual" FORMAT "->>,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 81.72 BY 9.5
...
"CodDoc" "Doc." ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.CcbCDocu.NroDoc
"NroDoc" ? "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no "9.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FchVto" "F.Vencimiento" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"(IF CcbcDocu.CodMon = 1 THEN 'S/.' ELSE 'US$') @ CcbCDocu.FlgUbi" "Mon" "XXXX" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Asigna-Documentos L-table-Win 
...
DEFINE VARIABLE S-TOTAL AS DECIMAL.
...
DO I = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
   IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(I) THEN DO:      
      FIND FIRST MVTO WHERE
           MVTO.CodRef = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodDoc AND
           MVTO.NroRef = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.NroDoc 
           NO-LOCK NO-ERROR.
      IF NOT AVAILABLE MVTO THEN DO:
         S-TOTAL = 0.
         CREATE MVTO.
         ASSIGN MVTO.CodCia  = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodCia
                MVTO.codRef  = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodDoc 
                MVTO.NroRef  = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.NroDoc
                MVTO.TpoRef  = "O".
         IF {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodMon = S-CODMON THEN s-total = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.SdoAct.
         ELSE DO:
         IF {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodMon = 1 THEN s-total = ({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.SdoAct / S-TPOCMB).
            ELSE s-total = ({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.SdoAct * S-TPOCMB).
         END.
         MVTO.ImpTot  = s-Total.
      END.
   END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Deselect-All L-table-Win 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Select-All L-table-Win 
