...
&SCOPED-DEFINE CONDICION ( GN-DIVI.codcia = S-CODCIA )
...
"CodDiv" " Codigo" "x(8)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.GN-DIVI.DesDiv
"DesDiv" ? "X(60)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO I = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
   IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(I) THEN DO:
      output-var-1 = ROWID( {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} ).
         ...
         ELSE output-var-2 = output-var-2 + "," + {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodDiv.
         output-var-3 = "".
   END.
