...
DEFINE VARIABLE S-CODALM   AS CHAR.
...
ELSE DO:
    MESSAGE 'Movimiento de almacén no definido' SKIP
        'Revisar la condiguración del código:' s-coddoc
        VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.

&SCOPED-DEFINE CONDICION (Almcmov.CodCia = S-CODCIA AND Almcmov.CodAlm = X-CODALM AND ~
Almcmov.TipMov = "I" AND Almcmov.CodMov = S-CODMOV AND Almcmov.FlgEst = "P" )
...
DEFINE TEMP-TABLE ttExcelData
    FIELD   tNroIng     AS  INT     COLUMN-LABEL "No.Ingreso Almacen"
    FIELD   tFecha      AS  DATE    COLUMN-LABEL "Fecha Docum."
    FIELD   tFactura    AS  CHAR    COLUMN-LABEL "Factura"
    FIELD   tDevol      AS  CHAR    COLUMN-LABEL "Devolucion"
    FIELD   tnombre     AS  CHAR    COLUMN-LABEL "Nombre".
...
Almcmov.NroRf1 Almcmov.NroRf2 Almcmov.NomRef 
...
&Scoped-Define ENABLED-OBJECTS X-CodAlm FILL-IN-codigo CMB-filtro ~
FILL-IN-filtro br_table RECT-1 
&Scoped-Define DISPLAYED-OBJECTS X-CodAlm FILL-IN-codigo CMB-filtro ~
FILL-IN-filtro 
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.Almcmov.NroDoc|no
Descripción|||INTEGRAL.Almcmov.NomRef|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
DEFINE VARIABLE X-CodAlm AS CHARACTER FORMAT "X(4)":U 
     LABEL "Almacen" 
     VIEW-AS FILL-IN 
     SIZE 6.14 BY .81
     BGCOLOR 15  NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Almcmov.NroDoc COLUMN-LABEL "No.Ingreso!Almacén" FORMAT "999999999999":U
      Almcmov.FchDoc FORMAT "99/99/9999":U
      Almcmov.NroRf1 COLUMN-LABEL "Factura" FORMAT "XXX-XXXXXXXX":U
            WIDTH 10.72
      Almcmov.NroRf2 COLUMN-LABEL "Devolución" FORMAT "XXX-XXXXXXXX":U
      Almcmov.NomRef FORMAT "x(50)":U WIDTH 32.14
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 77.14 BY 8.62
...
DEFINE FRAME F-Main
     X-CodAlm AT ROW 1.19 COL 8 COLON-ALIGNED WIDGET-ID 2
     FILL-IN-codigo AT ROW 1.19 COL 37 COLON-ALIGNED
...
"Almcmov.NroDoc" "No.Ingreso!Almacén" "999999999999" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.Almcmov.FchDoc
     _FldNameList[3]   > INTEGRAL.Almcmov.NroRf1
"Almcmov.NroRf1" "Factura" "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no "10.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.Almcmov.NroRf2
"Almcmov.NroRf2" "Devolución" "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.Almcmov.NomRef
"Almcmov.NomRef" ? ? "character" ? ? ? ? ? ? no ? no no "32.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* No Ingreso Almacén */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL X-CodAlm L-table-Win
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY Almcmov.NroDoc DESCENDING
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY Almcmov.NomRef
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nrorf2.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE enviar-excel L-table-Win 
...
DO  WHILE AVAILABLE almcmov:
    CREATE ttExcelData.
        ASSIGN tNroIng = almcmov.nrodoc
                tFecha = almcmov.fchdoc
                tfactura = almcmov.nrorf1
                tDevol  = almcmov.nrorf2
                tnombre = almcmov.nomref.
...
run pi-crea-archivo-csv IN hProc (input  buffer ttExcelData:handle,
                        /*input  session:temp-directory + "file"*/ c-xls-file,
                        output c-csv-file) .

run pi-crea-archivo-xls  IN hProc (input  buffer ttExcelData:handle,
                        input  c-csv-file,
                        output c-xls-file) .
