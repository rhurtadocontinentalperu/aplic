...
DEFINE VAR cl-codcia AS INT NO-UNDO.
...
&SCOPED-DEFINE CODIGO GN-CLIE.codcia
...
&Scoped-define INTERNAL-TABLES T-CLIE

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table T-CLIE.CodCli T-CLIE.NomCli 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH T-CLIE WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH T-CLIE WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table T-CLIE
&Scoped-define FIRST-TABLE-IN-QUERY-br_table T-CLIE
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      T-CLIE.CodCli COLUMN-LABEL "<<<Codigo>>>" FORMAT "x(11)":U
      T-CLIE.NomCli FORMAT "x(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* SETTINGS FOR FILL-IN FILL-IN-codigo IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       FILL-IN-codigo:HIDDEN IN FRAME F-Main           = TRUE.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.T-CLIE"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > Temp-Tables.T-CLIE.CodCli
"T-CLIE.CodCli" "<<<Codigo>>>" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = Temp-Tables.T-CLIE.NomCli
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ccbcdocu NO-LOCK WHERE ccbcdocu.codcia = s-codcia
      ...
      AND Faccpedi.coddiv = input-var-1,
      FIRST gn-clie NO-LOCK WHERE gn-clie.codcia = cl-codcia
      AND gn-clie.codcli = ccbcdocu.codcli:
      FIND T-CLIE OF Gn-clie NO-LOCK NO-ERROR.
      IF NOT AVAILABLE T-CLIE THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "T-CLIE"}
