...
&SCOPED-DEFINE l-lookup h_l-almtmov-validos
...
DEFINE VARIABLE h_l-almtmov-validos AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.46 COL 51.29
     Btn_Cancel AT ROW 7.08 COL 51.29
     RECT-2 AT ROW 3.69 COL 49.29
     SPACE(0.70) SKIP(2.73)
...
ON WINDOW-CLOSE OF FRAME C-Dialog /* Movimientos */
DO:  
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-almtmov-validos.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-almtmov-validos ).
       RUN set-position IN h_l-almtmov-validos ( 1.08 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-almtmov-validos ( 10.92 , 47.43 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.38 , 48.86 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-almtmov-validos. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-almtmov-validos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-almtmov-validos ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-almtmov-validos , 'AFTER':U ).
    END. /* Page 0 */
