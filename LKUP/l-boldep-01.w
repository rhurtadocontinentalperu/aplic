...
&SCOPED-DEFINE CONDICION ccbcdocu.CodCia = S-CODCIA AND ccbcdocu.coddiv = s-coddiv ~
...
ccbcdocu.FlgUBi ccbcdocu.CodCta ccbcdocu.CodAge 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH ccbcdocu WHERE ~{&KEY-PHRASE} ~
      AND {&condicion} USE-INDEX LLAVE10 NO-LOCK ~
    BY ccbcdocu.CodCia DESCENDING ~
       BY ccbcdocu.CodDiv DESCENDING ~
        BY ccbcdocu.FchDoc DESCENDING INDEXED-REPOSITION
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 98.14 BY 8.5
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "INTEGRAL.ccbcdocu.CodCia|no,INTEGRAL.ccbcdocu.CodDiv|no,INTEGRAL.ccbcdocu.FchDoc|no"
     _Where[1]         = "{&condicion} USE-INDEX LLAVE10"
     _FldNameList[1]   > integral.ccbcdocu.CodDiv
"CodDiv" "Division" "XXXXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FchDoc" "Fecha Dcmto" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > integral.ccbcdocu.NroRef
"NroRef" "Numero!De Deposito" "X(15)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"C-MON @ C-MON" "Mon." "XXX" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[9]   = integral.ccbcdocu.CodAge
