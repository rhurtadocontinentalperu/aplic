...
almubimat.CodAlm = input-var-1 AND ~
almubimat.CodMat = input-var-2 )

&SCOPED-DEFINE CODIGO CodUbi

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( almubimat.CodUbi BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( almubimat.CodUbi , FILL-IN-filtro ) <> 0 )
...
&Scoped-define INTERNAL-TABLES almubimat almtubic Almmmate

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table almubimat.CodAlm almubimat.CodMat ~
almubimat.CodUbi almubimat.CodZona almtubic.Libre_d03 almtubic.Libre_d04 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH almubimat WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK, ~
      FIRST almtubic OF almubimat OUTER-JOIN NO-LOCK, ~
      FIRST Almmmate OF almubimat ~
      WHERE Almmmate.CodUbi = "" OUTER-JOIN NO-LOCK ~
    BY Almmmate.codmat ~
       BY almtubic.Libre_d03 ~
        BY almtubic.Libre_d04
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH almubimat WHERE ~{&KEY-PHRASE} ~
    ...
    BY Almmmate.codmat ~
       BY almtubic.Libre_d03 ~
        BY almtubic.Libre_d04.
&Scoped-define TABLES-IN-QUERY-br_table almubimat almtubic Almmmate
&Scoped-define FIRST-TABLE-IN-QUERY-br_table almubimat
&Scoped-define SECOND-TABLE-IN-QUERY-br_table almtubic
...
<FOREIGN-KEYS>
Nombres que inicien con|y||INTEGRAL.almubimat.CodUbi
Nombres que contengan|y||INTEGRAL.almubimat.CodUbi
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.almubimat.CodUbi|yes
Descripción|||INTEGRAL.almubimat.CodUbi|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      almubimat.CodAlm COLUMN-LABEL "Almacén" FORMAT "x(8)":U
      ...
      almtubic.Libre_d03 COLUMN-LABEL "Pasaje" FORMAT "x(2)":U
      almtubic.Libre_d04 COLUMN-LABEL "Piso" FORMAT "x(2)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.almubimat,INTEGRAL.almtubic OF INTEGRAL.almubimat,INTEGRAL.Almmmate OF INTEGRAL.almubimat"
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ", FIRST OUTER USED, FIRST OUTER USED"
     _OrdList          = "INTEGRAL.Almmmate.codmat|yes,INTEGRAL.almtubic.Libre_d03|yes,INTEGRAL.almtubic.Libre_d04|yes"
     _Where[1]         = "{&Condicion}"
     _Where[3]         = "Almmmate.CodUbi = """""
     _FldNameList[1]   > INTEGRAL.almubimat.CodAlm
"almubimat.CodAlm" "Almacén" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.almubimat.CodMat
"almubimat.CodMat" "Artículo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.almubimat.CodUbi
"almubimat.CodUbi" "Ubicación" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.almubimat.CodZona
"almubimat.CodZona" "Zona" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.almtubic.Libre_d03
"almtubic.Libre_d03" "Pasaje" "x(2)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.almtubic.Libre_d04
"almtubic.Libre_d04" "Piso" "x(2)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  IF AVAILABLE AlmUbiMat THEN DO:
      ...
      IF RETURN-VALUE = 'PRINCIPAL' THEN DO:
          ...
          almtubic.Libre_d03:FGCOLOR IN BROWSE {&browse-name} = 0.
          almtubic.Libre_d04:FGCOLOR IN BROWSE {&browse-name} = 0.
          almubimat.CodAlm:FGCOLOR IN BROWSE {&browse-name} = 0.
          ...
          almtubic.Libre_d03:BGCOLOR IN BROWSE {&browse-name} = 10.
          almtubic.Libre_d04:BGCOLOR IN BROWSE {&browse-name} = 10.
          almubimat.CodAlm:BGCOLOR IN BROWSE {&browse-name} = 10.
      END.
  END.
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY almubimat.CodUbi
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codubi
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codubi.
