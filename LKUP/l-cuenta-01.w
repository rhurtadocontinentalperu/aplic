...
&SCOPED-DEFINE CONDICION ( cb-ctas.CodCia = cb-codcia ~
AND LENGTH(cb-ctas.Codcta) >= 6 ~
AND LOOKUP (SUBSTRING(cb-ctas.codcta,1,3), input-var-1) > 0 ~
AND cb-ctas.codbco BEGINS input-var-2 )
               
&SCOPED-DEFINE CODIGO cb-ctas.Codcta

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( cb-ctas.Nomcta BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( cb-ctas.Nomcta , FILL-IN-filtro ) <> 0 )
...
&Scoped-define QUERY-STRING-br_table FOR EACH cb-ctas WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH cb-ctas WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
...
<FOREIGN-KEYS>
Nombres que inicien con|y||integral.cb-ctas.Nomcta
Nombres que contengan|y||integral.cb-ctas.Nomcta
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Código|y||integral.cb-ctas.CodCia|yes,integral.cb-ctas.Codcta|yes
Descripción|||integral.cb-ctas.Nomcta|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      cb-ctas.Codcta COLUMN-LABEL "<    Cuenta    >" FORMAT "x(10)":U
      cb-ctas.Nomcta FORMAT "x(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 49.86 BY 8.62
...
DEFINE FRAME F-Main
     ...
     FILL-IN-filtro AT ROW 2.23 COL 22.57 NO-LABEL
     br_table AT ROW 3.12 COL 1.29
...
"cb-ctas.Codcta" "<    Cuenta    >" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE
            {&CONDICION} AND
            ( {&CODIGO} BEGINS INPUT FILL-IN-codigo )
            NO-LOCK NO-ERROR.
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY cb-ctas.CodCia BY cb-ctas.Codcta
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY cb-ctas.Nomcta
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codcta
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nomcta.
