...
&SCOPED-DEFINE l-lookup h_l-comprobante-mostrador
...
DEFINE VARIABLE h_l-comprobante-mostrador AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.38 COL 88.43
     Btn_Cancel AT ROW 8.58 COL 88.43
     Btn_Help AT ROW 10.38 COL 88.43
     SPACE(2.99) SKIP(0.91)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LKUP/l-comprobante-mostrador.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-comprobante-mostrador ).
       RUN set-position IN h_l-comprobante-mostrador ( 1.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_l-comprobante-mostrador ( 10.69 , 84.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.19 , 86.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-comprobante-mostrador. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-comprobante-mostrador ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-comprobante-mostrador ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-comprobante-mostrador , 'AFTER':U ).
    END. /* Page 0 */
