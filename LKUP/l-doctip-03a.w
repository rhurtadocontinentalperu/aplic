...
/* x-coddoc = IF input-var-2 = "11111111" THEN "BOL" ELSE "FAC,BOL,LET,N/D". */
x-CodDoc = input-var-3.
IF TRUE <> (x-CodDoc > '') THEN x-coddoc = "FAC,BOL,LET,N/D,CHQ,DCO".

/* 16/01/2024: S.Leon G.Condor */
&SCOPED-DEFINE CONDICION (TRUE )

&SCOPED-DEFINE CODIGO t-CcbCDocu.NroDoc 

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( t-CcbCDocu.Nomcli BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( t-CcbCDocu.Nomcli , FILL-IN-filtro ) <> 0 )
...
&Scoped-define INTERNAL-TABLES t-CcbCDocu gn-ConVt CcbCDocu

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table t-CcbCDocu.CodDoc t-CcbCDocu.NroDoc ~
t-CcbCDocu.NomCli t-CcbCDocu.FchDoc t-CcbCDocu.FchVto ~
(IF t-CcbcDocu.CodMon = 1 THEN 'S/.' ELSE 'US$') @ t-CcbCDocu.FlgUbi ~
t-CcbCDocu.ImpTot t-CcbCDocu.SdoAct t-CcbCDocu.FmaPgo gn-ConVt.Nombr 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH t-CcbCDocu WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      FIRST gn-ConVt WHERE gn-ConVt.Codig = t-CcbCDocu.FmaPgo NO-LOCK, ~
      FIRST CcbCDocu OF t-CcbCDocu NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH t-CcbCDocu WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      FIRST gn-ConVt WHERE gn-ConVt.Codig = t-CcbCDocu.FmaPgo NO-LOCK, ~
      FIRST CcbCDocu OF t-CcbCDocu NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table t-CcbCDocu gn-ConVt CcbCDocu
&Scoped-define FIRST-TABLE-IN-QUERY-br_table t-CcbCDocu
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      t-CcbCDocu.CodDoc COLUMN-LABEL "Tipo!Doc." FORMAT "x(3)":U
      t-CcbCDocu.NroDoc FORMAT "X(12)":U WIDTH 10.29
      t-CcbCDocu.NomCli COLUMN-LABEL "Nombre Cliente" FORMAT "x(50)":U
      t-CcbCDocu.FchDoc COLUMN-LABEL "Fecha Emision" FORMAT "99/99/9999":U
      t-CcbCDocu.FchVto FORMAT "99/99/9999":U
      (IF t-CcbcDocu.CodMon = 1 THEN 'S/.' ELSE 'US$') @ t-CcbCDocu.FlgUbi COLUMN-LABEL "Mon" FORMAT "XXXX":U
      t-CcbCDocu.ImpTot FORMAT "->>,>>>,>>9.99":U
      t-CcbCDocu.SdoAct COLUMN-LABEL "Saldo Actual" FORMAT "->>,>>>,>>9.99":U
      t-CcbCDocu.FmaPgo COLUMN-LABEL "Cond.!Venta" FORMAT "X(8)":U
      gn-ConVt.Nombr COLUMN-LABEL "Cond.Vta!Descripcion" FORMAT "X(50)":U
            WIDTH 31.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE NO-SCROLLBAR-VERTICAL SIZE 115.14 BY 9.5
...
DEFINE FRAME F-Main
     FILL-IN-codigo AT ROW 1.19 COL 13.72 COLON-ALIGNED NO-LABEL
     CMB-filtro AT ROW 1.19 COL 28.72 NO-LABEL
     FILL-IN-filtro AT ROW 1.23 COL 49.57 NO-LABEL
     br_table AT ROW 2.19 COL 1.86
     "Documento #" VIEW-AS TEXT
          SIZE 12 BY .65 AT ROW 1.27 COL 2.43
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.t-CcbCDocu,INTEGRAL.gn-ConVt WHERE Temp-Tables.t-CcbCDocu ...,INTEGRAL.CcbCDocu OF Temp-Tables.t-CcbCDocu"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = "USED, FIRST, FIRST USED"
     _JoinCode[2]      = "INTEGRAL.gn-ConVt.Codig = Temp-Tables.t-CcbCDocu.FmaPgo"
     _FldNameList[1]   > Temp-Tables.t-CcbCDocu.CodDoc
"t-CcbCDocu.CodDoc" "Tipo!Doc." ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.t-CcbCDocu.NroDoc
"t-CcbCDocu.NroDoc" ? ? "character" ? ? ? ? ? ? no ? no no "10.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.t-CcbCDocu.NomCli
"t-CcbCDocu.NomCli" "Nombre Cliente" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.t-CcbCDocu.FchDoc
"t-CcbCDocu.FchDoc" "Fecha Emision" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   = Temp-Tables.t-CcbCDocu.FchVto
     _FldNameList[6]   > "_<CALC>"
"(IF t-CcbcDocu.CodMon = 1 THEN 'S/.' ELSE 'US$') @ t-CcbCDocu.FlgUbi" "Mon" "XXXX" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   = Temp-Tables.t-CcbCDocu.ImpTot
     _FldNameList[8]   > Temp-Tables.t-CcbCDocu.SdoAct
"t-CcbCDocu.SdoAct" "Saldo Actual" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.t-CcbCDocu.FmaPgo
"t-CcbCDocu.FmaPgo" "Cond.!Venta" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > INTEGRAL.gn-ConVt.Nombr
"gn-ConVt.Nombr" "Cond.Vta!Descripcion" ? "character" ? ? ? ? ? ? no ? no no "31.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
   ...
   IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(I) THEN DO:
      output-var-1 = ROWID( {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} ).
         IF output-var-2 = "" THEN 
            output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodDoc + 
                           {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.NroDoc .
         ELSE output-var-2 = output-var-2 + "," + 
...
DO j = 1 TO NUM-ENTRIES(input-var-1):
    FOR EACH ccbcdocu NO-LOCK WHERE ccbcdocu.codcia = s-codcia AND
        ccbcdocu.codcli = input-var-2 AND
        ccbcdocu.flgest = ENTRY(j, input-var-1):
        IF LOOKUP(TRIM(ccbcdocu.coddoc), x-coddoc) = 0 THEN NEXT.
        IF ccbcdocu.flgsit = "X" THEN NEXT.
        CREATE t-ccbcdocu.
