...
DEF SHARED VAR s-codalm AS CHAR.    /* RHC 19/01/2016 */

&SCOPED-DEFINE CONDICION ( faccpedi.codcia = s-codcia ~
AND faccpedi.coddiv = s-coddiv ~
AND faccpedi.coddoc = input-var-1 ~
AND LOOKUP(faccpedi.flgest, input-var-2) > 0 ~
AND LOOKUP(faccpedi.flgsit, input-var-3) > 0) ~
AND faccpedi.codalm = s-codalm

&SCOPED-DEFINE CODIGO faccpedi.nroped

RUN Carga-Temporal.     /* Temporal de trabajo */

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( T-CPEDI.nomcli BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( T-CPEDI.nomcli , FILL-IN-filtro ) <> 0 )
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.NomCli FORMAT "x(50)":U WIDTH 44.29
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"FacCPedi.NomCli" ? ? "character" ? ? ? ? ? ? no ? no no "44.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = {&SECOND-TABLE-IN-QUERY-{&BROWSE-NAME}}.nroped
        output-var-3 = {&SECOND-TABLE-IN-QUERY-{&BROWSE-NAME}}.codcli.
...
DO k = 1 TO NUM-ENTRIES(input-var-2):
    FOR EACH Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-codcia
        AND Faccpedi.coddiv = s-coddiv
        AND Faccpedi.coddoc = input-var-1
        AND Faccpedi.flgest = ENTRY(k, input-var-2):
        IF LOOKUP(faccpedi.flgsit, input-var-3) > 0 
            AND faccpedi.codalm = s-codalm THEN DO:
            CREATE T-CPEDI.
