...
AND ccbcmvto.coddiv = input-var-1 ~
AND ccbcmvto.coddoc = input-var-2 ~
AND ccbcmvto.nrodoc BEGINS input-var-3 )

&SCOPED-DEFINE CODIGO ccbcmvto.nrodoc
...
&Scoped-define FIELDS-IN-QUERY-br_table CcbCMvto.CodDoc ~
SUBSTRING(nrodoc,3) @ nrodoc CcbCMvto.FchDoc CcbCMvto.CodCli gn-clie.NomCli 
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.CcbCMvto.NroDoc|no
Descripción|||INTEGRAL.gn-clie.NomCli|yes,INTEGRAL.CcbCMvto.NroDoc|no
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbCMvto.CodDoc FORMAT "x(3)":U
      SUBSTRING(nrodoc,3) @ nrodoc COLUMN-LABEL "Número"
      CcbCMvto.FchDoc FORMAT "99/99/9999":U
      CcbCMvto.CodCli FORMAT "x(11)":U WIDTH 9.57
      gn-clie.NomCli FORMAT "x(250)":U WIDTH 35.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   = INTEGRAL.CcbCMvto.CodDoc
     _FldNameList[2]   > "_<CALC>"
"SUBSTRING(nrodoc,3) @ nrodoc" "Número" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = INTEGRAL.CcbCMvto.FchDoc
     _FldNameList[4]   > INTEGRAL.CcbCMvto.CodCli
"CcbCMvto.CodCli" ? ? "character" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.gn-clie.NomCli
"gn-clie.NomCli" ? ? "character" ? ? ? ? ? ? no ? no no "35.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* Buscar el Numero */
DO:
    ...
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE
            {&CONDICION} AND
            ( {&CODIGO} = input-var-3 + STRING(INPUT FILL-IN-codigo, '999999') )
            NO-LOCK NO-ERROR.
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY CcbCMvto.NroDoc DESCENDING
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY gn-clie.NomCli BY CcbCMvto.NroDoc DESCENDING
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
