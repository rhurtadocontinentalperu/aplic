...
&SCOPED-DEFINE l-lookup h_l-almtab02
...
&Scoped-Define ENABLED-OBJECTS RECT-19 Btn_OK Btn_Cancel Btn_Help 
...
DEFINE VARIABLE h_l-almtab02 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.42 COL 50.43
     Btn_Cancel AT ROW 7.08 COL 50.43
     Btn_Help AT ROW 9.73 COL 50.43
     RECT-19 AT ROW 3.54 COL 47.86
     SPACE(1.56) SKIP(0.21)
 ...
 *         IF i = 1 
 *         THEN x-Canales = TRIM(AlmTabla.Codigo).
 *         ELSE x-Canales = x-Canales + ',' + TRIM(AlmTabla.Codigo).
 *     END.
 *   END.*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-almtab03.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-almtab02 ).
       RUN set-position IN h_l-almtab02 ( 1.19 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.92 , 45.43 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 47.86 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-almtab02. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-almtab02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-almtab02 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-almtab02 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE RECT-19 Btn_OK Btn_Cancel Btn_Help 
      WITH FRAME C-Dialog.
