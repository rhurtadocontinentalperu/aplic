...
Faccpedm.CodAlm = input-var-1 AND ~
Faccpedm.CodDoc = input-var-2 AND ~
Faccpedm.FlgEst = input-var-3 

&SCOPED-DEFINE CODIGO Faccpedm.CodCli 
...
&Scoped-define FIELDS-IN-QUERY-br_table Faccpedm.NroPed Faccpedm.FchPed ~
Faccpedm.CodCli Faccpedm.NomCli 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH Faccpedm WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    BY Faccpedm.FchPed DESCENDING
...
<SORTBY-OPTIONS>
Numero|y||integral.Faccpedm.CodCia|yes,integral.Faccpedm.CodDoc|yes,integral.Faccpedm.NroPed|yes
Cliente|||integral.Faccpedm.CodCia|yes,integral.Faccpedm.CodDoc|yes,integral.Faccpedm.CodCli|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "':U + 'Numero,Cliente' + '",
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Faccpedm.NroPed COLUMN-LABEL "Pedido" FORMAT "999-999999":U
            WIDTH 10
      Faccpedm.FchPed COLUMN-LABEL "     Fecha!       Emisión" FORMAT "99/99/9999":U
      Faccpedm.CodCli FORMAT "x(11)":U WIDTH 13
      Faccpedm.NomCli FORMAT "x(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 87.14 BY 8.62
...
DEFINE FRAME F-Main
     FILL-IN-codigo AT ROW 1.19 COL 24.14 COLON-ALIGNED
     CMB-filtro AT ROW 2.08 COL 1.86 NO-LABEL
     FILL-IN-filtro AT ROW 2.12 COL 26.14 NO-LABEL
     br_table AT ROW 3.08 COL 1.86
     "Buscar x" VIEW-AS TEXT
          SIZE 8.29 BY .58 AT ROW 1.31 COL 11.72
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     ...
     _TblOptList       = ", FIRST"
     _OrdList          = "INTEGRAL.Faccpedm.FchPed|no"
     _Where[1]         = "{&CONDICION}"
     _FldNameList[1]   > integral.Faccpedm.NroPed
"Faccpedm.NroPed" "Pedido" "999-999999" "character" ? ? ? ? ? ? no ? no no "10" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.Faccpedm.FchPed
"Faccpedm.FchPed" "     Fecha!       Emisión" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.Faccpedm.CodCli
"Faccpedm.CodCli" ? ? "character" ? ? ? ? ? ? no ? no no "13" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   = INTEGRAL.Faccpedm.NomCli
...
/*     FILL-IN-codigo:SCREEN-VALUE = input-var-1 + STRING(INTEGER(FILL-IN-codigo:SCREEN-VALUE),"999999"). */
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Numero':U THEN DO:
      &Scope SORTBY-PHRASE BY Faccpedm.CodCia BY Faccpedm.CodDoc BY Faccpedm.NroPed
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    WHEN 'Cliente':U THEN DO:
      &Scope SORTBY-PHRASE BY Faccpedm.CodCia BY Faccpedm.CodDoc BY Faccpedm.CodCli
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    OTHERWISE DO:
