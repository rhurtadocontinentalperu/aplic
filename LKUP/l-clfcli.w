...
&SCOPED-DEFINE CODIGO ClfClie.Categoria

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( ClfClie.DesCat BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( ClfClie.DesCat , FILL-IN-filtro ) <> 0 )
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH ClfClie WHERE ~{&KEY-PHRASE} ~
...
<FOREIGN-KEYS>
Nombres que inicien con|y||integral.ClfClie.DesCat
Nombres que contengan|y||integral.ClfClie.DesCat
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Código|y||integral.ClfClie.Categoria|yes
Descripción|||integral.ClfClie.DesCat|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ClfClie.Categoria
      ClfClie.DesCat
      ClfClie.MtoCmp COLUMN-LABEL "Monto de!la Compra"
      ClfClie.PorDsc COLUMN-LABEL "% DSCTO.!PROPIOS"
      ClfClie.PorDsc1 COLUMN-LABEL "% DSCTO.!TERCEROS"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 53.29 BY 9
...
DEFINE FRAME F-Main
     ...
     br_table AT ROW 3.04 COL 1.72
     RECT-1 AT ROW 1 COL 1
     "Buscar x :" VIEW-AS TEXT
          SIZE 8.86 BY .62 AT ROW 1.27 COL 7.29
          FONT 6
...
"MtoCmp" "Monto de!la Compra" ? "decimal" ? ? ? ? ? ? no ?
     _FldNameList[4]   > integral.ClfClie.PorDsc
"PorDsc" "% DSCTO.!PROPIOS" ? "decimal" ? ? ? ? ? ? no ?
     _FldNameList[5]   > integral.ClfClie.PorDsc1
"PorDsc1" "% DSCTO.!TERCEROS" ? "decimal" ? ? ? ? ? ? no ?
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY ClfClie.Categoria
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY ClfClie.DesCat
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.Categoria 
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.DesCat.
