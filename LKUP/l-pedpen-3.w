...
FacCPedi.CodDiv = input-var-1 AND ~
FacCPedi.CodDoc = input-var-2 AND ~
FacCPedi.FlgEst = 'P' AND ~
FacCPedi.Cmpbnte = input-var-3 )

&SCOPED-DEFINE CODIGO FacCPedi.CodCli 
...
&Scoped-define FIELDS-IN-QUERY-br_table FacCPedi.CodCli FacCPedi.NroPed ~
gn-clie.NomCli FacCPedi.FchPed C-MON @ C-MON FacCPedi.ImpTot FacCPedi.Hora 
...
<SORTBY-OPTIONS>
Numero|y||integral.FacCPedi.CodCia|yes,integral.FacCPedi.CodDoc|yes,integral.FacCPedi.NroPed|yes
Cliente|||integral.FacCPedi.CodCia|yes,integral.FacCPedi.CodDoc|yes,integral.FacCPedi.CodCli|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.NroPed COLUMN-LABEL "Pedido" FORMAT "999-999999":U
            WIDTH 10
...
/* Query rebuild information for BROWSE br_table
  ...
  AND integral.gn-clie.CodCli = integral.FacCPedi.CodCli"
     _FldNameList[1]   > INTEGRAL.FacCPedi.CodCli
"FacCPedi.CodCli" ? ? "character" ? ? ? ? ? ? no ? no no "13" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.FacCPedi.NroPed
"FacCPedi.NroPed" "Pedido" "999-999999" "character" ? ? ? ? ? ? no ? no no "10" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"C-MON @ C-MON" "Mon" "X(4)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[7]   > integral.FacCPedi.Hora
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    ...
    WHEN 'Cliente':U THEN DO:
      &Scope SORTBY-PHRASE BY FacCPedi.CodCia BY FacCPedi.CodDoc BY FacCPedi.CodCli
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    OTHERWISE DO:
