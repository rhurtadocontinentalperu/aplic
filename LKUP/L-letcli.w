...
DEFINE VARIABLE x-coddoc    AS CHAR INIT "LET".        
...
CcbCDocu.FchDoc gn-clie.NomCli CcbCDocu.FlgEst CcbCDocu.FlgSit ~
CcbCDocu.FlgUbi CcbCDocu.CodMon CcbCDocu.SdoAct 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH CcbCDocu ~
      WHERE {&CONDICION}  ~
    NO-LOCK, ~
      FIRST gn-clie WHERE gn-clie.CodCia = cl-codcia ~
  AND gn-clie.CodCli = CcbCDocu.CodCli NO-LOCK
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH CcbCDocu ~
      WHERE {&CONDICION}  ~
    NO-LOCK, ~
      FIRST gn-clie WHERE gn-clie.CodCia = cl-codcia ~
  AND gn-clie.CodCli = CcbCDocu.CodCli NO-LOCK.
...
<FOREIGN-KEYS>
Nombres que inicien con|||integral.CcbCDocu.NroDoc
Nombres que contengan|||integral.CcbCDocu.NomCli
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Número|y||integral.CcbCDocu.NroDoc|yes
Nombre Cliente|||integral.CcbCDocu.NomCli|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "Número,Nombre Cliente",
     Sort-Case = Número':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.NroDoc COLUMN-LABEL "Nro. de Letra" FORMAT "X(12)":U
      CcbCDocu.FchDoc COLUMN-LABEL "Fch. de Letra" FORMAT "99/99/9999":U
      gn-clie.NomCli FORMAT "x(30)":U
      ...
      CcbCDocu.CodMon FORMAT "9":U
      CcbCDocu.SdoAct FORMAT "->>,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 78.14 BY 8.62
...
"CcbCDocu.NroDoc" "Nro. de Letra" "X(12)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.CcbCDocu.FchDoc
"CcbCDocu.FchDoc" "Fch. de Letra" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > integral.gn-clie.NomCli
"gn-clie.NomCli" ? "x(30)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > integral.CcbCDocu.FlgEst
"CcbCDocu.FlgEst" "Est." ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > integral.CcbCDocu.FlgSit
"CcbCDocu.FlgSit" "Sit." ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > integral.CcbCDocu.FlgUbi
"CcbCDocu.FlgUbi" "Ubc." ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   = integral.CcbCDocu.CodMon
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* Número de Documento */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Número':U THEN DO:
      &Scope SORTBY-PHRASE BY CcbCDocu.NroDoc
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    WHEN 'Nombre Cliente':U THEN DO:
