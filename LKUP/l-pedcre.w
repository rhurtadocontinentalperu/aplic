...
&SCOPED-DEFINE CONDICION Faccpedi.codcia = s-codcia ~
    ...
    AND Faccpedi.nroped BEGINS input-var-3
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacCPedi.NroPed COLUMN-LABEL "Numero dePedido"
      FacCPedi.FchPed FORMAT "99/99/99"
      FacCPedi.CodCli COLUMN-LABEL "Codigo Cliente"
      FacCPedi.NomCli
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"FacCPedi.NroPed" "Numero dePedido" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   > INTEGRAL.FacCPedi.FchPed
"FacCPedi.FchPed" ? "99/99/99" "date" ? ? ? ? ? ? no ?
     _FldNameList[3]   > INTEGRAL.FacCPedi.CodCli
"FacCPedi.CodCli" "Codigo Cliente" ? "character" ? ? ? ? ? ? no ?
...
DO:
    ...
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE
            {&CONDICION} AND
            ( {&CODIGO} = input-var-1 + INPUT FILL-IN-codigo )
            NO-LOCK NO-ERROR.
