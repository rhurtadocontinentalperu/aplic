...
DEFINE TEMP-TABLE T-Users NO-UNDO LIKE FacUsers.
...
&SCOPED-DEFINE CODIGO FacUsers.Usuario

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( FacUsers.Niveles BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( FacUsers.Niveles , FILL-IN-filtro ) <> 0 )
...
&Scoped-define INTERNAL-TABLES T-Users

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table T-Users.Usuario T-Users.Niveles 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH T-Users WHERE ~{&KEY-PHRASE} NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH T-Users WHERE ~{&KEY-PHRASE} NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table T-Users
&Scoped-define FIRST-TABLE-IN-QUERY-br_table T-Users


/* Definitions for FRAME F-Main                                         */

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-filtro br_table CMB-filtro RECT-1 
...
<FOREIGN-KEYS>
Nombre|y|y|INTEGRAL.FacUsers.Niveles
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Descripcion|y||INTEGRAL.FacCPedi.CodCia|yes,INTEGRAL.FacCPedi.CodDoc|yes,INTEGRAL.FacCPedi.NomCli|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "':U + 'Descripcion' + '",
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      T-Users.Usuario COLUMN-LABEL "Codigo" FORMAT "x(10)":U
      T-Users.Niveles COLUMN-LABEL "Nombre" FORMAT "X(40)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 54.14 BY 9.42
         BGCOLOR 15 FGCOLOR 0 FONT 4 ROW-HEIGHT-CHARS .54.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     FILL-IN-filtro AT ROW 1.31 COL 25.57 NO-LABEL
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: T-Users T "?" NO-UNDO INTEGRAL FacUsers
   END-TABLES.
 */
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.T-Users"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE"
     _FldNameList[1]   > Temp-Tables.T-Users.Usuario
"Usuario" "Codigo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.T-Users.Niveles
"Niveles" "Nombre" "X(40)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN DO:
    FIND FacUsers OF T-Users NO-LOCK.
    ASSIGN
        output-var-1 = ROWID( FacUsers )
        output-var-2 = FacUsers.Usuario
        output-var-3 = T-Users.NIveles.
END.
&ENDIF
...
FOR EACH FacUsers NO-LOCK WHERE FacUsers.codcia = s-codcia
    AND FacUsers.coddiv = input-var-1:
    CREATE T-Users.
    BUFFER-COPY Facusers TO T-Users.
    FIND _User WHERE _User._USERID = Facusers.usuario NO-LOCK NO-ERROR.
    IF AVAILABLE _User THEN T-Users.niveles = _User._User-name.
END.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "Nombre" "FacUsers" "Niveles"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "T-Users"}
