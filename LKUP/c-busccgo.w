...
&SCOPED-DEFINE l-lookup h_l-busccgo
...
DEFINE VARIABLE h_l-busccgo AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.85 COL 72
     Btn_Cancel AT ROW 6.77 COL 72
     Btn_Help AT ROW 8.69 COL 72
     RECT-2 AT ROW 4.46 COL 70
     SPACE(1.71) SKIP(1.56)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-busccgo.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-busccgo ).
       RUN set-position IN h_l-busccgo ( 1.19 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.92 , 67.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.58 , 70.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 14.29 ) NO-ERROR.

       /* Links to SmartLookup h_l-busccgo. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-busccgo ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-busccgo ,
