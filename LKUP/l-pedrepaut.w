...
AND Almcrepo.AlmPed = input-var-1 ~
AND Almcrepo.flgest = "P" ~
AND Almcrepo.flgsit = "A"

&SCOPED-DEFINE CODIGO Almcrepo.codalm

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( Almcrepo.usuario BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( Almcrepo.usuario , FILL-IN-filtro ) <> 0 )
...
&Scoped-define FIELDS-IN-QUERY-br_table almcrepo.TipMov almcrepo.NroSer ~
almcrepo.NroDoc almcrepo.FchDoc almcrepo.Hora almcrepo.Fecha ~
almcrepo.CodAlm Almacen.Descripcion 
...
<SORTBY-OPTIONS>
Almacen|y||INTEGRAL.almcrepo.CodAlm|yes,INTEGRAL.almcrepo.NroSer|yes,INTEGRAL.almcrepo.NroDoc|yes
Serie|||INTEGRAL.almcrepo.NroSer|yes,INTEGRAL.almcrepo.NroDoc|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "':U + 'Almacen,Serie' + '",
     SortBy-Case = ':U + 'Almacen').
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      almcrepo.TipMov COLUMN-LABEL "Tipo" FORMAT "X":U WIDTH 3.57
      almcrepo.NroSer FORMAT "999":U
      almcrepo.NroDoc FORMAT "999999":U
      almcrepo.FchDoc FORMAT "99/99/9999":U
      almcrepo.Hora FORMAT "x(5)":U
      almcrepo.Fecha COLUMN-LABEL "Entregar" FORMAT "99/99/99":U
      almcrepo.CodAlm FORMAT "x(3)":U
      Almacen.Descripcion FORMAT "X(40)":U WIDTH 27.14
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 75.14 BY 8.65
...
/* Query rebuild information for BROWSE br_table
     ...
     _TblOptList       = ", FIRST"
     _Where[1]         = "{&CONDICION}"
     _FldNameList[1]   > INTEGRAL.almcrepo.TipMov
"almcrepo.TipMov" "Tipo" ? "character" ? ? ? ? ? ? no ? no no "3.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.almcrepo.NroSer
     _FldNameList[3]   = INTEGRAL.almcrepo.NroDoc
     _FldNameList[4]   = INTEGRAL.almcrepo.FchDoc
     _FldNameList[5]   = INTEGRAL.almcrepo.Hora
     _FldNameList[6]   > INTEGRAL.almcrepo.Fecha
"almcrepo.Fecha" "Entregar" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   = INTEGRAL.almcrepo.CodAlm
     _FldNameList[8]   > INTEGRAL.Almacen.Descripcion
"Almacen.Descripcion" ? ? "character" ? ? ? ? ? ? no ? no no "27.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* Almacen */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Almacen':U THEN DO:
      &Scope SORTBY-PHRASE BY almcrepo.CodAlm BY almcrepo.NroSer BY almcrepo.NroDoc
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    WHEN 'Serie':U THEN DO:
      &Scope SORTBY-PHRASE BY almcrepo.NroSer BY almcrepo.NroDoc
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-3 = STRING ({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.NroSer, '999' ) +
