...
&SCOPED-DEFINE CONDICION ( CcbCDocu.CodCia = S-CODCIA AND ~
                           ...
                           CcbCDocu.CodCli BEGINS input-var-1 and ~
                           CcbCDocu.TpoFac = "G" )
...
No.Guia|||integral.CcbCDocu.CodCia|yes,integral.CcbCDocu.CodDoc|yes,integral.CcbCDocu.NroDoc|yes,integral.CcbCDocu.NroPed|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbCDocu.NroDoc COLUMN-LABEL "Guía" FORMAT "XXX-XXXXXXXX"
      CcbCDocu.NomCli FORMAT "x(40)"
      CcbCDocu.NroPed FORMAT "XXX-XXXXXXXX"
      CcbCDocu.FchDoc COLUMN-LABEL "Fch. Emisión"
      CcbCDocu.FchVto COLUMN-LABEL "Fch. Vncmto."
      C-MON @ C-MON COLUMN-LABEL "Mon." FORMAT "XXX"
      CcbCDocu.ImpTot
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 85.43 BY 4.81
...
DEFINE FRAME F-Main
     FILL-IN-codigo AT ROW 1.19 COL 13 COLON-ALIGNED
     CMB-filtro AT ROW 1.19 COL 24 NO-LABEL
     FILL-IN-filtro AT ROW 1.19 COL 47.57 NO-LABEL
     br_table AT ROW 2.12 COL 1.86
     RECT-1 AT ROW 1 COL 1
     "Buscar x" VIEW-AS TEXT
          SIZE 8.43 BY .54 AT ROW 1.35 COL 2
          FONT 6
...
"CcbCDocu.NroDoc" "Guía" "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   > integral.CcbCDocu.NomCli
"CcbCDocu.NomCli" ? "x(40)" "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   > integral.CcbCDocu.NroPed
"CcbCDocu.NroPed" ? "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ?
     _FldNameList[4]   > integral.CcbCDocu.FchDoc
"CcbCDocu.FchDoc" "Fch. Emisión" ? "date" ? ? ? ? ? ? no ?
     _FldNameList[5]   > integral.CcbCDocu.FchVto
"CcbCDocu.FchVto" "Fch. Vncmto." ? "date" ? ? ? ? ? ? no ?
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         ...
         WHEN 'No.Guia':U THEN DO:
           &Scope SORTBY-PHRASE BY CcbCDocu.CodCia BY CcbCDocu.CodDoc BY CcbCDocu.NroDoc BY CcbCDocu.NroPed
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
DO:
   ...
   FOR EACH LIQU :
      IF output-var-2 = "" THEN output-var-2 = LIQU.NroRef.
      IF LOOKUP(LIQU.NroRef,output-var-2) = 0 THEN
         output-var-2 = output-var-2 + "," + LIQU.NroRef.
   END.
END.
&ENDIF
