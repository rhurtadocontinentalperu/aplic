...
&SCOPED-DEFINE l-lookup h_l-gn-ccos
...
&Scoped-Define ENABLED-OBJECTS RECT-25 Btn_OK Btn_Cancel Btn_Help 
...
DEFINE VARIABLE h_l-gn-ccos AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 5.15 COL 54.57
     Btn_Cancel AT ROW 8.96 COL 54.57
     Btn_Help AT ROW 12.58 COL 54.72
     RECT-25 AT ROW 3.27 COL 53
     SPACE(0.85) SKIP(0.14)
...
ON WINDOW-CLOSE OF FRAME C-Dialog /* Vendedores */
DO:  
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-gn-ccos.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-gn-ccos ).
       RUN set-position IN h_l-gn-ccos ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-gn-ccos ( 15.35 , 50.86 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.08 , 52.43 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-gn-ccos. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-gn-ccos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-gn-ccos ,
...
------------------------------------------------------------------------------*/
  ENABLE RECT-25 Btn_OK Btn_Cancel Btn_Help 
      WITH FRAME C-Dialog.
  VIEW FRAME C-Dialog.
