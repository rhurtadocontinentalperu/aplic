...
&SCOPED-DEFINE CONDICION ( AlmRepos.CodCia = S-CODCIA AND AlmRepos.CodAlm = input-var-1 ~
AND AlmRepos.TipMat = input-var-2)
&SCOPED-DEFINE CODIGO AlmRepos.CodAlm 

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( Almacen.Descripcion BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( Almacen.Descripcion , FILL-IN-filtro ) <> 0 )
...
&Scoped-define INTERNAL-TABLES almrepos Almacen

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table almrepos.Orden almrepos.AlmPed ~
Almacen.Descripcion 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH almrepos WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK, ~
      EACH Almacen WHERE Almacen.CodCia = almrepos.CodCia ~
  AND Almacen.CodAlm = almrepos.AlmPed NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH almrepos WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK, ~
      EACH Almacen WHERE Almacen.CodCia = almrepos.CodCia ~
  AND Almacen.CodAlm = almrepos.AlmPed NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table almrepos Almacen
...
<FOREIGN-KEYS>
Nombres que inicien con|y||integral.Almacen.Descripcion
Nombres que contengan|y||integral.Almacen.Descripcion
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Codigo|y||INTEGRAL.almrepos.Orden|yes,INTEGRAL.almrepos.AlmPed|yes
Descripcion|||integral.Almacen.Descripcion|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      almrepos.AlmPed FORMAT "x(3)":U
      Almacen.Descripcion FORMAT "X(40)":U WIDTH 38.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 58.14 BY 8.62
...
DEFINE FRAME F-Main
     ...
     FILL-IN-codigo AT ROW 1.19 COL 21.14 COLON-ALIGNED
     CMB-filtro AT ROW 2.08 COL 1.86 NO-LABEL
     FILL-IN-filtro AT ROW 2.15 COL 23.14 NO-LABEL
     "Buscar x" VIEW-AS TEXT
          SIZE 8.14 BY .46 AT ROW 1.38 COL 9.29
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "integral.almrepos,integral.Almacen WHERE integral.almrepos ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "{&CONDICION}"
     _JoinCode[2]      = "integral.Almacen.CodCia = integral.almrepos.CodCia
  AND integral.Almacen.CodAlm = integral.almrepos.AlmPed"
     _FldNameList[1]   = integral.almrepos.Orden
     _FldNameList[2]   = integral.almrepos.AlmPed
     _FldNameList[3]   > integral.Almacen.Descripcion
"Almacen.Descripcion" ? ? "character" ? ? ? ? ? ? no ? no no "42.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Codigo':U THEN DO:
           &Scope SORTBY-PHRASE BY almrepos.Orden BY almrepos.AlmPed
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripcion':U THEN DO:
           &Scope SORTBY-PHRASE BY Almacen.Descripcion
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
DO I = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
   IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(I) THEN DO:    
    ASSIGN
        ...
        output-var-3 = {&SECOND-TABLE-IN-QUERY-{&BROWSE-NAME}}.Descripcion
        output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.almped.
   END.     
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE captura-datos-back L-table-Win 
PROCEDURE captura-datos-back :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        output-var-1 = ROWID( {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} )
        output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codalm
        output-var-3 = {&SECOND-TABLE-IN-QUERY-{&BROWSE-NAME}}.Descripcion.
