...
&SCOPED-DEFINE l-lookup h_l-str
...
DEFINE VARIABLE h_l-str AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.46 COL 91.43
     Btn_Cancel AT ROW 8.65 COL 91.43
     SPACE(3.28) SKIP(2.64)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-str.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-str ).
       RUN set-position IN h_l-str ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-str ( 10.77 , 87.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 89.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-str. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-str ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-str ,
