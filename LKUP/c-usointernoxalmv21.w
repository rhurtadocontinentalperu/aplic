...
&SCOPED-DEFINE l-lookup h_l-usointernoxalmv21
...
&Scoped-Define ENABLED-OBJECTS RECT-28 Btn_OK Btn_Cancel 
...
DEFINE VARIABLE h_l-usointernoxalmv21 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 3.88 COL 139
     Btn_Cancel AT ROW 5.62 COL 139
     "ARTICULOS SOLICITADOS" VIEW-AS TEXT
          SIZE 107 BY .5 AT ROW 12.15 COL 3 WIDGET-ID 2
          BGCOLOR 9 FGCOLOR 15 
     RECT-28 AT ROW 11.96 COL 1 WIDGET-ID 4
     SPACE(17.28) SKIP(0.22)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LKUP/l-usointernoxalmv21.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Almacen':U ,
             OUTPUT h_l-usointernoxalmv21 ).
       RUN set-position IN h_l-usointernoxalmv21 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-usointernoxalmv21 ( 10.96 , 137.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.19 , 138.00 ) NO-ERROR.
       ...
       RUN set-position IN h_busointerno ( 12.73 , 3.00 ) NO-ERROR.
       RUN set-size IN h_busointerno ( 8.08 , 107.00 ) NO-ERROR.

       /* Links to SmartLookup h_l-usointernoxalmv21. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-usointernoxalmv21 ).

       /* Links to SmartBrowser h_busointerno. */
       RUN add-link IN adm-broker-hdl ( h_l-usointernoxalmv21 , 'Record':U , h_busointerno ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-usointernoxalmv21 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-usointernoxalmv21 , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ENABLE RECT-28 Btn_OK Btn_Cancel 
      WITH FRAME C-Dialog.
  VIEW FRAME C-Dialog.
