...
&SCOPED-DEFINE CONDICION ( PikSacadores.CodCia = s-codcia ~
    AND ( input-var-1 = '' OR PikSacadores.CodDiv = input-var-1 ) ~
    AND ( input-var-2 = '' OR PikSacadores.FlgEst = input-var-2 ) ~
    AND ( input-var-3 = '' OR PikSacadores.FlgTarea = input-var-3 ) ~
        )
&SCOPED-DEFINE CODIGO Almacen.CodAlm 
...
&Scoped-define FIELDS-IN-QUERY-br_table PikSacadores.CodPer PL-PERS.patper ~
PL-PERS.matper PL-PERS.nomper PikSacadores.Sector 
...
<SORTBY-OPTIONS>
Codigo|y||integral.Almacen.CodCia|yes,integral.Almacen.CodAlm|yes
Descripcion|||integral.Almacen.CodCia|yes,integral.Almacen.Descripcion|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      PikSacadores.Sector FORMAT "x(20)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 74.14 BY 8.62
...
"PikSacadores.Sector" ? "x(20)" "character" ? ? ? ? ? ? no ? no no "18.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Codigo':U THEN DO:
           &Scope SORTBY-PHRASE BY Almacen.CodCia BY Almacen.CodAlm
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripcion':U THEN DO:
           &Scope SORTBY-PHRASE BY Almacen.CodCia BY Almacen.Descripcion
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
DO I = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
   IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(I) THEN DO:    
    ASSIGN
        ...
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codper.
   END.     
END.
