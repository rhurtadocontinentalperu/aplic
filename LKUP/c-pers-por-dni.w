...
&SCOPED-DEFINE l-lookup h_l-pers-por-dni
...
DEFINE VARIABLE h_l-pers-por-dni AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.46 COL 107.43
     Btn_Cancel AT ROW 8.65 COL 107.43
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-pers-por-dni.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-pers-por-dni ).
       RUN set-position IN h_l-pers-por-dni ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-pers-por-dni ( 10.92 , 100.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 105.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-pers-por-dni. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-pers-por-dni ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-pers-por-dni ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-pers-por-dni , 'AFTER':U ).
    END. /* Page 0 */
