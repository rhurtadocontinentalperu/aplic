...
&SCOPED-DEFINE l-lookup h_l-transport-customer
...
DEFINE VARIABLE h_l-transport-customer AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.46 COL 90.43
     Btn_Cancel AT ROW 8.65 COL 90.43
     SPACE(3.42) SKIP(2.64)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-transport-customer.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-transport-customer ).
       RUN set-position IN h_l-transport-customer ( 1.27 , 2.00 ) NO-ERROR.
       RUN set-size IN h_l-transport-customer ( 10.92 , 82.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 88.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-transport-customer. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-transport-customer ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-transport-customer ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-transport-customer , 'AFTER':U ).
    END. /* Page 0 */
