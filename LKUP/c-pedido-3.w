...
&SCOPED-DEFINE l-lookup h_l-pedido-3
...
DEFINE VARIABLE h_l-pedido-3 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-pedido-3.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-pedido-3 ).
       RUN set-position IN h_l-pedido-3 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-pedido-3 ( 10.96 , 70.72 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-pedido-3. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-pedido-3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-pedido-3 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-pedido-3 , 'AFTER':U ).
    END. /* Page 0 */
