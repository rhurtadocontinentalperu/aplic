...
&SCOPED-DEFINE CONDICION ( FacCPedm.CodCia = S-CODCIA AND ~
                           LOOKUP(FacCPedm.CodDoc, input-var-1) > 0  AND ~
                           FacCPedm.CodCli BEGINS input-var-2 AND ~
                           FacCPedm.NroPed BEGINS C-NROSER AND ~
                           FacCPedm.FlgEst = "S" AND ~
                           FacCPedm.CodDiv = S-CODDIV  AND ~
                           FacCPedm.TipVta BEGINS input-var-3)
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Faccpedm.CodDoc COLUMN-LABEL "Cod."
      Faccpedm.NroPed COLUMN-LABEL "Documento No." FORMAT "XXX-XXXXXXXX"
      Faccpedm.NomCli FORMAT "x(35)"
      Faccpedm.FchPed COLUMN-LABEL "Fecha Emision"
      Faccpedm.ImpTot
...
"Faccpedm.CodDoc" "Cod." ? "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   > integral.Faccpedm.NroPed
"Faccpedm.NroPed" "Documento No." "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   > integral.Faccpedm.NomCli
"Faccpedm.NomCli" ? "x(35)" "character" ? ? ? ? ? ? no ?
     _FldNameList[4]   > integral.Faccpedm.FchPed
"Faccpedm.FchPed" "Fecha Emision" ? "date" ? ? ? ? ? ? no ?
