...
AND Vtadtabla.tabla = "CFG_TIPO_NC" ~
AND Vtadtabla.llave = input-var-1 ~
AND Vtadtabla.tipo  = "CONCEPTO" ~
)

&SCOPED-DEFINE CODIGO Vtadtabla.LlaveDetalle

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( CcbTabla.Nombre BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( CcbTabla.Nombre , FILL-IN-filtro ) <> 0 )
...
&Scoped-define INTERNAL-TABLES VtaDTabla CcbTabla
...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaDTabla WHERE ~{&KEY-PHRASE} ~
      ...
      EACH CcbTabla WHERE CcbTabla.CodCia = VtaDTabla.CodCia ~
  AND CcbTabla.Codigo = VtaDTabla.LlaveDetalle ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH VtaDTabla WHERE ~{&KEY-PHRASE} ~
  ...
  AND CcbTabla.Codigo = VtaDTabla.LlaveDetalle ~
      AND CcbTabla.Tabla = "N/C" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table VtaDTabla CcbTabla
...
<FOREIGN-KEYS>
Nombres que inicien con|y||INTEGRAL.CcbTabla.Nombre
Nombres que contengan|y||INTEGRAL.CcbTabla.Nombre
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.VtaDTabla.LlaveDetalle|yes
Descripción|||INTEGRAL.VtaDTabla.LlaveDetalle|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaDTabla.LlaveDetalle COLUMN-LABEL "Concepto" FORMAT "x(10)":U
      CcbTabla.Nombre FORMAT "x(80)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaDTabla,INTEGRAL.CcbTabla WHERE INTEGRAL.VtaDTabla ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "{&Condicion}"
     _JoinCode[2]      = "CcbTabla.CodCia = VtaDTabla.CodCia
  AND CcbTabla.Codigo = VtaDTabla.LlaveDetalle"
     _Where[2]         = "CcbTabla.Tabla = ""N/C"""
     _FldNameList[1]   > INTEGRAL.VtaDTabla.LlaveDetalle
"VtaDTabla.LlaveDetalle" "Concepto" "x(10)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY VtaDTabla.LlaveDetalle
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.LlaveDetalle
        output-var-3 = {&SECOND-TABLE-IN-QUERY-{&BROWSE-NAME}}.Nombre
    .
