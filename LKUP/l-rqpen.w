...
DEFINE VARIABLE C-NroPed    AS CHAR.
/* Preprocesadores para condiciones */
&SCOPED-DEFINE CONDICION ( PR-RQPROC.CodCia = S-CODCIA AND PR-RQPROC.AlmDes = S-CODALM AND PR-RQPROC.CodAlm BEGINS input-var-1 AND PR-RQPROC.FlgEst = "P" )
&SCOPED-DEFINE CODIGO PR-RQPROC.NroDoc

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( PR-RQPROC.Observ BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( PR-RQPROC.Observ , FILL-IN-filtro ) <> 0 )
...
&Scoped-define FIELDS-IN-QUERY-br_table C-NroPed @ C-NroPed ~
PR-RQPROC.NumOrd PR-RQPROC.CodAlm PR-RQPROC.HorGen PR-RQPROC.FchDoc ~
PR-RQPROC.Observ 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH PR-RQPROC WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    BY PR-RQPROC.FchDoc DESCENDING
...
<SORTBY-OPTIONS>
Codigo|y||integral.Almcrequ.CodCia|yes,integral.Almcrequ.CodAlm|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "Codigo",
     Sort-Case = Codigo':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      C-NroPed @ C-NroPed COLUMN-LABEL "Numero!Pedido" FORMAT "XXX-XXXXXXXXX":U
      PR-RQPROC.NumOrd COLUMN-LABEL "Orden !Produccion" FORMAT "X(8)":U
      PR-RQPROC.CodAlm FORMAT "x(3)":U
      PR-RQPROC.HorGen FORMAT "X(8)":U
      PR-RQPROC.FchDoc COLUMN-LABEL "Fecha     de!  Emision" FORMAT "99/99/9999":U
      PR-RQPROC.Observ FORMAT "X(45)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 70.29 BY 8.62
...
DEFINE FRAME F-Main
     FILL-IN-codigo AT ROW 1.19 COL 25.29 COLON-ALIGNED
     CMB-filtro AT ROW 2.08 COL 1.86 NO-LABEL
     FILL-IN-filtro AT ROW 2.12 COL 27.29 NO-LABEL
     br_table AT ROW 3.08 COL 1.86
     "Buscar x" VIEW-AS TEXT
          SIZE 7.86 BY .65 AT ROW 1.27 COL 13.43
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "INTEGRAL.PR-RQPROC.FchDoc|no"
     _Where[1]         = "{&Condicion}"
     _FldNameList[1]   > "_<CALC>"
"C-NroPed @ C-NroPed" "Numero!Pedido" "XXX-XXXXXXXXX" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.PR-RQPROC.NumOrd
"PR-RQPROC.NumOrd" "Orden !Produccion" "X(8)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = integral.PR-RQPROC.CodAlm
     _FldNameList[4]   = integral.PR-RQPROC.HorGen
     _FldNameList[5]   > integral.PR-RQPROC.FchDoc
"PR-RQPROC.FchDoc" "Fecha     de!  Emision" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > integral.PR-RQPROC.Observ
"PR-RQPROC.Observ" ? "X(45)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON FIND OF PR-RQPROC
DO:
  C-NROPED = STRING(PR-RQPROC.NroSer,"999") + STRING(PR-RQPROC.NroDoc,"999999").
END.
/* ***************************  Main Block  *************************** */
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Codigo':U THEN DO:
      &Scope SORTBY-PHRASE BY Almcrequ.CodCia BY Almcrequ.CodAlm
