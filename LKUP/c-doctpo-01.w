...
&SCOPED-DEFINE l-lookup h_l-doctpo-01
...
DEFINE VARIABLE h_l-doctpo-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.73 COL 84
     Btn_Cancel AT ROW 8.92 COL 84
     SPACE(4.42) SKIP(2.37)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-doctpo-01.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-doctpo-01 ).
       RUN set-position IN h_l-doctpo-01 ( 1.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_l-doctpo-01 ( 10.92 , 81.00 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-doctpo-01. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-doctpo-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-doctpo-01 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-doctpo-01 , 'AFTER':U ).
    END. /* Page 0 */
