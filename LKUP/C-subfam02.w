...
&SCOPED-DEFINE l-lookup h_l-subfam02
...
&Scoped-Define ENABLED-OBJECTS RECT-18 Btn_OK Btn_Cancel Btn_Help 
...
DEFINE VARIABLE h_l-subfam02 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.85 COL 51
     Btn_Cancel AT ROW 7.42 COL 51.14
     Btn_Help AT ROW 9.96 COL 51.14
     RECT-18 AT ROW 3.27 COL 47.86
     SPACE(0.00) SKIP(0.22)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-subfam02.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-subfam02 ).
       RUN set-position IN h_l-subfam02 ( 1.15 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 10.92 , 46.14 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.19 , 49.43 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-subfam02. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-subfam02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-subfam02 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-subfam02 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE RECT-18 Btn_OK Btn_Cancel Btn_Help 
      WITH FRAME C-Dialog.
  VIEW FRAME C-Dialog.
