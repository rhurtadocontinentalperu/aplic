...
AND Faccpedi.flgest = input-var-3 )
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.FacCPedi.CodCia|yes,INTEGRAL.FacCPedi.NroPed|yes
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacCPedi.NroPed COLUMN-LABEL "Nº de Control" FORMAT "X(9)":U
...
"FacCPedi.NroPed" "Nº de Control" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.FacCPedi.NomCli
"FacCPedi.NomCli" "Cliente" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = INTEGRAL.FacCPedi.ImpTot
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY FacCPedi.CodCia BY FacCPedi.NroPed
