...
&SCOPED-DEFINE CONDICION ( CcbCDocu.Codcia = S-CODCIA AND ~
                           CcbCDocu.FlgEst = input-var-1 AND ~
                           CcbCDocu.Codcli = input-var-2 AND ~
                           CcbCDocu.FlgSit <> 'X' AND ~
...
CcbCDocu.SdoAct CcbCDocu.FmaPgo CcbCDocu.ImpTot gn-ConVt.Nombr 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH CcbCDocu WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} USE-INDEX LLAVE03 NO-LOCK, ~
      EACH gn-ConVt WHERE gn-ConVt.Codig = CcbCDocu.FmaPgo NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbCDocu.CodDoc COLUMN-LABEL "Tipo!Doc." FORMAT "x(3)":U
      CcbCDocu.NroDoc FORMAT "XXX-XXXXXXXX":U WIDTH 10.72
      CcbCDocu.NomCli COLUMN-LABEL "Nombre Cliente" FORMAT "x(40)":U
...
"CcbCDocu.CodDoc" "Tipo!Doc." ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.NomCli" "Nombre Cliente" "x(40)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.SdoAct" "Saldo Actual" ? "decimal" 11 ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > integral.CcbCDocu.FmaPgo
"CcbCDocu.FmaPgo" "Cond.!Venta" ? "character" ? ? ? ? ? ? no ? no no "4.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
