...
&SCOPED-DEFINE l-lookup h_l-sectores
...
DEFINE VARIABLE h_l-sectores AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-sectores.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-sectores ).
       RUN set-position IN h_l-sectores ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-sectores ( 10.92 , 54.72 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-sectores. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-sectores ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-sectores ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-sectores , 'AFTER':U ).
    END. /* Page 0 */
