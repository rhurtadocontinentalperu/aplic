...
&SCOPED-DEFINE l-lookup h_l-almcdocu
...
DEFINE VARIABLE h_l-almcdocu AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LKUP/l-almcdocu.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Almacen':U ,
             OUTPUT h_l-almcdocu ).
       RUN set-position IN h_l-almcdocu ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-almcdocu ( 10.96 , 101.00 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-almcdocu. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-almcdocu ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-almcdocu ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-almcdocu , 'AFTER':U ).
    END. /* Page 0 */
