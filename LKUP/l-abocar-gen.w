...
&SCOPED-DEFINE CONDICION ( CcbTabla.CodCia = s-codcia ~
AND CcbTabla.Tabla = input-var-1 ~
AND CcbTabla.Libre_L02 = LOGICAL(input-var-2) ~
AND CcbTabla.Reservado = LOGICAL(input-var-3) ~
    )

&SCOPED-DEFINE CODIGO CcbTabla.Codigo
...
&Scoped-define FIELDS-IN-QUERY-br_table CcbTabla.Tabla CcbTabla.Codigo ~
CcbTabla.Nombre CcbTabla.Codcta CcbTabla.Afecto 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbTabla.Tabla FORMAT "XXX":U
      CcbTabla.Codigo FORMAT "x(10)":U
      CcbTabla.Nombre FORMAT "x(80)":U
      CcbTabla.Codcta FORMAT "x(10)":U
      CcbTabla.Afecto FORMAT "Si/No":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 55.14 BY 8.62
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "{&Condicion}"
     _FldNameList[1]   > INTEGRAL.CcbTabla.Tabla
"CcbTabla.Tabla" ? "XXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = integral.CcbTabla.Codigo
     _FldNameList[3]   = integral.CcbTabla.Nombre
     _FldNameList[4]   = integral.CcbTabla.Codcta
     _FldNameList[5]   > integral.CcbTabla.Afecto
"CcbTabla.Afecto" ? "Si/No" "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
