...
&SCOPED-DEFINE l-lookup h_l-almrep
...
DEFINE VARIABLE h_l-almrep AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.77 COL 64.57
     Btn_Cancel AT ROW 7.23 COL 64.57
     SPACE(4.85) SKIP(3.45)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-almrep.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Codigo':U ,
             OUTPUT h_l-almrep ).
       RUN set-position IN h_l-almrep ( 1.04 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-almrep ( 10.92 , 60.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.58 , 62.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-almrep. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-almrep ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-almrep ,
