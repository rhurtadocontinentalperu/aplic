...
DEFINE TEMP-TABLE Almmmatg NO-UNDO LIKE Almmmatg
       field f-flgstk as char.
...
&Scoped-define FIELDS-IN-QUERY-br_table F-FLGSTK @ F-FLGSTK Almmmatg.codfam ~
Almmmatg.subfam Almmmatg.codmat Almmmatg.DesMat Almmmatg.UndBas ~
F-STKACT @ F-STKACT F-STKCAC @ F-STKCAC F-STKOTR @ F-STKOTR ~
F-STKCOT @ F-STKCOT F-PRESOLA @ F-PRESOLA F-PREUSSA @ F-PREUSSA ~
F-PRESOLB @ F-PRESOLB F-PREUSSB @ F-PREUSSB F-PRESOLC @ F-PRESOLC ~
F-PREUSSC @ F-PREUSSC F-PRESOLD @ F-PRESOLD F-PREUSSD @ F-PREUSSD ~
F-PRECIO1 @ F-PRECIO1 F-PRECIO2 @ F-PRECIO2 
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: Almmmatg T "?" NO-UNDO INTEGRAL Almmmatg
      ADDITIONAL-FIELDS:
          field f-flgstk as char
      END-FIELDS.
   END-TABLES.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.Almmmatg"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > "_<CALC>"
"F-FLGSTK @ F-FLGSTK" ? ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = Temp-Tables.Almmmatg.codfam
     _FldNameList[3]   = Temp-Tables.Almmmatg.subfam
     _FldNameList[4]   = Temp-Tables.Almmmatg.codmat
     _FldNameList[5]   = Temp-Tables.Almmmatg.DesMat
     _FldNameList[6]   = Temp-Tables.Almmmatg.UndBas
...
------------------------------------------------------------------------------*/
    ...
    /*BUFFER-COPY integral.Almmmatg TO Almmmatg.*/
    MESSAGE "init".
    
    FOR EACH integral.almmmatg WHERE integral.Almmmatg.CodCia = S-CODCIA 
        AND LOOKUP(S-CODALM,integral.Almmmatg.almacenes) <> 0 
        AND (integral.Almmmatg.Codfam <> "008"   
             AND integral.Almmmatg.Codfam <> "009" ) 
        AND F-FLGSTK = R-Stock 
        AND integral.Almmmatg.FchCes = ? 
        AND integral.Almmmatg.codfam BEGINS F-Codfam
        AND integral.Almmmatg.TpoArt = "A" NO-LOCK:        
        BUFFER-COPY integral.almmmatg TO almmmatg.
    END.
