...
AND ccbcdocu.tpofac = 'CO' ~
AND LOOKUP(ccbcdocu.coddoc, input-var-2) > 0 ~
...
&Scoped-define FIELDS-IN-QUERY-br_table CcbCDocu.FchDoc CcbCDocu.CodDoc ~
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.CcbCDocu.FchDoc|no,INTEGRAL.CcbCDocu.NroDoc|no
Descripción|||INTEGRAL.CcbCDocu.NomCli|yes,INTEGRAL.CcbCDocu.NroDoc|no
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.NroDoc FORMAT "X(12)":U WIDTH 11.43
      CcbCDocu.CodCli FORMAT "x(11)":U WIDTH 10.43
      CcbCDocu.NomCli FORMAT "x(50)":U
      CcbCDocu.ImpTot FORMAT "->>,>>>,>>9.99":U WIDTH 6.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 83.14 BY 8.62
...
"CcbCDocu.NroDoc" ? ? "character" ? ? ? ? ? ? no ? no no "11.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.ImpTot" ? ? "decimal" ? ? ? ? ? ? no ? no no "6.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY CcbCDocu.FchDoc DESCENDING BY CcbCDocu.NroDoc DESCENDING
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY CcbCDocu.NomCli BY CcbCDocu.NroDoc DESCENDING
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
