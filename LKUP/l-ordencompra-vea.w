...
&SCOPED-DEFINE FILTRO1 ( factabla.codigo BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( factabla.codigo , FILL-IN-filtro ) <> 0 )
...
&Scoped-define QUERY-STRING-br_table FOR EACH FacTabla WHERE ~{&KEY-PHRASE} ~
      AND factabla.codcia = 1 and  ~
factabla.tabla = 'OC PLAZA VEA' and ~
index(factabla.codigo,"anulado") = 0 NO-LOCK ~
    BY FacTabla.Codigo DESCENDING
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacTabla.Codigo COLUMN-LABEL "Orden!Compra" FORMAT "x(15)":U
            WIDTH 20.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "INTEGRAL.FacTabla.Codigo|no"
     _Where[1]         = "factabla.codcia = 1 and 
factabla.tabla = 'OC PLAZA VEA' and
index(factabla.codigo,""anulado"") = 0"
     _FldNameList[1]   > INTEGRAL.FacTabla.Codigo
"Codigo" "Orden!Compra" "x(15)" "character" ? ? ? ? ? ? no ? no no "20.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
