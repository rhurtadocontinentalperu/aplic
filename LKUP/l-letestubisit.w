...
&SCOPED-DEFINE CONDICION ( CcbCDocu.CodCia = S-CODCIA AND ~
                           ...
                           CcbCDocu.FlgUbi = input-var-2 AND ~
                           CcbCDocu.FlgSit BEGINS input-var-3 )
...
&Scoped-define FIELDS-IN-QUERY-br_table CcbCDocu.NroDoc CcbCDocu.CodCli ~
CcbCDocu.NomCli CcbCDocu.FchDoc CcbCDocu.FchVto ~
(IF CcbcDocu.CodMon = 2 THEN  'US$' ELSE 'S/.') @ CcbCDocu.FlgUbiA ~
CcbCDocu.ImpTot 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbCDocu.NroDoc COLUMN-LABEL "Numero!Letra" FORMAT "X(12)":U
            WIDTH 9.57
      CcbCDocu.CodCli COLUMN-LABEL "RUC" FORMAT "x(11)":U WIDTH 10.43
      CcbCDocu.NomCli COLUMN-LABEL "Nombre Cliente" FORMAT "x(45)":U
      ...
      (IF CcbcDocu.CodMon = 2 THEN  'US$' ELSE 'S/.') @ CcbCDocu.FlgUbiA COLUMN-LABEL "Mon." FORMAT "XXX":U
      CcbCDocu.ImpTot FORMAT "->>,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 90.14 BY 9.35
...
DEFINE FRAME F-Main
     FILL-IN-codigo AT ROW 1.19 COL 14.86 COLON-ALIGNED
     CMB-filtro AT ROW 1.19 COL 29.57 NO-LABEL
     FILL-IN-filtro AT ROW 1.19 COL 51.14 NO-LABEL
...
"CcbCDocu.NroDoc" "Numero!Letra" "X(12)" "character" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.CcbCDocu.CodCli
"CcbCDocu.CodCli" "RUC" ? "character" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.CcbCDocu.NomCli
"CcbCDocu.NomCli" "Nombre Cliente" "x(45)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"(IF CcbcDocu.CodMon = 2 THEN  'US$' ELSE 'S/.') @ CcbCDocu.FlgUbiA" "Mon." "XXX" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* Letra Nro. */
DO:
