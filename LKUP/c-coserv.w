...
&SCOPED-DEFINE l-lookup h_l-coserv
...
DEFINE VARIABLE h_l-coserv AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 3.54 COL 93.29
     Btn_Cancel AT ROW 4.73 COL 93.29
     SPACE(5.42) SKIP(6.56)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-coserv.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Numero':U ,
             OUTPUT h_l-coserv ).
       RUN set-position IN h_l-coserv ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.92 , 92.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.38 , 93.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-coserv. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-coserv ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-coserv ,
