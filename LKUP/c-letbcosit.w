...
&SCOPED-DEFINE l-lookup h_l-letbcosit
...
DEFINE VARIABLE h_l-letbcosit AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.54 COL 78
     Btn_Cancel AT ROW 8.73 COL 78
     SPACE(3.28) SKIP(2.56)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-letbcosit.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-letbcosit ).
       RUN set-position IN h_l-letbcosit ( 1.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_l-letbcosit ( 10.92 , 75.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.19 , 76.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-letbcosit. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-letbcosit ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-letbcosit ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-letbcosit , 'AFTER':U ).
    END. /* Page 0 */
