...
&SCOPED-DEFINE l-lookup h_l-clfcli
...
DEFINE VARIABLE h_l-clfcli AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.46 COL 59.29
     Btn_Cancel AT ROW 7.08 COL 59.29
     Btn_Help AT ROW 9.62 COL 59.29
     RECT-26 AT ROW 3.54 COL 57.29
     SPACE(1.56) SKIP(0.25)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-clfcli.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-clfcli ).
       RUN set-position IN h_l-clfcli ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.31 , 56.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.38 , 57.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-clfcli. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-clfcli ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-clfcli ,
