...
&SCOPED-DEFINE CONDICION ( CcbCDocu.CodCia = S-CODCIA AND ~
                           ...
                           CcbCDocu.FlgEst = input-var-3 AND ~
                           LOOKUP(CcbCDocu.CodDoc, input-var-1) > 0 )
...
"CcbCDocu.NroDoc" ? "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no "9.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.FchVto" "F.Vencimiento" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.SdoAct" "Saldo Actual" ? "decimal" ? ? ? ? ? ? no ? no no "11" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
   ...
   IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(I) THEN DO:      
      ...
      IF NOT AVAILABLE MVTO THEN DO:
         ...
         ASSIGN MVTO.CodCia  = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodCia
                ...
                MVTO.NroRef  = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.NroDoc
                MVTO.TpoRef  = "O".
         IF {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodMon = S-CODMON 
             THEN s-total = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.SdoAct.
         ELSE DO:
             IF {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodMon = 1 
                 THEN s-total = ({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.SdoAct / S-TPOCMB).
