...
&SCOPED-DEFINE l-lookup h_l-ncndxotros
...
DEFINE VARIABLE h_l-ncndxotros AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LKUP/l-ncndxotros.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-ncndxotros ).
       RUN set-position IN h_l-ncndxotros ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-ncndxotros ( 10.92 , 63.00 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-ncndxotros. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-ncndxotros ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-ncndxotros ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-ncndxotros , 'AFTER':U ).
    END. /* Page 0 */
