...
&SCOPED-DEFINE l-lookup h_l-pre-hojaruta
...
DEFINE VARIABLE h_l-pre-hojaruta AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.38 COL 71.43
     Btn_Cancel AT ROW 8.58 COL 71.43
     SPACE(4.56) SKIP(2.71)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-pre-hojaruta.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Numero':U ,
             OUTPUT h_l-pre-hojaruta ).
       RUN set-position IN h_l-pre-hojaruta ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-pre-hojaruta ( 10.92 , 66.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.19 , 69.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-pre-hojaruta. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-pre-hojaruta ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-pre-hojaruta ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-pre-hojaruta , 'AFTER':U ).
    END. /* Page 0 */
