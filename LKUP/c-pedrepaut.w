...
&SCOPED-DEFINE l-lookup h_l-pedrepaut
...
DEFINE VARIABLE h_l-pedrepaut AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.58 COL 81.29
     Btn_Cancel AT ROW 7.15 COL 81.29
     RECT-27 AT ROW 3.69 COL 79.86
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-pedrepaut.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Almacen':U ,
             OUTPUT h_l-pedrepaut ).
       RUN set-position IN h_l-pedrepaut ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-pedrepaut ( 10.96 , 77.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 79.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-pedrepaut. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-pedrepaut ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-pedrepaut ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-pedrepaut , 'AFTER':U ).
    END. /* Page 0 */
