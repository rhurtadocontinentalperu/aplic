...
AND Ccbcdocu.coddiv = input-var-2 ~
AND Ccbcdocu.nrodoc BEGINS STRING(s-NroSer, '999') ~
AND (input-var-3 = "" OR Ccbcdocu.flgest = input-var-3) ~
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.CcbCDocu.NroDoc|no
...
"CcbCDocu.NroDoc" "Nº de Control" ? "character" ? ? ? ? ? ? no ? no no "10.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* Correlativo */
DO:
    ...
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE
            {&CONDICION} AND
            {&CODIGO} BEGINS STRING(s-NroSer, '999') AND
            INTEGER(SUBSTRING({&CODIGO},4)) = INPUT FILL-IN-codigo
            /*( {&CODIGO} = STRING(s-NroSer, '999') + STRING(INPUT FILL-IN-codigo, '999999') )*/
            NO-LOCK NO-ERROR.
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY CcbCDocu.NroDoc DESCENDING
