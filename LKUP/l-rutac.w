...
&SCOPED-DEFINE CONDICION ( Di-RutaC.CodCia = INTEGER(input-var-1) ~
AND Di-RutaC.CodDiv = input-var-2 AND Di-RutaC.CodDoc = input-var-3 )
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      DI-RutaC.NroDoc COLUMN-LABEL "<<Numero>>"
      DI-RutaC.FchDoc COLUMN-LABEL "<<<Fecha>>>"
      DI-RutaC.Observ COLUMN-LABEL "Observaciones"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"DI-RutaC.NroDoc" "<<Numero>>" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   > INTEGRAL.DI-RutaC.FchDoc
"DI-RutaC.FchDoc" "<<<Fecha>>>" ? "date" ? ? ? ? ? ? no ?
     _FldNameList[3]   > INTEGRAL.DI-RutaC.Observ
"DI-RutaC.Observ" "Observaciones" ? "character" ? ? ? ? ? ? no ?
