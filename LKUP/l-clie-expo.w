...
&SCOPED-DEFINE CODIGO VtaCliVip.CodCli
...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaCliVip WHERE ~{&KEY-PHRASE} ~
      AND VtaCliVip.CodDiv = input-var-1 ~
 AND VtaCliVip.CodCia = cl-codcia NO-LOCK, ~
      FIRST gn-clie WHERE gn-clie.CodCia = VtaCliVip.CodCia ~
  AND gn-clie.CodCli = VtaCliVip.CodCli NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH VtaCliVip WHERE ~{&KEY-PHRASE} ~
      AND VtaCliVip.CodDiv = input-var-1 ~
 AND VtaCliVip.CodCia = cl-codcia NO-LOCK, ~
      FIRST gn-clie WHERE gn-clie.CodCia = VtaCliVip.CodCia ~
  AND gn-clie.CodCli = VtaCliVip.CodCli NO-LOCK ~
...
<SORTBY-OPTIONS>
Codigo|y||INTEGRAL.VtaCliVip.CodCli|yes
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      gn-clie.CodCli COLUMN-LABEL "Cliente" FORMAT "x(11)":U WIDTH 10.57
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaCliVip,INTEGRAL.gn-clie WHERE INTEGRAL.VtaCliVip ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST"
     _Where[1]         = "VtaCliVip.CodDiv = input-var-1
 AND VtaCliVip.CodCia = cl-codcia"
     _JoinCode[2]      = "gn-clie.CodCia = VtaCliVip.CodCia
  AND gn-clie.CodCli = VtaCliVip.CodCli"
     _FldNameList[1]   > INTEGRAL.gn-clie.CodCli
"gn-clie.CodCli" "Cliente" ? "character" ? ? ? ? ? ? no ? no no "10.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Codigo':U THEN DO:
           &Scope SORTBY-PHRASE BY VtaCliVip.CodCli
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-3 = {&SECOND-TABLE-IN-QUERY-{&BROWSE-NAME}}.nomcli.
