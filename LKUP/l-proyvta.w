...
&SCOPED-DEFINE CONDICION ( Factabla.CodCia = S-CODCIA AND Factabla.tabla = 'PVTA' )
&SCOPED-DEFINE CODIGO Factabla.Codigo

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( FacTabla.Nombre BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( FacTabla.Nombre , FILL-IN-filtro ) <> 0 )
...
<FOREIGN-KEYS>
Nombres que inicien con|y||INTEGRAL.FacTabla.Nombre
Nombres que contengan|y||INTEGRAL.FacTabla.Nombre
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Codigo|y||INTEGRAL.FacTabla.Codigo|yes
Descripcion|||INTEGRAL.FacTabla.Nombre|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
DEFINE VARIABLE FILL-IN-codigo AS CHARACTER FORMAT "X(5)":U 
     LABEL "Codigo" 
     VIEW-AS FILL-IN 
     SIZE 7.86 BY .81
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 133.14 BY 8.62
...
"FacTabla.Valor[1]" "Enero" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacTabla.Valor[2]
"FacTabla.Valor[2]" "Febrero" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.FacTabla.Valor[3]
"FacTabla.Valor[3]" "Marzo" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.FacTabla.Valor[4]
"FacTabla.Valor[4]" "Abril" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.FacTabla.Valor[5]
"FacTabla.Valor[5]" "Mayo" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > INTEGRAL.FacTabla.Valor[6]
"FacTabla.Valor[6]" "Junio" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > INTEGRAL.FacTabla.Valor[7]
"FacTabla.Valor[7]" "Julio" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > INTEGRAL.FacTabla.Valor[8]
"FacTabla.Valor[8]" "Agosto" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > INTEGRAL.FacTabla.Valor[9]
"FacTabla.Valor[9]" "Setiembre" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > INTEGRAL.FacTabla.Valor[10]
"FacTabla.Valor[10]" "Octubre" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > INTEGRAL.FacTabla.Valor[11]
"FacTabla.Valor[11]" "Noviembre" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[14]   > INTEGRAL.FacTabla.Valor[12]
"FacTabla.Valor[12]" "Diciembre" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Codigo':U THEN DO:
           &Scope SORTBY-PHRASE BY FacTabla.Codigo
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripcion':U THEN DO:
           &Scope SORTBY-PHRASE BY FacTabla.Nombre
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
