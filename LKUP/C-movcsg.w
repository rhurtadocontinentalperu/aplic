...
&SCOPED-DEFINE l-lookup h_l-movcmp
...
DEFINE VARIABLE h_l-movcmp AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 3.65 COL 62.43
     Btn_Cancel AT ROW 6.54 COL 62.14
     Btn_Help AT ROW 9.62 COL 62.14
     SPACE(3.28) SKIP(0.75)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-movcsg.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-movcmp ).
       RUN set-position IN h_l-movcmp ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.92 , 58.14 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.38 , 59.86 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-movcmp. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-movcmp ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-movcmp ,
