...
&SCOPED-DEFINE l-lookup h_l-cliruc
...
DEFINE VARIABLE h_l-cliruc AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 5.46 COL 52
     Btn_Cancel AT ROW 8.96 COL 52
     Btn_Help AT ROW 12.46 COL 52
     RECT-24 AT ROW 3.15 COL 50.72
     SPACE(1.42) SKIP(0.57)
...
ON CHOOSE OF Btn_OK IN FRAME C-Dialog /* Crear Codigo */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-cliruc.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-cliruc ).
       RUN set-position IN h_l-cliruc ( 1.00 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 15.50 , 48.57 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.00 , 50.43 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 13.29 ) NO-ERROR.

       /* Links to SmartLookup h_l-cliruc. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-cliruc ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-cliruc ,
