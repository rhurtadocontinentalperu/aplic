...
&SCOPED-DEFINE l-lookup h_l-vtadtabla
...
DEFINE VARIABLE h_l-vtadtabla AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-vtadtabla.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-vtadtabla ).
       RUN set-position IN h_l-vtadtabla ( 1.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_l-vtadtabla ( 10.92 , 54.72 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-vtadtabla. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-vtadtabla ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-vtadtabla ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-vtadtabla , 'AFTER':U ).
    END. /* Page 0 */
