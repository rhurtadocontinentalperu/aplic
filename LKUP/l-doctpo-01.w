...
/* AND ccbcdocu.tpofac = input-var-3                     */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.NroDoc COLUMN-LABEL "<<Numero>>>" FORMAT "XXX-XXXXXXXX":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 79.14 BY 8.62
...
"NroDoc" "<<Numero>>>" "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.CcbCDocu.CodCli
"CodCli" "<<Cliente>>>" ? "character" ? ? ? ? ? ? no ? no no "10.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/*             {&CONDICION} AND                                    */
/*             ( {&CODIGO} = input-var-2 + INPUT FILL-IN-codigo )  */
/*             NO-LOCK NO-ERROR.                                   */
    
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE {&CONDICION}
            AND INTEGER(SUBSTRING({&CODIGO}, 4)) = INPUT FILL-IN-codigo
            NO-LOCK NO-ERROR.
