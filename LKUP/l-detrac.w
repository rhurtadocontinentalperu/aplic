...
&SCOPED-DEFINE CONDICION cb-cdetr.codcia = INTEGER(input-var-1) AND cb-cdetr.periodo = INTEGER(input-var-2)
&SCOPED-DEFINE CODIGO cb-cdetr.nrolot

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( cb-cdetr.observaciones BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( cb-cdetr.observaciones , FILL-IN-filtro ) <> 0 )
...
&Scoped-define INTERNAL-TABLES cb-cdetr

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table cb-cdetr.NroLot cb-cdetr.Fchdoc ~
cb-cdetr.Observaciones 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH cb-cdetr WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table cb-cdetr
&Scoped-define FIRST-TABLE-IN-QUERY-br_table cb-cdetr
...
<FOREIGN-KEYS>
Nombres que inicien con|y||INTEGRAL.cb-cdetr.Observaciones
Nombres que contengan|y||INTEGRAL.cb-cdetr.Observaciones
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.cb-cdetr.NroLot|yes
Descripción|||INTEGRAL.cb-cdetr.Observaciones|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      cb-cdetr.NroLot COLUMN-LABEL "<<Lote>>"
      cb-cdetr.Fchdoc COLUMN-LABEL "Fec. Emision"
      cb-cdetr.Observaciones COLUMN-LABEL "Observaciones"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.cb-cdetr"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "{&CONDICION}"
     _FldNameList[1]   > INTEGRAL.cb-cdetr.NroLot
"NroLot" "<<Lote>>" ? "integer" ? ? ? ? ? ? no ?
     _FldNameList[2]   > INTEGRAL.cb-cdetr.Fchdoc
"Fchdoc" "Fec. Emision" ? "date" ? ? ? ? ? ? no ?
     _FldNameList[3]   > INTEGRAL.cb-cdetr.Observaciones
"Observaciones" "Observaciones" ? "character" ? ? ? ? ? ? no ?
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY cb-cdetr.NroLot
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY cb-cdetr.Observaciones
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = STRING({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nrolot)
        output-var-3 = STRING({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nrolot).
