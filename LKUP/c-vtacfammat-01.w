...
&SCOPED-DEFINE l-lookup h_l-vtacfammat-01
...
DEFINE VARIABLE h_l-vtacfammat-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.73 COL 52.43
     Btn_Cancel AT ROW 7.19 COL 52.43
     Btn_Help AT ROW 9.58 COL 52.43
     RECT-2 AT ROW 4 COL 50
     SPACE(1.13) SKIP(0.33)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-vtacfammat-01.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-vtacfammat-01 ).
       RUN set-position IN h_l-vtacfammat-01 ( 1.04 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-vtacfammat-01 ( 10.92 , 47.72 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.54 , 49.86 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-vtacfammat-01. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-vtacfammat-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-vtacfammat-01 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-vtacfammat-01 , 'AFTER':U ).
    END. /* Page 0 */
