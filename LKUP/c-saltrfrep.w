...
&SCOPED-DEFINE l-lookup h_l-saltrfrep
...
DEFINE VARIABLE h_l-saltrfrep AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LKUP/l-saltrfrep.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-saltrfrep ).
       RUN set-position IN h_l-saltrfrep ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-saltrfrep ( 10.92 , 59.00 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-saltrfrep. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-saltrfrep ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-saltrfrep ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-saltrfrep , 'AFTER':U ).
    END. /* Page 0 */
