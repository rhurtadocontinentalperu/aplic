...
&SCOPED-DEFINE CONDICION ( CustomerTransports.CodCia = s-CodCia ~
AND CustomerTransports.CustCode = input-var-1 AND CustomerTransports.FILENAME > '' )
&SCOPED-DEFINE CODIGO CustomerTransports.SupCode
...
&Scoped-define INTERNAL-TABLES CustomerTransports gn-prov gn-clie

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table CustomerTransports.SupCode ~
gn-prov.NomPro 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH CustomerTransports WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK, ~
      EACH gn-prov WHERE gn-prov.Ruc = CustomerTransports.SupCode ~
      AND gn-prov.CodCia = pv-codcia NO-LOCK, ~
      EACH gn-clie WHERE gn-clie.codcli = CustomerTransports.CustCode ~
      AND gn-clie.CodCia = cl-codcia ~
 AND gn-clie.CodCli = input-var-1 NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH CustomerTransports WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK, ~
      EACH gn-prov WHERE gn-prov.Ruc = CustomerTransports.SupCode ~
      AND gn-prov.CodCia = pv-codcia NO-LOCK, ~
      EACH gn-clie WHERE gn-clie.codcli = CustomerTransports.CustCode ~
 ...
 AND gn-clie.CodCli = input-var-1 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table CustomerTransports gn-prov gn-clie
&Scoped-define FIRST-TABLE-IN-QUERY-br_table CustomerTransports
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.CustomerTransports.SupCode|yes
...
DEFINE VARIABLE FILL-IN-codigo AS CHARACTER FORMAT "X(20)":U 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CustomerTransports.SupCode COLUMN-LABEL "RUC del Transportista" FORMAT "x(20)":U
      gn-prov.NomPro FORMAT "x(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 74.14 BY 8.62
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.CustomerTransports,INTEGRAL.gn-prov WHERE INTEGRAL.CustomerTransports ...,INTEGRAL.gn-clie WHERE INTEGRAL.CustomerTransports ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "{&Condicion}"
     _JoinCode[2]      = "gn-prov.Ruc = CustomerTransports.SupCode"
     _Where[2]         = "gn-prov.CodCia = pv-codcia"
     _JoinCode[3]      = "gn-clie.codcli = CustomerTransports.CustCode"
     _Where[3]         = "gn-clie.CodCia = cl-codcia
 AND gn-clie.CodCli = input-var-1"
     _FldNameList[1]   > INTEGRAL.CustomerTransports.SupCode
"CustomerTransports.SupCode" "RUC del Transportista" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY CustomerTransports.SupCode
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = {&SECOND-TABLE-IN-QUERY-{&BROWSE-NAME}}.codpro
        output-var-3 = {&SECOND-TABLE-IN-QUERY-{&BROWSE-NAME}}.nompro.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "CustomerTransports"}
