...
&Scoped-define INTERNAL-TABLES Almtmovm
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almtmovm WHERE ~{&KEY-PHRASE} ~
      AND {&condicion} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Almtmovm
...
<FOREIGN-KEYS>
Nombres que inicien con|y||integral.Almtmovm.Codmov
Nombres que contengan|y||integral.Almtmovm.Codmov
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Código|y||integral.Almtmovm.Codmov|yes
Descripción|||integral.Almtmovm.Codmov|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Almtmovm.Codmov COLUMN-LABEL "Codigo!Movmto." FORMAT "99":U
      Almtmovm.Desmov FORMAT "X(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 46 BY 8.62
...
DEFINE FRAME F-Main
     FILL-IN-codigo AT ROW 1.19 COL 20.29 COLON-ALIGNED
     CMB-filtro AT ROW 2.08 COL 1.86 NO-LABEL
     FILL-IN-filtro AT ROW 2.12 COL 22.29 NO-LABEL
     br_table AT ROW 3.08 COL 1.86
     "Buscar x" VIEW-AS TEXT
          SIZE 8 BY .58 AT ROW 1.31 COL 8.72
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "integral.Almtmovm"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "{&condicion}"
     _FldNameList[1]   > integral.Almtmovm.Codmov
"Almtmovm.Codmov" "Codigo!Movmto." ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.Almtmovm.Desmov
"Almtmovm.Desmov" ? "X(50)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY Almtmovm.Codmov
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN DO WITH FRAME {&FRAME-NAME}:
    ASSIGN
    ...
    DO k = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS:
        IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(k) THEN DO:
            output-var-2 = output-var-2 + (IF TRUE <> (output-var-2 > '') THEN '' ELSE ',') +
                STRING( {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codmov, "99" ).
        END.
    END.
END.
