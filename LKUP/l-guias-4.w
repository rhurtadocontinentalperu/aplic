...
AND CcbCDocu.NroDoc BEGINS C-NROSER ~
AND CcbCDocu.FlgEst = "P" ~
AND CcbCDocu.TpoFac = "R" ~
AND CcbCDocu.CodCli BEGINS input-var-1 )
...
<SORTBY-OPTIONS>
No.Pedido|y||integral.CcbCDocu.CodCia|yes,integral.CcbCDocu.CodDiv|yes,INTEGRAL.CcbCDocu.FchDoc|no,INTEGRAL.CcbCDocu.NroDoc|no
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "':U + 'No.Pedido' + '",
     SortBy-Case = ':U + 'No.Pedido').
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbCDocu.NroDoc COLUMN-LABEL "Guía" FORMAT "XXX-XXXXXXXX":U
...
"CcbCDocu.NroDoc" "Guía" "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'No.Pedido':U THEN DO:
      &Scope SORTBY-PHRASE BY CcbCDocu.CodCia BY CcbCDocu.CodDiv BY CcbCDocu.FchDoc DESCENDING BY CcbCDocu.NroDoc DESCENDING
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       RHC 23.06.06 se bloqueó el chequeo de la orden de despacho
------------------------------------------------------------------------------*/
...
if num-entries(output-var-2) > 0 then do:
    do i = 1 to num-entries(output-var-2):
        find first ccbcdocu
          ...
          else "La G/R " + entry(i,output-var-2) + " no tiene detalle".
    end.
    for each t-fac:
        t-fac.item = 0.
        for each t-deta 
