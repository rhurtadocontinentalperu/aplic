...
&SCOPED-DEFINE CONDICION lg-coser.codcia = INTEGER(input-var-1) ~
AND lg-coser.coddoc = input-var-2
&SCOPED-DEFINE CODIGO lg-coser.nrodoc
...
lg-coser.CodPro gn-prov.NomPro lg-coser.NroRef 
...
&Scoped-define QUERY-STRING-br_table FOR EACH lg-coser WHERE ~{&KEY-PHRASE} ~
  ...
  AND gn-prov.CodPro = lg-coser.CodPro NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
<SORTBY-OPTIONS>
Numero|y||INTEGRAL.lg-coser.CodCia|yes,INTEGRAL.lg-coser.NroDoc|no
Descripción|||INTEGRAL.lg-coser.CodCia|yes,INTEGRAL.lg-coser.CodPro|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "Numero,Descripción",
     SortBy-Case = Numero':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      lg-coser.NroDoc COLUMN-LABEL "<Orden>" FORMAT "999999":U
      lg-coser.Fchdoc COLUMN-LABEL "Fecha de Emision" FORMAT "99/99/9999":U
      lg-coser.CodPro COLUMN-LABEL "Proveedor" FORMAT "x(11)":U
      gn-prov.NomPro FORMAT "x(50)":U
      lg-coser.NroRef FORMAT "X(30)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 90.14 BY 8.62
...
/* Query rebuild information for BROWSE br_table
  ...
  AND INTEGRAL.gn-prov.CodPro = INTEGRAL.lg-coser.CodPro"
     _FldNameList[1]   > INTEGRAL.lg-coser.NroDoc
"lg-coser.NroDoc" "<Orden>" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.lg-coser.Fchdoc
"lg-coser.Fchdoc" "Fecha de Emision" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.lg-coser.CodPro
"lg-coser.CodPro" "Proveedor" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   = INTEGRAL.gn-prov.NomPro
     _FldNameList[5]   > INTEGRAL.lg-coser.NroRef
"lg-coser.NroRef" ? "X(30)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Numero':U THEN DO:
           &Scope SORTBY-PHRASE BY lg-coser.CodCia BY lg-coser.NroDoc DESCENDING
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY lg-coser.CodCia BY lg-coser.CodPro
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
