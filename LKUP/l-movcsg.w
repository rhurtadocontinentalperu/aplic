...
&SCOPED-DEFINE CONDICION (Almcmov.CodCia = S-CODCIA AND Almcmov.CodAlm = input-var-1 AND ~
Almcmov.TipMov = INPUT-VAR-2 AND Almcmov.CodMov = INTEGER(INPUT-VAR-3) AND Almcmov.FlgEst <> "A" AND Almcmov.FlgSit = "")
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almcmov.FchDoc COLUMN-LABEL "      Fecha    !    Emision"
      Almcmov.NomRef COLUMN-LABEL "Descripcion" FORMAT "x(43)"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 56.43 BY 8.62
...
"Almcmov.NroDoc" "Documento" ? "integer" ? ? ? ? ? ? no ?
     _FldNameList[3]   > integral.Almcmov.FchDoc
"Almcmov.FchDoc" "      Fecha    !    Emision" ? "date" ? ? ? ? ? ? no ?
     _FldNameList[4]   > integral.Almcmov.NomRef
"Almcmov.NomRef" "Descripcion" "x(43)" "character" ? ? ? ? ? ? no ?
...
DO I = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
   IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(I) THEN DO:
      ...
      IF output-var-2 = "" THEN 
            output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.NroRf2.
      ELSE output-var-2 = output-var-2 + "," + {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.NroRf2.
