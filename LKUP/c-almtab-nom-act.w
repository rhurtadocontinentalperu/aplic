...
&SCOPED-DEFINE l-lookup h_l-almtab-nom-act
...
&Scoped-Define ENABLED-OBJECTS RECT-19 Btn_OK Btn_Cancel 
...
DEFINE VARIABLE h_l-almtab-nom-act AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.69 COL 58.57
     Btn_Cancel AT ROW 7.35 COL 58.57
     RECT-19 AT ROW 3.81 COL 56
     SPACE(2.99) SKIP(2.75)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LKUP/l-almtab-nom-act.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-almtab-nom-act ).
       RUN set-position IN h_l-almtab-nom-act ( 1.00 , 1.43 ) NO-ERROR.
       RUN set-size IN h_l-almtab-nom-act ( 10.92 , 54.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.54 , 56.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-almtab-nom-act. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-almtab-nom-act ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-almtab-nom-act ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-almtab-nom-act , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE RECT-19 Btn_OK Btn_Cancel 
      WITH FRAME C-Dialog.
  VIEW FRAME C-Dialog.
