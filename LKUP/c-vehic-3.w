...
&SCOPED-DEFINE l-lookup h_l-vehic-3
...
DEFINE VARIABLE h_l-vehic-3 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.46 COL 71.43
     Btn_Cancel AT ROW 8.65 COL 71.43
     Btn_Help AT ROW 10.46 COL 71.43
     SPACE(3.99) SKIP(0.83)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-vehic-3.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Placa':U ,
             OUTPUT h_l-vehic-3 ).
       RUN set-position IN h_l-vehic-3 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-vehic-3 ( 10.92 , 67.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 69.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-vehic-3. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-vehic-3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-vehic-3 ,
