...
&SCOPED-DEFINE l-lookup h_l-notcre-1
...
DEFINE VARIABLE h_l-notcre-1 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 5.04 COL 77.72
     Btn_Cancel AT ROW 7.58 COL 77.72
     RECT-26 AT ROW 4.19 COL 76.14
     SPACE(2.13) SKIP(2.23)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-notcre-1.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Numero':U ,
             OUTPUT h_l-notcre-1 ).
       RUN set-position IN h_l-notcre-1 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-notcre-1 ( 10.96 , 73.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.81 , 75.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-notcre-1. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-notcre-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-notcre-1 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-notcre-1 , 'AFTER':U ).
    END. /* Page 0 */
