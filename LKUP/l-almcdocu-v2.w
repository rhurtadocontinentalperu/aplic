...
AND Almcdocu.FlgEst BEGINS input-var-2 ~
AND AlmCDocu.CodLlave BEGINS input-var-3

&SCOPED-DEFINE CODIGO Almcdocu.nrodoc

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( Almcdocu.libre_c05 BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( Almcdocu.libre_c05 , FILL-IN-filtro ) <> 0 )
...
INTEGER(SUBSTRING(AlmCDocu.nrodoc, LENGTH(Almcdocu.CodLlave) + 1)) @ x-NroDoc ~
AlmCDocu.FchDoc AlmCDocu.UsrCreacion AlmCDocu.Libre_c05 cb-auxi.Nomaux ~
AlmCDocu.Libre_c01 
...
&Scoped-define QUERY-STRING-br_table FOR EACH AlmCDocu WHERE ~{&KEY-PHRASE} ~
 ...
 AND cb-auxi.CodCia = cb-codcia NO-LOCK ~
    BY AlmCDocu.NroDoc DESCENDING INDEXED-REPOSITION
...
<SORTBY-OPTIONS>
NroDoc|y||INTEGRAL.AlmCDocu.NroDoc|no
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      INTEGER(SUBSTRING(AlmCDocu.nrodoc, LENGTH(Almcdocu.CodLlave) + 1)) @ x-NroDoc COLUMN-LABEL "Número" FORMAT "99999999":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 99.14 BY 8.65
...
/* Query rebuild information for BROWSE br_table
     ...
     _TblOptList       = ", FIRST"
     _OrdList          = "INTEGRAL.AlmCDocu.NroDoc|no"
     _Where[1]         = "{&Condicion}"
     _JoinCode[2]      = "cb-auxi.Codaux = AlmCDocu.Libre_c05"
     _Where[2]         = "cb-auxi.Clfaux = ""CCO""
 AND cb-auxi.CodCia = cb-codcia"
     _FldNameList[1]   > "_<CALC>"
"INTEGER(SUBSTRING(AlmCDocu.nrodoc, LENGTH(Almcdocu.CodLlave) + 1)) @ x-NroDoc" "Número" "99999999" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"AlmCDocu.UsrCreacion" "Solicitado por" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.AlmCDocu.Libre_c05
...
"AlmCDocu.Libre_c01" "Observaciones" "x(60)" "character" ? ? ? ? ? ? no ? no no "27.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'NroDoc':U THEN DO:
      &Scope SORTBY-PHRASE BY AlmCDocu.NroDoc DESCENDING
