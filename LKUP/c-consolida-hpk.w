...
&SCOPED-DEFINE l-lookup h_l-consolida-hpk
...
DEFINE VARIABLE h_l-consolida-hpk AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.46 COL 80.43
     Btn_Cancel AT ROW 8.65 COL 80.43
     Btn_Help AT ROW 10.46 COL 80.43
     SPACE(2.99) SKIP(0.83)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-consolida-hpk.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-consolida-hpk ).
       RUN set-position IN h_l-consolida-hpk ( 1.27 , 2.00 ) NO-ERROR.
       RUN set-size IN h_l-consolida-hpk ( 10.92 , 74.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 78.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-consolida-hpk. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-consolida-hpk ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-consolida-hpk ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-consolida-hpk , 'AFTER':U ).
    END. /* Page 0 */
