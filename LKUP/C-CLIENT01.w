...
&SCOPED-DEFINE l-lookup h_l-client01
...
DEFINE VARIABLE h_l-client01 AS HANDLE NO-UNDO.
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.00 , 50.57 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 13.29 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-client01.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-client01 ).
       RUN set-position IN h_l-client01 ( 1.15 , 1.43 ) NO-ERROR.
       RUN set-size IN h_l-client01 ( 15.50 , 48.57 ) NO-ERROR.

       /* Links to SmartLookup h_l-client01. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-client01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-client01 ,
             h_p-option , 'AFTER':U ).
    END. /* Page 0 */
