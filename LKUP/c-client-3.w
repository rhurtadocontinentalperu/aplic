...
&SCOPED-DEFINE l-lookup h_l-client-3
...
DEFINE VARIABLE h_l-client-3 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-client-3.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-client-3 ).
       RUN set-position IN h_l-client-3 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-client-3 ( 10.92 , 54.72 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-client-3. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-client-3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-client-3 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-client-3 , 'AFTER':U ).
    END. /* Page 0 */
