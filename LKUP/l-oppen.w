...
&SCOPED-DEFINE CONDICION ( PR-ODPC.CodCia = S-CODCIA AND LOOKUP(PR-ODPC.FlgEst,input-var-1) = 0 AND PR-ODPC.CodAlm = S-CODALM)

&SCOPED-DEFINE CODIGO PR-ODPC.NumOrd 
/* Preprocesadores para cada campo filtro */

&SCOPED-DEFINE FILTRO1 ( PR-ODPC.CodArt BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( PR-ODPC.CodArt , FILL-IN-filtro ) <> 0 )
...
&Scoped-define FIELDS-IN-QUERY-br_table PR-ODPC.FchOrd PR-ODPC.NumOrd ~
PR-ODPC.CodAlm PR-ODPC.CodPro PR-ODPC.codart PR-ODPC.Usuario ~
PR-ODPC.Observ[1] 
...
&Scoped-Define ENABLED-OBJECTS RECT-1 FILL-IN-codigo CMB-filtro br_table 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      PR-ODPC.FchOrd
      PR-ODPC.NumOrd COLUMN-LABEL "Orden No" FORMAT "X(8)"
      PR-ODPC.CodAlm COLUMN-LABEL "C.Conver"
      PR-ODPC.CodPro COLUMN-LABEL "Centro !Conversion" FORMAT "x(10)"
      PR-ODPC.codart COLUMN-LABEL "Codigo !Articulo" FORMAT "X(8)"
      PR-ODPC.Usuario
      PR-ODPC.Observ[1] FORMAT "X(45)"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 77.29 BY 9.23
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[2]   > integral.PR-ODPC.NumOrd
"PR-ODPC.NumOrd" "Orden No" "X(8)" "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   > integral.PR-ODPC.CodAlm
"PR-ODPC.CodAlm" "C.Conver" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[4]   > integral.PR-ODPC.CodPro
"PR-ODPC.CodPro" "Centro !Conversion" "x(10)" "character" ? ? ? ? ? ? no ?
     _FldNameList[5]   > integral.PR-ODPC.codart
"PR-ODPC.codart" "Codigo !Articulo" "X(8)" "character" ? ? ? ? ? ? no ?
     _FldNameList[6]   = integral.PR-ODPC.Usuario
     _FldNameList[7]   > integral.PR-ODPC.Observ[1]
"PR-ODPC.Observ[1]" ? "X(45)" "character" ? ? ? ? ? ? no ?
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* No. O/P */
DO:
    ASSIGN FILL-IN-codigo.
    IF FILL-IN-codigo = 0 THEN RETURN.
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE
            {&CONDICION} AND
            ( {&CODIGO} = STRING(FILL-IN-codigo,"999999") )
            NO-LOCK NO-ERROR.
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodPro.
