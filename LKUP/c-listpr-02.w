...
&SCOPED-DEFINE l-lookup h_l-listpr-02
...
DEFINE VARIABLE h_l-listpr-02 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 14.27 COL 61.14
     Btn_Cancel AT ROW 14.27 COL 72.14
     SPACE(62.28) SKIP(2.38)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-listpr-02.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Orden-Lista':U ,
             OUTPUT h_l-listpr-02 ).
       RUN set-position IN h_l-listpr-02 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-listpr-02 ( 13.15 , 143.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-detart ).
       RUN set-position IN h_v-detart ( 14.19 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 14.62 , 39.29 ) NO-ERROR.
       RUN set-size IN h_p-option ( 2.00 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-listpr-02. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-listpr-02 ).

       /* Links to SmartViewer h_v-detart. */
       RUN add-link IN adm-broker-hdl ( h_l-listpr-02 , 'Record':U , h_v-detart ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-listpr-02 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-detart ,
             h_l-listpr-02 , 'AFTER':U ).
