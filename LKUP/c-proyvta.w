...
&SCOPED-DEFINE l-lookup h_l-proyvta
...
DEFINE VARIABLE h_l-proyvta AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 12.31 COL 23
     Btn_Cancel AT ROW 12.31 COL 35
     SPACE(100.99) SKIP(0.99)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-proyvta.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Codigo':U ,
             OUTPUT h_l-proyvta ).
       RUN set-position IN h_l-proyvta ( 1.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_l-proyvta ( 10.92 , 127.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 12.04 , 5.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-proyvta. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-proyvta ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-proyvta ,
