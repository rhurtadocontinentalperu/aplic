...
&SCOPED-DEFINE l-lookup h_l-ocpen-01
...
DEFINE VARIABLE h_l-ocpen-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 12.54 COL 29.72
     Btn_Cancel AT ROW 12.54 COL 46.72
     Btn_Help AT ROW 12.54 COL 63.43
     SPACE(13.13) SKIP(0.33)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-ocpen-01.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-ocpen-01 ).
       RUN set-position IN h_l-ocpen-01 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-ocpen-01 ( 10.92 , 87.43 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 12.19 , 4.29 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-ocpen-01. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-ocpen-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-ocpen-01 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-ocpen-01 , 'AFTER':U ).
    END. /* Page 0 */
