...
&SCOPED-DEFINE l-lookup h_l-docflg-2
...
DEFINE VARIABLE h_l-docflg-2 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-docflg-2.r':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Documento':U ,
             OUTPUT h_l-docflg-2 ).
       RUN set-position IN h_l-docflg-2 ( 1.00 , 1.57 ) NO-ERROR.
       RUN set-size IN h_l-docflg-2 ( 10.92 , 83.57 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-docflg-2. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-docflg-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-docflg-2 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-docflg-2 , 'AFTER':U ).
    END. /* Page 0 */
