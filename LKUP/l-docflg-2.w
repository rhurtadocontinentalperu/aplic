...
DEFINE VAR x-bd-pedido AS CHAR INIT "BD-PEDIDO".

DEFINE VAR x-open-query AS LOG.
...
<SORTBY-OPTIONS>
Documento|y||integral.CcbCDocu.NroDoc|yes
...
RUN set-attribute-list (
    'SortBy-Options = "':U + 'Documento,Cliente' + '",
     SortBy-Case = ':U + 'Documento').
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 81.72 BY 9.5
...
ON FIND OF ccbcdocu DO:
    ...
    IF x-open-query = YES THEN DO:
        IF ccbcdocu.coddoc = 'BD' THEN DO:
            FIND FIRST vtatabla WHERE vtatabla.codcia = ccbcdocu.codcia AND
                        vtatabla.tabla = x-bd-pedido AND
                        vtatabla.llave_c1 = ccbcdocu.nrodoc AND
                        TRUE <> (vtatabla.libre_c03 > "")          /* Que no este anulado */
                        NO-LOCK NO-ERROR.
            IF AVAILABLE vtatabla THEN RETURN ERROR.
        END.
    END.
