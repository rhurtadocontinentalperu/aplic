...
&SCOPED-DEFINE CONDICION ( FacTabla.CodCia = s-codcia AND FacTabla.Tabla = input-var-1 )
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.FacTabla.Codigo|yes
Descripción|||INTEGRAL.FacTabla.Nombre|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacTabla.Codigo FORMAT "x(8)":U WIDTH 11.57
      FacTabla.Nombre FORMAT "x(40)":U WIDTH 37.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"FacTabla.Codigo" ? ? "character" ? ? ? ? ? ? no ? no no "11.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.FacTabla.Nombre
"FacTabla.Nombre" ? ? "character" ? ? ? ? ? ? no ? no no "37.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
