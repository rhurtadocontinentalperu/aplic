...
&SCOPED-DEFINE CONDICION ( gn-ccos.CodCia = S-CODCIA)
&SCOPED-DEFINE CODIGO gn-ccos.Cco

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( gn-ccos.DesCco BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( gn-ccos.DesCco , FILL-IN-filtro ) <> 0 )
...
&Scoped-define FIELDS-IN-QUERY-br_table gn-ccos.Cco gn-ccos.DesCco ~
gn-ccos.Estado 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH gn-ccos WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH gn-ccos WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
<FOREIGN-KEYS>
Nombres que inicien con|y||integral.gn-ven.NomVen
Nombres que contengan|y||integral.gn-ven.NomVen
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Código|y||integral.gn-ven.CodCia|yes,integral.gn-ven.CodVen|yes
Nombre|||integral.gn-ven.CodCia|yes,integral.gn-ven.NomVen|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "Código,Nombre",
     Sort-Case = Código':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      gn-ccos.Cco COLUMN-LABEL "Codigo" FORMAT "x(10)":U
      gn-ccos.DesCco COLUMN-LABEL "Descripcion" FORMAT "x(40)":U
      gn-ccos.Estado COLUMN-LABEL "Imputable" FORMAT "yes/no":U
            VIEW-AS TOGGLE-BOX
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 49.43 BY 13.04
...
"Cco" "Codigo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.gn-ccos.DesCco
"DesCco" "Descripcion" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.gn-ccos.Estado
"Estado" "Imputable" ? "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "TOGGLE-BOX" "," ? ? 5 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY gn-ven.CodCia BY gn-ven.CodVen
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Nombre':U THEN DO:
           &Scope SORTBY-PHRASE BY gn-ven.CodCia BY gn-ven.NomVen
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.cco.
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.cco.
