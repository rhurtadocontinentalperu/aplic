...
&SCOPED-DEFINE l-lookup h_l-vtactabla
...
DEFINE VARIABLE h_l-vtactabla AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-establecimientos.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-vtactabla ).
       RUN set-position IN h_l-vtactabla ( 1.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_l-vtactabla ( 10.92 , 54.72 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-vtactabla. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-vtactabla ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-vtactabla ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-vtactabla , 'AFTER':U ).
    END. /* Page 0 */
