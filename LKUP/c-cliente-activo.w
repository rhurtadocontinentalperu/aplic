...
&SCOPED-DEFINE l-lookup h_l-cliente-activo
...
DEFINE VARIABLE h_l-cliente-activo AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.46 COL 86.43
     Btn_Cancel AT ROW 8.65 COL 86.43
     SPACE(2.42) SKIP(2.64)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-cliente-activo.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-cliente-activo ).
       RUN set-position IN h_l-cliente-activo ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-cliente-activo ( 10.92 , 79.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 84.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-cliente-activo. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-cliente-activo ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-cliente-activo ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-cliente-activo , 'AFTER':U ).
    END. /* Page 0 */
