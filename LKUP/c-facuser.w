...
&SCOPED-DEFINE l-lookup h_l-facuser
...
DEFINE VARIABLE h_l-facuser AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.58 COL 59.29
     Btn_Cancel AT ROW 7.15 COL 59.29
     RECT-27 AT ROW 3.69 COL 57.86
     SPACE(0.56) SKIP(0.19)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-facuser.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Descripcion':U ,
             OUTPUT h_l-facuser ).
       RUN set-position IN h_l-facuser ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-facuser ( 10.96 , 56.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 57.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-facuser. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-facuser ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-facuser ,
