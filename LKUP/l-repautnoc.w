...
AND almcrepo.TipMov = input-var-1 ~
AND almcrepo.CodAlm = input-var-2 ~
AND almcrepo.FlgEst = input-var-3 )

&SCOPED-DEFINE CODIGO almcrepo.nrodoc

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( almcrepo.glosa BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( almcrepo.glosa , FILL-IN-filtro ) <> 0 )
...
almcrepo.NroDoc almcrepo.FchDoc almcrepo.AlmPed almcrepo.Glosa 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      almcrepo.TipMov COLUMN-LABEL "Tipo" FORMAT "x(3)":U
      ...
      almcrepo.Glosa FORMAT "x(60)":U WIDTH 41.86
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 85.14 BY 8.62
...
"TipMov" "Tipo" "x(3)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"NroDoc" "Numero" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.almcrepo.FchDoc
"FchDoc" "Fecha" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.almcrepo.AlmPed
"AlmPed" "Almacén!Despacho" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.almcrepo.Glosa
"Glosa" ? ? "character" ? ? ? ? ? ? no ? no no "41.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = STRING( {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nroser, "999" ) +
                        STRING( {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nrodoc, "99999999" )
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.glosa.
