...
ccbboldep.CodCli BEGINS input-var-1 AND ~
ccbboldep.CodDiv BEGINS s-coddiv    AND ~
ccbboldep.flgest BEGINS input-var-2 AND ~
ccbboldep.flgsit BEGINS input-var-3)
...
&Scoped-define FIELDS-IN-QUERY-br_table CcbBolDep.CodDiv CcbBolDep.NroDoc ~
CcbBolDep.NomCli CcbBolDep.FchDoc CcbBolDep.NroRef C-MON @ C-MON ~
CcbBolDep.SdoAct CcbBolDep.ImpTot CcbBolDep.Autorizo CcbBolDep.CodCta ~
CcbBolDep.CodAge 
...
&Scoped-Define ENABLED-OBJECTS RECT-1 FILL-IN-codigo br_table 
...
<FOREIGN-KEYS>
Nombres que inicien|y||integral.CcbBolDep.NomCli
...
<SORTBY-OPTIONS>
Documento|y||integral.CcbBolDep.CodCia|yes,integral.CcbBolDep.CodDoc|yes,integral.CcbBolDep.NroDoc|yes
Cliente|||integral.CcbBolDep.CodCia|yes,integral.CcbBolDep.CodCli|yes,integral.CcbBolDep.CodDoc|yes,integral.CcbBolDep.NroDoc|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbBolDep.NroDoc FORMAT "XXX-XXXXXXXX"
      CcbBolDep.NomCli FORMAT "x(40)"
      CcbBolDep.FchDoc COLUMN-LABEL "Fecha Dcmto"
      CcbBolDep.NroRef COLUMN-LABEL "Numero!De Deposito" FORMAT "X(15)"
      C-MON @ C-MON COLUMN-LABEL "Mon." FORMAT "XXX"
      CcbBolDep.SdoAct COLUMN-LABEL "Saldo Actual"
      CcbBolDep.ImpTot
      CcbBolDep.Autorizo
      CcbBolDep.CodCta
      CcbBolDep.CodAge
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "{&condicion}"
     _FldNameList[1]   > integral.CcbBolDep.CodDiv
"CcbBolDep.CodDiv" "Division" "XXXXX" "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   > integral.CcbBolDep.NroDoc
"CcbBolDep.NroDoc" ? "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   > integral.CcbBolDep.NomCli
"CcbBolDep.NomCli" ? "x(40)" "character" ? ? ? ? ? ? no ?
     _FldNameList[4]   > integral.CcbBolDep.FchDoc
"CcbBolDep.FchDoc" "Fecha Dcmto" ? "date" ? ? ? ? ? ? no ?
     _FldNameList[5]   > integral.CcbBolDep.NroRef
"CcbBolDep.NroRef" "Numero!De Deposito" "X(15)" "character" ? ? ? ? ? ? no ?
     _FldNameList[6]   > "_<CALC>"
"C-MON @ C-MON" "Mon." "XXX" ? ? ? ? ? ? ? no ?
     _FldNameList[7]   > integral.CcbBolDep.SdoAct
"CcbBolDep.SdoAct" "Saldo Actual" ? "decimal" ? ? ? ? ? ? no ?
     _FldNameList[8]   = integral.CcbBolDep.ImpTot
     _FldNameList[9]   = integral.CcbBolDep.Autorizo
     _FldNameList[10]   = integral.CcbBolDep.CodCta
     _FldNameList[11]   = integral.CcbBolDep.CodAge
...
ON FIND OF ccbboldep
DO:
   C-MON = "S/.".
   IF ccbboldep.CodMon = 2 THEN C-MON = "US$".
END.
    ...
    WHEN 'Nombres que inicien':U THEN DO:
       ...
       CASE RETURN-VALUE:
         ...
         WHEN 'Cliente':U THEN DO:
           &Scope SORTBY-PHRASE BY CcbBolDep.CodCia BY CcbBolDep.CodCli BY CcbBolDep.CodDoc BY CcbBolDep.NroDoc
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
