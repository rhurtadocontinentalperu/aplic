...
&SCOPED-DEFINE l-lookup h_l-Letcli
...
DEFINE VARIABLE h_l-letcli AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 12.08 COL 52.57
     Btn_Cancel AT ROW 12.08 COL 67.14
     SPACE(1.85) SKIP(0.64)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-letcli.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-letcli ).
       RUN set-position IN h_l-letcli ( 1.08 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.92 , 80.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 12.15 , 1.14 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 23.14 ) NO-ERROR.

       /* Links to SmartLookup h_l-letcli. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-letcli ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-letcli ,
