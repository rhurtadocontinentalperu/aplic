...
AND LOOKUP(faccpedi.flgest, input-var-3) > 0)
...
SUBSTRING(NroRef,1,3) + STRING(SUBSTRING(NroRef,4), '999999') @ NroRef 
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.FacCPedi.NroPed|no
Descripción|||INTEGRAL.FacCPedi.NomCli|yes,INTEGRAL.FacCPedi.NroPed|no
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacCPedi.NroPed COLUMN-LABEL "Numero" FORMAT "X(9)":U WIDTH 7.57
      FacCPedi.FchPed FORMAT "99/99/9999":U
      FacCPedi.CodCli COLUMN-LABEL "Solicitante" FORMAT "x(8)":U
      FacCPedi.NomCli FORMAT "x(40)":U WIDTH 46.29
      SUBSTRING(NroRef,1,3) + STRING(SUBSTRING(NroRef,4), '999999') @ NroRef COLUMN-LABEL "Reposición" FORMAT "XXX-XXXXXX":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"NroPed" "Numero" ? "character" ? ? ? ? ? ? no ? no no "7.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.FacCPedi.FchPed
     _FldNameList[3]   > INTEGRAL.FacCPedi.CodCli
"CodCli" "Solicitante" "x(8)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacCPedi.NomCli
"NomCli" ? "x(40)" "character" ? ? ? ? ? ? no ? no no "46.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"SUBSTRING(NroRef,1,3) + STRING(SUBSTRING(NroRef,4), '999999') @ NroRef" "Reposición" "XXX-XXXXXX" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
