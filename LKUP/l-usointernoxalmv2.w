...
DEF VAR x-Solicitante AS CHAR NO-UNDO.
DEF VAR x-Aprobadopor AS CHAR NO-UNDO.
...
AND Almcdocu.CodDoc = input-var-1 ~
AND Almcdocu.FlgEst = input-var-2 ~
AND AlmCDocu.Libre_c02 = ENTRY(1,input-var-3,'|') ~
AND AlmCDocu.Libre_c03 = ENTRY(2,input-var-3,'|')


&SCOPED-DEFINE CODIGO Almcdocu.nrodoc

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( Almcdocu.cco BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( Almcdocu.cco , FILL-IN-filtro ) <> 0 )
...
&Scoped-define FIELDS-IN-QUERY-br_table AlmCDocu.NroDoc AlmCDocu.FchDoc ~
AlmCDocu.UsrCreacion fSolicitante() @ x-Solicitante AlmCDocu.CodLlave ~
cb-auxi.Nomaux AlmCDocu.Libre_c01 fAprobadopor() @ x-Aprobadopor 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH AlmCDocu WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK, ~
      FIRST cb-auxi WHERE cb-auxi.Codaux = AlmCDocu.CodLlave ~
      AND cb-auxi.Clfaux = "CCO" ~
 AND cb-auxi.CodCia = cb-codcia NO-LOCK ~
...
<SORTBY-OPTIONS>
Numero|y||INTEGRAL.AlmCDocu.NroDoc|yes
Cco|||INTEGRAL.AlmCDocu.CodLlave|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "':U + 'Numero,Cco' + '",
     SortBy-Case = ':U + 'Numero').
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fAprobadopor L-table-Win 
FUNCTION fAprobadopor RETURNS CHARACTER
  ( /* parameter-definitions */ )  FORWARD.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fSolicitante L-table-Win 
FUNCTION fSolicitante RETURNS CHARACTER
  ( /* parameter-definitions */ )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      AlmCDocu.NroDoc FORMAT "X(20)":U WIDTH 9.57
      AlmCDocu.FchDoc FORMAT "99/99/9999":U WIDTH 9.29
      AlmCDocu.UsrCreacion COLUMN-LABEL "Solicitado por" FORMAT "x(8)":U
      fSolicitante() @ x-Solicitante COLUMN-LABEL "Nombre" FORMAT "x(30)":U
            WIDTH 21.57
      AlmCDocu.CodLlave COLUMN-LABEL "Cco" FORMAT "x(8)":U
      cb-auxi.Nomaux FORMAT "x(30)":U WIDTH 27
      AlmCDocu.Libre_c01 COLUMN-LABEL "Observaciones" FORMAT "x(30)":U
            WIDTH 31
      fAprobadopor() @ x-Aprobadopor COLUMN-LABEL "Aprobación Final" FORMAT "x(30)":U
            WIDTH 14.43
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 135 BY 8.65
...
DEFINE FRAME F-Main
     FILL-IN-codigo AT ROW 1.23 COL 23.57 COLON-ALIGNED
     ...
     "Buscar x" VIEW-AS TEXT
          SIZE 7.72 BY .54 AT ROW 1.38 COL 7.86
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[2]         = "cb-auxi.Clfaux = ""CCO""
 AND cb-auxi.CodCia = cb-codcia"
     _FldNameList[1]   > INTEGRAL.AlmCDocu.NroDoc
"AlmCDocu.NroDoc" ? "X(20)" "character" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.AlmCDocu.FchDoc
"AlmCDocu.FchDoc" ? ? "date" ? ? ? ? ? ? no ? no no "9.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.AlmCDocu.UsrCreacion
"AlmCDocu.UsrCreacion" "Solicitado por" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > "_<CALC>"
"fSolicitante() @ x-Solicitante" "Nombre" "x(30)" ? ? ? ? ? ? ? no ? no no "21.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.AlmCDocu.CodLlave
"AlmCDocu.CodLlave" "Cco" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.cb-auxi.Nomaux
"cb-auxi.Nomaux" ? "x(30)" "character" ? ? ? ? ? ? no ? no no "27" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.AlmCDocu.Libre_c01
"AlmCDocu.Libre_c01" "Observaciones" "x(30)" "character" ? ? ? ? ? ? no ? no no "31" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > "_<CALC>"
"fAprobadopor() @ x-Aprobadopor" "Aprobación Final" "x(30)" ? ? ? ? ? ? ? no ? no no "14.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* N'umero */
DO:
...
ON FIND OF Faccpedi
DO:
   C-MON = IF Faccpedi.CodMon = 1 THEN "S/." ELSE "US$".
END.
/* ***************************  Main Block  *************************** */
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Numero':U THEN DO:
      &Scope SORTBY-PHRASE BY AlmCDocu.NroDoc
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    WHEN 'Cco':U THEN DO:
      &Scope SORTBY-PHRASE BY AlmCDocu.CodLlave
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.CodLlave.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fAprobadopor L-table-Win 
FUNCTION fAprobadopor RETURNS CHARACTER
...
------------------------------------------------------------------------------*/
  ...
  FIND _User WHERE _User._UserId = AlmCDocu.UsrAprobacion NO-LOCK NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fSolicitante L-table-Win 
FUNCTION fSolicitante RETURNS CHARACTER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
