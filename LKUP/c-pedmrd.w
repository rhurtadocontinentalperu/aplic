...
&SCOPED-DEFINE l-lookup h_l-pedmrd
...
DEFINE VARIABLE h_l-pedmrd AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.35 COL 82
     Btn_Cancel AT ROW 8.54 COL 82
     Btn_Help AT ROW 10.35 COL 82
     SPACE(5.85) SKIP(0.94)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'lkup/l-pedmrd.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-pedmrd ).
       RUN set-position IN h_l-pedmrd ( 1.00 , 1.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.38 , 82.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-pedmrd. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-pedmrd ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-pedmrd ,
