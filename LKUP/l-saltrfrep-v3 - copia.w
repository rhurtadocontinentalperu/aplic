...
DEF VAR LocalCodMov AS INTE NO-UNDO.
DEF VAR LocalNroSer AS INTE NO-UNDO.
LocalTipMov = input-var-1.
LocalCodMov = INTEGER(input-var-2).
LocalNroSer = INTEGER(input-var-3).
&SCOPED-DEFINE CONDICION (Almcmov.CodCia = S-CODCIA AND ~
Almcmov.CodRef = "OTR" AND ~
Almcmov.TipMov = LocalTipMov AND ~
Almcmov.CodMov = LocalCodMov AND ~
Almcmov.NroSer = LocalNroSer )
...
Almcmov.NroDoc Almcmov.FchDoc Almcmov.NomRef 
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.Almcmov.FchDoc|no,INTEGRAL.Almcmov.NroDoc|no
...
DEFINE VARIABLE FILL-IN-codigo AS INTEGER FORMAT "999999999":U INITIAL 0 
     LABEL "Documento" 
     VIEW-AS FILL-IN 
     SIZE 9.72 BY .81
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almcmov.NroSer COLUMN-LABEL "Serie" FORMAT "999":U WIDTH 4.14
      Almcmov.NroDoc COLUMN-LABEL "Documento" FORMAT "999999999":U
            WIDTH 9.43
      Almcmov.FchDoc COLUMN-LABEL "      Fecha    !    Emision" FORMAT "99/99/9999":U
      Almcmov.NomRef COLUMN-LABEL "Descripcion" FORMAT "x(43)":U
            WIDTH 30.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 64.14 BY 8.62
...
"Almcmov.NroSer" "Serie" ? "integer" ? ? ? ? ? ? no ? no no "4.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.Almcmov.NroDoc
"Almcmov.NroDoc" "Documento" ? "integer" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > integral.Almcmov.FchDoc
"Almcmov.FchDoc" "      Fecha    !    Emision" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > integral.Almcmov.NomRef
"Almcmov.NomRef" "Descripcion" "x(43)" "character" ? ? ? ? ? ? no ? no no "30.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY Almcmov.FchDoc DESCENDING BY Almcmov.NroDoc DESCENDING
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = STRING( {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nrodoc, "999999999" )
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nomref.
