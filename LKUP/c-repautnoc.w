...
&SCOPED-DEFINE l-lookup h_l-repautnoc
...
DEFINE VARIABLE h_l-repautnoc AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.46 COL 93.43
     Btn_Cancel AT ROW 8.65 COL 93.43
     Btn_Help AT ROW 10.46 COL 93.43
     SPACE(3.56) SKIP(0.83)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-repautnoc.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_l-repautnoc ).
       RUN set-position IN h_l-repautnoc ( 1.27 , 3.00 ) NO-ERROR.
       RUN set-size IN h_l-repautnoc ( 10.92 , 87.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 91.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-repautnoc. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-repautnoc ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-repautnoc ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-repautnoc , 'AFTER':U ).
    END. /* Page 0 */
