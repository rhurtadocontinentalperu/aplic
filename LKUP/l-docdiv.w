...
AND ccbcdocu.coddiv BEGINS input-var-3 )
...
CcbCDocu.ImpTot CcbCDocu.SdoAct CcbCDocu.FlgEst 
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.CcbCDocu.CodDoc|yes
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.FlgEst COLUMN-LABEL "E" FORMAT "X":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 60.14 BY 8.62
...
"NroDoc" "<<Numero>>" ? "character" ? ? ? ? ? ? no ? no no "10.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"SdoAct" "Saldo Actual" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.CcbCDocu.FlgEst
"FlgEst" "E" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY CcbCDocu.CodDoc
