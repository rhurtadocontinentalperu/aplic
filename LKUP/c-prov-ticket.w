...
&SCOPED-DEFINE l-lookup h_l-prov-ticket
...
DEFINE VARIABLE h_l-prov-ticket AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.73 COL 79.57
     Btn_Cancel AT ROW 7.19 COL 79.57
     Btn_Help AT ROW 9.58 COL 79.57
     RECT-2 AT ROW 4 COL 77.14
     SPACE(2.14) SKIP(0.33)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'LKUP/l-prov-ticket.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Codigo':U ,
             OUTPUT h_l-prov-ticket ).
       RUN set-position IN h_l-prov-ticket ( 1.04 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-prov-ticket ( 10.92 , 75.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.54 , 77.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-prov-ticket. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-prov-ticket ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-prov-ticket ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-prov-ticket , 'AFTER':U ).
    END. /* Page 0 */
