...
AND cb-cmov.codope = cb-oper.codope
...
AND OOCb-cmov.codope = cb-oper.codope
...
&SCOPED-DEFINE Condicion3 cb-cmov.codcia = s-codcia ~
AND cb-cmov.periodo = COMBO-BOX-Periodo ~
AND cb-cmov.nromes = COMBO-BOX-NroMes ~
AND ( COMBO-BOX-CodOpe = 'Todas' OR cb-cmov.codope = COMBO-BOX-CodOpe) ~
AND (TOGGLE-1 = NO OR (cb-cmov.fchast >= FILL-IN-FchAst-1 AND cb-cmov.fchast <= FILL-IN-FchAst-2))
...
&Scoped-define QUERY-STRING-BROWSE-5 FOR EACH cb-cmov ~
      WHERE {&Condicion3} NO-LOCK INDEXED-REPOSITION
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Borrar-OO wWin 
PROCEDURE Borrar-OO :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
FOR EACH cb-oper WHERE cb-oper.codcia = cb-codcia NO-LOCK:
    IF COMBO-BOX-CodOpe <> 'Todas' AND NOT cb-oper.codope = COMBO-BOX-CodOpe THEN NEXT.
    FOR EACH OOCb-cmov WHERE {&Condicion2} EXCLUSIVE-LOCK:
        IF TOGGLE-1 = YES AND NOT (OOCb-cmov.fchast >= FILL-IN-FchAst-1 AND OOCb-cmov.fchast <= FILL-IN-FchAst-2) THEN NEXT.
        FOR EACH OOCb-dmov OF OOCb-cmov EXCLUSIVE-LOCK:
            DELETE OOCb-dmov.
        END.
        DELETE OOCb-cmov.
    END.
END.
IF AVAILABLE OOCb-cmov THEN RELEASE OOCb-cmov.
IF AVAILABLE OOCb-dmov THEN RELEASE OOCb-dmov.
...
/* 1ro borramos datos */
Fi-Mensaje = "BORRANDO TABLA DE TRABAJO".
...
FOR EACH cb-oper WHERE cb-oper.codcia = cb-codcia NO-LOCK:
    IF COMBO-BOX-CodOpe <> 'Todas' AND NOT cb-oper.codope = COMBO-BOX-CodOpe THEN NEXT.
    FOR EACH cb-cmov WHERE {&Condicion} NO-LOCK:
        IF TOGGLE-1 = YES AND NOT (cb-cmov.fchast >= FILL-IN-FchAst-1 AND cb-cmov.fchast <= FILL-IN-FchAst-2) THEN NEXT.
        Fi-Mensaje = "ASIENTO: " + cb-cmov.codope + " " + cb-cmov.nroast.
        DISPLAY Fi-Mensaje WITH FRAME F-Proceso.
        CREATE OOCb-cmov.
        BUFFER-COPY Cb-cmov
            TO OOCb-cmov
