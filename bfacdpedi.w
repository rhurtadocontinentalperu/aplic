...
&Scoped-define DATA-FIELD-DEFS "aplic/dfacdpedi.i"
...
&Scoped-define FIELDS-IN-QUERY-br_table rowObject.codmat ~
rowObject.DesMatCalc rowObject.CanPed 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table rowObject.codmat ~
rowObject.CanPed 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      rowObject.DesMatCalc FORMAT "x(40)":U WIDTH 78.57
      rowObject.CanPed FORMAT ">,>>>,>>9.9999":U
  ENABLE
      rowObject.codmat HELP "?"
      rowObject.CanPed
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-AUTO-VALIDATE NO-ROW-MARKERS SEPARATORS SIZE 110 BY 6.65 FIT-LAST-COLUMN.
...
"rowObject.codmat" ? ? "character" ? ? ? ? ? ? yes "?" no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > _<SDO>.rowObject.DesMatCalc
"rowObject.DesMatCalc" ? ? "character" ? ? ? ? ? ? no ? no no "78.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > _<SDO>.rowObject.CanPed
"rowObject.CanPed" ? ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE enableFields bTableWin 
PROCEDURE enableFields :
/*------------------------------------------------------------------------------
  Purpose:     Super Override
  Parameters:
  Notes:       CodMat solo se puede cambiar durante Add o Copy.
------------------------------------------------------------------------------*/
  DEFINE VARIABLE cNewRecord AS CHARACTER NO-UNDO.

  RUN SUPER.

  /* NewRecord ya fue establecido por ADM2 antes de insertar la fila.
     RowMod todavía puede estar vacío en este punto. */
  {get NewRecord cNewRecord}.

  IF CAN-DO('ADD,COPY':U, cNewRecord) THEN
    ASSIGN RowObject.CodMat:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO.
  ELSE
    ASSIGN RowObject.CodMat:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES.
