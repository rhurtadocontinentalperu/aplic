...
DEFINE SHARED TEMP-TABLE T-MATPRO LIKE Almmmatg
       FIELD PreUni AS DEC
       FIELD CanPed AS DEC
...
FacCPedi.FchEnt FacCPedi.DirCli FacCPedi.CodPos FacCPedi.CodVen ~
FacCPedi.CodMon FacCPedi.FmaPgo FacCPedi.Glosa 
&Scoped-define ENABLED-TABLES FacCPedi
&Scoped-define FIRST-ENABLED-TABLE FacCPedi
&Scoped-Define ENABLED-OBJECTS RECT-21 
&Scoped-Define DISPLAYED-FIELDS FacCPedi.CodCli FacCPedi.NroPed ~
FacCPedi.FchPed FacCPedi.NomCli FacCPedi.TpoCmb FacCPedi.RucCli ~
...
&Scoped-Define DISPLAYED-OBJECTS F-Estado F-Nomtar FILL-IN-Postal C-TpoVta ~
F-nOMvEN F-CndVta 
...
DO:
  ...
  IF SELF:SCREEN-VALUE = '11111111112' THEN C-TpoVta:SCREEN-VALUE = 'Factura'.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Asigna-Producto V-table-Win 
...
------------------------------------------------------------------------------*/
  ...
  RUN provmay/d-vtaprov (OUTPUT s-Ok).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Productos V-table-Win 
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH PEDI BY PEDI.NroItm:
    x-NroItm = PEDI.NroItm + 1.
  END.
  
  FOR EACH T-MATPRO WHERE T-MATPRO.CanPed > 0 BY T-MATPRO.DesMat:
    FIND PEDI WHERE PEDI.CodMat = T-MATPRO.CodMat EXCLUSIVE-LOCK NO-ERROR.
    ...
    ASSIGN
        ...
        PEDI.CodMat = T-MATPRO.CodMat
        PEDI.Factor = 1
        PEDI.CanPed = T-MATPRO.CanPed
        PEDI.UndVta = T-MATPRO.Chr__01
        PEDI.AlmDes = s-CodAlm
        PEDI.Por_DSCTOS[2] = T-MATPRO.PorMax
        PEDI.FlgEst = T-MATPRO.ClfCli.
  END.
  RUN Procesa-Handle IN lh_Handle ('Recalculo').
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH PEDI NO-LOCK: 
    ...
    IF AVAILABLE Almmmatg THEN DO:
        ...
        THEN X-STANDFORD = X-STANDFORD + PEDI.ImpLin.
        IF PEDI.Por_Dsctos[3] = 0 THEN DO:
           IF Almmmatg.CodFam = "001" 
           THEN X-LINEA1 = X-LINEA1 + PEDI.ImpLin.
...
------------------------------------------------------------------------------*/
  ...
  IF FacCPedi.FlgEst <> "A" THEN DO:
      ...
      RUN vtamay/R-CotExpLib (ROWID(FacCPedi)).
  END.
...
/*        AND FacCPedi.CodVen:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '002'*/
