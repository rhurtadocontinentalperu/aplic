...
DEF INPUT PARAMETER x-FlgEst AS CHAR.
...
/*message s-tpocmb 'precio base 1' f-prebas.*/

/* RHC 27.01.06 Tipo de cambio especial para Roiny Chumpitas */
/*IF Almmmatg.codfam = '010' THEN DO:
 *     f-PreBas = f-PreBas / s-tpocmb * 3.45.
 * END.*/
/* RHC 22.08.06 */
IF Almmmatg.codfam = '010' THEN DO:     /* Productos propios */
    FIND Almtfami OF Almmmatg NO-LOCK NO-ERROR.
    IF AVAILABLE Almtfami AND Almmmatg.MonVta <> s-CodMon THEN DO:
        IF s-CodMon = 1
        THEN f-PreBas = f-PreBas / s-tpocmb * Almtfami.TpoCmb.
        ELSE f-PreBas = f-PreBas * s-tpocmb / Almtfami.TpoCmb.
    END.
END.
/*message 'precio base 2' f-prebas.*/
...
/*message x-clfcli skip 'por clasificacion ' maxcat skip
    'por condicion ' maxvta.*/
...
/*message 'precio venta antes' f-prevta skip s-cndvta.*/
...
/*message 'precio venta despues' f-prevta.*/
/************************************************/
RUN BIN/_ROUND1(F-PREVTA,X-NRODEC,OUTPUT F-PREVTA).


    /* CALCULO ESPECIAL DE PRECIOS */
    FIND Facctmat WHERE Facctmat.codcia = s-codcia
        AND Facctmat.codmat = Almmmatg.codmat
        NO-LOCK NO-ERROR.
    IF AVAILABLE Facctmat THEN DO:
        {provmay/preciovt.i}
    END.
