...
&Scoped-Define ENABLED-OBJECTS C-CodDoc C-NroSer BUTTON-1 BUTTON-11 
...
DEFINE VARIABLE h_v-faccre AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     C-CodDoc AT ROW 7.08 COL 1 NO-LABEL
     C-NroSer AT ROW 7.08 COL 8.57 NO-LABEL
     BUTTON-1 AT ROW 6.96 COL 72
     Btn-Print AT ROW 7.23 COL 79.57
     Btn-Guias AT ROW 8.54 COL 78.29
     Btn-Pedido AT ROW 9.5 COL 78.29
     BUTTON-11 AT ROW 17.35 COL 79 HELP
          "Salida"
     RECT-20 AT ROW 6.96 COL 79.14
...
/* SETTINGS FOR FRAME F-Main
                                                                        */
/* SETTINGS FOR BUTTON Btn-Guias IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       Btn-Guias:HIDDEN IN FRAME F-Main           = TRUE.

/* SETTINGS FOR BUTTON Btn-Pedido IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       Btn-Pedido:HIDDEN IN FRAME F-Main           = TRUE.

/* SETTINGS FOR BUTTON Btn-Print IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       Btn-Print:HIDDEN IN FRAME F-Main           = TRUE.

/* SETTINGS FOR COMBO-BOX C-CodDoc IN FRAME F-Main
   ALIGN-L                                                              */
/* SETTINGS FOR COMBO-BOX C-NroSer IN FRAME F-Main
   ALIGN-L                                                              */
/* SETTINGS FOR RECTANGLE RECT-20 IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
...
DO:
  RUN Asigna-Guias IN h_v-faccre.
END.
...
DO:
    RUN Asigna-Pedidos IN h_v-faccre.
END.
...
DO:
  RUN dispatch IN h_v-faccre ('imprime':U).
END.
...
ON CHOOSE OF BUTTON-11 IN FRAME F-Main /* Salir */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/v-faccre.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-faccre ).
       RUN set-position IN h_v-faccre ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.96 , 86.14 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 6.96 , 25.14 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 1.27 , 16.29 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/q-docmto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-docmto ).
       RUN set-position IN h_q-docmto ( 6.96 , 15.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-faccre. */
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-faccre ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'cto/b-docmto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-docmto ).
       RUN set-position IN h_b-docmto ( 8.35 , 1.14 ) NO-ERROR.
       /* Size in UIB:  ( 9.15 , 87.00 ) */
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta/b-tmpdoc.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpdoc ).
       RUN set-position IN h_b-tmpdoc ( 8.31 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.31 , 76.00 ) */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE C-CodDoc C-NroSer BUTTON-1 BUTTON-11 
      WITH FRAME F-Main IN WINDOW W-Win.
