...
CcbDDocu.PreUni CcbDDocu.Por_Dsctos[1] CcbDDocu.ImpLin 
...
&Scoped-Define ENABLED-OBJECTS RECT-11 br_table 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbDDocu.codmat COLUMN-LABEL "Articulo" FORMAT "X(8)"
      Almmmatg.DesMat FORMAT "X(38)"
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(10)"
      CcbDDocu.UndVta COLUMN-LABEL "Unidad" FORMAT "X(8)"
      CcbDDocu.AlmDes COLUMN-LABEL "Alm!Des"
      CcbDDocu.CanDes FORMAT ">>>,>>9.99"
      CcbDDocu.PreUni COLUMN-LABEL "Precio!Unitario" FORMAT ">>,>>9.999999"
      CcbDDocu.Por_Dsctos[1]
      CcbDDocu.ImpLin FORMAT ">>>>,>>9.99"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 87 BY 7.65
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     F-TotBrt AT ROW 9.38 COL 1 NO-LABEL
     F-ImpDes AT ROW 9.38 COL 9.72 COLON-ALIGNED NO-LABEL
     F-ValVta AT ROW 9.38 COL 20.72 COLON-ALIGNED NO-LABEL
     F-ImpIgv AT ROW 9.38 COL 31.43 COLON-ALIGNED NO-LABEL
     F-ImpFle AT ROW 9.38 COL 42.14 COLON-ALIGNED NO-LABEL
     F-ImpExo AT ROW 9.38 COL 55 NO-LABEL
     F-ImpTot AT ROW 9.38 COL 63.72 COLON-ALIGNED NO-LABEL
     Btn-Otros AT ROW 9.04 COL 77.29
     "I.G.V." VIEW-AS TEXT
          SIZE 5.14 BY .5 AT ROW 8.88 COL 37.72
     "T.Descuento" VIEW-AS TEXT
          SIZE 9.43 BY .5 AT ROW 8.88 COL 11.86
     "T.Exonerado" VIEW-AS TEXT
          SIZE 9.43 BY .5 AT ROW 8.88 COL 55.29
     RECT-11 AT ROW 8.73 COL 1
     "Flete" VIEW-AS TEXT
          SIZE 4.57 BY .5 AT ROW 8.88 COL 47.57
     "Total Bruto" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 8.88 COL 2.72
     "Valor Venta" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 8.88 COL 24.14
     "Total Importe" VIEW-AS TEXT
          SIZE 9.43 BY .5 AT ROW 8.88 COL 66
...
/* SETTINGS FOR BUTTON Btn-Otros IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       Btn-Otros:HIDDEN IN FRAME F-Main           = TRUE.
...
"CcbDDocu.codmat" "Articulo" "X(8)" "character" ? ? ? ? ? ? no ?
...
"CcbDDocu.UndVta" "Unidad" "X(8)" "character" ? ? ? ? ? ? no ?
     _FldNameList[5]   > integral.CcbDDocu.AlmDes
"CcbDDocu.AlmDes" "Alm!Des" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[6]   > integral.CcbDDocu.CanDes
"CcbDDocu.CanDes" ? ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ?
     _FldNameList[7]   > integral.CcbDDocu.PreUni
"CcbDDocu.PreUni" "Precio!Unitario" ">>,>>9.999999" "decimal" ? ? ? ? ? ? no ?
     _FldNameList[8]   = integral.CcbDDocu.Por_Dsctos[1]
     _FldNameList[9]   > integral.CcbDDocu.ImpLin
"CcbDDocu.ImpLin" ? ">>>>,>>9.99" "decimal" ? ? ? ? ? ? no ?
