...
&Scoped-define INTERNAL-TABLES PR-LIQC

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for QUERY Query-Main                                     */
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH PR-LIQC WHERE ~{&KEY-PHRASE} ~
      AND PR-LIQC.Codcia = S-CODCIA NO-LOCK ~
    BY PR-LIQC.FchLiq DESCENDING INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main PR-LIQC
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main PR-LIQC
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.PR-LIQC"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE"
     _OrdList          = "INTEGRAL.PR-LIQC.FchLiq|no"
     _Where[1]         = "integral.PR-LIQC.Codcia = S-CODCIA"
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND PR-LIQC WHERE PR-LIQC.Codcia = S-CODCIA AND
                   PR-LIQC.NumLiq = F-Tabla NO-LOCK NO-ERROR.
IF AVAILABLE PR-LIQC THEN output-var-1 = ROWID(PR-LIQC).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
