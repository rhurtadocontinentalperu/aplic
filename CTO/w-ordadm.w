...
DEFINE NEW SHARED TEMP-TABLE RUTA LIKE INTEGRAL.DI-RutaC.
DEFINE NEW SHARED TEMP-TABLE T-DOSER LIKE INTEGRAL.lg-doser.
...
DEF NEW SHARED VAR s-coddoc AS CHAR INIT 'OCA'.
...
&Scoped-Define ENABLED-OBJECTS RECT-3 BUTTON-9 
...
DEFINE FRAME F-Main
     BUTTON-9 AT ROW 1.19 COL 101 HELP
          "Salida"
     RECT-3 AT ROW 2.92 COL 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 109.43 BY 21.23
         FONT 4.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "ORDENES DE COMPRA ADMINISTRATIVA"
         HEIGHT             = 21.23
         WIDTH              = 109.43
         MAX-HEIGHT         = 21.23
         MAX-WIDTH          = 109.43
...
ON END-ERROR OF W-Win /* ORDENES DE COMPRA ADMINISTRATIVA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ORDENES DE COMPRA ADMINISTRATIVA */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ordser ).
       RUN set-position IN h_v-ordser ( 3.12 , 3.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ordser ).
       RUN set-position IN h_b-ordser ( 12.92 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ordser-1 ).
       RUN set-position IN h_v-ordser-1 ( 20.23 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-ordser ).
       RUN set-position IN h_q-ordser ( 1.38 , 80.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_t-ordser ).
       RUN set-position IN h_t-ordser ( 12.54 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.04 , 109.43 ) */
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH DETALLE,
        ...
        BREAK BY DETALLE.CodVeh BY DETALLE.FchDoc:
    IF FIRST-OF(DETALLE.CodVeh) OR FIRST-OF(DETALLE.FchDoc)
    THEN ASSIGN
            x-Despacho = 0
            x-Abastecimiento = 0
            x-Recojo = 0
    ...
    IF LAST-OF(DETALLE.CodVeh) OR LAST-OF(DETALLE.FchDoc) THEN DO:
        CREATE T-DOSER.
        ASSIGN
            T-DOSER.CodCia = s-codcia
            T-DOSER.codser = 'S0000019'
            T-DOSER.desser = ' ' + STRING(DETALLE.FchDoc) +
                            ' ' + DETALLE.CodVeh
            T-DOSER.CanAten = 1 
