...
&Scoped-define INTERNAL-TABLES PR-FORMC

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for QUERY Query-Main                                     */
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH PR-FORMC WHERE ~{&KEY-PHRASE} ~
      AND PR-FORMC.Codcia = s-codcia NO-LOCK ~
    BY PR-FORMC.FchIng DESCENDING INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main PR-FORMC
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main PR-FORMC


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS RECT-2 F-Tabla 
...
DEFINE VARIABLE F-Tabla AS CHARACTER FORMAT "X(6)":U 
     VIEW-AS FILL-IN 
     SIZE 7.72 BY .69
     FONT 2 NO-UNDO.
...
DEFINE FRAME FRAME-B
     F-Tabla AT ROW 1.19 COL 1.86 NO-LABEL
...
/* SETTINGS FOR FRAME FRAME-B
                                                                        */
/* SETTINGS FOR FILL-IN F-Tabla IN FRAME FRAME-B
   ALIGN-L                                                              */
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.PR-FORMC"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE"
     _OrdList          = "INTEGRAL.PR-FORMC.FchIng|no"
     _Where[1]         = "integral.PR-FORMC.Codcia = s-codcia"
     _Design-Parent    is WINDOW q-tables @ ( 1 , 6.43 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
&Scoped-define SELF-NAME F-Tabla
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-Tabla q-tables
ON LEAVE OF F-Tabla IN FRAME FRAME-B
OR "RETURN":U OF F-Tabla
DO:
  IF F-tabla:SCREEN-VALUE = "" THEN RETURN.
  ASSIGN F-tabla.
  F-tabla = STRING(INTEGER(F-tabla),"999999").
  RUN Ubica-Registro.
  F-tabla:SCREEN-VALUE = "".
...
FIND FIRST PR-Formc WHERE PR-Formc.Codcia = S-CODCIA AND
                          PR-Formc.CodArt = F-Tabla NO-LOCK NO-ERROR.
IF AVAILABLE PR-Formc THEN output-var-1 = ROWID(PR-Formc).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
