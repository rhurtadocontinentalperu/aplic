...
DEFINE VARIABLE h_b-mov-m-pr AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 11.19 COL 65 HELP
          "Salida"
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Registro de Horas/OT (Empleados)"
         HEIGHT             = 12.54
         WIDTH              = 73
         MAX-HEIGHT         = 12.54
         MAX-WIDTH          = 73
...
ON END-ERROR OF W-Win /* Registro de Horas/OT (Empleados) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Registro de Horas/OT (Empleados) */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'cto/b-mov-m-pr.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-mov-m-pr ).
       RUN set-position IN h_b-mov-m-pr ( 4.15 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 8.77 , 61.57 ) */
       ...
       /* Links to SmartBrowser h_b-mov-m-pr. */
       RUN add-link IN adm-broker-hdl ( h_q-flg-m , 'Record':U , h_b-mov-m-pr ).
