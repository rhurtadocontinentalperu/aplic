...
END.        
IF c-CodMov = '' THEN DO:
    MESSAGE 'No se han definido movimientos de almacén' VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
...
END.
IF x-Ok = NO THEN DO:
    MESSAGE 'El usuario NO tiene permiso para ejecutar estos movimientos en el almacén'
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 1 COL 90 WIDGET-ID 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Ingresos de Almacen - SOLO CONSULTA"
         HEIGHT             = 16.35
         WIDTH              = 96
         MAX-HEIGHT         = 17.73
         MAX-WIDTH          = 99.43
...
ON END-ERROR OF W-Win /* Ingresos de Almacen - SOLO CONSULTA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Ingresos de Almacen - SOLO CONSULTA */
DO:
...
DO:
  RUN dispatch IN h_v-ingres ('imprime':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ingrso ).
       RUN set-position IN h_b-ingrso ( 8.54 , 1.00 ) NO-ERROR.
