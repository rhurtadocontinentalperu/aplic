...
DEFINE VARIABLE h_dorderline AS HANDLE NO-UNDO.
DEFINE VARIABLE h_pnavico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_pupdsav AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vorderline AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'aplic/dorderline.wDB-AWARE':U ,
             INPUT  FRAME fMain:HANDLE ,
             INPUT  'AppServiceASInfoASUsePrompt?CacheDuration0CheckCurrentChangedyesDestroyStatelessyesDisconnectAppServernoServerOperatingModeNONEShareDatanoUpdateFromSourcenoForeignFieldsObjectNamedorderlineOpenOnInityesPromptColumns(NONE)PromptOnDeleteyesRowsToBatch200RebuildOnReposnoToggleDataTargetsyes':U ,
             OUTPUT h_dorderline ).
       RUN repositionObject IN h_dorderline ( 5.04 , 59.00 ) NO-ERROR.
       /* Size in AB:  ( 1.50 , 7.72 ) */

       RUN constructObject (
             INPUT  'aplic/vorderline.w':U ,
             INPUT  FRAME fMain:HANDLE ,
             INPUT  'EnabledObjFldsToDisable?ModifyFields(All)DataSourceNamesUpdateTargetNamesLogicalObjectNameLogicalObjectNamePhysicalObjectNameDynamicObjectnoRunAttributeHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_vorderline ).
       RUN repositionObject IN h_vorderline ( 6.38 , 8.00 ) NO-ERROR.
       /* Size in AB:  ( 8.08 , 42.14 ) */
       ...
       RUN constructObject (
             ...
             OUTPUT h_pnavico ).
       RUN repositionObject IN h_pnavico ( 2.88 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_pnavico ( 1.77 , 18.00 ) NO-ERROR.

       /* Links to SmartDataObject h_dorderline. */
       RUN addLink ( h_pnavico , 'Navigation':U , h_dorderline ).

       /* Links to SmartDataViewer h_vorderline. */
       RUN addLink ( h_dorderline , 'Data':U , h_vorderline ).
       RUN addLink ( h_vorderline , 'Update':U , h_dorderline ).
       RUN addLink ( h_pupdsav , 'TableIO':U , h_vorderline ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_pnavico ,
             h_pupdsav , 'AFTER':U ).
       RUN adjustTabOrder ( h_vorderline ,
             h_pnavico , 'AFTER':U ).
    END. /* Page 0 */
