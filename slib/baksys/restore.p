...
define temp-table ttDb no-undo
    ...
    field cArchiveLastIncAimage as char
...
procedure initializeDb:
    ...
    define var cArchiveLastIncAimage    as char no-undo.
        ...
        for each ttDb:
            ...
            {slib/err_try}:
                        ...
                        if avail ttArchiveZip then do:
                            ...
                            for each  ttArchiveZip
                                ...
                                assign
                                    ...
                                    cArchiveLastIncAimage   = ttArchiveZip.cAimageFullPath.
                ...
                assign
                    ...
                    ttDb.cArchiveLastIncAimage  = cArchiveLastIncAimage.
...
procedure rollFrwdAi:
    ...
    if pbDb.cArchiveLastIncAimage = ? then
        return.
    ...
    {slib/err_try}:
        ...
        run zip_list(
            input   pbDb.cArchiveLastIncAimage,
            input   "aimage*.ai",
            output  table zip_ttFile ).

        if can-find( first zip_ttFile ) then
        ...
        for each  zip_ttFile
            where ( tRestDate = ? and iRestTime = ? )

               or ( zip_ttFile.tDate < tRestDate
                 or zip_ttFile.tDate = tRestDate and zip_ttFile.iTime <= iRestTime )

            use-index cPath:

            assign
                cFileName   = os_getSubPath( zip_ttFile.cPath, "file", "ext" )
        ...
        and ( tEndDate < tRestDate
           or tEndDate = tRestDate and iEndTime < iRestTime ) then do:
            ...
            find first zip_ttFile
                 where zip_ttFile.tDate > tRestDate
                    or zip_ttFile.tDate = tRestDate and zip_ttFile.iTime >= iRestTime
                 use-index cPath
                 no-error.
