...
&if "{&opsys}" begins "win" &then
    ...
    &global xCmdFullBackupOnline    '_mprshut "%pdbname%" -C backup online "%bakfile%" -com -red 5 -Bp 10 -verbose'
    &global xCmdIncBackupOnline     '_mprshut "%pdbname%" -C backup online incremental "%bakfile%" -com -io %ioverlap% -red 5 -Bp 10 -verbose' 
    &global xCmdFullBackupOffline   '_dbutil probkup "%pdbname%" "%bakfile%" -com -red 5 -Bp 10 -verbose'
    &global xCmdIncBackupOffline    '_dbutil probkup "%pdbname%" incremental "%bakfile%" -com -io %ioverlap% -red 5 -Bp 10 -verbose'
    ...
    {slib/err_try}:
        ...
        &if "{&opsys}" begins "win" &then
            ...
            run win_batch(
                input 'proenv ~n'
                    + 'cd /d "' + os_getSubPath( pbDb.cPDbName, "dir", "dir" ) + '" ~n'
                    + replace( replace( replace( cCmd, "%ioverlap%", "{&xIncOverlap}" ), "%pdbname%", os_getSubPath( pbDb.cPDbName, "file", "file" ) ), "%bakfile%", cBakTempFile ),
                input 'wait' ).

            if win_iErrorLevel <> 0 then
                {slib/err_throw "'error'" "'Backup of database ' + pbDb.cLDbName + ' failed.'"}.
...
procedure setBusy:
    ...
    {slib/err_try}:
        ...
        if  file-info:full-pathname <> ?
        and date_getTimeInterval( today, time, file-info:file-create-date, file-info:file-create-time ) > {&xAimageLkTimeout} then
        ...
        repeat while os_isFileExists( cAimageLkFile ):

            if not lMsg then do:

                run log_writeMessage( "stLog", "Backup waiting for aimage backup to complete." ).
                lMsg = yes.

            end. /* not lMsg */

            if etime( no ) > {&xAimageWaitTimeout} * 1000 then
        ...
        repeat while os_isFileExists( cArchiveLkFile ):
            ...
            if etime( no ) > {&xArchiveWaitTimeout} * 1000 then
