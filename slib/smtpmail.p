...
09/08/2007 Tony Benn - Change get-tz procedure to use 
                       'timezone' function for OE10                          
10/03/2007 Jan Keirse - Change getFile not to return a mempointer but work on mgetfile because it caused a memory leak                       
...
/* ALONB - 02/07/09 */
DEF VAR cB64Encode                  AS CHAR     NO-UNDO.
DEF VAR cGetHostName                AS CHAR     NO-UNDO.
DEF VAR cGetMailHub                 AS CHAR     NO-UNDO.



/* ALONB - 02/07/09 ***/
cB64Encode = pro_getRunFile( "slib/b64encode.p" ).

/***
IF cB64Encode = ? THEN
   cB64Encode = pro_getRunFile( "b64encode.p" ).

IF cB64Encode = ? THEN
   cB64Encode = pro_getRunFile( "xx/b64encode.p" ).

IF cB64Encode = ? THEN
   cB64Encode = pro_getRunFile( "us/xx/b64encode.p" ).
***/
...
cGetHostName = pro_getRunFile( "slib/gethostname.p" ).

/***
IF cGetHostName = ? THEN
   cGetHostName = pro_getRunFile( "gethostname.p" ).

IF cGetHostName = ? THEN
   cGetHostName = pro_getRunFile( "xx/gethostname.p" ).

IF cGetHostName = ? THEN
   cGetHostName = pro_getRunFile( "us/xx/gethostname.p" ).
***/
...
IF mailhub = ? THEN DO:
    ...
    cGetMailHub = pro_getRunFile( "slib/getmailhub.p" ).

    /***
    IF cGetMailHub = ? THEN
       cGetMailHub = pro_getRunFile( "getmailhub.p" ).

    IF cGetMailHub = ? THEN
       cGetMailHub = pro_getRunFile( "xx/getmailhub.p" ).

    IF cGetMailHub = ? THEN
       cGetMailHub = pro_getRunFile( "us/xx/getmailhub.p" ).
    ***/

    IF cGetMailHub <> ? THEN
    RUN VALUE( cGetMailHub ) ( OUTPUT mailhub ) NO-ERROR.
...
IF EmailFrom = ? THEN
   EmailFrom = "mfgmail@abc.com".

ASSIGN
   EmailTo = REPLACE( EmailTo, ";", "," )
   EmailCC = REPLACE( EmailCC, ";", "," ).

IF NUM-ENTRIES( EmailTo ) > 1 THEN DO:

    DO icnt = 2 TO NUM-ENTRIES( EmailTo ):

        EmailCC = EmailCC
                + ( IF EmailCC <> "" THEN "," ELSE "" )
                + ENTRY( icnt, EmailTo ).

    END. /* 2 to num-entries */

    EmailTo = ENTRY( 1, EmailTo ).
...
loglevel = 3.  /* Minimal logging = 3 Verbose loggin = 1 */
...
ELSE RUN VALUE( cGetHostName ) (OUTPUT vLocalHostName). /* ALONB - 02/07/09 */
...
/*** alonb ***/
ETIME(YES).
REPEAT WHILE sending:
  WAIT-FOR "READ-RESPONSE" OF hSocket PAUSE 1.
  IF ETIME(NO) >= 15000 THEN DO:
    sending = NO.
...
ELSE IF SESSION:BATCH-MODE THEN DO:
  ...
  DO WHILE sending:
    ...
    IF start-etime /*** alonb *** + 60000 ***/ + 15000 /*** alonb ***/ < ETIME THEN DO:
      sending = NO.
      {&LOGGER} "Connection Timed out." SKIP.
      RUN Cleanup.
    END.
        ...
        IF v = 250 THEN do:    /* loop through all to's and cc's  */
          ...
          ELSE IF NUM-ENTRIES(EmailtoTmp) = 1 THEN DO:
            ...
            vstate = IF EmailCCTmp = "" THEN /* If cc stay as state 6 */
                newState(7, "RCPT TO: " + cRcpTo + crlf,hsocket)  /* 05/19/2003 LK replace 4 to 7*/
            ELSE newState(6, "RCPT TO: " + cRcpTo + crlf,hsocket).  /* 05/19/2003 LK replace 3 to 6*/
      ...
      WHEN 8 THEN DO: /* 05/19/2003 LK replace 5 to 8*/
        ...
        IF v = 354 THEN do:
          ...
          smtpcmd = smtpcmd + "Subject: " + REPLACE(Subject, "~n", " ") + crlf.
            /* Derek Marley 02/25/03, update Carl Verbiest 06/22/06 only add headers if not default */
            if Importance = 1 or Importance = 3
            then do:
                smtpcmd = smtpcmd + SUBSTITUTE("X-Priority: &1", Importance) + crlf.
                smtpcmd = smtpcmd + "Priority: " + entry(Importance, "Urgent,Normal,Non-Urgent") + crlf.
                smtpcmd = smtpcmd + "Importance: " + entry(Importance, "high,normal,low") + crlf.
            end.
            /* Sample format    Date: 27 March 2001 10:30:00 EST */
            smtpcmd = smtpcmd + "Date: " + STRING(DAY(TODAY)) + " " +
                      entry(MONTH(TODAY),cMonth) + " " +
                      STRING(YEAR(TODAY),"9999") + " " +
                      STRING(TIME,"hh:mm:ss") + " " + timezonestring + crlf.
...
PROCEDURE dofiles:
  ...
  IF cFileType = "Binary" THEN DO:
    ...
    ELSE RUN VALUE( cB64Encode ) (cBinaryFile, cLocalFile) NO-ERROR. /* ALONB - 02/07/09 */
