...
06/07/08  alonb     * Added office-html. Mainly for working with Excel and Word
                      Office documents saved as html files.
...
&GLOBAL-DEFINE OFFICE-HTML-IS-SET CAN-DO(p_options,"office-html":U) /*** alonb */
...
PROCEDURE init-tags:
  ...
  */
  ...
  /* alonb ***/
  /*
  ** Plain non WebSpeed tag. After all office-htmls can be used without WebSpeed.
  */
  CREATE tag.
  ASSIGN tag.tag-state = {&STMNT-TAG-OPEN}
         tag.open-tag = "<!--4GL":U
  ...
  /*
  ** In Excel or Word office documents if Progress experssions are marked using `` 
  ** and not as comments some of the characters will be escaped. 
  ** For example ">>9.9<<" will be converted to &quot;&gt;&gt;9.9&lt;&lt;&quot;   
  */
  CREATE tag.
  ASSIGN tag.tag-state = {&EXPR-TAG-OPEN}
         tag.open-tag = "<!--`":U
...
IF NOT {&WEB-OBJECT-IS-SET} AND
   NOT {&INCLUDE-IS-SET} AND
   NOT {&OFFICE-HTML-IS-SET} /*** alonb */ THEN DO:
...
IF p_output-file = "" THEN DO:
  ...
  IF i > 0 THEN
    p_output-file = SUBSTRING(p_html-file,1,i) +
      (IF {&WEB-OBJECT-IS-SET} THEN "w":U ELSE ( 
       IF {&OFFICE-HTML-IS-SET} THEN "p":U /*** alonb */ ELSE "i":U)).
  ELSE
...
/* adds an encode html function that is embedded in the new procedure and is completely indepenedant. */
ELSE IF {&OFFICE-HTML-IS-SET} THEN DO:

  PUT STREAM outstr UNFORMATTED
    '~n'
    'function encodeHtml returns char ( pcStr as char ):~n'
    '~n'
    '    define var RetVal   as char no-undo.~n'
    '    define var ch       as char no-undo.~n'
    '    define var i        as int no-undo.~n'
    '~n'
    '    RetVal = "".~n'
    '~n'
    '    if pcStr = "" then ~n'
    ...
    '    else do:~n'
    '~n'
    '        do i = 1 to length( pcStr ):~n'
    '~n'
    '            ch = substr( pcStr, i, 1 ).~n'
    '~n'
    '            if ch = "&" then~n'
    '                RetVal = RetVal + "&amp;".~n'
    '~n'
    ...
    '~n'
    '            else~n'
    '            if asc( ch ) >= 224 and asc( ch ) <= 250 and session:cpinternal = "ISO8859-8" then~n' 
    '                RetVal = RetVal + "&#" + string( asc( ch ) + 1264 ) + ";".~n'
    '~n'
    '            else~n' 
    '            if asc( ch ) >= 128 and asc( ch ) <= 154  and session:cpinternal = "IBM862"  then ~n'
    '                RetVal = RetVal + "&#" + string( asc( ch ) + 1360 ) + ";".~n' 
    '~n'
    '            else~n'
    '            RetVal = RetVal + ch.~n'
    '~n'
    ...
    '~n'
    '        assign~n'
    '            pcStr = replace( pcStr, "&", "&amp~~;" )~n'
    '            pcStr = replace( pcStr, "~~"", "&quot~~;" )~n'
    '            pcStr = replace( pcStr, "<", "&lt~~;" )~n'
    '            pcStr = replace( pcStr, ">", "&gt~~;" ).~n'
    '~n'
    '    end. /* else */~n'
    '~n'
    '    return pcStr.~n'
    ...
    */
    WHEN {&INITIAL-STATE} THEN DO:
      ...
      IF line-num > 0 THEN DO:
        ...
        /* alonb ***/
        IF {&OFFICE-HTML-IS-SET} THEN DO:

          /* Links to other external files can cause problems, especially, long delays when used with browsers.
             Excel automatically adds several standard links that point to non-existing files. */

          IF out-buf MATCHES '*<link*' THEN
             out-buf = ' /* ' + out-buf + ' */'.




          /* There are cases, for example, percantages that Excel specifies the value using x:num="<some value>"
             and ignores the value between the <td> tags. */

          IF out-buf MATCHES '*x:num="*' THEN DO:

              ASSIGN
                  i = INDEX( out-buf, ' x:num="' )
                  j = INDEX( out-buf, '"', i + length( ' x:num="' ) ).

              IF  i <> 0
              AND j <> 0 THEN
                SUBSTR( out-buf, i, j - i + 1 ) = " x:num"
                    + "' /*" + SUBSTR( out-buf, i, j - i + 1 ) + " */ '".

          END. /* matches '*x:num="' */

          ELSE
          IF out-buf MATCHES '*x:str="*' THEN DO:

              ASSIGN
                  i = INDEX( out-buf, ' x:str="' )
                  j = INDEX( out-buf, '"', i + length( ' x:str="' ) ).
              ...
              ASSIGN
                  i = i + LENGTH( ' x:str=' ) - 1
                  SUBSTR( out-buf, i, j - i + 1 ) = "' /* " + SUBSTR( out-buf, i, j - i + 1 ) + " */ '".

          END. /* matches '*x:str="' */



          /* If there's no num or str type make type num as default */
          
          IF  out-buf MATCHES "*<td*<!--`*`-->*</td>*"
          AND NOT ( out-buf MATCHES "*x:num*" 
                 OR out-buf MATCHES "*x:str*" ) THEN DO:

            i = INDEX( out-buf, ">" ).

            IF i > 0 THEN SUBSTR( out-buf, i, 0 ) = " x:num".
          ...
          /* Fixes the problem of missing left border */

          IF out-buf MATCHES "*<td*style=''*border-left:none*''*" THEN DO:

              ASSIGN
                  i = INDEX( out-buf, " style=''" )
                  j = INDEX( out-buf, "''", i + length( " style=''" ) ).
              ...
              ASSIGN
                  i = i + LENGTH( " style=''" )
                  SUBSTR( out-buf, i, j - i  ) = TRIM( REPLACE( SUBSTR( out-buf, i, j - i), "border-left:none", "" ), "~~;" )

                  out-buf = REPLACE( out-buf, " style=''''", "" ).
          ...
          IF out-buf MATCHES "*<x:LeftColumnVisible*" 
          OR out-buf MATCHES "*<x:ActiveRow*"
          OR out-buf MATCHES "*<x:ActiveCol*" THEN
             out-buf = ' /* ' + out-buf + ' */'.



          /* Remove &nbsp; added by excel for blank cells that if needed will be added with encodeHtml( ) */

          IF  out-buf MATCHES "*/~*Tag=`-->~*/ '&nbsp~~~~;</td>*" THEN
              out-buf = REPLACE( out-buf, 
                    "/*Tag=`-->*/ '&nbsp~~;</td>", 
                    "/*Tag=`--> '&nbsp~~;' */ '</td>" ).



          IF out-buf MATCHES "*![if*" THEN
          RUN switchWssRmks.
    ...
    */
    WHEN {&NORMAL-STATE} THEN DO:
      ...
      ELSE IF NOT (TRIM(tmp-buf) = "" OR
              ...
              /* alonb ***/ (curr-state = {&STMNT-4GL} OR curr-state = {&EXPR-4GL}) /*** alonb */ AND TRIM(in-buf) = html-close-tag)
      THEN
        ASSIGN
          out-buf = out-buf +
                (IF out-buf = "" OR prev-state = {&STMNT-4GL}
                 THEN ( IF {&OFFICE-HTML-IS-SET} THEN "PUT UNFORMATTED ":U ELSE "~{&OUT~} ":U ) 
                 ELSE "") +
    ...
    */
    WHEN {&EXPR-TAG-OPEN} THEN DO:
      ...
      IF out-buf = "" OR prev-state = {&STMNT-4GL} THEN
        ASSIGN out-buf = out-buf + ( IF {&OFFICE-HTML-IS-SET} THEN "PUT UNFORMATTED ":U ELSE "~{&OUT~} ":U ).
      /* Only output remainder of input line preceding the tag if it's
         non blank. */
      IF TRIM(tmp-buf) <> "" THEN
...
/* alonb ***/
PROCEDURE switchWssRmks:

    DEFINE VAR rmks AS CHAR NO-UNDO.
    DEFINE VAR wss  AS CHAR NO-UNDO.
    ...
    INPUT FROM VALUE( html-file ) NO-ECHO.
    SEEK INPUT TO SEEK( instr ).

    _main:

    REPEAT ON ENDKEY UNDO, LEAVE _main:

        IMPORT UNFORMATTED str.

        IF str MATCHES "*<![endif]>*" THEN
             rmks = rmks  + ( IF rmks <> "" THEN "~n" ELSE "" ) + "PUT UNFORMATTED '" + REPLACE( SUBSTR( str, 1, INDEX( str, "<![endif]>" ) + LENGTH( "<![endif]>" ) - 1 ), "'", "''" ) + "~~n'.".
        ELSE rmks = rmks  + ( IF rmks <> "" THEN "~n" ELSE "" ) + "PUT UNFORMATTED '" + REPLACE( str, "'", "''" ) + "~~n'.".

        IF str MATCHES "*<![endif]>*" THEN DO:

            IF NOT str MATCHES "*<!--wss*" THEN
            REPEAT ON ENDKEY UNDO, LEAVE _main:
            ...
            IF str MATCHES "*<!--wss*" THEN DO:
                wss = "/*Tag=" + SUBSTR( str, INDEX( str, "<!--wss" ) ) + "*/".
                ...
                REPEAT ON ENDKEY UNDO, LEAVE _main:
                    ...
                    IF str MATCHES "*-->*" THEN
                         wss = wss  + ( IF wss <> "" THEN "~n" ELSE "" ) + REPLACE( SUBSTR( str, 1, INDEX( str, "-->" ) + LENGTH( "-->" ) - 1 ), "-->", "/*Tag=-->*/" ).
                    ELSE wss = wss  + ( IF wss <> "" THEN "~n" ELSE "" ) + str.

                    IF str MATCHES "*-->*" THEN DO:

                        out-buf = wss + "~n" + rmks.
                        SEEK STREAM instr TO SEEK( INPUT ).
