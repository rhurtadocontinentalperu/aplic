...
FacCPedi.CodCli FacCPedi.NomCli FacCPedi.ImpTot FacCPedi.Libre_d01 ~
FacCPedi.Libre_d02 _ImpBD() @ x-ImpBD 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.ImpTot FORMAT "->>,>>>,>>9.99":U COLUMN-FGCOLOR 15 COLUMN-BGCOLOR 2
      FacCPedi.Libre_d01 COLUMN-LABEL "Proyectado" FORMAT "->>>,>>>,>>9.99<<<":U
            COLUMN-FGCOLOR 9 COLUMN-BGCOLOR 11
      FacCPedi.Libre_d02 COLUMN-LABEL "Negociado" FORMAT "->>>,>>>,>>9.99<<<":U
            COLUMN-FGCOLOR 15 COLUMN-BGCOLOR 12
      _ImpBD() @ x-ImpBD COLUMN-LABEL "Saldo de Depósitos"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     ...
     f-Mensaje AT ROW 8 COL 2 NO-LABEL WIDGET-ID 4
...
"FacCPedi.ImpTot" ? ? "decimal" 2 15 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.FacCPedi.Libre_d01
"FacCPedi.Libre_d01" "Proyectado" ? "decimal" 11 9 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.FacCPedi.Libre_d02
"FacCPedi.Libre_d02" "Negociado" ? "decimal" 12 15 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > "_<CALC>"
"_ImpBD() @ x-ImpBD" "Saldo de Depósitos" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
  ...
  FOR EACH BDPEDI OF BCPEDI WHERE BDPEDI.Libre_d02 > 0,
      ...
      BUFFER-COPY BDPEDI 
          ...
          ASSIGN
            ...
            FacDpedi.CanAte = FacDPedi.CanPed.      /* CONTROL */
      ASSIGN 
...
DO TRANSACTION ON ERROR UNDO, RETURN ERROR ON STOP UNDO, RETURN ERROR:
    ...
    FOR EACH Faccpedi WHERE {&condicion}, EACH facdpedi OF faccpedi:
        ASSIGN
            FacDPedi.canate = 0     
            FacDPedi.Libre_d01 = 0  /* Proyectado */
            FacDPedi.Libre_d02 = 0  /* Negociado */
            FacCPedi.Libre_d01 = 0  /* Proyectado */
            FacCPedi.Libre_d02 = 0. /* Negociado */
    END.
    /* cargamos ventas de acuerdo al saldo del almacén */
    FOR EACH Faccpedi WHERE {&condicion}:
        FOR EACH Facdpedi OF Faccpedi, FIRST Almmmate NO-LOCK WHERE Almmmate.codcia = Facdpedi.codcia
            AND Almmmate.codalm = Facdpedi.almdes
            AND Almmmate.codmat = Facdpedi.codmat:
            f-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'PROCESANDO: ' + Faccpedi.nroped + ' ' + Facdpedi.codmat.
            x-StkAct = Almmmate.stkact.
            RUN gn/stock-comprometido (Facdpedi.codmat, Facdpedi.almdes, OUTPUT x-StkComprometido).
            FOR EACH BCPEDI NO-LOCK WHERE BCPEDI.codcia = s-codcia
                AND BCPEDI.coddiv = s-coddiv
                AND BCPEDI.coddoc = s-coddoc
                AND BCPEDI.flgest = 'P'
                AND ROWID(BCPEDI) <> ROWID(Faccpedi),
                EACH BDPEDI OF BCPEDI NO-LOCK WHERE BDPEDI.codmat = Facdpedi.codmat:
                x-StkComprometido = x-StkComprometido + BDPEDI.CanAte.
            END.
            IF x-StkComprometido > x-StkAct THEN NEXT.
            ASSIGN
                Facdpedi.Libre_d01 = MINIMUM( (x-StkAct - x-StkComprometido), Facdpedi.CanPed )
                Facdpedi.Libre_d02 = Facdpedi.Libre_d01.
        END.
        /* Totales */
        FOR EACH FacDPedi OF FacCPedi NO-LOCK: 
            ASSIGN 
              x-ImpDto = ROUND( Facdpedi.PreUni * Facdpedi.Libre_d01 * (Facdpedi.Por_Dsctos[1] / 100),4 )
              x-ImpLin = ROUND( Facdpedi.PreUni * Facdpedi.Libre_d01 , 2 ) - x-ImpDto.
            FacCPedi.Libre_d01 = FacCPedi.Libre_d01 + x-ImpLin.
        END.
        IF FacCPedi.PorDto > 0 THEN DO:
          ASSIGN
              FacCPedi.Libre_d01 = ROUND(FacCPedi.Libre_d01 * (1 - FacCPedi.PorDto / 100),2).
        END.
        ASSIGN
            FacCPedi.Libre_d02 = FacCPedi.Libre_d01.
    END.
    RELEASE Faccpedi.
    RELEASE Facdpedi.
END.
...
DEF VAR pSdoMn1 AS DEC NO-UNDO.
DEF VAR pSdoMn2 AS DEC NO-UNDO.
