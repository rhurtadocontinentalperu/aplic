...
DEF VAR x-CanCot LIKE Facdpedi.canped NO-UNDO.
...
Almmmatg.DesMat Almmmatg.DesMar FacDPedi.UndVta _CanCot() @ x-CanCot ~
FacDPedi.CanPed FacDPedi.Libre_d02 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table FacDPedi.Libre_d02 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD _CanCot B-table-Win 
FUNCTION _CanCot RETURNS DECIMAL
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      _CanCot() @ x-CanCot COLUMN-LABEL "Cantidad Cotizada" FORMAT ">,>>>,>>9.9999":U
      FacDPedi.CanPed COLUMN-LABEL "Cantidad Aprobada" FORMAT ">,>>>,>>9.9999":U
      FacDPedi.Libre_d02 COLUMN-LABEL "Cantidad Negociada" FORMAT "->>>,>>>,>>9.99<<<":U
  ENABLE
      FacDPedi.Libre_d02
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 130 BY 6.69
         FONT 4.
...
"_CanCot() @ x-CanCot" "Cantidad Cotizada" ">,>>>,>>9.9999" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.FacDPedi.CanPed
"FacDPedi.CanPed" "Cantidad Aprobada" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > INTEGRAL.FacDPedi.Libre_d02
"FacDPedi.Libre_d02" "Cantidad Negociada" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON 'return':U OF FacDPedi.Libre_d02
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      x-ImpLin = Facdpedi.Libre_d02 * Facdpedi.PreUni * 
  ...
  IF Facdpedi.Por_Dsctos[1] = 0 AND Facdpedi.Por_Dsctos[2] = 0 AND Facdpedi.Por_Dsctos[3] = 0 
      THEN x-ImpDto = 0.
  ELSE x-ImpDto = Facdpedi.Libre_d02 * Facdpedi.PreUni - x-ImpLin.
  ASSIGN
      x-ImpLin = ROUND(x-ImpLin, 2)
      x-ImpDto = ROUND(x-ImpDto, 2).
  FacCPedi.Libre_d02 = FacCPedi.Libre_d02 - x-ImpLin.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      IF NOT AVAILABLE Faccpedi THEN UNDO, RETURN 'ADM-ERROR'.
      ASSIGN 
        x-ImpDto = ROUND( Facdpedi.PreUni * Facdpedi.Libre_d02 * (Facdpedi.Por_Dsctos[1] / 100),4 )
        x-ImpLin = ROUND( Facdpedi.PreUni * Facdpedi.Libre_d02 , 2 ) - x-ImpDto.
      IF FacCPedi.PorDto > 0 THEN DO:
        ASSIGN
            x-ImpLin = ROUND(x-ImpLin * (1 - FacCPedi.PorDto / 100),2).
      END.
      FacCPedi.Libre_d02 = FacCPedi.Libre_d02 - x-ImpLin.
      /* actualizamos la COTIZACION */
      FIND BDPEDI WHERE BDPEDI.codcia = Facdpedi.codcia
          AND BDPEDI.coddoc = 'COT'
          AND BDPEDI.nroped = Faccpedi.nroref
          AND BDPEDI.codmat = Facdpedi.codmat
          EXCLUSIVE-LOCK NO-ERROR.
      IF NOT AVAILABLE BDPEDI THEN UNDO, RETURN 'ADM-ERROR'.
      ASSIGN
          BDPEDI.CanAte = BDPEDI.CanAte - Facdpedi.CanPed.
      RELEASE BDPEDI.
...
IF DEC (FacDPedi.Libre_d02:SCREEN-VALUE IN BROWSE {&browse-name}) > FacDPedi.CanPed THEN DO:
    MESSAGE 'La cantidad NO puede ser mayor que la proyectada' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO FacDPedi.Libre_d02.
    RETURN 'ADM-ERROR'.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION _CanCot B-table-Win 
FUNCTION _CanCot RETURNS DECIMAL
...
FIND B-DPEDI WHERE B-DPEDI.codcia = Faccpedi.codcia
    ...
    NO-LOCK NO-ERROR.
IF AVAILABLE B-DPEDI THEN RETURN B-DPEDI.CanPed.
