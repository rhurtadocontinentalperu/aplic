...
ON CHOOSE OF BUTTON-13 IN FRAME F-Main /* Chequear Pedido */
DO:
...
DO:
  ...
  IF NOT AVAILABLE Faccpedi THEN DO:
    ...
    RETURN NO-APPLY.
  END.
  IF Faccpedi.flgest <> 'P' OR Faccpedi.flgsit <> 'X' THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Facdpedi NO-LOCK WHERE Facdpedi.codcia = s-codcia
    ...
    IF NOT AVAILABLE C-Detalle THEN DO:
        MESSAGE 'No se ha registrado el artículo' Facdpedi.codmat
            VIEW-AS ALERT-BOX ERROR.
        RETURN 'ADM-ERROR'.
    END.
    IF C-Detalle.CanPed <> Facdpedi.CanPed THEN DO:
        MESSAGE 'Diferencia de cantidades en el artículo' Facdpedi.codmat
            VIEW-AS ALERT-BOX ERROR.
        RETURN 'ADM-ERROR'.
    END.
  END.        
