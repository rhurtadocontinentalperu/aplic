...
DEF TEMP-TABLE detalle
    ...
    FIELD cansav LIKE facdpedi.canped
    FIELD impped LIKE facdpedi.implin
    FIELD impsav LIKE facdpedi.implin
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "REPORTE DE PROYECCION DE DESPACHOS POR SEMANAS (40 y 40b)"
         HEIGHT             = 15.58
         WIDTH              = 73.72
         MAX-HEIGHT         = 32.23
         MAX-WIDTH          = 205.72
...
ON END-ERROR OF W-Win /* REPORTE DE PROYECCION DE DESPACHOS POR SEMANAS (40 y 40b) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* REPORTE DE PROYECCION DE DESPACHOS POR SEMANAS (40 y 40b) */
DO:
...
DEF VAR x-CanSav AS DEC NO-UNDO.
DEF VAR x-ImpPed AS DEC NO-UNDO.
DEF VAR x-ImpSav AS DEC NO-UNDO.
...
FOR EACH faccpedi NO-LOCK WHERE faccpedi.codcia = s-codcia
    ...
    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "** ACUMULANDO SALDOS " +
        STRING(faccpedi.fchped) + ' ' + faccpedi.nroped.
    ...
    ASSIGN
        ...
        x-ImpPed = 0
        x-ImpSav = 0
        x-PreUni = (facdpedi.implin / facdpedi.canped).
    IF LOOKUP(Almmmatg.codfam, '010,011,012,013') > 0 THEN DO:
        IF Faccpedi.fchped < 01/01/2011 OR Faccpedi.UsrSac = "*" THEN DO:
            ASSIGN
                x-CanSav = (facdpedi.canped - facdpedi.canate) * facdpedi.factor
                x-ImpSav = (facdpedi.canped - facdpedi.canate) * x-PreUni.
        END.
        ELSE DO:
            FIND FIRST t-Excdtabla WHERE t-Excdtabla.CodCia = s-codcia
                ...
                NO-LOCK NO-ERROR.
            IF NOT AVAILABLE t-Excdtabla 
            THEN ASSIGN
                    x-CanSav = (facdpedi.canped - facdpedi.canate) * facdpedi.factor
                    x-ImpSav = (facdpedi.canped - facdpedi.canate) * x-PreUni.
            ELSE ASSIGN
                    x-CanPed = (facdpedi.canped - facdpedi.canate) * facdpedi.factor
                    x-ImpPed = (facdpedi.canped - facdpedi.canate) * x-PreUni.
        END.
    END.
    ELSE ASSIGN
            x-CanPed = (facdpedi.canped - facdpedi.canate) * facdpedi.factor
    ...
    END.
    ASSIGN
        detalle.canped = detalle.canped + x-CanPed
        detalle.cansav = detalle.cansav + x-CanSav
        detalle.impped = detalle.impped + x-impped
        detalle.impsav = detalle.impsav + x-impsav.
END.
...
DEF VAR x-CanSav AS DEC FORMAT '>>>,>>>,>>9.99' NO-UNDO.
DEF VAR x-ImpPed AS DEC FORMAT '>>>,>>>,>>9.99' NO-UNDO.
DEF VAR x-ImpSav AS DEC FORMAT '>>>,>>>,>>9.99' NO-UNDO.
DEF VAR x-CodAlm AS CHAR INIT '40,35' NO-UNDO.
DEF VAR x-TitAlm AS CHAR NO-UNDO.
...
FOR EACH EvtSemanas NO-LOCK WHERE EvtSemanas.codcia = s-codcia
    ...
    x-Semana = 'HUANTA Del ' + STRING(EvtSemanas.FecIni, '99/99/9999') +
        ' al ' + STRING(EvtSemanas.FecFin, '99/99/9999').
    x-Titulo = x-Titulo + TRIM(x-Semana) + '|Valorizado S/|'.
END.
FOR EACH EvtSemanas NO-LOCK WHERE EvtSemanas.codcia = s-codcia
    AND Evtsemanas.periodo >= x-Periodo-1
    AND Evtsemanas.periodo <= x-Periodo-2:
    ...
    IF Evtsemanas.Periodo = x-Periodo-2
        AND EvtSemanas.NroSem > x-Semana-2 THEN NEXT.
    x-Semana = 'SAVAR Del ' + STRING(EvtSemanas.FecIni, '99/99/9999') +
        ' al ' + STRING(EvtSemanas.FecFin, '99/99/9999').
    x-Titulo = x-Titulo + TRIM(x-Semana) + '|Valorizado S/|'.
END.
...
DO j = 1 TO NUM-ENTRIES(x-CodAlm):
    x-TitAlm = ENTRY(j, x-CodAlm).
    IF x-TitAlm = '40' THEN x-TitAlm = '40 y 40b'.
    x-Titulo = x-Titulo + 'Almacen ' + x-TitAlm  + '|'.
END.
x-Titulo = x-Titulo + 'Requerido|Transferir|O/C en curso|Para comprar|Empaque|'.
...
FOR EACH detalle NO-LOCK USE-INDEX Llave01,
    ...
    IF FIRST-OF(detalle.codmat) THEN DO:
        ...
        FIND b-detalle WHERE b-detalle.codcia = s-codcia
            ...
            AND b-detalle.periodo = x-Periodo-1
            AND b-detalle.nrosem = x-Semana-1
            NO-LOCK NO-ERROR.
        IF AVAILABLE b-detalle THEN DO:
            x-Llave = x-LLave + STRING(b-detalle.canped, '>>>,>>>,>>9.99') + '|' +
                STRING(b-detalle.impped, '>>>,>>>,>>9.99') + '|' +
                STRING(B-detalle.cansav, '>>>,>>>,>>9.99') + '|' +
                STRING(b-detalle.impsav, '>>>,>>>,>>9.99') + '|'.
            ASSIGN
                x-CanPed = x-CanPed + b-detalle.canped
                x-CanSav = x-CanSav + b-detalle.cansav.
        END.
        ELSE DO:
            x-Llave = x-LLave + ' ' + '|'.
        END.
    ...
    IF LAST-OF(detalle.codmat) THEN DO:
        ...
        DO j = 1 TO NUM-ENTRIES(x-CodAlm):
            ...
            THEN x-StkAct = Almmmate.StkAct.
            ELSE x-StkAct = 0.
            IF ENTRY(j, x-codalm) = '40' THEN DO:
                FIND Almmmate WHERE Almmmate.codcia = s-codcia
                    AND Almmmate.codalm = '40b'
                    AND Almmmate.codmat = detalle.codmat
                    NO-LOCK NO-ERROR.
                IF AVAILABLE Almmmate THEN x-StkAct = x-StkAct + Almmmate.stkact.
            END.
            IF x-StkAct <> 0
            THEN x-Llave = x-LLave + STRING(x-StkAct, '->>>,>>>,>>9.99') + '|'.
        ...
        x-Comprar = 0.
        x-Transito = 0.
        x-Stk40 = x-Stk40 - x-CanSav.   /* Saldo del almacen de Savar */
