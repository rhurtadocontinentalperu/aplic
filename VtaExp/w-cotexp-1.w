...
DEFINE VARIABLE h_b-cotexp-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-pedido AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-cotexp-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotexp-1 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BTN-Excel AT ROW 8.69 COL 92
     BUTTON-2 AT ROW 8.88 COL 97
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 112.72 BY 20.35
         FONT 4.
...
DO:
  RUN Genera-Excel IN h_v-cotexp-1.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtaexp/v-cotexp-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotexp-1 ).
       RUN set-position IN h_v-cotexp-1 ( 1.04 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 7.69 , 112.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv01 ).
       RUN set-position IN h_p-updv01 ( 8.69 , 29.00 ) NO-ERROR.
       RUN set-size IN h_p-updv01 ( 1.42 , 63.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtaexp/q-pedido.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-pedido ).
       ...
       /* Links to SmartViewer h_v-cotexp-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_v-cotexp-1 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-cotexp-1 ).

       /* Links to SmartQuery h_q-pedido. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotexp-1 ,
             BTN-Excel:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-cotexp-1 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtaexp/b-cotexp-1.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotexp-1 ).
       RUN set-position IN h_b-cotexp-1 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotexp-1 ( 10.35 , 105.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotexp-1. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_b-cotexp-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotexp-1 ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtaexp/t-cotexp-1.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotexp-1 ).
       RUN set-position IN h_t-cotexp-1 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotexp-1 ( 8.88 , 110.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotexp-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotexp-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotexp-1 ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotexp-1 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-cotexp-1 <> ? THEN RUN dispatch IN h_t-cotexp-1 ('open-query':U).
      END.
...
/*         B-CCTE:SENSITIVE = YES.*/
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        RUN Recalcular-Precios IN h_t-cotexp-1.
      END.
