...
/* variables de almacen de despacho */
...
/*     c-TpoVta:SCREEN-VALUE = ENTRY(INTEGER(COTIZACION.TipVta), c-TpoVta:LIST-ITEMS). */
    ...
    FOR EACH FacDPedi OF COTIZACION WHERE (FacDPedi.CanPed - FacDPedi.CanAte) > 0 NO-LOCK:
        ...
        FIND Almmmate WHERE Almmmate.codcia = s-codcia
            AND Almmmate.codalm = y-CodAlm      /* FacDPedi.AlmDes*/
            AND Almmmate.codmat = Facdpedi.CodMat
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE Almmmate THEN DO:
            MESSAGE 'Material' Facdpedi.codmat 'NO asignado al almacén' y-CodAlm    /*FacDPedi.AlmDes*/
...
END CASE.
    ...
    ped = FacCPedi.NroPed. 
    IF FacCPedi.CodAlm <> '' THEN cCodAlm = FacCPedi.CodAlm.
    ELSE cCodAlm = s-codalm.
...
DEFINE FRAME f-cab
        ...
        "---------------------------------------------------------------------------------------------------------------------------------------" SKIP
        "Itm Código  Descripción                                                    Marca                  Unidad      Cantidad Ubicación            " SKIP
        "---------------------------------------------------------------------------------------------------------------------------------------" SKIP
...
/*FOR EACH Reporte BREAK BY Reporte.NroPed BY Reporte.CodUbi:*/
FOR FIRST Reporte BREAK BY Reporte.NroPed BY Reporte.CodUbi:    
         VIEW STREAM Report FRAME F-Cab.
         VIEW STREAM Report FRAME F-FtrPed. 
         IF LAST-OF(Reporte.NroPed) THEN DO:
                PAGE STREAM Report.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
    ...
    input-var-4 = ''
    output-var-1 = ?.
  RUN vtamay/c-cotpen-2-2 ('Cotizaciones Pendientes').
  IF OUTPUT-VAR-1 = ? THEN RETURN 'ADM-ERROR'.
  /* ALMACENES DE DESPACHO */
  RUN vtaexp/d-almdes (s-coddiv, OUTPUT y-CodAlm).
  IF y-CodAlm = '' THEN RETURN "ADM-ERROR".
  /* ********************* */
...
------------------------------------------------------------------------------*/
  ...
  IF FacCPedi.FlgEst = "P" THEN DO:
       MESSAGE "No puede eliminar un pedido aprobado" SKIP
...
/*     END.                                                                                                */
    ...
    /* RHC 22.12.09 Control de pedidos en trámite */
    IF s-adm-new-record = 'YES' THEN DO:
        ...
        IF NOT AVAILABLE COTIZACION THEN DO:
            MESSAGE 'ERROR GRAVE: No se ha podido ubicar la cotización' Faccpedi.nroref:SCREEN-VALUE
                VIEW-AS ALERT-BOX ERROR.
            RETURN 'ADM-ERROR'.
        END.
        IF COTIZACION.FlgEst = 'T' THEN DO:
            MESSAGE 'En estos momentos se está procesando un PRE-PEDIDO' SKIP
