...
&Scoped-Define DISPLAYED-OBJECTS x-nomcli x-monlc x-importe 
...
DEFINE VARIABLE h_b-cabcatvta00 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-detcatvta_01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-lisprov_02 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     x-monlc AT ROW 1.38 COL 104 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     x-importe AT ROW 1.38 COL 113 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     "Cliente:" VIEW-AS TEXT
          SIZE 7 BY .62 AT ROW 1.54 COL 3.57 WIDGET-ID 8
          BGCOLOR 8 
     "Línea Crédito:" VIEW-AS TEXT
          SIZE 12 BY .62 AT ROW 1.54 COL 91.43 WIDGET-ID 10
          BGCOLOR 8 
     RECT-70 AT ROW 1.08 COL 2 WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 148 BY 26.04 WIDGET-ID 100.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/b-lisprov_02.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-lisprov_02 ).
       RUN set-position IN h_b-lisprov_02 ( 2.88 , 2.29 ) NO-ERROR.
       RUN set-size IN h_b-lisprov_02 ( 4.58 , 84.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtaexp/b-cabcatvta00.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cabcatvta00 ).
       RUN set-position IN h_b-cabcatvta00 ( 7.46 , 2.72 ) NO-ERROR.
       RUN set-size IN h_b-cabcatvta00 ( 5.00 , 65.72 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/b-detcatvta_01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-detcatvta_01 ).
       RUN set-position IN h_b-detcatvta_01 ( 12.65 , 1.43 ) NO-ERROR.
       RUN set-size IN h_b-detcatvta_01 ( 14.00 , 144.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cabcatvta00. */
       RUN add-link IN adm-broker-hdl ( h_b-lisprov_02 , 'Record':U , h_b-cabcatvta00 ).

       /* Links to SmartBrowser h_b-detcatvta_01. */
       RUN add-link IN adm-broker-hdl ( h_b-cabcatvta00 , 'Record':U , h_b-detcatvta_01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-lisprov_02 ,
             x-importe:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cabcatvta00 ,
             h_b-lisprov_02 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-detcatvta_01 ,
             h_b-cabcatvta00 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  DISPLAY x-nomcli x-monlc x-importe 
...
------------------------------------------------------------------------------*/
    ...
    CASE pParameter:
        ...
        WHEN 'Open-Query-3' THEN DO:
            RUN dispatch IN h_b-detcatvta_01 ('open-query').
        END.
    END CASE.
END PROCEDURE.
