...
DEF TEMP-TABLE detalle LIKE almmmatg
    FIELD CanSavar AS DEC
    FIELD ImpSavar AS DEC
    FIELD CanHuanta AS DEC
    FIELD ImpHuanta AS DEC.
...
DEF VAR x-CanHuanta AS DEC NO-UNDO.
DEF VAR x-CanSavar AS DEC NO-UNDO.
DEF VAR x-ImpHuanta AS DEC NO-UNDO.
DEF VAR x-ImpSavar AS DEC NO-UNDO.
...
DO j = 1 TO NUM-ENTRIES(f-Division):
    ...
    FOR EACH faccpedi NO-LOCK WHERE faccpedi.codcia = s-codcia
        ...
        IF FIRST-OF(faccpedi.nroref) THEN DO:
            ...
            FOR EACH B-DPEDI OF B-CPEDI NO-LOCK WHERE B-DPEDI.CanPed > B-DPEDI.CanAte, FIRST Almmmatg OF B-DPEDI NO-LOCK:
                ...
                IF LOOKUP(Almmmatg.codfam, '010,011,012,013') > 0 THEN DO:
                    IF B-CPEDI.fchped < 01/01/2011 OR B-CPEDI.UsrSac = "*" THEN DO:
                        ASSIGN
                            x-CanSavar = (B-DPEDI.canped - B-DPEDI.canate) * B-DPEDI.factor
                            x-ImpSavar = (B-DPEDI.canped - B-DPEDI.canate) * x-PreUni.
                        END.
                    ELSE DO:
                        FIND FIRST Excdtabla WHERE Excdtabla.CodCia = s-codcia
                            AND Excdtabla.Tabla = B-CPEDI.coddiv
                            AND Excdtabla.Codigo = 'HUANTA'
                            AND Excdtabla.Llaves = B-CPEDI.codcli
                            NO-LOCK NO-ERROR.
                        IF NOT AVAILABLE Excdtabla 
                            THEN ASSIGN
                                    x-CanSavar = (B-DPEDI.canped - B-DPEDI.canate) * B-DPEDI.factor
                                    x-ImpSavar = (B-DPEDI.canped - B-DPEDI.canate) * x-PreUni.
                        ELSE ASSIGN
                                x-CanHuanta = (B-DPEDI.canped - B-DPEDI.canate) * B-DPEDI.factor
                                x-ImpHuanta = (B-DPEDI.canped - B-DPEDI.canate) * x-PreUni.
                    END.
                END.
                ELSE ASSIGN
                        x-CanHuanta = (B-DPEDI.canped - B-DPEDI.canate) * B-DPEDI.factor
                        x-ImpHuanta = (B-DPEDI.canped - B-DPEDI.canate) * x-PreUni.
                ASSIGN
                    detalle.CanHuanta = detalle.CanHuanta + x-CanHuanta
                    detalle.ImpHuanta = detalle.ImpHuanta + x-ImpHuanta
                    detalle.CanSavar = detalle.CanSavar + x-CanSavar
                    detalle.ImpSavar = detalle.ImpSavar + x-ImpSavar.
            END.
        END.
    END.
END.
...
x-Titulo = 'Producto|Marca|Linea|Sublinea|Unidad|Proveedor|Saldo Huanta|Importe saldo Huanta S/|' +
    'Saldo Savar|Importe saldo Savar S/|Stock Huanta|Stock Savar|'.
...
FOR EACH detalle NO-LOCK,
    ...
    x-Llave = x-Llave + STRING(detalle.CanHuanta, '->>>,>>>,>>9.99') + '|' +
        STRING(detalle.ImpHuanta, '->>>,>>>,>>9.99') + '|'.
    x-Llave = x-Llave + STRING(detalle.CanSavar, '->>>,>>>,>>9.99') + '|' +
        STRING(detalle.ImpSavar, '->>>,>>>,>>9.99') + '|'.
    FIND Almmmate WHERE Almmmate.codcia = s-codcia
        AND Almmmate.codmat = detalle.codmat
        AND Almmmate.codalm = '35'
        NO-LOCK NO-ERROR.
