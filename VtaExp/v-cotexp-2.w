...
"US$", 2
     ...
     BUTTON-5 AT ROW 6.65 COL 101 WIDGET-ID 2
...
DO:
    ...
    ASSIGN
        ...
        output-var-1 = ?.
    RUN vtaexp/c-pages ('Cotizaciones Pendientes').
    IF OUTPUT-VAR-1 = ? THEN RETURN 'ADM-ERROR'.
    
    FIND AlmCatVtaC WHERE ROWID(AlmCatVtaC) = OUTPUT-VAR-1 NO-LOCK NO-ERROR.
    IF NOT AVAILABLE AlmCatVtaC THEN DO:
        MESSAGE 'No pudo ubicar la Cotizacion' VIEW-AS ALERT-BOX ERROR.
        RETURN 'ADM-ERROR'.
    END.
    MESSAGE almcatvtac.nropag.    
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH almcatvtad WHERE almcatvtad.codcia = almcatvtac.codcia
        ...
        CREATE PEDI.
        ASSIGN
            PEDI.Codcia = AlmCatVtaD.CodCia 
            PEDI.NroItm = AlmCatVtaD.NroSec
            PEDI.CodDiv = AlmCatVtaD.CodDiv 
            PEDI.codmat = AlmCatVtaD.codmat. 
    END.
