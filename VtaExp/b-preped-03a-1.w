...
DO TRANSACTION ON ERROR UNDO, RETURN ERROR ON STOP UNDO, RETURN ERROR:
    ...
    FOR EACH B-CPEDI:
        ...
        FOR EACH B-DPEDI NO-LOCK WHERE B-DPEDI.codcia = B-CPEDI.codcia
                ...
                AND B-DPEDI.nroped = B-CPEDI.nroped:
            ASSIGN
                x-ImpLin = B-DPEDI.Libre_d02 * B-DPEDI.PreUni * 
                              ( 1 - B-DPEDI.Por_Dsctos[1] / 100 ) *
                              ( 1 - B-DPEDI.Por_Dsctos[2] / 100 ) *
                              ( 1 - B-DPEDI.Por_Dsctos[3] / 100 ).
            IF B-DPEDI.Por_Dsctos[1] = 0 AND B-DPEDI.Por_Dsctos[2] = 0 AND B-DPEDI.Por_Dsctos[3] = 0 
                THEN x-ImpDto = 0.
                ELSE x-ImpDto = B-DPEDI.Libre_d02 * B-DPEDI.PreUni - x-ImpLin.
            ASSIGN
