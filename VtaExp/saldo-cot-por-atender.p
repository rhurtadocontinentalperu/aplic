...
FOR EACH faccpedi NO-LOCK WHERE codcia = 1
    ...
    detalle.implin = detalle.implin + (facdpedi.canped - facdpedi.canate) * (facdpedi.implin / facdpedi.canped).
...
FOR EACH faccpedi NO-LOCK WHERE codcia = 1
    ...
    IF NOT AVAILABLE detalle THEN DO:
        ...
        BUFFER-COPY almmmatg TO detalle.
    END.
    detalle.canate = detalle.canate + (facdpedi.canped - facdpedi.canate).
    detalle.implin2 = detalle.implin2 + (facdpedi.canped - facdpedi.canate) * (facdpedi.implin / facdpedi.canped).
    FIND almmmate WHERE almmmate.codcia = facdpedi.codcia
        AND almmmate.codalm = '15'
        AND almmmate.codmat = facdpedi.codmat
        NO-LOCK NO-ERROR.
    IF AVAILABLE almmmate THEN detalle.stkact = almmmate.stkact.
END.
OUTPUT TO m:\tmp\por-atender.txt.
FOR EACH detalle NO-LOCK:
    ...
    DISPLAY 
        ...
        detalle.canped  COLUMN-LABEL 'Saldo!Cotizaciones'
        detalle.implin  COLUMN-LABEL 'Saldo!Cotizaciones!Valorizado'
        detalle.canate  COLUMN-LABEL 'Saldo!Pedidos'
        detalle.implin2 COLUMN-LABEL 'Saldo!Pedidos!Valorizado'
