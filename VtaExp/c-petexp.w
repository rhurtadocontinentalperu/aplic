...
&SCOPED-DEFINE l-lookup h_l-petexp
...
DEFINE VARIABLE h_l-petexp AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.58 COL 70.43
     Btn_Cancel AT ROW 8.77 COL 70.43
     Btn_Help AT ROW 10.58 COL 70.43
     SPACE(3.85) SKIP(1.18)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/l-petexp.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-petexp ).
       RUN set-position IN h_l-petexp ( 1.19 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-petexp ( 10.92 , 67.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.38 , 68.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-petexp. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-petexp ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-petexp ,
