...
DEFINE VARIABLE h_v-pedido-2-1 AS HANDLE NO-UNDO.
...
DO:
   RUN Asigna_datos IN h_v-pedido-2-1.
END.
...
DO:
    ...
    RUN Envia_chequeo IN h_v-pedido-2-1.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/v-pedido-2-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedido-2-1 ).
       RUN set-position IN h_v-pedido-2-1 ( 1.00 , 1.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-pedido-2-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-pedido-2-1 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-pedido-2-1 ).

       /* Links to SmartQuery h_q-pedido. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedido-2-1 ,
             B-CCTE:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-pedido-2-1 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-cotiza-2-1 ( 10.23 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_t-pedido-2-1 ( 10.23 , 2.00 ) NO-ERROR.
