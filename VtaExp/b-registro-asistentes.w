...
DEFINE VAR x-col-ubigeo AS CHAR.
...
ExpAsist.NomCli gn-clie.Ruc ubigeo(ExpAsist.CodCli) @ x-col-ubigeo ~
ExpAsist.FecPro ExpAsist.HoraPro ExpAsist.FecAsi[1] ExpAsist.HoraAsi[1] ~
ExpAsist.FecAsi[2] ExpAsist.HoraAsi[2] ExpAsist.FecAsi[3] ~
ExpAsist.HoraAsi[3] ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD ubigeo B-table-Win 
FUNCTION ubigeo RETURNS CHARACTER
  ( pCodClie AS CHAR )  FORWARD.
...
DEFINE MENU POPUP-MENU-br_table 
       MENU-ITEM m_Reimprime_Etiqueta LABEL "Reimprime Etiqueta"
       MENU-ITEM m_Registra_Asistencia LABEL "Registra Asistencia"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      ubigeo(ExpAsist.CodCli) @ x-col-ubigeo COLUMN-LABEL "Ubigeo" FORMAT "x(50)":U
            WIDTH 22.29
...
DEFINE FRAME F-Main
     ...
     br_table AT ROW 2.35 COL 1.57
     "Presione F10 para Reimprimir Etiquetas" VIEW-AS TEXT
          SIZE 41 BY .54 AT ROW 1.27 COL 81 WIDGET-ID 46
...
"ubigeo(ExpAsist.CodCli) @ x-col-ubigeo" "Ubigeo" "x(50)" ? ? ? ? ? ? ? no ? no no "22.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    FIND FIRST b-ExpAsist WHERE ROWID(b-ExpAsist) = ROWID(expasist) NO-ERROR.
    IF AVAIL b-ExpAsist AND b-ExpAsist.estado[2] = 'N' THEN DO:  
        ...
        IF lchoice THEN DO:
            ...
            /**/
            ASSIGN b-ExpAsist.libre_d05 = "ANULADO".  
        END.
    END.
    ELSE DO:
...
&Scoped-define SELF-NAME m_Registra_Asistencia
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL m_Registra_Asistencia B-table-Win
ON CHOOSE OF MENU-ITEM m_Registra_Asistencia /* Registra Asistencia */
DO:
  IF NOT AVAILABLE ExpAsist THEN RETURN.
  RUN Registra-Asistencia (ExpAsist.CodCli).
END.
...
&Scoped-define SELF-NAME m_Reimprime_Etiqueta
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL m_Reimprime_Etiqueta B-table-Win
ON CHOOSE OF MENU-ITEM m_Reimprime_Etiqueta /* Reimprime Etiqueta */
DO:
  RUN Imprime-Barras.
END.
...
IF NOT CAN-FIND(FIRST ExpAsist WHERE {&Condicion} AND ExpAsist.CodCli = pCodCli NO-LOCK)
    ...
    RETURN 'ADM-ERROR'.
END.
FIND FIRST ExpAsist WHERE {&Condicion} AND ExpAsist.CodCli = pCodCli NO-LOCK.
...
ASSIGN b-ExpAsist.libre_d05 = "GEN3EROS".

/* Generar PRE-PEDIDOS (PPT) */
/* DEF VAR pMensaje AS CHAR NO-UNDO.                                                                         */
/* RUN vtaexp/p-prepedido-terceros (INPUT pCodDiv,                                                           */
/*                                  INPUT pCodCli,                                                           */
/*                                  OUTPUT pMensaje).                                                        */
/* IF RETURN-VALUE = 'ADM-ERROR' THEN DO:                                                                    */
/*     IF TRUE <> (pMensaje > '') THEN pMensaje = 'NO se pudo generar el Pre-Pedido de Terceros' + CHR(10) + */
/*         'Avisa al administrador'.                                                                         */
/*     MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR TITLE 'ERROR PRE-PEDIDO TERCEROS'.                           */
/*     UNDO, RETURN 'ADM-ERROR'.                                                                             */
/* END.                                                                                                      */
/* */

x-fecasi2 = b-ExpAsist.FecAsi[2].
x-fecasi3 = b-ExpAsist.FecAsi[3].
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION ubigeo B-table-Win 
FUNCTION ubigeo RETURNS CHARACTER
  ( pCodClie AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR x-texto-ubigeo AS CHAR INIT "".
  
  FIND FIRST gn-clieD WHERE gn-clieD.codcia = cl-codcia AND
                            gn-clieD.codcli = pCodClie AND
                            gn-clieD.sede = "@@@" NO-LOCK NO-ERROR.     /* Direccion Fiscal */
  IF AVAILABLE gn-clieD THEN DO:
      FIND FIRST tabdepto WHERE tabdepto.coddepto = gn-clieD.coddept NO-LOCK NO-ERROR.
      IF AVAILABLE tabdepto THEN x-texto-ubigeo = TRIM(tabdepto.nomdepto).
      FIND FIRST tabprovi WHERE tabprovi.coddepto = gn-clieD.coddept AND 
                                tabprovi.codprovi = gn-clieD.codprov NO-LOCK NO-ERROR.
      IF AVAILABLE tabprovi THEN x-texto-ubigeo = x-texto-ubigeo + " - " + TRIM(tabprovi.nomprovi).
  END.

  RETURN x-texto-ubigeo.   /* Function return value. */
