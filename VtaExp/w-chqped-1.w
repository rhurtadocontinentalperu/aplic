...
DEFINE VARIABLE h_b-chqped-1 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CHEQUEO DE PEDIDOS EXPOLIBRERIA"
         HEIGHT             = 16.12
         WIDTH              = 97.14
         MAX-HEIGHT         = 32.31
         MAX-WIDTH          = 205.72
...
ON END-ERROR OF W-Win /* CHEQUEO DE PEDIDOS EXPOLIBRERIA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* CHEQUEO DE PEDIDOS EXPOLIBRERIA */
DO:
...
DO:
  RUN Cierre-de-guia IN h_b-chqped-1.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/b-chqped-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-chqped-1 ).
       RUN set-position IN h_b-chqped-1 ( 2.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-chqped-1 ( 14.00 , 95.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-chqped-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-chqped-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-chqped-1 ,
             BUTTON-9:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_b-chqped-1 , 'AFTER':U ).
    END. /* Page 0 */
