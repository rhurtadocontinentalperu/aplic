...
"PEDI.UndVta" "Undidad" "x(10)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    IF NOT AVAILABLE Almmmate THEN DO:
       ...
       RETURN NO-APPLY.   
    END.
    /* VERIFICA SI PERTENECE A LA COTIZACION */
    ...
    IF NOT AVAILABLE Facdpedi THEN DO:
        MESSAGE 'Este producto NO está registrado en la cotización'
...
DO:
    ...
    END.
    /* ************************************************** */
    /* RHC 26.01.10 BUSCAMOS LOS PRECIOS EN LA COTIZACION */
    FIND Facdpedi WHERE Facdpedi.codcia = s-codcia
        AND Facdpedi.coddoc = 'COT'
        AND Facdpedi.nroped = s-nrocot
    ...
    ELSE ASSIGN
            ...
            z-Dsctos = Facdpedi.Por_Dsctos[1].
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      PEDI.AlmDes:READ-ONLY IN BROWSE {&browse-name} = NO.
      PEDI.codmat:READ-ONLY IN BROWSE {&browse-name} = NO.
      /*APPLY 'ENTRY':U TO PEDI.codmat IN BROWSE {&browse-name}.*/
  END.
  ELSE DO:
      PEDI.AlmDes:READ-ONLY IN BROWSE {&browse-name} = YES.
  ...
  IF AVAILABLE Facdpedi THEN DO:
      ...
      THEN DO:
        MESSAGE 'NO se puede pedir mas de ' STRING(Facdpedi.CanPed - Facdpedi.CanAte + x-CanAte)
            PEDI.UndVta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} SKIP
            VIEW-AS ALERT-BOX ERROR.
        APPLY 'ENTRY':U TO PEDI.CanPed  IN BROWSE {&BROWSE-NAME}. 
        RETURN "ADM-ERROR".
      END.
