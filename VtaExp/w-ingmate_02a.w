...
DEFINE VARIABLE h_b-detcatvta_prov-2a AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-listaprov_prov-2a AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     x-monlc AT ROW 1.38 COL 119.72 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     x-importe AT ROW 1.38 COL 124.72 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     ...
     "L.C Disponible:" VIEW-AS TEXT
          SIZE 13 BY .62 AT ROW 1.54 COL 108 WIDGET-ID 24
          BGCOLOR 8 
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-listaprov_prov-2a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-listaprov_prov-2a ).
       RUN set-position IN h_b-listaprov_prov-2a ( 2.81 , 2.14 ) NO-ERROR.
       RUN set-size IN h_b-listaprov_prov-2a ( 8.15 , 74.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-detcatvta_prov-2a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-detcatvta_prov-2a ).
       RUN set-position IN h_b-detcatvta_prov-2a ( 12.04 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-detcatvta_prov-2a ( 13.46 , 147.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pagcatvtas. */
       RUN add-link IN adm-broker-hdl ( h_b-listaprov_prov-2a , 'Record':U , h_b-pagcatvtas ).

       /* Links to SmartBrowser h_b-detcatvta_prov-2a. */
       RUN add-link IN adm-broker-hdl ( h_b-pagcatvtas , 'Record':U , h_b-detcatvta_prov-2a ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-listaprov_prov-2a ,
             x-importe:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pagcatvtas ,
             h_b-listaprov_prov-2a , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-detcatvta_prov-2a ,
...
------------------------------------------------------------------------------*/
    ...
    CASE pParameter:
        ...
        WHEN 'Open-Query-3' THEN DO:
            RUN dispatch IN h_b-detcatvta_prov-2a ('open-query').
        END.
    END CASE.
END PROCEDURE.
