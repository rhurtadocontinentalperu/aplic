...
DEFINE VARIABLE h_b-detcatvta_prov-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-listaprov_prov-1 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/b-listaprov_prov-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-listaprov_prov-1 ).
       RUN set-position IN h_b-listaprov_prov-1 ( 2.81 , 2.14 ) NO-ERROR.
       RUN set-size IN h_b-listaprov_prov-1 ( 8.15 , 74.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/b-detcatvta_prov-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-detcatvta_prov-1 ).
       RUN set-position IN h_b-detcatvta_prov-1 ( 12.04 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-detcatvta_prov-1 ( 13.46 , 137.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pagcatvtas. */
       RUN add-link IN adm-broker-hdl ( h_b-listaprov_prov-1 , 'Record':U , h_b-pagcatvtas ).

       /* Links to SmartBrowser h_b-detcatvta_prov-1. */
       RUN add-link IN adm-broker-hdl ( h_b-pagcatvtas , 'Record':U , h_b-detcatvta_prov-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-listaprov_prov-1 ,
             x-importe:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pagcatvtas ,
             h_b-listaprov_prov-1 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-detcatvta_prov-1 ,
...
------------------------------------------------------------------------------*/
    ...
    CASE pParameter:
        ...
        WHEN 'Open-Query-3' THEN DO:
            RUN dispatch IN h_b-detcatvta_prov-1 ('open-query').
        END.
    END CASE.
END PROCEDURE.
