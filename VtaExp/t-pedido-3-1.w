...
&Scoped-define QUERY-STRING-br_table FOR EACH PEDI WHERE true NO-LOCK, ~
      EACH Almmmatg OF PEDI NO-LOCK ~
    BY PEDI.NroItm
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH PEDI WHERE true NO-LOCK, ~
      EACH Almmmatg OF PEDI NO-LOCK ~
    BY PEDI.NroItm.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      PEDI.UndVta FORMAT "X(10)":U
      PEDI.AlmDes COLUMN-LABEL "Alm!Des" FORMAT "x(3)":U
...
"PEDI.UndVta" ? "X(10)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.PEDI.AlmDes
"PEDI.AlmDes" "Alm!Des" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"PEDI.PreUni" ? ">>,>>9.99999" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  ASSIGN
      PEDI.ImpDto = ROUND( PEDI.PreUni * PEDI.CanPed * (PEDI.Por_Dsctos[1] / 100), 2 )
...
------------------------------------------------------------------------------*/
  ...
  /* RESTRICCION */
  IF DECIMAL(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > PEDI.Libre_d02 THEN DO:
      MESSAGE 'No puede solicitar más de' PEDI.Libre_d02 PEDI.UndVta
          VIEW-AS ALERT-BOX ERROR.
      APPLY 'entry':u TO PEDI.CanPed.
      RETURN 'ADM-ERROR'.
  END.
