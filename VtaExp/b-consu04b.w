...
DEFINE SHARED VARIABLE s-ImpLin AS DEC.
...
FacDPedi.PorDto FacDPedi.Por_Dsctos[1] FacDPedi.Por_Dsctos[2] ~
FacDPedi.Por_Dsctos[3] FacDPedi.PreUni FacDPedi.ImpLin 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH FacDPedi OF FacCPedi WHERE ~{&KEY-PHRASE} ~
      AND (s-ImpLin = 0 OR FacDPedi.ImpLin <= s-ImpLin) NO-LOCK, ~
      FIRST Almmmatg OF FacDPedi NO-LOCK ~
    BY FacDPedi.NroItm
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH FacDPedi OF FacCPedi WHERE ~{&KEY-PHRASE} ~
      AND (s-ImpLin = 0 OR FacDPedi.ImpLin <= s-ImpLin) NO-LOCK, ~
      FIRST Almmmatg OF FacDPedi NO-LOCK ~
    BY FacDPedi.NroItm.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 141 BY 11.04
         BGCOLOR 15 FONT 4.
...
DEFINE FRAME F-Main
     ...
     F-TotBrt AT ROW 13.46 COL 18 NO-LABEL
     F-ImpExo AT ROW 13.46 COL 30 NO-LABEL
     F-ValVta AT ROW 13.46 COL 52.14 COLON-ALIGNED NO-LABEL
     F-ImpIsc AT ROW 13.46 COL 64.86 COLON-ALIGNED NO-LABEL
     F-ImpIgv AT ROW 13.46 COL 77.43 COLON-ALIGNED NO-LABEL
     F-ImpTot AT ROW 13.46 COL 90 COLON-ALIGNED NO-LABEL
     F-ImpDes AT ROW 13.5 COL 40 COLON-ALIGNED NO-LABEL
     "T.Exonerado" VIEW-AS TEXT
          SIZE 9.43 BY .5 AT ROW 12.88 COL 30.57
     "I.S.C." VIEW-AS TEXT
          SIZE 4.57 BY .5 AT ROW 12.88 COL 70.57
     "Total Bruto" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 12.88 COL 20
     "T.Descuento" VIEW-AS TEXT
          SIZE 9.43 BY .5 AT ROW 12.92 COL 42.72
     "Total Importe" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 12.88 COL 92.14
     "Valor Venta" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 12.92 COL 55.86
     "I.G.V." VIEW-AS TEXT
          SIZE 5.14 BY .5 AT ROW 12.88 COL 82.14
     RECT-11 AT ROW 12.04 COL 1
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "(s-ImpLin = 0 OR FacDPedi.ImpLin <= s-ImpLin)"
...
"FacDPedi.Por_Dsctos[1]" "% Dscto!Manual" ? "decimal" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > integral.FacDPedi.Por_Dsctos[2]
