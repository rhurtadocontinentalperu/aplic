...
DEFINE VARIABLE h_v-cotexp-2x AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-6 AT ROW 8.69 COL 89.86 WIDGET-ID 4
     BUTTON-2 AT ROW 8.69 COL 101
     RECT-69 AT ROW 10.69 COL 2.57 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 112.72 BY 21.31
         FONT 4.
...
ON CHOOSE OF BUTTON-6 IN FRAME F-Main /* FINALIZA */
DO:
  RUN Finaliza_1 IN h_v-cotexp-2x.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/v-cotexp-2x.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotexp-2x ).
       RUN set-position IN h_v-cotexp-2x ( 1.27 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 8.69 , 10.43 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv06 ).
       RUN set-position IN h_p-updv06 ( 8.69 , 29.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-pedido2x ).
       RUN set-position IN h_q-pedido2x ( 8.65 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-cotexp-2x. */
       RUN add-link IN adm-broker-hdl ( h_p-updv06 , 'TableIO':U , h_v-cotexp-2x ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido2x , 'Record':U , h_v-cotexp-2x ).

       /* Links to SmartQuery h_q-pedido2x. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido2x ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotexp-2x ,
             BUTTON-6:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-cotexp-2x , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotexp-2 ).
       RUN set-position IN h_b-cotexp-2 ( 10.96 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-cotexp-2 ( 10.50 , 106.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotexp-2. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido2x , 'Record':U , h_b-cotexp-2 ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_t-cotexp-2x ( 10.96 , 4.00 ) NO-ERROR.
