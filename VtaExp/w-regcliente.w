...
&Scoped-Define ENABLED-OBJECTS RECT-1 RECT-2 BUTTON-3 BUTTON-1 BUTTON-2 ~
FILL-IN-1 FILL-IN-2 FILL-IN-3 FILL-IN-11 FILL-IN-4 FILL-IN-8 FILL-IN-5 ~
...
DO:
    RUN Asigna-Variables.  
    RUN Valida-Datos.
    IF NOT RETURN-VALUE = 'adm-error' THEN DO:
        RUN Graba-Datos.
        IF tg-print THEN RUN Imprimir.
        RUN Limpia-Campos.
    END.
...
DO:
    ...
    IF fill-in-1 <> '' THEN DO:
        FIND LAST ExpAsist WHERE ExpAsist.Codcia = s-codcia
            ...
            AND ExpAsist.Fecha     >= 12/01/2011
            AND ExpAsist.Estado[1] = "C" NO-LOCK NO-ERROR.
...
DO:
    ...
    **/
    ...
    FIND FIRST gn-ven WHERE gn-ven.codcia = s-codcia
        AND gn-ven.codven = SUBSTRING(txt-CodVen,3) NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ASSIGN 
            ...
            txt-codven tg-print FILL-IN-15 FILL-IN-17.
    END.
...
------------------------------------------------------------------------------*/
    ...
    DO iInt = 1 TO 2:
        FIND FIRST w-report WHERE task-no = s-task-no 
            ...
            AND llave-i = iInt NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 RECT-2 BUTTON-3 BUTTON-1 BUTTON-2 FILL-IN-1 FILL-IN-2 FILL-IN-3 
         FILL-IN-11 FILL-IN-4 FILL-IN-8 FILL-IN-5 FILL-IN-6 FILL-IN-7 FILL-IN-9 
         FILL-IN-10 FILL-IN-17 rs-docu FILL-IN-15 tg-print txt-codven 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST gn-clie WHERE ROWID(gn-clie) = x-rowid
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAIL ExpTurno THEN DO:
      ...
      ASSIGN
          ...
          ExpTurno.Estado  = "G".       /* Generado en recepción */
  END.
