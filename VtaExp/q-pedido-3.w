...
/* AND FacCPedi.TpoPed = ""*/ ~
 NO-LOCK, ~
      EACH ExpPromotor OF FacCPedi NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main FacCPedi ExpPromotor
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main FacCPedi
&Scoped-define SECOND-TABLE-IN-QUERY-Query-Main ExpPromotor
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.FacCPedi,INTEGRAL.ExpPromotor OF INTEGRAL.FacCPedi"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "integral.FacCPedi.CodCia = S-CODCIA
 AND integral.FacCPedi.CodDiv  = S-CODDIV
 AND integral.FacCPedi.usuario = S-USER-ID
 AND integral.FacCPedi.CodDoc  = S-CODDOC
/* AND integral.FacCPedi.TpoPed = """"*/
"
...
FIND FacCPedi WHERE FacCPedi.CodCia = S-CODCIA 
    ...
    AND FacCPedi.Usuario = s-User-Id
    AND FacCPedi.CodDoc = s-coddoc 
    AND FacCPedi.NroPed = STRING(I-NroSer,"999") + STRING(F-Numero,"999999") 
     NO-LOCK NO-ERROR.
IF AVAILABLE FacCPedi 
THEN output-var-1 = ROWID(FacCPedi).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
