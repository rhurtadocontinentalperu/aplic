...
DO:
  RUN dispatch IN h_v-pedido-3 ('imprime').
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv98 , 'TableIO':U , h_v-pedido-3 ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-cotiza-3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotiza-3 ).
       RUN set-position IN h_b-cotiza-3 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotiza-3 ( 10.35 , 100.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/t-pedido-3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedido-3 ).
       RUN set-position IN h_t-pedido-3 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedido-3 ( 10.77 , 108.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv95 ).
       RUN set-position IN h_p-updv95 ( 11.77 , 111.00 ) NO-ERROR.
       RUN set-size IN h_p-updv95 ( 4.58 , 12.00 ) NO-ERROR.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_b-cotiza-3 <> ? THEN RUN dispatch IN h_b-cotiza-3 ('open-query':U).
