...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  IF Ccbcdocu.NroPed <> '' THEN DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR"
...
------------------------------------------------------------------------------*/
  ...
  IF Ccbcdocu.fchdoc < 12/28/2010 THEN DO:
      IF S-CODDOC = "FAC" THEN DO:
        MESSAGE "SE IMPRIMIRA LA FACTURA" VIEW-AS ALERT-BOX.
        RUN vtamay/r-fac00002a (ROWID(ccbcdocu)).
      END.    
  ...
  IF S-CODDOC = "FAC" THEN DO:
    MESSAGE "SE IMPRIMIRA LA FACTURA" VIEW-AS ALERT-BOX.
    RUN vtamay/r-fac00002a-1 (ROWID(ccbcdocu)).
  END.    
  IF S-CODDOC = "BOL" THEN DO:
      MESSAGE "SE IMPRIMIRA LA BOLETA" VIEW-AS ALERT-BOX.
      RUN vtaexp/r-bolexpo-1 (ROWID(CcbCDocu)).
  END.
...
------------------------------------------------------------------------------*/
  ...
  input-var-3 = ''.
  RUN vtaexp\C-Pedido-1-1 ("Pedidos Pendientes").
...
------------------------------------------------------------------------------*/
  ...
  /* Detalle de la Factura/Boleta OK */ 
  RUN Genera-Detalle.       
  IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
  ...
  /* OJO: IMPRIME SI SOLO ES UNA FACTURA */
  ...
  REPEAT:
    ...
    BUFFER-COPY CcbCDocu 
        EXCEPT Ccbcdocu.ImpTot2     /* OJO >> Control de Aplic. Fact. por Adelantos */
        TO B-CDOCU
        ASSIGN 
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Ccbdcaja NO-LOCK WHERE Ccbdcaja.codcia = s-codcia
      AND LOOKUP(Ccbdcaja.coddoc, 'FAC,BOL,A/C') > 0
      AND Ccbdcaja.codref = Ccbcdocu.coddoc
      AND Ccbdcaja.nroref = CCbcdocu.nrodoc:
