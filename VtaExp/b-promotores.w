...
&Scoped-define FIELDS-IN-QUERY-br_table VtaTabla.Llave_c1 gn-prov.NomPro ~
VtaTabla.Llave_c2 VtaTabla.Libre_c01 VtaTabla.Libre_c02 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      VtaTabla.Libre_c02 COLUMN-LABEL "Equipo" FORMAT "x(10)":U
            WIDTH 11.72 VIEW-AS COMBO-BOX SORT INNER-LINES 15
...
"VtaTabla.Llave_c1" "Proveedor" "x(11)" "character" ? ? ? ? ? ? yes ? no no "11.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaTabla.Llave_c2" "Promotor" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaTabla.Libre_c02" "Equipo" "x(10)" "character" ? ? ? ? ? ? yes ? no no "11.72" yes no no "U" "" "" "DROP-DOWN-LIST" "," "ROJO,CELESTE,AMARILLO,VERDE,NARANJA,AGUA MARINA,AZUL,BLANCO" ? 15 yes 0 no no
...
ON LEAVE OF VtaTabla.Llave_c1 IN BROWSE br_table /* Proveedor */
DO:
...
ON 'RETURN':U OF VtaTabla.Libre_c01, VtaTabla.Libre_c02, VtaTabla.Llave_c1, VtaTabla.Llave_c2
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
FIND GN-PROV WHERE gn-prov.codcia = pv-codcia
      AND gn-prov.codpro = VtaTabla.Llave_c1:SCREEN-VALUE IN BROWSE {&browse-name}
      NO-LOCK NO-ERROR.
IF VtaTabla.Llave_c1:SCREEN-VALUE IN BROWSE {&browse-name} = ''
    ...
    RETURN 'ADM-ERROR'.
END.
IF VtaTabla.Llave_c2:SCREEN-VALUE IN BROWSE {&browse-name} = '' THEN DO:
