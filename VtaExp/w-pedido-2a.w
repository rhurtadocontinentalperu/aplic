...
DEFINE NEW SHARED VARIABLE S-TPOPED   AS CHAR INIT '2'.     /* LIBRE */
...
DEFINE VARIABLE h_t-pedido-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-pedido-2 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "PEDIDOS AL CREDITO EXPOLIBRERIA - LIBRE"
         HEIGHT             = 19.92
         WIDTH              = 112.72
         MAX-HEIGHT         = 32.81
         MAX-WIDTH          = 205.72
...
ON END-ERROR OF W-Win /* PEDIDOS AL CREDITO EXPOLIBRERIA - LIBRE */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* PEDIDOS AL CREDITO EXPOLIBRERIA - LIBRE */
DO:
...
DO:
   RUN Asigna_datos IN h_v-pedido-2.
END.
...
DO:
    ...
    RUN Envia_chequeo IN h_v-pedido-2.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/v-pedido-2a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedido-2 ).
       RUN set-position IN h_v-pedido-2 ( 1.00 , 1.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-pedido-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-pedido-2 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-pedido-2 ).

       /* Links to SmartQuery h_q-pedido. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedido-2 ,
             B-CCTE:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-pedido-2 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotiza-2 ).
       RUN set-position IN h_b-cotiza-2 ( 10.23 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/t-pedido-2a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedido-2 ).
       RUN set-position IN h_t-pedido-2 ( 10.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedido-2 ( 8.88 , 111.00 ) NO-ERROR.
       ...
       RUN set-size IN h_p-updv12 ( 1.54 , 39.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-pedido-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedido-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedido-2 ,
             B-CHEQUEO:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-pedido-2 , 'AFTER':U ).
    END. /* Page 2 */
