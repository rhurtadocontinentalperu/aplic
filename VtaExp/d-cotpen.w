...
&Scoped-Define ENABLED-OBJECTS RECT-41 RECT-43 BUTTON-1 f-clien ~
FILL-IN-Marca R-Tipo f-desde f-hasta FILL-IN-FchPed-1 FILL-IN-FchPed-2 ~
F-entrega1 F-entrega2 Btn_OK-2 Btn_OK Btn_Cancel Btn_Help 
&Scoped-Define DISPLAYED-OBJECTS F-Division f-clien f-nomcli FILL-IN-Marca ~
R-Tipo f-desde f-hasta FILL-IN-FchPed-1 FILL-IN-FchPed-2 F-entrega1 ~
F-entrega2 
...
DEFINE VARIABLE F-Division AS CHARACTER FORMAT "X(60)":U INITIAL "00015,10015" 
     LABEL "Division" 
     VIEW-AS FILL-IN 
     SIZE 43 BY .81
...
DEFINE FRAME D-Dialog
     F-Division AT ROW 1.58 COL 9 COLON-ALIGNED WIDGET-ID 10
     BUTTON-1 AT ROW 1.58 COL 54 WIDGET-ID 8
     f-clien AT ROW 2.35 COL 9 COLON-ALIGNED
     f-nomcli AT ROW 2.35 COL 21 COLON-ALIGNED NO-LABEL
     FILL-IN-Marca AT ROW 3.15 COL 9 COLON-ALIGNED
     R-Tipo AT ROW 4.23 COL 10 NO-LABEL
     f-desde AT ROW 5.31 COL 28 COLON-ALIGNED
     f-hasta AT ROW 5.31 COL 46 COLON-ALIGNED
     FILL-IN-FchPed-1 AT ROW 6.12 COL 28 COLON-ALIGNED
     FILL-IN-FchPed-2 AT ROW 6.12 COL 46 COLON-ALIGNED
     F-entrega1 AT ROW 6.92 COL 28 COLON-ALIGNED WIDGET-ID 2
     F-entrega2 AT ROW 6.92 COL 46 COLON-ALIGNED WIDGET-ID 4
     Btn_OK-2 AT ROW 9.35 COL 15.43 WIDGET-ID 6
     Btn_OK AT ROW 9.35 COL 28
     Btn_Cancel AT ROW 9.35 COL 40.43
     Btn_Help AT ROW 9.35 COL 53
     " Configuración de Impresión" VIEW-AS TEXT
          SIZE 67 BY .62 AT ROW 8.54 COL 3
          BGCOLOR 1 FGCOLOR 1 FONT 6
     RECT-41 AT ROW 1.08 COL 1.72
     RECT-43 AT ROW 8.27 COL 2
...
DO k = 1 TO NUM-ENTRIES(f-Division):
    ...
    FOR EACH FacCpedi NO-LOCK WHERE
         ...
         (f-Clien = '' OR FacCpedi.Codcli = f-clien) AND
         ...
         IF FILL-IN-FchPed-1 <> ? OR FILL-IN-FchPed-2 <> ? THEN DO:
             FIND FIRST B-FacCPedi USE-INDEX Llave03 WHERE B-FacCPedi.CodCia = s-CodCia
                 ...
                 AND B-FacCPedi.CodRef = FacCPedi.CodDoc
...
FOR EACH Tmp-Tempo:
    ...
    IF Tmp-Tempo.t-totsol = ? THEN Tmp-Tempo.t-totsol = 0.
    IF Tmp-Tempo.t-totdol = ? THEN Tmp-Tempo.t-totdol = 0.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY F-Division f-clien f-nomcli FILL-IN-Marca R-Tipo f-desde f-hasta 
          FILL-IN-FchPed-1 FILL-IN-FchPed-2 F-entrega1 F-entrega2 
      WITH FRAME D-Dialog.
  ENABLE RECT-41 RECT-43 BUTTON-1 f-clien FILL-IN-Marca R-Tipo f-desde f-hasta 
...
chWorkSheet:Range("F3"):VALUE = "Stock".
...
------------------------------------------------------------------------------*/
  ...
  IF s-acceso-total = NO THEN DO WITH FRAME {&FRAME-NAME}:
...
/*                          NO-LOCK NO-ERROR.                        */
/*      If Available Almmmate Then X-STOCK = Almmmate.StkAct.        */
     DO k = 1 TO NUM-ENTRIES(tmp-tempo.t-CodAlm):
         FIND Almmmate WHERE Almmmate.CodCia = S-CODCIA 
             AND Almmmate.CodAlm = ENTRY(k, tmp-tempo.t-CodAlm)
             AND Almmmate.CodmAT = tmp-tempo.t-CodMat
             NO-LOCK NO-ERROR. 
         IF AVAILABLE Almmmate THEN X-STOCK = Almmmate.StkAct.
     END.
...
RUN vta2/stock-comprometido (s-CodMat, s-CodAlm, OUTPUT x-CanPed).
