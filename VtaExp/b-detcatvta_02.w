...
Almmmatg.DesMar DATOS.UndBas DATOS.PreAlt[6] DATOS.PreAlt[5] ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      DATOS.PreAlt[6] COLUMN-LABEL "% Dscto 1" FORMAT "->>,>>9.99":U
            WIDTH 6
      DATOS.PreAlt[5] COLUMN-LABEL "% Dscto 2" FORMAT "->>,>>9.99":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 143 BY 12.92
         FONT 4
...
"DATOS.PreAlt[6]" "% Dscto 1" ? "decimal" ? ? ? ? ? ? no ? no no "6" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.DATOS.PreAlt[5]
"DATOS.PreAlt[5]" "% Dscto 2" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.DATOS.PreAlt[4]
"DATOS.PreAlt[4]" "Precio!Unitario" "->>,>>>,>>9.9999" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.DATOS.Libre_d01
"DATOS.Libre_d01" "Cantidad" ? "decimal" ? 9 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.DATOS.NroSec
...
DO:
    ...
    datos.prealt[6]:BGCOLOR IN BROWSE {&BROWSE-NAME} = 8.
...
DO:
    ...
    datos.prealt[6]:BGCOLOR IN BROWSE {&BROWSE-NAME} = 15.
