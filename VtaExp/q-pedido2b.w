...
DEFINE SHARED VARIABLE S-TPOPED  AS CHAR.   /* 1 NORMAL  2 LIBRE */
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH VtaCDocu WHERE ~{&KEY-PHRASE} ~
      AND VtaCDocu.CodCia = s-codcia ~
 AND VtaCDocu.CodDiv = s-coddiv ~
 AND VtaCDocu.Usuario = s-user-id ~
 AND VtaCDocu.CodPed = s-coddoc ~
 AND VtaCDocu.FlgEst <> "A" ~
 AND (VtaCDocu.FlgSit = "S" ~
 OR VtaCDocu.FlgSit = "T") NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH VtaCDocu WHERE ~{&KEY-PHRASE} ~
      AND VtaCDocu.CodCia = s-codcia ~
 AND VtaCDocu.CodDiv = s-coddiv ~
 AND VtaCDocu.Usuario = s-user-id ~
 AND VtaCDocu.CodPed = s-coddoc ~
 ...
 OR VtaCDocu.FlgSit = "T") NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main VtaCDocu
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main VtaCDocu


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS b-busca txt-nroser txt-nrodoc x-codcli 
&Scoped-Define DISPLAYED-OBJECTS txt-nroser txt-nrodoc x-codcli x-nomcli 
...
DEFINE VARIABLE txt-nroser AS CHARACTER FORMAT "X(9)":U INITIAL "015" 
     LABEL "Documto" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME FRAME-A
     b-busca AT ROW 1.08 COL 85.72 WIDGET-ID 8
     txt-nroser AT ROW 1.27 COL 7.43 COLON-ALIGNED WIDGET-ID 2
     txt-nrodoc AT ROW 1.27 COL 13.43 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     x-codcli AT ROW 1.27 COL 29 COLON-ALIGNED WIDGET-ID 4
     x-nomcli AT ROW 1.27 COL 42 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     "-" VIEW-AS TEXT
          SIZE 1.43 BY .5 AT ROW 1.42 COL 13.86 WIDGET-ID 14
...
/* SETTINGS FOR FILL-IN x-nomcli IN FRAME FRAME-A
   NO-ENABLE                                                            */
...
/* Query rebuild information for QUERY Query-Main
 ...
 AND VtaCDocu.FlgEst <> ""A""
 AND (VtaCDocu.FlgSit = ""S""
 OR VtaCDocu.FlgSit = ""T"")"
     _Design-Parent    is WINDOW q-tables @ ( 1.27 , 24.14 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
DO:
    ...
    ASSIGN txt-nroser txt-nrodoc x-codcli.
    x-nrodoc = txt-nroser + STRING(txt-nrodoc,"999999").

    IF x-nrodoc = "" THEN DO:
        MESSAGE "INGRESE NUMERO DE DOCUMENTO A DIGITAR"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        APPLY "ENTRY" TO txt-nroser.
        RETURN "ADM-ERROR".
    END.

    IF x-codcli = "" THEN DO:
        MESSAGE "DEBE INGRESAR CODIGO DE CLIENTE"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        APPLY "ENTRY" TO X-CODCLI.
        RETURN "ADM-ERROR".
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL x-codcli q-tables
ON LEAVE OF x-codcli IN FRAME FRAME-A /* Cliente */
DO:
   FIND FIRST gn-clie WHERE gn-clie.codcia = cl-codcia
       AND gn-clie.codcli = {&SELF-NAME}:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN vtaexp/c-cotexp ("Cotizaciones").
...
FIND VtaCDocu WHERE VtaCDocu.CodCia = S-CODCIA AND
     VtaCDocu.CodPed = "PET" AND
     VtaCDocu.NroPed = x-nrodoc AND
     VtaCDocu.CodCli = x-codcli
     NO-LOCK NO-ERROR.
IF AVAILABLE VtaCDocu THEN output-var-1 = ROWID(VtaCDocu).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
