...
------------------------------------------------------------------------------*/
    ...
    FOR EACH almcatvtac WHERE almcatvtac.codcia = s-codcia
        ...
        /*cDsctoExpo = if(almcatvtad.libre_c01 = ? OR almcatvtad.libre_c01 = "") THEN "" ELSE TRIM(almcatvtad.libre_c01).*/
        cDsctoExpo = IF Almcatvtad.Libre_c01 = "SI" THEN "SI" ELSE "NO".
        /*
        */
        FIND FIRST w-report WHERE task-no = s-task-no
            AND w-report.llave-c = almcatvtac.codpro    /*tt-clientes.codcli*/
        ...
        IF NOT AVAIL w-report THEN DO:
            ...
            FIND FIRST vtalistamay WHERE vtalistamay.codcia = s-codcia
                AND VtaListaMay.CodDiv = COMBO-BOX-Division /*s-coddiv*/
                AND VtaListaMay.codmat = almcatvtad.codmat NO-LOCK NO-ERROR.
            IF AVAIL vtalistamay THEN DO: 
                dDescuento = VtaListaMay.libre_d03.
                dPrecioSoles = vtalistamay.preofi.
...
/*                  ).                                                    */
             ...
             IF RETURN-VALUE <> 'ADM-ERROR' THEN dPrecioSoles = dPrecioSoles + f-FleteUnitario.
             IF RETURN-VALUE <> 'ADM-ERROR' AND f-FleteUnitario > 0 THEN DO:
                 dPrecioNeto = dPrecioNeto + f-FleteUnitario.
                 dPrecioSoles = ROUND(dPrecioNeto / (1 - dDescuento / 100 ), 4).
             END.
            /* **************************************************************** */
            /* **************************************************************** */
...
------------------------------------------------------------------------------*/
    ...
    /*RB-REPORT-NAME = 'Imprime Catalogo Precios'.*/
    /*RB-REPORT-NAME = 'xxx'.*/
