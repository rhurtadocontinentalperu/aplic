...
&Scoped-Define ENABLED-OBJECTS BUTTON-6 BUTTON-7 BtnDone FILL-IN_FchEnt ~
BUTTON-1 BUTTON-5 BUTTON-4 BUTTON-3 BUTTON-2 
...
DEFINE VARIABLE h_b-preped-01a AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-preped-01b AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-6 AT ROW 3.15 COL 111 WIDGET-ID 18
     BUTTON-7 AT ROW 6.38 COL 111 WIDGET-ID 20
     BtnDone AT ROW 1.27 COL 119 WIDGET-ID 16
     FILL-IN_FchEnt AT ROW 1.27 COL 19 COLON-ALIGNED WIDGET-ID 4
     BUTTON-1 AT ROW 1 COL 41 WIDGET-ID 6
     BUTTON-5 AT ROW 1 COL 57 WIDGET-ID 14
     BUTTON-4 AT ROW 7.46 COL 111 WIDGET-ID 12
     BUTTON-3 AT ROW 5.31 COL 111 WIDGET-ID 10
     BUTTON-2 AT ROW 4.23 COL 110.86 WIDGET-ID 8
...
DO:
    ...
    RUN Carga-Temporal IN h_b-preped-01a ( INPUT FILL-IN_FchEnt).
    RUN dispatch IN h_b-preped-01a ('open-query').
...
DO:
    MESSAGE 'Eliminamos el registro' VIEW-AS ALERT-BOX QUESTION
        BUTTONS YES-NO UPDATE rpta AS LOG.
    IF rpta = NO THEN RETURN NO-APPLY.
    RUN Eliminar IN h_b-preped-01a.
    /*RUN dispatch IN h_b-preped-01a ('row-available').*/
END.
...
ON CHOOSE OF BUTTON-5 IN FRAME F-Main /* (2) GENERAR PRE-PEDIDOS */
DO:
...
DO:
    RUN Poner-Primero IN h_b-preped-01a.
END.
...
DO:
    RUN Poner-Ultimo IN h_b-preped-01a.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtaexp/b-preped-01b.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-preped-01b ).
       RUN set-position IN h_b-preped-01b ( 10.42 , 6.00 ) NO-ERROR.
       RUN set-size IN h_b-preped-01b ( 6.69 , 111.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtaexp/b-preped-01a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-preped-01a ).
       RUN set-position IN h_b-preped-01a ( 2.35 , 6.00 ) NO-ERROR.
       RUN set-size IN h_b-preped-01a ( 7.73 , 104.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-preped-01b. */
       RUN add-link IN adm-broker-hdl ( h_b-preped-01a , 'Record':U , h_b-preped-01b ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-preped-01b ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-preped-01a ,
             h_b-preped-01b , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-6 BUTTON-7 BtnDone FILL-IN_FchEnt BUTTON-1 BUTTON-5 BUTTON-4 
         BUTTON-3 BUTTON-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH B-CPEDI:
      ...
      CREATE Faccpedi.
      BUFFER-COPY B-CPEDI 
          EXCEPT B-CPEDI.Libre_d01 B-CPEDI.Libre_d02
          TO Faccpedi
          ASSIGN 
      ...
      FOR EACH B-DPEDI WHERE B-DPEDI.codcia = B-CPEDI.codcia
          ...
          CREATE FacDPedi.
          BUFFER-COPY B-DPEDI 
              EXCEPT B-DPEDI.Libre_d01 B-DPEDI.Libre_d02
              TO FacDPedi
              ASSIGN
                  ...
                  FacDPedi.CanPed = B-DPEDI.Libre_d01.
      END.
      ...
      FIND Faccpedi WHERE Faccpedi.codcia = B-CPEDI.codcia
          AND Faccpedi.coddoc = B-CPEDI.coddoc
          AND Faccpedi.nroped = B-CPEDI.nroped
          EXCLUSIVE-LOCK NO-ERROR.
      FOR EACH B-DPEDI WHERE B-DPEDI.codcia = B-CPEDI.codcia
            AND B-DPEDI.coddoc = B-CPEDI.coddoc
            AND B-DPEDI.nroped = B-CPEDI.nroped:
          FIND Facdpedi OF Faccpedi WHERE Facdpedi.codmat = B-DPEDI.codmat
              EXCLUSIVE-LOCK NO-ERROR.
          ASSIGN
              Facdpedi.canate = Facdpedi.canate + B-DPEDI.Libre_d01.
      END.
  ...
  RUN dispatch IN h_b-preped-01a ('open-query').
  RUN dispatch IN h_b-preped-01b ('open-query').
...
RUN Pinta-Browse IN h_b-preped-01b
    ( INPUT pRowid /* ROWID */).
