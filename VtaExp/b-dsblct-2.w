...
FacCPedi.CodVen gn-ven.NomVen FacCPedi.NomCli FacCPedi.ImpTot 
...
&Scoped-define QUERY-STRING-br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacCPedi.Libre_c05 = "*" ~
 AND FacCPedi.FchPed = x-FchPed NO-LOCK, ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacCPedi.FchPed = x-FchPed NO-LOCK, ~
      EACH gn-ven OF FacCPedi NO-LOCK ~
    BY FacCPedi.NroPed DESCENDING.
&Scoped-define TABLES-IN-QUERY-br_table FacCPedi gn-ven
...
/* Query rebuild information for BROWSE br_table
 ...
 AND INTEGRAL.FacCPedi.Libre_c05 = ""*""
 AND INTEGRAL.FacCPedi.FchPed = x-FchPed"
...
"gn-ven.NomVen" ? ? "character" ? ? ? ? ? ? no ? no no "26.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.FacCPedi.NomCli
"FacCPedi.NomCli" "Nombre del Cliente" ? "character" ? ? ? ? ? ? no ? no no "30.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  MESSAGE 'Desea desbloquear la Cotizacion' FacCPedi.NroPed '?'
  ...
  IF AVAILABLE ExpTurno THEN Expturno.Estado = 'P'.
  RELEASE FacCPedi.
