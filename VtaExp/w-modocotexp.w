...
DEFINE VARIABLE h_v-cabcotiza AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     COMBO-NroSer AT ROW 1.42 COL 4 NO-LABEL WIDGET-ID 4
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 115 BY 10.77 WIDGET-ID 100.
...
ON END-ERROR OF W-Win /* Consulta Cotizaciones */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Consulta Cotizaciones */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 1.27 , 27.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 1.50 , 23.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv97 ).
       RUN set-position IN h_p-updv97 ( 1.27 , 51.00 ) NO-ERROR.
       RUN set-size IN h_p-updv97 ( 1.54 , 34.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtaexp/v-cabcotiza.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cabcotiza ).
       RUN set-position IN h_v-cabcotiza ( 3.15 , 5.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.69 , 107.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtaexp/q-cotexp-4-2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-cotexp-4-2 ).
       RUN set-position IN h_q-cotexp-4-2 ( 1.27 , 16.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 9.00 ) */

       /* Links to SmartViewer h_v-cabcotiza. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_v-cabcotiza ).
       RUN add-link IN adm-broker-hdl ( h_q-cotexp-4-2 , 'Record':U , h_v-cabcotiza ).
