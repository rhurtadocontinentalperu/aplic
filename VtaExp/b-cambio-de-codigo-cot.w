...
DEF VAR x-Listas AS CHAR NO-UNDO.
...
&Scoped-define FIELDS-IN-QUERY-br_table FacCPedi.CodDiv ~
FacCPedi.Lista_de_Precios T-CPEDI.NroPed FacCPedi.FchPed FacCPedi.NomCli 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.Lista_de_Precios FORMAT "x(8)":U
      T-CPEDI.NroPed FORMAT "X(12)":U WIDTH 10.57
      FacCPedi.FchPed FORMAT "99/99/9999":U
      FacCPedi.NomCli COLUMN-LABEL "Cliente" FORMAT "x(80)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 99 BY 15.88
         FONT 4
         TITLE "COTIZACIONES VALIDAS".
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[2]   = INTEGRAL.FacCPedi.Lista_de_Precios
     _FldNameList[3]   > Temp-Tables.T-CPEDI.NroPed
"Temp-Tables.T-CPEDI.NroPed" ? ? "character" ? ? ? ? ? ? no ? no no "10.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   = INTEGRAL.FacCPedi.FchPed
     _FldNameList[5]   > INTEGRAL.FacCPedi.NomCli
"INTEGRAL.FacCPedi.NomCli" "Cliente" "x(80)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* COTIZACIONES VALIDAS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* COTIZACIONES VALIDAS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* COTIZACIONES VALIDAS */
DO:
...
DO k = 1 TO NUM-ENTRIES(x-Divisiones):
    ...
    FOR EACH Faccpedi NO-LOCK WHERE FacCPedi.CodCia = s-CodCia AND
        ...
        FacCPedi.FchPed <= x-FchPed-2:
        IF LOOKUP(FacCPedi.Lista_de_Precios, x-Listas) = 0 THEN NEXT.
...
IF NOT CAN-FIND(FIRST T-CPEDI NO-LOCK) THEN RETURN 'ADM-ERROR'.
RETURN 'OK'.
...
DEF OUTPUT PARAMETER TABLE FOR T-CPEDI.
