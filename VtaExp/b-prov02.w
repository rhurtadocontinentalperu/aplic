...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 67 BY 2.19
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     "** Dar doble chic a registro antes de ingresar los datos" VIEW-AS TEXT
          SIZE 56.57 BY .62 AT ROW 5.31 COL 3 WIDGET-ID 4
          FONT 5
...
"w-report.Campo-C[1]" "Proveedores" "X(50)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ENDIF
/*
FOR EACH almcatvtac WHERE almcatvtac.codcia = s-codcia NO-LOCK:
    FIND FIRST PROV WHERE PROV.codcia = pv-codcia
        AND PROV.codpro = almcatvtac.codpro NO-ERROR.
    IF NOT AVAIL PROV THEN DO:
        FIND FIRST gn-prov WHERE gn-prov.codcia = pv-codcia
            AND gn-prov.codpro = almcatvtac.codpro NO-LOCK NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE busqueda-secuencial B-table-Win 
