...
&Scoped-Define ENABLED-OBJECTS FILL-IN-file BUTTON-5 x-desc x-Copias ~
btn-print BUTTON-8 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-file x-desc x-Copias 
...
DEFINE VARIABLE x-desc AS CHARACTER FORMAT "X(20)":U 
     LABEL "Cliente" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME F-Main
     FILL-IN-file AT ROW 3.15 COL 14 COLON-ALIGNED WIDGET-ID 22
     BUTTON-5 AT ROW 3.15 COL 60 WIDGET-ID 6
     x-desc AT ROW 4.23 COL 14 COLON-ALIGNED WIDGET-ID 20
     x-Copias AT ROW 5.31 COL 14 COLON-ALIGNED WIDGET-ID 16
     btn-print AT ROW 5.85 COL 45 WIDGET-ID 4
     BUTTON-8 AT ROW 5.85 COL 54 WIDGET-ID 18
     "Impresión de Etiquetas Clientes" VIEW-AS TEXT
...
ON END-ERROR OF W-Win /* ETIQUETAS CLIENTES */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ETIQUETAS CLIENTES */
DO:
...
------------------------------------------------------------------------------*/
    ...
    FIND tt-cliente NO-LOCK NO-ERROR.
    IF NOT AVAIL tt-cliente THEN DO:
        CREATE tt-cliente.
        ASSIGN tt-codcli = x-desc.
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-file x-desc x-Copias 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE FILL-IN-file BUTTON-5 x-desc x-Copias btn-print BUTTON-8 
      WITH FRAME F-Main IN WINDOW W-Win.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE VARIABLE cParm01 AS CHARACTER FORMAT 'X(54)'  NO-UNDO.
    DEFINE VARIABLE cParm02 AS CHARACTER FORMAT 'X(54)'  NO-UNDO.
    DEFINE VARIABLE cParm03 AS CHARACTER FORMAT 'X(54)'  NO-UNDO.
    ...
    FOR EACH tt-cliente, 
        FIRST gn-clie WHERE gn-clie.codcia = 0
            AND gn-clie.codcli = tt-cliente.tt-codcli NO-LOCK:
        cParm01 = gn-clie.nomcli .
        cParm02 = gn-clie.dircli.
        FIND FIRST TabDistr
            WHERE TabDistr.CodDepto = gn-clie.CodDept 
            AND TabDistr.CodProvi = gn-clie.CodProv
            AND TabDistr.CodDistr = gn-clie.CodDist NO-LOCK NO-ERROR.
        IF AVAIL TabDistr THEN cParm03 = TabDistr.NomDistr.

    OUTPUT STREAM REPORTE TO PRINTER.
        PUT STREAM REPORTE '^XA^LH000,012'                        SKIP.   /* Inicia formato */

          PUT STREAM reporte '^FO100,10'                          SKIP.  
          PUT STREAM reporte '^A0,25,25'                          SKIP.
          PUT STREAM reporte '^FD'                                SKIP.  
          PUT STREAM reporte  cParm01.
          PUT STREAM reporte '^FS'                                SKIP.

          PUT STREAM reporte '^FO100,35'                          SKIP.  
          PUT STREAM reporte '^A0,25,25'                          SKIP.
          PUT STREAM reporte '^FD'                                SKIP.  
          PUT STREAM reporte  cParm02 .
          PUT STREAM reporte '^FS'                                SKIP.

          PUT STREAM reporte '^FO100,60'                          SKIP.  
          PUT STREAM reporte '^A0,25,25'                          SKIP.
          PUT STREAM reporte '^FD'                                SKIP.  
          PUT STREAM reporte  cParm03 .
          PUT STREAM reporte '^FS'                                SKIP.
        
          PUT STREAM reporte '^FO100,85'                          SKIP.  
          PUT STREAM REPORTE '^BC,60'                             SKIP.   /* Codigo 128 */
