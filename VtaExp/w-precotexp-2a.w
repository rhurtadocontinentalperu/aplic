...
DEFINE VARIABLE h_t-precotexp-2a AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-precotexp-2a AS HANDLE NO-UNDO.
...
DO:
    RUN Asigna-Cotizacion IN h_v-precotexp-2a.  
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-precotexp-2a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-precotexp-2a ).
       RUN set-position IN h_v-precotexp-2a ( 1.27 , 2.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-precotexp-2a. */
       RUN add-link IN adm-broker-hdl ( h_p-updv06 , 'TableIO':U , h_v-precotexp-2a ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido2x , 'Record':U , h_v-precotexp-2a ).

       /* Links to SmartQuery h_q-pedido2x. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido2x ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-precotexp-2a ,
             BUTTON-6:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-precotexp-2a , 'AFTER':U ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  't-precotexp-2a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-precotexp-2a ).
       RUN set-position IN h_t-precotexp-2a ( 12.15 , 1.57 ) NO-ERROR.
       RUN set-size IN h_t-precotexp-2a ( 11.08 , 131.57 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-precotexp-2a ,
             BUTTON-6:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-precotexp-2a <> ? THEN RUN dispatch IN h_t-precotexp-2a ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        RUN Recalcular-Precios IN h_t-precotexp-2a.
      END.
