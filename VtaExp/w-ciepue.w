...
DEFINE VARIABLE h_t-ciepue AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-ciepue AS HANDLE NO-UNDO.
...
DO:
  RUN Genera-Excel IN h_v-ciepue.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/v-ciepue.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ciepue ).
       RUN set-position IN h_v-ciepue ( 1.00 , 1.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-ciepue. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_v-ciepue ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-ciepue ).

       /* Links to SmartQuery h_q-pedido. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ciepue ,
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/t-ciepue.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-ciepue ).
       RUN set-position IN h_t-ciepue ( 10.42 , 1.00 ) NO-ERROR.
       RUN set-size IN h_t-ciepue ( 8.88 , 110.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-ciepue. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-ciepue ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-ciepue ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-ciepue <> ? THEN RUN dispatch IN h_t-ciepue ('open-query':U).
      END.
...
/*         B-CCTE:SENSITIVE = YES.*/
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        RUN Recalcular-Precios IN h_t-ciepue.
      END.
