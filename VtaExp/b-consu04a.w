...
DEF VAR x-Alarma AS CHAR.

DEF VAR x-Estado AS CHAR.
DEF SHARED VAR s-user-id AS CHAR.

DEF SHARED VAR s-ImpLin AS DEC INIT 0.
...
&Scoped-define INTERNAL-TABLES FacCPedi gn-ven FacDPedi
...
FacCPedi.CodCli FacCPedi.NomCli gn-ven.NomVen FacCPedi.ImpTot ~
x-Estado @ x-Estado 
...
&Scoped-define QUERY-STRING-br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
 ...
 AND LOOKUP (INTEGRAL.FacCPedi.FlgEst, "W,A") = 0 ~
 AND FacCPedi.FchPed >= FILL-IN-FchPed-1 ~
 AND FacCPedi.FchPed <= FILL-IN-FchPed-2 NO-LOCK, ~
      EACH gn-ven OF FacCPedi NO-LOCK, ~
      FIRST FacDPedi OF FacCPedi ~
      WHERE (FILL-IN-ImpLin = 0 OR Facdpedi.ImpLin <= FILL-IN-ImpLin) NO-LOCK ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacCPedi.FchPed <= FILL-IN-FchPed-2 NO-LOCK, ~
      ...
      WHERE (FILL-IN-ImpLin = 0 OR Facdpedi.ImpLin <= FILL-IN-ImpLin) NO-LOCK ~
    BY FacCPedi.NroPed DESCENDING.
&Scoped-define TABLES-IN-QUERY-br_table FacCPedi gn-ven FacDPedi
&Scoped-define FIRST-TABLE-IN-QUERY-br_table FacCPedi
&Scoped-define SECOND-TABLE-IN-QUERY-br_table gn-ven
&Scoped-define THIRD-TABLE-IN-QUERY-br_table FacDPedi
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fAlarma B-table-Win 
FUNCTION fAlarma RETURNS CHARACTER
...
DEFINE VARIABLE FILL-IN-FchPed-1 AS DATE FORMAT "99/99/9999":U 
     LABEL "FILTRAR POR FECHAS: Emitidos desde" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE FILL-IN-ImpLin AS DECIMAL FORMAT ">>>,>>9.99":U INITIAL 0 
     LABEL "IMPORTES MENORES O IGUALES A" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .81 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.CodCli COLUMN-LABEL "Cliente" FORMAT "x(11)":U WIDTH 12
      FacCPedi.NomCli FORMAT "x(50)":U
      gn-ven.NomVen FORMAT "X(40)":U
      FacCPedi.ImpTot FORMAT "->>,>>>,>>9.99":U
      x-Estado @ x-Estado COLUMN-LABEL "Estado" FORMAT "x(12)":U
            WIDTH 10.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     FILL-IN-FchPed-1 AT ROW 1 COL 30 COLON-ALIGNED WIDGET-ID 10
     FILL-IN-FchPed-2 AT ROW 1 COL 53 COLON-ALIGNED WIDGET-ID 12
     FILL-IN-ImpLin AT ROW 1 COL 95 COLON-ALIGNED WIDGET-ID 14
...
/* BROWSE-TAB br_table FILL-IN-ImpLin F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.FacCPedi,INTEGRAL.gn-ven OF INTEGRAL.FacCPedi,INTEGRAL.FacDPedi OF INTEGRAL.FacCPedi"
 ...
 AND FacCPedi.FchPed <= FILL-IN-FchPed-2"
     _Where[3]         = "(FILL-IN-ImpLin = 0 OR Facdpedi.ImpLin <= FILL-IN-ImpLin)"
     _FldNameList[1]   > INTEGRAL.FacCPedi.NroPed
"FacCPedi.NroPed" "Cotización" ? "character" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.CodCli" "Cliente" ? "character" ? ? ? ? ? ? no ? no no "12" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   = INTEGRAL.FacCPedi.NomCli
     _FldNameList[5]   = INTEGRAL.gn-ven.NomVen
     _FldNameList[6]   = INTEGRAL.FacCPedi.ImpTot
     _FldNameList[7]   > "_<CALC>"
"x-Estado @ x-Estado" "Estado" "x(12)" ? ? ? ? ? ? ? no ? no no "10.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FILL-IN-FchPed-1 IN FRAME F-Main /* FILTRAR POR FECHAS: Emitidos desde */
DO:
  IF {&SELF-NAME} = INPUT {&SELF-NAME} THEN RETURN.
  ASSIGN {&SELF-NAME}.
  /*RUN dispatch IN THIS-PROCEDURE ('open-query':U).*/
  RUN procesa-handle IN lh_handle ('browses').
END.
...
&Scoped-define SELF-NAME FILL-IN-ImpLin
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-ImpLin B-table-Win
ON LEAVE OF FILL-IN-ImpLin IN FRAME F-Main /* IMPORTES MENORES O IGUALES A */
DO:
    IF {&SELF-NAME} = INPUT {&SELF-NAME} THEN RETURN.
    ASSIGN {&SELF-NAME}.
    s-ImpLin = {&SELF-NAME}.
...
ON FIND OF faccpedi
DO:
    RUN vtagn/p-faccpedi-flgest (Faccpedi.flgest, Faccpedi.coddoc, OUTPUT x-Estado).
    /*
...
------------------------------------------------------------------------------*/
    ...
    MESSAGE 'Confirme la anulacion de la cotizacion' Faccpedi.nroped
...
IF AVAILABLE FacCPedi THEN RUN vtaexp/R-CotExpLib-1-1 (ROWID(FacCPedi)).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fAlarma B-table-Win 
FUNCTION fAlarma RETURNS CHARACTER
...
IF Faccpedi.imptot <= 0 THEN RETURN "".
FOR EACH facdpedi OF faccpedi NO-LOCK:
    IF (Facdpedi.implin / Faccpedi.imptot) * 100 > 10 THEN RETURN '> 10%'.
END.
