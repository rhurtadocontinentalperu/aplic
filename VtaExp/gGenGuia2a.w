...
FIND Vtatrack03 WHERE VtaTrack03.CodCia = s-codcia
    ...
    AND VtaTrack03.CodDoc = 'PED'
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE Vtatrack03 THEN DO:
    MESSAGE 'La división' Faccpedi.coddiv 'no tiene definido tracking'
        VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
...
DEFINE FRAME gDialog
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         FONT 4
         TITLE "Generando guia(s) interna(s) para Orden de Despacho..." WIDGET-ID 100.
...
ON WINDOW-CLOSE OF FRAME gDialog /* Generando guia(s) interna(s) para Orden de Despacho... */
DO:  
    ...
    DO TRANSACTION ON ERROR UNDO trloop, RETURN 'ADM-ERROR' ON STOP UNDO trloop, RETURN 'ADM-ERROR':
        ...
        FOR EACH FacDPedi OF FacCPedi WHERE FacDPedi.CanPed > 0,
            ...
            IF CcbDDocu.AftIgv THEN
                CcbDDocu.ImpIgv = CcbDDocu.ImpLin - ROUND(CcbDDocu.ImpLin / (1 + (FacCfgGn.PorIgv / 100)),4).
            IF CcbDDocu.AftIsc THEN
                    CcbDDocu.ImpIsc = ROUND(CcbDDocu.PreBas * CcbDDocu.CanDes * Almmmatg.PorIsc / 100,2).
            ASSIGN
