...
/*
    ...
    --------    ----------- --------------------------------------------
    MLR-1       22/Set/2008 Habilitación de los campos cb-cmov.NroTra
                (Número de Detracción) y cb-cmov.FchMov (Fecha de
                Detracción). Llama a la subrutina cja/p-updast.p la
                cual actualiza la provisión de compras con los datos
                mencionados.
    ML02        10/Dic/2009 Quita valor por defecto del campo cb-cmov.CodDoc.
                Habilita lookup para Códigos de Documento en registros
                cuyo año sea 2010 en adelante.
    ML03        19/Dic/2009 Manejo de comprobantes de Retención.
*/
...
DEFINE VARIABLE cOldGloAst AS CHARACTER NO-UNDO.
...
/*MLR-1*/ DEFINE VARIABLE old-NroTra AS CHARACTER NO-UNDO.
/*MLR-1*/ DEFINE VARIABLE old-FchMod AS DATE NO-UNDO.
...
cb-cmov.FchAst cb-cmov.NroTra cb-cmov.FchMod cb-cmov.CodDoc cb-cmov.NroChq ~
cb-cmov.TpoCmb cb-cmov.C-FCaja cb-cmov.Usuario cb-cmov.ImpChq 
...
cb-cmov.NotAst cb-cmov.FchAst cb-cmov.NroTra cb-cmov.FchMod cb-cmov.CodDoc ~
cb-cmov.NroChq cb-cmov.TpoCmb cb-cmov.C-FCaja 
...
DEFINE FRAME F-maestro
     ...
     cb-cmov.CtaCja AT ROW 3.96 COL 9 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 8 BY .69
     x-NomCta AT ROW 3.96 COL 17 COLON-ALIGNED NO-LABEL
     cb-cmov.CodAux AT ROW 4.77 COL 9 COLON-ALIGNED
          LABEL "Auxiliar"
          VIEW-AS FILL-IN 
          SIZE 10 BY .69
     ...
     cb-cmov.FchAst AT ROW 2.35 COL 23 COLON-ALIGNED FORMAT "99/99/99"
          VIEW-AS FILL-IN 
          SIZE 8 BY .69
     cb-cmov.NroTra AT ROW 2.35 COL 43 COLON-ALIGNED
          LABEL "Nro Detracción" FORMAT "X(10)"
          VIEW-AS FILL-IN 
          SIZE 10.14 BY .69
     cb-cmov.FchMod AT ROW 2.35 COL 60 COLON-ALIGNED
          LABEL "Fecha" FORMAT "99/99/99"
          VIEW-AS FILL-IN 
          SIZE 8 BY .69
     cb-cmov.CodDoc AT ROW 4.77 COL 26 COLON-ALIGNED
          LABEL "Doc" FORMAT "X(3)"
          VIEW-AS FILL-IN 
          SIZE 6 BY .69
     cb-cmov.NroChq AT ROW 4.77 COL 38 COLON-ALIGNED
          LABEL "Nro"
          VIEW-AS FILL-IN 
          SIZE 15 BY .69
     cb-cmov.TpoCmb AT ROW 2.35 COL 77 COLON-ALIGNED
          LABEL "T. Cambio"
          VIEW-AS FILL-IN 
          SIZE 9 BY .69
     Moneda AT ROW 3.42 COL 77 COLON-ALIGNED
     F-SdoCta AT ROW 4.23 COL 77 COLON-ALIGNED
     cb-cmov.C-FCaja AT ROW 5.04 COL 77 COLON-ALIGNED
          LABEL "Con. Flj. Caja"
          VIEW-AS FILL-IN 
          SIZE 10.14 BY .69
...
/* SETTINGS FOR FRAME F-maestro
   UNDERLINE Custom                                                     */
/* BROWSE-TAB BRW-DETALLE CodAux F-maestro */
...
/* SETTINGS FOR FILL-IN cb-cmov.FchAst IN FRAME F-maestro
   EXP-FORMAT                                                           */
/* SETTINGS FOR FILL-IN cb-cmov.FchMod IN FRAME F-maestro
   EXP-LABEL EXP-FORMAT                                                 */
...
DO:
    ...
    DISABLE  cb-cmov.Fchast 
             ...
             cb-cmov.Nrochq 
             cb-cmov.Tpocmb
/*MLR-1*/    cb-cmov.nrotra
/*MLR-1*/    cb-cmov.fchmod
...
DO:
  ...
  RUN cja\d-provpe.r.
/*ML03*/ RUN proc_verifica-retencion.
  FOR EACH RMOV:
...
DO:
    ...
    /*ML02* Inicio de bloque ***/
    IF YEAR(INPUT cb-cmov.FchAst) > 2009 AND
        NOT CAN-FIND(FIRST cb-tabl WHERE
        cb-tabl.Tabla = "S01" AND
        cb-tabl.Codigo = cb-cmov.CodDoc:SCREEN-VALUE) THEN DO:
        MESSAGE
            "Tipo de Medio de Pago no Registrado"
            VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO cb-cmov.CodDoc IN FRAME F-Maestro.
        RETURN NO-APPLY.
    END.
    /*ML02* Fin de bloque ***/
...
/*MLR-1*/ IF cb-cmov.NroTra <> "" THEN RUN cja/p-updast(RECID(cb-cmov)).
...
/*MLR-1*/ IF cb-cmov.NroTra <> old-NroTra OR
/*MLR-1*/   cb-cmov.FchMod <> old-FchMod THEN RUN cja/p-updast(RECID(cb-cmov)).
...
/*MLR-1*/ old-NroTra = cb-cmov.NroTra.
/*MLR-1*/ old-FchMod = cb-cmov.FchMod.
...
*MLR* 25/04/08 ***/
/*MLR*/ IF INPUT FRAME F-maestro cb-cmov.tpocmb <= 0 and cb-ctas.codmon = 2 THEN DO:
...
DO:
    ...
    DO ON ENDKEY UNDO, LEAVE ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
        ...
        ASSIGN 
            cb-dmov.CodMon = x-CodMon
            /*cb-dmov.FchDoc = INPUT cb-cmov.fchast*/.
        IF cb-dmov.TpoCmb = 0 THEN cb-dmov.TpoCmb = INPUT cb-cmov.TpoCmb.
...
/*ML02* Inicio de bloque ***/
OR MOUSE-SELECT-DBLCLICK OF cb-cmov.CodDoc DO:

    IF INPUT cb-cmov.FchAst = ? THEN DO:
        MESSAGE
            "Debe ingresar la Fecha"
            VIEW-AS ALERT-BOX ERROR.
        APPLY 'Entry':U TO cb-cmov.CodDoc.
    END.

    IF YEAR(INPUT cb-cmov.FchAst) < 2010 THEN
        RUN cbd/q-clfaux.w("02", OUTPUT RECID-stack).
    ELSE
        RUN cbd/q-clfaux.w("S01", OUTPUT RECID-stack).
/*ML02* Fin de bloque ***/
    ...
    IF RECID-stack <> 0 THEN DO:
        ...
        END.
        ELSE MESSAGE "No existen Registros de Tipos de Pago." VIEW-AS ALERT-BOX ERROR
            BUTTONS OK.
    END.
...
DO:
    ...
    IF LOOKUP(LAST-EVENT:FUNCTION, "ENDKEY,ERROR,END-ERROR,CHOOSE") <> 0 THEN RETURN.
    ...
    THEN DO:
        ...
        THEN DO:
            ...
            CASE integral.cb-ctas.codmon:
                ...
                WHEN 2 THEN moneda:SCREEN-VALUE = "Dólares".
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE cb-cmov THEN 
    DISPLAY cb-cmov.NroAst cb-cmov.CodDiv cb-cmov.CtaCja cb-cmov.CodAux 
          ...
          cb-cmov.NroTra cb-cmov.FchMod cb-cmov.CodDoc cb-cmov.NroChq 
          cb-cmov.TpoCmb cb-cmov.C-FCaja cb-cmov.Usuario cb-cmov.ImpChq 
      WITH FRAME F-maestro IN WINDOW W-Maestro.
  ENABLE C-CodOpe cb-cmov.NroAst cb-cmov.CodDiv cb-cmov.CtaCja cb-cmov.CodAux 
         BRW-DETALLE cb-cmov.NroVou cb-cmov.Girado cb-cmov.NotAst 
         cb-cmov.FchAst cb-cmov.NroTra cb-cmov.FchMod cb-cmov.CodDoc 
         cb-cmov.NroChq cb-cmov.TpoCmb cb-cmov.C-FCaja B-impresoras RECT-2 
      WITH FRAME F-maestro IN WINDOW W-Maestro.
...
/*ML03*/      cb-dmov.Chr_01  = RMOV.Chr_01
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE lEsAgente       AS LOGICAL NO-UNDO.
    DEFINE VARIABLE lEsPrimero      AS LOGICAL NO-UNDO.
    DEFINE VARIABLE lTieneIGV       AS LOGICAL NO-UNDO.
    DEFINE VARIABLE dMontoMinimo    AS DECIMAL NO-UNDO.
    DEFINE VARIABLE dMontoAcumula   AS DECIMAL NO-UNDO.
    ...
    DEFINE VARIABLE iPeriodo        AS INTEGER NO-UNDO.
    DEFINE VARIABLE iNroMes         AS INTEGER NO-UNDO.

    DEFINE BUFFER b_RMOV FOR RMOV.
    DEFINE BUFFER b_cb-cmov FOR cb-cmov.
    DEFINE BUFFER b_cb-dmov FOR cb-dmov.

/*ML02* Criterio para creación de Retenciones ***
    1. Que no sea agente de retención.
    2. Que no sea buen contribuyente.
    3. Que no sea agente de percepción.
    4. Que la provisión no esté afecta a detracción % de det (op. 060).
    5. Que no sea servicio público (Tipo de Doc 14).
    6. Que la operación esté gravada.
    7. Que la suma de los comprobantes sea > que Monto Mínimo (ver cb-tabl.digitos).
*ML02* ***/
    ...
    END.
    ASSIGN
        cCodCta = cb-tabl.CodCta
        dPorRetencion = DECIMAL(cb-tabl.Codigo)
        dMontoMinimo = DECIMAL(cb-tabl.Digitos) NO-ERROR.

    IF dPorRetencion = 0 OR dPorRetencion = ? THEN DO:
        MESSAGE
            "Porcentaje de Retención es CERO." SKIP
            "No se calcularán las Retenciones"
            VIEW-AS ALERT-BOX ERROR.
        RETURN.
    END.
    ...
    loopr:
    FOR EACH RMOV:
        IF NOT lEsPrimero THEN DO:
            lEsPrimero = TRUE.
            FOR FIRST gn-prov
                FIELDS (gn-prov.codcia gn-prov.codpro gn-prov.NomPro
                    gn-prov.Libre_C01 gn-prov.Libre_C02 gn-prov.Libre_C03)
                WHERE gn-prov.codcia = pv-codcia
                AND gn-prov.codpro = RMOV.codaux
                NO-LOCK:
            END.
            ...
            IF lEsAgente THEN LEAVE loopr.
        END.
        IF NUM-ENTRIES(RMOV.Nroref,"-") > 1 THEN DO:
            cCodOpe = ENTRY(1,RMOV.Nroref,"-").
            cNroAst = ENTRY(2,RMOV.Nroref,"-").
        END.
        ELSE DO:
            cNroAst = "".
            cCodOpe = "".
        ...
        /* Si es Asiento de Apertura */
        IF RMOV.NroMes = 0 AND INTEGER(cCodOpe) = 0 THEN DO:
            FOR EACH b_cb-dmov 
                FIELDS ( b_cb-dmov.CodCia b_cb-dmov.Periodo b_cb-dmov.NroMes
                    b_cb-dmov.CodOpe b_cb-dmov.NroAst b_cb-dmov.CodCta b_cb-dmov.CodAux
                    b_cb-dmov.CodDoc b_cb-dmov.NroDoc )
                USE-INDEX DMOV02
                WHERE b_cb-dmov.CodCia = RMOV.CodCia
                AND b_cb-dmov.Periodo = ( RMOV.Periodo - 1 )    /* Se restringe al año pasado */
                AND b_cb-dmov.NroMes > 00
                AND b_cb-dmov.CodCta = RMOV.CodCta
                AND b_cb-dmov.CodAux = RMOV.CodAux
                AND b_cb-dmov.CodDoc = RMOV.CodDoc
                AND b_cb-dmov.NroDoc = RMOV.NroDoc
                NO-LOCK,
                FIRST Cp-TPro NO-LOCK WHERE Cp-Tpro.CodDoc = b_cb-dmov.CodDoc
                AND Cp-Tpro.CodCta = b_cb-dmov.CodCta
                AND CP-Tpro.CodOpe = b_cb-dmov.CodOpe
                AND Cp-Tpro.Correlativo = YES:
                ASSIGN
                    iPeriodo = b_cb-dmov.Periodo
                    iNroMes = b_cb-dmov.NroMes
                    cCodOpe = b_cb-dmov.CodOpe
                    cNroAst = b_cb-dmov.NroAst.
            END.
        END.
        ELSE DO:
            iPeriodo = RMOV.Periodo.
            iNroMes = RMOV.NroMes.
        END.

        IF cNroAst = "" THEN NEXT.
        IF RMOV.CodDoc = "14" THEN NEXT.    /* Servicio */
        /* Busca Cabecera */
        FOR FIRST b_cb-cmov
            FIELDS (b_cb-cmov.codcia b_cb-cmov.periodo b_cb-cmov.nromes
                b_cb-cmov.CodOpe b_cb-cmov.NroAst b_cb-cmov.PorDet)
            WHERE b_cb-cmov.CodCia = RMOV.CodCia
            AND b_cb-cmov.Periodo = iPeriodo
            AND b_cb-cmov.NroMes = iNroMes
            AND b_cb-cmov.CodOpe = cCodOpe
            AND b_cb-cmov.NroAst = cNroAst
            NO-LOCK:
        END.

        IF NOT AVAILABLE b_cb-cmov THEN NEXT.
        IF b_cb-cmov.PorDet <> 0 THEN NEXT. /* Detracción */
        lTieneIGV = FALSE.
        FOR EACH b_cb-dmov
            FIELDS ( b_cb-dmov.CodCia b_cb-dmov.Periodo b_cb-dmov.NroMes
                b_cb-dmov.CodOpe b_cb-dmov.NroAst b_cb-dmov.TM b_cb-dmov.ImpMn1 )
            WHERE b_cb-dmov.CodCia = RMOV.CodCia
            AND b_cb-dmov.Periodo = iPeriodo
            AND b_cb-dmov.NroMes = iNroMes
            AND b_cb-dmov.CodOpe = cCodOpe
            AND b_cb-dmov.NroAst = cNroAst
            NO-LOCK:
            IF b_cb-dmov.TM = 06 THEN lTieneIGV = TRUE.
        END.
        IF NOT lTieneIGV THEN NEXT. /* Sin IGV */

        IF RMOV.tpomov THEN dMontoAcumula = dMontoAcumula - RMOV.ImpMn1.
        ELSE dMontoAcumula = dMontoAcumula + RMOV.ImpMn1.

        ASSIGN RMOV.Chr_10 = "Ok_procesa" + RMOV.Chr_10.
    END.

    IF NOT lEsAgente THEN DO:
        IF dMontoAcumula > dMontoMinimo THEN DO:
            MESSAGE
                "¿Desea Generar Retención para" cNomPro "?"
                VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
            ...
            IF dMontoAcumula > dMontoMinimo THEN DO:
                ...
                BUFFER-COPY RMOV 
                    ...
                    ASSIGN
                        ...
                        b_RMOV.ImpMn1 = ROUND((RMOV.ImpMn1 * dPorRetencion / 100),2)
                        b_RMOV.ImpMn2 = ROUND((RMOV.ImpMn2 * dPorRetencion / 100),2)
                        b_RMOV.Chr_01 = STRING(faccorre.nroser, "999") + STRING(faccorre.correlativo, "999999").
/*ML01* Inicio de bloque 20/Mar/2010
        Guarda el importe en Soles al tipo de cambio Pago */
                IF moneda <> "Soles" THEN DO WITH FRAME F-Maestro:
                    b_RMOV.ImpMn1 = ROUND(((RMOV.ImpMn2 * INPUT cb-cmov.Tpocmb) * dPorRetencion / 100),2).
                END.
/*ML01* Fin de bloque 20/Mar/2010*/
                /* RHC 16.03.2012 buscar la fecha de emision de la provisión */
                FOR EACH Cp-tpro NO-LOCK  WHERE 
                    Cp-tpro.CODCIA = cb-codcia AND
                    Cp-tpro.CORRELATIVO = YES AND   /* Proveedores */
                    Cp-tpro.CodDoc = RMOV.coddoc:
                    FOR EACH DETALLE NO-LOCK WHERE 
                        ...
                        DETALLE.CodAux  = RMOV.codaux   AND
                        DETALLE.CODDOC  = CP-TPRO.CODDOC AND
                        DETALLE.NroDoc  = RMOV.nrodoc:
                        IF DETALLE.FchDoc <> ? THEN b_RMOV.FchDoc = DETALLE.FchDoc.
                    END.
                END.
                /* ********************************************************* */
            END.
...
/* calculo de cuenta 104?????  */
    ...
    FOR EACH detalle WHERE detalle.codcia = s-codcia AND
        ...
        IF LAST-OF (DETALLE.CodDiv) THEN DO:
           ...
           cb-dmov.CodMon  = x-CodMon
           cb-dmov.TpoCmb  = INPUT cb-cmov.TpoCmb
           cb-dmov.FchDoc  = INPUT cb-cmov.FchAst   /* ¿por qué estaba desactivado? */
    ...
    /* RHC 25.06.07 EN CASO DE CHEQUES SIN IMPORTE (ANULADOS) */
    IF t-TotalCh1 + t-TotalCh2 = 0 THEN DO:
        CREATE cb-dmov.
        ASSIGN 
           cb-dmov.CodCia  = s-codcia
           cb-dmov.Periodo = s-periodo
           ...
           cb-dmov.ImpMn2  = ABS(x-totalch2)
           cb-dmov.CodDoc  = INPUT cb-cmov.CODDOC
           cb-dmov.CODDIV  = s-coddiv
           cb-dmov.nrodoc  = cb-cmov.nrochq:SCREEN-VALUE
           cb-dmov.TpoItm  = "B".
    END.
