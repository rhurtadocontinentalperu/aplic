...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BRW-DETALLE W-Maestro _STRUCTURED
  QUERY BRW-DETALLE NO-LOCK DISPLAY
      ...
      cb-dmov.Codaux COLUMN-LABEL "Código!auxiliar" FORMAT "x(13)"
...
"cb-dmov.Codaux" "Código!auxiliar" "x(13)" "character" ? ? ? ? ? ? no ?
...
DO:
    ...
    IF CB-CMOV.FLGEST = "A" THEN DO:
       ...
       RETURN NO-APPLY.
    END.
    IF CB-CMOV.FchAst <> TODAY THEN DO:
       MESSAGE "Registro no puede ser anulado, es de otro dia"
       VIEW-AS ALERT-BOX INFORMA.
       RETURN NO-APPLY.
    END.
...
DO:
    ...
    END.
    IF CB-CMOV.FchAst <> TODAY THEN DO:
       MESSAGE "Registro no puede ser modificado, es de otro dia"
       VIEW-AS ALERT-BOX INFORMA.
       RETURN NO-APPLY.
    END.
