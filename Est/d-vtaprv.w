...
DEFINE TEMP-TABLE tmp-tempo NO-UNDO
    ...
    FIELD t-stkact  LIKE Almmmate.StkAct    FORMAT "->>>>>,>>9.99" EXTENT 15
...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 BUTTON-2 BUTTON-3 BUTTON-4 BUTTON-5 ~
...
DEFINE FRAME F-Main
     ...
     " Moneda" VIEW-AS TEXT
          ...
          FONT 6
     "Almacenes (Se recomienda hasta 10)" VIEW-AS TEXT
...
ON END-ERROR OF W-Win /* Ventas por Proveedor */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Ventas por Proveedor */
DO:
...
DEFINE VAR lQAlmacenes AS INTEGER.

X-DIA = HastaF - DesdeF.

lQAlmacenes = NUM-ENTRIES(F-Almacen).
...
FOR EACH tmp-Tempo:
    DO I = 1 TO NUM-ENTRIES(F-Almacen) :
        ...
        IF AVAIL almmmate THEN DO: 
            ...
            t-stkact[lQAlmacenes + 1] = t-stkact[lQAlmacenes + 1] + Almmmate.StkAct.
        END.
    END.
...
DO x-Item = 1 TO NUM-ENTRIES(x-CodDiv):
    FOR EACH VentasxProducto NO-LOCK WHERE VentasxProducto.coddiv = ENTRY(x-item, x-coddiv)
        AND (VentasxProducto.codmat >= DesdeC AND VentasxProducto.CodMat <= HastaC)
        AND VentasxProducto.DateKey >= DesdeF
        AND VentasxProducto.DateKey <= HastaF,
        FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
        AND Almmmatg.codmat = VentasxProducto.codmat 
        ...
        DISPLAY VentasxProducto.CodMat @ Fi-Mensaje LABEL "Codigo de Articulo "
        ...
        ASSIGN
            ...
            F-Salida  = 0.
        ASSIGN
            F-Salida = VentasxProducto.Cantidad
            T-VtaMn  = VentasxProducto.ImpNacCIGV
            T-VtaMe  = VentasxProducto.ImpExtCIGV
            T-CtoMn  = VentasxProducto.CostoNacCIGV
            T-CtoMe  = VentasxProducto.CostoExtCIGV.
        FIND tmp-tempo WHERE t-codcia  = S-CODCIA 
            AND t-codmat  = VentasxProducto.codmat
            NO-ERROR.
        IF NOT AVAIL tmp-tempo THEN DO:
            ...
            ASSIGN 
                ...
                t-codmat  = VentasxProducto.codmat
                t-desmat  = Almmmatg.DesMat
                t-desmar  = Almmmatg.DesMar
                t-undbas  = Almmmatg.UndStk
        ...
        X-ENTRA = FALSE.
        IF F-DIVISION-1 <> "" AND VentasxProducto.Coddiv = F-DIVISION-1 THEN DO:
            ASSIGN 
                ...
                X-ENTRA = TRUE.
        END.       
        IF F-DIVISION-2 <> "" AND VentasxProducto.Coddiv = F-DIVISION-2 THEN DO:
            ASSIGN 
                ...
                X-ENTRA = TRUE.
        END.       
        IF F-DIVISION-3 <> "" AND VentasxProducto.Coddiv = F-DIVISION-3 THEN DO:
            ASSIGN 
                ...
                X-ENTRA = TRUE.
        END.
        IF F-DIVISION-4 <> "" AND VentasxProducto.Coddiv = F-DIVISION-4 THEN DO:
            ASSIGN 
                ...
                X-ENTRA = TRUE.
        END.
        IF F-DIVISION-5 <> "" AND VentasxProducto.Coddiv = F-DIVISION-5 THEN DO:
            ASSIGN 
                ...
                X-ENTRA = TRUE.
        END.
        IF F-DIVISION-6 <> "" AND VentasxProducto.Coddiv = F-DIVISION-6 THEN DO:
            ASSIGN 
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-1 BUTTON-2 BUTTON-3 BUTTON-4 BUTTON-5 F-DIVISION F-CodFam 
         F-SubFam F-Marca F-prov1 DesdeC HastaC DesdeF HastaF F-Almacen T-dias 
         DIASINI DIASFIN F-DIVISION-1 F-DIVISION-2 F-DIVISION-3 F-DIVISION-4 
...
DEFINE VARIABLE lQAlmacenes AS INT.
...
chWorkSheet:Range("L11"):Value = "Total".
...
IF F-DIVISION-1 <> "" THEN chWorkSheet:Range("E12"):Value = "Div.(" + F-DIVISION-1 + ")" .
IF F-DIVISION-2 <> "" THEN chWorkSheet:Range("F12"):Value = "Div.(" + F-DIVISION-2 + ")" .
IF F-DIVISION-3 <> "" THEN chWorkSheet:Range("G12"):Value = "Div.(" + F-DIVISION-3 + ")" .
IF F-DIVISION-4 <> "" THEN chWorkSheet:Range("H12"):Value = "Div.(" + F-DIVISION-4 + ")" .
IF F-DIVISION-5 <> "" THEN chWorkSheet:Range("I12"):Value = "Div.(" + F-DIVISION-5 + ")" .
IF F-DIVISION-6 <> "" THEN chWorkSheet:Range("J12"):Value = "Div.(" + F-DIVISION-6 + ")" .
chWorkSheet:Range("K12"):Value = "Total Cantidad".
chWorkSheet:Range("L12"):Value = "Importe".
...
FOR EACH tmp-tempo BREAK BY t-codcia
    ...
    cRange = "I" + cColumn.
    chWorkSheet:Range(cRange):Value = t-canti[5].
    cRange = "J" + cColumn.
    chWorkSheet:Range(cRange):Value = t-canti[6].
    cRange = "K" + cColumn.
    ...
    chWorkSheet:Range(cRange):Value = t-stkact[lQAlmacenes + 1].
...
lQAlmacenes = NUM-ENTRIES(F-Almacen).
    ...
    DEFINE FRAME F-REPORTE
        ...
        t-stkact[lQAlmacenes + 1]  FORMAT "->>>>>>9.99"
        t-nro_dia    FORMAT ">>>9"
