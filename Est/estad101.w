...
FOR EACH dwh_ventas_vend NO-LOCK WHERE dwh_ventas_vend.codcia = s-codcia
    ...
    END.
     ...
     ASSIGN
         ...
         /*Detalle.CanxMes   = Detalle.CanxMes   + dwh_ventas_vend.Cantidad*/
         Detalle.VtaxMesMe = Detalle.VtaxMesMe + dwh_ventas_vend.ImpExtCIGV
         Detalle.VtaxMesMn = Detalle.VtaxMesMn + dwh_ventas_vend.ImpNacCIGV
         Detalle.CtoxMesMe = Detalle.CtoxMesMe + dwh_ventas_vend.CostoExtCIGV
         Detalle.CtoxMesMn = Detalle.CtoxMesMn + dwh_ventas_vend.CostoNacCIGV
         Detalle.ProxMesMe = Detalle.ProxMesMe + dwh_ventas_vend.PromExtCIGV
         Detalle.ProxMesMn = Detalle.ProxMesMn + dwh_ventas_vend.PromNacCIGV.
END.
...
FOR EACH dwh_ventas_resmat NO-LOCK WHERE dwh_ventas_resmat.codcia = s-codcia
    ...
    IF TOGGLE-CodMat = YES THEN DO:
        IF TOGGLE-Resumen-Linea = YES THEN DO:
            IF x-Llave = '' THEN x-Llave = dwh_ventas_resmat.codfam + '|'.
            ELSE x-Llave = x-Llave + dwh_ventas_resmat.codfam + '|'.
            x-Llave = x-Llave + dwh_ventas_resmat.subfam + '|'.
            ASSIGN
                cLinea = dwh_ventas_resmat.codfam + ' ' + dwh_Linea.NomFam 
                cSublinea = dwh_ventas_resmat.subfam + ' ' + dwh_Lineas.NomSubFam
                lOptions = lOptions + (IF SUBSTRING(lOptions, LENGTH(lOptions), 1) = ':' THEN '' ELSE ',') +
                           'Linea,Sublinea'.
        ...
        IF TOGGLE-Resumen-Marca = YES THEN DO:
            ...
            ASSIGN
                cMarca = dwh_ventas_resmat.desmar
                lOptions = lOptions + (IF SUBSTRING(lOptions, LENGTH(lOptions), 1) = ':' THEN '' ELSE ',') +
                           'Marca'.
        END.
        x-Llave = x-Llave + dwh_ventas_resmat.licencia + '|'.
        x-Llave = x-Llave + dwh_ventas_resmat.codpro + '|'.
        ...
        ASSIGN
            cLicencia = dwh_ventas_resmat.licencia + (IF AVAILABLE dwh_Licencia THEN ' ' + dwh_Licencia.Descripcion ELSE '')
            cProveedor = dwh_ventas_resmat.codpro + (IF AVAILABLE dwh_Proveedor THEN ' ' + dwh_Proveedor.NomPro ELSE '')
            lOptions = lOptions + (IF SUBSTRING(lOptions, LENGTH(lOptions), 1) = ':' THEN '' ELSE ',') +
                        'Licencia,Proveedor'.
   END.
   /* ******************************************** */
   IF TOGGLE-CodVen = YES THEN DO:
       ...
       ASSIGN
           cVendedor = dwh_ventas_resmat.codven + (IF AVAILABLE dwh_Vendedor THEN ' ' + dwh_Vendedor.NomVen ELSE '')
           lOptions = lOptions + (IF SUBSTRING(lOptions, LENGTH(lOptions), 1) = ':' THEN '' ELSE ',') +
                         'Vendedor'.
      END.
      /* ******************************************** */
      FIND Detalle WHERE Detalle.llave = x-Llave NO-ERROR.
        ...
        ASSIGN
         ...
         Detalle.Tipo = cTipo
         Detalle.Vendedor = cVendedor
         /*Detalle.CanxMes   = Detalle.CanxMes   + dwh_ventas_resmat.Cantidad*/
         Detalle.VtaxMesMe = Detalle.VtaxMesMe + dwh_ventas_resmat.ImpExtCIGV
         Detalle.VtaxMesMn = Detalle.VtaxMesMn + dwh_ventas_resmat.ImpNacCIGV
         Detalle.CtoxMesMe = Detalle.CtoxMesMe + dwh_ventas_resmat.CostoExtCIGV
         Detalle.CtoxMesMn = Detalle.CtoxMesMn + dwh_ventas_resmat.CostoNacCIGV
         Detalle.ProxMesMe = Detalle.ProxMesMe + dwh_ventas_resmat.PromExtCIGV
         Detalle.ProxMesMn = Detalle.ProxMesMn + dwh_ventas_resmat.PromNacCIGV.
END.
...
FOR EACH dwh_ventas_mat NO-LOCK WHERE dwh_ventas_mat.codcia = s-codcia
    ...
    IF TOGGLE-CodMat = YES THEN DO:
     ...
     ASSIGN
         ...
         Detalle.Tipo = cTipo
         Detalle.Vendedor = cVendedor
         Detalle.CanxMes   = Detalle.CanxMes   + dwh_ventas_mat.Cantidad
         Detalle.VtaxMesMe = Detalle.VtaxMesMe + dwh_ventas_mat.ImpExtCIGV
         Detalle.VtaxMesMn = Detalle.VtaxMesMn + dwh_ventas_mat.ImpNacCIGV
         Detalle.CtoxMesMe = Detalle.CtoxMesMe + dwh_ventas_mat.CostoExtCIGV
         Detalle.CtoxMesMn = Detalle.CtoxMesMn + dwh_ventas_mat.CostoNacCIGV
         Detalle.ProxMesMe = Detalle.ProxMesMe + dwh_ventas_mat.PromExtCIGV
         Detalle.ProxMesMn = Detalle.ProxMesMn + dwh_ventas_mat.PromNacCIGV.
END.
