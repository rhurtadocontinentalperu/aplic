...
------------------------------------------------------------------------------*/
    ...
    FOR EACH tt-datos:
        FIND Almmmatg WHERE Almmmatg.codcia = s-codcia
            AND Almmmatg.codmat = tt-datos.tt-codigo
            NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    DEF VAR x-linea LIKE gn-clie.codcli.
...
/*     END.                                  */
    ...
    FOR EACH tt-cliente:
        FIND gn-clie WHERE gn-clie.codcia = cl-codcia
            AND gn-clie.codcli = tt-cliente.tt-codcli
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE gn-clie THEN DELETE tt-cliente.
    END.
...
/* CARGAMOS EL EXCEL */
RUN lib/filetext-to-excel(x-Archivo, 'Detallado', NO).
...
FOR EACH dwh_ventas_vend NO-LOCK WHERE dwh_ventas_vend.codcia = s-codcia
    ...
    IF TOGGLE-CodDiv THEN DO:
         ...
         FIND gn-divi WHERE gn-divi.codcia = s-codcia
             AND gn-divi.coddiv = dwh_ventas_vend.coddiv
             NO-LOCK NO-ERROR.
...
FOR EACH dwh_ventas_mat NO-LOCK WHERE dwh_ventas_mat.codcia = s-codcia
    ...
    FIRST Almmmatg OF dwh_ventas_mat NO-LOCK WHERE Almmmatg.codfam BEGINS x-CodFam
    AND Almmmatg.CodPr1 BEGINS x-CodPro
    AND Almmmatg.subfam BEGINS x-SubFam,
    FIRST dwh_tiempo WHERE dwh_tiempo.codcia = s-codcia
    AND dwh_tiempo.fecha = dwh_ventas_mat.fecha NO-LOCK:
    IF x-CuentaReg = 0 OR ( x-CuentaReg MODULO x-MuestraReg ) = 0 THEN
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** TABLA POR PRODUCTO ' + 
        ' FECHA ' + STRING (dwh_Tiempo.NroDia, '99') +
        ' ' + dwh_Tiempo.NombreMes + ' ' + STRING (dwh_Tiempo.Periodo, '9999') +
        ' DIVISION ' + coddiv + ' PRODUCTO ' + dwh_ventas_mat.codmat + ' **'.
    x-CuentaReg = x-CuentaReg + 1.
    /* ARMAMOS LA LLAVE */
    x-Llave = ''.
    /* LLAVE INICIAL */
    ...
    IF TOGGLE-CodDiv THEN DO:
         ...
         FIND gn-divi WHERE gn-divi.codcia = s-codcia
             AND gn-divi.coddiv = dwh_ventas_mat.coddiv
             NO-LOCK NO-ERROR.
         IF AVAILABLE gn-divi THEN x-Llave = x-Llave + gn-divi.CanalVenta + '|'.
         ELSE x-Llave = x-Llave + '|'.
    END.
...
FOR EACH dwh_ventas_resmat NO-LOCK WHERE dwh_ventas_resmat.codcia = s-codcia
    ...
    IF TOGGLE-CodDiv THEN DO:
         ...
         FIND gn-divi WHERE gn-divi.codcia = s-codcia
             AND gn-divi.coddiv = dwh_ventas_resmat.coddiv
             NO-LOCK NO-ERROR.
         IF AVAILABLE gn-divi THEN x-Llave = x-Llave + gn-divi.CanalVenta + '|'.
         ELSE x-Llave = x-Llave + '|'.
    END.
