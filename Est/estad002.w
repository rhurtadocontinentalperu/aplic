...
FOR EACH EcpPvArti NO-LOCK WHERE EcpPvArti.CodCia = s-codcia
    AND EcpPvArti.Nrofch >= x-Fecha-1
    AND EcpPvArti.Nrofch <= x-Fecha-2,
    FIRST Almmmatg OF EcpPvArti NO-LOCK WHERE Almmmatg.codfam BEGINS x-CodFam
    AND Almmmatg.CodPr1 BEGINS x-CodPro
    AND Almmmatg.subfam BEGINS x-SubFam:
    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ' ** VENTAS ' + STRING(EcpPvArti.codmat) + ' ' + STRING(EcpPvArti.nrofch).
    FIND detalle WHERE detalle.codcia = s-codcia
        AND detalle.codmat = EcpPvArti.codmat
        AND detalle.coddiv = EcpPvArti.coddiv
        NO-ERROR.
    IF NOT AVAILABLE detalle THEN CREATE detalle.
    ASSIGN
        detalle.CodCia = EcpPvArti.CodCia
        detalle.CodDiv = EcpPvArti.CodDiv 
        detalle.CodMat = EcpPvArti.codmat 
        detalle.CodPro = EcpPvArti.CodProv 
        detalle.CostoNacSIGV = detalle.CostoNacSIGV + EcpPvArti.CmpxMesMn.
END.
...
FOR EACH detalle:
    ...
    FIND gn-divi WHERE gn-divi.codcia = s-codcia
        AND gn-divi.coddiv = detalle.coddiv
        NO-LOCK NO-ERROR.
