...
FOR EACH dwh_ventas_cli NO-LOCK WHERE dwh_ventas_cli.codcia = s-codcia
    ...
    AND dwh_tiempo.fecha = dwh_ventas_cli.fecha NO-LOCK:
    ...
    IF x-Fecha >= DesdeF AND x-Fecha <= HastaF THEN DO:
        ASSIGN
            /*F-Salida[1]  = F-Salida[1]  + dwh_ventas_cli.Cantidad*/
            T-Vtamn[1]   = T-Vtamn[1]   + dwh_ventas_cli.ImpNacCIGV
            T-Vtame[1]   = T-Vtame[1]   + dwh_ventas_cli.ImpExtCIGV
            T-Ctomn[1]   = T-Ctomn[1]   + dwh_ventas_cli.CostoNacCIGV
            T-Ctome[1]   = T-Ctome[1]   + dwh_ventas_cli.CostoExtCIGV
            T-Promn[1]   = T-Promn[1]   + dwh_ventas_cli.PromNacCIGV
            T-Prome[1]   = T-Prome[1]   + dwh_ventas_cli.PromExtCIGV.
    END.
    /* ACUMULADO PERIODO ACTUAL */
    IF x-Fecha >= DATE(01,01,YEAR(DesdeF)) AND x-Fecha <= HastaF THEN DO:
        ASSIGN
            /*F-Salida[2]  = F-Salida[2]  + dwh_ventas_cli.Cantidad*/
            T-Vtamn[2]   = T-Vtamn[2]   + dwh_ventas_cli.ImpNacCIGV
            T-Vtame[2]   = T-Vtame[2]   + dwh_ventas_cli.ImpExtCIGV
            T-Ctomn[2]   = T-Ctomn[2]   + dwh_ventas_cli.CostoNacCIGV
            T-Ctome[2]   = T-Ctome[2]   + dwh_ventas_cli.CostoExtCIGV
            T-Promn[2]   = T-Promn[2]   + dwh_ventas_cli.PromNacCIGV
            T-Prome[2]   = T-Prome[2]   + dwh_ventas_cli.PromExtCIGV.
    END.
    /* PERIODO ANTERIOR */
    IF x-Fecha >= DATE(MONTH(DesdeF),DAY(DesdeF),YEAR(DesdeF) - 1) AND x-Fecha <= DATE(MONTH(HastaF),DAY(HastaF),YEAR(HastaF) - 1) THEN DO:
        ASSIGN
            /*F-Salida[3]  = F-Salida[3]  + dwh_ventas_cli.Cantidad*/
            T-Vtamn[3]   = T-Vtamn[3]   + dwh_ventas_cli.ImpNacCIGV
            T-Vtame[3]   = T-Vtame[3]   + dwh_ventas_cli.ImpExtCIGV
            T-Ctomn[3]   = T-Ctomn[3]   + dwh_ventas_cli.CostoNacCIGV
            T-Ctome[3]   = T-Ctome[3]   + dwh_ventas_cli.CostoExtCIGV
            T-Promn[3]   = T-Promn[3]   + dwh_ventas_cli.PromNacCIGV
            T-Prome[3]   = T-Prome[3]   + dwh_ventas_cli.PromExtCIGV.
    END.
    ...
    IF x-Fecha >= DATE(01,01,YEAR(DesdeF) - 1) AND x-Fecha <= DATE(MONTH(HastaF),DAY(HastaF),YEAR(HastaF) - 1) THEN DO:
        ASSIGN
            /*F-Salida[4]  = F-Salida[4]  + dwh_ventas_cli.Cantidad*/
            T-Vtamn[4]   = T-Vtamn[4]   + dwh_ventas_cli.ImpNacCIGV
            T-Vtame[4]   = T-Vtame[4]   + dwh_ventas_cli.ImpExtCIGV
            T-Ctomn[4]   = T-Ctomn[4]   + dwh_ventas_cli.CostoNacCIGV
            T-Ctome[4]   = T-Ctome[4]   + dwh_ventas_cli.CostoExtCIGV
            T-Promn[4]   = T-Promn[4]   + dwh_ventas_cli.PromNacCIGV
            T-Prome[4]   = T-Prome[4]   + dwh_ventas_cli.PromExtCIGV.
    END.
    /* LLAVE INICIAL */
    ...
    IF TOGGLE-CodDiv THEN DO:
         ...
         FIND gn-divi WHERE gn-divi.codcia = s-codcia
             AND gn-divi.coddiv = dwh_ventas_cli.coddiv
             NO-LOCK NO-ERROR.
    ...
    IF TOGGLE-CodCli = YES THEN DO:
        ...
        FIND gn-clie WHERE gn-clie.codcia = cl-codcia
            AND gn-clie.codcli = dwh_ventas_cli.codcli
            NO-LOCK NO-ERROR.
