...
FOR EACH dwh_ventas NO-LOCK WHERE dwh_ventas.codcia = s-codcia
    ...
    AND  (x-CodDiv = '' OR dwh_ventas.coddiv BEGINS x-CodDiv)
    AND dwh_ventas.codven BEGINS x-CodVen,
    FIRST dwh_Division OF dwh_ventas NO-LOCK,
    FIRST dwh_Vendedor OF dwh_ventas NO-LOCK,
    FIRST dwh_Cliente NO-LOCK WHERE dwh_Cliente.codcia = 0 AND dwh_Cliente.codcli = dwh_ventas.codcli,
    ...
    IF TOGGLE-CodDiv THEN DO:
         IF x-Llave = '' THEN x-Llave = dwh_ventas.coddiv + ' - ' + dwh_Division.DesDiv + '|'.
         ELSE x-Llave = x-LLave + dwh_ventas.coddiv + ' - ' + dwh_Division.DesDiv + '|'.
    ...
    IF TOGGLE-CodMat = YES THEN DO:
        IF (TOGGLE-Resumen-Linea = NO AND TOGGLE-Resumen-Marca = NO AND TOGGLE-Resumen-Solo-Linea = NO ) THEN DO:
             ...
             FIND T-MATG WHERE T-MATG.codcia = s-codcia
                 ...
                 NO-LOCK NO-ERROR.
             IF AVAILABLE T-MATG THEN x-Llave = x-Llave + dwh_Productos.CodAsoc + ' - ' + T-MATG.desmat + '|'.
             ELSE x-Llave = x-Llave + dwh_Productos.CodAsoc + '|'.
         END.
         ELSE DO:
