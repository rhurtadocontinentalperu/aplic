...
/* Ic - 2012-03-16*/
DEFINE VAR lTieneUsuarioDivision AS LOGICAL INIT FALSE.
...
DEF BUFFER T-MATG FOR dwh_Producto.
...
BUTTON-7 BtnDone TOGGLE-CodDiv TOGGLE-CodCli TOGGLE-CodMat TOGGLE-CodVen ~
...
DEFINE BUTTON BUTTON-7 
     ...
     LABEL "Button 7" 
     SIZE 6 BY 1.62 TOOLTIP "Migrar a Texto".
...
DEFINE FRAME fMain
     ...
     BUTTON-7 AT ROW 1 COL 112 WIDGET-ID 88
     ...
     TOGGLE-Resumen-solo-linea AT ROW 9.08 COL 81 WIDGET-ID 90
     COMBO-BOX-SubFam AT ROW 9.35 COL 29 COLON-ALIGNED WIDGET-ID 38
     TOGGLE-Resumen-Marca AT ROW 9.88 COL 81 WIDGET-ID 62
     FILL-IN-CodMat AT ROW 10.69 COL 29 COLON-ALIGNED WIDGET-ID 70
     FILL-IN-DesMat AT ROW 10.69 COL 44.14 COLON-ALIGNED NO-LABEL WIDGET-ID 72
     BUTTON-6 AT ROW 11.73 COL 108 WIDGET-ID 74
     FILL-IN-file-2 AT ROW 11.77 COL 29 COLON-ALIGNED NO-LABEL WIDGET-ID 76
     FILL-IN-CodPro AT ROW 12.85 COL 29 COLON-ALIGNED WIDGET-ID 40
     FILL-IN-NomPro AT ROW 12.85 COL 44 COLON-ALIGNED NO-LABEL WIDGET-ID 42
     TOGGLE-CodVen AT ROW 14.46 COL 11 WIDGET-ID 18
     FILL-IN-CodVen AT ROW 14.46 COL 29 COLON-ALIGNED NO-LABEL WIDGET-ID 44
     FILL-IN-NomVen AT ROW 14.46 COL 36 COLON-ALIGNED NO-LABEL WIDGET-ID 46
     RADIO-SET-Tipo AT ROW 16.08 COL 31 NO-LABEL WIDGET-ID 56
     DesdeF AT ROW 17.15 COL 29 COLON-ALIGNED WIDGET-ID 10
     HastaF AT ROW 17.15 COL 49 COLON-ALIGNED WIDGET-ID 12
     RECT-1 AT ROW 2.62 COL 8 WIDGET-ID 78
     RECT-2 AT ROW 4.5 COL 8 WIDGET-ID 80
     RECT-3 AT ROW 8 COL 8 WIDGET-ID 82
     RECT-4 AT ROW 14.19 COL 8 WIDGET-ID 84
     RECT-5 AT ROW 15.81 COL 8 WIDGET-ID 86
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 128.72 BY 18.46 WIDGET-ID 100.
...
DO:   
    IF lTieneUsuarioDivision = TRUE THEN DO:
        /* Si esta inscrito en la tabla de usuario-division de la DB estadisticas 
            Inabilitar el Check de Division y habilitar el DropDownList de division
        */
        DO WITH FRAME {&FRAME-NAME}:
            TOGGLE-CodDiv:CHECKED = YES.
            TOGGLE-CodDiv:ENABLED = NO.
            TOGGLE-CodDiv:SENSITIVE = FALSE.

            COMBO-BOX-CodDiv:ENABLED = TRUE.
            COMBO-BOX-CodDiv:SENSITIVE = TRUE.
        END.
    END.
END.
...
/* DAMOS VUELTAS POR LA DIVISION */
DEF VAR k AS INT NO-UNDO.
DEF VAR ListaDivisiones AS CHAR NO-UNDO.

ListaDivisiones = x-CodDiv.
IF x-CodDiv = "" THEN DO WITH FRAME {&FRAME-NAME}:
    ListaDivisiones = ''.
    DO k = 1 TO COMBO-BOX-CodDiv:NUM-ITEMS:
        IF ENTRY(k, COMBO-BOX-CodDiv:LIST-ITEMS) BEGINS 'Todos' THEN NEXT.
        IF ListaDivisiones = '' THEN ListaDivisiones = ENTRY(1, ENTRY(k, COMBO-BOX-CodDiv:LIST-ITEMS), ' - ').
        ELSE ListaDivisiones = ListaDivisiones + ',' + ENTRY(1, ENTRY(k, COMBO-BOX-CodDiv:LIST-ITEMS), ' - ').
    END.
END.

SESSION:SET-WAIT-STATE('GENERAL').
DO k = 1 TO NUM-ENTRIES(ListaDivisiones):
    x-CodDiv = ENTRY(k, ListaDivisiones).
    /* FILTROS */
    ...
    IF TOGGLE-CodCli = YES AND TOGGLE-CodMat = YES AND TOGGLE-CodVen = NO
        THEN DO:
        IF TOGGLE-Resumen-Linea = YES OR TOGGLE-Resumen-Marca = YES OR TOGGLE-Resumen-solo-linea = YES
        THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 RECT-2 RECT-3 RECT-4 RECT-5 BUTTON-1 BUTTON-7 BtnDone 
         TOGGLE-CodDiv TOGGLE-CodCli TOGGLE-CodMat TOGGLE-CodVen RADIO-SET-Tipo 
         DesdeF HastaF 
      WITH FRAME fMain IN WINDOW wWin.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Excel-old wWin 
PROCEDURE Excel-old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR lUnaVez AS LOGICAL INIT TRUE.
  ASSIGN
    DesdeF = TODAY - DAY(TODAY) + 1
    HastaF = TODAY.

 /* Ic - 2012-03-16 */  
  IF CAN-FIND( FIRST dwh_usuario_division WHERE dwh_usuario_division.codcia = s-codcia AND 
              dwh_usuario_division.USER-ID = s-user-id NO-LOCK) THEN DO :
      ASSIGN lTieneUsuarioDivision = TRUE.
  END.
  ELSE DO:
      ASSIGN lTieneUsuarioDivision = FALSE.     
  END.
  IF lTieneUsuarioDivision = FALSE THEN DO:
      /* Usuario no esta en la tabla Usuario-Division */
/*     DO WITH FRAME {&FRAME-NAME}:                 */
/*         COMBO-BOX-CodDiv:ADD-LAST('Todos').      */
/*         COMBO-BOX-CodDiv:SCREEN-VALUE = 'Todos'. */
/*     END.                                         */
    FOR EACH integral.gn-divi NO-LOCK WHERE integral.gn-divi.codcia = s-codcia:
        COMBO-BOX-CodDiv:ADD-LAST(integral.gn-divi.coddiv + ' - ' + integral.gn-divi.DesDiv) IN FRAME {&FRAME-NAME}.
    END.    
    
  END.
  ELSE DO:
      ASSIGN lUnaVez = TRUE.
      FOR EACH dwh_usuario_division NO-LOCK WHERE dwh_usuario_division.codcia = s-codcia AND dwh_usuario_division.USER-ID = s-user-id:
         FIND FIRST integral.gn-divi WHERE integral.gn-divi.codcia = s-codcia AND integral.gn-divi.coddiv = dwh_usuario_division.coddiv NO-LOCK NO-ERROR.
         IF AVAILABLE integral.gn-divi THEN DO:
            COMBO-BOX-CodDiv:ADD-LAST(integral.gn-divi.coddiv + ' - ' + integral.gn-divi.DesDiv) IN FRAME {&FRAME-NAME}.
            IF lUnaVez = TRUE THEN DO:
                COMBO-BOX-CodDiv:SCREEN-VALUE = integral.gn-divi.coddiv + ' - ' + integral.gn-divi.DesDiv.
                ASSIGN lUnaVez = TRUE.
            END.
         END.    
     END.
...
FOR EACH dwh_ventas_vend NO-LOCK WHERE dwh_ventas_vend.codcia = s-codcia
    ...
    ASSIGN
        ...
        tmp-detalle.ProxMesMe = tmp-detalle.ProxMesMe + dwh_ventas_vend.PromExtCIGV
        tmp-detalle.ProxMesMn = tmp-detalle.ProxMesMn + dwh_ventas_vend.PromNacCIGV.
END.
...
FOR EACH dwh_ventas_mat NO-LOCK WHERE dwh_ventas_mat.codcia = s-codcia
    ...
    END.
    IF TOGGLE-CodMat = YES THEN DO:
        IF (TOGGLE-Resumen-Linea = NO AND TOGGLE-Resumen-Solo-Linea = NO AND TOGGLE-Resumen-Marca = NO) THEN DO:
            IF x-Llave = '' THEN x-Llave = trim(dwh_ventas_mat.codmat) + ' - ' + dwh_Productos.DesMat + '|'.
            ELSE x-Llave = x-Llave + trim(dwh_ventas_mat.codmat) + ' - ' + dwh_Productos.DesMat + '|'.
            ...
            x-Llave = x-Llave + dwh_Productos.SubTipo + '|'.
            FIND T-MATG WHERE T-MATG.codcia = s-codcia
                AND T-MATG.codmat = dwh_Productos.CodAsoc
                NO-LOCK NO-ERROR.
            IF AVAILABLE T-MATG THEN x-Llave = x-Llave + trim(dwh_Productos.CodAsoc) + ' - ' + T-MATG.DesMat + '|'.
            ELSE x-Llave + dwh_Productos.CodAsoc + '|'.
         END.
         ELSE DO:
             IF TOGGLE-Resumen-Linea = YES THEN DO:
...
FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "** GENERANDO EL TEXTO **".
