...
DEF TEMP-TABLE detalle
    ...
    FIELD fmapgo LIKE faccpedi.fmapgo           LABEL 'Forma de Pago'
    ...
    FIELD FlagAut AS CHAR FORMAT 'x(20)' LABEL 'Línea de Crédito'
...
FILL-IN-NroPed RADIO-SET-1 BUTTON-1 BtnDone 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-Mensaje FILL-IN-Fecha-1 ~
FILL-IN-Fecha-2 FILL-IN-NroPed RADIO-SET-1 
...
DEFINE FRAME fMain
     ...
     FILL-IN-NroPed AT ROW 1.19 COL 36 COLON-ALIGNED WIDGET-ID 116
...
DO:
    ASSIGN
        FILL-IN-Fecha-1 FILL-IN-Fecha-2 RADIO-SET-1  FILL-IN-NroPed
        pOptions = "".
    /* CONSISTENCIA */
...
END.
/* Linea de Credito */
FOR EACH Detalle WHERE Detalle.codcli <> '', FIRST Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-codcia
    AND Faccpedi.coddoc = "COT"
    AND Faccpedi.nroped = Detalle.nroped:
    Detalle.FmaPgo = Faccpedi.FmaPgo.
    CASE Detalle.FlagAut:
        WHEN "" THEN Detalle.FlagAut = "Sin Autorizar".
        WHEN "A" THEN Detalle.FlagAut = "Autorizada".
        WHEN "R" THEN Detalle.FlagAut = "Rechazada".
    END CASE.
END.
...
CASE RADIO-SET-1:
    ...
    END.
    WHEN 2 THEN DO:
        lOptions = lOptions + 'coddiv,lisbas,nroped,fchped,fchven,fchent,nomcli,clfcli,desfam,dessub,implin,impfac,almsalida,flagaut'.
        FOR EACH detalle,
            FIRST gn-clie NO-LOCK WHERE gn-clie.codcia = cl-codcia
    ...
    END.
    WHEN 3 THEN DO:
        lOptions = lOptions + 'coddiv,lisbas,nroped,fchped,fchven,fchent,nomcli,clfcli,implin,impfac,almsalida,flagaut'.
        FOR EACH detalle,
            FIRST gn-clie NO-LOCK WHERE gn-clie.codcia = cl-codcia
            AND gn-clie.codcli = detalle.codcli:
            ASSIGN
    ...
    END.
    WHEN 5 THEN DO:
        lOptions = lOptions + 'coddiv,lisbas,despgo,nroped,fchped,fchven,fchent,nomven,nomcli,clfcli,desmat,desmar,desfam,dessub,subtipo,undbas,canped,implin,canate,impate,saldo,impsal,canfac,impfac,flgest,departamento,provincia,distrito,pesmat,volumen,almsalida,flagaut'.
        FOR EACH detalle,
            FIRST gn-clie NO-LOCK WHERE gn-clie.codcia = cl-codcia
            AND gn-clie.codcli = detalle.codcli,
            FIRST almmmatg OF detalle NO-LOCK,
            ...
            ASSIGN 
                detalle.despgo = detalle.fmapgo + ' ' + (IF AVAILABLE gn-convt THEN gn-ConVt.Nombr ELSE '')
    ...
    END.
    WHEN 6 THEN DO:
        lOptions = lOptions + 'coddiv,lisbas,nroped,fchped,fchven,fchent,nompro,implin,impfac,almsalida,fmapgo'.
        FOR EACH detalle:
            x-nompro = 'NN'.
            FIND gn-prov WHERE gn-prov.codcia = pv-codcia
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-Mensaje FILL-IN-Fecha-1 FILL-IN-Fecha-2 FILL-IN-NroPed 
          RADIO-SET-1 
      WITH FRAME fMain IN WINDOW wWin.
  ENABLE BROWSE-1 FILL-IN-Fecha-1 FILL-IN-Fecha-2 FILL-IN-NroPed RADIO-SET-1 
         BUTTON-1 BtnDone 
      WITH FRAME fMain IN WINDOW wWin.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT AVAILABLE detalle THEN DO:
        ...
        FIND gn-clie WHERE gn-clie.codcia = cl-codcia
            AND gn-clie.codcli = detalle.codcli
            NO-LOCK NO-ERROR.
        IF AVAILABLE gn-clie THEN detalle.flagaut = gn-clie.flagaut.
        x-Cuenta = x-Cuenta + 1.
    END.
    ASSIGN
        detalle.canped = detalle.canped + facdpedi.canped * facdpedi.factor
