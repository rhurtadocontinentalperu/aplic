...
DEF TEMP-TABLE Detalle 
    ...
    FIELD TrfTransito AS DEC LABEL "Transferencia en Transito" FORMAT ">>>,>>>,>>9.99"
    FIELD CmpTransito AS DEC LABEL "Compras en Transito" FORMAT ">>>,>>>,>>9.99"
    FIELD CanPedi   AS DEC  LABEL "Pedido"      FORMAT ">>>,>>>,>>9.99"
    FIELD CanAten   AS DEC  LABEL "Atendido"    FORMAT ">>>,>>>,>>9.99"
    FIELD Saldo     AS DEC  LABEL "Saldo"       FORMAT ">>>,>>>,>>9.99".
...
&Scoped-Define ENABLED-OBJECTS BUTTON-8 BtnDone RECT-2 RECT-6 BROWSE-3 ~
BUTTON-10 BUTTON-2 ChkBoxSoloConStock TOGGLE-Comprometido FILL-IN-CodAlm ~
BUTTON-3 FILL-IN-CodPro BUTTON-1 BUTTON-9 
&Scoped-Define DISPLAYED-OBJECTS ChkBoxSoloConStock TOGGLE-Comprometido ~
FILL-IN-CodAlm FILL-IN-CodPro FILL-IN-NomPro FILL-IN-Mensaje ~
FILL-IN-filetxt 
...
DEFINE FRAME F-Main
     BUTTON-8 AT ROW 1 COL 80 WIDGET-ID 40
     BtnDone AT ROW 1 COL 87 WIDGET-ID 38
     BROWSE-3 AT ROW 1.19 COL 3 HELP
          "Presione CRTL para seleccionar más de un registro" WIDGET-ID 200
     BUTTON-10 AT ROW 2.92 COL 67 WIDGET-ID 2
     BUTTON-2 AT ROW 4.08 COL 67 WIDGET-ID 70
     ChkBoxSoloConStock AT ROW 6.96 COL 67 WIDGET-ID 66
     TOGGLE-Comprometido AT ROW 7.73 COL 67 WIDGET-ID 68
...
IF FILL-IN-filetxt <> "" THEN DO:
    FOR EACH tt-txt :
        FOR EACH estavtas.DimProducto NO-LOCK WHERE estavtas.DimProducto.codmat = tt-txt.codigo,
            ...
            AND Almmmatg.codmat = estavtas.DimProducto.CodMat:
...
FOR EACH tt-DimProducto,
    ...
    FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
    AND Almmmatg.codmat = tt-DimProducto.CodMat:
    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} =
         "CARGANDO STOCK: " + tt-DimProducto.CodMat.

    FOR EACH Almmmate NO-LOCK WHERE Almmmate.CodCia = s-codcia
        AND Almmmate.codmat = tt-DimProducto.CodMat
        AND LOOKUP(Almmmate.codalm, FILL-IN-CodAlm) > 0
        AND (chkboxSoloConStock = NO OR Almmmate.stkact <> 0 ),
        FIRST Almacen OF Almmmate NO-LOCK WHERE Almacen.codalm <> '999':
        FIND FIRST almtabla WHERE almtabla.tabla = 'MK'  AND almtabla.codigo = Almmmatg.codmar 
            NO-LOCK NO-ERROR.
        CREATE Detalle.
        ASSIGN
            Detalle.CodAlm  = Almmmate.codalm
            Detalle.Almacen = TRIM(Almacen.CodAlm) + ' ' + Almacen.Descripcion
            ...
            /* 23Jul2013 - Ic */
            Detalle.clasificacion = Almmmatg.tiprot[1]
            Detalle.iranking = Almmmatg.ordtmp
            Detalle.clsfutlx = Almmmatg.undalt[3]
            Detalle.rnkgutlx = Almmmatg.libre_d04
            Detalle.clsfmayo = Almmmatg.undalt[4]
            Detalle.rnkgmayo = Almmmatg.libre_d05
            /* Fin 23Jul2013 Ic */
            /* 11Dic2014 Ic */
            detalle.tpoart      = Almmmatg.tpoart
            detalle.almacenes   = Almmmatg.tpomrg
        ...
        IF Almmmatg.monvta = 1 THEN
            ASSIGN
            Detalle.CostoMn = Almmmatg.CtoLis.
        ELSE 
            ASSIGN
            Detalle.CostoMn = Almmmatg.CtoLis * Almmmatg.TpoCmb.
        /* Costo Promedio */
        FIND LAST AlmStkge WHERE AlmStkge.CodCia = s-codcia
            AND AlmStkge.codmat = tt-DimProducto.codmat
            AND AlmStkge.Fecha <= TODAY 
            NO-LOCK NO-ERROR.
        IF AVAILABLE AlmStkge THEN Detalle.PromedioMn = AlmStkge.CtoUni.
        ...
        END CASE.
        CASE Almacen.campo-c[3]:
            WHEN "Si" THEN Detalle.AlmRem = "Si".
            OTHERWISE Detalle.AlmRem = "No".
        END CASE.
        Detalle.AlmCom = Almacen.campo-c[6].
        Detalle.CodDiv = Almacen.coddiv.
        Detalle.StkMin = Almmmate.StkMin.
        Detalle.StkMax = Almmmate.StkMax.
        detalle.codmar = almmmatg.codmar.
        detalle.desmar = IF (AVAILABLE almtabla) THEN almtabla.nombre ELSE ''.
    END.
END.
/* Cargsmos Stocks Comprometidos */
IF TOGGLE-Comprometido = YES THEN DO:
    FOR EACH Detalle:
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} =
            "STOCK COMPROMETIDO: " + Detalle.codalm + ' ' + Detalle.Producto.

        RUN vta2/stock-comprometido (Detalle.codalm, Detalle.producto, OUTPUT x-StockComprometido).
        ASSIGN
...
FOR EACH Almacen NO-LOCK WHERE Almacen.codcia = s-codcia 
    AND LOOKUP(Almacen.codalm, FILL-IN-CodAlm) > 0,
    ...
    FIND FIRST Detalle WHERE Detalle.codalm = Almacen.codalm AND Detalle.Producto = Almdmov.CodMat
        NO-ERROR.
    IF AVAILABLE Detalle THEN DO:
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} =
             "CARGANDO TRANSFERENCIAS: " + Detalle.codalm + ' ' + Detalle.producto.
        ASSIGN Detalle.TrfTransito = Detalle.TrfTransito + Almdmov.candes.
    END.
END.
/* Ordenes de Compra en Transito */
FOR EACH Lg-cocmp NO-LOCK WHERE LG-COCmp.CodCia = S-CODCIA 
    AND Lg-cocmp.coddiv = "00000"
    AND LOOKUP(Lg-cocmp.codalm, FILL-IN-CodAlm) > 0
    AND LG-COCmp.FchVto >= TODAY
    AND LG-COCmp.FlgSit = 'P',
    EACH Lg-docmp OF Lg-cocmp NO-LOCK WHERE Lg-docmp.canpedi > Lg-docmp.canate:
    FIND FIRST Detalle WHERE Detalle.codalm = Lg-cocmp.codalm AND Detalle.Producto = Lg-docmp.codmat
        NO-ERROR.
    IF AVAILABLE Detalle THEN DO:
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} =
             "CARGANDO COMPRAS: " + Detalle.codalm + ' ' + Detalle.producto.
        ASSIGN
        Detalle.CanPedi = Detalle.CanPedi + Lg-docmp.CanPedi 
        Detalle.CanAten = Detalle.CanAten + Lg-docmp.CanAten
...
------------------------------------------------------------------------------*/
  DISPLAY ChkBoxSoloConStock TOGGLE-Comprometido FILL-IN-CodAlm FILL-IN-CodPro 
          FILL-IN-NomPro FILL-IN-Mensaje FILL-IN-filetxt 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE BUTTON-8 BtnDone RECT-2 RECT-6 BROWSE-3 BUTTON-10 BUTTON-2 
