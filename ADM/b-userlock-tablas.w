...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      tt-w-report.Campo-C[1] COLUMN-LABEL "Tabla" FORMAT "X(25)":U
            WIDTH 19.43
      tt-w-report.Campo-C[2] COLUMN-LABEL "Tipo" FORMAT "X(10)":U
      tt-w-report.Campo-C[3] COLUMN-LABEL "Flags" FORMAT "X(10)":U
      tt-w-report.Campo-C[4] COLUMN-LABEL "RecID" FORMAT "X(12)":U
            WIDTH 19.29
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 66 BY 13.85
         FONT 4.
...
"tt-w-report.Campo-C[1]" "Tabla" "X(25)" "character" ? ? ? ? ? ? no ? no no "19.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-w-report.Campo-C[2]
"tt-w-report.Campo-C[2]" "Tipo" "X(10)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-w-report.Campo-C[3]
"tt-w-report.Campo-C[3]" "Flags" "X(10)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-w-report.Campo-C[4]
"tt-w-report.Campo-C[4]" "RecID" "X(12)" "character" ? ? ? ? ? ? no ? no no "19.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DEFINE INPUT PARAMETER pUserLockId AS INT64 NO-UNDO.
...
IF AVAILABLE _userlock THEN DO:
    ...
    REPEAT x-sec = 1 TO 512:
        IF _userlock._userlock-table[x-sec] <> ? THEN DO:
            FIND FIRST _file WHERE _file._file-number = _userlock._userlock-table[x-sec] NO-LOCK NO-ERROR.
            IF AVAILABLE _file THEN DO:
                CREATE tt-w-report.
                ASSIGN tt-w-report.campo-c[1] = _file._file-name
                        tt-w-report.campo-c[2] = _userlock._userlock-type[x-sec]
                        tt-w-report.campo-c[3] = _userlock._userlock-flags[x-sec]
                        tt-w-report.campo-c[4] = STRING(_userlock._userlock-RECID[x-sec],"->>>>>>>>>>>>>>>>>>>9").
            END.
        END.
    END.
END.
