...
&Scoped-Define ENABLED-OBJECTS RECT-12 RECT-6 RECT-5 RECT-4 RADIO-SET-1 ~
r-sortea B-imprime B-impresoras x-desde x-hasta RB-NUMBER-COPIES B-cancela ~
RB-BEGIN-PAGE RB-END-PAGE 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-1 r-sortea x-desde x-hasta ~
...
DEFINE VARIABLE x-desde AS CHARACTER FORMAT "X(10)":U 
     LABEL "Desde" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .69
     BGCOLOR 15  NO-UNDO.

DEFINE VARIABLE x-hasta AS CHARACTER FORMAT "x(10)":U 
     LABEL "Hasta" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .69
     BGCOLOR 15  NO-UNDO.
...
DEFINE FRAME DIALOG-1
     ...
     r-sortea AT ROW 3.42 COL 7.14 NO-LABEL
     B-imprime AT ROW 12.08 COL 9.29
     B-impresoras AT ROW 9 COL 15
     b-archivo AT ROW 10 COL 15
     RB-OUTPUT-FILE AT ROW 10.23 COL 23.72 COLON-ALIGNED NO-LABEL
     x-desde AT ROW 3.62 COL 29.86 COLON-ALIGNED
     x-hasta AT ROW 4.38 COL 29.86 COLON-ALIGNED
     RB-NUMBER-COPIES AT ROW 8.12 COL 29.86 COLON-ALIGNED
     B-cancela AT ROW 12.08 COL 39.29
     RB-BEGIN-PAGE AT ROW 8.12 COL 47.14 COLON-ALIGNED
     RB-END-PAGE AT ROW 9.12 COL 47.14 COLON-ALIGNED
     RECT-12 AT ROW 2.92 COL 6.14
     "Ordenado Por :" VIEW-AS TEXT
          SIZE 11.14 BY .62 AT ROW 2.81 COL 7.72
     RECT-6 AT ROW 11.5 COL 1
     RECT-5 AT ROW 7.5 COL 1
     " Configuración de Impresión" VIEW-AS TEXT
          SIZE 56.72 BY .62 AT ROW 6.81 COL 1
          BGCOLOR 1 FGCOLOR 15 
     "Reporte de Proveedores" VIEW-AS TEXT
          SIZE 24.29 BY 1 AT ROW 1.5 COL 5.57
          FONT 11
...
ON GO OF FRAME DIALOG-1 /* Clientes */
DO:
...
DO:
  ASSIGN  r-sortea x-desde x-hasta.   
  CASE r-sortea:
    WHEN 1 THEN DO:  
        /*
       IF x-desde = "" THEN x-hasta =  "          ".
       IF x-hasta = "" THEN x-hasta =  "9999999999".
       */
        IF x-desde = ''
        THEN DO:
            FIND FIRST gn-prov USE-INDEX IDX01 WHERE gn-prov.Codcia = pv-codcia
                NO-LOCK NO-ERROR.
            IF AVAILABLE gn-prov THEN x-desde = gn-prov.codpro.
        END.
        IF x-hasta = ''
        THEN DO:
            FIND LAST gn-prov USE-INDEX IDX01 WHERE gn-prov.Codcia = pv-codcia
                NO-LOCK NO-ERROR.
            IF AVAILABLE gn-prov THEN x-desde = gn-prov.codpro.
        END.
    END.
    WHEN 2 THEN DO:  
        IF x-hasta = '' THEN x-hasta = TRIM(x-hasta) + CHR(255).
    END.
  END CASE.
    P-largo  = 66.
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-1 r-sortea x-desde x-hasta RB-NUMBER-COPIES RB-BEGIN-PAGE 
          RB-END-PAGE 
      WITH FRAME DIALOG-1.
  ENABLE RECT-12 RECT-6 RECT-5 RECT-4 RADIO-SET-1 r-sortea B-imprime 
         B-impresoras x-desde x-hasta RB-NUMBER-COPIES B-cancela RB-BEGIN-PAGE 
         RB-END-PAGE 
      WITH FRAME DIALOG-1.
...
-------------------------------------------------------------*/
   DEFINE FRAME f-cab 
    gn-prov.codpro gn-prov.nompro gn-prov.dirpro 
    gn-prov.ruc
    HEADER
        empresas.nomcia
        "FECHA :" TO 108      TODAY TO 118
       SKIP
        empresas.direccion 
        "PAGINA :" TO 108     c-Pagina FORMAT "ZZ9" TO 118
       SKIP
       "P R O V E E D O R E S":C117
       SKIP(1)
    WITH DOWN WIDTH 140 NO-BOX STREAM-IO.
    
        FOR EACH gn-prov WHERE gn-prov.Codcia  = pv-codcia AND
                                   gn-prov.codpro  <=  x-hasta AND
                                   gn-prov.codpro  >=  x-desde
                          BY codpro
                ON ERROR UNDO, LEAVE:
            {&NEW-PAGE}.
            DISPLAY STREAM report codpro FORMAT "X(8)" nompro dirpro gn-prov.ruc WITH FRAME f-cab.
        END.
END PROCEDURE.
   ...
   DEFINE FRAME f-cab 
    ...
    HEADER
       ...
       SKIP
        ...
        "PAGINA :" TO 108     c-Pagina FORMAT "ZZ9" TO 118
       SKIP
        "P R O V E E D O R E S":C120
       SKIP(1)
    WITH DOWN WIDTH 140 NO-BOX STREAM-IO.
        FOR EACH gn-prov WHERE gn-prov.Codcia = pv-codcia  AND
                                gn-prov.Nompro >= x-desde       AND
                                gn-prov.Nompro <= x-hasta      
                                BY nompro
                ON ERROR UNDO, LEAVE:
            {&NEW-PAGE}.
            DISPLAY STREAM report codpro FORMAT "X(8)" nompro dirpro gn-prov.ruc WITH FRAME f-cab.
