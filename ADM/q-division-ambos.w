...
&Scoped-define QUERY-STRING-Query-Main FOR EACH GN-DIVI WHERE ~{&KEY-PHRASE} ~
 ...
 AND (GN-DIVI.Campo-Char[1] = "A" ~
  OR GN-DIVI.Campo-Char[1] = "D") NO-LOCK ~
...
&Scoped-Define ENABLED-OBJECTS RECT-5 BUTTON-1 F-Codigo 
&Scoped-Define DISPLAYED-OBJECTS F-Codigo 
...
DEFINE VARIABLE F-Codigo AS CHARACTER FORMAT "X(5)":U 
     VIEW-AS FILL-IN 
...
DEFINE FRAME FRAME-C
     BUTTON-1 AT ROW 1.27 COL 12 WIDGET-ID 2
     F-Codigo AT ROW 1.54 COL 1.57 NO-LABEL WIDGET-ID 4
     "Buscar" VIEW-AS TEXT
          SIZE 6 BY .5 AT ROW 1 COL 2 WIDGET-ID 8
          FONT 1
     RECT-5 AT ROW 1.27 COL 1 WIDGET-ID 6
...
/* SETTINGS FOR FILL-IN F-Codigo IN FRAME FRAME-C
   ALIGN-L                                                              */
...
/* Query rebuild information for QUERY Query-Main
 ...
 AND (GN-DIVI.Campo-Char[1] = ""A""
  OR GN-DIVI.Campo-Char[1] = ""D"")"
     _Design-Parent    is WINDOW q-tables @ ( 1.04 , 19.86 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-Codigo q-tables
ON LEAVE OF F-Codigo IN FRAME FRAME-C
OR "RETURN":U OF F-Codigo
DO:
  IF F-Codigo:SCREEN-VALUE = "" THEN RETURN.
  ASSIGN F-Codigo.
  RUN Ubica-Registro.
  F-Codigo:SCREEN-VALUE = "". 
END.
...
------------------------------------------------------------------------------*/
    FIND Gn-Divi WHERE Gn-Divi.CodCia = s-codcia AND
        Gn-Divi.CodDiv = F-Codigo AND
        LOOKUP(GN-DIVI.Campo-Char[1], 'D,A') > 0 
        NO-LOCK NO-ERROR.
    IF AVAILABLE Gn-Divi THEN output-var-1 = ROWID(Gn-Divi).
    ELSE output-var-1 = ?.
    IF OUTPUT-VAR-1 <> ? THEN DO:
       FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
