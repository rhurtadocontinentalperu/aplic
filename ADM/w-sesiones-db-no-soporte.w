...
DEFINE VAR x-por-liberar AS LOG INIT YES.

DEFINE BUFFER x-AdmCtrlUsers FOR AdmCtrlUsers.
...
tt-w-report.Campo-C[4] tt-w-report.Campo-I[2] tt-w-report.Campo-C[5] ~
tt-w-report.Campo-C[6] tt-w-report.Campo-C[7] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-4 W-Win _STRUCTURED
  QUERY BROWSE-4 NO-LOCK DISPLAY
      ...
      tt-w-report.Campo-C[2] COLUMN-LABEL "Terminal!Server Conectado" FORMAT "X(25)":U
            WIDTH 13.29
      tt-w-report.Campo-C[3] COLUMN-LABEL "Equipo!Conectado" FORMAT "X(25)":U
            WIDTH 14.43
      tt-w-report.Campo-C[4] COLUMN-LABEL "Inicio Conexion" FORMAT "X(25)":U
            WIDTH 15.43
      tt-w-report.Campo-I[2] COLUMN-LABEL "PID" FORMAT "->>>>>>9":U
            WIDTH 8.43
      tt-w-report.Campo-C[5] COLUMN-LABEL "Intento Conectarse!Terminal Server" FORMAT "X(25)":U
            WIDTH 14.43
      tt-w-report.Campo-C[6] COLUMN-LABEL "Intento conectarse!PC-Usuario" FORMAT "X(25)":U
            WIDTH 13.43
      tt-w-report.Campo-C[7] COLUMN-LABEL "Intento conectarse!Fecha/Hora" FORMAT "X(25)":U
            WIDTH 21.43
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 131 BY 15.62
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     FILL-IN-usuario AT ROW 1.19 COL 69 COLON-ALIGNED WIDGET-ID 6
     FILL-IN-conexion AT ROW 1.19 COL 93.57 COLON-ALIGNED WIDGET-ID 8
     BROWSE-4 AT ROW 2.42 COL 3 WIDGET-ID 300
     "   Usuarios con inconvenientes de conexion" VIEW-AS TEXT
          SIZE 59.57 BY 1.15 AT ROW 1.12 COL 3.43 WIDGET-ID 4
          BGCOLOR 15 FGCOLOR 1 FONT 11
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 133.72 BY 17.58 WIDGET-ID 100.
...
"tt-w-report.Campo-C[1]" "Usuario" "X(15)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-w-report.Campo-C[2]
"tt-w-report.Campo-C[2]" "Terminal!Server Conectado" "X(25)" "character" ? ? ? ? ? ? no ? no no "13.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-w-report.Campo-C[3]
"tt-w-report.Campo-C[3]" "Equipo!Conectado" "X(25)" "character" ? ? ? ? ? ? no ? no no "14.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.tt-w-report.Campo-C[4]
"tt-w-report.Campo-C[4]" "Inicio Conexion" "X(25)" "character" ? ? ? ? ? ? no ? no no "15.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.tt-w-report.Campo-I[2]
"tt-w-report.Campo-I[2]" "PID" "->>>>>>9" "integer" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.tt-w-report.Campo-C[5]
"tt-w-report.Campo-C[5]" "Intento Conectarse!Terminal Server" "X(25)" "character" ? ? ? ? ? ? no ? no no "14.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.tt-w-report.Campo-C[6]
"tt-w-report.Campo-C[6]" "Intento conectarse!PC-Usuario" "X(25)" "character" ? ? ? ? ? ? no ? no no "13.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.tt-w-report.Campo-C[7]
"tt-w-report.Campo-C[7]" "Intento conectarse!Fecha/Hora" "X(25)" "character" ? ? ? ? ? ? no ? no no "21.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-4 */
&ANALYZE-RESUME
...
DO:
    ...
    MESSAGE "Aun no implementada....espere" VIEW-AS ALERT-BOX INFORMATION.
...
FOR EACH _Connect WHERE _Connect._Connect-Type = "REMC" NO-LOCK:
    ...
    CREATE tt-w-report.
        ASSIGN tt-w-report.campo-i[1] = _Connect._Connect-Usr
                ...
                tt-w-report.campo-c[3] = pc-de-usuario(_Connect-PID) /*pc-de-usuario(_Connect._Connect-Usr)*/
...
FOR EACH logvtatabla WHERE logvtatabla.codcia = 1 AND
                            logvtatabla.tabla = x-tabla AND
                            logvtatabla.logdate >= TODAY NO-LOCK:
    /*
    FIND LAST AdmCtrlUsers WHERE AdmCtrlUsers.userDB = logvtatabla.llave_c1 AND
                                    AdmCtrlUsers.fechinicio <= logvtatabla.logdate AND 
                                    AdmCtrlUsers.campo-c[2] = STRING(logvtatabla.numid) NO-LOCK NO-ERROR.
    */
    /*
    FIND FIRST _Connect WHERE _Connect._Connect-Usr = logvtatabla.numid NO-LOCK NO-ERROR.

    IF AVAILABLE _Connect THEN DO:
        FIND LAST AdmCtrlUsers WHERE AdmCtrlUsers.num-PID = logvtatabla.valor[2] /*_Connect._Connect-pid*/ AND
                                        AdmCtrlUsers.fechinicio <= TODAY NO-LOCK NO-ERROR.

    END.
    ELSE DO:
        /* Forzar a ERROR */
        FIND LAST AdmCtrlUsers WHERE AdmCtrlUsers.num-PID = -9999 AND
                                        AdmCtrlUsers.fechinicio <= TODAY NO-LOCK NO-ERROR.
    END.
    */
    ...
    CREATE tt-w-report.
        ASSIGN tt-w-report.campo-i[1] = logvtatabla.numid
                tt-w-report.campo-c[1] = logvtatabla.llave_c1
                tt-w-report.campo-c[2] = IF (AVAILABLE AdmCtrlUsers) THEN AdmCtrlUsers.pccliente ELSE ""
                tt-w-report.campo-c[3] = IF (AVAILABLE AdmCtrlUsers) THEN AdmCtrlUsers.pcusuario ELSE ""
                tt-w-report.campo-c[4] = IF (AVAILABLE AdmCtrlUsers) THEN STRING(AdmCtrlUsers.fechinicio,"99/99/9999") ELSE ""
                tt-w-report.campo-c[4] = tt-w-report.campo-c[4] + " " + IF (AVAILABLE AdmCtrlUsers) THEN AdmCtrlUsers.horainicio ELSE ""
                tt-w-report.campo-i[2] = IF (AVAILABLE AdmCtrlUsers) THEN AdmCtrlUsers.Numid ELSE 0.

    /* Quien intento conectarse */
    ASSIGN tt-w-report.campo-c[5] = IF(NUM-ENTRIES(logvtatabla.libre_c02,"|") >= 1) THEN ENTRY(1,logvtatabla.libre_c02,"|") ELSE ""
           tt-w-report.campo-c[6] = IF(NUM-ENTRIES(logvtatabla.libre_c02,"|") >= 2) THEN ENTRY(2,logvtatabla.libre_c02,"|") ELSE ""
           tt-w-report.campo-c[7] = IF(NUM-ENTRIES(logvtatabla.libre_c02,"|") >= 4) THEN ENTRY(4,logvtatabla.libre_c02,"|") ELSE ""
               .
END.
...
x-datos-de-la-pc = x-datos-de-la-pc + "|IDUSUARIO@" + IF (lRemote-user = ? OR lRemote-user = "") THEN lUserName ELSE lRemote-user. /* Usuario de Windows */
x-datos-de-la-pc = x-datos-de-la-pc + "|FECHAHORAINICIO@" + STRING(TODAY,"99/99/9999") + " " + STRING(TIME,"HH:MM:SS").
...
_Connect._Connect-Type
    ...
    CREATE logvtatabla.
        ASSIGN logvtatabla.codcia = 1
                ...
                logvtatabla.libre_c02 = x-datos-de-la-pc.
