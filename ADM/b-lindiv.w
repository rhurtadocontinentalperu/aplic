...
DEF VAR s-Tabla AS CHAR INIT "DIVFACXLIN" NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c2 COLUMN-LABEL "Línea" FORMAT "x(3)":U
      Almtfami.desfam FORMAT "X(30)":U WIDTH 31.14
      VtaTabla.Valor[1] COLUMN-LABEL "Flete (%)" FORMAT ">>>,>>9.999999":U
            WIDTH 12.43
      VtaTabla.Valor[2] COLUMN-LABEL "Sobretasa (%)" FORMAT "->>>,>>9.999999":U
  ENABLE
      VtaTabla.Llave_c2
      VtaTabla.Valor[1]
      VtaTabla.Valor[2]
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 65 BY 6.15
         FONT 4
...
"VtaTabla.Llave_c2" "Línea" "x(3)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almtfami.desfam
"Almtfami.desfam" ? ? "character" ? ? ? ? ? ? no ? no no "31.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaTabla.Valor[1]
"VtaTabla.Valor[1]" "Flete (%)" ">>>,>>9.999999" "decimal" ? ? ? ? ? ? yes ? no no "12.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.VtaTabla.Valor[2]
"VtaTabla.Valor[2]" "Sobretasa (%)" "->>>,>>9.999999" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
FIND Almtfami WHERE Almtfami.CodCia = gn-divi.codcia
...
IF NOT AVAILABLE Almtfami THEN DO:
    ...
    RETURN 'ADM-ERROR'.
END.
/* IF DECIMAL( VtaTabla.Valor[1]:SCREEN-VALUE IN BROWSE {&browse-name}) = 0 */
/*     THEN DO:                                                             */
