...
&SCOPED-DEFINE CONDICION Almtmovm.CodCia = S-CODCIA AND Almtmovm.Tipmov = input-var-1 ~
AND LOOKUP(TRIM(STRING(Almtmovm.CodMov)), input-var-2) > 0

&SCOPED-DEFINE CODIGO Almtmovm.Codmov 

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( Almtmovm.Desmov BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( Almtmovm.Desmov , FILL-IN-filtro ) <> 0 )
...
&Scoped-define INTERNAL-TABLES Almtmovm Almtdocm gre_movalm_mottraslado ~
...
&Scoped-define FIELDS-IN-QUERY-br_table Almtmovm.Codmov Almtmovm.Desmov ~
gre_movalm_mottraslado.codmotivotraslado ~
Sunat_Fact_Electr_Detail.Description 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH Almtmovm WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK, ~
      EACH Almtdocm WHERE Almtdocm.CodCia = Almtmovm.CodCia ~
  AND Almtdocm.TipMov = Almtmovm.Tipmov ~
  AND Almtdocm.CodMov = Almtmovm.Codmov ~
  AND Almtdocm.CodAlm = S-CODALM NO-LOCK, ~
      EACH gre_movalm_mottraslado WHERE Almtmovm.Codmov = gre_movalm_mottraslado.codmov NO-LOCK, ~
      FIRST Sunat_Fact_Electr_Detail WHERE Sunat_Fact_Electr_detail.catalogue = 20 and  ~
Sunat_Fact_Electr_detail.code = gre_movalm_mottraslado.codmotivotraslado NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almtmovm WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK, ~
      EACH Almtdocm WHERE Almtdocm.CodCia = Almtmovm.CodCia ~
  AND Almtdocm.TipMov = Almtmovm.Tipmov ~
  AND Almtdocm.CodMov = Almtmovm.Codmov ~
...
Sunat_Fact_Electr_detail.code = gre_movalm_mottraslado.codmotivotraslado NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Almtmovm Almtdocm ~
gre_movalm_mottraslado Sunat_Fact_Electr_Detail
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Almtmovm
&Scoped-define SECOND-TABLE-IN-QUERY-br_table Almtdocm
&Scoped-define THIRD-TABLE-IN-QUERY-br_table gre_movalm_mottraslado
&Scoped-define FOURTH-TABLE-IN-QUERY-br_table Sunat_Fact_Electr_Detail
...
<FOREIGN-KEYS>
Nombres que inicien con|y||integral.Almtmovm.Desmov
Nombres que contengan|y||integral.Almtmovm.Desmov
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Codigo|y||integral.Almtmovm.CodCia|yes,integral.Almtmovm.Codmov|yes
Descripcion|||integral.Almtmovm.CodCia|yes,integral.Almtmovm.Desmov|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
DEFINE VARIABLE FILL-IN-codigo AS INTEGER FORMAT "99":U INITIAL 0 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Almtmovm.Codmov FORMAT "99":U
      Almtmovm.Desmov FORMAT "X(60)":U WIDTH 33.14
      gre_movalm_mottraslado.codmotivotraslado COLUMN-LABEL "Motivo!Sunat" FORMAT "x(5)":U
            WIDTH 5.43
      Sunat_Fact_Electr_Detail.Description COLUMN-LABEL "Descripcion!Motivo Sunat" FORMAT "x(80)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 68.14 BY 9.12
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Almtmovm,INTEGRAL.Almtdocm WHERE INTEGRAL.Almtmovm ...,INTEGRAL.gre_movalm_mottraslado WHERE INTEGRAL.Almtmovm ...,INTEGRAL.Sunat_Fact_Electr_Detail WHERE INTEGRAL.gre_movalm_mottraslado ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",,, FIRST"
     _Where[1]         = "{&CONDICION}"
     _JoinCode[2]      = "Almtdocm.CodCia = Almtmovm.CodCia
  AND Almtdocm.TipMov = Almtmovm.Tipmov
  AND Almtdocm.CodMov = Almtmovm.Codmov
  AND Almtdocm.CodAlm = S-CODALM"
     _JoinCode[3]      = "Almtmovm.Codmov = gre_movalm_mottraslado.codmov"
     _JoinCode[4]      = "Sunat_Fact_Electr_detail.catalogue = 20 and 
...
"Almtmovm.Desmov" ? "X(60)" "character" ? ? ? ? ? ? no ? no no "33.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.gre_movalm_mottraslado.codmotivotraslado
"gre_movalm_mottraslado.codmotivotraslado" "Motivo!Sunat" "x(5)" "character" ? ? ? ? ? ? no ? no no "5.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.Sunat_Fact_Electr_Detail.Description
"Sunat_Fact_Electr_Detail.Description" "Descripcion!Motivo Sunat" "x(80)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Codigo':U THEN DO:
           &Scope SORTBY-PHRASE BY Almtmovm.CodCia BY Almtmovm.Codmov
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripcion':U THEN DO:
           &Scope SORTBY-PHRASE BY Almtmovm.CodCia BY Almtmovm.Desmov
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ...
    ASSIGN
        output-var-1 = ROWID( almtdocm /*{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}*/ )
        output-var-2 = STRING( {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codmov) /*, "99" )*/
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.desmov.
