...
&SCOPED-DEFINE l-lookup h_l-otr-v2
...
DEFINE VARIABLE h_l-otr-v2 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.38 COL 81.43
     Btn_Cancel AT ROW 8.58 COL 81.43
     Btn_Help AT ROW 10.38 COL 81.43
     SPACE(4.99) SKIP(2.30)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 4
         TITLE "Listado de OTRs"
...
ON WINDOW-CLOSE OF FRAME C-Dialog /* Listado de OTRs */
DO:  
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'gre/l-otr-lkup-gre.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-otr-v2 ).
       RUN set-position IN h_l-otr-v2 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-otr-v2 ( 12.50 , 76.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.19 , 79.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-otr-v2. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-otr-v2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-otr-v2 ,
