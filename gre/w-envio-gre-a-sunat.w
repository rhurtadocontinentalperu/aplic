...
DEFINE FRAME F-Main
     ...
     " [ filtros ]" VIEW-AS TEXT
          SIZE 13 BY .54 AT ROW 2.42 COL 175 WIDGET-ID 204
...
FOR EACH gre_header WHERE gre_header.m_rspta_sunat = 'CON TRANSPORTISTA' AND
    ...
    ASSIGN gre_header_disponibles.nombrePuertoAeropuerto = cNroPHR
        gre_header_seleccionados.nombrePuertoAeropuerto = cNroPHR.
    ...
    ASSIGN gre_header_disponibles.m_libre_c05 = ""
        gre_header_seleccionados.m_libre_c05 = ""
        gre_header_seleccionados.m_fechahorareg = ?.
    ...
    IF AVAILABLE gre_vehiculo_hdr THEN DO:
        ASSIGN gre_header_disponibles.m_libre_c05 = gre_vehiculo_hdr.placa
                ...
                gre_header_seleccionados.m_libre_c05 = gre_vehiculo_hdr.placa
    ...
    IF iRowsSelected > 0 THEN DO:
        ...
        DO iRow = 1 TO iRowsSelected :
            IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(iRow) THEN DO:
                ...
                /* Ic - 3Jun2025, verificamos si el comprobante ó otr no se hayan anulado */
                cMsgRet = "".
                RUN verificar-documentacion(b-gre_header.m_coddoc, b-gre_header.m_nroser, b-gre_header.m_nrodoc, OUTPUT cMsgRet).
                IF cMsgRet <> "" THEN DO:
                    /* El comprobante o OTR no existe o esta anulado */
                    NEXT.
                END.
                ...
                */
                ...
                IF NOT LOCKED b-gre_header THEN DO:
                    ...
                    FIND CURRENT b-gre_header NO-LOCK NO-ERROR.                
                    /*
                    MESSAGE "Se envia a SUNAT en modo SIMULACION"
                    
                    (***) 
                    
                    Activar esta linea cuando ya no se hagan pruebas
...
/* La linea esta comentada (***) */
MESSAGE "Se enviaron a SUNAT en modo SIMULACION por pruebas" 
    VIEW-AS ALERT-BOX INFORMATION.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    /* Serie x Ventas */
      /* Se usa para el PLAN B de GRE - es pausa, verificar 'ELECTRONICA' x q no exite en
...
*/
    ...
    ASSIGN gre_header_disponibles.m_cco = "NO MOSTRAR"
            gre_header_seleccionados.m_cco = "NO MOSTRAR".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE verificar-documentacion W-Win 
PROCEDURE verificar-documentacion :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE INPUT PARAMETER pCodDoc AS CHAR.     /* FAC, BOL, OTR */
DEFINE INPUT PARAMETER pSerieDoc AS INT.
DEFINE INPUT PARAMETER pNroDoc AS INT.
DEFINE OUTPUT PARAMETER pMsgRetVal AS CHAR.

IF TRUE <> (pCodDoc > '') THEN DO:
    pMsgRetVal = "".
    RETURN "OK".
END.

DEFINE VAR cNroDoc AS CHAR.
pMsgRetVal = "TIPO DE DOCUMENTO INCORRECTO".

IF LOOKUP(pCodDoc,"FAC,BOL,FAI") > 0 THEN DO:
    cNroDoc = STRING(pSerieDoc,"999") + STRING(pNroDoc,"99999999").
    IF pCodDoc = 'FAI' THEN cNroDoc = STRING(pSerieDoc,"999") + STRING(pNroDoc,"999999").

    FIND FIRST ccbcdocu WHERE ccbcdocu.codcia = 1 AND ccbcdocu.coddoc = pCodDoc AND
                                ccbcdocu.nrodoc = cNroDoc NO-LOCK NO-ERROR.
    IF NOT AVAILABLE ccbcdocu THEN DO:
        pMsgRetVal = "DOCUMENTO NO EXISTE EN CCBCDOCU".
        RETURN "ADM-ERROR".
    END.
    IF ccbcdocu.flgest = 'A' THEN DO:
        pMsgRetVal = "DOCUMENTO ESTA ANULADO".
        RETURN "ADM-ERROR".
    END.        
    ...
    RETURN "OK".
END.
IF LOOKUP(pCodDoc,"OTR") > 0 THEN DO:
    cNroDoc = STRING(pSerieDoc,"999") + STRING(pNroDoc,"999999").
    ...
    IF NOT AVAILABLE faccpedi THEN DO:
        pMsgRetVal = "DOCUMENTO NO EXISTE EN FACCPEDI".
        RETURN "ADM-ERROR".
    END.
