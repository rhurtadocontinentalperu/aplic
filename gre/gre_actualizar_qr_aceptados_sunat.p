...
DEFINE VAR fDesde AS DATE.
DEFINE VAR fHasta AS DATE.
...
MESSAGE fdesde SKIP
    fhasta.
*/
FOR EACH gre_header WHERE gre_header.fechaEmisionGuia >= fDesde AND gre_header.fechaEmisionGuia <= fHasta AND 
                gre_header.m_rspta_sunat BEGINS 'ACEPTADO POR SUNAT' NO-LOCK:
    /*IF gre_header.ncorrelatio = 168936 THEN MESSAGE "es".*/
    ...
    /*IF iConteo < 6 THEN MESSAGE gre_header.ncorrelatio.*/
END.
/*
...
cFiler = fwrite_log("INICIO - Cantidad de guias ---> " + STRING(iConteo)).
            ...
            IF LOOKUP(tTagsEstadoDoc.cValue,"RC_05,AC_03,ERROR") > 0 THEN DO:
                ...
                IF cDesEstado = "ACEPTADO POR SUNAT" THEN DO:
                    ...
                    IF NOT AVAILABLE gre_header_qr THEN DO:
                        ...
                        cFiler = fwrite_log("Guia --->: " + cTextoQR ).
                    END.                
                END.
                        ...
                        IF NOT AVAILABLE gre_header_qr THEN DO:
                            CREATE gre_header_qr.
                                ASSIGN gre_header_qr.ncorrelativo = gre_header_disponibles.ncorrelatio
                                        gre_header_qr.USER_crea = "batch" NO-ERROR.
                                IF ERROR-STATUS:ERROR THEN DO:
                                    cFiler = fwrite_log("Guia --->: " + cNroDoc + " 2a.- ERROR:" + ERROR-STATUS:GET-MESSAGE(1)).
                                    UNDO grabar, LEAVE grabar.
                                END.

                            cFiler = fwrite_log("Guia --->: " + cNroDoc + " Grabamos el QR en header_QR" ).
                        END.
                        ASSIGN gre_header_qr.data_QR = ctextoQR NO-ERROR.
                        IF ERROR-STATUS:ERROR THEN DO:
                            cFiler = fwrite_log("Guia --->: " + cNroDoc + " 2b.- ERROR:" + ERROR-STATUS:GET-MESSAGE(1)).
                            UNDO grabar, LEAVE grabar.
                        END.
                        iGuias = iGuias + 1.
                    END.
                ...
                IF ERROR-STATUS:ERROR THEN DO:
                    ...
                    DO iConteo = 1 TO ERROR-STATUS:NUM-MESSAGES:
                        cFiler = fwrite_log("Guia --->: " + cNroDoc + " 3.- ERROR:" + ERROR-STATUS:GET-MESSAGE(iConteo)).
                    END.
...
cFiler = fwrite_log("FIN - Cantidad de guias con QR recuperadas " + STRING(iGUias) + " de un total de " + STRING(iConteo) ).
...
IF AVAILABLE tTagsEstadoDoc THEN DO:
    ...
    RUN lib/http-get-contenido.r(cUrlCDR,output v-result,output v-response,output v-content).
                      /*
    MESSAGE "v-result " v-result SKIP
            "v-response " string(v-response) SKIP
...
------------------------------------------------------------------------------*/
    ...
    x-archivo = session:TEMP-DIRECTORY + "log-Actualiza-QR-SUNAT-" + x-file + ".txt".
