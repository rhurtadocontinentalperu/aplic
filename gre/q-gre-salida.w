...
&Scoped-define INTERNAL-TABLES Almtdocm gre_movalm_mottraslado Almtmovm
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH Almtdocm WHERE ~{&KEY-PHRASE} ~
 ...
 AND LOOKUP(TRIM(STRING(Almtdocm.CodMov)), C-CODMOV) > 0 NO-LOCK, ~
      FIRST gre_movalm_mottraslado WHERE Almtdocm.CodMov = gre_movalm_mottraslado.codmov NO-LOCK, ~
      FIRST Almtmovm OF Almtdocm ~
      WHERE Almtmovm.ReqGuia = yes and ~
Almtmovm.indicador[6] = no   /* activo */ NO-LOCK ~
...
Almtmovm.indicador[6] = no   /* activo */ NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main Almtdocm gre_movalm_mottraslado ~
Almtmovm
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main Almtdocm
&Scoped-define SECOND-TABLE-IN-QUERY-Query-Main gre_movalm_mottraslado
&Scoped-define THIRD-TABLE-IN-QUERY-Query-Main Almtmovm
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.Almtdocm,INTEGRAL.gre_movalm_mottraslado WHERE INTEGRAL.Almtdocm ...,INTEGRAL.Almtmovm OF INTEGRAL.Almtdocm"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST, FIRST"
     _Where[1]         = "Almtdocm.CodCia = S-CODCIA
 ...
 AND LOOKUP(TRIM(STRING(Almtdocm.CodMov)), C-CODMOV) > 0"
     _JoinCode[2]      = "Almtdocm.CodMov = gre_movalm_mottraslado.codmov"
     _Where[3]         = "Almtmovm.ReqGuia = yes and
Almtmovm.indicador[6] = no   /* activo */"
...
------------------------------------------------------------------------------*/
  ...
  L-NroSer = pSeries.
  /*
  IF TRUE <> (L-NroSer > '') THEN L-NroSer = '000'.
  ELSE IF LOOKUP('000', L-NroSer) = 0 THEN L-NroSer = '000,' + L-NroSer.
  */
...
ASSIGN  input-var-1 = S-TPOMOV
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    ...
    RUN gre\C-gre-CRRALM-01 ("Movimientos de Salida"). 
    IF OUTPUT-VAR-1 <> ? 
