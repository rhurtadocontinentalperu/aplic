...
DEFINE VARIABLE num-records AS INT.
...
IF NUM-ENTRIES(input-var-3,"|") > 1 THEN DO:
    cDivisionOrigen = ENTRY(2,input-var-3,"|").
    input-var-3 = ENTRY(1,input-var-3,"|").    
END.

&SCOPED-DEFINE CONDICION ( faccpedi.codcia = s-codcia ~
AND faccpedi.coddiv = s-coddiv ~
AND faccpedi.coddoc = input-var-1 ~
AND faccpedi.flgest = input-var-2 ~
AND faccpedi.flgsit = input-var-3 ~
AND (COMBO-BOX_CodAlm = 'Todos' OR Faccpedi.codcli = COMBO-BOX_CodAlm) )
...
DEFINE VAR iMaxRegSeleccionados AS INT INIT 10.
...
FacCPedi.CodAlm FacCPedi.CodCli FacCPedi.NomCli 
...
&Scoped-Define ENABLED-OBJECTS COMBO-BOX_CodAlm FILL-IN-codigo CMB-filtro ~
FILL-IN-filtro br_table RECT-1 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX_CodAlm FILL-IN-codigo CMB-filtro ~
FILL-IN-filtro 
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.FacCPedi.NroPed|yes
Descripción|||INTEGRAL.FacCPedi.NomCli|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.CodAlm COLUMN-LABEL "Almacen!Despacho" FORMAT "x(8)":U
            WIDTH 9.57
      FacCPedi.CodCli COLUMN-LABEL "Solicitante" FORMAT "x(8)":U
      FacCPedi.NomCli FORMAT "x(100)":U WIDTH 34.14
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 74.14 BY 10.23
...
DEFINE FRAME F-Main
     COMBO-BOX_CodAlm AT ROW 2.08 COL 64 COLON-ALIGNED WIDGET-ID 2
...
"FacCPedi.NroPed" "Numero" ? "character" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[3]   > INTEGRAL.FacCPedi.CodAlm
"FacCPedi.CodAlm" "Almacen!Despacho" "x(8)" "character" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.NomCli" ? ? "character" ? ? ? ? ? ? no ? no no "34.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  /* This ADM trigger code must be preserved in order to notify other
    ...
    num-records = br_table:NUM-SELECTED-ROWS.

  IF num-records > iMaxRegSeleccionados THEN DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL COMBO-BOX_CodAlm L-table-Win
ON VALUE-CHANGED OF COMBO-BOX_CodAlm IN FRAME F-Main /* Solicitante */
DO:
  ASSIGN {&SELF-NAME}.
  RUN dispatch IN THIS-PROCEDURE('open-query-cases':U).
END.
...
ON FIND OF FaccPedi DO:
    ...
    cCodDoc = FaccPedi.coddoc.
    iSerieDoc = INTEGER(SUBSTRING(FaccPedi.nroped,1,3)).
    iNumeroDoc = INTEGER(SUBSTRING(FaccPedi.nroped,4)).

    FIND FIRST gre_header WHERE gre_header.m_coddoc = cCodDoc AND gre_header.m_nroser = iSerieDoc AND 
                                gre_header.m_nrodoc = iNumeroDoc AND gre_header.m_rspta_sunat = "ACEPTADO POR SUNAT" NO-LOCK NO-ERROR.
    If AVAILABLE gre_header Then do:
        /* ya tiene PRE-GUIA */
            RETURN ERROR.
    end.

    FIND FIRST almacen WHERE almacen.codcia = 1 AND almacen.codalm = faccpedi.codcli NO-LOCK NO-ERROR.
...
&IF DEFINED (FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}) > 0 &THEN
    ...
    */
    ...
    DO WITH FRAME {&FRAME-NAME}:
            ...
            DO iConteo = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS:
                IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(iConteo) THEN DO:
                IF output-var-2 > "" THEN output-var-2 = output-var-2 + "|".
                IF output-var-3 > "" THEN output-var-3 = output-var-3 + "|".

                ASSIGN output-var-2 = output-var-2 + {&first-TABLE-IN-QUERY-{&BROWSE-NAME}}.nroped.
                ASSIGN output-var-3 = output-var-3 + {&first-TABLE-IN-QUERY-{&BROWSE-NAME}}.codcli.
        ...
        DO  WHILE AVAILABLE faccpedi:
            /*ASSIGN output-var-1 = ROWID( {&first-TABLE-IN-QUERY-{&BROWSE-NAME}} ).*/
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH Faccpedi WHERE {&Condicion} NO-LOCK BREAK BY Faccpedi.CodCli:
        IF FIRST-OF(Faccpedi.codcli) THEN DO WITH FRAME {&FRAME-NAME}:
            COMBO-BOX_CodAlm:ADD-LAST(Faccpedi.codcli).
        END.
    END.
