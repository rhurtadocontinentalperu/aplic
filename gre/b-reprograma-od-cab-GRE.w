...
ON ROW-DISPLAY OF br_table IN FRAME F-Main /* GRE - ACTIVA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-ENTRY OF br_table IN FRAME F-Main /* GRE - ACTIVA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* GRE - ACTIVA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON START-SEARCH OF br_table IN FRAME F-Main /* GRE - ACTIVA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* GRE - ACTIVA */
DO:
    ...
    IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(LocalItem) THEN DO:
        ...
        */
        ...
        FOR EACH Di-RutaD NO-LOCK WHERE DI-RutaD.CodCia = s-CodCia
            ...
            IF B-CDOCU.flgest = 'NE' THEN DO:       /* NO ENTREGADO */
                {gre/i-reprograma-od-cab-gre.i}
            END.
        END.
        /* ************************************************************************************** */
...
FIND FIRST x-faccpedi WHERE x-faccpedi.codcia = s-codcia AND 
    x-faccpedi.coddoc = x-almcdocu.coddoc AND /* O/D */
...
FOR EACH Di-RutaD NO-LOCK WHERE DI-RutaD.CodCia = s-CodCia
    ...
    IF NOT (B-CDOCU.flgest = 'NE' OR B-CDOCU.flgest = 'DT') THEN NEXT.      /* Pasamos al siguiente registro */
    /* ************************************************************************ */
    /* 
        06/10/2023: La GRE debe estar RECHAZADO POR SUNAT o BAJA SUNAT o ANULADO 
...
/*         END.                                                                                         */
    END.
    /* 19/10/2023: En el CIERRE de HR (2) ya se da de BAJA EN SUNAT a la GRE */
    /* 12/03/2026: En el CIERRE de HR (2) ya >>> NO <<< se da de BAJA EN SUNAT a la GRE, lo van a hacer manualmente */
...
END.  
IF NOT CAN-FIND(FIRST T-CDOCU NO-LOCK) THEN DO:
    MESSAGE 'NO hay comprobantes válidos' AlmCDocu.Libre_c01 AlmCDocu.Libre_c02 SKIP
        'Proceso abortado' VIEW-AS ALERT-BOX WARNING.
    RETURN 'ADM-ERROR'. 
END.
...
IF ERROR-STATUS:ERROR OR fFchEnt = ? THEN RETURN 'ADM-ERROR'.
