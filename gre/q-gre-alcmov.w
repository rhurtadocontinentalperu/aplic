...
&SCOPED-DEFINE Condicion gre_header.m_CodAlm = Almtdocm.CodAlm ~
AND gre_header.m_TipMov = Almtdocm.TipMov ~
AND gre_header.m_CodMov = Almtdocm.CodMov/* ~
AND gre_header.serieGuia = S-NROSER*/
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH gre_header WHERE {&Condicion} NO-LOCK ~
    BY gre_header.ncorrelatio INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH gre_header WHERE {&Condicion} NO-LOCK ~
    BY gre_header.ncorrelatio INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main gre_header
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main gre_header
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.gre_header WHERE INTEGRAL.Almtdocm <external> ..."
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE"
     _OrdList          = "INTEGRAL.gre_header.ncorrelatio|yes"
...
/*  RUN Carga-Series.*/
  ...
  */
   ...
   /*ASSIGN C-NroSer:VISIBLE IN FRAME {&FRAME-NAME} = NO .*/
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    ...
    IF input-var-2 = "I" THEN T-TITULO = "Ingresos de Almacen".
       ELSE T-TITULO = "Salidas de Almacen GRE".
    
    
    RUN gre\C-gre-MovAlm (T-TITULO).
...
FIND FIRST gre_header WHERE {&Condicion} 
     AND gre_header.ncorrelatio = F-Numero NO-LOCK NO-ERROR.

/*
FIND FIRST gre_header WHERE gre_header.m_CodAlm = Almtdocm.CodAlm
    AND gre_header.m_TipMov = Almtdocm.TipMov
    AND gre_header.m_CodMov = Almtdocm.CodMov
    AND gre_header.ncorrelatio = F-Numero NO-LOCK NO-ERROR.
*/
IF AVAILABLE gre_header THEN DO:
    output-var-1 = ROWID(gre_header).
END.
ELSE DO:
    output-var-1 = ?.
END.
...
IF NOT OUTPUT-VAR-1 = ? THEN DO:
   FIND FIRST gre_header WHERE ROWID(gre_header) = OUTPUT-VAR-1 NO-LOCK NO-ERROR.   
   IF AVAIL gre_header THEN DO:       
