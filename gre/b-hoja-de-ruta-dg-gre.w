...
Di-RutaDG.CodAlm Di-RutaDG.Glosa 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Di-RutaDG.NroRef COLUMN-LABEL "Correlativo" FORMAT "X(11)":U
      Di-RutaDG.HorEst COLUMN-LABEL "Hora!Estimada" FORMAT "XX:XX":U
      Di-RutaDG.CodAlm COLUMN-LABEL "Almacén!Origen" FORMAT "x(3)":U
      Di-RutaDG.Glosa COLUMN-LABEL "Descripcion" FORMAT "x(100)":U
            WIDTH 110.43
  ENABLE
      Di-RutaDG.NroRef
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 136 BY 6.92
...
DO:
  ...
  FIND gre_header WHERE gre_header.m_divorigen = s-coddiv AND
      gre_header.m_coddoc = "" AND      /* Guia "itinerante" en blanco */
      gre_header.SerieGuia = INTEGER(SUBSTRING(SELF:SCREEN-VALUE,1,3)) AND
      gre_header.NumeroGuia = INTEGER(SUBSTRING(SELF:SCREEN-VALUE,4)) AND
      gre_header.m_rspta_sunat = "ACEPTADO POR SUNAT"
      NO-LOCK NO-ERROR.

  IF AVAILABLE gre_header THEN 
      ASSIGN
      Di-RutaDG.codalm:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = gre_header.m_codalm
      /*Di-RutaDG.tipdes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = Ccbcdocu.codant*/
      Di-RutaDG.glosa:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = gre_header.razonSocialDestinatario.
END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN 
    ...
    Di-RutaDG.CodAlm = gre_header.m_CodAlm
    /*Di-RutaDG.TipDes = Ccbcdocu.CodAnt*/
    /*Di-RutaDG.CodDes = Ccbcdocu.CodCli*/
    Di-RutaDG.CodDes = gre_header.m_cliente
    Di-RutaDG.Glosa  = gre_header.descripcionMotivoTraslado + " / " +
                        gre_header.direccionPtoLlegada.
...
------------------------------------------------------------------------------*/
  ...
  IF di-rutac.flgest = "C" THEN DO:
      MESSAGE "Hoja de Ruta esta CERRADA" SKIP "Acceso Denegado" VIEW-AS ALERT-BOX ERROR.
      RETURN "ADM-ERROR".
  END.

  /* Longitud del campo */
  IF LENGTH(TRIM(Di-RutaDG.NroRef:SCREEN-VALUE IN BROWSE {&browse-name})) <> 11 
      THEN DO:
      MESSAGE 'Debe ingresar 11 caracteres' SKIP
          'Los tres primeros corresponden a la serie' SKIP
          'Los ocho restante corresponden al correlativo'
          VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO Di-RutaDG.NroRef.
    RETURN 'ADM-ERROR'.
  END.
  ...
  FIND gre_header WHERE gre_header.m_divorigen = s-coddiv AND
      gre_header.m_coddoc = "" AND      /* Guia "itinerante" en blanco */
      gre_header.SerieGuia = INTEGER(SUBSTRING(Di-RutaDG.NroRef:SCREEN-VALUE IN BROWSE {&browse-name},1,3)) AND
      gre_header.NumeroGuia = INTEGER(SUBSTRING(Di-RutaDG.NroRef:SCREEN-VALUE IN BROWSE {&browse-name},4)) AND
      gre_header.m_rspta_sunat = "ACEPTADO POR SUNAT"
      NO-LOCK NO-ERROR.
  IF NOT AVAILABLE gre_header THEN DO:
      MESSAGE 'Guia NO encontrada' VIEW-AS ALERT-BOX ERROR.
      RETURN 'ADM-ERROR'.
  END.

  RUN GET-ATTRIBUTE("ADM-NEW-RECORD").
  IF RETURN-VALUE = "YES" THEN DO:
      IF CAN-FIND(FIRST DI-RutaDG OF DI-RutaC WHERE DI-RutaDG.nroref = DI-RutaDG.NroRef:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} NO-LOCK)
          THEN DO:
