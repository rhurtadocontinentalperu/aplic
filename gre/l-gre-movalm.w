...
Almcmov.CodAlm = input-var-1 AND ~
Almcmov.TipMov = INPUT-VAR-2 AND ~
Almcmov.CodMov = INTEGER(INPUT-VAR-3)
*/
&SCOPED-DEFINE CONDICION gre_header.m_CodAlm = input-var-1 AND ~
gre_header.m_TipMov = INPUT-VAR-2 AND ~
gre_header.m_CodMov = INTEGER(INPUT-VAR-3)

/*&SCOPED-DEFINE CODIGO ALMCMOV.NRODOC */
/*&SCOPED-DEFINE CODIGO gre_header.ncorrelatio*/
&SCOPED-DEFINE SERIE gre_header.serieGuia
&SCOPED-DEFINE CODIGO gre_header.numeroGuia

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( gre_header.m_nomref BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX (  gre_header.m_nomref , FILL-IN-filtro ) <> 0 )
...
&Scoped-define INTERNAL-TABLES gre_header

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table gre_header.ncorrelatio ~
gre_header.serieGuia gre_header.numeroGuia gre_header.fechaEmisionGuia ~
gre_header.m_nomref gre_header.m_rspta_sunat 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH gre_header WHERE ~{&KEY-PHRASE} ~
      AND {&condicion} NO-LOCK ~
    BY gre_header.ncorrelatio DESCENDING INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH gre_header WHERE ~{&KEY-PHRASE} ~
      AND {&condicion} NO-LOCK ~
    BY gre_header.ncorrelatio DESCENDING INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table gre_header
&Scoped-define FIRST-TABLE-IN-QUERY-br_table gre_header


/* Definitions for FRAME F-Main                                         */

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-serie FILL-IN-codigo CMB-filtro ~
FILL-IN-filtro br_table RECT-1 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-serie FILL-IN-codigo CMB-filtro ~
FILL-IN-filtro 
...
<FOREIGN-KEYS>
Nombres que inicien con|y||INTEGRAL.gre_header.m_nomref
Nombres que contengan|y||INTEGRAL.gre_header.m_nomref
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.gre_header.fechaEmisionGuia|yes,INTEGRAL.gre_header.ncorrelatio|yes
Descripción|||INTEGRAL.gre_header.m_nomref|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
DEFINE VARIABLE FILL-IN-serie AS INTEGER FORMAT "9999":U INITIAL 0 
     LABEL "Serie" 
     VIEW-AS FILL-IN 
     SIZE 4.72 BY .81
     BGCOLOR 15 FGCOLOR 0  NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      gre_header.ncorrelatio COLUMN-LABEL "Correlativo" FORMAT "9999999999":U
            WIDTH 7.57
      gre_header.serieGuia COLUMN-LABEL "Serie!Guia" FORMAT "999":U
            WIDTH 4.43
      gre_header.numeroGuia COLUMN-LABEL "Nro!Guia" FORMAT "99999999":U
            WIDTH 9.43
      gre_header.fechaEmisionGuia COLUMN-LABEL "Fecha!Emision" FORMAT "99/99/9999":U
            WIDTH 8.43
      gre_header.m_nomref COLUMN-LABEL "Descripcion" FORMAT "x(100)":U
            WIDTH 31.72
      gre_header.m_rspta_sunat COLUMN-LABEL "Respuesta!Sunat" FORMAT "x(25)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 79.14 BY 12.35
...
DEFINE FRAME F-Main
     FILL-IN-serie AT ROW 1.23 COL 21.57 COLON-ALIGNED
     FILL-IN-codigo AT ROW 1.23 COL 30.86 COLON-ALIGNED
     CMB-filtro AT ROW 2.08 COL 2.14 NO-LABEL
     FILL-IN-filtro AT ROW 2.12 COL 23.57 NO-LABEL
     br_table AT ROW 3.08 COL 2.14
     "Buscar x" VIEW-AS TEXT
          SIZE 8 BY .58 AT ROW 1.35 COL 6
          FONT 6
     RECT-1 AT ROW 1 COL 1.43
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.gre_header"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE"
     _OrdList          = "INTEGRAL.gre_header.ncorrelatio|no"
     _Where[1]         = "{&condicion}"
     _FldNameList[1]   > INTEGRAL.gre_header.ncorrelatio
"gre_header.ncorrelatio" "Correlativo" "9999999999" "int64" ? ? ? ? ? ? no ? no no "7.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.gre_header.serieGuia
"gre_header.serieGuia" "Serie!Guia" "999" "integer" ? ? ? ? ? ? no ? no no "4.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.gre_header.numeroGuia
"gre_header.numeroGuia" "Nro!Guia" "99999999" "integer" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.gre_header.fechaEmisionGuia
"gre_header.fechaEmisionGuia" "Fecha!Emision" ? "date" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.gre_header.m_nomref
"gre_header.m_nomref" "Descripcion" ? "character" ? ? ? ? ? ? no ? no no "31.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.gre_header.m_rspta_sunat
"gre_header.m_rspta_sunat" "Respuesta!Sunat" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* Nro. */
DO:
...
ON LEAVE OF FILL-IN-serie IN FRAME F-Main /* Serie */
DO:
    IF INPUT FILL-IN-codigo = 0 THEN RETURN.
    /*
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY gre_header.fechaEmisionGuia BY gre_header.ncorrelatio
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY gre_header.m_nomref
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = STRING( {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.ncorrelatio, "999999" )
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.m_nomref.
