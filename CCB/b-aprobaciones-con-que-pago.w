...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      tt-w-report.Campo-C[4] COLUMN-LABEL "Doc.!Pago" FORMAT "X(8)":U
            WIDTH 6.43
      tt-w-report.Campo-C[5] COLUMN-LABEL "Numero!Pago" FORMAT "X(15)":U
            WIDTH 10.43
      tt-w-report.Campo-C[6] COLUMN-LABEL "Mone!Pago" FORMAT "X(8)":U
            WIDTH 7.43
      tt-w-report.Campo-D[2] COLUMN-LABEL "Fecha!Pago" FORMAT "99/99/9999":U
            WIDTH 8.43
      tt-w-report.Campo-F[2] COLUMN-LABEL "Importe!Pagado" FORMAT "->>,>>>,>>9.99":U
      fEstado(tt-w-report.Campo-C[7]) @ x-col-estado COLUMN-LABEL "Estado" FORMAT "x(25)":U
            WIDTH 12.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 62.57 BY 4.73
         FONT 4
         TITLE "Documentos de pagos".
...
"Campo-C[4]" "Doc.!Pago" ? "character" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-w-report.Campo-C[5]
"Campo-C[5]" "Numero!Pago" "X(15)" "character" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-w-report.Campo-C[6]
"Campo-C[6]" "Mone!Pago" ? "character" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-w-report.Campo-D[2]
"Campo-D[2]" "Fecha!Pago" ? "date" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.tt-w-report.Campo-F[2]
"Campo-F[2]" "Importe!Pagado" "->>,>>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Documentos de pagos */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Documentos de pagos */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Documentos de pagos */
DO:
...
/*
/* Los A/C que cancelaron la factura del despacho */
FOR EACH ccbdcaja WHERE ccbdcaja.codcia = s-codcia AND
                        ccbdcaja.codref = pCodDOc AND 
                        ccbdcaja.nroref = pNroDoc AND
                        ccbdcaja.coddoc = 'A/C' NO-LOCK:
    /* Que el A/C no este anulado */
    FIND FIRST ccbcdocu WHERE ccbcdocu.codcia = s-codcia AND
                                ccbcdocu.coddoc = ccbdcaja.coddoc AND
                                ccbcdocu.nrodoc = ccbdcaja.nrodoc AND
                                ccbcdocu.flgest <> 'A' NO-LOCK NO-ERROR.
    IF AVAILABLE ccbcdocu THEN DO:
        /* Los ingresos del dinero a caja (I/C) que generaron los A/C */
*/
    /*FOR EACH ttComprobMismaCotizacion WHERE ttComprobMismaCotizacion.tttipo = 'COMPROB':*/
        ...
        FOR EACH x-ccbdcaja WHERE x-ccbdcaja.codcia = s-codcia AND
                                    x-ccbdcaja.coddoc = 'I/C' AND 
                                    x-ccbdcaja.codref = pCodDoc AND /*pCodDoc */         /* FAC,BOL */
                                    x-ccbdcaja.nroref = pNroDoc /* pNroDoc*/ NO-LOCK :
