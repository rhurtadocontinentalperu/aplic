...
&SCOPED-DEFINE CONDICION ccbcdocu.codcia = s-codcia AND (ccbcdocu.codcli = txt-cliente OR txt-cliente = "") ~
         AND (LOOKUP(ccbcdocu.coddoc,documto) > 0) AND ccbcdocu.flgest = 'J' AND ~
         ccbcdocu.fchdoc >= x-Desde AND ccbcdocu.fchdoc <= x-Hasta
...
CcbCDocu.CodDiv CcbCDocu.CodCli CcbCDocu.NomCli gn-clie.Canal ~
CcbCDocu.FchDoc CcbCDocu.FchUbi ~
...
&Scoped-Define ENABLED-OBJECTS txt-cliente btn-Actualizar BUTTON-7 txt-docs ~
x-desde x-hasta txt-canal br_table BUTTON-1 
&Scoped-Define DISPLAYED-OBJECTS txt-cliente txt-docs x-desde x-hasta ~
txt-canal x-TotalMn x-TotalMe 
...
DEFINE VARIABLE txt-canal AS CHARACTER FORMAT "X(256)":U 
     LABEL "Canal" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE txt-docs AS CHARACTER FORMAT "X(256)":U INITIAL "Todas" 
     LABEL "Documentos" 
     VIEW-AS FILL-IN 
     SIZE 50 BY .81 NO-UNDO.

DEFINE VARIABLE x-desde AS DATE FORMAT "99/99/99":U 
     LABEL "Desde" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .81 NO-UNDO.

DEFINE VARIABLE x-hasta AS DATE FORMAT "99/99/99":U 
     LABEL "Hasta" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .81 NO-UNDO.
...
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 103 BY 13.27
         FONT 4
         TITLE "Documentos Cobranza Dudosa".


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     txt-cliente AT ROW 1.27 COL 10.86 COLON-ALIGNED WIDGET-ID 16
     btn-Actualizar AT ROW 1.54 COL 71 WIDGET-ID 14
     BUTTON-7 AT ROW 2.08 COL 64 WIDGET-ID 18
     txt-docs AT ROW 2.15 COL 10.86 COLON-ALIGNED WIDGET-ID 20
     x-desde AT ROW 3.04 COL 11 COLON-ALIGNED WIDGET-ID 22
     x-hasta AT ROW 3.04 COL 31.29 COLON-ALIGNED WIDGET-ID 24
     txt-canal AT ROW 3.96 COL 11 COLON-ALIGNED WIDGET-ID 26
     br_table AT ROW 5.27 COL 2
     x-TotalMn AT ROW 18.73 COL 82.72 COLON-ALIGNED WIDGET-ID 2
     BUTTON-1 AT ROW 18.73 COL 96.72 WIDGET-ID 6
     x-TotalMe AT ROW 19.54 COL 82.72 COLON-ALIGNED WIDGET-ID 4
...
/* BROWSE-TAB br_table txt-canal F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "INTEGRAL.CcbCDocu.CodCli|yes,INTEGRAL.CcbCDocu.FchDoc|yes"
     _Where[1]         = "{&CONDICION}"
...
"CcbCDocu.CodDiv" "Division" "x(5)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.CodCli" ? ? "character" ? ? ? ? ? ? no ? no no "9.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[6]   = INTEGRAL.gn-clie.Canal
...
"IF (CcbCDocu.CodMon =1 ) THEN (CcbCDocu.ImpTot) ELSE (0) @ ImpMndS" "Importe S/." "->>,>>>,>>9.99" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > "_<CALC>"
"IF (CcbCDocu.CodMon =2 ) THEN (CcbCDocu.ImpTot) ELSE (0) @ ImpMndD" "Importe $" "->>,>>>,>>9.99" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Documentos Cobranza Dudosa */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Documentos Cobranza Dudosa */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Documentos Cobranza Dudosa */
DO:
...
DO:  
  ...
  IF txt-docs = "Todas" THEN 
      documto = "FAC,BOL,LET,CHQ,N/C,N/D".
  ELSE documto = txt-docs.
...
DO:
    DEF VAR docs AS CHARACTER NO-UNDO.
    RUN ccb\w-documtos (OUTPUT docs).
    ASSIGN txt-docs = docs.
    DISPLAY docs @ txt-docs WITH FRAME {&FRAME-NAME}.
