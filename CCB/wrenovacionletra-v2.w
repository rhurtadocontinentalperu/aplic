...
DEFINE NEW SHARED VARIABLE S-IMPTOT   AS DECIMAL.
...
&Scoped-Define ENABLED-OBJECTS RECT-58 COMBO-NroSer 
...
DEFINE VARIABLE h_trenovacionletra AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vrenovacionletra AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     RECT-58 AT ROW 8 COL 2 WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 108.14 BY 25.85
...
ON END-ERROR OF W-Win /* RENOVACION DE LETRAS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* RENOVACION DE LETRAS */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_p-updv14 ( 1.42 , 54.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/ccb/vrenovacionletra.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vrenovacionletra ).
       RUN set-position IN h_vrenovacionletra ( 8.27 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.85 , 102.57 ) */
       ...
       /* Links to SmartViewer h_vrenovacionletra. */
       RUN add-link IN adm-broker-hdl ( h_p-updv14 , 'TableIO':U , h_vrenovacionletra ).
       RUN add-link IN adm-broker-hdl ( h_qcanjexletra-v2 , 'Record':U , h_vrenovacionletra ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_bcanjexletra-02 ( 17.42 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/ccb/trenovacionletra.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_trenovacionletra ).
       RUN set-position IN h_trenovacionletra ( 17.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_trenovacionletra ( 8.50 , 79.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv95 ).
       RUN set-position IN h_p-updv95 ( 18.50 , 82.00 ) NO-ERROR.
       RUN set-size IN h_p-updv95 ( 5.65 , 15.00 ) NO-ERROR.

       /* Links to SmartBrowser h_trenovacionletra. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_trenovacionletra ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_trenovacionletra ,
             h_vrenovacionletra , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv95 ,
             h_trenovacionletra , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_tcanjexletra-03 ( 17.42 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv97 ).
       RUN set-position IN h_p-updv97 ( 18.50 , 67.00 ) NO-ERROR.
       RUN set-size IN h_p-updv97 ( 5.12 , 14.00 ) NO-ERROR.
...
CASE L-Handle:
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:    
        RUN select-page(2).
        IF VALID-HANDLE(h_trenovacionletra) THEN RUN dispatch IN h_trenovacionletra ('open-query':U).
