...
&SCOPED-DEFINE Condicion ( CcbTabla.CodCia = s-CodCia AND CcbTabla.Tabla = s-Tabla AND CcbTabla.Codigo = s-Codigo )
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH CcbTabla WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH CcbTabla WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main CcbTabla
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main CcbTabla
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Tabla q-tables 
...
FIND FIRST CcbTabla WHERE {&Condicion} NO-LOCK NO-ERROR.
