...
DEF VAR s-Acceso-Total AS LOG INIT YES NO-UNDO.
DEF VAR s-Mensaje-de-Error AS CHAR NO-UNDO.
DEF VAR s-Tipo-de-Cliente AS CHAR INIT 'NORMAL' NO-UNDO.
...
Gn-ClieL.LCImpDiv[1] Gn-ClieL.LCImpDiv[2] Gn-ClieL.LCSdoDiv 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table Gn-ClieL.FchIni ~
Gn-ClieL.FchFin Gn-ClieL.ImpLC Gn-ClieL.LCImpDiv[1] Gn-ClieL.LCImpDiv[2] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Gn-ClieL.LCImpDiv[1] COLUMN-LABEL "Div. 00150!ATLAS" FORMAT ">>>,>>>,>>9.99":U
      Gn-ClieL.LCImpDiv[2] COLUMN-LABEL "Div. 00070!PERU COMPRAS" FORMAT ">>>,>>>,>>9.99":U
      Gn-ClieL.LCSdoDiv COLUMN-LABEL "OTROS" FORMAT "->>>,>>>,>>9.99":U
            WIDTH 6.72
  ENABLE
      Gn-ClieL.FchIni
      Gn-ClieL.FchFin
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 102.72 BY 6.08
...
"Gn-ClieL.ImpLC" ? ">,>>>,>>>,>>9.99" "decimal" ? ? ? ? ? ? yes ? no no "16" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[7]   > INTEGRAL.Gn-ClieL.LCImpDiv[1]
"Gn-ClieL.LCImpDiv[1]" "Div. 00150!ATLAS" ">>>,>>>,>>9.99" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > INTEGRAL.Gn-ClieL.LCImpDiv[2]
"Gn-ClieL.LCImpDiv[2]" "Div. 00070!PERU COMPRAS" ">>>,>>>,>>9.99" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > INTEGRAL.Gn-ClieL.LCSdoDiv
"Gn-ClieL.LCSdoDiv" "OTROS" ? "decimal" ? ? ? ? ? ? no ? no no "6.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  /* *************************************************** */
  /* Verificar si pertenece a un grupo y si es el master */
  /* *************************************************** */
  RUN Valida-Grupo.
  IF s-Acceso-Total = NO THEN DO:
      MESSAGE s-Mensaje-de-error SKIP(1) 'Proceso Abortado' VIEW-AS ALERT-BOX ERROR.
      RETURN 'ADM-ERROR'.
  END.
  /* *************************************************** */
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      Gn-ClieL.LCCodDiv[1] = '00150'    /* ATLAS */
      Gn-ClieL.LCCodDiv[2] = '00070'.   /* PERU COMPRAS */
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
/*   IF s-Tipo-de-Cliente = 'AGRUPADOR' THEN                            */
/*       ASSIGN                                                         */
/*       Gn-ClieL.LCImpDiv[1]:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO   */
/*       Gn-ClieL.LCImpDiv[2]:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO.  */
/*   ELSE                                                               */
/*       ASSIGN                                                         */
/*       Gn-ClieL.LCImpDiv[1]:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES  */
/*       Gn-ClieL.LCImpDiv[2]:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES. */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Valida-Grupo B-table-Win 
PROCEDURE Valida-Grupo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
  ...
  IF AVAILABLE Vtadtabla THEN DO:
      ...
      IF AVAILABLE Vtactabla THEN DO:
          FIND B-CLIE WHERE B-CLIE.codcia = cl-codcia
              AND B-CLIE.codcli = Vtactabla.Llave
              NO-LOCK NO-ERROR.
          IF AVAILABLE B-CLIE THEN DO:
              s-Acceso-Total = NO.
              s-Mensaje-de-Error = 'Este cliente pertenece a un grupo' + CHR(10) +
                  'Cliente Principal: ' + B-CLIE.nomcli.
          END.
      END.
  END.
  FIND FIRST Vtactabla WHERE VtaCTabla.CodCia = s-codcia
      AND VtaCTabla.Tabla = 'CLGRP'
      AND VtaCTabla.Llave = gn-clie.CodCli
      NO-LOCK NO-ERROR.
  IF AVAILABLE Vtactabla THEN s-Tipo-de-Cliente = 'AGRUPADOR'.
  /* *************************************************** */
