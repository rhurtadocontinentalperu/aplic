...
ASSIGN
    ...
    /* 09.09.09 CONSISTNCIA DEL PEDIDO MOSTRADOR */
    ...
    /*Busca Promocion*/
    RUN vta/r-promociones (d_rowid,OUTPUT cMess).      
        ...
        IF AVAILABLE CcbCDocu THEN DO:
            IF Ccbcdocu.CodDoc = "FAC" THEN DO:
                CASE Ccbcdocu.coddiv:
                    WHEN '00003' THEN RUN ccb/r-fact01 (ROWID(ccbcdocu)).
                    OTHERWISE RUN ccb/r-fact02 (ROWID(ccbcdocu)).
                END CASE.
...
/*                 END.                                                         */
            ...
            IF AVAIL CcbDdocu THEN DO:
                CASE s-coddiv:
                    ...
                    OTHERWISE RUN vtamay/r-odesp-001a (ROWID(ccbcdocu), CcbCDocu.CodAlm, 'ORIGINAL').
                END CASE.
            END.
        END.
    END.
/*     /* RHC 18.06.10 */                                                       */
/*     /* BARREMOS TODOS LOS DOCUMENTOS RELACIONADOS CON EL PEDIDO MOSTRADOR */ */
/*     DEF VAR x-Almacenes AS CHAR NO-UNDO.                                     */
/*     FOR EACH Facdpedm OF Faccpedm NO-LOCK BREAK BY Facdpedm.almdes:          */
/*         IF FIRST-OF(Facdpedm.almdes) THEN DO:                                */
/*             IF x-Almacenes = '' THEN x-Almacenes = Facdpedm.AlmDes.          */
/*             ELSE x-Almacenes = x-Almacenes + ',' + Facdpedm.AlmDes.          */
/*         END.                                                                 */
/*     END.                                                                     */
/*     RUN vtamay/r-odesp-01b ( ROWID(Faccpedm), x-Almacenes, 'ORIGINAL').      */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE proc_CreaComprobante L-table-Win 
