...
CcbCDocu.FchDoc CcbCDocu.ImpTot CcbCDocu.imptot2 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table CcbCDocu.imptot2 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.imptot2 COLUMN-LABEL "Anticipo por Aplicar" FORMAT ">>>,>>>,>>9.99":U
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.CcbCDocu WHERE INTEGRAL.gn-clie ..."
...
"
     ...
     _FldNameList[5]   > INTEGRAL.CcbCDocu.imptot2
"CcbCDocu.imptot2" "Anticipo por Aplicar" ">>>,>>>,>>9.99" "decimal" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  RUN Saldo-Actual (OUTPUT x-imptot, OUTPUT x-sdoact).

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'assign-statement':U ) .

  /* Code placed here will execute AFTER standard behavior.    */
  /* actualizamos el control de cierres */
  FIND FIRST ccbdmov WHERE ccbdmov.codcia = ccbcdocu.codcia
      AND ccbdmov.coddoc = ccbcdocu.coddoc
      AND ccbdmov.nrodoc = ccbcdocu.nrodoc
      AND ccbdmov.codref = 'CIE'
  ...
  END.
  ASSIGN
      ccbdmov.imptot = (ccbcdocu.imptot - x-imptot) - ccbcdocu.imptot2.
  IF ccbdmov.imptot = 0 THEN DELETE ccbdmov.
  RELEASE ccbdmov.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Saldo-Actual B-table-Win 
PROCEDURE Saldo-Actual :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  DEF OUTPUT PARAMETER pImpTot AS DEC.
  DEF OUTPUT PARAMETER pSdoAct AS DEC.

  /* calculamos las aplicaciones */
  pImpTot = 0.
  FOR EACH ccbdmov NO-LOCK WHERE ccbdmov.codcia = s-codcia
      AND ccbdmov.coddoc = ccbcdocu.coddoc
      AND ccbdmov.nrodoc = ccbcdocu.nrodoc
      AND ccbdmov.codref <> 'CIE':
      IF ccbcdocu.codmon = ccbdmov.codmon THEN DO:
          pimptot = pimptot + ccbdmov.imptot.
      END.
      ELSE IF ccbcdocu.codmon = 1 THEN DO:
          pimptot = pimptot + ccbdmov.imptot * ccbdmov.tpocmb.
      END.
          ...
          END.
  END.
  psdoact = ccbcdocu.imptot - pimptot.
...
------------------------------------------------------------------------------*/
  ...
  RUN Saldo-Actual (OUTPUT x-imptot, OUTPUT x-sdoact).
  IF DECIMAL(CcbCDocu.imptot2:SCREEN-VALUE IN BROWSE {&browse-name}) > x-sdoact THEN DO:
      MESSAGE 'El importe no debe superar los' x-sdoact
          VIEW-AS ALERT-BOX ERROR.
      APPLY 'ENTRY':U TO CcbCDocu.imptot2 IN BROWSE {&browse-name}.
      RETURN 'ADM-ERROR'.
  END.
