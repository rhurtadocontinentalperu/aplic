...
/* 
Sintaxis:
    run ccb/waprobarncndotrosv2.r(NC|N|)
    run ccb/waprobarncndotrosv2.r(NC|N|REBATE)
    run ccb/waprobarncndotrosv2.r(ND|N|)
Parámetros:
...
DEFINE VARIABLE h_b-anticipos-detalle AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 7.19 COL 102
     BUTTON-2 AT ROW 8.73 COL 102
     BtnDone AT ROW 10.27 COL 102 WIDGET-ID 6
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_baprobarncndotrosv2 ( 6.38 , 1.14 ) NO-ERROR.
       RUN set-size IN h_baprobarncndotrosv2 ( 5.88 , 97.00 ) NO-ERROR.
       ...
       RUN set-position IN h_vnotacreditodebito ( 12.35 , 1.29 ) NO-ERROR.
       /* Size in UIB:  ( 8.92 , 115.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/ccb/b-anticipos-detalle.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-anticipos-detalle ).
       RUN set-position IN h_b-anticipos-detalle ( 21.38 , 9.00 ) NO-ERROR.
       RUN set-size IN h_b-anticipos-detalle ( 4.81 , 85.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-anticipos-detalle ,
...
RUN refrescar-anticipos IN h_b-anticipos-detalle(INPUT pCoddoc, INPUT pNroDoc).
/*RUN refrescar-anticipos IN h_b-aprobaciones-con-que-pago(INPUT pCoddoc, INPUT pNroDoc).*/
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER pxCodDoc AS CHAR.
