...
ASSIGN
    ...
    x-Sem7-1 = EvtSemana.FecIni + 7 * 5
    x-Sem7-2 = EvtSemana.FecFin + 7 * 5
    x-Sem8-1 = EvtSemana.FecIni + 7 * 6
    x-Sem8-2 = EvtSemana.FecFin + 7 * 6
    x-Sem9-1 = EvtSemana.FecIni + 7 * 7
    x-Sem9-2 = EvtSemana.FecFin + 7 * 7
    x-Sem10-1 = EvtSemana.FecIni + 7 * 8
    x-Sem10-2 = EvtSemana.FecFin + 7 * 8.
...
FIND NEXT EvtSemana NO-LOCK NO-ERROR.
IF AVAILABLE EvtSemana THEN x-Label-6 = "Sem " + STRING(EvtSemana.NroSem, '99').
FIND NEXT EvtSemana NO-LOCK NO-ERROR.
IF AVAILABLE EvtSemana THEN x-Label-7 = "Sem " + STRING(EvtSemana.NroSem, '99').
FIND NEXT EvtSemana NO-LOCK NO-ERROR.
IF AVAILABLE EvtSemana THEN x-Label-8 = "Sem " + STRING(EvtSemana.NroSem, '99').
FIND NEXT EvtSemana NO-LOCK NO-ERROR.
IF AVAILABLE EvtSemana THEN x-Label-9 = "Sem " + STRING(EvtSemana.NroSem, '99').

x-Label-10 = "Por Vencer".
...
DEF VAR VendProv AS CHARACTER.
DEF VAR VendSupMer AS CHARACTER.
DEF VAR VendMesaRed AS CHARACTER.

VendProv = "015,017,173,900,901,902".
VendSupMer = "151".
...
DO i = 1 TO NUM-ENTRIES(x-FlgEst):
    FOR EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-codcia,
        ...
        /* 30-04-2012, Enrique Macchiu, todos los dctos 
        ...
        */
        ...
        /* 30-04-2012, enrique Macchiu, ampliar a 4 semanas mas
        IF Detalle.FchCobranza >= x-Sem6-1 THEN Detalle.Periodo = x-Label-6.*/
        IF Detalle.FchCobranza >= x-Sem6-1 AND Detalle.FchCobranza <= x-Sem6-2
            THEN Detalle.Periodo = x-Label-6.
        IF Detalle.FchCobranza >= x-Sem7-1 AND Detalle.FchCobranza <= x-Sem7-2
            THEN Detalle.Periodo = x-Label-7.
        IF Detalle.FchCobranza >= x-Sem8-1 AND Detalle.FchCobranza <= x-Sem8-2
            THEN Detalle.Periodo = x-Label-8.
        IF Detalle.FchCobranza >= x-Sem9-1 AND Detalle.FchCobranza <= x-Sem9-2
            THEN Detalle.Periodo = x-Label-9.
        IF Detalle.FchCobranza >= x-Sem10-1 THEN Detalle.Periodo = x-Label-10.
