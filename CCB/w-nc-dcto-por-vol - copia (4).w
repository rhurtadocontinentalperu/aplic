...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "LIQUIDACION NOTAS DE CREDITO/NOTAS DE DEBITO POR DESCUENTO POR VOLUMEN"
         HEIGHT             = 25.58
         WIDTH              = 135.43
         MAX-HEIGHT         = 25.58
         MAX-WIDTH          = 178.72
...
ON END-ERROR OF W-Win /* LIQUIDACION NOTAS DE CREDITO/NOTAS DE DEBITO POR DESCUENTO POR VOLUMEN */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* LIQUIDACION NOTAS DE CREDITO/NOTAS DE DEBITO POR DESCUENTO POR VOLUMEN */
DO:
...
DO k = 1 TO NUM-ENTRIES(x-Clientes):
    ...
    /* Solo cotizaciones de las divisiones válidas */
        ...
        FOR EACH PriCDtoVolAcu NO-LOCK WHERE PriCDtoVolAcu.CodCia = s-codcia 
            ...
            FOR EACH PEDIDO NO-LOCK WHERE PEDIDO.codcia = s-codcia
                ...
                FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia
                    ...
                    /* Solo los artículos válidos de la promoción */
...
FOR EACH PriCDtoVolAcu NO-LOCK WHERE PriCDtoVolAcu.CodCia = s-codcia AND PriCDtoVolAcu.Inactivo = NO:
    ...
    /* 2do.                                */
    /* Calculamos importe teórico EN SOLES */
    FOR EACH PriDDtoVolAcu OF PriCDtoVolAcu NO-LOCK WHERE PriDDtoVolAcu.Tabla = "SKU":
        FOR EACH T-DDOCU NO-LOCK WHERE T-DDOCU.codmat = PriDDtoVolAcu.Llave_c1:
...
FOR EACH PriCDtoVolAcu NO-LOCK WHERE PriCDtoVolAcu.CodCia = s-codcia AND PriCDtoVolAcu.Inactivo = NO:
    /* Definimos los rangos de fecha válidos */
            ...
            FOR EACH PEDIDO NO-LOCK WHERE PEDIDO.codcia = s-codcia
                ...
                FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia
                    ...
                    IF pOk THEN DO:
                        ...
                        END.
                    END.
                    /* Buscamos si la Factura tiene N/C por devolución de mercadería */
                    FOR EACH CREDITO NO-LOCK WHERE CREDITO.codcia = s-codcia
                        AND CREDITO.coddoc = "N/C"
