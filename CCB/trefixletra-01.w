...
DEFINE VAR x-Doc-Valido AS CHAR INIT 'LET,N/D,DCO' NO-UNDO.
...
DEFINE VAR x-tabla-factabla AS CHAR INIT "TD-REFINAN".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      MVTO.CodRef FORMAT "x(3)":U WIDTH 6.43 VIEW-AS COMBO-BOX INNER-LINES 3
                      LIST-ITEMS "Ninguno" 
                      DROP-DOWN-LIST 
...
DEFINE FRAME F-Main
     ...
     BUTTON-1 AT ROW 6.38 COL 3 WIDGET-ID 2
...
"MVTO.CodRef" ? ? "character" ? ? ? ? ? ? yes ? no no "6.43" yes no no "U" "" "" "DROP-DOWN-LIST" "," "Ninguno" ? 3 no 0 no no
...
&Scoped-define SELF-NAME MVTO.CodRef
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL MVTO.CodRef br_table _BROWSE-COLUMN B-table-Win
ON ENTRY OF MVTO.CodRef IN BROWSE br_table /* Codigo */
DO:
  IF SELF:SCREEN-VALUE = '' THEN SELF:SCREEN-VALUE = 'LET'.
END.
...
DO:
    input-var-1 = x-Doc-Valido.      /*"LET,N/D".*/
    input-var-2 = S-CODCLI.
    input-var-3 = 'P'.

    RUN lkup\c-doccliest ("Documento Pendientes").
...
------------------------------------------------------------------------------*/
   ...
   IF LOOKUP(MVTO.CodRef:SCREEN-VALUE IN BROWSE {&browse-name}, x-Doc-Valido) = 0 
       THEN DO:
       MESSAGE 'Documento NO válido' SKIP 'Debe ser:' x-Doc-Valido
           VIEW-AS ALERT-BOX ERROR.
       APPLY 'ENTRY':U TO MVTO.CodRef IN BROWSE {&browse-name}.
       RETURN "ADM-ERROR".
   END.
   ...
   IF CcbCDocu.FlgEst <> "P" THEN DO:
      ...
      RETURN "ADM-ERROR".
   END.
/*    IF S-CONDIC = '' THEN S-CONDIC = CcbCDocu.FmaPgo.            */
/*    IF CcbCDocu.FmaPgo <> S-CONDIC THEN DO:                      */
