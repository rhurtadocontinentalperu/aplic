...
DEFINE FRAME F-Main
     ...
     "DEPOSITO DE CHEQUES EN BANCOS" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "ASIENTO CONTABLE POR DEPOSITO DE CHEQUES"
         HEIGHT             = 14.23
         WIDTH              = 94.57
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 95.57
...
ON END-ERROR OF W-Win /* ASIENTO CONTABLE POR DEPOSITO DE CHEQUES */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ASIENTO CONTABLE POR DEPOSITO DE CHEQUES */
DO:
...
FOR EACH t-prev BREAK BY t-prev.coddiv BY t-prev.coddoc BY t-prev.nrodoc:
    ...
    IF LAST-OF(t-prev.coddiv) THEN DO:
       ...
       ASSIGN
           ...
           cb-cmov.NOTAST = 'DEPOSITO DE CHEQUES'
           cb-cmov.GLOAST = ''.
    END.
END.
RUN Actualiza-Flag.
...
FIND Facdocum WHERE Facdocum.codcia = s-codcia
    AND Facdocum.coddoc = "CHD"
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE Facdocum OR FacDocum.CodCta[1] = '' OR Facdocum.CodCta[2] = '' THEN DO:
    MESSAGE 'NO está configurado el documento CHD' VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.
...
FOR EACH CcbCDocu NO-LOCK WHERE CcbCDocu.codcia = s-codcia AND
    ...
    CcbCDocu.FchUbi >= FILL-IN-fchast-1 AND
    CcbCDocu.FchUbi <= FILL-IN-fchast-2 AND
    CcbCDocu.FlgEst <> "A":
    CREATE T-CDOC.
...
FOR EACH T-CDOC BREAK BY T-CDOC.CodDiv BY T-CDOC.Cco BY T-CDOC.NroDoc:
    cRuc = T-CDOC.RucCli.
    /* CUENTA 104XXXXX */
    RUN Graba-Documentos ("13", T-CDOC.codcta, NO, T-CDOC.ImpTot, T-CDOC.NroOrd, T-CDOC.codmon).
    /* CUENTA 101221YY */
    x-codcta  = (IF T-CDOC.codmon = 1 THEN FacDocum.CodCta[1] ELSE FacDocum.CodCta[2]).
    RUN Graba-Documentos ("32", x-codcta, YES, T-CDOC.ImpTot, T-CDOC.NroDoc, T-CDOC.codmon).
END.
...
FIND gn-tcmb WHERE gn-tcmb.fecha = T-CDOC.FchUbi NO-LOCK NO-ERROR.
IF NOT AVAILABLE Gn-tcmb 
THEN MESSAGE 'Tipo de cambio del dia' T-CDOC.FchUbi  'NO está configurado'
        VIEW-AS ALERT-BOX WARNING.
ELSE ASSIGN
