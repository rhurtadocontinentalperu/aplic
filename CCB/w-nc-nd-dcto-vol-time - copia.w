...
COMBO-BOX_Father SELECT_Sons COMBO-BOX_NroSerNC COMBO-BOX_NroSerND ~
BUTTON_Calcular BUTTON_Generar 
...
DEFINE VARIABLE SELECT_Sons AS CHARACTER INITIAL "Seleccione" 
...
DEFINE FRAME F-Main
     ...
     "Seleccione Pack de Productos:" VIEW-AS TEXT
          SIZE 22 BY .5 AT ROW 3.15 COL 9 WIDGET-ID 16
...
FOR EACH T-CDOCU NO-LOCK WHERE LOOKUP(T-CDOCU.CodDoc, 'FAC,BOL') > 0,
    ...
    /*MESSAGE tt-ddocu.coddoc tt-ddocu.nrodoc tt-ddocu.codmat tt-ddocu.candes tt-ddocu.preuni tt-ddocu.implin TT-DDOCU.Por_Dsctos[3].*/
END.
FOR EACH T-DPEDI EXCLUSIVE-LOCK WHERE T-DPEDI.Libre_c01 = "*":
...
FOR EACH T-DPEDI WHERE T-DPEDI.CodDoc = "PNC" AND T-DPEDI.Libre_c01 = "*":      /* OJO */
    x-CanDes = T-DPEDI.CanPed.      /* Cantidad Acumulada a repartir */
    FOR EACH T-CDOCU WHERE LOOKUP(T-CDOCU.CodDoc, 'FAC,BOL') > 0, 
        FIRST T-DDOCU OF T-CDOCU WHERE T-DDOCU.CodMat = T-DPEDI.CodMat,
        FIRST Ccbddocu OF T-CDOCU NO-LOCK WHERE Ccbddocu.CodMat = T-DDOCU.CodMat,
        FIRST Almmmatg OF Ccbddocu NO-LOCK
        BY T-CDOCU.ImpTot DESC:
        CREATE txCcbddocu.
        BUFFER-COPY T-DDOCU TO txCcbddocu
            ASSIGN 
            txCcbddocu.CanDes = MINIMUM(x-CanDes, T-DDOCU.CanDes * T-DDOCU.Factor)
            txCcbddocu.Factor = 1
            txCcbddocu.UndVta = Almmmatg.undstk
            .
        /* Importe de la PNC: aplicamos factor por cantidades */
        ASSIGN
            txCcbddocu.impdto = ABS(T-DDOCU.ImpLin - Ccbddocu.ImpLin) / (Ccbddocu.CanDes * Ccbddocu.Factor) * txCcbddocu.CanDes.
        IF NOT CAN-FIND(FIRST txCcbcdocu OF T-CDOCU NO-LOCK) THEN DO:
            CREATE txCcbcdocu.
            BUFFER-COPY T-CDOCU TO txCcbcdocu.
        END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 RECT-2 FILL-IN_CodCli COMBO-BOX_Father SELECT_Sons 
         COMBO-BOX_NroSerNC COMBO-BOX_NroSerND BUTTON_Calcular BUTTON_Generar 
      WITH FRAME F-Main IN WINDOW W-Win.
...
DO TRANSACTION ON ERROR UNDO GRABAR_DOCUMENTO, LEAVE GRABAR_DOCUMENTO ON STOP UNDO GRABAR_DOCUMENTO, LEAVE GRABAR_DOCUMENTO:
    ...
    FOR EACH ttCcbddocu ON ERROR UNDO, THROW:
        ...
        IF ERROR-STATUS:ERROR THEN DO:
            ...
            UNDO GRABAR_DOCUMENTO, RETURN 'ADM-ERROR'.
        END.
/*         MESSAGE b-ccbddocu.coddoc b-ccbcdocu.nrodoc b-ccbcdocu.codref b-ccbcdocu.nroref */
/*             b-ccbddocu.codmat b-ccbddocu.candes b-ccbddocu.preuni b-ccbddocu.implin     */
/*             VIEW-AS ALERT-BOX.                                                          */
    END.
