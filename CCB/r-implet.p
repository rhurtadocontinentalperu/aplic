...
&Scoped-Define ENABLED-OBJECTS tg-formato FILL-IN-let FILL-IN-let-1 ~
RADIO-tipo_letra BUTTON-print BUTTON-exit RECT-3 
...
DEFINE FRAME F-Main
     tg-formato AT ROW 4.5 COL 35 WIDGET-ID 2
     FILL-IN-let AT ROW 1.77 COL 12 COLON-ALIGNED
     FILL-IN-let-1 AT ROW 1.77 COL 35 COLON-ALIGNED
     RADIO-tipo_letra AT ROW 3.12 COL 14 NO-LABEL
     BUTTON-print AT ROW 6.38 COL 32
     BUTTON-exit AT ROW 6.38 COL 41
     "Tipo de Letra:" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 3.12 COL 4
     RECT-3 AT ROW 5.81 COL 2
...
------------------------------------------------------------------------------*/
  ...
  ENABLE tg-formato FILL-IN-let FILL-IN-let-1 RADIO-tipo_letra BUTTON-print 
         BUTTON-exit RECT-3 
      WITH FRAME F-Main IN WINDOW mesW-Win.
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ASSIGN
            ...
            RADIO-tipo_letra
            tg-formato.
        IF FILL-IN-let-1 = "" THEN FILL-IN-let-1 = "ZZZZZZZZ".
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Imprimir02 mesW-Win 
PROCEDURE Imprimir02 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    RB-REPORT-NAME = 'Imp_letra'.
    GET-KEY-VALUE SECTION 'Startup' KEY 'Base' VALUE RB-REPORT-LIBRARY.
    RB-REPORT-LIBRARY = RB-REPORT-LIBRARY + "vta/rbccb.prl".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE proc_carga-temporal-2 mesW-Win 
PROCEDURE proc_carga-temporal-2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE VARIABLE cNroRef AS CHARACTER INITIAL "-" NO-UNDO.
DEFINE VARIABLE cImpLetras AS CHARACTER NO-UNDO.

FOR EACH ccbcdocu NO-LOCK WHERE
    ccbcdocu.codcia = s-codcia AND
    ccbcdocu.coddoc = "LET" AND
    ccbcdocu.nrodoc >= FILL-IN-let AND
    ccbcdocu.nrodoc <= FILL-IN-let-1 AND
    ccbcdocu.codref = RADIO-tipo_letra:

    IF ccbcdocu.flgest = "A" THEN NEXT.

    RUN bin/_numero(ccbcdocu.imptot, 2, 1, OUTPUT cImpLetras).
    cImpLetras = cImpLetras +
        IF ccbcdocu.codmon = 1 THEN " NUEVOS SOLES" ELSE " DOLARES AMERICANOS".
    ...
    FIND CcbcMvto WHERE CcbCMvto.codcia = s-codcia
        ...
        NO-LOCK NO-ERROR.
    IF AVAILABLE Ccbcmvto THEN cNroRef = Ccbcmvto.NroRef.
    CREATE w-report.
    ASSIGN
        w-report.Task-No = s-task-no                /* ID Tarea */
        w-report.Llave-C = s-user-id                /* ID Usuario */
    ...
    IF w-report.Campo-C[13] <> '' THEN DO:
        /*
        w-report.Campo-C[9] = TRIM (w-report.Campo-C[9]) + ' / ' + w-report.Campo-C[11].
        w-report.Campo-C[13] = TRIM (w-report.Campo-C[13]) + ' / ' + w-report.Campo-C[14].        
        w-report.Campo-C[11] = ''.
        */
        w-report.Campo-C[9] = TRIM (w-report.Campo-C[9]).
        w-report.Campo-C[13] = TRIM (w-report.Campo-C[13]).        
