...
DEFINE VAR x-col-estado-item AS CHAR.
...
fEstado(ttCcbDDocu.Flg_Factor) @ x-col-estado-item ttCcbDDocu.NroItm ~
...
FUNCTION fEstado RETURNS CHARACTER
  ( INPUT pFlg_factor AS CHAR )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      fEstado(ttCcbDDocu.Flg_Factor) @ x-col-estado-item COLUMN-LABEL "Estado" FORMAT "x(20)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS SIZE 132.86 BY 10.5
...
"fEstado(ttCcbDDocu.Flg_Factor) @ x-col-estado-item" "Estado" "x(20)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO TRANSACTION ON ERROR UNDO GRABAR_DATOS, LEAVE GRABAR_DATOS:
    ...
    DO  WHILE AVAILABLE ttccbddocu:
        ...
        FIND FIRST ccbddocu USE-INDEX llave04 WHERE ccbddocu.codcia = ttccbddocu.codcia AND
                                                ccbddocu.coddiv = ttccbddocu.coddiv AND
                                                ccbddocu.coddoc = ttccbddocu.coddoc AND
                                                ccbddocu.nrodoc = ttccbddocu.nrodoc AND
                                                ccbddocu.codmat = ttccbddocu.codmat EXCLUSIVE-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT (TRUE <> (pFlg_factor > "")) THEN DO:
        IF NUM-ENTRIES(pFlg_factor,"|") > 1 THEN DO:
            x-retval = ENTRY(1,pFlg_factor,"|").
        END.
    END.
