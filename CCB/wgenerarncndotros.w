...
/* 
Sintaxis:
    run ccb/wgenerarncndotros.r(NC|N|)
    run ccb/wgenerarncndotros.r(NC|N|REBATE)
    run ccb/wgenerarncndotros.r(ND|N|)
Parámetros:
...
&Scoped-Define ENABLED-OBJECTS COMBO-NroSer BUTTON-1 BUTTON-2 BtnDone 
...
DEFINE VARIABLE h_baprobarncndotros AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     COMBO-NroSer AT ROW 1.19 COL 9.14 WIDGET-ID 10
     BUTTON-1 AT ROW 2.54 COL 95
     BUTTON-2 AT ROW 4.08 COL 95
     BtnDone AT ROW 5.62 COL 95 WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1.04
         SIZE 117.72 BY 20.38
         FONT 4.
...
DO:
  ...
  IF rpta-1 = Yes THEN DO:
    RUN Aprobacion IN h_baprobarncndotros.
  END.
END.
...
DO:
  MESSAGE '¿Confirma RECHAZO?' VIEW-AS ALERT-BOX QUESTION
    BUTTONS YES-NO UPDATE rpta-1 AS LOG.
  IF rpta-1 = Yes THEN DO:
    RUN Rechazo IN h_baprobarncndotros.
  END.
END.
...
ON VALUE-CHANGED OF COMBO-NroSer IN FRAME F-Main /* Seleccione la Serie */
DO:
    ASSIGN COMBO-NroSer.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'CCB/baprobarncndotros.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_baprobarncndotros ).
       RUN set-position IN h_baprobarncndotros ( 2.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_baprobarncndotros ( 5.19 , 91.00 ) NO-ERROR.
       ...
       RUN set-position IN h_vnotacreditodebito ( 7.92 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.65 , 115.00 ) */

       /* Links to SmartViewer h_vnotacreditodebito. */
       RUN add-link IN adm-broker-hdl ( h_baprobarncndotros , 'Record':U , h_vnotacreditodebito ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_baprobarncndotros ,
...
------------------------------------------------------------------------------*/
  ...
  ENABLE COMBO-NroSer BUTTON-1 BUTTON-2 BtnDone 
      WITH FRAME F-Main IN WINDOW W-Win.
