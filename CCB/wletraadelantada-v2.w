...
DEFINE NEW SHARED VAR s-codcli AS CHAR.
DEFINE NEW SHARED VAR s-codmon AS INT.
DEFINE NEW SHARED VAR S-CONDIC   AS CHAR.
...
&Scoped-Define ENABLED-OBJECTS RECT-57 COMBO-NroSer BUTTON-PRELETRA 
...
DEFINE VARIABLE h_qcanjexletra-v2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_tcanjexletra-03 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_tletraadelantada-02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vletraadelantada AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BUTTON-PRELETRA 
     LABEL "IMPORTAR PRE-LETRA" 
     SIZE 20 BY 1.12.
...
DEFINE FRAME F-Main
     COMBO-NroSer AT ROW 6.38 COL 56.71 WIDGET-ID 6
     BUTTON-PRELETRA AT ROW 12.58 COL 67 WIDGET-ID 10
     RECT-57 AT ROW 7.73 COL 2 WIDGET-ID 8
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 108.43 BY 24.42
...
ON END-ERROR OF W-Win /* LETRAS ADELANTADAS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* LETRAS ADELANTADAS */
DO:
...
&Scoped-define SELF-NAME BUTTON-PRELETRA
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-PRELETRA W-Win
ON CHOOSE OF BUTTON-PRELETRA IN FRAME F-Main /* IMPORTAR PRE-LETRA */
DO:
  RUN Importar-preletras IN h_vletraadelantada.
END.
...
DO:
    ...
    RUN dispatch IN h_qcanjexletra-v2 ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_b-division-canje ( 1.00 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 6.12 , 80.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv13 ).
       RUN set-position IN h_p-updv13 ( 6.38 , 2.00 ) NO-ERROR.
       ...
       RUN set-position IN h_vletraadelantada ( 8.00 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.04 , 102.72 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/ccb/qcanjexletra-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'SortBy-Case = NroDoc':U ,
             OUTPUT h_qcanjexletra-v2 ).
       RUN set-position IN h_qcanjexletra-v2 ( 6.38 , 68.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 12.00 ) */

       /* Links to SmartViewer h_vletraadelantada. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_vletraadelantada ).
       RUN add-link IN adm-broker-hdl ( h_qcanjexletra-v2 , 'Record':U , h_vletraadelantada ).

       /* Links to SmartQuery h_qcanjexletra-v2. */
       RUN add-link IN adm-broker-hdl ( h_b-division-canje , 'Record':U , h_qcanjexletra-v2 ).
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_qcanjexletra-v2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_qcanjexletra-v2 ,
             BUTTON-PRELETRA:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_bcanjexletra-02 ( 12.58 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-aderec ).
       RUN set-position IN h_b-aderec ( 20.92 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-aderec ( 4.04 , 59.00 ) NO-ERROR.

       /* Links to SmartBrowser h_bcanjexletra-02. */
       RUN add-link IN adm-broker-hdl ( h_qcanjexletra-v2 , 'Record':U , h_bcanjexletra-02 ).

       /* Links to SmartBrowser h_b-aderec. */
       RUN add-link IN adm-broker-hdl ( h_qcanjexletra-v2 , 'Record':U , h_b-aderec ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/ccb/tletraadelantada-02.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tletraadelantada-02 ).
       RUN set-position IN h_tletraadelantada-02 ( 12.58 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tletraadelantada-02 ( 8.50 , 79.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv95 ).
       RUN set-position IN h_p-updv95 ( 13.65 , 82.00 ) NO-ERROR.
       RUN set-size IN h_p-updv95 ( 5.65 , 15.00 ) NO-ERROR.

       /* Links to SmartBrowser h_tletraadelantada-02. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_tletraadelantada-02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tletraadelantada-02 ,
             h_vletraadelantada , 'AFTER':U ).
    ...
    WHEN 3 THEN DO:
       ...
       RUN set-position IN h_tcanjexletra-03 ( 12.58 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv97 ).
       RUN set-position IN h_p-updv97 ( 13.65 , 67.00 ) NO-ERROR.
       RUN set-size IN h_p-updv97 ( 5.12 , 12.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-57 COMBO-NroSer BUTTON-PRELETRA 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "Pide-Serie" THEN DO:
         ...
         RUN dispatch IN h_qcanjexletra-v2 ('open-query':U).
    END.
    WHEN "Pre-Letra" THEN DO:
        IF VALID-HANDLE(h_qcanjexletra-v2) THEN RUN dispatch IN h_qcanjexletra-v2 ('open-query':U).
    END.
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:    
        RUN select-page(1).
        IF VALID-HANDLE(h_bcanjexletra-02) THEN RUN dispatch IN h_bcanjexletra-02 ('open-query':U).
        IF VALID-HANDLE(h_tletraadelantada-02) THEN RUN dispatch IN h_tletraadelantada-02 ('open-query':U).
        RUN dispatch IN h_qcanjexletra-v2 ('enable':U).
        COMBO-NroSer:SENSITIVE IN FRAME {&FRAME-NAME} = YES.
        BUTTON-PRELETRA:VISIBLE = YES.
        BUTTON-PRELETRA:SENSITIVE = YES.
    END.
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:    
        RUN select-page(2).
        IF VALID-HANDLE(h_tletraadelantada-02) THEN RUN dispatch IN h_tletraadelantada-02 ('open-query':U).
        RUN dispatch IN h_qcanjexletra-v2 ('disable':U).
        COMBO-NroSer:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
        BUTTON-PRELETRA:VISIBLE = NO.
        BUTTON-PRELETRA:SENSITIVE = NO.
    END.
