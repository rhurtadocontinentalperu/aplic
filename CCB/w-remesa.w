...
DEF NEW SHARED VAR s-tipo   AS CHAR INITIAL "REMESAS".
...
FIND CcbDTerm WHERE 
  ...
  IF NOT AVAILABLE ccbdterm THEN DO:
      MESSAGE "Egreso de caja no esta configurado en este terminal" VIEW-AS ALERT-BOX ERROR.
      RETURN "ADM-ERROR".
  END.
...
&Scoped-Define ENABLED-OBJECTS CB-CodDoc Btn-Print 
&Scoped-Define DISPLAYED-OBJECTS CB-CodDoc 
...
DEFINE VARIABLE h_v-remesa-x AS HANDLE NO-UNDO.
...
DEFINE VARIABLE CB-CodDoc AS CHARACTER FORMAT "X(256)":U INITIAL "E/C" 
...
DEFINE FRAME F-Main
     CB-CodDoc AT ROW 8.92 COL 1 NO-LABEL
     Btn-Print AT ROW 8.58 COL 73.14
...
/* SETTINGS FOR FRAME F-Main
                                                                        */
/* SETTINGS FOR COMBO-BOX CB-CodDoc IN FRAME F-Main
   ALIGN-L                                                              */
...
ON END-ERROR OF W-Win /* Remesas de Caja */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Remesas de Caja */
DO:
...
DO:
  RUN dispatch IN h_v-remesa-x ('imprime':U).
END.
...
ON VALUE-CHANGED OF CB-CodDoc IN FRAME F-Main
DO:
   ASSIGN CB-CodDoc.
   s-coddoc  = CB-CodDoc.
   FIND CcbDTerm WHERE 
        ...
        CcbDTerm.CodTer = s-codter NO-LOCK NO-ERROR.
   IF NOT AVAILABLE ccbdterm THEN DO:
      MESSAGE "Documento no esta configurado en este terminal" VIEW-AS ALERT-BOX ERROR.
      RETURN NO-APPLY.
  END.
  s-ptovta = ccbdterm.nroser.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 8.50 , 19.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv10 ).
       RUN set-position IN h_p-updv10 ( 8.50 , 36.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-remesa ).
       RUN set-position IN h_q-remesa ( 8.81 , 7.72 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'g:/sie/sie_co/on_at_co/aplic/ccb/v-remesa-x.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-remesa-x ).
       RUN set-position IN h_v-remesa-x ( 1.08 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 7.62 , 76.57 ) */

       /* Links to SmartViewer h_v-remesa-x. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-remesa-x ).
       RUN add-link IN adm-broker-hdl ( h_q-remesa , 'Record':U , h_v-remesa-x ).
