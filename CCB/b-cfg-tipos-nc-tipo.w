...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaDTabla OF VtaCTabla WHERE ~{&KEY-PHRASE} ~
      ...
      FIRST CcbTabla WHERE CcbTabla.CodCia = VtaDTabla.CodCia ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbTabla.Nombre FORMAT "x(80)":U WIDTH 65.14
  ENABLE
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaDTabla OF INTEGRAL.VtaCTabla,INTEGRAL.CcbTabla WHERE INTEGRAL.VtaDTabla ..."
...
"VtaDTabla.LlaveDetalle" "Concepto" "x(10)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.CcbTabla.Nombre
"CcbTabla.Nombre" ? ? "character" ? ? ? ? ? ? no ? no no "65.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF NOT CAN-FIND(FIRST Ccbtabla WHERE CcbTabla.CodCia = s-codcia AND
                ...
                CcbTabla.Codigo = VtaDTabla.LlaveDetalle:SCREEN-VALUE IN BROWSE {&browse-name}
                NO-LOCK)
    THEN DO:
