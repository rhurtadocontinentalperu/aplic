...
DEF TEMP-TABLE T-ACUMULADO
    FIELD CodCia AS INTE
    FIELD CodMat AS CHAR
    FIELD CanDes AS DECI
    FIELD ImpLin AS DECI        /* Antes de aplicar el descuento */
    FIELD ImpTot AS DECI        /* Después de aplicar el descuento */
    FIELD FchPed AS DATE
    FIELD PorDto AS DECI
    FIELD ImpTeo AS DECI
    .

DEF NEW SHARED VAR lh_handle AS HANDLE.

DEF TEMP-TABLE TT-CDOCU LIKE T-CDOCU.
...
IF NOT AVAILABLE CcbTabla THEN DO:
    MESSAGE 'NO está configurados los conceptos de PRE-N/C y N/D' SKIP
        'Comunicarse con el Créditos y Cobranzas'
        VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
IF TRUE <> (CcbTabla.Libre_c01 > '') THEN DO:
    MESSAGE 'NO está configurados el conceptos de PRE-N/C' SKIP
        'Comunicarse con el Créditos y Cobranzas'
        VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
...
IF NOT CAN-FIND(FIRST FacCorre WHERE FacCorre.CodCia = s-codcia AND
    ...
    THEN DO:
    MESSAGE 'NO hay ningún correlativo definido para la PRE-N/C' SKIP
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "GENERACION DE PRE-NC Y/O ND POR DESCUENTOS POR VOLUMEN ACUMULADOS"
         HEIGHT             = 25.58
         WIDTH              = 135.57
         MAX-HEIGHT         = 25.58
         MAX-WIDTH          = 135.57
...
ON END-ERROR OF W-Win /* GENERACION DE PRE-NC Y/O ND POR DESCUENTOS POR VOLUMEN ACUMULADOS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* GENERACION DE PRE-NC Y/O ND POR DESCUENTOS POR VOLUMEN ACUMULADOS */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_tab95 ( 16.42 , 133.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_b-nc-dcto-por-vol-doc ( 11.23 , 4.00 ) NO-ERROR.
...
DEFINE VAR x-ImpLin AS DECI NO-UNDO.
...
/* *************************************************************************************** */
/* 1ro. Barremos las COT CLIENTE por CLIENTE */
/* *************************************************************************************** */
DO k = 1 TO NUM-ENTRIES(x-Clientes):
    x-CodCli = ENTRY(k, x-Clientes).
    /* Barremos las COT válidas */
    FOR EACH COTIZACION NO-LOCK WHERE COTIZACION.codcia = s-codcia
        ...
        FOR EACH PriCDtoVolAcu NO-LOCK WHERE PriCDtoVolAcu.CodCia = s-codcia 
            ...
            FIND FIRST PriDDtoVolAcu OF PriCDtoVolAcu WHERE PriDDtoVolAcu.Tabla = 'DIVISION' AND
                PriDDtoVolAcu.Llave_c1 = COTIZACION.Lista_de_Precios NO-LOCK NO-ERROR.
            IF AVAILABLE PriDDtoVolAcu THEN DO:
                /* Si encontramos al menos una COT lo guardamos */
                CREATE T-CCOTI.
                BUFFER-COPY COTIZACION TO T-CCOTI.
                FOR EACH Facdpedi OF COTIZACION NO-LOCK:
                    CREATE T-DCOTI.
                    BUFFER-COPY Facdpedi TO T-DCOTI ASSIGN T-DCOTI.Flg_Factor = "".     /* Campo de control */
                END.
                LEAVE RLOOP.
            END.
        END.
...
END.
/* *************************************************************************************** */
/* 2do. Por cada COT nos quedamos solo con los artículos válidos */
/* *************************************************************************************** */
FOR EACH T-CCOTI NO-LOCK:
    FOR EACH PriCDtoVolAcu NO-LOCK WHERE PriCDtoVolAcu.CodCia = s-codcia 
        AND PriCDtoVolAcu.Inactivo = NO
        AND PriCDtoVolAcu.Desde <= T-CCOTI.FchPed 
        AND PriCDtoVolAcu.Hasta >= T-CCOTI.FchPed
        AND CAN-FIND(FIRST PriDDtoVolAcu OF PriCDtoVolAcu WHERE 
                     PriDDtoVolAcu.Tabla = 'DIVISION' AND
                     PriDDtoVolAcu.Llave_c1 = T-CCOTI.Lista_de_Precios NO-LOCK):
        FOR EACH PriDDtoVolAcu OF PriCDtoVolAcu NO-LOCK WHERE PriDDtoVolAcu.Tabla = "SKU":
            FIND FIRST T-DCOTI OF T-CCOTI WHERE T-DCOTI.codmat = PriDDtoVolAcu.Llave_c1 EXCLUSIVE-LOCK NO-ERROR.
            IF AVAILABLE T-DCOTI THEN T-DCOTI.Flg_Factor = "*".     /* Marcado */
        END.
    END.
END.
/* Eliminamos lo sobrante */
FOR EACH T-DCOTI EXCLUSIVE-LOCK WHERE T-DCOTI.Flg_Factor <> "*":
    DELETE T-DCOTI.
END.
FOR EACH T-CCOTI EXCLUSIVE-LOCK:
    IF NOT CAN-FIND(FIRST T-DCOTI OF T-CCOTI NO-LOCK) THEN DELETE T-CCOTI.
END.
/* *************************************************************************************** */
/* 3ro. Buscamos los Comprobantes */
/* *************************************************************************************** */
EMPTY TEMP-TABLE T-ACUMULADO.
FOR EACH T-CCOTI NO-LOCK:
    FOR EACH PEDIDO NO-LOCK WHERE PEDIDO.codcia = s-codcia
        AND PEDIDO.codref = T-CCOTI.coddoc
        AND PEDIDO.nroref = T-CCOTI.nroped
        ...
        FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia
            ...
            AND Ccbcdocu.flgcie <> "C":     /* OJO: Control de los que ya pasaron por este proceso */
            CREATE T-CDOCU.
            BUFFER-COPY Ccbcdocu TO T-CDOCU.        /* ASSIGN T-CDOCU.ImpTot = CcbCDocu.TotalVenta.*/
             FOR EACH Ccbddocu OF Ccbcdocu NO-LOCK,
                FIRST T-DCOTI OF T-CCOTI NO-LOCK WHERE T-DCOTI.codmat = Ccbddocu.codmat:
                CREATE T-DDOCU.
                BUFFER-COPY Ccbddocu TO T-DDOCU.
                /* Acumulamos cantidades e importes */
                FIND T-ACUMULADO WHERE T-ACUMULADO.codmat = Ccbddocu.codmat EXCLUSIVE-LOCK NO-ERROR.
                IF NOT AVAILABLE T-ACUMULADO THEN CREATE T-ACUMULADO.
                ASSIGN
                    T-ACUMULADO.codcia = s-codcia
                    T-ACUMULADO.codmat = Ccbddocu.codmat
                    T-ACUMULADO.candes = T-ACUMULADO.candes + (Ccbddocu.candes * Ccbddocu.factor)
                    /*T-ACUMULADO.implin = T-ACUMULADO.implin + ROUND(Ccbddocu.candes * Ccbddocu.preuni,2)*/
                    T-ACUMULADO.fchped = T-CCOTI.fchped
                    .
                /* Los importes en base a SUNAT */
                x-ImpLin = 0.
                IF Ccbddocu.cTipoAfectacion = "GRAVADA" 
                    THEN x-ImpLin =  ROUND(Ccbddocu.CanDes * Ccbddocu.cPreUniSinImpuesto * (1 + Ccbddocu.TasaIGV),2).
                    ELSE x-ImpLin =  ROUND(Ccbddocu.CanDes * Ccbddocu.cPreUniSinImpuesto,2).
                ASSIGN T-ACUMULADO.implin = T-ACUMULADO.implin + x-ImpLin.
                /* SOLO los que son Descuento por Vol (VOL) */
                CASE TRUE:
                    WHEN Ccbddocu.Por_Dsctos[1] > 0 OR Ccbddocu.Por_Dsctos[2] > 0 OR Ccbddocu.Por_Dsctos[3] > 0 OR 
                        Ccbddocu.Dcto_Otros_Factor > 0 THEN DO:
                        IF Ccbddocu.Dcto_Otros_Factor = 0 
                            THEN T-ACUMULADO.imptot = T-ACUMULADO.imptot + Ccbddocu.cImporteTotalConImpuesto.
                        ELSE T-ACUMULADO.imptot = T-ACUMULADO.imptot + ROUND(x-ImpLin * 
                                                                             (1 - Ccbddocu.Por_Dsctos[1] / 100) * 
                                                                             (1 - Ccbddocu.Por_Dsctos[2] / 100) * 
                                                                             (1 - Ccbddocu.Por_Dsctos[3] / 100), 2).
                    END.
                    OTHERWISE DO:
                        T-ACUMULADO.imptot = T-ACUMULADO.imptot + x-ImpLin.
                    END.
                END CASE.
            END.
        END.
...
FOR EACH T-CDOCU EXCLUSIVE-LOCK:
    IF NOT CAN-FIND(FIRST T-DDOCU OF T-CDOCU NO-LOCK) THEN DELETE T-CDOCU.
END.
/* *************************************************************************************** */
/* 4to. Reducimos las N/C por devolución */
/* *************************************************************************************** */
EMPTY TEMP-TABLE TT-CDOCU.

FOR EACH T-CDOCU NO-LOCK:
    FOR EACH CREDITO NO-LOCK WHERE CREDITO.codcia = s-codcia
        AND CREDITO.codref = T-CDOCU.coddoc
        AND CREDITO.nroref = T-CDOCU.nrodoc
        AND CREDITO.flgest <> "A"
        AND CREDITO.coddoc = "N/C"
        AND CREDITO.CndCre = "D"
        AND CREDITO.flgcie <> "C":
        FOR EACH Ccbddocu OF CREDITO, 
            FIRST T-ACUMULADO EXCLUSIVE-LOCK WHERE T-ACUMULADO.codcia = s-codcia AND
            T-ACUMULADO.codmat = Ccbddocu.codmat:
            FIND FIRST TT-CDOCU OF CREDITO NO-LOCK NO-ERROR.
            IF NOT AVAILABLE TT-CDOCU THEN DO:
                CREATE TT-CDOCU.
                BUFFER-COPY CREDITO TO TT-CDOCU.
            END.
            ASSIGN
                T-DDOCU.candes = T-DDOCU.candes - (Ccbddocu.candes * Ccbddocu.factor)
                T-ACUMULADO.implin = T-ACUMULADO.implin - Ccbddocu.cImporteTotalConImpuesto
                T-ACUMULADO.imptot = T-ACUMULADO.imptot - Ccbddocu.cImporteTotalConImpuesto.
                .
        END.
    END.
END.
...
/* *************************************************************************************** */
/* 5to. Calculamos el descuento teórico */
/* *************************************************************************************** */
...
FOR EACH PriCDtoVolAcu NO-LOCK WHERE PriCDtoVolAcu.CodCia = s-codcia AND PriCDtoVolAcu.Inactivo = NO:
    /* Filtramos las que están fuera de rango */
    IF PriCDtoVolAcu.Hasta < FILL-IN_Desde THEN NEXT.
    IF PriCDtoVolAcu.Desde > FILL-IN_Hasta THEN NEXT.
    x-Acumulado = 0.
    FOR EACH PriDDtoVolAcu OF PriCDtoVolAcu NO-LOCK WHERE PriDDtoVolAcu.Tabla = "SKU":
        FOR EACH T-ACUMULADO NO-LOCK WHERE T-ACUMULADO.codmat = PriDDtoVolAcu.Llave_c1:
            x-Acumulado = x-Acumulado + T-ACUMULADO.candes.
        END.
    END.
    /* Buscar la escala */
    x-PorDto = 0.
    DO k = 1 TO 10:
        IF x-Acumulado >= PriCDtoVolAcu.DtoVolR[k] AND PriCDtoVolAcu.DtoVolR[k] > 0
            THEN x-PorDto = PriCDtoVolAcu.DtoVolP[k].
    END.
    /* Aplicamos la escala */
    IF x-PorDto > 0 THEN DO:
        FOR EACH PriDDtoVolAcu OF PriCDtoVolAcu NO-LOCK WHERE PriDDtoVolAcu.Tabla = "SKU",
            FIRST T-ACUMULADO EXCLUSIVE-LOCK WHERE T-ACUMULADO.codmat = PriDDtoVolAcu.Llave_c1:
            ASSIGN
                T-ACUMULADO.pordto = x-PorDto
                T-ACUMULADO.impteo = ROUND(T-ACUMULADO.implin * (1 - x-PorDto / 100), 2).
        END.
    END.
END.
/* *************************************************************************************** */
/* 5to. Limpiamos tabla */
/* *************************************************************************************** */
FOR EACH T-ACUMULADO EXCLUSIVE-LOCK WHERE T-ACUMULADO.pordto <= 0:
    DELETE T-ACUMULADO.
END.

EMPTY TEMP-TABLE T-DDOCU.
FOR EACH T-ACUMULADO:
    CREATE T-DDOCU.
    ASSIGN
        T-DDOCU.CanDes = T-ACUMULADO.candes
        T-DDOCU.CodCia = s-codcia 
        T-DDOCU.codmat = T-ACUMULADO.codmat
        T-DDOCU.ImpLin = T-ACUMULADO.imptot     /* <<< OJO <<< */
        T-DDOCU.ImpPro = T-ACUMULADO.impteo
        T-DDOCU.ImpDto = T-ACUMULADO.imptot - T-ACUMULADO.impteo
        T-DDOCU.PorDto = T-ACUMULADO.PorDto
        T-DDOCU.ImpCto = T-ACUMULADO.implin
        .
END.
...
FOR EACH T-CDOCU BY T-CDOCU.ImpTot DESC:
    ...
    FOR EACH T-DDOCU WHERE T-DDOCU.ImpDto > 0:
        /* Buscamos si está registrado en el comprobante */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Devuelve-TDDOCU W-Win 
PROCEDURE Devuelve-TDDOCU :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF OUTPUT PARAMETER TABLE FOR T-DDOCU.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      FILL-IN_Desde = ADD-INTERVAL(TODAY, -1, 'month').
      FILL-IN_Hasta = TODAY.
