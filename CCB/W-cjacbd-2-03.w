...
DEFINE FRAME F-Main
     ...
     "Fecha de Proceso" VIEW-AS TEXT
          SIZE 13.43 BY .5 AT ROW 1.38 COL 35.43 WIDGET-ID 12
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN
        ...
        /* 30/07/2026: Junior Porras */
        /*T-PREV-2.glodoc  = 'Diferencia de Cambio'*/
        T-PREV-2.glodoc  = 'Ajuste por redondeo'
...
------------------------------------------------------------------------------*/
    ...
    IF ccbcdocu.coddoc <> "LET" THEN DO:
        ...
        THEN FIND FIRST Cb-cfgrv WHERE
        ...
        ELSE FIND FIRST Cb-cfgrv WHERE
            Cb-cfgrv.Codcia = S-CODCIA AND
            Cb-cfgrv.CodDiv = Ccbcdocu.CodDiv AND
            Cb-cfgrv.Coddoc = Ccbcdocu.Coddoc AND
            Cb-cfgrv.Fmapgo = Ccbcdocu.Fmapgo AND
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VAR x-imptot AS DECIMAL NO-UNDO.
    DEFINE VAR x-imptotant AS DECIMAL NO-UNDO.
...
/*                 RUN proc_Graba-Caja(CcbCCaja.Coddiv, 2, CcbCCaja.ImpUsa[1], '@CL', CcbCCaja.Codcli, SUBSTRING(CcbCCaja.Voucher[1],4,15), x-fchvto, x-tpocmb). */
            ...
            END.
        END.                
        IF CcbCCaja.ImpNac[1] <> 0 OR CcbCCaja.ImpNac[4] <> 0 THEN DO:
            x-tpocmb = 0.
            IF CcbCCaja.Tipo = "ANTREC" THEN DO:
                x-codcbd = '34'.
                x-codcta = cb-cfgcja.codcta1[7].
                ...
                x-imptotant = CcbCCaja.ImpNac[1].
                IF CcbCCaja.ImpNac[4] > 0 THEN x-imptotant = x-imptotant + CcbCCaja.ImpNac[4].
                IF CcbCCaja.ImpNac[3] > 0 THEN x-imptotant = x-imptotant + CcbCCaja.ImpNac[3].
                ...
                RUN proc_Graba-Caja(CcbCCaja.Coddiv, 1, x-imptotant, '@CL', CcbCCaja.Codcli, SUBSTRING(CcbCCaja.Voucher[1],4,15), x-fchvto, x-tpocmb).
/*                 x-codcta = cb-cfgcja.codcta1[1].                                                                                                              */
/*                 x-tpomov = FALSE.                                                                                                                             */
/*                 RUN proc_Graba-Caja(CcbCCaja.Coddiv, 1, CcbCCaja.ImpNac[1], '@CL', CcbCCaja.Codcli, SUBSTRING(CcbCCaja.Voucher[1],4,15), x-fchvto, x-tpocmb). */
                
                /* 28Nov2025 - Susana Leon / Aracelly Gutierrez */
                IF CcbCCaja.ImpNac[4] > 0 OR CcbCCaja.ImpNac[3] > 0 THEN DO:    /* Pago con Tarjeta - Billetera electronica */
                    x-codcta = cb-cfgcja.codcta1[4].
                    x-codaux = CcbCCaja.Codcli.
                    x-codcbd = '34'.
                    x-tpomov = FALSE.
                    x-tpocmb = 0.
                    x-imptotant = 0.
                    x-glodoc = 'Canc-Tarjeta-Credito' + CcbCcaja.CodDoc + '-' + CcbCCaja.Nrodoc + ' Cli.' + CcbCCaja.Codcli.
                    IF CcbCCaja.ImpNac[4] > 0 THEN x-imptotant = CcbCCaja.ImpNac[4].
                    IF CcbCCaja.ImpNac[3] > 0 THEN x-imptotant = x-imptotant + CcbCCaja.ImpNac[3].
                    ...
                    END.
                    RUN proc_Graba-Caja(CcbCCaja.Coddiv, 1, x-imptotant, '@CL', x-codaux, CcbCCaja.Voucher[4], x-fchvto, x-tpocmb).       
                END.
