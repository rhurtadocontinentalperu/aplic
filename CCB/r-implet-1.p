...
BUTTON-print BUTTON-exit RECT-3 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-let FILL-IN-let-1 RADIO-tipo_letra 
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-let FILL-IN-let-1 RADIO-tipo_letra 
...
------------------------------------------------------------------------------*/
    ...
    IF RADIO-tipo_letra = "RNV" 
    THEN RUN proc_carga-temporal-2.
    ELSE RUN proc_carga-temporal.

    RUN ccb/r-letform3 (s-task-no, s-user-id).
...
FOR EACH CCBDMVTO NO-LOCK WHERE
    ...
    IF w-report.Campo-C[13] <> '' THEN DO:
        w-report.Campo-C[9] = TRIM (w-report.Campo-C[9]) + ' / ' + w-report.Campo-C[13].
        w-report.Campo-C[11] = TRIM (w-report.Campo-C[11]) + ' / ' + w-report.Campo-C[14].
    END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE proc_carga-temporal-2 W-Win 
