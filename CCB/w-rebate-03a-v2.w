...
BUTTON-NC x-Concepto x-FchDoc RADIO-SET-tipo BROWSE-5 BROWSE-7 
&Scoped-Define DISPLAYED-OBJECTS c-NroSer FILL-IN-1 x-Mensaje x-CodMon ~
x-Concepto x-NomCon x-FchDoc RADIO-SET-tipo FILL-IN-ImpTot 
...
DEFINE VARIABLE FILL-IN-ImpTot AS DECIMAL FORMAT "ZZ,ZZZ,ZZ9.99":U INITIAL 0 
     LABEL "IMPORTE TOTAL >>>" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .81
...
DEFINE VARIABLE RADIO-SET-tipo AS CHARACTER 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "REBATE", "REBATE",
"OTROS", "OTROS"
     SIZE 23 BY .73 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-5 W-Win _STRUCTURED
  QUERY BROWSE-5 NO-LOCK DISPLAY
      ...
      Docu.NroDoc FORMAT "X(12)":U WIDTH 10.14
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-7 W-Win _STRUCTURED
  QUERY BROWSE-7 NO-LOCK DISPLAY
      ...
      Docu-2.NroDoc FORMAT "X(12)":U WIDTH 9.14
      Docu-2.ImpTot COLUMN-LABEL "Importe" FORMAT "->>,>>>,>>9.99":U
            WIDTH 15.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     ...
     RADIO-SET-tipo AT ROW 3.81 COL 38 NO-LABEL WIDGET-ID 102
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-5 RADIO-SET-tipo F-Main */
...
"Docu.NroDoc" ? ? "character" ? ? ? ? ? ? no ? no no "10.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Docu.ImpTot" "Importe del Rebade" ? "decimal" ? ? ? ? ? ? no ? no no "12.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-5 */
&ANALYZE-RESUME
...
"Docu-2.NroDoc" ? ? "character" ? ? ? ? ? ? no ? no no "9.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.Docu-2.ImpTot
"Docu-2.ImpTot" "Importe" ? "decimal" ? ? ? ? ? ? no ? no no "15.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-7 */
&ANALYZE-RESUME
...
DO:
  ...
  ASSIGN
      c-NroSer x-CodMon x-Concepto x-NomCon x-FchDoc FILL-IN-ImpTot radio-set-tipo.
  /* Consistencias */
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH DOCU, FIRST ccbcdocu NO-LOCK WHERE ccbcdocu.codcia = s-codcia
        ...
        ASSIGN
            ...
            t-cdocu.fchvto = ADD-INTERVAL (x-FchDoc, 1, 'years')
            ...
            t-cdocu.tpofac = "REBATE"
            t-cdocu.codcta = x-Concepto
            t-cdocu.tipo   = "CREDITO"
            t-cdocu.codcaja= "".
...
DO TRANSACTION ON ERROR UNDO, RETURN ERROR ON STOP UNDO, RETURN ERROR:
    ...
    FOR EACH DOCU, FIRST B-CDOCU NO-LOCK WHERE B-CDOCU.codcia = s-codcia
        AND B-CDOCU.coddoc = DOCU.coddoc
        AND B-CDOCU.nrodoc = DOCU.nrodoc:
        CREATE Ccbcdocu.
        ASSIGN
            ...
            Ccbcdocu.nrodoc = STRING(Faccorre.nroser, ENTRY(1, x-Formato, '-')) + 
                                STRING(Faccorre.correlativo, ENTRY(2, x-Formato, '-'))
            Ccbcdocu.fchdoc = x-FchDoc       /* OJO */
            Ccbcdocu.fchvto = ADD-INTERVAL (x-FchDoc, 1, 'years')
            Ccbcdocu.codcli = B-CDOCU.codcli
            Ccbcdocu.ruccli = B-CDOCU.ruccli
            Ccbcdocu.nomcli = B-CDOCU.nomcli
            Ccbcdocu.dircli = B-CDOCU.dircli
            Ccbcdocu.porigv = ( IF B-CDOCU.porigv > 0 THEN B-CDOCU.porigv ELSE FacCfgGn.PorIgv )
            Ccbcdocu.codmon = x-CodMon
            ...
            Ccbcdocu.tpofac = radio-set-tipo    /*"REBATE"*/
            Ccbcdocu.codcta = x-Concepto
            Ccbcdocu.tipo   = "CREDITO"
            Ccbcdocu.codcaja= ""
            NO-ERROR.
        IF ERROR-STATUS:ERROR THEN DO:
            pMensaje = "ERROR al grabar el comprobante " + STRING(Faccorre.nroser, ENTRY(1, x-Formato, '-')) + 
                STRING(Faccorre.correlativo, ENTRY(2, x-Formato, '-')).
            UNDO, LEAVE RLOOP.
        END.
        ...
        ASSIGN
            Ccbddocu.codmat = CcbTabla.Codigo
            Ccbddocu.factor = 1
            Ccbddocu.candes = 1
            Ccbddocu.preuni = DOCU.ImpTot.
        ASSIGN
            Ccbddocu.implin = Ccbddocu.CanDes * Ccbddocu.PreUni.
        IF CcbTabla.Afecto THEN
            ASSIGN
            Ccbddocu.AftIgv = Yes
            Ccbddocu.ImpIgv = (Ccbddocu.CanDes * Ccbddocu.PreUni) * ((Ccbcdocu.PorIgv / 100) / (1 + (Ccbcdocu.PorIgv / 100))).
        ELSE
            ASSIGN
            Ccbddocu.AftIgv = No
            Ccbddocu.ImpIgv = 0.
...
/*                                          OUTPUT pMensaje ).                 */
/*         IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, LEAVE RLOOP.               */
...
REPEAT:
    ...
    DISPLAY "** IMPORTANDO ** " + DOCU.CodDoc + ' ' + STRING(DOCU.NroDoc, 'x(12)') @ x-mensaje WITH FRAME {&FRAME-NAME}.
    /* IMPORTE */
    cRange = "C" + TRIM(STRING(iCountLine)).
    cValue = chWorkSheet:Range(cRange):VALUE.
    ASSIGN
        DOCU.ImpTot = ROUND(DECIMAL (cValue), 2)
        NO-ERROR.
    IF ERROR-STATUS:ERROR THEN DO:
...
FOR EACH DOCU:
    ...
    IF gn-convt.libre_c01 <> 'SI' THEN DO:
        MESSAGE "La condicion de venta del documento (" + DOCU.coddoc + " " + DOCU.nrodoc ")"  SKIP
    ...
    /* SOLO OPENORANGE */
    RUN gn/clienteopenorange (cl-codcia, Ccbcdocu.CodCli, Ccbcdocu.CodDoc, OUTPUT pClienteOpenOrange).
    IF pClienteOpenOrange = YES THEN DO:
        MESSAGE "Cliente" Ccbcdocu.codcli "NO se puede antender por Continental" SKIP
