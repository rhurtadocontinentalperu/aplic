...
DEF VAR x-Mon AS CHAR FORMAT 'x(3)' LABEL 'Mon.'.
...
CcbCDocu.FchDoc fMoneda() @ x-Mon CcbCDocu.ImpTot CcbCDocu.imptot2 
...
&Scoped-define QUERY-STRING-br_table FOR EACH CcbCDocu WHERE CcbCDocu.CodCli = gn-clie.CodCli ~
 ...
 AND ( (x-Estado = 'Pendientes' AND CcbCDocu.imptot2 > 0) ~
OR (x-Estado = 'Aplicadas' AND Ccbcdocu.imptot2 <= 0) ) NO-LOCK ~
...
&Scoped-Define ENABLED-OBJECTS x-Estado br_table 
...
DEFINE VARIABLE x-Estado AS CHARACTER FORMAT "X(256)":U INITIAL "Pendientes" 
     LABEL "Estado" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "Pendientes","Aplicadas" 
     DROP-DOWN-LIST
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.FchDoc COLUMN-LABEL "<<Fecha>>" FORMAT "99/99/99":U
      fMoneda() @ x-Mon
      CcbCDocu.ImpTot FORMAT "->>,>>>,>>9.99":U
      CcbCDocu.imptot2 COLUMN-LABEL "Por Aplicar" FORMAT "->>>,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     x-Estado AT ROW 1 COL 9 COLON-ALIGNED WIDGET-ID 2
...
/* BROWSE-TAB br_table x-Estado F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "INTEGRAL.CcbCDocu.FchDoc|no"
 ...
 AND CcbCDocu.FlgEst = ""C""
 AND ( (x-Estado = 'Pendientes' AND CcbCDocu.imptot2 > 0)
OR (x-Estado = 'Aplicadas' AND Ccbcdocu.imptot2 <= 0) )"
...
"CcbCDocu.FchDoc" "<<Fecha>>" "99/99/99" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > "_<CALC>"
"fMoneda() @ x-Mon" ? ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   = INTEGRAL.CcbCDocu.ImpTot
     _FldNameList[6]   > INTEGRAL.CcbCDocu.imptot2
"CcbCDocu.imptot2" "Por Aplicar" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL x-Estado B-table-Win
ON VALUE-CHANGED OF x-Estado IN FRAME F-Main /* Estado */
DO:
  ASSIGN {&self-name}.
   RUN adm-open-query.
END.
