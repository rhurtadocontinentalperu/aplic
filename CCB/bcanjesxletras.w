...
&SCOPED-DEFINE Condicion ccbcmvto.codcia = gn-div.codcia ~
...
(FILL-IN-Filtro = '' OR  ~
INDEX(gn-clie.nomcli, FILL-IN-Filtro) <> 0 ) ) NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&Scoped-Define ENABLED-OBJECTS CMB-filtro FILL-IN-filtro FILL-IN-FchDoc-1 ~
FILL-IN-FchDoc-2 br_table 
&Scoped-Define DISPLAYED-OBJECTS CMB-filtro FILL-IN-filtro FILL-IN-FchDoc-1 ~
FILL-IN-FchDoc-2 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbCMvto.NroDoc FORMAT "X(9)":U WIDTH 9.43
      CcbCMvto.FchDoc FORMAT "99/99/9999":U
      gn-clie.NomCli FORMAT "x(50)":U WIDTH 43.86
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 66 BY 4.81
...
DEFINE FRAME F-Main
     CMB-filtro AT ROW 1.19 COL 7 NO-LABEL WIDGET-ID 8
     FILL-IN-filtro AT ROW 1.19 COL 29 NO-LABEL WIDGET-ID 10
     FILL-IN-FchDoc-1 AT ROW 2.15 COL 14 COLON-ALIGNED WIDGET-ID 4
     FILL-IN-FchDoc-2 AT ROW 2.15 COL 34 COLON-ALIGNED WIDGET-ID 6
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "{&Condicion}"
     _JoinCode[2]      = "gn-clie.CodCli = CcbCMvto.CodCli"
     _Where[2]         = "gn-clie.CodCia = cl-codcia
 AND ( CMB-Filtro = 'Todos' OR
(FILL-IN-Filtro = '' OR 
INDEX(gn-clie.nomcli, FILL-IN-Filtro) <> 0 ) )"
     _FldNameList[1]   > INTEGRAL.CcbCMvto.NroDoc
"CcbCMvto.NroDoc" ? ? "character" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.CcbCMvto.FchDoc
     _FldNameList[3]   > INTEGRAL.gn-clie.NomCli
"gn-clie.NomCli" ? ? "character" ? ? ? ? ? ? no ? no no "43.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FILL-IN-FchDoc-1 IN FRAME F-Main /* Canjes desde el */
DO:
  ASSIGN {&self-name}.
  RUN dispatch IN THIS-PROCEDURE ('open-query':U).
END.
...
ON LEAVE OF FILL-IN-FchDoc-2 IN FRAME F-Main /* Hasta el */
DO:
    ASSIGN {&self-name}.
    RUN dispatch IN THIS-PROCEDURE ('open-query':U).
END.
