...
DEFINE VARIABLE F-ImpTot AS DECIMAL FORMAT "(ZZZ,ZZZ,ZZ9.99)" INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 15 BY .81
...
DEFINE FRAME F-Main
     ...
     F-ImpTot AT ROW 20.15 COL 52 COLON-ALIGNED NO-LABEL WIDGET-ID 44
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "GENERACION DE NOTAS DE CREDITO AUTOMATICAS POR REBATE"
         HEIGHT             = 21.65
         WIDTH              = 139.57
         MAX-HEIGHT         = 21.65
         MAX-WIDTH          = 141.72
...
ON END-ERROR OF W-Win /* GENERACION DE NOTAS DE CREDITO AUTOMATICAS POR REBATE */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* GENERACION DE NOTAS DE CREDITO AUTOMATICAS POR REBATE */
DO:
...
/* SOLO VAMOS A TOMAR FACTURA QUE TENGAN POR LO MENOS UN PRODUCTO DE LA FAMILIA 010 O 012 */
...
DEF VAR x-ProdPropio AS DEC NO-UNDO.
DEF VAR x-FactorPropio AS DEC NO-UNDO.
FOR EACH T-DMvto-1:
    /* ACUMULAMOS IMPORTES POR PRODUCTOS PROPIOS */
    x-ProdPropio = 0.
    FIND Ccbcdocu WHERE CCbcdocu.codcia = s-codcia
        AND Ccbcdocu.coddoc = T-DMvto-1.codref
        AND Ccbcdocu.nrodoc = T-DMvto-1.nroref
        NO-LOCK.
    FOR EACH Ccbddocu OF Ccbcdocu NO-LOCK,
        FIRST Almmmatg OF Ccbddocu NO-LOCK WHERE LOOKUP(Almmmatg.codfam, '010,012') > 0:
        x-ProdPropio = x-ProdPropio + Ccbddocu.implin.
    END.
/*     ASSIGN                               */
/*         T-DMvto-1.ImpDoc = x-ProdPropio. */
    FIND FIRST Ccbddocu OF Ccbcdocu WHERE Ccbddocu.implin < 0 NO-LOCK NO-ERROR.
    IF AVAILABLE Ccbddocu 
    THEN x-FactorPropio = x-ProdPropio / ( Ccbcdocu.imptot + ABS(Ccbddocu.implin) ).
    ELSE x-FactorPropio = x-ProdPropio / Ccbcdocu.imptot.
    /* ACUMULAMOS IMPORTES POR NOTA DE CREDITO POR OTROS CONCEPTOS */
    FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia
        ...
        ASSIGN
            T-DMvto-1.DepNac[1] = T-DMvto-1.DepNac[1] + Ccbcdocu.ImpTot * x-FactorPropio.
    END.
    /* ACUMULAMOS IMPORTES POR NOTA DE CREDITO POR DEVOLUCION */
    FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia
        AND Ccbcdocu.coddoc = 'N/C'
        AND Ccbcdocu.codref = T-DMvto-1.codref
        AND Ccbcdocu.nroref = T-DMvto-1.nroref
        ...
        FOR EACH Ccbddocu OF Ccbcdocu NO-LOCK,
            FIRST Almmmatg OF Ccbddocu NO-LOCK WHERE LOOKUP(Almmmatg.codfam, '010,012') > 0:
            T-DMvto-1.DepNac[1] = T-DMvto-1.DepNac[1] + Ccbddocu.ImpLin.
        END.
    END.
    IF T-DMvto-1.DepNac[1] < x-ProdPropio 
    THEN T-DMvto-1.ImpTot = ( x-ProdPropio - T-DMvto-1.DepNac[1] ) *  x-porreb / 100.
END.
{&OPEN-QUERY-BROWSE-2}
{&OPEN-QUERY-BROWSE-3}
RUN Imp-Total.
