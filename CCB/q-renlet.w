...
&SCOPED-DEFINE CONDICION ( CcbCMvto.CodCia = s-codcia AND ~
                          CcbCMvto.CodDoc = s-coddoc AND ~
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH CcbCMvto WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH CcbCMvto WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
...
&Scoped-Define ENABLED-OBJECTS F-Numero F-Letra 
&Scoped-Define DISPLAYED-OBJECTS F-Numero F-Letra 
...
DEFINE VARIABLE F-Letra AS CHARACTER FORMAT "X(12)":U 
     LABEL "Letra" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME FRAME-A
     F-Numero AT ROW 1.19 COL 3.28
     F-Letra AT ROW 1.96 COL 2.71
...
/* SETTINGS FOR FILL-IN F-Letra IN FRAME FRAME-A
   ALIGN-L                                                              */
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "integral.CcbCMvto"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "{&CONDICION}"
     _Design-Parent    is WINDOW q-tables @ ( 1 , 18.72 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
&Scoped-define SELF-NAME F-Letra
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-Letra q-tables
ON LEAVE OF F-Letra IN FRAME FRAME-A /* Letra */
OR "RETURN":U OF F-Letra
DO:
  IF SELF:SCREEN-VALUE = "" THEN RETURN.
  ASSIGN F-Letra.
  RUN Ubica-Registro-2.
  SELF:SCREEN-VALUE = "". 
END.
...
ON LEAVE OF F-Numero IN FRAME FRAME-A /* Nro. */
...
FIND CcbCMvto WHERE 
     ...
     CcbCMvto.CodDoc = s-coddoc AND
     CcbCMvto.NroDoc = STRING(I-NroSer,"999") + STRING(F-Numero,"999999") 
     NO-LOCK NO-ERROR.
IF AVAILABLE CcbCMvto THEN output-var-1 = ROWID(CcbCMvto).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Ubica-Registro-2 q-tables 
PROCEDURE Ubica-Registro-2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FIND CcbCMvto WHERE 
     CcbCMvto.CodCia = S-CODCIA AND
     CcbCMvto.CodDoc = s-coddoc AND
     CcbCMvto.NroDoc BEGINS STRING(I-NroSer,"999") AND
     CcbCMvto.NroRef = f-Letra
     NO-LOCK NO-ERROR.
IF AVAILABLE CcbCMvto THEN output-var-1 = ROWID(CcbCMvto).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
