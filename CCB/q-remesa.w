...
DEF SHARED VAR s-FlgSit AS CHAR.    /* INIT "YES".     Bloqueado x defecto */

&Scoped-define Condicion ( Ccbccaja.codcia = s-codcia AND ~
                           ccbccaja.coddiv = s-coddiv AND ~
                           ccbccaja.coddoc = s-coddoc AND ~
                           (s-FlgSit = "NO" OR ccbccaja.usuario= s-user-id) AND ~
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH CcbCCaja WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    BY CcbCCaja.NroDoc DESCENDING INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH CcbCCaja WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    BY CcbCCaja.NroDoc DESCENDING INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main CcbCCaja
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main CcbCCaja
...
DEFINE FRAME FRAME-A
     FILL-IN-1 AT ROW 1.19 COL 1.57 NO-LABEL
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "{&Condicion}"
     _Design-Parent    is WINDOW q-tables @ ( 1.12 , 13.72 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
DO:
    ...
    FIND ccbccaja WHERE {&Condicion} AND nrodoc = SELF:SCREEN-VALUE NO-LOCK NO-ERROR.
    IF AVAILABLE ccbccaja THEN output-var-1 = ROWID(ccbccaja).
    ELSE output-var-1 = ?.
