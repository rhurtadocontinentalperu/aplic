...
DEFINE VARIABLE C-Tipo AS CHARACTER FORMAT "X(256)":U INITIAL "- Todos -" 
     VIEW-AS COMBO-BOX INNER-LINES 4
     LIST-ITEMS "- Todos -","Canje de Letras","Letras Adelantadas" 
     DROP-DOWN-LIST
...
DEFINE FRAME F-Main
     ...
     C-Tipo AT ROW 4.46 COL 1 COLON-ALIGNED NO-LABEL
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Generacion de Asiento Contable (Canje de Letras)"
         HEIGHT             = 15.58
         WIDTH              = 92.57
         MAX-HEIGHT         = 15.58
         MAX-WIDTH          = 92.57
...
/* SETTINGS FOR COMBO-BOX C-Tipo IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       C-Tipo:HIDDEN IN FRAME F-Main           = TRUE.
...
ON END-ERROR OF W-Win /* Generacion de Asiento Contable (Canje de Letras) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Generacion de Asiento Contable (Canje de Letras) */
DO:
...
DO:
    ...
    DISPLAY C-tipo FILL-IN-periodo WITH FRAME {&FRAME-NAME}.
...
DO:
    RUN Abrir-Browse IN h_b-ccbcbd (INPUT SELF:SCREEN-VALUE).
END.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH CCBDMVTO NO-LOCK WHERE
        ...
        ccbdmvto.coddoc = "CJE" AND
        ccbdmvto.nrodoc >= "" AND
        ccbdmvto.tporef = "L" AND
        ccbdmvto.fchemi = FILL-IN-fchast,
        FIRST ccbcmvto WHERE
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-ccbcbd ,
             C-Tipo:HANDLE IN FRAME F-Main , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE cMomCli LIKE ccbcdocu.nomcli NO-UNDO.
    ...
    cTipo = "Letras Adelantadas".
    /* Letras Adelantadas */
    FOR EACH CCBDMVTO NO-LOCK WHERE
        ccbdmvto.codcia = s-codcia AND
        ccbdmvto.coddiv = FILL-IN-division AND
        ccbdmvto.coddoc = "CLA" AND
        ccbdmvto.nrodoc >= "" AND
        ccbdmvto.tporef = "L" AND
        ccbdmvto.fchemi = FILL-IN-fchast,
        ...
        ccbcmvto.codcia = ccbdmvto.codcia AND
        ccbcmvto.coddoc = ccbdmvto.coddoc AND
        ccbcmvto.nrodoc = ccbdmvto.nrodoc AND
        ccbcmvto.FlgEst = "C" BREAK BY ccbdmvto.nrodoc:

        IF FIRST-OF(ccbdmvto.nrodoc) THEN DO:
            /* Letra Generada */
            FOR EACH CcbCDocu WHERE
                ...
                CcbCDocu.CodDoc = "A/R" NO-LOCK:

                IF CcbCDocu.CodMon = 1 THEN cCtaAux = cCodCta[3].
                ELSE cCtaAux = cCodCta[4].
...
/*MLR-01* ***
                    ccbdmvto.Imptot,
*MLR-01* ***/
/*MLR-01*/          ccbcdocu.imptot,
                    cTipo).
                cMomCli = ccbcdocu.nomcli.
                LEAVE.
    ...
    cTipo = "Canje de Letras".
    FOR EACH CCBDMVTO NO-LOCK WHERE
        ccbdmvto.codcia = s-codcia AND
        ccbdmvto.coddiv = FILL-IN-division AND
        LOOKUP(ccbdmvto.coddoc,"CJE,CLA") > 0 AND
        ccbdmvto.nrodoc >= "" AND
        ccbdmvto.tporef = "L" AND
        ccbdmvto.fchemi = FILL-IN-fchast,
        FIRST ccbcmvto NO-LOCK WHERE
...
/*MLR* ***/
        ...
        cMomCli = ?.
        /* Documento Origen */
        IF FIRST-OF(ccbdmvto.nrodoc) THEN FOR EACH b_dmvto WHERE
            b_dmvto.CodCia = ccbdmvto.codcia AND
            b_dmvto.CodDoc = ccbdmvto.coddoc AND
            b_dmvto.NroDoc = ccbdmvto.nrodoc AND
            b_dmvto.TpoRef = "O" NO-LOCK,
            EACH CcbCDocu WHERE
            CcbCDocu.CodCia = b_dmvto.CodCia AND
            CcbCDocu.CodDoc = b_dmvto.CodRef AND
            CcbCDocu.NroDoc = b_dmvto.NroRef NO-LOCK:

            FIND FacDocum WHERE
                FacDocum.CodCia = CcbCDocu.codcia AND
                FacDocum.CodDoc = CcbCDocu.CodDoc NO-LOCK NO-ERROR.
            IF NOT AVAILABLE FacDocum THEN NEXT.

            FIND Cb-cfgrv WHERE
                Cb-cfgrv.Codcia = CcbCDocu.Codcia AND
                Cb-cfgrv.CodDiv = CcbCDocu.Coddiv AND
                Cb-cfgrv.Coddoc = CcbCDocu.Coddoc AND
                Cb-cfgrv.Fmapgo = CcbCDocu.Fmapgo AND
                Cb-cfgrv.Codmon = CcbCDocu.Codmon
                NO-LOCK NO-ERROR.
            IF NOT AVAILABLE Cb-cfgrv OR Cb-cfgrv.Codcta = "" THEN DO:
/*                 IF NOT AVAILABLE Facdocum THEN DO:                     */
/*                     MESSAGE                                            */
/*                         'Cuenta No Configurada Para:' SKIP             */
/*                         'División:' CcbCDocu.Coddiv SKIP               */
/*                         'Documento:' CcbCDocu.Coddoc SKIP              */
/*                         'Número:' CcbCDocu.Nrodoc SKIP                 */
/*                         'Con Pago:' CcbCDocu.FmaPgo VIEW-AS ALERT-BOX. */
/*                     NEXT.                                              */
/*                 END.                                                   */
                RUN Graba-Documento(
                    FacDocum.Codcbd,
            ...
            cMomCli = CcbCDocu.CodDoc + " " + ccbcdocu.nomcli.
 
        END.

        IF ccbdmvto.coddoc = "CLA" AND cMomCli = ? THEN NEXT.
    ...
    IF CAN-FIND(FIRST t-prev) THEN RUN Carga-Ajuste(cTipo).
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-division FILL-IN-fchast FILL-IN-periodo C-Tipo 
...
------------------------------------------------------------------------------*/
    ...
    FIND t-prev WHERE
        ...
        t-prev.nrodoc = ccbdmvto.Nrodoc AND
        t-prev.codcta = cCodCta[param_CodMon] AND
    ...
    IF NOT AVAILABLE t-prev THEN DO:
        ...
        ASSIGN
            ...
            t-prev.nromes  = x-mes
            t-prev.codope  = cCodOpe
            t-prev.coddiv  = ccbdmvto.CodDiv 
            t-prev.codcta  = cCodCta[param_CodMon]
            t-prev.fchdoc  = param_Fchdoc
            t-prev.Fchvta  = ccbdmvto.FchVto
            t-prev.tpomov  = lTpoDoc
            t-prev.clfaux  = '@CL'
            t-prev.codaux  = ccbdmvto.CodCli
            t-prev.codmon  = param_CodMon
            t-prev.coddoc  = cCodCbd
            t-prev.nrodoc  = ccbdmvto.nroref
            t-prev.Tpocmb  = param_TpoCmb
            t-prev.glodoc  = param_NomCli.
    END.

    IF param_CodMon = 1 THEN
        ASSIGN t-prev.impmn1 = t-prev.impmn1 + ccbdmvto.Imptot.
    ELSE
        ASSIGN
            t-prev.impmn2 = t-prev.impmn2 + ccbdmvto.Imptot
            t-prev.impmn1 = t-prev.impmn1 + ROUND((ccbdmvto.Imptot * t-prev.Tpocmb),2).
