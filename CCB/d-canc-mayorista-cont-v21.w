...
RECT-10 RECT-6 RECT-33 BUTTON-1 FILL-IN_ImpNac1 FILL-IN_ImpUsa1 ~
FILL-IN_ImpNac4 FILL-IN_ImpUsa4 FILL-IN_Voucher4a FILL-IN_Voucher4c ~
FILL-IN_CodBco4 COMBO_TarjCred BUTTON-TpoCmb Btn_OK Btn_Cancel 
...
COMBO_TarjCred FILL-IN-4 FILL-IN_ImpNac6 FILL-IN_ImpUsa6 FILL-IN_Voucher4 ~
...
DEFINE VARIABLE FILL-IN_Voucher4a AS CHARACTER FORMAT "99" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .69 TOOLTIP "Ingrese los 2 primeros dígitos de la tarjeta"
     BGCOLOR 11 FGCOLOR 0 .
...
DEFINE VARIABLE FILL-IN_Voucher4c AS CHARACTER FORMAT "9999" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .69 TOOLTIP "Ingrese los 4 últimos dígitos de la tarjeta"
     BGCOLOR 11 FGCOLOR 0 .
...
DEFINE FRAME D-Dialog
     ...
     FILL-IN_ImpNac4 AT ROW 6.12 COL 14 COLON-ALIGNED
     FILL-IN_ImpUsa4 AT ROW 6.12 COL 26 COLON-ALIGNED NO-LABEL
     FILL-IN_Voucher4a AT ROW 6.12 COL 38 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     FILL-IN_Voucher4b AT ROW 6.12 COL 42 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     FILL-IN_Voucher4c AT ROW 6.12 COL 48 COLON-ALIGNED NO-LABEL WIDGET-ID 8
     FILL-IN_CodBco4 AT ROW 6.12 COL 55.29 COLON-ALIGNED NO-LABEL
     COMBO_TarjCred AT ROW 6.12 COL 61.72 COLON-ALIGNED NO-LABEL
     FILL-IN-4 AT ROW 6.12 COL 79 COLON-ALIGNED NO-LABEL
     FILL-IN_ImpNac6 AT ROW 6.92 COL 14 COLON-ALIGNED
     FILL-IN_ImpUsa6 AT ROW 6.92 COL 26 COLON-ALIGNED NO-LABEL
     FILL-IN_Voucher4 AT ROW 6.92 COL 72 COLON-ALIGNED
     BUTTON-TpoCmb AT ROW 8.19 COL 39
     RADIO-SET_Codmon AT ROW 8.23 COL 8.72 NO-LABEL
     FILL-IN_VueNac AT ROW 8.31 COL 14 COLON-ALIGNED NO-LABEL
     FILL-IN_VueUsa AT ROW 8.31 COL 26 COLON-ALIGNED NO-LABEL
     Btn_OK AT ROW 9.54 COL 2
     Btn_Cancel AT ROW 9.54 COL 14.14
     FILL-IN_T_Venta AT ROW 9.54 COL 46 COLON-ALIGNED
     FILL-IN-coddoc AT ROW 9.73 COL 57 COLON-ALIGNED NO-LABEL
     FILL-IN-nrodoc AT ROW 9.73 COL 65 COLON-ALIGNED NO-LABEL
     FILL-IN_T_Compra AT ROW 10.31 COL 46 COLON-ALIGNED
     "Banco" VIEW-AS TEXT
...
DO:
    ...
    IF PgoTarjCr = "T" THEN DO:
        IF (FILL-IN_ImpNac4 + FILL-IN_ImpUsa4) = 0 THEN DO:
            MESSAGE 'NO ha ingresado el importe por Tarjeta de Crédito' VIEW-AS ALERT-BOX WARNING.
    ...
    IF FILL-IN_ImpNac4 <> 0 OR FILL-IN_ImpUsa4 <> 0 THEN DO:
        IF FILL-IN_ImpNac4 <> 0 AND
            ...
            MESSAGE "El monto es mayor al saldo a pagar"
        ...
        IF FILL-IN_Voucher4:SCREEN-VALUE EQ "" THEN DO:
            ...
            RETURN NO-APPLY.
        END.
        IF COMBO_TarjCred:SCREEN-VALUE EQ ? OR COMBO_TarjCred:SCREEN-VALUE EQ "" THEN DO:
            MESSAGE "Seleccione una tarjeta de crédito"
                VIEW-AS ALERT-BOX WARNING.
    ...
    IF FILL-IN_VueUsa < 0 OR FILL-IN_VueNac < 0 THEN DO:
        MESSAGE "El pago no cubre el monto adeudado" VIEW-AS ALERT-BOX WARNING.
        RETURN NO-APPLY.
    END.
    ...
    IF RADIO-SET_Codmon = 1 THEN DO:
        IF FILL-IN_VueNac + (FILL-IN_VueUSA * FILL-IN_T_Compra) >
            ...
            MESSAGE "Solo se permiten vueltos para cancelaciones en efectivo"
                VIEW-AS ALERT-BOX WARNING.
            RETURN NO-APPLY.
        END.
    END.
...
DO:
    ...
    IF RADIO-SET_Codmon = 1 THEN DO:
        IF Moneda = 1 THEN 
            FILL-IN_VueNac = fSumSol + (ROUND(fSumDol * FILL-IN_T_Compra,2) - FILL-IN_SaldoNac).
        IF Moneda = 2 THEN 
            FILL-IN_VueNac = fSumSol + (ROUND(fSumDol * FILL-IN_T_Venta,2) - FILL-IN_SaldoNac).
    END.
    ELSE DO:
        IF Moneda = 1 THEN 
            FILL-IN_VueUSA = fSumDol + (ROUND(fSumSol / FILL-IN_T_Compra,2) - FILL-IN_SaldoUSA).
        IF Moneda = 2 THEN 
            FILL-IN_VueUSA = fSumDol + (ROUND(fSumSol / FILL-IN_T_Venta,2) - FILL-IN_SaldoUSA).
    END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'LABEL-FONT = 4,
                     ...
                     LABELS = Notas de Crédito':U ,
             OUTPUT h_tab95 ).
       RUN set-position IN h_tab95 ( 11.27 , 2.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_t-canc-mayorista-cont-v2 ( 12.58 , 4.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 13.85 , 72.00 ) NO-ERROR.
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    ASSIGN
        ...
        FILL-IN_ImpUsa6
        .
    fSumSol = (FILL-IN_ImpNac1 + FILL-IN_ImpNac4 + FILL-IN_ImpNac6).
    fSumDol = (FILL-IN_ImpUsa1 + FILL-IN_ImpUsa4 + FILL-IN_ImpUsa6 ).
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN_NomCli FILL-IN_MonNac FILL-IN_SaldoNac FILL-IN_MonUSA 
          ...
          FILL-IN_Voucher4c FILL-IN_CodBco4 COMBO_TarjCred FILL-IN-4 
          FILL-IN_ImpNac6 FILL-IN_ImpUsa6 FILL-IN_Voucher4 RADIO-SET_Codmon 
          FILL-IN_VueNac FILL-IN_VueUsa FILL-IN-coddoc FILL-IN-nrodoc 
          FILL-IN_T_Compra 
      WITH FRAME D-Dialog.
  ENABLE RECT-8 RECT-32 RECT-9 RECT-7 RECT-5 RECT-4 RECT-10 RECT-6 RECT-33 
         BUTTON-1 FILL-IN_ImpNac1 FILL-IN_ImpUsa1 FILL-IN_ImpNac4 
         FILL-IN_ImpUsa4 FILL-IN_Voucher4a FILL-IN_Voucher4c FILL-IN_CodBco4 
         COMBO_TarjCred BUTTON-TpoCmb Btn_OK Btn_Cancel 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      ASSIGN
          ...
          FILL-IN-nrodoc = STRING(FacCorre.NroSer,ENTRY(1,x-Formato,'-')) +
                            STRING(FacCorre.Correlativo,ENTRY(2,x-Formato,'-')) 
          FILL-IN_NomCli = wnomcli
          .
      /* Tipo de Cambio Caja */
      ...
      FOR EACH FacTabla WHERE FacTabla.CodCia = s-CodCia AND
          ...
          COMBO_TarjCred:ADD-LAST(FacTabla.Codigo + " " + FacTabla.Nombre).
      END.
      IF Moneda = 1 THEN FILL-IN_ImpNac1 = FILL-IN_SaldoNac.
      ELSE FILL-IN_ImpUSA1 = FILL-IN_SaldoUSA.
      CASE PgoTarjCr:
          WHEN "T" THEN DO:
...
DO WITH FRAME {&FRAME-NAME}:
    ASSIGN
        FILL-IN_ImpNac1 FILL-IN_ImpNac4 FILL-IN_ImpNac6 
        FILL-IN_ImpUsa1 FILL-IN_ImpUsa4 FILL-IN_ImpUsa6
        FILL-IN_SaldoNac FILL-IN_SaldoUsa 
        FILL-IN_Voucher4a FILL-IN_Voucher4c FILL-IN_CodBco4 COMBO_TarjCred
        COMBO_TarjCred FILL-IN-4
        .
    FILL-IN_T_Venta:VISIBLE = NO.
    ASSIGN
        FILL-IN_EmiCheq:SCREEN-VALUE = ""
        FILL-IN_EmiCheq:SENSITIVE = NO.
    /* Por defecto */
    FILL-IN_ImpNac1:SENSITIVE = YES.
    ...
    FILL-IN_CodBco4:SENSITIVE = NO.
    COMBO_TarjCred:SENSITIVE = NO.
    IF PgoTarjCr = "T" THEN DO:
        FILL-IN_ImpNac4:SENSITIVE = YES.
        FILL-IN_ImpUsa4:SENSITIVE = NO.     /* OJO: No activamos pago por Tarj.Cred. en USD */
        FILL-IN_Voucher4a:SENSITIVE = YES.
        FILL-IN_Voucher4c:SENSITIVE = YES.
        FILL-IN_CodBco4:SENSITIVE = YES.
        COMBO_TarjCred:SENSITIVE = YES.
    END.

    /* Activamos/Desactivamos de acuerdo a los importes en pantalla */
    CASE TRUE:
        WHEN FILL-IN_ImpNac4 >= (FILL-IN_SaldoNac - FILL-IN_ImpNac6) THEN DO:
            FILL-IN_ImpNac1 = 0.
            FILL-IN_ImpUsa1 = 0.
            FILL-IN_ImpNac1:SENSITIVE = NO.
            FILL-IN_ImpUsa1:SENSITIVE = NO.
        END.
        WHEN (FILL-IN_ImpNac1 + FILL-IN_ImpNac4) >= (FILL-IN_SaldoNac - FILL-IN_ImpNac6) THEN DO:
            FILL-IN_ImpUsa1 = 0.
            FILL-IN_ImpUsa1:SENSITIVE = NO.
        END.
    END CASE.
