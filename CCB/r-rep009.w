...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Ccbboldep NO-LOCK WHERE Ccbboldep.codcia = s-codcia
    ...
    FOR EACH Ccbccaja NO-LOCK WHERE Ccbccaja.codcia = s-codcia
        ...
        END.
    END.            
    IF ABSOLUTE(Ccbboldep.SdoAct - x-SdoAct) > 0.5 THEN DO:
        CREATE T-BOLE.
        BUFFER-COPY Ccbboldep TO T-BOLE.
    END.
  END.        
