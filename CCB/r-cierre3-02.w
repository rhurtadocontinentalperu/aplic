...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE w-report THEN DO:
        MESSAGE 'Se han encontrado inconsistencias en las ventas manuales' SKIP
            'Se va a imprimir un reporte'
            VIEW-AS ALERT-BOX WARNING.
        GET-KEY-VALUE SECTION 'STARTUP' KEY 'BASE' VALUE RB-REPORT-LIBRARY.
        ASSIGN
            RB-REPORT-LIBRARY = RB-REPORT-LIBRARY + "ccb/rbccb.prl"
            RB-REPORT-NAME = "Diferencia precios venta manual"
            RB-INCLUDE-RECORDS = "O"
            RB-FILTER =
                "w-report.task-no = " + STRING(s-task-no) +
                " AND w-report.Llave-C = '" + s-user-id + "'"
