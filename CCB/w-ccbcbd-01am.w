...
DEFINE NEW SHARED VARIABLE S-TIPO      AS CHAR INITIAL "@CJEM".
...
FILL-IN-fchast-2 x-NroMes f-Division FILL-IN-Tpocmb FILL-IN-TcCompra C-Tipo 
...
DEFINE FRAME F-Main
     ...
     "CUENTAS   POR   COBRAR" VIEW-AS TEXT
          SIZE 25.43 BY .85 AT ROW 1.38 COL 63
          FONT 12
...
/*         ccbdmvto.fchemi >= FILL-IN-fchast-1 AND */
/*         ccbdmvto.fchemi <= FILL-IN-fchast-2,    */
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH t-prev BREAK BY t-prev.tipo BY t-prev.fchast:
        IF FIRST-OF(t-prev.tipo) OR FIRST-OF(t-prev.fchast) 
            THEN ASSIGN dCargo = 0 dAbono = 0.
        IF t-prev.tpomov THEN dCargo = dCargo + t-prev.impmn1.
        ELSE dAbono = dAbono + t-prev.impmn1.
        IF LAST-OF(t-prev.tipo) OR LAST-OF(t-prev.fchast) 
            THEN DO:
            CREATE tt-prev.
            ASSIGN
                tt-prev.tipo    = t-prev.tipo
                tt-prev.fchast  = t-prev.fchast
                tt-prev.periodo = INTEGER(x-Periodo)
                tt-prev.nromes  = INTEGER(x-NroMes)
                tt-prev.codope  = cCodOpe
                tt-prev.coddiv  = f-Division
        ...
        */
        ...
        IF FIRST-OF(B-cdocu.codref) OR FIRST-OF(B-cdocu.nroref) THEN 
            ...
            FIND Cb-cfgrv WHERE Cb-cfgrv.Codcia = CcbCDocu.Codcia 
                ...
                NO-LOCK NO-ERROR.
            /* EL IMPORTE CANJEADO DEPENDE DE LA MONEDA DEL COMPROBANTE Y DEL CANJE */
            fImpTot = b_dmvto.Imptot.
            IF Ccbcdocu.codmon <> Ccbcmvto.codmon THEN DO:
                IF Ccbcdocu.codmon = 1 THEN fImpTot = fImpTot * Ccbcmvto.tpocmb.
                ELSE fImpTot = fImpTot / Ccbcmvto.tpocmb.
            END.
            /* ******************************************************************** */
...
------------------------------------------------------------------------------*/
  DISPLAY x-Periodo FILL-IN-fchast-1 FILL-IN-fchast-2 x-NroMes f-Division 
          FILL-IN-Tpocmb FILL-IN-TcCompra C-Tipo 
...
------------------------------------------------------------------------------*/
    ...
    FIND t-prev WHERE
        t-prev.fchast = PARAM_fchast AND
        t-prev.coddiv = f-Division AND
...
------------------------------------------------------------------------------*/
    ...
    IF param_CodMon = 1 THEN ASSIGN t-prev.impmn1 = B-cdocu.Imptot.
    ELSE ASSIGN
            t-prev.impmn2 = B-cdocu.Imptot
            t-prev.impmn1 = ROUND((B-cdocu.Imptot * t-prev.Tpocmb),2).
    
    
/*     FIND t-prev WHERE t-prev.fchast = PARAM_fchast                                     */
/*         AND t-prev.coddiv = ccbcmvto.Coddiv                                            */
...
FOR EACH t-prev BREAK BY t-prev.tipo BY t-prev.fchast:
    IF FIRST-OF(t-prev.tipo) OR FIRST-OF(t-prev.fchast) THEN DO:
        /* Verifica si el movimiento se realizó anteriormente */
        FIND cb-control WHERE
            cb-control.CodCia  = s-codcia AND
            cb-control.Coddiv  = f-Division AND
            cb-control.tipo    = s-Tipo AND
            cb-control.fchpro  = t-prev.fchast AND
            cb-control.tipmov  = t-prev.tipo NO-ERROR.
