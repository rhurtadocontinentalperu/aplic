...
FIND FIRST FacDocum WHERE FacDocum.CodCia = s-codcia
    AND FacDocum.CodDoc = "PRP"
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE FacDocum THEN DO:
    MESSAGE "No está configurado el documento PRP (Pre-Percepciones)"
        VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
FIND FIRST FacCorre WHERE FacCorre.CodCia = S-CODCIA 
    AND FacCorre.CodDoc = "PRC" 
    AND FacCorre.CodDiv = s-CodDiv 
    AND FacCorre.FlgEst = YES
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE Faccorre THEN DO:
    MESSAGE 'NO está configurado el correlativo para el documento: PRC (CONTROL PERCEPCION CARGOS)'  SKIP
        'para la división' s-coddiv
        VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
FIND FIRST FacCorre WHERE FacCorre.CodCia = S-CODCIA 
    AND FacCorre.CodDoc = "PRA" 
    AND FacCorre.CodDiv = s-CodDiv 
    AND FacCorre.FlgEst = YES
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE Faccorre THEN DO:
    MESSAGE 'NO está configurado el correlativo para el documento: PRA (CONTROL PERCEPCION ABONOS)'  SKIP
        'para la división' s-coddiv
        VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
...
DEFINE VARIABLE h_b-canc-mayorista-cont-v3 AS HANDLE NO-UNDO.
...
DO:
    ...
    RUN proc_CanPed IN h_b-canc-mayorista-cont-v3.
...
DO:
    RUN dispatch IN h_b-canc-mayorista-cont-v3 ('open-query-cases':U).
    RUN proc_CalTot IN h_b-canc-mayorista-cont-v3.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'CCB/b-canc-mayorista-cont-v3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-canc-mayorista-cont-v3 ).
       RUN set-position IN h_b-canc-mayorista-cont-v3 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-canc-mayorista-cont-v3 ( 15.77 , 86.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-canc-mayorista-cont-v3 ,
