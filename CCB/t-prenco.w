...
&Scoped-define INTERNAL-TABLES PEDI CcbTabla
...
CcbTabla.Nombre PEDI.CanPed PEDI.PreUni PEDI.ImpLin 
...
&Scoped-define QUERY-STRING-br_table FOR EACH PEDI WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH CcbTabla WHERE CcbTabla.CodCia = PEDI.CodCia ~
  AND CcbTabla.Tabla = 'N/C' ~
  AND CcbTabla.Codigo = PEDI.codmat NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH PEDI WHERE ~{&KEY-PHRASE} NO-LOCK, ~
  ...
  AND CcbTabla.Codigo = PEDI.codmat NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table PEDI CcbTabla
...
&Scoped-Define DISPLAYED-OBJECTS F-ImpExo F-ImpBrt F-ValVta F-ImpIgv ~
F-ImpTot 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      PEDI.NroItm COLUMN-LABEL "Item" FORMAT ">>9":U COLUMN-FONT 2
      PEDI.codmat COLUMN-LABEL "Codigo" FORMAT "X(6)":U
      CcbTabla.Nombre COLUMN-LABEL "Descripcion" FORMAT "x(60)":U
            WIDTH 49.14
      PEDI.CanPed FORMAT ">,>>>,>>9.9999":U WIDTH 8.72
      PEDI.PreUni COLUMN-LABEL "P. Unitario" FORMAT ">,>>>,>>9.99999":U
            WIDTH 12.29
      PEDI.ImpLin FORMAT "->>,>>>,>>9.99":U WIDTH 11.72
  ENABLE
      PEDI.codmat
      PEDI.CanPed
      PEDI.PreUni
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 97 BY 6.35
...
DEFINE FRAME F-Main
     ...
     F-ImpExo AT ROW 8.38 COL 46 COLON-ALIGNED NO-LABEL
     F-ImpBrt AT ROW 8.5 COL 34 NO-LABEL
     F-ValVta AT ROW 8.5 COL 58 COLON-ALIGNED NO-LABEL
     F-ImpIgv AT ROW 8.5 COL 70.86 COLON-ALIGNED NO-LABEL
     F-ImpTot AT ROW 8.5 COL 83.43 COLON-ALIGNED NO-LABEL
     "Total Bruto" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 7.92 COL 36
     "Total Importe" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 7.92 COL 85.57
     "I.G.V." VIEW-AS TEXT
          SIZE 5.14 BY .5 AT ROW 7.92 COL 75.57
     "Valor Venta" VIEW-AS TEXT
          SIZE 8 BY .54 AT ROW 7.92 COL 61.72
     "Exonerado" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 7.92 COL 48.57
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.PEDI,INTEGRAL.CcbTabla WHERE Temp-Tables.PEDI ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[2]      = "INTEGRAL.CcbTabla.CodCia = Temp-Tables.PEDI.CodCia
  AND INTEGRAL.CcbTabla.Tabla = 'N/C'
  AND INTEGRAL.CcbTabla.Codigo = Temp-Tables.PEDI.codmat"
     _FldNameList[1]   > Temp-Tables.PEDI.NroItm
"PEDI.NroItm" "Item" ? "integer" ? ? 2 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.PEDI.codmat
"PEDI.codmat" "Codigo" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.CcbTabla.Nombre
"CcbTabla.Nombre" "Descripcion" "x(60)" "character" ? ? ? ? ? ? no ? no no "49.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.PEDI.CanPed
"PEDI.CanPed" ? ? "decimal" ? ? ? ? ? ? yes ? no no "8.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.PEDI.PreUni
"PEDI.PreUni" "P. Unitario" ? "decimal" ? ? ? ? ? ? yes ? no no "12.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.PEDI.ImpLin
"PEDI.ImpLin" ? ? "decimal" ? ? ? ? ? ? no ? no no "11.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF PEDI.codmat IN BROWSE br_table /* Codigo */
DO:
  ...
  lCodigo = SELF:SCREEN-VALUE.
  lRowid = ROWID(PEDI).

  FIND FIRST b-PEDI WHERE PEDI.CodMat = lCodigo NO-LOCK NO-ERROR.

  IF AVAILABLE b-pedi AND ROWID(b-pedi) <> lRowId THEN DO:
      MESSAGE "Codigo ya existe..." VIEW-AS ALERT-BOX.
      RETURN NO-APPLY.
  END.
  ELSE DO:
      ASSIGN
        SELF:SCREEN-VALUE = STRING(INTEGER(SELF:SCREEN-VALUE), '99999')
        NO-ERROR.
      FIND CcbTabla WHERE CcbTabla.CodCia = s-codcia AND 
           CcbTabla.Tabla  = 'N/C' AND 
           CcbTabla.Codigo = PEDI.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
           NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  RUN Procesa-Handle IN lh_handle ('Valida-Detalle').
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAILABLE FacCfgGn THEN DO:
    MESSAGE 'No existe la configuración general (faccfggn)'
  ...
  IF CcbTabla.Afecto THEN
    ASSIGN
        PEDI.AftIgv = Yes
        PEDI.ImpIgv = (PEDI.CanPed * PEDI.PreUni) * ((s-PorIgv / 100) / (1 + (s-PorIgv / 100))).
  ELSE
    ASSIGN
        PEDI.AftIgv = No
        PEDI.ImpIgv = 0.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAILABLE CcbTabla THEN DO:
    MESSAGE 'Codigo no registrado' VIEW-AS ALERT-BOX ERROR.
