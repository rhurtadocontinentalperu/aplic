...
&Scoped-define Condicion CCbCChq.codcia = s-codcia 
/*AND CCbCChq.coddiv = s-coddiv*/
...
&Scoped-define INTERNAL-TABLES CCbCChq

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for QUERY Query-Main                                     */
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH CCbCChq WHERE ~{&KEY-PHRASE} ~
      AND CCbCChq.CodCia = s-codcia ~
 NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main CCbCChq
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main CCbCChq
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "integral.CCbCChq"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "integral.CCbCChq.CodCia = s-codcia
"
     _Design-Parent    is WINDOW q-tables @ ( 1.12 , 1 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
DO:
  ...
  RUN lkup/c-boldep ("Boletas de Depositos").
...
DO:
  ...
  FIND CCbCChq WHERE {&Condicion} AND nrodoc = SELF:SCREEN-VALUE NO-LOCK NO-ERROR.
  IF AVAILABLE CCbCChq
  THEN output-var-1 = ROWID(CCbCChq).
  ELSE output-var-1 = ?.
  RUN dispatch IN THIS-PROCEDURE ('qbusca':U).
  SELF:SCREEN-VALUE = "".
END.
