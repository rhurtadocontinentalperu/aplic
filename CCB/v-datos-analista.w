...
&IF DEFINED(s-nivel-acceso) &THEN
    DEFINE SHARED VAR s-nivel-acceso AS INT NO-UNDO.
&ELSE
    DEFINE VARIABLE s-nivel-acceso AS INTEGER NO-UNDO INIT 0.
    &SCOPED-DEFINE s-nivel-acceso
&ENDIF

IF USERID("DICTDB") = "ADMIN" THEN DO:
    /*MESSAGE "v-datos-analista".*/
END.
...
&Scoped-Define ENABLED-FIELDS gn-clie.JfeLog[3] gn-clie.JfeLog[4] 
&Scoped-define ENABLED-TABLES gn-clie
&Scoped-define FIRST-ENABLED-TABLE gn-clie
&Scoped-Define DISPLAYED-FIELDS gn-clie.JfeLog[3] gn-clie.JfeLog[4] 
...
DEFINE FRAME F-Main
     gn-clie.JfeLog[3] AT ROW 1.23 COL 22.43 COLON-ALIGNED WIDGET-ID 42
          LABEL "e-mail analista" FORMAT "X(150)"
          VIEW-AS FILL-IN 
          SIZE 48.57 BY .81
          BGCOLOR 11 
     gn-clie.JfeLog[4] AT ROW 2.04 COL 22.43 COLON-ALIGNED WIDGET-ID 44
          LABEL "e-mail cobranzas cliente" FORMAT "X(150)"
          VIEW-AS FILL-IN 
          SIZE 48.57 BY .81
          BGCOLOR 11 
     FILL-IN-1 AT ROW 2.92 COL 25.72 COLON-ALIGNED NO-LABEL WIDGET-ID 40
...
DO WITH FRAME {&FRAME-NAME} :
    ...
    IF gn-clie.jfelog[3]:SCREEN-VALUE > '' THEN DO:
        x-Nro-EMails = NUM-ENTRIES(gn-clie.jfelog[3]:SCREEN-VALUE,';').
        DO x-Item = 1 TO x-Nro-EMails:
            RUN gn/valida-email (ENTRY(x-Item,gn-clie.jfelog[3]:SCREEN-VALUE,';'), 
                                 OUTPUT pError).
            IF pError > '' THEN DO:
                MESSAGE 'e-mail del contacto mal registrado' VIEW-AS ALERT-BOX ERROR.
                APPLY 'ENTRY':U TO gn-clie.jfelog[3].
                RETURN 'ADM-ERROR'.
            END.
        END.
    END.
    IF gn-clie.jfelog[4]:SCREEN-VALUE > '' THEN DO:
        x-Nro-EMails = NUM-ENTRIES(gn-clie.jfelog[4]:SCREEN-VALUE,';').
        DO x-Item = 1 TO x-Nro-EMails:
            RUN gn/valida-email (ENTRY(x-Item,gn-clie.jfelog[4]:SCREEN-VALUE,';'), 
                                 OUTPUT pError).
            IF pError > '' THEN DO:
                MESSAGE 'e-mail facturacion electronica mal registrado' VIEW-AS ALERT-BOX ERROR.
                APPLY 'ENTRY':U TO gn-clie.jfelog[4].
                RETURN 'ADM-ERROR'.
            END.
        END.
    END.
