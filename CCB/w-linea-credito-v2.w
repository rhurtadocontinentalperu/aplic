...
DEFINE NEW SHARED VAR s-nivel-acceso AS INT INIT 0 NO-UNDO.
...
DEFINE VARIABLE h_b-lc-gn-cliel-v2 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-autcli-2 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'CCB/v-autcli-2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-autcli-2 ).
       RUN set-position IN h_v-autcli-2 ( 2.73 , 2.00 ) NO-ERROR.
       ...
       RUN set-size IN h_folder ( 10.23 , 100.14 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-autcli-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_v-autcli-2 ).
       RUN add-link IN adm-broker-hdl ( h_q-client , 'Record':U , h_v-autcli-2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv97 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-autcli-2 ,
             h_p-updv97 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_v-autcli-2 , 'AFTER':U ).
    ...
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/b-lc-gn-cliel-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-lc-gn-cliel-v2 ).
       RUN set-position IN h_b-lc-gn-cliel-v2 ( 17.92 , 5.00 ) NO-ERROR.
       RUN set-size IN h_b-lc-gn-cliel-v2 ( 7.58 , 68.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12-2 ).
       RUN set-position IN h_p-updv12-2 ( 19.04 , 74.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12-2 ( 4.58 , 14.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-lc-gn-cliel-v2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12-2 , 'TableIO':U , h_b-lc-gn-cliel-v2 ).
       RUN add-link IN adm-broker-hdl ( h_q-client , 'Record':U , h_b-lc-gn-cliel-v2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-lc-gn-cliel-v2 ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12-2 ,
             h_b-lc-gn-cliel-v2 , 'AFTER':U ).
    END. /* Page 4 */
    WHEN 5 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
...
RUN riesgo-crediticio IN h_v-autcli-2(INPUT pLComoDNI).
