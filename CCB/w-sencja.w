...
DEFINE NEW SHARED VARIABLE s-tipo    AS CHAR INITIAL "SENCILLO".
...
DEFINE VARIABLE h_v-sencja AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Sencillo de Caja */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Sencillo de Caja */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-remesa ).
       RUN set-position IN h_q-remesa ( 7.92 , 6.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/v-sencja.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-sencja ).
       RUN set-position IN h_v-sencja ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.73 , 77.00 ) */

       /* Links to SmartViewer h_v-sencja. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-sencja ).
       RUN add-link IN adm-broker-hdl ( h_q-remesa , 'Record':U , h_v-sencja ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-sencja ,
             h_p-updv10 , 'AFTER':U ).
    END. /* Page 1 */
