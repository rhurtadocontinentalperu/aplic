...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      IF CcbBolDep.TipDep = 'EFE' THEN 'Efectivo' ELSE 'Cheque' @ x-CndPgo COLUMN-LABEL "Condición!Pago"
      CcbBolDep.NroRef COLUMN-LABEL "Número!Depósito" FORMAT "X(12)"
      CcbBolDep.FchReg COLUMN-LABEL "Fecha de!Depósito" FORMAT "99/99/99"
      IF CcbBolDep.CodMon = 1 THEN 'S/.' ELSE 'US$' @ x-moneda COLUMN-LABEL "Moneda"
      CcbBolDep.ImpTot
      CcbBolDep.CodCta
      CcbBolDep.DesCta COLUMN-LABEL "Descripción"
      CcbBolDep.usuario COLUMN-LABEL "Usuario!Registro"
      CcbBolDep.Autorizo
      CcbBolDep.FchAut COLUMN-LABEL "Fecha!Autorización"
      CcbBolDep.FlgSit
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"IF CcbBolDep.TipDep = 'EFE' THEN 'Efectivo' ELSE 'Cheque' @ x-CndPgo" "Condición!Pago" ? ? ? ? ? ? ? ? no ?
     _FldNameList[4]   > integral.CcbBolDep.NroRef
"NroRef" "Número!Depósito" "X(12)" "character" ? ? ? ? ? ? no ?
     _FldNameList[5]   > integral.CcbBolDep.FchReg
"FchReg" "Fecha de!Depósito" "99/99/99" "date" ? ? ? ? ? ? no ?
...
"DesCta" "Descripción" ? "character" ? ? ? ? ? ? no ?
...
"FchAut" "Fecha!Autorización" ? "date" ? ? ? ? ? ? no ?
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE CcbCdocu THEN DO:
        IF CcbCdocu.SdoAct <> CcbCdocu.ImpTot THEN DO:
            MESSAGE
                'Depósito presenta cancelaciones'
