...
&Scoped-Define ENABLED-OBJECTS B-pre-asto x-Periodo x-NroMes-1 BUTTON-1 ~
x-NroMes-2 
&Scoped-Define DISPLAYED-OBJECTS x-Periodo FILL-IN-fchast-1 ~
FILL-IN-fchast-2 x-NroMes-1 FILL-IN-Tpocmb FILL-IN-TcCompra x-NroMes-2 
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       /* Links to SmartBrowser h_b-cjacbd-3. */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv10 ,
             B-pre-asto:HANDLE IN FRAME F-Main , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  DISPLAY x-Periodo FILL-IN-fchast-1 FILL-IN-fchast-2 x-NroMes-1 FILL-IN-Tpocmb 
          FILL-IN-TcCompra x-NroMes-2 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE B-pre-asto x-Periodo x-NroMes-1 BUTTON-1 x-NroMes-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH CcbDMvto WHERE CcbDMvto.CodCia = s-codcia 
        ...
        FIND cb-cfgcja WHERE
                ...
                cb-cfgcja.CodDiv = CcbDMvto.CodDiv
                NO-LOCK NO-ERROR.
        IF NOT AVAILABLE cb-cfgcja THEN DO:
            MESSAGE
                "Configuracion Cierre de Caja(" + CcbDMvto.CodDiv + ") no existe." SKIP
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN
        ...
        t-prev.nromes  = MONTH(Ccbdmvto.FchEmi)
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH t-prev NO-LOCK BREAK BY t-prev.tipo BY t-prev.codcta:
...
FIND Gn-Divi WHERE Gn-Divi.Codcia = S-CODCIA AND
                   Gn-Divi.Coddiv = F-DIVISIONXX NO-LOCK NO-ERROR.
...
FOR EACH cb-control WHERE cb-control.CodCia  = s-codcia AND
    cb-control.CodDiv  = F-DIVISIONXX AND
...
FOR EACH t-prev BREAK BY t-prev.Tipo:
    IF FIRST-OF(t-prev.Tipo) THEN DO:
        ...
        ASSIGN
            ...
            cb-control.coddiv  = F-DIVISIONXX            /* t-prev.coddiv. */
            cb-control.tipmov  = t-prev.tipo.
        ASSIGN
...
/*MLR-1*/   detalle.CodRef   = cb-dmov.CodRef.
    ...
    IF LAST-OF(t-prev.tipo) THEN DO:
       ...
       END.
       cb-cmov.Coddiv = F-DIVISIONXX.         /* t-prev.coddiv. */
       ...
       cb-cmov.NOTAST = 'CIERRE DE CAJA TIENDA ' + F-DIVISIONXX + "-" + Gn-Divi.DesDiv.
       cb-cmov.GLOAST = 'CIERRE DE CAJA TIENDA ' + F-DIVISIONXX + "-" + Gn-Divi.DesDiv.
