...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacCPedi.NroPed FORMAT "XXX-XXXXXXXX":U WIDTH 10.43
...
"FacCPedi.NroPed" ? "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.usuario" "Usuario" ? "character" ? ? ? ? ? ? no ? no no "6" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
FOR EACH T-CODES NO-LOCK, FIRST FacCPedi WHERE FacCPedi.codcia = T-CODES.codcia
    ...
    FOR EACH FacDPedi OF FacCPedi NO-LOCK BY FacDPedi.NroItm:
        ...
        IF NOT AVAILABLE Almmmate THEN DO:
            pMensaje = "Artículo " + FacDPedi.CodMat + " NO está asignado al almacén " + Faccpedi.codalm + CHR(10) +
                'Proceso abortado'.
...
/*     END.                                                                */
    FOR EACH T-CODES NO-LOCK, FIRST FacCPedi NO-LOCK WHERE FacCPedi.codcia = T-CODES.codcia
        ...
        FOR EACH FacDPedi OF FacCPedi NO-LOCK,
            ...
            /* RHC Quiebre solo para división 00060 */
/*             IF Faccpedi.coddiv = "00060" AND LOOKUP(s-coddoc, 'BOL,TCK') > 0                               */
/*                 AND iCountItem > 1 AND lCreaHeader = NO                                                    */
/*                 THEN DO:                                                                                   */
/*                 IF Faccpedi.codmon = 1 AND x-ImporteAcumulado + Facdpedi.ImpLin > 1500                     */
/*                     THEN lCreaHeader = TRUE.                                                               */
/*                 IF Faccpedi.codmon = 2 AND (x-ImporteAcumulado + Facdpedi.ImpLin) * Faccpedi.tpocmb > 1500 */
/*                     THEN lCreaHeader = TRUE.                                                               */
...
/* ***************************************************** */
/* RHC 18/01/2016 SOLO para la división 00067 (Trujillo) */
/*
IF s-CodDiv = "00067" AND s-CodDoc = "FAC" THEN s-CodDoc = "TCK".    /* Forzamos TCK */
*/
/* ***************************************************** */
...
pMensaje = "".  /* Si hay un mensaje => ahay un error */
PRINCIPAL:
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
