...
&Scoped-define INTERNAL-TABLES CcbCDocu CcbTabla tt-w-report-conceptos
...
CcbCDocu.ImpTot CcbCDocu.CodCta CcbTabla.Nombre 
...
&Scoped-define QUERY-STRING-BROWSE-10 FOR EACH CcbCDocu ~
 ...
 AND CcbCDocu.FlgEst = "X" NO-LOCK, ~
      EACH CcbTabla WHERE TRUE /* Join to CcbDDocu incomplete */ ~
      AND CcbTabla.CodCia = CcbcDocu.CodCia ~
  AND CcbTabla.Tabla = CcbcDocu.CodDoc ~
  AND CcbTabla.Codigo = CcbcDocu.codcta NO-LOCK, ~
      EACH tt-w-report-conceptos WHERE tt-w-report-conceptos.Llave-C = CcbCDocu.CodCta OUTER-JOIN NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-10 OPEN QUERY BROWSE-10 FOR EACH CcbCDocu ~
 ...
 AND CcbCDocu.FlgEst = "X" NO-LOCK, ~
  ...
  AND CcbTabla.Codigo = CcbcDocu.codcta NO-LOCK, ~
      EACH tt-w-report-conceptos WHERE tt-w-report-conceptos.Llave-C = CcbCDocu.CodCta OUTER-JOIN NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-10 CcbCDocu CcbTabla ~
tt-w-report-conceptos
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-10 CcbCDocu
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-10 CcbTabla
&Scoped-define THIRD-TABLE-IN-QUERY-BROWSE-10 tt-w-report-conceptos
...
DEFINE VARIABLE RADIO-SET-1 AS CHARACTER INITIAL "OTROS" 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "N/C por Rebate", "REBATE",
"N/C por A/C", "ADELANTO",
"N/C por Otros", "OTROS"
     SIZE 47.72 BY .96 NO-UNDO.
...
DEFINE FRAME fMain
     RADIO-SET-1 AT ROW 1.15 COL 72 NO-LABEL WIDGET-ID 4
     SELECT-CodDiv AT ROW 1.69 COL 4.29 NO-LABEL WIDGET-ID 10
     BUTTON-9 AT ROW 2.31 COL 81 WIDGET-ID 14
     BUTTON-10 AT ROW 3.46 COL 81 WIDGET-ID 16
     BROWSE-10 AT ROW 6.58 COL 2 WIDGET-ID 200
     "20 documentos por vez" VIEW-AS TEXT
          SIZE 64.57 BY 1.73 AT ROW 4.69 COL 56.43 WIDGET-ID 18
          FGCOLOR 12 FONT 8
     "Seleccione una división" VIEW-AS TEXT
          SIZE 17 BY .5 AT ROW 1.12 COL 4.29 WIDGET-ID 12
          BGCOLOR 4 FGCOLOR 15 
     "Filtrar:" VIEW-AS TEXT
          SIZE 5 BY .5 AT ROW 1.27 COL 65.43 WIDGET-ID 8
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 124.43 BY 20.08
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "PROBACION O RECHAZO DE NOTAS DE CREDITO - OTROS"
         HEIGHT             = 20.08
         WIDTH              = 124.43
         MAX-HEIGHT         = 39.12
         MAX-WIDTH          = 274.29
...
/* Query rebuild information for BROWSE BROWSE-10
     _TblList          = "INTEGRAL.CcbCDocu,INTEGRAL.CcbTabla WHERE INTEGRAL.CcbDDocu ...,Temp-Tables.tt-w-report-conceptos WHERE INTEGRAL.CcbCDocu ..."
 ...
 AND INTEGRAL.CcbCDocu.FlgEst = ""X"""
     _Where[2]         = "INTEGRAL.CcbTabla.CodCia = INTEGRAL.CcbcDocu.CodCia
  AND INTEGRAL.CcbTabla.Tabla = INTEGRAL.CcbcDocu.CodDoc
  AND INTEGRAL.CcbTabla.Codigo = INTEGRAL.CcbcDocu.codcta"
     _JoinCode[3]      = "Temp-Tables.tt-w-report-conceptos.Llave-C = CcbCDocu.CodCta"
...
ON END-ERROR OF wWin /* PROBACION O RECHAZO DE NOTAS DE CREDITO - OTROS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* PROBACION O RECHAZO DE NOTAS DE CREDITO - OTROS */
DO:
...
x-cuantos = BROWSE-10:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}.

IF x-cuantos > 20 THEN DO:
    MESSAGE "El proceso solo acepta como maximo 20 documentos por cada proceso".
    RETURN "ADM-ERROR".
END.
...
DO k = {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME} TO 1 BY -1 
    ...
    IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(k) IN FRAME {&FRAME-NAME} THEN DO:        
        ...
        /* Verificar si no supera al monto de la factura */
        RUN validar(INPUT Ccbcdocu.codref,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE validar wWin 
PROCEDURE validar :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
RUN notas-creditos-supera-comprobante IN hxProc (INPUT pCodDoc, 
                                             INPUT pNroDoc,
                                             OUTPUT pRetVal).
