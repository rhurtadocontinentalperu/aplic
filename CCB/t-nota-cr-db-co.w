...
DEFINE VARIABLE S-PORIGV AS DEC NO-UNDO.
...
Almmmatg.DesMat Almmmatg.DesMar DETA.UndVta DETA.CanDes DETA.ImpLin 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      DETA.codmat COLUMN-LABEL "Artículo" FORMAT "X(8)":U
      Almmmatg.DesMat FORMAT "X(100)":U WIDTH 84
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(30)":U WIDTH 10.43
      DETA.UndVta COLUMN-LABEL "Unidad" FORMAT "x(8)":U WIDTH 10.43
      DETA.CanDes FORMAT ">,>>>,>>9.9999":U WIDTH 8.43
      DETA.ImpLin COLUMN-LABEL "Importe!con IGV" FORMAT "->>,>>>,>>9.99":U
            WIDTH 11.72
  ENABLE
      DETA.CanDes
/* _UIB-CODE-BLOCK-END */
...
DEFINE FRAME F-Main
     ...
     F-ImpTot AT ROW 13.12 COL 124 COLON-ALIGNED WIDGET-ID 44
     RECT-2 AT ROW 12.85 COL 1 WIDGET-ID 50
...
"DETA.NroItm" "No" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.DETA.codmat
"DETA.codmat" "Artículo" "X(8)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.Almmmatg.DesMat
"Almmmatg.DesMat" ? "X(100)" "character" ? ? ? ? ? ? no ? no no "84" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.Almmmatg.DesMar
"Almmmatg.DesMar" "Marca" ? "character" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.DETA.UndVta
"DETA.UndVta" "Unidad" ? "character" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.DETA.CanDes
"DETA.CanDes" ? ? "decimal" ? ? ? ? ? ? yes ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.DETA.ImpLin
"DETA.ImpLin" "Importe!con IGV" ? "decimal" ? ? ? ? ? ? no ? no no "11.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DEF INPUT PARAMETER TABLE FOR DETA.
...
IF DECIMAL(DETA.CanDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) <= 0 THEN DO:
   ...
   APPLY 'ENTRY':U TO DETA.CanDes IN BROWSE {&browse-name}.
   RETURN "ADM-ERROR".   
END.
IF DECIMAL(DETA.CanDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > DETA.CanDev THEN DO:
   MESSAGE "Cantidad no puede ser mayor a " DETA.CanDev VIEW-AS ALERT-BOX ERROR.
   APPLY 'ENTRY':U TO DETA.CanDes IN BROWSE {&browse-name}.
   RETURN "ADM-ERROR".   
END.
