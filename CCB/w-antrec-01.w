...
DEFINE VARIABLE h_v-antrec-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn-Print AT ROW 8.77 COL 73.29
...
DO:
    RUN dispatch IN h_v-antrec-01 ('imprime':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 8.69 , 19.14 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv10 ).
       RUN set-position IN h_p-updv10 ( 8.69 , 36.14 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-remesa ).
       RUN set-position IN h_q-remesa ( 8.77 , 6.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/v-antrec-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-antrec-01 ).
       RUN set-position IN h_v-antrec-01 ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.69 , 77.00 ) */

       /* Links to SmartViewer h_v-antrec-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-antrec-01 ).
       RUN add-link IN adm-broker-hdl ( h_q-remesa , 'Record':U , h_v-antrec-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-antrec-01 ,
             Btn-Print:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
