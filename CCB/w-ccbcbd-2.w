...
cCodCtab[1] = "123103".             /* Cuenta Soles */
cCodCtab[2] = "123104".             /* Cuenta Dólares */
/* Operación */
cCodOpe = "064".
...
DEFINE VARIABLE h_b-ccbcbd-2 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Generacion de Asiento Contable (Letras al Banco)"
         HEIGHT             = 15.58
         WIDTH              = 130.57
         MAX-HEIGHT         = 15.58
         MAX-WIDTH          = 130.57
         VIRTUAL-HEIGHT     = 15.58
         VIRTUAL-WIDTH      = 130.57
         RESIZE             = no
         SCROLL-BARS        = no
         STATUS-AREA        = no
         BGCOLOR            = ?
...
ON END-ERROR OF W-Win /* Generacion de Asiento Contable (Letras al Banco) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Generacion de Asiento Contable (Letras al Banco) */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/ccb/b-ccbcbd-2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ccbcbd-2 ).
       RUN set-position IN h_b-ccbcbd-2 ( 5.58 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-ccbcbd-2 ( 10.77 , 127.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-ccbcbd-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_b-ccbcbd-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-ccbcbd-2 ,
             FILL-IN-periodo:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv10 ,
             h_b-ccbcbd-2 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH CcbCMvto NO-LOCK WHERE
        ...
        CcbCDocu.NroDoc = CcbDMvto.NroRef NO-LOCK
        BREAK BY ccbdmvto.nrodoc:

        DISPLAY
            "  Procesando Letra: " + ccbdmvto.nrodoc @
            Fi-Mensaje
    ...
    RUN dispatch IN h_b-ccbcbd-2 ('open-query':U).
    RUN Totales IN h_b-ccbcbd-2.
...
/*ML01*/
    IF CcbCDocu.Imptot <> 0 THEN DO:
        IF ccbcmvto.CodMon = 1 THEN
            ASSIGN t-prev.impmn1 = t-prev.impmn1 + CcbCDocu.Imptot.
        ELSE
            ASSIGN
                t-prev.impmn2 = t-prev.impmn2 + CcbCDocu.Imptot
                t-prev.impmn1 = t-prev.impmn1 + ROUND((CcbCDocu.Imptot * t-prev.Tpocmb),2).
    END.
/*ML01*/
/*ML01*/ ELSE DO:
    IF ccbcmvto.CodMon = 1 THEN
