...
FIND FacDocum WHERE FacDocum.codcia = s-codcia
  AND FacDocum.coddoc = 'ASG' 
  NO-LOCK NO-ERROR.
IF NOT AVAILABLE FacDocum
THEN DO:
  MESSAGE 'La Asignacion de N/C (ASG) no está configurada en el maestro de documentos'
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "ASIENTO CONTABLE APLICACIONES NOTAS DE CREDITO"
         HEIGHT             = 15.23
         WIDTH              = 108.43
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 108.43
...
ON END-ERROR OF W-Win /* ASIENTO CONTABLE APLICACIONES NOTAS DE CREDITO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ASIENTO CONTABLE APLICACIONES NOTAS DE CREDITO */
DO:
...
FOR EACH T-DMOV BREAK BY T-DMOV.coddiv:
    ...
    IF LAST-OF(T-DMOV.coddiv) THEN DO:
        ...
        ASSIGN
            ...
            cb-cmov.NOTAST = 'Aplicaciones de Notas de Credito del ' + STRING(FILL-IN-fchdoc)
            cb-cmov.GLOAST = 'Aplicaciones de Notas de Credito del ' + STRING(FILL-IN-fchdoc).
    END.
END.
RUN Actualiza-Flag.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH CcbDCaja NO-LOCK WHERE ccbdcaja.codcia = s-codcia
        ...
        FIRST CcbCDocu NO-LOCK WHERE ccbcdocu.codcia = s-codcia
            ...
            AND ccbcdocu.coddiv = COMBO-BOX-CodDiv:
    /* CUENTA AL HABER */
    /* DETERMINAMOS LA CUENTA DEL DOCUMENTO DE REFERENCIA */
    FIND FacDocum WHERE FacDocum.codcia = s-codcia
      AND FacDocum.coddoc = ccbcdocu.coddoc
      NO-LOCK NO-ERROR.
    IF NOT AVAILABLE FacDocum
    THEN DO:
      MESSAGE 'El documento' ccbcdocu.coddoc 'no está configurada en el maestro de documentos'
          VIEW-AS ALERT-BOX WARNING.
      RETURN.
    END.
    ...
    x-Importe = CcbDCaja.ImpTot.
    IF ccbcdocu.codmon <> ccbdcaja.codmon   /* Importe de acuerdo al origen */
    THEN IF ccbcdocu.codmon = 1 
        THEN x-Importe = ROUND(x-Importe * ccbdcaja.tpocmb, 2).
        ELSE x-Importe = ROUND(x-Importe / ccbdcaja.tpocmb, 2).
    ...
    /* CUENTA AL DEBE */
    FIND FacDocum WHERE FacDocum.codcia = s-codcia
      AND FacDocum.coddoc = ccbdcaja.coddoc
      NO-LOCK NO-ERROR.
    IF NOT AVAILABLE FacDocum
    THEN DO:
      MESSAGE 'El documento' ccbdcaja.coddoc 'no está configurada en el maestro de documentos'
          VIEW-AS ALERT-BOX WARNING.
      RETURN.
    END.
    x-CodCta = IF ccbcdocu.codmon = 1 THEN FacDocum.CodCta[1] ELSE FacDocum.CodCta[2].
  ...
  DEF VAR x-DebeMn1  AS DEC INIT 0 NO-UNDO.
  DEF VAR x-HaberMn1 AS DEC INIT 0 NO-UNDO.
  DEF VAR x-DebeMn2  AS DEC INIT 0 NO-UNDO.
  DEF VAR x-HaberMn2 AS DEC INIT 0 NO-UNDO.
  
  FOR EACH T-DMOV:
    IF T-DMOV.TpoMov = NO
    THEN ASSIGN
            x-DebeMn1 = x-DebeMn1 + T-DMOV.ImpMn1
            x-DebeMn2 = x-DebeMn2 + T-DMOV.ImpMn2.
    ELSE ASSIGN
            x-HaberMn1 = x-HaberMn1 + T-DMOV.ImpMn1
            x-HaberMn2 = x-HaberMn2 + T-DMOV.ImpMn2.
  END.
  IF x-DebeMn1 <> x-HaberMn1
  THEN DO:
    FIND cb-cfgg WHERE cb-cfgg.CodCia = cb-codcia 
        AND cb-cfgg.Codcfg = 'C01' NO-LOCK NO-ERROR.
    IF AVAILABLE cb-cfgg
    THEN DO:
        x-CodCta = IF x-DebeMn1 < x-HaberMn1 THEN cb-cfgg.codcta[1] ELSE cb-cfgg.codcta[2].
        x-TpoMov = IF x-DebeMn1 < x-HaberMn1 THEN NO ELSE YES.
        RUN Graba-Detalle (x-codope,
                        COMBO-BOX-CodDiv,
                        '',
                        x-CodCta,
  ...
  /* CASO DE VENTA HORIZONTAL DIVISION 00014 */
  FOR EACH T-DMOV WHERE T-DMOV.CodDiv = '00014' 
        AND T-DMOV.CodOpe = '076' 
        AND T-DMOV.CodCta BEGINS '121102':
    ASSIGN
        T-DMOV.CodCta = '121201'
        T-DMOV.CodAux = '11111111'.
  END.
