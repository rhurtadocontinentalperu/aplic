...
DEF NEW SHARED VAR s-tipo   AS CHAR INITIAL "OTROS".
...
DEFINE VARIABLE h_v-remesa AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* EGRESO DE CAJA - Otros */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* EGRESO DE CAJA - Otros */
DO:
...
DO:
  RUN dispatch IN h_v-remesa ('imprime':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/v-remesas-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-remesa ).
       RUN set-position IN h_v-remesa ( 1.00 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 6.73 , 76.57 ) */

       /* Links to SmartViewer h_v-remesa. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-remesa ).
       RUN add-link IN adm-broker-hdl ( h_q-remesa , 'Record':U , h_v-remesa ).
