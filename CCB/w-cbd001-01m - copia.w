...
DO:
  IF L-BUTTON-1 = YES
  THEN DO:
    ASSIGN f-Division FILL-IN-TcCompra FILL-IN-Tpocmb x-NroMes x-Periodo FILL-IN-fchast-1 FILL-IN-fchast-2.
...
/*ML01*/ CB-DMOV.NroRef = T-DMOV.NroRef
    ...
    IF LAST-OF(T-dmov.FCHAST) THEN DO:
        ...
        ASSIGN
            ...
            cb-cmov.NOTAST = 'Boletas de Deposito del ' + STRING(T-DMOV.fchast)
            cb-cmov.GLOAST = 'Boletas de Deposito del ' + STRING(T-DMOV.fchast).
    END.
END.
RUN Actualiza-Flag.
...
FOR EACH ccbcdocu NO-LOCK WHERE
    ...
    ccbcdocu.fchate >= FILL-IN-fchast-1 AND     /* Observado por Andrea Tupac Yupanqui */
    ccbcdocu.fchate <= FILL-IN-fchast-2:
    /* CUENTA AL DEBE */
    RUN Graba-Detalle(
        ccbcdocu.fchdoc,
        x-codope,
