...
DEFINE VARIABLE h_tdevletbco AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vdevletbco AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "DEVOLUCION DE LETRAS EN BANCO"
         HEIGHT             = 13.58
         WIDTH              = 106.72
         MAX-HEIGHT         = 22.65
         MAX-WIDTH          = 112.72
...
ON END-ERROR OF W-Win /* DEVOLUCION DE LETRAS EN BANCO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* DEVOLUCION DE LETRAS EN BANCO */
DO:
...
CASE s-FlgSit:
    WHEN "T" THEN W-Win:TITLE = W-Win:TITLE + "- TRANSITO".
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/vdevletbco.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vdevletbco ).
       RUN set-position IN h_vdevletbco ( 2.73 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_vdevletbco. */
       RUN add-link IN adm-broker-hdl ( h_p-updv14 , 'TableIO':U , h_vdevletbco ).
       RUN add-link IN adm-broker-hdl ( h_qcanjexletra , 'Record':U , h_vdevletbco ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_benvletbco ).
       RUN set-position IN h_benvletbco ( 6.38 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/tdevletbco.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tdevletbco ).
       RUN set-position IN h_tdevletbco ( 6.58 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tdevletbco ( 6.69 , 88.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 7.15 , 91.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 5.38 , 14.00 ) NO-ERROR.

       /* Links to SmartBrowser h_tdevletbco. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tdevletbco ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tdevletbco ,
             h_vdevletbco , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tdevletbco , 'AFTER':U ).
    END. /* Page 2 */
