...
DEF VAR cRucCia AS CHAR NO-UNDO.

FIND gn-cias WHERE gn-cias.codcia = s-codcia NO-LOCK NO-ERROR.
IF AVAILABLE gn-cias THEN cRucCia = GN-CIAS.LIBRE-C[1].
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH w-report WHERE
        ...
        PUT UNFORMATTED
            ...
            SPACE(38) STRING(campo-c[10],'x(85)') " R.U.C. " cRucCia.
        PAGE.
    END.

END PROCEDURE.
/*
            SPACE(95) "R.U.C. " STRING(cRucCia,'99999999999') SKIP
  */
