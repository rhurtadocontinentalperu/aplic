...
/*
&SCOPED-DEFINE CONDICION ccbcdocu.codcia = s-codcia AND ccbcdocu.flgest = 'X'
*/

/*RD01 26.07.10
&SCOPED-DEFINE CONDICION ccbcdocu.codcia = s-codcia AND ccbcdocu.flgest = 'S'
*/
...
CcbCDocu.FchUbi CcbCDocu.ImpTot CcbCDocu.SdoAct CcbCDocu.usuario 
...
&Scoped-Define ENABLED-OBJECTS br_table txt-cliente x-docs btn-Actualizar ~
BUTTON-1 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.NroDoc COLUMN-LABEL "<<Numero>>" FORMAT "XXX-XXXXXX":U
      CcbCDocu.CodDiv COLUMN-LABEL "Division" FORMAT "x(5)":U
      CcbCDocu.CodCli FORMAT "x(11)":U WIDTH 8.57
      ...
      CcbCDocu.usuario FORMAT "x(10)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     br_table AT ROW 1.23 COL 19.57
     txt-cliente AT ROW 1.27 COL 5 COLON-ALIGNED WIDGET-ID 16
     x-docs AT ROW 2.62 COL 7 NO-LABEL WIDGET-ID 10
     btn-Actualizar AT ROW 7.73 COL 2 WIDGET-ID 14
     x-TotalMn AT ROW 14.69 COL 102.57 COLON-ALIGNED WIDGET-ID 2
     BUTTON-1 AT ROW 14.69 COL 116.57 WIDGET-ID 6
     x-TotalMe AT ROW 15.5 COL 102.57 COLON-ALIGNED WIDGET-ID 4
...
"CcbCDocu.NroDoc" "<<Numero>>" "XXX-XXXXXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.CodCli" ? ? "character" ? ? ? ? ? ? no ? no no "8.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.FchDoc" "<Emision>" "99/99/99" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.CcbCDocu.FchUbi
...
"CcbCDocu.SdoAct" "Saldo Actual" "->,>>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   = INTEGRAL.CcbCDocu.usuario
...
------------------------------------------------------------------------------*/
    ...
    IF documto = "Todas" THEN documto = "FAC,BOL,LET,CHQ,N/C,N/D,DCO".
...
------------------------------------------------------------------------------*/
  ...
  MESSAGE "Está seguro de sacar el documento de la situación de castigado?"
  ...
  FIND CcbDMvto WHERE CcbDMvto.CodCia = Ccbcdocu.codcia
      AND CcbDMvto.CodDoc = "DCA"       /* Castigado */
  ...
  IF AVAILABLE CcbDMvto THEN DO:
      IF CcbDMvto.FlgCbd = YES THEN DO:
          MESSAGE 'Este documento ya ha sido transferido a contabilidad' SKIP
              'No se puede anular la situación de castigo'
  ...
  ASSIGN
    ...
    CcbCDocu.FlgEst = 'J'       /* a cobranza dudosa */
