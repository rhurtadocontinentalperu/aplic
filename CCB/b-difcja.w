...
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table CcbTabla.Codigo ~
CcbTabla.Nombre 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbTabla.Codigo COLUMN-LABEL "Código" FORMAT "x(5)":U
      CcbTabla.Nombre COLUMN-LABEL "Descripción" FORMAT "x(40)":U
            WIDTH 54
  ENABLE
      CcbTabla.Codigo
      CcbTabla.Nombre
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 66 BY 14.62
...
"Codigo" "Código" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.CcbTabla.Nombre
"Nombre" "Descripción" ? "character" ? ? ? ? ? ? yes ? no no "54" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
-------------------------------------------------------------*/
  ...
  IF p-state = 'update-begin':U THEN DO:
     ...
     APPLY 'ENTRY':U TO CcbTabla.Nombre IN BROWSE {&BROWSE-NAME}.
  END.
...
IF RETURN-VALUE = 'YES' THEN DO:
    IF CAN-FIND(FIRST CcbTabla WHERE CcbTabla.CodCia = s-codcia
                AND CcbTabla.Tabla = s-tabla
                AND CcbTabla.Codigo = CcbTabla.Codigo:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
                NO-LOCK)
        THEN DO:
        MESSAGE 'Código repetido' VIEW-AS ALERT-BOX ERROR.
        APPLY 'ENTRY':U TO CcbTabla.Codigo.
        RETURN 'ADM-ERROR'.
    END.
END.
RETURN "OK".
