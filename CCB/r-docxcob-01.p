...
&Scoped-Define ENABLED-OBJECTS tg-dudosa FILL-IN-fecha FILL-IN-fecha-1 ~
BUTTON-1 RADIO-SET-codmon BUTTON-print BUTTON-exit BUTTON-print-2 
&Scoped-Define DISPLAYED-OBJECTS x-mensaje tg-dudosa FILL-IN-fecha ~
FILL-IN-fecha-1 FILL-IN-Division RADIO-SET-codmon 
...
DEFINE FRAME F-Main
     x-mensaje AT ROW 9.08 COL 2.86 COLON-ALIGNED NO-LABEL WIDGET-ID 8
     ...
     BUTTON-print AT ROW 6.77 COL 41
     BUTTON-exit AT ROW 6.77 COL 50
     BUTTON-print-2 AT ROW 6.77 COL 31.86 WIDGET-ID 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Reporte Documentos por Cobrar POR DIVISION DE ORIGEN"
         HEIGHT             = 9.85
         WIDTH              = 71.72
         MAX-HEIGHT         = 9.85
         MAX-WIDTH          = 71.72
...
ON END-ERROR OF W-Win /* Reporte Documentos por Cobrar POR DIVISION DE ORIGEN */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Reporte Documentos por Cobrar POR DIVISION DE ORIGEN */
DO:
...
ON LEAVE OF FILL-IN-Division IN FRAME F-Main /* División ORIGEN */
DO:
...
------------------------------------------------------------------------------*/
  DISPLAY x-mensaje tg-dudosa FILL-IN-fecha FILL-IN-fecha-1 FILL-IN-Division 
          RADIO-SET-codmon 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE tg-dudosa FILL-IN-fecha FILL-IN-fecha-1 BUTTON-1 RADIO-SET-codmon 
         BUTTON-print BUTTON-exit BUTTON-print-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
    ...
    cRange = "T" + cColumn.
    chWorkSheet:Range(cRange):Value = "Cond de Venta".
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        IF FILL-IN-fecha = ? THEN FILL-IN-fecha = 01/01/1970.
        IF FILL-IN-fecha-1 = ? THEN FILL-IN-fecha-1 = 12/31/3999.
        CASE RADIO-SET-codmon:
            WHEN 1 THEN s-titulo = "DOCUMENTOS EN NUEVOS SOLES".
            WHEN 2 THEN s-titulo = "DOCUMENTOS EN DÓLARES".
            WHEN 3 THEN s-titulo = "DOCUMENTOS EN AMBAS MONEDAS".
        END CASE.
    END.
    ...
    ASSIGN RB-REPORT-NAME = "Documentos por Cobrar_2".
    ASSIGN
...
FOR EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-codcia:
    FOR EACH integral.CcbCDocu USE-INDEX Llave02 NO-LOCK WHERE
        ...
        LOOKUP(integral.CcbCDocu.divori, cDivi) > 0 AND
        ...
        IF RADIO-SET-codmon <> 3 AND CcbCDocu.codmon <> RADIO-SET-codmon THEN NEXT.
        ...
        ASSIGN
            ...
            integral.w-report.Campo-C[9] = cDircli                        /* Direccion */
        ...
        /* RHC 13/04/2015 Información adicional */
        ASSIGN
            w-report.Campo-C[10] = Ccbcdocu.divori
            w-report.Campo-C[11] = Ccbcdocu.fmapgo
            w-report.Campo-C[12] = Ccbcdocu.codven.
    END.
END.
