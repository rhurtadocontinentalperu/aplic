...
DEFINE VARIABLE h_t-nota-cr-db-co AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-nota-cr-db-co AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'CCB/v-nota-cr-db-co.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-nota-cr-db-co ).
       RUN set-position IN h_v-nota-cr-db-co ( 2.62 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-nota-cr-db-co. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_v-nota-cr-db-co ).
       RUN add-link IN adm-broker-hdl ( h_qnotacreditodebito , 'Record':U , h_v-nota-cr-db-co ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/t-nota-cr-db-co.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-nota-cr-db-co ).
       RUN set-position IN h_t-nota-cr-db-co ( 10.15 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-nota-cr-db-co ( 13.19 , 140.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv95 ).
       RUN set-position IN h_p-updv95 ( 23.31 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv95 ( 1.54 , 26.14 ) NO-ERROR.

       /* Links to SmartBrowser h_t-nota-cr-db-co. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_t-nota-cr-db-co ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-nota-cr-db-co ,
             h_v-nota-cr-db-co , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv95 ,
             h_t-nota-cr-db-co , 'AFTER':U ).
    END. /* Page 2 */
...
IF h_t-nota-cr-db-co <> ? THEN RUN Import-Temp-Table IN h_t-nota-cr-db-co ( OUTPUT TABLE DETA).
...
CASE L-Handle:
    ...
    WHEN "browse" THEN DO:
        IF h_t-nota-cr-db-co <> ? THEN DO:
           RUN Import-Temp-Table IN h_v-nota-cr-db-co (OUTPUT TABLE DETA).
           RUN Export-Temp-Table IN h_t-nota-cr-db-co (INPUT TABLE DETA).
           RUN dispatch IN h_t-nota-cr-db-co ('open-query':U).
        END.
