...
&ENDIF
                FIND FIRST Detalle WHERE Detalle.codcli = Ccbcdocu.codcli EXCLUSIVE-LOCK NO-ERROR.
                ...
                /* ************************************************************************************************************** */
                /* CALCULAMOS A PARTIR DEL DETALLE */
                ...
                FOR EACH Ccbddocu FIELDS (codcia coddiv coddoc nrodoc codmat implin cImporteTotalConImpuesto) OF Ccbcdocu NO-LOCK, 
                    ...
                    IF x-ImpLin = 0 THEN x-ImpLin = Ccbddocu.ImpLin.
...
------------------------------------------------------------------------------*/    
    ...
    CASE Almmmatg.codfam:
        ...
        WHEN "011" THEN DO:
            CASE x-Contador:
                WHEN 1 THEN Detalle.Compras_01_06 = Detalle.Compras_01_06 + x-ImpLin.
                WHEN 2 THEN Detalle.Compras_02_06 = Detalle.Compras_02_06 + x-ImpLin.
                WHEN 3 THEN Detalle.Compras_03_06 = Detalle.Compras_03_06 + x-ImpLin.
                WHEN 4 THEN Detalle.Compras_04_06 = Detalle.Compras_04_06 + x-ImpLin.
                WHEN 5 THEN Detalle.Compras_05_06 = Detalle.Compras_05_06 + x-ImpLin.
            END CASE.
        END.
        WHEN "012" THEN DO:
            CASE x-Contador:
...
DEF VAR iControl AS INTE NO-UNDO.
DEF VAR cAval_1 AS CHAR NO-UNDO.
DEF VAR cAval_2 AS CHAR NO-UNDO.
...
&ENDIF
    ...
    END.
    /* 26/03/2026: Otros datos Gina Cóndor */
    ASSIGN
        Detalle.NMRepr = gn-clie.RepLeg[1]
        Detalle.DRRepr = gn-clie.RepLeg[4]
        Detalle.DNIRepr = gn-clie.RepLeg[2]
        Detalle.FNRepr = gn-clie.FNRepr
        Detalle.DNI = gn-clie.DNI
        .
    cAval_1 = ENTRY(1,gn-clie.aval1[1],'|').
    cAval_2 = ''.
    IF NUM-ENTRIES(gn-clie.aval1[1],'|') >= 2 THEN cAval_2 = ENTRY(2,gn-clie.aval1[1],'|').
    ASSIGN
        Detalle.Aval = cAval_1 + (IF cAval_2 > '' THEN ';' ELSE '') + cAval_2
        .
END.
HIDE FRAME f-Proceso.
...
&ENDIF
    ...
    /* Cargamos el año más antiguo */
    FOR EACH Gn-ClieL 
        FIELDS(fchini fchfin implc)
        NO-LOCK 
        WHERE Gn-ClieL.CodCia = cl-CodCia AND Gn-ClieL.CodCli = pCodCli
        BY gn-cliel.fchini BY gn-cliel.fchfin:
        IF Gn-ClieL.ImpLC > 0 THEN DO:
            Detalle.LinCredIni = YEAR(gn-cliel.fchini).
            LEAVE.
        END.
    END.
