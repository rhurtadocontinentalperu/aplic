...
/* OJO : NO SE TOMARÁ EN CUENTA LOS DOCUMENTOS ANTES DEL 2010, SOLO EN FORMA RESUMIDA */
...
FOR EACH w-report WHERE w-report.Task-No = pTask-No
    ...
    FOR EACH CCBDMOV NO-LOCK USE-INDEX Llave03 WHERE CCBDMOV.CodCia = s-CodCia
        ...
        IF w-report.Campo-F[1] = 0 THEN w-report.Campo-C[5] = "NO".     /* NO se imprime en el reporte */
    END.
END.
...
DO k = 1 TO NUM-ENTRIES(x-Cargos):
    ...
    FOR EACH ccbdcaja NO-LOCK WHERE ccbdcaja.codcia = s-codcia
        ...
        IF LOOKUP(ccbdcaja.coddoc, "A/C") > 0 THEN NEXT.    /* OJO */
