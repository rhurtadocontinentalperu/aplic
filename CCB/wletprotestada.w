...
ASSIGN
    S-FLGUBI = ENTRY(1,pCodDoc).

CASE s-FlgUbi:
    WHEN 'C' THEN S-CODDOC = 'P/X'.
    WHEN 'B' THEN S-CODDOC = 'P/C'.
...
DEFINE VARIABLE h_benvletbco AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv14 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_qcanjexletra AS HANDLE NO-UNDO.
DEFINE VARIABLE h_tletprotestada AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vletprotestada AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* PROTESTO DE LETRAS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* PROTESTO DE LETRAS */
DO:
...
CASE s-FlgUbi:
    WHEN "C" THEN W-Win:TITLE = W-Win:TITLE + "- EN CARTERA".
    WHEN "B" THEN W-Win:TITLE = W-Win:TITLE + "- EN BANCOS".
END CASE.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/vletprotestada.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vletprotestada ).
       RUN set-position IN h_vletprotestada ( 2.73 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.54 , 102.00 ) */
       ...
       /* Size in UIB:  ( 1.35 , 10.00 ) */

       /* Links to SmartViewer h_vletprotestada. */
       RUN add-link IN adm-broker-hdl ( h_p-updv14 , 'TableIO':U , h_vletprotestada ).
       RUN add-link IN adm-broker-hdl ( h_qcanjexletra , 'Record':U , h_vletprotestada ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_p-updv14 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vletprotestada ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_qcanjexletra ,
             h_vletprotestada , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/ccb/benvletbco.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_benvletbco ).
       RUN set-position IN h_benvletbco ( 5.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_benvletbco ( 7.73 , 93.00 ) NO-ERROR.

       /* Links to SmartBrowser h_benvletbco. */
       RUN add-link IN adm-broker-hdl ( h_qcanjexletra , 'Record':U , h_benvletbco ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_benvletbco ,
             h_vletprotestada , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/tletprotestada.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tletprotestada ).
       RUN set-position IN h_tletprotestada ( 5.81 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tletprotestada ( 6.69 , 88.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 6.38 , 91.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 5.38 , 14.00 ) NO-ERROR.

       /* Links to SmartBrowser h_tletprotestada. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tletprotestada ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tletprotestada ,
             h_vletprotestada , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tletprotestada , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:    
        RUN select-page(1).
        IF VALID-HANDLE(h_benvletbco) THEN RUN dispatch IN h_benvletbco ('open-query':U).
