...
IF LOOKUP(ENTRY(1,pParametros,'|'), 'ND,NC,PNC,NCI') = 0 THEN RETURN.
...
CASE s-coddoc:
    WHEN 'ND' THEN s-tabla = 'N/D'.
    WHEN 'NC' OR WHEN 'NCI' OR WHEN 'PNC' THEN s-tabla = 'N/C'.
...
END.
/* RHC 27/06/18 */
DEF VAR pTipo AS CHAR INIT 'CREDITO' NO-UNDO.
IF LOOKUP(s-CodDoc, 'PNC') > 0 THEN pTipo = ''.
...
DEFINE FRAME F-Main
     ...
     "Buscar Nro." VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 1.38 COL 95 WIDGET-ID 6
...
ON END-ERROR OF W-Win /* Generacion de PNC/PND */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Generacion de PNC/PND */
DO:
...
CASE s-CodDoc:
    ...
    WHEN "PNC" THEN {&WINDOW-NAME}:TITLE = "PRE-NOTAS DE CREDITO".
    WHEN "NCI" THEN {&WINDOW-NAME}:TITLE = "NOTAS DE CREDITO INTERNAS".
END CASE.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_vnotacreditodebito ( 2.54 , 1.00 ) NO-ERROR.
       ...
       RUN set-position IN h_qnotacreditodebito ( 1.19 , 106.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.15 , 10.00 ) */

       /* Links to SmartViewer h_vnotacreditodebito. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_vnotacreditodebito ).
       RUN add-link IN adm-broker-hdl ( h_qnotacreditodebito , 'Record':U , h_vnotacreditodebito ).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE refrescar-anticipos W-Win 
...
/* Solo para proposito de compatibilidad */
