...
DEFINE VAR x-tabla AS CHAR INIT "TD-REFINAN".
...
&Scoped-define INTERNAL-TABLES FacTabla FacDocum

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table FacTabla.Codigo FacDocum.NomDoc ~
...
factabla.tabla = x-tabla NO-LOCK, ~
      FIRST FacDocum WHERE facdocum.codcia = s-codcia and ~
facdocum.coddoc = factabla.codigo NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
facdocum.coddoc = factabla.codigo NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table FacTabla FacDocum
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacTabla.Codigo FORMAT "x(8)":U
      FacDocum.NomDoc FORMAT "X(30)":U WIDTH 37.86
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 75 BY 11.54
         FONT 4
         TITLE "DOCUMENTOS VALIDOS PARA LA REFINANCIACION".
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.FacTabla,INTEGRAL.FacDocum WHERE INTEGRAL.FacTabla ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST"
     _Where[1]         = "factabla.codcia = s-codcia and
factabla.tabla = x-tabla"
     _JoinCode[2]      = "facdocum.codcia = s-codcia and
facdocum.coddoc = factabla.codigo"
     _FldNameList[1]   > INTEGRAL.FacTabla.Codigo
"FacTabla.Codigo" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.FacDocum.NomDoc
"FacDocum.NomDoc" ? ? "character" ? ? ? ? ? ? no ? no no "37.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.FacTabla.Campo-D[1]
"FacTabla.Campo-D[1]" "Desde" "99/99/9999" "date" ? ? ? ? ? ? yes ? no no "12.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacTabla.Campo-D[2]
"FacTabla.Campo-D[2]" "Hasta" "99/99/9999" "date" ? ? ? ? ? ? yes ? no no "12.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* DOCUMENTOS VALIDOS PARA LA REFINANCIACION */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* DOCUMENTOS VALIDOS PARA LA REFINANCIACION */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* DOCUMENTOS VALIDOS PARA LA REFINANCIACION */
DO:
...
ON LEAVE OF FacTabla.Codigo IN BROWSE br_table /* Codigo */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FacTabla.Codigo br_table _BROWSE-COLUMN B-table-Win
ON MOUSE-SELECT-DBLCLICK OF FacTabla.Codigo IN BROWSE br_table /* Codigo */
DO:
  ...
  RUN lkup/c-coddoc ('Tipo de documentos').
...
ON 'RETURN':U OF factabla.codigo, factabla.campo-d[1], factabla.campo-d[2] IN BROWSE {&BROWSE-NAME} DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
      ...
      IF RETURN-VALUE = 'YES' THEN DO:
        ...
        DO WITH FRAME {&FRAME-NAME} :
            ASSIGN {&FIRST-TABLE-IN-QUERY-br_table}.codigo:SCREEN-VALUE IN BROWSE {&browse-name} = "XXX".
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
    ...
    logtabla.ValorLlave = x-tabla + "|" + factabla.codigo + "|" + STRING(factabla.campo-d[1],"99/99/9999") + "|" + STRING(factabla.campo-d[2],"99/99/9999") + "|PRIC:" + String(pRCID,"999999999999").
...
DO WITH FRAME {&FRAME-NAME}:
    x-coddoc = factabla.codigo:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.
...
IF RETURN-VALUE = "YES" THEN DO:
    ...
    IF NOT AVAILABLE x-facdocum THEN DO:
        MESSAGE "Codigo de tipo de documento no existe" 
            VIEW-AS ALERT-BOX INFORMATION.
        RETURN "ADM-ERROR".
    END.
    IF x-facdocum.tpodoc = NO THEN DO:
        MESSAGE "El documento debe ser de CARGO mas no de ABONO" 
            VIEW-AS ALERT-BOX INFORMATION.
        RETURN "ADM-ERROR".
    END.
    ...
    FIND FIRST x-factabla WHERE x-factabla.codcia = s-codcia AND
                                ...
                                x-factabla.codigo = x-coddoc NO-LOCK NO-ERROR.
    IF AVAILABLE x-factabla THEN DO:
        MESSAGE "Codigo de Tipo de Documento ya esta registrado" 
            VIEW-AS ALERT-BOX INFORMATION.
        RETURN "ADM-ERROR".
    END.
