...
DEF SHARED VAR s-coddoc AS CHAR INITIAL "E/C".
DEF SHARED VAR s-tipo   AS CHAR INITIAL "REMEBOV".
...
&Scoped-define FIELDS-IN-QUERY-br_table CcbCCaja.CodDiv CcbCCaja.NroDoc ~
CcbCCaja.usuario CcbCCaja.FchDoc ~
(IF CodMon = 1 THEN 'S/.' ELSE (IF CodMon = 2 THEN 'US$' ELSE '?')) @ CcbCCaja.Voucher[1] ~
(ImpNac[1] + ImpUsa[1]) @ Importe 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      (IF CodMon = 1 THEN 'S/.' ELSE (IF CodMon = 2 THEN 'US$' ELSE '?')) @ CcbCCaja.Voucher[1] COLUMN-LABEL "Moneda" FORMAT "x(3)":U
      (ImpNac[1] + ImpUsa[1]) @ Importe COLUMN-LABEL "Importe" FORMAT ">>>,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[3]   > INTEGRAL.CcbCCaja.usuario
"CcbCCaja.usuario" "Usuario" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   = INTEGRAL.CcbCCaja.FchDoc
     _FldNameList[5]   > "_<CALC>"
"(IF CodMon = 1 THEN 'S/.' ELSE (IF CodMon = 2 THEN 'US$' ELSE '?')) @ CcbCCaja.Voucher[1]" "Moneda" "x(3)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"(ImpNac[1] + ImpUsa[1]) @ Importe" "Importe" ">>>,>>>,>>9.99" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
{lib\lock-genericov21.i &Tabla="B-CCAJA" ~
    &Condicion="ROWID(B-CCAJA)=ROWID(Ccbccaja)" ~
    &Bloqueo="EXCLUSIVE-LOCK" ~
    &Accion="RETRY" ~
    &Mensaje="YES" ~
    &TipoError="RETURN" }
    
IF (B-CCAJA.FlgEst = "E" AND B-CCAJA.FlgCie = "P") THEN DO:
    ASSIGN
        B-CCAJA.FlgEst = "C"
/*         B-CCAJA.CodCta[10] = s-user-id */
/*         B-CCAJA.FchVto[10] = TODAY     */
        B-CCAJA.UsrApr = s-user-id
        B-CCAJA.FchApr = TODAY
        B-CCAJA.HorApr = STRING(TIME, 'HH:MM:SS').
END.
ELSE DO:
...
/*         B-CCAJA.CodCta[10] = s-user-id */
/*         B-CCAJA.FchVto[10] = TODAY     */
        B-CCAJA.UsrRec = s-user-id
        B-CCAJA.FchRec = TODAY
        B-CCAJA.HorRec = STRING(TIME, 'HH:MM:SS').
END.
ELSE DO:
    MESSAGE 'El registro ha sido modificado por otro usuario' SKIP
        'Proceso abortado' VIEW-AS ALERT-BOX WARNING.
