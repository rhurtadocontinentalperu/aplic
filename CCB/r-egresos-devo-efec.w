...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "EGRESOS DE CAJA POR DEVOLUCION DE EFECTIVO"
         HEIGHT             = 9.62
         WIDTH              = 62.29
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 80
...
ON END-ERROR OF W-Win /* EGRESOS DE CAJA POR DEVOLUCION DE EFECTIVO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* EGRESOS DE CAJA POR DEVOLUCION DE EFECTIVO */
DO:
...
------------------------------------------------------------------------------*/
  ...
  DO i = 1 TO NUM-ENTRIES(f-Division):
    ...
    FOR EACH Ccbccaja NO-LOCK WHERE Ccbccaja.codcia = s-codcia
        ...
        AND LOOKUP(Ccbccaja.tipo, 'ANTREC,DEVONC,DEVOBD,DEVOAC') > 0
        ...
        AND Ccbcdocu.coddoc = SUBSTRING(Ccbccaja.voucher[1],1,3)
        AND Ccbcdocu.nrodoc = SUBSTRING(Ccbccaja.voucher[1],4):
        CREATE Detalle.
...
ASSIGN
    chWorkSheet:Range("A1"):Value = CAPS(s-nomcia) + " - EGRESOS DE CAJA POR DEVOLUCION DE EFECTIVO" 
