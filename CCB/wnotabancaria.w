...
DEFINE VARIABLE h_tnotabancaria AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vnotabancaria AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     btnFromExcel AT ROW 13.08 COL 92.14 WIDGET-ID 6
     RECT-25 AT ROW 2.54 COL 2 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 106.29 BY 15.31
...
DO:
    ...
    RUN ue-desde-excel IN h_vnotabancaria.
    ...
    lSumaInteres = 0.
    RUN ue-desde-excel IN h_tnotabancaria(OUTPUT lSumaInteres).   

    IF lSumaInteres > 0 THEN DO:
        RUN ue-pinta-ingresos IN h_vnotabancaria(INPUT lSumaInteres).
    END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/vnotabancaria.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vnotabancaria ).
       RUN set-position IN h_vnotabancaria ( 2.73 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_vnotabancaria. */
       RUN add-link IN adm-broker-hdl ( h_p-updv14 , 'TableIO':U , h_vnotabancaria ).
       RUN add-link IN adm-broker-hdl ( h_qcanjexletra , 'Record':U , h_vnotabancaria ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_p-updv14 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vnotabancaria ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_bnotabancaria ,
             h_vnotabancaria , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/tnotabancaria.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tnotabancaria ).
       RUN set-position IN h_tnotabancaria ( 7.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tnotabancaria ( 6.69 , 88.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 7.46 , 91.43 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 5.38 , 14.00 ) NO-ERROR.

       /* Links to SmartBrowser h_tnotabancaria. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tnotabancaria ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tnotabancaria ,
             h_vnotabancaria , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tnotabancaria , 'AFTER':U ).
    END. /* Page 2 */
