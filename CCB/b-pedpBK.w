...
&SCOPED-DEFINE CONDICION ( Faccpedm.CodCia = S-CODCIA AND integral.Faccpedm.CodDoc = "P/M" ~
                           AND integral.Faccpedm.FlgEst = "P" AND integral.Faccpedm.CodDiv = S-CODDIV)
...
&Scoped-Define ENABLED-OBJECTS RECT-1 br_table FILL-IN-filtro CMB-filtro 
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH T-CPEDM NO-LOCK 
      ...
      IF AVAILABLE CcbCDocu THEN  DO:
         IF Ccbcdocu.CodDoc = "FAC" THEN RUN ccb/r-factBK (ROWID(ccbcdocu)).
         IF Ccbcdocu.CodDoc = "BOL" THEN RUN ccb/r-boleBK (ROWID(ccbcdocu)).
         FIND FIRST Ccbddocu OF Ccbcdocu NO-LOCK NO-ERROR.
         IF AVAIL CcbDdocu THEN 
         RUN ccb/r-odeBK (ROWID(ccbcdocu), CcbDDocu.AlmDes).
      END.
  END.
...
------------------------------------------------------------------------------*/
   ...
   FOR EACH T-CPEDM NO-LOCK BY T-CPEDM.NroPed ON ERROR UNDO, RETURN "ADM-ERROR":
       FIND CcbCDocu WHERE CcbCDocu.CodCia = S-CodCia 
                      ...
                      AND  CcbCDocu.CodAlm = T-CPEDM.CodAlm
                     EXCLUSIVE-LOCK NO-ERROR. 
       IF AVAILABLE CcbCDocu THEN  DO:
...
/*               message "crea cabecera" SKIP
 *                        CcbCDocu.CodCia SKIP
 *                        CcbCDocu.CodAlm SKIP
 *                        "S" SKIP
 *                        CcbCDocu.CodMov SKIP
 *                        S-PtoVta SKIP
 *                        Almacen.CorrSal SKIP
 *                        view-as alert-box.*/

/*integral.Almtdocm.CodCia = S-CODCIA
 *  AND integral.Almtdocm.CodAlm = S-CODALM
 *  AND integral.Almtdocm.TipMov = S-TPOMOV
 *  AND integral.Almtdocm.CodMov <> S-MOVCMP 
 *  AND integral.Almtdocm.CodMov <> S-MOVTRF */
          CREATE Almcmov.
