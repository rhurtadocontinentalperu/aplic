...
DEF VAR s-Tipo AS CHAR NO-UNDO.
...
Almtfami.desfam VtaDTabla.Libre_c01 AlmSFami.dessub 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaDTabla.LlaveDetalle COLUMN-LABEL "Línea" FORMAT "x(8)":U
      Almtfami.desfam FORMAT "X(30)":U WIDTH 30.86
      VtaDTabla.Libre_c01 COLUMN-LABEL "Sub-Línea" FORMAT "x(8)":U
            WIDTH 7.72
      AlmSFami.dessub FORMAT "X(30)":U WIDTH 27.43
  ENABLE
      VtaDTabla.LlaveDetalle
      VtaDTabla.Libre_c01
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 81 BY 10.5
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "INTEGRAL.VtaDTabla.LlaveDetalle|yes"
...
"VtaDTabla.LlaveDetalle" "Línea" "x(8)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaDTabla.Libre_c01" "Sub-Línea" ? "character" ? ? ? ? ? ? yes ? no no "7.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.AlmSFami.dessub
"AlmSFami.dessub" ? ? "character" ? ? ? ? ? ? no ? no no "27.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON RETURN OF VtaDTabla.Libre_c01, VtaDTabla.LlaveDetalle DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
IF RETURN-VALUE = 'YES' THEN DO:
    IF CAN-FIND(FIRST VtaDTabla WHERE VtaDTabla.CodCia = Vtactabla.codcia AND
                VtaDTabla.Tabla = Vtactabla.tabla AND
                VtaDTabla.Llave = Vtactabla.llave AND
                VtaDTabla.Tipo   = s-tipo AND
                VtaDTabla.LlaveDetalle = VtaDTabla.LlaveDetalle:SCREEN-VALUE IN BROWSE {&browse-name} AND
                VtaDTabla.Libre_c01 = VtaDTabla.Libre_c01:SCREEN-VALUE IN BROWSE {&browse-name} 
                NO-LOCK)
        THEN DO:
