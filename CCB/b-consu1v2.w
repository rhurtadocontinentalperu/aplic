...
gn-clie.Telfnos[1] gn-clie.FaxCli X-MON @ X-MON fImpLC() @ ImpLC 
...
<SORTBY-OPTIONS>
Codigo|y||integral.gn-clie.CodCli|yes
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fImpLC L-table-Win 
FUNCTION fImpLC RETURNS DECIMAL
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      gn-clie.NomCli COLUMN-LABEL "NOMBRE O RAZON SOCIAL" FORMAT "x(48)":U
      gn-clie.CodCli COLUMN-LABEL "CODIGO" FORMAT "x(11)":U
      gn-clie.Telfnos[1] COLUMN-LABEL "TELEFONO" FORMAT "X(13)":U
      gn-clie.FaxCli COLUMN-LABEL "FAX" FORMAT "x(13)":U
      X-MON @ X-MON COLUMN-LABEL "MON" FORMAT "X(4)":U
      fImpLC() @ ImpLC COLUMN-LABEL "LINEA DE CREDITO" FORMAT ">>>,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 85.43 BY 12.38
...
DEFINE FRAME F-Main
     FILL-IN-codigo AT ROW 1.23 COL 6.43 COLON-ALIGNED
     CMB-filtro AT ROW 1.23 COL 25 NO-LABEL
     FILL-IN-filtro AT ROW 1.23 COL 48 NO-LABEL
...
"gn-clie.NomCli" "NOMBRE O RAZON SOCIAL" "x(48)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.gn-clie.CodCli
"gn-clie.CodCli" "CODIGO" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.gn-clie.Telfnos[1]
"gn-clie.Telfnos[1]" "TELEFONO" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > integral.gn-clie.FaxCli
"gn-clie.FaxCli" "FAX" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"X-MON @ X-MON" "MON" "X(4)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"fImpLC() @ ImpLC" "LINEA DE CREDITO" ">>>,>>>,>>9.99" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    FOR EACH gn-cliel OF gn-clie NO-LOCK BY gn-cliel.fchini BY gn-cliel.fchfin:
        dMonLC = gn-cliel.monlc.
    END.

    IF dMONLC = 1 THEN ASSIGN X-MON = "S/." .
    ELSE ASSIGN X-MON = "US$".
END.
/* ***************************  Main Block  *************************** */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fImpLC L-table-Win 
FUNCTION fImpLC RETURNS DECIMAL
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR dImpLCred AS DEC INIT 0.
  DEF VAR dMonLC AS INT.
