...
&Scoped-define QUERY-STRING-Query-Main FOR EACH CcbCDocu WHERE ~{&KEY-PHRASE} ~
    ...
    BY CcbCDocu.CodCia ~
       BY CcbCDocu.CodDiv ~
        BY CcbCDocu.FchDoc INDEXED-REPOSITION
...
&Scoped-Define ENABLED-OBJECTS RECT-3 BUTTON-1 FILL-IN-1 
...
DEFINE FRAME FRAME-A
     BUTTON-1 AT ROW 1 COL 12
     FILL-IN-1 AT ROW 1.19 COL 2 NO-LABEL
...
/* Query rebuild information for QUERY Query-Main
     ...
     _OrdList          = "INTEGRAL.CcbCDocu.CodCia|yes,INTEGRAL.CcbCDocu.CodDiv|yes,INTEGRAL.CcbCDocu.FchDoc|yes"
     _Where[1]         = "{&Condicion} USE-INDEX LLAVE10"
     _Design-Parent    is WINDOW q-tables @ ( 1 , 19.14 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
DO:
  ...
  FIND Ccbcdocu WHERE {&Condicion} 
    AND nrodoc = STRING(s-NroSer, '999') + SELF:SCREEN-VALUE NO-LOCK NO-ERROR.
  IF AVAILABLE ccbboldep
  THEN output-var-1 = ROWID(ccbboldep).
  ELSE output-var-1 = ?.
  RUN dispatch IN THIS-PROCEDURE ('qbusca':U).
  SELF:SCREEN-VALUE = "".
END.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN lkup/c-boldep-01 ("Boletas de Depositos").
