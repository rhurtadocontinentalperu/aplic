...
FIND FacDocum WHERE FacDocum.CodCia = s-codcia AND FacDocum.CodDoc = "LET" NO-LOCK NO-ERROR.
IF NOT AVAILABLE FacDocum THEN DO:
    MESSAGE 'Documento LETRA NO ESTA CONFIGURADO en el sistema' VIEW-AS ALERT-BOX WARNING.
    RETURN.
END.
lTpoDoc = NOT FacDocum.TpoDoc.
cCodCbd = FacDocum.CodCbd.

FIND FacDocum WHERE FacDocum.CodCia = s-codcia AND FacDocum.CodDoc = "N/B" NO-LOCK NO-ERROR.
IF NOT AVAILABLE FacDocum THEN DO:
    MESSAGE 'Documento NOTA BANCARIA NO ESTA CONFIGURADO en el sistema' VIEW-AS ALERT-BOX WARNING.
    RETURN.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fCodCta W-Win 
FUNCTION fCodCta RETURNS CHARACTER
  ( INPUT pCodMon AS INTE, INPUT pItem AS INTE )  FORWARD.
...
------------------------------------------------------------------------------*/
    ...
    DEF VAR cCodCta AS CHAR NO-UNDO.
    DEF VAR localTpoDoc AS LOG NO-UNDO.
    ...
    FOR EACH CcbCMvto NO-LOCK WHERE CcbCMvto.CodCia = s-codcia 
        ...
        IF FIRST-OF(CcbCMvto.NroDoc) THEN DO:
            ...
            IF CcbCMvto.Libre_dec[2] <> 0 THEN DO:
                localTpoDoc = NO.
                cCodCta = DYNAMIC-FUNCTION('fCodCta':U, Ccbcmvto.codmon, 1).    /* Debe */
                IF TRUE <> (cCodCta > "") THEN DO:
                    localTpoDoc = YES.
                    cCodCta = DYNAMIC-FUNCTION('fCodCta':U, Ccbcmvto.codmon, 2).    /* Haber */
                END.
                IF TRUE <> (cCodCta > "") THEN NEXT.
                RUN Graba-Letra(
                    Ccbcmvto.fchcbd,
                    cCodCta,
                    ccbcmvto.FchDoc,
            ...
            IF CcbCMvto.Libre_dec[1] <> 0 THEN DO:
                localTpoDoc = NO.
                cCodCta = DYNAMIC-FUNCTION('fCodCta':U, Ccbcmvto.codmon, 3).    /* Debe */
                IF TRUE <> (cCodCta > "") THEN DO:
                    localTpoDoc = YES.
                    cCodCta = DYNAMIC-FUNCTION('fCodCta':U, Ccbcmvto.codmon, 4).    /* Haber */
                END.
                IF TRUE <> (cCodCta > "") THEN NEXT.
                RUN Graba-Letra(
                    Ccbcmvto.fchcbd,
        ...
        /* Letras */
        localTpoDoc = NO.
        cCodCta = DYNAMIC-FUNCTION('fCodCta':U, Ccbcmvto.codmon, 5).    /* Debe */
        IF TRUE <> (cCodCta > "") THEN DO:
            localTpoDoc = YES.
            cCodCta = DYNAMIC-FUNCTION('fCodCta':U, Ccbcmvto.codmon, 6).    /* Haber */
        END.
        IF TRUE <> (cCodCta > "") THEN NEXT.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fCodCta W-Win 
FUNCTION fCodCta RETURNS CHARACTER
  ( INPUT pCodMon AS INTE, INPUT pItem AS INTE ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  ...
  CASE pCodMon:
      WHEN 1 THEN DO:
          IF Facdocum.codcta[pItem] > "" THEN RETURN ENTRY(1, Facdocum.codcta[pItem], ":").
      END.
      WHEN 2 THEN DO:
          IF Facdocum.codcta[pItem] > "" AND NUM-ENTRIES(Facdocum.codcta[pItem],":") > 1 THEN RETURN ENTRY(2, Facdocum.codcta[pItem], ":").
      END.
  END CASE.
