...
DEF VAR s-coddoc AS CHAR INIT 'E/C'.
...
&Scoped-define FIELDS-IN-QUERY-br_table CcbCCaja.FchDoc CcbCCaja.FchCie CcbCCaja.Tipo CcbCCaja.NroDoc _FlgEst() @ x-Estado CcbCCaja.CodCli CcbCCaja.NomCli CcbCCaja.CodCaja CcbCCaja.usuario   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define SELF-NAME br_table
&Scoped-define QUERY-STRING-br_table FOR EACH CcbCCaja USE-INDEX Llave07 WHERE ~{&KEY-PHRASE}       AND CcbCCaja.CodCia = s-codcia  AND CcbCCaja.CodDiv = x-coddiv  AND CcbCCaja.CodDoc = s-coddoc  AND CcbCCaja.FchDoc >= x-FchDoc-1  AND CcbCCaja.FchDoc <= x-FchDoc-2 NO-LOCK     ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH CcbCCaja USE-INDEX Llave07 WHERE ~{&KEY-PHRASE}       AND CcbCCaja.CodCia = s-codcia  AND CcbCCaja.CodDiv = x-coddiv  AND CcbCCaja.CodDoc = s-coddoc  AND CcbCCaja.FchDoc >= x-FchDoc-1  AND CcbCCaja.FchDoc <= x-FchDoc-2 NO-LOCK     ~{&SORTBY-PHRASE}.
...
&Scoped-Define ENABLED-OBJECTS x-Divisiones x-FchDoc-1 x-FchDoc-2 ~
CMB-filtro FILL-IN-filtro br_table 
&Scoped-Define DISPLAYED-OBJECTS x-Divisiones x-FchDoc-1 x-FchDoc-2 ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _FREEFORM
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCCaja.usuario COLUMN-LABEL "Cajero" FORMAT "x(10)":U WIDTH 10.29
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
chWorkSheet:Range("B2"):VALUE = "Cierre".
chWorkSheet:Range("C2"):VALUE = "Tipo".
chWorkSheet:Range("D2"):VALUE = "Numero".
chWorkSheet:Range("E2"):VALUE = "Estado".
chWorkSheet:Range("F2"):VALUE = "Cliente".
chWorkSheet:Range("G2"):VALUE = "Nombre".
chWorkSheet:Range("H2"):VALUE = "Terminal".
chWorkSheet:Range("I2"):VALUE = "Cajero".
