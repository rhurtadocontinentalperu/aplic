...
&Scoped-Define DISPLAYED-OBJECTS F-ImpBrt F-ImpExo F-ValVta F-ImpIgv ~
F-ImpTot F-ImpDto F-ImpIsc 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      DETA.codmat COLUMN-LABEL "Codigo" FORMAT "X(8)":U WIDTH 7.43
      Almmmatg.DesMat FORMAT "X(60)":U WIDTH 73.43
      DETA.CanDes FORMAT ">,>>>,>>9.9999":U WIDTH 10.43
      DETA.ImpLin FORMAT "->>,>>>,>>9.99":U WIDTH 8.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     ...
     F-ImpBrt AT ROW 7.92 COL 3.14 WIDGET-ID 34
     F-ImpExo AT ROW 7.92 COL 31 COLON-ALIGNED WIDGET-ID 38
     F-ValVta AT ROW 7.92 COL 52 COLON-ALIGNED WIDGET-ID 48
     F-ImpIgv AT ROW 7.92 COL 72 COLON-ALIGNED WIDGET-ID 40
     F-ImpTot AT ROW 7.92 COL 93 COLON-ALIGNED WIDGET-ID 44
     F-ImpDto AT ROW 8.88 COL 31 COLON-ALIGNED WIDGET-ID 36
     F-ImpIsc AT ROW 8.88 COL 72 COLON-ALIGNED WIDGET-ID 42
     RECT-11 AT ROW 7.69 COL 1 WIDGET-ID 50
...
"DETA.codmat" "Codigo" "X(8)" "character" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almmmatg.DesMat
"Almmmatg.DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ? no no "73.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.DETA.CanDes
"DETA.CanDes" ? ? "decimal" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.DETA.ImpLin
"DETA.ImpLin" ? ? "decimal" ? ? ? ? ? ? no ? no no "8.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
FOR EACH B-DETA:
    ...
    F-ImpExo = F-ImpExo + (IF B-DETA.Aftigv = No  THEN (B-DETA.PreUni * B-DETA.Candes) ELSE 0).
    IF B-DETA.Aftigv = Yes THEN 
       ASSIGN
          F-ImpBrt = F-ImpBrt + (B-DETA.PreUni * B-DETA.Candes)
          F-ValVta = F-ValVta + ((B-DETA.PreUni * B-DETA.Candes) - (B-DETA.ImpIgv + B-DETA.ImpDto)).
END.
FIND FIRST FacCfgGn NO-LOCK NO-ERROR.
F-ImpIgv = F-ImpTot - (F-ImpTot / (1 + (s-PorIgv / 100))).
F-ImpBrt = F-ImpTot - (F-ImpIgv + F-ImpDto). 
F-ValVta = F-ImpBrt - F-ImpDto.
