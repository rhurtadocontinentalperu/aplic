...
DEFINE FRAME F-Main
     ...
     CcbCDocu.ImpTot AT ROW 8.69 COL 94 COLON-ALIGNED WIDGET-ID 76
          ...
          SIZE 17.43 BY .81
          BGCOLOR 15 FGCOLOR 0 
     "ANULACION DE PRE-NOTAS DE CREDITO" VIEW-AS TEXT
          SIZE 35.43 BY .92 AT ROW 1 COL 56.57 WIDGET-ID 126
          BGCOLOR 15 FGCOLOR 12 FONT 6
     "..." VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 6.96 COL 105 WIDGET-ID 124
     "Moneda:" VIEW-AS TEXT
...
MESSAGE "Accion no permitida" VIEW-AS ALERT-BOX INFORMATION.                   
...
/*   IF pStatus = YES THEN DO:     /* Ya iniciaron las actividades */        */
  ...
  */
  ...
  IF Ccbcdocu.flgest <> 'P' AND Ccbcdocu.flgest <> 'E'  THEN DO:
      MESSAGE 'Imposible Anular, segun el estado del documento' VIEW-AS ALERT-BOX ERROR.
      RETURN 'ADM-ERROR'.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      IF (Ccbcdocu.coddoc = "PNC" AND Ccbcdocu.FlgEst = "E") THEN FILL-IN-Estado = "POR APROBAR".
      IF (Ccbcdocu.coddoc = "PNC" AND Ccbcdocu.FlgEst = "P") THEN FILL-IN-Estado = "APROBADO".
