...
DEFINE TEMP-TABLE CAJAS FIELD CAJERA AS CHAR FORMAT "X(10)".
...
&Scoped-Define ENABLED-OBJECTS RECT-49 RECT-5 RECT-48 FILL-IN-CodDiv ~
f-desde R-S-TipImp RB-NUMBER-COPIES RB-BEGIN-PAGE RADIO-SET-1 B-impresoras ~
RB-END-PAGE B-imprime B-cancela B-imprime-2 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-CodDiv f-desde R-S-TipImp ~
RB-NUMBER-COPIES RB-BEGIN-PAGE RADIO-SET-1 RB-END-PAGE 
...
DEFINE FRAME D-Dialog
     ...
     f-desde AT ROW 2.12 COL 31.29 COLON-ALIGNED
     R-S-TipImp AT ROW 2.38 COL 3.29 NO-LABEL
     RB-NUMBER-COPIES AT ROW 5.81 COL 24.43 COLON-ALIGNED
     RB-BEGIN-PAGE AT ROW 5.81 COL 44.29 COLON-ALIGNED
     RADIO-SET-1 AT ROW 5.92 COL 4 NO-LABEL
     B-impresoras AT ROW 6.85 COL 17.57
     RB-END-PAGE AT ROW 6.96 COL 44 COLON-ALIGNED
     b-archivo AT ROW 8.08 COL 16.86
     RB-OUTPUT-FILE AT ROW 8.31 COL 22.14 COLON-ALIGNED NO-LABEL
     B-imprime AT ROW 10.04 COL 3
     B-cancela AT ROW 10.04 COL 17
     B-imprime-2 AT ROW 10.04 COL 31 WIDGET-ID 2
     " Configuración de Impresión" VIEW-AS TEXT
          SIZE 55.29 BY .62 AT ROW 4.15 COL 2.86
          BGCOLOR 1 FGCOLOR 15 FONT 6
     RECT-49 AT ROW 1.58 COL 21.14
     RECT-5 AT ROW 5.35 COL 1.43
     RECT-48 AT ROW 1.04 COL 1.57
     SPACE(0.99) SKIP(6.92)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 4
         TITLE "Reporte de Cheques x Caja".
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Reporte de Cheques x Caja */
DO:  
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  DO c-Copias = 1 to P-copias ON ERROR UNDO, LEAVE
        ...
        ASSIGN s-CODDIV=FILL-IN-Coddiv.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH CcbcDocu NO-LOCK WHERE 
           ...
           FIND Cajas WHERE Cajas.Cajera = CcbcDocu.Usuario no-lock no-error.
            IF NOT AVAILABLE Cajas THEN DO:
              CREATE cajas.
              Cajas.cajera = CcbcDocu.Usuario.
           END.
...
------------------------------------------------------------------------------*/
 ...
 FOR EACH CcbCcaja WHERE 
         ...
         (CcbcCaja.ImpNac[2] <> 0 OR CcbcCaja.ImpNac[3] <> 0 OR
          ...
          CcbcCaja.ImpUsa[5] <> 0 OR CcbcCaja.ImpNac[5] <> 0 )
...
------------------------------------------------------------------------------*/
 ...
 DEFINE VAR X-SOL    AS DECIMAL INIT 0.
 DEFINE VAR X-DOL    AS DECIMAL INIT 0.
 DEFINE VAR x-Voucher LIKE CcbcCaja.voucher[2].
 DEFINE VAR x-FchVto  LIKE CcbcCaja.fchvto[2]. 
 DEFINE VAR x-CodBco LIKE Ccbccaja.CodBco[2].
 ...
 DEFINE FRAME f-cab3
        ...
        HEADER
        {&PRN2} + {&PRN7A} + {&PRN6A} + S-NOMCIA + {&PRN6B} + {&PRN7B} + {&PRN3} FORMAT "X(45)" SKIP
        {&PRN2} + {&PRN6A} + "( " + FILL-IN-CodDiv + ")" + {&PRN6B} + {&PRN3} AT 1 FORMAT "X(15)"
 ...
 FOR EACH CcbCcaja WHERE 
          CcbcCaja.CodCia  = S-CODCIA AND 
          CcbcCaja.CodDiv  = FILL-IN-CodDiv AND 
          CcbcCaja.CodDoc  = "I/C"    AND
          CcbcCaja.FchDoc = F-desde  AND 
          CCbcCaja.Flgest  <> 'A' AND
         (CcbcCaja.ImpNac[2] <> 0 OR CcbcCaja.ImpNac[3] <> 0 OR
        ...
        BY TRIM(Ccbccaja.Voucher[2]) + TRIM(Ccbccaja.Voucher[3]):
/*          BREAK BY CcbcCaja.CodCia 
 *                 BY CcbCcaja.Tipo  
 *                 BY CcbcCaja.NroDoc:*/
    ...
    {&NEW-PAGE}.       
        ...
        IF CcbCCaja.ImpUsa[2] <> 0 OR CcbCCaja.ImpUsa[3] <> 0 THEN DO:
          ...
          x-dol     = x-dol + x-impo.
        END.   
        x-Voucher = TRIM(Ccbccaja.Voucher[2]) + TRIM(Ccbccaja.Voucher[3]).
        x-CodBco = TRIM(Ccbccaja.CodBco[2]) + TRIM(Ccbccaja.CodBco[3]).
        IF Ccbccaja.FchVto[2] = ?
        THEN x-FchVto = Ccbccaja.FchVto[3].
        ELSE x-FchVto = Ccbccaja.FchVto[2].
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-CodDiv f-desde R-S-TipImp RB-NUMBER-COPIES RB-BEGIN-PAGE 
          RADIO-SET-1 RB-END-PAGE 
      WITH FRAME D-Dialog.
  ENABLE RECT-49 RECT-5 RECT-48 FILL-IN-CodDiv f-desde R-S-TipImp 
         RB-NUMBER-COPIES RB-BEGIN-PAGE RADIO-SET-1 B-impresoras RB-END-PAGE 
         B-imprime B-cancela B-imprime-2 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
ASSIGN
    chWorkSheet:Range("A1"):Value = CAPS(s-nomcia) + " - RELACION DE CHEQUES RECEPCIONADOS" 
    chWorkSheet:Range("A2"):Value = "DIVISION: " + FILL-IN-CodDiv
    chWorkSheet:Range("A3"):Value = "Tipo"
    chWorkSheet:Range("B3"):Value = "Fecha"
    chWorkSheet:Range("C3"):Value = "Código"
    chWorkSheet:Range("D3"):Value = "Nombre"
    chWorkSheet:Range("E3"):Value = "Número Cheque"
    chWorkSheet:Range("F3"):Value = "Banco"
    chWorkSheet:Range("G3"):Value = "Cuenta"
    chWorkSheet:Range("H3"):Value = "Fecha Vcto."
...
FOR EACH CcbCcaja WHERE 
    ...
    ASSIGN
        t-Column = t-Column + 1
        chWorkSheet:Cells(t-Row, t-Column):VALUE = x-Voucher.
    ASSIGN
        t-Column = t-Column + 1
        chWorkSheet:Cells(t-Row, t-Column):VALUE = x-CodBco.
    ASSIGN
        t-Column = t-Column + 1
        chWorkSheet:Cells(t-Row, t-Column):VALUE = CcbcCaja.CodCta[2].
    ASSIGN
        t-Column = t-Column + 1
        chWorkSheet:Cells(t-Row, t-Column):VALUE = x-FchVto.
    ASSIGN
        t-Column = t-Column + 1
        chWorkSheet:Cells(t-Row, t-Column):VALUE = x-mon.
    ASSIGN
        t-Column = t-Column + 1
        chWorkSheet:Cells(t-Row, t-Column):VALUE = x-impo.
END.
  ...
  DEFINE FRAME f-cab
    ...
    FOR EACH NOTCRE WHERE NOTCRE.Flgest <> 'A'
        ...
        IF FIRST-OF(NOTCRE.Fmapgo + NOTCRE.Coddoc) THEN DO:
             ASSIGN X-SOL = 0 X-DOL = 0.
             x-titu1 = " ".
             FIND facdocum WHERE facdocum.codcia  = NOTCRE.codcia 
               and facdocum.coddoc = NOTCRE.coddoc NO-LOCK NO-ERROR.
             IF AVAILABLE facdocum THEN do:
        ...
        IF NOTCRE.Coddoc = 'N/C' THEN DO:
          ...
          FIND FIRST CCBDDOCU USE-INDEX LLAVE01 WHERE CCBDDOCU.CODCIA = NOTCRE.Codcia AND 
                             CCBDDOCU.CODDOC = NOTCRE.Coddoc AND
                             CCBDDOCU.NRODOC = NOTCRE.Nrodoc NO-LOCK NO-ERROR .
           IF AVAILABLE CCBDDOCU THEN DO:
            FIND CCBTABLA WHERE CCBTABLA.CODCIA = CCBDDOCU.Codcia AND
                                CCBTABLA.TABLA  = 'N/C' AND
                                CCBTABLA.CODIGO = CCBDDOCU.CODMAT NO-LOCK NO-ERROR.
            IF AVAILABLE CCBTABLA THEN x-glo = CCBTABLA.Nombre.                  
           END.
          END. 
          FIND FIRST CCBCDOCU USE-INDEX LLAVE01 WHERE CCBCDOCU.CODCIA = NOTCRE.Codcia AND 
                              CCBCDOCU.CODDOC = NOTCRE.Coddoc AND
                              CCBCDOCU.NRODOC = NOTCRE.Nrodoc NO-LOCK NO-ERROR .
...
------------------------------------------------------------------------------*/
 ...
 IF  R-S-TipImp = 2 THEN DO:
  ...
  PUT STREAM REPORT   
  ' Cajera(o)                     '   AT 10 'Administrador                  ' AT 90  SKIP(3). 
  PUT STREAM REPORT
  'CAJERAS PARTICIPANTES ' .
