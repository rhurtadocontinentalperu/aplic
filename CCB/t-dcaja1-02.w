...
DEF SHARED VAR s-FlgSit AS CHAR.    /* YES: Solo N/C de la división  NO: de todas las divisiones */
...
------------------------------------------------------------------------------*/
    ...
    IF s-FlgSit = "NO" THEN dFchVto = ADD-INTERVAL(dFChVto, 1, 'year').
    /* ******************************************************************* */
    IF s-FlgSit = "YES" AND dFchVto < TODAY THEN DO:
        MESSAGE 'N/C venció el' Ccbcdocu.fchvto VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY":U TO wrk_dcaja.nroref.
        RETURN "ADM-ERROR".
    END.
    /* RHC 09/08/2016 UTILEX */
    /* 10Ago2016 - Ic, OR gn-divi.coddiv = ccbcdocu.coddiv, validar ambos */
    FIND gn-divi WHERE gn-divi.codcia = s-codcia
        AND (gn-divi.coddiv = ccbcdocu.divori OR gn-divi.coddiv = ccbcdocu.coddiv )
        NO-LOCK NO-ERROR.
    IF AVAILABLE gn-divi AND GN-DIVI.CanalVenta = "MIN" AND Ccbcdocu.Tipo = "MOSTRADOR"
        THEN DO:
        MESSAGE 'La N/C proviene de una venta mostrador' VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY":U TO wrk_dcaja.nroref.
        RETURN "ADM-ERROR".
    END.
    IF s-FlgSit = "YES" THEN DO:
        IF NOT (CcbCDocu.DivOri = s-coddiv OR CcbCDocu.coddiv = s-coddiv) THEN DO:
            MESSAGE 'La N/C pertenece a la división:' Ccbcdocu.DivOri VIEW-AS ALERT-BOX ERROR.
            APPLY "ENTRY":U TO wrk_dcaja.nroref.
            RETURN "ADM-ERROR".
        END.
    END.
