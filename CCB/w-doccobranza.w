...
/* Sintaxis : x{,y}
    x: Documento
        DCO: Documento de Cobranza
    y: Tipo (opcional)
        VALES: por Vales Continental
*/
...
IF s-CodDoc = "" THEN DO:
    MESSAGE 'NO ha definido ek tipo de documento como primer parámetro'
        VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
IF NUM-ENTRIES(pParametro) > 1 THEN s-TpoFac = ENTRY(2, pParametro).
...
DEFINE VARIABLE h_v-doccobranza AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'CCB/v-doccobranza.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-doccobranza ).
       RUN set-position IN h_v-doccobranza ( 2.88 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-doccobranza. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_v-doccobranza ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-doccobranza ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-doccobranza ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-docmto ,
             h_v-doccobranza , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      {&WINDOW-NAME}:TITLE = FacDocum.NomDoc.
  END.
