...
DEFINE VARIABLE s-Tipo AS CHAR INIT '@N/B' NO-UNDO.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH CcbCMvto NO-LOCK WHERE CcbCMvto.CodCia = s-codcia 
        ...
        AND CcbCMvto.CodDiv = FILL-IN-division 
        AND CcbCMvto.FchCbd = FILL-IN-fchast,
...
FOR EACH t-prev BREAK BY t-prev.nroref:
    IF FIRST(t-prev.nroref) THEN DO:
    ...
    END.
    IF LAST(t-prev.nroref) THEN DO:
        FIND FIRST cb-cmov WHERE
            cb-cmov.codcia  = s-codcia AND
            cb-cmov.PERIODO = FILL-IN-periodo AND
            cb-cmov.NROMES  = X-Mes AND
