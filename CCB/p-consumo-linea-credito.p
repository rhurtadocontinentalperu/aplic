...
DEF VAR cEstadoPedidos AS CHAR NO-UNDO.
DEF VAR cDocmtosPendConsumeLC AS CHAR NO-UNDO.
/*
cEstadoPedidos = "G,X,P,W,WX,WL".
cDocmtosPendConsumeLC = 'FAC,BOL,N/C,N/D,BD,A/C,LPA,A/R,LET,CHQ,DCO,FAI,NCI,PAG'.
*/
FIND FIRST factabla WHERE factabla.codcia = 1 AND 
            factabla.tabla = 'AFECTA_LINEACREDITO' AND
            factabla.codigo = 'DOCUMENTOS' NO-LOCK NO-ERROR.
IF AVAILABLE factabla THEN cDocmtosPendConsumeLC = TRIM(factabla.campo-c[1]).

FIND FIRST factabla WHERE factabla.codcia = 1 AND 
            factabla.tabla = 'AFECTA_LINEACREDITO' AND 
            factabla.codigo = "FLGEST" NO-LOCK NO-ERROR.
IF AVAILABLE factabla THEN cEstadoPedidos = TRIM(factabla.campo-c[1]).
...
DO iCuenta = 1 TO NUM-ENTRIES(pRelacionados):
    ...
    FOR EACH CcbCDocu NO-LOCK USE-INDEX Llave06 WHERE CcbCDocu.CodCia = S-CODCIA 
        ...
        /* ******************************************************* */
        IF LOOKUP(CcbCDocu.CodDoc, cDocmtosPendConsumeLC) = 0 THEN NEXT. 
    ...
    DO k = 1 TO NUM-ENTRIES(cEstadoPedidos):
        FOR EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-codcia,
            ...
            AND FacCPedi.FlgEst = ENTRY(k,cEstadoPedidos)
