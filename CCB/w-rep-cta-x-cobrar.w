...
&Scoped-Define ENABLED-OBJECTS FILL-IN_FchDoc BUTTON-1 BtnDone 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN_FchDoc FILL-IN_Mensaje 
...
DEFINE VARIABLE FILL-IN_FchDoc AS DATE FORMAT "99/99/9999":U 
     VIEW-AS FILL-IN 
     SIZE 20 BY 1.35
...
DEFINE FRAME F-Main
     FILL-IN_FchDoc AT ROW 3.15 COL 39 COLON-ALIGNED NO-LABEL WIDGET-ID 8
     BUTTON-1 AT ROW 6.92 COL 4 WIDGET-ID 2
     BtnDone AT ROW 6.92 COL 42 WIDGET-ID 4
     FILL-IN_Mensaje AT ROW 10.15 COL 3 COLON-ALIGNED NO-LABEL WIDGET-ID 14
     "FECHA DE CORTE:" VIEW-AS TEXT
          SIZE 31 BY 2.15 AT ROW 2.88 COL 9 WIDGET-ID 12
          FONT 8
...
DO:
    ...
    IF FILL-IN_FchDoc = ? THEN RETURN NO-APPLY.
    ...
    FILL-IN_Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "EXPORTANDO A EXCEL".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Ayer W-Win 
PROCEDURE Carga-Ayer :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAMETER cListaDocAbono AS CHAR NO-UNDO.

DEF VAR iItem AS INTE NO-UNDO.
DEF VAR x-TpoCmbCmp AS DECI NO-UNDO.
DEF VAR x-TpoCmbVta AS DECI NO-UNDO.
DEF VAR dSdoAct AS DECI NO-UNDO.
...
DO iItem = 1 TO NUM-ENTRIES(cListaDocAbono):
    FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia AND
            Ccbcdocu.fchdoc <= FILL-IN_FchDoc AND
        ...
        IF Ccbcdocu.flgest = "C" AND ccbcdocu.fchcan <= FILL-IN_FchDoc THEN NEXT.
        ...
        IF Ccbcdocu.coddoc = 'LET' THEN DO:
            FIND Ccbcmvto WHERE Ccbcmvto.codcia = Ccbcdocu.codcia
                ...
                NO-LOCK NO-ERROR.
            IF NOT (AVAILABLE Ccbcmvto AND Ccbcmvto.flgest = 'E' AND Ccbcmvto.FchApr <= FILL-IN_FchDoc) THEN NEXT.
        END.
        /* ************************************************************** */

        FILL-IN_Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} =
            Ccbcdocu.coddoc + " " + Ccbcdocu.nrodoc + " " + STRING(Ccbcdocu.fchdoc, '99/99/9999').
        ...
        FOR EACH ccbdmov NO-LOCK WHERE
            ...
            ccbdmov.FchMov <= FILL-IN_FchDoc:
            dSdoAct = dSdoAct - ccbdmov.ImpTot.
        END.
/*         IF LOOKUP(ccbcdocu.coddoc,"BD,A/R,N/C,LPA") > 0 THEN DO: */
/*             IF CAN-FIND(FIRST ccbdmov WHERE                      */
/*                 ccbdmov.codcia = ccbcdocu.codcia AND             */
/*                 ccbdmov.coddoc = ccbcdocu.coddoc AND             */
/*                 ccbdmov.nrodoc = ccbcdocu.nrodoc) THEN DO:       */
...
/*             ccbdcaja.fchdoc <= FILL-IN_FchDoc:                   */
/*             dSdoAct = dSdoAct - ccbdcaja.imptot.                 */
/*             lOk = TRUE.                                          */
/*         END.                                                     */
        /*IF NOT lOk AND ccbcdocu.coddoc = "N/C" THEN dSdoAct = ccbcdocu.sdoact.*/
        ...
        CASE Ccbcdocu.codmon:
            WHEN 1 THEN ASSIGN Detalle.ImpMn = Ccbcdocu.imptot Detalle.SdoMn = dSdoAct.
            WHEN 2 THEN ASSIGN Detalle.ImpMe = Ccbcdocu.imptot Detalle.SdoMe = dSdoAct.
        END CASE.
        ASSIGN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Hoy W-Win 
PROCEDURE Carga-Hoy :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
ASSIGN
    cListaDocCargo = "FAC,BOL,N/D,LET,DCO"
...
/* 2 casos:
    a. A la fecha de hoy
    b. A una fecha anterior 
*/
EMPTY TEMP-TABLE Detalle.
CASE TRUE:
    WHEN FILL-IN_FchDoc >= TODAY THEN DO:
        {ccb/i-rep-cta-x-cobrar.i &FlgEst="P"}
        {ccb/i-rep-cta-x-cobrar.i &FlgEst="J"}
        RUN Carga-Hoy (INPUT cListaDocAbono).
    END.
    OTHERWISE DO:
        {ccb/i-rep-cta-x-cobrar-ayer.i}
        RUN Carga-Ayer (INPUT cListaDocAbono).
    END.
END CASE.
...
FOR EACH Detalle EXCLUSIVE-LOCK:
    ...
    Detalle.FchRep = (IF FILL-IN_FchDoc >= TODAY THEN TODAY ELSE FILL-IN_FchDoc).
    /* Ubigeo */
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN_FchDoc FILL-IN_Mensaje 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE FILL-IN_FchDoc BUTTON-1 BtnDone 
      WITH FRAME F-Main IN WINDOW W-Win.
