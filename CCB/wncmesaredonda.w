...
DEFINE VARIABLE h_qncmesaredonda AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vncmesaredonda AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-2 AT ROW 1 COL 89 WIDGET-ID 10
...
DO:
  RUN dispatch IN h_vncmesaredonda ('imprime':U).
END.
...
DO:
    ...
    RUN dispatch IN h_qncmesaredonda ('open-query':U).
END.
...
CASE s-coddoc:
    WHEN "N/C" THEN  {&WINDOW-NAME}:TITLE = "NOTAS DE CREDITO MESA REDONDA".
    WHEN "N/D" THEN  {&WINDOW-NAME}:TITLE = "NOTAS DE DEBITO MESA REDONDA".
END CASE.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'CCB/vncmesaredonda.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vncmesaredonda ).
       RUN set-position IN h_vncmesaredonda ( 2.73 , 3.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'CCB/qncmesaredonda.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_qncmesaredonda ).
       RUN set-position IN h_qncmesaredonda ( 1.19 , 77.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.00 , 10.00 ) */

       /* Links to SmartViewer h_vncmesaredonda. */
       RUN add-link IN adm-broker-hdl ( h_p-updv98 , 'TableIO':U , h_vncmesaredonda ).
       RUN add-link IN adm-broker-hdl ( h_qncmesaredonda , 'Record':U , h_vncmesaredonda ).

       /* Links to SmartBrowser h_bncmesaredonda. */
       RUN add-link IN adm-broker-hdl ( h_qncmesaredonda , 'Record':U , h_bncmesaredonda ).

       /* Links to SmartViewer h_vncmesaredonda-02. */
       RUN add-link IN adm-broker-hdl ( h_bncmesaredonda , 'Record':U , h_vncmesaredonda-02 ).

       /* Links to SmartQuery h_qncmesaredonda. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_qncmesaredonda ).
