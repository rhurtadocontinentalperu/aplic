...
DEFINE VAR x-tabla AS CHAR INIT "TEXTOS-VARIOS".
...
&Scoped-define FIELDS-IN-QUERY-br_table FacTabla.Codigo FacTabla.Campo-C[1] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacTabla.Campo-C[1] COLUMN-LABEL "Texto" FORMAT "x(100)":U
            WIDTH 53.57
  ENABLE
      FacTabla.Codigo
      FacTabla.Campo-C[1]
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 67.72 BY 8.96
         FONT 4
         TITLE "CONFIGURADOR DE TEXTOS".
...
"FacTabla.Campo-C[1]" "Texto" "x(100)" "character" ? ? ? ? ? ? yes ? no no "53.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* CONFIGURADOR DE TEXTOS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* CONFIGURADOR DE TEXTOS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* CONFIGURADOR DE TEXTOS */
DO:
...
ON LEAVE OF FacTabla.Campo-C[1] IN BROWSE br_table /* Texto */
DO:
  SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} = CAPS(SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
END.
...
ON 'RETURN':U OF factabla.codigo, factabla.campo-c[1] IN BROWSE {&BROWSE-NAME} DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
    ...
    logtabla.ValorLlave = x-tabla + '|' + factabla.codigo + "|" + factabla.campo-c[1]
...
------------------------------------------------------------------------------*/
  ...
  x-codigo = factabla.codigo.
  x-texto = factabla.campo-c[1].
...
/* Code placed here will execute AFTER standard behavior.    */
  ...
  ASSIGN
    ...
    logtabla.Tabla = 'FACTABLA'
    logtabla.Usuario = USERID("DICTDB")
    logtabla.ValorLlave = x-tabla + '|' + x-codigo + "|" + x-texto
    logtabla.NumId = pRCID.
