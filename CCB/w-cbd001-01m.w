...
DEF VAR s-Tipo AS CHAR INIT '@BDM' NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS x-Periodo x-NroMes f-Division BUTTON-1 ~
BUTTON-3 BROWSE-1 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 W-Win _STRUCTURED
  QUERY BROWSE-1 DISPLAY
      ...
      T-DMOV.ImpMn1 FORMAT "-ZZ,ZZZ,ZZ9.99":U
      T-DMOV.ImpMn2 FORMAT "-ZZ,ZZZ,ZZ9.99":U
...
DEFINE FRAME F-Main
     ...
     BUTTON-1 AT ROW 4.46 COL 5
     BUTTON-2 AT ROW 4.46 COL 18
     BUTTON-3 AT ROW 4.46 COL 33
     BROWSE-1 AT ROW 6.58 COL 3
     FILL-IN-Debe-1 AT ROW 15.81 COL 77 COLON-ALIGNED
     FILL-IN-Debe-2 AT ROW 15.81 COL 92 COLON-ALIGNED
     FILL-IN-Haber-1 AT ROW 16.77 COL 77 COLON-ALIGNED
     FILL-IN-Haber-2 AT ROW 16.77 COL 92 COLON-ALIGNED
     "Fecha de Proceso" VIEW-AS TEXT
          SIZE 13.43 BY .5 AT ROW 1.38 COL 36 WIDGET-ID 12
     "HABER:" VIEW-AS TEXT
          SIZE 6 BY .5 AT ROW 16.96 COL 69
     "DEBE:" VIEW-AS TEXT
          SIZE 5 BY .5 AT ROW 16 COL 69
...
"T-DMOV.ImpMn1" ? "-ZZ,ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > Temp-Tables.T-DMOV.ImpMn2
"T-DMOV.ImpMn2" ? "-ZZ,ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  THEN DO:
      ASSIGN 
          f-Division FILL-IN-TcCompra FILL-IN-Tpocmb x-NroMes x-Periodo FILL-IN-fchast-1 FILL-IN-fchast-2.
      RUN Carga-Temporal.
      {&OPEN-QUERY-{&BROWSE-NAME}}
      RUN Totales.
      ASSIGN
          f-Division:SENSITIVE = NO
          x-Periodo:SENSITIVE = NO
          x-NroMes:SENSITIVE = NO
  ...
  ELSE DO:
      ASSIGN
          f-Division:SENSITIVE = YES
          x-Periodo:SENSITIVE = YES
          x-NroMes:SENSITIVE = YES
...
DO:
  ...
  FIND cb-peri WHERE cb-peri.CodCia  = s-codcia  
    AND cb-peri.Periodo = p-Periodo NO-LOCK NO-ERROR.
  IF AVAILABLE cb-peri 
  THEN s-NroMesCie = cb-peri.MesCie[p-Mes + 1].
...
------------------------------------------------------------------------------*/
  FOR EACH CcbBolDep WHERE ccbboldep.codcia = s-codcia
        ...
        AND ccbboldep.coddiv = f-Division
        AND ccbboldep.flgest <> 'A'
        AND ccbboldep.fchdoc >= FILL-IN-fchast-1
        AND ccbboldep.fchdoc <= FILL-IN-fchast-2:
    ASSIGN 
...
FOR EACH cb-control WHERE cb-control.CodCia  = s-codcia 
    ...
    /*DELETE cb-control.*/
END.
FOR EACH T-DMOV BREAK BY T-DMOV.fchast:
    IF FIRST-OF(T-DMOV.fchast) THEN DO:
       ...
       FIND FIRST cb-control WHERE cb-control.CodCia  = s-codcia 
           AND cb-control.Coddiv  = f-Division 
           AND cb-control.tipo    = s-Tipo
           AND cb-control.fchpro  = T-DMOV.fchast
           NO-ERROR.
       IF NOT AVAILABLE cb-control THEN CREATE cb-control.
       ELSE p-nroast = cb-control.nroast.
       CREATE cb-control.
       ASSIGN
          ...
          cb-control.fchpro  = T-DMOV.fchast
...
/*ML01*/ CB-DMOV.NroRef = T-DMOV.NroRef
    ...
    IF cb-dmov.CtaAut <> "" AND cb-dmov.CtrCta <> "" THEN DO:
        ...
        END.
    END.
    IF LAST-OF(T-dmov.FCHAST) THEN DO:
        FIND cb-cmov WHERE
        ...
        ASSIGN
            ...
            cb-cmov.NOTAST = 'Boletas registradas el ' + STRING(T-DMOV.fchast)
            cb-cmov.GLOAST = 'Boletas registradas el ' + STRING(T-DMOV.fchast).
    END.
END.
RUN Actualiza-Flag.
...
/* 12/01/2024: S.Leon por fecha automática */
FOR EACH ccbcdocu NO-LOCK WHERE ccbcdocu.codcia = s-codcia AND
    ccbcdocu.coddoc = 'BD' AND
    ccbcdocu.coddiv = f-Division AND
    LOOKUP(ccbcdocu.flgest, 'A,E,R') = 0 AND
    ccbcdocu.fchdoc >= FILL-IN-fchast-1 AND 
    ccbcdocu.fchdoc <= FILL-IN-fchast-2:
    /* CUENTA AL DEBE */
    RUN Graba-Detalle(
        ccbcdocu.fchdoc,    /* Fecha de registro del depósito (automático) */
        x-codope,
        f-Division,
        FacDocum.CodCbd,
        ccbcdocu.CodCta,
        ...
        NO,     /* Debe */
        ccbcdocu.ImpTot,
        ccbcdocu.FchAte,        /* Fecha del abono (manual) */
        ccbcdocu.NroDoc,
        '10',
        ccbcdocu.NroRef,
        ccbcdocu.NomCli).
...
------------------------------------------------------------------------------*/
  DISPLAY x-Periodo FILL-IN-fchast-1 FILL-IN-fchast-2 x-NroMes f-Division 
          FILL-IN-Tpocmb FILL-IN-TcCompra FILL-IN-Debe-1 FILL-IN-Debe-2 
          FILL-IN-Haber-1 FILL-IN-Haber-2 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE x-Periodo x-NroMes f-Division BUTTON-1 BUTTON-3 BROWSE-1 
      WITH FRAME F-Main IN WINDOW W-Win.
...
FIND CB-CTAS WHERE CB-CTAS.codcia = cb-codcia 
    ...
    NO-LOCK NO-ERROR.
/* buscamos por la equivalencia en caso queden remanentes */
IF NOT AVAILABLE CB-CTAS THEN FIND CB-CTAS WHERE CB-CTAS.codcia = cb-codcia 
    AND CB-CTAS.ctaant = x-codcta
    NO-LOCK NO-ERROR.
IF AVAILABLE CB-CTAS 
THEN ASSIGN
        x-CodCta = CB-CTAS.CodCta   /* OJO */
