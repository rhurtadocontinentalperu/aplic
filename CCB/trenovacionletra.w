...
&Scoped-define FIELDS-IN-QUERY-br_table DOCU.NroDoc DOCU.FchDoc DOCU.FchVto ~
DOCU.ImpTot 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table DOCU.FchDoc DOCU.FchVto ~
DOCU.ImpTot 
...
&Scoped-Define ENABLED-OBJECTS br_table f-NroLet F-DiaLet BUTTON-2 
&Scoped-Define DISPLAYED-OBJECTS f-NroLet F-DiaLet f-ImpTot 
...
DEFINE VARIABLE F-DiaLet AS INTEGER FORMAT ">>9":U INITIAL 30 
     LABEL "Plazo" 
     VIEW-AS FILL-IN 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 43 BY 7.31
         FONT 4.
...
DEFINE FRAME F-Main
     br_table AT ROW 1 COL 37
     f-NroLet AT ROW 1.38 COL 21 COLON-ALIGNED WIDGET-ID 18
     F-DiaLet AT ROW 2.54 COL 21 COLON-ALIGNED WIDGET-ID 20
     BUTTON-2 AT ROW 3.69 COL 6 WIDGET-ID 16
     f-ImpTot AT ROW 8.5 COL 62 COLON-ALIGNED WIDGET-ID 14
     "dias" VIEW-AS TEXT
          SIZE 5 BY .62 AT ROW 2.73 COL 28 WIDGET-ID 22
...
"DOCU.NroDoc" ? ? "character" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.DOCU.FchDoc
"DOCU.FchDoc" "Fecha de!Emisión" ? "date" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.DOCU.FchVto
"DOCU.FchVto" ? ? "date" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.DOCU.ImpTot
...
ON CHOOSE OF BUTTON-2 IN FRAME F-Main /* GENERAR LETRAS */
DO:
  ASSIGN
      F-DiaLet f-NroLet.
  RUN Genera-Letras.
...
&Scoped-define SELF-NAME F-DiaLet
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-DiaLet B-table-Win
ON LEAVE OF F-DiaLet IN FRAME F-Main /* Plazo */
DO:
  IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '' THEN DO:
     MESSAGE 'Plazo debe ser mayor a cero'.
     RETURN 'NO-APPLY'.
  END.
END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia
      ...
      AND Ccbcdocu.nrodoc BEGINS SUBSTRING(s-nroref,1,9):
      x-Conta = x-Conta + 1.
      x-fchvto = CcbCDocu.FchVto + F-DiaLet.
  END.
  FIND Ccbcdocu WHERE Ccbcdocu.codcia = s-codcia
      AND Ccbcdocu.coddoc = 'LET'
      AND Ccbcdocu.nrodoc = s-nroref NO-LOCK NO-ERROR.
  IF NOT AVAILABLE Ccbcdocu THEN RETURN.

  s-conta  = 1.
  IF x-Conta > 0 THEN x-nroref = SUBSTRING(s-nroref,1,9) + '-' + STRING(x-conta, '9') .
  ...
  DO s-conta = 1 TO F-NroLet:
      ...
      ASSIGN 
          ...
          DOCU.CodDoc = "LET"
          DOCU.NroDoc = x-nroref
          DOCU.FchDoc = TODAY
          DOCU.FchVto = x-fchvto
          DOCU.NroSal = Ccbcdocu.NroSal
          DOCU.ImpTot = IF (s-conta = F-NroLet) THEN
              (s-imptot - ((s-conta - 1) * s-implet))
              ELSE s-implet. 
        ASSIGN
            x-fchvto = x-fchvto + F-DiaLet
            x-conta  = x-conta  + 1
            x-nroref = SUBSTRING(s-nroref,1,9) + '-' + STRING(x-conta, '9') .
  END.
...
ASSIGN
    x-FchIni = DATE(DOCU.FchDoc:SCREEN-VALUE IN BROWSE {&browse-name})
    x-FchFin = DATE(DOCU.FchVto:SCREEN-VALUE IN BROWSE {&browse-name}).

IF x-FchFin <= x-FchIni THEN DO:
    MESSAGE 'Ingrese correctamente las fechas' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO DOCU.FchDoc.
    RETURN 'ADM-ERROR'.
END.
