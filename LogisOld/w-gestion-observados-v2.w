...
&Scoped-define INTERNAL-TABLES ChkTareas VtaDDocu Almmmatg
...
FUNCTION chequeador RETURNS CHARACTER
  ( INPUT ppCodDoc AS CHAR, INPUT ppNroPed AS CHAR )  FORWARD.
...
DEFINE FRAME F-Main
     ...
     "DoubleClick en Orden - Cerrar observacion" VIEW-AS TEXT
          ...
          FGCOLOR 12 FONT 11
     "  Documento HPK" VIEW-AS TEXT
          SIZE 16 BY .62 AT ROW 1.12 COL 3 WIDGET-ID 6
...
"tiempo-observado(chktareas.fechainicio, chktareas.horainicio) @ x-tiempo-observado-col" "Tiempo Observado" "x(25)" ? ? ? 0 ? ? ? no ? no no "18.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-14 */
&ANALYZE-RESUME
...
DO:
  ...
  IF AVAILABLE chktareas THEN DO:
        ...
        browse-15:TITLE = chktareas.coddoc + " " + x-nroped + " " + cliente(chktareas.coddoc, chktareas.nroped).
  END.
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    IF chktareas.coddoc = 'HPK' THEN DO:
        ...
        END.
        ASSIGN b-vtacdocu.flgsit = 'POL'.
        /* ********************************************************************************* */
        /* REFLEJAMOS TRACKING DE LA OD */
        /* ********************************************************************************* */
        RUN logis/actualiza-flgsit (INPUT ROWID(b-VtaCDocu)).
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            x-ProcesoOK = 'Error: NO se pudo actualizar el tracking de O/D'.
            UNDO GRABAR_REGISTROS, LEAVE GRABAR_REGISTROS.
        END.
        /* ********************************************************************************* */
    END.
    ELSE DO:
        FIND FIRST b-faccpedi WHERE b-faccpedi.codcia = s-codcia AND 
                                        ...
                                        b-faccpedi.nroped = chktareas.nroped EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
...
FUNCTION chequeador RETURNS CHARACTER
  ( INPUT ppCodDoc AS CHAR, INPUT ppNroPed AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
...
IF pCodDoc = "HPK" THEN DO:
    ...
    IF NOT AVAILABLE x-faccpedi THEN DO:
        RETURN x-retval.
    END.
    pCodDoc = vtacdocu.codref.  /* O/D */
    pNroPed = vtacdocu.nroref.
END.
ELSE DO:
    pCodDoc = ppCodDoc.
    pNroPed = ppNroPed.
