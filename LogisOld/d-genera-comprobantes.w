...
RECT-1 RECT-25 
&Scoped-Define DISPLAYED-OBJECTS cboCobranza txtMecanismo txtMetodoPago ~
COMBO-BOX-Guias COMBO-NroSer-Guia FILL-IN-NroPed FILL-IN-Cliente ~
FILL-IN-DirClie COMBO-NroSer FILL-IN-NroDoc FILL-IN-items FILL-IN-Glosa ~
FILL-IN-LugEnt FILL-IN-NroDoc-GR 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 80 BY 12.35
...
DEFINE FRAME mgDialog
     BROWSE-2 AT ROW 10.15 COL 2 WIDGET-ID 200
     cboCobranza AT ROW 8.92 COL 9.57 WIDGET-ID 42
     ...
     COMBO-BOX-Guias AT ROW 4.5 COL 72 COLON-ALIGNED NO-LABEL WIDGET-ID 36
     ...
     "<<=== Generamos Guía de Remisión?:" VIEW-AS TEXT
          SIZE 32 BY .77 AT ROW 4.5 COL 42 WIDGET-ID 38
...
/* SETTINGS FOR FILL-IN FILL-IN-DirClie IN FRAME mgDialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FILL-IN-Glosa IN FRAME mgDialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FILL-IN-items IN FRAME mgDialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FILL-IN-LugEnt IN FRAME mgDialog
   NO-ENABLE                                                            */
...
ON VALUE-CHANGED OF cboCobranza IN FRAME mgDialog /* Courier */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Inicial mgDialog 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Devolucion-Mercaderia mgDialog 
...
------------------------------------------------------------------------------*/
  DISPLAY cboCobranza txtMecanismo txtMetodoPago COMBO-BOX-Guias 
          ...
          FILL-IN-NroDoc-GR 
      WITH FRAME mgDialog.
  ENABLE BROWSE-2 COMBO-NroSer Btn_OK Btn_Cancel RECT-1 RECT-25 
      WITH FRAME mgDialog.
  VIEW FRAME mgDialog.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Genera-NC mgDialog 
PROCEDURE Genera-NC :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       SOLO PARA LISTA EXPRESS
            El Comprobante se genera CANCELADO AUTOMATICAMENTE y se genera un DCO
            para el proveedor cboCobranza
------------------------------------------------------------------------------*/    

    IF NOT (COTIZACION.TpoPed = "LF") THEN RETURN 'OK'.

    IF NOT CAN-FIND(FIRST gn-clie WHERE gn-clie.codcia = cl-codcia AND
                    gn-clie.codcli = cboCobranza NO-LOCK)
        THEN DO:
        pMensaje = "NO está configurado el COURIER en el maestro de clientes" + 
            s-CodDiv + CHR(10) + "Proceso Abortado".
        RETURN 'ADM-ERROR'.
    END.
    IF NOT CAN-FIND(FIRST FacCorre WHERE FacCorre.codcia = s-codcia AND
                    FacCorre.coddoc = "DCO" AND
                    FacCorre.coddiv = s-coddiv AND
                    FacCorre.FlgEst = YES NO-LOCK)
        THEN DO:
        pMensaje = "NO está configurado el correlativo para el documento DCO en la división " + 
            s-CodDiv + CHR(10) + "Proceso Abortado".
        RETURN 'ADM-ERROR'.
    END.

    DEF BUFFER B-CDOCU FOR Ccbcdocu.
    DEF VAR xComision AS DEC NO-UNDO.
    ...
    DO TRANSACTION ON ERROR UNDO trloop, RETURN 'ADM-ERROR' ON STOP UNDO trloop, RETURN 'ADM-ERROR':
        /* Bloqueamos el control de correlativos */
        /* Correlativo */
        {lib\lock-genericov3.i ~
            &Tabla="FacCorre" ~
            &Condicion="FacCorre.CodCia = s-CodCia ~
        ...
        FOR EACH Reporte NO-LOCK WHERE LOOKUP(Reporte.CodDoc, 'FAC,BOL') > 0,
            FIRST Ccbcdocu OF Reporte EXCLUSIVE-LOCK:
            xComision = 0.  /* Comision 0 */
            CREATE B-CDOCU.
            BUFFER-COPY Ccbcdocu TO B-CDOCU
                ASSIGN 
                    B-CDOCU.CodDoc = "DCO"
                    B-CDOCU.NroDoc = STRING(FacCorre.NroSer, '999') + STRING(FacCorre.Correlativo, '999999')
                    B-CDOCU.CodCli = cboCobranza
                    B-CDOCU.FmaPgo = "000"   /* CONTADO */
                    B-CDOCU.Sede = ''
            ...
            ASSIGN
                FacCorre.Correlativo = FacCorre.Correlativo + 1.
            RELEASE FacCorre.
            /* Marcamos el Comprobante como CANCELADO */
            ASSIGN
                Ccbcdocu.FlgEst = "C"
                Ccbcdocu.SdoAct = 0
                Ccbcdocu.FchCan = TODAY
                CcbCDocu.UsrCobranza = s-User-Id.
            /* Blanqueamos Datos, solo dejamos el ImpTot y SdoAct */
            ASSIGN
                B-CDOCU.ImpBrt  = 0
                B-CDOCU.ImpCto  = 0
                B-CDOCU.ImpDto  = 0
                ...
                B-CDOCU.ImpVta  = 0.
            /* Datos del cliente */
            FIND gn-clie WHERE gn-clie.codcia = cl-codcia AND gn-clie.codcli = B-CDOCU.codcli
                NO-LOCK NO-ERROR.
            IF AVAILABLE gn-clie THEN DO:
                ASSIGN
                    B-CDOCU.RucCli = gn-clie.Ruc
                    B-CDOCU.DirCli = gn-clie.DirCli
                    B-CDOCU.NomCli = gn-clie.NomCli.
            END.
            /* Quitamos la comision */
            FIND VtaTabla WHERE VtaTabla.CodCia = s-CodCia AND
                VtaTabla.Tabla = "%COMIECOM" AND
                VtaTabla.Llave_c1 = B-CDOCU.codcli
                NO-LOCK NO-ERROR.
            IF AVAILABLE VtaTabla AND (VtaTabla.Valor[2] + VtaTabla.Valor[3]) > 0
                THEN DO:
                /* Comision porcentual */
                xComision = B-CDOCU.ImpTot * VtaTabla.Valor[3] / 100.
                /* Comision fija */
                IF VtaTabla.Valor[1] = B-CDOCU.CodMon THEN DO:
                    xComision = xComision + VtaTabla.Valor[2].
                END.
                ELSE DO:
                    IF B-CDOCU.CodMon = 1 THEN DO:
                        xComision = xComision + ROUND(VtaTabla.Valor[2] * xTCVenta, 2).
                    END.
                    ELSE DO:
                        xComision = xComision + ROUND(VtaTabla.Valor[2] / xTCCompra, 2).
                    END.
                END.
                IF xComision > 0 THEN DO:
                    /* Recalculamos */
                    ASSIGN
                        B-CDOCU.ImpTot = B-CDOCU.ImpTot - xComision
        ...
        FOR EACH Reporte NO-LOCK WHERE LOOKUP(Reporte.CodDoc, 'FAC,BOL,FAI') > 0, 
            ...
            IF lCreaHeader THEN DO:
                ...
                FIND Gn-ClieD WHERE Gn-ClieD.CodCia = cl-CodCia AND
                    Gn-ClieD.CodCli = Ccbcdocu.CodCli AND
                    Gn-ClieD.Sede   = Ccbcdocu.Sede NO-LOCK NO-ERROR.
                IF AVAILABLE Gn-ClieD THEN 
                    ASSIGN
                    CcbCDocu.CodDpto = Gn-ClieD.CodDept 
                    CcbCDocu.CodProv = Gn-ClieD.CodProv
                    CcbCDocu.CodDist = Gn-ClieD.CodDist 
                    CcbCDocu.LugEnt  = Gn-ClieD.DirCli.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Graba-Temp-FeLogErrores mgDialog 
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      IF s-CodDiv = '00506' THEN DO:
          ...
          END.
          cboCobranza:DELETE(1).
          cboCobranza:DELIMITER = '|'.
          FOR EACH vtatabla NO-LOCK WHERE VtaTabla.CodCia = s-codcia AND VtaTabla.Tabla = '%COMIECOM',
              FIRST gn-clie WHERE gn-clie.CodCia = cl-codcia AND gn-clie.CodCli = VtaTabla.Llave_c1:
              cboCobranza:ADD-LAST(gn-clie.codcli + ' ' + gn-clie.nomcli,gn-clie.codcli).
              cboCobranza:SCREEN-VALUE = gn-clie.codcli.
          END.
/*           FOR EACH gn-clie NO-LOCK WHERE gn-clie.codcia = cl-codcia AND                   */
/*               LOOKUP(gn-clie.codcli, '20557738828,20601085641,20569281033') > 0:          */
/*               cboCobranza:ADD-LAST(gn-clie.codcli + ' ' + gn-clie.nomcli,gn-clie.codcli). */
/*               cboCobranza:SCREEN-VALUE = gn-clie.codcli.                                  */
/*           END.                                                                            */
          ENABLE cboCobranza.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE MASTER-TRANSACTION mgDialog 
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    IF AVAILABLE T-CcbADocu THEN DO:
        ...
        IF B-ADOCU.Libre_C[9] > '' THEN DO:  /* AGENCIA DE TRANSPORTE */
            FIND gn-provd WHERE gn-provd.CodCia = pv-codcia AND 
                gn-provd.CodPro = B-ADOCU.Libre_C[9] AND 
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH T-DDOCU OF T-CDOCU:
      ...
      IF NOT T-DDOCU.AftIgv THEN T-CDOCU.ImpExo = T-CDOCU.ImpExo + T-DDOCU.ImpLin.
      IF T-DDOCU.AftIgv = YES
      THEN T-CDOCU.ImpDto = T-CDOCU.ImpDto + ROUND(T-DDOCU.ImpDto / (1 + T-CDOCU.PorIgv / 100), 2).
      ELSE T-CDOCU.ImpDto = T-CDOCU.ImpDto + T-DDOCU.ImpDto.

      IF NOT T-DDOCU.AftIgv THEN x-Dto2xExonerados = x-Dto2xExonerados + T-DDOCU.ImpDto2.
      ELSE x-Dto2xAfectosIgv = x-Dto2xAfectosIgv + T-DDOCU.ImpDto2.
  END.
  ASSIGN
  ...
  ASSIGN
      T-CDOCU.ImpBrt = T-CDOCU.ImpVta + T-CDOCU.ImpDto
      T-CDOCU.SdoAct = T-CDOCU.ImpTot.
  /* RHC 06/05/2014 En caso tenga descuento por Encarte */
  IF T-CDOCU.ImpDto2 > 0 THEN DO:
      ASSIGN
          T-CDOCU.ImpTot = T-CDOCU.ImpTot - T-CDOCU.ImpDto2
          T-CDOCU.ImpIgv = T-CDOCU.ImpIgv -  ~
                ROUND(x-Dto2xAfectosIgv / ( 1 + T-CDOCU.PorIgv / 100) * T-CDOCU.PorIgv / 100, 2)
          T-CDOCU.ImpExo = T-CDOCU.ImpExo - x-Dto2xExonerados
          T-CDOCU.ImpVta = T-CDOCU.ImpTot - T-CDOCU.ImpExo - T-CDOCU.ImpIgv
          T-CDOCU.ImpBrt = T-CDOCU.ImpVta + T-CDOCU.ImpDto.
  END.
  ...
  /* ************************************* */
  IF T-CDOCU.FmaPgo = "900" THEN
      ASSIGN
        T-CDOCU.ImpBrt = 0
        T-CDOCU.ImpDto = 0
        T-CDOCU.ImpExo = T-CDOCU.ImpTot
        T-CDOCU.ImpVta = 0
        T-CDOCU.ImpIgv = 0.
