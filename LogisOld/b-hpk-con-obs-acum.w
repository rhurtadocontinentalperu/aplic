...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        pError = 'NO se pudo actualizar el tracking de O/D'.
        UNDO, LEAVE.
    END.
    /* ********************************************************************************* */
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    IF NOT CAN-FIND(FIRST Vtaddocu OF Vtacdocu NO-LOCK)
        THEN ASSIGN Vtacdocu.CodPed = Vtacdocu.CodPed + "XX".   /* Artificio C.I. */
    /* ************************************************************ */
    /* RHC 29/01/2019 BUSCAMOS SI AUN LA O/D ESTA EN LA PHR */
    /* Barremos c/u de los documentos */
    DEF VAR pFound AS LOG NO-UNDO.
