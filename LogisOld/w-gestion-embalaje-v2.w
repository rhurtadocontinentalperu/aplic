...
&Scoped-define FIELDS-IN-QUERY-BROWSE-15 tt-w-report.Campo-I[1] ~
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 87 BY 10.58
         FONT 4
...
DEFINE FRAME F-Main
     ...
     "DoubleClick en Orden - Cerrar EMBALADO" VIEW-AS TEXT
          SIZE 40.43 BY .96 AT ROW 13.04 COL 90.14 WIDGET-ID 4
          FGCOLOR 9 FONT 11
...
"tt-w-report.Campo-I[1]" "Item" ">,>>9" "integer" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"tt-w-report.Campo-C[3]" "Marca" "X(30)" "character" ? ? 0 ? ? ? no ? no no "18.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.tt-w-report.Campo-F[1]
...
/*                                         b-faccpedi.nroped = ttOrdenes.tnrodoc EXCLUSIVE-LOCK NO-ERROR NO-WAIT. */
...
/*     END.                                                                                                       */
  ...
  DO:
    IF chktareas.coddoc = 'HPK' THEN DO:
        ...
        RUN verifica-hpr-cerrada(INPUT b-vtacdocu.codori, INPUT b-vtacdocu.nroori, OUTPUT x-hpr-ok).
        IF x-hpr-ok = 'OK' THEN DO:
            FIND FIRST di-rutaC WHERE di-rutaC.codcia = s-codcia AND
                di-rutaC.coddoc = b-vtacdocu.codori AND
                di-rutaC.nrodoc = b-vtacdocu.nroori EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
            IF NOT AVAILABLE di-rutaC THEN DO:
                x-ProcesoOk = "No se pudo cerrar la HPR".
                UNDO GRABAR_REGISTROS, LEAVE GRABAR_REGISTROS.
            END.
            ASSIGN  
                di-rutaC.flgest = 'P'.
