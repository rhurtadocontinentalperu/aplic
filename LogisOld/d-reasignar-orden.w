...
DEFINE INPUT PARAMETER pCodPHR AS CHAR.
DEFINE INPUT PARAMETER pNroPHR AS CHAR.
...
DI-RutaC.NroDoc DI-RutaC.CodDiv 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 50 BY 10.58 FIT-LAST-COLUMN.
...
DEFINE FRAME gDialog
     ...
     FILL-IN-orden AT ROW 4 COL 59.72 COLON-ALIGNED WIDGET-ID 6
     FILL-IN-phr AT ROW 7.54 COL 54 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     Btn_Cancel AT ROW 14.46 COL 21
     Btn_OK AT ROW 14.54 COL 4
     "Origen de la Orden" VIEW-AS TEXT
          SIZE 30 BY 1.15 AT ROW 6.19 COL 56 WIDGET-ID 8
          BGCOLOR 15 FGCOLOR 9 FONT 9
     SPACE(2.13) SKIP(9.77)
...
GRABAR_DATOS:
DO TRANSACTION ON ERROR UNDO GRABAR_DATOS, RETURN 'ADM-ERROR'
        ON STOP UNDO GRABAR_DATOS, RETURN 'ADM-ERROR':
    ...
    FOR EACH di-rutaD EXCLUSIVE-LOCK WHERE di-rutaD.codcia = s-codcia AND
        ...
        di-rutaD.codref = pCodOrden AND     /* P/D */
        di-rutaD.nroref = pNroOrden ON ERROR UNDO, THROW:
        ASSIGN di-rutaD.nrodoc = di-rutaC.nrodoc.
    END.
END.
RELEASE x-vtacdocu.
RELEASE di-rutaD.
