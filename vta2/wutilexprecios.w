...
ASSIGN
    ...
    chWorkSheet:Range("H2"):Value = "MONEDA COSTO"
    chWorkSheet:Range("I2"):Value = "TC"
    chWorkSheet:Range("J2"):Value = "COSTO S/.".
ASSIGN
    t-Row = 2.
FOR EACH Almmmatg NO-LOCK WHERE {&Condicion}, FIRST Almtfami OF Almmmatg NO-LOCK, Almsfami OF Almmmatg NO-LOCK:
    ...
    IF AVAILABLE Almtconv THEN F-FACTOR = Almtconv.Equival.
    x-CtoTot = x-Ctotot * f-Factor.
    IF Almmmatg.MonVta = 2 THEN ASSIGN x-CtoTot = x-CtoTot * Almmmatg.TpoCmb.
    IF Almmmatg.MonVta = 2 THEN ASSIGN x-PreOfi = x-PreOfi * Almmmatg.TpoCmb.
    ASSIGN
        t-Column = 0
        t-Row    = t-Row + 1.
    ASSIGN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Excel-5-Old W-Win 
PROCEDURE Excel-5-Old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Era antes Lista de Terceros
------------------------------------------------------------------------------*/
