...
&Scoped-define INTERNAL-TABLES B-Tabla Almmmatg

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table B-Tabla.Llave_c2 Almmmatg.DesMat ~
B-Tabla.Valor[1] 
...
&Scoped-define QUERY-STRING-br_table FOR EACH B-Tabla WHERE B-Tabla.CodCia = VtaTabla.CodCia ~
  ...
  AND Almmmatg.codmat = B-Tabla.Llave_c2 NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH B-Tabla WHERE B-Tabla.CodCia = VtaTabla.CodCia ~
  ...
  AND Almmmatg.codmat = B-Tabla.Llave_c2 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table B-Tabla Almmmatg
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      B-Tabla.Llave_c2 COLUMN-LABEL "Artículo" FORMAT "x(6)":U
      Almmmatg.DesMat FORMAT "X(60)":U
      B-Tabla.Valor[1] COLUMN-LABEL "% Descuento" FORMAT ">>9.99":U
  ENABLE
      B-Tabla.Llave_c2
      B-Tabla.Valor[1]
/* _UIB-CODE-BLOCK-END */
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "B-Tabla WHERE INTEGRAL.VtaTabla <external> ...,INTEGRAL.Almmmatg WHERE B-Tabla ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST"
     _JoinCode[1]      = "B-Tabla.CodCia = INTEGRAL.VtaTabla.CodCia
  AND B-Tabla.Llave_c1 = INTEGRAL.VtaTabla.Llave_c1"
     _Where[1]         = "B-Tabla.Tabla = ""UTILEXD-CD"""
     _JoinCode[2]      = "INTEGRAL.Almmmatg.CodCia = B-Tabla.CodCia
  AND INTEGRAL.Almmmatg.codmat = B-Tabla.Llave_c2"
     _FldNameList[1]   > Temp-Tables.B-Tabla.Llave_c2
"B-Tabla.Llave_c2" "Artículo" "x(6)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almmmatg.DesMat
"Almmmatg.DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.B-Tabla.Valor[1]
"B-Tabla.Valor[1]" "% Descuento" ">>9.99" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
