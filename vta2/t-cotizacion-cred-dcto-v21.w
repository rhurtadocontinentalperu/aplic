...
"Temp-Tables.ITEM.NroItm" "No" ">>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.ITEM.codmat
"Temp-Tables.ITEM.codmat" "Articulo" "X(13)" "character" ? ? ? ? ? ? no ? no no "7" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.Almmmatg.DesMat
"integral.Almmmatg.DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ? no no "53.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > integral.Almmmatg.DesMar
"integral.Almmmatg.DesMar" "Marca" "X(12)" "character" ? ? ? ? ? ? no ? no no "13.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.ITEM.UndVta
"Temp-Tables.ITEM.UndVta" "Unidad" "x(10)" "character" ? ? ? ? ? ? no ? no no "5.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Temp-Tables.ITEM.PreUni" "Precio!Unitario" ">>,>>9.99999" "decimal" ? ? ? ? ? ? no ? no no "6" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.ITEM.Por_Dsctos[1]
"Temp-Tables.ITEM.Por_Dsctos[1]" "% Dscto.!Manual" "->>9.9999" "decimal" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.ITEM.Por_Dsctos[2]
"Temp-Tables.ITEM.Por_Dsctos[2]" "% Dscto!Evento" "->>9.9999" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > Temp-Tables.ITEM.Por_Dsctos[3]
"Temp-Tables.ITEM.Por_Dsctos[3]" "% Dscto!Vol/Prom" "->>9.9999" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > "_<CALC>"
"fMargen() @ x-Margen" "Margen!(%)" "->,>>9.99" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > Temp-Tables.ITEM.ImpLin
"Temp-Tables.ITEM.ImpLin" ? ">,>>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  IF DEC(ITEM.Por_DSCTOS[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) <= 0 THEN RETURN 'OK'.
  ...
  FIND FIRST BDctoDivLin WHERE BDctoDivLin.codcia = s-codcia
      ...
      NO-LOCK NO-ERROR.
  CASE TRUE:
      WHEN Almmmatg.CHR__02 = "T" THEN DO:     /* Productos de Terceros */
          /* Por defecto vale el de la división y línea */
          IF AVAILABLE BDctoLin AND BDctoLin.Valor[1] > 0 THEN x-DtoMax = BDctoLin.Valor[1].
          IF AVAILABLE BDctoDivLin AND BDctoDivLin.Valor[1] > 0 THEN x-DtoMax = BDctoDivLin.Valor[1].
          ...
          END.
      END.
      WHEN Almmmatg.CHR__02 = "P" THEN DO:     /* Productos Propios */
          /* **************************************************************************** */
          /* NO HAY DESCUENTOS PARA PRODUCTOS PROPIOS, SALVO LAS EXCEPCIONES CONFIGURADAS */
          /* **************************************************************************** */
          /* Debe haber al menos un descuento */
          IF NOT (AVAILABLE BDctoLin OR AVAILABLE BDctoDivLin) THEN DO:
              MESSAGE 'Descuento NO configurado para productos PROPIOS de la línea' Almmmatg.codfam 
                  VIEW-AS ALERT-BOX ERROR.
              APPLY "ENTRY" TO ITEM.Por_DSCTOS[1].
              RETURN 'ADM-ERROR'.
          END.
          ...
          IF NOT AVAILABLE BDctoDivLin OR BDctoDivLin.Valor[2] <= 0 THEN DO:
              /* Segundo por Línea */
              IF NOT AVAILABLE BDctoLin OR BDctoLin.Valor[2] <= 0 THEN DO:
                  MESSAGE 'Descuento NO configurado para productos PROPIOS de la línea' Almmmatg.codfam 
                      VIEW-AS ALERT-BOX ERROR.
                  APPLY "ENTRY" TO ITEM.Por_DSCTOS[1].
                  RETURN 'ADM-ERROR'.
          ...
          IF AVAILABLE Almtconv THEN DO:
              ...
              END.
          END.
          IF AVAILABLE BDctoLin AND BDctoLin.Valor[2] > 0       THEN x-DtoMax = BDctoLin.Valor[2].
          IF AVAILABLE BDctoDivLin AND BDctoDivLin.Valor[2] > 0 THEN x-DtoMax = BDctoDivLin.Valor[2].
          IF DEC(ITEM.Por_DSCTOS[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > x-DtoMax THEN DO:
...
------------------------------------------------------------------------------*/
      ...
      /* RHC 07/07/220 Salvo la línea 019 y tiendas LIMA */
/*       FIND gn-divi WHERE gn-divi.codcia = s-codcia AND                        */
/*           gn-divi.coddiv = s-coddiv NO-LOCK NO-ERROR.                         */
/*       IF AVAILABLE gn-divi THEN DO:                                           */
/*           IF NOT ( Almmmatg.codfam = '019' AND gn-divi.campo-char[3] = "15" ) */
/*               THEN DO:                                                        */
