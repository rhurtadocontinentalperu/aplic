...
DO:
    ...
    ASSIGN
        s-Porpercepcion = 0.
    /* DETERMINAMOS EL % DE PERCEPCION */
    FIND gn-clie WHERE gn-clie.codcia = cl-codcia AND gn-clie.codcli = Ccbcdocu.codcli NO-LOCK NO-ERROR.
    IF NOT AVAILABLE gn-clie THEN LEAVE PERCEPCION.
    ...
    /* DETERMINAMOS SI ES AFECTO A PERCEPCION */
    ASSIGN
        f-ImpPercepcion = 0.
    ...
    IF Ccbcdocu.coddoc = "BOL" THEN DO:
        IF Ccbcdocu.codmon = 1 AND f-ImpPercepcion <= ImpMinPercep THEN s-PorPercepcion = 0.
        IF Ccbcdocu.codmon = 2 AND f-ImpPercepcion * Ccbcdocu.tpocmb <= ImpMinPercep THEN s-PorPercepcion = 0.
    END.
    IF Ccbcdocu.FmaPgo = "900" THEN s-PorPercepcion = 0.     /* NO Transferencias gratuitas */
    FIND FIRST Vtatabla WHERE Vtatabla.codcia = s-codcia
        AND Vtatabla.tabla = 'CLNOPER'
        AND VtaTabla.Llave_c1 = CcbCDocu.RucCli
        NO-LOCK NO-ERROR.
    IF AVAILABLE Vtatabla THEN s-PorPercepcion = 0.
    /* ******************************** */
    FOR EACH ccbddocu OF ccbcdocu, FIRST Almmmatg OF ccbddocu NO-LOCK, FIRST Almsfami OF Almmmatg NO-LOCK:
        ASSIGN
