...
T-LISTA.Chr__01 T-LISTA.Dec__01 T-LISTA.PreOfi 
...
&Scoped-define QUERY-STRING-br_table FOR EACH T-LISTA WHERE T-LISTA.CodCia = VtaTabla.CodCia ~
  AND T-LISTA.TpoPed = VtaTabla.Tabla ~
  AND T-LISTA.NroLista = INTEGER(INTEGRAL.VtaTabla.Valor[1]) NO-LOCK, ~
      FIRST Almmmatg OF T-LISTA NO-LOCK, ~
      FIRST VtaListaPrecios OF T-LISTA NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      T-LISTA.Dec__01 COLUMN-LABEL "% Uti Ofi" FORMAT "->>,>>9.99":U
...
/* Query rebuild information for BROWSE br_table
  ...
  AND Temp-Tables.T-LISTA.TpoPed = INTEGRAL.VtaTabla.Tabla
  AND Temp-Tables.T-LISTA.NroLista = INTEGER(INTEGRAL.VtaTabla.Valor[1])"
     _FldNameList[1]   > Temp-Tables.T-LISTA.codmat
"Temp-Tables.T-LISTA.codmat" "Articulo" "X(13)" "character" 14 0 ? ? ? ? yes ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"INTEGRAL.Almmmatg.UndBas" "Unidad" "X(6)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"IF (Almmmatg.MonVta = 2) THEN (Almmmatg.CtoTot * Almmmatg.TpoCmb) ELSE (Almmmatg.CtoTot) @ Almmmatg.CtoTot" "Costo Total!S/." ">>>,>>9.9999" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.T-LISTA.Chr__01
"Temp-Tables.T-LISTA.Chr__01" "Und" "X(6)" "character" 12 15 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.T-LISTA.Dec__01
"Temp-Tables.T-LISTA.Dec__01" "% Uti Ofi" "->>,>>9.99" "decimal" 12 15 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > Temp-Tables.T-LISTA.PreOfi
"Temp-Tables.T-LISTA.PreOfi" "Precio Oficina!S/." ? "decimal" 12 15 ? ? ? ? yes ? no no "10.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&Scoped-define SELF-NAME T-LISTA.Dec__01
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL T-LISTA.Dec__01 br_table _BROWSE-COLUMN B-table-Win
ON LEAVE OF T-LISTA.Dec__01 IN BROWSE br_table /* % Uti Ofi */
DO:
    ASSIGN
        F-MrgUti = DECIMAL(T-LISTA.DEC__01:SCREEN-VALUE IN BROWSE {&BROWSE-NAME})
        X-CTOUND = IF (Almmmatg.MonVta = 2) THEN (Almmmatg.CtoTot * Almmmatg.TpoCmb) ELSE (Almmmatg.CtoTot).
    F-FACTOR = 1.
    F-PreVta = 0.
    ...
    DISPLAY F-PreVta @ T-LISTA.PreOfi WITH BROWSE {&BROWSE-NAME}.
END.
...
ON 'RETURN':U OF T-LISTA.Dec__01, T-LISTA.codmat, T-LISTA.PreOfi
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
FOR EACH VtaListaPrecios WHERE VtaListaPrecios.CodCia = VtaTabla.CodCia
    AND VtaListaPrecios.TpoPed = VtaTabla.Tabla
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAILABLE VtaTabla THEN DO:
      MESSAGE 'Primero debe crear la lista antes de ingresar los artículos'
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      ASSIGN
          ...
          T-LISTA.TpoPed = VtaTabla.Tabla
          T-LISTA.NroLista = INTEGER(VtaTabla.Valor[1])
          T-LISTA.CHR__01 = T-LISTA.CHR__01:SCREEN-VALUE IN BROWSE {&browse-name}.
  ...
  ASSIGN
      VtaListaPrecios.Dec__01 = ( (VtaListaPrecios.PreOfi / (Almmmatg.Ctotot * f-Factor) ) - 1 ) * 100
      VtaListaPrecios.PreVta[1] = VtaListaPrecios.PreOfi
      T-LISTA.Dec__01 = VtaListaPrecios.Dec__01.
