...
DEFINE VARIABLE h_t-pedmos-may AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-pedmos-mayb AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BTN-Excel AT ROW 2.88 COL 121 WIDGET-ID 6
     BTN-COTIZA AT ROW 4.23 COL 121 WIDGET-ID 8
     Btn-Grupos AT ROW 26.31 COL 36 WIDGET-ID 2
     B-RES AT ROW 26.31 COL 56 WIDGET-ID 10
...
DO:
  RUN Resumen-por-Division IN h_v-pedmos-may.
...
DO:
  RUN vtamay/d-pedmos-4.
  RUN dispatch IN h_t-pedmos-may ('open-query').
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-pedmos-may.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmos-may ).
       RUN set-position IN h_t-pedmos-may ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedmos-may ( 15.54 , 147.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 26.04 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.42 , 34.14 ) NO-ERROR.

       /* Links to SmartBrowser h_t-pedmos-may. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedmos-may ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedmos-may ,
             BTN-COTIZA:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-pedmos-may , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-pedmos-mayb.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmos-mayb ).
       RUN set-position IN h_t-pedmos-mayb ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedmos-mayb ( 15.54 , 147.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv96 ).
       RUN set-position IN h_p-updv96 ( 26.04 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv96 ( 1.54 , 26.14 ) NO-ERROR.

       /* Links to SmartBrowser h_t-pedmos-mayb. */
       RUN add-link IN adm-broker-hdl ( h_p-updv96 , 'TableIO':U , h_t-pedmos-mayb ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedmos-mayb ,
             BTN-COTIZA:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv96 ,
             h_t-pedmos-mayb , 'AFTER':U ).
    END. /* Page 3 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-73 COMBO-NroSer BTN-Excel BTN-COTIZA Btn-Grupos B-RES 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-pedmos-may <> ? THEN RUN dispatch IN h_t-pedmos-may ('open-query':U).
          IF h_t-pedmos-mayb <> ? THEN RUN dispatch IN h_t-pedmos-mayb ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-pedmos-may <> ? THEN RUN Recalcular-Precios IN h_t-pedmos-may.
