...
IF TRUE <> (pTipoVenta > "") THEN pTipoVenta = '000'.

/*&SCOPED-DEFINE Precio-de-Venta pri/p-precio-mayor-credito*/
/* &SCOPED-DEFINE precio-de-Venta pri/PrecioVentaMayorCredito */
&SCOPED-DEFINE precio-de-Venta web/PrecioFinalContadoMayorista  /* Princing Set2022*/
...
DEFINE TEMP-TABLE ttListaPreciosEventosPromocional
    ...
    FIELDS  tPrecLista  AS  DEC     FORMAT '->>,>>9.9999'   COLUMN-LABEL "Precio Peldaño Bruto"
    ...
    FIELDS  tmsg    AS  CHAR     FORMAT 'x(255)'     COLUMN-LABEL "Mensaje"
    INDEX idx01 tDesmat
    .
...
COMBO-BOX-familia COMBO-BOX-subfamilia BUTTON-1 COMBO-BOX-condvtas ~
COMBO-BOX-ClasificacionCliente BUTTON-3 RADIO-SET-calculo ~
...
COMBO-BOX-subfamilia COMBO-BOX-condvtas COMBO-BOX-ClasificacionCliente ~
RADIO-SET-calculo RADIO-SET-tipocliente RADIO-SET-proveedor ~
FILL-IN-proveedor FILL-IN-nomprovee FILL-IN-ruta 
...
DEFINE VARIABLE COMBO-BOX-ClasificacionCliente-2 AS CHARACTER FORMAT "X(5)":U 
...
ttListaPreciosEventosPromocional.tmsg    FORMAT 'x(100)'     COLUMN-LABEL "Observa"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
ttListaPreciosEventosVolumen.tmsg    FORMAT 'x(100)'     COLUMN-LABEL "Observa"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     COMBO-BOX-listaprecios AT ROW 1.58 COL 16 COLON-ALIGNED WIDGET-ID 4
     COMBO-BOX-ClasificacionCliente-2 AT ROW 1.58 COL 88 COLON-ALIGNED WIDGET-ID 40
     COMBO-BOX-familia AT ROW 2.92 COL 16 COLON-ALIGNED WIDGET-ID 6
     COMBO-BOX-subfamilia AT ROW 2.92 COL 65 COLON-ALIGNED WIDGET-ID 8
     BUTTON-1 AT ROW 3.12 COL 115 WIDGET-ID 10
     COMBO-BOX-condvtas AT ROW 4.08 COL 47 COLON-ALIGNED WIDGET-ID 42
     COMBO-BOX-ClasificacionCliente AT ROW 4.12 COL 16 COLON-ALIGNED WIDGET-ID 16
     BUTTON-3 AT ROW 4.65 COL 115 WIDGET-ID 38
     RADIO-SET-calculo AT ROW 5.23 COL 17 NO-LABEL WIDGET-ID 18
     RADIO-SET-tipocliente AT ROW 5.62 COL 87 NO-LABEL WIDGET-ID 12
...
/* SETTINGS FOR COMBO-BOX COMBO-BOX-ClasificacionCliente-2 IN FRAME F-Main
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       COMBO-BOX-ClasificacionCliente-2:HIDDEN IN FRAME F-Main           = TRUE.
...
/* SETTINGS FOR RADIO-SET RADIO-SET-tipocliente IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       RADIO-SET-tipocliente:HIDDEN IN FRAME F-Main           = TRUE.
...
&Scoped-define SELF-NAME COMBO-BOX-ClasificacionCliente-2
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL COMBO-BOX-ClasificacionCliente-2 W-Win
ON VALUE-CHANGED OF COMBO-BOX-ClasificacionCliente-2 IN FRAME F-Main /* Clasif. Cliente */
DO:
  RUN cargar-subfamilia(INPUT COMBO-BOX-familia:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY COMBO-BOX-listaprecios COMBO-BOX-familia COMBO-BOX-subfamilia 
          COMBO-BOX-condvtas COMBO-BOX-ClasificacionCliente RADIO-SET-calculo 
          RADIO-SET-tipocliente RADIO-SET-proveedor FILL-IN-proveedor 
          FILL-IN-nomprovee FILL-IN-ruta 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-1 COMBO-BOX-listaprecios COMBO-BOX-familia COMBO-BOX-subfamilia 
         BUTTON-1 COMBO-BOX-condvtas COMBO-BOX-ClasificacionCliente BUTTON-3 
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    COMBO-BOX-condvtas:DELIMITER = "|".
      ...
      FOR EACH ClfClie NO-LOCK:
          IF pTipoVenta = 'MOSTRADOR' THEN DO:
...
x-codclie = IF(pTipoVenta <> "MOSTRADOR") THEN "00000000100" ELSE "11111111111".
        ...
        IF radio-set-calculo <> 1 THEN DO:
            ...
            REPEAT x-sec = 1 TO 10:
                IF almmmatg.DtoVolR[x-sec] > 0 THEN DO:
                    ...
                    RUN {&Precio-de-Venta} (INPUT s-codcia, 
                    ...
                    IF X-TIPDTO <> "VOL" THEN DO:
                        ...
                        IF x-sec = 1 THEN tPrecFin01 = if(y-dsctos <> 0) THEN x-prevta * ( 1 - y-dsctos / 100) ELSE x-prevta.
                        ...
                        IF x-sec = 2 THEN tPrecFin02 = if(y-dsctos <> 0) THEN x-prevta * ( 1 - y-dsctos / 100) ELSE x-prevta.
                        ...
                        IF x-sec = 3 THEN tPrecFin03 = if(y-dsctos <> 0) THEN x-prevta * ( 1 - y-dsctos / 100) ELSE x-prevta.
                        ...
                        IF x-sec = 4 THEN tPrecFin04 = if(y-dsctos <> 0) THEN x-prevta * ( 1 - y-dsctos / 100) ELSE x-prevta.
                        ...
                        IF x-sec = 5 THEN tPrecFin05 = if(y-dsctos <> 0) THEN x-prevta * ( 1 - y-dsctos / 100) ELSE x-prevta.
                        ...
                        IF x-sec = 6 THEN tPrecFin06 = if(y-dsctos <> 0) THEN x-prevta * ( 1 - y-dsctos / 100) ELSE x-prevta.
                        ...
                        IF x-sec = 7 THEN tPrecFin07 = if(y-dsctos <> 0) THEN x-prevta * ( 1 - y-dsctos / 100) ELSE x-prevta.
                        ...
                        IF x-sec = 8 THEN tPrecFin08 = if(y-dsctos <> 0) THEN x-prevta * ( 1 - y-dsctos / 100) ELSE x-prevta.
                        ...
                        IF x-sec = 9 THEN tPrecFin09 = if(y-dsctos <> 0) THEN x-prevta * ( 1 - y-dsctos / 100) ELSE x-prevta.
                        ...
                        IF x-sec = 10 THEN tPrecFin10 = if(y-dsctos <> 0) THEN x-prevta * ( 1 - y-dsctos / 100) ELSE x-prevta.
