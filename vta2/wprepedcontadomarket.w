...
DEFINE VARIABLE h_tprepedcontadomarket AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vprepedcontadomarket AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/vprepedcontadomarket.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vprepedcontadomarket ).
       RUN set-position IN h_vprepedcontadomarket ( 2.73 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.12 , 123.00 ) */
       ...
       /* Links to SmartViewer h_vprepedcontadomarket. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_vprepedcontadomarket ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_vprepedcontadomarket ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-pedmos-utilex ( 6.19 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/tprepedcontadomarket.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tprepedcontadomarket ).
       RUN set-position IN h_tprepedcontadomarket ( 6.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tprepedcontadomarket ( 14.23 , 141.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 20.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.54 , 51.00 ) NO-ERROR.

       /* Links to SmartBrowser h_tprepedcontadomarket. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tprepedcontadomarket ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tprepedcontadomarket ,
             h_vprepedcontadomarket , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tprepedcontadomarket , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_tprepedcontadomarket <> ? THEN RUN dispatch IN h_tprepedcontadomarket ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_vprepedcontadomarket <> ? THEN RUN Recalcular-Precios IN h_vprepedcontadomarket.
      END.
