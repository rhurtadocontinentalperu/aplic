...
DO:
    IF DECIMAL(SELF:SCREEN-VALUE) = 0 THEN RETURN.
    IF VtaCMatriz.FlgMinimo = NO OR T-DMatriz.Minimo[7] = 0 THEN RETURN.
    FIND Almmmatg WHERE Almmmatg.codcia = s-codcia
        AND Almmmatg.codmat = T-DMatriz.CodMat[7] 
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almmmatg THEN RETURN.
    ...
    DEF VAR x-Factor AS DEC NO-UNDO.
    FIND Almtconv WHERE Almtconv.CodUnid  = Almmmatg.UndBas 
        AND Almtconv.Codalter = T-DMatriz.UndVta[7]
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almtconv THEN RETURN.
    x-FACTOR = Almtconv.Equival / Almmmatg.FacEqu.
    IF DECIMAL(SELF:SCREEN-VALUE) * x-Factor < T-DMatriz.Minimo[7] THEN DO:
        MESSAGE 'ERROR EL EL PRODUCTO:' Almmmatg.desmat SKIP(1)
            'La venta es de ' SELF:SCREEN-VALUE T-DMatriz.UndVta[7] SKIP
            'NO se puede vender menos de' T-DMatriz.Minimo[7] Almmmatg.UndBas
            VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    FILL-IN-DesMat:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL T-DMatriz.Cantidad[7] br_table _BROWSE-COLUMN B-table-Win
ON LEFT-MOUSE-DBLCLICK OF T-DMatriz.Cantidad[7] IN BROWSE br_table
DO:
    /****    Selecciona las unidades de medida   ****/
    RUN vtagn/c-uniabc ("Unidades de Venta",
                        T-DMatriz.CodMat[7],
    ...
    IF output-var-1 <> ? THEN 
        T-DMatriz.UndVta[7]:SCREEN-VALUE IN BROWSE {&browse-name} = output-var-2.
...
DO:
    IF DECIMAL(SELF:SCREEN-VALUE) = 0 THEN RETURN.
    IF VtaCMatriz.FlgMinimo = NO OR T-DMatriz.Minimo[8] = 0 THEN RETURN.
    FIND Almmmatg WHERE Almmmatg.codcia = s-codcia
        AND Almmmatg.codmat = T-DMatriz.CodMat[8] 
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almmmatg THEN RETURN.
    ...
    DEF VAR x-Factor AS DEC NO-UNDO.
    FIND Almtconv WHERE Almtconv.CodUnid  = Almmmatg.UndBas 
        AND Almtconv.Codalter = T-DMatriz.UndVta[8]
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almtconv THEN RETURN.
    x-FACTOR = Almtconv.Equival / Almmmatg.FacEqu.
    IF DECIMAL(SELF:SCREEN-VALUE) * x-Factor < T-DMatriz.Minimo[8] THEN DO:
        MESSAGE 'ERROR EL EL PRODUCTO:' Almmmatg.desmat SKIP(1)
            'La venta es de ' SELF:SCREEN-VALUE T-DMatriz.UndVta[8] SKIP
            'NO se puede vender menos de' T-DMatriz.Minimo[8] Almmmatg.UndBas
            VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    FILL-IN-DesMat:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL T-DMatriz.Cantidad[8] br_table _BROWSE-COLUMN B-table-Win
ON LEFT-MOUSE-DBLCLICK OF T-DMatriz.Cantidad[8] IN BROWSE br_table
DO:
    /****    Selecciona las unidades de medida   ****/
    RUN vtagn/c-uniabc ("Unidades de Venta",
                        T-DMatriz.CodMat[8],
    ...
    IF output-var-1 <> ? THEN 
        T-DMatriz.UndVta[8]:SCREEN-VALUE IN BROWSE {&browse-name} = output-var-2.
...
DO:
    IF DECIMAL(SELF:SCREEN-VALUE) = 0 THEN RETURN.
    IF VtaCMatriz.FlgMinimo = NO OR T-DMatriz.Minimo[9] = 0 THEN RETURN.
    FIND Almmmatg WHERE Almmmatg.codcia = s-codcia
        AND Almmmatg.codmat = T-DMatriz.CodMat[9] 
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almmmatg THEN RETURN.
    ...
    DEF VAR x-Factor AS DEC NO-UNDO.
    FIND Almtconv WHERE Almtconv.CodUnid  = Almmmatg.UndBas 
        AND Almtconv.Codalter = T-DMatriz.UndVta[9]
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almtconv THEN RETURN.
    x-FACTOR = Almtconv.Equival / Almmmatg.FacEqu.
    IF DECIMAL(SELF:SCREEN-VALUE) * x-Factor < T-DMatriz.Minimo[9] THEN DO:
        MESSAGE 'ERROR EL EL PRODUCTO:' Almmmatg.desmat SKIP(1)
            'La venta es de ' SELF:SCREEN-VALUE T-DMatriz.UndVta[9] SKIP
            'NO se puede vender menos de' T-DMatriz.Minimo[9] Almmmatg.UndBas
            VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    FILL-IN-DesMat:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL T-DMatriz.Cantidad[9] br_table _BROWSE-COLUMN B-table-Win
ON LEFT-MOUSE-DBLCLICK OF T-DMatriz.Cantidad[9] IN BROWSE br_table
DO:
    /****    Selecciona las unidades de medida   ****/
    RUN vtagn/c-uniabc ("Unidades de Venta",
                        T-DMatriz.CodMat[9],
    ...
    IF output-var-1 <> ? THEN 
        T-DMatriz.UndVta[9]:SCREEN-VALUE IN BROWSE {&browse-name} = output-var-2.
...
DO:
    IF DECIMAL(SELF:SCREEN-VALUE) = 0 THEN RETURN.
    IF VtaCMatriz.FlgMinimo = NO OR T-DMatriz.Minimo[10] = 0 THEN RETURN.
    FIND Almmmatg WHERE Almmmatg.codcia = s-codcia
        AND Almmmatg.codmat = T-DMatriz.CodMat[10] 
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almmmatg THEN RETURN.
    ...
    DEF VAR x-Factor AS DEC NO-UNDO.
    FIND Almtconv WHERE Almtconv.CodUnid  = Almmmatg.UndBas 
        AND Almtconv.Codalter = T-DMatriz.UndVta[10]
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almtconv THEN RETURN.
    x-FACTOR = Almtconv.Equival / Almmmatg.FacEqu.
    IF DECIMAL(SELF:SCREEN-VALUE) * x-Factor < T-DMatriz.Minimo[10] THEN DO:
        MESSAGE 'ERROR EL EL PRODUCTO:' Almmmatg.desmat SKIP(1)
            'La venta es de ' SELF:SCREEN-VALUE T-DMatriz.UndVta[10] SKIP
            'NO se puede vender menos de' T-DMatriz.Minimo[10] Almmmatg.UndBas
            VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    FILL-IN-DesMat:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL T-DMatriz.Cantidad[10] br_table _BROWSE-COLUMN B-table-Win
ON LEFT-MOUSE-DBLCLICK OF T-DMatriz.Cantidad[10] IN BROWSE br_table
DO:
    /****    Selecciona las unidades de medida   ****/
    RUN vtagn/c-uniabc ("Unidades de Venta",
                        T-DMatriz.CodMat[10],
    ...
    IF output-var-1 <> ? THEN 
        T-DMatriz.UndVta[10]:SCREEN-VALUE IN BROWSE {&browse-name} = output-var-2.
...
DO:
    IF DECIMAL(SELF:SCREEN-VALUE) = 0 THEN RETURN.
    IF VtaCMatriz.FlgMinimo = NO OR T-DMatriz.Minimo[11] = 0 THEN RETURN.
    FIND Almmmatg WHERE Almmmatg.codcia = s-codcia
        AND Almmmatg.codmat = T-DMatriz.CodMat[11] 
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almmmatg THEN RETURN.
    ...
    DEF VAR x-Factor AS DEC NO-UNDO.
    FIND Almtconv WHERE Almtconv.CodUnid  = Almmmatg.UndBas 
        AND Almtconv.Codalter = T-DMatriz.UndVta[11]
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almtconv THEN RETURN.
    x-FACTOR = Almtconv.Equival / Almmmatg.FacEqu.
    IF DECIMAL(SELF:SCREEN-VALUE) * x-Factor < T-DMatriz.Minimo[11] THEN DO:
        MESSAGE 'ERROR EL EL PRODUCTO:' Almmmatg.desmat SKIP(1)
            'La venta es de ' SELF:SCREEN-VALUE T-DMatriz.UndVta[11] SKIP
            'NO se puede vender menos de' T-DMatriz.Minimo[11] Almmmatg.UndBas
            VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    FILL-IN-DesMat:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL T-DMatriz.Cantidad[11] br_table _BROWSE-COLUMN B-table-Win
ON LEFT-MOUSE-DBLCLICK OF T-DMatriz.Cantidad[11] IN BROWSE br_table
DO:
    /****    Selecciona las unidades de medida   ****/
    RUN vtagn/c-uniabc ("Unidades de Venta",
                        T-DMatriz.CodMat[11],
    ...
    IF output-var-1 <> ? THEN 
        T-DMatriz.UndVta[11]:SCREEN-VALUE IN BROWSE {&browse-name} = output-var-2.
...
DO:
    IF DECIMAL(SELF:SCREEN-VALUE) = 0 THEN RETURN.
    IF VtaCMatriz.FlgMinimo = NO OR T-DMatriz.Minimo[12] = 0 THEN RETURN.
    FIND Almmmatg WHERE Almmmatg.codcia = s-codcia
        AND Almmmatg.codmat = T-DMatriz.CodMat[12] 
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almmmatg THEN RETURN.
    ...
    DEF VAR x-Factor AS DEC NO-UNDO.
    FIND Almtconv WHERE Almtconv.CodUnid  = Almmmatg.UndBas 
        AND Almtconv.Codalter = T-DMatriz.UndVta[12]
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almtconv THEN RETURN.
    x-FACTOR = Almtconv.Equival / Almmmatg.FacEqu.
    IF DECIMAL(SELF:SCREEN-VALUE) * x-Factor < T-DMatriz.Minimo[12] THEN DO:
        MESSAGE 'ERROR EL EL PRODUCTO:' Almmmatg.desmat SKIP(1)
            'La venta es de ' SELF:SCREEN-VALUE T-DMatriz.UndVta[12] SKIP
            'NO se puede vender menos de' T-DMatriz.Minimo[12] Almmmatg.UndBas
            VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    FILL-IN-DesMat:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL T-DMatriz.Cantidad[12] br_table _BROWSE-COLUMN B-table-Win
ON LEFT-MOUSE-DBLCLICK OF T-DMatriz.Cantidad[12] IN BROWSE br_table
DO:
    /****    Selecciona las unidades de medida   ****/
    RUN vtagn/c-uniabc ("Unidades de Venta",
                        T-DMatriz.CodMat[12],
    ...
    IF output-var-1 <> ? THEN 
        T-DMatriz.UndVta[12]:SCREEN-VALUE IN BROWSE {&browse-name} = output-var-2.
