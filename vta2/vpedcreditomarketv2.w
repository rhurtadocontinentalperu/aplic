...
DEF VAR s-Carga-Promocion AS LOG NO-UNDO.
...
FacCPedi.FlgIgv FacCPedi.CodVen FacCPedi.NroRef FacCPedi.FmaPgo ~
FacCPedi.ordcmp FacCPedi.CodAlm FacCPedi.Glosa 
...
FacCPedi.usuario FacCPedi.Glosa FacCPedi.TpoLic 
&Scoped-define DISPLAYED-TABLES FacCPedi
&Scoped-define FIRST-DISPLAYED-TABLE FacCPedi
&Scoped-Define DISPLAYED-OBJECTS F-Estado FILL-IN-Lista F-Nomtar ~
FILL-IN-sede f-NomVen F-CndVta f-NomAlm 
...
DEFINE FRAME F-Main
     ...
     FacCPedi.CodAlm AT ROW 8.27 COL 16 COLON-ALIGNED WIDGET-ID 110
          ...
          SIZE 5 BY .81
          BGCOLOR 8 FGCOLOR 0 
     f-NomAlm AT ROW 8.31 COL 22 COLON-ALIGNED NO-LABEL WIDGET-ID 112
     FacCPedi.usuario AT ROW 8.31 COL 99 COLON-ALIGNED WIDGET-ID 34
          LABEL "Digitado por"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE x-Empaque AS DEC NO-UNDO.
  DEFINE VARIABLE x-PorAtender AS DEC NO-UNDO.
  ...
  FOR EACH Facdpedi OF B-CPEDI NO-LOCK WHERE (Facdpedi.CanPed - Facdpedi.CanAte) > 0,
      ...
      x-PorAtender = (FacDPedi.CanPed - FacDPedi.CanAte).
      ALMACENES:
      DO i = 1 TO NUM-ENTRIES(s-CodAlm):    /* 65s y 65 en eses orden */
          f-CanPed = x-PorAtender.
...
/*           RUN vta2/Stock-Comprometido (Facdpedi.CodMat, x-CodAlm, OUTPUT s-StkComprometido). */
          ...
          END.
          IF x-CodAlm = "65s" THEN x-Empaque = (IF Almmmatg.StkRep > 0 THEN Almmmatg.StkRep ELSE x-Empaque).
          f-CanPed = f-CanPed * f-Factor.
          f-CanPed = (TRUNCATE((f-CanPed / x-Empaque),0) * x-Empaque).
          ...
          x-PorAtender = x-PorAtender - f-CanPed.
          IF x-PorAtender <= 0 THEN LEAVE.
      END.
  END.
...
DO TRANSACTION ON ERROR UNDO, RETURN ON STOP UNDO, RETURN:
    ...
    IF Faccpedi.FlgEst = "X" THEN DO:
        pMensaje = "NO pasó línea de crédito".
        LEAVE.
    END.
...
------------------------------------------------------------------------------*/
  ...
  IF s-Carga-Promocion = YES THEN DO:
  ...
  DETALLE:
  FOR EACH PEDI, FIRST Almmmatg OF PEDI NO-LOCK BREAK BY PEDI.AlmDes BY PEDI.NroItm: 
...
/*       RUN vta2/Stock-Comprometido (PEDI.CodMat, PEDI.AlmDes, OUTPUT s-StkComprometido). */
      ...
      x-Empaque = 1.
      IF s-StkDis < x-CanPed THEN DO:
          IF PEDI.AlmDes = '65s' THEN x-Empaque = (IF Almmmatg.StkRep > 0 THEN Almmmatg.StkRep ELSE x-Empaque).
          ...
          /* EMPAQUE */
          x-CanPed = PEDI.CanPed * f-Factor.
          x-CanPed = (TRUNCATE((x-CanPed / x-Empaque),0) * x-Empaque).
      ...
      ASSIGN
          Faccpedi.CodAlm = PEDI.AlmDes.    /* OJO */
      IF LAST-OF(PEDI.AlmDes) THEN DO:
          DELETE PEDI.
          LEAVE DETALLE.
      END.
      DELETE PEDI.
...
------------------------------------------------------------------------------*/
  ...
  RUN vta2/dcotpendcredmarketv2 (OUTPUT s-NroCot).
  IF s-NroCot = "" THEN RETURN "ADM-ERROR".
  /* RHC 26/08/2015
    Del alamcén 65S solo se puede sacar mercadería por empaques cerrado
    Del almacén 65 se puede sacar al menudeo
    */
  s-CodAlm = "65s,65".
...
------------------------------------------------------------------------------*/
  ...
  /* RHC 26/08/2015 Grabamos tantos PED coo almacenes de despacho hay */
  REPEAT:
      FIND FIRST PEDI NO-LOCK NO-ERROR.
      ...
      FIND Faccpedi WHERE ROWID(Faccpedi) = ROWID(B-CPEDI) EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
      IF ERROR-STATUS:ERROR THEN DO:
          pMensaje = "No se pudo generar mas de un pedido"  + CHR(10) +
              "Proceso abortado".
          UNDO, RETURN "ADM-ERROR".
      END.
      ASSIGN
...
/*         IF RETURN-VALUE = 'ADM-ERROR' THEN DO:                                                                                */
/*             MESSAGE "NO se pudo actualizar el pedido" VIEW-AS ALERT-BOX ERROR.                                                */
/*             UNDO, RETURN 'ADM-ERROR'.                                                                                         */
/*         END.                                                                                                                  */
/*                                                                                                                               */
/*         ASSIGN                                                                                                                */
...
/*     END.                                                         */
    IF gn-clie.flagaut = "R" THEN DO:
        ...
        RETURN 'ADM-ERROR'.
    END.
    IF /*LOOKUP(FacCPedi.FmaPgo:SCREEN-VALUE, '000,001,002,900') = 0
        AND*/ gn-clie.flagaut = "" THEN DO:
        MESSAGE 'Su Línea de Crédito aún NO ha sido Autorizada'
            VIEW-AS ALERT-BOX WARNING.
/*         APPLY "ENTRY" TO FacCPedi.Glosa. */
/*         RETURN 'ADM-ERROR'.              */
    END.
