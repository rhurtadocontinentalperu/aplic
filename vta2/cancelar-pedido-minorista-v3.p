...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    RUN vta2/d-cancelar-pedido-minorista-v3 (
        ...
        ROWID(B-CPEDM),     /* Puntero del Pedido Mostrador */
    ...
    /* RHC 15/04/2015 ACTUALIZA CONTROL DE PUNTOS - ARTE */
    RUN Actualiza-Puntos-Arte.
    IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
    /* ************************************************* */
...
FOR EACH T-CDOCU NO-LOCK BY T-CDOCU.NroPed:         
    ...
    IF AVAILABLE CcbCDocu THEN DO:
        IF Ccbcdocu.CodDoc = "FAC" THEN DO:                
            RUN vtamin/r-tick500-caja (ROWID(ccbcdocu), "O").
        END.
        IF Ccbcdocu.CodDoc = "TCK" THEN RUN vtamin/r-tick500-caja (ROWID(ccbcdocu), "O").
    END.
END.
...
&IF DEFINED(EXCLUDE-Actualiza-Puntos-Arte) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Actualiza-Puntos-Arte Procedure 
PROCEDURE Actualiza-Puntos-Arte :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* 1ro Nos aseguramos que esté registrado */
FIND BaseArte WHERE BaseArte.CodCia = s-codcia
    AND BaseArte.CodDiv = B-CPEDM.coddiv
    AND BaseArte.NroDocumento = B-CPEDM.atencion
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE BaseArte THEN RETURN 'OK'.

/* Bloqueamos registro */
{lib/lock-genericov2.i &Tabla="BaseArte" ~
    &Condicion="BaseArte.CodCia = s-codcia ~
    AND BaseArte.CodDiv = B-CPEDM.coddiv ~
    AND BaseArte.NroDocumento = B-CPEDM.atencion" ~
...
/* 2do. DESCARGAMOS CONSUMO DE PUNTOS */
DEF VAR fPorRepartir AS DEC NO-UNDO.
DEF VAR fPorAsignar AS DEC NO-UNDO.
...
IF AVAILABLE Ccbccaja AND CcbCCaja.PuntosNac > 0 THEN DO:
    /* repartimos puntos consumidos */
    ASSIGN 
        BaseArte.SaldoPuntos = BaseArte.SaldoPuntos - CcbCCaja.PuntosNac
        fPorRepartir = CcbCCaja.PuntosNac.
    FOR EACH PuntosArte OF BaseArte WHERE (PuntosArte.QttyPtosAsignados - PuntosArte.QttyPtosConsumidos) > 0
        BY PuntosArte.FechaHora:
        ASSIGN 
            fPorAsignar = MINIMUM(PuntosArte.QttyPtosAsignados - PuntosArte.QttyPtosConsumidos, fPorRepartir)
            PuntosArte.QttyPtosConsumidos = PuntosArte.QttyPtosConsumidos + fPorAsignar
            fPorRepartir = fPorRepartir - fPorAsignar.
        IF fPorRepartir <= 0 THEN LEAVE.
    END.
END.
/* ***************************** */
/* 3ro. FINALMENTE: Cargamos puntos por la compra actual */
DEF VAR fPuntosArte AS DEC INIT 0 NO-UNDO.
DEF VAR fImporteArte AS DEC INIT 0 NO-UNDO.

/* Solo se carga puntos a partir de la segunda compra */
IF BaseArte.SwPrimeraCompra = NO THEN DO:
    FOR EACH Facdpedi OF B-CPEDM NO-LOCK, FIRST Almmmatg OF Facdpedi WHERE Almmmatg.codfam = '014':
        /*fImporteArte = fImporteArte + (Facdpedi.ImpLin - Facdpedi.ImpDto2).*/
        fImporteArte = fImporteArte + Facdpedi.ImpLin.
    END.
    /* Por cada 20 soles de compra se carga un punto */
    fPuntosArte = TRUNCATE(fImporteArte / 20, 0).
    IF fPuntosArte > 0 THEN DO:
        FIND FIRST Ccbdcaja WHERE Ccbdcaja.codcia = s-codcia
            AND Ccbdcaja.coddoc = s-CodCja
            AND Ccbdcaja.nrodoc = NroDocCja
            NO-LOCK NO-ERROR.
        IF AVAILABLE Ccbdcaja THEN DO:
            CREATE PuntosArte.
            BUFFER-COPY BaseArte TO PuntosArte.
            ASSIGN
                PuntosArte.CodCja = s-CodCja
                PuntosArte.NroCja = NroDocCja
                PuntosArte.CodDoc = Ccbdcaja.codref
                PuntosArte.NroDoc = Ccbdcaja.nroref
                PuntosArte.FchDoc = Ccbdcaja.fchdoc
                PuntosArte.ImpTot = Ccbdcaja.imptot
                PuntosArte.FechaHora = DATETIME(TODAY, MTIME)
                PuntosArte.ImpArte = fImporteArte
                PuntosArte.QttyPtosAsignados = fPuntosArte
                PuntosArte.SwPrimeraCompra = NO.
            ASSIGN
                BaseArte.SaldoPuntos = BaseArte.SaldoPuntos + fPuntosArte.
        END.
    END.
END.
/* 4to. Cerramos */
ASSIGN 
    BaseArte.SwPrimeraCompra = NO
    BaseArte.FechaUltCompra = DATETIME(TODAY, MTIME).
...
------------------------------------------------------------------------------*/
    ...
    DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
        ...
        ASSIGN
            ...
            CcbCCaja.FLGEST     = "C"
            CcbCCaja.PuntosNac  = T-CcbCCaja.PuntosNac 
            CcbCCaja.PuntosUsa  = T-CcbCCaja.PuntosUsa.
