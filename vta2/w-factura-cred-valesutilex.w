...
/* ********************************************************************************** */
    ...
    DEFINE NEW SHARED VAR s-CodAlm AS CHAR INIT '11'.
    DEFINE NEW SHARED VAR s-CodCia AS INT INIT 1.
    DEFINE NEW SHARED VAR S-DESALM AS CHAR .
    ...
    IF NOT AVAILABLE Almacen OR NOT CAN-FIND(gn-divi WHERE gn-divi.codcia = s-codcia
                                             ...
                                             NO-LOCK)
        THEN DO:
        MESSAGE 'Código del almacén:' s-codalm 'mal configurado'
            VIEW-AS ALERT-BOX ERROR.
        RETURN ERROR.
    END.
    ASSIGN s-DesAlm = Almacen.Descripcion.
    /* Definimos la división de trabajo en base al almacén seleccionado */
    ...
    IF NOT AVAILABLE gn-divi THEN DO:
        MESSAGE 'Código de división:' s-coddiv 'del almacén:' s-codalm
            'MAL configurado'
            VIEW-AS ALERT-BOX ERROR.
        RETURN ERROR.
    END.
...
DEFINE NEW SHARED VARIABLE S-CODDOC   AS CHAR INITIAL "FAI".
DEFINE NEW SHARED VARIABLE S-TPOFAC   AS CHAR INITIAL "CR".     /* CREDITO */
...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 RECT-66 RECT-67 BUTTON-KITS ~
BUTTON-8 BUTTON-9 BUTTON-11 BtnDone C-NroSer 
...
DEFINE VARIABLE C-CodDoc AS CHARACTER FORMAT "X(256)":U INITIAL "FAI" 
     LABEL "Comprobante" 
...
DO:
   RUN dispatch IN h_v-factura-cred ('delete-record':U).
END.
...
DO:
  RUN dispatch IN h_v-factura-cred ('imprime':U).
END.
...
DO:
   RUN Anexo-Kits IN h_v-factura-cred.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-factura-cred.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-factura-cred ).
       RUN set-position IN h_v-factura-cred ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.27 , 117.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-factura-cred.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-factura-cred ).
       RUN set-position IN h_b-factura-cred ( 10.15 , 2.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-factura-cred ).

       /* Links to SmartBrowser h_b-factura-cred. */
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_b-factura-cred ).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-1 RECT-66 RECT-67 BUTTON-KITS BUTTON-8 BUTTON-9 BUTTON-11 
         BtnDone C-NroSer 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-factura-cred ('open-query':U).
      END.
END CASE.
