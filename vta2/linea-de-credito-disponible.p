...
END.
END.
/* IF AVAILABLE gn-convt AND gn-convt.tipvta = "1" AND             */
/*     gn-convt.libre_l01 = NO AND gn-convt.libre_l02 = NO AND     */
/*     gn-convt.libre_l03  = NO THEN DO:       /* CONTADO */       */
/*     /* contado y no es fai, ni venta unica ni anticipo (a/c) */ */
/*     RETURN 'OK'.                                                */
/* END.                                                            */
...
DEF VAR dTolerancia AS DEC NO-UNDO.
DEF VAR disponibleLC AS DEC NO-UNDO.
...
/* si no es a/c y no LC no va */
/* si es a/c y no tiene saldo de a/c no va */
RUN ccb/p-consumo-linea-credito.r (
    pCodCli,
    pCodDiv,
    f-MonLC,
    OUTPUT f-Deuda
...
dTolerancia = 10.   /* default */
FIND FIRST factabla WHERE factabla.codcia = 1 AND 
            factabla.tabla = 'AFECTA_LINEACREDITO' AND
            factabla.codigo = 'EXCEDENTE_PERMITIDO' NO-LOCK NO-ERROR.
IF AVAILABLE factabla THEN dTolerancia = factabla.valor[1].
...
dTolerancia = 1 + (dTolerancia / 100).
disponibleLC = (f-ImpLC * dTolerancia) - f-deuda.
IF disponibleLC < 0 THEN disponibleLC = 0.

IF (disponibleLC - pImporte) < 0 THEN DO:
