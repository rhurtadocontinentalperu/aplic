...
&Scoped-define QUERY-STRING-br_table FOR EACH Almtfami WHERE ~{&KEY-PHRASE} ~
 ...
 AND LOOKUP(Almtfami.codfam, "013,017") > 0 NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almtfami.desfam FORMAT "X(30)":U WIDTH 35.86
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 46 BY 11.35
         FONT 4
         TITLE "Familias" FIT-LAST-COLUMN.
...
/* Query rebuild information for BROWSE br_table
 ...
 AND Almtfami.SwComercial = TRUE
 AND LOOKUP(Almtfami.codfam, ""013,017"") > 0"
     _FldNameList[1]   = INTEGRAL.Almtfami.codfam
     _FldNameList[2]   > INTEGRAL.Almtfami.desfam
"Almtfami.desfam" ? ? "character" ? ? ? ? ? ? no ? no no "35.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  RUN Procesa-Handle IN lh_handle ('Open-Query').
END.
