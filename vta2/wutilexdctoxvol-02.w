...
&Scoped-Define ENABLED-OBJECTS RECT-3 BtnDone BUTTON-1 BUTTON-2 
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/vutilexdctoxvol-02.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_vutilexdctoxvol-02 ).
...
IF NOT (cValue BEGINS "CONTINENTAL - DESCUENTOS POR VOLUMEN"
        ...
        OR cValue BEGINS s-coddiv + " - DESCUENTOS POR VOLUMEN")
    THEN DO:
    MESSAGE 'Formato del archivo Excel errado' VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.
...
CASE TRUE:
    ...
    WHEN cNombreLista BEGINS s-coddiv + " - DESCUENTOS POR VOLUMEN" THEN RADIO-SET-1 = 3.
    OTHERWISE RETURN.
END CASE.
CASE TRUE:
...
REPEAT:
    ...
    DO iCountLine = 1 TO 5:
        ...
        IF x-DtoVolR = ? OR x-DtoVolR = 0 OR x-DtoVolD = ? OR x-DtoVolD = 0 THEN NEXT.
        ASSIGN
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-3 BtnDone BUTTON-1 BUTTON-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE RADIO-SET-1:
    ...
    WHEN 3 THEN DO:
            ...
            DO k = 1 TO 10:
                IF T-MATG.DtoVolD[k] <> 0 THEN DO:
                    RUN vtagn/p-margen-utilidad (
                        T-MATG.CodMat,
                        T-MATG.PreVta[1] * ( 1 - (T-MATG.DtoVolD[k] / 100) ),
                        T-MATG.CHR__01,
                        1,                      /* Moneda */
                        T-MATG.TpoCmb,
                        NO,                     /* Muestra error? */
        ...
        FOR EACH T-MATG ON STOP UNDO, RETURN ON ERROR UNDO, RETURN:
            FIND VtaListaMay OF T-MATG WHERE VtaListaMay.coddiv = s-coddiv
