...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      DOCU.NroRef FORMAT "X(12)":U WIDTH 9.57
...
"DOCU.Libre_d01" "Importe N/C" ">>>,>>>,>>9.99" "decimal" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF NOT AVAILABLE Gn-tccja THEN DO:
    MESSAGE 'NO se ha registrado el T.C. de Caja' VIEW-AS ALERT-BOX ERROR.
    RETURN "ADM-ERROR".
END.
ASSIGN
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    BUFFER-COPY Ccbcdocu
        ...
        B-NC.tpofac = "ADELANTO"
        B-NC.Tipo   = "CREDITO"     /* SUNAT */
        B-NC.CodCaja= ""
        B-NC.CodCta = pConcepto
        NO-ERROR.
    IF ERROR-STATUS:ERROR THEN DO:
        MESSAGE 'Error al grabar la N/C:' STRING(FacCorre.nroser, '999') STRING(Faccorre.correlativo, ENTRY(2,x-Formato,'-'))
  ...
  /* **************************** */
    ...
    &IF {&ARITMETICA-SUNAT} &THEN
        ...
        RUN tabla-ccbcdocu IN hProc (INPUT B-NC.CodDiv,
                                     INPUT B-NC.CodDoc,
                                     INPUT B-NC.NroDoc,
                                     OUTPUT pMensaje).
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       LA N/C SERÁ APROBADA EN UN PROCESO POSTERIOR
------------------------------------------------------------------------------*/
...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    ...
    IF Ccbcdocu.FlgEst <> "P" THEN DO:
        MESSAGE 'El' Ccbcdocu.coddoc Ccbcdocu.nrodoc 'ya NO está pendiente'
            VIEW-AS ALERT-BOX ERROR.
        UNDO, RETURN 'ADM-ERROR'.
    END.
    RUN FIRST-TRANSACTION (INPUT pCodDoc, INPUT pNroSer, INPUT pConcepto, OUTPUT pRowid).
    IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
    RUN SECOND-TRANSACTION (INPUT pRowid).
