...
/* Variable que permite seleccionar o no divisiones */
DEFINE NEW SHARED VARIABLE S-TIPO     AS CHAR.
s-Tipo = pParametro.
IF LOOKUP(s-Tipo, 'YES,NO') = 0 THEN s-Tipo = 'NO'.
...
&Scoped-Define ENABLED-OBJECTS btlTrackExcel 
...
DEFINE VARIABLE h_bconpedcred AS HANDLE NO-UNDO.
DEFINE VARIABLE h_tab95 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON btlTrackExcel 
     LABEL "Enviar a Excel el Tracking" 
     SIZE 25 BY 1.12.
...
DEFINE FRAME F-Main
     btlTrackExcel AT ROW 18.23 COL 121 WIDGET-ID 2
...
&Scoped-define SELF-NAME btlTrackExcel
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btlTrackExcel W-Win
ON CHOOSE OF btlTrackExcel IN FRAME F-Main /* Enviar a Excel el Tracking */
DO:
  RUN tracking-a-excel IN h_bconpedcre-2a.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/bconpedcred.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = CodDiv':U ,
             OUTPUT h_bconpedcred ).
       RUN set-position IN h_bconpedcred ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_bconpedcred ( 11.92 , 144.86 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'LABEL-FONT = 4,
                     ...
                     LABELS = Detalle Pedido|Seguimiento':U ,
             OUTPUT h_tab95 ).
       RUN set-position IN h_tab95 ( 12.92 , 1.00 ) NO-ERROR.
       RUN set-size IN h_tab95 ( 14.46 , 146.00 ) NO-ERROR.

       /* Links to SmartTab95 h_tab95. */
       RUN add-link IN adm-broker-hdl ( h_tab95 , 'Page':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bconpedcred ,
             btlTrackExcel:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_tab95 ,
             h_bconpedcred , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_b-cotizacion-cred ( 13.92 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotizacion-cred ( 12.65 , 141.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotizacion-cred. */
       RUN add-link IN adm-broker-hdl ( h_bconpedcred , 'Record':U , h_b-cotizacion-cred ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_bconpedcre-1-2 ( 13.88 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bconpedcre-1-2 ( 5.65 , 62.00 ) NO-ERROR.
       ...
       RUN set-position IN h_bconpedcre-2a ( 19.31 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bconpedcre-2a ( 7.81 , 144.00 ) NO-ERROR.

       /* Links to SmartBrowser h_bconpedcre-1-2. */
       RUN add-link IN adm-broker-hdl ( h_bconpedcred , 'Record':U , h_bconpedcre-1-2 ).

       /* Links to SmartBrowser h_bconpedcre-2a. */
       RUN add-link IN adm-broker-hdl ( h_bconpedcred , 'Record':U , h_bconpedcre-2a ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bconpedcre-1-2 ,
             h_tab95 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_bconpedcre-2a ,
             btlTrackExcel:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
CASE pParameter:
    WHEN 'Open-Query' THEN DO:
        RUN dispatch IN h_bconpedcred ('row-changed').
    END.
