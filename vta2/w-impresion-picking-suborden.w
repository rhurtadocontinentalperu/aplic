...
&Scoped-Define ENABLED-OBJECTS RADIO-SET-CodDoc BUTTON-13 txtDesde txtHasta ~
ChkChequeo txtOrdenCompra chbxImpresos BUTTON-14 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-CodDoc txtDesde txtHasta ~
ChkChequeo txtOrdenCompra chbxImpresos 
...
DEFINE VARIABLE h_b-cabecera-suborden AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-detalle-suborden AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     RADIO-SET-CodDoc AT ROW 1.19 COL 11 NO-LABEL WIDGET-ID 52
     ...
     ChkChequeo AT ROW 2.81 COL 62.29 WIDGET-ID 22
     txtOrdenCompra AT ROW 3.58 COL 28 COLON-ALIGNED WIDGET-ID 48
     chbxImpresos AT ROW 3.69 COL 62.29 WIDGET-ID 50
     BUTTON-14 AT ROW 23.12 COL 118.57 WIDGET-ID 62
     "Filtrar por:" VIEW-AS TEXT
          SIZE 9 BY .62 AT ROW 1.38 COL 2 WIDGET-ID 40
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 140.43 BY 24.58 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "IMPRESION PICKING SUB-ORDEN"
         HEIGHT             = 24.58
         WIDTH              = 140.43
         MAX-HEIGHT         = 24.58
         MAX-WIDTH          = 140.43
...
ON END-ERROR OF W-Win /* IMPRESION PICKING SUB-ORDEN */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* IMPRESION PICKING SUB-ORDEN */
DO:
...
DO:
    ASSIGN txtDesde txtHasta txtOrdenCompra chkchequeo chbxImpresos .
    ltxtDesde = txtDesde.
    ltxtHasta = txtHasta.
    ...
    RUN dispatch IN h_b-cabecera-suborden ('open-query':U).
    /*
...
ON CHOOSE OF BUTTON-14 IN FRAME F-Main /* Imprimir SUB-ORDEN */
DO:
  RUN imprimir-orden IN h_b-cabecera-suborden.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-cabecera-suborden.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cabecera-suborden ).
       RUN set-position IN h_b-cabecera-suborden ( 4.85 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cabecera-suborden ( 8.85 , 138.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/b-detalle-suborden.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-detalle-suborden ).
       RUN set-position IN h_b-detalle-suborden ( 14.04 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-detalle-suborden ( 11.15 , 114.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-detalle-suborden. */
       RUN add-link IN adm-broker-hdl ( h_b-cabecera-suborden , 'Record':U , h_b-detalle-suborden ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cabecera-suborden ,
             chbxImpresos:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-detalle-suborden ,
             h_b-cabecera-suborden , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-CodDoc txtDesde txtHasta ChkChequeo txtOrdenCompra 
          chbxImpresos 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RADIO-SET-CodDoc BUTTON-13 txtDesde txtHasta ChkChequeo txtOrdenCompra 
         chbxImpresos BUTTON-14 
      WITH FRAME F-Main IN WINDOW W-Win.
