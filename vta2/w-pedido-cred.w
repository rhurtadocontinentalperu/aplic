...
&Scoped-Define ENABLED-OBJECTS Btn-Excel RECT-25 COMBO-NroSer btn-margen ~
BTN-CCTE B-AGTRANS BUTTON-REDONDEO 
...
DEFINE VARIABLE h_v-pedido-cred AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn-Excel AT ROW 1 COL 122 WIDGET-ID 20
...
DO:
   RUN Asigna-datos IN h_v-pedido-cred.
END.
...
DO:
  RUN Redondeo IN h_v-pedido-cred.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-pedido-cred.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedido-cred ).
       RUN set-position IN h_v-pedido-cred ( 2.62 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-pedido-cred. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_v-pedido-cred ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-pedido-cred ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedido-cred ,
             h_p-navico , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE Btn-Excel RECT-25 COMBO-NroSer btn-margen BTN-CCTE B-AGTRANS 
         BUTTON-REDONDEO 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
 ...
 WHEN "Disable-Head" THEN DO:
     ...
     RUN dispatch IN h_v-pedido-cred ('disable-fields').
   END.
 WHEN "Enable-Head" THEN DO:
     RUN dispatch IN h_p-updv04 ('enable':U).
     RUN dispatch IN h_v-pedido-cred ('enable-fields').
   END.
END CASE.
