...
------------------------------------------------------------------------------*/
  ...
  IF B-CPEDI.FchVen < TODAY AND NOT AVAILABLE B-DPEDI THEN DO:
      MESSAGE 'Cotización VENCIDA' VIEW-AS ALERT-BOX ERROR.
      RETURN "ADM-ERROR".
  END.
  ...
  FOR EACH Facdpedi OF B-CPEDI NO-LOCK WHERE (Facdpedi.CanPed - Facdpedi.CanAte) > 0,
      ...
      /* SE VA A DESCARGAR DEL ALMACEN QUE SE COTIZÓ */
      ASSIGN
      ...
      IF s-StkDis <= 0 THEN DO:
          MESSAGE 'NO hay stock el el almacén' x-codalm 'para el producto' facdpedi.codmat
              VIEW-AS ALERT-BOX WARNING.
          NEXT DETALLES.
      END.
      ...
      IF f-CanPed <= 0 THEN DO:
          MESSAGE 'NO se cubre el mínimo de despacho para el producto' facdpedi.codmat
              VIEW-AS ALERT-BOX WARNING.
          NEXT DETALLES.
      END.
...
------------------------------------------------------------------------------*/
  ...
  IF TODAY <= 08/31/2013 THEN
  RUN vta2/lima-promocion-final ("GRAFITO", INPUT-OUTPUT TABLE ITEM).    /* PUEDE COMPRAR PRODUCTOS PROMOCIONALES */
  IF TODAY <= 08/31/2013 THEN
  RUN vta2/lima-promocion-final ("RAYADO", INPUT-OUTPUT TABLE ITEM).    /* PUEDE COMPRAR PRODUCTOS PROMOCIONALES */
...
------------------------------------------------------------------------------*/
  ...
  DEFINE BUFFER BPEDI FOR Faccpedi.
  ...
  /* RHC EN CASO DE SER LA DIVISION DE AREQUIPA Y SOLO BOLETAS Y TICKETS */
  IF s-coddiv = '00060' AND LOOKUP(FacCPedi.Cmpbnte, "BOL,TCK") > 0 THEN DO:
      /* SOLO SI superamos el ImpMinPercep */
      IF Faccpedi.ImpTot > ImpMinPercep THEN DO:
          /* Volvemos a crear los pedidos */
          EMPTY TEMP-TABLE ITEM.
          FOR EACH facdpedi OF faccpedi:
              CREATE ITEM.
              BUFFER-COPY facdpedi TO ITEM.
          ...
          /* Primero cargamos el P/M activo */
          x-ImpTot = 0.
          FOR EACH ITEM BY ITEM.NroItm:
              IF x-ImpTot + ITEM.ImpLin <= ImpMinPercep THEN DO:
                  CREATE Facdpedi.
                  BUFFER-COPY ITEM TO Facdpedi.
          ...
          /* Grabamos Totales */
          RUN Graba-Totales.
          /* Segundo cargamos el resto */
          REPEAT:
              CREATE BPEDI.
              BUFFER-COPY Faccpedi
                  TO BPEDI
                  ASSIGN
                  BPEDI.NroPed = STRING(FacCorre.NroSer,"999") + STRING(FacCorre.Correlativo,"999999").
              FIND Faccpedi WHERE ROWID(Faccpedi) = ROWID(BPEDI).
              ASSIGN
                  FacCorre.Correlativo = FacCorre.Correlativo + 1.
              x-ImpTot = 0.
              i-nItem = 1.
