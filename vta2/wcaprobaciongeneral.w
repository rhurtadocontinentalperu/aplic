...
DEF VAR s-FlgEst AS CHAR INIT "X".
...
"FacCPedi.Libre_c05" "Motivo" ? "character" ? ? ? ? ? ? no ? no no "31.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO k = 1 TO {&browse-name}:NUM-SELECTED-ROWS TRANSACTION ON ERROR UNDO, NEXT ON STOP UNDO, NEXT:
    ...
    END.
    /* Provincias pasa a APROBACION POR SECR. GG */
    /*IF LOOKUP(Faccpedi.codven, '017,173,015,901') > 0 THEN Faccpedi.FlgEst = "W".*/
    /* Supermercados pasa a APROBACION POR SECR. GG */
    IF LOOKUP(Faccpedi.codven, '151') > 0 THEN Faccpedi.FlgEst = "W".
    /* TRACKING */
