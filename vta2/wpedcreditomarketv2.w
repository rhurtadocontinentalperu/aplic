...
DEFINE VARIABLE h_tpedcreditomarketv2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vpedcreditomarketv2 AS HANDLE NO-UNDO.
...
DO:
   RUN Asigna-datos IN h_vpedcreditomarketv2.
END.
...
DO:
  ...
  RUN Excel IN h_vpedcreditomarketv2.
...
DO:
  RUN Margen IN h_vpedcreditomarketv2.
END.
...
DO:
  RUN Despachar-Pedido IN h_vpedcreditomarketv2.
END.
...
DO:
  RUN Redondeo IN h_vpedcreditomarketv2.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/vpedcreditomarketv2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vpedcreditomarketv2 ).
       RUN set-position IN h_vpedcreditomarketv2 ( 2.62 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_vpedcreditomarketv2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_vpedcreditomarketv2 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_vpedcreditomarketv2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_vpedcreditomarketv2 ,
             h_p-navico , 'AFTER':U ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/tpedcreditomarketv2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tpedcreditomarketv2 ).
       RUN set-position IN h_tpedcreditomarketv2 ( 11.77 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tpedcreditomarketv2 ( 12.12 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_tpedcreditomarketv2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_tpedcreditomarketv2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tpedcreditomarketv2 ,
             BUTTON-REDONDEO:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv95 ,
             h_tpedcreditomarketv2 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_tpedcreditomarketv2 <> ? THEN RUN dispatch IN h_tpedcreditomarketv2 ('open-query':U).
      END.
 ...
 WHEN "Disable-Head" THEN DO:
     ...
     RUN dispatch IN h_vpedcreditomarketv2 ('disable-fields').
   END.
 WHEN "Enable-Head" THEN DO:
     RUN dispatch IN h_p-updv04 ('enable':U).
     RUN dispatch IN h_vpedcreditomarketv2 ('enable-fields').
   END.
END CASE.
