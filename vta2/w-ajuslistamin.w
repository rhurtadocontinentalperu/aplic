...
DEFINE TEMP-TABLE T-Lista LIKE VtaListaMin.
...
DEF VAR x-NewOfi LIKE VtaListaMin.PreOfi.
...
IF gn-divi.VentaMinorista = 1 THEN DO:
    MESSAGE 'Acceso denegado' SKIP
        'Tiene que usar la LISTA MINORISTA GENERAL - ACTUALIZACION DE PRECIOS EN BLOQUE'
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: T-Lista T "?" ? INTEGRAL VtaListaMin
   END-TABLES.
 */
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "LISTA MINORISTA POR DIVISION - ACTUALIZACION DE PRECIOS EN BLOQUE"
         HEIGHT             = 18.81
         WIDTH              = 124.29
         MAX-HEIGHT         = 28.81
         MAX-WIDTH          = 146.14
...
ON END-ERROR OF wWin /* LISTA MINORISTA POR DIVISION - ACTUALIZACION DE PRECIOS EN BLOQUE */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* LISTA MINORISTA POR DIVISION - ACTUALIZACION DE PRECIOS EN BLOQUE */
DO:
...
FOR EACH VtaListaMin NO-LOCK WHERE Vtalistamin.codcia = s-codcia
    AND Vtalistamin.coddiv = s-coddiv,
    FIRST Almmmatg OF Vtalistamin NO-LOCK WHERE Almmmatg.desmar BEGINS FILL-IN-Marca
    AND Almmmatg.codpr1 BEGINS FILL-IN-CodPro
    AND Almmmatg.codfam BEGINS x-codfam
    AND Almmmatg.subfam BEGINS x-subfam:
    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** PROCESANDO ' + Vtalistamin.codmat + ' **'.
    CREATE T-Lista.
    BUFFER-COPY VtaListaMin
        TO T-Lista
        ASSIGN T-Lista.CanEmp = VtaListaMin.PreOfi * ( 1 + FILL-IN-Tasa / 100).
END.
...
FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** UN MOMENTO POR FAVOR **'.
FOR EACH VtaListaMin EXCLUSIVE-LOCK WHERE Vtalistamin.codcia = s-codcia
    AND Vtalistamin.coddiv = s-coddiv,
    FIRST Almmmatg OF Vtalistamin NO-LOCK WHERE Almmmatg.desmar BEGINS FILL-IN-Marca
    AND Almmmatg.codpr1 BEGINS FILL-IN-CodPro
    AND Almmmatg.codfam BEGINS x-codfam
    AND Almmmatg.subfam BEGINS x-subfam:
    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '** ACTUALIZANDO ' + Vtalistamin.codmat + ' **'.
    ASSIGN
        ...
        VtaListaMin.PreOfi = IF (COMBO-BOX-Tipo = 'Incremento') 
            THEN (VtaListaMin.PreOfi * ( 1 + FILL-IN-Tasa / 100)) 
            ELSE (VtaListaMin.PreOfi * ( 1 - FILL-IN-Tasa / 100)).
...
END.
IF AVAILABLE(VtaListaMin) THEN RELEASE VtaListaMin.
FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ''.
EMPTY TEMP-TABLE T-Lista.
{&OPEN-QUERY-{&BROWSE-NAME}}
