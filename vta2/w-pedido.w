...
DEFINE VARIABLE h_t-pedido AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-pedido AS HANDLE NO-UNDO.
...
DO:
   RUN Asigna-datos IN h_v-pedido.
END.
...
DO:
  RUN Resumen-por-Division IN h_v-pedido.
  ...
  IF pRpta = "YES" THEN DO:
      ...
      FOR EACH T-CPEDI WHERE T-CPEDI.FlgIgv = NO,
          ...
          END.
      END.
      RUN dispatch IN h_t-pedido ('open-query').
  END.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-pedido.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedido ).
       RUN set-position IN h_v-pedido ( 2.88 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-pedido. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_v-pedido ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-pedido ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv04 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedido ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-size IN h_b-pedido ( 13.38 , 146.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-pedido.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedido ).
       RUN set-position IN h_t-pedido ( 11.23 , 1.72 ) NO-ERROR.
       RUN set-size IN h_t-pedido ( 12.31 , 147.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-pedido. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_t-pedido ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedido ,
             btn-margen:HANDLE IN FRAME F-Main , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-12 COMBO-NroSer B-CCTE B-AGTRANS btn-margen B-RES 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-pedido <> ? THEN RUN dispatch IN h_t-pedido ('open-query':U).
      END.
    ...
    WHEN "Disable-Head" THEN DO:
        ...
        RUN dispatch IN h_v-pedido ('disable-fields').
        B-RES:SENSITIVE = NO.
      END.
    WHEN "Enable-Head" THEN DO:
        RUN dispatch IN h_p-updv04 ('enable':U).
        RUN dispatch IN h_v-pedido ('enable-fields').
        B-RES:SENSITIVE = YES.
      END.
END CASE.
