...
&Scoped-Define ENABLED-OBJECTS BtnDone RECT-1 BUTTON-13 txtDesde txtHasta ~
ChkChequeo BtnExcel RADIO-SET-1 BUTTON-aDistribucion BUTTON-PorZona ~
BUTTON-Alfabeticamente 
&Scoped-Define DISPLAYED-OBJECTS txtDesde txtHasta ChkChequeo RADIO-SET-1 ~
txtArtSinPeso 
...
DEFINE VARIABLE h_b-cimprime-odv2 AS HANDLE NO-UNDO.
...
DEFINE BUTTON BUTTON-aDistribucion 
     LABEL "A DISTRIBUCION" 
     SIZE 17 BY 1.12.
...
DEFINE FRAME F-Main
     BtnDone AT ROW 1 COL 147 WIDGET-ID 10
     BUTTON-13 AT ROW 1.19 COL 45.29 WIDGET-ID 4
     txtDesde AT ROW 1.35 COL 6.57 COLON-ALIGNED WIDGET-ID 16
     txtHasta AT ROW 1.35 COL 28 COLON-ALIGNED WIDGET-ID 20
     ChkChequeo AT ROW 1.38 COL 62.72 WIDGET-ID 22
     BtnExcel AT ROW 1.58 COL 94 WIDGET-ID 34
     RADIO-SET-1 AT ROW 1.58 COL 109 NO-LABEL WIDGET-ID 28
     BUTTON-aDistribucion AT ROW 1.58 COL 127 WIDGET-ID 14
     BUTTON-PorZona AT ROW 23.31 COL 3.29 WIDGET-ID 6
     BUTTON-Alfabeticamente AT ROW 23.31 COL 25.29 WIDGET-ID 8
     txtArtSinPeso AT ROW 23.77 COL 54.29 COLON-ALIGNED NO-LABEL WIDGET-ID 24
     "Lista de Articulos que NO tienen PESO en la O/D" VIEW-AS TEXT
          SIZE 43.14 BY .62 AT ROW 23.08 COL 56.72 WIDGET-ID 26
          BGCOLOR 15 FGCOLOR 9 FONT 6
     RECT-1 AT ROW 1.38 COL 93 WIDGET-ID 32
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 153.29 BY 24.31 WIDGET-ID 100.
...
DO:
  ASSIGN RADIO-SET-1.                 
  CASE RADIO-SET-1:
      WHEN 1 THEN RUN envia-excel IN h_b-cimprime-odv2.
      WHEN 2 THEN RUN envia-excel-detalle IN h_b-cimprime-odv2.
  END CASE.
  
    /*RUN dispatch IN h_b-cimprime-od ('ToExcel':U).*/
END.
...
DO:
    ...
    RUN dispatch IN h_b-cimprime-odv2 ('open-query':U).
...
&Scoped-define SELF-NAME BUTTON-aDistribucion
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-aDistribucion sW-Win
ON CHOOSE OF BUTTON-aDistribucion IN FRAME F-Main /* A DISTRIBUCION */
DO:
   RUN A-Distribucion IN h_b-cimprime-odv2.
END.
...
DO:
  ...
  RUN Imprimir-Formato IN h_b-cimprime-odv2 ( INPUT "ALFABETICO").
END.
...
DO:
  /*RUN dispatch IN h_b-cimprime-od ('imprime':U).*/
    RUN Imprimir-Formato IN h_b-cimprime-odv2 ( INPUT "ZONA").
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-cimprime-odv2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = FchPed':U ,
             OUTPUT h_b-cimprime-odv2 ).
       RUN set-position IN h_b-cimprime-odv2 ( 3.12 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cimprime-odv2 ( 7.50 , 151.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b-dimprime-od ( 10.85 , 2.29 ) NO-ERROR.
       RUN set-size IN h_b-dimprime-od ( 12.12 , 147.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-dimprime-od. */
       RUN add-link IN adm-broker-hdl ( h_b-cimprime-odv2 , 'Record':U , h_b-dimprime-od ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cimprime-odv2 ,
             BUTTON-aDistribucion:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-od ,
             h_b-cimprime-odv2 , 'AFTER':U ).
    END. /* Page 0 */
...
OCXFile = SEARCH( "w-listado-picking-odv2.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-listado-picking-odv2.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  DISPLAY txtDesde txtHasta ChkChequeo RADIO-SET-1 txtArtSinPeso 
      WITH FRAME F-Main IN WINDOW sW-Win.
  ENABLE BtnDone RECT-1 BUTTON-13 txtDesde txtHasta ChkChequeo BtnExcel 
         RADIO-SET-1 BUTTON-aDistribucion BUTTON-PorZona BUTTON-Alfabeticamente 
      WITH FRAME F-Main IN WINDOW sW-Win.
