...
DEFINE VAR x-tabla-campana AS CHAR INIT "TIPO_DSCTO_CAMPANA".
...
COMBO-BOX-motivo BUTTON-cancelar BUTTON-grabar 
...
DEFINE BUTTON BUTTON-grabar 
     LABEL "Grabar factores" 
     SIZE 15 BY 1.12.

DEFINE VARIABLE COMBO-BOX-motivo AS CHARACTER FORMAT "X(25)":U INITIAL "MVTAS" 
     LABEL "Campaña" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEM-PAIRS "Documentos de Campaña","MVTAS",
...
DEFINE VARIABLE FILL-IN-titulo AS CHARACTER FORMAT "X(50)":U INITIAL "Tipo de descuentos - %Factor descuento pronto pago" 
     VIEW-AS FILL-IN 
     SIZE 52.86 BY 1.12
...
DEFINE FRAME F-Main
     FILL-IN-titulo AT ROW 1.08 COL 1.57 NO-LABEL WIDGET-ID 16
     COMBO-BOX-procesos AT ROW 2.46 COL 13 COLON-ALIGNED WIDGET-ID 2
     BUTTON-cargar AT ROW 3.42 COL 45.43 WIDGET-ID 20
     COMBO-BOX-motivo AT ROW 3.5 COL 13 COLON-ALIGNED WIDGET-ID 18
     BUTTON-cancelar AT ROW 12.92 COL 37 WIDGET-ID 22
     BUTTON-grabar AT ROW 17.58 COL 47.14 WIDGET-ID 10
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Tipo de descuento - Factor x Campaña"
         HEIGHT             = 18.23
         WIDTH              = 62
         MAX-HEIGHT         = 21.5
         MAX-WIDTH          = 135.57
...
ON END-ERROR OF W-Win /* Tipo de descuento - Factor x Campaña */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Tipo de descuento - Factor x Campaña */
DO:
...
ON CHOOSE OF BUTTON-grabar IN FRAME F-Main /* Grabar factores */
DO:
...
ON ENTRY OF COMBO-BOX-motivo IN FRAME F-Main /* Campaña */
DO:
  RUN browse-off.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL COMBO-BOX-procesos W-Win
ON VALUE-CHANGED OF COMBO-BOX-procesos IN FRAME F-Main /* Tipo de descuento */
DO:
  /**/

    DEFINE VAR x-tipo-proceso AS CHAR.
    DEFINE VAR x-campana AS CHAR.
    DEFINE VAR x-nombre AS CHAR.

    IF NOT (TRUE <> (COMBO-BOX-motivo:LIST-ITEM-PAIRS > "")) THEN 
            COMBO-BOX-motivo:DELETE(COMBO-BOX-motivo:LIST-ITEM-PAIRS).

    x-tipo-proceso = SELF:SCREEN-VALUE.

    FOR EACH vtatabla WHERE vtatabla.codcia = s-codcia AND
                              vtatabla.tabla = x-tabla-campana AND
                                vtatabla.llave_c1 = x-tipo-proceso NO-LOCK:

        x-campana = vtatabla.llave_c2.

        x-nombre = vtatabla.llave_c2 + " : ...".

        FIND FIRST x-vtatabla WHERE x-vtatabla.codcia = s-codcia AND 
                                    x-vtatabla.tabla = "CAMPAÑAS" AND
                                    x-vtatabla.llave_c1 = vtatabla.llave_c2 NO-LOCK NO-ERROR.
        IF AVAILABLE x-vtatabla THEN DO:
            x-nombre = vtatabla.llave_c2 + " : " + STRING(x-vtatabla.rango_fecha[1],"99/99/9999") + " - " +
                        STRING(x-vtatabla.rango_fecha[2],"99/99/9999").
        END.


        COMBO-BOX-motivo:ADD-LAST(x-nombre, vtatabla.llave_c2).
        IF TRUE <> (COMBO-BOX-motivo:SCREEN-VALUE > "") THEN DO:
          ASSIGN COMBO-BOX-motivo:SCREEN-VALUE = vtatabla.llave_c2.
        END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/d-tipo-dscto-campana-factor-ppago.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-rbte-config-linea-movimiento ).
       RUN set-position IN h_b-rbte-config-linea-movimiento ( 4.73 , 11.00 ) NO-ERROR.
       RUN set-size IN h_b-rbte-config-linea-movimiento ( 12.31 , 24.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-rbte-config-linea-movimiento ,
             COMBO-BOX-motivo:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv10 ,
             BUTTON-cancelar:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      IF NOT (TRUE <> (x-motivo > "")) THEN DO:
          RUN carga-valores IN h_b-rbte-config-linea-movimiento(INPUT x-proceso, INPUT x-motivo).

          RUN browse-on.
      END.
      ELSE DO:
          MESSAGE "Elija campaña, por favor!!!" VIEW-AS ALERT-BOX INFORMATION.
      END.
  END.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-titulo COMBO-BOX-procesos COMBO-BOX-motivo 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE COMBO-BOX-procesos BUTTON-cargar COMBO-BOX-motivo BUTTON-cancelar 
         BUTTON-grabar 
      WITH FRAME F-Main IN WINDOW W-Win.
