...
"FacCPedi.Libre_c03" "Número" "x(10)" "character" ? ? ? ? ? ? no ? no no "16.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/*     MESSAGE 'La Orden viene de un Cross Docking' SKIP 'Acceso Denegado' VIEW-AS ALERT-BOX ERROR. */
/*     RETURN 'ADM-ERROR'.                                                                          */
/* END.                                                                                             */
...
IF AVAILABLE Ccbcdocu THEN DO:
    ...
    RETURN "ADM-ERROR".
END.
/* RHC 13/02/2018 NO Cross Docking */
/* DEF BUFFER ORIGEN FOR Faccpedi.                        */
/* FIND ORIGEN WHERE ORIGEN.codcia = Faccpedi.codcia      */
/*     AND ORIGEN.coddoc = Faccpedi.codref                */
/*     AND ORIGEN.nroped = Faccpedi.nroref                */
/*     AND ORIGEN.crossdocking = YES                      */
/*     NO-LOCK NO-ERROR.                                  */
/* IF AVAILABLE ORIGEN THEN DO:                           */
/*     MESSAGE 'Acceso Denegado' VIEW-AS ALERT-BOX ERROR. */
/*     RETURN 'ADM-ERROR'.                                */
/* END.                                                   */

/* RHC 25/10/2012 CASO ESPECIAL: Ordenes Fraccionadas */
RUN Anula-Orden-Normal.
/* IF Faccpedi.Libre_C02 = "O/D" THEN RUN Anula-Orden-Fraccionada. */
/* ELSE RUN Anula-Orden-Normal.                                    */
...
/*    {lib/lock-wait.i &Tabla=B-CPedi &Condicion="ROWID(B-CPedi) = ROWID(FacCPedi)"} */
   ASSIGN 
       B-CPedi.FlgEst = "A"
       B-CPedi.Glosa = " A N U L A D O".
   RELEASE B-CPedi.
