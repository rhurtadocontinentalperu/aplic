...
DEFINE VARIABLE h_bcanjeadelxnc AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-2 AT ROW 4.23 COL 66 WIDGET-ID 72
...
ON END-ERROR OF W-Win /* CANJE DE ANTICIPOS DE CAMPAÑA POR NOTA DE CREDITO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* CANJE DE ANTICIPOS DE CAMPAÑA POR NOTA DE CREDITO */
DO:
...
DO:
  ...
  RUN Genera-NC IN h_bcanjeadelxnc
   ( INPUT cCodDoc,
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/bcanjeadelxnc.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bcanjeadelxnc ).
       RUN set-position IN h_bcanjeadelxnc ( 7.15 , 5.00 ) NO-ERROR.
       RUN set-size IN h_bcanjeadelxnc ( 9.42 , 80.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_bcanjeadelxnc. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_bcanjeadelxnc ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bcanjeadelxnc ,
             COMBO-BOX-Concepto:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv97 ,
             h_bcanjeadelxnc , 'AFTER':U ).
    END. /* Page 0 */
...
END.
RUN dispatch IN h_bcanjeadelxnc ('open-query':U).
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      /* Conceptos N/C */
      cListItems = "".
      FOR EACH CcbTabla NO-LOCK WHERE CcbTabla.CodCia = s-codcia AND CcbTabla.Tabla = cCodDoc:
