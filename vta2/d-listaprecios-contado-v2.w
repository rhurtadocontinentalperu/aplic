...
DO:
  /*RUN Carga-Precios.*/
    ...
    RUN web_api_unit_price IN hProc (INPUT t-Almmmatg.CodMat,
                                     INPUT "C",
                                     INPUT "000",
                                     INPUT s-CodCli,
                                     INPUT s-CodDiv,
...
FOR EACH t-Almmmatg EXCLUSIVE-LOCK, 
    FIRST Almmmate OF t-Almmmatg NO-LOCK WHERE Almmmate.codalm = COMBO-BOX-CodAlm:
    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} =
        "STOCK: " + t-Almmmatg.codmat + " " + t-Almmmatg.desmat.
    t-Almmmatg.StkMax = 0.
    pComprometido = 0.
    ASSIGN 
        t-Almmmatg.StkMax = Almmmate.StkAct.
...
FOR EACH t-Almmmatg EXCLUSIVE-LOCK, FIRST Almmmate WHERE {&CONDICION2} NO-LOCK:
    t-Almmmatg.StkMax = 0.
    pComprometido = 0.
...
CASE TRUE:
    WHEN FILL-IN-codigo > '' THEN DO:
        FOR EACH Almmmate NO-LOCK WHERE Almmmate.codcia = s-codcia
                ...
                AND Almmmate.codmat = FILL-IN-codigo,
            FIRST Almmmatg OF Almmmate NO-LOCK,
            FIRST Almtfami OF Almmmatg NO-LOCK:

            IF Almmmate.StkAct < 0 OR (RADIO-SET-Stock = "C" AND Almmmate.StkAct <= 0) THEN NEXT.
...
/*             IF CMB-FILTRO = 'Nombres que contengan'                           */
/*                 AND FILL-IN-filtro > ''                                       */
/*                 AND INDEX(Almmmatg.desmat, FILL-IN-filtro) = 0                */
/*                 THEN NEXT.                                                    */
    ...
    WHEN CMB-FILTRO = 'Nombres que contengan' AND FILL-IN-filtro > '' THEN DO:
        ...
        FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.DesMat CONTAINS LocalCadena
            ...
            FIRST Almtfami OF Almmmatg NO-LOCK,
            FIRST Almmmate OF Almmmatg NO-LOCK WHERE Almmmate.codalm = COMBO-BOX-CodAlm:
    ...
    WHEN f-Marca > '' THEN DO:
        ...
        FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND
                Almmmatg.desmar BEGINS f-Marca,
            FIRST Almtfami OF Almmmatg NO-LOCK,
            FIRST Almmmate OF Almmmatg NO-LOCK WHERE Almmmate.codalm = COMBO-BOX-CodAlm:
