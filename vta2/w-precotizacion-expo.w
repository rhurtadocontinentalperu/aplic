...
DEFINE VARIABLE h_t-precotizacion-expo AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-precotizacion-expo AS HANDLE NO-UNDO.
...
DO:
    RUN Asigna-Cotizacion IN h_v-precotizacion-expo.  
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-precotizacion-expo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-precotizacion-expo ).
       RUN set-position IN h_v-precotizacion-expo ( 1.27 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.04 , 111.14 ) */
       ...
       /* Links to SmartViewer h_v-precotizacion-expo. */
       RUN add-link IN adm-broker-hdl ( h_p-updv06 , 'TableIO':U , h_v-precotizacion-expo ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido2x , 'Record':U , h_v-precotizacion-expo ).

       /* Links to SmartQuery h_q-pedido2x. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido2x ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-precotizacion-expo ,
             BUTTON-6:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-precotizacion-expo , 'AFTER':U ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  't-precotizacion-expo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-precotizacion-expo ).
       RUN set-position IN h_t-precotizacion-expo ( 12.15 , 1.57 ) NO-ERROR.
       RUN set-size IN h_t-precotizacion-expo ( 11.08 , 131.57 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-precotizacion-expo ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-precotizacion-expo <> ? THEN RUN dispatch IN h_t-precotizacion-expo ('open-query':U).
      END.
...
/*         B-CCTE:SENSITIVE = YES.*/
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        RUN Recalcular-Precios IN h_t-precotizacion-expo.
      END.
