...
/*       ELSE FacCPedi.FchEnt:SENSITIVE = YES.                              */
...
------------------------------------------------------------------------------*/
  ...
  pMensaje = ''.
  RUN Fecha-Entrega ( FacCPedi.CodDoc + ',' + FacCPedi.NroPed,
                      TODAY,
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    ELSE FacCPedi.Libre_c03:SCREEN-VALUE = ''.
     ...
     /* ************************************************************** */
     /* RHC 16/09/17 FECHA DE ENTREGA SUGERIDA */
     /* ************************************************************** */
     pMensaje = "".
     DEF VAR pFchEnt AS DATE NO-UNDO.
     ...
     RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
     IF RETURN-VALUE = 'YES' THEN DO:
         FOR EACH PEDI NO-LOCK, FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND Almmmatg.codmat = PEDI.codmat:
             pNroSKU = pNroSKU + 1.
         ...
         RUN Fecha-Entrega ( ",",
                             ...
                             STRING(TIME,'HH:MM:SS'),
                             ENTRY(1,s-CodAlm),
                             (IF FacCPedi.TipVta:SCREEN-VALUE = "Si" THEN "CR" ELSE FacCPedi.CodPos:SCREEN-VALUE),
                             pNroSKU,
                             pPeso,
                             FacCPedi.CodCli:SCREEN-VALUE,
                             s-CodDiv,                          
                             "COT," + FacCPedi.NroRef:SCREEN-VALUE,
                             INPUT-OUTPUT pFchEnt,
                             OUTPUT pMensaje
                             ).
         IF pMensaje <> '' THEN DO:
         ...
         END.
     END.
     DISPLAY pFchEnt @ FacCPedi.FchEnt.      /* FECHA DE ENTREGA SUGERIDA */
END.
