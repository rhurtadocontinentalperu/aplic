...
DEFINE VARIABLE h_t-cotiza-min-01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotiza-min-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-1 AT ROW 5.58 COL 122 WIDGET-ID 16
...
DO:
  RUN Excel IN h_v-cotiza-min-01
    ( INPUT YES /* LOGICAL */).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-cotiza-min-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotiza-min-01 ).
       RUN set-position IN h_v-cotiza-min-01 ( 2.88 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-cotiza-min-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_v-cotiza-min-01 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-cotiza-min-01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv01 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotiza-min-01 ,
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-cotiza-min-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotiza-min-01 ).
       RUN set-position IN h_t-cotiza-min-01 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotiza-min-01 ( 12.31 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotiza-min-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotiza-min-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotiza-min-01 ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotiza-min-01 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-cotiza-min-01 <> ? THEN RUN dispatch IN h_t-cotiza-min-01 ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-cotiza-min-01 <> ? THEN RUN Recalcular-Precios IN h_t-cotiza-min-01.
      END.
