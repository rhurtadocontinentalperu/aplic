...
FacCPedi.FmaPgo FacCPedi.CodRef FacCPedi.NroRef FacCPedi.NroCard ~
FacCPedi.usuario 
...
"US$", 2
     ...
     FacCPedi.FlgIgv AT ROW 6.65 COL 96 NO-LABEL WIDGET-ID 40
          VIEW-AS RADIO-SET HORIZONTAL
          RADIO-BUTTONS 
                    "Si", yes,
"No", no
...
DEFINE FRAME F-Main
     ...
     FacCPedi.CodRef AT ROW 7.46 COL 94 COLON-ALIGNED WIDGET-ID 32
          VIEW-AS FILL-IN 
          SIZE 5 BY .81
     FacCPedi.NroRef AT ROW 7.46 COL 99 COLON-ALIGNED NO-LABEL WIDGET-ID 34 FORMAT "X(12)"
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
     ...
     FacCPedi.usuario AT ROW 8.27 COL 94 COLON-ALIGNED WIDGET-ID 36
          LABEL "Digitado por" FORMAT "x(10)"
          VIEW-AS FILL-IN 
          SIZE 11 BY .81
     "Moneda:" VIEW-AS TEXT
          SIZE 7 BY .5 AT ROW 6.12 COL 89
     "Afecto a IGV:" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 6.92 COL 86 WIDGET-ID 44
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1 SCROLLABLE 
         FONT 4.
...
DO:
  ...
  DISPLAY 
    ...
    ENTRY(1, gn-clie.CndVta) @ FacCPedi.FmaPgo  /* La primera del cliente */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FacCPedi.CodCli V-table-Win
ON LEFT-MOUSE-DBLCLICK OF FacCPedi.CodCli IN FRAME F-Main /* Codigo */
...
DO:
  ASSIGN
      ...
      input-var-2 = ''
      input-var-3 = ''.
  IF s-TpoPed = "M" THEN ASSIGN input-var-1 = "0006".
...
DO:
    IF INPUT {&self-name} = ? OR INPUT {&self-name} < TODAY THEN DO:
        MESSAGE 'Fecha mal registrada' VIEW-AS ALERT-BOX WARNING.
        RETURN NO-APPLY.
    END.
END.
...
DO:
    S-CODIGV = IF INPUT FacCPedi.FlgIgv = YES THEN 1 ELSE 2.
    RUN Procesa-Handle IN lh_Handle ('Recalculo').
END.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH FacDPedi OF FacCPedi TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      DELETE FacDPedi.
    END.    
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH facdPedi OF faccPedi NO-LOCK:
        CREATE PEDI.
        ASSIGN
            PEDI.CodCia = Facdpedi.codcia
            PEDI.CodDiv = Facdpedi.coddiv
            PEDI.CodDoc = Facdpedi.coddoc
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Genera-Excel-Utilex V-table-Win 
PROCEDURE Genera-Excel-Utilex :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF VAR C-SimMon    AS CHAR FORMAT "X(7)"  NO-UNDO.
...
cColumn = STRING(t-Column).
cRange = "A" + cColumn.
chWorkSheet:Range(cRange):Value = "Señor(es) :" + FaccPedi.codcli + ' - ' + FaccPedi.Nomcli. 
t-Column = t-Column + 1.
cColumn = STRING(t-Column).
cRange = "A" + cColumn.
...
FOR EACH facdpedi OF faccpedi NO-LOCK,
    ...
    IF l-incigv THEN ASSIGN F-ImpLin = FacDPedi.ImpLin.
    ELSE ASSIGN F-ImpLin = ROUND(FacDPedi.ImpLin / (1 + FacCPedi.PorIgv / 100),2).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER l-incigv AS LOGICAL NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  IF FacCorre.FlgEst = NO THEN DO:
      ...
      RETURN 'ADM-ERROR'.
  END.
  IF s-TpoPed = "NXT" THEN DO:
      MESSAGE 'Permiso denegado' VIEW-AS ALERT-BOX WARNING.
      RETURN 'ADM-ERROR'.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      ...
      ASSIGN 
          ...
          /*FacCPedi.PorIgv = (IF s-CodIgv = 1 THEN s-PorIgv ELSE 0 )*/
          FacCPedi.PorIgv = (IF FacCPedi.FlgIgv = YES THEN s-PorIgv ELSE 0)
      ...
      /* SUPERMERCADOS */
      IF s-import-ibc = YES AND s-pendiente-ibc = YES THEN Faccpedi.flgest = 'E'.   /* POR APROBAR */
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      /* Pedidos por NEXTEL */
      IF Faccpedi.codref = 'PNX' THEN DO:
          ...
          IF AVAILABLE Vtacdocu THEN Vtacdocu.flgest = "X".
          RELEASE Vtacdocu.
      END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Migrar-a-Ate V-table-Win 
PROCEDURE Migrar-a-Ate :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF NOT AVAILABLE facCPedi THEN RETURN.
ASSIGN
    FacCPedi.FlgEst = "P".      /* APROBADA */
CREATE B-CPEDI.
BUFFER-COPY FacCPedi TO B-CPEDI
    ASSIGN
        B-CPEDI.CodDiv = '00000'.
FOR EACH FacDPedi OF FacCPedi NO-LOCK:
    CREATE B-DPEDI.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME} :
      ...
      IF s-TpoPed = "M" AND gn-clie.canal <> '0006' THEN DO:
          MESSAGE 'Cliente no permitido en este canal de venta'
             VIEW-AS ALERT-BOX ERROR.
         APPLY "ENTRY" TO FacCPedi.CodCli.
         RETURN "ADM-ERROR".   
     END.
