...
DEFINE VAR z-zona AS CHAR INIT 'Todos'.
DEFINE VAR z-subzona AS CHAR INIT 'Todos'.

&SCOPED-DEFINE CONDICION2 (ordenes.codcia = s-codcia ~
AND (z-canal = 'Todos' OR ordenes.coddiv = z-canal) ~
AND (z-zona = "Todos" OR ordenes.xZona = z-zona) ~
AND (z-subzona = "Todos" OR ordenes.xsubZona = z-subzona) ~
...
ORDENES.Libre_c01 ordenes.czona @ x-zona ordenes.csubzona @ x-subzona ~
ordenes.cdistrito @ x-distrito ORDENES.CodDoc ORDENES.NroPed ORDENES.FchPed ~
ORDENES.Hora ORDENES.FchEnt ORDENES.NomCli ORDENES.Libre_d01 ~
fDestinoFinal() @ x-DestinoFinal ORDENES.AcuBon[1] ORDENES.AcuBon[2] ~
ORDENES.AcuBon[3] ORDENES.AcuBon[4] ORDENES.AcuBon[5] FacCPedi.UsrImpOD ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      ordenes.czona @ x-zona COLUMN-LABEL "Zona" FORMAT "x(30)":U
            WIDTH 10.43
      ordenes.csubzona @ x-subzona COLUMN-LABEL "Sub - Zona" FORMAT "x(30)":U
            WIDTH 10.43
...
"ordenes.czona @ x-zona" "Zona" "x(30)" ? ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > "_<CALC>"
"ordenes.csubzona @ x-subzona" "Sub - Zona" "x(30)" ? ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"ordenes.cdistrito @ x-distrito" "Distrito" "x(40)" ? ? ? 0 ? ? ? no ? no no "23.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.ORDENES.CodDoc
"ORDENES.CodDoc" ? ? "character" ? ? ? ? ? ? no ? no no "5.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.ORDENES.NroPed
"ORDENES.NroPed" "Numero" ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.ORDENES.FchPed
"ORDENES.FchPed" "Emisión" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.ORDENES.Hora
"ORDENES.Hora" ? ? "character" ? ? ? ? ? ? no ? no no "4.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   = Temp-Tables.ORDENES.FchEnt
     _FldNameList[11]   > Temp-Tables.ORDENES.NomCli
"ORDENES.NomCli" "Cliente" ? "character" ? ? ? ? ? ? no ? no no "35.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > Temp-Tables.ORDENES.Libre_d01
...
REPEAT z-sec = 1 TO NUM-ENTRIES(z-docmntos,","):
    ...
    */
        ...
        IF x-pickeado = 2 THEN DO:
            /* No Esta Pickeado NO VA*/
            /*IF NOT (faccpedi.flgsit = 'T') THEN NEXT.*/
        ...
        */
        ...
        ASSIGN
            ...
            ORDENES.Libre_c02 = "".
        ASSIGN ORDENES.Libre_d01 = fPeso().
        ASSIGN ORDENES.Libre_d02 = fNroItm()
        ...
        */
        ...
        /*
        IF AVAILABLE vtaCtabla THEN pZona = vtaCtabla.descripcion.
        IF AVAILABLE vtaDtabla THEN pSubZona = vtaCtabla.descripcion.
        */
    
        ASSIGN ordenes.xZona = pZona
...
DEFINE INPUT PARAMETER pZona AS CHAR.
DEFINE INPUT PARAMETER pSubZona AS CHAR.
