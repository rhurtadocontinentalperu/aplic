...
DEFINE VARIABLE h_t-cotiza-min-a AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-cotiza-min ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-cotiza-min-a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotiza-min-a ).
       RUN set-position IN h_t-cotiza-min-a ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotiza-min-a ( 12.31 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotiza-min-a. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotiza-min-a ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotiza-min-a ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotiza-min-a , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN 'IBC' THEN DO:
        RUN IBC-Diferencias IN h_t-cotiza-min-a.
    END.
    WHEN "browse" THEN DO:
          IF h_b-cotiza <> ? THEN RUN dispatch IN h_b-cotiza ('open-query':U).
          IF h_t-cotiza-min-a <> ? THEN RUN dispatch IN h_t-cotiza-min-a ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-cotiza-min-a <> ? THEN RUN Recalcular-Precios IN h_t-cotiza-min-a.
      END.
