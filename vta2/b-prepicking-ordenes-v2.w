...
/* Ic - 11Feb2016, Optimizar el Pre-Picking */
DEFINE VAR ixItemsOrden AS INT INIT 0.
DEFINE VAR ixItemsPickeados AS INT INIT 0.
DEFINE VAR ixBlink AS LOG INIT NO.
...
&Scoped-Define ENABLED-OBJECTS BtnFaltante ChkBxAutomatico x-CodDoc ~
x-NroDoc BUTTON-13 BUTTON-15 txtTTrans br_table RECT-65 
&Scoped-Define DISPLAYED-OBJECTS txtMensajes ChkBxAutomatico txtTTxx ~
x-CodDoc x-NroDoc x-FchDoc x-CodCli x-NomCli x-NroItem txtTTrans ~
x-NroItemPickeados 
...
DEFINE VARIABLE txtMensajes AS CHARACTER FORMAT "X(256)":U INITIAL "Linea de Mensajes..." 
      VIEW-AS TEXT 
     SIZE 119 BY .85
...
DEFINE VARIABLE x-NroItem AS INTEGER FORMAT "9999":U INITIAL 0 
     LABEL "Nro Items de la ORDEN" 
     VIEW-AS FILL-IN 
     SIZE 4.72 BY .81 NO-UNDO.

DEFINE VARIABLE x-NroItemPickeados AS INTEGER FORMAT "9999":U INITIAL 0 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Detalle.codmat COLUMN-LABEL "Codigo" FORMAT "X(14)":U WIDTH 9.14
      Almmmatg.DesMat FORMAT "X(45)":U WIDTH 39.72
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(30)":U WIDTH 25.43
      Detalle.UndVta COLUMN-LABEL "Unidad" FORMAT "XXXX":U
      Detalle.CanPick COLUMN-LABEL "Cantidad" FORMAT ">,>>>,>>9.9999":U
            WIDTH 9.86
      Detalle.Factor FORMAT ">>,>>9.9999":U WIDTH 7.43
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 124 BY 19.04
...
DEFINE FRAME F-Main
     BtnFaltante AT ROW 1.96 COL 108 WIDGET-ID 30
     txtMensajes AT ROW 25.5 COL 2 NO-LABEL WIDGET-ID 26
     ChkBxAutomatico AT ROW 5.15 COL 85.14 WIDGET-ID 22
     txtTTxx AT ROW 4.15 COL 50.72 RIGHT-ALIGNED NO-LABEL WIDGET-ID 20
     ...
     BUTTON-13 AT ROW 1.27 COL 70.57 WIDGET-ID 4
     BUTTON-15 AT ROW 1.27 COL 89.43 WIDGET-ID 8
     x-NomCli AT ROW 2.62 COL 25 COLON-ALIGNED NO-LABEL
     x-NroItem AT ROW 2.62 COL 96 COLON-ALIGNED WIDGET-ID 10
     txtTTrans AT ROW 4.15 COL 49.43 COLON-ALIGNED WIDGET-ID 14
     br_table AT ROW 6.19 COL 1.72
     x-NroItemPickeados AT ROW 4.15 COL 96 COLON-ALIGNED WIDGET-ID 28
     RECT-65 AT ROW 1 COL 1.14
     SPACE(19.15) SKIP(4.54)
...
/* SETTINGS FOR FILL-IN txtMensajes IN FRAME F-Main
   NO-ENABLE ALIGN-L                                                    */
ASSIGN 
...
/* SETTINGS FOR FILL-IN x-NroItem IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN x-NroItemPickeados IN FRAME F-Main
   NO-ENABLE                                                            */
...
"Detalle.codmat" "Codigo" "X(14)" "character" ? ? ? ? ? ? yes ? no no "9.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.Almmmatg.DesMat
"Almmmatg.DesMat" ? ? "character" ? ? ? ? ? ? no ? no no "39.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.Almmmatg.DesMar
"Almmmatg.DesMar" "Marca" ? "character" ? ? ? ? ? ? no ? no no "25.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Detalle.CanPick" "Cantidad" ? "decimal" ? ? ? ? ? ? yes ? no no "9.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.Detalle.Factor
"Detalle.Factor" ? ? "decimal" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.Detalle.CanPed
"Detalle.CanPed" "Cantidad!Calculada" ? "decimal" ? ? ? ? ? ? no ? no no "13.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&Scoped-define SELF-NAME BtnFaltante
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BtnFaltante B-table-Win
ON CHOOSE OF BtnFaltante IN FRAME F-Main /* Situacion O/D */
DO:
  RUN vta2/w-pre-picking-estado.r(INPUT TABLE T-DPEDI).
END.
...
IF ixBlink = YES THEN DO:
    ixBlink = NO.
END.
ELSE DO:
    ixBlink = YES.   
END.
txtMensajes:VISIBLE = ixBlink.
...
OR RETURN OF {&SELF-NAME}
    DO:
        ...
        IF x-NroDoc <> SELF:SCREEN-VALUE THEN DO:
            ...
            txtMensajes:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
            txtMensajes = "".
...
FIND FIRST T-DPEDI WHERE T-DPEDI.codmat = DETALLE.codmat NO-ERROR.
IF AVAILABLE T-DPEDI THEN DO:
    ASSIGN T-DPEDI.CanAte = T-DPEDI.CanAte - (Detalle.CanPick * Detalle.Factor).
    IF T-DPEDI.CanAte < 0 THEN ASSIGN T-DPEDI.CanAte = 0.
END.
...
/* Este es nuestro archivo consolidadox articulo */                         
...
FOR EACH Facdpedi NO-LOCK WHERE Facdpedi.codcia = s-codcia
    ...
    IF NOT AVAILABLE T-DPEDI THEN DO:
        ...
        ASSIGN
            ...
            T-DPEDI.CanAte = 0 /* Cantidad Pickeada */.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR lxFaltante AS DEC.

  lCount = 0.
  lMensaje = ''.
  lxMarca = ''.
  FOR EACH T-DPEDI:
      IF T-DPEDI.canAte < T-DPEDI.CanPed THEN DO:
        lCount = lCount + 1.
        IF lCount < 11 THEN DO:
            lxDesMat = "".
            lxMarca = "".
            FIND FIRST i-almmmatg WHERE i-almmmatg.codcia = s-codcia AND 
                                        i-almmmatg.codmat = T-DPEDI.codmat NO-LOCK NO-ERROR.
            IF AVAILABLE i-almmmatg THEN lxDesMat = TRIM(i-almmmatg.desmat).
            IF AVAILABLE i-almmmatg THEN lxMarca = TRIM(i-almmmatg.desmar).
            IF lxMarca = ? THEN lxMarca = ''.
            IF lxDesMat = ? THEN lxDesMat = ''.
            lxDesMat = lxDesMat + FILL(' ',40).
            lxMarca = lxMarca + FILL(' ',20).
            lxFaltante = T-DPEDI.CanPed - T-DPEDI.canAte.

            IF lMensaje <> "" THEN lMensaje = lMensaje + CHR(10).
            lMensaje = lMensaje + T-DPEDI.CodMat + CHR(9) + SUBSTRING(lxDesMat,1,40) + 
                CHR(9) + SUBSTRING(lxMarca,1,20) + CHR(9) + STRING(lxFaltante,"99999.99").
        END.
        ELSE DO:
            IF lCount = 10 THEN DO:
                lMensaje = lMensaje + CHR(10).
                lMensaje = lMensaje + "Existen mas..." + CHR(10).
            END.
            ELSE LEAVE.
        END.
      END.
  END.
  IF lCount > 0 THEN DO:
      MESSAGE '     Diferencia de cantidades en los Articulos' SKIP(2)
                lMensaje 
        VIEW-AS ALERT-BOX ERROR.
      RETURN 'ADM-ERROR'.
  END.
...
OCXFile = SEARCH( "b-prepicking-ordenes-v2.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "b-prepicking-ordenes-v2.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISABLE BUTTON-13 WITH FRAME {&FRAME-NAME}.
  DISABLE BUTTON-15 WITH FRAME {&FRAME-NAME}.
  DISABLE BtnFaltante WITH FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
    ...
    END.
    */
      ixItemsPickeados = ixItemsPickeados + 1.
  END.
  ...
  ASSIGN
    ...
    Detalle.nroitm = ixItemsPickeados  /*x-item*/
  ...
  /* Refresh */
  x-NroItemPickeados = ixItemsPickeados.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-13 WITH FRAME {&FRAME-NAME}.
  ENABLE BUTTON-15 WITH FRAME {&FRAME-NAME}.
  ENABLE BtnFaltante WITH FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN x-NroItemPickeados = ixItemsPickeados.
...
FOR EACH w-report WHERE w-report.task-no = s-task-no
    ...
    /* Actualizo el archivo Resumen  T-DPEDI */
    FIND FIRST T-DPEDI WHERE T-DPEDI.codmat = w-report.Campo-C[2] NO-ERROR.
    IF AVAILABLE T-DPEDI THEN DO:
        ASSIGN T-DPEDI.canate = T-DPEDI.canate + (w-report.Campo-F[2] * w-report.Campo-F[3]).
    END.
    ELSE DO:
        MESSAGE "Existe una inconsistencia entre la O/D y lo pickeado (Recarga Items)".
    END.
END.
...
------------------------------------------------------------------------------*/
  ...
  END.
    ...
    ixItemsPickeados = ixItemsPickeados - 1.
    x-NroItemPickeados = ixItemsPickeados.
    IF (ixItemsPickeados < 0) THEN ixItemsPickeados = 0.
...
/* CHequeamos acumulado */
  ...
  DEFINE VAR ixCantPickeada AS DEC.

  txtMensajes:SCREEN-VALUE = "".
  ASSIGN txtMensajes.
  ...
  */
  ...
  ixCantPickeada = DECIMAL(Detalle.canped:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).

  FIND T-DPEDI WHERE T-DPEDI.codmat = Detalle.codmat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
    NO-LOCK NO-ERROR.
  
  IF AVAILABLE T-DPEDI AND (T-DPEDI.CanAte + ixCantPickeada) > T-DPEDI.canped THEN DO:      
    txtMensajes:SCREEN-VALUE = "CANTIDAD ingresada SUPERA la del pedido".
    ASSIGN txtMensajes.
    /*ASSIGN txtMensajes = "CANTIDAD ingresada SUPERA la del pedido".*/
    RETURN 'ADM-ERROR'.
  END.
  ...
  IF lChxBoxAutomatico = 'NO' THEN DO:      
      IF AVAILABLE T-DPEDI AND (T-DPEDI.CanAte + ixCantPickeada) < T-DPEDI.canped THEN DO:
          txtMensajes:SCREEN-VALUE = "CANTIDAD ingresada ES MENOR a la del pedido".
          ASSIGN txtMensajes.
      END.
  END.
