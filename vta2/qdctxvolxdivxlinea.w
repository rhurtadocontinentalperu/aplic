...
&Scoped-define QUERY-STRING-Query-Main FOR EACH FacTabla WHERE TRUE /* Join to GN-DIVI incomplete */ ~
      AND FacTabla.codcia = s-codcia ~
 AND FacTabla.Tabla = s-Tabla ~
 AND FacTabla.Codigo = TRIM(s-coddiv) + "|" + ~
TRIM(s-codfam) + "|" + TRIM(s-subfam) NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH FacTabla WHERE TRUE /* Join to GN-DIVI incomplete */ ~
      AND FacTabla.codcia = s-codcia ~
 AND FacTabla.Tabla = s-Tabla ~
 AND FacTabla.Codigo = TRIM(s-coddiv) + "|" + ~
TRIM(s-codfam) + "|" + TRIM(s-subfam) NO-LOCK ~
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.FacTabla WHERE INTEGRAL.GN-DIVI ... ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "FacTabla.codcia = s-codcia
 AND FacTabla.Tabla = s-Tabla
 AND FacTabla.Codigo = TRIM(s-coddiv) + ""|"" +
TRIM(s-codfam) + ""|"" + TRIM(s-subfam)"
