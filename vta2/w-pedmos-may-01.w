...
DEFINE NEW SHARED VARIABLE s-TpoPed  AS CHAR INIT "".
...
/* END.                                                                                                          */
  ...
  RUN vtagn/p-alm-despacho (s-coddiv, YES, "", OUTPUT s-codalm).
  IF s-codalm = '' THEN DO:
      MESSAGE 'NO se han definido los almacenes de ventas para la división' s-coddiv
...
&Scoped-Define ENABLED-OBJECTS RECT-73 COMBO-NroSer BTN-Excel BTN-COTIZA ~
Btn-Grupos 
...
DEFINE VARIABLE h_t-pedmos-may-alm AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-pedmos-may-res AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-pedmos-may AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BTN-Excel AT ROW 2.88 COL 132 WIDGET-ID 6
     BTN-COTIZA AT ROW 4.23 COL 132 WIDGET-ID 8
     Btn-Grupos AT ROW 10.69 COL 37 WIDGET-ID 2
     RECT-73 AT ROW 2.62 COL 2 WIDGET-ID 16
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 149.14 BY 26.58
         FONT 4.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "PEDIDO MOSTRADOR MAYORISTA"
         HEIGHT             = 26.58
         WIDTH              = 149.14
         MAX-HEIGHT         = 27
         MAX-WIDTH          = 160.57
         VIRTUAL-HEIGHT     = 27
         VIRTUAL-WIDTH      = 160.57
         RESIZE             = no
         SCROLL-BARS        = no
         STATUS-AREA        = no
         BGCOLOR            = ?
...
ON END-ERROR OF W-Win /* PEDIDO MOSTRADOR MAYORISTA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* PEDIDO MOSTRADOR MAYORISTA */
DO:
...
DO:
  RUN Asigna-Cotizacion IN h_v-pedmos-may.
END.
...
DO:
  RUN Genera-Excel IN h_v-pedmos-may.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-pedmos-may.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedmos-may ).
       RUN set-position IN h_v-pedmos-may ( 2.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.27 , 105.00 ) */
       ...
       /* Links to SmartViewer h_v-pedmos-may. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-pedmos-may ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-pedmos-may ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv02 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedmos-may ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedido ).
       RUN set-position IN h_b-pedido ( 10.42 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.42 , 34.14 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-pedmos-may-res.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmos-may-res ).
       RUN set-position IN h_t-pedmos-may-res ( 11.77 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedmos-may-res ( 10.50 , 113.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12-2 ).
       RUN set-position IN h_p-updv12-2 ( 11.77 , 115.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12-2 ( 1.15 , 34.14 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-pedmos-may-alm.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmos-may-alm ).
       RUN set-position IN h_t-pedmos-may-alm ( 12.85 , 115.00 ) NO-ERROR.
       RUN set-size IN h_t-pedmos-may-alm ( 6.69 , 34.00 ) NO-ERROR.
       ...
       RUN set-position IN h_t-pedido-div ( 22.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedido-div ( 4.31 , 74.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-pedmos-may-res. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedmos-may-res ).

       /* Links to SmartBrowser h_t-pedmos-may-alm. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12-2 , 'TableIO':U , h_t-pedmos-may-alm ).
       RUN add-link IN adm-broker-hdl ( h_t-pedmos-may-res , 'Record':U , h_t-pedmos-may-alm ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             BTN-COTIZA:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedmos-may-res ,
             Btn-Grupos:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12-2 ,
             h_t-pedmos-may-res , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedmos-may-alm ,
             h_p-updv12-2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedido-div ,
             h_t-pedmos-may-alm , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-73 COMBO-NroSer BTN-Excel BTN-COTIZA Btn-Grupos 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-pedmos-may-res <> ? THEN RUN dispatch IN h_t-pedmos-may-res ('open-query':U).
          IF h_t-pedido-div <> ? THEN RUN dispatch IN h_t-pedido-div ('open-query').
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-pedmos-may-res <> ? THEN RUN Recalcular-Precios IN h_t-pedmos-may-res.
        /*IF h_t-pedmos-mayb <> ? THEN RUN Recalcular-Precios IN h_t-pedmos-mayb.*/
      END.
    ...
    WHEN "Pinta-Resumen-Producto" THEN DO:
        RUN dispatch IN h_t-pedmos-may-res ('display-fields').
        RUN Imp-Total IN h_t-pedmos-may-res.
    END.
