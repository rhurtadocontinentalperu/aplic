...
&Scoped-Define ENABLED-OBJECTS RECT-69 COMBO-NroSer B-AGTRANS 
...
DEFINE VARIABLE h_t-orddes AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     B-AGTRANS AT ROW 3.15 COL 131 WIDGET-ID 2
     RECT-69 AT ROW 2.62 COL 2 WIDGET-ID 8
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 148.57 BY 27.19
         FONT 4.
...
DO:
   RUN Asigna-datos IN h_v-orddes.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-orddes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-orddes ).
       RUN set-position IN h_v-orddes ( 2.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.08 , 106.86 ) */
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_v-orddes ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-orddes ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-orddes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-orddes ).
       RUN set-position IN h_t-orddes ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-orddes ( 16.88 , 147.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-orddes ,
             B-AGTRANS:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-69 COMBO-NroSer B-AGTRANS 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-orddes <> ? THEN RUN dispatch IN h_t-orddes ('open-query':U).
      END.
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      END.
    WHEN "Recalcular" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-orddes <> ? THEN RUN Recalcular IN h_t-orddes.
      END.
