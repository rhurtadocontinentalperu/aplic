...
DEFINE TEMP-TABLE APROBADOS NO-UNDO LIKE FacCPedi.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: APROBADOS T "?" NO-UNDO INTEGRAL FacCPedi
...
FOR EACH FacCPedi WHERE {&Condicion} USE-INDEX Llave09 NO-LOCK, FIRST GN-DIVI OF FacCPedi NO-LOCK:
    IF NOT (s-busqueda = '' OR (i-tipo-busqueda = 1 AND faccpedi.nomcli BEGINS s-busqueda) 
            OR (faccpedi.nomcli MATCHES s-busqueda))
        THEN NEXT.
    lOrdenCompra = "*".
...
FIND gn-divi WHERE gn-divi.codcia = s-codcia
    ...
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE gn-divi OR gn-divi.campo-dec[5] <= 0 THEN DO:
    MESSAGE 'NO está definido el factor de consolidación'
        VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.

MESSAGE 'Se va a proceder a consolidar las O/D seleccionadas' SKIP
    'Se van a agrupar hasta 10 O/D que superen el' x-Factor '% de coincidencias' SKIP
    'Continuamos con el proceso?' VIEW-AS ALERT-BOX QUESTION
    BUTTONS YES-NO UPDATE rpta AS LOG.
IF rpta = NO THEN RETURN.

SESSION:SET-WAIT-STATE('GENERAL').
/* 1ro seleccionamos las ordenes a trabajar */
EMPTY TEMP-TABLE T-CPEDI.
EMPTY TEMP-TABLE T-DPEDI.
EMPTY TEMP-TABLE APROBADOS.

DO k = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS:
    IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(k) THEN DO:
        CREATE T-CPEDI.
        BUFFER-COPY ORDENES TO T-CPEDI.
    END.
END.
/* Buscamos cuales son válidas de acuerdo a la cantidad de items 
    y no mas de 10 Ordenes y que cubra el 50% de los items */
DEF VAR x-CuentaODs AS INT INIT 1 NO-UNDO.
DEF VAR x-CuentaItems AS INT NO-UNDO.
DEF VAR x-TotalItems  AS INT NO-UNDO.
/* Se va a hacer 2 pasadas */
DEF VAR x-Pasada AS INT NO-UNDO.
CICLO:
DO x-Pasada = 1 TO 2:
    FOR EACH T-CPEDI BY T-CPEDI.Libre_d02 DESC:     /* De mayor cantidad de items al menor */
        IF x-CuentaODs = 1 THEN DO:
            ...
            CREATE APROBADOS.
            BUFFER-COPY T-CPEDI TO APROBADOS.
            DELETE T-CPEDI.
            NEXT.
        END.
        x-CuentaItems = 0.
        x-TotalItems = 0.
        FOR EACH Facdpedi OF T-CPEDI NO-LOCK:
            FIND FIRST T-DPEDI WHERE T-DPEDI.codmat = Facdpedi.codmat NO-LOCK NO-ERROR.
            IF AVAILABLE T-DPEDI THEN x-CuentaItems = x-CuentaItems + 1.
            x-TotalItems = x-TotalItems + 1.
        END.
        IF x-CuentaItems < x-TotalItems * x-Factor / 100 THEN DO:
            /*DELETE T-CPEDI.*/
            NEXT.
        END.
        FOR EACH Facdpedi OF T-CPEDI NO-LOCK:
        ...
        DELETE T-CPEDI.
        x-CuentaODs = x-CuentaODS + 1.
        IF x-CuentaODS > 10 THEN LEAVE CICLO.
    END.
END.
IF x-CuentaODS <= 2 THEN DO:
    SESSION:SET-WAIT-STATE('').
    MESSAGE 'Las Ordenes Seleccionadas NO cumplen los requisitos para ser consolidadas'
        VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.
EMPTY TEMP-TABLE T-CPEDI.
FOR EACH APROBADOS:
    /*MESSAGE aprobados.nroped.*/
    CREATE T-CPEDI.
    BUFFER-COPY APROBADOS TO T-CPEDI.
END.
/*RETURN.*/
...
DO TRANSACTION ON ERROR UNDO, RETURN ERROR ON STOP UNDO, RETURN ERROR WITH FRAME {&FRAME-NAME}:
    ...
    /* Detalle */
    x-NroItm = 0.
    FOR EACH T-CPEDI, FIRST Faccpedi OF T-CPEDI:
