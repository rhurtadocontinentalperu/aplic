...
DEFINE VARIABLE h_tcotizaciontablet AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vcotizaciontablet AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     B-CLI AT ROW 1 COL 81 WIDGET-ID 22
     B-CCTE AT ROW 1 COL 106 WIDGET-ID 12
     BUTTON-PED AT ROW 1 COL 123 WIDGET-ID 24
     COMBO-NroSer AT ROW 1.27 COL 51.71 WIDGET-ID 4
     Btn-Grupos-2 AT ROW 19.31 COL 32 WIDGET-ID 30
     "Buscar:" VIEW-AS TEXT
          SIZE 6 BY .5 AT ROW 1.54 COL 65 WIDGET-ID 6
     RECT-24 AT ROW 2.62 COL 1 WIDGET-ID 8
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 133.14 BY 19.38
...
/* SETTINGS FOR BUTTON BUTTON-PED IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       BUTTON-PED:HIDDEN IN FRAME F-Main           = TRUE.
...
DO:
   RUN Actualiza-datos-cliente IN h_vcotizaciontablet.
END.
...
&Scoped-define SELF-NAME BUTTON-PED
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-PED W-Win
ON CHOOSE OF BUTTON-PED IN FRAME F-Main /* PEDIDO */
DO:
  RUN Generar-Pedido IN h_vcotizaciontablet.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_p-updv09 ( 1.62 , 50.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 2.62 , 126.00 ) NO-ERROR.
       RUN set-size IN h_p-navico-2 ( 5.38 , 8.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/vcotizaciontablet.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vcotizaciontablet ).
       RUN set-position IN h_vcotizaciontablet ( 2.88 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.85 , 122.43 ) */
       ...
       /* Links to SmartViewer h_vcotizaciontablet. */
       RUN add-link IN adm-broker-hdl ( h_p-updv09 , 'TableIO':U , h_vcotizaciontablet ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_vcotizaciontablet ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv09 ,
             B-CLI:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico-2 ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vcotizaciontablet ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_bcotizaciontablet ( 8.00 , 1.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_bcotizaciontablet ,
             h_vcotizaciontablet , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/tcotizaciontablet.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tcotizaciontablet ).
       RUN set-position IN h_tcotizaciontablet ( 8.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_tcotizaciontablet ( 11.31 , 125.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 18.50 , 1.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.62 , 30.00 ) NO-ERROR.

       /* Links to SmartBrowser h_tcotizaciontablet. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tcotizaciontablet ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tcotizaciontablet ,
             h_vcotizaciontablet , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tcotizaciontablet , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         ASSIGN
             ...
             /*BUTTON-PED:SENSITIVE = NO*/
             .
         RUN dispatch IN h_q-pedido ('disable':U).
         RUN dispatch IN h_tcotizaciontablet ('open-query':U).
      END.
    WHEN "Disable-Head" THEN DO:
        ...
        RUN dispatch IN h_vcotizaciontablet ('disable-fields':U).
    END.
    WHEN "Enable-Head" THEN DO:
        ...
        RUN dispatch IN h_vcotizaciontablet ('enable-fields':U).
    END.
    WHEN "browse" THEN DO:
          IF h_bcotizaciontablet <> ? THEN RUN dispatch IN h_bcotizaciontablet ('open-query':U).
          IF h_tcotizaciontablet <> ? THEN RUN dispatch IN h_tcotizaciontablet ('open-query':U). 
      END.
    WHEN "Recalculo" THEN RUN Recalcular-Precios IN h_vcotizaciontablet.
