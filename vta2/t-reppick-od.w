...
Almmmatg.DesMat PEDI.CanPed PEDI.canate PEDI.UndVta 
...
&Scoped-define QUERY-STRING-br_table FOR EACH PEDI OF FacCPedi WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Almmmatg OF PEDI NO-LOCK ~
    BY PEDI.NroItm
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH PEDI OF FacCPedi WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Almmmatg OF PEDI NO-LOCK ~
    BY PEDI.NroItm.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      PEDI.CanPed COLUMN-LABEL "Cantidad a Pickear" FORMAT ">,>>>,>>9.9999":U
      PEDI.canate COLUMN-LABEL "Cantidad Pedida" FORMAT ">,>>>,>>9.9999":U
            COLUMN-FGCOLOR 7 COLUMN-BGCOLOR 11
      PEDI.UndVta FORMAT "x(6)":U
  ENABLE
      PEDI.CanPed
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 97 BY 13.19
         FONT 4.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.PEDI OF INTEGRAL.FacCPedi,INTEGRAL.Almmmatg OF Temp-Tables.PEDI"
...
"PEDI.CanPed" "Cantidad a Pickear" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.PEDI.canate
"PEDI.canate" "Cantidad Pedida" ? "decimal" 11 7 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.PEDI.UndVta
"PEDI.UndVta" ? "x(6)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF INPUT BROWSE {&BROWSE-NAME} PEDI.CanPed = 0 THEN DO:
    MESSAGE 'Debe ingresar la cantidad' VIEW-AS ALERT-BOX ERROR.
    APPLY 'entry' TO PEDI.CanPed IN BROWSE {&BROWSE-NAME}.
    RETURN 'ADM-ERROR'.
END.
IF INPUT BROWSE {&BROWSE-NAME} PEDI.CanPed > INPUT BROWSE {&BROWSE-NAME} PEDI.CanAte THEN DO:
    MESSAGE 'No puede sacar más de lo que se ha pedido' VIEW-AS ALERT-BOX ERROR.
    APPLY 'entry' TO PEDI.CanPed IN BROWSE {&BROWSE-NAME}.
    RETURN 'ADM-ERROR'.
END.
RETURN "OK".
