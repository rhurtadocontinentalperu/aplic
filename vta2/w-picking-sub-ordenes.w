...
&SCOPED-DEFINE Condicion VtaCDocu.CodCia = s-codcia ~
    ...
    AND DATE(DATETIME(ENTRY(2, Vtacdocu.Libre_c03, '|'))) = x-FchPicking
...
&Scoped-Define ENABLED-OBJECTS RADIO-SET-CodDoc FILL-IN-NroPed rsCuales ~
COMBO-BOX-2 BROWSE-2 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-CodDoc FILL-IN-NroPed rsCuales ~
COMBO-BOX-2 
...
DEFINE FRAME F-Main
     ...
     rsCuales AT ROW 3.12 COL 48 NO-LABEL WIDGET-ID 12
...
OR RETURN OF FILL-IN-NroPed DO:
    ...
    IF NOT (Vtacdocu.flgest = 'P' AND Vtacdocu.flgsit = 'T') THEN DO:
        ...
        RETURN NO-APPLY.
    END.
    /* Buscamos si la tarea está activa */
    IF NOT CAN-FIND(FIRST PikTareas WHERE PikTareas.CodCia = s-codcia
                    AND PikTareas.CodDiv = s-coddiv
                    AND PikTareas.CodPed = RADIO-SET-CodDoc
                    AND PikTareas.NroPed = FILL-IN-NroPed
                    AND PikTareas.FlgEst = 'A' NO-LOCK)
        THEN DO:
        MESSAGE 'La Sub-Orden AUN NO ha sido asignada para sacado' VIEW-AS ALERT-BOX ERROR.
        SELF:SCREEN-VALUE = ''.
        RETURN NO-APPLY.
    END.
/*     IF VtaCDocu.FecSac = ? THEN DO:                                                      */
/*         MESSAGE 'Sub-Orden AUN NO ha sido asignada para sacado' VIEW-AS ALERT-BOX ERROR. */
/*         SELF:SCREEN-VALUE = ''.                                                          */
/*         RETURN NO-APPLY.                                                                 */
/*     END.                                                                                 */
    ...
    RUN vta2\d-cierre-picking-sub-ordenes ( ROWID(Vtacdocu),
                                            OUTPUT x-UsrChq,
                                            OUTPUT pZonaPickeo).
    IF x-UsrChq = "" AND pZonaPickeo = "" THEN RETURN NO-APPLY.

    pError = "".
    RUN Cierre-de-guia.
    IF pError > "" THEN MESSAGE pError VIEW-AS ALERT-BOX ERROR.
    /*IF RETURN-VALUE = 'ADM-ERROR' THEN MESSAGE 'NO se pudo cerrar la sub-orden' VIEW-AS ALERT-BOX ERROR.*/
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      IF NOT AVAILABLE Vtacdocu THEN DO:
          pError = "NO se pudo bloquear la sub-orden " + RADIO-SET-CodDoc + " " + FILL-IN-NroPed.
          UNDO, LEAVE CICLO.
      END.
      /* Volvemos a chequear las condiciones */
      IF NOT (Vtacdocu.flgest = 'P' AND Vtacdocu.flgsit = 'T') THEN DO:
          pError =  'ERROR en la Sub-Orden Ya NO está pendiente de Pickeo'.
          UNDO, LEAVE CICLO.
      END.
      ASSIGN 
          Vtacdocu.flgsit = 'P'         /* Pickeado */
      ...
      IF NOT AVAILABLE PikTareas THEN DO:
          pError = "NO existe la tarea o ya fue cerrada".
          UNDO, LEAVE CICLO.
      END.
      ASSIGN  
          /*Piktareas.codper    = x-usrchq*/
          Piktareas.fchfin = NOW
          Piktareas.usuariofin = s-user-id
          PikTareas.FlgEst = "C".       /* Tarea CERRADA */
      ...
      FIND FIRST PikSacadores WHERE PikSacadores.codcia = s-codcia 
          ...
          AND PikSacadores.codper = PikTareas.CodPer    /*x-usrchq */
          AND PikSacadores.flgtarea = 'O' 
          AND PikSacadores.flgest = 'A' 
          EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
      IF NOT AVAILABLE piksacadores THEN DO:
          pError = 'Pickeador no esta disponible o no esta activo, código: ' + PikTareas.CodPer.
          UNDO, LEAVE CICLO.
      END.
      ASSIGN  
      ...
      IF AVAILABLE gn-divi AND GN-DIVI.FlgBarras = NO THEN Vtacdocu.FlgSit = "C".
      /* ********************************************* */
      /* RHC 25.08.2011 */
      IF Vtacdocu.CodPed = 'O/M' AND Vtacdocu.FlgSit = "C" THEN Vtacdocu.FlgEst = "C".
      /* TRACKING */
      ...
      IF NOT AVAILABLE Faccpedi THEN DO:
          ...
          UNDO, LEAVE CICLO.
      END.
      IF NOT CAN-FIND(FIRST Vtacdocu WHERE Vtacdocu.codcia = Faccpedi.codcia
      ...
      FOR EACH b-vtacdocu WHERE b-vtacdocu.codcia = s-codcia 
          ...
          AND SUBSTRING(b-vtacdocu.nroped,1,9) = x-nroref :
          ASSIGN b-VtacDocu.libre_c05 = IF(lOrdenLista = YES) THEN "COMPLETADO" ELSE "FALTANTES".
      END.
  END.
  ...
  IF lOrdenLista = YES THEN DO:
      MESSAGE "O/D listo para CIERRE" VIEW-AS ALERT-BOX INFORMATION.
  END.
  RETURN "OK".
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-CodDoc FILL-IN-NroPed rsCuales COMBO-BOX-2 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RADIO-SET-CodDoc FILL-IN-NroPed rsCuales COMBO-BOX-2 BROWSE-2 
      WITH FRAME F-Main IN WINDOW W-Win.
