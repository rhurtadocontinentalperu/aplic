...
&Scoped-Define ENABLED-OBJECTS RECT-1 RADIO-SET-CodDoc BtnDone BUTTON-13 ~
...
DEFINE VARIABLE h_b-prepicking-ordenes AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-prepicking-ordenes-otr AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BtnDone AT ROW 1.19 COL 138 WIDGET-ID 10
     ...
     txtcodref AT ROW 24.08 COL 111.57 COLON-ALIGNED NO-LABEL WIDGET-ID 44
     txtNroRef AT ROW 24.08 COL 117.86 COLON-ALIGNED NO-LABEL WIDGET-ID 46
     BUTTON-PorZona AT ROW 24.27 COL 1 WIDGET-ID 6
     BUTTON-Alfabeticamente AT ROW 24.27 COL 23 WIDGET-ID 8
     txtArtSinPeso AT ROW 25.04 COL 51 COLON-ALIGNED NO-LABEL WIDGET-ID 24
     "Referencia :" VIEW-AS TEXT
          SIZE 11 BY .62 AT ROW 24.31 COL 102 WIDGET-ID 42
     ...
     "Lista de Articulos que NO tienen PESO en la O/D" VIEW-AS TEXT
          SIZE 43.14 BY .62 AT ROW 24.23 COL 54.72 WIDGET-ID 26
          BGCOLOR 15 FGCOLOR 9 FONT 6
     RECT-1 AT ROW 2.54 COL 93 WIDGET-ID 32
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 144.14 BY 25.15 WIDGET-ID 100.
...
/* CtrlFrame OCXINFO:CREATE-CONTROL from: {F0B88A90-F5DA-11CF-B545-0020AF6ED35A} type: PSTimer */
      CtrlFrame:MOVE-AFTER(BtnDone:HANDLE IN FRAME F-Main).
...
ON END-ERROR OF sW-Win /* CONTROL DE IMPRESION DE ORDENES PARA PRE-PICKING */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL sW-Win sW-Win
ON WINDOW-CLOSE OF sW-Win /* CONTROL DE IMPRESION DE ORDENES PARA PRE-PICKING */
DO:
...
DO:
  ...
  CASE s-CodDoc:
      WHEN 'O/D' OR WHEN 'O/M' THEN DO:
          CASE RADIO-SET-1:
              WHEN 1 THEN RUN envia-excel IN h_b-prepicking-ordenes.
              WHEN 2 THEN RUN envia-excel-detalle IN h_b-prepicking-ordenes.
          END CASE.
      END.
      WHEN 'OTR' THEN DO:
          CASE RADIO-SET-1:
              WHEN 1 THEN RUN envia-excel IN h_b-prepicking-ordenes-otr.
              WHEN 2 THEN RUN envia-excel-detalle IN h_b-prepicking-ordenes-otr.
          END CASE.
      END.
  END CASE.
END.
...
DO:
    ...
    CASE s-CodDoc:
        WHEN 'O/D' OR WHEN 'O/M' THEN RUN dispatch IN h_b-prepicking-ordenes ('open-query':U).
        WHEN 'OTR' THEN RUN dispatch IN h_b-prepicking-ordenes-otr ('open-query':U).
    END CASE.
...
DO:
  CASE s-CodDoc:
      WHEN 'O/D' OR WHEN 'O/M' THEN RUN Imprimir-Formato IN h_b-prepicking-ordenes ( INPUT "ALFABETICO").
      WHEN 'OTR' THEN RUN Imprimir-Formato IN h_b-prepicking-ordenes-otr ( INPUT "ALFABETICO").
  END CASE.
END.
...
DO:
    CASE s-CodDoc:
        WHEN 'O/D' OR WHEN 'O/M' THEN RUN Imprimir-Formato IN h_b-prepicking-ordenes ( INPUT "ZONA").
        WHEN 'OTR' THEN RUN Imprimir-Formato IN h_b-prepicking-ordenes-otr ( INPUT "ZONA").
    END CASE.
...
------------------------------------------------------------------------------*/
     ...
     RUN dispatch IN h_b-prepicking-ordenes ('open-query':U).
...
DO:
  ...
  CASE s-CodDoc:
      ...
      WHEN "OTR" THEN DO:
           RUN select-page('2').
           RUN dispatch IN h_b-prepicking-ordenes-otr ('open-query':U).
      END.
  END CASE.
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-listado-prepicking-ordenes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = FchPed':U ,
             OUTPUT h_b-prepicking-ordenes ).
       RUN set-position IN h_b-prepicking-ordenes ( 4.27 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-prepicking-ordenes ( 7.50 , 144.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b-dimprime-od ( 11.96 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-dimprime-od ( 11.92 , 144.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-dimprime-od. */
       RUN add-link IN adm-broker-hdl ( h_b-prepicking-ordenes , 'Record':U , h_b-dimprime-od ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-prepicking-ordenes ,
             RADIO-SET-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-od ,
             h_b-prepicking-ordenes , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-listado-prepicking-ordenes-otr.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = NroPed':U ,
             OUTPUT h_b-prepicking-ordenes-otr ).
       RUN set-position IN h_b-prepicking-ordenes-otr ( 4.08 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-prepicking-ordenes-otr ( 7.50 , 144.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b-dimprime-od-2 ( 11.96 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-dimprime-od-2 ( 11.92 , 144.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-dimprime-od-2. */
       RUN add-link IN adm-broker-hdl ( h_b-prepicking-ordenes-otr , 'Record':U , h_b-dimprime-od-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-prepicking-ordenes-otr ,
             RADIO-SET-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-od-2 ,
             h_b-prepicking-ordenes-otr , 'AFTER':U ).
    END. /* Page 2 */
...
OCXFile = SEARCH( "w-listado-prepicking-ordenes.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-listado-prepicking-ordenes.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 RADIO-SET-CodDoc BtnDone BUTTON-13 txtDesde txtHasta ChkChequeo 
