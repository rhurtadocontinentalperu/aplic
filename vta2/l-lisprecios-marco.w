...
&SCOPED-DEFINE CONDICION ( Almmmatp.Codfam BEGINS F-Codfam ~
AND Almmmatp.Subfam BEGINS F-Subfam ~
AND Almmmatp.DesMar BEGINS F-marca )

&SCOPED-DEFINE CODIGO Almmmatp.codmat

/* Preprocesadores para cada campo filtro */ 
&SCOPED-DEFINE FILTRO1 Almmmatp.DesMat BEGINS FILL-IN-filtro
&SCOPED-DEFINE FILTRO2 INDEX ( Almmmatp.DesMat , FILL-IN-filtro ) <> 0 
...
&Scoped-define INTERNAL-TABLES Almmmatp Almmmatg Almmmate

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table Almmmatp.codfam Almmmatp.subfam ~
Almmmatp.codmat Almmmatp.DesMat Almmmatp.DesMar F-PRESOLA @ Almmmatp.PreOfi ~
Almmmatp.Chr__01 _StkAct() @ F-STKACT ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almmmatp WHERE ~{&KEY-PHRASE} ~
 ...
 AND (RADIO-SET-Stock = 'T' OR Almmmate.StkAct > 0) NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table Almmmatp Almmmatg Almmmate
...
<FOREIGN-KEYS>
Nombres que inicien con|y||INTEGRAL.Almmmatp.DesMat
Nombres que contengan|y||INTEGRAL.Almmmatp.DesMat
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Codigo|||INTEGRAL.Almmmatp.CodCia|yes,INTEGRAL.Almmmatp.codmat|yes
Familia|||INTEGRAL.Almmmatp.codfam|yes
Descripcion|y||INTEGRAL.Almmmatp.DesMat|yes
Marca|||INTEGRAL.Almmmatp.DesMar|yes
Sub-Familia|||INTEGRAL.Almmmatp.codfam|yes,INTEGRAL.Almmmatp.subfam|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Almmmatp.codfam COLUMN-LABEL "Cód!Fam" FORMAT "X(3)":U
      Almmmatp.subfam COLUMN-LABEL "Sub!Fam" FORMAT "X(3)":U
      Almmmatp.codmat COLUMN-LABEL "Codigo!Articulo" FORMAT "X(6)":U
      Almmmatp.DesMat FORMAT "X(80)":U WIDTH 50.14
      Almmmatp.DesMar COLUMN-LABEL "Marca" FORMAT "X(30)":U WIDTH 19.86
      F-PRESOLA @ Almmmatp.PreOfi COLUMN-LABEL "Precio Venta S/." FORMAT ">>>,>>9.9999":U
      Almmmatp.Chr__01 COLUMN-LABEL "Unidad!Venta" FORMAT "X(8)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 124 BY 10.38
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Almmmatp,INTEGRAL.Almmmatg OF INTEGRAL.Almmmatp,INTEGRAL.Almmmate OF INTEGRAL.Almmmatg"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",, FIRST, FIRST"
     _Where[1]         = "{&CONDICION}"
     _Where[3]         = "Almmmate.CodAlm = x-codalm
 AND (RADIO-SET-Stock = 'T' OR Almmmate.StkAct > 0)"
     _FldNameList[1]   > INTEGRAL.Almmmatp.codfam
"Almmmatp.codfam" "Cód!Fam" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almmmatp.subfam
"Almmmatp.subfam" "Sub!Fam" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.Almmmatp.codmat
"Almmmatp.codmat" "Codigo!Articulo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.Almmmatp.DesMat
"Almmmatp.DesMat" ? "X(80)" "character" ? ? ? ? ? ? no ? no no "50.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.Almmmatp.DesMar
"Almmmatp.DesMar" "Marca" ? "character" ? ? ? ? ? ? no ? no no "19.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"F-PRESOLA @ Almmmatp.PreOfi" "Precio Venta S/." ">>>,>>9.9999" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.Almmmatp.Chr__01
"Almmmatp.Chr__01" "Unidad!Venta" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  IF Almmmatg.MonVta = 1 THEN DO:
    F-PRESOLA = Almmmatp.PreOfi.
    F-PREUSSA = Almmmatp.PreOfi / Almmmatp.TpoCmb.
  END.
  ELSE DO:
    F-PREUSSA = Almmmatp.PreOfi.
    F-PRESOLA = Almmmatp.PreOfi * Almmmatp.TpoCmb.
  END.
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Codigo':U THEN DO:
           &Scope SORTBY-PHRASE BY Almmmatp.CodCia BY Almmmatp.codmat
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Familia':U THEN DO:
           &Scope SORTBY-PHRASE BY Almmmatp.codfam
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripcion':U THEN DO:
           &Scope SORTBY-PHRASE BY Almmmatp.DesMat
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Marca':U THEN DO:
           &Scope SORTBY-PHRASE BY Almmmatp.DesMar
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Sub-Familia':U THEN DO:
           &Scope SORTBY-PHRASE BY Almmmatp.codfam BY Almmmatp.subfam
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
