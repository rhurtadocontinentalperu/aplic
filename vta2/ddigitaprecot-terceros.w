...
DEFINE VARIABLE h_bdigitaprecot3rosprov AS HANDLE NO-UNDO.
DEFINE VARIABLE h_bdigitaprecoti3rosdetalle AS HANDLE NO-UNDO.
...
DO:
  RUN Graba-Datos IN h_bdigitaprecoti3rosdetalle.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/bdigitaprecot3rosprov.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bdigitaprecot3rosprov ).
       RUN set-position IN h_bdigitaprecot3rosprov ( 2.92 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bdigitaprecot3rosprov ( 8.46 , 74.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/bdigitaprecoti3rosdetalle.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bdigitaprecoti3rosdetalle ).
       RUN set-position IN h_bdigitaprecoti3rosdetalle ( 11.58 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bdigitaprecoti3rosdetalle ( 13.23 , 137.00 ) NO-ERROR.

       /* Links to SmartBrowser h_bpagcatvtas. */
       RUN add-link IN adm-broker-hdl ( h_bdigitaprecot3rosprov , 'Record':U , h_bpagcatvtas ).

       /* Links to SmartBrowser h_bdigitaprecoti3rosdetalle. */
       RUN add-link IN adm-broker-hdl ( h_bpagcatvtas , 'Record':U , h_bdigitaprecoti3rosdetalle ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bdigitaprecot3rosprov ,
             x-importe:HANDLE , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_bpagcatvtas ,
             h_bdigitaprecot3rosprov , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_bdigitaprecoti3rosdetalle ,
...
------------------------------------------------------------------------------*/
    ...
    CASE pParameter:
        ...
        WHEN 'Open-Query-3' THEN DO:
            RUN dispatch IN h_bdigitaprecoti3rosdetalle ('open-query').
        END.
    END CASE.
