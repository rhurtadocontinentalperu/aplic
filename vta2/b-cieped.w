...
&Scoped-define OPEN-QUERY-br_table CASE COMBO-BOX-1:     WHEN 'Todos' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc          AND LOOKUP(FacCPedi.FlgEst, ~
       s-flgest) > 0 NO-LOCK BY Faccpedi.FchPed DESC.     WHEN 'Inicien con' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc          AND LOOKUP(FacCPedi.FlgEst, ~
       s-flgest) > 0          AND FacCPedi.NomCli BEGINS x-NomCli NO-LOCK BY Faccpedi.FchPed DESC.     WHEN 'Contengan' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc          AND LOOKUP(FacCPedi.FlgEst, ~
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 69 BY 6.69
         FONT 4.
...
CASE COMBO-BOX-1:
    WHEN 'Todos' THEN
        OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}
         ...
         AND LOOKUP(FacCPedi.FlgEst, s-flgest) > 0 NO-LOCK BY Faccpedi.FchPed DESC.
    WHEN 'Inicien con' THEN
        OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}
              AND FacCPedi.CodCia = s-codcia
         AND FacCPedi.CodDiv = s-coddiv
         AND FacCPedi.CodDoc = s-coddoc
         AND LOOKUP(FacCPedi.FlgEst, s-flgest) > 0
...
------------------------------------------------------------------------------*/
  ...
  MESSAGE 'Desea CERRAR el Pedido?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE rpta AS LOG.
