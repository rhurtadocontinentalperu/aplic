...
DEF VAR s-Tabla AS CHAR INIT 'CN-GN' NO-UNDO.
...
&Scoped-define INTERNAL-TABLES VtaTabla B-TABLA

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table VtaTabla.Llave_c2 B-TABLA.Nombre ~
VtaTabla.Libre_c01 
...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaTabla WHERE VtaTabla.Llave_c1 = almtabla.Codigo ~
 ...
 AND VtaTabla.Tabla = s-Tabla NO-LOCK, ~
      FIRST B-TABLA WHERE B-TABLA.Codigo = VtaTabla.Llave_c2 ~
      AND B-TABLA.Tabla = "GN" OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH VtaTabla WHERE VtaTabla.Llave_c1 = almtabla.Codigo ~
 ...
 AND VtaTabla.Tabla = s-Tabla NO-LOCK, ~
      ...
      AND B-TABLA.Tabla = "GN" OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table VtaTabla B-TABLA
&Scoped-define FIRST-TABLE-IN-QUERY-br_table VtaTabla
&Scoped-define SECOND-TABLE-IN-QUERY-br_table B-TABLA
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c2 COLUMN-LABEL "Giro" FORMAT "x(8)":U
      B-TABLA.Nombre FORMAT "x(40)":U
      VtaTabla.Libre_c01 COLUMN-LABEL "Institución!Pública" FORMAT "x(8)":U
...
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 66 BY 8.08
         FONT 4
         TITLE "CONTROL GRUPO DE CLIENTES vs GIRO" FIT-LAST-COLUMN.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaTabla WHERE INTEGRAL.almtabla <external> ...,B-TABLA WHERE INTEGRAL.VtaTabla ..."
 ...
 AND VtaTabla.Tabla = s-Tabla"
     _JoinCode[2]      = "B-TABLA.Codigo = VtaTabla.Llave_c2"
     _Where[2]         = "B-TABLA.Tabla = ""GN"""
     _FldNameList[1]   > INTEGRAL.VtaTabla.Llave_c2
"VtaTabla.Llave_c2" "Giro" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = Temp-Tables.B-TABLA.Nombre
     _FldNameList[3]   > INTEGRAL.VtaTabla.Libre_c01
"VtaTabla.Libre_c01" "Institución!Pública" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," "SI,NO" ? 5 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* CONTROL GRUPO DE CLIENTES vs GIRO */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* CONTROL GRUPO DE CLIENTES vs GIRO */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* CONTROL GRUPO DE CLIENTES vs GIRO */
DO:
...
ON LEAVE OF VtaTabla.Llave_c2 IN BROWSE br_table /* Giro */
DO:
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      RUN lib/logtabla (INPUT 'vtatabla',
                        INPUT STRING(VtaTabla.CodCia,'999') + '|' +
                        VtaTabla.Tabla + '|' +
                        VtaTabla.Llave_c1 + '|' + 
                        VtaTabla.Llave_c2 + '|' +
                        VtaTabla.LLave_c3,
...
IF RETURN-VALUE = 'YES' THEN DO:
    ...
    END.
END.
IF NOT CAN-FIND(FIRST B-TABLA WHERE B-TABLA.Tabla = 'GN'
                AND B-TABLA.Codigo = VtaTabla.Llave_c2:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
                NO-LOCK)
    THEN DO:
    MESSAGE 'Giro errado' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO VtaTabla.Llave_c2.
    RETURN 'ADM-ERROR'.
END.
RETURN "OK".
...
/* MESSAGE 'Acceso Inválido' VIEW-AS ALERT-BOX ERROR. */
/* RETURN "ADM-ERROR".                                */
