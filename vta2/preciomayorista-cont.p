...
/* RHC CHALACAZO CAMBIAMOS A LA DIVISION */
/* IF LOOKUP(s-CodDiv, "00061,00062") > 0 THEN s-CodDiv = "00060". */
/* IF LOOKUP(s-CodDiv, "00002,00003") > 0 THEN s-CodDiv = "00001". */
/* ************************************************************* */
...
DO:
    ...
    END.
END.
/* IF LOOKUP(Almmmatg.CodFam, "011,013") = 0 THEN DO:  /* NO Para Fotocopias ni importados */ */
/*     FIND FIRST VtaTabla WHERE VtaTabla.codcia = Almmmatg.codcia                            */
/*         AND VtaTabla.tabla = "DTOPROLIMA"                                                  */
/*         AND VtaTabla.llave_c1 = Almmmatg.codmat                                            */
/*         AND VtaTabla.llave_c2 = s-CodDiv                                                   */
...
/* RHC Válido solo hasta el 31 de Marzo del 2014 => NO Descuento por Volumen */
IF s-CodDiv = "00060"   /* Arequipa */
    AND TODAY <= 03/31/2014
    AND LOOKUP(Almmmatg.CodMat, "000979,000980,000981,000982,000983,000984") > 0
    AND y-Dsctos > 0 THEN x-CanPed = 0.
/* ********************************************* */
