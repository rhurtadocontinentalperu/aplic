...
FILL-IN-filtro B-Imprimir BUTTON-20 x-CodDiv BUTTON-Division xcodmod ~
C-CMPBTE c-Serie f-desde f-hasta x-CndCre x-FchVto-1 x-FchVto-2 ~
x-FchCan-desde x-FchCan-hasta COMBO-BOX-5 BUTTON-19 F-FMAPGO f-codven ~
br_table 
...
DEFINE FRAME F-Main
     ...
     B-Imprimir AT ROW 1.19 COL 135
     BUTTON-20 AT ROW 1.27 COL 119.14 WIDGET-ID 98
     x-CodDiv AT ROW 2.15 COL 24 NO-LABEL WIDGET-ID 90
     BUTTON-Division AT ROW 2.35 COL 87 WIDGET-ID 76
     xcodmod AT ROW 2.35 COL 100 NO-LABEL
     C-CMPBTE AT ROW 4.85 COL 24 NO-LABEL WIDGET-ID 6
     c-Serie AT ROW 4.85 COL 39 COLON-ALIGNED
     f-desde AT ROW 4.85 COL 81 COLON-ALIGNED
     f-hasta AT ROW 4.85 COL 97 COLON-ALIGNED
     x-CndCre AT ROW 5.81 COL 41 NO-LABEL
     x-FchVto-1 AT ROW 5.81 COL 81 COLON-ALIGNED
     x-FchVto-2 AT ROW 5.81 COL 97 COLON-ALIGNED
     BUTTON-4 AT ROW 6 COL 116
     x-FchCan-desde AT ROW 6.77 COL 81 COLON-ALIGNED WIDGET-ID 94
     x-FchCan-hasta AT ROW 6.77 COL 97 COLON-ALIGNED WIDGET-ID 96
     COMBO-BOX-5 AT ROW 7.81 COL 81 COLON-ALIGNED
     BUTTON-19 AT ROW 8.31 COL 122 WIDGET-ID 86
     F-FMAPGO AT ROW 8.69 COL 22 COLON-ALIGNED
     F-CndVta AT ROW 8.69 COL 29.14 COLON-ALIGNED NO-LABEL
     f-codven AT ROW 8.77 COL 81 COLON-ALIGNED
     F-nomven AT ROW 8.77 COL 87 COLON-ALIGNED NO-LABEL
     ...
     "Separado por |" VIEW-AS TEXT
          SIZE 13.57 BY .85 AT ROW 2.38 COL 120.14 WIDGET-ID 100
          FGCOLOR 9 FONT 6
     "Moneda:" VIEW-AS TEXT
          SIZE 6 BY .5 AT ROW 2.54 COL 93 WIDGET-ID 78
     ...
     "Comprobante:" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 5.04 COL 14 WIDGET-ID 8
     "N/C:" VIEW-AS TEXT
          SIZE 4 BY .5 AT ROW 5.81 COL 37 WIDGET-ID 80
...
ON CHOOSE OF BUTTON-20 IN FRAME F-Main /* Archivo de TEXTO */
DO:
  RUN texto.
END.
...
c-xls-file = 'd:\xpciman\ttCanjes.xlsx'.

run pi-crea-archivo-csv IN hProc (input  buffer ttCanjes:handle,
                        /*input  session:temp-directory + "file"*/ c-xls-file,
                        output c-csv-file) .

run pi-crea-archivo-xls  IN hProc (input  buffer ttCanjes:handle,
                        input  c-csv-file,
                        output c-xls-file) .
...
FOR EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-codcia AND (TRUE <> (x-CodDiv > '') OR LOOKUP(gn-divi.coddiv, x-CODDIV) > 0):
    DO j = 1 TO NUM-ENTRIES(x-CodDoc):
        cCodDoc = ENTRY(j, x-CodDoc).
        REPEAT x-fecha = f-desde TO f-hasta: 
