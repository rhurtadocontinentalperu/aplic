...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-ImpTot FILL-IN-ColorEntero ~
...
DEFINE FRAME F-Main
     ...
     FILL-IN-ColorEntero AT ROW 12.15 COL 100 COLON-ALIGNED WIDGET-ID 10
     FILL-IN-Cuadernos AT ROW 12.92 COL 100 COLON-ALIGNED WIDGET-ID 4
     FILL-IN-Practiforros AT ROW 13.69 COL 100 COLON-ALIGNED WIDGET-ID 6
     FILL-IN-Variacion AT ROW 13.69 COL 119 COLON-ALIGNED WIDGET-ID 8
...
ASSIGN
    ...
    FILL-IN-Variacion = 0.
FOR EACH B-ITEM NO-LOCK, FIRST Almmmatg OF B-ITEM NO-LOCK:
    CASE Almmmatg.CodFam:
        WHEN '010' THEN DO:
            IF Almmmatg.SubFam = '012' OR Almmmatg.SubFam = '013' 
                THEN FILL-IN-Cuadernos = FILL-IN-Cuadernos + B-ITEM.ImpLin.
        END.
        WHEN '012' THEN DO:
            IF Almmmatg.SubFam = '063'
                THEN FILL-IN-Practiforros = FILL-IN-Practiforros + B-ITEM.ImpLin.
        END.
    END CASE.
    IF LOOKUP(B-ITEM.CodMat, "035721,040365,039909,039905,039910,039906") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
    IF LOOKUP(B-ITEM.CodMat, "037911,045810,045811,033618,024693,040361") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
    IF LOOKUP(B-ITEM.CodMat, "033854,033853,033855,033852,035724,040364") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
    IF LOOKUP(B-ITEM.CodMat, "039908,039912,039913,039904,037912,045808") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
    IF LOOKUP(B-ITEM.CodMat, "033619,024694,040362,033861,033862,033860") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
    IF LOOKUP(B-ITEM.CodMat, "033859,035727,040366,039918,039919,039920") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
    IF LOOKUP(B-ITEM.CodMat, "039921,037918,024695,040363,036239,036236") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
    IF LOOKUP(B-ITEM.CodMat, "036238,036237,045398,024698,033268,035728") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
    IF LOOKUP(B-ITEM.CodMat, "024696,035729,024697,046171,035730,046172") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
    IF LOOKUP(B-ITEM.CodMat, "033264,046173,033263,033265,033266,026601") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
    IF LOOKUP(B-ITEM.CodMat, "026652,033259,033260") > 0
        THEN FILL-IN-ColorEntero = FILL-IN-ColorEntero + B-ITEM.ImpLin.
END.
...
/*       END.                                                                                              */
  ...
  IF s-FlgMinVenta = YES THEN DO:
      f-CanPed = DECIMAL(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&browse-name}) * f-Factor.
      FIND AlmCatVtaD WHERE AlmCatVtaD.CodCia = s-codcia
          AND AlmCatVtaD.CodDiv = s-coddiv
          AND AlmCatVtaD.codmat = ITEM.CodMat:SCREEN-VALUE IN BROWSE {&browse-name}
          NO-LOCK NO-ERROR.
      IF AVAILABLE AlmCatVtaD AND AlmCatVtaD.Libre_d02 > 0 THEN DO:
          IF f-CanPed < AlmCatVtaD.Libre_d02 THEN DO:
              MESSAGE 'Solo puede vender como mínimo' AlmCatVtaD.Libre_d02 Almmmatg.UndBas
                  VIEW-AS ALERT-BOX ERROR.
              APPLY 'ENTRY' TO ITEM.CanPed IN BROWSE {&browse-name}.
              RETURN 'ADM-ERROR'.
          END.
          /* Múltiplos */
          IF AlmCatVtaD.Libre_d03 > 0 THEN DO:
              f-CanPed = (TRUNCATE((f-CanPed / AlmCatVtaD.Libre_d03),0) * AlmCatVtaD.Libre_d03).
              IF f-CanPed <> DECIMAL(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&browse-name}) * f-Factor THEN DO:
                  MESSAGE 'Solo puede vender en empaques de' AlmCatVtaD.Libre_d03 Almmmatg.UndBas
                      VIEW-AS ALERT-BOX ERROR.
                  APPLY 'ENTRY':U TO ITEM.CanPed IN BROWSE {&browse-name}.
                  RETURN "ADM-ERROR".
              END.
...
/*       END.                                                                             */
  ...
  IF TODAY <= 10/12/2012 THEN RETURN 'OK'.  /* OJO >>> PARCHE */
