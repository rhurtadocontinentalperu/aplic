...
DEFINE TEMP-TABLE tt-vtaftrkped NO-UNDO LIKE vtaftrkped.
...
DEF INPUT PARAMETER pParametro AS CHAR.  /* Division,Lista de Precios */

DEFINE NEW SHARED VAR s-busqueda AS CHAR.
s-busqueda = pParametro.
...
DEFINE VARIABLE h_b-logtrkods AS HANDLE NO-UNDO.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: tt-vtaftrkped T "?" NO-UNDO INTEGRAL vtaftrkped
   END-TABLES.
 */
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CONSULTA DE COTIZACIONES - EXPOLIBRERIA"
         HEIGHT             = 27.08
         WIDTH              = 160
         MAX-HEIGHT         = 27.69
         MAX-WIDTH          = 178.29
...
ON END-ERROR OF W-Win /* CONSULTA DE COTIZACIONES - EXPOLIBRERIA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* CONSULTA DE COTIZACIONES - EXPOLIBRERIA */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/bconcotcre-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bconcotcre ).
       RUN set-position IN h_bconcotcre ( 1.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bconcotcre ( 10.50 , 125.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_tab95 ).
       RUN set-position IN h_tab95 ( 11.77 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tab95 ( 16.15 , 158.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-cotizacion-cred ( 13.38 , 3.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_bconcotcre-1 ( 13.12 , 4.00 ) NO-ERROR.
       ...
       RUN set-position IN h_bconpedcre-1 ( 13.12 , 94.00 ) NO-ERROR.
       RUN set-size IN h_bconpedcre-1 ( 6.19 , 62.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtaexp/b-logtrkods.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-logtrkods ).
       RUN set-position IN h_b-logtrkods ( 19.46 , 93.00 ) NO-ERROR.
       RUN set-size IN h_b-logtrkods ( 8.08 , 66.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/bconpedcre-3a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bconpedcre-2a ).
       RUN set-position IN h_bconpedcre-2a ( 19.50 , 4.00 ) NO-ERROR.
       RUN set-size IN h_bconpedcre-2a ( 8.31 , 88.00 ) NO-ERROR.

       /* Links to SmartBrowser h_bconpedcre-1. */
       RUN add-link IN adm-broker-hdl ( h_bconcotcre-1 , 'Record':U , h_bconpedcre-1 ).

       /* Links to SmartBrowser h_b-logtrkods. */
       RUN add-link IN adm-broker-hdl ( h_bconcotcre-1 , 'Record':U , h_b-logtrkods ).
...
CASE pParameter:
    ...
    WHEN 'Open-Query-3' THEN DO:
        IF h_bconpedcre-2a <> ? THEN RUN dispatch IN h_bconpedcre-2a ('open-query').
    END.
