...
FacCPedi.NomCli FacCPedi.TpoCmb FacCPedi.CodVen FacCPedi.Ubigeo[1] 
...
FacCPedi.CodVen FacCPedi.Ubigeo[1] 
...
DEFINE FRAME F-Main
     ...
     FacCPedi.Ubigeo[1] AT ROW 3.31 COL 9 COLON-ALIGNED WIDGET-ID 2 PASSWORD-FIELD 
          LABEL "Canasta" FORMAT "x(20)"
          VIEW-AS FILL-IN 
          SIZE 16 BY .81
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      ...
      IF NOT AVAILABLE Vtambasket THEN DO:
          MESSAGE 'No se pudo actualizar el control de canasta' VIEW-AS ALERT-BOX ERROR.
          UNDO, RETURN 'ADM-ERROR'.
      END.
      ASSIGN
          Vtambasket.FlgEst = 'O'     /* Ocupado */
          Vtambasket.CodDoc = s-coddoc
          Vtambasket.NroPed = Faccpedi.NroPed.
      CREATE LogTabla.
      ASSIGN
          logtabla.codcia = s-codcia
          logtabla.Dia = TODAY
          logtabla.Evento = Vtambasket.FlgEst
          logtabla.Hora = STRING(TIME, 'HH:MM:SS')
          /*logtabla.NumId */
          logtabla.Tabla = 'Vtambasket'
          logtabla.Usuario = s-user-id
          logtabla.ValorLlave = 'CANASTA|' + STRING(s-coddiv) + '|' + Vtambasket.Codigo + '|' +
                                s-coddoc + '|' + Faccpedi.NroPed.
      /* fin de bloqueo de canasta */
  END.
  ELSE DO:
...
------------------------------------------------------------------------------*/
    ...
    FIND Vtambasket WHERE Vtambasket.codcia = s-codcia
        ...
        AND Vtambasket.Codigo = FacCPedi.Ubigeo[1]
        NO-LOCK NO-ERROR.
    FOR LAST LogTabla USE-INDEX Llave01 NO-LOCK WHERE logtabla.codcia = s-codcia
        AND logtabla.Tabla = 'Vtambasket'
        AND logtabla.ValorLlave BEGINS 'CANASTA|' + s-coddiv + '|' + FacCPedi.Ubigeo[1]:
        x-CodRef = ENTRY(4,logtabla.ValorLlave,'|').
        x-NroRef = ENTRY(5,logtabla.ValorLlave,'|').
    END.
    IF NOT AVAILABLE Vtambasket 
        OR Vtambasket.FlgEst <> "O" 
        OR NOT AVAILABLE LogTabla 
        OR Faccpedi.coddoc <> x-CodRef
        OR Faccpedi.nroped <> x-NroRef
        THEN DO:
        MESSAGE 'La canasta se encuentra en otra área' SKIP
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      FIND CURRENT LogTabla EXCLUSIVE-LOCK NO-ERROR.
      IF NOT AVAILABLE LogTabla THEN RETURN 'ADM-ERROR'.
      FIND CURRENT Faccpedi EXCLUSIVE-LOCK NO-ERROR.
      IF NOT AVAILABLE Faccpedi THEN RETURN 'ADM-ERROR'.
      /* Extornos Canasta */
      ASSIGN Vtambasket.FlgEst = "L".
      DELETE LogTabla.
      /* TRACKING */
      RUN vtagn/pTracking-04 (s-CodCia,
                        s-CodDiv,
      ...
      FIND CURRENT Vtambasket NO-LOCK.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
    IF RETURN-VALUE = 'NO' THEN FacCPedi.Ubigeo[1]:SENSITIVE = NO.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME} :
    ...
    IF Faccpedi.Ubigeo[1]:SCREEN-VALUE = '' THEN DO:
        MESSAGE 'Ingrese el código de la canasta' VIEW-AS ALERT-BOX ERROR.
