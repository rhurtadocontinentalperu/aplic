...
DEFINE VARIABLE h_tpedidotiendasutilex AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vpedidotiendasutilex AS HANDLE NO-UNDO.
...
DO:
  ...
  RUN Cancelar-Pedido IN h_vpedidotiendasutilex.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vpedidotiendasutilex.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vpedidotiendasutilex ).
       RUN set-position IN h_vpedidotiendasutilex ( 2.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.19 , 131.00 ) */
       ...
       /* Links to SmartViewer h_vpedidotiendasutilex. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_vpedidotiendasutilex ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_vpedidotiendasutilex ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vpedidotiendasutilex ,
             BUTTON-Canc:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido-4 ,
             h_vpedidotiendasutilex , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-pedmos-utilex.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedmos-utilex ).
       RUN set-position IN h_b-pedmos-utilex ( 11.50 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'tpedidotiendasutilex.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tpedidotiendasutilex ).
       RUN set-position IN h_tpedidotiendasutilex ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tpedidotiendasutilex ( 14.23 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_tpedidotiendasutilex. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tpedidotiendasutilex ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tpedidotiendasutilex ,
             h_vpedidotiendasutilex , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tpedidotiendasutilex , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_tpedidotiendasutilex <> ? THEN RUN dispatch IN h_tpedidotiendasutilex ('open-query':U).
...
/*       END.                                            */
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_vpedidotiendasutilex <> ? THEN RUN Recalcular-Precios IN h_vpedidotiendasutilex.
