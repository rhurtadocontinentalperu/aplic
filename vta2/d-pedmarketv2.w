...
/*DEFINE OUTPUT PARAMETER X-FACTOR AS DECI.*/

DEFINE VARIABLE lFlag AS LOGICAL     NO-UNDO.
DEFINE VAR X-FACTOR AS DECI NO-UNDO.
ASSIGN
    x-CodMat = ?
    x-CanPed = 0
    x-Factor = 1.
...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-Unidad F-CODMAT F-CANPED F-FACTOR 
...
DEFINE VARIABLE F-CANPED AS DECIMAL FORMAT "->>,>>9.99":U INITIAL 1 
     LABEL "Cantidad" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE F-FACTOR AS DECIMAL FORMAT "->>,>>9.99":U INITIAL 1 
     LABEL "Factor" 
     VIEW-AS FILL-IN 
     SIZE 9 BY .88
...
DEFINE FRAME D-Dialog
     FILL-IN-Unidad AT ROW 2.54 COL 21 COLON-ALIGNED NO-LABEL WIDGET-ID 16
     F-CODMAT AT ROW 1.38 COL 4.14
     F-CANPED AT ROW 2.54 COL 3.14
     Btn_OK AT ROW 1.35 COL 42.57
     Btn_Done AT ROW 2.54 COL 42.57 WIDGET-ID 12
     F-FACTOR AT ROW 3.69 COL 4.86 WIDGET-ID 14
     SPACE(37.28) SKIP(0.38)
...
/* SETTINGS FOR FILL-IN F-CODMAT IN FRAME D-Dialog
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN F-FACTOR IN FRAME D-Dialog
   NO-ENABLE ALIGN-L                                                    */
/* SETTINGS FOR FILL-IN FILL-IN-Unidad IN FRAME D-Dialog
   NO-ENABLE                                                            */
...
DO:
  ...
  ASSIGN 
      ...
      x-canped = DECI(F-Canped:SCREEN-VALUE) * DECI(F-FACTOR:SCREEN-VALUE).
END.
...
DO:
    ...
    RUN alm/p-codbrr (INPUT-OUTPUT pCodMat, INPUT-OUTPUT x-Factor, s-codcia).
    ...
    END.
    DISPLAY
        Almmmatg.CHR__01   @ FILL-IN-Unidad
        WITH FRAME {&FRAME-NAME}.
END.
...
&Scoped-define SELF-NAME F-FACTOR
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-FACTOR D-Dialog
ON LEAVE OF F-FACTOR IN FRAME D-Dialog /* Factor */
DO:
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-Unidad F-CODMAT F-CANPED F-FACTOR 
      WITH FRAME D-Dialog.
