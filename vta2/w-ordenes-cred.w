...
&Scoped-Define ENABLED-OBJECTS RECT-70 COMBO-NroSer BUTTON-11 
...
DEFINE VARIABLE h_t-ordenes-cred AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-ordenes-cred AS HANDLE NO-UNDO.
...
DEFINE BUTTON BUTTON-11 
     ...
     LABEL "Button 11" 
     SIZE 7 BY 1.62 TOOLTIP "Imprimir Hoja de Transportista".
...
DEFINE FRAME F-Main
     ...
     B-AGTRANS AT ROW 7.19 COL 133 WIDGET-ID 2
     BUTTON-11 AT ROW 9.08 COL 135 WIDGET-ID 14
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         ...
         MAX-HEIGHT         = 28.31
         MAX-WIDTH          = 158.57
         VIRTUAL-HEIGHT     = 28.31
         VIRTUAL-WIDTH      = 158.57
         RESIZE             = no
         SCROLL-BARS        = no
         STATUS-AREA        = no
         BGCOLOR            = ?
...
DO:
   RUN Asigna-datos IN h_v-ordenes-cred.
   RUN dispatch IN h_v-ordenes-cred ('display-fields':U).
END.
...
DO:
     RUN Imprimir-Transportista IN h_v-ordenes-cred.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 2.35 , 134.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 4.85 , 10.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-ordenes-cred.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ordenes-cred ).
       RUN set-position IN h_v-ordenes-cred ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.69 , 128.43 ) */
       ...
       /* Links to SmartViewer h_v-ordenes-cred. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_v-ordenes-cred ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-ordenes-cred ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ordenes-cred ,
             h_p-navico , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-size IN h_b-pedido-cred ( 12.65 , 141.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-ordenes-cred.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-ordenes-cred ).
       RUN set-position IN h_t-ordenes-cred ( 12.58 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-ordenes-cred ( 12.12 , 141.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-ordenes-cred ,
             BUTTON-11:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-70 COMBO-NroSer BUTTON-11 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-ordenes-cred <> ? THEN RUN dispatch IN h_t-ordenes-cred ('open-query':U).
      END.
