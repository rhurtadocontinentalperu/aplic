...
FacCPedi.FchPed FacCPedi.FchEnt FacCPedi.CodAlm FacCPedi.ImpTot ~
_FlgEst() @ x-Estado 
...
&Scoped-define QUERY-STRING-br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacCPedi.CodRef = s-coddoc ~
 AND FacCPedi.NroRef = s-nroped ~
 /*AND FacCPedi.CodCli = s-codcli*/ ~
 AND LOOKUP(FacCPedi.FlgEst, "A,E") = 0 NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      _FlgEst() @ x-Estado COLUMN-LABEL "Estado" FORMAT "x(30)":U
            WIDTH 20.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 81 BY 5.65
         FONT 4
         TITLE "PEDIDOS RELACIONADOS" FIT-LAST-COLUMN.
...
/* Query rebuild information for BROWSE br_table
 ...
 AND FacCPedi.NroRef = s-nroped
 /*AND FacCPedi.CodCli = s-codcli*/
 AND LOOKUP(FacCPedi.FlgEst, ""A,E"") = 0"
...
"FacCPedi.FchPed" "Emision" "99/99/99" "date" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacCPedi.FchEnt
"FacCPedi.FchEnt" "Entrega" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.FacCPedi.CodAlm
...
"_FlgEst() @ x-Estado" "Estado" "x(30)" ? ? ? ? ? ? ? no ? no no "20.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEFT-MOUSE-DBLCLICK OF br_table IN FRAME F-Main /* PEDIDOS RELACIONADOS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-ENTRY OF br_table IN FRAME F-Main /* PEDIDOS RELACIONADOS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* PEDIDOS RELACIONADOS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* PEDIDOS RELACIONADOS */
DO:
  ...
  {src/adm/template/brschnge.i}
    ...
    IF AVAILABLE Faccpedi 
      ...
      */
      IF AVAILABLE Faccpedi THEN RUN Procesa-Handle IN lh_handle ('Open-Query-3':U).
...
/*RUN gn/fFlgEstPED(pFlgEst, OUTPUT pEstado).*/
