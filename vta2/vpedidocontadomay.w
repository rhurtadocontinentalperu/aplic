...
/*   END.                                                         */
  ...
  IF Gn-Clie.Canal = '006' THEN DO:
      ...
      RETURN NO-APPLY.
  END.
  s-CodCli = SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
  /* *********************************************** */
  ...
  DISPLAY 
    ...
    IF s-CodCli = "11111111111" THEN DO:        
        Faccpedi.nomCli:SCREEN-VALUE IN FRAME {&FRAME-NAME} = gn-clie.NomCli.
    END.
  ...
  FOR EACH ITEM, FIRST Almmmatg OF ITEM NO-LOCK WHERE ITEM.codmat <> x-articulo-ICBPER BY ITEM.NroItm : 
      ...
      IF s-StkDis < x-CanPed THEN DO:
          ...
          IF ITEM.Libre_C05 = "OF" THEN DO:
              /*MESSAGE "Se agotó la Promoción:" ITEM.CodMat Almmmatg.desmat VIEW-AS ALERT-BOX WARNING.*/
...
/*           IF ITEM.Libre_c05 = "OF" THEN DO:                                                                                     */
/*               pMensaje = 'NO hay stock disponible para la promoción: ' + ITEM.codmat + ' '  + Almmmatg.desmat + CHR(10) +       */
/*                           'Promoción: ' + STRING(ITEM.CanPed * ITEM.factor, '>,>>9.99') + ' ' + ITEM.UndVta + CHR(10) +         */
/*                           'Stock Disponible para el Almacén ' + ITEM.AlmDes + ':' + STRING(s-StkDis, '->>>,>>9.99') + CHR(10) + */
