...
DEFINE VARIABLE h_b-chqtransf AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW sW-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CHEQUEO DE GUIAS POR TRANSFERENCIAS"
         HEIGHT             = 17.04
         WIDTH              = 115
         MAX-HEIGHT         = 32.31
         MAX-WIDTH          = 205.72
...
ON END-ERROR OF sW-Win /* CHEQUEO DE GUIAS POR TRANSFERENCIAS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL sW-Win sW-Win
ON WINDOW-CLOSE OF sW-Win /* CHEQUEO DE GUIAS POR TRANSFERENCIAS */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-chqtransf.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-chqtransf ).
       RUN set-position IN h_b-chqtransf ( 1.27 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-chqtransf ( 15.88 , 95.29 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-chqtransf. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-chqtransf ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-chqtransf ,
             Btn_Done:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_b-chqtransf , 'AFTER':U ).
    END. /* Page 0 */
