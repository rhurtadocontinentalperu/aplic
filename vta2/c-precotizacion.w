...
&SCOPED-DEFINE l-lookup h_l-precotizacion
...
DEFINE VARIABLE h_l-precotizacion AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.69 COL 74
     Btn_Cancel AT ROW 8.88 COL 74
     Btn_Help AT ROW 10.69 COL 74
     SPACE(2.42) SKIP(0.18)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/l-precotizacion.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-precotizacion ).
       RUN set-position IN h_l-precotizacion ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-precotizacion ( 10.92 , 71.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.54 , 72.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-precotizacion. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-precotizacion ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-precotizacion ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-precotizacion , 'AFTER':U ).
    END. /* Page 0 */
