...
DEFINE NEW SHARED VAR i-tipo-busqueda AS INT INIT 0.
...
txtDesde txtHasta rbtn_cliente ChkChequeo BtnExcel RADIO-SET-1 txtCliente ~
txtOrdenCompra chbxImpresos BUTTON-PorZona BUTTON-Alfabeticamente 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-CodDoc txtDesde txtHasta ~
rbtn_cliente ChkChequeo RADIO-SET-1 txtCliente txtOrdenCompra chbxImpresos ~
...
DEFINE VARIABLE rbtn_cliente AS INTEGER 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "Que Empieze", 1,
"Que Contenga", 2
     SIZE 35.43 BY .77 NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-13 AT ROW 1.19 COL 119 WIDGET-ID 4
     txtDesde AT ROW 2.5 COL 6.57 COLON-ALIGNED WIDGET-ID 16
     txtHasta AT ROW 2.5 COL 27 COLON-ALIGNED WIDGET-ID 20
     rbtn_cliente AT ROW 2.62 COL 76.86 NO-LABEL WIDGET-ID 54
     ChkChequeo AT ROW 2.69 COL 47.86 WIDGET-ID 22
     BtnExcel AT ROW 3.08 COL 113.86 WIDGET-ID 34
     RADIO-SET-1 AT ROW 3.08 COL 128.86 NO-LABEL WIDGET-ID 28
     txtCliente AT ROW 3.5 COL 75 COLON-ALIGNED NO-LABEL WIDGET-ID 52
     txtOrdenCompra AT ROW 3.58 COL 27 COLON-ALIGNED WIDGET-ID 48
     chbxImpresos AT ROW 3.58 COL 47.86 WIDGET-ID 50
     BUTTON-PorZona AT ROW 24.65 COL 3 WIDGET-ID 6
     BUTTON-Alfabeticamente AT ROW 24.65 COL 25 WIDGET-ID 8
     txtcodref AT ROW 24.65 COL 113.86 COLON-ALIGNED NO-LABEL WIDGET-ID 44
     txtNroRef AT ROW 24.65 COL 120.14 COLON-ALIGNED NO-LABEL WIDGET-ID 46
     txtArtSinPeso AT ROW 25.81 COL 47 COLON-ALIGNED NO-LABEL WIDGET-ID 24
     "Referencia :" VIEW-AS TEXT
          SIZE 11 BY .62 AT ROW 24.88 COL 104.29 WIDGET-ID 42
     ...
     RECT-1 AT ROW 2.88 COL 112.86 WIDGET-ID 32
...
DO:
    ASSIGN txtDesde txtHasta chkchequeo txtOrdenCompra chbxImpresos rbtn_cliente txtCliente.
    ltxtDesde = txtDesde.
    ltxtHasta = txtHasta.
    lChequeados = chkchequeo.
    pOrdenCompra = TRIM(txtOrdenCompra).
    pSoloImpresos = chbxImpresos.
    s-busqueda = TRIM(txtCliente).
    i-tipo-busqueda = rbtn_cliente.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-listado-picking-ordenes-todos-v3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = FchPed':U ,
             OUTPUT h_b-picking-ordenes-todos ).
       RUN set-position IN h_b-picking-ordenes-todos ( 4.65 , 2.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b-dimprime-od ( 12.38 , 2.29 ) NO-ERROR.
...
OCXFile = SEARCH( "w-listado-picking-ordenes-v3x.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-listado-picking-ordenes-v3x.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-CodDoc txtDesde txtHasta rbtn_cliente ChkChequeo RADIO-SET-1 
          txtCliente txtOrdenCompra chbxImpresos txtcodref txtNroRef 
          txtArtSinPeso 
      WITH FRAME F-Main IN WINDOW sW-Win.
  ENABLE RECT-1 BtnDone RADIO-SET-CodDoc BUTTON-13 txtDesde txtHasta 
         rbtn_cliente ChkChequeo BtnExcel RADIO-SET-1 txtCliente txtOrdenCompra 
         chbxImpresos BUTTON-PorZona BUTTON-Alfabeticamente 
      WITH FRAME F-Main IN WINDOW sW-Win.
