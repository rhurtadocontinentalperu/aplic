...
DEFINE VARIABLE h_bgeneracioncomprobantes AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 2.92 COL 99 WIDGET-ID 20
     BUTTON-3 AT ROW 4.04 COL 99 WIDGET-ID 22
     BUTTON-create-2 AT ROW 5.19 COL 99 WIDGET-ID 26
     BUTTON-create AT ROW 6.35 COL 99 WIDGET-ID 24
     BUTTON-exit AT ROW 7.5 COL 99 WIDGET-ID 28
...
DO:
  RUN dispatch IN h_bgeneracioncomprobantes ('open-query':U).
END.
...
DO:
    RUN Fraccionar-Orden IN h_bgeneracioncomprobantes.
    RUN dispatch IN h_bgeneracioncomprobantes ('open-query':U).
END.
...
DO:
    RUN proc_GeneraGuia IN h_bgeneracioncomprobantes.
    RUN dispatch IN h_bgeneracioncomprobantes ('open-query':U).
END.
...
DO:
    RUN proc_GeneraGuiaManual IN h_bgeneracioncomprobantes.
    RUN dispatch IN h_bgeneracioncomprobantes ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/bgeneracioncomprobantes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bgeneracioncomprobantes ).
       RUN set-position IN h_bgeneracioncomprobantes ( 1.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bgeneracioncomprobantes ( 9.04 , 96.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_bgeneracioncomprobantes , 'Record':U , h_b-genera-factura-credb ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bgeneracioncomprobantes ,
