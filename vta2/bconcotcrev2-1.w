...
DEFINE TEMP-TABLE T-Detalle
    ...
    FIELD LugEnt    AS CHAR FORMAT 'x(50)' LABEL "Lugar de Entrega"
    FIELD DistEnt   AS CHAR FORMAT 'x(30)' LABEL "Distrito Entrega"
    FIELD CodVen LIKE gn-ven.CodVen LABEL "Vendedor"
    FIELD NomVen LIKE gn-ven.NomVen LABEL "Nombre"
    ...
    FIELD Items     AS INT FORMAT '>>>,>>9' LABEL "Total de Items"
    FIELD Peso      AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Total Peso Kg."
    ...
    FIELD codfam LIKE Almtfami.codfam LABEL "Linea"
    FIELD desfam LIKE Almtfami.desfam LABEL "Descripcion"
    FIELD UndVta LIKE Facdpedi.undvta LABEL "Unidad"
    FIELD CodPro LIKE gn-prov.CodPro LABEL 'Proveedor'
    FIELD NomPro LIKE gn-prov.NomPro LABEL 'Nombre'
    FIELD CanPed LIKE Facdpedi.canped FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Cantidad Aprobada"
    FIELD CanAtePCO LIKE Facdpedi.canate FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Cantidad Atendida Pre-Ped.Logístico"
    FIELD CanAte LIKE Facdpedi.canate FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Cantidad Atendida Ped.Logístico"
    FIELD PreUni LIKE Facdpedi.preuni FORMAT '->>>,>>>,>>>,>>9.9999' LABEL "Precio Unitario"
    FIELD DctoManual  AS DEC FORMAT '>>>,>>9.99' LABEL "% Dscto Manual"
    FIELD DctoEvento  AS DEC FORMAT '>>>,>>9.99' LABEL "% Dscto Evento"
    FIELD DctoVolProm AS DEC FORMAT '>>>,>>9.99' LABEL "% Dscto Vol/Prom"
    FIELD ImpLin LIKE Facdpedi.implin FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Importe"
    FIELD PesoUnit  AS DEC FORMAT '>>>,>>9.99' LABEL "Peso Unit"
    FIELD CubiUnit  AS DEC FORMAT '>>>,>>9.99' LABEL "Cubic Unit"
    FIELD PesoLin   AS DEC FORMAT '>>>,>>9.99' LABEL "Peso por Item"
    FIELD PesoAte   AS DEC FORMAT '>>>,>>9.99' LABEL "Peso Atendido Ped.Logístico"
    FIELD PesoAtePCO AS DEC FORMAT '>>>,>>9.99' LABEL "Peso Atendido Pre-Ped.Logístico"
    ...
    FIELD CantxAte  AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Cantidad por Atender"
    FIELD PesoxAte  AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Peso por Atender"
    FIELD ImpxAte   AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Import. por Atender"
    FIELD ImpFac    AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Import. Facturado"
    FIELD CanFac    AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Cantidad Facturado"
    FIELD SdoImpFac AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Import. por Facturar"
    FIELD SdoCanFac AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Cantidad por Facturar"
    FIELD ImpNC     AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Import. N/C"
    FIELD CanNC     AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Cantidad N/C"
    .
...
------------------------------------------------------------------------------*/
    ...
    /* 21/01/2025 Carla Tenazoa: Datos adicionales */
    FOR EACH T-Detalle EXCLUSIVE-LOCK:
        FIND gn-ven WHERE gn-ven.CodCia = s-codcia AND gn-ven.CodVen = T-Detalle.codven NO-LOCK NO-ERROR.
        IF AVAILABLE gn-ven THEN T-Detalle.nomven = gn-ven.NomVen.
    END.
    ...
    lOptions = lOptions + "coddiv,libre_c01,nroped,fchped,fchven,fchent,nomcli," +
        "lincre,sdoant,codven,nomven,imptot,estado,avance,items,peso,cuota,atendido,pendiente,codalm,pco".
...
------------------------------------------------------------------------------*/
    ...
    REPEAT WHILE AVAILABLE COTIZACION:
        ...
        FOR EACH DETALLE OF Faccpedi NO-LOCK,FIRST Almmmatg OF DETALLE NO-LOCK :
            ...
            /* 22/10/2025: Carla Tenazoa */
            /* Lo facturado */
            FIND FIRST Facdpedi WHERE Facdpedi.codcia = s-codcia
                AND Facdpedi.codmat = DETALLE.codmat
                AND Facdpedi.coddoc = COTIZACION.coddoc
                AND Facdpedi.nroped = COTIZACION.nroped
                NO-LOCK NO-ERROR.
            IF AVAILABLE Facdpedi THEN DO:
                FOR EACH PEDIDO NO-LOCK WHERE PEDIDO.codcia = s-codcia 
                        ...
                        AND PEDIDO.flgest <> "A",
                    FIRST Facdpedi OF PEDIDO NO-LOCK WHERE Facdpedi.codmat = DETALLE.codmat:
                    FOR EACH FACTURA NO-LOCK WHERE FACTURA.codcia = s-codcia
                            AND FACTURA.codped = PEDIDO.coddoc 
                            AND FACTURA.nroped = PEDIDO.nroped
                            AND FACTURA.flgest <> "A":
                        IF NOT (FACTURA.coddoc = "FAC" OR FACTURA.coddoc = "BOL") THEN NEXT.
                        FOR EACH Ccbddocu OF FACTURA NO-LOCK WHERE Ccbddocu.codmat = DETALLE.codmat:
                            T-Detalle.ImpFac = T-Detalle.ImpFac + Ccbddocu.ImpLin.
                            T-Detalle.CanFac = T-Detalle.CanFac + (Ccbddocu.CanDes * Ccbddocu.Factor).
                        END.
                        /* **************************************** */
                        /* 15/12/2025: Alex Tisza, notas de crédito */
                        /* **************************************** */
                        FOR EACH DEVOLUCION NO-LOCK WHERE DEVOLUCION.codcia = s-codcia
                                AND DEVOLUCION.codref = FACTURA.coddoc
                                AND DEVOLUCION.nroref = FACTURA.nrodoc
                                AND DEVOLUCION.coddoc = "N/C"
                                AND DEVOLUCION.flgest <> "A":
                            FOR EACH Ccbddocu OF DEVOLUCION NO-LOCK WHERE Ccbddocu.codmat = DETALLE.codmat:
                                T-Detalle.ImpNC = T-Detalle.ImpNC + Ccbddocu.ImpLin.
                                T-Detalle.CanNC = T-Detalle.CanNC + (Ccbddocu.CanDes * Ccbddocu.Factor).
                            END.
                        END.
                        /* **************************************** */
                    END.
    ...
    FOR EACH T-Detalle EXCLUSIVE-LOCK, 
        FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND
            Almmmatg.codmat = T-Detalle.codmat,
        FIRST Almtfami NO-LOCK OF Almmmatg:
        ASSIGN
            T-Detalle.codfam = Almmmatg.codfam
            T-Detalle.desfam = Almtfami.desfam.
        ...
        IF AVAILABLE gn-prov THEN ASSIGN T-Detalle.codpro = gn-prov.CodPro T-Detalle.nompro = gn-prov.NomPro.
    END.

    FOR EACH T-Detalle:
        ASSIGN
            T-Detalle.SdoImpFac = T-Detalle.ImpLin - T-Detalle.ImpFac
            T-Detalle.SdoCanFac = T-Detalle.CanPed - T-Detalle.CanFac.
        IF T-Detalle.SdoImpFac <> 0 AND T-Detalle.SdoCanFac <= 0 THEN T-Detalle.SdoImpFac = 0.
    END.
    ...
    lOptions = lOptions + "coddiv,libre_c01,nroped,fchped,fchven,fchent,codcli,nomcli," +
        "lincre,sdoant,lugent,distent,codven,nomven,estado,peso,codalm," +
        "nroitm,codmat,desmat,desmar,codfam,desfam,codpro,nompro,undvta,pesounit,cubiunit,canped,pesolin,preuni,dctomanual," +
        "dctoevento,dctovolprom,implin,pesoate,impate,cantxate,pesoxate,impxate," +
        "canatepco,canate,pesoatepco,ImpAtePCO,ImpFac,CanFac,SdoImpFac,SdoCanFac,ImpNC,CanNC".   /*atendido,PCO*/
    pOptions = pOptions + CHR(1) + lOptions.
    /* El archivo se va a generar en un archivo temporal de trabajo antes 
    de enviarlo a su directorio destino */
    cArchivo = LC(pArchivo).
