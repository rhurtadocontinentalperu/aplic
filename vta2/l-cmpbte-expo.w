...
CcbCDocu.PorDto X-EST @ X-EST fUbicacion() @ x-Ubicacion fBanco() @ x-Banco ~
...
f-desde f-hasta xcodmod f-codven x-FchVto-1 x-FchVto-2 br_table 
...
"NroDoc" ? "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    IF CMB-filtro = CMB-filtro:SCREEN-VALUE 
        ...
        AND C-CMPBTE = C-CMPBTE:SCREEN-VALUE 
        AND x-CndCre = x-CndCre:SCREEN-VALUE 
        AND x-CodDiv = x-CodDiv:SCREEN-VALUE
        THEN RETURN.
    ASSIGN
       FILL-IN-filtro
       CMB-filtro
