...
/* PROGRAMA EXCLUSIVO PARA LA EXPOLIBRERIA:
SE VAN A INGRESAR 2 PARAMETROS:
1ro. El dódigo del documento. Ej. COT
2do. El tipo de documento. Ej E: Expo Enero     N: Expo Octubre
*/

/* Local Variable Definitions ---                                       */
DEF INPUT PARAMETER pParametro AS CHAR.
IF NUM-ENTRIES(pParametro) < 2 THEN RETURN ERROR.
...
DEF NEW SHARED VAR s-codven AS CHAR.    /* SOLO PARA EXPOLIBRERIA */
...
/* 
Posibles valores para s-CodDoc:
COT: Cotizaciones
PED: Pedidos
...
&Scoped-Define ENABLED-OBJECTS RECT-24 BTN-Excel BUTTON-2 COMBO-NroSer 
...
DEFINE VARIABLE h_t-cotizacion-cred-01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotizacion-expo AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BTN-Excel AT ROW 1 COL 116 WIDGET-ID 16
     BUTTON-2 AT ROW 1 COL 126 WIDGET-ID 12
     COMBO-NroSer AT ROW 1.27 COL 79.71 WIDGET-ID 4
     B-CCTE AT ROW 7.46 COL 119 WIDGET-ID 18
...
DO:
   RUN Genera-Excel IN h_v-cotizacion-expo.
END.
...
DO:
  /*RUN vta/wconcotcli.*/
    RUN vtaexp/gestadisticas (s-codcli).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 2.35 , 119.00 ) NO-ERROR.
       RUN set-size IN h_p-navico-2 ( 5.12 , 10.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-cotizacion-expo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotizacion-expo ).
       RUN set-position IN h_v-cotizacion-expo ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.15 , 115.00 ) */
       ...
       /* Links to SmartViewer h_v-cotizacion-expo. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_v-cotizacion-expo ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-cotizacion-expo ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotizacion-expo ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-cotizacion-cred ( 11.96 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-cotizacion-cred-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotizacion-cred-01 ).
       RUN set-position IN h_t-cotizacion-cred-01 ( 11.96 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotizacion-cred-01 ( 11.85 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotizacion-cred-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotizacion-cred-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotizacion-cred-01 ,
             BUTTON-COT:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotizacion-cred-01 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-24 BTN-Excel BUTTON-2 COMBO-NroSer 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         RUN dispatch IN h_t-cotizacion-cred-01 ('open-query':U).
      END.
    ...
    WHEN "Add-Record" THEN DO:
        RUN dispatch IN h_t-cotizacion-cred-01 ('add-record':U).
      END.
    WHEN "browse" THEN DO:
          IF h_b-cotizacion-cred <> ? THEN RUN dispatch IN h_b-cotizacion-cred ('open-query':U).
          IF h_t-cotizacion-cred-01 <> ? THEN RUN dispatch IN h_t-cotizacion-cred-01 ('open-query':U). 
      END.
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN Recalcular-Precios IN h_t-cotizacion-cred-01.
      END.
    WHEN 'IBC' THEN DO:
        RUN IBC-Diferencias IN h_t-cotizacion-cred-01.
    END.
END CASE.
