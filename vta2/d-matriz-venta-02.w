...
DEFINE VARIABLE h_t-vtadmatriz-a-02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-vtadmatriz-p-02 AS HANDLE NO-UNDO.
...
DO:
   RUN Graba-Registros IN h_t-vtadmatriz-a-02.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Cantidades|Precios' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 7.73 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 17.31 , 144.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-vtadmatriz-a-02.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-vtadmatriz-a-02 ).
       RUN set-position IN h_t-vtadmatriz-a-02 ( 8.88 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-vtadmatriz-a-02 ( 15.88 , 142.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-vtadmatriz-a-02. */
       RUN add-link IN adm-broker-hdl ( h_b-vtacmatrizactiva , 'Record':U , h_t-vtadmatriz-a-02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-vtadmatriz-a-02 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/t-vtadmatriz-p-02.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-vtadmatriz-p-02 ).
       RUN set-position IN h_t-vtadmatriz-p-02 ( 8.88 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-vtadmatriz-p-02 ( 13.27 , 142.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-vtadmatriz-p-02. */
       RUN add-link IN adm-broker-hdl ( h_b-vtacmatrizactiva , 'Record':U , h_t-vtadmatriz-p-02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-vtadmatriz-p-02 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
