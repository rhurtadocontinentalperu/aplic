...
DEFINE VARIABLE h_t-cotizacion-cont AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotizacion-cont AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BTN-Excel AT ROW 1 COL 119 WIDGET-ID 16
     COMBO-NroSer AT ROW 1.27 COL 79.71 WIDGET-ID 4
     Btn-Grupos AT ROW 23.08 COL 37 WIDGET-ID 10
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "COTIZACIONES AL CONTADO - MOSTRADOR"
         HEIGHT             = 23.42
         WIDTH              = 143.43
         MAX-HEIGHT         = 31.62
         MAX-WIDTH          = 164.43
...
ON END-ERROR OF W-Win /* COTIZACIONES AL CONTADO - MOSTRADOR */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* COTIZACIONES AL CONTADO - MOSTRADOR */
DO:
...
DO:
  RUN Genera-Excel IN h_v-cotizacion-cont.
END.
...
DO:
    RUN vta2/d-codagrupado-cont.
    RUN dispatch IN h_t-cotizacion-cont ('open-query').
END.
...
CASE s-CodDoc:
    ...
    WHEN "C/M" THEN W-Win:TITLE = "COTIZACIONES MAYORISTAS".
END CASE.
CASE s-TpoPed:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-cotizacion-cont.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotizacion-cont ).
       RUN set-position IN h_v-cotizacion-cont ( 2.62 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-cotizacion-cont. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_v-cotizacion-cont ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-cotizacion-cont ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotizacion-cont ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedido-cont ,
             h_v-cotizacion-cont , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-cotizacion-cont.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotizacion-cont ).
       RUN set-position IN h_t-cotizacion-cont ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotizacion-cont ( 11.85 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotizacion-cont. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotizacion-cont ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotizacion-cont ,
             h_v-cotizacion-cont , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotizacion-cont , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-24 BTN-Excel COMBO-NroSer 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         RUN dispatch IN h_t-cotizacion-cont ('open-query':U).
      END.
    ...
    WHEN "Add-Record" THEN DO:
        RUN dispatch IN h_t-cotizacion-cont ('add-record':U).
      END.
    WHEN "browse" THEN DO:
          IF h_b-pedido-cont <> ? THEN RUN dispatch IN h_b-pedido-cont ('open-query':U).
          IF h_t-cotizacion-cont <> ? THEN RUN dispatch IN h_t-cotizacion-cont ('open-query':U). 
      END.
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN Recalcular-Precios IN h_t-cotizacion-cont.
      END.
END CASE.
