...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almtfami.desfam FORMAT "X(40)":U WIDTH 37.14
      VtaDTabla.Libre_c01 COLUMN-LABEL "Sublinea" FORMAT "x(3)":U
            COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 11
      AlmSFami.dessub FORMAT "X(40)":U WIDTH 34.57
  ENABLE
...
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 91 BY 6.69
         FONT 4
         TITLE "LINEAS/SUBLINEAS EXCEPTAS DE PROMOCION" FIT-LAST-COLUMN.
...
"Almtfami.desfam" ? "X(40)" "character" ? ? ? ? ? ? no ? no no "37.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaDTabla.Libre_c01
"VtaDTabla.Libre_c01" "Sublinea" "x(3)" "character" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.AlmSFami.dessub
"AlmSFami.dessub" ? "X(40)" "character" ? ? ? ? ? ? no ? no no "34.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* LINEAS/SUBLINEAS EXCEPTAS DE PROMOCION */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* LINEAS/SUBLINEAS EXCEPTAS DE PROMOCION */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* LINEAS/SUBLINEAS EXCEPTAS DE PROMOCION */
DO:
...
FIND Almtfami WHERE Almtfami.codcia = Vtactabla.codcia
...
IF VtaDTabla.Libre_c01:SCREEN-VALUE IN BROWSE {&browse-name} <> "" THEN DO:
    FIND Almsfami WHERE Almsfami.codcia = Vtactabla.codcia
...
IF RETURN-VALUE = 'YES' THEN DO:
    IF CAN-FIND(FIRST Vtadtabla OF Vtactabla WHERE VtaDTabla.Tipo = 'XL'
