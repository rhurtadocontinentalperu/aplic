...
DEFINE VAR x-tabla AS CHAR INIT "TIPO_DSCTO_FACTOR".
...
&Scoped-define FIELDS-IN-QUERY-br_table tt-w-report.Campo-D[1] ~
tt-w-report.Campo-F[1] 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table tt-w-report.Campo-D[1] ~
tt-w-report.Campo-F[1] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      tt-w-report.Campo-D[1] COLUMN-LABEL "Fecha" FORMAT "99/99/9999":U
            WIDTH 11.29
      tt-w-report.Campo-F[1] COLUMN-LABEL "% Dscto" FORMAT ">>9.9999":U
  ENABLE
      tt-w-report.Campo-D[1]
      tt-w-report.Campo-F[1]
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS SIZE 23.86 BY 12.27
         FONT 4.
...
"tt-w-report.Campo-D[1]" "Fecha" ? "date" ? ? ? ? ? ? yes ? no no "11.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-w-report.Campo-F[1]
"tt-w-report.Campo-F[1]" "% Dscto" ">>9.9999" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON 'RETURN':U OF tt-w-report.campo-d[1], tt-w-report.campo-f[1] IN BROWSE {&BROWSE-NAME} DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      FOR EACH vtatabla WHERE vtatabla.codcia = s-codcia AND
                                ...
                                vtatabla.llave_c2 = x-motivo NO-LOCK:
          CREATE tt-w-report.
          ASSIGN    tt-w-report.campo-d[1] = vtatabla.rango_fecha[1]
                    tt-w-report.campo-f[1] = vtatabla.rango_valor[1].
      END.
...
------------------------------------------------------------------------------*/
    ...
    MESSAGE 'Seguro de GRABAR los registros ?' VIEW-AS ALERT-BOX QUESTION
  ...
  */
  ...
  FOR EACH vtatabla WHERE vtatabla.codcia = s-codcia AND
                                        ...
                                        vtatabla.llave_c2 = x-motivo NO-LOCK:
      CREATE ytt-w-report.
      ASSIGN    ytt-w-report.campo-d[1] = vtatabla.rango_fecha[1]
          .
  END.
  ...
  FOR EACH ytt-w-report :
      ...
      FIND FIRST xtt-w-report WHERE xtt-w-report.campo-d[1] = ytt-w-report.campo-d[1]  NO-LOCK NO-ERROR.
      ...
      DO:
          ...
          FOR EACH ytt-w-report WHERE ytt-w-report.campo-c[10] = "X" :
              FIND FIRST vtatabla WHERE vtatabla.codcia = s-codcia AND
                                            vtatabla.tabla = x-tabla AND
                                            vtatabla.llave_c1 = x-proceso AND
                                            vtatabla.llave_c2 = x-motivo AND
                                            vtatabla.rango_fecha[1] = ytt-w-report.campo-d[1]
          ...
          FOR EACH xtt-w-report :
              ...
              FIND FIRST vtatabla WHERE vtatabla.codcia = s-codcia AND
                                            ...
                                            vtatabla.llave_c1 = x-proceso AND
                                            vtatabla.llave_c2 = x-motivo AND
                                            vtatabla.rango_fecha[1] = xtt-w-report.campo-d[1]
                                            EXCLUSIVE-LOCK NO-ERROR.
              IF NOT AVAILABLE vtatabla THEN DO:
                  CREATE vtatabla.
                    ASSIGN vtatabla.codcia = s-codcia
                            ...
                            vtatabla.llave_c2 = x-motivo
                            vtatabla.rango_fecha[1] = xtt-w-report.campo-d[1] NO-ERROR
                        .
              END.
              ...
              ASSIGN    vtatabla.rango_valor[1] = xtt-w-report.campo-f[1]
                        vtatabla.libre_c01 = xtt-w-report.campo-c[5]
...
x-fecha = DATE(tt-w-report.campo-d[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
x-factor = DEC(tt-w-report.campo-f[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).

IF x-fecha = ? THEN DO:
    MESSAGE "Ingresa la fecha por favor!!!" VIEW-AS ALERT-BOX INFORMATION.
    RETURN "ADM-ERROR".
END.
IF NOT ( x-factor > 0 AND x-factor <= 100 ) THEN DO:
    MESSAGE "%Dscto " VIEW-AS ALERT-BOX INFORMATION.
    RETURN "ADM-ERROR".
END.
FIND FIRST xtt-w-report WHERE xtt-w-report.campo-d[1] = x-fecha NO-LOCK NO-ERROR.
IF AVAILABLE xtt-w-report THEN DO:
    MESSAGE "Fecha Ya esta registrada" VIEW-AS ALERT-BOX INFORMATION.
    RETURN "ADM-ERROR".
END.
...
DEFINE VAR x-todos AS CHAR INIT "NO".
...
END.
ELSE DO:
    FIND FIRST xtt-w-report WHERE xtt-w-report.campo-c[1] = '*' NO-LOCK NO-ERROR.
    IF AVAILABLE xtt-w-report THEN DO:
        x-todos = 'SI'.
        IF x-signo = '+' THEN DO:
            MESSAGE "Existe conflicto, no puede adicionar el registro por que existe uno general (*)" VIEW-AS ALERT-BOX INFORMATION.
            RETURN "ADM-ERROR".
        END.
    END.        
    ...
    IF x-sublinea = '*' THEN DO:
        ...
        IF AVAILABLE xtt-w-report THEN DO:
            MESSAGE "Existe conflicto con la linea con sub-lineas del mismo signo" VIEW-AS ALERT-BOX INFORMATION.
            RETURN "ADM-ERROR".
        END.
        IF x-signo = '-' THEN DO:
            FIND FIRST xtt-w-report WHERE xtt-w-report.campo-c[1] = x-linea AND
                                                xtt-w-report.campo-c[5] = '+' NO-LOCK NO-ERROR.
            IF x-todos = 'NO' AND NOT AVAILABLE xtt-w-report THEN DO:
                MESSAGE "Debe existir la Linea con singo +" VIEW-AS ALERT-BOX INFORMATION.
                RETURN "ADM-ERROR".
            END.
        END.
    END.
    ELSE DO:
        ...
        IF AVAILABLE xtt-w-report THEN DO:
            IF x-signo = '+' AND xtt-w-report.campo-c[5] = x-signo  THEN DO:
                MESSAGE "Existe conflicto, no puede adicionar el registro por que existe uno general (*)" VIEW-AS ALERT-BOX INFORMATION.
                RETURN "ADM-ERROR".
            END.
        END.        
