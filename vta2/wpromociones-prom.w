...
DEFINE VARIABLE h_bpromoofertas-prom AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_qvtactabla AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vpromociones AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vpromociones-03-2 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 1.27 COL 3 WIDGET-ID 6
     RECT-1 AT ROW 2.62 COL 2 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 147.57 BY 24.85 WIDGET-ID 100.
...
DO:
  RUN dispatch IN h_qvtactabla ('qbusca':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/bpromoofertas-prom.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bpromoofertas-prom ).
       RUN set-position IN h_bpromoofertas-prom ( 5.31 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bpromoofertas-prom ( 20.19 , 100.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/vpromociones-03.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vpromociones-03-2 ).
       RUN set-position IN h_vpromociones-03-2 ( 5.58 , 104.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_bpromoofertas-prom. */
       RUN add-link IN adm-broker-hdl ( h_qvtactabla , 'Record':U , h_bpromoofertas-prom ).

       /* Links to SmartViewer h_vpromociones-03-2. */
       RUN add-link IN adm-broker-hdl ( h_qvtactabla , 'Record':U , h_vpromociones-03-2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_bpromoofertas-prom ,
             h_vpromociones , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vpromociones-03-2 ,
             h_bpromoofertas-prom , 'AFTER':U ).
    END. /* Page 0 */
