...
&Scoped-Define ENABLED-OBJECTS RECT-1 BtnDone BUTTON-13 txtDesde txtHasta ~
ChkChequeo BtnExcel RADIO-SET-1 BUTTON-PorZona BUTTON-Alfabeticamente 
...
DEFINE VARIABLE h_b-listado-prepicking-od AS HANDLE NO-UNDO.
...
DO:
  ASSIGN RADIO-SET-1.                 
  CASE RADIO-SET-1:
      WHEN 1 THEN RUN envia-excel IN h_b-listado-prepicking-od.
      WHEN 2 THEN RUN envia-excel-detalle IN h_b-listado-prepicking-od.
  END CASE.
...
DO:
    ...
    RUN dispatch IN h_b-listado-prepicking-od ('open-query':U).
...
DO:
  ...
  RUN Imprimir-Formato IN h_b-listado-prepicking-od ( INPUT "ALFABETICO").
END.
...
DO:
  /*RUN dispatch IN h_b-cimprime-od ('imprime':U).*/
    RUN Imprimir-Formato IN h_b-listado-prepicking-od ( INPUT "ZONA").
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-listado-prepicking-od.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = FchPed':U ,
             OUTPUT h_b-listado-prepicking-od ).
       RUN set-position IN h_b-listado-prepicking-od ( 3.12 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-listado-prepicking-od ( 7.50 , 151.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-listado-prepicking-od , 'Record':U , h_b-dimprime-od ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-listado-prepicking-od ,
             RADIO-SET-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-od ,
             h_b-listado-prepicking-od , 'AFTER':U ).
    END. /* Page 0 */
...
OCXFile = SEARCH( "w-listado-prepicking-od.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-listado-prepicking-od.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 BtnDone BUTTON-13 txtDesde txtHasta ChkChequeo BtnExcel 
         RADIO-SET-1 BUTTON-PorZona BUTTON-Alfabeticamente 
      WITH FRAME F-Main IN WINDOW sW-Win.
