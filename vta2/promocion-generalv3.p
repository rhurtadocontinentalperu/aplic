...
DEF INPUT PARAMETER pPorIgv LIKE Ccbcdocu.PorIgv.
/* Enviar el detalle */
DEF INPUT-OUTPUT PARAMETER TABLE FOR ITEM.
/* Mensaje de Error */
DEF OUTPUT PARAMETER pError AS CHAR NO-UNDO.
/* RHC 22/02/2019 Parámetros adicionales: */
DEF INPUT PARAMETER pFechaControl AS DATE.      /* Si se envía "?" toma la fecha de HOY */
DEF INPUT PARAMETER pPreguntar AS LOG.          /* Si hay mas de uno pregunta o no */

IF pFechaControl = ? THEN pFechaControl = TODAY.
IF pPreguntar = ? THEN pPreguntar = YES.

DEF SHARED VAR s-codcia AS INT.               

IF LOOKUP(pCodDoc, 'PED,P/M,COT') = 0 THEN RETURN "OK". 
IF LOOKUP(pFmaPgo, '900,999,899') > 0 THEN RETURN "OK".                          
...
p-CodAlm = ENTRY(1, pCodAlm).   /* EL ALMACÉN POR DEFECTO */
...
FOR EACH Vtactabla NO-LOCK WHERE Vtactabla.codcia = s-codcia
    ...
    AND (pFechaControl >= VtaCTabla.FechaInicial AND pFechaControl <= VtaCTabla.FechaFinal)
    ...
    AND CAN-FIND(FIRST Vtadtabla OF Vtactabla WHERE Vtadtabla.tipo = "D"    /* División */
                 ...
                 AND ((pCodDoc = "P/M" AND VtaDTabla.Libre_l01 = YES)
                      OR (LOOKUP(pCodDoc, "PED,COT") > 0 AND VtaDTabla.Libre_l02 = YES))
                 NO-LOCK):
...
FOR EACH Promocion NO-LOCK WHERE Promocion.Libre_d04 > 0,
    ...
    */
    ...
    ELSE ITEM.ImpIsc = 0.
    IF ITEM.AftIgv 
    THEN ITEM.ImpIgv = ITEM.ImpLin - ROUND( ITEM.ImpLin  / ( 1 + (pPorIgv / 100) ), 4 ).
    ELSE ITEM.ImpIgv = 0.
END.
RETURN "OK".
