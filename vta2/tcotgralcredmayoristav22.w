...
DEFINE SHARED VAR s-ListaTerceros AS LOG.
...
fDesMat() @ x-DesMat ITEM.UndVta ITEM.CanPed ITEM.TipVta ITEM.PreUni ~
ITEM.Por_Dsctos[1] ITEM.Por_Dsctos[2] ITEM.Por_Dsctos[3] ITEM.CanApr ~
ITEM.ImpDto ITEM.ImpLin 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table ITEM.codmat ITEM.CanPed ~
ITEM.TipVta ITEM.PreUni 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      ITEM.TipVta COLUMN-LABEL "Lista" FORMAT "X(1)":U WIDTH 7.29
            COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 11 VIEW-AS COMBO-BOX INNER-LINES 4
                      LIST-ITEM-PAIRS " "," ",
                                      "1","1",
                                      "2","2",
                                      "3","3"
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SIZE 144 BY 12.88
...
DEFINE FRAME F-Main
     ...
     BUTTON-2 AT ROW 13.88 COL 38 WIDGET-ID 8
     FILL-IN-ImpTot AT ROW 13.88 COL 71 COLON-ALIGNED WIDGET-ID 6
     FILL-IN-ImpPer AT ROW 13.88 COL 95 COLON-ALIGNED WIDGET-ID 4
     FILL-IN-ImpTot-2 AT ROW 13.88 COL 127 COLON-ALIGNED WIDGET-ID 2
...
"fDesMat() @ x-DesMat" "Descripción" "x(60)" ? ? ? ? ? ? ? no ? no no "50.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"ITEM.TipVta" "Lista" ? "character" 11 0 ? ? ? ? yes ? no no "7.29" yes no no "U" "" "" "DROP-DOWN-LIST" "," ? " , ,1,1,2,2,3,3" 4 no 0 no no
...
"ITEM.Por_Dsctos[1]" "% Dscto.!Manual" "->>9.9999" "decimal" ? ? ? ? ? ? no ? no no "6.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"ITEM.ImpLin" "Importe!SIN Perc." ">,>>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    IF Almmmatg.CHR__02 = "T" AND s-ListaTerceros = YES 
          THEN ITEM.TipVta:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO.
    ELSE ITEM.TipVta:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES.
END.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = "NO" THEN DO:
      /* Consistencia productos terceros */
      IF Almmmatg.CHR__02 = "T" AND s-ListaTerceros = YES 
          THEN ITEM.TipVta:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO.
  END.
  CASE TRUE:
