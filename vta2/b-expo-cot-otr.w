...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      VtaTabla.LLave_c3 COLUMN-LABEL "Número" FORMAT "x(12)":U
            WIDTH 9.86
      FacCPedi.NomCli COLUMN-LABEL "Nombre o Razón Social" FORMAT "x(50)":U
            WIDTH 45.43
      FacCPedi.ImpTot FORMAT "->>,>>>,>>9.99":U WIDTH 10.43
      VtaTabla.Llave_c5 COLUMN-LABEL "Almacén!Salida" FORMAT "x(8)":U
            WIDTH 6.43
      B-ALM-1.Descripcion FORMAT "X(30)":U WIDTH 22.43
      VtaTabla.Llave_c6 COLUMN-LABEL "Almacén!Destino" FORMAT "x(8)":U
            WIDTH 6.29
...
"VtaTabla.LLave_c3" "Número" "x(12)" "character" ? ? ? ? ? ? yes ? no no "9.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacCPedi.NomCli
"FacCPedi.NomCli" "Nombre o Razón Social" ? "character" ? ? ? ? ? ? no ? no no "45.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaTabla.Llave_c5" "Almacén!Salida" ? "character" ? ? ? ? ? ? yes ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaTabla.Llave_c6" "Almacén!Destino" ? "character" ? ? ? ? ? ? yes ? no no "6.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF VtaTabla.Llave_c5 IN BROWSE br_table /* Almacén!Salida */
DO:
...
ON LEAVE OF VtaTabla.Llave_c6 IN BROWSE br_table /* Almacén!Destino */
DO:
...
IF NOT AVAILABLE Faccpedi THEN DO:
    MESSAGE 'Cotización NO registrada' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO VtaTabla.LLave_c3 IN BROWSE {&BROWSE-NAME}.
    RETURN 'ADM-ERROR'.
END.
IF Faccpedi.flgest <> "P" THEN DO:
    MESSAGE 'La cotización NO está pendiente de atención' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO VtaTabla.LLave_c3 IN BROWSE {&BROWSE-NAME}.
    RETURN 'ADM-ERROR'.
END.
IF CAN-FIND(FIRST Facdpedi OF Faccpedi WHERE Facdpedi.canate <> 0 NO-LOCK) 
    THEN DO:
    MESSAGE 'La cotización YA tiene despachos' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO VtaTabla.LLave_c3 IN BROWSE {&BROWSE-NAME}.
    RETURN 'ADM-ERROR'.
END.
...
IF RETURN-VALUE = 'YES' THEN DO:
    IF CAN-FIND(FIRST VtaTabla WHERE VtaTabla.CodCia = gn-divi.codcia
                ...
                NO-LOCK)
        THEN DO:
        MESSAGE 'La cotización YA ha sido registrada anteriormente' VIEW-AS ALERT-BOX ERROR.
        APPLY 'ENTRY':U TO VtaTabla.LLave_c3 IN BROWSE {&BROWSE-NAME}.
        RETURN 'ADM-ERROR'.
    END.
END.
IF NOT CAN-FIND(FIRST B-ALM-1 WHERE B-ALM-1.CodCia = s-codcia
                ...
                NO-LOCK)
    THEN DO:
    MESSAGE 'Almacén de Salida NO registrado' VIEW-AS ALERT-BOX ERROR.
...
IF NOT CAN-FIND(FIRST B-ALM-2 WHERE B-ALM-2.CodCia = s-codcia
                ...
                NO-LOCK)
    THEN DO:
    MESSAGE 'Almacén de Destino NO registrado' VIEW-AS ALERT-BOX ERROR.
