...
&Scoped-Define ENABLED-OBJECTS RADIO-SET-CodDoc BtnDone BUTTON-13 ~
rbtn-impresos txtDesde txtHasta ChkChequeo rbtn-cliente txtOC-SMP ~
txtCliente BUTTON-PorZona BUTTON-Alfabeticamente BUTTON-PorEmpaque 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-CodDoc rbtn-impresos txtDesde ~
txtHasta ChkChequeo rbtn-cliente txtOC-SMP txtCliente 
...
DEFINE VARIABLE h_b-listado-prepicking-odc AS HANDLE NO-UNDO.
...
DEFINE VARIABLE RADIO-SET-CodDoc AS CHARACTER INITIAL "O/D" 
     ...
     RADIO-BUTTONS 
          "Orden de Despacho Consolidad (ODC)", "ODC"
     SIZE 38 BY 1
...
DEFINE FRAME F-Main
     ...
     BUTTON-13 AT ROW 1.38 COL 120 WIDGET-ID 4
     rbtn-impresos AT ROW 2.42 COL 80 NO-LABEL WIDGET-ID 52
     txtDesde AT ROW 2.5 COL 6.57 COLON-ALIGNED WIDGET-ID 16
     txtHasta AT ROW 2.5 COL 28 COLON-ALIGNED WIDGET-ID 20
     ChkChequeo AT ROW 2.54 COL 50 WIDGET-ID 22
     rbtn-cliente AT ROW 3.58 COL 56.43 NO-LABEL WIDGET-ID 48
     txtOC-SMP AT ROW 3.65 COL 27.86 COLON-ALIGNED WIDGET-ID 42
     txtCliente AT ROW 4.38 COL 54 COLON-ALIGNED WIDGET-ID 46
     BUTTON-PorZona AT ROW 25.81 COL 2 WIDGET-ID 6
     BUTTON-Alfabeticamente AT ROW 25.81 COL 24 WIDGET-ID 8
     BUTTON-PorEmpaque AT ROW 25.81 COL 53 WIDGET-ID 56
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW sW-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CONTROL DE IMPRESION DE ORDENES CONSOLIDADAS PARA PRE-PICKING"
         HEIGHT             = 26.23
         WIDTH              = 144.14
         MAX-HEIGHT         = 27.38
         MAX-WIDTH          = 159.43
...
/* CtrlFrame OCXINFO:CREATE-CONTROL from: {F0B88A90-F5DA-11CF-B545-0020AF6ED35A} type: PSTimer */
      CtrlFrame:MOVE-AFTER(txtCliente:HANDLE IN FRAME F-Main).
...
ON END-ERROR OF sW-Win /* CONTROL DE IMPRESION DE ORDENES CONSOLIDADAS PARA PRE-PICKING */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL sW-Win sW-Win
ON WINDOW-CLOSE OF sW-Win /* CONTROL DE IMPRESION DE ORDENES CONSOLIDADAS PARA PRE-PICKING */
DO:
...
DO:
    ...
    RUN Carga-Temporal IN h_b-listado-prepicking-odc.
    RUN dispatch IN h_b-listado-prepicking-odc ('open-query':U).
...
DO:
  RUN Imprimir-Formato-OD IN h_b-listado-prepicking-odc ( INPUT "EMPAQUE").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-PorEmpaque sW-Win
...
DO:
    RUN Imprimir-Formato-OD IN h_b-listado-prepicking-odc ( INPUT "ZONA").
    APPLY 'CHOOSE':U TO  BUTTON-13.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-listado-prepicking-odc.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = FchPed':U ,
             OUTPUT h_b-listado-prepicking-odc ).
       RUN set-position IN h_b-listado-prepicking-odc ( 5.69 , 1.14 ) NO-ERROR.
       RUN set-size IN h_b-listado-prepicking-odc ( 7.50 , 144.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-listado-prepicking-odc , 'Record':U , h_b-dimprime-od ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-listado-prepicking-odc ,
             txtCliente:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-od ,
             h_b-listado-prepicking-odc , 'AFTER':U ).
    END. /* Page 0 */
...
OCXFile = SEARCH( "w-listado-prepicking-odc.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-listado-prepicking-odc.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-CodDoc rbtn-impresos txtDesde txtHasta ChkChequeo 
          rbtn-cliente txtOC-SMP txtCliente 
      WITH FRAME F-Main IN WINDOW sW-Win.
  ENABLE RADIO-SET-CodDoc BtnDone BUTTON-13 rbtn-impresos txtDesde txtHasta 
         ChkChequeo rbtn-cliente txtOC-SMP txtCliente BUTTON-PorZona 
         BUTTON-Alfabeticamente BUTTON-PorEmpaque 
      WITH FRAME F-Main IN WINDOW sW-Win.
