...
&SCOPED-DEFINE CONDICION INTEGRAL.Almcmov.TipMov = "S" AND INTEGRAL.Almcmov.CodMov = 03 ~
        AND ((lChequeados = NO ) OR (INTEGRAL.Almcmov.Libre_c02 <> "C" AND INTEGRAL.Almcmov.FlgSit = "T"))  ~
...
&Scoped-define INTERNAL-TABLES Almacen Almcmov
...
Almcmov.FchDoc Almcmov.HorSal fDestino(Almcmov.AlmDes) @ Almcmov.NomRef ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almacen WHERE ~{&KEY-PHRASE} ~
 ...
 NO-LOCK, ~
    ...
    BY Almcmov.FchDoc DESCENDING ~
       BY Almcmov.HraDoc DESCENDING.
&Scoped-define TABLES-IN-QUERY-br_table Almacen Almcmov
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      fDestino(Almcmov.AlmDes) @ Almcmov.NomRef COLUMN-LABEL "Destino" FORMAT "x(50)":U
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Almacen,INTEGRAL.Almcmov OF INTEGRAL.Almacen"
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ","
     _OrdList          = "INTEGRAL.Almcmov.FchDoc|no,INTEGRAL.Almcmov.HraDoc|no"
...
"fDestino(Almcmov.AlmDes) @ Almcmov.NomRef" "Destino" "x(50)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  IF Almcmov.Libre_c01 = '' OR Almcmov.Libre_c04 <> '' THEN RETURN.
  MESSAGE 'Enviamos la Guia por Transferencia a Distribución?'
      VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE rpta AS LOG.
  IF rpta = NO THEN RETURN.
  FIND CURRENT Almcmov EXCLUSIVE-LOCK NO-ERROR.
  IF NOT AVAILABLE Almcmov THEN DO:
      MESSAGE 'No se pudo bloquear la Guia' VIEW-AS ALERT-BOX ERROR.
      RETURN.
  END.
  ASSIGN
      Almcmov.Libre_c04 = s-user-id + '|' + STRING(DATETIME(TODAY, MTIME), '99/99/9999 HH:MM:SS').
