...
Almmmatg.DesMat Almmmatg.UndBas VtaDTabla.Libre_c01 VtaDTabla.Libre_d01 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaDTabla.LlaveDetalle COLUMN-LABEL "Artículo" FORMAT "x(6)":U
      ...
      VtaDTabla.Libre_c01 COLUMN-LABEL "Tipo" FORMAT "x(8)":U WIDTH 8.57
      ...
      VtaDTabla.Libre_d01 COLUMN-LABEL "Mínimo" FORMAT ">>>,>>9.99":U
            WIDTH 4.72 COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 11
  ENABLE
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 94 BY 8.35
         FONT 4
         TITLE "ARTICULOS" FIT-LAST-COLUMN.
...
DEFINE FRAME F-Main
     ...
     BUTTON-3 AT ROW 8 COL 95 WIDGET-ID 2
...
"VtaDTabla.LlaveDetalle" "Artículo" "x(6)" "character" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaDTabla.Libre_c01" "Tipo" ? "character" 11 0 ? ? ? ? yes ? no no "8.57" yes no no "U" "" "" "DROP-DOWN-LIST" "," "Importe,Cantidad" ? 2 no 0 no no
     _FldNameList[5]   > INTEGRAL.VtaDTabla.Libre_d01
"VtaDTabla.Libre_d01" "Mínimo" ">>>,>>9.99" "decimal" 11 0 ? ? ? ? yes ? no no "4.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* ARTICULOS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* ARTICULOS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* ARTICULOS */
DO:
...
DO:
  RUN vta2/dpromoarticulos (OUTPUT TABLE Promocion).
  FIND FIRST Promocion NO-LOCK NO-ERROR.
  IF NOT AVAILABLE Promocion THEN RETURN NO-APPLY.
  FOR EACH VtaDTabla OF VtaCTabla WHERE VtaDTabla.Tipo = "M":
      DELETE VtaDTabla.
  END.
  FOR EACH Promocion:
      CREATE VtaDTabla.
...
ON 'RETURN':U OF  VtaDTabla.Libre_c01, VtaDTabla.Libre_d01, VtaDTabla.LlaveDetalle
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
IF NOT AVAILABLE Almmmatg THEN DO:
    MESSAGE 'Ingrese correctamente el artículo' VIEW-AS ALERT-BOX ERROR.
    APPLY "ENTRY":U TO Vtadtabla.llavedetalle IN BROWSE {&browse-name}.
    RETURN "ADM-ERROR".
END.
IF LOOKUP(VtaDTabla.Libre_c01:SCREEN-VALUE IN BROWSE {&browse-name}, 'Importe,Cantidad') = 0
    THEN DO:
    MESSAGE 'Debe seleccionar un Tipo' VIEW-AS ALERT-BOX ERROR.
    APPLY "ENTRY":U TO Vtadtabla.llavedetalle IN BROWSE {&browse-name}.
    RETURN "ADM-ERROR".
END.
RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
