...
&Scoped-define QUERY-STRING-Query-Main FOR EACH FacTabla WHERE FacTabla.Codigo =  TRIM(AlmSFami.codfam ) + TRIM(AlmSFami.subfam) ~
  AND FacTabla.CodCia = AlmSFami.CodCia ~
      AND FacTabla.Codigo =  TRIM(AlmSFami.codfam ) + TRIM(AlmSFami.subfam) ~
  AND FacTabla.CodCia = AlmSFami.CodCia ~
 AND FacTabla.Tabla = "DVXSF" NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH FacTabla WHERE FacTabla.Codigo =  TRIM(AlmSFami.codfam ) + TRIM(AlmSFami.subfam) ~
  AND FacTabla.CodCia = AlmSFami.CodCia ~
      AND FacTabla.Codigo =  TRIM(AlmSFami.codfam ) + TRIM(AlmSFami.subfam) ~
  AND FacTabla.CodCia = AlmSFami.CodCia ~
 AND FacTabla.Tabla = "DVXSF" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main FacTabla
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main FacTabla
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.FacTabla WHERE INTEGRAL.AlmSFami <external> ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[1]      = "FacTabla.Codigo =  TRIM(AlmSFami.codfam ) + TRIM(AlmSFami.subfam)
  AND FacTabla.CodCia = AlmSFami.CodCia"
     _Where[1]         = "FacTabla.Codigo =  TRIM(AlmSFami.codfam ) + TRIM(AlmSFami.subfam)
  AND FacTabla.CodCia = AlmSFami.CodCia
