...
PEDI.ImpLin 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table PEDI.codmat PEDI.UndVta ~
PEDI.AlmDes PEDI.CanPed 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      PEDI.UndVta COLUMN-LABEL "Unidad" FORMAT "x(10)":U WIDTH 7.43
      ...
      PEDI.Por_Dsctos[3] COLUMN-LABEL "% Dscto!Vol/Prom" FORMAT "->>9.99":U
            WIDTH 7.43
      PEDI.ImpLin FORMAT ">,>>>,>>9.99":U WIDTH 5.72
  ENABLE
      PEDI.codmat
      PEDI.UndVta
      PEDI.AlmDes
...
"PEDI.UndVta" "Unidad" "x(10)" "character" ? ? ? ? ? ? yes ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.PEDI.AlmDes
"PEDI.AlmDes" "Alm!Desp" ? "character" ? ? ? ? ? ? yes ? no no "4.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"PEDI.ImpLin" ? ">,>>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "5.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ENTRY OF PEDI.codmat IN BROWSE br_table /* Articulo */
DO:
  /* SOLO PASA POR AQUI CUANDO SE CREA UN NUEVO REGISTRO
  APROVECHAMOS PARA INICIALIZAR ALGUNOS VALORES */
    DISPLAY
        ENTRY(1, s-CodAlm) WHEN PEDI.AlmDes:SCREEN-VALUE IN BROWSE {&browse-name} = "" @ PEDI.AlmDes 
        1 WHEN DEC(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&browse-name}) = 0 @ PEDI.CanPed
...
DO:
    ...
    DISPLAY 
        ...
        Almmmatg.UndA WHEN PEDI.undvta:SCREEN-VALUE IN BROWSE {&browse-name} = "" @ PEDI.undvta
        WITH BROWSE {&BROWSE-NAME}.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAILABLE Almmmate THEN DO:
      MESSAGE "Articulo no asignado al almacén " PEDI.AlmDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} 
           VIEW-AS ALERT-BOX ERROR.
...
/*                                 OUTPUT s-StkComprometido).                         */
  RUN vta2/Stock-Comprometido-v2 (PEDI.CodMat:SCREEN-VALUE IN BROWSE {&browse-name}, 
                                PEDI.AlmDes:SCREEN-VALUE IN BROWSE {&browse-name}, 
                                OUTPUT s-StkComprometido).
  ...
  IF (x-StkAct - s-StkComprometido) < x-CanPed
      THEN DO:
        MESSAGE "No hay STOCK suficiente" SKIP(1)
                ...
                "   STOCK DISPONIBLE : " (x-StkAct - s-StkComprometido) Almmmatg.undbas SKIP(1)
                "¿Continuamos con la grabación?"
                VIEW-AS ALERT-BOX QUESTION UPDATE rpta AS LOG.
        IF rpta = NO THEN DO:
            APPLY 'ENTRY':U TO PEDI.CanPed.
            RETURN "ADM-ERROR".
       END.
