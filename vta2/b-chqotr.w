...
DEFINE TEMP-TABLE C-Detalle NO-UNDO LIKE INTEGRAL.Facdpedi.
DEFINE TEMP-TABLE Detalle NO-UNDO LIKE INTEGRAL.Facdpedi
       INDEX LLave01 AS PRIMARY CodCia CodMat.
DEFINE TEMP-TABLE T-DPEDI NO-UNDO LIKE INTEGRAL.FacDPedi.
...
DEF VAR x-CodDoc AS CHAR INIT "OTR" NO-UNDO.
...
INTEGRAL.Almmmatg.DesMat Detalle.UndVta Detalle.CanPed 
...
txtTTrans br_table 
&Scoped-Define DISPLAYED-OBJECTS x-FchDoc x-NroDoc x-CodCli x-NomCli ~
x-NroItem txtTTxx txtTTrans 
...
DEFINE FRAME F-Main
     ...
     x-CodCli AT ROW 2.54 COL 12 COLON-ALIGNED
     x-NomCli AT ROW 2.54 COL 20 COLON-ALIGNED NO-LABEL
...
ON ENTRY OF x-NroDoc IN FRAME F-Main /* Nro. de Orden */
DO:
    DO WITH FRAME {&FRAME-NAME}:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL x-NroDoc B-table-Win
ON LEAVE OF x-NroDoc IN FRAME F-Main /* Nro. de Orden */
OR RETURN OF {&SELF-NAME}
    DO:
        ...
        IF NOT AVAILABLE Faccpedi THEN DO:
            MESSAGE 'Orden de Transferencia NO registrada' VIEW-AS ALERT-BOX ERROR.
            SELF:SCREEN-VALUE = ''.
            RETURN NO-APPLY.
        END.
        /*RD01- Verifica PICKING OK */
        /*IF NOT (Faccpedi.flgest = 'P' AND Faccpedi.flgsit <> 'C') THEN DO:*/
        IF NOT (Faccpedi.flgest = 'P' AND Faccpedi.flgsit = 'P') THEN DO:
            MESSAGE 'Orden de Transferencia NO se puede chequear' VIEW-AS ALERT-BOX ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      END.
      ASSIGN 
          Faccpedi.flgsit = 'C'         /* Cierre de chequeo DE SALIDA */
...
END.
ELSE MESSAGE "b-chqotr.wrx":U SKIP(1)
