...
Almmmatg.DesMat Almmmatg.UndStk Promocion.Libre_d04 Promocion.Libre_d05 ~
Promocion.Libre_d03 
...
&Scoped-define QUERY-STRING-br_table FOR EACH Promocion WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Almmmatg WHERE Almmmatg.CodCia = Promocion.CodCia ~
  AND Almmmatg.codmat = Promocion.LlaveDetalle NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Promocion WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH Almmmatg WHERE Almmmatg.CodCia = Promocion.CodCia ~
  AND Almmmatg.codmat = Promocion.LlaveDetalle NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
DEFINE VARIABLE FILL-IN-1 AS DECIMAL FORMAT ">>,>>9.99":U INITIAL 0 
     LABEL "TOTAL A PAGAR POR PROMOCIONES S/." 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Promocion.Libre_d04 COLUMN-LABEL "Cantidad" FORMAT ">>>9":U
            COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 11
      Promocion.Libre_d05 COLUMN-LABEL "Máximo" FORMAT ">>>9":U
            COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 14
      Promocion.Libre_d03 COLUMN-LABEL "P.U. S/." FORMAT "->>,>>9.9999":U
  ENABLE
      Promocion.Libre_d04
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     ...
     FILL-IN-1 AT ROW 7.73 COL 65 COLON-ALIGNED WIDGET-ID 2
...
"Promocion.LlaveDetalle" "Promoción" "x(6)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Promocion.Libre_d04" "Cantidad" ">>>9" "decimal" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.Promocion.Libre_d05
"Promocion.Libre_d05" "Máximo" ">>>9" "decimal" 14 0 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.Promocion.Libre_d03
"Promocion.Libre_d03" "P.U. S/." ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  RUN Procesa-Handle IN lh_handle ("RETURN").
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Borra-el-resto B-table-Win 
PROCEDURE Borra-el-resto :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Borra todo menos el registro activo
------------------------------------------------------------------------------*/

DEF VAR x-Rowid AS ROWID NO-UNDO.

IF NOT AVAILABLE Promocion THEN RETURN.
x-Rowid = ROWID(Promocion).
FOR EACH Promocion WHERE ROWID(Promocion) <> x-Rowid:
    DELETE Promocion.
END.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEF OUTPUT PARAMETER pCuenta AS INT.

DEF BUFFER b-Promocion FOR Promocion.

pCuenta = 0.
FOR EACH b-Promocion NO-LOCK WHERE b-Promocion.Libre_d04 > 0:
    pCuenta = pCuenta + 1.
END.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
  /* Solo se puede borrar si s-Tipo = "OR" y no tiene precio unitario */
  IF NOT (s-Tipo = "OR" AND Promocion.Libre_d03 = 0) THEN DO:
      MESSAGE 'Accesoo Denegado' VIEW-AS ALERT-BOX ERROR.
      RETURN 'ADM-ERROR'.
  END.
...
------------------------------------------------------------------------------*/
FILL-IN-1 = 0.
DEF BUFFER BPromocion FOR Promocion.
    FOR EACH BPromocion NO-LOCK:
        FILL-IN-1 = FILL-IN-1 + (BPromocion.Libre_d04 * BPromocion.Libre_d03).
    END.
...
IF DECIMAL(Promocion.Libre_d04:SCREEN-VALUE IN BROWSE {&browse-name}) >  Promocion.Libre_d05
    ...
    RETURN "ADM-ERROR".
END.
IF Promocion.Libre_d03 = 0 THEN DO: /* NO tiene precio unitario */
    IF DECIMAL(Promocion.Libre_d04:SCREEN-VALUE IN BROWSE {&browse-name}) > 0 
        AND DECIMAL(Promocion.Libre_d04:SCREEN-VALUE IN BROWSE {&browse-name}) <  Promocion.Libre_d05
        THEN DO:
        MESSAGE 'Solo se aceptan los siguientes valores:' SKIP
            '0 (cero) ó' Promocion.Libre_d05
...
IF s-Tipo = "AND" AND Promocion.Libre_d03 = 0 THEN DO:
