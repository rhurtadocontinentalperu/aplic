...
DEFINE VARIABLE h_v-facman-cred AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-facman-cred-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-facman-cred ).
       RUN set-position IN h_v-facman-cred ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.46 , 134.29 ) */
       ...
       /* Links to SmartViewer h_v-facman-cred. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-facman-cred ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-facman-cred ).
...
CASE L-Handle:
    ...
    WHEN "Importe-de-control" THEN DO:
        RETURN 'OK'.
/*         RUN Importe-de-control IN h_v-facman-cred.             */
