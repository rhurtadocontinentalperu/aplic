...
/* Si se manda un tercer parámetro sería la DIVISION */
...
DEFINE BUTTON btn-masivo 
     LABEL "Masivo desde TXTs" 
     SIZE 18 BY .92.
...
DEFINE FRAME F-Main
     ...
     Btn-Grupos-2 AT ROW 24.08 COL 27 WIDGET-ID 30
     Btn-Grupos AT ROW 24.08 COL 47 WIDGET-ID 10
     BUTTON-SUPERMERCADOS AT ROW 24.08 COL 67 WIDGET-ID 18
     BUTTON-CISSAC AT ROW 24.08 COL 95 WIDGET-ID 20
     BUTTON-FROM-EXCEL AT ROW 24.08 COL 114 WIDGET-ID 28
     btn-masivo AT ROW 24.12 COL 6.86 WIDGET-ID 32
...
DO:
    ...
    RUN dispatch IN h_t-cotizacion-cred-01 ('open-query').
END.
...
&Scoped-define SELF-NAME btn-masivo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn-masivo W-Win
ON CHOOSE OF btn-masivo IN FRAME F-Main /* Masivo desde TXTs */
DO:
  RUN Importacion-masiva IN h_v-cotizacion-cred.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             btn-masivo:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  't-cotizacion-cred-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotizacion-cred-01 ).
       RUN set-position IN h_t-cotizacion-cred-01 ( 11.50 , 2.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-24 COMBO-NroSer BTN-Excel BUTTON-EDI BUTTON-FROM-EXCEL btn-masivo 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         ASSIGN
             ...
             Btn-masivo:VISIBLE = NO. 
             Btn-masivo:SENSITIVE = NO.
         ...
         CASE s-TpoPed:
             ...
             WHEN "S" THEN DO:  /* Supermercados */
                 ...
                 Btn-masivo:VISIBLE = YES. 
                 Btn-masivo:SENSITIVE = YES.
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         RUN dispatch IN h_t-cotizacion-cred-01 ('open-query':U).
      END.
    WHEN "Disable-Head" THEN RUN dispatch IN h_p-updv01 ('disable':U).
    WHEN "Enable-Head" THEN RUN dispatch IN h_p-updv01 ('enable':U).
    WHEN "Disable-Button-IBC" THEN BUTTON-SUPERMERCADOS:SENSITIVE = NO.
    WHEN "Disable-Button-CISSAC" THEN BUTTON-CISSAC:SENSITIVE = NO.
    WHEN "Add-Record" THEN RUN dispatch IN h_t-cotizacion-cred-01 ('add-record':U).
    WHEN "browse" THEN DO:
      ...
      END.
    WHEN "Recalculo" THEN RUN Recalcular-Precios IN h_t-cotizacion-cred-01.
    WHEN 'IBC' THEN RUN IBC-Diferencias IN h_t-cotizacion-cred-01.
    WHEN 'Ultimo-Registro' THEN RUN Ultimo-Registro IN h_q-pedido.
END CASE.
