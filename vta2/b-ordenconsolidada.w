...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacDPedi.CanPed FORMAT ">>>,>>9.99":U WIDTH 7.57
  ENABLE
      FacDPedi.CanPed
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SIZE 109 BY 11.04
...
"Almmmatg.DesMat" ? "x(60)" "character" ? ? ? ? ? ? no ? no no "60" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacDPedi.CanPed" ? ">>>,>>9.99" "decimal" ? ? ? ? ? ? yes ? no no "7.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Borra-Pedido-Consolidado B-table-Win 
PROCEDURE Borra-Pedido-Consolidado :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
  ...
  pRowid1 = ROWID(Faccpedi).     /* Puntero de la ODC */
  pRowid2 = ROWID(Facdpedi).

  CICLO:
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      /* Bloqueamos detalle ODC */
      FIND CURRENT Facdpedi EXCLUSIVE-LOCK NO-ERROR.
      IF ERROR-STATUS:ERROR = YES THEN RETURN 'ADM-ERROR'.
      x-CodMat = Facdpedi.CodMat.
      /*DELETE Facdpedi.*/

      FOR EACH PEDI WHERE PEDI.codmat = x-CodMat:
          /* Apuntamos a la O/D */
          FIND Faccpedi WHERE Faccpedi.codcia = PEDI.codcia
              AND Faccpedi.coddiv = PEDI.coddiv
              AND Faccpedi.coddoc = PEDI.coddoc
              AND Faccpedi.nroped = PEDI.nroped
              EXCLUSIVE-LOCK NO-ERROR.
          IF NOT AVAILABLE Faccpedi THEN DO:
              FIND Faccpedi WHERE ROWID(Faccpedi) = pRowid1 NO-LOCK.
              UNDO CICLO, RETURN 'ADM-ERROR'.
          END.
          RUN Borra-Pedido-Consolidado.
      ...
      END.
      FIND Faccpedi WHERE ROWID(Faccpedi) = pRowid1 NO-LOCK.
      FIND Facdpedi WHERE ROWID(Facdpedi) = pRowid2 NO-LOCK.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Repitar-Registro B-table-Win 
PROCEDURE Repitar-Registro :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
