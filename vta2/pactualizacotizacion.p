...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* RHC 11/08/2014 Solicitud de Transferencia STR */
...
&IF DEFINED(EXCLUDE-Actualiza_RA) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Actualiza_RA Procedure 
PROCEDURE Actualiza_RA PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE Revisando-Saldos PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF VAR cFlgEst AS CHAR INIT "C,G,X,W,WX,WL" NO-UNDO.

FOR EACH PEDIDO NO-LOCK WHERE PEDIDO.codcia = B-CPEDI.codcia
    AND PEDIDO.codref = B-CPEDI.coddoc      /* COT */
    AND PEDIDO.nroref = B-CPEDI.nroped:
    IF NOT (PEDIDO.coddiv = B-CPEDI.coddiv AND PEDIDO.coddoc = 'PED' AND LOOKUP(TRIM(PEDIDO.flgest), cFlgEst) > 0) THEN NEXT.
    FOR EACH DETALLE OF PEDIDO NO-LOCK WHERE DETALLE.codmat = facdpedi.codmat:
        x-canate = x-canate + DETALLE.canped.
    END.
END.
