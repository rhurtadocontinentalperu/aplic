...
&Scoped-define INTERNAL-TABLES Almmmatg Almmmate AlmSFami VtaTabla
...
Almmmatg.codmat Almmmatg.DesMat _PrecioVenta() @ Almmmatg.PreOfi ~
_UnidadVenta() @ Almmmatg.UndStk Almmmate.StkAct ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH Almmmatg WHERE ~{&KEY-PHRASE} ~
 ...
 AND (RADIO-SET-Stock = "T" OR Almmmate.StkAct >= 0) NO-LOCK, ~
      ...
      EACH VtaTabla WHERE VtaTabla.CodCia = Almmmatg.CodCia ~
  AND VtaTabla.Llave_c1 = Almmmatg.codmat ~
      AND VtaTabla.Tabla = "REMATES" NO-LOCK ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almmmatg WHERE ~{&KEY-PHRASE} ~
 ...
 AND (RADIO-SET-Stock = "T" OR Almmmate.StkAct >= 0) NO-LOCK, ~
  ...
  AND VtaTabla.Llave_c1 = Almmmatg.codmat ~
      AND VtaTabla.Tabla = "REMATES" NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table Almmmatg Almmmate AlmSFami VtaTabla
...
<SORTBY-OPTIONS>
Codigo|||INTEGRAL.Almmmatg.codmat|yes
Familia|||INTEGRAL.Almmmatg.codfam|yes
Descripcion|y||INTEGRAL.Almmmatg.DesMat|yes
Marca|||INTEGRAL.Almmmatg.DesMar|yes
Sub-Familia|||INTEGRAL.Almmmatg.codfam|yes,INTEGRAL.Almmmatg.subfam|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      _PrecioVenta() @ Almmmatg.PreOfi COLUMN-LABEL "Precio Venta S/."
      _UnidadVenta() @ Almmmatg.UndStk COLUMN-LABEL "Unidad!Venta"
      Almmmate.StkAct FORMAT "(ZZZ,ZZZ,ZZ9.99)":U WIDTH 8.57
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Almmmatg,INTEGRAL.Almmmate OF INTEGRAL.Almmmatg,INTEGRAL.AlmSFami OF INTEGRAL.Almmmatg,INTEGRAL.VtaTabla WHERE INTEGRAL.Almmmatg ..."
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST, FIRST,"
     _Where[1]         = "{&Condicion}"
     _Where[2]         = "INTEGRAL.Almmmate.CodAlm = x-codalm
 AND (RADIO-SET-Stock = ""T"" OR INTEGRAL.Almmmate.StkAct >= 0)"
     _JoinCode[4]      = "INTEGRAL.VtaTabla.CodCia = INTEGRAL.Almmmatg.CodCia
  AND INTEGRAL.VtaTabla.Llave_c1 = INTEGRAL.Almmmatg.codmat"
     _Where[4]         = "INTEGRAL.VtaTabla.Tabla = ""REMATES"""
...
"_PrecioVenta() @ Almmmatg.PreOfi" "Precio Venta S/." ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"_UnidadVenta() @ Almmmatg.UndStk" "Unidad!Venta" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.Almmmate.StkAct
"Almmmate.StkAct" ? ? "decimal" ? ? ? ? ? ? no ? no no "8.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > "_<CALC>"
"_Comprometido(Almmmatg.codmat, x-codalm) @ f-StockComprometido" "Stock!Comprometido" "->>>>,>>9.99" ? ? ? ? ? ? ? no ? no no "11.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         ...
         WHEN 'Sub-Familia':U THEN DO:
           &Scope SORTBY-PHRASE BY Almmmatg.codfam BY Almmmatg.subfam
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
DEF VAR x-TipDto AS CHAR.
    ...
    RUN vta2/PrecioMayorista-Cred (
        ...
        OUTPUT z-Dsctos,
        OUTPUT x-TipDto,
        FALSE       /* NO muestra mensaje de error */
        ).
    RETURN f-PreVta.
