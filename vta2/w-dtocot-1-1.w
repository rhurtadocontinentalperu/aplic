...
DEFINE VARIABLE h_q-dtocot-1-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-dtocot-1-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-dtocot-1-1 AS HANDLE NO-UNDO.
...
DO:
  RUN dispatch IN h_v-dtocot-1-1 ('imprime':U).
END.
...
DO:
    ...
    RUN dispatch IN h_q-dtocot-1-1 ('open-query':U).
END.
...
CASE s-CodDoc:
    ...
    WHEN 'P/M' THEN W-Win:TITLE = "DESCUENTO PEDIDO MOSTRADOR".
    WHEN 'PED' THEN W-Win:TITLE = "DESCUENTO PEDIDOS AL CREDITO".
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-dtocot-1-1.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-dtocot-1-1 ).
       RUN set-position IN h_v-dtocot-1-1 ( 2.35 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.50 , 100.86 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/q-dtocot-1-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-dtocot-1-1 ).
       RUN set-position IN h_q-dtocot-1-1 ( 1.00 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-dtocot-1-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_v-dtocot-1-1 ).
       RUN add-link IN adm-broker-hdl ( h_q-dtocot-1-1 , 'Record':U , h_v-dtocot-1-1 ).

       /* Links to SmartQuery h_q-dtocot-1-1. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-dtocot-1-1 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv97 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-dtocot-1-1 ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-dtocot-1-1 ,
             h_v-dtocot-1-1 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/b-cotiza.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotiza ).
       RUN set-position IN h_b-cotiza ( 6.12 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotiza ( 13.38 , 141.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotiza. */
       RUN add-link IN adm-broker-hdl ( h_q-dtocot-1-1 , 'Record':U , h_b-cotiza ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-dtocot-1-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-dtocot-1-1 ).
       RUN set-position IN h_t-dtocot-1-1 ( 6.12 , 1.43 ) NO-ERROR.
       RUN set-size IN h_t-dtocot-1-1 ( 11.50 , 149.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv96 ).
       RUN set-position IN h_p-updv96 ( 17.69 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv96 ( 1.42 , 34.29 ) NO-ERROR.

       /* Links to SmartBrowser h_t-dtocot-1-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv96 , 'TableIO':U , h_t-dtocot-1-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-dtocot-1-1 ,
             h_v-dtocot-1-1 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv96 ,
             h_t-dtocot-1-1 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-cotiza ('open-query':U).
          RUN dispatch IN h_t-dtocot-1-1 ('open-query':U).
      END.
    WHEN "Recalcular-Precios" THEN DO:
        RUN dispatch IN h_t-dtocot-1-1 ('recalcular-precios':U).
    END.
