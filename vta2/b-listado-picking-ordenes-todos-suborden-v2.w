...
DEFINE VAR x-sort-column-current AS CHAR NO-UNDO.
...
DISPLAY "INICIO DE PROCESO" @ Fi-Mensaje WITH FRAME F-Proceso.

REPEAT z-sec = 1 TO NUM-ENTRIES(z-docmntos,","):
    z-coddoc = ENTRY(z-sec,z-docmntos,",").

    CASE TRUE:
        WHEN x-pendientes-impresion = 1 THEN DO:
            FOR EACH Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-codcia 
                    AND Faccpedi.divdes = s-coddiv
                    AND Faccpedi.coddoc = z-coddoc
                ...
                /* Rutina General */
                {vta2/i-listado-picking-ordenes-todos-suborden-v2.i}

            END.
        END.
        WHEN x-pendientes-impresion = 2 THEN DO:
            FOR EACH Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-codcia 
                    AND Faccpedi.divdes = s-coddiv
                    AND Faccpedi.coddoc = z-coddoc
                    AND Faccpedi.flgest = 'P'
...
FOR EACH VtaCDocu NO-LOCK WHERE VtaCDocu.codcia = s-codcia 
    ...
    IF NOT (Vtacdocu.flgest <> 'X') THEN NEXT.
