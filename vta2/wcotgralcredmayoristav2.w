...
DEFINE VARIABLE h_tcotgralcredmayoristav2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_tcotizacioncreditomay-a AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vcotgralcredmayoristav2 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-POR-ATENDER AT ROW 1.19 COL 127 WIDGET-ID 42
     COMBO-NroSer AT ROW 1.27 COL 66.71 WIDGET-ID 4
     BTN-Excel AT ROW 2.35 COL 119.57 WIDGET-ID 16
     btnExcVend AT ROW 2.46 COL 129.86 WIDGET-ID 40
     B-CLI AT ROW 3.69 COL 119 WIDGET-ID 22
     B-CCTE AT ROW 4.85 COL 119 WIDGET-ID 12
     BUTTON-COT AT ROW 6 COL 119 WIDGET-ID 14
     BUTTON-IMPORTAR-EXCEL AT ROW 7.15 COL 119 WIDGET-ID 50
     BUTTON-TRF-SDO-EXPO AT ROW 9.46 COL 119 WIDGET-ID 46
     BUTTON-EDI AT ROW 10.62 COL 119 WIDGET-ID 24
     BUTTON-IMP-HIST-COT AT ROW 11.77 COL 119 WIDGET-ID 36
     ...
     BUTTON-IMP-OPENORANGE AT ROW 26.19 COL 112 WIDGET-ID 34
     BUTTON-Importar-Pedido AT ROW 26.19 COL 116 WIDGET-ID 48
     "Buscar el número:" VIEW-AS TEXT
          SIZE 13 BY .5 AT ROW 1.54 COL 79 WIDGET-ID 6
     RECT-24 AT ROW 2.35 COL 2 WIDGET-ID 8
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 143.43 BY 26.23
...
DO:
   RUN Actualiza-datos-cliente IN h_vcotgralcredmayoristav2.
END.
...
/*     END CASE.                                                                    */
    CASE TRUE :
        WHEN INDEX(gn-divi.canalventa, "MIN") > 0
            THEN RUN Excel_Utilex IN h_vcotgralcredmayoristav2 (INPUT lchoice) .
        OTHERWISE RUN Genera-Excel2 IN h_vcotgralcredmayoristav2 (INPUT lchoice) .
    END CASE.
END.
...
DO:
    ...
    RUN dispatch IN h_tcotgralcredmayoristav2 ('open-query').
END.
...
/*     END CASE.                                                                    */
    IF lChoice = TRUE THEN DO:
        RUN Genera-Excel-vendedores IN h_vcotgralcredmayoristav2 (INPUT lchoice) .
    END.
END.
...
DO:
  RUN Importar-entre-companias IN h_vcotgralcredmayoristav2.
  IF RETURN-VALUE = "ADM-ERROR" THEN RETURN NO-APPLY.
  RUN dispatch IN h_tcotgralcredmayoristav2 ('open-query':U).
END.
...
DO:
  RUN Edi-Comparativo IN h_vcotgralcredmayoristav2.
END.
...
DO:
   RUN Captura-Historico-Cotizaciones IN h_vcotgralcredmayoristav2.
END.
...
DO:
   RUN Importar-Excel-OpenOrange IN h_vcotgralcredmayoristav2.
END.
...
DO:
   RUN Importar-Excel-Provincias IN h_vcotgralcredmayoristav2.
END.
...
DO:
   RUN Importar-Excel-Marco IN h_vcotgralcredmayoristav2.
END.
...
DO:
    RUN Importar-Supermercados IN h_vcotgralcredmayoristav2.
    IF RETURN-VALUE = "ADM-ERROR" THEN RETURN NO-APPLY.
    RUN dispatch IN h_tcotgralcredmayoristav2 ('open-query':U).
END.
...
DO:
    RUN Importar-Supermercados-B2B IN h_vcotgralcredmayoristav2.
    IF RETURN-VALUE = "ADM-ERROR" THEN RETURN NO-APPLY.
    RUN dispatch IN h_tcotgralcredmayoristav2 ('open-query':U).
END.
...
DO:
   ...
   RUN Transferencia-de-Saldos IN h_vcotgralcredmayoristav2
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 1.00 , 102.00 ) NO-ERROR.
       RUN set-size IN h_p-navico-2 ( 1.35 , 20.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/vcotgralcredmayoristav2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vcotgralcredmayoristav2 ).
       RUN set-position IN h_vcotgralcredmayoristav2 ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.04 , 115.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-pedido ).
       RUN set-position IN h_q-pedido ( 1.00 , 92.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 9.00 ) */

       /* Links to SmartViewer h_vcotgralcredmayoristav2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_vcotgralcredmayoristav2 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_vcotgralcredmayoristav2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico-2 ,
             h_p-updv01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vcotgralcredmayoristav2 ,
             btnExcVend:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             BUTTON-Importar-Pedido:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_b-cotizacion-cred ( 12.92 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/tcotgralcredmayoristav2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tcotgralcredmayoristav2 ).
       RUN set-position IN h_tcotgralcredmayoristav2 ( 12.92 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tcotgralcredmayoristav2 ( 12.54 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_tcotgralcredmayoristav2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tcotgralcredmayoristav2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tcotgralcredmayoristav2 ,
             BUTTON-IMP-HIST-COT:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tcotgralcredmayoristav2 , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/tcotizacioncreditomay-a.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tcotizacioncreditomay-a ).
       RUN set-position IN h_tcotizacioncreditomay-a ( 12.92 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tcotizacioncreditomay-a ( 12.54 , 141.00 ) NO-ERROR.
...
CASE L-Handle:
    ...
    WHEN "Disable-Head" THEN DO:
        ...
        RUN dispatch IN h_vcotgralcredmayoristav2 ('disable-fields':U).
    END.
    WHEN "Enable-Head" THEN DO:
        ...
        RUN dispatch IN h_vcotgralcredmayoristav2 ('enable-fields':U).
    END.
    ...
    WHEN "browse" THEN DO:
          ...
          IF h_tcotgralcredmayoristav2 <> ? THEN RUN dispatch IN h_tcotgralcredmayoristav2 ('open-query':U). 
          IF h_tcotizacioncreditomay-a <> ? THEN RUN dispatch IN h_tcotizacioncreditomay-a ('open-query':U). 
      END.
    WHEN "Recalculo" THEN RUN Recalcular-Precios IN h_vcotgralcredmayoristav2.
    WHEN 'IBC' THEN RUN IBC-Diferencias IN h_tcotgralcredmayoristav2.
