...
FacCPedi.FlgIgv FacCPedi.FmaPgo FacCPedi.NroRef FacCPedi.NroCard 
...
DEFINE FRAME F-Main
     ...
     FacCPedi.FlgSit AT ROW 1 COL 59 NO-LABEL
...
"Tarjeta de Crédito", "T":U
     ...
     FacCPedi.Atencion AT ROW 3.42 COL 29 COLON-ALIGNED WIDGET-ID 4
          ...
          SIZE 13 BY .81
          BGCOLOR 11 FGCOLOR 9 
     FacCPedi.ordcmp AT ROW 3.42 COL 92 COLON-ALIGNED WIDGET-ID 32
          LABEL "O/Compra"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
          BGCOLOR 11 FGCOLOR 9 
     FacCPedi.LugEnt AT ROW 4.23 COL 9 COLON-ALIGNED WIDGET-ID 20
          ...
          SIZE 62 BY .81
          BGCOLOR 11 FGCOLOR 9 
     FacCPedi.Cmpbnte AT ROW 4.23 COL 92 COLON-ALIGNED
          LABEL "Comprobante"
     ...
     FacCPedi.Glosa AT ROW 5.04 COL 6.28
          ...
          SIZE 62 BY .81
          BGCOLOR 11 FGCOLOR 9 
     FacCPedi.CodMon AT ROW 5.04 COL 94 NO-LABEL
          VIEW-AS RADIO-SET HORIZONTAL
          RADIO-BUTTONS 
                    "S/.", 1,
"US$", 2
     ...
     FacCPedi.FlgIgv AT ROW 5.85 COL 94 NO-LABEL WIDGET-ID 28
          VIEW-AS RADIO-SET HORIZONTAL
          RADIO-BUTTONS 
                    "Si", yes,
"No", no
...
DEFINE FRAME F-Main
     FacCPedi.NroRef AT ROW 6.65 COL 92 COLON-ALIGNED WIDGET-ID 10
          LABEL "Cotizacion"
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
     ...
     "Moneda:" VIEW-AS TEXT
          SIZE 7 BY .5 AT ROW 5.04 COL 87
     "Cancela con:" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 1.27 COL 49
     "Con IGV:" VIEW-AS TEXT
          SIZE 6.43 BY .81 AT ROW 5.85 COL 87 WIDGET-ID 26
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1 SCROLLABLE 
         FONT 4.
...
DO:
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    Faccpedi.Cmpbnte:SCREEN-VALUE = IF Faccpedi.RucCli:SCREEN-VALUE <> '' THEN 'FAC' ELSE 'BOL'.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'NO' THEN DO:
    FOR EACH Facdpedi OF Faccpedi NO-LOCK WHERE Facdpedi.Libre_c05 <> 'OF'  /* SIN PROMOCIONES BIC */
        BREAK BY Facdpedi.CodMat:
        CREATE PEDI.
        BUFFER-COPY Facdpedi TO PEDI.
        IF FIRST-OF(Facdpedi.Codmat) THEN DO:
            CREATE PEDI-1.
            BUFFER-COPY PEDI TO PEDI-1.
        END.
    END.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:  
      ...
      /* ASIGNA DETALLE DE LA COTIZACION */
      RUN Asigna-Detalle-Cotizacion.
      /* ******************************* */
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Facdpedi OF B-CPEDI NO-LOCK WHERE (Facdpedi.CanPed - Facdpedi.CanAte) > 0,
      ...
      BUFFER-COPY Facdpedi 
          ...
          PEDI-1.CodDiv = s-coddiv
          PEDI-1.CodDoc = s-coddoc
          PEDI-1.NroPed = ''
          PEDI-1.Libre_d01 = (FacDPedi.CanPed - FacDPedi.CanAte)
          PEDI-1.CanPed = (Facdpedi.CanPed - Facdpedi.CanAte).
      ...
      IF NOT AVAILABLE PEDI THEN DO:
          MESSAGE 'NO hay stock suficiente para cubrir el producto' Facdpedi.codmat
      ...
      FOR EACH PEDI:
          ...
          IF pError = "ADM-ERROR" THEN UNDO DETALLE, NEXT.
      END.
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Distribuye-por-almacenes V-table-Win 
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH PEDI-1:
      {vta2/i-pedmos-may-alm-01.i}
  END.

  FOR EACH PEDI WHERE PEDI.CanPed > 0 BY PEDI.NroItm:
      I-NPEDI = I-NPEDI + 1.
      CREATE Facdpedi.
...
------------------------------------------------------------------------------*/
  ...
  MESSAGE 'Entregar en otras sedes?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO-CANCEL UPDATE s-FlgEnv.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      ...
      ASSIGN 
        ...
        FacCPedi.CodAlm = ENTRY(1, s-CodAlm)      /* SOLO INFORMATIVO */
  ...
  /* RHC 08.08.11 agrgamos las promociones */
  RUN vta2/promociones-por-proveedor ( ROWID(Faccpedi) ).
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
  IF Faccpedi.NroRef <> '' THEN DO:
      MESSAGE 'NO se puede copiar un pedido originado por una cotización'
  ...
  FOR EACH Facdpedi OF Faccpedi NO-LOCK BREAK BY Facdpedi.CodMat:
      IF FIRST-OF(Facdpedi.CodMat) THEN DO:
          CREATE PEDI-1.
          BUFFER-COPY Facdpedi 
              EXCEPT Facdpedi.CanAte Facdpedi.Libre_d01
              TO PEDI-1
              ASSIGN 
              PEDI-1.CodCia = s-codcia
              PEDI-1.CodDiv = s-coddiv
              ...
              PEDI-1.NroPed = ''.
      END.
      ELSE PEDI-1.CanPed = PEDI-1.CanPed + Facdpedi.CanPed.
  END.
...
IF Faccpedi.FlgEst <> 'P' THEN DO:
    ...
    RETURN "ADM-ERROR".
END.
/* Descuento por administrador */
FIND FIRST Facdpedi OF Faccpedi WHERE Facdpedi.por_dsctos[1] > 0
    NO-LOCK NO-ERROR.
IF AVAILABLE Facdpedi THEN DO:
    MESSAGE 'NO se puede modificar un pedido que ya tiene descuento por el Administrador'
        VIEW-AS ALERT-BOX ERROR.
    RETURN 'ADM-ERROR'.
END.
ASSIGN
