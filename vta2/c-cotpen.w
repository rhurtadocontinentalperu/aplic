...
&SCOPED-DEFINE l-lookup h_l-cotpen
...
DEFINE VARIABLE h_l-cotpen AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.69 COL 83
     Btn_Cancel AT ROW 8.88 COL 83
     Btn_Help AT ROW 10.69 COL 83
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/l-cotpen.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-cotpen ).
       RUN set-position IN h_l-cotpen ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-cotpen ( 10.92 , 79.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.54 , 81.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-cotpen. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-cotpen ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-cotpen ,
