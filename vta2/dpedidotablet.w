...
DEF VAR S-TPOPED AS CHAR INIT "CR" NO-UNDO.
...
IF B-CPEDI.LugEnt2 > '' 
    AND CAN-FIND(Almacen NO-LOCK WHERE Almacen.codcia = s-codcia AND Almacen.codalm = B-CPEDI.LugEnt2) 
...
DO k = 1 TO NUM-ENTRIES(s-CodAlm):
    ...
    REPEAT:
        RUN lib/_gen_password ( INPUT 12,
                                OUTPUT x-NroPed).
...
DO i = 1 TO NUM-ENTRIES(s-CodAlm):
    ...
    FOR EACH PEDI WHERE PEDI.AlmDes = x-CodAlm, FIRST Almmmatg OF PEDI NO-LOCK:
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* ********************************************************************************************** */
    /* GENERAMOS INMEDIATAMENTE LA O/D SI PASA LOS FILTROS */
    /* ********************************************************************************************** */
    pMensajeFinal = ''.
    RUN Despachar-Pedido.
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        IF TRUE <> (pMensaje > '') THEN pMensaje = "NO se pudo generar la O/D".
        UNDO, RETURN 'ADM-ERROR'.
    END.
    IF TRUE <> (pMensajeFinal > '') 
        THEN pMensajeFinal = 'Documento Generado: ' + Faccpedi.CodDoc + ' ' + Faccpedi.NroPed.
    ELSE pMensajeFinal = 'Documento Generado: ' + Faccpedi.CodDoc + ' ' + Faccpedi.NroPed + CHR(10) +
        'NO se pudo generar la O/D automáticamente' + CHR(10) + pMensajeFinal.
END.
RETURN 'OK'.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Despachar-Pedido D-Dialog 
...
IF Faccpedi.flgest <> "G" THEN RETURN 'ADM-ERROR'.
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    {vta2/verifica-cliente-01.i &VarMensaje = pMensajeFinal}
    ...
    IF Faccpedi.FlgEst = "X" THEN DO:
        ...
        /* ******************************* */
        /* ******************************* */
        IF TRUE <> (pMensajeFinal > '') THEN pMensajeFinal = "NO pasó la línea de crédito".
        LEAVE RLOOP.
    END.
    ...
    IF LOOKUP(s-CodDiv, '00000,00017,00018') > 0 THEN DO:
        ...
        IF Faccpedi.FlgEst = "W" THEN DO:
            pMensajeFinal = FacCPedi.Libre_c05.
            LEAVE RLOOP.
        END.
    END.
    /* CREAMOS LA ORDEN */
...
END.
RUN vta2/linea-de-credito-v2 (B-CPEDI.CodDiv,
                              B-CPEDI.CodCli,
                              B-CPEDI.FmaPgo,
                              B-CPEDI.CodMon,
                              f-Importe,
