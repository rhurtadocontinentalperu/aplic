...
------------------------------------------------------------------------------*/
  ...
  MESSAGE 'Despacho desde otras sedes?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO-CANCEL UPDATE s-FlgEnv.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' AND s-FlgEnv = YES THEN DO:
      ...
      RUN vta2/orde-des-por-div (s-codcia, s-codalm, s-codmon, s-tpocmb, s-flgenv).
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  IF FacCPedi.FlgEst <> "A" THEN RUN VTA2\R-ImpPed (ROWID(FacCPedi)).
