...
&Scoped-Define ENABLED-OBJECTS BtnDone BUTTON-13 BtnExcel ~
BUTTON-aDistribucion txtDesde txtHasta ChkChequeo BUTTON-PorZona ~
BUTTON-Alfabeticamente 
&Scoped-Define DISPLAYED-OBJECTS txtDesde txtHasta ChkChequeo txtArtSinPeso 
...
DEFINE VARIABLE h_b-cimprime-od AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BtnDone AT ROW 1 COL 132 WIDGET-ID 10
     BUTTON-13 AT ROW 1.19 COL 45.29 WIDGET-ID 4
     BtnExcel AT ROW 1.19 COL 98.14 WIDGET-ID 12
     BUTTON-aDistribucion AT ROW 1.19 COL 113.57 WIDGET-ID 14
     ...
     BUTTON-PorZona AT ROW 22.73 COL 3 WIDGET-ID 6
     BUTTON-Alfabeticamente AT ROW 22.73 COL 25 WIDGET-ID 8
     txtArtSinPeso AT ROW 23.19 COL 54 COLON-ALIGNED NO-LABEL WIDGET-ID 24
     "Lista de Articulos que NO tienen PESO en la O/D" VIEW-AS TEXT
          SIZE 43.14 BY .62 AT ROW 22.5 COL 56.43 WIDGET-ID 26
          BGCOLOR 15 FGCOLOR 9 FONT 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 153.29 BY 23.42 WIDGET-ID 100.
...
DO:
    ...
    RUN envia-excel IN h_b-cimprime-od.
...
DO:
    ...
    RUN dispatch IN h_b-cimprime-od ('open-query':U).
...
DO:
   RUN A-Distribucion IN h_b-cimprime-od.
END.
...
DO:
  ...
  RUN Imprimir-Formato IN h_b-cimprime-od ( INPUT "ALFABETICO").
END.
...
DO:
  /*RUN dispatch IN h_b-cimprime-od ('imprime':U).*/
    RUN Imprimir-Formato IN h_b-cimprime-od ( INPUT "ZONA").
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b-cimprime-od.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cimprime-od ).
       RUN set-position IN h_b-cimprime-od ( 2.54 , 1.72 ) NO-ERROR.
       RUN set-size IN h_b-cimprime-od ( 7.50 , 151.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b-dimprime-od ( 10.27 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-dimprime-od ( 12.12 , 134.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-dimprime-od. */
       RUN add-link IN adm-broker-hdl ( h_b-cimprime-od , 'Record':U , h_b-dimprime-od ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cimprime-od ,
             ChkChequeo:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-od ,
             h_b-cimprime-od , 'AFTER':U ).
    END. /* Page 0 */
...
OCXFile = SEARCH( "w-listado-picking-od.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-listado-picking-od.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  DISPLAY txtDesde txtHasta ChkChequeo txtArtSinPeso 
      WITH FRAME F-Main IN WINDOW sW-Win.
  ENABLE BtnDone BUTTON-13 BtnExcel BUTTON-aDistribucion txtDesde txtHasta 
         ChkChequeo BUTTON-PorZona BUTTON-Alfabeticamente 
      WITH FRAME F-Main IN WINDOW sW-Win.
