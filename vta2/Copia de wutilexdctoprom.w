...
"Provincias - Productos de 3ros", 5,
"Provincias - Productos Propios", 6
...
ASSIGN
    ...
    chWorkSheet:Range("H2:Z2"):NumberFormat = "@".
CASE RADIO-SET-1:
...
/* Sedes */
CASE RADIO-SET-1:
    WHEN 1 THEN f-Divisiones = "00001,00002,00003,00005,00011,00026,00038,00039,00014,00032,00060,00061,00024".
    WHEN 2 THEN f-Divisiones = "00023,00027,00501,00502,00503,00504,00507,00508".
...
DO k = 1 TO NUM-ENTRIES(f-Divisiones):
    ...
    chWorkSheet:Range(cRange + "1"):Value = GN-DIVI.DesDiv.
    chWorkSheet:Range(cRange + "2"):Value = GN-DIVI.CodDiv.
    ...
    CASE RADIO-SET-1:
        WHEN 1 THEN DO:
            ...
            DO k = 1 TO NUM-ENTRIES(f-Divisiones):
                ...
                END.
/*                 DO j = 1 TO 10:                                                                                     */
/*                     IF Almmmatg.PromDivi[j] = ENTRY(k, f-Divisiones) THEN DO:                                       */
/*                         ASSIGN                                                                                      */
/*                             chWorkSheet:Cells(t-Row, 8):VALUE = Almmmatg.PromFchD[j]                                */
/*                             chWorkSheet:Cells(t-Row, 9):VALUE = Almmmatg.PromFchH[j]                                */
/*                             chWorkSheet:Cells(t-Row, t-Column):VALUE = (IF RADIO-SET-2 = 1 THEN Almmmatg.PromDto[j] */
/*                                 ELSE ROUND(x-PreOfi * ( 1 - ( Almmmatg.PromDto[j] / 100 ) ),4) )                    */
/*                             NO-ERROR.                                                                               */
/*                         LEAVE.                                                                                      */
/*                     END.                                                                                            */
/*                 END.                                                                                                */
        ...
        WHEN 2 THEN DO:
            ...
            DO k = 1 TO NUM-ENTRIES(f-Divisiones):
                ASSIGN
                    t-Column = t-Column + 1.
                DO j = 1 TO 10:
                    IF Vtalistamingn.PromDivi[j] = ENTRY(k, f-Divisiones) THEN DO:
                        ASSIGN
                            chWorkSheet:Cells(t-Row, 8):VALUE = Vtalistamingn.PromFchD[j] 
                            chWorkSheet:Cells(t-Row, 9):VALUE = Vtalistamingn.PromFchH[j] 
                            chWorkSheet:Cells(t-Row, t-Column):VALUE = (IF RADIO-SET-2 = 1 THEN Vtalistamingn.PromDto[j]
                                ELSE ROUND(x-PreOfi * ( 1 - ( Vtalistamingn.PromDto[j] / 100 ) ),4) )
                            NO-ERROR.
                        LEAVE.
                    END.
                END.
...
/*                         ELSE ROUND(x-PreOfi * ( 1 - ( Almmmatg.PromMinDto[1] / 100 ) ),4) )                    */
                ...
                ASSIGN
                    chWorkSheet:Cells(t-Row, t-Column):VALUE = x-PreOfi
/*                     chWorkSheet:Cells(t-Row, t-Column):VALUE = (IF RADIO-SET-2 = 1 THEN Almmmatg.PromMinDto[2] */
/*                         ELSE ROUND(x-PreOfi * ( 1 - ( Almmmatg.PromMinDto[2] / 100 ) ),4) )                    */
                    NO-ERROR.
            END.
        END.
        WHEN 6 THEN DO:
