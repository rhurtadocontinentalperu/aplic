...
FOR EACH VtaAlmDiv NO-LOCK WHERE Vtaalmdiv.codcia = s-codcia
    ...
    AND Vtaalmdiv.codalm <> '':
    IF s-codalm  = '' THEN s-codalm = Vtaalmdiv.codalm.
    ELSE s-codalm = s-codalm + ',' + Vtaalmdiv.codalm.
END.
IF s-codalm = '' THEN DO:
    MESSAGE 'NO se ha definido un almacén por defecto para la división:' s-coddiv
        VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
...
DEFINE VARIABLE h_b-facman-utilex AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv05 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-docmto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-facman-utilex AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-facman-utilex AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "REGISTRO FACTURAS MANUALES"
         HEIGHT             = 21.58
         WIDTH              = 139.43
         MAX-HEIGHT         = 32.23
         MAX-WIDTH          = 205.72
...
ON END-ERROR OF W-Win /* REGISTRO FACTURAS MANUALES */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* REGISTRO FACTURAS MANUALES */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 1.00 , 84.29 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv05 ).
       RUN set-position IN h_p-updv05 ( 1.04 , 30.00 ) NO-ERROR.
       RUN set-size IN h_p-updv05 ( 1.31 , 54.29 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-facman-utilex-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-facman-utilex ).
       RUN set-position IN h_v-facman-utilex ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.46 , 131.29 ) */
       ...
       /* Links to SmartViewer h_v-facman-utilex. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-facman-utilex ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-facman-utilex ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-facman-utilex ,
             C-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-docmto ,
             h_v-facman-utilex , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-facman-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-facman-utilex ).
       RUN set-position IN h_b-facman-utilex ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-facman-utilex ( 12.85 , 137.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-facman-utilex. */
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_b-facman-utilex ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-facman-utilex ,
             h_v-facman-utilex , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-facman-utilex-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-facman-utilex ).
       RUN set-position IN h_t-facman-utilex ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-facman-utilex ( 11.50 , 127.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 20.92 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.35 , 39.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-facman-utilex. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-facman-utilex ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-facman-utilex ,
             h_v-facman-utilex , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-facman-utilex , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-facman-utilex ('open-query':U).
          RUN dispatch IN h_t-facman-utilex ('open-query':U).
      END.
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      END.
    WHEN "Importe-de-control" THEN DO:
        RETURN 'OK'.
/*         RUN Importe-de-control IN h_v-facman-utilex.           */
