...
Almmmatg.DesMar VtaTabla.Valor[1] VtaTabla.Valor[2] 
...
vtatabla.tabla = "VAMOS_AL_MUNDIAL" NO-LOCK, ~
      EACH Almmmatg WHERE Almmmatg.CodCia = VtaTabla.CodCia and ~
Almmmatg.codmat =  VtaTabla.Llave_c1 NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c1 COLUMN-LABEL "Cod!Art." FORMAT "x(6)":U
            WIDTH 9
      Almmmatg.DesMat FORMAT "X(45)":U
      Almmmatg.DesMar FORMAT "X(30)":U
      VtaTabla.Valor[1] COLUMN-LABEL "Valor S/." FORMAT "->>,>>9.99":U
            WIDTH 11
      VtaTabla.Valor[2] COLUMN-LABEL "Puntos" FORMAT "->,>>9.99":U
            WIDTH 15.72
  ENABLE
      VtaTabla.Llave_c1
      VtaTabla.Valor[1]
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 103 BY 21.23
         FONT 4 ROW-HEIGHT-CHARS .69.
...
vtatabla.tabla = ""VAMOS_AL_MUNDIAL"""
     _JoinCode[2]      = "Almmmatg.CodCia = VtaTabla.CodCia and
Almmmatg.codmat =  VtaTabla.Llave_c1"
     _FldNameList[1]   > INTEGRAL.VtaTabla.Llave_c1
"VtaTabla.Llave_c1" "Cod!Art." "x(6)" "character" ? ? ? ? ? ? yes ? no no "9" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.Almmmatg.DesMat
     _FldNameList[3]   = INTEGRAL.Almmmatg.DesMar
     _FldNameList[4]   > INTEGRAL.VtaTabla.Valor[1]
"VtaTabla.Valor[1]" "Valor S/." "->>,>>9.99" "decimal" ? ? ? ? ? ? yes ? no no "11" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.VtaTabla.Valor[2]
"VtaTabla.Valor[2]" "Puntos" "->,>>9.99" "decimal" ? ? ? ? ? ? yes ? no no "15.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  ASSIGN vtatabla.tabla = "VAMOS_AL_MUNDIAL"
...
x-codmat = vtatabla.llave_c1:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.
...
IF RETURN-VALUE = "YES" THEN DO:
    FIND FIRST b-vtatabla WHERE b-vtatabla.codcia = s-codcia AND
                                b-vtatabla.tabla = "VAMOS_AL_MUNDIAL" AND
                                b-vtatabla.llave_C1 = x-codmat NO-LOCK NO-ERROR.
...
x-cantidad = DECIMAL(vtatabla.valor[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
IF x-cantidad < 0 THEN DO:
    MESSAGE "Valor S/ debe ser mayor a cero".
    APPLY 'ENTRY':U TO vtatabla.valor[1] IN BROWSE {&BROWSE-NAME}.
    RETURN 'ADM-ERROR'.
END.                                     
x-cantidad = DECIMAL(vtatabla.valor[2]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
IF x-cantidad < 0 THEN DO:
    MESSAGE "Los puntos debe ser mayor a cero".
