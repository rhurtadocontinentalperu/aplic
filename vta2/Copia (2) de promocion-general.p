...
FOR EACH Vtadtabla OF Vtactabla NO-LOCK WHERE Vtadtabla.Tipo = "L":
    ...
    x-ImpLin = 0.
    x-Cantidad = 0.
    FOR EACH ITEM NO-LOCK, FIRST Almmmatg OF ITEM NO-LOCK WHERE Almmmatg.codfam = Vtadtabla.LlaveDetalle
        AND (Vtadtabla.Libre_c01 = "" OR Almmmatg.subfam = Vtatabla.Libre_c01):
        /* Buscamos si es una excepción */
        RUN Excepcion-Linea (OUTPUT pExcepcion).
        IF pExcepcion = YES THEN NEXT.
        /* Fin de excepciones */
        x-ImpLin = x-ImpLin + ITEM.ImpLin.
        x-Cantidad = x-Cantidad + (ITEM.CanPed * ITEM.Factor).
    END.
    /* Buscamos si existe al menos un registro válido */
    FIND FIRST ITEM NO-LOCK WHERE CAN-FIND(FIRST Almmmatg OF ITEM WHERE Almmmatg.codfam = Vtadtabla.LlaveDetalle
        AND (Vtadtabla.Libre_c01 = "" OR Almmmatg.subfam = Vtatabla.Libre_c01) NO-LOCK)
        NO-ERROR.
    /* Verificamos el "Operador" */
    CASE TRUE:
