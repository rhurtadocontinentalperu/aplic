...
&Scoped-Define ENABLED-OBJECTS BUTTON-Despachar RECT-25 COMBO-NroSer ~
...
DEFINE VARIABLE h_t-pedido-credv2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vpedidocreditomayv2 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     Btn-Excel AT ROW 7.46 COL 136 WIDGET-ID 20
     btn-margen AT ROW 7.54 COL 121 WIDGET-ID 2
     BTN-CCTE AT ROW 8.5 COL 121 WIDGET-ID 22
     B-AGTRANS AT ROW 9.46 COL 121 WIDGET-ID 24
     BUTTON-REDONDEO AT ROW 10.42 COL 121 WIDGET-ID 26
...
DO:
   RUN Asigna-datos IN h_vpedidocreditomayv2.
END.
...
DO:
  ...
  RUN Excel IN h_vpedidocreditomayv2.
...
DO:
  RUN Margen IN h_vpedidocreditomayv2.
END.
...
DO:
  RUN Despachar-Pedido IN h_vpedidocreditomayv2.
END.
...
DO:
  RUN Redondeo IN h_vpedidocreditomayv2.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/vpedidocreditomayv2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vpedidocreditomayv2 ).
       RUN set-position IN h_vpedidocreditomayv2 ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.88 , 117.00 ) */
       ...
       /* Links to SmartViewer h_vpedidocreditomayv2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_vpedidocreditomayv2 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_vpedidocreditomayv2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_vpedidocreditomayv2 ,
             h_p-navico , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-size IN h_b-pedido-cred ( 13.85 , 141.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-pedido-credv2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedido-credv2 ).
       RUN set-position IN h_t-pedido-credv2 ( 11.77 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedido-credv2 ( 12.12 , 141.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv95 ).
       RUN set-position IN h_p-updv95 ( 23.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv95 ( 1.42 , 34.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-pedido-credv2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_t-pedido-credv2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedido-credv2 ,
             BUTTON-REDONDEO:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv95 ,
             h_t-pedido-credv2 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-Despachar RECT-25 COMBO-NroSer Btn-Excel btn-margen BTN-CCTE 
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-pedido-credv2 <> ? THEN RUN dispatch IN h_t-pedido-credv2 ('open-query':U).
      END.
 ...
 WHEN "Disable-Head" THEN DO:
     ...
     RUN dispatch IN h_vpedidocreditomayv2 ('disable-fields').
   END.
 WHEN "Enable-Head" THEN DO:
     RUN dispatch IN h_p-updv04 ('enable':U).
     RUN dispatch IN h_vpedidocreditomayv2 ('enable-fields').
   END.
END CASE.
