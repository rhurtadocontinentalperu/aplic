...
FILL-IN-nomprovee FILL-IN-ruta RADIO-SET-tipocliente FILL-IN-tipoventa 
...
DEFINE VARIABLE RADIO-SET-calculo AS INTEGER INITIAL 1 
     ...
     RADIO-BUTTONS 
          "Todos", 1,
"Solo Articulos con Descuentos Promocionales", 2,
"Solo Articulos con descuentos x Volumen x Item", 3,
"Solo Articulos con descuentos x Volumen x Saldos", 4
...
DEFINE FRAME F-Main
     ...
     RADIO-SET-tipocliente AT ROW 8.69 COL 107 NO-LABEL WIDGET-ID 12
...
ON END-ERROR OF W-Win /* SIMULADOR DE PRECIOS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* SIMULADOR DE PRECIOS */
DO:
...
DO:
  ...
  IF radio-set-proveedor = 4 THEN DO:
      ...
      IF fill-in-desde > fill-in-hasta THEN DO:
          MESSAGE "Fechas estan incorrectas".
          RETURN NO-APPLY.
      END.
      */
  END.
...
FOR EACH factabla WHERE factabla.codcia = s-codcia AND
    ...
    FOR EACH almmmatg NO-LOCK WHERE almmmatg.codcia = s-codcia AND 
        ...
        almmmatg.subfam = y-subfamilia AND almmmatg.tpoart <> 'D',
    ...
    FOR EACH almmmatg NO-LOCK WHERE almmmatg.codcia = s-codcia AND 
        ...
        REPEAT x-sec = 1 TO 10:
            IF factabla.Valor[x-sec] > 0 THEN DO:
                ...
                /**/
                RUN _graba_detalle_2(INPUT x-sec, INPUT x-cantidad, INPUT y-dsctos, INPUT x-prevta).
...
------------------------------------------------------------------------------*/
  DISPLAY COMBO-BOX-listaprecios COMBO-BOX-familia COMBO-BOX-subfamilia 
          ...
          FILL-IN-ruta RADIO-SET-tipocliente FILL-IN-tipoventa 
...
/*x-codclie = IF(pTipoVenta <> "MOSTRADOR") THEN "00000000100" ELSE .*/
...
SESSION:SET-WAIT-STATE('GENERAL').
IF radio-set-calculo = 4 THEN DO:
    /* Dsctos x Volumen x Saldos - Acumulado */
