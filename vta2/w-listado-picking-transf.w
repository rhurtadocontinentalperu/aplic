...
&Scoped-Define ENABLED-OBJECTS BtnDone BUTTON-17 BUTTON-13 BUTTON-16 ~
txtDesde txtHasta ChkPendientes BUTTON-14 BUTTON-15 
...
DEFINE VARIABLE h_b-cimprime-transf AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-16 AT ROW 1.19 COL 107.72 WIDGET-ID 14
...
DO:
    ...
    RUN dispatch IN h_b-cimprime-transf ('open-query':U).
END.
...
DO:
  /*RUN dispatch IN h_b-cimprime-od ('imprime':U).*/
    RUN Imprimir-Formato IN h_b-cimprime-transf ( INPUT "ZONA").
END.
...
DO:
  /*RUN dispatch IN h_b-cimprime-od ('imprime':U).*/
  RUN Imprimir-Formato IN h_b-cimprime-transf ( INPUT "ALFABETICO").
END.
...
DO:
   RUN A-Distribucion IN h_b-cimprime-transf.
END.
...
DO:
  RUN dispatch IN h_b-cimprime-transf ('ue-excel':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-cimprime-transf.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = FchDoc':U ,
             OUTPUT h_b-cimprime-transf ).
       RUN set-position IN h_b-cimprime-transf ( 2.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cimprime-transf ( 7.50 , 123.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-cimprime-transf , 'Record':U , h_b-dimprime-transf ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cimprime-transf ,
             ChkPendientes:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-transf ,
             h_b-cimprime-transf , 'AFTER':U ).
    END. /* Page 0 */
...
OCXFile = SEARCH( "w-listado-picking-transf.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-listado-picking-transf.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BtnDone BUTTON-17 BUTTON-13 BUTTON-16 txtDesde txtHasta ChkPendientes 
         BUTTON-14 BUTTON-15 
      WITH FRAME F-Main IN WINDOW W-Win.
