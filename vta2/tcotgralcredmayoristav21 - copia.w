...
DEFINE FRAME F-Main
     ...
     "F8: Stocks por Almacén" VIEW-AS TEXT
          SIZE 17 BY .5 AT ROW 13.69 COL 53 WIDGET-ID 10
...
DO:
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN NO-APPLY.
    /*
    MESSAGE "s-TpoPed " s-TpoPed SKIP
    ...
    "f-FleteUnitario "  f-FleteUnitario SKIP 
...
DO:
  /*RUN vta2/d-captura-productos (s-TpoPed).*/
  ...
  /* Control de stock */
/*   DEF VAR mCodigos AS CHAR NO-UNDO.                                                            */
/*   mCodigos = "No hay stock disponible en el almacén " + ENTRY(1,s-CodAlm).                     */
...
/*           IF ITEM.CanPed > (Almmmate.StkAct - s-StkComprometido) THEN DO:                      */
/*               mCodigos = mCodigos + CHR(10) +                                                  */
/*                         "  CODIGO: " + ITEM.codmat + ' ' +                                     */
/*                         "  COTIZADO: " + STRING(ITEM.canped) + ' ' +                           */
/*                         "  STOCK DISPONIBLE : " + STRING(almmmate.StkAct - s-StkComprometido). */
/*           END.                                                                                 */
/*       END.                                                                                     */
/*   END.                                                                                         */
/*   IF NUM-ENTRIES(mCodigos,CHR(10)) > 1 THEN MESSAGE mCodigos.                                  */
...
/*MESSAGE 'inicio'.*/
  ...
  ASSIGN 
      ...
      ITEM.AftIgv = (IF s-FmaPgo = '900' THEN NO ELSE Almmmatg.AftIgv)  /* OJO: TRANSF. GRATUIT */
  ...
  IF f-FleteUnitario > 0 THEN DO:
      ...
      IF s-user-id = 'ADMIN' THEN DO:
          /*MESSAGE "DESPUES " f-FleteUnitario.*/
      END.
...
------------------------------------------------------------------------------*/
  ...
  END.
  /* EMPAQUE */
  /* RHC 30/12/16 Veamos si es EXPOLIBRERIA  (pCodDiv) */
  /* RHC 31/08/2017 Empaque Macchiu EXPO */
  ...
  IF s-FlgEmpaque = YES THEN DO:
      ...
      /*
      f-CanPed = DECIMAL(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&browse-name}) * f-Factor.
      f-Empaque = Almmmatg.StkMax.     /* Mínimo Venta Expolibreria */
      f-Sugerido = 0.
      EMPAQUE:
      DO:
      CASE s-TpoPed:
          ...
          OTHERWISE DO:
              /* RHC solo se va a trabajar con una lista general */
              IF f-Empaque > 0 THEN DO:
                  f-CanPed = TRUNCATE((f-CanPed / f-Empaque),0) * f-Empaque.
                  IF f-CanPed <> DECIMAL(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&browse-name}) * f-Factor THEN DO:
                      MESSAGE 'Solo puede vender en empaques de' f-Empaque Almmmatg.UndBas
                          VIEW-AS ALERT-BOX ERROR.
                      APPLY 'ENTRY':U TO ITEM.CanPed.
                      RETURN "ADM-ERROR".
                  END.
  ...
  IF s-FlgMinVenta = YES THEN DO:
      CASE s-CodDiv:
              ...
              IF AVAILABLE Almmmatg AND Almmmatg.PesoBruto > 0 THEN DO:
                  ...
                  IF f-Minimo > 0 THEN DO:
                      ...
                      IF f-CanPed > f-Minimo AND Almmmatg.Paquete > 0 THEN DO:
                          IF (f-CanPed - f-Minimo) MODULO Almmmatg.Paquete > 0
                              THEN DO:
                              MESSAGE "ERROR el el artículo "  Almmmatg.codmat SKIP
          ...
          OTHERWISE DO:
              f-CanPed = DECIMAL(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&browse-name}) * f-Factor.
              f-Minimo = Almmmatg.DEC__03.  /* LISTA GENERAL */
              IF f-Minimo > 0 THEN DO:
                  IF f-CanPed < f-Minimo THEN DO:
...
/*                                    OUTPUT s-StkComprometido).                            */
  ...
  IF S-FMAPGO = "900" THEN RETURN "OK".
  /* ******************************************************************************* */
