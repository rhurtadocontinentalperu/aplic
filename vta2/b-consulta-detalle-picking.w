...
&Scoped-define QUERY-STRING-br_table FOR EACH AlmDDocu WHERE {&Condicion} NO-LOCK, ~
      EACH Almmmatg WHERE Almmmatg.CodCia = AlmDDocu.CodCia ~
  AND Almmmatg.codmat = AlmDDocu.Codigo NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH AlmDDocu WHERE {&Condicion} NO-LOCK, ~
      EACH Almmmatg WHERE Almmmatg.CodCia = AlmDDocu.CodCia ~
  AND Almmmatg.codmat = AlmDDocu.Codigo NO-LOCK ~
    ~{&SORTBY-PHRASE}.
