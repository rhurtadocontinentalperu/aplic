...
DEFINE VARIABLE h_t-dtopedmin AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Descuentos de VENTAS al Credito"
         HEIGHT             = 18.81
         WIDTH              = 150.86
         MAX-HEIGHT         = 20.92
         MAX-WIDTH          = 160.43
...
ON END-ERROR OF W-Win /* Descuentos de VENTAS al Credito */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Descuentos de VENTAS al Credito */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/q-dtopedmin.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-dtocot ).
       RUN set-position IN h_q-dtocot ( 1.00 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-dtocot. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_v-dtocot ).
       RUN add-link IN adm-broker-hdl ( h_q-dtocot , 'Record':U , h_v-dtocot ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_q-dtocot , 'Record':U , h_b-cotiza ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-dtopedmin.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-dtopedmin ).
       RUN set-position IN h_t-dtopedmin ( 6.12 , 1.43 ) NO-ERROR.
       RUN set-size IN h_t-dtopedmin ( 11.50 , 149.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-dtopedmin. */
       RUN add-link IN adm-broker-hdl ( h_p-updv96 , 'TableIO':U , h_t-dtopedmin ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-dtopedmin ,
             h_v-dtocot , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv96 ,
             h_t-dtopedmin , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          RUN dispatch IN h_t-dtopedmin ('open-query':U).
      END.
    WHEN "Recalcular-Precios" THEN DO:
        RUN dispatch IN h_t-dtopedmin ('recalcular-precios':U).
    END.
