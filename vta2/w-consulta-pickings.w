...
DEFINE VARIABLE h_b-consulta-detalle-picking AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-consulta-detalle-prepicking AS HANDLE NO-UNDO.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 38.72 BY 5.62 FIT-LAST-COLUMN.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     BROWSE-5 AT ROW 7.46 COL 94 WIDGET-ID 200
     BtnDone AT ROW 1.38 COL 111 WIDGET-ID 10
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 134 BY 27.54 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CONSULTA DE PICKINGS Y PRE-PICKINGS"
         HEIGHT             = 27.54
         WIDTH              = 134
         MAX-HEIGHT         = 27.54
         MAX-WIDTH          = 159.43
...
"tt-w-report.Campo-C[2]" "Nro. Docto" "X(15)" "character" ? ? ? ? ? ? no ? no no "11.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-w-report.Campo-C[3]
"tt-w-report.Campo-C[3]" "H.Ruta" "X(15)" "character" ? ? ? ? ? ? no ? no no "17.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-5 */
&ANALYZE-RESUME
...
ON END-ERROR OF W-Win /* CONSULTA DE PICKINGS Y PRE-PICKINGS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* CONSULTA DE PICKINGS Y PRE-PICKINGS */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Picking|Pre-Picking' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 14.31 , 2.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 14.04 , 116.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_b-consulta-pickings , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/b-consulta-detalle-picking.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-consulta-detalle-picking ).
       RUN set-position IN h_b-consulta-detalle-picking ( 15.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-consulta-detalle-picking ( 11.73 , 107.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-consulta-detalle-picking. */
       RUN add-link IN adm-broker-hdl ( h_b-consulta-pickings , 'Record':U , h_b-consulta-detalle-picking ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-consulta-detalle-picking ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/b-consulta-detalle-prepicking.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-consulta-detalle-prepicking ).
       RUN set-position IN h_b-consulta-detalle-prepicking ( 16.00 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-consulta-detalle-prepicking ( 11.73 , 107.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-consulta-detalle-prepicking. */
       RUN add-link IN adm-broker-hdl ( h_b-consulta-pickings , 'Record':U , h_b-consulta-detalle-prepicking ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-consulta-detalle-prepicking ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
