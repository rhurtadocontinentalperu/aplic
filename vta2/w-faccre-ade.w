...
&Scoped-Define ENABLED-OBJECTS RECT-21 C-CodDoc C-NroSer 
...
DEFINE VARIABLE h_v-faccre-ade AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     C-NroSer AT ROW 1.27 COL 84.71
     "Buscar el número:" VIEW-AS TEXT
          SIZE 13 BY .5 AT ROW 1.54 COL 99 WIDGET-ID 6
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 2.35 , 120.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 5.12 , 9.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-faccre-ade.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-faccre-ade ).
       RUN set-position IN h_v-faccre-ade ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.08 , 115.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-docmto ).
       RUN set-position IN h_q-docmto ( 1.27 , 112.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-faccre-ade. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_v-faccre-ade ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-faccre-ade ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-faccre-ade ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-docmto ,
             h_v-faccre-ade , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-21 C-CodDoc C-NroSer 
      WITH FRAME F-Main IN WINDOW W-Win.
