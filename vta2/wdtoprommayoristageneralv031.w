...
DEFINE FRAME F-Main
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 141.14 BY 21.19 WIDGET-ID 100.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_t-lima-dtopromv2 ( 2.92 , 88.00 ) NO-ERROR.
       RUN set-size IN h_t-lima-dtopromv2 ( 12.88 , 45.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-lima-dtopromv2. */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-lima-dtopromv2 ,
             h_butilexdctoprom-02-2 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
    ...
    WHEN 4 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-utilex-dtopromv31.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
...
REPEAT WHILE RADIO-SET-1 <> 5:
    ...
    IF RADIO-SET-1 > 2 THEN IF NUM-ENTRIES(f-Division) = 10 THEN LEAVE.     /* NO mas de 10 (por ahora) */
END.
IF RADIO-SET-1 = 3 AND f-Division <> COMBO-BOX-Division THEN DO:
    pMensaje = 'No puede actualizar la división ' + f-Division + CHR(10) +
        'La división que le corresponde es la ' + COMBO-BOX-Division.
    RETURN.
END.
...
FOR EACH T-MATG:
    ...
    END CASE.
    CASE RADIO-SET-1:
        WHEN 4  THEN DO:    /* Los precios están siempre en soles */
