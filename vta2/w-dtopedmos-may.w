...
DEFINE VARIABLE h_q-dtopedmos-may AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-dtocot AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-dtocot AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "DESCUENTO PEDIDO MOSTRADOR MAYORISTA"
         HEIGHT             = 18.81
         WIDTH              = 150.86
         MAX-HEIGHT         = 20.92
         MAX-WIDTH          = 160.43
...
ON END-ERROR OF W-Win /* DESCUENTO PEDIDO MOSTRADOR MAYORISTA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* DESCUENTO PEDIDO MOSTRADOR MAYORISTA */
DO:
...
DO:
  RUN dispatch IN h_v-dtocot ('imprime':U).
END.
...
DO:
    ...
    RUN dispatch IN h_q-dtopedmos-may ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-dtocot.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-dtocot ).
       RUN set-position IN h_v-dtocot ( 2.35 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.35 , 100.86 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/q-dtopedmos-may.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-dtopedmos-may ).
       RUN set-position IN h_q-dtopedmos-may ( 1.00 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-dtocot. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_v-dtocot ).
       RUN add-link IN adm-broker-hdl ( h_q-dtopedmos-may , 'Record':U , h_v-dtocot ).

       /* Links to SmartQuery h_q-dtopedmos-may. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-dtopedmos-may ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv97 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-dtocot ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-dtopedmos-may ,
             h_v-dtocot , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN add-link IN adm-broker-hdl ( h_q-dtopedmos-may , 'Record':U , h_b-cotiza ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-dtocot.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-dtocot ).
       RUN set-position IN h_t-dtocot ( 6.12 , 1.43 ) NO-ERROR.
       RUN set-size IN h_t-dtocot ( 11.50 , 149.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-dtocot. */
       RUN add-link IN adm-broker-hdl ( h_p-updv96 , 'TableIO':U , h_t-dtocot ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-dtocot ,
             h_v-dtocot , 'AFTER':U ).
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          RUN dispatch IN h_t-dtocot ('open-query':U).
      END.
    WHEN "Recalcular-Precios" THEN DO:
        RUN dispatch IN h_t-dtocot ('recalcular-precios':U).
    END.
