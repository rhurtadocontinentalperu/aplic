...
FILL-IN-items COMBO-NroSer FILL-IN-NroDoc 
...
DEFINE FRAME F-Main
     ...
     COMBO-NroSer AT ROW 3.96 COL 3.43 WIDGET-ID 2
     FILL-IN-NroDoc AT ROW 3.96 COL 17 COLON-ALIGNED NO-LABEL WIDGET-ID 26
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-4 FILL-IN-NroDoc F-Main */
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAMETER pCanalModerno AS LOG.
    ...
    FOR EACH Detalle , FIRST Almmmatg OF Detalle NO-LOCK BREAK BY Detalle.CodCia:
        ...
        IF lCreaHeader THEN DO:
            ...
            ASSIGN
                ...
                Ccbcdocu.NroRef = "".
            FOR EACH Cabecera-FAI:
                Ccbcdocu.NroRef = Ccbcdocu.NroRef + (IF Ccbcdocu.NroRef = '' THEN '' ELSE '|') + Cabecera-FAI.NroDoc. 
            END.
            ASSIGN
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    FOR EACH T-CDOCU NO-LOCK, FIRST Ccbcdocu OF T-CDOCU,
        ...
        BREAK BY COTIZACION.ordcmp:     /* Quebrado por Orden de Compra */
        /* REGISTRO DE CONTROL */
        CREATE Cabecera-FAI.
        BUFFER-COPY Ccbcdocu TO Cabecera-FAI.
        /* Marcamos los FAI como cancelados */
        ...
        FOR EACH Ccbddocu OF Ccbcdocu NO-LOCK, FIRST Almmmatg OF Ccbddocu NO-LOCK:
            ...
            ASSIGN
                ...
                Detalle.CodMon = Ccbcdocu.CodMon
                Detalle.TpoCmb = Ccbcdocu.TpoCmb.
            /* RHC 24/01/2020 Se considera que el precio es invariable */
            ASSIGN
                Detalle.ImpIsc = Detalle.ImpIsc + Ccbddocu.ImpIsc
                Detalle.ImpIgv = Detalle.ImpIgv + Ccbddocu.ImpIgv
                Detalle.PreUni = Ccbddocu.PreUni
                Detalle.PreBas = Ccbddocu.PreBas.
            /* Recalculamos */
/*             ASSIGN                                                                                                */
/*                 Detalle.PreUni = Detalle.ImpLin / Detalle.CanDes                                                  */
/*                 Detalle.PreBas = Detalle.ImpLin / Detalle.CanDes.                                                 */
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-Division COMBO-BOX-Clientes FILL-IN-items COMBO-NroSer 
          FILL-IN-NroDoc 
...
IF x-CanalModerno = YES THEN DO:
    ...
    REPEAT TRANSACTION ON ERROR UNDO trloop, RETURN 'ADM-ERROR' ON STOP UNDO trloop, RETURN 'ADM-ERROR':
        ...
        RUN Carga-Comprobantes (INPUT x-CanalModerno).
        IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO trloop, RETURN 'ADM-ERROR'.
