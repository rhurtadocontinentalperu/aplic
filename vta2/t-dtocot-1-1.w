...
PEDI.Por_Dsctos[3] PEDI.Libre_d01 PEDI.ImpLin 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      PEDI.Libre_d01 COLUMN-LABEL "Margen" FORMAT "->>>,>>9.99":U
...
"PEDI.Por_Dsctos[3]" "% Dscto!Vol/Prom" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > Temp-Tables.PEDI.Libre_d01
"PEDI.Libre_d01" "Margen" "->>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[14]   > Temp-Tables.PEDI.ImpLin
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE gn-clieds THEN DO:
      ...
      IF PEDI.Por_Dsctos[1] > f-DtoMax THEN DO:
  ...
  ELSE DO:
      ...
      END CASE.
      IF PEDI.Por_Dsctos[1] > f-DtoMax THEN DO:
          MESSAGE "Máximo Descuento permitido para el nivel de Usuario" SKIP
              TRIM(s-nivel) " : " almtabla.Nombre " es : " f-DtoMax " %" SKIP
              VIEW-AS ALERT-BOX ERROR.
          UNDO, RETURN 'ADM-ERROR'.
      END.
  END.
  /* FIN DESCUENTO MAXIMO */
