...
DEFINE VARIABLE h_tpedcontadomarketv2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vpedidocontadomayv2 AS HANDLE NO-UNDO.
...
DO:
  RUN Genera-Orden-Despacho IN h_vpedidocontadomayv2.
END.
...
DO:
  RUN Verifica-Pedido IN h_vpedidocontadomayv2.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/vpedcontadomarketv2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vpedidocontadomayv2 ).
       RUN set-position IN h_vpedidocontadomayv2 ( 2.62 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_vpedidocontadomayv2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_vpedidocontadomayv2 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_vpedidocontadomayv2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_vpedidocontadomayv2 ,
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/tpedcontadomarketv2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tpedcontadomarketv2 ).
       RUN set-position IN h_tpedcontadomarketv2 ( 13.50 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tpedcontadomarketv2 ( 12.27 , 140.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_tpedcontadomarketv2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tpedcontadomarketv2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tpedcontadomarketv2 ,
             BUTTON-Verifica-Orden:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tpedcontadomarketv2 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         RUN dispatch IN h_tpedcontadomarketv2 ('open-query':U).
      END.
    ...
    WHEN "Add-Record" THEN DO:
        RUN dispatch IN h_tpedcontadomarketv2 ('add-record':U).
      END.
    WHEN "browse" THEN DO:
          IF h_bpedcontadomarketv2 <> ? THEN RUN dispatch IN h_bpedcontadomarketv2 ('open-query':U).
          IF h_tpedcontadomarketv2 <> ? THEN RUN dispatch IN h_tpedcontadomarketv2 ('open-query':U). 
      END.
    WHEN "browse2" THEN DO:
          RUN dispatch IN h_tpedcontadomarketv2 ('open-query':U). 
      END.
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN Recalcular-Precios IN h_vpedidocontadomayv2.
         IF h_tpedcontadomarketv2 <> ? THEN RUN dispatch IN h_tpedcontadomarketv2 ('open-query':U). 
      END.
END CASE.
