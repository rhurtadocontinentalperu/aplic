...
DEFINE VARIABLE h_vprepedidotiendasutilex AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* PRE-PEDIDOS EN VITRINA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* PRE-PEDIDOS EN VITRINA */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/vprepedidotiendasutilex.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vprepedidotiendasutilex ).
       RUN set-position IN h_vprepedidotiendasutilex ( 2.73 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.35 , 123.00 ) */
       ...
       /* Links to SmartViewer h_vprepedidotiendasutilex. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_vprepedidotiendasutilex ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_vprepedidotiendasutilex ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vprepedidotiendasutilex ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido-4 ,
             h_vprepedidotiendasutilex , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_b-pedmos-utilex ( 5.62 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_tpedidotiendasutilex ( 5.62 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 19.85 , 2.00 ) NO-ERROR.
...
CASE L-Handle:
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_vprepedidotiendasutilex <> ? THEN RUN Recalcular-Precios IN h_vprepedidotiendasutilex.
      END.
