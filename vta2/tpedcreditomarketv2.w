...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      PEDI.codmat COLUMN-LABEL "Articulo" FORMAT "X(6)":U WIDTH 6
      Almmmatg.DesMat FORMAT "X(60)":U WIDTH 51.43
      Almmmatg.DesMar COLUMN-LABEL "Marca" FORMAT "X(20)":U WIDTH 10.72
...
"PEDI.codmat" "Articulo" ? "character" ? ? ? ? ? ? no ? no no "6" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.Almmmatg.DesMat
"Almmmatg.DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ? no no "51.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > integral.Almmmatg.DesMar
"Almmmatg.DesMar" "Marca" "X(20)" "character" ? ? ? ? ? ? no ? no no "10.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.PEDI.AlmDes
"PEDI.AlmDes" "Alm!Desp" ? "character" 14 0 ? ? ? ? no ? no no "4.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"PEDI.Por_Dsctos[1]" "% Dscto.!Manual" ? "decimal" ? ? ? ? ? ? no ? no no "6.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > Temp-Tables.PEDI.Por_Dsctos[2]
"PEDI.Por_Dsctos[2]" "% Dscto!Evento" ? "decimal" ? ? ? ? ? ? no ? no no "5.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  IF PEDI.AlmDes = "65s" AND Almmmatg.StkRep > 0 THEN DO:
      f-CanPed = DECIMAL(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&browse-name}) * f-Factor.
      f-CanPed = (TRUNCATE((f-CanPed / Almmmatg.StkRep),0) * Almmmatg.StkRep).
      IF f-CanPed <> DECIMAL(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&browse-name}) * f-Factor THEN DO:
          MESSAGE 'Solo puede vender en empaques de' Almmmatg.StkRep Almmmatg.UndBas
              VIEW-AS ALERT-BOX ERROR.
          APPLY 'ENTRY':U TO PEDI.CanPed.
          RETURN "ADM-ERROR".
      END.
...
/*   RUN vta2/Stock-Comprometido (PEDI.CodMat,                */
/*                                 PEDI.AlmDes,               */
/*                                 OUTPUT s-StkComprometido). */
  RUN vta2/Stock-Comprometido-v2 (PEDI.CodMat, 
                                PEDI.AlmDes, 
                                OUTPUT s-StkComprometido).
  IF s-adm-new-record = 'NO' THEN DO:
      FIND Facdpedi WHERE Facdpedi.codcia = s-codcia
          ...
          AND Facdpedi.almdes = PEDI.AlmDes
          NO-LOCK NO-ERROR.
