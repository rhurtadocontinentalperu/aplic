...
&Scoped-define INTERNAL-TABLES FacCPedi w-report-tmp
...
FacCPedi.NomCli FacCPedi.ImpTot FacCPedi.FmaPgo FacCPedi.Ubigeo[1] 
...
txtOrdenCompra BUTTON-11 BROWSE-4 Btn_Cancel COMBO-NroSer RECT-54 RECT-55 ~
RECT-56 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-4 D-Dialog _STRUCTURED
  QUERY BROWSE-4 NO-LOCK DISPLAY
      ...
      FacCPedi.NroPed COLUMN-LABEL "Numero" FORMAT "X(12)":U WIDTH 8
...
"FacCPedi.FchPed" "Fecha de!Emisión" ? "date" ? ? ? ? ? ? no ? no no "9.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacCPedi.fchven
"FacCPedi.fchven" "Fecha de!Vencimiento" "99/99/9999" "date" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.FacCPedi.FchEnt
"FacCPedi.FchEnt" "Fecha de!Entrega" ? "date" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.NomCli" ? ? "character" ? ? ? ? ? ? no ? no no "31.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.FmaPgo" "Condicion!de Venta" ? "character" ? ? ? ? ? ? no ? no no "7.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > INTEGRAL.FacCPedi.Ubigeo[1]
"FacCPedi.Ubigeo[1]" "Tienda" "x(45)" "character" ? ? ? ? ? ? no ? no no "13.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-4 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* GENERAR PEDIDOS PZVEA */
DO:  
...
DO:
    ...
    DEFINE VAR x-paso AS LOG INIT NO.
...
*/
    ...
    /* 
        Ic - 30Oct2017,  Segun Lucy Mesia la cotizaciones de Canal moderno
                        no pasan por su proceso.
    */
        
        MESSAGE 'Seguro de GENERAR el pedido?' VIEW-AS ALERT-BOX QUESTION
                BUTTONS YES-NO UPDATE rpta AS LOG.
        IF rpta = NO THEN RETURN NO-APPLY.
    /*
    s-CodAlm = ENTRY(1, COMBO-BOX_CodAlm_010, " - ") + ',' +
                ENTRY(1, COMBO-BOX_CodAlm_011, " - ") + ',' +
                ENTRY(1, COMBO-BOX_CodAlm_017, " - ") + ',' +
                ENTRY(1, COMBO-BOX_CodAlm_Otras, " - ").
     */
...
------------------------------------------------------------------------------*/
  ...
  DO ON STOP UNDO GetLock, RETRY GetLock:
      IF RETRY THEN DO:
          ...
          IF iLocalCounter = 5 THEN LEAVE GetLock.
      END.
      FIND FIRST COTIZACION OF faccpedi EXCLUSIVE-LOCK NO-ERROR.
  END. 
  IF iLocalCounter = 5 OR NOT AVAILABLE FacCorre THEN DO:
      /*pMensaje = "NO se pudo bloquear la Cotización Referenciada".*/
      CREATE w-report-tmp.
      ASSIGN  w-report-tmp.campo-c[1] = faccpedi.nroped
  ...
  CREATE B-CPEDI.
        ASSIGN B-CPEDI.codcia   = s-codcia
                B-CPEDI.coddoc = 'PED'
                ...
                B-CPEDI.Hora   = STRING(TIME,"HH:MM:SS")
                B-CPEDI.coddiv = S-CODDIV
                B-CPEDI.PorIgv = s-PorIgv
                B-CPEDI.FlgEst = "G"
                B-CPEDI.TpoPed = "CR"
                B-CPEDI.tpocmb = Faccpedi.TpoCmb
                B-CPEDI.fchent = IF (faccpedi.FchEnt >= TODAY) THEN faccpedi.FchEnt ELSE TODAY
                B-CPEDI.fchven = TODAY + s-DiasVtoPed
                ...
                B-CPEDI.CodRef = 'COT'
                B-CPEDI.NroRef = FacCPedi.Nroped /* Nro Cotizacion */
                B-CPEDI.OrdCmp = FacCPedi.OrdCmp
                B-CPEDI.FaxCli = FacCPedi.FaxCli
                B-CPEDI.CodPos = FacCPedi.CodPos
                B-CPEDI.Libre_c01 = FacCPedi.Libre_c01
                B-CPEDI.codalm = s-codalm
                B-CPEDI.tipvta = ""     /* Cliente Recoge : Vacio = NO */
                B-CPEDI.cmpbnte = faccpedi.cmpbnte
                B-CPEDI.codmon = faccpedi.codmon
                B-CPEDI.flgigv = faccpedi.flgigv
                B-CPEDI.usuario = s-user-id NO-ERROR
                .
  ...
  /* Division destino */
  /* ********************************************************************************************** */
  FIND Almacen OF B-CPEDI NO-LOCK NO-ERROR.
  IF AVAILABLE Almacen THEN B-CPEDI.DivDes = Almacen.CodDiv.
  /* ********************************************************************************************** */
  /* ********************************************************************************************** */
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH PEDI WHERE PEDI.AlmDes = B-CPEDI.CodAlm, FIRST Almmmatg OF PEDI NO-LOCK BY PEDI.NroItm: 
      /* FILTROS */
      FIND Almmmate WHERE Almmmate.codcia = s-codcia
  ...
  END.
  /* RECALCULAMOS */
  FOR EACH PEDI WHERE PEDI.AlmDes = B-CPEDI.CodAlm:
      ASSIGN
  ...
  FIND FIRST Facdpedi OF B-CPEDI NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BROWSE-7 BUTTON-12 FILL-IN-CodCli txtOrdenCompra BUTTON-11 BROWSE-4 
         Btn_Cancel COMBO-NroSer RECT-54 RECT-55 RECT-56 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR' :
    ...
    RUN vta2/pactualizacotizacion ( ROWID(B-CPEDI), "C", OUTPUT pMensaje).
...
/*
    RUN Fecha-Entrega ( ",",
                      ...
                      B-CPEDI.CodAlm,
                      (IF B-CPEDI.TipVta = "Si" THEN "CR" ELSE B-CPEDI.CodPos),
                      0,
                      0,
                      B-CPEDI.CodCli,
                      B-CPEDI.CodDiv,                          
