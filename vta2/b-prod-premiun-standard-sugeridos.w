...
DEFINE VAR x-tabla AS CHAR INIT "PROD.SUGERIDOS".

DEFINE BUFFER x-vtatabla FOR vtatabla.

DEFINE SHARED VAR S-CodigoMatriz AS CHAR.
...
&Scoped-define INTERNAL-TABLES VtaTabla tAlmmmatg1

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table VtaTabla.Llave_c2 VtaTabla.Valor[1] ~
VtaTabla.Libre_c02 tAlmmmatg1.DesMat tAlmmmatg1.DesMar VtaTabla.Libre_c01 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table VtaTabla.Llave_c2 ~
VtaTabla.Valor[1] VtaTabla.Libre_c02 VtaTabla.Libre_c01 
...
vtatabla.llave_c1 = s-CodigoMatriz ~
 NO-LOCK, ~
      FIRST tAlmmmatg1 WHERE tAlmmmatg1.CodCia = s-codcia and ~
tAlmmmatg1.codmat = VtaTabla.llave_c2 OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
tAlmmmatg1.codmat = VtaTabla.llave_c2 OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table VtaTabla tAlmmmatg1
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c2 COLUMN-LABEL "Cod.Art!Sugerido" FORMAT "x(8)":U
      VtaTabla.Valor[1] COLUMN-LABEL "Orden de!Prioridad" FORMAT ">>>,>>9.99":U
      VtaTabla.Libre_c02 COLUMN-LABEL "Sugerido!Primordial" FORMAT "x(8)":U
            VIEW-AS COMBO-BOX INNER-LINES 5
                      LIST-ITEMS "SI","NO" 
                      DROP-DOWN-LIST 
      tAlmmmatg1.DesMat FORMAT "X(45)":U WIDTH 25.29
      tAlmmmatg1.DesMar COLUMN-LABEL "Marca" FORMAT "X(30)":U WIDTH 21.43
      VtaTabla.Libre_c01 COLUMN-LABEL "Estado" FORMAT "x(8)":U
            WIDTH 13.43 VIEW-AS COMBO-BOX INNER-LINES 5
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 89.43 BY 15.08
         FONT 4.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaTabla,Temp-Tables.tAlmmmatg1 WHERE INTEGRAL.VtaTabla ..."
...
tAlmmmatg1.codmat = VtaTabla.llave_c2"
     _FldNameList[1]   > INTEGRAL.VtaTabla.Llave_c2
"VtaTabla.Llave_c2" "Cod.Art!Sugerido" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.VtaTabla.Valor[1]
"VtaTabla.Valor[1]" "Orden de!Prioridad" ">>>,>>9.99" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaTabla.Libre_c02
"VtaTabla.Libre_c02" "Sugerido!Primordial" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," "SI,NO" ? 5 no 0 no no
     _FldNameList[4]   > Temp-Tables.tAlmmmatg1.DesMat
"tAlmmmatg1.DesMat" ? ? "character" ? ? ? ? ? ? no ? no no "25.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.tAlmmmatg1.DesMar
"tAlmmmatg1.DesMar" "Marca" ? "character" ? ? ? ? ? ? no ? no no "21.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.VtaTabla.Libre_c01
"VtaTabla.Libre_c01" "Estado" ? "character" ? ? ? ? ? ? yes ? no no "13.43" yes no no "U" "" "" "DROP-DOWN-LIST" "," "ACTIVO,INACTIVO" ? 5 no 0 no no
...
DO:
    ...
    addRow = 0.  /* Ei en browse esta activo la barra de titulo poner */
...
ON 'RETURN':U OF vtatabla.llave_c2, vtatabla.libre_c01, vtatabla.libre_c02, vtatabla.valor[1] IN BROWSE {&BROWSE-NAME} DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR recomendado AS CHAR.
  ...
  cod1 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.llave_c2:SCREEN-VALUE IN BROWSE {&browse-name}.
  estado = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.libre_c01:SCREEN-VALUE IN BROWSE {&browse-name}.
  recomendado = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.libre_c02:SCREEN-VALUE IN BROWSE {&browse-name}.

  IF TRUE <> (cod1 > "") THEN DO:
      MESSAGE "Debe ingresar el codigo del articulo sugerido" VIEW-AS ALERT-BOX INFORMATION.
  ...
  END.
  IF TRUE <> (recomendado > "") THEN DO:
      MESSAGE "Debe seleccionar si es sugerido primordial" VIEW-AS ALERT-BOX INFORMATION.
  ...
  IF x-es-nuevo THEN DO:
      FIND FIRST x-vtatabla WHERE x-vtatabla.codcia = s-codcia AND
                                    ...
                                    x-vtatabla.llave_c1 = S-CodigoMatriz AND
                                    x-vtatabla.llave_c2 = cod1 NO-LOCK NO-ERROR.
      IF AVAILABLE x-vtatabla THEN DO:
          MESSAGE "El codigo " cod1 " ya esta registrado" VIEW-AS ALERT-BOX INFORMATION.
          APPLY 'ENTRY':U TO vtatabla.llave_c2 IN BROWSE {&BROWSE-NAME}.
          RETURN "ADM-ERROR".
      END.
  END.
