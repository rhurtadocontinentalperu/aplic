...
&Scoped-Define ENABLED-OBJECTS RECT-24 COMBO-NroSer BTN-Excel btnExcVend ~
BUTTON-EDI 
...
DEFINE FRAME F-Main
     ...
     B-CCTE AT ROW 5.23 COL 119 WIDGET-ID 12
     BUTTON-COT AT ROW 6.31 COL 119 WIDGET-ID 14
     B-CLI AT ROW 7.38 COL 119 WIDGET-ID 22
     BUTTON-IMP-PROV AT ROW 8.5 COL 119 WIDGET-ID 32
     BUTTON-IMP-OPENORANGE AT ROW 9.65 COL 119 WIDGET-ID 34
     BUTTON-IMP-HIST-COT AT ROW 10.81 COL 119 WIDGET-ID 36
     Btn-Grupos-2 AT ROW 25.42 COL 7.57 WIDGET-ID 30
     Btn-Grupos AT ROW 25.42 COL 28 WIDGET-ID 10
     BUTTON-SUPERMERCADOS AT ROW 25.42 COL 48.72 WIDGET-ID 18
     BUTTON-SUPERMERCADOS-B2B AT ROW 25.42 COL 77.43 WIDGET-ID 38
     BUTTON-CISSAC AT ROW 25.42 COL 101.72 WIDGET-ID 20
...
DO:
   RUN Actualiza-datos-cliente IN h_vcotizacioncreditomay.
END.
...
/*     END CASE.                                                                    */
    CASE TRUE :
        WHEN INDEX(gn-divi.desdiv, "UTILEX") > 0
            THEN RUN Excel_Utilex IN h_vcotizacioncreditomay (INPUT lchoice) .
        OTHERWISE RUN Genera-Excel2 IN h_vcotizacioncreditomay (INPUT lchoice) .
    END CASE.
END.
...
/*     END CASE.                                                                    */
    IF lChoice = TRUE THEN DO:
        RUN Genera-Excel-vendedores IN h_vcotizacioncreditomay (INPUT lchoice) .
    END.
END.
...
DO:
  RUN Importar-entre-companias IN h_vcotizacioncreditomay.
...
DO:
  RUN Edi-Comparativo IN h_vcotizacioncreditomay.
END.
...
DO:
   RUN Captura-Historico-Cotizaciones IN h_vcotizacioncreditomay.
END.
...
DO:
   RUN Importar-Excel-OpenOrange IN h_vcotizacioncreditomay.
END.
...
DO:
   RUN Importar-Excel-Provincias IN h_vcotizacioncreditomay.
END.
...
DO:
    RUN Importar-Supermercados IN h_vcotizacioncreditomay.
    IF RETURN-VALUE = "ADM-ERROR" THEN RETURN NO-APPLY.
    RUN dispatch IN h_tcotizacioncreditomay ('open-query':U).
END.
...
DO:
    RUN Importar-Supermercados-B2B IN h_vcotizacioncreditomay.
    IF RETURN-VALUE = "ADM-ERROR" THEN RETURN NO-APPLY.
    RUN dispatch IN h_tcotizacioncreditomay ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/vcotizacioncreditomay.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vcotizacioncreditomay ).
       RUN set-position IN h_vcotizacioncreditomay ( 2.62 , 3.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_vcotizacioncreditomay ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_vcotizacioncreditomay ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico-2 ,
             h_p-updv01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vcotizacioncreditomay ,
             btnExcVend:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             BUTTON-CISSAC:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN set-position IN h_b-cotizacion-cred ( 12.15 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/tcotizacioncreditomay.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tcotizacioncreditomay ).
       RUN set-position IN h_tcotizacioncreditomay ( 12.15 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 23.88 , 2.00 ) NO-ERROR.
    ...
    WHEN 3 THEN DO:
       ...
       RUN set-position IN h_tcotizacioncreditomay-a ( 12.15 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12-2 ).
       RUN set-position IN h_p-updv12-2 ( 23.88 , 2.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-24 COMBO-NroSer BTN-Excel btnExcVend BUTTON-EDI 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    ...
    WHEN "Disable-Head" THEN DO:
        ...
        RUN dispatch IN h_vcotizacioncreditomay ('disable-fields':U).
    END.
    WHEN "Enable-Head" THEN DO:
        ...
        RUN dispatch IN h_vcotizacioncreditomay ('enable-fields':U).
    END.
    ...
    WHEN "Recalculo" THEN RUN Recalcular-Precios IN h_vcotizacioncreditomay.
    WHEN 'IBC' THEN RUN IBC-Diferencias IN h_tcotizacioncreditomay.
