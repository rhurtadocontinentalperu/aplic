...
&Scoped-Define ENABLED-OBJECTS x-CodDoc x-NroPed br_table BUTTON-13 ~
BUTTON-15 
&Scoped-Define DISPLAYED-OBJECTS x-CodDoc x-NroPed 
...
DEFINE FRAME F-Main
     x-CodDoc AT ROW 1.27 COL 16 COLON-ALIGNED WIDGET-ID 10
     x-NroPed AT ROW 1.27 COL 23 COLON-ALIGNED NO-LABEL WIDGET-ID 2
...
"PEDI.CanPick" ? ? "decimal" 12 15 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  /* NO se va a cerrar la orden miestras existan diferencias */
  IF lDif THEN DO:
      MESSAGE ' La Orden de despacho presenta diferencias ' SKIP
              '      Se imprimirán las diferencias        ' SKIP
              '           ¿Desea Continuar?               '
...
ON LEAVE OF x-NroPed IN FRAME F-Main
...
/*       RETURN NO-APPLY.                                                        */
/*   END.                                                                        */
  IF Faccpedi.CodAlm <> s-CodAlm THEN DO:
      MESSAGE "NO tiene almacenes de despacho pertenecientes a esta división"
          VIEW-AS ALERT-BOX WARNING.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Facdpedi NO-LOCK WHERE Facdpedi.codcia = s-codcia
      ...
      FIND FIRST PEDI WHERE PEDI.codmat = Facdpedi.codmat NO-ERROR.
      IF NOT AVAILABLE PEDI THEN DO:
          CREATE PEDI.
          BUFFER-COPY Facdpedi 
              EXCEPT Facdpedi.CanPed Facdpedi.CanPick
              TO PEDI.
      END.
      ASSIGN
          PEDI.CanPick = PEDI.CanPick + Facdpedi.CanPick
          PEDI.CanPed = PEDI.CanPed + Facdpedi.CanPed.
  END.
  RUN dispatch IN THIS-PROCEDURE ('open-query').
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      IF NOT AVAILABLE B-CPedi THEN UNDO, RETURN 'ADM-ERROR'.
      FIND B-CCOTI WHERE B-CCOTI.codcia = B-CPEDI.codcia
          AND B-CCOTI.coddoc = B-CPEDI.codref
          AND B-CCOTI.nroped = B-CPEDI.nroref
          NO-LOCK NO-ERROR.
