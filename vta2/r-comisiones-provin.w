...
&Scoped-Define ENABLED-OBJECTS x-CodDiv BUTTON-4 Btn_Done x-FchDoc-1 ~
x-FchDoc-2 f-vendedor BUTTON-1 
...
DEFINE FRAME F-Main
     ...
     BUTTON-4 AT ROW 1.27 COL 65 WIDGET-ID 10
     Btn_Done AT ROW 1.27 COL 72 WIDGET-ID 144
     ...
     x-mensaje AT ROW 5.31 COL 9 COLON-ALIGNED NO-LABEL WIDGET-ID 12
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-codcia
        ...
        AND (f-Vendedor = '' OR LOOKUP(dwh_ventas_cab.CodVen, f-vendedor) > 0):
        ...
        FOR EACH dwh_ventas_det OF dwh_ventas_cab NO-LOCK:
            ...
            FIND FIRST t-ddoc WHERE t-ddoc.codcia = s-codcia
                AND t-ddoc.coddiv = dwh_ventas_det.coddiv
                AND t-ddoc.coddoc = dwh_ventas_det.coddoc
                AND t-ddoc.nrodoc = dwh_ventas_det.nrodoc
                AND t-ddoc.codmat = dwh_ventas_det.codmat NO-LOCK NO-ERROR.
            IF NOT AVAIL t-ddoc THEN DO:
                /*Crea Tabla*/
                CREATE t-ddoc.
                BUFFER-COPY dwh_ventas_det
                    TO t-ddoc
                    ASSIGN 
                    t-ddoc.codven = dwh_ventas_cab.codven
                    t-ddoc.fmapgo = dwh_ventas_cab.fmapgo
                    t-ddoc.clfcli = cClfCli
                    t-ddoc.implin = estavtas.dwh_ventas_det.ImpNacSIGV.
            END.
        END.
        DISPLAY 'PROCESANDO INFORMACION .... ' + dwh_ventas_cab.coddoc + ' ' + dwh_ventas_cab.nrodoc @ x-mensaje 
    ...
    FOR EACH t-ddoc,
        ...
        IF AVAIL vtatabla THEN DO: 
            ASSIGN
                ...
                t-ddoc.impcom = t-ddoc.implin * (VtaTabla.Valor[3] / 100)
                t-ddoc.cllave = VtaTabla.Llave_c1
                t-ddoc.codfam = cCodFam
                t-ddoc.subfam = cSubFam.

            IF t-ddoc.fmapgo = '002' AND t-ddoc.coddoc <> 'N/C' 
                THEN impadd = (t-ddoc.implin * 0.1 / 100).
            ELSE impadd = 0.
        END.
    END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE x-CodDiv BUTTON-4 Btn_Done x-FchDoc-1 x-FchDoc-2 f-vendedor BUTTON-1 
      WITH FRAME F-Main IN WINDOW W-Win.
...
DEFINE VAR iFactor   AS INT  INIT 1 NO-UNDO.
...
FOR EACH t-ddoc NO-LOCK,
    ...
    FIND FIRST gn-ven WHERE gn-ven.codcia = s-codcia
        AND gn-ven.codven = t-ddoc.codven NO-LOCK NO-ERROR.
    IF AVAIL gn-ven THEN x-Vend = gn-ven.codven + '-' + gn-ven.nomven.
    ELSE x-Vend = t-ddoc.codven.
    FIND FIRST ccbcdocu WHERE ccbcdocu.codcia = s-codcia
        AND ccbcdocu.coddoc = t-ddoc.coddoc
        AND ccbcdocu.nrodoc = t-ddoc.nrodoc
        NO-LOCK NO-ERROR.

    /*Busca Condicion de Venta*/
    FIND FIRST gn-convt WHERE gn-convt.codig = t-ddoc.fmapgo 
        NO-LOCK NO-ERROR.
    IF AVAIL gn-convt THEN cConVta = gn-convt.codig + '-' + gn-convt.nombr.
    ELSE cConVta = t-ddoc.fmapgo.

    t-column = t-column + 1.
    cColumn = STRING(t-Column).      
    cRange = "A" + cColumn.   
    chWorkSheet:Range(cRange):Value = (IF AVAILABLE ccbcdocu THEN ccbcdocu.codcli + '-' + ccbcdocu.nomcli ELSE '').
    cRange = "B" + cColumn.   
    chWorkSheet:Range(cRange):Value = t-ddoc.codmat + '-' + almmmatg.desmat .
    cRange = "C" + cColumn.   
    chWorkSheet:Range(cRange):Value = t-ddoc.coddoc + '-' + t-ddoc.nrodoc.
    cRange = "D" + cColumn.   
    chWorkSheet:Range(cRange):Value = "'" + (IF AVAILABLE ccbcdocu THEN ccbcdocu.nroped ELSE '').
    cRange = "E" + cColumn.   
    chWorkSheet:Range(cRange):Value = (IF AVAILABLE ccbcdocu THEN ccbcdocu.fchdoc ELSE ?).
    cRange = "F" + cColumn.   
    ...
    IF t-ddoc.coddoc = 'N/C' THEN DO:
        cRange = "S" + cColumn.   
        chWorkSheet:Range(cRange):Value = (IF AVAILABLE ccbcdocu THEN CcbCDocu.CodRef + '-' + CcbCDocu.NroRef ELSE '').
        cRange = "T" + cColumn.   
