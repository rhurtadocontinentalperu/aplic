...
AND x-FacCPedi.CodDoc = 'COT' ~
AND x-FacCPedi.FlgEst = s-flgest ~
AND (x-Faccpedi.nroped = '000124693' OR x-Faccpedi.nroped = '000124692') ~
)
*/
...
&Scoped-Define ENABLED-OBJECTS BUTTON-13 COMBO-BOX_CodAlm COMBO-NroSer ~
txtCodPos FILL-IN-CodCli txtOrdenCompra BUTTON-11 BROWSE-7 BROWSE-4 ~
Btn_Cancel BUTTON-12 RECT-54 RECT-55 RECT-56 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-msg COMBO-BOX_CodAlm COMBO-NroSer ~
txtCodPos FILL-IN-CodCli txtOrdenCompra txtdesPostal COMBO-BOX_CodAlm_010 ~
...
DEFINE VARIABLE FILL-IN-msg AS CHARACTER FORMAT "X(256)":U 
      VIEW-AS TEXT 
     SIZE 11.72 BY .62 NO-UNDO.

DEFINE VARIABLE txtCodPos AS CHARACTER FORMAT "X(4)":U 
     LABEL "Cod.Postal" 
     VIEW-AS FILL-IN 
     SIZE 5 BY .81 NO-UNDO.

DEFINE VARIABLE txtdesPostal AS CHARACTER FORMAT "X(40)":U 
     VIEW-AS FILL-IN 
     SIZE 36 BY .81
...
DEFINE FRAME D-Dialog
     FILL-IN-msg AT ROW 24.46 COL 23 COLON-ALIGNED NO-LABEL WIDGET-ID 40
     BUTTON-13 AT ROW 24.27 COL 8 WIDGET-ID 38
     COMBO-BOX_CodAlm AT ROW 2.08 COL 10 COLON-ALIGNED WIDGET-ID 2
     COMBO-NroSer AT ROW 3.69 COL 7.71 WIDGET-ID 30
     txtCodPos AT ROW 3.69 COL 26 COLON-ALIGNED WIDGET-ID 34
     ...
     txtdesPostal AT ROW 4.69 COL 18.29 COLON-ALIGNED NO-LABEL WIDGET-ID 36
     ...
     Btn_Cancel AT ROW 22.54 COL 17.86
     BUTTON-12 AT ROW 22.54 COL 2.86 WIDGET-ID 32
...
/* SETTINGS FOR DIALOG-BOX D-Dialog
   FRAME-NAME Custom                                                    */
/* BROWSE-TAB BROWSE-7 BUTTON-11 D-Dialog */
/* BROWSE-TAB BROWSE-4 COMBO-BOX_CodAlm_Otras D-Dialog */
ASSIGN 
...
/* SETTINGS FOR FILL-IN FILL-IN-msg IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN txtdesPostal IN FRAME D-Dialog
   NO-ENABLE                                                            */
...
DO:
    ...
    IF txtCodPos = "" OR txtCodPos = ? THEN DO:
        MESSAGE "Ingrese Codigo Postal".
        RETURN NO-APPLY.
    END.

    txtDesPostal:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".

    FIND FIRST almtabla WHERE almtabla.tabla = 'CP' AND 
                            almtabla.codigo = txtCodPos NO-LOCK NO-ERROR.
    IF NOT AVAILABLE almtabla THEN DO:
        MESSAGE "Codigo Postal NO EXISTE".
        RETURN NO-APPLY.
    END.
    txtDesPostal:SCREEN-VALUE IN FRAME {&FRAME-NAME} = almtabla.nombre.
...
ON CHOOSE OF BUTTON-13 IN FRAME D-Dialog /* Pre-Validar */
DO:
  RUN ue-valida.
END.
...
&Scoped-define SELF-NAME txtCodPos
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL txtCodPos D-Dialog
ON LEAVE OF txtCodPos IN FRAME D-Dialog /* Cod.Postal */
DO:
    ASSIGN txtCodPos.
...
FOR EACH Facdpedi OF tt-faccpedi NO-LOCK WHERE (Facdpedi.CanPed - Facdpedi.CanAte) > 0,
  ...
  RUN gn/stock-comprometido-v2.r (Almmmate.CodMat, Almmmate.CodAlm, NO, OUTPUT s-StkComprometido).
  ...
  IF s-StkDis <= 0 THEN DO:
      ...
      RETURN 'ADM-ERROR'.
      */
      /* NO existe STOCK, Siguiente articulo */
      NEXT.
  END.
...
------------------------------------------------------------------------------*/
  ...
  fill-in-msg:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "1.-" + faccpedi.nroped.
  RUN vta2/promocion-generalv2.r (COTIZACION.Libre_c01,   /* División Lista Precio */
                                faccpedi.CodCli,
                                INPUT-OUTPUT TABLE PEDI, 
                                OUTPUT pMensaje).
  fill-in-msg:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "2.-" + faccpedi.nroped.
      ...
      IF s-StkDis <= 0 THEN DO:
          ...
          CREATE w-report-tmp.
            ASSIGN  w-report-tmp.campo-c[1] = pedi.nroped
                    w-report-tmp.campo-c[2] = PEDI.CodMat + " el stock esta en CERO, almacen " + PEDI.AlmDes.
          RETURN 'ADM-ERROR'.
          */
          /* Siguiente articulo */
          NEXT.
  ...
  IF NOT AVAILABLE Facdpedi THEN DO:
      CREATE w-report-tmp.
        ASSIGN  w-report-tmp.campo-c[1] = tt-faccpedi.nroped
                w-report-tmp.campo-c[2] = "La cotizacion no genero ningun Item".
      RETURN 'ADM-ERROR'.
  END.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-msg COMBO-BOX_CodAlm COMBO-NroSer txtCodPos FILL-IN-CodCli 
          txtOrdenCompra txtdesPostal COMBO-BOX_CodAlm_010 COMBO-BOX_CodAlm_011 
          COMBO-BOX_CodAlm_017 COMBO-BOX_CodAlm_Otras 
      WITH FRAME D-Dialog.
  ENABLE BUTTON-13 COMBO-BOX_CodAlm COMBO-NroSer txtCodPos FILL-IN-CodCli 
         txtOrdenCompra BUTTON-11 BROWSE-7 BROWSE-4 Btn_Cancel BUTTON-12 
...
------------------------------------------------------------------------------*/
    ...
    */
    ...
    CASE HANDLE-CAMPO:name:
        WHEN "txtCodPos" THEN input-var-1 = "CP".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ue-valida D-Dialog 
PROCEDURE ue-valida :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
DEFINE VAR x-saldo AS DEC INIT 0.   
...
FOR EACH tt-faccpedi NO-LOCK:
    FOR EACH Facdpedi OF tt-faccpedi NO-LOCK WHERE (Facdpedi.CanPed - Facdpedi.CanAte) > 0 :
      ...
      END.
      ELSE DO:
          x-cantidad = Facdpedi.CanPed - Facdpedi.CanAte.
          /* Stock comprometido */
          RUN gn/stock-comprometido-v2.r(INPUT Facdpedi.CodMat, INPUT s-CodAlm, NO, OUTPUT x-saldo).

          /* Saldo actual menos el comprometido */
          x-saldo = Almmmate.stkact - x-saldo.
          IF x-saldo < x-cantidad THEN DO:
              CREATE w-report-tmp.
                ASSIGN  w-report-tmp.campo-c[1] = Facdpedi.nroped
                        w-report-tmp.campo-c[2] = Facdpedi.CodMat + ", almacen :" + s-codalm + ", Stock:" + STRING(x-saldo,"->>,>>>,>>9.99") + 
                                            ", Cant." + STRING(x-cantidad,"->>,>>>,>>9.99").                        
          END.
      END.
    END.
END.
