...
DEF VAR s-flgest AS CHAR INIT 'P,DA'.
...
&Scoped-define FIELDS-IN-QUERY-br_table FacCPedi.CodDoc FacCPedi.NroPed FacCPedi.FchPed FacCPedi.CodCli FacCPedi.NomCli   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define SELF-NAME br_table
&Scoped-define OPEN-QUERY-br_table CASE COMBO-BOX-1:     WHEN 'Todos' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc          AND LOOKUP(TRIM(FacCPedi.FlgEst), ~
       s-flgest) > 0 NO-LOCK BY FacCPedi.NroPed DESC.          /*   ~{&SORTBY-PHRASE}.*/     WHEN 'Inicien con' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc          AND LOOKUP(TRIM(FacCPedi.FlgEst), ~
       s-flgest) > 0          AND FacCPedi.NomCli BEGINS x-NomCli NO-LOCK BY FacCPedi.NroPed DESC.          /*   ~{&SORTBY-PHRASE}.*/     WHEN 'Contengan' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc          AND LOOKUP(TRIM(FacCPedi.FlgEst), ~
       s-flgest) > 0          AND INDEX(FacCPedi.NomCli, ~
       x-NomCli) > 0 NO-LOCK BY FacCPedi.NroPed DESC.          /*   ~{&SORTBY-PHRASE}.*/ END.
...
<SORTBY-OPTIONS>
NroPed|y||INTEGRAL.FacCPedi.NroPed|no
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "':U + 'NroPed' + '",
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _FREEFORM
  QUERY br_table NO-LOCK DISPLAY
      FacCPedi.CodDoc COLUMN-LABEL "<Doc>"
...
CASE COMBO-BOX-1:
    WHEN 'Todos' THEN
        OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}
         ...
         AND LOOKUP(TRIM(FacCPedi.FlgEst), s-flgest) > 0 NO-LOCK BY FacCPedi.NroPed DESC.
         /*   ~{&SORTBY-PHRASE}.*/
    WHEN 'Inicien con' THEN
        OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}
         ...
         AND FacCPedi.CodDiv = s-coddiv
         AND FacCPedi.CodDoc = s-coddoc
         AND LOOKUP(TRIM(FacCPedi.FlgEst), s-flgest) > 0
         AND FacCPedi.NomCli BEGINS x-NomCli NO-LOCK BY FacCPedi.NroPed DESC.
         /*   ~{&SORTBY-PHRASE}.*/
    WHEN 'Contengan' THEN
        OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}
              AND FacCPedi.CodCia = s-codcia
         ...
         AND FacCPedi.CodDoc = s-coddoc
         AND LOOKUP(TRIM(FacCPedi.FlgEst), s-flgest) > 0
         AND INDEX(FacCPedi.NomCli, x-NomCli) > 0 NO-LOCK BY FacCPedi.NroPed DESC.
         /*   ~{&SORTBY-PHRASE}.*/
END.
     _END_FREEFORM
...
ON FIND OF Faccpedi DO:
    /* PCO en tramite */
    IF CAN-FIND(FIRST B-CPEDI WHERE B-CPEDI.CodCia = Faccpedi.codcia AND
    ...
    IF CAN-FIND(FIRST B-CPEDI WHERE B-CPEDI.CodCia = Faccpedi.codcia AND
                ...
                B-CPEDI.codref = Faccpedi.coddoc AND
                B-CPEDI.nroref = Faccpedi.nroped AND
                B-CPEDI.flgest = 'C' AND
                CAN-FIND(FIRST ORDEN WHERE ORDEN.codcia = B-CPEDI.codcia AND
                         ORDEN.coddiv = B-CPEDI.coddiv AND
                         ORDEN.coddoc = 'O/D' AND
                         ORDEN.codref = B-CPEDI.coddoc AND
                         ORDEN.nroref = B-CPEDI.nroped AND
                         ORDEN.flgest = 'P' NO-LOCK)
                NO-LOCK)
        THEN RETURN ERROR.
END.
...
------------------------------------------------------------------------------*/
  ...
  THEN DO:
      MESSAGE "La Cotización NO tiene atenciones parciales" SKIP
          "No se puede cerrar la cotización, debe anularla" SKIP
