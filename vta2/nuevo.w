...
DEFINE TEMP-TABLE tt-reg-tempo NO-UNDO
    ...
    FIELD P LIKE ccbcdocu.imptot
    FIELD Q LIKE ccbcdocu.imptot
    FIELD R LIKE ccbcdocu.imptot
    ...
    FIELD AN LIKE ccbcdocu.imptot /*lde_ImpFoto.*/
    FIELD AM AS CHAR FORMAT 'x(80)'
    FIELD AO LIKE ccbcdocu.imptot /*lde_ImpFoto.*/
...
DEF TEMP-TABLE Detalle NO-UNDO 
    ...
    FIELD codref        LIKE ccbcdocu.codref        LABEL 'Ref.'
    FIELD nroref        LIKE ccbcdocu.nroref        LABEL 'Numero'
    FIELD libre_c04     LIKE ccbcdocu.libre_c04     LABEL 'G/R Entrega Final'
    FIELD nroord        LIKE ccbcdocu.nroord        LABEL 'Pedido Cliente'
    FIELD codcli        LIKE ccbcdocu.codcli        LABEL 'Codigo'
    FIELD nomcli        LIKE ccbcdocu.nomcli        LABEL 'Cliente'
    FIELD ruccli        LIKE ccbcdocu.ruccli        LABEL 'RUC'
    FIELD fmapgo        LIKE ccbcdocu.fmapgo        LABEL 'Codd. Vta.'
    FIELD codven        LIKE ccbcdocu.codven        LABEL 'Vendedor'
    FIELD nomven        LIKE gn-ven.nomven          LABEL 'Nombre del Vendedor'
    FIELD fchdoc        LIKE ccbcdocu.fchdoc        LABEL 'Emision'           FORMAT '99/99/9999'
    FIELD fchate        LIKE ccbcdocu.fchate        LABEL 'Deposito'          FORMAT '99/99/9999'
    FIELD fchvto        LIKE ccbcdocu.fchvto        LABEL 'Vencimiento'       FORMAT '99/99/9999'
    FIELD fchcbd        LIKE ccbcdocu.fchcbd        LABEL 'Recepcion'         FORMAT '99/99/9999'
    FIELD fchcan        LIKE ccbcdocu.fchcan        LABEL 'Cancelacion'       FORMAT '99/99/9999'
    FIELD imptotnac_1   LIKE ccbcdocu.imptot        LABEL 'Importe S/'
    FIELD imptotnac_2   LIKE ccbcdocu.imptot        LABEL 'Importe S/. SIN Adelanto'
    FIELD sdoact_1      LIKE ccbcdocu.sdoact        LABEL 'Saldo S/.'
    FIELD imptotext_1   LIKE ccbcdocu.imptot        LABEL 'Importe US$'
    FIELD imptotext_2   LIKE ccbcdocu.imptot        LABEL 'Importe US$ SIN Adelanto'
    FIELD sdoact_2      LIKE ccbcdocu.sdoact        LABEL 'Saldo US$'
    FIELD estado        AS CHAR                     LABEL 'Estado'              FORMAT 'x(20)'
    FIELD usuanul       LIKE ccbaudit.usuario       LABEL 'Usuario anulacion'
    FIELD fchanu        LIKE ccbaudit.fecha         LABEL 'Fecha'               FORMAT '99/99/9999'
    FIELD horanu        LIKE ccbaudit.hora          LABEL 'Hora'
    FIELD motanu        LIKE ccbtabla.nombre        LABEL 'Motivo anulacion'
    FIELD divori        LIKE ccbcdocu.divori        LABEL 'Division'
    FIELD fchrep        AS DATE                     LABEL 'Fecha Reporte'       FORMAT '99/99/9999'
    FIELD ubicacion     AS CHAR                     LABEL 'Ubicacion'           FORMAT 'x(40)'
    FIELD banco         AS CHAR                     LABEL 'Banco'               FORMAT 'x(60)'
    FIELD situacion     AS CHAR                     LABEL 'Situacion'           FORMAT 'x(30)'
    FIELD fchent_1      AS DATE                     LABEL 'F.Entrega(DIST)'     FORMAT '99/99/9999'
    FIELD fchent_2      AS DATE                     LABEL 'F.Entrega(CRED)'     FORMAT '99/99/9999'
    FIELD dircli        LIKE ccbcdocu.dircli        LABEL 'DIRECC. CLIENTE'
    FIELD canje         AS CHAR                     LABEL 'CANJE'               FORMAT 'x(15)'
    FIELD coddep        AS CHAR                     LABEL 'DEPARTAMENTO'        FORMAT 'x(20)'
    FIELD codpro        AS CHAR                     LABEL 'PROVINCIA'           FORMAT 'x(20)'
    FIELD coddis        AS CHAR                     LABEL 'DISTRITO'            FORMAT 'x(20)'
    FIELD nrouni        AS CHAR                     LABEL 'No UNICO'            FORMAT 'x(20)'
    FIELD impfot_1      AS DECI                     LABEL 'Papel Fotocopia (S/)' FORMAT '>>>,>>>,>>9.99'
    FIELD impfot_2      AS DECI                     LABEL 'Papel Fotocopia ($)' FORMAT '>>>,>>>,>>9.99'
    FIELD codcta        AS CHAR                     LABEL 'Concepto N/C Otras'  FORMAT 'x(12)'
    FIELD nomcta        AS CHAR                     LABEL 'Descripcion'         FORMAT 'x(40)'
.
...
FILL-IN-filtro BUTTON-19 BUTTON-20 BUTTON-Division BUTTON_Texto xcodmod ~
C-CMPBTE c-Serie f-desde f-hasta x-CndCre x-FchVto-1 x-FchVto-2 ~
x-FchCan-desde x-FchCan-hasta COMBO-BOX-5 F-FMAPGO f-codven ~
RADIO-SET-CndVta br_table 
...
DEFINE FRAME F-Main
     ...
     BUTTON_Texto AT ROW 2.88 COL 132 WIDGET-ID 18
...
DO:
  ...
  IF f-desde > f-hasta THEN DO:
      MESSAGE "Fecha de emisión DESDE debe ser menor/igual a HASTA" VIEW-AS ALERT-BOX INFORMATION.
      RETURN NO-APPLY.
  END.
  
  /* 20/11/2024: Obervado por Gina Cóndor 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON_Texto L-table-Win
...
DEF VAR iItem_1 AS INTE NO-UNDO.
DEF VAR iItem_2 AS INTE NO-UNDO.
DEF VAR iItem_3 AS INTE NO-UNDO.
...
CASE TRUE:
    WHEN FILL-IN-CodCli > '' THEN DO:
        DO iItem_1 = 1 TO NUM-ENTRIES(f-Estado):
            cEstado = ENTRY(iItem_1, f-Estado).
            DO iItem_2 = 1 TO NUM-ENTRIES(x-CodDoc):
                cCodDoc = ENTRY(iItem_2, x-CodDoc).
                FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia AND
                    Ccbcdocu.codcli = FILL-IN-CodCli AND
                    Ccbcdocu.flgest = cEstado AND 
                    Ccbcdocu.coddoc = cCodDoc AND
                    Ccbcdocu.fchdoc >= f-Desde AND
                    Ccbcdocu.fchdoc <= f-Hasta
                    :
                    {&RUTINA-GENERAL}
                END.
            END.
        ...
        CASE CMB-Filtro: 
            WHEN 'Que inicien con' THEN DO:
                FOR EACH gn-clie NO-LOCK WHERE gn-clie.codcia = cl-codcia AND
                    gn-clie.nomcli BEGINS TRIM(FILL-IN-Filtro):
                    CREATE t-gn-clie.
            ...
            WHEN 'Que contegan' THEN DO:
                FOR EACH gn-clie NO-LOCK WHERE gn-clie.codcia = cl-codcia AND
                    gn-clie.nomcli CONTAINS (TRIM(FILL-IN-filtro) + "*"):
                    CREATE t-gn-clie.
                    BUFFER-COPY gn-clie USING gn-clie.codcia gn-clie.codcli TO t-gn-clie.
                END.
            END.
    ...
    END.
    OTHERWISE DO:
        DO iItem_3 = 1 TO NUM-ENTRIES(x-CodDiv):
            cCodDiv = ENTRY(iItem_3,x-CodDiv).
            DO iItem_1 = 1 TO NUM-ENTRIES(f-Estado):
                cEstado = ENTRY(iItem_1, f-Estado).
                DO iItem_2 = 1 TO NUM-ENTRIES(x-CodDoc):
                    cCodDoc = ENTRY(iItem_2, x-CodDoc).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Temporal-Old L-table-Win 
...
FOR EACH t-ccbcdocu NO-LOCK, FIRST ccbcdocu OF t-ccbcdocu NO-LOCK:
    ...
    END.
    CREATE Detalle.
    Detalle.coddoc = t-ccbcdocu.coddoc.
    Detalle.nrodoc = t-ccbcdocu.nrodoc.
    Detalle.codref = t-ccbcdocu.codref.
    Detalle.nroref = t-ccbcdocu.nroref.
    Detalle.libre_c04 = t-ccbcdocu.libre_c04.
    Detalle.nroord = t-ccbcdocu.nroord.
    Detalle.codcli = t-ccbcdocu.codcli.
    Detalle.nomcli = t-ccbcdocu.nomcli.
    Detalle.ruccli = t-ccbcdocu.ruccli.
    Detalle.fmapgo = t-ccbcdocu.fmapgo.
    Detalle.codven = t-ccbcdocu.codven.
    FIND gn-ven WHERE gn-ven.CodCia = s-codcia
        AND gn-ven.CodVen = t-ccbcdocu.codven NO-LOCK NO-ERROR.
    IF AVAILABLE gn-ven THEN DO:
        Detalle.nomven = gn-ven.NomVen.
    END.
    Detalle.fchdoc = t-ccbcdocu.fchdoc.
    Detalle.fchate = t-ccbcdocu.fchate.
    Detalle.fchvto = t-ccbcdocu.fchvto.
    Detalle.fchcbd = t-ccbcdocu.fchcbd.
    Detalle.fchcan = t-ccbcdocu.fchcan.
    x-ImpTot = t-ccbcdocu.imptot + fAdelanto(t-ccbcdocu.codcia, t-ccbcdocu.coddoc, t-ccbcdocu.nrodoc).
    x-SdoAct = t-ccbcdocu.sdoact.
    FIND Ccbaudit OF t-ccbcdocu NO-LOCK NO-ERROR.
    IF AVAILABLE Ccbaudit THEN FIND ccbtabla WHERE ccbtabla.codcia = t-ccbcdocu.codcia
        AND ccbtabla.tabla = 'MA' AND ccbtabla.codigo = ccbaudit.codref NO-LOCK NO-ERROR.
    IF t-ccbcdocu.codmon = 1 THEN DO:
        Detalle.imptotnac_1 = t-ccbcdocu.imptot.
        Detalle.imptotnac_2 = x-ImpTot.
        Detalle.sdoact_1 = x-sdoact.
    END.
    IF t-ccbcdocu.codmon = 2 THEN DO:
        Detalle.imptotext_1 = t-ccbcdocu.imptot.
        Detalle.imptotext_2 = x-ImpTot.
        Detalle.sdoact_2 = x-sdoact.
    END.
    Detalle.estado = fEstado().
    IF AVAILABLE ccbtabla AND AVAILABLE ccbaudit THEN DO:
        Detalle.usuanu = ccbaudit.usuario.
        Detalle.fchanu = ccbaudit.fecha.
        Detalle.horanu = ccbaudit.hora.
        Detalle.motanu = ccbtabla.nombre.
    END.
    Detalle.divori = t-ccbcdocu.DivOri.
    Detalle.fchrep = TODAY.
    Detalle.ubicacion = fUbicacion().
    Detalle.banco = fBanco().
    Detalle.situacion = fSituacion().
    Detalle.fchent_1 = lFchEntrega.
    Detalle.fchent_2 = t-ccbcdocu.fchcbd.
    Detalle.dircli = t-ccbcdocu.dircli.
    Detalle.codcta = x-CodCta.
    Detalle.nomcta = x-NomCta.
    ...
    IF AVAILABLE gn-clie THEN DO:
        ...
        Detalle.coddep = pNomDepto.
        Detalle.codpro = pNomProvi.
        Detalle.coddis = pNomDistr.
        Detalle.nrouni = (IF t-ccbcdocu.coddoc = 'LET' THEN t-ccbcdocu.nrosal ELSE '').
    END.
    /* Ic - 30Oct2018, pedido de Julissa Calderon*/
    lde_ImpFoto = 0.
    lde_ImpTotal = 0.
    ...
    IF lde_ImpFoto > 0 THEN DO:        
        IF t-ccbcdocu.codmon = 2 THEN DO:
            Detalle.impfot_1 = lde_ImpFoto.    /*lde_ImpFoto.*/
        END.
        ELSE DO:
            Detalle.impfot_2 = lde_ImpFoto.     /*lde_ImpFoto.*/
        END.
    END.
END.
SESSION:SET-WAIT-STATE('').
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE texto-old L-table-Win 
PROCEDURE texto-old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
