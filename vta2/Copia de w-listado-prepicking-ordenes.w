...
ChkChequeo RADIO-SET-1 txtArtSinPeso 
...
DEFINE VARIABLE h_b-listado-prepicking-ordenes AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-13 AT ROW 2.35 COL 47.29 WIDGET-ID 4
     txtDesde AT ROW 2.5 COL 8.72 COLON-ALIGNED WIDGET-ID 16
     txtHasta AT ROW 2.5 COL 30 COLON-ALIGNED WIDGET-ID 20
     ChkChequeo AT ROW 2.54 COL 64.72 WIDGET-ID 22
     BtnExcel AT ROW 2.73 COL 96 WIDGET-ID 34
     RADIO-SET-1 AT ROW 2.73 COL 111 NO-LABEL WIDGET-ID 28
     BUTTON-PorZona AT ROW 24.46 COL 3.29 WIDGET-ID 6
     BUTTON-Alfabeticamente AT ROW 24.46 COL 25.29 WIDGET-ID 8
     txtArtSinPeso AT ROW 24.92 COL 54.29 COLON-ALIGNED NO-LABEL WIDGET-ID 24
     ...
     "Lista de Articulos que NO tienen PESO en la O/D" VIEW-AS TEXT
          SIZE 43.14 BY .62 AT ROW 24.23 COL 56.72 WIDGET-ID 26
          BGCOLOR 15 FGCOLOR 9 FONT 6
     RECT-1 AT ROW 2.54 COL 95 WIDGET-ID 32
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 157.29 BY 25.38 WIDGET-ID 100.
...
DO:
  ASSIGN RADIO-SET-1.                 
  CASE RADIO-SET-1:
      WHEN 1 THEN RUN envia-excel IN h_b-listado-prepicking-ordenes.
      WHEN 2 THEN RUN envia-excel-detalle IN h_b-listado-prepicking-ordenes.
  END CASE.
...
DO:
    ...
    RUN dispatch IN h_b-listado-prepicking-ordenes ('open-query':U).
...
DO:
  ...
  RUN Imprimir-Formato IN h_b-listado-prepicking-ordenes ( INPUT "ALFABETICO").
END.
...
DO:
  /*RUN dispatch IN h_b-cimprime-od ('imprime':U).*/
    RUN Imprimir-Formato IN h_b-listado-prepicking-ordenes ( INPUT "ZONA").
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_b-listado-prepicking-ordenes ).
       RUN set-position IN h_b-listado-prepicking-ordenes ( 4.27 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-listado-prepicking-ordenes ( 7.50 , 151.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-listado-prepicking-ordenes , 'Record':U , h_b-dimprime-od ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-listado-prepicking-ordenes ,
             RADIO-SET-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-od ,
             h_b-listado-prepicking-ordenes , 'AFTER':U ).
    END. /* Page 0 */
