...
ChkChequeo BtnExcel RADIO-SET-1 BUTTON-16 BUTTON-14 BUTTON-15 
&Scoped-Define DISPLAYED-OBJECTS txtDesde txtHasta ChkChequeo RADIO-SET-1 ~
txtcodref txtNroRef txtArtSinPeso 
...
DEFINE VARIABLE h_b-cimprime-otr AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BtnDone AT ROW 1 COL 146 WIDGET-ID 10
     ...
     BtnExcel AT ROW 1.58 COL 94 WIDGET-ID 12
     RADIO-SET-1 AT ROW 1.58 COL 109 NO-LABEL WIDGET-ID 28
     BUTTON-16 AT ROW 1.58 COL 127 WIDGET-ID 14
     txtcodref AT ROW 23.46 COL 113 COLON-ALIGNED NO-LABEL WIDGET-ID 36
     txtNroRef AT ROW 23.46 COL 119.29 COLON-ALIGNED NO-LABEL WIDGET-ID 38
     BUTTON-14 AT ROW 23.92 COL 3 WIDGET-ID 6
     BUTTON-15 AT ROW 23.92 COL 25 WIDGET-ID 8
     txtArtSinPeso AT ROW 24.62 COL 54.72 COLON-ALIGNED NO-LABEL WIDGET-ID 24
     "Referencia :" VIEW-AS TEXT
          SIZE 11 BY .62 AT ROW 23.69 COL 103.43 WIDGET-ID 34
          BGCOLOR 15 FGCOLOR 14 FONT 10
     "Lista de Articulos que NO tienen PESO en la O/D" VIEW-AS TEXT
          SIZE 43.14 BY .62 AT ROW 23.88 COL 57 WIDGET-ID 26
          BGCOLOR 15 FGCOLOR 9 FONT 6
     RECT-1 AT ROW 1.38 COL 93 WIDGET-ID 32
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 154.29 BY 24.77 WIDGET-ID 100.
...
DO:
  ASSIGN RADIO-SET-1.                 
  CASE RADIO-SET-1:
      WHEN 1 THEN RUN envia-excel IN h_b-cimprime-otr.
      WHEN 2 THEN RUN envia-excel-detalle IN h_b-cimprime-otr.
  END CASE.
...
DO:
    ...
    RUN dispatch IN h_b-cimprime-otr ('open-query':U).
...
DO:
  /*RUN dispatch IN h_b-cimprime-od ('imprime':U).*/
    RUN Imprimir-Formato IN h_b-cimprime-otr ( INPUT "ZONA").
END.
...
DO:
  /*RUN dispatch IN h_b-cimprime-od ('imprime':U).*/
  RUN Imprimir-Formato IN h_b-cimprime-otr ( INPUT "ALFABETICO").
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-16 sW-Win
ON CHOOSE OF BUTTON-16 IN FRAME F-Main /* A DISTRIBUCION */
DO:
   RUN A-Distribucion IN h_b-cimprime-otr.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-cimprime-otr.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = NroPed':U ,
             OUTPUT h_b-cimprime-otr ).
       RUN set-position IN h_b-cimprime-otr ( 3.31 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cimprime-otr ( 7.50 , 152.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b-dimprime-od ( 11.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-dimprime-od ( 12.12 , 147.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-dimprime-od. */
       RUN add-link IN adm-broker-hdl ( h_b-cimprime-otr , 'Record':U , h_b-dimprime-od ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cimprime-otr ,
             BUTTON-16:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-od ,
             h_b-cimprime-otr , 'AFTER':U ).
    END. /* Page 0 */
...
OCXFile = SEARCH( "w-listado-picking-otr.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-listado-picking-otr.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  DISPLAY txtDesde txtHasta ChkChequeo RADIO-SET-1 txtcodref txtNroRef 
          txtArtSinPeso 
      WITH FRAME F-Main IN WINDOW sW-Win.
  ENABLE BtnDone RECT-1 BUTTON-13 txtDesde txtHasta ChkChequeo BtnExcel 
         RADIO-SET-1 BUTTON-16 BUTTON-14 BUTTON-15 
      WITH FRAME F-Main IN WINDOW sW-Win.
