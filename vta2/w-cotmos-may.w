...
&Scoped-Define ENABLED-OBJECTS RECT-70 COMBO-NroSer BTN-Excel Btn-Grupos 
...
DEFINE VARIABLE h_b-cotmos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-pedido AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-cotmos-may AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotmos-may AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BTN-Excel AT ROW 3.15 COL 123
     Btn-Grupos AT ROW 26.04 COL 37 WIDGET-ID 2
     RECT-70 AT ROW 2.62 COL 2 WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 141.43 BY 26.62
         FONT 4.
...
ON END-ERROR OF W-Win /* COTIZACION MOSTRADOR */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* COTIZACION MOSTRADOR */
DO:
...
DO:
  RUN Genera-Excel2 IN h_v-cotmos-may.
END.
...
DO:
  RUN vtamay/d-cotmos-4.
  RUN dispatch IN h_t-cotmos-may ('open-query').
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-cotmos-may.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotmos-may ).
       RUN set-position IN h_v-cotmos-may ( 2.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.27 , 109.00 ) */
       ...
       /* Links to SmartViewer h_v-cotmos-may. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-cotmos-may ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-cotmos-may ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv02 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotmos-may ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-cotmos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotmos ).
       RUN set-position IN h_b-cotmos ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotmos ( 16.88 , 139.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotmos. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_b-cotmos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotmos ,
             BTN-Excel:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-cotmos-may.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotmos-may ).
       RUN set-position IN h_t-cotmos-may ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotmos-may ( 15.54 , 139.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotmos-may. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotmos-may ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotmos-may ,
             BTN-Excel:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotmos-may , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-70 COMBO-NroSer BTN-Excel Btn-Grupos 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_b-cotmos <> ? THEN RUN dispatch IN h_b-cotmos ('open-query':U).
          IF h_t-cotmos-may <> ? THEN RUN dispatch IN h_t-cotmos-may ('open-query':U).
      END.
