...
DEFINE VARIABLE h_b-reprog-pedidos AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 108.14 BY 24.38 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW sW-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "REPROGRAMACION DE FECHA DE ENTREGAS PEDIDOS"
         HEIGHT             = 24.38
         WIDTH              = 108.14
         MAX-HEIGHT         = 29.54
         MAX-WIDTH          = 144.29
...
ON END-ERROR OF sW-Win /* REPROGRAMACION DE FECHA DE ENTREGAS PEDIDOS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL sW-Win sW-Win
ON WINDOW-CLOSE OF sW-Win /* REPROGRAMACION DE FECHA DE ENTREGAS PEDIDOS */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_b-divisiones ( 1.19 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-divisiones ( 5.38 , 66.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/b-reprog-pedidos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-reprog-pedidos ).
       RUN set-position IN h_b-reprog-pedidos ( 6.77 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-reprog-pedidos ( 16.54 , 103.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv97 ).
       RUN set-position IN h_p-updv97 ( 23.31 , 3.00 ) NO-ERROR.
       RUN set-size IN h_p-updv97 ( 1.54 , 34.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-reprog-pedidos. */
       RUN add-link IN adm-broker-hdl ( h_b-divisiones , 'Record':U , h_b-reprog-pedidos ).
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_b-reprog-pedidos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-reprog-pedidos ,
             h_b-divisiones , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv97 ,
             h_b-reprog-pedidos , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
   RUN Ubica-Registro IN h_b-divisiones ( INPUT '00015' /* CHARACTER */).
