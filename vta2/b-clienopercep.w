...
DEF TEMP-TABLE T-Tabla
    FIELD RucCli AS CHAR FORMAT 'x(11)' LABEL 'RUC'
    FIELD NomCli AS CHAR FORMAT 'x(60)' LABEL 'Nombre o Razon Social'.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c1 COLUMN-LABEL "RUC" FORMAT "x(11)":U WIDTH 12.43
      VtaTabla.Llave_c2 COLUMN-LABEL "Nombre o Razón Social" FORMAT "x(60)":U
            WIDTH 47.72
  ENABLE
      VtaTabla.Llave_c1
      VtaTabla.Llave_c2
...
"VtaTabla.Llave_c1" "RUC" "x(11)" "character" ? ? ? ? ? ? yes ? no no "12.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.VtaTabla.Llave_c2
"VtaTabla.Llave_c2" "Nombre o Razón Social" "x(60)" "character" ? ? ? ? ? ? yes ? no no "47.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON 'RETURN':U OF VtaTabla.Llave_c1, VtaTabla.Llave_c2
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
    ...
    pOptions = pOptions + CHR(1) + "FieldList:RucCli,NomCli".

    SESSION:SET-WAIT-STATE('GENERAL').
    EMPTY TEMP-TABLE T-Tabla.
    FOR EACH VtaTabla NO-LOCK WHERE {&Condicion}:
        CREATE T-Tabla.
        ASSIGN
            T-Tabla.RucCli = VtaTabla.Llave_c1
            T-Tabla.NomCli = VtaTabla.Llave_c2.
    END.
    RUN lib/tt-file (TEMP-TABLE T-Tabla:HANDLE, pArchivo, pOptions).
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN lkup/c-nopercepcion ('Clientes NO Percepción').
...
IF LENGTH(vtatabla.llave_c1:SCREEN-VALUE IN BROWSE {&browse-name}) < 11 
    THEN DO:
    MESSAGE 'Debe tener 11 dígitos' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO vtatabla.llave_c1 IN BROWSE {&browse-name}.
    RETURN 'ADM-ERROR'.
END.
IF LOOKUP(SUBSTRING(vtatabla.llave_c1:SCREEN-VALUE IN BROWSE {&browse-name},1,2), '10,20,15,17') = 0 
    THEN DO:
    MESSAGE 'Debe comenzar con 20, 10, 15 ó 17'  SKIP(1)
        'Continuamos?'
...
IF LOOKUP(SUBSTRING(vtatabla.llave_c1:SCREEN-VALUE IN BROWSE {&browse-name},1,2), '10,20,15,17') > 0  
    ...
    RUN lib/_ValRuc (vtatabla.llave_c1:SCREEN-VALUE, OUTPUT pResultado).
