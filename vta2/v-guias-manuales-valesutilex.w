...
DEFINE FRAME F-Main
     CcbCDocu.NroDoc AT ROW 1 COL 17 COLON-ALIGNED FORMAT "XXX-XXXXXX"
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
          FONT 1
     F-Estado AT ROW 1 COL 41 COLON-ALIGNED
     CcbCDocu.FchDoc AT ROW 1 COL 89 COLON-ALIGNED
          LABEL "Emisión"
          VIEW-AS FILL-IN 
          SIZE 9.14 BY .81
     ...
     CcbCDocu.CodAnt AT ROW 1.81 COL 62 COLON-ALIGNED WIDGET-ID 2
          ...
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
     CcbCDocu.CodRef AT ROW 1.81 COL 89 COLON-ALIGNED
          LABEL "Referencia"
          VIEW-AS FILL-IN 
          SIZE 5 BY .81
     CcbCDocu.NroRef AT ROW 1.81 COL 96 NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 12 BY .81 TOOLTIP "XXX-XXXXXX"
     CcbCDocu.NomCli AT ROW 2.62 COL 17 COLON-ALIGNED WIDGET-ID 8
          VIEW-AS FILL-IN 
          SIZE 59 BY .81
     CcbCDocu.CodPed AT ROW 2.62 COL 89 COLON-ALIGNED
          LABEL "Pedido"
          VIEW-AS FILL-IN 
          SIZE 5 BY .81
     CcbCDocu.NroPed AT ROW 2.62 COL 94 COLON-ALIGNED NO-LABEL FORMAT "XXX-XXXXXX"
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
     CcbCDocu.DirCli AT ROW 3.42 COL 17 COLON-ALIGNED WIDGET-ID 6
          VIEW-AS FILL-IN 
          SIZE 59 BY .81
     CcbCDocu.LugEnt AT ROW 4.27 COL 17 COLON-ALIGNED WIDGET-ID 12
          VIEW-AS FILL-IN 
          SIZE 59 BY .81
...
------------------------------------------------------------------------------*/
  ...
  IF output-var-1 = ? THEN RETURN "ADM-ERROR".
    ...
    FIND FIRST b-faccpedi WHERE b-faccpedi.codcia = s-codcia AND
                            ...
                            b-faccpedi.nroped = B-CDOCU.nroped NO-LOCK NO-ERROR.
    ...
    IF cEsValeUtilex = NO THEN DO:
        MESSAGE "La factura no pertenece a VALES UTILEX".
        RETURN "ADM-ERROR".
    END.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  /*IF Ccbcdocu.flgest = 'F' THEN RUN vtamay/r-impgma-03 (ROWID(CcbCDocu)).*/
  /*IF Ccbcdocu.flgest = 'F' THEN RUN vta2/p-imprime-guia (ROWID(CcbCDocu)).*/
...
IF B-CDOCU.CodRef = s-CodDoc THEN DO:
    ...
    RETURN "ADM-ERROR".
END.
/* IF B-CDOCU.DivOri <> s-coddiv THEN DO:                    */
/*     MESSAGE 'El comprobante NO está asignado a esta sede' */
/*         VIEW-AS ALERT-BOX ERROR.                          */
/*     RETURN 'ADM-ERROR'.                                   */
/* END.                                                      */
/* RHC 11/0/2012 */
/* IF B-CDOCU.CodAlm <> s-CodAlm THEN DO:                       */
/*     MESSAGE 'El comprobante no está asignado a este almacén' */
/*         VIEW-AS ALERT-BOX ERROR.                             */
/*     RETURN "ADM-ERROR".                                      */
/* END.                                                         */
