...
FOR EACH Facdpedi OF Faccpedi NO-LOCK BREAK BY Facdpedi.codmat:
    IF FIRST-OF(Facdpedi.codmat) THEN CREATE PEDI-1.
    BUFFER-COPY Facdpedi 
        TO PEDI-1
        ASSIGN
            PEDI-1.CanPed = PEDI-1.CanPed + Facdpedi.CanPed
            PEDI-1.CanAte = PEDI-1.CanAte + Facdpedi.CanAte
            PEDI-1.ImpLin = PEDI-1.ImpLin + Facdpedi.ImpLin
            PEDI-1.ImpDto = PEDI-1.ImpDto + Facdpedi.ImpDto
            PEDI-1.ImpIgv = PEDI-1.ImpIgv + Facdpedi.ImpIgv
            PEDI-1.ImpIsc = PEDI-1.ImpIsc + Facdpedi.ImpIsc.
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT AVAILABLE FacCPedi OR
        ...
        RETURN.
    END.
    RUN vta2\gGenGuia2a (ROWID(FacCPedi), 'A', 'G/R').       /* GUIA AUTOMATICA */
...
------------------------------------------------------------------------------*/
    IF NOT AVAILABLE FacCPedi OR
        ...
        RETURN.
    END.
    RUN vta2/w-guias (ROWID(FacCPedi)).
    IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN.
    RUN vta2\gGenGuia2a (ROWID(FacCPedi), 'M', 'G/R').       /* GUIA MANUAL */
