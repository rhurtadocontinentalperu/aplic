...
DEFINE VARIABLE h_t-pedido-utilex AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-pedido-utilex AS HANDLE NO-UNDO.
...
DO:
  ...
  RUN Cancelar-Pedido IN h_v-pedido-utilex.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v-pedmos-utilex.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedido-utilex ).
       RUN set-position IN h_v-pedido-utilex ( 2.88 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-pedido-utilex. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-pedido-utilex ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_v-pedido-utilex ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  't-pedido-utilex.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedido-utilex ).
       RUN set-position IN h_t-pedido-utilex ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedido-utilex ( 14.23 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-pedido-utilex. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedido-utilex ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedido-utilex ,
             h_v-pedido-utilex , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-pedido-utilex , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-pedido-utilex <> ? THEN RUN dispatch IN h_t-pedido-utilex ('open-query':U).
...
/*       END.                                            */
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-pedido-utilex <> ? THEN RUN Recalcular-Precios IN h_t-pedido-utilex.
