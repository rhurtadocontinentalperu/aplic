...
DEFINE MENU POPUP-MENU-br_table 
       ...
       MENU-ITEM m_VIP_HUANCAYO_-_Descuento_po LABEL "VIP HUANCAYO - Descuento por Volumen".
...
DO:
  ...
  /*IF AVAILABLE Almmmatg THEN RUN pri/D-Lima-DctoProm (Almmmatg.codmat).*/
END.
...
DO:
    ...
    /*IF AVAILABLE VtaListaMinGn THEN RUN Pri/D-Utilex-DctoProm (T-MATG.codmat).*/
    ELSE DO:
        MESSAGE 'Primero debe asignarle un PRECIO DE OFICINA UTILEX'
            VIEW-AS ALERT-BOX WARNING.
    END.
...
ON CHOOSE OF MENU-ITEM m_VIP_HUANCAYO_-_Descuento_po /* VIP HUANCAYO - Descuento por Volumen */
DO:
    /*IF AVAILABLE Almmmatg THEN RUN PRI/D-Lima-DctoVol-v2 (Almmmatg.codmat).*/
    IF AVAILABLE Almmmatg THEN RUN PRI/D-Huancayo-DctoVol (Almmmatg.codmat).
END.
...
/*                                                                                         */
/*       IF pError = "ADM-ERROR" THEN DO:                                                  */
/*           MESSAGE "Margen de utilidad UTILEX NO debe ser menor a" x-Limite "%"  SKIP(2) */
/*               VIEW-AS ALERT-BOX WARNING.                                                */
/*       END.                                                                              */
/*   END.                                                                                  */
  RUN dispatch IN THIS-PROCEDURE ('display-fields':U).
...
------------------------------------------------------------------------------*/
   ...
   /* Ic - 12Dic2016, No validar MARGEN DE UTILIDAD */
   IF NOT AVAILABLE vtatabla THEN DO:
   ...
   IF DECIMAL(T-MATG.MrgAlt[1]:SCREEN-VALUE IN BROWSE {&browse-name}) > 0
       ...
       END.
       /* Artificio:
       Como se va a usar en utilex buscamos una división válida para hacer la verificación
       */
       FIND FIRST gn-divi WHERE gn-divi.codcia = s-codcia AND 
           gn-divi.canalventa = "MIN" AND
           gn-divi.campo-log[1] = NO
           NO-LOCK NO-ERROR.
       IF AVAILABLE gn-divi THEN DO:
           RUN Margen-de-Utilidad (INPUT gn-divi.CodDiv,
                                   INPUT T-MATG.CodMat,
                                   INPUT DECIMAL(T-MATG.PreAlt[1]:SCREEN-VALUE IN BROWSE {&browse-name}),
                                   INPUT T-MATG.UndAlt[1]:SCREEN-VALUE IN BROWSE {&browse-name},
                                   INPUT T-MATG.TpoCmb,
                                   OUTPUT x-Limite,
                                   OUTPUT pError).
           IF pError = "ADM-ERROR" THEN DO:
               MESSAGE "Margen de utilidad para el Precio de Utilex NO debe ser menor a " +
                    TRIM(STRING(x-Limite, ">>>,>>9.99")) VIEW-AS ALERT-BOX ERROR.
               APPLY "ENTRY" TO T-MATG.PreAlt[1].
               RETURN "ADM-ERROR".      
           END.
       END.
   END.
