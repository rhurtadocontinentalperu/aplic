...
DEFINE SHARED VARIABLE s-ListaTerceros AS LOG.
...
FacCPedi.NroRef FacCPedi.NroCard 
&Scoped-define DISPLAYED-TABLES FacCPedi
&Scoped-define FIRST-DISPLAYED-TABLE FacCPedi
&Scoped-Define DISPLAYED-OBJECTS F-Estado FILL-IN-ListaTerceros FILL-IN-1 ~
FILL-IN-sede FILL-IN-Postal f-NomVen F-CndVta F-Nomtar 
...
DEFINE VARIABLE FILL-IN-ListaTerceros AS CHARACTER FORMAT "X(1)":U 
     VIEW-AS FILL-IN 
...
DEFINE FRAME F-Main
     ...
     FacCPedi.FaxCli AT ROW 1.77 COL 29 COLON-ALIGNED NO-LABEL WIDGET-ID 124 FORMAT "X(10)"
          ...
          SIZE 8 BY .81
          FONT 6
     FILL-IN-ListaTerceros AT ROW 1.77 COL 37 COLON-ALIGNED NO-LABEL WIDGET-ID 152
...
/* SETTINGS FOR FILL-IN FILL-IN-ListaTerceros IN FRAME F-Main
   NO-ENABLE                                                            */
...
DO:
  ...
  RUN Verifica-Lista-Terceros (s-CodCli).  /* Si es no Cliente VIP Terceros */
  FILL-IN-ListaTerceros:SCREEN-VALUE = ''.
  IF gn-clie.Libre_D01 > 0 THEN FILL-IN-ListaTerceros:SCREEN-VALUE = STRING(gn-clie.Libre_D01, '9').
...
IF s-ListaTerceros = NO THEN RETURN.
...
FOR EACH Facdpedi OF Faccpedi NO-LOCK WHERE Facdpedi.Libre_c05 <> "OF":
    CREATE ITEM.
    BUFFER-COPY Facdpedi TO ITEM ASSIGN ITEM.Libre_c04 = "" ITEM.Libre_d01 = 0.    /* Limpiamos anterior */
END. 
/* ******************************* */
...
IF x-ListaAnterior < 0 THEN x-ListaAnterior = 0.    /* Valor Mínimo */
...
DO x-Ciclos = 1 TO 2:
    ...
    Continuamos a ver si en los ITEMs se ha forzado la lista */
    ...
    FOR EACH ITEM WHERE ITEM.Libre_c05 <> "OF", 
        ...
        /* ***************************************************************** */
        /* RHC 16/12/2015 En caso que el usuario fuerce la lista de terceros */
        /* ***************************************************************** */
        IF x-ListaTerceros = 0 AND INTEGER(ITEM.TipVta) = 0 THEN NEXT.
        IF INTEGER(ITEM.TipVta) > 0 THEN DO:
            IF ListaTerceros.PreOfi[INTEGER(ITEM.TipVta)] = 0 THEN NEXT.
            F-PREBAS = ListaTerceros.PreOfi[INTEGER(ITEM.TipVta)].
        END.
        ELSE DO:
...
------------------------------------------------------------------------------*/
  ...
  /* **************************************************************** */
  /* RHC 17/12/2015 PARA VENTAS CONTRATO MARCO SOLO PUEDE SACAR DEL ALMACEN 11E */
  /* ************************************************************************** */
  IF FaccPedi.TpoPed = "M" OR Faccpedi.Libre_c04 = "SI" THEN DO:
      Faccpedi.LugEnt2 = "11M".
  END.
  ELSE IF Faccpedi.LugEnt2 = "11M" THEN Faccpedi.LugEnt2 = "".
  /* ************************************************************************** */
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE FacCPedi THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      FILL-IN-ListaTerceros:SCREEN-VALUE = ''.
      IF FacCPedi.TipBon[10] > 0 THEN FILL-IN-ListaTerceros:SCREEN-VALUE = STRING(FacCPedi.TipBon[10],'9').
  END.
      ...
      IF RETURN-VALUE = 'NO' THEN DO:
          ...
          */
          IF faccpedi.libre_c01 <> ? THEN DO:
              FIND FIRST gn-divi WHERE gn-divi.coddiv = faccpedi.libre_c01 /*FacCPedi.coddiv*/ NO-LOCK NO-ERROR.
              IF AVAILABLE gn-divi THEN DO:
                  /* Si es FERIA */
                  IF NOT gn-divi.canalventa = 'FER' THEN DO:
...
RUN Verifica-Lista-Terceros (Faccpedi.CodCli).  /* Si es no Cliente VIP Terceros */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Verifica-Lista-Terceros V-table-Win 
PROCEDURE Verifica-Lista-Terceros :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
