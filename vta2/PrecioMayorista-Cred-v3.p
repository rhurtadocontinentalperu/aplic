...
IF LOOKUP(s-TpoPed, 'R,S,M,MG,P,N,E,VU,LF,LU') = 0 THEN DO:
    IF pError = YES THEN MESSAGE 'El tipo de pedido:' s-TpoPed 'NO cae en las clasificaciones definidas'
...
/* *************************************** */
/* RHC 07/04/2017 CONTRATO MARCO GERENCIAL */
/* *************************************** */
CASE s-TpoPed:
    WHEN "E" THEN DO:
        /* RHC 13/10/2015 EXPOLIBRERIAS */
    ...
    WHEN "MG" THEN DO:
        ASSIGN
            x-ClfCli  = "C"         /* Valores por defecto */
            x-ClfCli2 = "C".
        IF AVAIL gn-clie AND gn-clie.CM_ClfCli_P <> '' THEN x-ClfCli  = gn-clie.CM_ClfCli_P.
        IF AVAIL gn-clie AND gn-clie.CM_ClfCli_T <> '' THEN x-ClfCli2 = gn-clie.CM_ClfCli_T.
    END.
END CASE.
/* ******************************************************************************* */
/* ******************************************************************************* */
...
/* ******************************************************************************* */
/* **************************** DESCUENTOS PORMOCIONALES ************************* */
/* ******************************************************************************* */
PROMOCIONAL:    /* RHC 25/03/2015 SOLO FAMILIA 011 y 013 */
DO:
    ...
    ASSIGN                                           
         ...
         x-ClfCli2 = "C".    
    /* RHC 19/10/2015 EXPOLIBRERIA */
    IF s-TpoPed = "E" THEN x-FlgDtoProm = YES.
END.
...
IF s-TpoPed = "P" AND LOOKUP(Almmmatg.CodFam, "010,012") > 0 THEN DO:
...
IF s-CodMon = 2 AND Almmmatg.CodFam = "011" THEN s-CndVta = "000".   /* Simula Contado */
...
/*  NOTA: En algunos casos afecta el precio en otros es el flete */
