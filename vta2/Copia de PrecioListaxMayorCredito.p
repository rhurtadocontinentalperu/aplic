...
/* **************************************************************************** */
/* SOLO PARA PROVINCIAS Y EXPOLIBRERIA EN TODOS LOS CASOS                       */
/* **************************************************************************** */
/* IF s-TpoPed = "E" THEN          /* EXPOLIBRERIA */        */
/*     ASSIGN                                                */
/*         x-ClfCli  = "C"         /* Valores por defecto */ */
/*         x-ClfCli2 = "C".                                  */
 IF LOOKUP(s-CodDiv, '10015,10060') = 0 AND LOOKUP(Almmmatg.CodFam, "011,013") > 0 THEN  
     ASSIGN                                           
         x-FlgDtoVol = YES                                
         x-FlgDtoProm = NO                                
         x-FlgDtoClfCli = YES                             
         ...
         x-ClfCli2 = "C".                                 
/* IF LOOKUP(s-TpoPed, "P,E") > 0 THEN     /* PROVINCIAS Y EXPOLIBRERIA */  */
/*     ASSIGN                                                               */
...
IF s-CodDiv = "00015" THEN RETURN "ADM-ERROR".  /* NO Expolibreria */
...
IF AVAILABLE gn-convt AND gn-convt.totdias <= 15 THEN DO:
    ...
    DO J = 1 TO 10:
        IF VtaListaPrecios.PromDivi[J] = S-CODDIV 
...
/*             AND TODAY <= Almmmatg.PromFchH[J]        */
/*             AND Almmmatg.PromDto[J] > 0 THEN DO:     */
/*             x-DctoPromocional = Almmmatg.PromDto[J]. */
/*         END.                                         */
/*         END."}                                       */
