...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE :
    ...
    */
    ...
    END.
    RUN gn/p-fchent-v2 (B-CPEDI.CodDoc + ',' + B-CPEDI.NroPed,            /* Pedido Base */
                        TODAY,        /* Fecha Base */
                        STRING(TIME,'HH:MM:SS'),         /* Hora Base */
                        FacCPedi.CodAlm,           /* Almacén Despacho */
                        (IF B-CPEDI.TipVta = "Si" THEN "CR" ELSE B-CPEDI.CodPos),           /* Ubigeo (Código Postal o DepProDistr) o CR (Cliente Recoge) */
                        (IF B-CPEDI.FchEnt = pFchEnt THEN 0 ELSE pNroSKU),
                        (IF B-CPEDI.FchEnt = pFchEnt THEN 0 ELSE pPeso),
                        FacCPedi.CodCli,
                        FacCPedi.CodDiv,           /* División donde se origina el documento */
                        B-CPEDI.CodRef + ',' + B-CPEDI.NroRef,      /* Cotización Base */
...
/*     lHoraTope = '17'.   /* 5pm */                                                       */
/*     lFechaPedido = TODAY.                                                               */
/*     IF FacCPedi.CodDiv = "00024" THEN lHoraTope = "18".     /* Institucionales */       */
...
/*     IF Faccpedi.FchEnt < lFechaPedido THEN Faccpedi.FchEnt = lFechaPedido.              */
    ...
    /* *************************************************************************** */
    /* RHC 26/07/2016 Debe ser aprobado por Logistica                              */
    /* *************************************************************************** */
/*     IF CAN-FIND(FIRST TabGener WHERE TabGener.CodCia = Faccpedi.codcia                 */
/*                 AND TabGener.Clave = "ZG"                                              */
/*                 AND TabGener.Libre_c01 = Faccpedi.codalm    /* Almacén de Despacho */  */
/*                 AND TabGener.Libre_l01 = YES                /* CD */                   */
/*                 NO-LOCK)                                                               */
/*         THEN DO:                                                                       */
/*         ASSIGN                                                                         */
/*             FacCPedi.FlgEst = "WL". /* APROBAR POR PLANIFICACION DE LOGISTICA */       */
/*         DEF BUFFER B-DIVI FOR GN-DIVI.                                                 */
/*         DEF VAR dFchEnt AS DATE NO-UNDO.                                               */
/*         DEF VAR cHora AS CHAR NO-UNDO.                                                 */
/*         FIND B-DIVI WHERE B-DIVI.codcia = Faccpedi.codcia                              */
/*             AND B-DIVI.coddiv = Faccpedi.CodDiv    /* División que Origina la Orden */ */
/*             NO-LOCK.                                                                   */
/*         IF AVAILABLE B-DIVI THEN DO:                                                   */
/*             dFchEnt = FacCPedi.FchEnt.                                                 */
/*             IF B-DIVI.campo-dec[2] > 0 THEN DO:                                        */
/*                 dFchEnt = ADD-INTERVAL (TODAY, INTEGER(B-DIVI.campo-dec[2]), 'days').  */
/*                 IF B-DIVI.campo-char[2] <> '' THEN DO:                                 */
/*                     cHora = REPLACE(STRING(TIME, 'HH:MM'),':','').                     */
/*                     /* Un día mas */                                                   */
/*                     IF cHora > B-DIVI.campo-char[2] THEN dFchEnt = dFchEnt + 1.        */
/*                 END.                                                                   */
/*             END.                                                                       */
/*             ASSIGN FacCPedi.FchEnt = MAXIMUM(FacCPedi.FchEnt, dFChEnt).                */
/*         END.                                                                           */
