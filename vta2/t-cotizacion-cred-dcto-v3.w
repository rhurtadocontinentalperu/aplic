...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE articulo-sin-dscto-alguno B-table-Win 
...
IF NOT (TRUE <> (ITEM.libre_c05 > "")) THEN DO:
    pRetVal = "Articulo con Descuento de ENCARTE/OFERTA(" + ITEM.libre_c05 + ")".
    RETURN.
END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR y-dscto-adm AS DEC.
  DEFINE VAR y-dsco-maximo AS DEC.
  DEFINE VAR x-PreUni AS DEC.
  DEFINE VAR x-Limite AS DEC.
  DEFINE VAR x-Error AS CHAR.
  DEFINE VAR x-valida-tope AS LOG.
  ...
  y-dscto-adm = DEC(ITEM.Por_DSCTOS[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).

  IF Almmmatg.CHR__02 = "T" THEN DO:
        y-dsco-maximo = s-DtoMax-terceros.
  END.
  ELSE DO:
        y-dsco-maximo = s-DtoMax-propios.
  END.
  ...
  /*
     ...
     Ic - 14Nov2019 - Excepcion a la politica general por el jefe de Linea
   */
  ...
  IF AVAILABLE VtaTabla THEN DO:
      IF Almmmatg.CHR__02 = "T" AND VtaTabla.Valor[1] <= 0 THEN DO:
          MESSAGE 'El jefe de Linea configuro, que la LINEA en Prod.Terceros no aplica descuentos : ' Almmmatg.codfam VIEW-AS ALERT-BOX INFORMATION.
          APPLY "ENTRY" TO ITEM.Por_DSCTOS[1].
          RETURN 'ADM-ERROR'.
      END.
      IF Almmmatg.CHR__02 = "P" AND VtaTabla.Valor[2] <= 0 THEN DO:
          MESSAGE 'El jefe de Linea configuro, que la LINEA en Prod.Propios no aplica descuentos : ' Almmmatg.codfam VIEW-AS ALERT-BOX INFORMATION.
          APPLY "ENTRY" TO ITEM.Por_DSCTOS[1].
          RETURN 'ADM-ERROR'.
      END.

      IF Almmmatg.CHR__02 = "T" AND VtaTabla.Valor[1] > 0 THEN y-dsco-maximo = VtaTabla.Valor[1].
      IF Almmmatg.CHR__02 = "P" AND VtaTabla.Valor[2] > 0 THEN y-dsco-maximo = VtaTabla.Valor[1].

      x-valida-tope = NO.
  END.  

  IF x-valida-tope = YES AND y-dscto-adm > y-dsco-maximo THEN DO:

      MESSAGE "El descuento maximo permitido para el articulo " SKIP
              "segun la linea " Almmmatg.CodFam ' es : ' string(y-dsco-maximo,"->>9.99") '%'
            VIEW-AS ALERT-BOX INFORMATION.
    /*
     MESSAGE "Maximo DSCTO.  permitido para el articulo" SKIP
             "para el nivel de Usuario" SKIP
             "para la línea" Almmmatg.CodFam SKIP
             " es : " y-dsco-maximo " %" SKIP
...
/*           END.                                                                               */
  ...
  */
      ...
      RUN vtagn/p-margen-utilidad-v2 (
          ...
          OUTPUT x-Margen,        /* Margen de utilidad */
          OUTPUT x-Limite,        /* Margen mínimo de utilidad */
          OUTPUT x-Error           /* Control de errores: "OK" "ADM-ERROR" */
          ).
      IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
          MESSAGE "El descuento es imcompatible con el MARGEN UTILIDAD" VIEW-AS ALERT-BOX ERROR.
          APPLY "ENTRY" TO ITEM.Por_DSCTOS[1].
          RETURN 'ADM-ERROR'.
      END.
...
/*           END.                                                                               */
      ...
      IF NOT AVAILABLE VtaTabla THEN DO:
          MESSAGE 'Descuento NO configurado para la línea' Almmmatg.codfam VIEW-AS ALERT-BOX ERROR.
          APPLY "ENTRY" TO ITEM.Por_DSCTOS[1].
          RETURN 'ADM-ERROR'.
      END.
      x-DtoMax = VtaTabla.Valor[1].
      IF DEC(ITEM.Por_DSCTOS[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) > x-DtoMax THEN DO:
         MESSAGE "Maximo DSCTO.  permitido PROPIOS" SKIP
                 "para el nivel de Usuario" SKIP
                 "para la línea" Almmmatg.CodFam SKIP
                 " es : " x-DtoMax " %" SKIP
                 VIEW-AS ALERT-BOX ERROR.
...
IF ITEM.libre_c04 begins "HP_CARTUCHOS"  THEN DO:
    MESSAGE "Articulo tiene promocion de HP " VIEW-AS ALERT-BOX ERROR.
    RETURN "ADM-ERROR".
END.
*/
...
x-retval = "".
RUN articulo-sin-dscto-alguno(OUTPUT x-retval).
