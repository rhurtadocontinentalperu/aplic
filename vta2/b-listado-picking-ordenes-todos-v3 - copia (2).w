...
DEF SHARED VAR lMsgRetorno AS CHAR.
...
DEF VAR x-Pinta-Browse AS LOG INIT NO NO-UNDO.  /* Falso ppor defecto */
...
ORDENES.CodDoc ORDENES.NroPed ORDENES.FchPed ORDENES.Hora ORDENES.FchEnt ~
ORDENES.Libre_c01 ORDENES.NomCli FacCPedi.UsrImpOD FacCPedi.FchImpOD ~
ORDENES.Libre_d01 fDestinoFinal() @ x-DestinoFinal ORDENES.Libre_c02 ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      ORDENES.Libre_c01 COLUMN-LABEL "Origen" FORMAT "x(40)":U
            WIDTH 6.29
      ORDENES.NomCli COLUMN-LABEL "Cliente" FORMAT "x(100)":U WIDTH 37.43
...
"FacCPedi.CrossDocking" "CrossDock" "SI/NO" "logical" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"ORDENES.Hora" ? ? "character" ? ? ? ? ? ? no ? no no "4.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   = Temp-Tables.ORDENES.FchEnt
     _FldNameList[7]   > Temp-Tables.ORDENES.Libre_c01
"ORDENES.Libre_c01" "Origen" "x(40)" "character" ? ? ? ? ? ? no ? no no "6.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.ORDENES.NomCli
"ORDENES.NomCli" "Cliente" ? "character" ? ? ? ? ? ? no ? no no "37.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.FchImpOD" "Fecha y!hora de impresión" "99/99/9999 HH:MM" "datetime" ? ? ? ? ? ? no ? no no "13.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > Temp-Tables.ORDENES.Libre_d01
...
"fDestinoFinal() @ x-DestinoFinal" "Destino Final" "x(40)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > Temp-Tables.ORDENES.Libre_c02
"ORDENES.Libre_c02" "ALMACEN" "x(20)" "character" ? ? ? ? ? ? no ? no no "16.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[14]   > Temp-Tables.ORDENES.Libre_c03
...
"ORDENES.Libre_d02" "Items" ">>>9" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[16]   = INTEGRAL.FacCPedi.Glosa
     _FldNameList[17]   > INTEGRAL.FacCPedi.CodRef
"FacCPedi.CodRef" "Cod!Ref" ? "character" ? ? ? ? ? ? no ? no no "4" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[18]   = INTEGRAL.FacCPedi.NroRef
     _FldNameList[19]   > Temp-Tables.ORDENES.ordcmp
...
DO k = 1 TO NUM-ENTRIES(lCodDoc):
    ...
    FOR EACH FacCPedi USE-INDEX Llave08 WHERE faccpedi.codcia = s-codcia AND
            ...
            faccpedi.flgest <> '**' AND
            (faccpedi.fchped >= ltxtDesde AND faccpedi.fchped <= ltxtHasta) NO-LOCK,
        FIRST GN-DIVI OF FacCPedi NO-LOCK:

        IF NOT ((lChequeados = NO AND faccpedi.flgest <> 'A') OR (faccpedi.flgest = 'P' AND faccpedi.flgsit = 'P')) THEN NEXT.
        ...
        IF lOrdenCompra <> '' AND (pSoloImpresos = NO OR lUserImpresion <> '') THEN DO:
            ...
            IF NUM-ENTRIES(Faccpedi.Libre_c02,'|') > 1 
                THEN ORDENES.Libre_c02 = ENTRY(2,Faccpedi.Libre_c02,'|').
            IF NUM-ENTRIES(Faccpedi.Libre_c03,'|') > 1 
                THEN ORDENES.Libre_c03 = ENTRY(2,Faccpedi.Libre_c03,'|').
            FOR EACH facdpedi OF faccpedi NO-LOCK, FIRST almmmatg OF facdpedi NO-LOCK :
                IF almmmatg.pesmat <> ? AND almmmatg.pesmat > 0 THEN DO:
                    ORDENES.Libre_d01 = ORDENES.Libre_d01 + (facdpedi.canped * almmmatg.pesmat).
                END.       
                ELSE DO:
                    ...
                    END.
                END.
                ORDENES.Libre_d02 = ORDENES.Libre_d02 + 1.
            END.
            CASE ENTRY(1,lCodDoc):
...
SESSION:SET-WAIT-STATE('GENERAL').
    ...
    MESSAGE 'Exportación a EXCEL terminada' VIEW-AS ALERT-BOX INFORMATION.
...
DEFINE VAR dtInicio AS DATETIME.
DEFINE VAR dtfin AS DATETIME.

dtInicio = NOW.

GET FIRST {&BROWSE-NAME}.
DO  WHILE AVAILABLE ORDENES:
    FIND FIRST x-faccpedi OF ordenes NO-LOCK.
    FIND FIRST gn-div OF faccpedi NO-LOCK.
    iCount = 0.
    /*
    FOR EACH facdpedi OF x-faccpedi NO-LOCK:
       iCount = iCount + 1.
    END.
    */
    FOR EACH facdpedi OF x-faccpedi NO-LOCK :        
        ...
        chWorkSheet:Range("A" + cColumn):Value = x-faccpedi.coddoc.
        chWorkSheet:Range("B" + cColumn):Value = x-faccpedi.nroped.             
        chWorkSheet:Range("C" + cColumn):Value = x-faccpedi.fchped.             
        chWorkSheet:Range("D" + cColumn):Value = x-faccpedi.hora.
        cRange = "E" + cColumn.
        chWorkSheet:Range(cRange):Value = IF(AVAILABLE gn-divi) THEN gn-divi.desdiv ELSE "".        
        cRange = "F" + cColumn.
        chWorkSheet:Range(cRange):Value = x-faccpedi.nomcli.
        cRange = "G" + cColumn.
        chWorkSheet:Range(cRange):Value = x-faccpedi.usrimpod.
        cRange = "H" + cColumn.
        chWorkSheet:Range(cRange):Value = x-faccpedi.fchimpod.

        cRange = "I" + cColumn.
        chWorkSheet:Range(cRange):Value = IF(NUM-ENTRIES(x-Faccpedi.Libre_c03,'|') > 1) THEN ENTRY(2,x-Faccpedi.Libre_c03,'|') ELSE "".

        cRange = "J" + cColumn.
        chWorkSheet:Range(cRange):Value = iCount.

        cRange = "K" + cColumn.
        chWorkSheet:Range(cRange):Value = x-faccpedi.glosa.


        lsFechaEmision = STRING(x-faccpedi.fchped, '99-99-9999').
        lsHoraEmision  = x-faccpedi.hora.

        ldtFechaEmision      = DATETIME(lsFechaEmision + ' ' + lsHoraEmision).
        ldtFechaDistribucion = DATETIME(IF(NUM-ENTRIES(x-Faccpedi.Libre_c03,'|') > 1) THEN ENTRY(2,x-Faccpedi.Libre_c03,'|') ELSE "").

        RUN lib\_time-passed.p (ldtFechaEmision, x-faccpedi.fchimpod, OUTPUT lsDifTempo_VtaAlm).
        RUN lib\_time-passed.p (x-faccpedi.fchimpod, ldtFechaDistribucion, OUTPUT lsDifTempo_AlmDist).
        RUN lib\_time-passed.p (ldtFechaEmision, ldtFechaDistribucion, OUTPUT lsDifTempo_VtaDist).

        chWorkSheet:Range("L" + cColumn):Value = lsDifTempo_VtaAlm.
        chWorkSheet:Range("M" + cColumn):Value = lsDifTempo_AlmDist.
        chWorkSheet:Range("N" + cColumn):Value = lsDifTempo_VtaDist.
        chWorkSheet:Range("O" + cColumn):Value = x-faccpedi.fchent.
        /*
        lPeso = fPeso().
        lPeso = b-facdpedi.canped * almmmatg.pesmat
        chWorkSheet:Range("P" + cColumn):Value = lpeso.
        */
        ...
        lPeso = facdpedi.canped * almmmatg.pesmat.
        chWorkSheet:Range("P" + cColumn):Value = lpeso.
        ...
        chWorkSheet:Range("AA" + cColumn):Value = "'" + IF(NUM-ENTRIES(x-Faccpedi.Libre_c03,'|') > 2) THEN ENTRY(3,x-Faccpedi.Libre_c03,'|') ELSE "".

         lCodUser = IF(NUM-ENTRIES(x-Faccpedi.Libre_c03,'|') > 2) THEN ENTRY(3,x-Faccpedi.Libre_c03,'|') ELSE "".
         lNomUser = fPersonal(lCodUser).
         cRange = "AB" + cColumn.
         chWorkSheet:Range(cRange):Value = "'" + lNomUser.
...
DEFINE FRAME f-cab
        ...
        {&PRN7A} + {&PRN6B} + "Fec. Despacho : " + STRING(faccpedi.fchent,"99/99/9999") + {&PRN6B} + {&PRN7B} AT 80 FORMAT "X(40)" SKIP
        {&PRN4} + {&PRN6A} + "   Vendedor: " + gn-ven.NomVen + {&PRN6B} FORMAT "X(60)" 
        {&PRN3} + {&PRN7A} + {&PRN6B} + "         Peso : " + STRING(lPeso,">>>,>>9.99") + {&PRN6B} + {&PRN7B} AT 80 FORMAT "X(40)" SKIP
...
DEF VAR x-NomFinal AS CHAR FORMAT 'x(80)' NO-UNDO.
FIND Almacen WHERE Almacen.codcia = s-codcia AND Almacen.codalm = FacCPedi.AlmacenXD NO-LOCK NO-ERROR.
IF AVAILABLE Almacen THEN x-NomFinal = Almacen.Descripcion.
ELSE DO:
    FIND gn-clie WHERE gn-clie.codcia = cl-codcia AND gn-clie.codcli = FacCPedi.AlmacenXD NO-LOCK NO-ERROR.
    IF AVAILABLE gn-clie THEN x-NomFinal = gn-clie.NomCli.
END.
DEFINE FRAME f-cab
        n-Item         FORMAT '>9'
        Reporte.CodMat FORMAT 'X(7)'
...
/*         {&PRN4} + {&PRN6A} + "    Destino: " + Faccpedi.codcli + ' ' + trim(Faccpedi.nomcli) + {&PRN6B} + {&PRN3} FORMAT "X(60)" SKIP */
    {&PRN4} + {&PRN6A} + "     Cross Docking: " + STRING(FacCPedi.CrossDocking, 'SI/NO') + {&PRN6B} + {&PRN3} FORMAT "X(80)" SKIP        
    {&PRN4} + {&PRN6A} + "Destino Intermedio: " + Faccpedi.codcli + ' ' + TRIM(Faccpedi.nomcli) + {&PRN6B} + {&PRN3} FORMAT "X(80)" SKIP        
    {&PRN4} + {&PRN6A} + "     Destino Final: " + FacCPedi.AlmacenXD + ' ' + x-NomFinal + {&PRN6B} + {&PRN3} FORMAT "X(80)" SKIP        
        {&PRN4} + {&PRN6B} + "       Hora: " STRING(TIME,"HH:MM") FORMAT "X(12)" SKIP
...
DEFINE FRAME f-cab
        ...
        {&PRN3} + {&PRN7A} + {&PRN6B} + "     N° PEDIDO: " + Reporte.CodRef + ' ' + Reporte.NroRef + {&PRN6B} + {&PRN7B} + {&PRN3} AT 80 FORMAT "X(40)" SKIP
        {&PRN3} + {&PRN7A} + {&PRN6B} + "N° COMPROBANTE: " + x-Comprobante + {&PRN6B} + {&PRN7B} + {&PRN3} AT 80 FORMAT "X(40)" SKIP
        {&PRN4} + {&PRN6A} + "     Origen: " + TRIM(GN-DIVI.DesDiv) + {&PRN6B} + {&PRN3} FORMAT "X(50)" SKIP
        {&PRN3} + {&PRN7A} + {&PRN6B} + "Fec. Despacho : " + STRING(faccpedi.fchent,"99/99/9999") + {&PRN6B} + {&PRN7B} + {&PRN3} AT 80 FORMAT "X(40)" SKIP
        {&PRN4} + {&PRN6A} + "   Vendedor: " + SUBSTRING(TRIM(gn-ven.NomVen),1,20) + {&PRN6B} + {&PRN3} FORMAT "X(50)"
        {&PRN3} + {&PRN7A} + {&PRN6B} + "         Peso : " + STRING(lPeso,">>>,>>9.99") + {&PRN6B} + {&PRN7B} + {&PRN3} AT 80 FORMAT "X(40)" SKIP
...
DEFINE FRAME f-cab
        ...
        {&PRN3} + {&PRN7A} + {&PRN6B} + "     N° PEDIDO: " + Reporte.CodRef + ' ' + Reporte.NroRef + {&PRN6B} + {&PRN7B} + {&PRN3} AT 80 FORMAT "X(40)" SKIP
        {&PRN3} + {&PRN7A} + {&PRN6B} + "N° COMPROBANTE: " + x-Comprobante + {&PRN6B} + {&PRN7B} + {&PRN3} AT 80 FORMAT "X(40)" SKIP
        {&PRN4} + {&PRN6A} + "     Origen: " + TRIM(GN-DIVI.DesDiv) + {&PRN6B} + {&PRN3} FORMAT "X(50)"
        {&PRN3} + {&PRN7A} + {&PRN6B} + "Fec. Despacho : " + STRING(faccpedi.fchent,"99/99/9999") + {&PRN6B} + {&PRN7B} + {&PRN3} AT 80 FORMAT "X(40)" SKIP
        {&PRN4} + {&PRN6A} + "   Vendedor: " + SUBSTRING(TRIM(gn-ven.NomVen),1,20) + {&PRN6B} + {&PRN3} FORMAT "X(50)"
        {&PRN3} + {&PRN7A} + {&PRN6B} + "         Peso : " + STRING(lPeso,">>>,>>9.99") + {&PRN6B} + {&PRN7B} + {&PRN3} AT 80 FORMAT "X(40)" SKIP
        {&PRN4} + {&PRN6A} + "    Cliente: " + Faccpedi.nomcli + {&PRN6B} + {&PRN3} FORMAT "X(50)" SKIP
...
DEF VAR x-NomFinal AS CHAR NO-UNDO.
FIND Almacen WHERE Almacen.codcia = s-codcia AND Almacen.codalm = FacCPedi.AlmacenXD NO-LOCK NO-ERROR.
IF AVAILABLE Almacen THEN x-NomFinal = Almacen.Descripcion.
ELSE DO:
    FIND gn-clie WHERE gn-clie.codcia = cl-codcia AND gn-clie.codcli = FacCPedi.AlmacenXD NO-LOCK NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Imprimir-Alfabeticamente B-table-Win 
PROCEDURE Imprimir-Alfabeticamente :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  CASE ORDENES.CodDoc:
      WHEN 'O/D' OR WHEN 'O/M' THEN RUN Imprimir-Formato-OD ( INPUT "ALFABETICO").
      WHEN 'OTR' THEN RUN Imprimir-Formato-OTR ( INPUT "ALFABETICO").
  END CASE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Imprimir-por-Zona B-table-Win 
PROCEDURE Imprimir-por-Zona :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    IF NOT AVAILABLE ORDENES THEN RETURN.
    CASE ORDENES.CodDoc:
        WHEN 'O/D' OR WHEN 'O/M' THEN RUN Imprimir-Formato-OD ( INPUT "ZONA").
        WHEN 'OTR' THEN RUN Imprimir-Formato-OTR ( INPUT "ZONA").
    END CASE.
...
GET FIRST {&BROWSE-NAME}.
DO WHILE AVAILABLE faccpedi:
    iCount1 = ORDENES.Libre_d02.
    lPeso = ORDENES.Libre_d01.
    FOR EACH facdpedi OF faccpedi NO-LOCK, FIRST Almmmatg OF Facdpedi NO-LOCK:       
        lsFechaEmision = STRING(faccpedi.fchped, '99-99-9999').
        lsHoraEmision  = faccpedi.hora.
...
MESSAGE 'Exportación a TEXTO terminada' VIEW-AS ALERT-BOX INFORMATION.
        ...
        FOR EACH i-ccbcdocu USE-INDEX llave15 WHERE i-ccbcdocu.codcia = s-codcia AND 
            ...
            IF i-ccbcdocu.coddoc = 'G/R' AND 
                ...
                IF AVAILABLE di-rutaD THEN DO:
                    ...
                    lRetVal = lRetVal + di-rutaD.nrodoc + "(" + STRING(di-rutaC.fchdoc,"99/99/9999") + ")".
                END.
            END.
        END.
    END.
        ...
        FOR EACH almcmov WHERE almcmov.codcia = s-codcia AND 
            ...
            */
            ...
            IF AVAILABLE di-rutaG THEN DO:
                ...
                IF lRetVal <> '' THEN lRetVal = lRetVal + ", ".
                lRetVal = lRetVal + di-rutaG.nrodoc + "(" + STRING(di-rutaC.fchdoc,"99/99/9999") + ")".
            END.
