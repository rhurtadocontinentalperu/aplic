...
/* RHC 12.12.2011 El tipo de pedido se va a clasificar en:
I: Ventas Institucionales
S: Ventas Supermercados
P: Ventas Provincias
M: Ventas Tiendas Mayoristas
NXT: Cotizaciones por Nextel
*/
...
ASSIGN
    s-TpoPed = pTpoPed.     /* OJO */
s-Import-IBC = (IF s-TpoPed = "S" THEN YES ELSE NO).    /* Supermercados */
/* ***************************** */
...
BTN-Excel Btn-Grupos Btn-Super BTN-CISSAC 
...
DEFINE VARIABLE h_t-cotiza-inst AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotiza-inst AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     Btn-Super AT ROW 23.62 COL 57 WIDGET-ID 8
     BTN-CISSAC AT ROW 23.62 COL 85 WIDGET-ID 10
...
&Scoped-define SELF-NAME BTN-CISSAC
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BTN-CISSAC W-Win
ON CHOOSE OF BTN-CISSAC IN FRAME F-Main /* Importar O/C CISSAC */
DO:
    IF s-Import-Ibc = YES THEN RETURN NO-APPLY.
    IF s-adm-new-record = 'NO' THEN RETURN NO-APPLY.
  RUN Importar-entre-companias IN h_v-cotiza-inst.
  RUN dispatch IN h_t-cotiza-inst ('open-query':U).
END.
...
DO:
    ...
    *******/
    ...
    CASE s-coddiv :
        WHEN '00023' OR WHEN '00024' THEN RUN Genera-Excel-Utilex IN h_v-cotiza-inst (INPUT lchoice) .
        OTHERWISE RUN Genera-Excel2 IN h_v-cotiza-inst (INPUT lchoice) .
    END CASE.
...
DO:
    ...
    RUN dispatch IN h_t-cotiza-inst ('open-query').
END.
...
&Scoped-define SELF-NAME Btn-Super
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn-Super W-Win
ON CHOOSE OF Btn-Super IN FRAME F-Main /* IMPORTAR SUPERMERCADOS */
DO:
    IF s-adm-new-record = 'NO' THEN RETURN NO-APPLY.
  RUN Asigna-Supermercado IN h_v-cotiza-inst.
  RUN dispatch IN h_t-cotiza-inst ('open-query':U).
END.
...
CASE s-TpoPed:
    WHEN "I" THEN {&WINDOW-NAME}:TITLE = "COTIZACIONES MAYORISTA - INSTITUCIONALES".
    WHEN "S" THEN {&WINDOW-NAME}:TITLE = "COTIZACIONES MAYORISTA - SUPERMERCADOS".
    WHEN "M" THEN {&WINDOW-NAME}:TITLE = "COTIZACIONES MAYORISTA - CONTRATO MARCO".
END CASE.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-cotiza-inst.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotiza-inst ).
       RUN set-position IN h_v-cotiza-inst ( 2.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.08 , 112.00 ) */
       ...
       /* Links to SmartViewer h_v-cotiza-inst. */
       RUN add-link IN adm-broker-hdl ( h_p-updv01 , 'TableIO':U , h_v-cotiza-inst ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-cotiza-inst ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv01 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotiza-inst ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido ,
             BTN-CISSAC:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-cotiza-inst.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotiza-inst ).
       RUN set-position IN h_t-cotiza-inst ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotiza-inst ( 12.31 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotiza-inst. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotiza-inst ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotiza-inst ,
             BTN-Excel:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotiza-inst , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-12 COMBO-NroSer B-CCTE BUTTON-COT BTN-Excel Btn-Grupos Btn-Super 
         BTN-CISSAC 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN 'IBC' THEN DO:
        RUN IBC-Diferencias IN h_t-cotiza-inst.
    END.
    WHEN "browse" THEN DO:
          IF h_b-cotiza <> ? THEN RUN dispatch IN h_b-cotiza ('open-query':U).
          IF h_t-cotiza-inst <> ? THEN RUN dispatch IN h_t-cotiza-inst ('open-query':U).
      END.
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         Btn-Super:VISIBLE = NO.
         Btn-Cissac:VISIBLE = NO.
    ...
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         CASE s-TpoPed:
             WHEN "S" THEN DO:      /* Supermercados */
                 Btn-Grupos:VISIBLE = NO.
                 Btn-Super:VISIBLE  = YES.
             END.
             WHEN "I" THEN DO:      /* Institucionales */
                 Btn-Grupos:VISIBLE = YES.
                 Btn-Cissac:VISIBLE = YES.
             END.
         END CASE.
      END.
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-cotiza-inst <> ? THEN RUN Recalcular-Precios IN h_t-cotiza-inst.
      END.
    WHEN "Disable-Head" THEN DO:
        RUN dispatch IN h_p-updv01 ('disable':U).
        Btn-Grupos:SENSITIVE = NO.
        Btn-Super:SENSITIVE = NO.
        Btn-Cissac:SENSITIVE = NO.
      END.
    WHEN "Enable-Head" THEN DO:
        RUN dispatch IN h_p-updv01 ('enable':U).
        Btn-Grupos:SENSITIVE = YES.
        Btn-Super:SENSITIVE = YES.
        Btn-Cissac:SENSITIVE = YES.
      END.
END CASE.
