...
/*     CcbDTerm.CodDoc = cCodDoc AND                                      */
/*     CcbDTerm.CodTer = s-codter NO-LOCK NO-ERROR.                       */
/* IF NOT AVAILABLE ccbdterm THEN DO:                                     */
/*     MESSAGE "DOCUMENTO" cCodDoc "NO ESTA CONFIGURADO EN ESTE TERMINAL" */
/*         VIEW-AS ALERT-BOX ERROR.                                       */
/*     RETURN ERROR.                                                      */
/* END.                                                                   */
...
/* IF NOT AVAILABLE FacDocum OR FacDocum.CodMov = 0 THEN DO:                          */
/*     MESSAGE                                                                        */
/*         "Codigo de Documento" cCodDoc "no configurado en el maestro de documentos" */
/*         VIEW-AS ALERT-BOX ERROR.                                                   */
/*     RETURN ERROR.                                                                  */
/* END.                                                                               */
/* cCodMov = FacDocum.CodMov.                                                         */
...
/*     MESSAGE                                                                        */
/*         "Codigo de Documento" cCodDoc "no configurado o desactivado" SKIP          */
/*         "en el control de correlativos"                                            */
/*         VIEW-AS ALERT-BOX ERROR.                                                   */
/*     RETURN ERROR.                                                                  */
/* END.                                                                               */
...
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-Guias COMBO-NroSer-Guia ~
...
DEFINE VARIABLE COMBO-BOX-Guias AS CHARACTER FORMAT "X(256)":U INITIAL "NO" 
...
/* SETTINGS FOR COMBO-BOX COMBO-BOX-Guias IN FRAME gDialog
   NO-ENABLE                                                            */
/* SETTINGS FOR COMBO-BOX COMBO-NroSer IN FRAME gDialog
   ALIGN-L                                                              */
/* SETTINGS FOR COMBO-BOX COMBO-NroSer-Guia IN FRAME gDialog
   NO-ENABLE                                                            */
...
DO:
    ...
    ELSE DO:
        MESSAGE
            "Se ha(n) generado" iCountGuide "Factura(s)" SKIP
            "Prepare los formatos para la impresión"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        RUN Imprimir-Comprobantes.
    END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL COMBO-BOX-Guias gDialog
ON VALUE-CHANGED OF COMBO-BOX-Guias IN FRAME gDialog
DO:
  ASSIGN {&self-name}.
...
------------------------------------------------------------------------------*/
  DISPLAY COMBO-BOX-Guias COMBO-NroSer-Guia FILL-IN-NroPed FILL-IN-Cliente 
          FILL-IN-DirClie COMBO-NroSer FILL-IN-NroDoc FILL-IN-items 
    ...
    DO TRANSACTION ON ERROR UNDO trloop, RETURN 'ADM-ERROR' ON STOP UNDO trloop, RETURN 'ADM-ERROR':
        ...
        FOR EACH PEDI WHERE PEDI.CanAte > 0,
            ...
            ASSIGN
                ...
                CcbDDocu.ImpDcto_Adelanto[4] = PEDI.Libre_d02.
            ASSIGN
            ...
            IF iCountItem > FILL-IN-items OR LAST-OF(PEDI.CodCia) THEN DO:
                ...
                END.
                /* FIN DE REDONDEO */
                /* RHC 12.07.2012 limipamos campos para G/R */
                ASSIGN
                    Ccbcdocu.codref = ""
                    Ccbcdocu.nroref = "".
                ...
                RUN sunat\progress-to-ppll ( INPUT ROWID(Ccbcdocu), OUTPUT pMensaje ) NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Imprimir-Comprobantes gDialog 
PROCEDURE Imprimir-Comprobantes :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* del nuevo sistema
  ...
  END.
*/
/* del antiguo sistema de facturacion
    CASE Ccbcdocu.coddoc:
        WHEN 'FAC' THEN DO:
            CASE s-coddiv:
                WHEN '00002' THEN RUN vtamay/r-fac00002-1anda (ROWID(ccbcdocu)). /*Andahuaylas*/
                WHEN '00014' THEN RUN vtamay/r-fac00002-hori (ROWID(ccbcdocu)).  /*Horizontal*/
                WHEN '00026' THEN RUN VTA/R-IMPFAC2-2 (ROWID(CcbCDocu)).  /*Horizontal*/
                OTHERWISE RUN vtamay/r-fac00002-1 (ROWID(ccbcdocu)).
            END CASE.
        END.
        WHEN 'BOL' THEN DO:
            RUN vtamay/R-BOLE01-1 (ROWID(CcbCDocu)).    
        END.
    END CASE.
*/
...
/*           END.                                                  */
    ...
    ASSIGN
        COMBO-NroSer:FORMAT = TRIM(ENTRY(1,x-Formato,'-'))
      ...
      IF AVAILABLE FacCorre THEN
          FILL-IN-NroDoc =
              ...
              STRING(FacCorre.Correlativo,ENTRY(2,x-Formato,'-')).
/*       cListItems = STRING(CcbDTerm.NroSer, '999').                                             */
/*       ASSIGN                                                                                   */
/*           COMBO-NroSer:LIST-ITEMS = cListItems                                                 */
/*           COMBO-NroSer = ENTRY(1,COMBO-NroSer:LIST-ITEMS)                                      */
/*           COMBO-NroSer:LABEL = 'SERIE DE ' + (IF cCodDoc = 'FAC' THEN 'FACTURA' ELSE 'BOLETA') */
