            ...
            IF lColumnDataNew > 0 THEN DO:
                ...
                IF s-FlgEmpaque = YES THEN DO:
                    ...
                    DO:
                        ...
                        IF f-MinimoVentas <= 0 THEN f-MinimoVentas = f-CanPed.  /*1.*/
                    ...
                    IF f-CanPed <> DECIMAL(SELF:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) THEN DO:
                        lColumnDataNew = f-CanPed.
                        SELF:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = STRING(f-CanPed).
                        /*MESSAGE 'Se ha ajustado la cantidad de acuerdo al empaque' VIEW-AS ALERT-BOX WARNING.*/
                    END.
                END.
                */
/*                 IF Almmmatg.Libre_d03 > 0 AND s-FlgEmpaque = YES THEN DO:                             */
/*                     f-CanPed = (TRUNCATE((f-CanPed / Almmmatg.Libre_d03),0) * Almmmatg.Libre_d03).    */
/*                     IF f-CanPed <> lColumnDataNew * f-Factor THEN DO:                                 */
/*                         MESSAGE 'Solo puede vender en empaques de' Almmmatg.Libre_d03 Almmmatg.UndBas */
/*                             VIEW-AS ALERT-BOX ERROR.                                                  */
/*                         RETURN NO-APPLY.                                                              */
/*                     END.                                                                              */
/*                 END.                                                                                  */
...
PROCEDURE Graba-linea:
    ...
    ASSIGN
        ...
        .
    FIND ExpoGrpo WHERE ExpoGrpo.CodCia = s-codcia
    ...
    IF AVAILABLE ttExpoTmpo THEN DO:
        FIND w-report WHERE w-report.task-no = s-task-no
            ...
            NO-LOCK NO-ERROR.
        IF AVAILABLE w-report AND
            TRUE <> (PEDI2.Libre_c03 > '')      /* Solo si no tiene promotor asignado */
  ...
  FOR EACH x-vtatabla WHERE x-vtatabla.codcia = s-codcia AND
    ...
    /*DO lSec = 1 TO 3:*/
      ...
      */
      ...
      FOR EACH PEDI2 WHERE NUM-ENTRIES(PEDI2.Libre_C03,'|') = 4:
          IF ENTRY(1,PEDI2.Libre_C03,'|') = s-codpro
              AND ENTRY(4,PEDI2.Libre_C03,'|') = w-report.Campo-C[10]
              THEN DO:
              ASSIGN
                  w-report.Campo-C[3] = PEDI2.Libre_C03.
              ...
              IF AVAILABLE VtaTabla 
                  ...
                  w-report.Campo-C[3] = s-codpro + '|' + VtaTabla.Llave_c2 + '|' + VtaTabla.Libre_c02 + '|' + w-report.Campo-C[10].
              LEAVE.
          END.
      END.
  END.
