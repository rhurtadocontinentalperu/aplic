...
FacCPedi.FchEnt fPeso() @ x-peso FacCPedi.UsrImpOD FacCPedi.FchImpOD ~
fAlmacen() @ Faccpedi.Libre_c02 fDistribucion() @ Faccpedi.Libre_c03 ~
fNroItm() @ x-NroItm FacCPedi.Glosa 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.NomCli COLUMN-LABEL "Cliente" FORMAT "x(50)":U WIDTH 30.43
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 151 BY 7.5
...
"FacCPedi.NomCli" "Cliente" ? "character" ? ? ? ? ? ? no ? no no "30.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   = INTEGRAL.FacCPedi.FchEnt
     _FldNameList[8]   > "_<CALC>"
"fPeso() @ x-peso" "Peso KG" ">>>,>>9.99" ? ? ? ? ? ? ? no ? no no "8.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"fAlmacen() @ Faccpedi.Libre_c02" "ALMACEN" "x(20)" ? ? ? ? ? ? ? no ? no no "15.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  END.
  ASSIGN
      FacCPedi.Libre_c03 = s-user-id + '|' + STRING(DATETIME(TODAY, MTIME), '99/99/9999 HH:MM:SS').
