...
&SCOPED-DEFINE Condicion2 ((lChequeados = NO AND faccpedi.flgest <> 'A') ~
    ...
    AND (pImpresos = 1 ~
         OR (pImpresos = 2 AND ENTRY(1, Faccpedi.Libre_c02, '|') <> '') ~
         OR (pImpresos = 3 AND (ENTRY(1, Faccpedi.Libre_c02, '|') = '' OR ENTRY(1, Faccpedi.Libre_c02, '|') = ?))) ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      ORDENES.NroPed COLUMN-LABEL "Numero" FORMAT "X(12)":U WIDTH 9.57
...
FOR EACH FacCPedi WHERE {&Condicion} USE-INDEX Llave09 NO-LOCK, FIRST GN-DIVI OF FacCPedi NO-LOCK:
    lOrdenCompra = "*".
    IF pOrdenCompra <> '' THEN lOrdenCompra = fOrdenCompra(INPUT FacCPedi.NroRef, INPUT pOrdenCompra).
...
/*         lPeso = fPeso(). */
        ...
        END.
        chWorkSheet:Range("Z" + cColumn):VALUE = (FacDPedi.CanPed * FacDPedi.Factor * Almmmatg.Pesmat ).
  ...
  DO c-Copias = 1 TO s-nro-copias ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
      ...
      CASE TRUE:
          ...
          WHEN pOrden = "EMPAQUE" THEN DO: 
              ...
              IF {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME} > 1 
                  THEN RUN Formato-Rb-Multi-Resumen2.
              RUN dispatch IN THIS-PROCEDURE ('open-query':U).
          END.
      END CASE.
      PAGE STREAM REPORT.
