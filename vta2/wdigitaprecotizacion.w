...
DEFINE VARIABLE h_bdigitaprecotizaciondetalle AS HANDLE NO-UNDO.
DEFINE VARIABLE h_bdigitaprecotizacionprov AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 147.86 BY 24.85 WIDGET-ID 100.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'bdigitaprecotizacionprov.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bdigitaprecotizacionprov ).
       RUN set-position IN h_bdigitaprecotizacionprov ( 2.81 , 2.14 ) NO-ERROR.
       RUN set-size IN h_bdigitaprecotizacionprov ( 8.15 , 74.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'bpagcatvtas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bpagcatvtas ).
       RUN set-position IN h_bpagcatvtas ( 2.81 , 77.00 ) NO-ERROR.
       RUN set-size IN h_bpagcatvtas ( 8.08 , 55.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'bdigitaprecotizaciondetalle.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bdigitaprecotizaciondetalle ).
       RUN set-position IN h_bdigitaprecotizaciondetalle ( 11.96 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bdigitaprecotizaciondetalle ( 13.46 , 137.00 ) NO-ERROR.

       /* Links to SmartBrowser h_bpagcatvtas. */
       RUN add-link IN adm-broker-hdl ( h_bdigitaprecotizacionprov , 'Record':U , h_bpagcatvtas ).

       /* Links to SmartBrowser h_bdigitaprecotizaciondetalle. */
       RUN add-link IN adm-broker-hdl ( h_bpagcatvtas , 'Record':U , h_bdigitaprecotizaciondetalle ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bdigitaprecotizacionprov ,
             x-importe:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_bpagcatvtas ,
             h_bdigitaprecotizacionprov , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_bdigitaprecotizaciondetalle ,
             h_bpagcatvtas , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
    ...
    CASE pParameter:
        ...
        WHEN 'Open-Query-3' THEN DO:
            RUN dispatch IN h_bdigitaprecotizaciondetalle ('open-query').
        END.
    END CASE.
END PROCEDURE.
