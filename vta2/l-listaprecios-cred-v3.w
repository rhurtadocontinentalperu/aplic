...
AND (F-marca = '' OR Almmmatg.DesMar BEGINS F-marca) ~
...
_PrecioVenta() @ Almmmatg.PreOfi _UnidadVenta() @ Almmmatg.UndStk ~
Almmmate.StkAct ~
...
Margen|||INTEGRAL.Almmmatg.Dec__01|no
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "':U + 'Codigo,Margen' + '",
...
"_PrecioVenta() @ Almmmatg.PreOfi" "Precio Venta!S/." ? ? ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > "_<CALC>"
"_UnidadVenta() @ Almmmatg.UndStk" "Unidad!Venta" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > INTEGRAL.Almmmate.StkAct
...
DO:
    ...
    RUN PRI_Valida-Margen-Utilidad IN hProc (INPUT pCodDiv,     /*s-CodDiv,*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         ...
         END.
         WHEN 'Margen':U THEN DO:
           &Scope SORTBY-PHRASE BY Almmmatg.Dec__01 DESCENDING
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
