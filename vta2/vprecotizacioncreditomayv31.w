...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Importar pre-pedidos móvil
------------------------------------------------------------------------------*/
...
DO WITH FRAME {&FRAME-NAME}:
    IF VtaCDocu.CodRef:SCREEN-VALUE = "PPX" THEN RETURN.
    ...
    FOR EACH Facdpedi OF Faccpedi NO-LOCK BY Facdpedi.NroItm:
        ...
        BUFFER-COPY FacDPedi 
            ...
            ASSIGN 
                ...
                /*PEDI2.Libre_c03 = '51135890'.   /* Standford */ Ya viene el proveedor y promotor*/
/*         FIND AlmCatVtaD WHERE AlmCatVtaD.CodCia = s-codcia       */
/*             AND AlmCatVtaD.CodDiv = pCodDiv                      */
/*             AND AlmCatVtaD.codmat = PEDI2.codmat                 */
/*             NO-LOCK NO-ERROR.                                    */
/*         IF AVAILABLE AlmCatVtaD THEN                             */
/*             ASSIGN                                               */
/*             PEDI2.Libre_c03 = AlmCatVtaD.CodPro                  */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE tomar-pedido-tablet V-table-Win 
PROCEDURE tomar-pedido-tablet :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    FIND LAST Faccpedi WHERE Faccpedi.codcia = s-codcia
        ...
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Faccpedi THEN DO:
        MESSAGE "No existe pedido WEB disponible".
        RETURN.
    END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME} :
      ...
      IF VtaCDocu.CodCli:SCREEN-VALUE = "" THEN DO:
         ...
         RETURN "ADM-ERROR".   
      END.
/*       RUN vtagn/p-clie-expo (VtaCDocu.CodCli:SCREEN-VALUE , s-tpoped, pCodDiv). */
/*       IF RETURN-VALUE = "ADM-ERROR" THEN RETURN "ADM-ERROR".                    */
