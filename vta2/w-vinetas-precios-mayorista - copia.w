...
DO:
    ...
    ASSIGN
        fill-in-codmat FILL-IN-file TOGGLE-Promocional. /*rs-modimp  txtCuantos.*/

    /*RUN Imprime-Etiqueta4.*/

    /*
    IF TOGGLE-Promocional = YES THEN DO:
        /* Formato RETAIL (Utilex) */
        RUN generar-data-precio-retail.
    END.
    ELSE DO:
        /* Formato MAYORISTA / RETAIL */
        RUN generar-data-precio.
    END.
    */
...
/*
    TOGGLE-Promocional = Considerar Descuento Promocional
*/
...
------------------------------------------------------------------------------*/
  DISPLAY fill-in-codmat FILL-IN-file 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE fill-in-codmat BUTTON-5 FILL-IN-file btn-print BUTTON-8 
      WITH FRAME F-Main IN WINDOW W-Win.
    ...
    FOR EACH tt-articulos NO-LOCK:
        ...
        /*IF gn-divi.CanalVenta = "MIN" THEN DO:*/
            ...
            */
            ...
            IF almmmatg.MonVta = 2 THEN dPreUni = almmmatg.PreOfi * almmmatg.TpoCmb.
            ELSE dPreUni = almmmatg.PreOfi.
            ...
            */
            ...
            IF almmmatg.DtoVolR[1] > 0 THEN DO:
                ...
                dDstoVo1 = ROUND(almmmatg.PreOfi * ( 1 - ( DECI(almmmatg.DtoVolD[1]) / 100 ) ),4).
                IF almmmatg.MonVta = 2 THEN dPrecio = dDstoVo1 * almmmatg.TpoCmb.
                ELSE dPrecio = dDstoVo1.                
                x-DtoVol  = 'x ' + STRING(almmmatg.DtoVolR[1],'>>9') + ' ' + Almmmatg.undbas + '= S/' + STRING(dPrecio,">>9.99") + ' c/u'. 
            END.
            IF almmmatg.DtoVolR[2] > 0 THEN DO:
                dDstoVo2 = ROUND(almmmatg.PreOfi * ( 1 - ( DECI(almmmatg.DtoVolD[2]) / 100 ) ),4).
                IF almmmatg.MonVta = 2 THEN dPrecio = dDstoVo2 * almmmatg.TpoCmb.
                ELSE dPrecio = dDstoVo2.                
                x-DtoVo2  = 'x ' + STRING(almmmatg.DtoVolR[2],'>>9') + ' ' + Almmmatg.undbas + '= S/' + STRING(dPrecio,">>9.99") + ' c/u'. 
            END.
        /*END.*/
        ASSIGN 
...
------------------------------------------------------------------------------*/
    ...
    x-file-zpl = "d:\tmp\huancayo" + "-pruebas.txt".
    ...
    FOR EACH tt-articulos NO-LOCK:
        ...
        IF NOT (TRUE <> (almmmatg.undB > "")) THEN DO:
            ...
            */
                ...
                RUN gn/factor-porcentual-flete-v2(INPUT s-coddiv, INPUT almmmatg.codmat, 
            ...
            ELSE DO:
                ...
                x-implin = ROUND(x-canped * f-PreVta, 2). 
