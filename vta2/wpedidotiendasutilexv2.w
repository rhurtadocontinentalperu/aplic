...
DEFINE VARIABLE h_vpedidotiendasutilexv2 AS HANDLE NO-UNDO.
...
DO:
  ...
  RUN Cancelar-Pedido IN h_vpedidotiendasutilexv2.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/vpedidotiendasutilexv2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vpedidotiendasutilexv2 ).
       RUN set-position IN h_vpedidotiendasutilexv2 ( 2.81 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.12 , 131.00 ) */
       ...
       /* Links to SmartViewer h_vpedidotiendasutilexv2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_vpedidotiendasutilexv2 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_vpedidotiendasutilexv2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vpedidotiendasutilexv2 ,
             BUTTON-Canc:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido-4 ,
             h_vpedidotiendasutilexv2 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/tpedidotiendasutilex.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
...
/*       END.                                            */
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_vpedidotiendasutilexv2 <> ? THEN RUN Recalcular-Precios IN h_vpedidotiendasutilexv2.
