...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almmmatg.UndBas COLUMN-LABEL "Unidad" FORMAT "X(6)":U WIDTH 5.72
  ENABLE
      VtaDTabla.LlaveDetalle
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 79 BY 9.23
         FONT 4
         TITLE "ARTICULOS EXCEPTOS DE PROMOCION" FIT-LAST-COLUMN.
...
DEFINE FRAME F-Main
     ...
     BUTTON-3 AT ROW 1 COL 80 WIDGET-ID 2
...
"Almmmatg.UndBas" "Unidad" "X(6)" "character" ? ? ? ? ? ? no ? no no "5.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* ARTICULOS EXCEPTOS DE PROMOCION */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* ARTICULOS EXCEPTOS DE PROMOCION */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* ARTICULOS EXCEPTOS DE PROMOCION */
DO:
...
DO:
  RUN vta2/dpromonoarticulos (OUTPUT TABLE Promocion).
  FIND FIRST Promocion NO-LOCK NO-ERROR.
  IF NOT AVAILABLE Promocion THEN RETURN NO-APPLY.
  FOR EACH VtaDTabla OF VtaCTabla WHERE VtaDTabla.Tipo = "XM":
      DELETE VtaDTabla.
  END.
  FOR EACH Promocion:
      CREATE VtaDTabla.
...
IF RETURN-VALUE = 'YES' THEN DO:
    IF CAN-FIND(FIRST Vtadtabla OF Vtactabla WHERE VtaDTabla.Tipo = 'XM'
