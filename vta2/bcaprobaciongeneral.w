...
DO k = 1 TO {&browse-name}:NUM-SELECTED-ROWS:
    ...
    /* ********************************************************* */
    /* Rutina de APROBACION y GENERACION de O/Ds y OTROS */
    /* ********************************************************* */
    RUN MASTER-TRANSACTION (OUTPUT pMensaje).
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        pMensaje = 'ERROR al momento de aprobar el ' + Faccpedi.CodDoc + ' ' + Faccpedi.NroPed + CHR(10) +
            (IF pMensaje > '' THEN 'ERROR: ' + pMensaje ELSE '').
        MESSAGE pMensaje VIEW-AS ALERT-BOX WARNING.
    END.
    /* ********************************************************* */
    FIND Faccpedi WHERE ROWID(Faccpedi) = rRowid NO-LOCK NO-ERROR.
END.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FIND gn-divi WHERE gn-divi.codcia = COTIZACION.codcia AND gn-divi.coddiv = COTIZACION.coddiv NO-LOCK.
    ...
    CASE TRUE:
        WHEN Faccpedi.CrossDocking = NO THEN DO:
            ...
            RUN VTA_Genera-OD-v2 IN hProc ( pRowid, OUTPUT pComprobante, OUTPUT pMensaje).
