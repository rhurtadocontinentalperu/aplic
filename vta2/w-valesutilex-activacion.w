...
tmp-CcbDDocu.ImpPro tmp-CcbDDocu.ImpDto2 
...
TxtSerie txtNroFactura txtBruto txtIgv txt-nomcli txt-codcli ~
txtGuiaRemision txtFecha txtTotalVta 
...
DEFINE VARIABLE txtGuiaRemision AS CHARACTER FORMAT "X(25)":U 
     LABEL "Guia de remision emitida" 
     VIEW-AS FILL-IN 
     SIZE 36 BY 1
     BGCOLOR 15 FGCOLOR 9 FONT 11 NO-UNDO.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 98.43 BY 9.73 FIT-LAST-COLUMN.
...
DEFINE FRAME F-Main
     ...
     txtGuiaRemision AT ROW 6.5 COL 36 COLON-ALIGNED WIDGET-ID 96
     BROWSE-4 AT ROW 8.15 COL 1.57 WIDGET-ID 200
     txtFecha AT ROW 18.12 COL 6.86 COLON-ALIGNED WIDGET-ID 94
     btnAprobar AT ROW 18.12 COL 25.29 WIDGET-ID 88
     txtTotalVta AT ROW 18.27 COL 59.43 COLON-ALIGNED WIDGET-ID 86
     "Activacion de los VALES UTILEX" VIEW-AS TEXT
          SIZE 43 BY .96 AT ROW 1.04 COL 31.43 WIDGET-ID 90
          FGCOLOR 9 FONT 9
     RECT-1 AT ROW 7.69 COL 1 WIDGET-ID 76
     RECT-2 AT ROW 2.08 COL 1 WIDGET-ID 92
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1.04
         SIZE 109.86 BY 18.88 WIDGET-ID 100.
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-4 txtGuiaRemision F-Main */
...
/* SETTINGS FOR FILL-IN txtFecha IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN txtGuiaRemision IN FRAME F-Main
   NO-ENABLE                                                            */
...
"tmp-CcbDDocu.ImpLin" ? ? "decimal" ? ? ? ? ? ? no ? no no "11.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.tmp-CcbDDocu.ImpPro
"tmp-CcbDDocu.ImpPro" "Desde" "->>>,>>>,>>9" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.tmp-CcbDDocu.ImpDto2
"tmp-CcbDDocu.ImpDto2" "Hasta" "->>>,>>>,>>9" "decimal" ? ? ? ? ? ? no ? no no "12.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-4 */
&ANALYZE-RESUME
  ...
  */
  ...
  FOR EACH ccbddocu OF ccbcdocu NO-LOCK:
      ...
      IF lPosi  > 0 THEN DO:
          ...
          ASSIGN  tmp-ccbddocu.impPro = ccbddocu.impdcto_adelanto[1]
                  tmp-ccbddocu.impDto2 = ccbddocu.impdcto_adelanto[2]
                  tmp-ccbddocu.almdes = cDenominacion.

          lSuma = lSuma + (tmp-ccbddocu.candes * tmp-ccbddocu.preuni).
...
ON CHOOSE OF btnAprobar IN FRAME F-Main /* Activacion de Vales */
DO:
  ...
  MESSAGE 'Seguro de Activar los vales de la FACTURA?' VIEW-AS ALERT-BOX QUESTION
...
------------------------------------------------------------------------------*/
  DISPLAY cboProvProd txt-codpro txtProd txtTotal TxtSerie txtNroFactura 
          txtBruto txtIgv txt-nomcli txt-codcli txtGuiaRemision txtFecha 
          txtTotalVta 
...
DEFINE VAR cDenominacion AS CHAR.
DEFINE VAR lNroVale AS INT.
DEFINE VAR cNroVale AS CHAR.
...
FOR EACH tmp-ccbddocu :
    REPEAT lNroVale = tmp-ccbddocu.imppro TO tmp-ccbddocu.impdto2 :

        cNroVale = STRING(lNroVale,"999999999").
        ...
        FIND FIRST vtatabla WHERE vtatabla.codcia = s-codcia AND 
                                ...
                                vtatabla.llave_c2 = txt-CodPro AND
                                vtatabla.llave_c3 = txtProd AND 
                                vtatabla.llave_c4 = tmp-ccbddocu.Almdes AND 
                                vtatabla.llave_c5 = cNroVale NO-ERROR.
        IF AVAILABLE vtatabla THEN DO:
            ASSIGN vtatabla.libre_c01 = txt-codcli
                    vtatabla.libre_c02 = cLogMov.
                    vtatabla.libre_c03 = xCodDoc + "|" + lNroDcto.
        END.
    END.
END.
...
FIND FIRST ccbcdocu WHERE ccbcdocu.codcia = s-codcia AND 
                        ...
                        ccbcdocu.flgest <> 'A' NO-ERROR.
IF AVAILABLE ccbcdocu THEN DO:
    cLogMov = S-USER-ID + "|" + STRING(TODAY,"99/99/9999") + "|" + STRING(TIME,"HH:MM:SS").
    ASSIGN ccbcdocu.nroast = cLogMov.
...
FOR EACH ccbddocu OF ccbcdocu NO-LOCK : 
    ...
    IF AVAILABLE tmp-ccbddocu THEN DO:
        ...
        FIND FIRST vtatabla WHERE vtatabla.codcia = s-codcia AND 
                                ...
                                vtatabla.llave_c4 = cDenominacion NO-ERROR.
        IF AVAILABLE vtatabla THEN DO:
...
FIND FIRST b-ccbcdocu WHERE ROWID(b-ccbcdocu) = rRowId2 NO-ERROR.
...
DISPLAY "" @ txtGuiaRemision WITH FRAME {&FRAME-NAME}.
...
FIND FIRST x-ccbcdocu WHERE x-ccbcdocu.codcia = s-codcia AND
                                ...
                                x-ccbcdocu.nroref = ccbcdocu.nrodoc
                                NO-LOCK NO-ERROR.

IF AVAILABLE x-ccbcdocu THEN DO:
    DISPLAY x-ccbcdocu.coddoc + "-" + x-ccbcdocu.nrodoc @ txtGuiaRemision WITH FRAME {&FRAME-NAME}.
END.


IF ccbcdocu.flgest <> 'P' THEN DO:
    MESSAGE "La factura aun no esta APROBADA".
    RETURN "ADM-ERROR".
END.
IF ccbcdocu.sede = '' OR ccbcdocu.sede = ? THEN DO:
    /* Estos campos no deben estar vacios si la factura esta aprobada */
    MESSAGE "La factura tiene una INCONSISTENCIA...Comunicar a Sistemas".
    RETURN "ADM-ERROR".
END.

IF ccbcdocu.nroast <> ? AND ccbcdocu.nroast <> '' THEN DO:
    /* Este campo debe estar vacio si aun no estan ACTIVADOS los vales */
    MESSAGE "Ya se activaron los vales de esta FACTURA".
    RETURN "ADM-ERROR".
END.
