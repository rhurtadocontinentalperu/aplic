...
DEFINE VARIABLE h_bdetcatvta_prov-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_blistaprov_prov-1 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'blistaprov_prov-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_blistaprov_prov-1 ).
       /* Position in AB:  ( 2.81 , 2.14 ) */
       /* Size in UIB:  ( 8.15 , 74.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'bdetcatvta_prov-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bdetcatvta_prov-1 ).
       RUN set-position IN h_bdetcatvta_prov-1 ( 11.96 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bdetcatvta_prov-1 ( 13.46 , 137.00 ) NO-ERROR.

       /* Links to SmartBrowser h_bpagcatvtas. */
       RUN add-link IN adm-broker-hdl ( h_blistaprov_prov-1 , 'Record':U , h_bpagcatvtas ).

       /* Links to SmartBrowser h_bdetcatvta_prov-1. */
       RUN add-link IN adm-broker-hdl ( h_bpagcatvtas , 'Record':U , h_bdetcatvta_prov-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bpagcatvtas ,
             x-importe:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_bdetcatvta_prov-1 ,
...
------------------------------------------------------------------------------*/
    ...
    CASE pParameter:
        ...
        WHEN 'Open-Query-3' THEN DO:
            RUN dispatch IN h_bdetcatvta_prov-1 ('open-query').
        END.
    END CASE.
END PROCEDURE.
