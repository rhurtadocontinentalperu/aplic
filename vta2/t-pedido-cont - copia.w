...
ITEM.CanPed ITEM.Libre_d01 ITEM.PreUni ITEM.Por_Dsctos[1] ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      fDesMar() @ x-DesMar COLUMN-LABEL "Marca" FORMAT "x(20)":U
            WIDTH 8
      ITEM.UndVta COLUMN-LABEL "Unidad" FORMAT "x(10)":U WIDTH 6.86
      ...
      ITEM.Por_Dsctos[3] COLUMN-LABEL "% Dscto!Vol/Prom" FORMAT "->>9.9999":U
            WIDTH 7.43
      ITEM.ImpDto FORMAT "->>>,>>9.99":U WIDTH 7.72
      ITEM.ImpLin FORMAT ">,>>>,>>9.99":U WIDTH 6.72
  ENABLE
      ITEM.codmat
      ITEM.UndVta
      ITEM.AlmDes
...
"fDesMat() @ x-DesMat" "Descripción" "x(60)" ? ? ? ? ? ? ? no ? no no "43.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > "_<CALC>"
"fDesMar() @ x-DesMar" "Marca" "x(20)" ? ? ? ? ? ? ? no ? no no "8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.ITEM.UndVta
"ITEM.UndVta" "Unidad" "x(10)" "character" ? ? ? ? ? ? yes ? no no "6.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.ITEM.AlmDes
"ITEM.AlmDes" "Alm!Desp" ? "character" ? ? ? ? ? ? yes ? no no "4.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"ITEM.Libre_d01" "Cantidad!Pedida" ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"ITEM.ImpDto" ? "->>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "7.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[14]   > Temp-Tables.ITEM.ImpLin
"ITEM.ImpLin" ? ">,>>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "6.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  IF f-FleteUnitario > 0 THEN DO:
      ...
      RUN gn/factor-porcentual-flete-v2(INPUT s-coddiv, INPUT ITEM.codmat, INPUT-OUTPUT f-FleteUnitario, INPUT s-TpoPed, INPUT f-factor, INPUT s-CodMon).
...
/*   END.                                                                                   */
  /* Unidad válida */
  FIND Unidades WHERE Unidades.Codunid = ITEM.UndVta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
      NO-LOCK NO-ERROR.
  IF NOT AVAILABLE Unidades OR Unidades.Libre_l01 = YES THEN DO:
      MESSAGE 'Unidad NO válida o inactiva' VIEW-AS ALERT-BOX ERROR.
  ...
  x-CanPed = DECIMAL(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
  i-CanPed = INTEGER(ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
  IF Unidades.Libre_L02 = YES THEN DO:
      /* Unidad indivisible */
      IF x-CanPed <> i-CanPed THEN DO:
          MESSAGE 'Solo se puede vender en valores enteros' VIEW-AS ALERT-BOX ERROR.
          ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = STRING(i-CanPed).
          APPLY 'ENTRY':U TO ITEM.CanPed.
          RETURN 'ADM-ERROR'.
      END.
  END.
  ELSE DO:
      ...
      IF Almtconv.Multiplos <> 0 THEN DO:
          IF (TRUNCATE(x-CanPed / Almtconv.Multiplos, 0) * Almtconv.Multiplos) <> x-CanPed THEN DO:
              MESSAGE 'Solo se puede vender en múltiplos de' Almtconv.Multiplos VIEW-AS ALERT-BOX ERROR.
              APPLY 'ENTRY':U TO ITEM.CanPed.
              RETURN 'ADM-ERROR'.
          END.
      END.
