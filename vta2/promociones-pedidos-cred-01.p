...
/* Se captura el mensaje de error para mostrarlo despues */
...
IF LOOKUP (FacCPedi.FmaPgo, '900,999') > 0 THEN RETURN 'OK'.
...
FIND B-CPEDI WHERE B-CPEDI.codcia = Faccpedi.codcia
    ...
    NO-LOCK NO-ERROR.
IF AVAILABLE(B-CPEDI) AND B-CPEDI.TpoPed = 'M' THEN RETURN 'OK'.     /* Contrato Marco */
IF AVAILABLE(B-CPEDI) AND B-CPEDI.TpoPed = 'S' THEN RETURN 'OK'.     /* Supermercados */
/* ********************************** */
...
FOR EACH Promocion, FIRST Almmmatg OF Promocion NO-LOCK
    ...
    IF NOT AVAILABLE Almmmate THEN DO:
        pError = 'Producto promocional ' + Promocion.CodMat + ' ' + Almmmatg.desmat + CHR(10) +
            'NO asignado al almacén ' + Promocion.Almdes.
    ...
    IF s-Ok = NO THEN DO:
        pError = 'NO hay stock disponible para la promoción: ' + Promocion.codmat + ' '  + Almmmatg.desmat + CHR(10) +
            'Promoción: ' + STRING(Promocion.CanPed * Promocion.factor, '>,>>9.99') + ' ' + Promocion.UndVta + CHR(10) +
            'Stock Disponible para el Almacén ' + Promocion.AlmDes + ':' + STRING(s-StkDis, '->>>,>>9.99') + CHR(10) + 
    ...
    IF ERROR-STATUS:ERROR THEN DO:
        pError = 'No se pudo grabar la promoción: ' + Promocion.codmat + CHR(10) + 'Código repetido'.
        UNDO, RETURN 'ADM-ERROR'.
    END.
END.
