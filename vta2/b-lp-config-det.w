...
&Scoped-define INTERNAL-TABLES VtaDTabla Almtfami VtaTabla

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table VtaDTabla.Tipo Almtfami.desfam ~
VtaDTabla.LlaveDetalle VtaDTabla.Libre_c01 fSubLinea() @ x-SubLinea ~
VtaDTabla.Libre_l01 VtaDTabla.Libre_l02 VtaDTabla.Libre_l03 ~
VtaDTabla.Libre_l04 VtaDTabla.Libre_c02 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table VtaDTabla.Tipo ~
VtaDTabla.LlaveDetalle VtaDTabla.Libre_c01 VtaDTabla.Libre_l01 ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaDTabla OF VtaCTabla WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      FIRST Almtfami WHERE Almtfami.CodCia = VtaDTabla.CodCia ~
  AND Almtfami.codfam = VtaDTabla.Tipo OUTER-JOIN NO-LOCK, ~
      EACH VtaTabla WHERE VtaTabla.CodCia = VtaDTabla.CodCia ~
  AND VtaTabla.Llave_c2 = VtaDTabla.Tipo ~
      AND VtaTabla.Tabla = "LP" ~
 AND VtaTabla.Llave_c1 = s-User-Id OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH VtaDTabla OF VtaCTabla WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      FIRST Almtfami WHERE Almtfami.CodCia = VtaDTabla.CodCia ~
  AND Almtfami.codfam = VtaDTabla.Tipo OUTER-JOIN NO-LOCK, ~
      EACH VtaTabla WHERE VtaTabla.CodCia = VtaDTabla.CodCia ~
  AND VtaTabla.Llave_c2 = VtaDTabla.Tipo ~
 ...
 AND VtaTabla.Llave_c1 = s-User-Id OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table VtaDTabla Almtfami VtaTabla
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaDTabla.Tipo COLUMN-LABEL "Línea" FORMAT "x(3)":U
      Almtfami.desfam FORMAT "X(30)":U
      VtaDTabla.LlaveDetalle COLUMN-LABEL "Sub-Línea!+" FORMAT "x(3)":U
      VtaDTabla.Libre_c01 COLUMN-LABEL "Sub-Línea!-" FORMAT "x(3)":U
      fSubLinea() @ x-SubLinea COLUMN-LABEL "Descripción" FORMAT "x(30)":U
      ...
      VtaDTabla.Libre_l04 COLUMN-LABEL "PRODUCTO!Dcto. Promocional" FORMAT "yes/no":U
            COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 14 VIEW-AS COMBO-BOX INNER-LINES 5
                      LIST-ITEM-PAIRS "SI",yes ,
                                      "NO",no
                      DROP-DOWN-LIST 
      VtaDTabla.Libre_c02 COLUMN-LABEL "Tipo" FORMAT "x(15)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SIZE 138 BY 14.81
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaDTabla OF INTEGRAL.VtaCTabla,INTEGRAL.Almtfami WHERE INTEGRAL.VtaDTabla ...,INTEGRAL.VtaTabla WHERE INTEGRAL.VtaDTabla ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST OUTER USED, OUTER"
     _JoinCode[2]      = "Almtfami.CodCia = VtaDTabla.CodCia
  AND Almtfami.codfam = VtaDTabla.Tipo"
     _JoinCode[3]      = "VtaTabla.CodCia = VtaDTabla.CodCia
  AND VtaTabla.Llave_c2 = VtaDTabla.Tipo"
     _Where[3]         = "VtaTabla.Tabla = ""LP""
 AND VtaTabla.Llave_c1 = s-User-Id"
     _FldNameList[1]   > INTEGRAL.VtaDTabla.Tipo
"VtaDTabla.Tipo" "Línea" "x(3)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.Almtfami.desfam
     _FldNameList[3]   > INTEGRAL.VtaDTabla.LlaveDetalle
"VtaDTabla.LlaveDetalle" "Sub-Línea!+" "x(3)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.VtaDTabla.Libre_c01
"VtaDTabla.Libre_c01" "Sub-Línea!-" "x(3)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"fSubLinea() @ x-SubLinea" "Descripción" "x(30)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.VtaDTabla.Libre_l01
"VtaDTabla.Libre_l01" "AFECTA LISTA DE!xCondic. de Venta" ? "logical" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "SI,yes ,NO,no" 5 no 0 no no
     _FldNameList[7]   > INTEGRAL.VtaDTabla.Libre_l02
"VtaDTabla.Libre_l02" "PRECIOS!xClasific. Cliente" ? "logical" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "SI,yes ,NO,no" 5 no 0 no no
     _FldNameList[8]   > INTEGRAL.VtaDTabla.Libre_l03
"VtaDTabla.Libre_l03" "DESCUENTOS POR!Dcto. x Volumen" ? "logical" 14 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "SI,yes ,NO,no" 5 no 0 no no
     _FldNameList[9]   > INTEGRAL.VtaDTabla.Libre_l04
"VtaDTabla.Libre_l04" "PRODUCTO!Dcto. Promocional" ? "logical" 14 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "SI,yes ,NO,no" 5 no 0 no no
     _FldNameList[10]   > INTEGRAL.VtaDTabla.Libre_c02
"VtaDTabla.Libre_c02" "Tipo" "x(15)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "EXCLUYENTES,,ACUMULADOS,A,MEJOR DESCUENTO,M" 5 no 0 no no
...
ON LEAVE OF VtaDTabla.Tipo IN BROWSE br_table /* Línea */
DO:
  FIND Almtfami WHERE Almtfami.CodCia = s-CodCia AND
      Almtfami.codfam = VtaDTabla.Tipo:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
      NO-LOCK NO-ERROR.
...
ON LEAVE OF VtaDTabla.LlaveDetalle IN BROWSE br_table /* Sub-Línea!+ */
DO:
    FIND Almtfami WHERE Almtfami.CodCia = s-CodCia AND
...
ON LEAVE OF VtaDTabla.Libre_c01 IN BROWSE br_table /* Sub-Línea!- */
DO:
    FIND Almtfami WHERE Almtfami.CodCia = s-CodCia AND
        Almtfami.codfam = SELF:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
        NO-LOCK NO-ERROR.
...
ON 'RETURN':U OF VtaDTabla.Libre_c01, VtaDTabla.Libre_c02, VtaDTabla.Libre_l01, 
    VtaDTabla.Libre_l02, VtaDTabla.Libre_l03, VtaDTabla.Libre_l04, 
    VtaDTabla.LlaveDetalle, VtaDTabla.Tipo DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
IF NOT CAN-FIND(FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia AND 
                ...
                VtaTabla.Llave_c2 = VtaDTabla.Tipo:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} NO-LOCK)
    THEN DO:
    MESSAGE 'Línea NO autorizada para su usuario' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO VtaDTabla.Tipo.
    RETURN 'ADM-ERROR'.
END.
IF VtaDTabla.LlaveDetalle:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > '' THEN DO:
    IF NOT CAN-FIND(Almsfami WHERE AlmSFami.CodCia = s-CodCia AND
                    AlmSFami.codfam =  VtaDTabla.Tipo:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} AND
                    AlmSFami.subfam = VtaDTabla.LlaveDetalle:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
                    NO-LOCK)
        THEN DO:
...
IF VtaDTabla.Libre_c01:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > '' THEN DO:
    ...
    END.
END.
IF VtaDTabla.LlaveDetalle:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > '' AND
    VtaDTabla.Libre_c01:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > ''
    THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  FIND AlmSFami WHERE AlmSFami.CodCia = s-CodCia AND 
      AlmSFami.codfam = VtaDTabla.Tipo AND
      AlmSFami.subfam = (VtaDTabla.LlaveDetalle + VtaDTabla.Libre_c01)
      NO-LOCK NO-ERROR.
