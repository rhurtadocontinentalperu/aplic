...
&Scoped-Define ENABLED-OBJECTS RECT-65 BUTTON-1 C-CodDoc C-NroSer 
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-faccre.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-faccre ).
       RUN set-position IN h_v-faccre ( 2.62 , 3.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-faccre ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-faccre.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-faccre ).
       RUN set-position IN h_b-faccre ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-faccre ( 12.85 , 119.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-faccre. */
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_b-faccre ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-faccre ,
             h_v-faccre , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-faccre.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-faccre ).
       RUN set-position IN h_t-faccre ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-faccre ( 12.85 , 119.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-faccre ,
             h_v-faccre , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-65 BUTTON-1 C-CodDoc C-NroSer 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-faccre ('open-query':U).
          RUN dispatch IN h_t-faccre ('open-query':U).
      END.
