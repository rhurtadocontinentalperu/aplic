...
DEFINE VARIABLE h_b-prepicking-transf AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CONTROL DE IMPRESION DE SALIDAS POR TRANSFERENCIA PARA PRE-PICKING"
         HEIGHT             = 22.27
         WIDTH              = 133.29
         MAX-HEIGHT         = 27.23
         MAX-WIDTH          = 159.43
...
ON END-ERROR OF W-Win /* CONTROL DE IMPRESION DE SALIDAS POR TRANSFERENCIA PARA PRE-PICKING */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* CONTROL DE IMPRESION DE SALIDAS POR TRANSFERENCIA PARA PRE-PICKING */
DO:
...
DO:
    ...
    RUN dispatch IN h_b-prepicking-transf ('open-query':U).
END.
...
DO:
  /*RUN dispatch IN h_b-cimprime-od ('imprime':U).*/
    RUN Imprimir-Formato IN h_b-prepicking-transf ( INPUT "ZONA").
END.
...
DO:
  /*RUN dispatch IN h_b-cimprime-od ('imprime':U).*/
  RUN Imprimir-Formato IN h_b-prepicking-transf ( INPUT "ALFABETICO").
END.
...
DO:
  RUN dispatch IN h_b-prepicking-transf ('ue-excel':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-prepicking-transf.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = FchDoc':U ,
             OUTPUT h_b-prepicking-transf ).
       RUN set-position IN h_b-prepicking-transf ( 2.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-prepicking-transf ( 7.50 , 123.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-prepicking-transf , 'Record':U , h_b-dimprime-transf ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-prepicking-transf ,
             ChkPendientes:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-transf ,
             h_b-prepicking-transf , 'AFTER':U ).
    END. /* Page 0 */
...
OCXFile = SEARCH( "w-listado-prepicking-transf.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-listado-prepicking-transf.wrx":U SKIP(1)
