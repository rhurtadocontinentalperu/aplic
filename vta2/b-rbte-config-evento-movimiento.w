...
DEFINE VAR x-todas-divisiones AS LOG INIT NO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      tt-w-report.Campo-C[1] COLUMN-LABEL "Lista!Precio" FORMAT "X(6)":U
            WIDTH 5.43
      tt-w-report.Campo-C[2] COLUMN-LABEL "Descripcion de la Lista de Precio" FORMAT "X(50)":U
            WIDTH 38.86
      tt-w-report.Campo-C[3] COLUMN-LABEL "Signo" FORMAT "X(1)":U
            WIDTH 8 VIEW-AS COMBO-BOX INNER-LINES 5
                      LIST-ITEMS "+","-" 
                      DROP-DOWN-LIST 
  ENABLE
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS SIZE 59 BY 13.46
         FONT 4.
...
"tt-w-report.Campo-C[1]" "Lista!Precio" "X(6)" "character" ? ? ? ? ? ? yes ? no no "5.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-w-report.Campo-C[2]
"tt-w-report.Campo-C[2]" "Descripcion de la Lista de Precio" "X(50)" "character" ? ? ? ? ? ? yes ? no no "38.86" yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-w-report.Campo-C[3]
"tt-w-report.Campo-C[3]" "Signo" "X(1)" "character" ? ? ? ? ? ? yes ? no no "8" yes no no "U" "" "" "DROP-DOWN-LIST" "," "+,-" ? 5 no 0 no no
...
ON LEAVE OF tt-w-report.Campo-C[1] IN BROWSE br_table /* Lista!Precio */
DO:
    DEFINE VAR x-division AS CHAR.
    
    x-division = tt-w-report.campo-c[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.

    IF NOT (TRUE >(x-division > "") ) THEN DO:
        IF x-division = '*' THEN DO:

            IF x-todas-divisiones = YES THEN DO:
                MESSAGE "Todas la Listas de precios ya esta registrado" VIEW-AS ALERT-BOX INFORMATION.
                APPLY 'ENTRY':U TO tt-w-report.campo-c[1] IN BROWSE {&BROWSE-NAME}.
                RETURN NO-APPLY.
            END.
            ELSE DO:

                ASSIGN {&FIRST-TABLE-IN-QUERY-br_table}.campo-c[2]:SCREEN-VALUE IN BROWSE br_table = "Todas las Listas de precio".
        ...
        ELSE DO:
            ...
            IF NOT AVAILABLE gn-divi THEN DO:
                MESSAGE "Lista de precio NO existe" VIEW-AS ALERT-BOX INFORMATION.
                /*APPLY 'ENTRY':U TO tt-w-report.campo-c[1] IN BROWSE {&BROWSE-NAME}.*/
                RETURN NO-APPLY.                
            END.

            ASSIGN {&FIRST-TABLE-IN-QUERY-br_table}.campo-c[2]:SCREEN-VALUE IN BROWSE br_table = gn-divi.desdiv.        
            ASSIGN {&FIRST-TABLE-IN-QUERY-br_table}.campo-c[3]:SCREEN-VALUE IN BROWSE br_table = "+".
            IF x-todas-divisiones = YES THEN ASSIGN {&FIRST-TABLE-IN-QUERY-br_table}.campo-c[3]:SCREEN-VALUE IN BROWSE br_table = "-".
...
ON 'RETURN':U OF tt-w-report.campo-c[1], tt-w-report.campo-c[2], 
                tt-w-report.campo-c[3] IN BROWSE {&BROWSE-NAME} DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      FOR EACH rbte_evento_movimiento WHERE rbte_evento_movimiento.codcia = s-codcia AND
                                            ...
                                            rbte_evento_movimiento.codrazon = x-motivo NO-LOCK:
          CREATE tt-w-report.
          ASSIGN    tt-w-report.campo-c[1] = rbte_evento_movimiento.coddiv
                    tt-w-report.campo-c[3] = rbte_evento_movimiento.signo
              .
          /* Lista de precios DIVISIONES */
          IF tt-w-report.campo-c[1] = "*" THEN DO:
             ASSIGN tt-w-report.campo-c[2] = "<Todas las listas de precio>".
             x-todas-divisiones = YES.
          END.
          ELSE DO:
              ASSIGN tt-w-report.campo-c[2] = "LISTA DE PRECIO ERRADA".
              FIND FIRST gn-divi WHERE gn-divi.codcia = s-codcia AND
                                            gn-divi.coddiv = tt-w-report.campo-c[1]
                                            NO-LOCK NO-ERROR.
              IF AVAILABLE gn-divi THEN ASSIGN tt-w-report.campo-c[2] = gn-divi.desdiv.
          END.
      END.
...
------------------------------------------------------------------------------*/
  ...
  */
  ...
  FOR EACH rbte_evento_movimiento WHERE rbte_evento_movimiento.codcia = s-codcia AND
                                        ...
                                        rbte_evento_movimiento.codrazon = x-motivo NO-LOCK:
      CREATE ytt-w-report.
      ASSIGN    ytt-w-report.campo-c[1] = rbte_evento_movimiento.coddiv
                ytt-w-report.campo-c[3] = rbte_evento_movimiento.signo
          .
  END.
      ...
      DO:
          ...
          FOR EACH ytt-w-report WHERE ytt-w-report.campo-c[10] = "X" :
              FIND FIRST rbte_evento_movimiento WHERE rbte_evento_movimiento.codcia = s-codcia AND
                                                        rbte_evento_movimiento.codproceso = x-proceso AND
                                                        rbte_evento_movimiento.codrazon = x-motivo AND
                                                        rbte_evento_movimiento.coddiv = ytt-w-report.campo-c[1]
                                                        EXCLUSIVE-LOCK NO-ERROR.
              IF AVAILABLE rbte_evento_movimiento THEN DO:
                  DELETE rbte_evento_movimiento NO-ERROR.
              END.
          ...
          FOR EACH xtt-w-report :
              FIND FIRST rbte_evento_movimiento WHERE rbte_evento_movimiento.codcia = s-codcia AND
                                                        rbte_evento_movimiento.codproceso = x-proceso AND
                                                        rbte_evento_movimiento.codrazon = x-motivo AND
                                                        rbte_evento_movimiento.coddiv = xtt-w-report.campo-c[1]
                                                        EXCLUSIVE-LOCK NO-ERROR.
              
              IF NOT AVAILABLE rbte_evento_movimiento THEN DO:
                  CREATE rbte_evento_movimiento.
                    ASSIGN rbte_evento_movimiento.codcia = s-codcia
                            rbte_evento_movimiento.codproceso = x-proceso
                            rbte_evento_movimiento.codrazon = x-motivo
                            rbte_evento_movimiento.coddiv = xtt-w-report.campo-c[1]
                            rbte_evento_movimiento.campo-c[1] = USERID("DICTDB") + "|" + STRING(TODAY,"99/99/9999") + "|" + STRING(TIME,"HH:MM:SS")
                            NO-ERROR
                            .
              END.
              ASSIGN rbte_evento_movimiento.signo = xtt-w-report.campo-c[3]
                        rbte_evento_movimiento.campo-c[2] = USERID("DICTDB") + "|" + STRING(TODAY,"99/99/9999") + "|" + STRING(TIME,"HH:MM:SS")
                  NO-ERROR.
  ...
  RELEASE rbte_evento_movimiento.
...
------------------------------------------------------------------------------*/
  ...
  x-coddiv = tt-w-report.campo-c[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.

  IF x-coddiv = '*' THEN x-todas-divisiones = YES.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  IF x-coddiv = '*' THEN x-todas-divisiones = NO.
...
x-division = tt-w-report.campo-c[1]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.
x-signo = tt-w-report.campo-c[3]:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.

FIND FIRST gn-divi WHERE gn-divi.codcia = s-codcia AND
                            gn-divi.coddiv = x-division NO-LOCK NO-ERROR.
IF x-division <> '*' AND NOT AVAILABLE gn-divi THEN DO:
    MESSAGE "Lista de precio NO existe" VIEW-AS ALERT-BOX INFORMATION.
    RETURN "ADM-ERROR".
END.

FIND FIRST xtt-w-report WHERE xtt-w-report.campo-c[1] = x-division NO-LOCK NO-ERROR.
IF AVAILABLE xtt-w-report THEN DO:
    MESSAGE "Ya existe la Lista de precios" VIEW-AS ALERT-BOX INFORMATION.
    RETURN "ADM-ERROR".
END.
ELSE DO:
    IF x-todas-divisiones = YES THEN DO:
        IF x-signo = '+' THEN DO:
            MESSAGE "Existe inconsistencia de la Lista de Precio con el signo(1)" VIEW-AS ALERT-BOX INFORMATION.
            RETURN "ADM-ERROR".
        END.
    END.
END.
...
IF x-division = '*' THEN DO:
    FIND FIRST xtt-w-report WHERE xtt-w-report.campo-c[3] = "+" NO-LOCK NO-ERROR.
