...
&Scoped-define INTERNAL-TABLES DETA INTEGRAL.Almmmatg
...
INTEGRAL.Almmmatg.DesMat DETA.CanDes DETA.UndVta DETA.PreUni DETA.ImpLin 
...
&Scoped-define QUERY-STRING-br_table FOR EACH DETA WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH INTEGRAL.Almmmatg OF DETA NO-LOCK ~
    BY DETA.NroItm
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH DETA WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH INTEGRAL.Almmmatg OF DETA NO-LOCK ~
    BY DETA.NroItm.
&Scoped-define TABLES-IN-QUERY-br_table DETA INTEGRAL.Almmmatg
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      DETA.NroItm COLUMN-LABEL "Item" FORMAT ">>9":U
      DETA.codmat COLUMN-LABEL "Articulo" FORMAT "X(6)":U
      INTEGRAL.Almmmatg.DesMat FORMAT "X(60)":U WIDTH 60.43
      DETA.CanDes FORMAT ">>>,>>9.99":U
      DETA.UndVta FORMAT "x(6)":U
      DETA.PreUni COLUMN-LABEL "Unitario" FORMAT ">>>,>>9.99999":U
      DETA.ImpLin FORMAT "->>,>>>,>>9.99":U WIDTH 4.29
  ENABLE
      DETA.CanDes
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     ...
     FILL-IN-ImpTot AT ROW 7.73 COL 89 COLON-ALIGNED WIDGET-ID 2
...
"DETA.NroItm" "Item" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"DETA.UndVta" ? "x(6)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"DETA.ImpLin" ? ? "decimal" ? ? ? ? ? ? no ? no no "4.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
 ...
 FILL-IN-ImpTot = 0.
 FOR EACH B-DETA:
     FILL-IN-ImpTot = FILL-IN-ImpTot + B-DETA.ImpLin.
 END.
 DISPLAY
     FILL-IN-ImpTot WITH FRAME {&FRAME-NAME}.
