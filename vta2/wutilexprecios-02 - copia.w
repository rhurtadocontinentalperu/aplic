...
DEFINE VARIABLE h_butilexprecios-02d AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_p-updv97-4 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 142.43 BY 19.77 WIDGET-ID 100.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_butilexprecios-02a ( 2.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_butilexprecios-02a ( 16.35 , 140.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv97-2 ).
       RUN set-position IN h_p-updv97-2 ( 19.04 , 2.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_butilexprecios-02b ( 2.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_butilexprecios-02b ( 16.35 , 135.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv97 ).
       RUN set-position IN h_p-updv97 ( 18.88 , 2.00 ) NO-ERROR.
    ...
    WHEN 3 THEN DO:
       ...
       RUN set-position IN h_butilexprecios-02c ( 2.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_butilexprecios-02c ( 16.35 , 139.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv97-3 ).
       RUN set-position IN h_p-updv97-3 ( 18.88 , 2.00 ) NO-ERROR.
    ...
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vta2/butilexprecios-02d.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_butilexprecios-02d ).
       RUN set-position IN h_butilexprecios-02d ( 2.54 , 2.00 ) NO-ERROR.
       RUN set-size IN h_butilexprecios-02d ( 16.35 , 138.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv97-4 ).
       RUN set-position IN h_p-updv97-4 ( 18.88 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv97-4 ( 1.54 , 34.00 ) NO-ERROR.

       /* Links to SmartBrowser h_butilexprecios-02d. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97-4 , 'TableIO':U , h_butilexprecios-02d ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_butilexprecios-02d ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv97-4 ,
             h_butilexprecios-02d , 'AFTER':U ).
    END. /* Page 4 */
...
CASE RADIO-SET-1:
    ...
    WHEN 4 THEN DO: 
        RUN select-page ('4').
        RUN Recalcular IN h_butilexprecios-02d.
        RUN dispatch IN h_butilexprecios-02d ('open-query':U).
    END.
END CASE.
...
REPEAT:
    ...
    ASSIGN
        ...
        NO-ERROR.
    /*IF t-row = 3 THEN MESSAGE t-matg.codmat t-matg.preofi.*/
    IF ERROR-STATUS:ERROR THEN DO:
        pMensaje = "Error en la linea " + TRIM(STRING(t-Row, '>>>9')) + ":"  + CHR(10) 
...
FOR EACH T-MATG:
    ...
    ASSIGN
        ...
        T-MATG.UndBas  = Almmmatg.UndBas
        T-MATG.DesMar  = Almmmatg.DesMar
        T-MATG.CHR__01 = (IF AVAILABLE Almmmatp THEN Almmmatp.Chr__01 ELSE Almmmatg.Chr__01)
        T-MATG.MonVta  = 1      /* SIEMPRE EN SOLES Almmmatg.MonVta */
        T-MATG.TpoCmb  = Almmmatg.TpoCmb.
END.
...
FOR EACH T-MATG:
    ...
    IF T-MATG.MrgUti-A < 0 THEN DO:
        ...
        BUFFER-COPY T-MATG 
            TO E-MATG
            ASSIGN E-MATG.Libre_c01 = "Margen de utilidad para la lista A NO debe negativo".
        DELETE T-MATG.
        NEXT.
    END.
    IF T-MATG.MrgUti-B < 0 THEN DO:
        ...
        BUFFER-COPY T-MATG 
            TO E-MATG
            ASSIGN E-MATG.Libre_c01 = "Margen de utilidad para la lista B NO debe negativo".
        DELETE T-MATG.
        NEXT.
    END.
    IF T-MATG.MrgUti-C < 0 THEN DO:
        ...
        BUFFER-COPY T-MATG 
            TO E-MATG
            ASSIGN E-MATG.Libre_c01 = "Margen de utilidad para la lista C NO debe negativo".
        DELETE T-MATG.
        NEXT.
    END.
    IF T-MATG.DEC__01 < 0 THEN DO:
        ...
        BUFFER-COPY T-MATG 
            TO E-MATG
            ASSIGN E-MATG.Libre_c01 = "Margen de utilidad de Oficina NO debe negativo".
        DELETE T-MATG.
        NEXT.
    END.
...
/*         ).                                                                 */
    IF pError = "ADM-ERROR" THEN DO:
        ...
        BUFFER-COPY T-MATG 
            TO E-MATG
            ASSIGN 
            E-MATG.Libre_c01 = "Margen de utilidad para el Precio de Oficina NO debe ser menor a " +
...
/*             ).                                                                 */
        IF pError = "ADM-ERROR" THEN DO:
            ...
            BUFFER-COPY T-MATG 
                TO E-MATG
                ASSIGN 
                E-MATG.Libre_c01 = "Margen de utilidad para la lista B NO debe ser menor a " +
                TRIM(STRING(x-Limite, ">>>,>>9.99")).
            DELETE T-MATG.
            NEXT.
        END.
...
/*             ).                                                                 */
        IF pError = "ADM-ERROR" THEN DO:
            ...
            BUFFER-COPY T-MATG 
                TO E-MATG
                ASSIGN 
                E-MATG.Libre_c01 = "Margen de utilidad para la lista C NO debe ser menor a " +
                TRIM(STRING(x-Limite, ">>>,>>9.99")).
            DELETE T-MATG.
            NEXT.
        END.
...
FOR EACH T-MATG, FIRST Almmmatg OF T-MATG NO-LOCK ON ERROR UNDO, RETURN ON STOP UNDO, RETURN:
    ...
    /* Oficina */
    x-PreUni = T-MATG.PreOfi.
    RUN vtagn/p-margen-utilidad-v11 (
        s-CodDiv,
        T-MATG.CodMat,
        x-PreUni,
        T-MATG.CHR__01,
...
FOR EACH T-MATG, FIRST Almmmatg OF T-MATG NO-LOCK ON ERROR UNDO, RETURN ON STOP UNDO, RETURN:
    ...
    IF T-MATG.PreOfi <= 0 THEN DO:
        ...
        NEXT.
    END.
    FIND Almmmatp OF T-MATG EXCLUSIVE-LOCK NO-ERROR.
    ...
    END.
    ASSIGN
        Almmmatp.MonVta  = 1        /* Siempre en SOLES T-MATG.MonVta */
...
FOR EACH T-MATG, FIRST Almmmatg OF T-MATG NO-LOCK ON ERROR UNDO, RETURN ON STOP UNDO, RETURN:
    ...
    END.
    /* Por margenes de utilidad */
    /* Artificio: Buscamos una división válida */
