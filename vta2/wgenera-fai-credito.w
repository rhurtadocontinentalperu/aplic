...
DEFINE VARIABLE h_bgenera-fai-credito AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "GENERACION DE FAI AL CREDITO"
         HEIGHT             = 20.65
         WIDTH              = 121
         MAX-HEIGHT         = 24.58
         MAX-WIDTH          = 124.43
...
ON END-ERROR OF W-Win /* GENERACION DE FAI AL CREDITO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* GENERACION DE FAI AL CREDITO */
DO:
...
DO:
  RUN dispatch IN h_bgenera-fai-credito ('open-query':U).
END.
...
DO:
    RUN Fraccionar-Orden IN h_bgenera-fai-credito.
    RUN dispatch IN h_bgenera-fai-credito ('open-query':U).
END.
...
DO:
    RUN proc_GeneraGuia IN h_bgenera-fai-credito.
    RUN dispatch IN h_bgenera-fai-credito ('open-query':U).
END.
...
DO:
    RUN proc_GeneraGuiaManual IN h_bgenera-fai-credito.
    RUN dispatch IN h_bgenera-fai-credito ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/bgenera-fai-credito.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bgenera-fai-credito ).
       RUN set-position IN h_bgenera-fai-credito ( 1.27 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bgenera-fai-credito ( 8.08 , 96.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_bgenera-fai-credito , 'Record':U , h_b-genera-factura-credb ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bgenera-fai-credito ,
