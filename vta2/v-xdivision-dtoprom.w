...
&Scoped-Define DISPLAYED-FIELDS VtaListaMay.PromFchD VtaListaMay.PromFchH ~
VtaListaMay.PromDto VtaListaMay.CodDiv 
...
DEFINE FRAME F-Main
     ...
     VtaListaMay.PromFchH AT ROW 2.35 COL 26 COLON-ALIGNED NO-LABEL WIDGET-ID 6
          VIEW-AS FILL-IN 
          SIZE 12.86 BY .81
     VtaListaMay.PromDto AT ROW 2.35 COL 43 COLON-ALIGNED NO-LABEL WIDGET-ID 2 FORMAT "->>>9.9999"
          VIEW-AS FILL-IN 
          SIZE 8.29 BY .81
...
/* SETTINGS FOR FILL-IN f-Precio IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN VtaListaMay.PromDto IN FRAME F-Main
   EXP-FORMAT                                                           */
...
DO:
    IF DECIMAL(SELF:SCREEN-VALUE) = 0 THEN DO:
        VtaListaMay.PromDto:SCREEN-VALUE = '0'.
        RETURN.
    END.
    PromDto:SCREEN-VALUE = STRING ( ROUND ( ( 1 -  DECIMAL(SELF:SCREEN-VALUE) / x-PreOfi  ) * 100, 4 ) ).
END.
...
&Scoped-define SELF-NAME VtaListaMay.PromDto
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL VtaListaMay.PromDto V-table-Win
ON LEAVE OF VtaListaMay.PromDto IN FRAME F-Main /* PromDto */
DO:
    F-PRECIO = ROUND(x-PreOfi * ( 1 - ( DECI(SELF:SCREEN-VALUE) / 100 ) ),4).
...
------------------------------------------------------------------------------*/
  ...
  IF VtaListaMay.PromDto = 0 OR VtaListaMay.PromFchD = ?  OR VtaListaMay.PromFchH = ?
  THEN ASSIGN
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE VtaListaMay THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      F-PRECIO = ROUND(x-PreOfi * ( 1 - ( VtaListaMay.PromDto / 100 ) ),4).
      IF VtaListaMay.PromDto = 0 THEN F-PRECIO = 0.
      DISPLAY F-PRECIO WITH FRAME {&FRAME-NAME}.
  END.
