...
&Scoped-Define ENABLED-OBJECTS BUTTON-Canc RECT-24 COMBO-NroSer BUTTON-PEN 
...
DEFINE VARIABLE h_bpedidotiendasutilexv3 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv05 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-pedido-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_tpedidotiendasutilexv3 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vpedidotiendasutilexv3 AS HANDLE NO-UNDO.
...
DEFINE BUTTON BUTTON-Canc 
     ...
     LABEL "" 
     SIZE 7 BY 1.54 TOOLTIP "Cancelación".
...
DEFINE FRAME F-Main
     BUTTON-Canc AT ROW 1 COL 135 WIDGET-ID 12
...
DO:
  ...
  RUN Cancelar-Pedido IN h_vpedidotiendasutilexv3.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/vpedidotiendasutilexv3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vpedidotiendasutilexv3 ).
       RUN set-position IN h_vpedidotiendasutilexv3 ( 2.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.08 , 132.57 ) */
       ...
       /* Links to SmartViewer h_vpedidotiendasutilexv3. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_vpedidotiendasutilexv3 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_vpedidotiendasutilexv3 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             BUTTON-Canc:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vpedidotiendasutilexv3 ,
             BUTTON-PEN:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido-4 ,
             h_vpedidotiendasutilexv3 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/bpedidotiendasutilexv3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bpedidotiendasutilexv3 ).
       RUN set-position IN h_bpedidotiendasutilexv3 ( 11.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bpedidotiendasutilexv3 ( 15.88 , 141.00 ) NO-ERROR.

       /* Links to SmartBrowser h_bpedidotiendasutilexv3. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_bpedidotiendasutilexv3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bpedidotiendasutilexv3 ,
             h_vpedidotiendasutilexv3 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'tpedidotiendasutilexv3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_tpedidotiendasutilexv3 ).
       /* Position in AB:  ( 11.23 , 2.00 ) */
       /* Size in UIB:  ( 14.23 , 141.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 25.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.54 , 51.00 ) NO-ERROR.

       /* Links to  h_tpedidotiendasutilexv3. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tpedidotiendasutilexv3 ).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-Canc RECT-24 COMBO-NroSer BUTTON-PEN 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_bpedidotiendasutilexv3 <> ? THEN RUN dispatch IN h_bpedidotiendasutilexv3 ('open-query':U).
          IF h_tpedidotiendasutilexv3 <> ? THEN RUN dispatch IN h_tpedidotiendasutilexv3 ('open-query':U).
...
/*       END.                                            */
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_vpedidotiendasutilexv3 <> ? THEN RUN Recalcular-Precios IN h_vpedidotiendasutilexv3.
