...
DEFINE SHARED VAR pImpresos AS INT.
...
AND ((lChequeados = NO AND faccpedi.flgest <> 'A') ~
     OR (faccpedi.flgest = 'P' ~
         AND faccpedi.flgsit = 'T')) ~
AND (faccpedi.fchped >= ltxtDesde AND faccpedi.fchped <= ltxtHasta) ~
AND (pImpresos = 1 OR (pImpresos = 2 AND ENTRY(1, Faccpedi.Libre_c02, '|') <> '') ~
     OR (ENTRY(1, Faccpedi.Libre_c02, '|') = '' OR ENTRY(1, Faccpedi.Libre_c02, '|') = ?)) ~
...
DEFINE TEMP-TABLE Reporte
    FIELDS Llave     AS CHAR
    FIELDS CodDoc    LIKE FacCPedi.CodDoc
...
ORDENES.Libre_c03 ORDENES.Libre_d02 ORDENES.ordcmp 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fPeso2 B-table-Win 
FUNCTION fPeso2 RETURNS DECIMAL
  ( /* parameter-definitions */ )  FORWARD.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 144 BY 7.5
...
"ORDENES.Libre_d02" "Items" ">>9" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[14]   = Temp-Tables.ORDENES.ordcmp
...
/* OJO: No es multiseleccion */
IF NOT AVAILABLE ORDENES THEN RETURN.

DEF VAR k AS INT NO-UNDO.
SESSION:SET-WAIT-STATE('GENERAL').
DO k = 1 TO NUM-ENTRIES(ORDENES.NroRef):
    FIND B-CPEDI WHERE B-CPEDI.codcia = s-codcia
        AND B-CPEDI.coddoc = ORDENES.codref
        AND B-CPEDI.nroped = ENTRY(k, ORDENES.NroRef)
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE B-CPEDI THEN NEXT.
    x-Ordenes = x-Ordenes + (IF x-Ordenes = '' THEN '' ELSE ',') + B-CPEDI.NroPed.
    /* NO KITS */
    FOR EACH B-DPEDI OF B-CPEDI NO-LOCK,
        FIRST Almmmatg NO-LOCK WHERE Almmmatg.CodCia = B-CPEDI.CodCia
        AND Almmmatg.CodMat = B-DPEDI.CodMat:
        FIND FIRST Almckits OF B-DPEDI NO-LOCK NO-ERROR.
        IF AVAILABLE Almckits THEN NEXT.
        FIND Reporte WHERE Reporte.CodMat = B-DPEDI.codmat NO-ERROR.
        IF NOT AVAILABLE Reporte THEN CREATE Reporte.
        ASSIGN 
            Reporte.CodDoc = ORDENES.CodDoc
            Reporte.NroPed = ORDENES.NroPed
/*             Reporte.CodDoc = B-CPEDI.CodDoc */
/*             Reporte.NroPed = B-CPEDI.NroPed */
            Reporte.CodRef = B-CPEDI.CodRef
            Reporte.NroRef = B-CPEDI.NroRef
            Reporte.CodMat = B-DPEDI.CodMat
            Reporte.DesMat = Almmmatg.DesMat
            Reporte.DesMar = Almmmatg.DesMar
            Reporte.UndBas = Almmmatg.UndBas
            Reporte.CanPed = Reporte.CanPed + ( B-DPEDI.CanPed * B-DPEDI.Factor )
            Reporte.CodAlm = B-CPEDI.CodAlm
            Reporte.CodUbi = "G-0"
            Reporte.CodZona = "G-0".
        FIND FIRST Almmmate WHERE Almmmate.CodCia = B-CPEDI.CodCia
        ...
        END.
        /* Cargamos Resumen */
        FIND Resumen WHERE Resumen.codmat = B-DPEDI.codmat NO-ERROR.
        ...
        ASSIGN
            ...
            Resumen.nroped[k] = B-CPEDI.nroped
            Resumen.canped[k] =  B-DPEDI.CanPed * B-DPEDI.Factor.

        /* Cargamos Resumen2 */
        FIND Resumen2 WHERE Resumen2.x-codcli = B-CPEDI.codcli AND 
                        resumen2.coddoc = B-CPEDI.CodDoc AND  
                        resumen2.nroped = B-CPEDI.NroPed NO-ERROR.
        IF NOT AVAILABLE Resumen2 THEN DO : 
            CREATE Resumen2.
                ASSIGN
                    Resumen2.x-codcli = B-CPEDI.CodCli
                    Resumen2.X-NomCli = B-CPEDI.NomCli
                    resumen2.CodDoc   = B-CPEDI.CodDoc
                    resumen2.NroPed   = B-CPEDI.NroPed
                    resumen2.X-FECHA  = B-CPEDI.fchped
                    resumen2.X-fchent = B-CPEDI.fchent
                    resumen2.x-qtyitm = 0
                    resumen2.x-peso   = 0.
        END.
        ASSIGN resumen2.x-qtyitm = resumen2.x-qtyitm + 1.
               resumen2.x-peso  = resumen2.x-peso + (( B-DPEDI.CanPed * B-DPEDI.Factor ) * almmmatg.pesmat).

    END.
    /* SOLO KITS */
    FOR EACH B-DPEDI OF B-CPEDI NO-LOCK,
        FIRST Almckits OF B-DPEDI NO-LOCK,
...
/*             Reporte.NroPed = B-CPEDI.NroPed */
            ...
            Reporte.DesMat = TRIM(Almmmatg.DesMat) + ' (KIT ' + TRIM(B-DPEDI.codmat) + ')'
            Reporte.DesMar = Almmmatg.DesMar
            Reporte.UndBas = Almmmatg.UndBas
            Reporte.CanPed = Reporte.CanPed + ( B-DPEDI.CanPed * B-DPEDI.Factor *  AlmDKits.Cantidad )
            Reporte.CodAlm = B-CPEDI.CodAlm
            Reporte.CodUbi = "G-0"
            Reporte.CodZona = "G-0".
        FIND FIRST Almmmate WHERE Almmmate.CodCia = B-CPEDI.CodCia
        ...
        ASSIGN
            Resumen.codmat = Almmmatg.codmat
            Resumen.desmat = TRIM(Almmmatg.DesMat) + ' (KIT ' + TRIM(B-DPEDI.codmat) + ')'
            Resumen.undbas = Almmmatg.undbas
            Resumen.desmat = Almmmatg.desmat
            Resumen.nroped[k] = B-CPEDI.nroped
            Resumen.canped[k] = ( B-DPEDI.CanPed * B-DPEDI.Factor *  AlmDKits.Cantidad ).
        ...
        IF NOT AVAILABLE Resumen2 THEN DO : 
            CREATE Resumen2.
                ASSIGN
                    ...
                    resumen2.CodDoc   = B-CPEDI.CodDoc
                    resumen2.NroPed   = B-CPEDI.NroPed
                    resumen2.X-FECHA  = B-CPEDI.Libre_f01
                    resumen2.X-fchent = B-CPEDI.fchent
                    resumen2.x-qtyitm = 0
                    resumen2.x-peso   = 0.
        END.
        ASSIGN resumen2.x-qtyitm = resumen2.x-qtyitm + 1.
               resumen2.x-peso  = resumen2.x-peso + ((B-DPEDI.CanPed * B-DPEDI.Factor *  AlmDKits.Cantidad ) * almmmatg.pesmat).
...
------------------------------------------------------------------------------*/
      ...
      FIND Faccpedi OF ORDENES EXCLUSIVE-LOCK NO-ERROR.
      IF AVAILABLE Faccpedi AND Faccpedi.Libre_c02 = "" THEN DO:
          ASSIGN
  ...
  MESSAGE 'selected' {&browse-name}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}.
  DO k = 1 TO {&browse-name}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
      MESSAGE ordenes.nroped.
      IF NOT {&browse-name}:FETCH-SELECTED-ROW(k) THEN NEXT.
      IF FacCPedi.Libre_c02 <> "" THEN NEXT.
      FIND CURRENT Faccpedi EXCLUSIVE-LOCK NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Formato-Rb-Empaque B-table-Win 
PROCEDURE Formato-Rb-Empaque :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* Resumimos y separamos la información */
EMPTY TEMP-TABLE T-Reporte.
DEF VAR x-Cociente AS DEC NO-UNDO.
DEF VAR x-Saldo    AS DEC NO-UNDO.

SESSION:SET-WAIT-STATE('GENERAL').
FOR EACH Reporte, FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
    AND Almmmatg.codmat = Reporte.codmat:
    CASE TRUE:
        WHEN Almmmatg.codfam = '010' THEN DO:
            /* Lo pasamos tal cual */
            CREATE T-Reporte.
            BUFFER-COPY Reporte
                TO T-Reporte
                ASSIGN T-Reporte.Llave = "A".
        END.
        OTHERWISE DO:
            /* Los acumulamos por empaque Master */
            IF Almmmatg.CanEmp > 0 AND Reporte.CanPed >= Almmmatg.CanEmp THEN DO:
                /* Lo separamos por empaque y saldo */
                x-Cociente = (Reporte.CanPed - (Reporte.CanPed MODULO Almmmatg.CanEmp) ) / Almmmatg.CanEmp.
                CREATE T-Reporte.
                BUFFER-COPY Reporte
                    TO T-Reporte
                    ASSIGN T-Reporte.CanPed = (Almmmatg.CanEmp * x-Cociente) T-Reporte.Llave = "B".
                x-Saldo = Reporte.CanPed - T-Reporte.CanPed.
                IF x-Saldo > 0 THEN DO:
                    CREATE T-Reporte.
                    BUFFER-COPY Reporte
                        TO T-Reporte
                        ASSIGN T-Reporte.CanPed = x-Saldo T-Reporte.Llave = "C".
                END.
            END.
            ELSE DO:
                /* Lo pasamos tal cual */
                ...
                BUFFER-COPY Reporte
                    TO T-Reporte
                    ASSIGN T-Reporte.Llave = "C".
            END.
        END.
    END CASE.
END.
EMPTY TEMP-TABLE Reporte.
FOR EACH T-Reporte NO-LOCK:
    CREATE Reporte.
    BUFFER-COPY T-Reporte TO Reporte.
END.
SESSION:SET-WAIT-STATE('').
...
conta = 1.
t-Items = 0.
FOR EACH Reporte BREAK BY Reporte.Llave BY Reporte.CodZon BY Reporte.CodUbi BY Reporte.CodMat:
    conta = conta + 1.
    t-Items = t-Items + 1.
    IF conta > c-items OR LAST-OF(Reporte.Llave) THEN DO:
        npage = npage + 1.
        conta = 1.
    END.
END.
...
FOR EACH Reporte BREAK BY Reporte.Llave BY Reporte.CodZon BY Reporte.CodUbi BY Reporte.CodMat:
    CASE Reporte.Llave:
        WHEN "A" THEN x-Titulo = "POR LINEA 010".
        WHEN "B" THEN x-Titulo = "POR EMPAQUE MASTER".
        WHEN "C" THEN x-Titulo = "MATERIAL SUELTO".
    END CASE.
    DEFINE FRAME f-cab
            n-Item         FORMAT '>9'
            Reporte.CodMat FORMAT 'X(7)'
            ...
            {&PRN3} + {&PRN7A} + {&PRN6B} + "         Peso : " + STRING(lPeso,">>>,>>9.99") + {&PRN6B} + {&PRN7B} + {&PRN3} AT 80 FORMAT "X(40)" SKIP        
            {&PRN3} + {&PRN7A} + {&PRN6B} + x-Titulo + {&PRN6B} + {&PRN7B} + {&PRN3} AT 10 FORMAT "X(40)" SKIP        
...
IF NOT AVAILABLE ORDENES THEN RETURN.
  ...
  DO c-Copias = 1 TO s-nro-copias ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
      ...
      CASE TRUE:
          WHEN pOrden = "ZONA" THEN DO: 
              RUN Formato-Rb-Multi.
              RUN Formato-Rb-Multi-Resumen2.
              /*RUN dispatch IN THIS-PROCEDURE ('open-query':U).*/
          END.
          WHEN pOrden = "EMPAQUE" THEN DO: 
              RUN Formato-Rb-Empaque.
              RUN Formato-Rb-Multi-Resumen2.
              /*RUN dispatch IN THIS-PROCEDURE ('open-query':U).*/
          END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fPeso2 B-table-Win 
FUNCTION fPeso2 RETURNS DECIMAL
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH Reporte NO-LOCK,
        FIRST almmmatg WHERE Almmmatg.codmat = Reporte.codmat NO-LOCK :
        IF almmmatg.pesmat <> ? AND almmmatg.pesmat > 0 THEN DO:
            lPeso = lPeso + (Reporte.canped * almmmatg.pesmat).
        END.       
    END.
  RETURN lPeso.   /* Function return value. */
