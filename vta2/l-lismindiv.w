...
&SCOPED-DEFINE CONDICION ( Vtalistamin.Codfam BEGINS F-Codfam ~
AND Vtalistamin.CodDiv = s-coddiv ~
AND Vtalistamin.Subfam BEGINS F-Subfam ~
AND Vtalistamin.DesMar BEGINS F-marca )

&SCOPED-DEFINE CODIGO Vtalistamin.codmat

/* Preprocesadores para cada campo filtro */ 
&SCOPED-DEFINE FILTRO1 Vtalistamin.DesMat BEGINS FILL-IN-filtro
&SCOPED-DEFINE FILTRO2 INDEX ( Vtalistamin.DesMat , FILL-IN-filtro ) <> 0 
...
&Scoped-define INTERNAL-TABLES Vtalistamin Almmmatg Almmmate

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table Vtalistamin.codfam ~
Vtalistamin.subfam Vtalistamin.codmat Vtalistamin.DesMat Vtalistamin.DesMar ~
F-PRESOLA @ Vtalistamin.PreOfi Vtalistamin.Chr__01 Almmmatg.UndBas ~
_StkAct() @ F-STKACT ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Vtalistamin WHERE ~{&KEY-PHRASE} ~
 ...
 AND (RADIO-SET-Stock = 'T' OR Almmmate.StkAct > 0) NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table Vtalistamin Almmmatg Almmmate
...
<FOREIGN-KEYS>
Nombres que inicien con|y||INTEGRAL.Vtalistamin.DesMat
Nombres que contengan|y||INTEGRAL.Vtalistamin.DesMat
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Codigo|||INTEGRAL.Vtalistamin.CodCia|yes,INTEGRAL.Vtalistamin.codmat|yes
Familia|||INTEGRAL.Vtalistamin.codfam|yes
Descripcion|y||INTEGRAL.Vtalistamin.DesMat|yes
Marca|||INTEGRAL.Vtalistamin.DesMar|yes
Sub-Familia|||INTEGRAL.Vtalistamin.codfam|yes,INTEGRAL.Vtalistamin.subfam|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Vtalistamin.codfam COLUMN-LABEL "Cód!Fam" FORMAT "X(3)":U
      Vtalistamin.subfam COLUMN-LABEL "Sub!Fam" FORMAT "X(3)":U
      Vtalistamin.codmat COLUMN-LABEL "Codigo!Articulo" FORMAT "X(6)":U
      Vtalistamin.DesMat FORMAT "X(80)":U WIDTH 50.14
      Vtalistamin.DesMar COLUMN-LABEL "Marca" FORMAT "X(30)":U
            WIDTH 19.86
      F-PRESOLA @ Vtalistamin.PreOfi COLUMN-LABEL "Precio Venta S/." FORMAT ">>>,>>9.9999":U
      Vtalistamin.Chr__01 COLUMN-LABEL "Unidad!Venta" FORMAT "X(8)":U
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Vtalistamin,INTEGRAL.Almmmatg OF INTEGRAL.Vtalistamin,INTEGRAL.Almmmate OF INTEGRAL.Almmmatg"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",, FIRST, FIRST"
     _Where[1]         = "{&CONDICION}"
     _Where[3]         = "Almmmate.CodAlm = x-codalm
 AND (RADIO-SET-Stock = 'T' OR Almmmate.StkAct > 0)"
     _FldNameList[1]   > INTEGRAL.Vtalistamin.codfam
"Vtalistamin.codfam" "Cód!Fam" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Vtalistamin.subfam
"Vtalistamin.subfam" "Sub!Fam" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.Vtalistamin.codmat
"Vtalistamin.codmat" "Codigo!Articulo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.Vtalistamin.DesMat
"Vtalistamin.DesMat" ? "X(80)" "character" ? ? ? ? ? ? no ? no no "50.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.Vtalistamin.DesMar
"Vtalistamin.DesMar" "Marca" ? "character" ? ? ? ? ? ? no ? no no "19.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"F-PRESOLA @ Vtalistamin.PreOfi" "Precio Venta S/." ">>>,>>9.9999" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.Vtalistamin.Chr__01
"Vtalistamin.Chr__01" "Unidad!Venta" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  IF Almmmatg.MonVta = 1 THEN DO:
    F-PRESOLA = Vtalistamin.PreOfi.
    F-PREUSSA = Vtalistamin.PreOfi / Vtalistamin.TpoCmb.
  END.
  ELSE DO:
    F-PREUSSA = Vtalistamin.PreOfi.
    F-PRESOLA = Vtalistamin.PreOfi * Vtalistamin.TpoCmb.
  END.
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Codigo':U THEN DO:
           &Scope SORTBY-PHRASE BY Vtalistamin.CodCia BY Vtalistamin.codmat
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Familia':U THEN DO:
           &Scope SORTBY-PHRASE BY Vtalistamin.codfam
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripcion':U THEN DO:
           &Scope SORTBY-PHRASE BY Vtalistamin.DesMat
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Marca':U THEN DO:
           &Scope SORTBY-PHRASE BY Vtalistamin.DesMar
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Sub-Familia':U THEN DO:
           &Scope SORTBY-PHRASE BY Vtalistamin.codfam BY Vtalistamin.subfam
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
