...
&Scoped-define INTERNAL-TABLES FacDPedi Almmmatg Almmmate almtubic
...
Almmmatg.Pesmat Almmmatg.DesMar FacDPedi.CanPed FacDPedi.UndVta ~
FacDPedi.AlmDes almtubic.CodZona Almmmate.CodUbi ~
(integral.FacDPedi.CanPed * integral.Almmmatg.Pesmat ) @ lPesoVenta 
...
&Scoped-define QUERY-STRING-br_table FOR EACH FacDPedi OF FacCPedi WHERE ~{&KEY-PHRASE} NO-LOCK, ~
  ...
  AND Almmmate.codmat = FacDPedi.codmat OUTER-JOIN NO-LOCK, ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH FacDPedi OF FacCPedi WHERE ~{&KEY-PHRASE} NO-LOCK, ~
  ...
  AND Almmmate.codmat = FacDPedi.codmat OUTER-JOIN NO-LOCK, ~
    ...
    BY Almmmate.CodUbi ~
       BY almtubic.CodZona ~
        BY FacDPedi.codmat.
&Scoped-define TABLES-IN-QUERY-br_table FacDPedi Almmmatg Almmmate almtubic
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacDPedi.codmat COLUMN-LABEL "Articulo" FORMAT "X(8)":U WIDTH 7.43
      ...
      FacDPedi.CanPed FORMAT ">>>,>>9.99":U WIDTH 9.43
      FacDPedi.UndVta COLUMN-LABEL "Unidad" FORMAT "X(10)":U WIDTH 6.43
      FacDPedi.AlmDes COLUMN-LABEL "Almacén" FORMAT "x(3)":U
      almtubic.CodZona COLUMN-LABEL "Zona" FORMAT "x(8)":U
      Almmmate.CodUbi FORMAT "x(8)":U WIDTH 8.72
      (integral.FacDPedi.CanPed * integral.Almmmatg.Pesmat ) @ lPesoVenta COLUMN-LABEL "Peso Total"
            WIDTH 8.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 111 BY 11.92
         BGCOLOR 15 FONT 4
         TITLE BGCOLOR 15 "DETALLE DE LA ORDEN" FIT-LAST-COLUMN.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "integral.FacDPedi OF integral.FacCPedi,integral.Almmmatg OF integral.FacDPedi,integral.Almmmate WHERE integral.FacDPedi ...,integral.almtubic OF integral.Almmmate"
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ", FIRST, FIRST OUTER, FIRST OUTER"
     _OrdList          = "integral.Almmmate.CodUbi|yes,integral.almtubic.CodZona|yes,integral.FacDPedi.codmat|yes"
     _JoinCode[3]      = "integral.Almmmate.CodCia = integral.FacDPedi.CodCia
  AND integral.Almmmate.CodAlm = integral.FacDPedi.AlmDes
  AND integral.Almmmate.codmat = integral.FacDPedi.codmat"
     _FldNameList[1]   > integral.FacDPedi.codmat
"FacDPedi.codmat" "Articulo" "X(8)" "character" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > "_<CALC>"
"(IF FacDPedi.Libre_c05 <> '' AND FacDPedi.Libre_c05 <> 'OF' THEN FacDPedi.Libre_c05 + ' - '  ELSE '') + Almmmatg.DesMat @ Almmmatg.DesMat" "Descripción" "X(60)" ? ? ? ? ? ? ? no ? no no "33.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacDPedi.CanPed" ? ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > integral.FacDPedi.UndVta
"FacDPedi.UndVta" "Unidad" "X(10)" "character" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > integral.FacDPedi.AlmDes
"FacDPedi.AlmDes" "Almacén" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"(integral.FacDPedi.CanPed * integral.Almmmatg.Pesmat ) @ lPesoVenta" "Peso Total" ? ? ? ? ? ? ? ? no ? no no "8.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
