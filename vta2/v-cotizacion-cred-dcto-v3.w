...
DEFINE SHARED VARIABLE s-DtoMax-Terceros   AS DECIMAL.
DEFINE SHARED VARIABLE s-DtoMax-Propios   AS DECIMAL.
...
DEFINE VAR x-dscto-propios AS DEC.
DEFINE VAR x-dscto-terceros AS DEC.
...
FacCPedi.PorDto FacCPedi.CodCli FacCPedi.FaxCli FacCPedi.usuario ~
FacCPedi.NomCli FacCPedi.CodMon FacCPedi.CodVen FacCPedi.TpoCmb ~
...
DEFINE BUTTON BUTTON-8 
     LABEL "Ingresar descuento del administrador" 
     SIZE 27 BY 1.12.
...
DEFINE FRAME F-Main
     ...
     FacCPedi.FchPed AT ROW 1.27 COL 99 COLON-ALIGNED
          ...
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     FacCPedi.PorDto AT ROW 1.38 COL 71 COLON-ALIGNED
...
"US$", 2
     ...
     BUTTON-8 AT ROW 5.04 COL 86 WIDGET-ID 126
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-8 V-table-Win
ON CHOOSE OF BUTTON-8 IN FRAME F-Main /* Ingresar descuento del administrador */
DO:
    /*
  RUN valida-update.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ITEM, FIRST Almmmatg OF ITEM NO-LOCK WHERE Almmmatg.CHR__02 = "T":
      ...
      ASSIGN ITEM.Por_Dsctos[1] = xPorDto.
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE aplica-descuento V-table-Win 
PROCEDURE aplica-descuento :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR x-art-sin-dscto AS CHAR.
  DEF VAR x-DtoMax AS DEC NO-UNDO.

  FOR EACH ITEM, FIRST Almmmatg OF ITEM NO-LOCK:
      x-art-sin-dscto = "".
      RUN articulo-sin-dscto-alguno(OUTPUT x-art-sin-dscto).

      IF x-art-sin-dscto = "OK" THEN DO:

          IF Almmmatg.CHR__02 = "T" THEN DO:
              xPorDto = x-dscto-terceros.
          END.
          ELSE DO:
              xPorDto = x-dscto-propios.
          END.
          x-DtoMax = xPorDto.

          /*  NIVELES DE DESCUENTO por Excepcion */          
          FIND FIRST VtaTabla WHERE VtaTabla.codcia = s-codcia
              AND VtaTabla.Tabla = 'NA'
              AND VtaTabla.Llave_c1 = s-Nivel
              AND VtaTabla.Llave_c2 = Almmmatg.CodFam
          ...
          IF AVAILABLE VtaTabla THEN DO:
              /*x-DtoMax = MINIMUM(VtaTabla.Valor[1], xPorDto).*/   /* x confirmar */
              /* Excepcion por el Jefe de Linea = 0, no aplica descuento */
              IF Almmmatg.CHR__02 = "T" AND VtaTabla.Valor[1] <= 0 THEN NEXT.              
              IF Almmmatg.CHR__02 = "P" AND VtaTabla.Valor[2] <= 0 THEN NEXT.
              
              IF Almmmatg.CHR__02 = "T" AND VtaTabla.Valor[1] > 0 THEN x-DtoMax = VtaTabla.Valor[1].
              IF Almmmatg.CHR__02 = "P" AND VtaTabla.Valor[2] > 0 THEN x-DtoMax = VtaTabla.Valor[2].
          END.              

            /*
                Ic - 14Nov2019, esta validacion se quito a pedido de Daniel Llican

          /* 
            Buscamos la equivalencia con el MLL - Si la venta es >= MILLAR y
                y % Dscto > al 1% no va
           */
          xPorDto = x-DtoMax    /*pPorDto*/.
          FIND Almtconv WHERE Almtconv.CodUnid = ITEM.UndVta
              AND Almtconv.Codalter = "MLL"
              NO-LOCK NO-ERROR.
          IF AVAILABLE Almtconv THEN DO:
              IF ITEM.CanPed / Almtconv.Equival >= 1 AND /* xPorDto*/ x-DtoMax > 1 THEN NEXT.
          END.
          ASSIGN ITEM.Por_Dsctos[1] = x-DtoMax.
          ...
          */
          ...
          /* *********************************************************************************** */
          x-PreUni = ITEM.PreUni *
              ( 1 - x-DtoMax /*xPorDto*/ / 100 ) *
              ( 1 - ITEM.Por_Dsctos[2] / 100 ) *
              ( 1 - ITEM.Por_Dsctos[3] / 100 ) .

      RUN vtagn/p-margen-utilidad-v2 (
          s-CodDiv,
          ITEM.CodMat,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE articulo-sin-dscto-alguno V-table-Win 
PROCEDURE articulo-sin-dscto-alguno :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF ITEM.Por_DSCTOS[2] > 0 THEN DO:
  pRetVal = "Articulo con Descuent, por Evento".
  RETURN.
END.
IF ITEM.Por_DSCTOS[3] > 0 THEN DO:
  pRetVal = "Articulo con Descuento Promocion / Volumen  ...".         
  RETURN.
END.
 /* RHC 07/02/1029 Caso de cuadernos */
IF ITEM.Libre_c04 = "DVXSALDOC" THEN DO:
    pRetVal = "Articulo con Descuento  ".
    RETURN.
END.
IF ITEM.libre_c04 begins "HP_CARTUCHOS" THEN DO:
    pRetVal = "Articulo con Descuento de HP ".
    RETURN.
END.
IF NOT (TRUE <> (ITEM.libre_c05 > "")) THEN DO:
    pRetVal = "Articulo con Descuento de ENCARTE ".
    RETURN.
END.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  button-8:VISIBLE IN FRAME {&FRAME-NAME}  = NO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pide-dscto V-table-Win 
PROCEDURE pide-dscto :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR y-dscto-propios AS DEC.
  DEFINE VAR y-dscto-terceros AS DEC.

  y-dscto-propios = x-dscto-propios.
  y-dscto-terceros = x-dscto-terceros.

  
  RUN vta2/v-cotizacion-cred-dcto-administrador.r(  INPUT s-nivel,
                                                    INPUT-OUTPUT y-dscto-propios,
                                                    INPUT-OUTPUT y-dscto-terceros,
                                                    OUTPUT x-retval).
  IF x-retval = 'OK' THEN DO:
        x-dscto-propios = y-dscto-propios.
        x-dscto-terceros = y-dscto-terceros.

        RUN aplica-descuento.
  END.
...
FIND FIRST AlmTabla WHERE AlmTabla.Tabla = "NA" AND 
                            almtabla.Codigo = s-nivel NO-LOCK NO-ERROR.
IF NOT AVAILABLE almtabla THEN DO:
    MESSAGE "Su usuario no esta asignado a aplicar la politica general de descuentos" SKIP
            "coordine con su gerencia.".
    RETURN "ADM-ERROR".
END.
...
FIND FacCfgGn WHERE FacCfgGn.CodCia = S-CODCIA NO-LOCK NO-ERROR.
CASE s-Nivel:
   WHEN "D1" THEN s-DtoMax-Propios = FacCfgGn.DtoMax.
   WHEN "D2" THEN s-DtoMax-Propios = FacCfgGn.DtoDis.
   WHEN "D3" THEN s-DtoMax-Propios = FacCfgGn.DtoMay.
   WHEN "D4" THEN s-DtoMax-Propios = FacCfgGn.DtoPro.
   OTHERWISE s-DtoMax-Propios = 0.
END CASE.
/* RHC 23/01/2014 */
FIND FIRST AlmTabla WHERE AlmTabla.Tabla = "NA" AND almtabla.Codigo = s-Nivel NO-LOCK NO-ERROR.
IF AVAILABLE AlmTabla THEN DO:
    s-DtoMax-Terceros = DECIMAL(AlmTabla.NomAnt).
    s-DtoMax-Propios = DECIMAL(AlmTabla.CodCta1).
END.
...
button-8:VISIBLE IN FRAME {&FRAME-NAME} = YES .
