...
fMonVta() @ x-MonVta Almmmatg.CtoTot VtaListaMinGn.MonVta ~
VtaListaMinGn.TpoCmb VtaListaMinGn.Chr__01 VtaListaMinGn.PreOfi ~
VtaListaMinGn.Dec__01 VtaListaMinGn.CanEmp Almmmatg.UndBas 
...
"VtaListaMinGn.MonVta" ? ? "integer" ? ? ? ? ? ? no ? no no "8.86" yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "Soles,1,Dolares,2" 5 no 0 no no
...
ON LEAVE OF VtaListaMinGn.codmat IN BROWSE br_table /* Codigo!Articulo */
DO:
...
/*         VtaListaMinGn.TpoCmb:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(Almmmatg.tpocmb).         */
        FIND LAST gn-tcvtamin WHERE gn-tcvtamin.codcia = s-codcia
            ...
            NO-LOCK NO-ERROR.
        ASSIGN
            VtaListaMinGn.MonVta:SCREEN-VALUE IN BROWSE {&browse-name} = '1'
            VtaListaMinGn.TpoCmb:SCREEN-VALUE IN BROWSE {&browse-name} = (IF AVAILABLE gn-tcvtamin THEN STRING(gn-tcvtamin.venta)
                                                                            ELSE '1').
    END.
END.
...
ON LEAVE OF VtaListaMinGn.Chr__01 IN BROWSE br_table /* Unidad */
DO:
...
FOR EACH VtaListaMinGn WHERE VtaListaMinGn.codcia = s-codcia NO-LOCK,
    FIRST almmmatg WHERE almmmatg.codcia = VtaListaMinGn.codcia
        AND almmmatg.codmat = VtaListaMinGn.codmat NO-LOCK:
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = "yes" 
      THEN ASSIGN
            ...
            VtaListaMinGn.TpoCmb = (IF AVAILABLE gn-tcvtamin THEN gn-tcvtamin.venta ELSE VtaListaMinGn.TpoCmb).
  ASSIGN
  ...
  ELSE IF VtaListaMinGn.monvta = 1 THEN x-CtoTot = Almmmatg.ctotot *  VtaListaMinGn.tpocmb.
  ELSE x-CtoTot = Almmmatg.ctotot /  VtaListaMinGn.tpocmb.
