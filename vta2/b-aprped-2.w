...
DEF VAR s-FlgEst AS CHAR INIT "W".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacCPedi.FaxCli COLUMN-LABEL "" FORMAT "x(10)":U
      ...
      gn-ven.NomVen FORMAT "X(30)":U WIDTH 14.86
      FacCPedi.FmaPgo COLUMN-LABEL "Condición" FORMAT "X(8)":U
            WIDTH 6.72
      gn-ConVt.Nombr FORMAT "X(30)":U WIDTH 19.43
      FacCPedi.Libre_c05 COLUMN-LABEL "Motivo" FORMAT "x(60)":U
            WIDTH 25.86
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 142 BY 8.04
         FONT 4
...
"FacCPedi.FaxCli" "" "x(10)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"gn-ven.NomVen" ? "X(30)" "character" ? ? ? ? ? ? no ? no no "14.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > INTEGRAL.FacCPedi.FmaPgo
"FacCPedi.FmaPgo" "Condición" ? "character" ? ? ? ? ? ? no ? no no "6.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > INTEGRAL.gn-ConVt.Nombr
"gn-ConVt.Nombr" ? "X(30)" "character" ? ? ? ? ? ? no ? no no "19.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > INTEGRAL.FacCPedi.Libre_c05
"FacCPedi.Libre_c05" "Motivo" ? "character" ? ? ? ? ? ? no ? no no "25.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF {&browse-name}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME} = 0 THEN DO:
    ...
    RETURN 'ADM-ERROR'.
END.
MESSAGE 'Procedemos con la pre-aprobación?'
...
DO k = 1 TO {&browse-name}:NUM-SELECTED-ROWS ON ERROR UNDO, NEXT ON STOP UNDO, NEXT:
    ...
    IF LOOKUP(Faccpedi.codven, '015,017,173') > 0 THEN Faccpedi.FlgEst = "WL".
    /* Supermercados pasa a APROBACION POR LOGISTICA */
    IF LOOKUP(Faccpedi.codven, '151') > 0 THEN Faccpedi.FlgEst = "WL".
