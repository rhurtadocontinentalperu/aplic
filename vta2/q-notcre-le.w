...
DEFINE FRAME FRAME-A
     F-Numero AT ROW 1.15 COL 1.57 NO-LABEL
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "{&Condicion}"
     _Design-Parent    is WINDOW q-tables @ ( 1 , 14.14 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
FIND INTEGRAL.CcbCDocu WHERE {&Condicion}
    AND INTEGRAL.CcbCDocu.NroDoc = STRING(s-NroSer,ENTRY(1,x-Formato,'-')) +
                            STRING(f-Numero,ENTRY(2,x-Formato,'-'))
    NO-LOCK NO-ERROR.
IF AVAILABLE INTEGRAL.CcbCDocu THEN output-var-1 = ROWID(INTEGRAL.CcbCDocu).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
