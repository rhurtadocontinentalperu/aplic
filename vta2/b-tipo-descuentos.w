...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c1 COLUMN-LABEL "Tipo!Descuento" FORMAT "x(15)":U
            WIDTH 21.43
      VtaTabla.Libre_c01 COLUMN-LABEL "Descripcion" FORMAT "x(60)":U
            WIDTH 39
      VtaTabla.Libre_c02 COLUMN-LABEL "Considerar!en ?" FORMAT "x(8)":U
            VIEW-AS COMBO-BOX INNER-LINES 5
                      LIST-ITEM-PAIRS "FAC/BOL","FAC",
                                      "N/C","N/C",
                                      "AMBOS","AMBOS"
                      DROP-DOWN-LIST 
  ENABLE
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 77 BY 16.92
         FONT 4.
...
"VtaTabla.Llave_c1" "Tipo!Descuento" "x(15)" "character" ? ? ? ? ? ? yes ? no no "21.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.VtaTabla.Libre_c01
"VtaTabla.Libre_c01" "Descripcion" "x(60)" "character" ? ? ? ? ? ? yes ? no no "39" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaTabla.Libre_c02
"VtaTabla.Libre_c02" "Considerar!en ?" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "FAC/BOL,FAC,N/C,N/C,AMBOS,AMBOS" 5 no 0 no no
...
ON LEAVE OF VtaTabla.Llave_c1 IN BROWSE br_table /* Tipo!Descuento */
DO:
...
ON LEAVE OF VtaTabla.Libre_c01 IN BROWSE br_table /* Descripcion */
DO:
  SELF:SCREEN-VALUE = CAPS(SELF:SCREEN-VALUE).
END.
...
DEFINE VAR x-aplica-en AS CHAR.

x-codigo = vtatabla.llave_c1:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.
x-aplica-en = vtatabla.libre_C02:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.

IF x-codigo = '' THEN DO:
    MESSAGE 'Ingrese el Codigo' VIEW-AS ALERT-BOX ERROR.
...
IF RETURN-VALUE <> "NO" THEN DO:
    FIND FIRST x-vtatabla WHERE x-vtatabla.codcia = s-codcia AND
                            ...
                            x-vtatabla.llave_c1 = x-codigo NO-LOCK NO-ERROR.
    IF AVAILABLE x-vtatabla THEN DO:
        MESSAGE 'Ya existe el registro' VIEW-AS ALERT-BOX ERROR.
        APPLY 'ENTRY':U TO vtatabla.llave_c1 IN BROWSE {&BROWSE-NAME}.
        RETURN 'ADM-ERROR'.
    END.
END.

IF LOOKUP(x-aplica-en,"FAC,N/C,AMBOS") = 0 THEN DO:
    MESSAGE 'Falta ingresar valores' VIEW-AS ALERT-BOX ERROR.
