...
w-report.Campo-C[1] w-report.Campo-F[1] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      w-report.Llave-C COLUMN-LABEL "Código" FORMAT "x(11)":U WIDTH 11.29
      w-report.Campo-C[1] COLUMN-LABEL "Proveedores" FORMAT "X(40)":U
            WIDTH 48.43
      w-report.Campo-F[1] COLUMN-LABEL "Promotor" FORMAT "99":U
            WIDTH 6.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"w-report.Llave-C" "Código" "x(11)" "character" ? ? ? ? ? ? no ? no no "11.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.w-report.Campo-C[1]
"w-report.Campo-C[1]" "Proveedores" "X(40)" "character" ? ? ? ? ? ? no ? no no "48.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.w-report.Campo-F[1]
"w-report.Campo-F[1]" "Promotor" "99" "decimal" ? ? ? ? ? ? no ? no no "6.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  IF AVAILABLE w-report THEN DO:
      RUN vta2/dPidePromotor (OUTPUT pPromotor).
      IF pPromotor > 0 THEN DO:
          FIND CURRENT w-report EXCLUSIVE-LOCK NO-ERROR.
          IF AVAILABLE w-report THEN w-report.campo-f[1] = pPromotor.
  ...
  IF AVAIL almcatvtac THEN DO:      
      ...
      FOR EACH almcatvtad USE-INDEX Index01 NO-LOCK WHERE almcatvtad.codcia = s-codcia
          ...
          IF NOT AVAIL datos THEN DO:
              ...
              /*IF datos.codmat = '031654' THEN MESSAGE datos.pordto pcoddiv s-tpoped.*/
          END.
          PAUSE 0.
      END.
  END.
...
FOR EACH VtaCatVenta NO-LOCK WHERE VtaCatVenta.CodCia = s-codcia
    ...
    IF NOT AVAIL w-report THEN DO:
        ...
        FIND FIRST PEDI2 WHERE PEDI2.Libre_C03 = gn-prov.codpro NO-LOCK NO-ERROR.
        IF AVAILABLE PEDI2 THEN w-report.campo-f[1] = PEDI2.Libre_d02.
    END.
    PAUSE 0.
END.
...
FOR EACH PEDI2 NO-LOCK WHERE PEDI2.codcia = s-codcia AND PEDI2.codped = "PET":
    FIND FIRST datos WHERE datos.codcia = s-codcia
