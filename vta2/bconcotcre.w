...
DEFINE FRAME F-Main
     ...
     "Filtros" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 1 COL 2 WIDGET-ID 96
...
DO:
  DO WITH FRAME {&FRAME-NAME}:
      ...
      /* Consistencias */
      IF TRUE <> (FILL-IN-CodDiv > "") THEN DO:
          MESSAGE 'NO a registrado la división' VIEW-AS ALERT-BOX WARNING.
          RETURN NO-APPLY.
      END.
      IF FILL-IN-FchPed-1 = ? OR FILL-IN-FchPed-2 = ? THEN DO:
          MESSAGE 'Debe registrar ambas fechas de emisión' VIEW-AS ALERT-BOX WARNING.
          APPLY 'ENTRY':U TO FILL-IN-FchPed-1.
          RETURN NO-APPLY.
      END.
      IF ABS(FILL-IN-FchPed-1 - FILL-IN-FchPed-2) > 30 THEN DO:
          MESSAGE 'El rango de fechas registrada supera los 30 días' SKIP
              'Puede que el proceso demore demasiado' SKIP
              'Desea continuar?' VIEW-AS ALERT-BOX QUESTION
...
DEF VAR x-Estados AS CHAR NO-UNDO.

CASE SELECT-FlgEst:
    WHEN "Todos" THEN x-Estados = "P,C".
    WHEN "Atendida en Proceso" OR WHEN "Atendida Total" THEN x-Estados = "C".
    WHEN "Pendiente" OR WHEN "En Proceso" THEN x-Estados = "P".
    OTHERWISE RETURN.
END CASE.
...
DO k = 1 TO NUM-ENTRIES(x-Estados):
    PRINCIPAL:
    FOR EACH Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-codcia AND
        Faccpedi.coddiv = FILL-IN-CodDiv AND
        Faccpedi.coddoc = "COT" AND
        Faccpedi.flgest = ENTRY(k, x-Estados) AND
        FacCPedi.FchPed >= FILL-IN-FchPed-1 AND
        FacCPedi.FchPed <= FILL-IN-FchPed-2:
        ...
        CASE SELECT-FlgEst:
            WHEN "Todos" THEN DO:
                /* No filtra nada */
                CREATE T-CPEDI.
                BUFFER-COPY FacCPedi TO T-CPEDI ASSIGN T-CPEDI.Libre_c02 = _FlgEst().
            END.
            /* ESTE GRUPO CUANDO FacCPedi.FlgEst = "P" */
            WHEN "Pendiente" THEN DO:
                IF CAN-FIND(FIRST FacDPedi OF FacCPedi WHERE FacDPedi.CanAte > 0 NO-LOCK) THEN NEXT.
                CREATE T-CPEDI.
                BUFFER-COPY FacCPedi TO T-CPEDI ASSIGN T-CPEDI.Libre_c02 = "PENDIENTE".
            END.
            WHEN "En Proceso" THEN DO:
                IF NOT CAN-FIND(FIRST FacDPedi OF FacCPedi WHERE FacDPedi.CanAte > 0 NO-LOCK) THEN NEXT.
                CREATE T-CPEDI.
                BUFFER-COPY FacCPedi TO T-CPEDI ASSIGN T-CPEDI.Libre_c02 = "EN PROCESO".
            END.
            /* ESTE GRUPO CUANDO FacCPedi.FlgEst = "C" */
            WHEN "Atendida Total" OR WHEN "Atendida en Proceso" THEN DO:
                CREATE T-CPEDI.
                BUFFER-COPY FacCPedi TO T-CPEDI ASSIGN T-CPEDI.Libre_c02 = _FlgEst().
/*                 BUFFER-COPY FacCPedi TO T-CPEDI ASSIGN T-CPEDI.Libre_c02 = "ATENDIDA TOTAL".   /* Por defecto */ */
/*                 RLOOP:                                                                                           */
...
/*                 END.                                                                                             */
                ...
                IF T-CPEDI.Libre_c02 <> SELECT-FlgEst THEN DELETE T-CPEDI.
            END.
        END CASE.
    END.
END.

/* FOR EACH T-CPEDI, FIRST Faccpedi OF T-CPEDI NO-LOCK:                                                                         */
/*     CASE T-CPEDI.Libre_c02:                                                                                                  */
/*         WHEN "PENDIENTE" THEN DO:                                                                                            */
/*             IF CAN-FIND(FIRST FacDPedi OF FacCPedi WHERE FacDPedi.CanAte > 0 NO-LOCK) THEN T-CPEDI.Libre_c02 = "EN PROCESO". */
/*         END.                                                                                                                 */
/*         WHEN "ATENDIDA TOTAL" THEN DO:                                                                                       */
/*             RLOOP:                                                                                                           */
/*             FOR EACH PEDIDOS NO-LOCK WHERE PEDIDOS.codcia = Faccpedi.codcia                                                  */
/*                 AND PEDIDOS.coddiv = Faccpedi.coddiv                                                                         */
/*                 AND PEDIDOS.coddoc = "PED"                                                                                   */
...
CASE Faccpedi.FlgEst:
    WHEN "P" THEN DO:
        IF CAN-FIND(FIRST FacDPedi OF FacCPedi WHERE FacDPedi.CanAte > 0 NO-LOCK) THEN pEstado = "EN PROCESO".
    END.
    WHEN "C" THEN DO:
        RLOOP:
        FOR EACH PEDIDOS NO-LOCK WHERE PEDIDOS.codcia = Faccpedi.codcia
