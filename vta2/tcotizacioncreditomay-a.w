...
fDesMat() @ x-DesMat fDesMar() @ x-DesMar ITEM.UndVta ITEM.TipVta ~
ITEM.CanPed ITEM.PreUni ITEM.Por_Dsctos[1] ITEM.Por_Dsctos[2] ~
ITEM.Por_Dsctos[3] ITEM.CanApr ITEM.ImpLin 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table ITEM.codmat ITEM.TipVta ~
ITEM.CanPed ITEM.PreUni 
...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-ImpTot-2 FILL-IN-ImpPer ~
FILL-IN-ImpTot 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      ITEM.codmat COLUMN-LABEL "Articulo" FORMAT "X(14)":U COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 11
      ...
      ITEM.TipVta COLUMN-LABEL "Lista" FORMAT "X(1)":U WIDTH 3.57
      ...
      ITEM.ImpLin FORMAT ">,>>>,>>9.99":U WIDTH 9.72
  ENABLE
      ITEM.codmat
      ITEM.TipVta HELP "Ingres A, B o deje en blanco"
      ITEM.CanPed
      ITEM.PreUni
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SIZE 141 BY 11.73
...
DEFINE FRAME F-Main
     ...
     FILL-IN-ImpTot-2 AT ROW 12.73 COL 76 COLON-ALIGNED WIDGET-ID 6
     FILL-IN-ImpPer AT ROW 12.73 COL 100 COLON-ALIGNED WIDGET-ID 4
     FILL-IN-ImpTot AT ROW 12.73 COL 126 COLON-ALIGNED WIDGET-ID 2
...
"ITEM.codmat" "Articulo" "X(14)" "character" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > "_<CALC>"
"fDesMat() @ x-DesMat" "Descripción" "x(60)" ? ? ? ? ? ? ? no ? no no "44.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"ITEM.UndVta" "Unidad" "x(10)" "character" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.ITEM.TipVta
"ITEM.TipVta" "Lista" ? "character" 11 0 ? ? ? ? yes "Ingres A, B o deje en blanco" no no "3.57" yes no no "U" "" "" "FILL-IN" "," ? ? 0 no 0 no no
...
"ITEM.PreUni" "Precio!Unitario" ">>,>>9.9999" "decimal" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.ITEM.Por_Dsctos[1]
"ITEM.Por_Dsctos[1]" "% Dscto.!Admins" "->>9.9999" "decimal" ? ? ? ? ? ? no ? no no "6" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > Temp-Tables.ITEM.Por_Dsctos[2]
"ITEM.Por_Dsctos[2]" "% Dscto!Evento" "->>9.9999" "decimal" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"ITEM.Por_Dsctos[3]" "% Dscto!Vol/Prom" "->>9.9999" "decimal" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > Temp-Tables.ITEM.CanApr
"ITEM.CanApr" "Percep.!Aprox." ">>>>9.99" "decimal" 14 0 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > Temp-Tables.ITEM.ImpLin
"ITEM.ImpLin" ? ">,>>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "9.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    RUN vta2/PrecioMayorista-Cred-v2 (
        ...
        INPUT ITEM.TipVta:SCREEN-VALUE IN BROWSE {&browse-name},
        TRUE
        ).
...
&Scoped-define SELF-NAME ITEM.TipVta
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ITEM.TipVta br_table _BROWSE-COLUMN B-table-Win
ON LEAVE OF ITEM.TipVta IN BROWSE br_table /* Lista */
DO:
    SELF:SCREEN-VALUE = CAPS(SELF:SCREEN-VALUE).
    IF NOT (SELF:SCREEN-VALUE = '' OR 
        SELF:SCREEN-VALUE = 'A' OR
        SELF:SCREEN-VALUE = 'B')
        THEN DO:
        MESSAGE 'Solo se permiten los valores A o B' SKIP
            'O deje en blanco' VIEW-AS ALERT-BOX WARNING.
        RETURN NO-APPLY.
    END.
    FIND Almmmatg WHERE Almmmatg.codcia = s-codcia
...
ON 'RETURN':U OF ITEM.CanPed, ITEM.codmat, ITEM.UndVta, ITEM.PreUni, ITEM.TipVta
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  END.
  /* ************************************ */
  /* RHC 14/10/2013 PARCHE PRODUCTOS DE TERCEROS */
  IF Almmmatg.CHR__02 <> "T" THEN ITEM.TipVta = "".
...
------------------------------------------------------------------------------*/
  ...
  RUN Procesa-Handle IN lh_handle ('Enable-Head').
  x-Adm-New-Record = "NO".
  IF s-status-record = "add-record" THEN RUN Procesa-Handle IN lh_handle ('Add-Record-A'). 
...
/*                                    OUTPUT s-StkComprometido).                         */
  ...
  IF LOOKUP(pCodDiv, "00015,10015,20015") > 0 THEN RETURN "OK".       /* NO MARGEN DE UTILIDAD */
...
/*         ITEM.TipVta:READ-ONLY IN BROWSE {&browse-name} = (IF Almmmatg.CHR__02 = "T" THEN NO ELSE YES). */
END.
RETURN "OK".
