...
DEFINE VAR x-tabla AS CHAR INIT "CV-AMBITO-APLICA".
...
&Scoped-define QUERY-STRING-br_table FOR EACH t-w-report WHERE TRUE /* Join to gn-ConVt incomplete */ NO-LOCK ~
    BY t-w-report.Campo-C[1]
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH t-w-report WHERE TRUE /* Join to gn-ConVt incomplete */ NO-LOCK ~
    BY t-w-report.Campo-C[1].
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      t-w-report.Campo-L[1] COLUMN-LABEL "" FORMAT "Si/No":U WIDTH 4.14
            VIEW-AS TOGGLE-BOX
      t-w-report.Campo-C[1] COLUMN-LABEL "Grupo de Aplicacion" FORMAT "X(80)":U
  ENABLE
      t-w-report.Campo-L[1]
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS SIZE 53.72 BY 9.31
         FONT 4.
...
DEFINE FRAME F-Main
     ...
     BUTTON-10 AT ROW 9.27 COL 56 WIDGET-ID 2
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.t-w-report WHERE INTEGRAL.gn-ConVt <external> ..."
     _Options          = "NO-LOCK KEY-PHRASE"
     _OrdList          = "Temp-Tables.t-w-report.Campo-C[1]|yes"
     _FldNameList[1]   > Temp-Tables.t-w-report.Campo-L[1]
"t-w-report.Campo-L[1]" "" ? "logical" ? ? ? ? ? ? yes ? no no "4.14" yes no no "U" "" "" "TOGGLE-BOX" "," ? ? 5 no 0 no no
     _FldNameList[2]   > Temp-Tables.t-w-report.Campo-C[1]
"t-w-report.Campo-C[1]" "Grupo de Aplicacion" "X(80)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON CHOOSE OF BUTTON-10 IN FRAME F-Main /* Grabar */
DO:
  RUN grabar.
END.
...
FOR EACH factabla WHERE factabla.codcia = s-codcia AND
    ...
    CREATE t-w-report.
        ASSIGN t-w-report.task-no = 999
                t-w-report.campo-c[1] = factabla.nombre
                t-w-report.campo-c[10] = factabla.codigo
                t-w-report.campo-l[1] = NO
                t-w-report.campo-l[2] = NO
            .

        FIND FIRST vtatabla WHERE vtatabla.codcia = s-codcia AND
                                    vtatabla.tabla = 'CV-AMBITO-GRUPO' AND
                                    vtatabla.llave_c1 = gn-convt.codig AND      
                                    vtatabla.llave_c2 = factabla.codigo NO-LOCK NO-ERROR.
...
FOR EACH t-w-report WHERE t-w-report.campo-l[2] = YES AND t-w-report.campo-l[1] = NO:
    FIND FIRST vtatabla WHERE vtatabla.codcia = s-codcia AND
                                vtatabla.tabla = 'CV-AMBITO-GRUPO' AND
                                vtatabla.llave_c1 = gn-convt.codig AND      
                                vtatabla.llave_c2 = t-w-report.campo-c[10] EXCLUSIVE-LOCK NO-ERROR.
    IF AVAILABLE vtatabla THEN DO:
       DELETE vtatabla.
    END.
    ...
    IF NOT AVAILABLE vtatabla THEN DO:
       CREATE vtatabla.
        ASSIGN vtatabla.codcia = s-codcia
                vtatabla.tabla = 'CV-AMBITO-GRUPO'
                vtatabla.llave_c1 = gn-convt.codig
                vtatabla.llave_c2 = t-w-report.campo-c[10].
    END.
