...
DEF BUFFER B-COTI  FOR Faccpedi.
...
&Scoped-define FIELDS-IN-QUERY-br_table FacCPedi.NroPed FacCPedi.FchPed FacCPedi.CodCli FacCPedi.NomCli FacCPedi.ImpTot   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define SELF-NAME br_table
&Scoped-define OPEN-QUERY-br_table CASE COMBO-BOX-1:     WHEN 'Todos' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.DivDes = s-coddiv          AND FacCPedi.CodDoc = s-coddoc /*          AND FacCPedi.Usuario = s-user-id */         AND FacCPedi.Libre_c02 <> "O/D"          AND FacCPedi.FlgEst = s-flgest NO-LOCK BY Faccpedi.FchPed DESC.     WHEN 'Inicien con' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.DivDes = s-coddiv          AND FacCPedi.CodDoc = s-coddoc /*          AND FacCPedi.Usuario = s-user-id */          AND FacCPedi.FlgEst = s-flgest         AND FacCPedi.Libre_c02 <> "O/D"          AND FacCPedi.NomCli BEGINS x-NomCli NO-LOCK BY Faccpedi.FchPed DESC.     WHEN 'Contengan' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.DivDes = s-coddiv          AND FacCPedi.CodDoc = s-coddoc /*          AND FacCPedi.Usuario = s-user-id */         AND FacCPedi.Libre_c02 <> "O/D"          AND FacCPedi.FlgEst = s-flgest          AND INDEX(FacCPedi.NomCli, ~
       x-NomCli) > 0 NO-LOCK BY Faccpedi.FchPed DESC. END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _FREEFORM
  QUERY br_table NO-LOCK DISPLAY
      FacCPedi.NroPed COLUMN-LABEL "<<<Orden>>>>"
      FacCPedi.FchPed COLUMN-LABEL "<<Emision>>"
      FacCPedi.CodCli COLUMN-LABEL "<<<Cliente>>>"
      FacCPedi.NomCli COLUMN-LABEL "Nombre"
      FacCPedi.ImpTot COLUMN-LABEL "Importe"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/*          AND FacCPedi.Usuario = s-user-id */
        AND FacCPedi.Libre_c02 <> "O/D"
         AND FacCPedi.FlgEst = s-flgest NO-LOCK BY Faccpedi.FchPed DESC.
...
/*          AND FacCPedi.Usuario = s-user-id */
        ...
        AND FacCPedi.Libre_c02 <> "O/D"
         AND FacCPedi.NomCli BEGINS x-NomCli NO-LOCK BY Faccpedi.FchPed DESC.
    WHEN 'Contengan' THEN
        OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}
              AND FacCPedi.CodCia = s-codcia
         AND FacCPedi.DivDes = s-coddiv
...
/*          AND FacCPedi.Usuario = s-user-id */
        AND FacCPedi.Libre_c02 <> "O/D"
         AND FacCPedi.FlgEst = s-flgest
         AND INDEX(FacCPedi.NomCli, x-NomCli) > 0 NO-LOCK BY Faccpedi.FchPed DESC.
END.
     _END_FREEFORM
...
MESSAGE 'Desea CERRAR la Ordene de Despacho?'
    VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
    UPDATE rpta AS LOG.
IF rpta = NO THEN RETURN.
RLOOP:
DO i = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME} TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(i) IN FRAME {&FRAME-NAME} THEN DO:
        FIND CURRENT FacCPedi EXCLUSIVE-LOCK NO-ERROR.
        IF NOT AVAILABLE FacCPedi THEN NEXT RLOOP.
        ASSIGN
            FacCPedi.FlgEst = 'S'         /* << OJO << */
        ...
        FIND B-CPEDI WHERE B-CPEDI.codcia = s-codcia
            ...
            AND B-CPEDI.coddoc = Faccpedi.codref    /* PED */
            AND B-CPEDI.nroped = Faccpedi.nroref
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE B-CPEDI THEN NEXT.
        /* ACTUALIZAMOS SALDO DEL PEDIDO SIN CAMBIAR SU ESTADO */
        ...
        END.    
        /* *************************************************** */
        /* ACTUALIZAMO SALDO DE LA COTIZACION Y ACTUALIZAMOS SU ESTADO */
        x-NroCot = B-CPEDI.NroRef.
        FOR EACH Facdpedi OF Faccpedi:
            FIND FIRST B-DPEDI WHERE B-DPEDI.CodCia = Faccpedi.CodCia 
                AND  B-DPEDI.CodDiv = B-CPEDI.CodDiv
                AND  B-DPEDI.CodDoc = B-CPEDI.CodRef    /* COT */
                AND  B-DPEDI.NroPed = B-CPEDI.NroRef
        ...
        END.    
        FIND B-COTI WHERE B-COTI.CodCia = S-CODCIA 
            AND B-COTI.CodDiv = B-CPEDI.CodDiv
            AND B-COTI.CodDoc = B-CPEDI.CodRef    /* COT */
            AND B-COTI.NroPed = B-CPEDI.NroRef
            EXCLUSIVE-LOCK NO-ERROR.
        IF AVAILABLE B-COTI THEN B-COTI.FlgEst = "P".
        /* *********************************************************** */
    END.
END.
...
IF AVAILABLE(B-COTI)   THEN RELEASE B-COTI.
RUN dispatch IN THIS-PROCEDURE ('open-query':U).
