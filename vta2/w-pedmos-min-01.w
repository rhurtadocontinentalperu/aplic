...
DEFINE VARIABLE h_b-pedmos-4min AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv05 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-pedido-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-pedmos-min-01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-pedmos-min-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-Canc AT ROW 3.15 COL 137 WIDGET-ID 12
     RECT-24 AT ROW 2.62 COL 2 WIDGET-ID 16
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 145.14 BY 26.96
         FONT 4.
...
DO:
  ...
  RUN Cancelar-Pedido IN h_v-pedmos-min-01.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-pedmos-min-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedmos-min-01 ).
       RUN set-position IN h_v-pedmos-min-01 ( 2.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.15 , 131.00 ) */
       ...
       /* Links to SmartViewer h_v-pedmos-min-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-pedmos-min-01 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_v-pedmos-min-01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedmos-min-01 ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedmos-4min ).
       RUN set-position IN h_b-pedmos-4min ( 11.50 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedmos-4min ( 15.88 , 130.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pedmos-4min. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_b-pedmos-4min ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedmos-4min ,
             BUTTON-Canc:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-pedmos-min-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmos-min-01 ).
       RUN set-position IN h_t-pedmos-min-01 ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedmos-min-01 ( 15.88 , 131.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-pedmos-min-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedmos-min-01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedmos-min-01 ,
             BUTTON-Canc:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-pedmos-min-01 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_b-pedmos-4min <> ? THEN RUN dispatch IN h_b-pedmos-4min ('open-query':U).
          IF h_t-pedmos-min-01 <> ? THEN RUN dispatch IN h_t-pedmos-min-01 ('open-query':U).
...
/*       END.                                            */
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-pedmos-min-01 <> ? THEN RUN Recalcular-Precios IN h_t-pedmos-min-01.
