...
/* 13/09/2021: Nota de Pedido de la División Activa */
&SCOPED-DEFINE Condicion (AlmCatVtaC.CodCia = s-codcia ~
    AND AlmCatVtaC.CodPro = w-report.Llave-C ~
...
DEFINE VARIABLE CanItem AS INT NO-UNDO.
...
DEFINE TEMP-TABLE tmp-detalle 
    FIELDS t-nropag AS INT
    FIELDS t-nrosec AS INT
    FIELDS t-codmat AS INT
    FIELDS t-desmat AS CHAR
    FIELDS t-desmar AS CHAR
    FIELDS t-undbas AS CHAR
    FIELDS t-precio AS DEC
    FIELDS t-canped AS DEC.
...
&Scoped-define QUERY-STRING-br_table FOR EACH AlmCatVtaC WHERE TRUE /* Join to w-report incomplete */ ~
      AND {&Condicion} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH AlmCatVtaC WHERE TRUE /* Join to w-report incomplete */ ~
      AND {&Condicion} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&Scoped-Define ENABLED-OBJECTS RECT-71 br_table 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      AlmCatVtaC.NroPag COLUMN-LABEL "Nro Pagina" FORMAT "9999":U
      AlmCatVtaC.DesPag FORMAT "x(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 51.72 BY 7.38
         FONT 4
         TITLE "Páginas de Catálogo".


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     br_table AT ROW 1.31 COL 2.29
     RECT-71 AT ROW 1.12 COL 1.57 WIDGET-ID 2
...
/* BROWSE-TAB br_table RECT-71 F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.AlmCatVtaC WHERE INTEGRAL.w-report <external> ... ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "{&Condicion}"
     _FldNameList[1]   > INTEGRAL.AlmCatVtaC.NroPag
"AlmCatVtaC.NroPag" "Nro Pagina" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.AlmCatVtaC.DesPag
"AlmCatVtaC.DesPag" ? "x(50)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Páginas de Catálogo */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Páginas de Catálogo */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Páginas de Catálogo */
DO:
...
FOR EACH almcatvtad WHERE almcatvtad.codcia = s-codcia
    ...
    BREAK BY almcatvtad.nropag DESC
        BY almcatvtad.nrosec DESC:
