...
DEFINE VAR cBultoAnterior AS CHAR INIT "".
...
Almmmatg.DesMat Almmmatg.DesMar Detalle.UndVta Detalle.CanPick ~
Detalle.Factor Detalle.CanPed Detalle.Libre_c01 
...
FILL-IN-NroEtq txtTTrans TOGGLE-1 BUTTON-13 COMBO-BOX-impresoras BUTTON-15 ~
RADIO-SET-impresion-rotulo 
&Scoped-Define DISPLAYED-OBJECTS txtTTxx x-CodDoc x-CodCli ~
FILL-IN-AlmacenXD x-NroDoc x-NomCli x-DestinoFinal FILL-IN-NroEtq x-FchDoc ~
txtTTrans TOGGLE-1 FILL-IN-Embalado COMBO-BOX-impresoras ~
RADIO-SET-impresion-rotulo x-NroItem FILL-IN-Bultos 
...
DEFINE VARIABLE FILL-IN-Embalado AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 54 BY 1.08
     BGCOLOR 0 FGCOLOR 14 FONT 8 NO-UNDO.
...
DEFINE VARIABLE x-DestinoFinal AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 53 BY .81
     BGCOLOR 14 FGCOLOR 0  NO-UNDO.
...
DEFINE VARIABLE RADIO-SET-impresion-rotulo AS INTEGER INITIAL 1 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "No imprimir", 1,
"Al cerrar bulto", 2,
"Al cerrar la orden", 3
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Detalle.UndVta COLUMN-LABEL "Unidad" FORMAT "XXXXXXX":U WIDTH 5.86
      Detalle.CanPick COLUMN-LABEL "Cantidad" FORMAT ">,>>>,>>9.9999":U
            WIDTH 7.43
      Detalle.Factor FORMAT ">>,>>9.9999":U WIDTH 6.43
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 127 BY 17.19
...
DEFINE FRAME F-Main
     br_table AT ROW 8.81 COL 1
     txtTTxx AT ROW 3.96 COL 55.29 RIGHT-ALIGNED NO-LABEL WIDGET-ID 20
     x-CodDoc AT ROW 1.27 COL 14 COLON-ALIGNED WIDGET-ID 12
     x-CodCli AT ROW 2.08 COL 14 COLON-ALIGNED
     FILL-IN-AlmacenXD AT ROW 2.88 COL 14 COLON-ALIGNED WIDGET-ID 34
     x-NroDoc AT ROW 1.27 COL 21 COLON-ALIGNED NO-LABEL
     x-NomCli AT ROW 2.08 COL 27 COLON-ALIGNED NO-LABEL
     x-DestinoFinal AT ROW 2.88 COL 27 COLON-ALIGNED NO-LABEL WIDGET-ID 36
     FILL-IN-NroEtq AT ROW 6.12 COL 25 COLON-ALIGNED WIDGET-ID 24
     x-FchDoc AT ROW 1.27 COL 58 COLON-ALIGNED
     txtTTrans AT ROW 3.96 COL 54 COLON-ALIGNED WIDGET-ID 14
     TOGGLE-1 AT ROW 6.12 COL 58 WIDGET-ID 28
     BUTTON-13 AT ROW 1 COL 70 WIDGET-ID 4
     FILL-IN-Embalado AT ROW 5.85 COL 74 COLON-ALIGNED NO-LABEL WIDGET-ID 40
     COMBO-BOX-impresoras AT ROW 7.92 COL 77 COLON-ALIGNED WIDGET-ID 160
     BUTTON-15 AT ROW 1 COL 84 WIDGET-ID 8
     RADIO-SET-impresion-rotulo AT ROW 7.04 COL 90 NO-LABEL WIDGET-ID 44
     x-NroItem AT ROW 2.35 COL 90.14 COLON-ALIGNED WIDGET-ID 10
     FILL-IN-Bultos AT ROW 4.08 COL 95 COLON-ALIGNED NO-LABEL WIDGET-ID 30
     "Impresión de rotulos (Zebra)" VIEW-AS TEXT
          SIZE 19.29 BY .81 AT ROW 7.04 COL 70.29 WIDGET-ID 48
     "BULTOS:" VIEW-AS TEXT
          SIZE 15 BY 1.15 AT ROW 4.08 COL 82 WIDGET-ID 32
          BGCOLOR 0 FGCOLOR 14 FONT 8
     RECT-65 AT ROW 1.04 COL 1.14
     RECT-66 AT ROW 3.77 COL 1.29 WIDGET-ID 22
     SPACE(20.00) SKIP(0.00)
...
/* SETTINGS FOR FILL-IN FILL-IN-Bultos IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FILL-IN-Embalado IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
...
/* SETTINGS FOR FILL-IN x-CodCli IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN x-DestinoFinal IN FRAME F-Main
   NO-ENABLE                                                            */
...
"Detalle.UndVta" "Unidad" "XXXXXXX" "character" ? ? ? ? ? ? no ? no no "5.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.Detalle.CanPick
"Detalle.CanPick" "Cantidad" ? "decimal" ? ? ? ? ? ? yes ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.Detalle.Factor
"Detalle.Factor" ? ? "decimal" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.Detalle.CanPed
"Detalle.CanPed" "Cantidad!Calculada" ? "decimal" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.Detalle.Libre_c01
"Detalle.Libre_c01" "Número de Etiqueta" "x(30)" "character" ? ? ? ? ? ? no ? no no "17.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  ASSIGN radio-set-impresion-rotulo combo-box-impresoras.
    ...
    yCodDoc = x-coddoc:SCREEN-VALUE.
    yNroDoc = x-nrodoc:SCREEN-VALUE.
    
    IF radio-set-impresion-rotulo = 3 THEN DO:
        /*
        /* Los Items */
        FOR EACH detalle WHERE Detalle.Libre_c01 BEGINS cBultoAnterior NO-LOCK:
            CREATE tmp-w-report.
              ASSIGN tmp-w-report.LLave-I = Detalle.NroItm
                tmp-w-report.Campo-C[2] = Detalle.codmat
                tmp-w-report.Campo-C[3] = Detalle.UndVta
                tmp-w-report.Campo-F[1] = Detalle.CanPed
                tmp-w-report.Campo-F[2] = Detalle.CanPick
                tmp-w-report.Campo-F[3] = Detalle.Factor
                tmp-w-report.Campo-C[4] = Detalle.Libre_C01.
        END.
        */
        xOrigen = "SECTORES CIERREORDEN".
    
      RUN logis/logis-library.r PERSISTENT SET hProc.
    
        /* Imprimir el Rotulos de la orden */
      RUN rotulos-sectores-online IN hProc (INPUT s-coddiv, 
                                         INPUT yCoddoc, 
                                         INPUT yNroDoc,
...
&Scoped-define SELF-NAME COMBO-BOX-impresoras
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL COMBO-BOX-impresoras B-table-Win
ON LEAVE OF COMBO-BOX-impresoras IN FRAME F-Main /* Impresoras */
DO:
  ASSIGN {&SELF-NAME}.
END.
...
&Scoped-define SELF-NAME RADIO-SET-impresion-rotulo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL RADIO-SET-impresion-rotulo B-table-Win
ON LEAVE OF RADIO-SET-impresion-rotulo IN FRAME F-Main
DO:
  ASSIGN {&SELF-NAME}.
END.
...
DO:
  ...
  radio-set-impresion-rotulo:VISIBLE = FALSE.
...
OR RETURN OF {&SELF-NAME} DO:
        ...
        IF x-NroDoc <> SELF:SCREEN-VALUE THEN DO:
            ...
            radio-set-impresion-rotulo:VISIBLE = TRUE.
            /*ocxTimer:ENABLED = TRUE.*/
            ASSIGN
                x-FchIni = TODAY
                x-HorIni = STRING(TIME, 'HH:MM:SS').
            ASSIGN {&SELF-NAME} x-coddoc.
            ASSIGN x-NroItem = FacCPedi.Items.
            /*RUN Calcula-NroItems.*/
            RUN Carga-con-kits.
            RUN Recarga-Items-Backup.   /* Inicializa s-task-no a cero */
            ASSIGN
            ...
            /* RHC 19/07/2018 Cross Docking */
            IF Faccpedi.CrossDocking = YES THEN DO:
                x-CodCli:LABEL = 'Destino Intermedio'.
                FILL-IN-AlmacenXD = Faccpedi.AlmacenXD.
                FIND Almacen WHERE Almacen.CodCia = s-codcia AND
                    Almacen.CodAlm = Faccpedi.AlmacenXD
                    NO-LOCK NO-ERROR.
                IF AVAILABLE Almacen THEN x-DestinoFinal = Almacen.Descripcion.
                ELSE DO:
                    FIND gn-clie WHERE gn-clie.codcia = cl-codcia
                        AND gn-clie.codcli = Faccpedi.AlmacenXD
                        NO-LOCK NO-ERROR.
                    IF AVAILABLE gn-clie THEN x-DestinoFinal = gn-clie.nomcli.
                END.
            END.
            DISPLAY
                x-fchdoc
        ...
        IF FacCPedi.EmpaqEspec = YES THEN DO:
            FILL-IN-Embalado:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "EMBALAR CON STRECH FILM".
        END.
        ELSE DO:
            FILL-IN-Embalado:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
        END.

        ASSIGN cBultoAnterior = "".
END.
...
------------------------------------------------------------------------------*/
  ...
  /* Cargamos nuevamente los items de la OTR (si alguien lo ha modificado) */
  RUN Carga-con-kits.

  /* Verificamos si se ha chequeado correctamente */
  ...
  FOR EACH T-DPEDI:
      ...
      IF NOT AVAILABLE C-Detalle THEN DO:
          l-Mensaje = l-Mensaje + (IF TRUE <> (l-Mensaje > '') THEN '' ELSE CHR(10)) + T-DPEDI.codmat.
          l-Error = YES.
          NEXT.
      END.
      IF C-Detalle.CanPed <> T-DPEDI.CanPed THEN DO:
          pMensaje = 'Diferencia de cantidades en el artículo' + CHR(10) + 
              T-DPEDI.codmat + " " + Almmmatg.desmat + CHR(10) +
              'DICE: ' + STRING(C-Detalle.CanPed) + CHR(10) + 
              'DEBE DECIR: ' + STRING(T-DPEDI.CanPed).
          FIND FIRST Detalle WHERE Detalle.codmat = c-Detalle.codmat.
          REPOSITION {&BROWSE-NAME} TO ROWID ROWID(Detalle).
          RETURN 'ADM-ERROR'.
      END.
  END.        
  IF l-Error = YES THEN DO:
      pMensaje = 'NO se han registrado los siguientes artículos:' + CHR(10) + l-Mensaje.
      RETURN 'ADM-ERROR'.
  END.
  /* Usuario de Chequeo */
  ...
  DEF VAR pMetros AS DEC NO-UNDO INIT 0.
  ...
  /* ************************************************************** */
  /* Empaque especial */
  FIND x-Faccpedi WHERE x-Faccpedi.codcia = s-codcia 
      AND x-Faccpedi.coddoc = x-coddoc:SCREEN-VALUE IN FRAME {&FRAME-NAME}
      AND x-Faccpedi.nroped = x-nrodoc:SCREEN-VALUE IN FRAME {&FRAME-NAME}
      NO-LOCK NO-ERROR.
  IF AVAILABLE x-Faccpedi AND x-faccpedi.empaqesp = YES THEN DO:
      RUN logis/d-rollos-film.r (OUTPUT pMetros, OUTPUT pOk, INPUT x-Faccpedi.coddoc, INPUT x-Faccpedi.nroped).
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      ASSIGN 
          ...
          faccpedi.acubon[7] = pMetros.         /* Embalado especial */
      /* RHC 25.08.2011 */
...
OCXFile = SEARCH( "b-picking-ordenes.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "b-picking-ordenes.wrx":U SKIP(1)
...
END.
ELSE DO:
    /* ESTO NO OCURRE -> NO se puede modificar un registro */
    FIND Logisdpiqueo WHERE Logisdpiqueo.CodCia = s-codcia
        AND Logisdpiqueo.CodDiv = s-coddiv
        AND Logisdpiqueo.CodPed = x-CodDoc 
        AND Logisdpiqueo.NroPed = x-NroDoc
        AND Logisdpiqueo.IDUsuario = STRING(ROWID(Detalle))
        EXCLUSIVE-LOCK NO-ERROR.
    IF NOT AVAILABLE Logisdpiqueo THEN RETURN.
END.
ASSIGN
    Logisdpiqueo.CodCia = s-codcia
    Logisdpiqueo.CodDiv = s-coddiv
    Logisdpiqueo.CodPed = x-CodDoc 
    Logisdpiqueo.NroPed = x-NroDoc
    Logisdpiqueo.NroItm = Detalle.NroItm
    Logisdpiqueo.IDUsuario = STRING(ROWID(Detalle))
    Logisdpiqueo.CodMat = Detalle.codmat 
    Logisdpiqueo.UndVta = Detalle.UndVta
    Logisdpiqueo.CanPed = Detalle.CanPed
    Logisdpiqueo.CanChk = Detalle.CanPick
    Logisdpiqueo.Factor = Detalle.Factor
    Logisdpiqueo.Etiqueta = Detalle.Libre_C01.
RELEASE Logisdpiqueo.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE imprimir-rotulo B-table-Win 
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR xBultoCerrado AS CHAR.

  DO WITH FRAME {&FRAME-NAME}:
      ASSIGN radio-set-impresion-rotulo combo-box-impresoras toggle-1.
  END.

  xBultoCerrado = fill-in-NroEtq.
  
  DEFINE VAR cNuevoBulto AS CHAR.
  DEFINE VAR xOrigen AS CHAR.
  DEFINE VAR hProc AS HANDLE.
  DEFINE VAR yCodDoc AS CHAR.
  DEFINE VAR yNroDoc AS CHAR.
  ...
  IF radio-set-impresion-rotulo = 2 AND NOT (TRUE <> (xBultoCerrado > "")) THEN DO:

      /* Los Items */
      FOR EACH detalle WHERE Detalle.Libre_c01 BEGINS xBultoCerrado NO-LOCK:
          IF toggle-1 = NO THEN DO:     /* NO ingreso multiple */
            IF Detalle.Libre_c01 <> xBultoCerrado THEN NEXT.
          END.
          CREATE tmp-w-report.
            ASSIGN tmp-w-report.LLave-I = Detalle.NroItm
              tmp-w-report.Campo-C[2] = Detalle.codmat
              ...
              tmp-w-report.Campo-F[3] = Detalle.Factor
              tmp-w-report.Campo-C[4] = Detalle.Libre_C01
              tmp-w-report.Campo-I[15] = Detalle.sdocot.
      END.

      /*xOrigen = "SECTORES CIERREBULTO|" + xBultoCerrado + "|" + fill-in-bultos:SCREEN-VALUE.*/
      xOrigen = "SECTORES CIERREBULTO".

    RUN logis/logis-library.r PERSISTENT SET hProc.

      /* Imprimir el Rotulo del bulto (cBultoAnterior) */
    RUN rotulos-sectores-online IN hProc (INPUT s-coddiv, 
                                       INPUT yCoddoc, 
                                       INPUT yNroDoc,
                                       INPUT TABLE tmp-w-report,
...
/*------------------------------------------------------------------------------
  Purpose:     Override standard ADM method
  Notes:       OJO >>> NO SE PUEDE MODIFICAR EL REGISTRO
------------------------------------------------------------------------------*/
  ...
  DEF VAR xFILL-IN-Bultos AS DEC NO-UNDO.
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  xFILL-IN-Bultos = INTEGER(fill-in-bultos:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
  ...
  IF s-generacion-multiple = YES AND x-Factor >= 1 THEN DO:
        ...
        xFILL-IN-Bultos = 0.
        FOR EACH B-Detalle BREAK BY B-Detalle.Libre_C01:
            IF FIRST-OF(B-Detalle.Libre_C01) THEN xFILL-IN-Bultos = xFILL-IN-Bultos + 1.
        END.
      ...
      DO k = 1 TO x-Cantidad:
          ...
          END.
          xFILL-IN-Bultos = xFILL-IN-Bultos + 1.
          ASSIGN
              ...
              detalle.sdocot = xFILL-IN-Bultos.
...
s-task-no = 0.  /* INICIALIZAMOS CONTROL */
EMPTY TEMP-TABLE Detalle.   /* LIMPIAMOS TEMPORAL */

FOR EACH Logisdpiqueo EXCLUSIVE-LOCK WHERE logisdpiqueo.CodCia = s-codcia
    AND logisdpiqueo.CodDiv = s-coddiv
    AND logisdpiqueo.CodPed = x-CodDoc 
    AND logisdpiqueo.NroPed = x-NroDoc:
    CREATE Detalle.
    ASSIGN
        Detalle.codcia = logisdpiqueo.CodCia 
        Detalle.coddiv = logisdpiqueo.CodDiv 
        Detalle.nroitm = logisdpiqueo.NroItm
        Detalle.codmat = logisdpiqueo.CodMat 
        Detalle.undvta = logisdpiqueo.UndVta 
        Detalle.canped = logisdpiqueo.CanPed 
        Detalle.canpick = logisdpiqueo.CanChk 
        Detalle.factor = logisdpiqueo.Factor 
        Detalle.libre_c01 = logisdpiqueo.Etiqueta 
        .
    ASSIGN 
        Logisdpiqueo.IDUsuario = STRING(ROWID(Detalle)).
END.
...
/*------------------------------------------------------------------------------
  ...
  Notes:  EN CASO DE ERROR RETORNAR : RETURN "ADM-ERROR"
            Siempre es CREAR, nunca MODIFICAR
------------------------------------------------------------------------------*/
...
/*   END.                                                                         */
  ...
  IF s-Generacion-Multiple = YES
      ...
      /* Ic - 27Nov2025, Valida que la cantidad pickeada NO sea mayor a 9999, por articulo solo se puede generar 9999 bultos - falta probar */
      DEFINE VAR x-Factor AS DEC.
      DEFINE VAR x-Cantidad AS DEC.

      x-Factor = DECIMAL(Detalle.factor:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
      x-Cantidad = INTEGER(Detalle.CanPick:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
      IF x-Factor <= 0 THEN x-Factor = 1.
      IF x-Cantidad > 9999 AND x-Factor >= 1 THEN DO:
          MESSAGE 'La cantidad NO puede ser mayor a 9999' VIEW-AS ALERT-BOX ERROR.
          RETURN 'ADM-ERROR'.
      END.
  END.
