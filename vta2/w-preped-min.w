...
DEFINE VARIABLE h_t-pedmin AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-preped-min AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/v-preped-min.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-preped-min ).
       RUN set-position IN h_v-preped-min ( 3.15 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-preped-min. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-preped-min ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_v-preped-min ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/t-pedmin.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmin ).
       RUN set-position IN h_t-pedmin ( 5.85 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedmin ( 15.88 , 131.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-pedmin. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedmin ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedmin ,
             h_v-preped-min , 'AFTER':U ).
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-pedmin <> ? THEN RUN dispatch IN h_t-pedmin ('open-query':U).
      END.
