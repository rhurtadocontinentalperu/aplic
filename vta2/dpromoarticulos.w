...
DEFINE VARIABLE FILL-IN-2 AS CHARACTER FORMAT "X(256)":U INITIAL "Importe" 
     VIEW-AS FILL-IN 
     SIZE 9 BY 1 NO-UNDO.

DEFINE VARIABLE FILL-IN-3 AS CHARACTER FORMAT "X(256)":U INITIAL "200.00" 
     VIEW-AS FILL-IN 
     SIZE 8 BY 1 NO-UNDO.
...
DEFINE VARIABLE FILL-IN-5 AS CHARACTER FORMAT "X(256)":U INITIAL "Cantidad" 
     VIEW-AS FILL-IN 
     SIZE 9 BY 1 NO-UNDO.
...
DEFINE FRAME D-Dialog
     ...
     FILL-IN-2 AT ROW 3.12 COL 17 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     FILL-IN-3 AT ROW 3.12 COL 28 COLON-ALIGNED NO-LABEL WIDGET-ID 8
     FILL-IN-4 AT ROW 4.27 COL 5 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     FILL-IN-5 AT ROW 4.27 COL 17 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     FILL-IN-6 AT ROW 4.27 COL 28 COLON-ALIGNED NO-LABEL WIDGET-ID 14
     "5" VIEW-AS TEXT
          SIZE 2 BY .62 AT ROW 6.77 COL 3 WIDGET-ID 90
          BGCOLOR 8 FGCOLOR 0 
     "MINIMO" VIEW-AS TEXT
          SIZE 8 BY .62 AT ROW 2.35 COL 30 WIDGET-ID 20
     "B" VIEW-AS TEXT
     ...
     "C" VIEW-AS TEXT
          ...
          BGCOLOR 8 FGCOLOR 0 
     "TIPO" VIEW-AS TEXT
          SIZE 8 BY .62 AT ROW 2.35 COL 20 WIDGET-ID 18
...
cValue = chWorkSheet:Cells(1,2):VALUE.
cValue = CAPS(cValue).
IF cValue = "" OR cValue = ? OR cValue <> "TIPO" THEN DO:
    MESSAGE 'Formato del archivo Excel errado' VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.
...
cValue = CAPS(cValue).
IF cValue = "" OR cValue = ? OR (cValue <> "MINIMO" AND cValue <> "MÍNIMO")
    THEN DO:
    MESSAGE 'Formato del archivo Excel errado' VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.
...
REPEAT:
    ...
    cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
    ASSIGN
        Promocion.Libre_c01 = CAPS(cValue).
...
/* Revisión final */
FOR EACH Promocion:
    IF LOOKUP(Promocion.Libre_c01, 'Importe,Cantidad') = 0 THEN DELETE Promocion.
END.
