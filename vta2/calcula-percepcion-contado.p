...
DEF INPUT PARAMETER pRowid-1 AS ROWID.
DEF INPUT PARAMETER pRowid-2 AS ROWID.
...
FIND Ccbcdocu WHERE ROWID(Ccbcdocu) = pRowid-1 EXCLUSIVE-LOCK NO-ERROR.
IF NOT AVAILABLE Ccbcdocu THEN RETURN "ADM-ERROR".
FIND Faccpedi WHERE ROWID(Faccpedi) = pRowid-2 NO-LOCK NO-ERROR.
...
DO:
    ...
    IF Ccbcdocu.coddoc = "BOL" THEN DO:
        IF Ccbcdocu.codmon = 1 AND f-ImpPercepcion <= ImpMinPercep THEN LEAVE PERCEPCION.
        IF Ccbcdocu.codmon = 2 AND f-ImpPercepcion * Ccbcdocu.tpocmb <= ImpMinPercep THEN LEAVE PERCEPCION.
    END.
