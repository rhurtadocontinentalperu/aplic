...
DEFINE VARIABLE h_b-clf-mantto-clf-calculos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-clf-mantto-clf-parametros AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv101 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-clf-mantto-clf-parametros AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-Excel AT ROW 1 COL 21 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 131.14 BY 26.15 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "MANTENIMIENTO DE CALCULO DE CLASIFICACIONES"
         HEIGHT             = 26.15
         WIDTH              = 131.14
         MAX-HEIGHT         = 26.15
         MAX-WIDTH          = 191.29
...
ON END-ERROR OF W-Win /* MANTENIMIENTO DE CALCULO DE CLASIFICACIONES */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* MANTENIMIENTO DE CALCULO DE CLASIFICACIONES */
DO:
...
DO:
  ...
  RUN Capture-Rowid IN h_b-clf-mantto-clf-parametros
    ( OUTPUT pRowid /* ROWID */).
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv101 ).
       RUN set-position IN h_p-updv101 ( 1.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv101 ( 1.54 , 18.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/clf/b-clf-mantto-clf-parametros.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-clf-mantto-clf-parametros ).
       RUN set-position IN h_b-clf-mantto-clf-parametros ( 2.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-clf-mantto-clf-parametros ( 10.50 , 53.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/clf/v-clf-mantto-clf-parametros.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-clf-mantto-clf-parametros ).
       RUN set-position IN h_v-clf-mantto-clf-parametros ( 2.62 , 56.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.88 , 63.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/clf/b-clf-mantto-clf-calculos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-clf-mantto-clf-calculos ).
       RUN set-position IN h_b-clf-mantto-clf-calculos ( 13.38 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-clf-mantto-clf-calculos ( 13.19 , 128.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-clf-mantto-clf-parametros. */
       RUN add-link IN adm-broker-hdl ( h_p-updv101 , 'TableIO':U , h_b-clf-mantto-clf-parametros ).

       /* Links to SmartViewer h_v-clf-mantto-clf-parametros. */
       RUN add-link IN adm-broker-hdl ( h_b-clf-mantto-clf-parametros , 'Record':U , h_v-clf-mantto-clf-parametros ).

       /* Links to SmartBrowser h_b-clf-mantto-clf-calculos. */
       RUN add-link IN adm-broker-hdl ( h_b-clf-mantto-clf-parametros , 'Record':U , h_b-clf-mantto-clf-calculos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv101 ,
             BUTTON-Excel:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-clf-mantto-clf-parametros ,
             BUTTON-Excel:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-clf-mantto-clf-parametros ,
             h_b-clf-mantto-clf-parametros , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-clf-mantto-clf-calculos ,
             h_v-clf-mantto-clf-parametros , 'AFTER':U ).
    END. /* Page 0 */
...
FIND Clf_Calc_Parameters WHERE ROWID(Clf_Calc_Parameters) = pRowid NO-LOCK NO-ERROR.
IF NOT AVAILABLE Clf_Calc_Parameters THEN RETURN.
...
c-xls-file = "PruebaClasificacion" + " " +
    Clf_Calc_Parameters.Descripcion + " " +
    Clf_Calc_Parameters.Codigo + " " +
    Clf_Calc_Parameters.Tipo_Calculo + 
    ".xlsx"
    .
...
FOR EACH Clf_Calc_Calculos WHERE Clf_Calc_Calculos.Id_Agrupador = Clf_Calc_Parameters.Id_Agrupador
    AND Clf_Calc_Calculos.Id_Periodo = Clf_Calc_Parameters.Id_Periodo NO-LOCK
    BY Clf_Calc_Calculos.CodMat:
    CREATE tclf_calculo.
    BUFFER-COPY Clf_Calc_Calculos TO tclf_calculo NO-ERROR.
END.
