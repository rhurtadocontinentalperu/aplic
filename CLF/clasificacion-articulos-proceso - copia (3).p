...
DEFINE VAR fSumaPuntajes AS DEC.
...
/*         END.                                                                            */
        ...
        END.
        OTHERWISE DO:
            tclf_calculo.dcantidad = (tclf_calculo.dcantidad_act - tclf_calculo.dcantidad_ant) / ABS(tclf_calculo.dcantidad_ant) * 100.
        END.
    END CASE.
    CASE TRUE:
        WHEN tclf_calculo.dimporte_act > 0 AND tclf_calculo.dimporte_ant < 0 THEN DO:
            tclf_calculo.dimporte = 100.
        END.
/*         WHEN tclf_calculo.dimporte_act > 0 AND tclf_calculo.dimporte_ant = 0 THEN DO: */
/*             tclf_calculo.dimporte = 100.                                              */
/*         END.                                                                          */
...
/*         END.                                                                          */
        ...
        END.
        OTHERWISE DO:
            tclf_calculo.dimporte = (tclf_calculo.dimporte_act - tclf_calculo.dimporte_ant) / ABS(tclf_calculo.dimporte_ant) * 100.
        END.
    END CASE.
END.
...
FOR EACH tclf_calculo EXCLUSIVE-LOCK:
    ...
    CASE cTipo_Calculo:
        ...
        END.
        WHEN "PPM" THEN DO:
            /* POR PROPORCION DEL MAYOR */
            IF fValorMaximoVentas > 0 THEN DO:
                ASSIGN
                    tclf_calculo.dpuntajeventas = (tclf_calculo.dventas / fValorMaximoVentas) * dFactorVentas.
            END.
            IF fValorMaximoUtilidad > 0 THEN DO:
                ASSIGN
                    tclf_calculo.dpuntajeutilidad = (tclf_calculo.dutilidad / fValorMaximoUtilidad) * dFactorUtilidad.
            END.
            IF fValorMaximoDeltaQty > 0 THEN DO:
                ASSIGN
                    tclf_calculo.dpuntajecantidad = (tclf_calculo.dcantidad / fValorMaximoDeltaQty) * dFactorCantidad.
            END.
            IF fValorMaximoDeltaImp > 0 THEN DO:
                ASSIGN
                    tclf_calculo.dpuntajeimporte = (tclf_calculo.dimporte / fValorMaximoDeltaImp) * dFactorImporte.
            END.
            ASSIGN
                tclf_calculo.dsumapuntaje = tclf_calculo.dpuntajeventas + 
                                            tclf_calculo.dpuntajeutilidad + 
...
FOR EACH tclf_calculo EXCLUSIVE-LOCK:
    ...
    iContador = iContador + 1.
    ASSIGN 
        tclf_calculo.dpesoventas = IF (fSumaPuntajes > 0)   THEN ROUND(tclf_calculo.dpuntajeventas / fSumaPuntajes,6)   ELSE 0
        tclf_calculo.dpesoutilidad = IF (fSumaPuntajes > 0) THEN ROUND(tclf_calculo.dpuntajeUtilidad / fSumaPuntajes,6) ELSE 0
        tclf_calculo.dpesocantidad = IF (fSumaPuntajes > 0) THEN ROUND(tclf_calculo.dpuntajeCantidad / fSumaPuntajes,6) ELSE 0
        tclf_calculo.dpesoimporte = IF (fSumaPuntajes > 0)  THEN ROUND(tclf_calculo.dpuntajeImporte / fSumaPuntajes,6)  ELSE 0
...
FOR EACH tclf_calculo EXCLUSIVE-LOCK WHERE tclf_calculo.dpesoacumulado >= 0 BY tclf_calculo.dpesoacumulado DESC:
    IF iContador MODULO 1000 = 0
        THEN DISPLAY "Calcula pesos - puntaje final " + tclf_calculo.codmat @ Fi-Mensaje WITH FRAME F-Proceso.
    iContador = iContador + 1.
    ASSIGN 
        tclf_calculo.dpesopuntajefinal = tclf_calculo.dpesoacumulado + fValorAnterior.
    /* Si supera 1 (100%) entonces es 1 (100%) */
    IF tclf_calculo.dpesopuntajefinal > 1 THEN DO:
...
FOR EACH ventas_detalle NO-LOCK
    ...
    AND ventas_detalle.datekey <= pdHasta,
    FIRST DimProducto NO-LOCK 
    WHERE DimProducto.codmat = Ventas_Detalle.codmat:
