...
&SCOPED-DEFINE l-lookup h_l-clf_temp_calculos
...
DEFINE VARIABLE h_l-clf_temp_calculos AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/clf/l-clf_temp_calculos.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-clf_temp_calculos ).
       RUN set-position IN h_l-clf_temp_calculos ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-clf_temp_calculos ( 10.92 , 54.72 ) NO-ERROR.
       ...
       /* Links to SmartLookup h_l-clf_temp_calculos. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-clf_temp_calculos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-clf_temp_calculos ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-clf_temp_calculos , 'AFTER':U ).
    END. /* Page 0 */
