...
DEF VAR X_MES AS INTEGER.
DEF VAR X_PERIODO AS INTEGER.
DEF VAR X_FINICIAL AS DATE.
DEF VAR X_FFINAL AS DATE.
...
THEN DO:
    ...
    /* RHC 15.11.2010 */
    ASSIGN
        X_mes = MONTH(x-Fecha-1)
        X_periodo = YEAR(x-Fecha-1).
    RUN src/bin/_dateif (X_mes, X_periodo, OUTPUT x_finicial, OUTPUT X_ffinal).
    IF x-Fecha-1 = x_finicial AND x-Fecha-2 = X_ffinal THEN DIAS = 30.
    RETURN.
END.
DIAS = 30 - DAY(x-Fecha-1) + 1.
...
REPEAT:
    ...
    THEN DO:
        /* RHC 15.11.2010 */
        ASSIGN
            X_mes = MONTH(x-Fecha-2)
            X_periodo = YEAR(x-Fecha-2).
        RUN src/bin/_dateif (X_mes, X_periodo, OUTPUT x_finicial, OUTPUT X_ffinal).
        IF x-Fecha-2 = X_ffinal 
        THEN DIAS = DIAS + 30.
        ELSE DIAS = DIAS + DAY(x-Fecha-2).
        LEAVE VUELTA.
    END.
    ELSE DIAS = DIAS + 30.
END.
