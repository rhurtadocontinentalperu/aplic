...
&Scoped-define QUERY-STRING-br_pl-flg-m FOR EACH x-tempo
&Scoped-define OPEN-QUERY-br_pl-flg-m OPEN QUERY {&SELF-NAME} FOR EACH x-tempo .
...
/* Query rebuild information for BROWSE br_pl-flg-m
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH x-tempo .
     _END_FREEFORM
...
DO:
    ...
    MESSAGE "Documentos fueron generados en su disco local"
...
x_fecemi = STRING(DAY(TODAY)) + " de " + ENTRY(MONTH(TODAY),x_meses) + " del " + STRING(YEAR(TODAY)).

Word:Documents:Add("CERSER001").
...
x_archivo = "certificado" + pl-pers.codper.
