...
/* External Tables                                                      */
&Scoped-define EXTERNAL-TABLES INTEGRAL.PL-DHABIENTE INTEGRAL.PL-FLG-MES
&Scoped-define FIRST-EXTERNAL-TABLE INTEGRAL.PL-DHABIENTE


/* Need to scope the external tables to this procedure                  */
DEFINE QUERY external_tables FOR INTEGRAL.PL-DHABIENTE, INTEGRAL.PL-FLG-MES.
...
"Femenino", "2":U
          SIZE 10 BY 1.35
     INTEGRAL.PL-DHABIENTE.FchAlta AT ROW 3.12 COL 77 COLON-ALIGNED FORMAT "99/99/9999"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
...
"Gestante", "4":U
     ...
     INTEGRAL.PL-DHABIENTE.FchBaja AT ROW 6.04 COL 58 COLON-ALIGNED FORMAT "99/99/9999"
          VIEW-AS FILL-IN 
          SIZE 11 BY .81
...
DEFINE FRAME F-Main
     ...
     INTEGRAL.PL-DHABIENTE.Ubigeo AT ROW 11.19 COL 42 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 9 BY .81
...
/* Settings for THIS-PROCEDURE
   Type: SmartViewer
   External Tables: INTEGRAL.PL-DHABIENTE,INTEGRAL.PL-FLG-MES
...
/* SETTINGS FOR COMBO-BOX COMBO-BOX-DocAcredita IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN INTEGRAL.PL-DHABIENTE.FchAlta IN FRAME F-Main
   EXP-FORMAT                                                           */
/* SETTINGS FOR FILL-IN INTEGRAL.PL-DHABIENTE.FchBaja IN FRAME F-Main
   EXP-FORMAT                                                           */
...
&Scoped-define SELF-NAME INTEGRAL.PL-DHABIENTE.SitDerHab
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL INTEGRAL.PL-DHABIENTE.SitDerHab V-table-Win
ON VALUE-CHANGED OF INTEGRAL.PL-DHABIENTE.SitDerHab IN FRAME F-Main /* Sit */
DO:
  CASE SELF:SCREEN-VALUE:
      WHEN "10" THEN 
          ASSIGN
          PL-DHABIENTE.FchAlta:SENSITIVE = YES
          PL-DHABIENTE.FchBaja:SENSITIVE = NO
          PL-DHABIENTE.TpoBaja:SENSITIVE = NO
          FILL-IN-TpoBaja:SCREEN-VALUE = '' 
          PL-DHABIENTE.TpoBaja:SCREEN-VALUE = ''.
      WHEN "11" THEN 
          ASSIGN
          PL-DHABIENTE.FchAlta:SENSITIVE = NO
          PL-DHABIENTE.FchBaja:SENSITIVE = YES
          PL-DHABIENTE.TpoBaja:SENSITIVE = YES.
  END CASE.
END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
        PL-DHABIENTE.SitDerHab:SCREEN-VALUE = "10".
  END.
  ...
  /* Code placed here will execute AFTER standard behavior.    */
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        IF RETURN-VALUE = 'YES' THEN DO:
            ASSIGN
                PL-DHABIENTE.CodCia = PL-FLG-MES.CodCia
                PL-DHABIENTE.Periodo = PL-FLG-MES.Periodo
                PL-DHABIENTE.CodPln = PL-FLG-MES.CodPln
                PL-DHABIENTE.NroMes = PL-FLG-MES.NroMes
                PL-DHABIENTE.CodPer = PL-FLG-MES.CodPer.
        END.
        ELSE IF PL-DHABIENTE.SitDerHab = '11' THEN DO:
            MESSAGE 'Se va a dar de baja al derechohabiente' SKIP
                'Continuamos?' VIEW-AS ALERT-BOX WARNING
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
      IF RETURN-VALUE = "YES" THEN PL-DHABIENTE.SitDerHab:SENSITIVE = NO.
      ELSE PL-DHABIENTE.SitDerHab:SENSITIVE = YES.
      IF RETURN-VALUE = "NO" AND PL-DHABIENTE.SitDerHab = "11" THEN PL-DHABIENTE.SitDerHab:SENSITIVE = NO.
      APPLY "VALUE-CHANGED":U TO PL-DHABIENTE.SitDerHab.
...
DO WITH FRAME {&FRAME-NAME} :
    ...
    IF PL-DHABIENTE.SitDerHab:SCREEN-VALUE = '11' AND
        ...
        RETURN "ADM-ERROR".
    END.
    IF PL-DHABIENTE.SitDerHab:SCREEN-VALUE = '10' AND INPUT PL-DHABIENTE.FchAlta = ? THEN DO:
        MESSAGE
            "Fecha de Alta no es Válido"
            VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO PL-DHABIENTE.FchAlta.
        RETURN "ADM-ERROR".
    END.
    IF PL-DHABIENTE.SitDerHab:SCREEN-VALUE = '11' AND INPUT PL-DHABIENTE.FchBaja = ? THEN DO:
        MESSAGE
            "Fecha de Baja no es Válido"
            VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO PL-DHABIENTE.FchBaja.
        RETURN "ADM-ERROR".
    END.
