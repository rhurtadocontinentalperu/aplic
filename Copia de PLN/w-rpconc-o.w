...
DEFINE VARIABLE h_b-p-bols AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Reporte de Conceptos Otorgados (Obreros)"
         HEIGHT             = 12.88
         WIDTH              = 70.72
         MAX-HEIGHT         = 12.88
         MAX-WIDTH          = 70.72
...
ON END-ERROR OF W-Win /* Reporte de Conceptos Otorgados (Obreros) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Reporte de Conceptos Otorgados (Obreros) */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pln/b-rpconc-o.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-p-bols ).
       RUN set-position IN h_b-p-bols ( 4.08 , 1.57 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pln/q-plan-s.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-plan-s ).
       RUN set-position IN h_q-plan-s ( 1.00 , 42.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.65 , 8.29 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pln/q-calc1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-calc1 ).
       RUN set-position IN h_q-calc1 ( 2.54 , 42.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.65 , 8.29 ) */
       ...
       /* Links to SmartBrowser h_b-p-bols. */
       RUN add-link IN adm-broker-hdl ( h_q-calc1 , 'Record':U , h_b-p-bols ).
       ...
       RUN add-link IN adm-broker-hdl ( h_q-plan-s , 'Record':U , h_q-calc1 ).
