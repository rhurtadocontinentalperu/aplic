...
DEF TEMP-TABLE DETALLE
    FIELD CodCia LIKE PL-FLG-MES.CodCia
    FIELD Ccosto LIKE PL-FLG-MES.Ccosto
    FIELD CodPer LIKE PL-PERS.CodPer
    FIELD NomPer AS CHAR
    FIELD FchMar LIKE as-marc.fchmar
    FIELD HorMar LIKE as-marc.hormar
    FIELD Orden  AS INT
    INDEX LLAVE01 CodCia CodPer FchMar Orden.

DEF TEMP-TABLE DETALLE1 LIKE DETALLE.
...
&Scoped-Define ENABLED-OBJECTS RECT-58 RECT-59 RECT-64 DesdeC HastaC Btn_OK ~
Btn_Cancel Btn_Help BUTTON-3 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Marcaciones W-Win 
PROCEDURE Carga-Marcaciones :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  DO fFecha = DesdeC TO HastaC:
    FOR EACH as-marc NO-LOCK where as-marc.codcia =  S-CODCIA 
            AND as-marc.fchmar = fFecha,
            FIRST PL-PERS NO-LOCK WHERE PL-PERS.CodCia = s-codcia
                AND PL-PERS.CodPer = As-Marc.CodPer
            BREAK BY as-marc.codcia BY as-marc.codper BY as-marc.fchmar BY as-marc.hormar:
        IF FIRST-OF(As-Marc.CodPer) THEN DO:
            i = 1.
        END.
        FIND LAST pl-flg-mes WHERE pl-flg-mes.codcia = s-codcia
            AND pl-flg-mes.codper = as-marc.codper
            NO-LOCK NO-ERROR.
        DISPLAY As-Marc.CodPer As-Marc.FchMar WITH FRAME f-Mensaje CENTERED OVERLAY VIEW-AS DIALOG-BOX NO-LABELS.
        CREATE DETALLE.
        ASSIGN
            DETALLE.CodCia = As-Marc.CodCia
            DETALLE.CodPer = As-Marc.CodPer
            DETALLE.FchMar = As-Marc.FchMar
            DETALLE.HorMar = As-Marc.HorMar
            DETALLE.Orden  = i
            DETALLE.NomPer = TRIM(pl-pers.patper) + " " + 
                            TRIM(pl-pers.matper) + ", " + 
                            TRIM(pl-pers.nomper).
        IF AVAILABLE PL-FLG-MES THEN DETALLE.Ccosto = PL-FLG-MES.Ccosto.
        i = i + 1.
        FIND DETALLE1 WHERE DETALLE1.CodCia = DETALLE.CodCia
            AND DETALLE1.CodPer = DETALLE.CodPer
            AND DETALLE1.Orden  = DETALLE.Orden
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE DETALLE1 THEN DO:
            CREATE DETALLE1.
            ASSIGN
                DETALLE1.CodCia = DETALLE.CodCia
                DETALLE1.CodPer = DETALLE.CodPer
                DETALLE1.Orden  = DETALLE.Orden
                DETALLE1.NomPer = DETALLE.NomPer
                DETALLE1.Ccosto = DETALLE.CCosto.
        END.
    END.        
  END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-58 RECT-59 RECT-64 DesdeC HastaC Btn_OK Btn_Cancel Btn_Help 
         BUTTON-3 
      WITH FRAME F-Main IN WINDOW W-Win.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
IF HastaC - DesdeC > 31 THEN DO:
    MESSAGE "El rango tiene mas de 31 dias" VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.

RUN Carga-Marcaciones.
FIND FIRST DETALLE1 NO-LOCK NO-ERROR.
IF NOT AVAILABLE DETALLE1 THEN DO:
    MESSAGE 'No hay marcaciones a imprimir' VIEW-AS ALERT-BOX WARNING.
    RETURN.
END.
...
DEFINE VARIABLE jRow                    AS INTEGER INITIAL 1.
DEFINE VARIABLE cColumn                 AS CHARACTER.
DEFINE VARIABLE cRange                  AS CHARACTER.
DEFINE VARIABLE jRange                  AS CHARACTER.
DEFINE VARIABLE x-ccosto                AS CHARACTER.
DEFINE VARIABLE xlCenter  AS CHAR.
DEFINE VARIABLE xlBottom  AS CHAR.
...
cColumn = STRING(iColumn).
DO fFecha = DesdeC TO HastaC:
    cRange = jRange + TRIM(CHR(iRow)) + cColumn.
    chWorkSheet:Range(cRange):Value = fFecha.
    iRow = iRow + 1.
    IF iRow > 90 THEN DO:
        iRow = 65.      /* 'A' */
        jRange = TRIM(CHR(jRow)).
        jRow = jRow + 1.
    END.
END.

iColumn = 2.
FOR EACH DETALLE1 BY DETALLE1.CodCia BY DETALLE1.CodPer BY DETALLE1.Orden:
    cColumn = STRING(iColumn).
    cRange = "A" + cColumn.
    chWorkSheet:Range(cRange):Value = "'" + DETALLE1.Ccosto.
    FIND cb-auxi WHERE cb-auxi.CodCia = cb-codcia
        AND cb-auxi.CLFAUX = "CCO" 
        AND cb-auxi.CodAUX = DETALLE1.Ccosto
        NO-LOCK NO-ERROR.
    IF AVAILABLE Cb-AUXI THEN DO:
        cRange = "B" + cColumn.
        chWorkSheet:Range(cRange):Value = cb-auxi.nomaux.
    END.        
    cRange = "C" + cColumn.
    chWorkSheet:Range(cRange):Value = "'" + STRING(DETALLE1.codper,"999999").
    cRange = "D" + cColumn.
    chWorkSheet:Range(cRange):Value = DETALLE1.NomPer.
    iRow = 69.      /* 'E' */
    jRow = 65.      /* 'A' */
    jRange = ''.
    DO fFecha = DesdeC TO HastaC:
        FIND FIRST DETALLE WHERE DETALLE.CodCia = DETALLE1.CodCia
            AND DETALLE.CodPer = DETALLE1.CodPer
            AND DETALLE.FchMar = fFecha
            AND DETALLE.Orden  = DETALLE1.Orden
            NO-LOCK NO-ERROR.
        IF AVAILABLE DETALLE THEN DO:
            cRange = jRange + TRIM(CHR(iRow)) + cColumn.
            chWorkSheet:Range(cRange):Value = DETALLE.HorMar.
        END.                
        iRow = iRow + 1.
        IF iRow > 90 THEN DO:
            iRow = 65.      /* 'A' */
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEF VAR x-PatPer LIKE Pl-pers.patper NO-UNDO.
DEF VAR x-MatPer LIKE Pl-pers.matper NO-UNDO.
DEF VAR x-NomPer LIKE Pl-pers.nomper NO-UNDO.
 ...
 *     end.*/
    ...
    DISPLAY As-marc.codper @ Fi-Mensaje LABEL "Codigo de Personal "
              FORMAT "X(11)" WITH FRAME F-Proceso.
    ASSIGN
        x-patper = ''
        x-matper = ''
        x-nomper = ''.
    FIND Pl-pers WHERE Pl-pers.codper = As-marc.codper NO-LOCK NO-ERROR.
    IF AVAILABLE Pl-pers
    THEN ASSIGN
            x-patper = Pl-pers.patper
            x-matper = Pl-pers.matper
            x-nomper = Pl-pers.nomper.
