...
DEFINE VARIABLE h_b-rpremu2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-calc2 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-plan ).
       RUN set-position IN h_v-plan ( 2.08 , 1.43 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pln/b-rpremu2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-rpremu2 ).
       RUN set-position IN h_b-rpremu2 ( 4.27 , 1.57 ) NO-ERROR.
       RUN set-size IN h_b-rpremu2 ( 9.50 , 69.72 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-plan-m ).
       RUN set-position IN h_q-plan-m ( 1.00 , 30.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.65 , 8.29 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pln/q-calc2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-calc2 ).
       RUN set-position IN h_q-calc2 ( 1.54 , 42.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.50 , 7.72 ) */

       /* Links to SmartViewer h_v-plan. */
       RUN add-link IN adm-broker-hdl ( h_q-plan-m , 'Record':U , h_v-plan ).

       /* Links to SmartBrowser h_b-rpremu2. */
       RUN add-link IN adm-broker-hdl ( h_q-calc2 , 'Record':U , h_b-rpremu2 ).

       /* Links to SmartQuery h_q-plan-m. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-plan-m ).

       /* Links to SmartQuery h_q-calc2. */
       RUN add-link IN adm-broker-hdl ( h_q-plan-m , 'Record':U , h_q-calc2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-plan , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-rpremu2 ,
             h_p-navico , 'AFTER':U ).
    END. /* Page 0 */
