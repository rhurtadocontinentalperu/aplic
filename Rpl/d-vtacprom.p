TRIGGER PROCEDURE FOR REPLICATION-DELETE OF vtacprom.

    {rpl/reptrig.i
    &Table  = vtacprom
    &Key    = "string(vtacprom.codcia,'999') + ~
        string(vtacprom.coddiv, 'x(5)') + ~
        string(vtacprom.coddoc, 'x(3)') + ~
        string(vtacprom.nrodoc)"
    &Prg    = r-vtacprom
    &Event  = DELETE
