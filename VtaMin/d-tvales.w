...
DEF INPUT PARAMETER para_DNICLie AS CHAR.
...
DEFINE NEW SHARED VARIABLE s-DNIClie  AS CHAR.
DEFINE NEW SHARED VARIABLE s-FlgSit   AS CHAR.
s-DNICLie = para_DNIClie.
s-FlgSit  = para_FlgSit.
...
DEFINE VARIABLE h_b-tvales AS HANDLE NO-UNDO.
...
DEFINE FRAME D-Dialog
     Btn_Done AT ROW 1 COL 92 WIDGET-ID 2
     SPACE(4.99) SKIP(8.52)
...
DO:
  ...
  FOR EACH T-VALES NO-LOCK:
    IF T-VALES.CodMon = 1
    THEN para_ImpNac = para_ImpNac + T-VALES.ImpVal.
    ELSE para_ImpUSA = para_ImpUSA + T-VALES.ImpVal.
  END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamin/b-tvales.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tvales ).
       RUN set-position IN h_b-tvales ( 2.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tvales ( 6.69 , 113.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-tvales. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-tvales ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             Btn_Done:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tvales ,
