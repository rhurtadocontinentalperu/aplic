...
&SCOPED-DEFINE l-lookup h_l-pedmos-04
...
DEFINE VARIABLE h_l-pedmos-04 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 4.58 COL 74
     Btn_Cancel AT ROW 7.15 COL 74
     RECT-27 AT ROW 3.69 COL 72.57
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/l-pedmos-04.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-pedmos-04 ).
       RUN set-position IN h_l-pedmos-04 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-pedmos-04 ( 10.96 , 70.72 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.27 , 71.72 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-pedmos-04. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-pedmos-04 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-pedmos-04 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-pedmos-04 , 'AFTER':U ).
    END. /* Page 0 */
