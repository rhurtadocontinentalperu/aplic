...
&SCOPED-DEFINE CONDICION ( CcbcDocu.CodCia = S-CODCIA AND ~
                           ...
                           CAN-DO("C,CO", CcbcDocu.TpoFac) AND ~
...
&Scoped-define FIELDS-IN-QUERY-br_table CcbCDocu.CodCaja CcbCDocu.CodDoc ~
CcbCDocu.NroDoc CcbCDocu.NomCli CcbCDocu.CodVen CcbCDocu.FchDoc ~
X-MON @ X-MON CcbCDocu.ImpTot X-EST @ X-EST 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbCDocu.CodCaja COLUMN-LABEL "Caja" FORMAT "X(10)":U
      CcbCDocu.CodDoc COLUMN-LABEL "Tipo!Comp" FORMAT "x(3)":U
      CcbCDocu.NroDoc FORMAT "XXX-XXXXXXXX":U WIDTH 10.86 COLUMN-BGCOLOR 11
            LABEL-BGCOLOR 8
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 96.86 BY 9.12
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > integral.CcbCDocu.CodCaja
"CcbCDocu.CodCaja" "Caja" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.NroDoc" ? "XXX-XXXXXXXX" "character" 11 ? ? 8 ? ? no ? no no "10.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  IF AVAILABLE CcbCDocu THEN  DO:
      RUN vtamay/r-odesp-001a (ROWID(Ccbcdocu), Ccbcdocu.codalm, 'DUPLICADO',YES).
/*       FIND Faccpedm WHERE Faccpedm.codcia = Ccbcdocu.codcia                        */
/*           AND Faccpedm.coddoc = Ccbcdocu.codped                                    */
/*           AND Faccpedm.nroped = Ccbcdocu.nroped                                    */
/*           NO-LOCK NO-ERROR.                                                        */
/*       IF AVAILABLE Faccpedm                                                        */
/*       THEN RUN vtamay/r-odesp-01b (ROWID(Faccpedm), Ccbcdocu.codalm, 'DUPLICADO'). */
/*       ELSE RUN vtamay/r-odesp-01a (ROWID(Ccbcdocu), Ccbcdocu.codalm, 'DUPLICADO'). */
  END.
...
------------------------------------------------------------------------------*/
  IF AVAILABLE CcbCDocu THEN  DO:
      IF s-Sunat-Activo = YES AND Ccbcdocu.CodDoc <> "TCK" THEN DO:
          ...
          ELSE DO:
              /* Matricial sin QR */
              RUN sunat\r-tickets-sunat ( ROWID(Ccbcdocu), "C", YES ).
          END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Imprime-Promocion L-table-Win 
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE CcbCDocu THEN  DO:
      ...
      IF AVAILABLE Faccpedi THEN
          RUN vta2/p-imprime-prom-cruzada (ROWID(Faccpedi),
                                           x-formato-tck,
                                           x-imprime-directo,
                                           x-nombre-impresora).
  END.
