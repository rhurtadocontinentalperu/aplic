...
/* NOTA: Puede contener mas de un almacen */
...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 BUTTON-PEN COMBO-NroSer 
...
DEFINE VARIABLE h_q-vtamos-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-vtaaut-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-vtaaut-4 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 1 COL 99
     BUTTON-PEN AT ROW 1 COL 107 WIDGET-ID 10
     COMBO-NroSer AT ROW 1.27 COL 2 NO-LABEL WIDGET-ID 4
     BUTTON-ADD AT ROW 10.96 COL 112 WIDGET-ID 6
     BUTTON-DEL AT ROW 12.85 COL 112 WIDGET-ID 8
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 126.57 BY 20.23
         FONT 4.
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* SETTINGS FOR BUTTON BUTTON-ADD IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR BUTTON BUTTON-DEL IN FRAME F-Main
   NO-ENABLE                                                            */
...
DO:
  RUN Cancelar-Pedido IN h_v-vtaaut-4.
END.
...
ON CHOOSE OF BUTTON-ADD IN FRAME F-Main /* ADICIONAR */
DO:
    RUN Procesa-Handle IN lh_handle ('Disable-Head').
    DEF VAR pReturnValue AS CHAR.
    RUN vtamin/gvtaaut4 (OUTPUT pReturnValue).
    IF pReturnValue = "OK" THEN RUN dispatch IN h_t-vtaaut-4 ('open-query').
...
&Scoped-define SELF-NAME BUTTON-DEL
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-DEL W-Win
ON CHOOSE OF BUTTON-DEL IN FRAME F-Main /* ELIMINAR */
DO:
    RUN Borra-Registro IN h_t-vtaaut-4.
END.
...
DO:
  RUN local-qbusca-1 IN h_q-vtamos-4.
END.
...
DO:
    ...
    RUN dispatch IN h_q-vtamos-4 ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_p-updv05 ( 1.54 , 62.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/v-vtaaut-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-vtaaut-4 ).
       RUN set-position IN h_v-vtaaut-4 ( 2.88 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.46 , 109.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/q-vtamos-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-vtamos-4 ).
       RUN set-position IN h_q-vtamos-4 ( 1.19 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-vtaaut-4. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-vtaaut-4 ).
       RUN add-link IN adm-broker-hdl ( h_q-vtamos-4 , 'Record':U , h_v-vtaaut-4 ).

       /* Links to SmartQuery h_q-vtamos-4. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-vtamos-4 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-vtaaut-4 ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-vtamos-4 ,
             BUTTON-DEL:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/b-pedmos-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedmos-4 ).
       RUN set-position IN h_b-pedmos-4 ( 9.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedmos-4 ( 11.15 , 109.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pedmos-4. */
       RUN add-link IN adm-broker-hdl ( h_q-vtamos-4 , 'Record':U , h_b-pedmos-4 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedmos-4 ,
             h_v-vtaaut-4 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/t-vtaaut-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-vtaaut-4 ).
       RUN set-position IN h_t-vtaaut-4 ( 9.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-vtaaut-4 ( 11.31 , 109.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-vtaaut-4 ,
             h_v-vtaaut-4 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-1 BUTTON-PEN COMBO-NroSer 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-vtaaut-4 <> ? THEN RUN dispatch IN h_t-vtaaut-4 ('open-query':U).
      END.
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
         RUN select-page(1).
         BUTTON-1:SENSITIVE = YES.
         BUTTON-ADD:SENSITIVE = NO.
         BUTTON-DEL:SENSITIVE = NO.
         BUTTON-PEN:SENSITIVE = YES.
         COMBO-NroSer:SENSITIVE = YES.
         RUN adm-enable IN h_q-vtamos-4.
      END.
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
         RUN select-page(2).
         BUTTON-1:SENSITIVE = NO.
         BUTTON-ADD:SENSITIVE = YES.
         BUTTON-DEL:SENSITIVE = YES.
         BUTTON-PEN:SENSITIVE = NO.
         COMBO-NroSer:SENSITIVE = NO.
         RUN adm-disable IN h_q-vtamos-4.
      END.
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-vtaaut-4 <> ? THEN RUN Recalcular-Precios IN h_t-vtaaut-4.
      END.
