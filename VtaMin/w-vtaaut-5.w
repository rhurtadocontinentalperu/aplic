...
DEFINE VARIABLE h_b-pedmos-5 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv05 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-vtamos-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-vtaaut-5 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-vtaaut-5 AS HANDLE NO-UNDO.
...
DO:
  RUN Cancelar-Pedido IN h_v-vtaaut-5.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/v-vtaaut-5.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-vtaaut-5 ).
       RUN set-position IN h_v-vtaaut-5 ( 2.88 , 2.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-vtaaut-5. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-vtaaut-5 ).
       RUN add-link IN adm-broker-hdl ( h_q-vtamos-4 , 'Record':U , h_v-vtaaut-5 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-vtaaut-5 ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-vtamos-4 ,
             h_v-vtaaut-5 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/b-pedmos-5.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedmos-5 ).
       RUN set-position IN h_b-pedmos-5 ( 9.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedmos-5 ( 12.65 , 109.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pedmos-5. */
       RUN add-link IN adm-broker-hdl ( h_q-vtamos-4 , 'Record':U , h_b-pedmos-5 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedmos-5 ,
             h_v-vtaaut-5 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/t-vtaaut-5.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-vtaaut-5 ).
       RUN set-position IN h_t-vtaaut-5 ( 9.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-vtaaut-5 ( 13.46 , 108.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 11.23 , 111.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 4.31 , 12.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-vtaaut-5. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-vtaaut-5 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-vtaaut-5 ,
             h_v-vtaaut-5 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-vtaaut-5 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_b-pedmos-5 <> ? THEN RUN dispatch IN h_b-pedmos-5 ('open-query':U).
          IF h_t-vtaaut-5 <> ? THEN RUN dispatch IN h_t-vtaaut-5 ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-vtaaut-5 <> ? THEN RUN Recalcular-Precios IN h_t-vtaaut-5.
      END.
