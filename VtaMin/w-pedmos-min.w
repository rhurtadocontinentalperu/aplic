...
&Scoped-Define ENABLED-OBJECTS RECT-24 COMBO-NroSer BUTTON-PEN BUTTON-Canc 
...
DEFINE VARIABLE h_b-pedmos-min AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv05 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-pedido-min AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-pedmos-min AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-pedmos-min AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-PEN AT ROW 1.27 COL 98 WIDGET-ID 14
     BUTTON-Canc AT ROW 3.15 COL 135 WIDGET-ID 12
     RECT-24 AT ROW 2.62 COL 2 WIDGET-ID 16
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 145.14 BY 26.27
         FONT 4.
...
DO:
  RUN Cancelar-Pedido IN h_v-pedmos-min.
END.
...
DO:
  RUN local-qbusca-1 IN h_q-pedido-min.
END.
...
DO:
    ...
    RUN dispatch IN h_q-pedido-min ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/v-pedmos-min.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedmos-min ).
       RUN set-position IN h_v-pedmos-min ( 2.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.15 , 130.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/q-pedido-min.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-pedido-min ).
       RUN set-position IN h_q-pedido-min ( 1.00 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 9.00 ) */

       /* Links to SmartViewer h_v-pedmos-min. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-pedmos-min ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-min , 'Record':U , h_v-pedmos-min ).

       /* Links to SmartQuery h_q-pedido-min. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido-min ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedmos-min ,
             BUTTON-PEN:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido-min ,
             BUTTON-Canc:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/b-pedmos-min.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedmos-min ).
       RUN set-position IN h_b-pedmos-min ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedmos-min ( 15.88 , 130.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pedmos-min. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido-min , 'Record':U , h_b-pedmos-min ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedmos-min ,
             BUTTON-Canc:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamin/t-pedmos-min.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmos-min ).
       RUN set-position IN h_t-pedmos-min ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedmos-min ( 15.88 , 131.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 12.58 , 134.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 8.08 , 11.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-pedmos-min. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedmos-min ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedmos-min ,
             BUTTON-Canc:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-pedmos-min , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-24 COMBO-NroSer BUTTON-PEN BUTTON-Canc 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_b-pedmos-min <> ? THEN RUN dispatch IN h_b-pedmos-min ('open-query':U).
          IF h_t-pedmos-min <> ? THEN RUN dispatch IN h_t-pedmos-min ('open-query':U).
...
/*       END.                                            */
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-pedmos-min <> ? THEN RUN Recalcular-Precios IN h_t-pedmos-min.
