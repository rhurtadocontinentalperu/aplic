...
PEDI.PreUni PEDI.PorDto PEDI.Por_Dsctos[2] PEDI.Por_Dsctos[3] PEDI.ImpLin 
...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-Mensaje F-ImpDto2 F-TotBrt ~
F-ImpExo F-ValVta F-ImpIsc F-ImpIgv F-ImpTot F-ImpDes F-ImpTot-S F-ImpTot-D 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 125 BY 9.96
         BGCOLOR 15 FONT 4.
...
/*             '1. No está registrado, o' SKIP                             */
/*             '2. No está en la lista de precios, o' SKIP                 */
/*             '3. Las unidades de venta están mal definidas' SKIP(1)      */
...
------------------------------------------------------------------------------*/
  ...
  /* RECALCULO */
  IF s-CodBko = "WI" AND TODAY <= 03/10/2011 THEN F-ImpDto2 = MINIMUM(F-ImpDto2, 50).   /* TOPE DE S/.50 */
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
  /*IF s-registro-activo = NO THEN RETURN "ADM-ERROR".*/
  FIND Almmmatg WHERE Almmmatg.CodCia = S-CODCIA 
...
FOR EACH B-PEDI, FIRST Almmmatg OF B-PEDI NO-LOCK:
    ...
    ASSIGN 
        ...
        B-PEDI.UndVta = s-UndVta
        B-PEDI.PreUni = F-PREVTA
        B-PEDI.PreBas = F-PreBas 
        B-PEDI.PorDto = F-DSCTOS  /* Ambos descuentos afectan */
        B-PEDI.PorDto2 = 0        /* el precio unitario */
        B-PEDI.ImpDto2 = 0
        B-PEDI.Por_Dsctos[2] = z-Dsctos
        B-PEDI.Por_Dsctos[3] = Y-DSCTOS 
        B-PEDI.AftIgv = ( IF s-CodIgv = 1 THEN Almmmatg.AftIgv ELSE NO )
        B-PEDI.AftIsc = Almmmatg.AftIsc
        B-PEDI.ImpIsc = 0
        B-PEDI.ImpIgv = 0
        B-PEDI.Libre_c04 = X-TIPDTO.
        
    {vtamin/i-cotvit.i &Tabla = "B-PEDI"}
