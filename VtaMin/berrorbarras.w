...
DEF VAR s-tabla AS CHAR INIT 'ERRBARRA' NO-UNDO.
...
VtaTabla.Rango_fecha[1] VtaTabla.Rango_valor[1] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c1 COLUMN-LABEL "Número" FORMAT "x(9)":U WIDTH 9.43
      VtaTabla.Rango_fecha[1] COLUMN-LABEL "Vencimiento" FORMAT "99/99/9999":U
            WIDTH 14.43
      VtaTabla.Rango_valor[1] COLUMN-LABEL "Importe en S/." FORMAT ">>9":U
            WIDTH 14.72
  ENABLE
      VtaTabla.Llave_c1
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 45 BY 13.85
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "INTEGRAL.VtaTabla.Llave_c1|yes"
     _Where[1]         = "VtaTabla.CodCia = s-codcia
 AND VtaTabla.Tabla = s-tabla"
     _FldNameList[1]   > INTEGRAL.VtaTabla.Llave_c1
"VtaTabla.Llave_c1" "Número" "x(9)" "character" ? ? ? ? ? ? yes ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.VtaTabla.Rango_fecha[1]
"VtaTabla.Rango_fecha[1]" "Vencimiento" ? "date" ? ? ? ? ? ? yes ? no no "14.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaTabla.Rango_valor[1]
"VtaTabla.Rango_valor[1]" "Importe en S/." ">>9" "decimal" ? ? ? ? ? ? yes ? no no "14.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF VtaTabla.Llave_c1:SCREEN-VALUE IN BROWSE {&browse-name} = ''
    THEN DO:
...
IF CAN-FIND(FIRST VtaTabla WHERE VtaTabla.codcia = s-codcia
            ...
            NO-LOCK)
    THEN DO:
    MESSAGE 'Vale de consumo repetido' VIEW-AS ALERT-BOX ERROR.
    APPLY 'entry':U TO VtaTabla.Llave_c1 IN BROWSE {&browse-name}.
    RETURN 'ADM-ERROR'.
END.
IF DECIMAL(VtaTabla.Rango_valor[1]:SCREEN-VALUE IN BROWSE {&browse-name}) <= 0 
    THEN DO:
...
IF INPUT VtaTabla.Rango_fecha[1] = ? 
    THEN DO:
    MESSAGE 'Ingrese el vencimiento del vale' VIEW-AS ALERT-BOX ERROR.
