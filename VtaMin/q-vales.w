...
&Scoped-define QUERY-STRING-Query-Main FOR EACH VtaVales WHERE ~{&KEY-PHRASE} ~
      AND VtaVales.CodCia = s-codcia NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH VtaVales WHERE ~{&KEY-PHRASE} ~
      AND VtaVales.CodCia = s-codcia NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main VtaVales
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main VtaVales


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS F-Numero 
...
DEFINE VARIABLE F-Numero AS CHARACTER FORMAT "X(15)":U 
     VIEW-AS FILL-IN 
     SIZE 15 BY .69
     FONT 4 NO-UNDO.
...
DEFINE FRAME FRAME-B
     F-Numero AT ROW 1 COL 2 NO-LABEL WIDGET-ID 4
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         THREE-D 
         AT COL 1 ROW 1
         SIZE 17 BY 1.35 WIDGET-ID 100.
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "INTEGRAL.VtaVales.CodCia = s-codcia"
     _Design-Parent    is WINDOW q-tables @ ( 1.23 , 14.86 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    ...
    */
    RUN lkup\c-vales ("Vales de Descuento").
...
FIND vtavales WHERE vtavales.codcia = s-codcia
    ...
    AND vtavales.nrodoc BEGINS F-Numero NO-LOCK NO-ERROR.

IF AVAILABLE VtaVales THEN output-var-1 = ROWID(VtaVales).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
        ROWID({&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}}) = OUTPUT-VAR-1 NO-LOCK NO-ERROR.
