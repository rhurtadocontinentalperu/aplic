...
------------------------------------------------------------------------------*/
    ...
    FOR EACH ccbdcaja OF ccbccaja NO-LOCK :
        FIND ccbcdocu WHERE
            ...
            ccbcdocu.nrodoc = ccbdcaja.nroref AND
            ccbcdocu.codcli = ccbdcaja.codcli
            NO-LOCK NO-ERROR.
        IF AVAILABLE ccbcdocu AND ccbcdocu.Flgest = "A" THEN DO:
            MESSAGE
                "La " + ccbcdocu.coddoc + "  Ya fue Anulada" SKIP
                "Anulacion Por Aplicacion de Documento" SKIP
                "Imposible anular el Ingreso"
                VIEW-AS ALERT-BOX ERROR.
            RETURN "ADM-ERROR".
        END.
        /* RHC 22.02.05 NO anular si ya salio por vigilancia */
        FIND AlmDCDoc WHERE
            Almdcdoc.codcia = ccbcdocu.codcia AND
            Almdcdoc.coddoc = ccbcdocu.coddoc AND
            Almdcdoc.nrodoc = ccbcdocu.nrodoc AND
            Almdcdoc.flgest = 'S' NO-LOCK NO-ERROR.
        IF AVAILABLE Almdcdoc THEN DO:
            MESSAGE
                "La" ccbcdocu.coddoc ccbcdocu.nrodoc "ya salió por Vigilancia" SKIP
                "Imposible anular el Ingreso"
                VIEW-AS ALERT-BOX ERROR.
            RETURN "ADM-ERROR".
        END.
 ...
 *                 END.*/
                ...
                FOR EACH ccbDdocu OF ccbcdocu EXCLUSIVE-LOCK:
                    DELETE ccbDdocu.
                END.
                /* RHC 05.10.05 Revertir los puntos bonus */
                FIND GN-CARD WHERE
                    GN-CARD.nrocard = Ccbcdocu.nrocard
                    EXCLUSIVE-LOCK NO-ERROR.
