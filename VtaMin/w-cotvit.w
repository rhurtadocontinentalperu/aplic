...
&Scoped-Define ENABLED-OBJECTS RECT-23 COMBO-NroSer 
...
DEFINE VARIABLE h_t-cotvit AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotvit AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* COTIZACIONES VITRINA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* COTIZACIONES VITRINA */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'VtaMin/v-cotvit.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotvit ).
       RUN set-position IN h_v-cotvit ( 3.15 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-cotvit. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-cotvit ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_v-cotvit ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotvit ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-size IN h_b-pedmos-4 ( 15.88 , 125.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'VtaMin/t-cotvit.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotvit ).
       RUN set-position IN h_t-cotvit ( 5.85 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotvit ( 15.88 , 125.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 6.92 , 128.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 8.08 , 15.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-cotvit. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotvit ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotvit ,
             h_v-cotvit , 'AFTER':U ).
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-cotvit <> ? THEN RUN dispatch IN h_t-cotvit ('open-query':U).
      END.
