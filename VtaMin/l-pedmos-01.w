...
&SCOPED-DEFINE CONDICION ( FacCPedm.CodCia = S-CODCIA AND ~
                           FacCPedm.CodDiv = S-CODDIV  AND ~
                           FacCPedm.CodDoc = input-var-1 AND ~
                           FacCPedm.NroPed BEGINS STRING(s-NroSer, '999') AND ~
                           FacCPedm.Usuario = input-var-2 AND ~
                           FacCPedm.FlgEst BEGINS input-var-3 )
...
&Scoped-define FIELDS-IN-QUERY-br_table Faccpedm.CodDoc Faccpedm.NroPed ~
Faccpedm.NomCli Faccpedm.FchPed Faccpedm.ImpTot C-MON @ C-MON 
...
<SORTBY-OPTIONS>
Código|y||integral.Faccpedm.CodCia|yes,integral.Faccpedm.CodDoc|yes,integral.Faccpedm.FchPed|no,integral.Faccpedm.NroPed|yes
Cliente|||integral.Faccpedm.CodCia|yes,integral.Faccpedm.NomCli|yes,integral.Faccpedm.FchPed|no
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "':U + 'Código,Cliente' + '",
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Faccpedm.CodDoc COLUMN-LABEL "Cod." FORMAT "x(3)":U
      Faccpedm.NroPed COLUMN-LABEL "Documento No." FORMAT "XXX-XXXXXXXX":U
      Faccpedm.NomCli FORMAT "x(35)":U
      Faccpedm.FchPed COLUMN-LABEL "Fecha Emision" FORMAT "99/99/9999":U
      Faccpedm.ImpTot FORMAT "->>,>>>,>>9.99":U
      C-MON @ C-MON COLUMN-LABEL "Mon" FORMAT "X(4)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 69.14 BY 8.65
...
/* Query rebuild information for BROWSE br_table
     ...
     _TblOptList       = ", FIRST"
     _Where[1]         = "{&CONDICION}"
     _FldNameList[1]   > integral.Faccpedm.CodDoc
"Faccpedm.CodDoc" "Cod." ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.Faccpedm.NroPed
"Faccpedm.NroPed" "Documento No." "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.Faccpedm.NomCli
"Faccpedm.NomCli" ? "x(35)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > integral.Faccpedm.FchPed
"Faccpedm.FchPed" "Fecha Emision" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   = integral.Faccpedm.ImpTot
...
DO:
    ...
    C-NROPED = C-NROSER + STRING(INPUT FILL-IN-codigo,"999999").
...
ON FIND OF Faccpedm
DO:
   C-MON = IF Faccpedm.CodMon = 1 THEN "S/." ELSE "US$".
END.
/* ***************************  Main Block  *************************** */
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY Faccpedm.CodCia BY Faccpedm.CodDoc BY Faccpedm.FchPed DESCENDING BY Faccpedm.NroPed
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Cliente':U THEN DO:
           &Scope SORTBY-PHRASE BY Faccpedm.CodCia BY Faccpedm.NomCli BY Faccpedm.FchPed DESCENDING
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
