...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbDCaja.NroRef FORMAT "XXX-XXXXXXXX":U
      C-Mon @ C-Mon COLUMN-LABEL "Mon" FORMAT "X(4)":U
      CcbDCaja.ImpTot FORMAT "->>,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 78.29 BY 6.65
...
"CcbCCaja.NomCli" "Nombre de Cliente" "X(40)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbDCaja.NroRef" ? "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Extorna-Puntos-Arte L-table-Win 
PROCEDURE Extorna-Puntos-Arte :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF CcbCCaja.PuntosNac <= 0 THEN RETURN.
/* Buscamos el DNI */
FIND FIRST Ccbdcaja OF Ccbccaja NO-LOCK NO-ERROR.
IF NOT AVAILABLE Ccbdcaja THEN RETURN ERROR.
FIND FIRST Ccbcdocu WHERE Ccbcdocu.codcia = Ccbdcaja.codcia
    AND Ccbcdocu.coddoc = Ccbdcaja.codref
    AND Ccbcdocu.nrodoc = Ccbdcaja.nroref
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE Ccbcdocu OR Ccbcdocu.CodAnt = '' THEN RETURN ERROR.
/* Extornamos */

FIND BaseArte WHERE BaseArte.CodCia = Ccbccaja.codcia
    AND BaseArte.CodDiv = Ccbccaja.coddiv
    AND BaseArte.NroDocumento = Ccbcdocu.codant
    EXCLUSIVE-LOCK NO-ERROR.
IF ERROR-STATUS:ERROR AND LOCKED BaseArte THEN RETURN ERROR.
/*
  Tabla: "Almacen"
...
{lib/lock-genericov2.i &Tabla="BaseArte" ~
    &Condicion="BaseArte.CodCia = Ccbccaja.codcia ~
    AND BaseArte.CodDiv = Ccbccaja.coddiv ~
    AND BaseArte.NroDocumento = Ccbcdocu.codant" ~
...
FOR EACH PuntosArte OF BaseArte WHERE PuntosArte.QttyPtosConsumidos > 0 BY PuntosArte.FechaHora:
    ASSIGN 
        fPorAsignar = MINIMUM(PuntosArte.QttyPtosConsumidos, fPorRepartir)
...
------------------------------------------------------------------------------*/
    ...
    /*RD01 - Ya no verifica Orden de Despacho***
    /* Verificando las ordenes de despacho */
    FOR EACH Ccbdcaja OF Ccbccaja NO-LOCK:
        FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = Ccbdcaja.codcia
            AND Ccbcdocu.coddoc = Ccbdcaja.codref
            AND Ccbcdocu.nrodoc = CCbdcaja.nroref:
            FOR EACH Faccpedi NO-LOCK WHERE Faccpedi.codcia = Ccbcdocu.codcia
                ...
                IF Faccpedi.FlgEst = "C" THEN DO:
                    MESSAGE "La Orden de Despacho" Faccpedi.NroPed "ya salió del almacén" SKIP
                        "Anulación abortada"
                        VIEW-AS ALERT-BOX ERROR.
                    RETURN "ADM-ERROR".
                END.
    ...
    DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN 'ADM-ERROR':
        /* Extorno de Ordenes de Despacho */
        FOR EACH Ccbdcaja OF Ccbccaja NO-LOCK:
            FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = Ccbdcaja.codcia
                AND Ccbcdocu.coddoc = Ccbdcaja.codref
                AND Ccbcdocu.nrodoc = CCbdcaja.nroref:
        ...
        FOR EACH ccbdcaja OF ccbccaja:
            ...
            FOR EACH b-ccbcdocu WHERE b-ccbcdocu.codcia = Ccbcdocu.codcia
        ...
        /* Anulamos TICKETS */
        FOR EACH VtaDTickets WHERE VtaDTickets.codcia = s-codcia
        ...
        /* RHC 21/04/2015 EXTORNO DE PUNTOS ARTE */
        RUN Extorna-Puntos-Arte NO-ERROR.
        IF ERROR-STATUS:ERROR THEN DO:
            MESSAGE 'No se pudo extornar los PUNTOS ARTE' SKIP
        ...
        RELEASE Faccpedi.
        IF AVAILABLE BaseArte THEN RELEASE BaseArte.
        IF AVAILABLE PuntosArte THEN RELEASE PuntosArte.
    END. /* DO TRANSACTION... */
