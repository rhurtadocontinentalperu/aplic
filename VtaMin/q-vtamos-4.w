...
&Scoped-define QUERY-STRING-Query-Main FOR EACH faccpedm WHERE ~{&KEY-PHRASE} ~
      AND faccpedm.CodCia = S-CODCIA ~
 AND faccpedm.CodDiv = S-CODDIV ~
 AND faccpedm.usuario = S-USER-ID ~
 AND faccpedm.CodDoc = s-coddoc ~
 AND FacCPedm.NroPed BEGINS STRING(S-NROSER,"999") NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH faccpedm WHERE ~{&KEY-PHRASE} ~
      AND faccpedm.CodCia = S-CODCIA ~
 AND faccpedm.CodDiv = S-CODDIV ~
 AND faccpedm.usuario = S-USER-ID ~
 AND faccpedm.CodDoc = s-coddoc ~
 AND FacCPedm.NroPed BEGINS STRING(S-NROSER,"999") NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main faccpedm
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main faccpedm
...
/* Query rebuild information for QUERY Query-Main
 ...
 AND integral.faccpedm.usuario = S-USER-ID
 AND integral.faccpedm.CodDoc = s-coddoc
 AND FacCPedm.NroPed BEGINS STRING(S-NROSER,""999"")"
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
      RUN vtamin/c-pedmos-01 ('PENDIENTES').
    IF OUTPUT-VAR-1 <> ? THEN DO:
         FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
              ROWID({&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}}) = OUTPUT-VAR-1
              NO-LOCK NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Localiza q-tables 
PROCEDURE Localiza :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER OUTPUT-VAR-1 AS ROWID.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND FacCPedm WHERE FacCPedi.CodCia = S-CODCIA AND
     FacCPedm.CodDoc = s-coddoc AND
     FacCPedm.NroPed = STRING(S-NROSER,"999") + STRING(F-Numero,"999999") 
     NO-LOCK NO-ERROR.
