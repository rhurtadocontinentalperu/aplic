...
&SCOPED-DEFINE Condicion ( FacCPedi.CodCia = s-CodCia ~
    ...
    AND (TRUE <> (cNomCli > '') OR INDEX(Faccpedi.nomcli,cNomCli) > 0) )
...
&Scoped-Define ENABLED-OBJECTS BUTTON-Filtrar FILL-IN-NomCli br_table 
...
DEFINE FRAME F-Main
     BUTTON-Filtrar AT ROW 1.12 COL 71 WIDGET-ID 6
     FILL-IN-NomCli AT ROW 1.27 COL 19 COLON-ALIGNED WIDGET-ID 4
...
"FacCPedi.ImpTot" "Importe!Total" ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "7.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  cNomCli = FILL-IN-NomCli:SCREEN-VALUE.
