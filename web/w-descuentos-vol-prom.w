...
DEFINE VARIABLE h_b-descuentos-vol-prom-may AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-descuentos-vol-prom-min AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 192.14 BY 26.38 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "DESCUENTO POR VOLUMEN Y PROMOCIONAL"
         HEIGHT             = 26.42
         WIDTH              = 101.72
         MAX-HEIGHT         = 27.85
         MAX-WIDTH          = 194.43
...
ON END-ERROR OF W-Win /* DESCUENTO POR VOLUMEN Y PROMOCIONAL */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* DESCUENTO POR VOLUMEN Y PROMOCIONAL */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'LABEL-FONT = 4,
                     ...
                     LABELS = Mayorista|Eventos|Por División|Utilex':U ,
             OUTPUT h_tab95 ).
       RUN set-position IN h_tab95 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_tab95 ( 26.38 , 100.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/web/b-descuentos-vol-prom-may.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-descuentos-vol-prom-may ).
       RUN set-position IN h_b-descuentos-vol-prom-may ( 2.08 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-descuentos-vol-prom-may ( 23.15 , 88.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-descuentos-vol-prom-may ,
             h_tab95 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'web/b-descuentos-vol-prom-expo.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-listas-mayorista-eventos-v2 ).
       RUN set-position IN h_b-listas-mayorista-eventos-v2 ( 2.08 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-listas-mayorista-eventos-v2 ( 23.42 , 87.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12-2 ).
       RUN set-position IN h_p-updv12-2 ( 25.50 , 2.00 ) NO-ERROR.
    ...
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'web/b-descuentos-vol-prom-min.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-descuentos-vol-prom-min ).
       RUN set-position IN h_b-descuentos-vol-prom-min ( 2.08 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-descuentos-vol-prom-min ( 23.15 , 98.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12-3 ).
       RUN set-position IN h_p-updv12-3 ( 25.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12-3 ( 1.42 , 34.14 ) NO-ERROR.

       /* Links to SmartBrowser h_b-descuentos-vol-prom-min. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12-3 , 'TableIO':U , h_b-descuentos-vol-prom-min ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-descuentos-vol-prom-min ,
             h_tab95 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12-3 ,
             h_b-descuentos-vol-prom-min , 'AFTER':U ).
    END. /* Page 4 */
