...
VtaDctoPromMin.Descuento VtaDctoPromMin.Tipo 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaDctoPromMin.CodDiv COLUMN-LABEL "División" FORMAT "x(8)":U
      ...
      VtaDctoPromMin.Tipo FORMAT "x(15)":U WIDTH 14.43
  ENABLE
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 84 BY 15.08
         FONT 4
...
"VtaDctoPromMin.CodDiv" "División" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaDctoPromMin.Descuento" "Descuento (%)" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.VtaDctoPromMin.Tipo
"VtaDctoPromMin.Tipo" ? ? "character" ? ? ? ? ? ? no ? no no "14.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF VtaDctoPromMin.CodDiv IN BROWSE br_table /* División */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL VtaDctoPromMin.CodDiv br_table _BROWSE-COLUMN B-table-Win
ON LEFT-MOUSE-DBLCLICK OF VtaDctoPromMin.CodDiv IN BROWSE br_table /* División */
...
x-PorDto =  DECIMAL(VtaDctoPromMin.Descuento:SCREEN-VALUE IN BROWSE {&browse-name}).
IF x-PorDto > 0 THEN DO:
    RUN PRI_Margen-Utilidad-Listas IN hProc (INPUT VtaDctoPromMin.CodDiv:SCREEN-VALUE IN BROWSE {&browse-name},
                                             INPUT FILL-IN-CodMat:SCREEN-VALUE IN FRAME {&FRAME-NAME},
