...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    /* NOTA: Para pasar un pedido urgente de Susana León volvemos a habilitar la rutina anterior */
    IF s-user-id BEGINS "MCS-00" 
        THEN RUN PED_Despachar-Pedido-v2 IN hProc (INPUT ROWID(Faccpedi), OUTPUT pMensaje).
    ELSE RUN PED_Despachar-Pedido IN hProc (INPUT ROWID(Faccpedi), OUTPUT pMensaje).
...
PROCEDURE verificar-linea-de-credito :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
