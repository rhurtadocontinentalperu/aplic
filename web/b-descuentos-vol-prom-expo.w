...
DEFINE FRAME F-Main
     ...
     BUTTON-8 AT ROW 2.08 COL 70 WIDGET-ID 20
...
DO:
    IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
        RUN web/d-descuento-prom-expo ({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codmat, COMBO-BOX-Division).
END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      RUN PRI_Divisiones-Validas IN hProc (INPUT 4,     /* Eventos */
                                           OUTPUT pDivisiones).
      x-ListaDivisiones = x-ListaDivisiones +
          (IF TRUE <> (x-ListaDivisiones > '') THEN '' ELSE ',') +
          pDivisiones.
