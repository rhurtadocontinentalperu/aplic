...
DEFINE TEMP-TABLE T-MATG NO-UNDO LIKE VtaListaMay.
...
(TRUE <> (x-DesMat > '') OR INDEX(Almmmatg.desmat, x-DesMat) > 0) )
...
&Scoped-define QUERY-STRING-br_table FOR EACH T-MATG WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      ...
      FIRST VtaListaMay OF T-MATG NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      T-MATG.codmat COLUMN-LABEL "Artículo" FORMAT "X(6)":U WIDTH 8.43
      Almmmatg.DesMat FORMAT "X(60)":U WIDTH 61.43
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: T-MATG T "?" NO-UNDO INTEGRAL VtaListaMay
   END-TABLES.
 */
...
"T-MATG.codmat" "Artículo" ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almmmatg.DesMat
"Almmmatg.DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ? no no "61.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
FOR EACH VtaListaMay NO-LOCK WHERE VtaListaMay.codcia = s-codcia
    ...
    AND (TRUE <> (x-CodMat > '') OR VtaListaMay.codmat = x-CodMat),
    FIRST Almmmatg OF VtaListaMay NO-LOCK WHERE {&Condicion}:
    CREATE T-MATG.
    BUFFER-COPY VtaListaMay TO T-MATG.
END.
...
FOR EACH T-MATG, FIRST Almmmatg OF T-MATG NO-LOCK, FIRST VtaListaMay OF T-MATG NO-LOCK:
