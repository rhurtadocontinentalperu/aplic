...
FILL-IN-NomPro RADIO-SET-Vigente x-DesMar 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 105 BY 13.23
...
DEFINE FRAME F-Main
     COMBO-BOX-CodFam AT ROW 8.54 COL 13 COLON-ALIGNED WIDGET-ID 8
     COMBO-BOX-SubFam AT ROW 9.5 COL 13 COLON-ALIGNED WIDGET-ID 10
     x-CodPro AT ROW 10.46 COL 13 COLON-ALIGNED WIDGET-ID 12
     FILL-IN-NomPro AT ROW 10.46 COL 25 COLON-ALIGNED NO-LABEL WIDGET-ID 14
     RADIO-SET-Vigente AT ROW 10.96 COL 76 NO-LABEL WIDGET-ID 40
     x-DesMar AT ROW 11.42 COL 13 COLON-ALIGNED WIDGET-ID 16
     BUTTON-1 AT ROW 11.5 COL 27 WIDGET-ID 18
     BROWSE-2 AT ROW 12.85 COL 2 WIDGET-ID 200
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "GENERAR PLANTILLA DESCUENTOS PROMOCIONALES - POR DIVISION"
         HEIGHT             = 25.54
         WIDTH              = 108.57
         MAX-HEIGHT         = 25.54
         MAX-WIDTH          = 143.57
...
ON END-ERROR OF W-Win /* GENERAR PLANTILLA DESCUENTOS PROMOCIONALES - POR DIVISION */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* GENERAR PLANTILLA DESCUENTOS PROMOCIONALES - POR DIVISION */
DO:
...
------------------------------------------------------------------------------*/
  DISPLAY COMBO-BOX-CodFam COMBO-BOX-SubFam x-CodPro FILL-IN-NomPro 
          RADIO-SET-Vigente x-DesMar 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE COMBO-BOX-CodFam COMBO-BOX-SubFam x-CodPro RADIO-SET-Vigente x-DesMar 
         BUTTON-1 BROWSE-2 
      WITH FRAME F-Main IN WINDOW W-Win.
        ...
        DO k = 1 TO NUM-ENTRIES(f-Divisiones):
            ...
            IF AVAILABLE t-VtaDctoProm THEN DO:
                ASSIGN
                    ...
                    NO-ERROR.
                    ASSIGN
                        chWorkSheet:Cells(t-Row, 10):VALUE = (IF RADIO-SET-2 = 1 THEN t-VtaDctoProm.DescuentoVIP
                            ELSE ROUND(x-PreOfi * ( 1 - ( t-VtaDctoProm.DescuentoVIP / 100 ) ),4) )
                        NO-ERROR.
                    IF t-VtaDctoProm.DescuentoVIP = 0 THEN chWorkSheet:Cells(t-Row, 10):VALUE = 0.
                    ASSIGN
                        chWorkSheet:Cells(t-Row, 11):VALUE = (IF RADIO-SET-2 = 1 THEN t-VtaDctoProm.DescuentoMR
                            ELSE ROUND(x-PreOfi * ( 1 - ( t-VtaDctoProm.DescuentoMR / 100 ) ),4) )
                        NO-ERROR.
                    IF t-VtaDctoProm.DescuentoMR = 0 THEN chWorkSheet:Cells(t-Row, 11):VALUE = 0.
                    ASSIGN
                        chWorkSheet:Cells(t-Row, 12):VALUE = (IF RADIO-SET-2 = 1 THEN t-VtaDctoProm.Descuento
                            ELSE ROUND(x-PreOfi * ( 1 - ( t-VtaDctoProm.Descuento / 100 ) ),4) )
                        NO-ERROR.
                    IF t-VtaDctoProm.Descuento = 0 THEN chWorkSheet:Cells(t-Row, 12):VALUE = 0.
            END.
        END.
    END.
END.
...
FOR EACH Almmmatg NO-LOCK WHERE {&Condicion},
    ...
    CASE TRUE:
        WHEN CAN-FIND(FIRST VtaDctoProm WHERE VtaDctoProm.CodCia = VtaListaMay.CodCia AND
                      VtaDctoProm.CodMat = VtaListaMay.CodMat AND
                      VtaDctoProm.CodDiv = VtaListaMay.CodDiv AND
                      TODAY >= VtaDctoProm.FchIni AND
                      TODAY <= VtaDctoProm.FchFin NO-LOCK) THEN DO:
            FOR EACH VtaDctoProm NO-LOCK WHERE VtaDctoProm.CodCia = VtaListaMay.CodCia AND
                VtaDctoProm.CodMat = VtaListaMay.CodMat AND
                VtaDctoProm.CodDiv = VtaListaMay.CodDiv AND
                TODAY >= VtaDctoProm.FchIni AND
                ...
                ASSIGN
                    t-Column = t-Column + 1.
                ASSIGN
                    chWorkSheet:Cells(t-Row, 8):VALUE = VtaDctoProm.FchIni 
                    chWorkSheet:Cells(t-Row, 9):VALUE = VtaDctoProm.FchFin
                    NO-ERROR.
                ASSIGN
                    chWorkSheet:Cells(t-Row, 10):VALUE = (IF RADIO-SET-2 = 1 THEN VtaDctoProm.DescuentoVIP
                        ELSE ROUND(x-PreOfi * ( 1 - ( VtaDctoProm.DescuentoVIP / 100 ) ),4) )
                    NO-ERROR.
                IF VtaDctoProm.DescuentoVIP = 0 THEN chWorkSheet:Cells(t-Row, 10):VALUE = 0.
                ASSIGN
                    chWorkSheet:Cells(t-Row, 11):VALUE = (IF RADIO-SET-2 = 1 THEN VtaDctoProm.DescuentoMR
                        ELSE ROUND(x-PreOfi * ( 1 - ( VtaDctoProm.DescuentoMR / 100 ) ),4) )
                    NO-ERROR.
                IF VtaDctoProm.DescuentoMR = 0 THEN chWorkSheet:Cells(t-Row, 11):VALUE = 0.
                ASSIGN
                    chWorkSheet:Cells(t-Row, 12):VALUE = (IF RADIO-SET-2 = 1 THEN VtaDctoProm.Descuento
                        ELSE ROUND(x-PreOfi * ( 1 - ( VtaDctoProm.Descuento / 100 ) ),4) )
                    NO-ERROR.
                IF VtaDctoProm.Descuento = 0 THEN chWorkSheet:Cells(t-Row, 12):VALUE = 0.
            END.
        END.
        OTHERWISE DO:
            ASSIGN
