...
DEFINE TEMP-TABLE ITEM-X NO-UNDO LIKE FacDPedi.
...
"5", 5
     ...
     FacCPedi.NroRef AT ROW 9.88 COL 111 COLON-ALIGNED NO-LABEL WIDGET-ID 122
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
     FacCPedi.NroOrigen AT ROW 10.15 COL 128.86 COLON-ALIGNED NO-LABEL WIDGET-ID 188
          VIEW-AS FILL-IN 
          SIZE 12.14 BY .81
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: ITEM-X T "?" NO-UNDO INTEGRAL FacDPedi
...
DO:
  ...
  ELSE DO:
      ...
      /* ****************************************************** */
  END.
  /*MESSAGE s-codcli SELF:SCREEN-VALUE LogRecalcular.*/
  s-CodCli = SELF:SCREEN-VALUE.
  /* **********************************************************
...
PROCEDURE actualiza-prepedido PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE Borra-Temporal PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE CREATE-TRANSACION PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE Delete-Items-Cbo PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
PROCEDURE Delete-Items-Cot PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE Delete-Items-Log PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE DESCUENTOS-FINALES PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* Consistencia básica */
IF TRUE <> (s-codcli > '') THEN DO:
    MESSAGE 'Debe ingresar el CLIENTE' VIEW-AS ALERT-BOX WARNING.
    APPLY 'ENTRY':U TO FacCPedi.CodCli IN FRAME {&FRAME-NAME}.
    RETURN.
END.

DO TRANSACTION:
    EMPTY TEMP-TABLE ITEM-X.
    FOR EACH ITEM:
        CREATE ITEM-X.
        BUFFER-COPY ITEM TO ITEM-X.
    END.
END.

DEFINE VAR hProc AS HANDLE NO-UNDO.
RUN vtagn/ventas-library PERSISTENT SET hProc.
RUN COT_Importar-Excel IN hProc (INPUT-OUTPUT TABLE ITEM, OUTPUT TABLE ITEM-1).
DELETE PROCEDURE hProc.
IF RETURN-VALUE = "ADM-ERROR" THEN RETURN.

/* ************************************ */
/* 04/03/2026: Validamos las cantidades */
/* ************************************ */
DEFINE VAR pcMensaje AS CHAR NO-UNDO.
DEFINE VAR pfCanPed AS DECI NO-UNDO.
RUN vtagn/ventas-library PERSISTENT SET hProc.
RUN SET_cliente IN hProc(INPUT s-CodCli).

FOR EACH ITEM-1 EXCLUSIVE-LOCK:
    pfCanPed = ITEM-1.CanPed.
    RUN VTA_Valida-Cantidad IN hProc (INPUT ITEM-1.CodMat,
                                      INPUT ITEM-1.UndVta,
                                      INPUT-OUTPUT pfCanPed,
                                      OUTPUT pcMensaje).
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        MESSAGE "INCONSISTENCIA ARTICULO:" ITEM-1.codmat SKIP(1)
            pcMensaje SKIP 
            'Tomar nota del artículo por favor'
            VIEW-AS ALERT-BOX WARNING.
        DELETE ITEM-1.
    END.
END.
DELETE PROCEDURE hProc.
/* *************************************************** */
/* 09/10/2025: Artificio, solo recalculamos los nuevos */
/* *************************************************** */
...
DO TRANSACTION:
    ...
    FOR EACH ITEM-1:
        ...
        /* 18/11/2025: Verificamos la unidad de venta */
        IF TRUE <> (ITEM-1.undvta > '') THEN DO:
            FIND Almmmatg WHERE Almmmatg.codcia = s-codcia 
                AND Almmmatg.codmat = ITEM-1.codmat NO-LOCK NO-ERROR.
            IF AVAILABLE Almmmatg THEN ITEM-1.undvta = Almmmatg.undstk.
        END.
    END.
/*     MESSAGE 'modo ANTIGUO'. */
/*     RUN Recalcular-Precios. */
    /* ARTIFICIO */
    /*MESSAGE 'modo NUEVO'.*/
    Fi-Mensaje = "RECALCULANDO PRECIOS".
    DISPLAY Fi-Mensaje WITH FRAME F-Proceso.
    RUN pri/pri-precio-mayorista (INPUT-OUTPUT TABLE ITEM,
                                  INPUT pCodDiv,
                                  INPUT s-CodCli,
                                  INPUT s-FmaPgo,
                                  INPUT s-TpoPed,
                                  INPUT s-CodMon,
                                  INPUT s-NroDec,
                                  OUTPUT pcError).     /* Es un LONGCHAR */
    HIDE FRAME F-Proceso.
    IF pcError > '' THEN DO:
        MESSAGE STRING(pcError) VIEW-AS ALERT-BOX WARNING TITLE 'Errores detectados' .
    END.
END.
DO TRANSACTION:
    FOR EACH ITEM-X:
        FIND FIRST ITEM WHERE ITEM.codmat = ITEM-X.codmat NO-LOCK NO-ERROR.
        IF AVAILABLE ITEM THEN NEXT.
        CREATE ITEM.
        BUFFER-COPY ITEM-X TO ITEM.
    END.
END.
DO TRANSACTION:
    FOR EACH ITEM EXCLUSIVE-LOCK WHERE TRUE <> (ITEM.AlmDes > ''):
        ITEM.AlmDes = ENTRY(1,s-CodAlm).
    END.
END.
...
IF TRUE <> (FacCPedi.CodCli:SCREEN-VALUE IN FRAME {&FRAME-NAME} > "") THEN DO:
    MESSAGE "Debe ingresar primero el c digo del cliente" VIEW-AS ALERT-BOX WARNING.
    APPLY "ENTRY":U TO FacCPedi.CodCli.
    RETURN "ADM-ERROR".
END.
...
IF NOT AVAILABLE cmedidefclientes THEN DO:
    MESSAGE 'NO est  configurado el cliente en ning n proveedor de archivos EDI' SKIP
...
DEF VAR x-ImpLinMOA LIKE ITEM.implin NO-UNDO.
DEF VAR x-ImpIgvMOA LIKE ITEM.impigv NO-UNDO.
DEF VAR x-PrecioUnitario LIKE ITEM.preuni NO-UNDO.
...
DEFINE VAR x-init-oc AS INT INIT 9  NO-UNDO.
DEFINE VAR x-long-oc AS INT INIT 35 NO-UNDO.
DEFINE VAR x-order-oc AS INT INIT 5 NO-UNDO.
DEFINE VAR cValues AS CHAR NO-UNDO.

DEFINE VAR x-Orden-LIN AS INTE INIT 3 NO-UNDO.  /* Valor por defecto */

/* Buscamos ubicaci n de acuerdo a la configuraci n */
/* Valor por defecto */
...
FIND FIRST cmediclientespec WHERE cmediclientespec.CodCia = s-codcia AND
    cmediclientespec.codcli = cmedidefclientes.codcli AND
    cmediclientespec.CodPro = cmedidefclientes.CodPro AND
    cmediclientespec.Identificador = "LIN" 
    NO-LOCK NO-ERROR.
IF AVAILABLE cmediclientespec THEN DO:
    FIND cmedidefdet WHERE cmedidefdet.CodCia = s-codcia AND
        cmedidefdet.CodPro = cmedidefclientes.CodPro AND
        cmedidefdet.Identificador = "LIN" AND 
        cmedidefdet.Order = cmediclientespec.Order
        NO-LOCK NO-ERROR.
    /* Se toma la cantidad de caracteres tal como est , nos interesa el ORDEN en el registro */
    IF AVAILABLE cmedidefdet THEN DO:
        x-Orden-LIN = cmediclientespec.Order + 1.     /* La columna + 1 */
    END.
END.

DEFINE VARIABLE lcContenido AS LONGCHAR  NO-UNDO.
DEFINE VARIABLE cLinea      AS CHARACTER NO-UNDO.
DEFINE VARIABLE iTotalLineas AS INTEGER   NO-UNDO.
DEFINE VARIABLE iContador    AS INTEGER   NO-UNDO.

DEFIN VAR lMOA AS LOG.
DEFIN VAR lTAXMOA AS LOG.

SESSION:SET-WAIT-STATE('GENERAL').

/* 1. Leer todo el archivo directamente a la variable */
COPY-LOB FROM FILE x-Archivo TO lcContenido.

/* 2. Contar cuántas líneas existen usando el salto de línea como delimitador */
ASSIGN iTotalLineas = NUM-ENTRIES(lcContenido, "~n").

/* 3. Recorrer cada línea del archivo */
TEXTO:
DO iContador = 1 TO iTotalLineas:
    ASSIGN x-Linea = ENTRY(iContador, lcContenido, "~n").
    
    /* Remover el carácter Carriage Return (~r) si el archivo proviene de Windows */
    ASSIGN x-Linea = REPLACE(x-Linea, "~r", "").

    x-Linea = TRIM(x-Linea).
    
    /* ---- AQUÍ VA TU LÓGICA CON LA VARIABLE cLinea ---- */
    /*DISPLAY cLinea FORMAT "x(60)".*/
    ...
    END.
    /* Cliente: C digo IBC registrado en el maestro de clientes */
    IF x-Linea BEGINS 'IVAD' AND NUM-ENTRIES(x-Linea) > 1 THEN DO:
        ...
        IF AVAILABLE gn-clie THEN DO:
            ...
            IF Faccpedi.codcli:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> gn-clie.CodCli THEN DO:
                MESSAGE "El c digo IBC del archivo texto NO coincide con el del cliente que usted ha registrado" + CHR(10) +
                    "Informaci n del archivo texto: " + x-Linea
                    VIEW-AS ALERT-BOX WARNING.
                APPLY "ENTRY":U TO FacCPedi.CodCli.
                RETURN.
            END.
    ...
    /* ******************************************************************************************* */
    /* DETALLE */
    /* 17/12/2024 Pilar Vega tomar el EAN de una tabla configurable */
    /* ******************************************************************************************* */
    ...
    IF x-Detalle = YES THEN DO:
        IF x-Linea BEGINS 'LIN' THEN DO:
            x-ImpLinMOA = 0.
            x-ImpIgvMOA = 0.
            x-Item = ENTRY(2,x-Linea).
            /* SKU */
            x-CodMat = TRIM(ENTRY(x-Orden-LIN, x-Linea)).   /* Por defecto c digo EAN */
            IF LENGTH(x-CodMat) < 6 THEN DO:
                ASSIGN x-CodMat = STRING(INTEGER(x-CodMat), '999999') NO-ERROR.
            END.
...
/*             IF ERROR-STATUS:ERROR THEN x-CodMat = ENTRY(3,x-Linea).                      */
        ...
        IF NOT AVAILABLE Almmmatg THEN DO:
            MESSAGE "El Item" x-Item x-codmat "no esta registrado en el cat logo" VIEW-AS ALERT-BOX ERROR.
            NEXT TEXTO.
        END.
        /*
        ...
        */
        IF x-Linea BEGINS 'QTY' THEN DO:

            cValues = REPLACE(x-Linea,"QTY,","").
            cValues = REPLACE(cValues,",NAR","").

            cValues = REPLACE(cValues,",","").
            x-CanPed = DECIMAL(cValues).
        END.                        
        IF x-Linea BEGINS 'PRI' THEN DO:
            cValues = REPLACE(ENTRY(2,x-Linea),",","").
            x-PrecioUnitario = DECIMAL(cValues).
        END.
            
        /*IF x-Linea BEGINS 'TAX' THEN DO:*/
        IF x-Linea BEGINS 'TAXMOA' OR x-Linea BEGINS 'MOA' THEN DO:
            /* 01Dic2025 - Cambios pedidos por Pilar, el nuevo TXT no viene con TAX, viene con TAXMOA pero debemos calcular IGV y ImpLIN
            ASSIGN
                x-ImpIgv = DECIMAL(ENTRY(3,x-Linea))
                x-NroItm = x-NroItm + 1.
            */

            IF x-Linea BEGINS 'TAXMOA' THEN DO:
                /* Segun indicaciones de Tomas el precio es sin IGV */
                x-PrecioUnitario = x-PrecioUnitario.
                x-ImpLin = x-CanPed * x-PrecioUnitario.
                x-ImpIgv = 0.

                IF s-PorIgv > 0 THEN DO:
                    x-ImpIgv = x-canped * (x-PrecioUnitario * (1 + (s-PorIgv / 100))).
                    x-ImpIgv = x-ImpIgv - x-implin.
                    /*x-PrecioUnitario = x-PrecioUnitario * (1 + (s-PorIgv / 100)).                */
                END.
                lTAXMOA = YES.
            END.
            IF x-Linea BEGINS 'MOA' THEN DO:
                cValues = REPLACE(x-Linea,"MOA,","").
                cValues = REPLACE(cValues,",","").
                x-ImpLin = DECIMAL(cValues).                
                x-ImpIgv = 0.
                IF s-PorIgv > 0 THEN DO:
                    x-ImpIgv = x-ImpLin - (x-ImpLin / (1 + (s-PorIgv / 100))).
                END.
                lMOA = YES.
                x-ImpLinMOA = x-ImpLin.
                x-ImpIgvMOA = x-ImpIgv.

            END.                        

            /* consistencia de duplicidad */
            x-CodMat = TRIM(x-CodMat).
            FIND FIRST ITEM WHERE ITEM.codmat = x-CodMat NO-LOCK NO-ERROR.
            IF NOT AVAILABLE ITEM THEN DO:
                /*MESSAGE "x-CodMat " x-CodMat.*/
                x-NroItm = x-NroItm + 1.
            ...
            END.
                ASSIGN ITEM.CanPed = x-CanPed.
                /* RHC 09.08.06 IGV de acuerdo al cliente */
                IF LOOKUP(TRIM(s-CodCli), '20100070970,20109072177,20100106915,20504912851') > 0 THEN DO:
                    /*
                    ASSIGN
                    ...
                    */
                    ASSIGN
                        ITEM.ImpIgv = x-ImpIgvMOA 
                        ITEM.ImpLin = x-ImpLinMOA
                        ITEM.PreUni = (ITEM.ImpLin / ITEM.CanPed).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Importar-Supermercados-Old V-table-Win 
PROCEDURE Importar-Supermercados-Old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF AVAILABLE cmediclientespec THEN DO:
    FIND cmedidefdet WHERE cmedidefdet.CodCia = s-codcia AND
        ...
        cmedidefdet.Order = cmediclientespec.Order
        NO-LOCK NO-ERROR.
    /* Se toma la cantidad de caracteres tal como está, nos interesa el ORDEN en el registro */
    IF AVAILABLE cmedidefdet THEN DO:
        x-Orden-LIN = cmediclientespec.Order + 1.     /* La columna + 1 */
    END.
END.
...
REPEAT:
    ...
    IF x-Detalle = YES THEN DO:
        IF x-Linea BEGINS 'LIN' THEN DO:
            x-Item = ENTRY(2,x-Linea).
            /* SKU */
            x-CodMat = TRIM(ENTRY(x-Orden-LIN, x-Linea)).   /* Por defecto código EAN */
            IF LENGTH(x-CodMat) < 6 THEN DO:
                ASSIGN x-CodMat = STRING(INTEGER(x-CodMat), '999999') NO-ERROR.
            END.
/*             IF NUM-ENTRIES(x-Linea) = 6                                                  */
...
/*             IF ERROR-STATUS:ERROR THEN x-CodMat = ENTRY(3,x-Linea).                      */
        ...
        END.
        IF NOT AVAILABLE Almmmatg THEN DO:
            MESSAGE "El Item" x-Item x-codmat "no esta registrado en el catálogo" VIEW-AS ALERT-BOX ERROR.
            NEXT TEXTO.
        END.
        /*
        IF x-Linea BEGINS 'QTY' THEN x-CanPed = DECIMAL(ENTRY(2,x-Linea)).
        ...
        IF x-Linea BEGINS 'TAXMOA' THEN DO:
            ...
            */
            ...
            IF NOT AVAILABLE ITEM THEN DO:
                ...
                /*
                MESSAGE "x-ImpIgv  " x-ImpIgv  SKIP
                    "x-ImpLin " x-ImpLin SKIP
                    "x-PrecioUnitario " x-PrecioUnitario SKIP
                    "s-PorIgv " s-PorIgv.
                */
                /* RHC 09.08.06 IGV de acuerdo al cliente */
                IF LOOKUP(TRIM(s-CodCli), '20100070970,20109072177,20100106915,20504912851') > 0
                ...
                ELSE  DO:
                    ...
                    ITEM.PreUni = (ITEM.ImpLin / ITEM.CanPed).
                    /*
                    MESSAGE "ITEM.ImpIgv " ITEM.ImpIgv SKIP
                        "ITEM.ImpLin " ITEM.ImpLin SKIP
                        "x-ImpIgv " x-ImpIgv SKIP
                        "x-ImpLin " x-ImpLin SKIP
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Importar-Supermercados_OLD2 V-table-Win 
PROCEDURE Importar-Supermercados_OLD2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
        ...
        IF x-Linea BEGINS 'TAXMOA' OR x-Linea BEGINS 'MOA' THEN DO:
            ...
            */
            ...
            IF x-Linea BEGINS 'TAXMOA' THEN DO:
                ...
                END.
                /*
                MESSAGE "x-PrecioUnitario " x-PrecioUnitario SKIP
                        "x-ImpLin " x-ImpLin SKIP
                        "x-ImpIgv " x-ImpIgv.
                */
            END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Insert_Items_Log V-table-Win 
PROCEDURE Insert_Items_Log PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
  ...
  RUN rutina-add-extra.
  /* ******************************************************************************** */
  /* 02/12/2025 : Nueva rutina por lotes */
  /* ******************************************************************************** */
  DEF VAR pcError AS LONGCHAR NO-UNDO.

  DISPLAY "Recalculando precios, un momento por favor..." @ Fi-Mensaje WITH FRAME F-proceso.
...
------------------------------------------------------------------------------*/
  ...
  MESSAGE "Grabación Exitosa" VIEW-AS ALERT-BOX INFORMATION.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Recalcular-Precio-TpoPed V-table-Win 
PROCEDURE Recalcular-Precio-TpoPed PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Recalcular-Precio-TpoPed-M V-table-Win 
PROCEDURE Recalcular-Precio-TpoPed-M PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* ******************************************************* */
/* ******************************************************* */
/*MESSAGE '(1)' s-tpoped '(2)' s-tpomarco.*/
/* ARTIFICIO */
Fi-Mensaje = "RECALCULANDO PRECIOS".
DISPLAY Fi-Mensaje WITH FRAME F-Proceso.
...
PROCEDURE rutina-add-extra PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE UPDATE-TRANSACION PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE valida-update PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
PROCEDURE WRITE-DETAIL PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE VAR cCatgCtbleFamSubFamValidos AS CHAR.
DEFINE VAR cCatgCtbleFamSubFamPedido AS CHAR.

/* Categgoria Contable */
/*
FIND FIRST b-factabla WHERE b-factabla.codcia = 1 AND b-factabla.tabla = 'CFG_CC_DCT_100' NO-LOCK NO-ERROR.
IF AVAILABLE b-factabla THEN cCatgCtbleFamSubFamValidos = TRIM(b-factabla.codigo).
*/
...
FOR EACH ITEM NO-LOCK,
    ...
    IF item.preuni <= 0 OR item.ImpLin < 0 THEN NEXT.

    cCatgCtbleFamSubFamPedido = trim(almmmatg.catconta[1]) + ":" + trim(almmmatg.codfam) + ":" + trim(almmmatg.subfam).
    /*
    IF AVAILABLE b-factabla THEN cCatgCtbleFamSubFamValidos = TRIM(b-factabla.codigo).

    /* Se aceptan descuentos 100%, Susana Leon/Carla Tenazoa 27Nov2024 */
    IF item.ImpLin = 0 AND cCatgCtbleFamSubFamPedido <> cCatgCtbleFamSubFamValidos THEN NEXT.
    */

    /* Se aceptan descuentos 100%, Susana Leon/Carla Tenazoa 27Nov2024 */
    IF item.ImpLin = 0 THEN DO:
        FIND FIRST b-factabla WHERE b-factabla.codcia = s-codcia 
            AND b-factabla.tabla = 'CFG_CC_DCT_100' 
            AND b-factabla.codigo = cCatgCtbleFamSubFamPedido NO-LOCK NO-ERROR.
        IF NOT AVAILABLE b-factabla THEN NEXT.
    END.    
...
PROCEDURE WRITE-HEADER PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* 09/01/2026: Un solo programa */
    /* ****************************************************************************************** */
    EMPTY TEMP-TABLE BONIFICACION.
    FOR EACH Facdpedi OF Faccpedi NO-LOCK:
        CREATE BONIFICACION.
        BUFFER-COPY Facdpedi TO BONIFICACION.
    END.
    RUN web/promocion-general-flash.p (INPUT Faccpedi.CodDiv,
                                       INPUT Faccpedi.CodCli,
                                       INPUT Faccpedi.CodAlm,
                                       INPUT "CREATE",
                                       INPUT-OUTPUT TABLE BONIFICACION,
                                       OUTPUT pMensaje).
    IF RETURN-VALUE = "ADM-ERROR" THEN UNDO, RETURN "ADM-ERROR".

    I-NITEM = 0.
    FOR EACH BONIFICACION WHERE BONIFICACION.Libre_c05 = "OF",          /* OJO */
