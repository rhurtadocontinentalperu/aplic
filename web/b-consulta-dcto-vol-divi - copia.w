...
FOR EACH VtaListaMay NO-LOCK WHERE VtaListaMay.codcia = s-codcia
    ...
    FIRST Almmmatg OF VtaListaMay NO-LOCK WHERE Almmmatg.TpoArt <> "D" AND
    (x-CodFam = 'Todas' OR Almmmatg.CodFam = x-CodFam)
    AND (TRUE <> (x-DesMar > '') OR Almmmatg.DesMar = x-DesMar):
    CREATE T-MATG.
