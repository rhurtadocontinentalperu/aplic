...
DEFINE VARIABLE h_b-listas-import-dctovol-v2 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-listas-import-dctovol-v2 ).
       RUN set-position IN h_b-listas-import-dctovol-v2 ( 2.92 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-listas-import-dctovol-v2 ( 19.04 , 88.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'web/v-import-descuentos-vol-may-min.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-listas-import-dctovol-v2 ).
       RUN set-position IN h_v-listas-import-dctovol-v2 ( 3.15 , 91.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.96 , 29.00 ) */

       /* Links to SmartViewer h_v-listas-import-dctovol-v2. */
       RUN add-link IN adm-broker-hdl ( h_b-listas-import-dctovol-v2 , 'Record':U , h_v-listas-import-dctovol-v2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-listas-import-dctovol-v2 ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-listas-import-dctovol-v2 ,
             h_b-listas-import-dctovol-v2 , 'AFTER':U ).
    END. /* Page 0 */
...
IF NOT (cValue BEGINS "CONTINENTAL - DESCUENTOS POR VOLUMEN"
        ...
        OR cValue BEGINS COMBO-BOX-Division + " - DESCUENTOS POR VOLUMEN")
    THEN DO:
    pMensaje = 'Formato del archivo Excel errado'.
    RETURN.
END.
...
CASE TRUE:
    ...
    OTHERWISE RETURN.
END CASE.
RUN Pinta-Titulo IN h_b-listas-import-dctovol-v2 ( INPUT cNombreLista /* CHARACTER */).
...
REPEAT:
    ...
    k = 1.
    /* 09/01/2023 Se amplia a 10 registros Cesar Camus */
    DO iCountLine = 1 TO 10:
        t-Column = t-Column + 1.
        cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
        ASSIGN
...
/*     END CASE.                                            */
    ...
    CASE RADIO-SET-2:
        WHEN 1 THEN DO:     /* Por % Descuento */
            DO iValue = 1 TO 10:
                IF T-MATG.DtoVolD[iValue] > 0 THEN DO:
                    T-MATG.DtoVolP[iValue] = ROUND(T-MATG.PreVta[1] * ( 1 - T-MATG.DtoVolD[iValue] / 100 ), 4).
                END.
            END.
        END.
        WHEN 2 THEN DO:     /* Por Importe */
            DO iValue = 1 TO 10:
                IF T-MATG.DtoVolD[iValue] > 0 THEN DO:
                    T-MATG.DtoVolP[iValue] = T-MATG.DtoVolD[iValue].
                    T-MATG.DtoVolD[iValue] = ROUND ( ( 1 -  T-MATG.DtoVolP[iValue] / T-MATG.PreVta[1]  ) * 100, 6 ).
                END.
            END.
        END.
    END CASE.
...
RUN dispatch IN h_b-listas-import-dctovol-v2 ('open-query':U).
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH T-MATG EXCLUSIVE-LOCK:
        DO k = 1 TO 10:
            IF T-MATG.DtoVolD[k] <= 0 THEN NEXT.
            /* ****************************************************************************************************** */
            /* Control Margen de Utilidad */
            /* ****************************************************************************************************** */
            /* 1ro. Calculamos el margen de utilidad */
            RUN PRI_Margen-Utilidad-Listas IN hProc (INPUT COMBO-BOX-Division,
                                                     INPUT T-MATG.CodMat,
                                                     INPUT T-MATG.CHR__01,
                                                     INPUT T-MATG.DtoVolD[k],
...
MESSAGE 'Procedemos a grabar los decuentos por volumen?'
...
CASE RADIO-SET-1:
    WHEN 1 THEN DO:
        {web/i-import-descuentos-vol-may-min.i &Tabla=Almmmatg}
    END.
    WHEN 2 THEN DO:
        {web/i-import-descuentos-vol-may-min.i &Tabla=Vtalistamingn}
    END.
    WHEN 3 THEN DO:
        /* ERRORES */
        FOR EACH T-MATG ON STOP UNDO, RETURN ON ERROR UNDO, RETURN:
            ...
            IF NOT AVAILABLE VtaListaMay THEN DO:
                CREATE VtaListaMay.
                ASSIGN
                    VtaListaMay.CodCia = T-MATG.CodCia
...
ASSIGN
    ...
    LogVtaTabla.Tabla  = 'DCTO_VOL'
    LogVtaTabla.Llave_c1 = FILL-IN-Archivo
    LogVtaTabla.Llave_c2 = COMBO-BOX-Division
    LogVtaTabla.LogDate = TODAY
    /*LogVtaTabla.LogEvento = {&WINDOW-NAME}:TITLE*/
...
CASE TRUE:
    WHEN RADIO-SET-1 = 1 THEN cNombreLista = "CONTINENTAL - DESCUENTOS POR VOLUMEN".
    WHEN RADIO-SET-1 = 2 THEN cNombreLista = "UTILEX - DESCUENTOS POR VOLUMEN".
    WHEN RADIO-SET-1 = 3 THEN cNombreLista = COMBO-BOX-Division + " - DESCUENTOS POR VOLUMEN".
    OTHERWISE RETURN.
END CASE.
CASE TRUE:
...
END CASE.
ASSIGN
    LogVtaTabla.LogEvento = cNombreLista.
