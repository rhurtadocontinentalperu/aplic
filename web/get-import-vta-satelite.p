...
FOR EACH FacCVtaOnLine WHERE FacCVtaOnLine.codcia = s-codcia
    ...
    /* Verificamos que no se duplique el pedido Riqra en el VTACDOCU - validacion temporal */
    FIND FIRST vtacdocu WHERE vtacdocu.codcia = FacCVtaOnLine.codcia AND 
                                vtacdocu.codori = FacCVtaOnLine.codori AND
                                vtacdocu.nroori = FacCVtaOnLine.nroori AND
                                vtacdocu.ordcmp = FacCVtaOnLine.ordcmp NO-LOCK NO-ERROR.
    IF AVAILABLE vtacdocu THEN NEXT.
