...
&Scoped-define QUERY-STRING-BROWSE-2 FOR EACH ITEM NO-LOCK, ~
      FIRST Almmmatg OF ITEM OUTER-JOIN NO-LOCK INDEXED-REPOSITION
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-2 W-Win _STRUCTURED
  QUERY BROWSE-2 NO-LOCK DISPLAY
      ...
      Almmmatg.DesMat FORMAT "X(45)":U WIDTH 103.86
      ITEM.CanPed FORMAT ">,>>>,>>9.9999":U WIDTH 11.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 133 BY 22.62 FIT-LAST-COLUMN.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     BUTTON-2 AT ROW 1.54 COL 4 WIDGET-ID 2
...
"INTEGRAL.Almmmatg.DesMat" ? ? "character" ? ? ? ? ? ? no ? no no "103.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.ITEM.CanPed
"Temp-Tables.ITEM.CanPed" ? ? "decimal" ? ? ? ? ? ? no ? no no "11.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
&ANALYZE-RESUME
...
ON CHOOSE OF BUTTON-2 IN FRAME F-Main /* CAPTURA */
DO:
  DEF VAR pOk AS LOG NO-UNDO.

  RUN web/d-listaexpress.w (INPUT-OUTPUT TABLE ITEM,
                            OUTPUT pOk).
