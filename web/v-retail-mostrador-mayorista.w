...
DEF VAR x-Old-Client AS CHAR NO-UNDO.   /* Variable de control para ver si el usuario cambia el cliente */

/* Levantamos en memoria la libreria */
DEFINE VAR hProcSalesLibrary AS HANDLE NO-UNDO.
RUN web/sales-library PERSISTENT SET hProcSalesLibrary.
...
ON ENTRY OF FacCPedi.CodCli IN FRAME F-Main /* Cliente */
DO:
  /* 25/2/2025: Variable de control en caso de que se cambie el cliente */
  x-Old-Client = SELF:SCREEN-VALUE.
END.
...
DO:
  ...
  /* 25/02/2025: Control de cambio de cliente */
  IF x-Old-Client <> s-CodCli THEN RUN Procesa-Handle IN lh_Handle ('Recalculo').
...
DEFINE VAR pcError AS CHAR NO-UNDO.
/* DEFINE VAR hProcSalesLibrary AS HANDLE NO-UNDO.         */
/*                                                         */
/* RUN web/sales-library PERSISTENT SET hProcSalesLibrary. */

RUN VALIDATE_Time IN hProcSalesLibrary (BUFFER Faccpedi, OUTPUT pcError).
IF pcError > '' THEN DO:
    MESSAGE pcError VIEW-AS ALERT-BOX WARNING.
    RETURN 'ADM-ERROR'.
END.
/* DELETE PROCEDURE hProcSalesLibrary. */
...
RUN Write_Header IN hProcSalesLibrary (BUFFER Faccpedi, OUTPUT pMensaje).
...
RUN Cancel_Invoice IN hProcSalesLibrary (BUFFER Faccpedi, OUTPUT pMensaje).
ASSIGN
    s-CodDoc = cCodDoc.
/* DELETE PROCEDURE hProcSalesLibrary. */
IF pMensaje > "" THEN RETURN 'ADM-ERROR'.
