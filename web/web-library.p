...
*/
/*MESSAGE STRING(pcreturn) VIEW-AS ALERT-BOX WARNING.*/
...
/*------------------------------------------------------------------------------
  Purpose:     Devuelve precio de venta, costo de reposición y tipo de cambio desde API
  Parameters:  
        pcArtCode          : Lista de artículos separados por comas
        pcCategoryCustomer : Clasificación del cliente
        pcSalesCondition   : Condición de venta
        pcCustomer         : Código de cliente
        pcOffice           : Oficina
        pcReturn           : Trama de retorno estructurada
        pcError            : Trama de errores recopilados
  Notes:       
        Sintaxis de pcReturn:
        <cCodMat>:<cCodMon>:<fTpoCmb>:<fPreUni>:<fRepositionCostInCurrencySale>:<cContrato>[,...]
------------------------------------------------------------------------------*/
...
/* ------------------------------------------------------------------------- */
/* 1. Validación de Configuración de API                                     */
/* ------------------------------------------------------------------------- */
FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-CodCia
    AND VtaTabla.Tabla = 'CONFIG-WEB-PRICING' NO-LOCK NO-ERROR.
...
x-Url = TRIM(VtaTabla.Llave_c6) + 
        "?ArtCode=" + pcArtCode +
        "&CategoryCustomer=" + pcCategoryCustomer +
        "&SalesCondition=" + pcSalesCondition +
        "&Customer=" + pcCustomer +
        "&Office=" + pcOffice.

/* ------------------------------------------------------------------------- */
/* 2. Consumo del Servicio Web HTTP GET                                      */
/* ------------------------------------------------------------------------- */
DEF VAR pResult   AS CHAR NO-UNDO.
DEF VAR pResponse AS LONGCHAR NO-UNDO.
DEF VAR pContent  AS LONGCHAR CASE-SENSITIVE NO-UNDO.
...
IF INDEX(pResult, '[]') > 0 OR pContent = "" OR pContent = ? THEN DO:
    pcError = "ERROR:" + CHR(10) + CHR(10) +
        "No se obtuvo información de la API para el/los artículo(s): " + pcArtCode.
    RETURN.
END.

/* ------------------------------------------------------------------------- */
/* 3. Lectura del JSON mediante lib/json10.p                                 */
/* ------------------------------------------------------------------------- */
IF NOT VALID-HANDLE(hJson) THEN DO:
    pcError = "ERROR: No se pudo instanciar la librería lib/json10.p".
    RETURN.
END.
...
/* Inicialización del parser JSON */
RUN iniJson IN hJson (INPUT pContent).
raiz = DYNAMIC-FUNCTION("parseVal":U IN hJson, INPUT 0, INPUT "").
arrayId = raiz.
TotalItems = DYNAMIC-FUNCTION("arrayCount":U IN hJson, INPUT arrayId).

IF TotalItems = 0 THEN DO:
    pcError = "ERROR:" + CHR(10) + CHR(10) +
        "La respuesta JSON no contiene elementos válidos para: " + pcArtCode.
    RETURN.
END.

/* Variables para atributos JSON */
...
DEF VAR cRepositionCostInCurrencySale AS CHAR NO-UNDO.

/* Variables de trabajo y conversión */
DEF VAR cCodMat                       AS CHAR NO-UNDO.
DEF VAR fPreUni                       AS DECI DECIMALS 4 NO-UNDO.
...
/* ------------------------------------------------------------------------- */
/* 4. Procesamiento de los Elementos Recibidos                               */
/* ------------------------------------------------------------------------- */
DO iRegistro = 1 TO TotalItems:
    itemId = DYNAMIC-FUNCTION("getArrayItem" IN hJson, INPUT arrayId, INPUT iRegistro).
    IF itemId > 0 THEN DO:
        /* Extracción de valores del ítem */
        ...
        cRepositionCostInCurrencySale = DYNAMIC-FUNCTION("getVal" IN hJson, INPUT itemId, INPUT "RepositionCostInCurrencySale").

        cCodMat = TRIM(cArtCode).
        ...
        ASSIGN fRepositionCostInCurrencySale = DECIMAL(cRepositionCostInCurrencySale) NO-ERROR.

        cCodMon = (IF TRIM(cCurrencySale) = "S/" THEN "1" ELSE "2").

        fTpoCmb = 1.
        ASSIGN fTpoCmb = DECIMAL(cExchangeRate) NO-ERROR.

        /* Determinación de precio por contrato o lista */
        ...
        /* Armado de la trama final pcReturn */
        pcReturn = pcReturn +
...
&IF DEFINED(EXCLUDE-web_api-unit-cost-price_old) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE web_api-unit-cost-price_old Procedure 
PROCEDURE web_api-unit-cost-price_old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
/*------------------------------------------------------------------------------
  ...
  Notes:        
        ...
        pcError : devuelve los artículos con errores
------------------------------------------------------------------------------*/
...
/* Variables para la extracción segura de listas */
DEF VAR cCondicionItem    AS CHAR NO-UNDO.
DEF VAR cClasifItem       AS CHAR NO-UNDO.
DEF VAR cStrCondiciones   AS CHAR NO-UNDO.
DEF VAR cStrClasificacion AS CHAR NO-UNDO.

cStrCondiciones   = STRING(pcListaCondiciones).
cStrClasificacion = STRING(pcListaClasificacion).
...
DO iRegistro = 1 TO TotalItems:
    ...
    IF itemId > 0 THEN DO:
        ...
        /* Extracción segura de la Condición de Venta para evitar Error 560 */
        IF NUM-ENTRIES(cStrCondiciones) >= iRegistro THEN
            cCondicionItem = ENTRY(iRegistro, cStrCondiciones).
        ELSE IF NUM-ENTRIES(cStrCondiciones) > 0 THEN
            cCondicionItem = ENTRY(1, cStrCondiciones).
        ELSE
            cCondicionItem = "".

        /* Extracción segura de la Clasificación para evitar Error 560 */
        IF NUM-ENTRIES(cStrClasificacion) >= iRegistro THEN
            cClasifItem = ENTRY(iRegistro, cStrClasificacion).
        ELSE IF NUM-ENTRIES(cStrClasificacion) > 0 THEN
            cClasifItem = ENTRY(1, cStrClasificacion).
        ELSE
            cClasifItem = "".
...
/*------------------------------------------------------------------------------
  Purpose:     Gestiona por lotes la llamada a web_api_unit_price
  Parameters:  
        pcCodCli : Código de Cliente
        pcCodDiv : División
        pcCodMat : Lista de artículos separados por comas
        pcClfCli : Lista de clasificaciones de cliente
        pcFmaPgo : Lista de condiciones de pago
        pcReturn : Devuelve los artículos válidos
        pcError  : Devuelve los artículos con errores
  Notes:
        Si no existe entrada correspondiente para el artículo iItem en pcClfCli 
        o pcFmaPgo, toma el último valor disponible de la lista respectiva.
------------------------------------------------------------------------------*/
...
/* Validar que existan artículos para procesar */
IF pcCodMat = "" OR pcCodMat = ? THEN RETURN.

DEF VAR iItem               AS INTE NO-UNDO.
DEF VAR cStrCodMat          AS CHAR NO-UNDO.
DEF VAR cStrClfCli          AS CHAR NO-UNDO.
DEF VAR cStrFmaPgo          AS CHAR NO-UNDO.

DEF VAR cClfItem            AS CHAR NO-UNDO.
DEF VAR cFmaItem            AS CHAR NO-UNDO.
...
ASSIGN 
    cStrCodMat = STRING(pcCodMat)
    cStrClfCli = STRING(pcClfCli)
    cStrFmaPgo = STRING(pcFmaPgo).

/* ------------------------------------------------------------------------- */
/* 1. Normalización Dinámica de Listas                                       */
/* Usa el elemento iItem o arrastra el último disponible de la lista          */
/* ------------------------------------------------------------------------- */
DO iItem = 1 TO NUM-ENTRIES(cStrCodMat):
    
    /* Acumulación de Artículos */
    cListaArticulos = cListaArticulos +
        (IF cListaArticulos > '' THEN ',' ELSE '') +
        ENTRY(iItem, cStrCodMat).

    /* Clasificación de Cliente */
    IF NUM-ENTRIES(cStrClfCli) >= iItem THEN
        cClfItem = ENTRY(iItem, cStrClfCli).
    ELSE IF NUM-ENTRIES(cStrClfCli) > 0 THEN
        cClfItem = ENTRY(NUM-ENTRIES(cStrClfCli), cStrClfCli). /* Toma el último valor enviado */
    ELSE
        cClfItem = "C".

    cListaClasificacion = cListaClasificacion +
        (IF cListaClasificacion > '' THEN ',' ELSE '') +
        cClfItem.

    /* Forma de Pago / Condición */
    IF NUM-ENTRIES(cStrFmaPgo) >= iItem THEN
        cFmaItem = ENTRY(iItem, cStrFmaPgo).
    ELSE IF NUM-ENTRIES(cStrFmaPgo) > 0 THEN
        cFmaItem = ENTRY(NUM-ENTRIES(cStrFmaPgo), cStrFmaPgo). /* Toma el último valor enviado */
    ELSE
        cFmaItem = "000".

    cListaCondiciones = cListaCondiciones +
        (IF cListaCondiciones > '' THEN ',' ELSE '') +
        cFmaItem.
END.

/* ------------------------------------------------------------------------- */
/* 2. Envío por Lotes (Batch Processing)                                    */
/* ------------------------------------------------------------------------- */
...
DEF VAR iLimiteEnvio        AS INTE INIT 5 NO-UNDO.
DEF VAR iTotalArticulos     AS INTE NO-UNDO.

iTotalArticulos = NUM-ENTRIES(STRING(cListaArticulos)).
xItem = 0.

DO WHILE xItem < iTotalArticulos:
    ASSIGN
        xListaArticulos     = ""
        xListaClasificacion = ""
        xListaCondiciones   = "".
    ...
    REPEAT:
        ...
        xListaArticulos = xListaArticulos +
            (IF xListaArticulos > '' THEN ',' ELSE '') +
            ENTRY(xItem, STRING(cListaArticulos)).

        xListaClasificacion = xListaClasificacion +
            (IF xListaClasificacion > '' THEN ',' ELSE '') +
            ENTRY(xItem, STRING(cListaClasificacion)).

        xListaCondiciones = xListaCondiciones +
            (IF xListaCondiciones > '' THEN ',' ELSE '') +
            ENTRY(xItem, STRING(cListaCondiciones)).

        IF xItem >= iTotalArticulos THEN LEAVE.
    ...
    RUN web_api_unit_price (INPUT xListaArticulos,
                            ...
                            INPUT pcCodDiv,
                            OUTPUT xReturn,
                            OUTPUT xError) NO-ERROR.

    /* Acumulación de errores y respuestas */
    IF xError > '' AND xError <> ? THEN
        pcError = pcError + (IF pcError > '' THEN CHR(10) ELSE '') + xError.

    IF xReturn > '' AND xReturn <> ? THEN
        pcReturn = pcReturn + (IF pcReturn > '' THEN ',' ELSE '') + xReturn.

    /* Abortar si ocurre un error sin retorno */
    IF (xError > '' AND xError <> ?) AND (xReturn = '' OR xReturn = ?) THEN RETURN.
END.
...
&IF DEFINED(EXCLUDE-web_api_unit_price_old) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE web_api_unit_price_old Procedure 
PROCEDURE web_api_unit_price_old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
