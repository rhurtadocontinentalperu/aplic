...
/*         pMessage = "Error en conexión de la URL". */
/*         RETURN 'ADM-ERROR'.                       */
/*     END.                                          */
/* END.                                              */
...
IF INDEX(x-Texto, '<error>') > 0 THEN DO:
    pMessage = "ERROR: " + x-Texto + CHR(10) + "Comunicarse con el Jefe de Línea".
    RETURN "ADM-ERROR".
    /* Solo regresamos sin ningún error, no está registrado el precio en la lista de precios */
    /*RETURN 'OK'.*/
END.
...
ELSE DO:
    ...
    IF ERROR-STATUS:ERROR = NO THEN DO:
        IF pCostoReposicion <= pPrecioDescontado THEN DO:
            pMessage = "Precio Unitario Peldaño es menor al Costo de Reposición".
            RETURN "ADM-ERROR".
        END.
    END.
END.
