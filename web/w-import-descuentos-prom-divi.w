...
&Scoped-Define ENABLED-OBJECTS RADIO-SET-Reemplazar BtnDone BUTTON-Capturar 
...
DEFINE VARIABLE h_b-import-descuentos-prom-divi AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BtnDone AT ROW 1.27 COL 124 WIDGET-ID 6
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 130.43 BY 25.69 WIDGET-ID 100.
...
DO:
    ...
    RUN dispatch IN h_b-import-descuentos-prom-divi ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'web/b-import-descuentos-prom-divi.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-import-descuentos-prom-divi ).
       RUN set-position IN h_b-import-descuentos-prom-divi ( 8.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-import-descuentos-prom-divi ( 18.31 , 116.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-div-expo ,
             RADIO-SET-Reemplazar:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-import-descuentos-prom-divi ,
...
/* ************************************************************************************* */
FOR EACH T-MATG:
    IF T-MATG.FchPrmD = ? OR T-MATG.FchPrmH = ?
        OR (T-MATG.Libre_d01 + T-MATG.Libre_d02 + T-MATG.Libre_d03) = 0 
        THEN DO:
        DELETE T-MATG.
        NEXT.
    END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RADIO-SET-Reemplazar BtnDone BUTTON-Capturar 
      WITH FRAME F-Main IN WINDOW W-Win.
...
SESSION:SET-WAIT-STATE('GENERAL').
RLOOP:
FOR EACH T-MATG EXCLUSIVE-LOCK, FIRST Almmmatg OF T-MATG NO-LOCK:
    FIND FIRST VtaListaMay WHERE VtaListaMay.CodCia = s-CodCia AND
        VtaListaMay.codmat = T-MATG.CodMat AND
        VtaListaMay.CodDiv = s-coddiv NO-LOCK NO-ERROR.
    IF NOT AVAILABLE VtaListaMay THEN DO:
        CREATE VtaListaMay.
        BUFFER-COPY Almmmatg EXCEPT Almmmatg.PromFchH Almmmatg.PromFchD Almmmatg.PromDto
            TO VtaListaMay 
            ASSIGN 
    ...
    RUN PRI_Graba-Dscto-Prom-Divi IN hProc (INPUT RADIO-SET-Reemplazar,
                                            INPUT s-CodDiv,
                                            INPUT T-MATG.CodMat,
                                            INPUT (IF AVAILABLE VtaListaMay THEN VtaListaMay.PreOfi ELSE 0),
...
cNombreLista = s-coddiv + " - DESCUENTOS PROMOCIONALES".
