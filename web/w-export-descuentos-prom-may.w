...
DEFINE VAR tTotal AS INTE NO-UNDO.  /* Total de SKUs */
...
FILL-IN-Mensaje FILL-IN-Avance 
...
DEFINE VARIABLE FILL-IN-Avance AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
...
DEFINE FRAME F-Main
     ...
     FILL-IN-Mensaje AT ROW 19.58 COL 2 NO-LABEL WIDGET-ID 44
     FILL-IN-Avance AT ROW 19.58 COL 81 COLON-ALIGNED NO-LABEL WIDGET-ID 46
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 107.43 BY 19.65
...
/* SETTINGS FOR FILL-IN FILL-IN-Avance IN FRAME F-Main
   NO-ENABLE                                                            */
...
DO:
    MESSAGE 'Continuamos con la generación de la plantilla?'
    ...
    RUN Carga-Catalogo.
    MESSAGE 'Se van a procesar un total de' tTotal 'artículos' SKIP
    ...
    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "PROCESO TERMINADO".
    FILL-IN-Avance:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Catalogo W-Win 
PROCEDURE Carga-Catalogo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
tTotal = 0.
/* Cargamos el temporal con los artículos válidos */
EMPTY TEMP-TABLE T-MATG.
CASE TRUE:
    WHEN x-CodFam > "" THEN DO:
        FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
            AND Almmmatg.codfam = x-CodFam 
            AND Almmmatg.tpoart <> "D":
            IF NOT (x-SubFam = 'Todas' OR Almmmatg.subfam = x-subfam) THEN NEXT.
            IF NOT (x-CodPro = '' OR Almmmatg.codpr1 = x-codpro) THEN NEXT.
            IF NOT (x-DesMar = '' OR Almmmatg.desmar BEGINS x-desmar) THEN NEXT.
            tTotal = tTotal + 1.
            CREATE T-MATG.
            BUFFER-COPY Almmmatg TO T-MATG.
            FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} =
                "CARGANDO: " + Almmmatg.codmat + " " + Almmmatg.desmat.
        END.
    END.
    WHEN x-CodPro > "" THEN DO:
        FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
            AND Almmmatg.codpr1 = x-CodPro
            AND Almmmatg.tpoart <> "D":
            IF NOT (x-DesMar = '' OR Almmmatg.desmar BEGINS x-desmar) THEN NEXT.
            tTotal = tTotal + 1.
            CREATE T-MATG.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAMETER f-Divisiones AS CHAR.
...
FOR EACH T-MATG NO-LOCK, FIRST Almmmatg OF T-MATG NO-LOCK:
    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "DESCUENTOS: " + Almmmatg.codmat + " " + Almmmatg.desmat.
    CASE RADIO-SET-1:
        WHEN 1 THEN DO:     /* CONTINENTAL */
            DO k = 1 TO NUM-ENTRIES(f-Divisiones):      
                FOR EACH VtaDctoProm NO-LOCK WHERE VtaDctoProm.CodCia = s-CodCia AND
                    ...
                    (VtaDctoProm.FchIni <= TODAY AND TODAY <= VtaDctoProm.FchFin) AND
                    VtaDctoProm.FlgEst = "A"
                    BREAK BY VtaDctoProm.Descuento DESC:
                    IF VtaDctoProm.FchIni = ? OR VtaDctoProm.FchFin = ? THEN NEXT.
        ...
        WHEN 2 THEN DO:     /* UTILEX */
            DO k = 1 TO NUM-ENTRIES(f-Divisiones):      
                FOR EACH VtaDctoPromMin NO-LOCK WHERE VtaDctoPromMin.CodCia = s-CodCia AND
                    ...
                    (VtaDctoPromMin.FchIni <= TODAY AND TODAY <= VtaDctoPromMin.FchFin) AND
                    VtaDctoPromMin.FlgEst = "A"
                    BREAK BY VtaDctoPromMin.Descuento DESC:
                    IF VtaDctoPromMin.FchIni = ? OR VtaDctoPromMin.FchFin = ? THEN NEXT.
...
FOR EACH T-MATG NO-LOCK, FIRST Almmmatg OF T-MATG NO-LOCK:
    FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} =
        "CARGA FINAL: " + Almmmatg.codmat + " " + Almmmatg.desmat.
    DO k = 1 TO NUM-ENTRIES(f-Divisiones):      
        IF NOT CAN-FIND(FIRST t-VtaDctoProm WHERE t-VtaDctoProm.CodCia = Almmmatg.CodCia
                        AND t-VtaDctoProm.CodMat = Almmmatg.CodMat 
        ...
        IF NOT CAN-FIND(FIRST t-VtaDctoPromMin WHERE t-VtaDctoPromMin.CodCia = Almmmatg.CodCia
                        AND t-VtaDctoPromMin.CodMat = Almmmatg.CodMat 
...
------------------------------------------------------------------------------*/
    ...
    DEF INPUT PARAMETER f-Divisiones AS CHAR.
    DEF INPUT-OUTPUT PARAMETER t-Column AS INTE.
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-1 COMBO-BOX-CodFam COMBO-BOX-SubFam x-CodPro FILL-IN-NomPro 
          RADIO-SET-Vigente x-DesMar FILL-IN-Mensaje FILL-IN-Avance 
...
/* Cargamos Temporales con los descuentos */
/* ********************************************************************************************* */
RUN Carga-Temporal (INPUT f-Divisiones).
/* ********************************************************************************************* */
/* ********************************************************************************************* */
...
FOR EACH T-MATG NO-LOCK, FIRST Almmmatg OF T-MATG NO-LOCK:
    iCount = iCount + 1.
    IF iCount MODULO 10 = 0 THEN DO:
        FILL-IN-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "EXCEL: " + Almmmatg.codmat + " " + Almmmatg.desmat.
        FILL-IN-Avance:SCREEN-VALUE = 'AVANCE: ' + STRING(iCount / tTotal * 100, '>>9.99') + " %".
    END.

    /* RHC 12/08/2020 puede haber mas un registro por artículo */
    /* Hacemos un barrido de los rangos de fechas posibles */
    RUN Carga-Temporal-2 (INPUT f-Divisiones, INPUT-OUTPUT t-Column).
    /* ******************************************************************************************** */
            ...
            WHEN 2 THEN DO:     /* UTILEX MINORISTA */
                DO k = 1 TO NUM-ENTRIES(f-Divisiones):
                    ...
                    FIND FIRST t-VtaDctoPromMin WHERE t-VtaDctoPromMin.CodCia = s-CodCia AND
                        t-VtaDctoPromMin.FlgEst = "A" AND
                        t-VtaDctoPromMin.CodDiv = ENTRY(k, f-Divisiones) AND
                        t-VtaDctoPromMin.FchIni = T-DPROM.FchIni AND
                        t-VtaDctoPromMin.FChFin = T-DPROM.FchFin 
                        NO-LOCK NO-ERROR.
            ...
            OTHERWISE DO:
                DO k = 1 TO NUM-ENTRIES(f-Divisiones):
                    ...
                    FIND FIRST t-VtaDctoProm WHERE t-VtaDctoProm.CodCia = s-CodCia AND
                        t-VtaDctoProm.FlgEst = "A" AND
                        t-VtaDctoProm.CodMat = Almmmatg.codmat AND
                        t-VtaDctoProm.CodDiv = ENTRY(k, f-Divisiones) AND
                        t-VtaDctoProm.FchIni = T-DPROM.FchIni AND
                        t-VtaDctoProm.FChFin = T-DPROM.FchFin 
                        NO-LOCK NO-ERROR.
