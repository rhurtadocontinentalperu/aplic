...
&IF DEFINED(EXCLUDE-COT_Cabecera_por_PET) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE COT_Cabecera_por_PET Procedure 
PROCEDURE COT_Cabecera_por_PET PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEF PARAMETER BUFFER bVtaCDocu FOR Vtacdocu.
DEF INPUT PARAMETER pCodDiv AS CHAR NO-UNDO.
DEF INPUT PARAMETER pCodAlm AS CHAR NO-UNDO.
DEF INPUT PARAMETER s-DiasVtoCot AS INTE NO-UNDO.
DEF INPUT PARAMETER s-MinimoDiasDespacho AS INTE NO-UNDO.
DEF OUTPUT PARAMETER pNroPed AS CHAR NO-UNDO.
DEF OUTPUT PARAMETER cRowid AS ROWID NO-UNDO.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    CREATE Faccpedi.
    IF AVAILABLE bVtaCDocu THEN DO:
        BUFFER-COPY bVtaCDocu 
            EXCEPT bVtaCDocu.Libre_C02 bVtaCDocu.Libre_C03 bVtaCDocu.Libre_c05
            TO Faccpedi
            ASSIGN 
            FacCPedi.CodDoc = s-coddoc 
            FacCPedi.CodDiv = s-CodDiv  /* Ic */
            FacCPedi.NroPed = x-serie-numero
            FacCPedi.Atencion = bVtaCDocu.DniCli
            FacCPedi.Libre_C02 = bVtaCDocu.Libre_C03    /* Última atención */
            NO-ERROR.
        ...
        /* 02/03/2024: C.Iman */
        ASSIGN
            FacCPedi.CodOrigen = bVtaCDocu.CodOri.
        /* 04/05/2024: C. Tenazoa */
        ASSIGN
            FacCPedi.NroOrigen = bVtaCDocu.NroOri        /* EXPO, B2C, etc */
            FacCPedi.Cliente_Recoge = bVtaCDocu.TpoLic.  /* Ic - 21May2024 */
        /* **************************************************************************** */
        /* 30/10/2025: Control de atenciones Expolibreria:
            1.- Si tiene una atención anterior => cerramos automáticamente 
            2.- Registramos la nueva atención
            */
        FOR EACH even_control_turno EXCLUSIVE-LOCK WHERE even_control_turno.codcia = s-codcia
            AND even_control_turno.coddiv = s-coddiv 
            AND even_control_turno.BLOCK = bVtaCDocu.codsed
            AND even_control_turno.estado = 40      /* En Atención */
            AND even_control_turno.fecha = TODAY:
            even_control_turno.estado = 50.
        END.
        RELEASE even_control_turno.
        RUN vtaexp/p-even_control_estado (INPUT "WRITE",
                                          INPUT s-coddiv,
                                          INPUT bVtaCDocu.codsed,
                                          INPUT bVtaCDocu.codcli,
                                          INPUT 40,       /* En Atención */
                                          OUTPUT pMensaje).
        /* **************************************************************************** */
    END.
    ELSE DO:
    ...
    /* ************************************** */
    /* 15/7/2024: Control de Almacén Despacho */
    /* ************************************** */
    ASSIGN
    ...
    RUN COT_Recalcula_Detalle_por_PET (BUFFER Faccpedi,
                                       INPUT pCodDiv, 
                                       INPUT pCodAlm,
                                       OUTPUT pMensaje).
    ...
    IF AVAILABLE bVtaCDocu THEN
        ASSIGN
            FacCPedi.CodRef = bVtaCDocu.CodPed
            FacCPedi.NroRef = bVtaCDocu.NroPed
            bVtaCDocu.CodRef = FacCPedi.CodDoc
            bVtaCDocu.NroRef = FacCPedi.NroPed
            bVtaCDocu.FlgSit = "T".
...
&IF DEFINED(EXCLUDE-COT_Carga_Detalle_por_PET) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE COT_Carga_Detalle_por_PET Procedure 
PROCEDURE COT_Carga_Detalle_por_PET PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    ...
    /* **************** */
    /* ************************************************ */
    /* GRABAMOS CABECERA Y DETALLE DEL PEDIDO COMERCIAL */
    RUN COT_Cabecera_por_PET (BUFFER Vtacdocu,
                              INPUT pCodDiv,
                              INPUT pCodAlm,
                              INPUT s-DiasVtoCot,
                              INPUT s-MinimoDiasDespacho,
                              OUTPUT pNroPed,
                              OUTPUT cRowid,
                              OUTPUT pMensaje).
    IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
...
&IF DEFINED(EXCLUDE-COT_Genera-COT-por-PET-DCTOS) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE COT_Genera-COT-por-PET-DCTOS Procedure 
PROCEDURE COT_Genera-COT-por-PET-DCTOS PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&IF DEFINED(EXCLUDE-COT_Recalcula_Detalle_por_PET) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE COT_Recalcula_Detalle_por_PET Procedure 
PROCEDURE COT_Recalcula_Detalle_por_PET PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH ITEM NO-LOCK, FIRST Almmmatg OF ITEM NO-LOCK:
        ...
        /* 15/7/24: Almacén Despacho */
        ASSIGN
            FacDPedi.AlmDes = ENTRY(1, pCodAlm).
    END.
