...
DEFINE FRAME F-Main
     FacCPedi.NroPed AT ROW 1.15 COL 15 COLON-ALIGNED WIDGET-ID 58
          LABEL "Número" FORMAT "XXX-XXXXXX"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     F-Estado AT ROW 1.15 COL 33 COLON-ALIGNED WIDGET-ID 114
     FacCPedi.Libre_c01 AT ROW 1.15 COL 83 COLON-ALIGNED WIDGET-ID 126
     ...
     FacCPedi.FchPed AT ROW 1.27 COL 109.14 COLON-ALIGNED WIDGET-ID 46
          ...
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     FacCPedi.CodCli AT ROW 1.96 COL 15 COLON-ALIGNED WIDGET-ID 38
          LABEL "Cliente"
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
          BGCOLOR 11 FGCOLOR 0 
     FacCPedi.FaxCli AT ROW 1.96 COL 28 COLON-ALIGNED NO-LABEL WIDGET-ID 124 FORMAT "X(10)"
          VIEW-AS FILL-IN 
          SIZE 9 BY .81
          FONT 6
     BUTTON-Turno-Retrocede AT ROW 1.96 COL 46 WIDGET-ID 34
     BUTTON-Turno-Avanza AT ROW 1.96 COL 51 WIDGET-ID 32
     FILL-IN-1 AT ROW 1.96 COL 54 COLON-ALIGNED NO-LABEL WIDGET-ID 36
     FacCPedi.Libre_c04 AT ROW 1.96 COL 76 NO-LABEL WIDGET-ID 144
...
"No", ""
     ...
     FILL-IN-marco AT ROW 2 COL 62.57 COLON-ALIGNED NO-LABEL WIDGET-ID 156
     FacCPedi.usuario AT ROW 2.08 COL 109 COLON-ALIGNED WIDGET-ID 66
          LABEL "Digitado por"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     FacCPedi.RucCli AT ROW 2.81 COL 15 COLON-ALIGNED WIDGET-ID 60
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
     FacCPedi.Atencion AT ROW 2.81 COL 33 COLON-ALIGNED WIDGET-ID 88
          LABEL "DNI" FORMAT "X(8)"
          VIEW-AS FILL-IN 
          SIZE 11 BY .81
          BGCOLOR 11 FGCOLOR 0 
     FacCPedi.e-mail AT ROW 2.85 COL 47.43 WIDGET-ID 184
...
DEFINE FRAME F-Main
     ...
     "Cliente Recoge?" VIEW-AS TEXT
          ...
          FGCOLOR 8 FONT 6
     RECT-27 AT ROW 1 COL 1.29 WIDGET-ID 180
...
------------------------------------------------------------------------------*/
  ...
  */
  FOR EACH logdsctosped NO-LOCK WHERE logdsctosped.CodCia = Faccpedi.codcia AND
      ...
      FIND FIRST b-logdsctosped WHERE ROWID(logdsctosped) = ROWID(b-logdsctosped) EXCLUSIVE-LOCK NO-ERROR.
      IF AVAILABLE b-logdsctosped THEN DO:
...
/*   DELETE PROCEDURE hProc.                                                                 */
  ...
  /* ****************************************************************************************** */
  /* 06/01/2023: Rutina de promociones actualizada */
  /*vta2/promocion-generalv2.p*/
  /* ****************************************************************************************** */
/*   EMPTY TEMP-TABLE BONIFICACION.                               */
/*   FOR EACH ITEM:                                               */
/*       CREATE BONIFICACION.                                     */
/*       BUFFER-COPY ITEM TO BONIFICACION.                        */
/*   END.                                                         */
/*   RUN {&Promocion} (INPUT Faccpedi.Libre_c01,                  */
/*                     INPUT Faccpedi.CodCli,                     */
/*                     INPUT-OUTPUT TABLE BONIFICACION,           */
/*                     OUTPUT pMensaje).                          */
/*   IF RETURN-VALUE = "ADM-ERROR" THEN UNDO, RETURN "ADM-ERROR". */
/*   FOR EACH BONIFICACION WHERE BONIFICACION.Libre_c05 <> "OF":  */
/*       DELETE BONIFICACION.                                     */
/*   END.                                                         */
  /* ****************************************************************************************** */
  /* ****************************************************************************************** */
...
/* ***************************** */
    ...
    {web/recalcular-cotizacion-unificada.i}
