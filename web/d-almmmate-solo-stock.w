...
/* OJO */
IF TRUE <> (input-var-2 > '') THEN input-var-2 = s-CodDiv.
...
"Almacen.Descripcion" ? "X(80)" "character" ? ? ? ? ? ? no ? no no "81.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"T-MATE.StkAct" "Stock Actual" ? "decimal" 11 ? ? ? ? ? no ? no no "18.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-5 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* STOCK ACTUAL */
DO:  
...
FOR EACH VtaAlmDiv NO-LOCK WHERE VtaAlmDiv.CodCia = s-codcia
        AND VtaAlmDiv.CodDiv = input-var-2,     /*s-coddiv,*/
