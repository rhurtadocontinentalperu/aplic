...
IF ERROR-STATUS:ERROR THEN DO:
    PUT UNFORMATTED TRIM(ERROR-STATUS:GET-MESSAGE(1)) SKIP.
    RETURN.
END.
...
FOR EACH FacCVtaOnLine EXCLUSIVE-LOCK WHERE FacCVtaOnLine.codcia = s-codcia
    ...
    IF ERROR-STATUS:ERROR = YES THEN DO:
        PUT UNFORMATTED FacCVtaOnLine.CodDoc + " " + FacCVtaOnLine.NroPed SKIP "ERROR: " TRIM(ERROR-STATUS:GET-MESSAGE(1)) SKIP.
        UNDO, NEXT.
    END.
    ...
    IF ERROR-STATUS:ERROR = YES THEN DO:
        PUT UNFORMATTED "Documento " + FacCVtaOnLine.CodDoc + " " + FacCVtaOnLine.NroPed SKIP "ERROR: " + TRIM(ERROR-STATUS:GET-MESSAGE(1)) SKIP.
        NEXT.
    END.
    /* ****************************************************************************************** */
...
/*     s-CodTer = CjaCVtaOnLine.CodCaja.     /* OJO */                                               */
/*                                                                                                   */
/*     /* Control del documento Ingreso Caja por terminal */                                         */
/*     FIND FIRST ccbdterm WHERE CcbDTerm.CodCia = s-codcia AND                                      */
/*         CcbDTerm.CodDiv = s-coddiv AND                                                            */
...
/*     IF NOT AVAILABLE ccbdterm THEN DO:                                                            */
/*         pMensaje = "No está configurado el terminal " + s-codter + " en la división " + s-coddiv. */
/*         UNDO, LEAVE.                                                                              */
/*     END.                                                                                          */
/*     s-sercja = ccbdterm.nroser.                                                                   */
    ...
    ASSIGN
        ...
        CcbCCaja.Voucher[4] = CjaCVtaOnLine.Voucher
        .
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':    
    ...
    CREATE FacCPedi.
    BUFFER-COPY FacCVtaOnLine TO FacCPedi
        ASSIGN 
