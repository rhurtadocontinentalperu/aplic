...
&SCOPED-DEFINE b-Condicion (~
                    b-gn-clie.CodCia = cl-codcia ~
                    AND b-gn-clie.FlgSit = "A")
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      gn-clie.CodCli FORMAT "x(11)":U WIDTH 9.43
...
"gn-clie.CodCli" ? ? "character" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    FIND NEXT b-gn-clie USE-INDEX Idx01 WHERE {&b-Condicion} AND
...
DO:
    ...
    SESSION:SET-WAIT-STATE('GENERAL').
    FIND PREV b-gn-clie USE-INDEX Idx01 WHERE {&b-Condicion} AND
        INDEX(b-gn-clie.NomCli, FILL-IN-NomCli) > 0 NO-LOCK NO-ERROR.
    IF AVAILABLE b-gn-clie THEN DO:
        REPOSITION {&BROWSE-NAME} TO ROWID ROWID(b-gn-clie) NO-ERROR.
    END.
...
DO:
    ...
    SESSION:SET-WAIT-STATE('GENERAL').
    FIND FIRST b-gn-clie USE-INDEX Idx01 WHERE {&b-Condicion} AND
        INDEX(b-gn-clie.NomCli, FILL-IN-NomCli) > 0 NO-LOCK NO-ERROR.
    IF AVAILABLE b-gn-clie THEN DO:
        REPOSITION {&BROWSE-NAME} TO ROWID ROWID(b-gn-clie) NO-ERROR.
        /*RUN dispatch IN THIS-PROCEDURE ('row-changed':U).*/
...
IF AVAILABLE gn-clie THEN pCodCli = gn-clie.codcli.
