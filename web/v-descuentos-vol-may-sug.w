...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE Almmmatg THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      RUN web_api_unit_price IN hProc (INPUT Almmmatg.codmat,
                                       INPUT (IF Almmmatg.CHR__02 = "P" THEN gn-clie.clfCli ELSE gn-clie.ClfCli2),
                                       INPUT s-FmaPgo,
                                       INPUT s-CodCli,
                                       INPUT pCodDiv,
                                       OUTPUT pcReturn,
...
*/
      ...
      END.
      ELSE DO:
          pMonVta = INTEGER(ENTRY(3,pcReturn,':')) NO-ERROR.
          pTpoCmb = DECIMAL(ENTRY(4,pcReturn,':')) NO-ERROR.
          pPrecioDescontado = DECIMAL(ENTRY(2,pcReturn,':')) NO-ERROR.
      END.
