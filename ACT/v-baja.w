...
&Scoped-Define ENABLED-FIELDS ac-cpape.Motivo ac-cpape.Responsable_Origen ~
...
ac-cpape.FchDoc ac-cpape.Motivo ac-cpape.Usuario ~
ac-cpape.Responsable_Origen ac-cpape.Observaciones 
&Scoped-define DISPLAYED-TABLES ac-cpape
&Scoped-define FIRST-DISPLAYED-TABLE ac-cpape
&Scoped-Define DISPLAYED-OBJECTS x-Estado x-Motivo x-NomPer 
...
DEFINE VARIABLE x-Motivo AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
...
DEFINE FRAME F-Main
     ...
     ac-cpape.FchDoc AT ROW 1.27 COL 87 COLON-ALIGNED WIDGET-ID 4
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     ac-cpape.Motivo AT ROW 2.08 COL 11 COLON-ALIGNED WIDGET-ID 26
          VIEW-AS FILL-IN 
          SIZE 4 BY .81
     x-Motivo AT ROW 2.08 COL 20 COLON-ALIGNED NO-LABEL WIDGET-ID 28
...
/* SETTINGS FOR FILL-IN x-Estado IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN x-Motivo IN FRAME F-Main
   NO-ENABLE                                                            */
...
&Scoped-define SELF-NAME ac-cpape.Motivo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ac-cpape.Motivo V-table-Win
ON LEAVE OF ac-cpape.Motivo IN FRAME F-Main /* Motivo */
DO:
    x-Motivo:SCREEN-VALUE = ''.
    FIND ac-tabl WHERE ac-tabl.codcia = s-codcia
        AND ac-tabl.ctabla = 'MB'
        AND ac-tabl.ccodigo = ac-cpape.Motivo:SCREEN-VALUE
        NO-LOCK NO-ERROR.
    IF AVAILABLE ac-tabl THEN x-Motivo:SCREEN-VALUE = AC-TABL.cDescri.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ac-cpape.Motivo V-table-Win
ON LEFT-MOUSE-DBLCLICK OF ac-cpape.Motivo IN FRAME F-Main /* Motivo */
OR F8 OF ac-cpape.Motivo
DO:
  ASSIGN
      input-var-1 = ''
      input-var-2 = ''
      input-var-3 = ''
      output-var-1 = ?.
  RUN act/c-motivo ('Motivo de Baja').
  IF output-var-1 <> ? THEN SELF:SCREEN-VALUE = output-var-2.
  APPLY 'entry':U TO ac-cpape.Motivo.
END.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
      ...
      ASSIGN
          ...
          ac-cpape.LLave_Motivo = 'MB'
          ac-cpape.CCosto_Destino = s-Ccosto_Origen.
      ASSIGN
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      FIND ac-tabl WHERE ac-tabl.codcia = s-codcia
          AND ac-tabl.ctabla = 'MB'
          AND ac-tabl.ccodigo = ac-cpape.Motivo
          NO-LOCK NO-ERROR.
      IF AVAILABLE ac-tabl THEN x-Motivo:SCREEN-VALUE = AC-TABL.cDescri.
      x-NomPer:SCREEN-VALUE = ''.
      FIND pl-pers WHERE pl-pers.codcia = s-codcia
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    IF NOT AVAILABLE pl-pers THEN DO:
        MESSAGE 'Responsable_Origen (código del personal) NO registrado'
            VIEW-AS ALERT-BOX ERROR.
        RETURN 'ADM-ERROR'.
    END.
END.
