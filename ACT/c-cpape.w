...
&SCOPED-DEFINE l-lookup h_l-cpape
...
DEFINE VARIABLE h_l-cpape AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 5.04 COL 75
     Btn_Cancel AT ROW 7.15 COL 75
     SPACE(4.85) SKIP(3.42)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/act/l-cpape.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Codigo':U ,
             OUTPUT h_l-cpape ).
       RUN set-position IN h_l-cpape ( 1.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_l-cpape ( 10.92 , 72.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.19 , 74.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-cpape. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-cpape ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-cpape ,
