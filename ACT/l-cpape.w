...
&SCOPED-DEFINE CONDICION ac-cpape.codcia = s-codcia  AND ~
ac-cpape.coddiv = input-var-1 AND ac-cpape.tipmov = input-var-2 AND ~
ac-cpape.nroser = INTEGER(input-var-3)

&SCOPED-DEFINE CODIGO ac-cpape.nrodoc

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( ac-cpape.observaciones BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( ac-cpape.observaciones , FILL-IN-filtro ) <> 0 )
...
&Scoped-define FIELDS-IN-QUERY-br_table ac-cpape.FchDoc ac-cpape.NroDoc ~
ac-cpape.Observaciones 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH ac-cpape WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH ac-cpape WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table ac-cpape
&Scoped-define FIRST-TABLE-IN-QUERY-br_table ac-cpape
...
<FOREIGN-KEYS>
Nombres que inicien con|y||INTEGRAL.ac-cpape.Observaciones
Nombres que contengan|y||INTEGRAL.ac-cpape.Observaciones
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Codigo|y||INTEGRAL.ac-cpape.NroDoc|yes
Descripción|||INTEGRAL.ac-cpape.Observaciones|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "':U + 'Codigo,Descripción' + '",
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ac-cpape.FchDoc FORMAT "99/99/9999":U
      ac-cpape.NroDoc FORMAT "999999":U
      ac-cpape.Observaciones FORMAT "x(60)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 70 BY 8.62
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "{&CONDICION}"
     _FldNameList[1]   = INTEGRAL.ac-cpape.FchDoc
     _FldNameList[2]   = INTEGRAL.ac-cpape.NroDoc
     _FldNameList[3]   > INTEGRAL.ac-cpape.Observaciones
"ac-cpape.Observaciones" ? "x(60)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE
            ...
            ac-cpape.nrodoc = FILL-IN-codigo
            NO-LOCK NO-ERROR.
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Codigo':U THEN DO:
           &Scope SORTBY-PHRASE BY ac-cpape.NroDoc
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY ac-cpape.Observaciones
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.Observaciones.
