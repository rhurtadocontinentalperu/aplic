...
DEFINE VARIABLE C-NRODOC AS INT NO-UNDO.
DEFINE VARIABLE C-NROSER AS INT NO-UNDO.
DEFINE VARIABLE C-CODALM AS CHAR NO-UNDO.
DEFINE SHARED VARIABLE S-CODCIA  AS INTEGER.
DEFINE SHARED VARIABLE S-CODALM  AS INTEGER.
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH PR-RQPROC WHERE ~{&KEY-PHRASE} ~
 ...
 AND PR-RQPROC.CodAlm = C-CODALM ~
 AND PR-RQPROC.NroSer = C-NROSER ~
 AND PR-RQPROC.NroDoc = C-NRODOC NO-LOCK ~
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "INTEGRAL.PR-RQPROC.CodCia = S-CODCIA
 AND INTEGRAL.PR-RQPROC.CodAlm = C-CODALM
 AND INTEGRAL.PR-RQPROC.NroSer = C-NROSER
 AND INTEGRAL.PR-RQPROC.NroDoc = C-NRODOC"
     _Design-Parent    is WINDOW q-tables @ ( 1.08 , 2.14 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Recibe-Parametros q-tables 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER C-ALM AS CHAR.
DEFINE INPUT PARAMETER C-SER AS INTEGER.
DEFINE INPUT PARAMETER C-DOC AS INTEGER.
