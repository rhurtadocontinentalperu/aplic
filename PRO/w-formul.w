...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 RECT-23 R-Tipo 
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 1 COL 76 WIDGET-ID 2
...
ON CHOOSE OF BUTTON-1 IN FRAME F-Main /* Copiar a Cissac */
DO:
   RUN Copiar-a-Cissac IN h_v-formul.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 4.85 , 10.29 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv10 ).
       RUN set-position IN h_p-updv10 ( 4.88 , 23.57 ) NO-ERROR.
       RUN set-size IN h_p-updv10 ( 1.42 , 39.86 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'pro/q-formul.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-formul ).
       RUN set-position IN h_q-formul ( 4.96 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 9.14 ) */

       /* Links to SmartViewer h_v-formul. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-formul ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'pro/b-formul.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-formul ).
       RUN set-position IN h_b-formul ( 6.38 , 1.57 ) NO-ERROR.
       RUN set-size IN h_b-formul ( 5.92 , 67.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 6.92 , 70.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 5.19 , 8.86 ) NO-ERROR.

       /* Links to SmartBrowser h_b-formul. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-formul ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'pro/b-formulm.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-formulm ).
       RUN set-position IN h_b-formulm ( 6.38 , 1.29 ) NO-ERROR.
       RUN set-size IN h_b-formulm ( 5.92 , 67.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv12-3 ).
       RUN set-position IN h_p-updv12-3 ( 7.19 , 69.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12-3 ( 5.19 , 9.86 ) NO-ERROR.

       /* Links to SmartBrowser h_b-formulm. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12-3 , 'TableIO':U , h_b-formulm ).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-1 RECT-23 R-Tipo 
      WITH FRAME F-Main IN WINDOW W-Win.
