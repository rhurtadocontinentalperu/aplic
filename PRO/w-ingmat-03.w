...
DEFINE VARIABLE h_v-ingmat-03 AS HANDLE NO-UNDO.
...
DO:
  RUN Asigna-Documento IN h_v-ingmat-03.
END.
...
DO:
  RUN dispatch IN h_v-ingmat-03 ('imprime':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'pro/v-ingmat-03.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ingmat-03 ).
       RUN set-position IN h_v-ingmat-03 ( 2.35 , 1.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-ingmat-03. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-ingmat-03 ).
       RUN add-link IN adm-broker-hdl ( h_q-alctrf , 'Record':U , h_v-ingmat-03 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ingmat-03 ,
             h_v-movmto , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico-2 ,
             h_v-ingmat-03 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-salida ).
       RUN set-position IN h_b-salida ( 7.15 , 1.00 ) NO-ERROR.
