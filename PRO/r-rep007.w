...
DEFINE TEMP-TABLE Reporte
    ...
    FIELDS CodPer   LIKE PR-LIQD2.CodPer
    FIELDS Name     AS CHARACTER FORMAT "X(50)"
    FIELDS CanHrs   LIKE PR-LIQD2.Horas
    FIELDS CtoHor   LIKE PR-LIQC.CtoHor
    FIELDS HorVal   LIKE PR-LIQD2.HorVal.
...
DEFINE FRAME F-Main
     FILL-IN-fecha AT ROW 1.96 COL 18 COLON-ALIGNED
     FILL-IN-fecha-2 AT ROW 2.92 COL 18 COLON-ALIGNED
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Costo trabajador por Orden de Trabajo"
         HEIGHT             = 6.12
         WIDTH              = 53.72
         MAX-HEIGHT         = 6.12
         MAX-WIDTH          = 53.72
...
ON END-ERROR OF W-Win /* Costo trabajador por Orden de Trabajo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Costo trabajador por Orden de Trabajo */
DO:
...
------------------------------------------------------------------------------*/
    ...
    DEF VAR Name AS CHARACTER FORMAT "X(50)".
    ...
    FOR EACH PR-LIQC NO-LOCK
            WHERE PR-LIQC.CodCia = s-codcia
            AND PR-LIQC.FchLiq >= FILL-IN-fecha
            AND PR-LIQC.FchLiq <= FILL-IN-fecha-2
            AND PR-LIQC.FlgEst <> "A",
            EACH PR-LIQD2 OF PR-LIQC,
            EACH PL-PERS NO-LOCK
            WHERE PL-PERS.CodPer = PR-LIQD2.CodPer:
        
           Name = PL-PERS.NomPer + " " + PL-PERS.PatPer + " " + PL-PERS.MatPer. 
            ...
            ASSIGN
               ...
               Reporte.CodPer  = PR-LIQD2.CodPer
               Reporte.Name    = Name
               Reporte.CanHrs  = PR-LIQD2.Horas
               Reporte.CtoHor  = PR-LIQD2.HorVal / PR-LIQD2.Horas
               Reporte.HorVal  = PR-LIQD2.HorVal.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE FRAME F-REPORTE
           ...
           Reporte.CodPer   COLUMN-LABEL "Código!Personal"
           Reporte.Name     COLUMN-LABEL "Nombre de Personal"   FORMAT "X(40)"
           Reporte.CanHrs   COLUMN-LABEL "Horas!Trabaj"         FORMAT ">>>9.99"
           Reporte.CtoHor   COLUMN-LABEL "Costo!Hora"           FORMAT ">>>9.99"
           Reporte.HorVal   COLUMN-LABEL "Importe Total"        FORMAT ">>>>9.99"
            HEADER
            "COSTO TRABAJADOR POR ORDEN DE TRABAJO" AT 35 SKIP(3)
            "DESDE EL" FILL-IN-Fecha "HASTA EL" FILL-IN-Fecha-2 SKIP(2)      
        WITH STREAM-IO DOWN WIDTH 320.
    
    FOR EACH Reporte NO-LOCK
        BREAK BY Reporte.NumOrd:
        DISPLAY STREAM REPORT      
               Reporte.NumOrd  WHEN FIRST-OF (Reporte.NumOrd) 
               Reporte.NumLiq  WHEN FIRST-OF (Reporte.NumOrd)
               Reporte.FchLiq  WHEN FIRST-OF (Reporte.NumOrd) 
               Reporte.CodPer   
               Reporte.Name  
               Reporte.CanHrs   
               Reporte.CtoHor   
