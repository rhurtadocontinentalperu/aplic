...
LPRDHRHM.HoraI LPRDHRHM.HoraF 
&Scoped-define ENABLED-TABLES-IN-QUERY-br_table LPRDHRHM
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br_table LPRDHRHM
&Scoped-define QUERY-STRING-br_table FOR EACH LPRDHRHM OF LPRCLPRO WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH PL-PERS OF LPRDHRHM NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&Scoped-Define ENABLED-OBJECTS b-cargar br_table 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      LPRDHRHM.CodPer COLUMN-LABEL "<<Código>>" FORMAT "X(6)":U
      PL-PERS.patper FORMAT "X(25)":U
      PL-PERS.matper FORMAT "X(25)":U
      PL-PERS.nomper FORMAT "X(25)":U
      LPRDHRHM.HoraI FORMAT "99:99":U
      LPRDHRHM.HoraF FORMAT "99:99":U
      LPRDHRHM.HoraT FORMAT "->>,>>9.99":U
  ENABLE
      LPRDHRHM.CodPer
      LPRDHRHM.HoraI
      LPRDHRHM.HoraF
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 92 BY 8.65
         FONT 4.
...
/* BROWSE-TAB br_table b-cargar F-Main */
ASSIGN 
...
"LPRDHRHM.CodPer" "<<Código>>" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.PL-PERS.patper
"PL-PERS.patper" ? "X(25)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.PL-PERS.matper
"PL-PERS.matper" ? "X(25)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.PL-PERS.nomper
"PL-PERS.nomper" ? "X(25)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.LPRDHRHM.HoraI
"LPRDHRHM.HoraI" ? "99:99" "date" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.LPRDHRHM.HoraF
"LPRDHRHM.HoraF" ? "99:99" "date" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL b-cargar B-table-Win
ON CHOOSE OF b-cargar IN FRAME F-Main /* Cargar Horas/Hombre */
DO:
  RUN PRO\d-carhrhm.w (OUTPUT p-fecha).
  IF p-Fecha = ? THEN RETURN "ADM-ERROR".
  FOR EACH LPRMHRHM WHERE LPRMHRHM.CodCia = LPRCLPRO.CodCia 
    AND LPRMHRHM.CodMaq = LPRCLPRO.CodMaq 
    NO-LOCK:
      CREATE LPRDHRHM.
      ASSIGN 
