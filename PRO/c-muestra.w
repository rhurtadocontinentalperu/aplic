...
&SCOPED-DEFINE l-lookup h_l-muestras
...
DEFINE VARIABLE h_l-muestras AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 3.88 COL 51
     Btn_Cancel AT ROW 5.62 COL 51
     Btn_Help AT ROW 7.54 COL 51
     SPACE(2.13) SKIP(3.68)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pro/l-muestras.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-muestras ).
       RUN set-position IN h_l-muestras ( 1.19 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.92 , 48.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.38 , 51.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.19 , 13.29 ) NO-ERROR.

       /* Links to SmartLookup h_l-muestras. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-muestras ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-muestras ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-muestras , 'AFTER':U ).
    END. /* Page 0 */
