...
&Scoped-define QUERY-STRING-Query-Main FOR EACH PR-RQPROC WHERE ~{&KEY-PHRASE} ~
      AND PR-RQPROC.CodCia = s-codcia ~
 AND PR-RQPROC.CodAlm = s-codalm NO-LOCK ~
    BY PR-RQPROC.NroDoc DESCENDING
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH PR-RQPROC WHERE ~{&KEY-PHRASE} ~
      AND PR-RQPROC.CodCia = s-codcia ~
 AND PR-RQPROC.CodAlm = s-codalm NO-LOCK ~
    BY PR-RQPROC.NroDoc DESCENDING.
&Scoped-define TABLES-IN-QUERY-Query-Main PR-RQPROC
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main PR-RQPROC


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS F-Pedido RECT-2 
&Scoped-Define DISPLAYED-OBJECTS F-Pedido 
...
DEFINE VARIABLE F-Pedido AS INTEGER FORMAT "999999":U INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 8 BY .69
     FONT 4 NO-UNDO.
...
DEFINE FRAME FRAME-D
     F-Pedido AT ROW 1.19 COL 1.86 NO-LABEL WIDGET-ID 2
     RECT-2 AT ROW 1.04 COL 1.29 WIDGET-ID 4
...
/* SETTINGS FOR FILL-IN F-Pedido IN FRAME FRAME-D
   ALIGN-L                                                              */
...
/* Query rebuild information for QUERY Query-Main
     ...
     _OrdList          = "INTEGRAL.PR-RQPROC.NroDoc|no"
     _Where[1]         = "PR-RQPROC.CodCia = s-codcia
 AND PR-RQPROC.CodAlm = s-codalm"
...
&Scoped-define SELF-NAME F-Pedido
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-Pedido q-tables
ON LEAVE OF F-Pedido IN FRAME FRAME-D
OR "RETURN":U OF F-Pedido
DO:
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND PR-RQPROC WHERE PR-RQPROC.Codcia = S-CODCIA AND
     PR-RQPROC.CodAlm = S-CODALM AND
     PR-RQPROC.NroDoc = F-Pedido NO-LOCK NO-ERROR.
IF AVAILABLE PR-RQPROC THEN output-var-1 = ROWID(PR-RQPROC).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
