...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH LPRCLPRO WHERE ~{&KEY-PHRASE} ~
      AND LPRCLPRO.CodCia = s-CodCia ~
 AND LPRCLPRO.Estado = "E" NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main LPRCLPRO
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main LPRCLPRO
...
/* Query rebuild information for QUERY Query-Main
     ...
     _OrdList          = "INTEGRAL.LPRCLPRO.NroDoc|yes"
     _Where[1]         = "LPRCLPRO.CodCia = s-CodCia
 AND LPRCLPRO.Estado = ""E"""
     _Design-Parent    is WINDOW q-tables @ ( 1.19 , 1.72 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
ON CHOOSE OF BUTTON-3 IN FRAME FRAME-F /* Button 3 */
DO:
  /*RUN dispatch IN THIS-PROCEDURE ('qbusca':U).*/
  
/*  DEFINE VARIABLE posicion AS ROWID.
 *     RUN PRO/D-LPROD.W (OUTPUT posicion).
 *     IF posicion <> ? THEN DO:
 *         REPOSITION {&QUERY-NAME} TO ROWID posicion NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    ...
    RUN PRO/d-lprod ("Lote de Producción").
