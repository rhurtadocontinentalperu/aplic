...
&Scoped-define QUERY-STRING-Query-Main FOR EACH PR-ODPC WHERE ~{&KEY-PHRASE} ~
      AND PR-ODPC.Codcia = S-CODCIA NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH PR-ODPC WHERE ~{&KEY-PHRASE} ~
      AND PR-ODPC.Codcia = S-CODCIA NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main PR-ODPC
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main PR-ODPC


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS F-Tabla RECT-2 
...
DEFINE FRAME FRAME-B
     F-Tabla AT ROW 1.19 COL 2 NO-LABEL
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "integral.PR-ODPC.Codcia = S-CODCIA"
...
FIND PR-Odpc WHERE PR-Odpc.Codcia = S-CODCIA AND
                   PR-Odpc.NumOrd = F-Tabla NO-LOCK NO-ERROR.
IF AVAILABLE PR-Odpc THEN output-var-1 = ROWID(PR-Odpc).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
