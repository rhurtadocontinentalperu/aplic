...
DEFINE VARIABLE X-ORDEN   AS CHAR NO-UNDO.
X-ORDEN = "".
...
&SCOPED-DEFINE CONDICION ( PR-RQPROC.CodCia = S-CODCIA  AND PR-RQPROC.NumOrd BEGINS X-ORDEN AND PR-RQPROC.FlgEst = "B" )
                           
&SCOPED-DEFINE CODIGO PR-RQPROC.NroDoc

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( (PR-RQPROC.NumOrd BEGINS F-ORDEN) )
&SCOPED-DEFINE FILTRO2 ( (INDEX ( PR-RQPROC.NumOrd, F-ORDEN ) <> 0))
...
&Scoped-define INTERNAL-TABLES PR-RQPROC

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table PR-RQPROC.CodAlm PR-RQPROC.Almdes ~
PR-RQPROC.FchDoc PR-RQPROC.HorGen PR-RQPROC.NumOrd PR-RQPROC.NroDoc ~
PR-RQPROC.Usuario PR-RQPROC.Observ 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH PR-RQPROC WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table PR-RQPROC
&Scoped-define FIRST-TABLE-IN-QUERY-br_table PR-RQPROC


/* Definitions for FRAME F-Main                                         */

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS br_table FILL-IN-codigo F-Orden CMB-filtro ~
FILL-IN-filtro 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-codigo F-Orden CMB-filtro ~
FILL-IN-filtro 
...
<FOREIGN-KEYS>
NumOrd||y|INTEGRAL.PR-RQPROC.NumOrd
CodAlm||y|INTEGRAL.PR-RQPROC.CodAlm
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
<SORTBY-OPTIONS>
Orden Trabajo|y||integral.PR-RQPROC.NumOrd|yes
Almacen|||integral.PR-RQPROC.CodAlm|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "Orden Trabajo,Almacen",
     SortBy-Case = Orden Trabajo':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      PR-RQPROC.CodAlm
      PR-RQPROC.Almdes
      PR-RQPROC.FchDoc COLUMN-LABEL "Fecha          !Emision"
      PR-RQPROC.HorGen COLUMN-LABEL "Hora!Emision"
      PR-RQPROC.NumOrd COLUMN-LABEL "Orden de!Trabajo"
      PR-RQPROC.NroDoc COLUMN-LABEL "Numero de!Requerimiento"
      PR-RQPROC.Usuario
      PR-RQPROC.Observ
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 86.29 BY 10.92
...
DEFINE FRAME F-Main
     br_table AT ROW 2.35 COL 87.15 RIGHT-ALIGNED
     FILL-IN-codigo AT ROW 1.31 COL 10 COLON-ALIGNED
     F-Orden AT ROW 1.31 COL 25.14 COLON-ALIGNED
     CMB-filtro AT ROW 1.38 COL 37.57
     FILL-IN-filtro AT ROW 1.38 COL 66.86 NO-LABEL
     "(*)  Doble Click - Visualiza Detalle" VIEW-AS TEXT
          SIZE 28.57 BY .62 AT ROW 13.38 COL 1.86
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "integral.PR-RQPROC"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "{&Condicion}"
     _FldNameList[1]   = integral.PR-RQPROC.CodAlm
     _FldNameList[2]   = integral.PR-RQPROC.Almdes
     _FldNameList[3]   > integral.PR-RQPROC.FchDoc
"PR-RQPROC.FchDoc" "Fecha          !Emision" ? "date" ? ? ? ? ? ? no ?
     _FldNameList[4]   > integral.PR-RQPROC.HorGen
"PR-RQPROC.HorGen" "Hora!Emision" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[5]   > integral.PR-RQPROC.NumOrd
"PR-RQPROC.NumOrd" "Orden de!Trabajo" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[6]   > integral.PR-RQPROC.NroDoc
"PR-RQPROC.NroDoc" "Numero de!Requerimiento" ? "integer" ? ? ? ? ? ? no ?
     _FldNameList[7]   = integral.PR-RQPROC.Usuario
     _FldNameList[8]   = integral.PR-RQPROC.Observ
...
DO:
  RUN pro/d-rqxapr.R (ROWID(pr-rqproc)).
...
DO:
    ...
    CASE CMB-filtro:
         WHEN 'Todos':U THEN DO:
              IF F-ORDEN <> "" THEN RUN set-attribute-list('Key-Name=Nombres que inicien con').
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-Orden L-table-Win
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* Requerimiento */
DO:
    ...
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE
            {&CONDICION} AND
             {&CODIGO} = integer(INPUT FILL-IN-codigo:screen-value)
            NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Orden Trabajo':U THEN DO:
      &Scope SORTBY-PHRASE BY PR-RQPROC.NumOrd
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    WHEN 'Almacen':U THEN DO:
      &Scope SORTBY-PHRASE BY PR-RQPROC.CodAlm
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-3 = STRING({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.NroDoc,"99999").
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "NumOrd" "PR-RQPROC" "NumOrd"}
  {src/adm/template/sndkycas.i "CodAlm" "PR-RQPROC" "CodAlm"}
