...
&Scoped-define QUERY-STRING-Query-Main FOR EACH ImCDua WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH ImCDua WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main ImCDua
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main ImCDua
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND ImCDua WHERE ImCDua.CodCia = S-CODCIA AND
                   ImCDua.NroDua = F-Numero 
                   NO-LOCK NO-ERROR.
IF AVAILABLE ImCDua THEN output-var-1 = ROWID(ImCDua).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
