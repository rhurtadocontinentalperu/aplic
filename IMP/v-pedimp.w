...
DEFINE SHARED TEMP-TABLE CCMP  LIKE ImCOCmp.
DEFINE SHARED TEMP-TABLE DCMP  LIKE ImDOCmp.
/*DEFINE SHARED TEMP-TABLE DCMP2 LIKE ImDOCmp.*/
DEFINE SHARED TEMP-TABLE DCOT  LIKE ImDOCmp.
...
&Scoped-Define ENABLED-FIELDS ImCOCmp.Periodo ImCOCmp.NroCst ImCOCmp.CodPro ~
ImCOCmp.Version ImCOCmp.FchProd ImCOCmp.TpoCmb ImCOCmp.ImpFob ~
ImCOCmp.ImpFle ImCOCmp.CodBco ImCOCmp.NroCreDoc ImCOCmp.Carga ~
ImCOCmp.NroCon ImCOCmp.FchIni ImCOCmp.Observaciones ImCOCmp.FchFin 
...
ImCOCmp.NroCst ImCOCmp.NroPed ImCOCmp.Fchdoc ImCOCmp.CodPro ImCOCmp.NomPro ~
...
DEFINE FRAME F-Main
     ...
     ImCOCmp.NroImp AT ROW 1.19 COL 41 COLON-ALIGNED
          ...
          SIZE 8 BY .81
          FONT 6
     ImCOCmp.NroCst AT ROW 1.27 COL 59 COLON-ALIGNED WIDGET-ID 20
          VIEW-AS FILL-IN 
          SIZE 8 BY .81
...
/* SETTINGS FOR FILL-IN ImCOCmp.CndCmp IN FRAME F-Main
   NO-ENABLE                                                            */
...
/* SETTINGS FOR FILL-IN ImCOCmp.Moneda IN FRAME F-Main
   NO-ENABLE                                                            */
...
/* SETTINGS FOR FILL-IN ImCOCmp.NroPed IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN ImCOCmp.NroPedPro IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN ImCOCmp.Pais IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN ImCOCmp.Proforma IN FRAME F-Main
   NO-ENABLE                                                            */
...
------------------------------------------------------------------------------*/
   ...
   FOR EACH ImCOCmp WHERE
            ImCOCmp.CodCia  = S-CodCia AND
            ImCOCmp.NroImp  = INTEGER(ImCOCmp.NroImp:SCREEN-VALUE IN FRAME {&FRAME-NAME}) NO-LOCK:
                  CREATE CCMP.
                         ASSIGN
                             CCMP.CodCia  = s-codcia
                             CCMP.NroImp  = ImCOCmp.NroImp
                             CCMP.CodDoc = ImCOCmp.CodDoc.
   END.
   FOR EACH DCOT:
       CREATE DCMP.
       ASSIGN
            DCMP.NroImp  = INTEGER(ImCOCmp.NroImp:SCREEN-VALUE IN FRAME {&FRAME-NAME})
            DCMP.CodCia  = DCOT.CodCia
            DCMP.CodDoc  = "O/C"
            DCMP.CodMat  = DCOT.CodMat
            DCMP.CanPedi = DCOT.CanPedi.
   END.

   FOR EACH ImDOCmp WHERE
            ImDOCmp.CodCia  = S-CodCia AND
            ImDOCmp.NroImp  = INTEGER(ImCOCmp.NroImp:SCREEN-VALUE IN FRAME {&FRAME-NAME}) AND
            ImDOCmp.CanPedi > 0 NO-LOCK:
                  CREATE DCOT.
                         ASSIGN
                             DCOT.CodCia  = s-codcia
                             DCOT.CodMat  = ImDOCmp.CodMat
                             DCOT.CanPedi = ImDOCmp.CanPedi.
   END.
   FOR EACH DCOT:
       CREATE DCMP.
       ASSIGN
...
/*             FIND FIRST B-CCMP WHERE                                                                     */
/*                 B-CCMP.NroReq = INTEGER(ImCOcmp.NroReq:SCREEN-VALUE IN FRAME {&FRAME-NAME})             */
/*                 AND B-CCMP.FlgSit <> 'A'                                                                */
/*                 NO-LOCK NO-ERROR.                                                                       */
/*                 IF AVAILABLE B-CCMP THEN DO:                                                            */
/* /*                     MESSAGE "YA EXISTE ORDENES" VIEW-AS ALERT-BOX. */                                */
/*                     FOR EACH B-DCMP WHERE B-DCMP.NroImp = B-CCMP.NroImp                                 */
...
/*                 END.                                                                                    */
/*                 ELSE DO:                                                                                */
/* /*                     MESSAGE " NOP EXISTE ORDENES" VIEW-AS ALERT-BOX. */                              */
/*                     CREATE DREQ.                                                                        */
/*                        ASSIGN                                                                           */
/*                          DREQ.CodCia  = s-codcia                                                        */
/*                          DREQ.CodMat  = Lg-DRequ.CodMat                                                 */
  ...
  FOR EACH DCMP:
    ...
    BUFFER-COPY DCMP TO ImDOcmp
        ASSIGN 
            ...
            ImDOCmp.CstTot      = ImDOCmp.PreUni * ImDOCmp.CanPedi.        
  END.
  RELEASE ImDOCmp.
...
------------------------------------------------------------------------------*/
  ...
  RUN IMP\d-cotpen (OUTPUT p-NroRef).
  ...
  /* Code placed here will execute AFTER standard behavior.    */
    ...
    FOR EACH B-CCMP WHERE B-CCMP.NroImp = p-NroRef. 
        DISPLAY p-NroRef   @ ImCOCmp.NroImp WITH FRAME {&FRAME-NAME}.
    END.
    RUN Carga-Temporal.
    DISPLAY
...
/*        END.                                                  */
       ASSIGN
             ...
             ImCOCmp.NroCst = integer(ImCOCmp.NroCst:screen-value in frame {&FRAME-NAME})
