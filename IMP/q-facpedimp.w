...
/*DEFINE VARIABLE S-TPODOC AS CHAR INIT "N" NO-UNDO.*/
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH ImCFacCom WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main ImCFacCom
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main ImCFacCom
...
/* SETTINGS FOR FILL-IN F-Numero IN FRAME FRAME-D
   ALIGN-L                                                              */
...
ON LEAVE OF F-Numero IN FRAME FRAME-D
 ...
 *      LG-COCmp.TpoDoc = S-TPODOC AND
 *      LG-COCmp.NroDoc = F-Numero NO-LOCK NO-ERROR.
 * IF AVAILABLE LG-COCmp THEN output-var-1 = ROWID(LG-COCmp).*/
 
FIND ImCFacCom WHERE ImCFacCom.CodCia = S-CODCIA AND
     ImCFacCom.CodDoc = 'FAC' AND
     ImCFacCom.NroFac = F-Numero NO-LOCK NO-ERROR.
IF AVAILABLE ImCFacCom THEN output-var-1 = ROWID(ImCFacCom). 
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
