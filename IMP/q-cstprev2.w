...
&Scoped-define QUERY-STRING-Query-Main FOR EACH ImCOCmp WHERE ~{&KEY-PHRASE} ~
      AND ImCOCmp.CodDoc = "COT" NO-LOCK ~
    BY ImCOCmp.NroImp INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH ImCOCmp WHERE ~{&KEY-PHRASE} ~
      AND ImCOCmp.CodDoc = "COT" NO-LOCK ~
    BY ImCOCmp.NroImp INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main ImCOCmp
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main ImCOCmp


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS BUTTON-1 F-NroCst RECT-2 
&Scoped-Define DISPLAYED-OBJECTS F-NroCst 
...
DEFINE VARIABLE F-NroCst AS INTEGER FORMAT "999999":U INITIAL 0 
     VIEW-AS FILL-IN 
...
DEFINE FRAME FRAME-D
     ...
     F-NroCst AT ROW 1.19 COL 1.86 NO-LABEL
...
/* SETTINGS FOR FILL-IN F-NroCst IN FRAME FRAME-D
   ALIGN-L                                                              */
...
/* Query rebuild information for QUERY Query-Main
     ...
     _OrdList          = "INTEGRAL.ImCOCmp.NroImp|yes"
     _Where[1]         = "ImCOCmp.CodDoc = ""COT"""
     _Design-Parent    is WINDOW q-tables @ ( 1.27 , 13.14 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
ON CHOOSE OF BUTTON-1 IN FRAME FRAME-D /* Button 1 */
DO:
...
&Scoped-define SELF-NAME F-NroCst
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-NroCst q-tables
ON LEAVE OF F-NroCst IN FRAME FRAME-D
OR "RETURN":U OF F-NroCst
DO:
   FIND ImCOCmp WHERE ImCOCmp.codcia = s-codcia
    AND ImCOCmp.CodDiv = "00000"
    AND ImCOCmp.NroCst = INPUT {&SELF-NAME}
    NO-LOCK NO-ERROR.
  IF AVAILABLE ImCOCmp
  THEN DO:
    REPOSITION {&QUERY-NAME}  TO ROWID ROWID(ImCOCmp) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN LKUP/c-pedimp ("Cotizaciones de Compra - Importaciones").
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND ImCOCmp WHERE ImCOCmp.CodCia = S-CODCIA AND
                   ImCOCmp.CodDiv = S-CODDIV AND
                   ImCOCmp.NroCst = F-NroCst
                   NO-LOCK NO-ERROR.
IF AVAILABLE ImCOCmp THEN output-var-1 = ROWID(ImCOCmp).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
