...
&SCOPED-DEFINE l-lookup h_l-gastosimp
...
DEFINE VARIABLE h_l-gastosimp AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.54 COL 58
     Btn_Cancel AT ROW 8.73 COL 58
     Btn_Help AT ROW 10.54 COL 58
     SPACE(3.71) SKIP(0.68)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 4
         TITLE "Tabla de Gastos de Importaciones"
...
ON WINDOW-CLOSE OF FRAME C-Dialog /* Tabla de Gastos de Importaciones */
DO:  
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/imp/l-gastosimp.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-gastosimp ).
       RUN set-position IN h_l-gastosimp ( 1.19 , 2.00 ) NO-ERROR.
       ...
       RUN set-size IN h_p-option ( 1.92 , 12.00 ) NO-ERROR.

       /* Links to SmartLookup h_l-gastosimp. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-gastosimp ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-gastosimp ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-gastosimp , 'AFTER':U ).
    END. /* Page 0 */
