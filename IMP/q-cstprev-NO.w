...
&Scoped-define INTERNAL-TABLES ImDCstP

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for QUERY Query-Main                                     */
&Scoped-define QUERY-STRING-Query-Main FOR EACH ImDCstP WHERE ~{&KEY-PHRASE} NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH ImDCstP WHERE ~{&KEY-PHRASE} NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main ImDCstP
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main ImDCstP
...
DO:
   FIND ImCCst WHERE ImCCst.codcia = s-codcia
    AND ImCCst.CodDiv = "00000"
    AND ImCCst.NroCst = INPUT {&SELF-NAME}
    NO-LOCK NO-ERROR.
  IF AVAILABLE ImCCst
  THEN DO:
    REPOSITION {&QUERY-NAME}  TO ROWID ROWID(ImCCst) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN LKUP/c-pedimp ("Costeos Previos de Compra - Importaciones").
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND ImCCst WHERE ImCCst.CodCia = S-CODCIA AND
                  ImCCst.CodDiv = S-CODDIV AND
                  ImCCst.NroCst = F-NroCst
                   NO-LOCK NO-ERROR.
IF AVAILABLE ImCCst THEN output-var-1 = ROWID(ImCCst).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
