...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH LG-CRequ WHERE ~{&KEY-PHRASE} NO-LOCK ~
    BY LG-CRequ.NroReq INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main LG-CRequ
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main LG-CRequ


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS RECT-2 F-NroReq BUTTON-1 
&Scoped-Define DISPLAYED-OBJECTS F-NroReq 
...
DEFINE VARIABLE F-NroReq AS INTEGER FORMAT "999999":U INITIAL 0 
     VIEW-AS FILL-IN 
...
DEFINE FRAME FRAME-D
     F-NroReq AT ROW 1.19 COL 1.86 NO-LABEL
...
/* SETTINGS FOR FILL-IN F-NroReq IN FRAME FRAME-D
   ALIGN-L                                                              */
...
/* Query rebuild information for QUERY Query-Main
     ...
     _OrdList          = "INTEGRAL.LG-CRequ.NroReq|yes"
...
&Scoped-define SELF-NAME F-NroReq
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-NroReq q-tables
ON LEAVE OF F-NroReq IN FRAME FRAME-D
OR "RETURN":U OF F-NroReq
DO:
   FIND Lg-CRequ WHERE Lg-CRequ.codcia = s-codcia
   /* AND ImCOCmp.coddoc = "O/C"*/
    AND Lg-CRequ.nroreq = INPUT {&SELF-NAME}
    NO-LOCK NO-ERROR.
  IF AVAILABLE Lg-CRequ
  THEN DO:
    REPOSITION {&QUERY-NAME}  TO ROWID ROWID(Lg-CRequ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN LKUP/c-req ("Requerimiento de Compra - Importaciones").
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND Lg-CRequ WHERE Lg-CRequ.CodCia = S-CODCIA AND
                   Lg-CRequ.NroReq = F-NroReq 
                   NO-LOCK NO-ERROR.
IF AVAILABLE Lg-CRequ THEN output-var-1 = ROWID(Lg-CRequ).
ELSE output-var-1 = ?.
IF OUTPUT-VAR-1 <> ? THEN DO:
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
