...
&Scoped-define QUERY-STRING-Query-Main FOR EACH ImCFacCom WHERE ~{&KEY-PHRASE} ~
      AND ImCFacCom.FlgEst <> "A" NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH ImCFacCom WHERE ~{&KEY-PHRASE} ~
      AND ImCFacCom.FlgEst <> "A" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
