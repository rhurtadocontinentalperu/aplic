...
RUN Generando-asientos.

/* Borramos errores  */
FOR EACH cb-cmov WHERE codcia = s-codcia
    AND periodo = s-periodo
    AND nromes = s-nromes
    AND codope = s-codope
    AND ABS(dbemn1 - hbemn1) > 1:
...
FOR EACH cb-cmov WHERE cb-cmov.codcia = s-codcia
    ...
    FOR EACH cb-dmov OF cb-cmov:
        ...
        END.
        /* NUMERO DE DOCUMENTO */
        IF (codcta BEGINS '421' OR codcta BEGINS '424' OR codcta BEGINS '422' OR codcta BEGINS '43')
            AND LENGTH(nrodoc) = 15
            THEN DO:
        ...
        END.
        IF (codcta BEGINS '423') AND LENGTH(nrodoc) = 15
            THEN DO:
            s-NroDoc = cb-dmov.NroDoc.
            ASSIGN
            ...
            ASSIGN
                cb-dmov.NroDoc = s-NroDoc.
        END.
        IF codcta BEGINS '422' THEN coddoc = '34'.  /* ANTICIPOS */
