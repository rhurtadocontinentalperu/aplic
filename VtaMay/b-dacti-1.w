...
VtaDActi.ImpVtaMe VtaDActi.Observaciones VtaDActi.Estado[1] 
...
&Scoped-Define ENABLED-OBJECTS BUTTON-4 BUTTON-5 FILL-IN-NomCli br_table 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      VtaDActi.Estado[1] COLUMN-LABEL "Impresiones" FORMAT "x(2)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 107 BY 11.35
         FONT 4.
...
"VtaDActi.CodCli" "<<<Cliente>>>" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaDActi.ImpVtaMe" "Compras en US$" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[5]   > INTEGRAL.VtaDActi.Estado[1]
"VtaDActi.Estado[1]" "Impresiones" "x(2)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
 ...
 *   END.      */
  ...
  REPEAT WHILE AVAILABLE VtaDActi:
    IF INDEX(gn-clie.nomcli, FILL-IN-NomCli:SCREEN-VALUE) > 0
    THEN DO:
        ASSIGN
            s-Registro-Actual = ROWID(VTaDActi).
    ...
    GET NEXT {&BROWSE-NAME}.
  END.
  IF NOT AVAILABLE VtaDActi THEN s-Registro-Actual = ?.
  IF s-Registro-Actual = ? THEN MESSAGE 'Fin de búsqueda' VIEW-AS ALERT-BOX WARNING.
END.
...
DO i = 1 TO {&browse-name}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
    IF {&browse-name}:FETCH-SELECTED-ROW(i) THEN DO:
        FIND CURRENT Vtadacti EXCLUSIVE-LOCK NO-ERROR.
        IF AVAILABLE Vtadacti THEN DO:
            VtaDActi.Estado[1] = TRIM( STRING( DECIMAL( VtaDActi.Estado[1] ) + 1 ) ).
        END.
        FIND CURRENT Vtadacti NO-LOCK NO-ERROR.
    END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Temporal-Sobres B-table-Win 
PROCEDURE Carga-Temporal-Sobres :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO i = 1 TO {&browse-name}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
    IF {&browse-name}:FETCH-SELECTED-ROW(i) THEN DO:
        FIND Gn-clie WHERE Gn-clie.codcia = cl-codcia
            AND Gn-clie.codcli = VtaDActi.CodCli
            NO-LOCK.
        CREATE w-report.
        ASSIGN
            w-report.Task-No = s-task-no
            w-report.Campo-C[1] = VtaDActi.CodCli
        ...
        IF AVAILABLE Tabdistr THEN w-report.Campo-C[4] = Tabdistr.Nomdistr.
        w-report.Campo-C[5] = Gn-clie.codpos.
        FIND  Tabprovi WHERE Tabprovi.CodDepto = gn-clie.CodDept
            AND Tabprovi.Codprovi = gn-clie.codprov NO-LOCK NO-ERROR.
        IF AVAILABLE Tabprovi THEN w-report.Campo-C[6] = Tabprovi.Nomprovi.
        FIND  TabDepto WHERE TabDepto.CodDepto = gn-clie.CodDept NO-LOCK NO-ERROR.
        IF AVAILABLE TabDepto THEN w-report.Campo-C[7] = TabDepto.NomDepto.
    END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Imprimir-Sobres B-table-Win 
...
DEF VAR RB-OTHER-PARAMETERS AS CHAR INITIAL "".     /* Otros parametros */
    ...
    ASSIGN
        ...
        RB-REPORT-NAME = 'Actividades - Sobres'
