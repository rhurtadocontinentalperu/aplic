...
DEFINE NEW SHARED VARIABLE S-TPOPED AS CHAR INIT "N".
...
&Scoped-Define ENABLED-OBJECTS RECT-12 COMBO-NroSer BUTTON-2 
...
DEFINE VARIABLE h_b-cotiza-4-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-cotexp-4-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-cotiza-4-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotiza-4exp-1 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON btn-estad 
     IMAGE-UP FILE "img/app-est.bmp":U
     LABEL "Button 8" 
     SIZE 11 BY 2.42.
...
DEFINE FRAME F-Main
     ...
     BUTTON-2 AT ROW 2.88 COL 124 WIDGET-ID 12
     BTN-Excel AT ROW 4.23 COL 124 WIDGET-ID 10
     btn-estad AT ROW 5.85 COL 124 WIDGET-ID 30
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "COTIZACION EXPOLIBRERIA ENE 2011"
         HEIGHT             = 24.88
         WIDTH              = 143
         MAX-HEIGHT         = 25.58
         MAX-WIDTH          = 160.86
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* SETTINGS FOR BUTTON btn-estad IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       btn-estad:HIDDEN IN FRAME F-Main           = TRUE.
...
ON END-ERROR OF W-Win /* COTIZACION EXPOLIBRERIA ENE 2011 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* COTIZACION EXPOLIBRERIA ENE 2011 */
DO:
...
&Scoped-define SELF-NAME btn-estad
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn-estad W-Win
ON CHOOSE OF btn-estad IN FRAME F-Main /* Button 8 */
DO:
  /*Resumen Estadisticas*/
  RUN aplic\bgn\p-estvtasexpo.
END.
...
DO:
  RUN Genera-Excel IN h_v-cotiza-4exp-1.
END.
...
DO:
    ...
    RUN dispatch IN h_q-cotexp-4-1 ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-cotiza-4exp-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotiza-4exp-1 ).
       RUN set-position IN h_v-cotiza-4exp-1 ( 2.88 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.88 , 111.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/q-cotexp-4-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-cotexp-4-1 ).
       RUN set-position IN h_q-cotexp-4-1 ( 1.00 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 9.00 ) */

       /* Links to SmartViewer h_v-cotiza-4exp-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-cotiza-4exp-1 ).
       RUN add-link IN adm-broker-hdl ( h_q-cotexp-4-1 , 'Record':U , h_v-cotiza-4exp-1 ).

       /* Links to SmartQuery h_q-cotexp-4-1. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-cotexp-4-1 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv02 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotiza-4exp-1 ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-cotexp-4-1 ,
             btn-estad:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-cotiza-4-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotiza-4-1 ).
       RUN set-position IN h_b-cotiza-4-1 ( 12.04 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotiza-4-1 ( 13.38 , 141.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotiza-4-1. */
       RUN add-link IN adm-broker-hdl ( h_q-cotexp-4-1 , 'Record':U , h_b-cotiza-4-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotiza-4-1 ,
             btn-estad:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-cotiza-4-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotiza-4-1 ).
       RUN set-position IN h_t-cotiza-4-1 ( 12.04 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotiza-4-1 ( 12.31 , 141.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotiza-4-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotiza-4-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotiza-4-1 ,
             btn-estad:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotiza-4-1 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-12 COMBO-NroSer BUTTON-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_b-cotiza-4-1 <> ? THEN RUN dispatch IN h_b-cotiza-4-1 ('open-query':U).
          IF h_t-cotiza-4-1 <> ? THEN RUN dispatch IN h_t-cotiza-4-1 ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-cotiza-4-1 <> ? THEN RUN Recalcular-Precios IN h_t-cotiza-4-1.
      END.
    WHEN "Disable-Head" THEN DO:
        RUN dispatch IN h_p-updv02 ('disable':U).
        /*RUN dispatch IN h_v-cotiza-4 ('disable-fields').*/
        /*Btn-Grupos:SENSITIVE = NO.*/
        /*B-CLI:SENSITIVE = NO.*/
      END.
    WHEN "Enable-Head" THEN DO:
        RUN dispatch IN h_p-updv02 ('enable':U).
        /*RUN dispatch IN h_v-cotiza-4 ('enable-fields').*/
