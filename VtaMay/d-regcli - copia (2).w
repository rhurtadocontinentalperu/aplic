...
&Scoped-Define ENABLED-OBJECTS RADIO-SET-Libre_C01 F-DNICli F-RucCli ~
f-ApePat f-ApeMat f-Nombre F-DirCli F-Telfnos f-E-Mail f-Transporte ~
RADIO-SET-Retenedor RADIO-SET-Percepcion f-Canal F-GirCli F-ClfCom ~
F-CodDept F-CodProv F-CodDist f-vende Btn_OK Btn_Cancel 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-Libre_C01 F-DNICli F-RucCli ~
F-CodCli f-ApePat f-ApeMat f-Nombre F-NomCli F-DirCli F-Telfnos f-E-Mail ~
f-Transporte RADIO-SET-Retenedor RADIO-SET-Percepcion f-Canal F-GirCli ~
F-ClfCom F-CodDept F-CodProv F-CodDist f-vende f-condvta F-NomCanal F-giro ~
f-Sector FILL-IN-DEP FILL-IN-PROV FILL-IN-DIS f-nomven f-descond ~
f-DateInscription 
...
DEFINE VARIABLE f-Canal AS CHARACTER FORMAT "X(8)":U 
     LABEL "Grupo de cliente (Canal)" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .81 NO-UNDO.

DEFINE VARIABLE F-ClfCom AS CHARACTER FORMAT "X(8)" 
...
DEFINE VARIABLE f-E-Mail AS CHARACTER FORMAT "X(150)":U 
     LABEL "e-mail Fact. Contacto" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME D-Dialog
     ...
     F-DNICli AT ROW 2.08 COL 18 COLON-ALIGNED HELP
          "DNI DEL (Cliente)"
     F-RucCli AT ROW 2.88 COL 18 COLON-ALIGNED
     ...
     f-E-Mail AT ROW 9.35 COL 18 COLON-ALIGNED WIDGET-ID 40
     f-Transporte AT ROW 10.15 COL 18 COLON-ALIGNED WIDGET-ID 44
     RADIO-SET-Retenedor AT ROW 11.23 COL 20 NO-LABEL WIDGET-ID 12
     RADIO-SET-Percepcion AT ROW 12 COL 20 NO-LABEL WIDGET-ID 18
     f-Canal AT ROW 12.85 COL 18 COLON-ALIGNED WIDGET-ID 46
     F-GirCli AT ROW 13.65 COL 18 COLON-ALIGNED
     F-ClfCom AT ROW 14.46 COL 18 COLON-ALIGNED WIDGET-ID 50
     F-CodDept AT ROW 15.27 COL 18 COLON-ALIGNED
     F-CodProv AT ROW 16.08 COL 18 COLON-ALIGNED
     F-CodDist AT ROW 16.88 COL 18 COLON-ALIGNED
     f-vende AT ROW 17.69 COL 18 COLON-ALIGNED
     f-condvta AT ROW 18.5 COL 18 COLON-ALIGNED
     F-NomCanal AT ROW 12.85 COL 25 COLON-ALIGNED NO-LABEL WIDGET-ID 48
     F-giro AT ROW 13.65 COL 25 COLON-ALIGNED NO-LABEL
     f-Sector AT ROW 14.46 COL 25 COLON-ALIGNED NO-LABEL WIDGET-ID 168
     FILL-IN-DEP AT ROW 15.27 COL 25 COLON-ALIGNED NO-LABEL
     FILL-IN-PROV AT ROW 16.08 COL 25 COLON-ALIGNED NO-LABEL
     FILL-IN-DIS AT ROW 16.88 COL 25 COLON-ALIGNED NO-LABEL
     f-nomven AT ROW 17.69 COL 25 COLON-ALIGNED NO-LABEL
     f-descond AT ROW 18.5 COL 25 COLON-ALIGNED NO-LABEL
     ...
     "Agente Retenedor:" VIEW-AS TEXT
          SIZE 13 BY .5 AT ROW 11.23 COL 7 WIDGET-ID 16
     "<<< VERIFICAR ESTOS DATOS EN SUNAT <<<" VIEW-AS TEXT
          SIZE 40 BY .5 AT ROW 11.31 COL 33 WIDGET-ID 32
     ...
     "Agente de Percepción:" VIEW-AS TEXT
          SIZE 16 BY .5 AT ROW 12.04 COL 4 WIDGET-ID 30
     SPACE(76.42) SKIP(8.18)
...
/* SETTINGS FOR FILL-IN F-giro IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN F-NomCanal IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN F-NomCli IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN f-nomven IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN f-Sector IN FRAME D-Dialog
   NO-ENABLE                                                            */
...
END.
   ...
   END.
    ...
    CASE TRUE:
        ...
        WHEN RADIO-SET-Libre_C01:SCREEN-VALUE = "N" THEN DO:
            IF F-RucCli:SCREEN-VALUE > '' THEN DO:
                IF LENGTH(F-RucCli:SCREEN-VALUE) < 11 OR LOOKUP(SUBSTRING(F-RucCli:SCREEN-VALUE,1,2), '10') = 0 THEN DO:
        ...
        WHEN RADIO-SET-Libre_C01:SCREEN-VALUE = "E" THEN DO:
            IF LENGTH(F-RucCli:SCREEN-VALUE) < 11 OR LOOKUP(SUBSTRING(F-RucCli:SCREEN-VALUE,1,2), '15,17') = 0 THEN DO:
    ...
    IF f-E-Mail > '' THEN DO:
        x-Nro-EMails = NUM-ENTRIES(f-E-Mail,';').
        DO x-Item = 1 TO x-Nro-EMails:
            RUN gn/valida-email (ENTRY(x-Item,f-E-Mail,';'), 
                                 OUTPUT pError).
            IF pError > '' THEN DO:
    ...
    IF f-Transporte > '' THEN DO:
        ...
        END.
    END.
    IF F-DNICli:SCREEN-VALUE > '' AND LENGTH(F-DNICli:SCREEN-VALUE) <> 8 THEN DO:
        MESSAGE 'El DNI debe tener 8 caracteres' VIEW-AS ALERT-BOX ERROR.
        APPLY 'ENTRY':U TO F-DNICli.
        RETURN NO-APPLY.
    END.
    ...
    CASE TRUE:
        WHEN RADIO-SET-Libre_C01:SCREEN-VALUE = "J" THEN DO:
            IF LENGTH(TRIM(f-nombre:SCREEN-VALUE)) < 5  THEN DO:
    ...
    /**/
    IF RADIO-SET-Libre_C01:SCREEN-VALUE = "N" THEN DO:        /* Apellido Paterno */        
...
ON LEFT-MOUSE-DBLCLICK OF f-Canal IN FRAME D-Dialog /* Grupo de cliente (Canal) */
DO:
    ASSIGN
      input-var-1 = 'CN'
      input-var-2 = ''
...
&Scoped-define SELF-NAME F-ClfCom
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-ClfCom D-Dialog
ON LEAVE OF F-ClfCom IN FRAME D-Dialog /* Sector Económico */
DO:
  ASSIGN F-GirCli.
  IF F-GirCli <> "" THEN DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-ClfCom D-Dialog
ON LEFT-MOUSE-DBLCLICK OF F-ClfCom IN FRAME D-Dialog /* Sector Económico */
DO:
    ASSIGN
      input-var-1 = 'SE'
      input-var-2 = ''
...
DO:
    ...
    END.
    */
    ASSIGN x-Integer = INTEGER(SELF:SCREEN-VALUE) NO-ERROR.
    ...
    SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(x-valor,"99999999").

    RADIO-SET-Libre_C01:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'N'.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-GirCli D-Dialog
ON LEFT-MOUSE-DBLCLICK OF F-GirCli IN FRAME D-Dialog /* Giro */
DO:
    DEFINE VAR x-grupo AS CHAR.

    ASSIGN
      input-var-1 = 'CN-GN'
      input-var-2 = f-canal:SCREEN-VALUE
      input-var-3 = 'GN'
      output-var-1 = ?            /* Rowid */
      output-var-2 = ''
      output-var-3 = ''.
...
DO:
    ...
    IF x-requiere-validacion-sunat = YES THEN DO:
      ...
      IF pError > '' THEN DO:
          ...
          /**/
              MESSAGE 'Desea continuar sin validar el R.U.C. en SUNAT' VIEW-AS ALERT-BOX QUESTION
                    BUTTONS YES-NO UPDATE rpta AS LOG.
            IF rpta = NO THEN DO:
                SELF:SCREEN-VALUE = ''.
                RETURN NO-APPLY.
            ...
            pBajaSunat = NO.
            x-manual = YES.
        /* RUN habilitar-campos(INPUT radio-set-Libre_C01:SCREEN-VALUE).*/
        END.    
    END.
    ELSE DO:
        x-manual = YES.
    END.
    IF x-manual = YES THEN DO:
        ASSIGN f-ApeMat:SENSITIVE = YES
        f-ApePat:SENSITIVE = YES
        f-Nombre:SENSITIVE = YES
  ...
  CASE RADIO-SET-Libre_C01:SCREEN-VALUE:
      WHEN "J" THEN DO:
          DISPLAY 
          SELF:SCREEN-VALUE @ f-CodCli
          pName @ f-Nombre
          pName @ f-NomCli
          pAddress @ f-DirCli
  ...
  IF SUBSTRING(SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME},1,2) = "20" THEN DO:
      RADIO-SET-Libre_C01:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'J'.
      F-DNICli:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ''.
  END.
  ELSE DO:    
      IF SUBSTRING(SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME},1,2) = "10" THEN DO:
          RADIO-SET-Libre_C01:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'N'.
          F-DNICli:SCREEN-VALUE IN FRAME {&FRAME-NAME} = SUBSTRING(SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME},3,8).
      END.
      ELSE DO:
          RADIO-SET-Libre_C01:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'E'.
      END.      
  END.      

  IF x-manual = NO THEN APPLY 'VALUE-CHANGED':U TO RADIO-SET-Libre_C01.
END.
...
DO TRANSACTION ON STOP UNDO, RETURN "ADM-ERROR":U ON ERROR UNDO, RETURN "ADM-ERROR":U
   ...
   /* 15/05/2023 */
   ASSIGN
       gn-clie.e-mail = f-e-mail
       gn-clie.Transporte[4] = f-Transporte
       gn-clie.Canal = f-Canal
       gn-clie.ClfCom = f-ClfCom.
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-Libre_C01 F-DNICli F-RucCli F-CodCli f-ApePat f-ApeMat 
          f-Nombre F-NomCli F-DirCli F-Telfnos f-E-Mail f-Transporte 
          RADIO-SET-Retenedor RADIO-SET-Percepcion f-Canal F-GirCli F-ClfCom 
          F-CodDept F-CodProv F-CodDist f-vende f-condvta F-NomCanal F-giro 
          f-Sector FILL-IN-DEP FILL-IN-PROV FILL-IN-DIS f-nomven f-descond 
          f-DateInscription 
      WITH FRAME D-Dialog.
  ENABLE RADIO-SET-Libre_C01 F-DNICli F-RucCli f-ApePat f-ApeMat f-Nombre 
         F-DirCli F-Telfnos f-E-Mail f-Transporte RADIO-SET-Retenedor 
         RADIO-SET-Percepcion f-Canal F-GirCli F-ClfCom F-CodDept F-CodProv 
...
------------------------------------------------------------------------------*/
  ...
  IF x-valor <= 99999999 OR SUBSTRING(S-CODCLI,1,2) = '10' THEN DO:
      /* DNI */
      IF x-valor <= 99999999 THEN DO:
          x-dni = STRING(x-valor,"99999999").
      END.
      ELSE DO:
          x-dni = SUBSTRING(S-CODCLI,3,8).
          DISPLAY S-CODCLI @ F-RUCCli WITH FRAME {&FRAME-NAME}.
      END.
      
      DISPLAY x-dni @ F-DNICli WITH FRAME {&FRAME-NAME}.
      radio-set-libre_c01:SCREEN-VALUE IN FRAME {&FRAME-NAME}= 'N' .

      RUN habilitar-campos(INPUT "N").

      APPLY "ENTRY" TO F-DniCli.
  
  END.
  ELSE DO:
      DISPLAY S-CODCLI @ F-RUCCli WITH FRAME {&FRAME-NAME}.  
      IF SUBSTRING(S-CODCLI,1,2) = '20' THEN DO:          
          radio-set-libre_c01:SCREEN-VALUE IN FRAME {&FRAME-NAME}= 'J' .      
          RUN habilitar-campos(INPUT "J").
      END.
      ELSE DO:
          radio-set-libre_c01:SCREEN-VALUE IN FRAME {&FRAME-NAME}= 'E' .
          RUN habilitar-campos(INPUT "E").
      END.
