...
DEFINE NEW SHARED VARIABLE S-TPOFAC   AS CHAR INITIAL "A".      /* ADELANTOS */
...
&Scoped-Define ENABLED-OBJECTS RECT-20 RECT-21 RECT-22 C-CodDoc C-NroSer ~
Btn-Print 
...
DEFINE VARIABLE h_q-docmto-2ade AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-faccre-4ade AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-totales-4 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     Btn-Print AT ROW 1.31 COL 81.86
     RECT-20 AT ROW 1.04 COL 81.29
     RECT-21 AT ROW 2.35 COL 2 WIDGET-ID 2
     RECT-22 AT ROW 12.58 COL 2 WIDGET-ID 4
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 110.14 BY 14.35
         FONT 4.
...
DO:
  RUN dispatch IN h_v-faccre-4ade ('imprime':U).
END.
...
DO:
  ...
  RUN dispatch IN h_q-docmto-2ade ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv10 ).
       RUN set-position IN h_p-updv10 ( 1.04 , 43.72 ) NO-ERROR.
       RUN set-size IN h_p-updv10 ( 1.31 , 37.57 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-faccre-4ade.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-faccre-4ade ).
       RUN set-position IN h_v-faccre-4ade ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.69 , 101.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/v-totales-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-totales-4 ).
       RUN set-position IN h_v-totales-4 ( 13.12 , 6.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.62 , 101.43 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'q-docmto-2ade.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-docmto-2ade ).
       RUN set-position IN h_q-docmto-2ade ( 1.04 , 17.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-faccre-4ade. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-faccre-4ade ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto-2ade , 'Record':U , h_v-faccre-4ade ).

       /* Links to SmartViewer h_v-totales-4. */
       RUN add-link IN adm-broker-hdl ( h_q-docmto-2ade , 'Record':U , h_v-totales-4 ).

       /* Links to SmartQuery h_q-docmto-2ade. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-docmto-2ade ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv10 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-faccre-4ade ,
             Btn-Print:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-totales-4 ,
             h_v-faccre-4ade , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-docmto-2ade ,
             h_v-totales-4 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-20 RECT-21 RECT-22 C-CodDoc C-NroSer Btn-Print 
      WITH FRAME F-Main IN WINDOW W-Win.
