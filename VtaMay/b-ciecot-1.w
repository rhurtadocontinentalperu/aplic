...
&Scoped-define OPEN-QUERY-br_table CASE COMBO-BOX-1:     WHEN 'Todos' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc          AND FacCPedi.FlgEst = s-flgest NO-LOCK             ~{&SORTBY-PHRASE}.     WHEN 'Inicien con' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc          AND FacCPedi.FlgEst = s-flgest          AND FacCPedi.NomCli BEGINS x-NomCli NO-LOCK             ~{&SORTBY-PHRASE}.     WHEN 'Contengan' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc          AND FacCPedi.FlgEst = s-flgest          AND INDEX(FacCPedi.NomCli, ~
...
&Scoped-Define ENABLED-OBJECTS br_table COMBO-BOX-1 x-NomCli 
