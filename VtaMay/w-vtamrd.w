...
DEFINE VARIABLE h_b-vtamrd AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-vtamrd AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-vtamrd-1 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/v-vtamrd.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-vtamrd ).
       RUN set-position IN h_v-vtamrd ( 2.92 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.65 , 97.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/b-vtamrd.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-vtamrd ).
       RUN set-position IN h_b-vtamrd ( 7.92 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-vtamrd ( 9.19 , 104.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/v-vtamrd-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-vtamrd-1 ).
       RUN set-position IN h_v-vtamrd-1 ( 16.19 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.38 , 88.43 ) */
       ...
       /* Links to SmartViewer h_v-vtamrd. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-vtamrd ).
       RUN add-link IN adm-broker-hdl ( h_q-faccpe , 'Record':U , h_v-vtamrd ).

       /* Links to SmartBrowser h_b-vtamrd. */
       RUN add-link IN adm-broker-hdl ( h_p-updv98 , 'TableIO':U , h_b-vtamrd ).
       RUN add-link IN adm-broker-hdl ( h_q-faccpe , 'Record':U , h_b-vtamrd ).

       /* Links to SmartViewer h_v-vtamrd-1. */
       RUN add-link IN adm-broker-hdl ( h_q-faccpe , 'Record':U , h_v-vtamrd-1 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_p-updv05 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-vtamrd ,
             BUTTON-MARGEN:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-vtamrd ,
             h_v-vtamrd , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-vtamrd-1 ,
             h_b-vtamrd , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
...
------------------------------------------------------------------------------*/
  ...
  CASE p-State:
    ...
    WHEN 'calcula-totales' THEN DO:
        RUN Graba-Totales IN h_v-vtamrd.
        RUN dispatch IN h_v-vtamrd-1 ('display-fields':U).
    END.
  END CASE.
