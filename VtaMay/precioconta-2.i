...
/* rhc 16.12.2010 mejor queda bloqueado */
/*
IF LOOKUP({&Catalogo}.CodFam, '000,001,002') > 0 AND LOOKUP(x-ClfCli2, x-Cols) > 0 THEN DO:
    ...
    DEF VAR MaxVta AS DEC INIT 0 NO-UNDO.
    ...
    F-PREVTA = {&Catalogo}.PreOfi * (1 - F-DSCTOS / 100) *  F-FACTOR.
...
*/
    ...
    IF {&Catalogo}.UndB <> "" AND NOT SW-LOG1 THEN DO:
        ...
        IF (x-CanPed * F-FACTOR) >= (0.25 * X-FACTOR) THEN DO:
            ...
            X-PREVTA2 = (X-PREVTA2 / X-FACTOR) * F-FACTOR.
        END.
        /*MESSAGE {&Catalogo}.MonVta {&Catalogo}.TpoCmb x-prevta1 f-dsctos sw-log1.*/
    END.
    /****   PRECIO A    ****/
