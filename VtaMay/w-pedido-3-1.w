...
&Scoped-Define ENABLED-OBJECTS B-CCTE B-AGTRANS 
...
DEFINE VARIABLE h_b-pedido-3-1 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     B-CCTE AT ROW 1.81 COL 112
     B-AGTRANS AT ROW 2.88 COL 112 WIDGET-ID 2
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-pedido-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedido-3-1 ).
       RUN set-position IN h_v-pedido-3-1 ( 1.00 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.69 , 109.00 ) */
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-pedido-3-1 ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-pedido-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedido-3-1 ).
       RUN set-position IN h_b-pedido-3-1 ( 10.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedido-3-1 ( 10.35 , 123.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pedido-3-1. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_b-pedido-3-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pedido-3-1 ,
             h_p-updv05 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-pedido-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedido-3-1 ).
       RUN set-position IN h_t-pedido-3-1 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedido-3-1 ( 8.88 , 124.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv95 ).
       RUN set-position IN h_p-updv95 ( 19.31 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv95 ( 1.35 , 27.00 ) NO-ERROR.

       /* Links to SmartBrowser h_t-pedido-3-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv95 , 'TableIO':U , h_t-pedido-3-1 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_t-pedido-3-1 ).
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_b-pedido-3-1 <> ? THEN RUN dispatch IN h_b-pedido-3-1 ('open-query':U).
