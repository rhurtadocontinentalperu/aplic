...
&Scoped-Define ENABLED-FIELDS FacCPedi.Atencion FacCPedi.NomCli ~
FacCPedi.DirCli FacCPedi.Glosa 
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      FOR EACH FacdPedi OF FaccPedi NO-LOCK:
          ...
          IF NOT AVAILABLE B-DPedi THEN DO:
              MESSAGE 'Producto' Facdpedi.codmat 'no se pudo actualizar en la cotización' Faccpedi.nroref SKIP
                  'Grabación abortada'
...
------------------------------------------------------------------------------*/
  ...
  /* RHC 18.11.2011 solicitamos almacen de despacho en caso de expolibreria */
  IF s-coddiv = '00015' THEN DO:
      RUN vtaexp/d-almdes (s-coddiv, OUTPUT pAlmDes).
      IF pAlmDes = '' THEN RETURN.
  END.
  ...
  DO WITH FRAME {&FRAME-NAME}:  
    ...
    /* RHC 02.12.2011 Parche para Expolibreria */
    IF s-coddiv = '00015' AND COTIZACION.FmaPgo = '001' AND COTIZACION.Cmpbnte = 'BOL'
        THEN ASSIGN 
    ...
    FOR EACH FacDPedi OF COTIZACION NO-LOCK WHERE (FacDPedi.CanPed - FacDPedi.CanAte) > 0,
        ...
        pAlmDes = IF s-coddiv = '00015' THEN pAlmDes ELSE Facdpedi.AlmDes.  /* OJO */
        ...
        IF NOT AVAILABLE Almmmate THEN DO:
            MESSAGE 'Material' Facdpedi.codmat 'NO asignado al almacén' pAlmDes
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
/*  RUN vtamay/r-impped-1 (ROWID(FacCPedi)). */
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
    ...
    input-var-4 = (IF s-user-id BEGINS 'ADMIN' THEN '' ELSE s-user-id)
    output-var-1 = ?
  ...
  THEN DO:
    ...
    RETURN 'ADM-ERROR'.
  END.
  IF COTIZACION.FchPed < 12/28/2010
  THEN DO:
    MESSAGE 'Cotización con formato antiguo' SKIP
        'Copiar la cotización y repetir el proceso'
        VIEW-AS ALERT-BOX ERROR.
    RETURN 'ADM-ERROR'.
  END.
  IF TODAY >= 12/30/2010 AND TODAY <= 01/01/2011
      AND LOOKUP (COTIZACION.FmaPgo, '000,001,002,003') = 0 THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  /* NO PARA EXPOLIBRERIA, SE VAN A GENERAR PEDIDOS POR PROMOCIONES */
  IF s-coddiv <> "00015" THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    /*Verificamos que las cantidades sean correctas*/
    FOR EACH PEDI WHERE TRIM(PEDI.UndVta) = 'UNI' NO-LOCK:
    ...
    /*Verifica Importe Minimo por Linea*/

    FOR EACH PEDI NO-LOCK:
        IF PEDI.ImpLin < 5.00 THEN DO:
            MESSAGE "Importe Total de Articulo " PEDI.CodMat + " debe ser mayor a 5.00"
                VIEW-AS ALERT-BOX INFO BUTTONS OK.            
        END.
    END.
