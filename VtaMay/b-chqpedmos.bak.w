...
DO:
  ...
  FIND FacDPedm WHERE FacDPedm.codcia = s-codcia
    ...
    AND FacDPedm.canpick > 0
    NO-LOCK NO-ERROR.
  IF NOT AVAILABLE FacDPedm THEN DO:
    MESSAGE 'Artículo NO registrado en el Pedido al Mostrador' SKIP
        'o NO hay mercaderia de acuerdo al PICKING'
...
DO:
  MESSAGE 'Cerramos el Pedido (SI-NO)?' VIEW-AS ALERT-BOX QUESTION
...
DO:
  ...
  IF NOT (FacCPedm.flgest = 'C') THEN DO:
    MESSAGE 'El pedido no se puede chequear' VIEW-AS ALERT-BOX ERROR.
    SELF:SCREEN-VALUE = ''.
    RETURN NO-APPLY.
  END.
  FIND FIRST Facdpedm OF Faccpedm  WHERE Facdpedm.almdes = s-codalm NO-LOCK NO-ERROR.
  IF NOT AVAILABLE Facdpedm THEN DO:
      MESSAGE 'El pedido no presenta artículos para este almacén' VIEW-AS ALERT-BOX ERROR.
      RETURN.
  END.
  IF Facdpedm.Libre_c01 = 'B' THEN DO:
      ...
      RETURN NO-APPLY.
  END.
  IF Facdpedm.Libre_c01 <> 'P' THEN DO:
      MESSAGE 'El pedido al mostrador NO ha pasado por PICKING'
          VIEW-AS ALERT-BOX ERROR.
      SELF:SCREEN-VALUE = ''.
      RETURN NO-APPLY.
  END.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF VAR x-UsrChq LIKE Faccpedm.Libre_c01 NO-UNDO.
  ...
  FOR EACH FacDPedm NO-LOCK WHERE FacDPedm.codcia = s-codcia
    ...
    FIND C-Detalle WHERE C-Detalle.codmat = FacDPedm.codmat
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE C-Detalle THEN DO:
        MESSAGE 'No se ha registrado el artículo' SKIP 
            FacDPedm.codmat Almmmatg.desmat
            VIEW-AS ALERT-BOX ERROR.
        RETURN 'ADM-ERROR'.
    END.
    IF C-Detalle.CanPed <> FacDPedm.CanPick THEN DO:
        MESSAGE 'Diferencia de cantidades en el artículo' SKIP FacDPedm.codmat Almmmatg.desmat
            VIEW-AS ALERT-BOX ERROR.
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    THEN DO:
        ...
        FOR EACH Facdpedm OF Faccpedm WHERE Facdpedm.almdes = s-codalm:
            ASSIGN
                Facdpedm.Libre_c01 = 'B'    /* marca BARRAS */
                Facdpedm.Libre_c02 = x-usrchq
        ...
        FOR EACH ccbcdocu USE-INDEX Llave14 WHERE ccbcdocu.codcia = faccpedm.codcia
            ...
            AND ccbcdocu.codalm = s-codalm:
              ASSIGN
                  CcbCDocu.Libre_c01 = 'B'        /* Marca de barras */
                  CcbCDocu.Libre_c02 = x-usrchq
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
    ...
    Detalle.undvta = FacDPedm.undvta
  ...
  FIND FacDPedm WHERE FacDPedm.codcia = s-codcia
    AND FacDPedm.coddoc = s-coddoc
    AND FacDPedm.nroped = x-nrodoc
    AND FacDPedm.codmat = Detalle.codmat
    NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  FIND FacDPedm WHERE FacDPedm.codcia = s-codcia
    ...
    NO-LOCK NO-ERROR.
  IF AVAILABLE FacDPedm AND x-CanPed > FacDPedm.CanPick THEN DO:
    MESSAGE 'CANTIDAD ingresada SUPERA la del pedido'
        VIEW-AS ALERT-BOX ERROR.
    RETURN 'ADM-ERROR'.
  END.
