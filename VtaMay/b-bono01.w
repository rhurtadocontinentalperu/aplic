...
DEF VAR x-Tabla AS CHAR INIT 'XB' NO-UNDO.
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH FacTabla WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacTabla.Tabla = x-tabla NO-LOCK, ~
      EACH Almmmatg WHERE Almmmatg.CodCia = FacTabla.CodCia ~
  AND Almmmatg.codmat = FacTabla.Codigo NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 59 BY 12.12
         FONT 2.
...
/* Query rebuild information for BROWSE br_table
 ...
 AND INTEGRAL.FacTabla.Tabla = x-tabla"
     _JoinCode[2]      = "INTEGRAL.Almmmatg.CodCia = INTEGRAL.FacTabla.CodCia
  AND INTEGRAL.Almmmatg.codmat = INTEGRAL.FacTabla.Codigo"
     _FldNameList[1]   > INTEGRAL.FacTabla.Codigo
"FacTabla.Codigo" ? ? "character" ? ? ? ? ? ? yes ?
...
------------------------------------------------------------------------------*/
  ...
  FIND Almmmatg WHERE Almmmatg.codcia = s-codcia
    AND Almmmatg.codmat = FacTabla.Codigo:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
    NO-LOCK NO-ERROR.
