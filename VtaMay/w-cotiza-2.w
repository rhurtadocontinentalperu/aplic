...
&Scoped-Define ENABLED-OBJECTS BTN-Excel BUTTON-2 B-CCTE 
...
DEFINE VARIABLE h_t-cotiza-2 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     BUTTON-2 AT ROW 8.81 COL 104 WIDGET-ID 2
     B-CCTE AT ROW 8.88 COL 91
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 108.14 BY 20.35
         FONT 4.
...
DO:
  RUN Genera-Excel IN h_v-cotiza-2.
END.
...
DO:
  RUN Genera-Excel2 IN h_v-cotiza-2.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/v-cotiza-2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_v-cotiza-2 ).
       RUN set-position IN h_v-cotiza-2 ( 1.00 , 1.00 ) NO-ERROR.
       ...
       /* Links to SmartObject h_v-cotiza-2. */
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/t-cotiza-2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotiza-2 ).
       RUN set-position IN h_t-cotiza-2 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotiza-2 ( 8.88 , 106.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotiza-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotiza-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotiza-2 ,
             B-CCTE:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotiza-2 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ENABLE BTN-Excel BUTTON-2 B-CCTE 
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-cotiza-2 <> ? THEN RUN dispatch IN h_t-cotiza-2 ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        RUN Recalcular-Precios IN h_t-cotiza-2.
      END.
