...
DEFINE VARIABLE h_b-listaprov AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 164.43 BY 25.5 WIDGET-ID 100.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-listaprov.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-listaprov ).
       RUN set-position IN h_b-listaprov ( 2.81 , 2.14 ) NO-ERROR.
       RUN set-size IN h_b-listaprov ( 8.15 , 74.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-detcatvta.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-detcatvta ).
       RUN set-position IN h_b-detcatvta ( 11.85 , 1.43 ) NO-ERROR.
       RUN set-size IN h_b-detcatvta ( 14.00 , 163.86 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pagcatvtas. */
       RUN add-link IN adm-broker-hdl ( h_b-listaprov , 'Record':U , h_b-pagcatvtas ).

       /* Links to SmartBrowser h_b-detcatvta. */
       RUN add-link IN adm-broker-hdl ( h_b-pagcatvtas , 'Record':U , h_b-detcatvta ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-listaprov ,
             x-importe:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-pagcatvtas ,
             h_b-listaprov , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
    ...
    CASE pParameter:
        ...
        WHEN 'Open-Query-3' THEN DO:
            RUN dispatch IN h_b-detcatvta ('open-query').
        END.
    END CASE.
END PROCEDURE.
