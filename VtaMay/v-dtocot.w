...
DEFINE FRAME F-Main
     ...
     FacCPedi.CodCli AT ROW 1.96 COL 7.43 COLON-ALIGNED
          ...
          VIEW-AS FILL-IN 
          SIZE 8.86 BY .69
     FacCPedi.NomCli AT ROW 1.96 COL 17 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 39 BY .69
 ...
 *        F-DTOMAX = F-DTOMAX + Almmmatg.PorMax * PEDI.CanPed * PEDI.PreUni.
 *     END.*/
END.
/*
...
DO WITH FRAME {&FRAME-NAME}:
   ...
   FOR EACH PEDI NO-LOCK:
        POR_DSCREM = ROUND((( 1 - 
                        ...
                        ( 1 + (ABSOLUTE(PEDI.PorDto) / 100) ) 
                     ) * 100), 2).
        DSCT_REM = DSCT_REM + (PEDI.PreUni * PEDI.CanPed) * (POR_DSCREM / 100).
        X-Bruto = X-Bruto + (PEDI.PreUni * PEDI.CanPed).
 ...
 *    END.*/
  ...
  END.
   ...
   END.
    ...
    IF ((DECIMAL(Faccpedi.PorDto:SCREEN-VALUE) / 100) * X-Bruto ) - (FaccPedi.Importe[1] + FaccPedi.Importe[2]) > ((F-DTOMAX / 100) * FaccPedi.Importe[3] ) THEN DO:
         MESSAGE "Se ha excedido DSCTO. de Usuario" SKIP
                 "Pedido con Descuentos Conti-StandFord,Tarjeta" SKIP 
                 "Posible Descuento  : " ROUND((((((F-DTOMAX / 100 ) * FaccPedi.Importe[3]) + (FaccPedi.Importe[1] + FaccPedi.Importe[2]) ) / X-Bruto ) * 100 ),2) - 0.01 " %" SKIP
                 VIEW-AS ALERT-BOX ERROR TITLE "Descuento por Monto".
         APPLY "ENTRY":U TO Faccpedi.PorDto.
         RETURN "ADM-ERROR".
    END.
