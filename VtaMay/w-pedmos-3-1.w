...
DEFINE NEW SHARED TEMP-TABLE ITEM NO-UNDO LIKE Facdpedm.
DEFINE NEW SHARED TEMP-TABLE ITEM-2 NO-UNDO LIKE Facdpedm.
DEFINE NEW SHARED TEMP-TABLE ITEM-3 NO-UNDO LIKE Facdpedm.
...
DEFINE NEW SHARED VARIABLE s-coddoc-2 AS CHAR INITIAL "V/F".    /* Venta Frustrada */
...
&Scoped-Define ENABLED-OBJECTS BTN-Excel BTN-COTIZA Btn-Grupos 
...
DEFINE VARIABLE h_t-pedmos-3 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-pedmos-3 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BTN-Excel AT ROW 7.73 COL 96
     BTN-COTIZA AT ROW 18.5 COL 37
     Btn-Grupos AT ROW 18.5 COL 52 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 125.29 BY 19.96
         FONT 4.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: ITEM T "NEW SHARED" NO-UNDO INTEGRAL Facdpedm
      TABLE: ITEM-2 T "NEW SHARED" NO-UNDO INTEGRAL Facdpedm
      TABLE: ITEM-3 T "NEW SHARED" NO-UNDO INTEGRAL Facdpedm
   END-TABLES.
 */
...
ON END-ERROR OF W-Win /* PEDIDOS EN MOSTRADOR */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* PEDIDOS EN MOSTRADOR */
DO:
...
DO:
  RUN Asigna-Cotizacion IN h_v-pedmos-3.
  RUN dispatch IN h_t-pedmos-3 ('open-query':U).
END.
...
DO:
  RUN Genera-Excel IN h_v-pedmos-3.
END.
...
DO:
  RUN vtamay/d-pedmos-3.
  RUN local-open-query IN h_t-pedmos-3.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-pedmos-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedmos-3 ).
       RUN set-position IN h_v-pedmos-3 ( 1.00 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 7.73 , 11.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv02 ).
       RUN set-position IN h_p-updv02 ( 7.73 , 29.00 ) NO-ERROR.
       RUN set-size IN h_p-updv02 ( 1.54 , 61.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/q-vtamos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-vtamos ).
       RUN set-position IN h_q-vtamos ( 7.73 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-pedmos-3. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-pedmos-3 ).
       RUN add-link IN adm-broker-hdl ( h_q-vtamos , 'Record':U , h_v-pedmos-3 ).

       /* Links to SmartQuery h_q-vtamos. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-vtamos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedmos-3 ,
             BTN-Excel:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-pedmos-3 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-pedmos-2-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedmos-2 ).
       RUN set-position IN h_b-pedmos-2 ( 9.46 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedmos-2 ( 10.35 , 114.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-pedmos-2. */
       RUN add-link IN adm-broker-hdl ( h_q-vtamos , 'Record':U , h_b-pedmos-2 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-pedmos-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmos-3 ).
       RUN set-position IN h_t-pedmos-3 ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedmos-3 ( 8.88 , 123.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 18.50 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.42 , 34.14 ) NO-ERROR.

       /* Links to SmartBrowser h_t-pedmos-3. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedmos-3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedmos-3 ,
             BTN-Excel:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-pedmos-3 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ENABLE BTN-Excel BTN-COTIZA Btn-Grupos 
...
------------------------------------------------------------------------------*/
  ...
  CASE L-Handle:
    WHEN "browse" THEN DO:
          IF h_b-pedmos-2 <> ? THEN RUN dispatch IN h_b-pedmos-2 ('open-query':U).
          IF h_t-pedmos-3 <> ? THEN RUN dispatch IN h_t-pedmos-3 ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-pedmos-3 <> ? THEN RUN Recalcular-Precios IN h_t-pedmos-3.
      END.
