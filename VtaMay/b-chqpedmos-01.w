...
DEFINE TEMP-TABLE C-Detalle NO-UNDO LIKE Facdpedm.
DEFINE TEMP-TABLE Detalle NO-UNDO LIKE Facdpedm
       INDEX LLave01 AS PRIMARY CodCia CodMat.
DEFINE TEMP-TABLE T-DPEDI NO-UNDO LIKE Facdpedm.
...
DEF VAR s-coddoc AS CHAR INIT 'P/M' NO-UNDO.    /* Pedido Mostrador */
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: C-Detalle T "?" NO-UNDO INTEGRAL Facdpedm
      TABLE: Detalle T "?" NO-UNDO INTEGRAL Facdpedm
      ADDITIONAL-FIELDS:
          INDEX LLave01 AS PRIMARY CodCia CodMat
      END-FIELDS.
      TABLE: T-DPEDI T "?" NO-UNDO INTEGRAL Facdpedm
   END-TABLES.
 */
...
/*     AND Facdpedm.nroped = x-nrodoc                              */
/*     AND Facdpedm.codmat = SELF:SCREEN-VALUE                     */
/*     NO-LOCK NO-ERROR.                                           */
...
/*   DISPLAY                                                       */
/*     Almmmatg.desmat                                             */
/*     Facdpedm.undvta @ Detalle.undvta                            */
/*     pCanPed @ Detalle.CanPed                                    */
/*     WITH BROWSE {&BROWSE-NAME}.                                 */
END.
...
DO:
  ...
  IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
      MESSAGE 'NO se pudo cerrar el pedido' VIEW-AS ALERT-BOX WARNING.
      RETURN NO-APPLY.
  END.
  RUN Borra-Temporal.
...
OR RETURN OF {&SELF-NAME}
    DO:
        ...
        FIND Faccpedm WHERE Faccpedm.codcia = s-codcia 
            ...
            AND Faccpedm.nroped = x-nrodoc:SCREEN-VALUE
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE Faccpedm THEN DO:
            ...
            RETURN NO-APPLY.
        END.
        /* Debe estar facturado */
        IF Faccpedm.FlgEst <> 'C' THEN DO:
            MESSAGE 'Pedido Mostrador NO facturado' VIEW-AS ALERT-BOX ERROR.
            SELF:SCREEN-VALUE = ''.
            RETURN NO-APPLY.
        END.
        /* RHC 09.07.10 EL ALMACEN DEBE PERTENECER A LA DIVISION ACTIVA */
        DEF VAR x-Ok AS LOG INIT NO NO-UNDO.
        FOR EACH Facdpedm OF Faccpedm NO-LOCK WHERE Facdpedm.Libre_c02 <> '*',
        ...
        END.
        IF x-Ok = NO THEN DO:
            MESSAGE 'Este pedido NO tiene productos a chequear en este local' SKIP
                'o YA fue chequeado'
        ...
        IF AVAILABLE VtaTrack04 THEN DO:
            MESSAGE 'Aún no se ha cerrado el sacador' VtaTrack04.codper SKIP
                'del almacén' VtaTrack04.codalm 'del día' DATE(VtaTrack04.FechaI)
                VIEW-AS ALERT-BOX WARNING.
            RETURN NO-APPLY.
        END.
        /* ********************************* */
        IF x-NroDoc <> SELF:SCREEN-VALUE THEN DO:
            ...
            RUN dispatch IN THIS-PROCEDURE ('open-query':U).
            ASSIGN
                x-fchdoc = Faccpedm.fchped
                x-codcli = Faccpedm.codcli
                x-nomcli = Faccpedm.nomcli.
            DISPLAY
                x-fchdoc
                x-codcli
                x-nomcli
...
FOR EACH Facdpedm NO-LOCK WHERE Facdpedm.codcia = s-codcia
    ...
    AND Almacen.codalm = Facdpedm.almdes
    AND Almacen.coddiv = s-coddiv:
/*     FIND Almckits OF Facdpedm NO-LOCK NO-ERROR. */
/*     IF NOT AVAILABLE Almckits THEN DO:          */
        FIND Almmmatg OF Facdpedm NO-LOCK.
        FIND T-DPEDI WHERE T-DPEDI.codmat = Facdpedm.codmat NO-ERROR.
        IF NOT AVAILABLE T-DPEDI THEN DO:
            CREATE T-DPEDI.
            ASSIGN
                T-DPEDI.codmat = Facdpedm.codmat
                T-DPEDI.canped = Facdpedm.canped * Facdpedm.Factor
                T-DPEDI.undvta = Almmmatg.undbas.
        END.
        ELSE T-DPEDI.canped = T-DPEDI.canped + Facdpedm.canped * Facdpedm.Factor.
/*     END.                                                                                                  */
/*     ELSE DO:                                                                                              */
...
/*                     T-DPEDI.canped = AlmDKits.Cantidad * Facdpedm.canped * Facdpedm.factor                */
/*                     T-DPEDI.undvta = ALmmmatg.undbas.                                                     */
/*             END.                                                                                          */
/*             ELSE T-DPEDI.canped = T-DPEDI.canped + AlmDKits.Cantidad * Facdpedm.canped * Facdpedm.factor. */
/*         END.                                                                                              */
/*     END.                                                                                                  */
END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH T-DPEDI:
    ...
    IF C-Detalle.CanPed <> T-DPEDI.CanPed THEN DO:
        MESSAGE 'Diferencia de cantidades en el artículo' SKIP T-DPEDI.codmat Almmmatg.desmat
            VIEW-AS ALERT-BOX ERROR.
        FIND FIRST detalle OF c-detalle.                                                      
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':      
      /* Marcamos cada item como chequeado */
      FIND LAST Faccpedm WHERE Faccpedm.codcia = s-codcia
        AND Faccpedm.coddoc = s-coddoc
        AND Faccpedm.nroped = x-nrodoc
        NO-LOCK NO-ERROR.
      FOR EACH Facdpedm OF Faccpedm,
          ...
          Facdpedm.Libre_c02 = '*'.
      END.
      FIND FIRST Facdpedm OF Faccpedm WHERE Facdpedm.Libre_c02 <> '*' NO-LOCK NO-ERROR.
      IF NOT AVAILABLE Facdpedm THEN DO:
          FIND LAST Faccpedm WHERE Faccpedm.codcia = s-codcia
            AND Faccpedm.coddoc = s-coddoc
            AND Faccpedm.nroped = x-nrodoc
          ...
          ASSIGN 
              Faccpedm.flgsit = 'P'
              Faccpedm.UsrChq = x-usrchq
              Faccpedm.FchChq = TODAY
              Faccpedm.HorChq = STRING(TIME,'HH:MM').
      END.
      /* MARCAMOS LOS O/M COMO CERRADAS */
      FOR EACH B-CPEDI USE-INDEX Llave07 WHERE B-CPEDI.codcia = s-codcia
          AND B-CPEDI.codref = Faccpedm.coddoc
          AND B-CPEDI.nroref = Faccpedm.nroped:
          IF B-CPEDI.coddoc = "O/M" AND B-CPEDI.divdes = s-coddiv
          THEN B-CPEDI.FlgSit = "C".     /* Con Control de Barras */
      END.
      /* ****************************** */
      /* TRACKING */
      /* NOTA: Por cada P/M se van a generar varias FAC para este almacen */
      FOR EACH Ccbcdocu USE-INDEX Llave15 NO-LOCK WHERE Ccbcdocu.codcia = s-codcia
          AND Ccbcdocu.codped = s-coddoc
          AND Ccbcdocu.nroped = x-nrodoc
          AND Ccbcdocu.codcli = Faccpedm.codcli
          AND LOOKUP(Ccbcdocu.coddoc, 'FAC,BOL') > 0 
          AND Ccbcdocu.flgest <> 'A',
          FIRST Almacen OF Ccbcdocu NO-LOCK WHERE Almacen.coddiv = s-coddiv:
...
FOR EACH w-report WHERE w-report.task-no = s-task-no
    AND w-report.Llave-C = s-coddoc + x-nrodoc:
    /* CHEQUEAMOS EL ALMACEN */
    FIND Facdpedm WHERE Facdpedm.codcia = s-codcia
        AND Facdpedm.coddoc = s-coddoc
        AND Facdpedm.nroped = x-nrodoc
        AND Facdpedm.codmat = w-report.Campo-C[2]
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Facdpedm THEN NEXT.
    FIND Almacen WHERE Almacen.codcia = s-codcia
        AND Almacen.codalm = Facdpedm.almdes
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almacen OR Almacen.coddiv <> s-coddiv THEN NEXT.
    /* ********************** */
    CREATE Detalle.
    ASSIGN
        Detalle.CodCia = s-codcia
...
/*     AND Facdpedm.coddoc = s-coddoc                                             */
/*     AND Facdpedm.nroped = x-nrodoc                                             */
/*     AND Facdpedm.codmat = Detalle.codmat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} */
/*     NO-LOCK NO-ERROR.                                                          */
/*   IF AVAILABLE Facdpedm AND x-CanPed > Facdpedm.canped THEN DO:                */
/*     MESSAGE 'CANTIDAD ingresada SUPERA la del pedido'                          */
/*         VIEW-AS ALERT-BOX ERROR.                                               */
/*     RETURN 'ADM-ERROR'.                                                        */
/*   END.                                                                         */
