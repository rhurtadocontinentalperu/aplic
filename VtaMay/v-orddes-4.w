...
FacCPedi.Glosa FacCPedi.CodVen FacCPedi.FmaPgo FacCPedi.NroCard 
...
------------------------------------------------------------------------------*/
   ...
   FOR EACH pedi WHERE codmat = '' OR almdes = '':
       DELETE pedi.
   END.
   /* definimos cuantas divisiones estan involucradas */
   ...
   FOR EACH PEDI,
       ...
       /* GRABAMOS LA DIVISION DESTINO EN LA CABECERA */
       ASSIGN
       ...
       IF AVAILABLE T-CPEDI AND T-CPEDI.FlgEst <> "P" THEN FacCPedi.FlgEst = "E".   /* POR APROBAR */
       /* ******************************************************* */
       DELETE PEDI.
   END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      ...
      input-var-2 = "".
  RUN lkup\C-Pedido ("Pedidos Pendientes").
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      /* RESUMEN POR DIVISION DESTINO */
      RUN vtagn/p-pedresdiv (s-codcia, B-CPEDI.codalm, B-CPEDI.codmon, B-CPEDI.tpocmb, B-CPEDI.flgenv).
      /* **************************** */
      BUFFER-COPY B-CPEDI 
          EXCEPT 
            B-CPEDI.TpoPed
  ...
  /* SE VAN A GENERAR TANTAS ORDENES COMO DIVISIONES DESTINO EXISTAN */
  RUN GET-ATTRIBUTE("ADM-NEW-RECORD").
...
------------------------------------------------------------------------------*/
   ...
   IF FacCPedi.FlgEst = "A" THEN DO:
    ...
    IF FacCPedi.FlgEst = "R" THEN DO:
        MESSAGE "La orden ya fue rechazada" VIEW-AS ALERT-BOX ERROR.
        RETURN "ADM-ERROR".
     END.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE FacCPedi THEN DO WITH FRAME {&FRAME-NAME}:
     ...
     CASE Faccpedi.FlgSit:
         ...
         OTHERWISE DISPLAY "FALTA PICK" @ F-Situac WITH FRAME {&FRAME-NAME}.
     END CASE.
...
/*------------------------------------------------------------------------------
  ...
  IF LOOKUP(FacCPedi.FlgEst,"R,X,C,A") > 0    THEN RETURN "ADM-ERROR".
  IF LOOKUP(FacCPedi.FlgSit,"P,C") > 0        THEN RETURN "ADM-ERROR".
  FIND FIRST FacDPedi OF FacCPedi WHERE Facdpedi.canate > 0 NO-LOCK NO-ERROR.
  IF AVAILABLE Facdpedi THEN DO:
