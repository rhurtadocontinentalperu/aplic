...
DEFINE VARIABLE h_t-cotiza-3 AS HANDLE NO-UNDO.
...
DO:
   RUN Actualiza-datos-cliente IN h_v-cotiza-3.
END.
...
DO:
  RUN Genera-Excel2 IN h_v-cotiza-3.
END.
...
DO:
  ...
  RUN dispatch IN h_t-cotiza-3 ('open-query').
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-cotiza-3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_v-cotiza-3 ).
       RUN set-position IN h_v-cotiza-3 ( 1.00 , 1.00 ) NO-ERROR.
       ...
       /* Links to SmartObject h_v-cotiza-3. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-cotiza-3 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-3 , 'Record':U , h_v-cotiza-3 ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-size IN h_b-cotiza-2 ( 10.35 , 105.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotiza-2. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido-3 , 'Record':U , h_b-cotiza-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotiza-2 ,
             B-CCTE:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-cotiza-3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotiza-3 ).
       RUN set-position IN h_t-cotiza-3 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotiza-3 ( 8.88 , 106.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotiza-3. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotiza-3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotiza-3 ,
             B-CCTE:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotiza-3 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-cotiza-3 <> ? THEN RUN dispatch IN h_t-cotiza-3 ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        RUN Recalcular-Precios IN h_t-cotiza-3.
      END.
