...
DEFINE VARIABLE h_b-chqpedmos AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CHEQUEO DE PEDIDOS MOSTRADOR"
         HEIGHT             = 16.12
         WIDTH              = 97.14
         MAX-HEIGHT         = 32.31
         MAX-WIDTH          = 205.72
...
ON END-ERROR OF W-Win /* CHEQUEO DE PEDIDOS MOSTRADOR */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* CHEQUEO DE PEDIDOS MOSTRADOR */
DO:
...
DO:
  RUN Cierre-de-guia IN h_b-chqpedmos.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-chqpedmos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-chqpedmos ).
       RUN set-position IN h_b-chqpedmos ( 2.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-chqpedmos ( 14.00 , 95.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-chqpedmos. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-chqpedmos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-chqpedmos ,
             BUTTON-9:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_b-chqpedmos , 'AFTER':U ).
    END. /* Page 0 */
