...
&Scoped-Define ENABLED-OBJECTS RECT-2 COMBO-NroSer B-CCTE BUTTON-COT ~
...
DEFINE VARIABLE h_v-cotiza-4-1 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     RECT-2 AT ROW 2.62 COL 2 WIDGET-ID 12
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 146.14 BY 24.04
         FONT 4.
...
DO:
  RUN Importar-entre-companias IN h_v-cotiza-4-1.
  RUN dispatch IN h_t-cotiza-4-1 ('open-query':U).
END.
...
DO:
  RUN Genera-Excel2 IN h_v-cotiza-4-1.
END.
...
DO:
  RUN vtamay/d-cotiza-4.
  RUN dispatch IN h_t-cotiza-4-1 ('open-query').
END.
...
DO:
  RUN Asigna-Supermercado IN h_v-cotiza-4-1.
  RUN dispatch IN h_t-cotiza-4-1 ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-cotiza-4-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotiza-4-1 ).
       RUN set-position IN h_v-cotiza-4-1 ( 2.88 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.08 , 106.57 ) */
       ...
       /* Links to SmartViewer h_v-cotiza-4-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-cotiza-4-1 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_v-cotiza-4-1 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv02 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotiza-4-1 ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-cotiza-4-1 ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotiza-4-1 ( 13.38 , 141.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotiza-4-1. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_b-cotiza-4-1 ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_t-cotiza-4-1 ( 11.23 , 2.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-2 COMBO-NroSer B-CCTE BUTTON-COT BTN-Excel Btn-Grupos Btn-Super 
         BTN-CISSAC 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN 'IBC' THEN DO:
        RUN IBC-Diferencias IN h_t-cotiza-4-1.
    END.
    WHEN "browse" THEN DO:
