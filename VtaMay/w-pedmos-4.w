...
DEFINE VARIABLE h_t-pedmos-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-pedmos-4b AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-pedmos-4 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     Btn-Grupos AT ROW 22.81 COL 37 WIDGET-ID 2
     B-RES AT ROW 22.81 COL 57 WIDGET-ID 10
     RECT-73 AT ROW 2.62 COL 2 WIDGET-ID 16
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 137.14 BY 23.31
         FONT 4.
...
ON END-ERROR OF W-Win /* PEDIDO MOSTRADOR */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* PEDIDO MOSTRADOR */
DO:
...
DO:
  RUN Resumen-por-Division IN h_v-pedmos-4.
...
DO:
  RUN Asigna-Cotizacion IN h_v-pedmos-4.
END.
...
DO:
  RUN Genera-Excel IN h_v-pedmos-4.
END.
...
DO:
  ...
  RUN dispatch IN h_t-pedmos-4 ('open-query').
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-pedmos-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedmos-4 ).
       RUN set-position IN h_v-pedmos-4 ( 2.88 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-pedmos-4. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-pedmos-4 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_v-pedmos-4 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv02 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedmos-4 ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-pedmos-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedmos-4 ).
       RUN set-position IN h_b-pedmos-4 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-pedmos-4 ( 13.38 , 135.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-pedmos-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmos-4 ).
       RUN set-position IN h_t-pedmos-4 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedmos-4 ( 12.31 , 135.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-pedmos-4. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-pedmos-4 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedmos-4 ,
             BTN-COTIZA:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-pedmos-4 , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/t-pedmos-4b.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmos-4b ).
       RUN set-position IN h_t-pedmos-4b ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-pedmos-4b ( 12.31 , 122.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv96 ).
       RUN set-position IN h_p-updv96 ( 22.77 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv96 ( 1.54 , 26.14 ) NO-ERROR.

       /* Links to SmartBrowser h_t-pedmos-4b. */
       RUN add-link IN adm-broker-hdl ( h_p-updv96 , 'TableIO':U , h_t-pedmos-4b ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-pedmos-4b ,
             BTN-COTIZA:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv96 ,
             h_t-pedmos-4b , 'AFTER':U ).
    END. /* Page 3 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-pedmos-4 <> ? THEN RUN dispatch IN h_t-pedmos-4 ('open-query':U).
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-pedmos-4 <> ? THEN RUN Recalcular-Precios IN h_t-pedmos-4.
