...
/* buscamos la FAC o BOL original */
...
END.
IF LOOKUP(B-CDOCU.codped, 'PED,P/M') = 0 THEN RETURN.
/* puntero en posicion */
CASE B-CDOCU.CodPed:
    WHEN 'PED' THEN DO:
    ...
    WHEN 'P/M' THEN DO:
        FIND Faccpedm WHERE Faccpedm.codcia = s-codcia
            AND Faccpedm.coddoc = B-CDOCU.codped
            AND Faccpedm.nroped = B-CDOCU.nroped
            NO-LOCK NO-ERROR.
      ...
      CASE B-CDOCU.CodPed:
          WHEN 'PED' THEN DO:
              FIND FacCPedi WHERE 
                   ...
                   FacCPedi.NROPED = B-CDOCU.NROPED EXCLUSIVE-LOCK NO-ERROR. 
          ...
          WHEN 'P/M' THEN DO:
              FIND FacCPedm WHERE 
                   ...
                   FacCPedm.NROPED = B-CDOCU.NROPED EXCLUSIVE-LOCK NO-ERROR. 
             IF AVAILABLE FacCPedm THEN DO:
                 ASSIGN
                      FacCPedm.Libre_c01 = F-CodTran:SCREEN-VALUE
                      FacCPedm.Libre_c02 = F-direccion:SCREEN-VALUE
                      FacCPedm.Libre_c03 = F-lugar:SCREEN-VALUE
                      FacCPedm.Libre_c04 = F-contacto:SCREEN-VALUE
                      FacCPedm.Libre_c05 = F-horario:SCREEN-VALUE
                      FacCPedm.Libre_f01 = DATE (F-fecha:SCREEN-VALUE)
                      FacCPedm.observa   = F-observ:SCREEN-VALUE.
                 RELEASE Faccpedm.
             END.
          END.
      END CASE.
...
/*           WHEN 'P/M'                               */
/*               THEN ASSIGN                          */
/*                         F-CodTran:SENSITIVE = NO   */
/*                         F-Contacto:SENSITIVE = NO  */
/*                         F-Direccion:SENSITIVE = NO */
/*                         F-Fecha:SENSITIVE = NO     */
/*                         F-horario:SENSITIVE = NO   */
/*                         F-Lugar:SENSITIVE = NO     */
/*                         F-Nombre:SENSITIVE = NO    */
/*                         f-NomTra:SENSITIVE = NO    */
/*                         F-Observ:SENSITIVE = NO    */
/*                         F-ruc:SENSITIVE = NO.      */
/*       END CASE.                                    */
/*   END.                                             */
...
------------------------------------------------------------------------------*/
  ...
  CASE B-CDOCU.CodPed:
      ...
      END.
      WHEN 'P/M' THEN DO:
          IF FacCPedm.Observa BEGINS "- LOS PRECIOS" THEN 
              ASSIGN F-observ    = " ".
          ELSE 
              ASSIGN F-observ    = FacCPedm.Observa.

          ASSIGN
               F-CodTran   = Faccpedm.Libre_c01
               F-direccion = Faccpedm.Libre_c02
               F-lugar     = Faccpedm.Libre_c03
               F-contacto  = Faccpedm.Libre_c04
               F-horario   = Faccpedm.Libre_c05
               F-fecha     = FacCPedm.Libre_f01.
          FIND gn-prov WHERE 
               gn-prov.CodCia = PV-CODCIA AND
               gn-prov.CodPro = Faccpedm.Libre_c01 NO-LOCK NO-ERROR.
