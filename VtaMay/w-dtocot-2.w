...
DEFINE VARIABLE h_b-tdtoCo AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-dtocot.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-dtocot ).
       RUN set-position IN h_v-dtocot ( 1.00 , 1.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_q-dtoped , 'Record':U , h_v-dtocot ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/b-dtocot.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-dtocot ).
       RUN set-position IN h_b-dtocot ( 6.38 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-dtocot ( 8.88 , 106.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-tdtoco.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tdtoCo ).
       RUN set-position IN h_b-tdtoCo ( 6.12 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tdtoCo ( 8.88 , 112.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-tdtoCo. */
       RUN add-link IN adm-broker-hdl ( h_p-updv96 , 'TableIO':U , h_b-tdtoCo ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tdtoCo ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          RUN dispatch IN h_b-tdtoco ('open-query':U).
      END.
