...
DEFINE NEW SHARED VARIABLE S-TPOFAC   AS CHAR INITIAL "R".      /* VENTA NORMAL */
...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 RECT-65 RECT-20 C-CodDoc C-NroSer ~
Btn-Print 
...
DEFINE VARIABLE h_b-faccre-04 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv10 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-docmto-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-faccre-04 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-faccre-04 AS HANDLE NO-UNDO.
...
DO:
  RUN dispatch IN h_v-faccre-04 ('imprime':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-faccre-04.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-faccre-04 ).
       RUN set-position IN h_v-faccre-04 ( 2.62 , 3.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-docmto-2 ).
       RUN set-position IN h_q-docmto-2 ( 1.04 , 17.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-faccre-04. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-faccre-04 ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto-2 , 'Record':U , h_v-faccre-04 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv10 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-faccre-04 ,
             Btn-Print:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-docmto-2 ,
             h_v-faccre-04 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/b-faccre-04.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-faccre-04 ).
       RUN set-position IN h_b-faccre-04 ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-faccre-04 ( 12.85 , 107.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-faccre-04. */
       RUN add-link IN adm-broker-hdl ( h_q-docmto-2 , 'Record':U , h_b-faccre-04 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-faccre-04 ,
             h_v-faccre-04 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-faccre-04.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-faccre-04 ).
       RUN set-position IN h_t-faccre-04 ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-faccre-04 ( 12.31 , 107.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-faccre-04 ,
             h_v-faccre-04 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-1 RECT-65 RECT-20 C-CodDoc C-NroSer Btn-Print 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-faccre-04 ('open-query':U).
          RUN dispatch IN h_t-faccre-04 ('open-query':U).
      END.
