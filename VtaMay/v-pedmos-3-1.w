...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'NO' THEN DO:
    FOR EACH facdPedm OF FacCPedm NO-LOCK WHERE Facdpedm.Libre_c05 <> 'OF':
        CREATE ITEM.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:  
    ...
    IF B-CPEDM.FchPed < 12/28/2010 THEN DO:
    ...
    FOR EACH FacDPedm OF B-CPedm NO-LOCK WHERE (FacDPedm.CanPed - FacDPedm.CanAte) > 0:
        ...
        IF F-CANPED > 0 THEN DO:
            ...
            /* **************** */
            CREATE ITEM.
            BUFFER-COPY FacDPedm 
                EXCEPT FacDPedm.CanAte 
                TO ITEM
                ASSIGN 
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      FOR EACH FacDPedm OF FacCPedm WHERE Facdpedm.LIbre_c05 <> 'OF':
  ...
  FOR EACH ITEM, FIRST Almmmatg OF ITEM NO-LOCK BY ITEM.NroItm: 
      ...
      IF NOT AVAILABLE Almtconv THEN DO:
          MESSAGE 'Error en el producto' ITEM.codmat SKIP
              'Revise la tabla de equivalencia' ITEM.undvta Almmmatg.undbas
              VIEW-AS ALERT-BOX ERROR.
          RETURN "ADM-ERROR".
      END.
...
------------------------------------------------------------------------------*/
  ...
  /* PROMOCIONES */
  RUN vta2/Promociones (ROWID(Faccpedm)).
  ...
  /* REGALOS BIC */
/*   DEF VAR l-Ok AS LOG.                                                       */
/*   RUN vtagn/regalos-bic-pem (Faccpedm.coddoc, Faccpedm.nroped, OUTPUT l-Ok). */
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  IF Faccpedm.FlgEst <> "A" THEN RUN vtamay/r-impvm-2 (ROWID(Faccpedm)).
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME} :
    ...
    IF FacCpedm.Cmpbnte:screen-value = "FAC" AND FacCpedm.RucCli:SCREEN-VALUE = '' THEN DO:
        MESSAGE "El Cliente NO tiene R.U.C." VIEW-AS ALERT-BOX ERROR.
        APPLY "ENTRY" TO FacCPedm.CodCli.
        RETURN "ADM-ERROR".   
    END.      
    IF FacCpedm.Cmpbnte:screen-value = "FAC" THEN DO:
        IF LENGTH(Faccpedm.ruccli:SCREEN-VALUE) < 11 OR LOOKUP(SUBSTRING(Faccpedm.ruccli:SCREEN-VALUE,1,2), '10,20,15,17') = 0 THEN DO:
            MESSAGE 'El RUC Debe tener 11 dígitos y comenzar con 20, 10 ó 15' VIEW-AS ALERT-BOX ERROR.
            APPLY "ENTRY" TO FacCPedm.CodCli.
            RETURN "ADM-ERROR".   
        END.
        /* dígito verificador */
        DEF VAR pResultado AS CHAR.
        RUN lib/_ValRuc (Faccpedm.ruccli:SCREEN-VALUE, OUTPUT pResultado).
