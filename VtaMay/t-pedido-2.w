...
DO:
    ...
    x-AlmValido = "03A,03C,04,04A,05A,19,35,35a".
...
------------------------------------------------------------------------------*/
  ...
  FIND B-PEDI WHERE B-PEDI.codcia = s-codcia
    AND B-PEDI.coddoc = 'COT'
    AND B-PEDI.nroped = s-NroCot
    AND B-PEDI.codmat = PEDI.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
    NO-LOCK NO-ERROR.
  IF (B-PEDI.CanPed - B-PEDI.CanAte) < DECIMAL(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME})
  THEN DO:
    MESSAGE 'NO se puede pedir mas de ' STRING(B-PEDI.CanPed - B-PEDI.CanAte)
        PEDI.UndVta SKIP
        VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO PEDI.AlmDes  IN BROWSE {&BROWSE-NAME}. 
    RETURN "ADM-ERROR".
