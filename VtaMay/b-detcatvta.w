...
&Scoped-define INTERNAL-TABLES DATOS Almmmatg Almmmate
...
Almmmatg.DesMar DATOS.UndBas Almmmate.StkAct DATOS.PreAlt[4] DATOS.PorDto ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH DATOS WHERE DATOS.CodCia = AlmCatVtaC.CodCia ~
  ...
  AND Almmmatg.codmat = DATOS.codmat NO-LOCK, ~
      ...
      WHERE Almmmate.CodAlm = s-codalm NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table DATOS Almmmatg Almmmate
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 154 BY 12.92
         FONT 4
...
DEFINE FRAME F-Main
     ...
     btn-save AT ROW 3.15 COL 155.86 WIDGET-ID 6
     x-TotCan AT ROW 14 COL 98 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     x-TotImp AT ROW 14 COL 114.57 COLON-ALIGNED NO-LABEL WIDGET-ID 16
     BUTTON-1 AT ROW 14.12 COL 70 WIDGET-ID 18
     "Totales:" VIEW-AS TEXT
          SIZE 8 BY .62 AT ROW 14.19 COL 86.86 WIDGET-ID 14
     "GRABAR" VIEW-AS TEXT
          SIZE 8 BY .62 AT ROW 5.58 COL 156.57 WIDGET-ID 8
          FONT 6
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.DATOS WHERE INTEGRAL.AlmCatVtaC <external> ...,INTEGRAL.Almmmatg WHERE Temp-Tables.DATOS ...,INTEGRAL.Almmmate OF INTEGRAL.Almmmatg"
...
"INTEGRAL.Almmmatg.DesMat" ? ? "character" ? ? ? ? ? ? no ? no no "40" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.Almmmatg.DesMar
"INTEGRAL.Almmmatg.DesMar" "Marca" "X(20)" "character" ? ? ? ? ? ? no ? no no "12" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.DATOS.UndBas
"Temp-Tables.DATOS.UndBas" "Unid" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.Almmmate.StkAct
"INTEGRAL.Almmmate.StkAct" "Stock Actual" ? "decimal" ? 12 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Temp-Tables.DATOS.PreAlt[4]" "Precio!Unitario" "->>,>>>,>>9.9999" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.DATOS.PorDto
"Temp-Tables.DATOS.PorDto" "% Dscto!Incluido" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.DATOS.Por_Dsctos[2]
"Temp-Tables.DATOS.Por_Dsctos[2]" "% Dscto!Evento" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.DATOS.Por_Dsctos[3]
"Temp-Tables.DATOS.Por_Dsctos[3]" "% Dscto!Vol/Prom" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > Temp-Tables.DATOS.Libre_d01
"Temp-Tables.DATOS.Libre_d01" "Cantidad" ? "decimal" ? 9 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > Temp-Tables.DATOS.NroSec
"Temp-Tables.DATOS.NroSec" "Sec" ? "integer" ? ? ? ? ? ? no ? no no "4.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > "_<CALC>"
"ImpUnit @ ImpUnit" "Importe" "->>,>>>,>>9.9999" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > "_<CALC>"
"PreUniRef @ PreUniRef" "Precio Unit!Referencial" "->>,>>>,>>9.9999" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    Almmmate.StkAct:BGCOLOR IN BROWSE {&BROWSE-NAME} = 8.
END.
...
DO:
    ...
    Almmmate.StkAct:BGCOLOR IN BROWSE {&BROWSE-NAME} = 15.
