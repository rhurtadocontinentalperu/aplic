      ...
      FOR EACH FacDPedi OF FacCPedi NO-LOCK WHERE FacDPedi.CanAte = 0:   /* RHC 14.10.05 */
         ...
         ASSIGN
             DETA.ImpLin = DETA.CanDes * DETA.PreUni * 
                       ( 1 - DETA.Por_Dsctos[1] / 100 ) *
                       ( 1 - DETA.Por_Dsctos[2] / 100 ) *
                       ( 1 - DETA.Por_Dsctos[3] / 100 ).
         IF DETA.Por_Dsctos[1] = 0 AND DETA.Por_Dsctos[2] = 0 AND DETA.Por_Dsctos[3] = 0 
             THEN DETA.ImpDto = 0.
         ELSE DETA.ImpDto = DETA.CanDes * DETA.PreUni - DETA.ImpLin.
         ASSIGN
            DETA.ImpLin = ROUND(DETA.ImpLin, 2)
            DETA.ImpDto = ROUND(DETA.ImpDto, 2).
         IF DETA.AftIgv THEN DETA.ImpIgv = DETA.ImpLin - ROUND( DETA.ImpLin  / ( 1 + (s-PorIgv / 100) ), 4 ).
...
------------------------------------------------------------------------------*/
  ...
  IF S-CODDOC = "FAC" THEN DO:
      ...
      END CASE.
  END.    
  IF S-CODDOC = "BOL" THEN RUN vtamay/R-BOLE01-1 (ROWID(CcbCDocu)).    

  IF L-CREA THEN DO:
    FIND FIRST b-cdocu WHERE ROWID(b-cdocu) = ROWID(ccbcdocu) NO-ERROR.
    IF AVAIL b-cdocu THEN DO:        
        /* RHC 03.09.2011 Seleccionar si la impresion es por FAC o por O/D */
        IF s-coddiv = '00014' THEN DO:
            ...
            IF AVAILABLE Faccpedi THEN DO:
                ...
                IF NOT b-cdocu.FlgCbd THEN DO:
                    RUN vtamay/r-odesp-001b (ROWID(Faccpedi), Faccpedi.CodAlm,'ORIGINAL').
                    ASSIGN b-cdocu.FlgCbd = YES.
                END.
                ELSE RUN vtamay/r-odesp-001b (ROWID(Faccpedi), Faccpedi.CodAlm,'DUPLICADO').
            END.
        END.
        ELSE DO:
            /* impresion anterior */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Imprimir-Orden V-table-Win 
...
------------------------------------------------------------------------------*/
  ...
  /* RHC 01.09.2011 Crea O/D en base al PEDIDO */
  RUN vta2/orden-de-despacho ( ROWID(B-CPEDI), 'CREATE' ).
  IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
  /* *********************** */
  ...
  FOR EACH DETA BREAK BY DETA.AlmDes BY DETA.NroItm:
      IF FIRST-OF(DETA.AlmDes) OR x-NuevoDocumento = YES THEN DO:
          ...
          ELSE DO:
              BUFFER-COPY B-CPEDI 
                  ...
                  ASSIGN 
                    ...
                    CcbCDocu.DivOri = B-CPEDI.CodDiv
          ...
          END.
          /* RHC 15.08.11 correcion de grabacion */
          ASSIGN
              CcbCDocu.FlgEst = "P"
              CcbCDocu.FlgSit = ""
              CcbCDocu.FlgAte = "P"
          ...
          /* *********************************** */
          /* RHC 02.09.2011 Grabamos la O/D creada automaticamente */
          FIND LAST Faccpedi WHERE Faccpedi.codcia = s-codcia
              ...
              /*AND Faccpedi.fchped = B-CPEDI.fchped*/
              AND Faccpedi.codalm = DETA.almdes
              AND Faccpedi.flgest <> 'A'
              NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* RHC 01.09.2011 Borra O/D */
    RUN vta2/orden-de-despacho ( ROWID(Ccbcdocu), 'DELETE' ).
    IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
    /* *********************** */
...
------------------------------------------------------------------------------*/
  ...
  IF S-CODDOC = "FAC" THEN DO:
    ...
    /*WHEN '00026' THEN RUN VTA/R-IMPFAC2-2 (ROWID(CcbCDocu)).    /* Ate */*/
