...
&SCOPED-DEFINE l-lookup h_l-cotpen-2-2
...
DEFINE VARIABLE h_l-cotpen-2-2 AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.69 COL 86
     Btn_Cancel AT ROW 8.88 COL 86
     Btn_Help AT ROW 10.69 COL 86
     SPACE(3.28) SKIP(0.18)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/l-cotpen-2-2.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-cotpen-2-2 ).
       RUN set-position IN h_l-cotpen-2-2 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-cotpen-2-2 ( 10.92 , 82.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.54 , 84.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-cotpen-2-2. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-cotpen-2-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-cotpen-2-2 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-cotpen-2-2 , 'AFTER':U ).
    END. /* Page 0 */
