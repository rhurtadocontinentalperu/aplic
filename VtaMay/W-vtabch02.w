...
DEFINE VARIABLE h_b-tmpvtm02 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-vtabch AS HANDLE NO-UNDO.
...
DO:
 RUN Asignar-Articulos IN h_b-tmpvtm02.
 RUN dispatch IN h_b-tmpvtm02 ('open-query':U).
END.
...
DO:
  RUN Asigna-Cotizacion IN h_v-vtabch.
  RUN dispatch IN h_b-tmpvtm02 ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-vtabch.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-vtabch ).
       RUN set-position IN h_v-vtabch ( 1.04 , 1.29 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-vtabch. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-vtabch ).
       RUN add-link IN adm-broker-hdl ( h_q-vtamos , 'Record':U , h_v-vtabch ).

       /* Links to SmartQuery h_q-vtamos. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-vtamos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-vtabch ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-vtamo1 ).
       RUN set-position IN h_v-vtamo1 ( 15.92 , 1.14 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-tmpvtm02.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpvtm02 ).
       RUN set-position IN h_b-tmpvtm02 ( 7.96 , 1.14 ) NO-ERROR.
       RUN set-size IN h_b-tmpvtm02 ( 9.27 , 89.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-tmpvtm02. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-tmpvtm02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tmpvtm02 ,
             h_p-updv02 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_b-tmpvtm02 , 'AFTER':U ).
    END. /* Page 2 */
...
/*         RUN dispatch IN h_b-vtamos ('open-query':U).*/
         RUN dispatch IN h_b-tmpvtm02 ('open-query':U).
         RUN dispatch IN h_b-tmpvtm02 ('apply-entry':U).
         RETURN NO-APPLY.
       END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN Recalcular-Precios IN h_b-tmpvtm02.
      END.
END CASE.
...
CASE L-Handle:
    ...
    WHEN "update-Btn-Cancel" THEN DO:
           ...
           RUN dispatch IN h_b-tmpvtm02 ('open-query':U).
           RUN dispatch IN h_b-tmpvtm02 ('display-fields':U).
           RETURN NO-APPLY.
       END.
END CASE.
