...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 123 BY 6.46
         FONT 4.
...
"ITEM.Por_Dsctos[1]" "% Dscto!Manual" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  FIND Almacen WHERE Almacen.codcia = s-codcia
      ...
      NO-LOCK NO-ERROR.
  IF NOT AVAILABLE ALmacen THEN DO:
      MESSAGE 'Almacén despacho NO registrado' VIEW-AS ALERT-BOX ERROR.
      APPLY 'ENTRY' TO ITEM.AlmDes IN BROWSE {&BROWSE-NAME}.
      RETURN 'ADM-ERROR'.
  END.
  IF Almmmatg.DEC__03 > 0 AND Almacen.Campo-c[3] <> "Si" THEN DO:   /* NO PARA ALMACENES DE REMATE */
      IF x-CanPed < Almmmatg.DEC__03 THEN DO:
          MESSAGE 'Solo está autorizado a vender como mínimo' Almmmatg.DEC__03 Almmmatg.UndBas
              VIEW-AS ALERT-BOX ERROR.
          RETURN "ADM-ERROR".
      END.
  END.
  ...
  RUN vtagn/p-margen-utilidad (
      ...
      ITEM.AlmDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME},    /* Almacén de despacho */
