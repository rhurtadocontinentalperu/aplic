...
"PEDI-1.Libre_c01" "Característica" "x(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"PEDI-1.AlmDes" "Alm!Des" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON 'RETURN':U OF PEDI-1.AlmDes, PEDI-1.CodMat, PEDI-1.CanPed
DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN 
/*     PEDI-1.CodCia = Faccpedm.codcia */
/*     PEDI-1.CodDiv = Faccpedm.coddiv */
/*     PEDI-1.CodDoc = Faccpedm.coddoc */
/*     PEDI-1.NroPed = Faccpedm.nroped */
    PEDI-1.CodCia = s-CodCia
    ...
    PEDI-1.Flg_factor = IF SW-LOG1 THEN "1" ELSE "0"   /* Add by C.Q. 23/03/2000 */
    ...
    PEDI-1.ImpDto = ROUND( PEDI-1.PreUni * (PEDI-1.Por_Dsctos[1] / 100) * (IF SW-LOG1 THEN PEDI-1.CanPed ELSE (PEDI-1.CanPed * F-FACTOR)), 2 )
    PEDI-1.ImpLin = ROUND( PEDI-1.PreUni * (IF SW-LOG1 THEN PEDI-1.CanPed ELSE (PEDI-1.CanPed * F-FACTOR)) , 2 ) - PEDI-1.ImpDto.
    PEDI-1.UndVta = PEDI-1.UndVta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.

  IF PEDI-1.AftIsc 
  THEN PEDI-1.ImpIsc = ROUND(PEDI-1.PreBas * (IF SW-LOG1 THEN PEDI-1.CanPed ELSE (PEDI-1.CanPed * F-FACTOR)) * (Almmmatg.PorIsc / 100),4).
  IF PEDI-1.AftIgv 
...
/*   END.                                                                        */
  ...
  IF NOT AVAILABLE Almmmatg THEN DO:
      ...
      RETURN "ADM-ERROR".
  END.
  IF PEDI-1.AlmDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = "" THEN DO:
  ...
  FIND Almmmate WHERE Almmmate.CodCia = S-CODCIA 
      ...
      NO-LOCK NO-ERROR.
  IF NOT AVAILABLE Almmmate THEN DO:
      MESSAGE "Articulo no asignado al almacen " PEDI-1.AlmDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} 
           VIEW-AS ALERT-BOX ERROR.
      APPLY 'ENTRY':U TO PEDI-1.CodMat.
      RETURN "ADM-ERROR".
  END.
  ...
  RUN gn/Stock-Comprometido (PEDI-1.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME},
                             PEDI-1.AlmDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME},
