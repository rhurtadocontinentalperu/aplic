...
&Scoped-Define ENABLED-OBJECTS BUTTON-11 RECT-20 COMBO-NroSer BUTTON-delete ~
...
DEFINE VARIABLE h_b-guias-04 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-guias AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-guias-04 AS HANDLE NO-UNDO.
...
DO:
  RUN Asigna-Transportista IN h_v-guias-04.
END.
...
DO:
    ...
    IF rpta = YES THEN RUN dispatch IN h_v-guias-04 ('delete-record':U).
END.
...
DO:
    RUN dispatch IN h_v-guias-04 ('imprime':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-guias-04.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-guias-04 ).
       RUN set-position IN h_v-guias-04 ( 2.35 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.73 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-guias-04.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-guias-04 ).
       RUN set-position IN h_b-guias-04 ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-guias-04 ( 9.96 , 91.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-guias ).
       RUN set-position IN h_q-guias ( 1.08 , 9.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-guias-04. */
       RUN add-link IN adm-broker-hdl ( h_q-guias , 'Record':U , h_v-guias-04 ).

       /* Links to SmartBrowser h_b-guias-04. */
       RUN add-link IN adm-broker-hdl ( h_q-guias , 'Record':U , h_b-guias-04 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-guias-04 ,
             BUTTON-Exit:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-guias-04 ,
             h_v-guias-04 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-guias ,
             h_b-guias-04 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-11 RECT-20 COMBO-NroSer BUTTON-delete BUTTON-Print BUTTON-Exit 
      WITH FRAME F-Main IN WINDOW W-Win.
