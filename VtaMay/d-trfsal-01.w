...
&Scoped-define FIELDS-IN-QUERY-BROWSE-4 ITEM.codmat Almmmatg.DesMat ~
Almmmatg.DesMar Almmmatg.UndBas ITEM.CanDes 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-4 
&Scoped-define QUERY-STRING-BROWSE-4 FOR EACH ITEM NO-LOCK, ~
      EACH Almmmatg OF ITEM NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-4 OPEN QUERY BROWSE-4 FOR EACH ITEM NO-LOCK, ~
      EACH Almmmatg OF ITEM NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-4 ITEM Almmmatg
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-4 ITEM
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 82 BY 11.58
         FONT 4
         TITLE "NO PUDIERON GRABARSE POR FALTA DE STOCK" FIT-LAST-COLUMN.
...
DEFINE FRAME gDialog
     ...
     Btn_OK AT ROW 13.38 COL 4
     SPACE(72.13) SKIP(0.65)
...
"Temp-Tables.ITEM.codmat" "<Codigo>" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Temp-Tables.ITEM.CanDes" ? "ZZZ,ZZ9.9999" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-4 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME gDialog /* MATERIALES RECHAZADOS */
DO:  
