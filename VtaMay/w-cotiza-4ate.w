...
DEFINE VARIABLE h_t-cotiza-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotiza-4 AS HANDLE NO-UNDO.
...
DO:
  RUN Importar-entre-companias IN h_v-cotiza-4.
  RUN dispatch IN h_t-cotiza-4 ('open-query':U).
END.
...
DO:
  RUN Genera-Excel2 IN h_v-cotiza-4.
END.
...
DO:
  ...
  RUN dispatch IN h_t-cotiza-4 ('open-query').
END.
...
DO:
  RUN Asigna-Supermercado IN h_v-cotiza-4.
  RUN dispatch IN h_t-cotiza-4 ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-cotiza-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotiza-4 ).
       RUN set-position IN h_v-cotiza-4 ( 2.88 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-cotiza-4. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-cotiza-4 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_v-cotiza-4 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv02 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotiza-4 ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotiza-4 ).
       RUN set-position IN h_b-cotiza-4 ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotiza-4 ( 13.38 , 137.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotiza-4. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_b-cotiza-4 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-cotiza-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotiza-4 ).
       RUN set-position IN h_t-cotiza-4 ( 11.23 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotiza-4 ( 12.31 , 136.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotiza-4. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotiza-4 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotiza-4 ,
             BTN-Excel:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotiza-4 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN 'IBC' THEN DO:
        RUN IBC-Diferencias IN h_t-cotiza-4.
    END.
    WHEN "browse" THEN DO:
          IF h_b-cotiza-4 <> ? THEN RUN dispatch IN h_b-cotiza-4 ('open-query':U).
          IF h_t-cotiza-4 <> ? THEN RUN dispatch IN h_t-cotiza-4 ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-cotiza-4 <> ? THEN RUN Recalcular-Precios IN h_t-cotiza-4.
      END.
