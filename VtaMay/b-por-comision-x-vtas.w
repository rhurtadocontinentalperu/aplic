...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacTabla.Codigo COLUMN-LABEL "Familia" FORMAT "x(8)":U WIDTH 9.43
      FacTabla.Nombre FORMAT "x(60)":U WIDTH 29.43
      FacTabla.Valor[1] COLUMN-LABEL "%Norte" FORMAT "->>,>>9.99":U
      FacTabla.Valor[2] COLUMN-LABEL "%Centro" FORMAT "->>,>>9.99":U
      FacTabla.Valor[3] COLUMN-LABEL "%Oriente" FORMAT "->>,>>9.99":U
      FacTabla.Valor[4] COLUMN-LABEL "%Sur" FORMAT "->>,>>9.99":U
      FacTabla.Valor[5] COLUMN-LABEL "%Supervisor" FORMAT "->>,>>9.99":U
            WIDTH 9.43
  ENABLE
      FacTabla.Codigo
      FacTabla.Valor[1]
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 89 BY 9.04
         FONT 4.
...
"FacTabla.Codigo" "Familia" ? "character" ? ? ? ? ? ? yes ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.FacTabla.Nombre
"FacTabla.Nombre" ? "x(60)" "character" ? ? ? ? ? ? no ? no no "29.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.FacTabla.Valor[1]
"FacTabla.Valor[1]" "%Norte" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacTabla.Valor[2]
"FacTabla.Valor[2]" "%Centro" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.FacTabla.Valor[3]
"FacTabla.Valor[3]" "%Oriente" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.FacTabla.Valor[4]
"FacTabla.Valor[4]" "%Sur" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.FacTabla.Valor[5]
"FacTabla.Valor[5]" "%Supervisor" ? "decimal" ? ? ? ? ? ? yes ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FacTabla.Codigo IN BROWSE br_table /* Familia */
DO:
    DEFINE VAR lCodFam AS CHAR.

    lCodFam =   factabla.codigo:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.

    FIND FIRST integral.almtfam WHERE almtfam.codcia = s-codcia AND 
        almtfam.codfam = lCodFam NO-LOCK NO-ERROR.
    IF NOT AVAILABLE integral.almtfam THEN DO:
        MESSAGE 'Codigo de Familia no Existe...' VIEW-AS ALERT-BOX WARNING.
        RETURN NO-APPLY.
    END.
    ELSE DO:
        /*factabla.nombre:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = almtfam.desfam.*/
        factabla.nombre:SCREEN-VALUE = almtfam.desfam.
    END.
