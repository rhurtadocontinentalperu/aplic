...
DEFINE NEW SHARED VARIABLE S-CODDOC   AS CHAR INITIAL "P/B".
DEFINE NEW SHARED VARIABLE C-CODDOC   AS CHAR INITIAL "FAC".
...
IF NOT AVAILABLE FacCorre THEN DO:
   ...
   RETURN ERROR.
END.
ELSE ASSIGN S-CodAlm = FacCorre.CodAlm.
...
DEF SHARED VAR s-codter LIKE ccbcterm.codter.
  ...
  FIND FacDocum WHERE facdocum.codcia = s-codcia 
                 AND  facdocum.coddoc = C-CODDOC
                NO-LOCK NO-ERROR.
  IF NOT AVAILABLE FacDocum THEN DO:
      MESSAGE "No esta definido el documento" C-CODDOC VIEW-AS ALERT-BOX ERROR.
      RETURN "ADM-ERROR".
  END.
...
&Scoped-Define ENABLED-OBJECTS Btn-Cotiza Btn-Asigna 
...
DEFINE VARIABLE h_b-tmpvtm03 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-vtabch03 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     Btn-Cotiza AT ROW 17.5 COL 78.86
     Btn-Asigna AT ROW 17.54 COL 69
...
ON END-ERROR OF W-Win /* Ventas Manuales */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Ventas Manuales */
DO:
...
DO:
 RUN Asignar-Articulos IN h_b-tmpvtm03.
 RUN dispatch IN h_b-tmpvtm03 ('open-query':U).
END.
...
DO:
  RUN Asigna-Cotizacion IN h_v-vtabch03.
  RUN dispatch IN h_b-tmpvtm03 ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-vtabch03.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-vtabch03 ).
       RUN set-position IN h_v-vtabch03 ( 1.04 , 1.29 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 6.54 , 10.43 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 1.35 , 21.72 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv02 ).
       RUN set-position IN h_p-updv02 ( 6.54 , 32.29 ) NO-ERROR.
       RUN set-size IN h_p-updv02 ( 1.35 , 56.43 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/q-vtamos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-vtamos ).
       RUN set-position IN h_q-vtamos ( 6.62 , 1.14 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-vtabch03. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-vtabch03 ).
       RUN add-link IN adm-broker-hdl ( h_q-vtamos , 'Record':U , h_v-vtabch03 ).

       /* Links to SmartQuery h_q-vtamos. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-vtamos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-vtabch03 ,
             Btn-Cotiza:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-vtabch03 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-vtamos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-vtamos ).
       RUN set-position IN h_b-vtamos ( 7.96 , 1.14 ) NO-ERROR.
       RUN set-size IN h_b-vtamos ( 7.92 , 88.86 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-vtamo1 ).
       RUN set-position IN h_v-vtamo1 ( 16.04 , 1.86 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-tmpvtm03.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpvtm03 ).
       RUN set-position IN h_b-tmpvtm03 ( 7.96 , 1.14 ) NO-ERROR.
       RUN set-size IN h_b-tmpvtm03 ( 9.27 , 89.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 17.38 , 29.14 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.31 , 38.43 ) NO-ERROR.

       /* Links to SmartBrowser h_b-tmpvtm03. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-tmpvtm03 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tmpvtm03 ,
             h_p-updv02 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_b-tmpvtm03 , 'AFTER':U ).
    END. /* Page 2 */
...
/*         RUN dispatch IN h_b-vtamos ('open-query':U).*/
         RUN dispatch IN h_b-tmpvtm03 ('open-query':U).
         RUN dispatch IN h_b-tmpvtm03 ('apply-entry':U).
         RETURN NO-APPLY.
       END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
         RUN Recalcular-Precios IN h_b-tmpvtm03.
      END.
END CASE.
...
CASE L-Handle:
    ...
    WHEN "update-Btn-Cancel" THEN DO:
           ...
           RUN dispatch IN h_b-tmpvtm03 ('open-query':U).
           RUN dispatch IN h_b-tmpvtm03 ('display-fields':U).
           RETURN NO-APPLY.
       END.
END CASE.
