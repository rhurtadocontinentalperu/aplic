...
DEF VAR x-Evento AS CHAR FORMAT 'x(15)' NO-UNDO.
...
&SCOPED-DEFINE CONDICION ( faccpedi.codcia = INTEGER(input-var-1) ~
AND faccpedi.coddoc = input-var-2 ~
AND faccpedi.coddiv = input-var-3 ~
AND faccpedi.flgest = 'P' ~
AND faccpedi.fchven >= TODAY ~
AND faccpedi.usuario BEGINS input-var-4 )
...
FacCPedi.fchven FacCPedi.NomCli fEvento() @ x-Evento 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fEvento L-table-Win 
FUNCTION fEvento RETURNS CHARACTER
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      fEvento() @ x-Evento COLUMN-LABEL "Evento" WIDTH 11.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 80.14 BY 8.62
...
"NroPed" "<<Cotizacion>>" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.FacCPedi.FchPed
     _FldNameList[3]   > INTEGRAL.FacCPedi.fchven
"fchven" ? "99/99/9999" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > INTEGRAL.FacCPedi.NomCli
"NomCli" "Cliente" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"fEvento() @ x-Evento" "Evento" ? ? ? ? ? ? ? ? no ? no no "11.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fEvento L-table-Win 
FUNCTION fEvento RETURNS CHARACTER
...
IF Faccpedi.fchped >= 01/01/2011 AND Faccpedi.UsrSac <> '*' THEN RETURN "EXPO ENE 2011".
IF Faccpedi.fchped < 01/01/2011 OR Faccpedi.UsrSac = "*" THEN RETURN "EXPO OCT 2010".
