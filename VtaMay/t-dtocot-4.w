...
PEDI.PorDto2 PEDI.Por_Dsctos[1] PEDI.PreUni PEDI.ImpLin 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 139 BY 9.15
         BGCOLOR 15 FONT 4.
...
DEFINE FRAME F-Main
     ...
     F-TotBrt AT ROW 11.27 COL 19 NO-LABEL
     F-ImpExo AT ROW 11.27 COL 31 NO-LABEL
     F-ValVta AT ROW 11.27 COL 53.14 COLON-ALIGNED NO-LABEL
     F-ImpIsc AT ROW 11.27 COL 65.86 COLON-ALIGNED NO-LABEL
     F-ImpIgv AT ROW 11.27 COL 78.43 COLON-ALIGNED NO-LABEL
     F-ImpTot AT ROW 11.27 COL 91 COLON-ALIGNED NO-LABEL
     F-ImpDes AT ROW 11.31 COL 41 COLON-ALIGNED NO-LABEL
     "T.Exonerado" VIEW-AS TEXT
          SIZE 9.43 BY .5 AT ROW 10.69 COL 31.43
     "T.Descuento" VIEW-AS TEXT
          SIZE 9.43 BY .5 AT ROW 10.73 COL 43.57
     "I.S.C." VIEW-AS TEXT
          SIZE 4.57 BY .5 AT ROW 10.69 COL 71.57
     "Valor Venta" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 10.73 COL 56.86
     "Total Importe" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 10.69 COL 93.14
     "Total Bruto" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 10.69 COL 20.86
     "I.G.V." VIEW-AS TEXT
          SIZE 5.14 BY .5 AT ROW 10.69 COL 83.14
...
"PEDI.PorDto" "% Dscto. 1" "->>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.PEDI.PorDto2
"PEDI.PorDto2" "% Dscto. 2" "->>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Temp-Tables.PEDI.Por_Dsctos[1]
"PEDI.Por_Dsctos[1]" "% Dscto.!Manual" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON RETURN OF PEDI.Por_Dsctos[1] DO:
    APPLY 'TAB':U.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  END CASE. 
    ...
    IF S-CODMON = 1 THEN DO:
        IF Almmmatg.MonVta = 1 THEN
            ASSIGN PRE2 = Almmmatg.Ctotot * PEDI.factor.
        ELSE
            ASSIGN PRE2 = Almmmatg.CtoTot * Almmmatg.TpoCmb * PEDI.Factor.
    END.
  ...
  PRE1 = ROUND(PEDI.Preuni * (1 - F-DSCTO / 100) , 4).

  IF PRE2 > PRE1 THEN DO:      
     MESSAGE "Se ha excedido DSCTO. del Articulo :" SKIP
             "Precio obtenido por debajo del costo " 
              VIEW-AS ALERT-BOX ERROR TITLE "Descuento por PEDI".             
