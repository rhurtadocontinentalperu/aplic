...
&Scoped-Define ENABLED-OBJECTS BTN-Excel RECT-70 COMBO-NroSer Btn-Grupos 
...
DEFINE VARIABLE h_t-cotmos-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotmos-4 AS HANDLE NO-UNDO.
...
DO:
  RUN Genera-Excel2 IN h_v-cotmos-4.
END.
...
DO:
  ...
  RUN dispatch IN h_t-cotmos-4 ('open-query').
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-cotmos-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotmos-4 ).
       RUN set-position IN h_v-cotmos-4 ( 2.88 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-cotmos-4. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-cotmos-4 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-4 , 'Record':U , h_v-cotmos-4 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv02 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-cotmos-4 ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotiza-4 ).
       RUN set-position IN h_b-cotiza-4 ( 10.42 , 2.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotiza-4 ,
             h_v-cotmos-4 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-cotmos-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotmos-4 ).
       RUN set-position IN h_t-cotmos-4 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotmos-4 ( 12.31 , 136.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotmos-4. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotmos-4 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotmos-4 ,
             h_v-cotmos-4 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotmos-4 , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BTN-Excel RECT-70 COMBO-NroSer Btn-Grupos 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-cotmos-4 <> ? THEN RUN dispatch IN h_t-cotmos-4 ('open-query':U).
      END.
