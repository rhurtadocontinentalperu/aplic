...
FIND Facdpedi WHERE ROWID(Facdpedi) = pRowid NO-LOCK.
...
DO:
    ...
    FIND Almacen WHERE Almacen.codcia = FacDPedi.codcia
        AND Almacen.codalm = FacDPedi.almdes NO-LOCK NO-ERROR.
    ...
    RUN vtagn/Stock-Comprometido (Facdpedi.CodMat, FILL-IN-CodAlm, OUTPUT s-StkComprometido).
    s-StkDis = x-StkAct - s-StkComprometido.
    IF s-StkDis < Facdpedi.CanPed * Facdpedi.factor THEN DO:
    ...
    IF NOT AVAILABLE FacDPedi THEN RETURN NO-APPLY.
    ASSIGN
        FacDPedi.almdes = FILL-IN-CodAlm.
    RELEASE FacDPedi.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      FILL-IN-CodAlm = Facdpedi.AlmDes.
  FIND Almacen WHERE Almacen.codcia = Facdpedi.codcia
      AND Almacen.codalm = Facdpedi.almdes NO-LOCK.
  ASSIGN
