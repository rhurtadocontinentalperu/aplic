...
&Scoped-define INTERNAL-TABLES FacCPedm gn-clie gn-ConVt
...
FacCPedm.CodVen FacCPedm.FchPed FacCPedm.fchven X-VTA @ X-VTA X-MON @ X-MON ~
FacCPedm.ImpTot X-STA @ X-STA 
...
&Scoped-define QUERY-STRING-br_table FOR EACH FacCPedm WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacCPedm.CodVen BEGINS S-CODVEN  ~
 AND FacCPedm.Nomcli BEGINS wclient ~
 AND FacCPedm.FlgEst BEGINS f-estado  NO-LOCK, ~
      FIRST gn-clie WHERE gn-clie.CodCia = CL-CODCIA  ~
  AND gn-clie.CodCli = FacCPedm.CodCli NO-LOCK, ~
      FIRST gn-ConVt WHERE gn-ConVt.Codig = FacCPedm.FmaPgo NO-LOCK ~
    BY FacCPedm.NroPed DESCENDING
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH FacCPedm WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacCPedm.FlgEst BEGINS f-estado  NO-LOCK, ~
  ...
  AND gn-clie.CodCli = FacCPedm.CodCli NO-LOCK, ~
      FIRST gn-ConVt WHERE gn-ConVt.Codig = FacCPedm.FmaPgo NO-LOCK ~
    BY FacCPedm.NroPed DESCENDING.
&Scoped-define TABLES-IN-QUERY-br_table FacCPedm gn-clie gn-ConVt
...
DEFINE VARIABLE COMBO-BOX-5 AS CHARACTER FORMAT "X(256)":U INITIAL "Todos" 
     LABEL "Condición" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "Todos","Pendiente","Cancelado","Anulado" 
     DROP-DOWN-LIST
     SIZE 11.57 BY 1
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacCPedm.NroPed COLUMN-LABEL "Cotización" FORMAT "XXX-XXXXXXXX":U
      FacCPedm.NomCli FORMAT "x(40)":U
      FacCPedm.CodVen COLUMN-LABEL "Vend." FORMAT "XXXX":U
      FacCPedm.FchPed COLUMN-LABEL "    Fecha    !   Emisión" FORMAT "99/99/9999":U
      FacCPedm.fchven COLUMN-LABEL "    Fecha    !    Vcmto." FORMAT "99/99/9999":U
      X-VTA @ X-VTA COLUMN-LABEL "Tip. !Vta." FORMAT "XXXX":U
      X-MON @ X-MON COLUMN-LABEL "Mon." FORMAT "x(4)":U
      FacCPedm.ImpTot FORMAT "->>,>>>,>>9.99":U
      X-STA @ X-STA COLUMN-LABEL "Estado" FORMAT "XXXX":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 88.14 BY 12.88
         FONT 4.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.FacCPedm,INTEGRAL.gn-clie WHERE INTEGRAL.FacCPedm ...,INTEGRAL.gn-ConVt WHERE INTEGRAL.FacCPedm ..."
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ", FIRST, FIRST"
     _OrdList          = "INTEGRAL.FacCPedm.NroPed|no"
     _Where[1]         = "integral.FacCPedm.CodCia = S-CODCIA
 AND integral.FacCPedm.CodDiv BEGINS S-CODDIV
 AND integral.FacCPedm.CodDoc BEGINS S-CODDOC
 AND integral.FacCPedm.CodVen BEGINS S-CODVEN 
 AND integral.FacCPedm.Nomcli BEGINS wclient
 AND integral.FacCPedm.FlgEst BEGINS f-estado "
     _JoinCode[2]      = "integral.gn-clie.CodCia = CL-CODCIA 
  AND integral.gn-clie.CodCli = integral.FacCPedm.CodCli"
     _JoinCode[3]      = "integral.gn-ConVt.Codig = integral.FacCPedm.FmaPgo"
     _FldNameList[1]   > integral.FacCPedm.NroPed
"FacCPedm.NroPed" "Cotización" "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.FacCPedm.NomCli
"FacCPedm.NomCli" ? "x(40)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.FacCPedm.CodVen
"FacCPedm.CodVen" "Vend." "XXXX" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > integral.FacCPedm.FchPed
"FacCPedm.FchPed" "    Fecha    !   Emisión" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > integral.FacCPedm.fchven
"FacCPedm.fchven" "    Fecha    !    Vcmto." "99/99/9999" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"X-VTA @ X-VTA" "Tip. !Vta." "XXXX" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > "_<CALC>"
"X-MON @ X-MON" "Mon." "x(4)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   = integral.FacCPedm.ImpTot
...
DO:
/*  RUN vta\d-peddet.r(FacCPedm.nroped,gn-clie.nomcli).
    RUN vta\d-peddet.r(FacCPedm.nroped,FacCPedm.nomcli,"COT").*/
  
    RUN vtamay/d-pedcon-2.r(FacCPedm.nroped,"COT").
...
ON LEAVE OF wcotiza IN FRAME F-Main /* Cotización # */
or "RETURN":U of wcotiza
DO:
  IF INPUT wcotiza = "" THEN RETURN.
  find first FacCPedm where FacCPedm.NroPed = substring(wcotiza:screen-value,1,3) +
             string(integer(substring(wcotiza:screen-value,5,6)),"999999") AND
             integral.FacCPedm.CodDoc = "COT" AND
             integral.FacCPedm.Codven BEGINS S-CODVEN AND
