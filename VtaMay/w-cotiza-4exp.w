...
&Scoped-Define ENABLED-OBJECTS BUTTON-2 COMBO-NroSer 
...
DEFINE VARIABLE h_q-cotexp-4 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BTN-Excel AT ROW 1 COL 97 WIDGET-ID 10
     BUTTON-2 AT ROW 1.19 COL 102 WIDGET-ID 12
     COMBO-NroSer AT ROW 1.27 COL 2 NO-LABEL WIDGET-ID 4
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 138.43 BY 24.88
         FONT 4.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "COTIZACION EXPOLIBRERIA OCT 2010"
         HEIGHT             = 24.88
         WIDTH              = 138.43
         MAX-HEIGHT         = 25.58
         MAX-WIDTH          = 138.43
         VIRTUAL-HEIGHT     = 25.58
         VIRTUAL-WIDTH      = 138.43
         RESIZE             = no
         SCROLL-BARS        = no
         STATUS-AREA        = no
         BGCOLOR            = ?
...
ON END-ERROR OF W-Win /* COTIZACION EXPOLIBRERIA OCT 2010 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* COTIZACION EXPOLIBRERIA OCT 2010 */
DO:
...
DO:
  RUN Genera-Excel IN h_v-cotiza-4.
END.
...
DO:
    ...
    RUN dispatch IN h_q-cotexp-4 ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-cotiza-4exp.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotiza-4 ).
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtaexp/q-cotexp-4.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-cotexp-4 ).
       RUN set-position IN h_q-cotexp-4 ( 1.00 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.35 , 9.00 ) */

       /* Links to SmartViewer h_v-cotiza-4. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-cotiza-4 ).
       RUN add-link IN adm-broker-hdl ( h_q-cotexp-4 , 'Record':U , h_v-cotiza-4 ).

       /* Links to SmartQuery h_q-cotexp-4. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-cotexp-4 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-cotexp-4 ,
             h_v-cotiza-4 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-cotiza-4exp.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotiza-4 ).
       RUN set-position IN h_b-cotiza-4 ( 12.04 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotiza-4 ( 13.38 , 137.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotiza-4. */
       RUN add-link IN adm-broker-hdl ( h_q-cotexp-4 , 'Record':U , h_b-cotiza-4 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-cotiza-4exp.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotiza-4 ).
       RUN set-position IN h_t-cotiza-4 ( 12.04 , 2.00 ) NO-ERROR.
