...
DEFINE VARIABLE h_v-factur-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     C-CodDoc AT ROW 6.5 COL 1 NO-LABEL
     C-NroSer AT ROW 6.5 COL 8.57 NO-LABEL
     Btn-Print AT ROW 6.65 COL 79.57
     Btn-Guias AT ROW 7.96 COL 78.29
     Btn-Pedido AT ROW 8.92 COL 78.29
     RECT-20 AT ROW 6.38 COL 79.14
...
DO:
  RUN Asigna-Guias IN h_v-factur-01.
END.
...
DO:
    RUN Asigna-Pedidos IN h_v-factur-01.
END.
...
DO:
  ...
  IF p-Ok = 'YES'
  THEN RUN Imprimir IN h_v-factur-01 (INPUT YES).
  ELSE RUN Imprimir IN h_v-factur-01 (INPUT NO).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-factur-01.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-factur-01 ).
       RUN set-position IN h_v-factur-01 ( 1.00 , 1.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 6.38 , 25.14 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv10 ).
       RUN set-position IN h_p-updv10 ( 6.38 , 41.72 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-docmto ).
       RUN set-position IN h_q-docmto ( 6.38 , 15.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartViewer h_v-factur-01. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-factur-01 ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-factur-01 ).

       /* Links to SmartQuery h_q-docmto. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-docmto ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-factur-01 ,
             C-CodDoc:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-factur-01 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-docmto ).
       RUN set-position IN h_b-docmto ( 7.73 , 1.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-factur-1 ).
       RUN set-position IN h_v-factur-1 ( 15.42 , 2.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_v-factur-01 , 'Record':U , h_v-factur-1 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/b-tmpdoc.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpdoc ).
       RUN set-position IN h_b-tmpdoc ( 7.73 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpdoc ( 9.31 , 76.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-20 C-CodDoc C-NroSer Btn-Print Btn-Guias Btn-Pedido 
      WITH FRAME F-Main IN WINDOW W-Win.
