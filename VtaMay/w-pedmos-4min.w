...
DO:
  ...
  RUN Cancelar-Pedido IN h_v-pedmos-4.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-pedmos-4min-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedmos-4 ).
       ...
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-pedmos-4 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/t-pedmos-4min.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-pedmos-4min ).
       RUN set-position IN h_t-pedmos-4min ( 11.23 , 2.00 ) NO-ERROR.
...
/*       END.                                            */
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-pedmos-4min <> ? THEN RUN Recalcular-Precios IN h_t-pedmos-4min.
