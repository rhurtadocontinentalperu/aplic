...
&SCOPED-DEFINE CONDICION ( AlmCatVtaC.codcia = INTEGER(input-var-1) ) 

&SCOPED-DEFINE CODIGO faccpedi.nroped

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( AlmCatVtaC.DesPag BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( AlmCatVtaC.DesPag , FILL-IN-filtro ) <> 0 )
...
&Scoped-define QUERY-STRING-br_table FOR EACH AlmCatVtaC WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH AlmCatVtaC WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} NO-LOCK.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      AlmCatVtaC.NroPag FORMAT "9999":U WIDTH 8
      AlmCatVtaC.DesPag FORMAT "x(30)":U WIDTH 40
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 56.14 BY 8.62
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.AlmCatVtaC"
     _Options          = "NO-LOCK KEY-PHRASE"
     _Where[1]         = "{&CONDICION}"
     _FldNameList[1]   > INTEGRAL.AlmCatVtaC.NroPag
"NroPag" ? ? "integer" ? ? ? ? ? ? no ? no no "8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.AlmCatVtaC.DesPag
"DesPag" ? ? "character" ? ? ? ? ? ? no ? no no "40" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = STRING({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.nropag)
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.despag.
