...
DEFINE VARIABLE h_t-dtocot-4-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-dtocot-4-1 AS HANDLE NO-UNDO.
...
DO:
  RUN dispatch IN h_v-dtocot-4-1 ('imprime':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-dtocot-4-1.r':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-dtocot-4-1 ).
       RUN set-position IN h_v-dtocot-4-1 ( 2.35 , 2.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-dtocot-4-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_v-dtocot-4-1 ).
       RUN add-link IN adm-broker-hdl ( h_q-dtocot-4 , 'Record':U , h_v-dtocot-4-1 ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-cotiza-4-1 ( 6.12 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-cotiza-4-1 ( 13.38 , 141.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotiza-4-1. */
       RUN add-link IN adm-broker-hdl ( h_q-dtocot-4 , 'Record':U , h_b-cotiza-4-1 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-dtocot-4-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-dtocot-4-1 ).
       RUN set-position IN h_t-dtocot-4-1 ( 6.12 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-dtocot-4-1 ( 11.50 , 142.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-dtocot-4-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv96 , 'TableIO':U , h_t-dtocot-4-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-dtocot-4-1 ,
             h_v-dtocot-4-1 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv96 ,
             h_t-dtocot-4-1 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-cotiza-4-1 ('open-query':U).
          RUN dispatch IN h_t-dtocot-4-1 ('open-query':U).
      END.
