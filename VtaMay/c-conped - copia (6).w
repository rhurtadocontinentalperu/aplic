...
FOR EACH almacen NO-LOCK WHERE almacen.codcia = s-codcia
    ...
    FOR EACH FacDPedi NO-LOCK WHERE FacDPedi.CodCia = s-codcia 
        ...
        IF Faccpedi.FchVen < TODAY THEN NEXT.    /* OJO */
    ...
    DO j = 1 TO 2:
        DO k = 1 TO 4:
            FOR EACH Almcrepo NO-LOCK WHERE Almcrepo.codcia = s-codcia
                ...
                EACH Almdrepo NO-LOCK WHERE Almdrepo.codcia = s-codcia
...
FOR EACH almacen NO-LOCK WHERE almacen.codcia = s-codcia
    ...
    EACH almmmate NO-LOCK WHERE almmmate.codcia = s-codcia
        AND almmmate.codalm = almacen.codalm
        AND almmmate.codmat = s-codmat:
    FOR EACH gre_detail NO-LOCK WHERE gre_detail.reserva_stock = "SI" AND 
            gre_detail.codmat = s-CodMat,
        FIRST gre_header NO-LOCK WHERE gre_detail.ncorrelativo = gre_header.ncorrelatio AND 
            gre_header.m_codalm = Almacen.CodAlm:
        CREATE tmp-tabla.
        ASSIGN 
            t-CodAlm = Almacen.CodAlm
            t-CodDoc = "PGRE"
