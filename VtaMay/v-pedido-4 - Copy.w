...
FacCPedi.Glosa FacCPedi.FlgIgv FacCPedi.CodVen FacCPedi.FmaPgo ~
...
DEFINE FRAME F-Main
     ...
     FacCPedi.FchPed AT ROW 1 COL 104 COLON-ALIGNED
          LABEL "Fecha"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     ...
     FacCPedi.NomCli AT ROW 1.81 COL 22 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 57 BY .81
     FacCPedi.TpoCmb AT ROW 1.81 COL 104 COLON-ALIGNED
          LABEL "T/  Cambio"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     FacCPedi.DirCli AT ROW 2.62 COL 9 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 70 BY .81
     FacCPedi.fchven AT ROW 2.62 COL 104 COLON-ALIGNED
     ...
     FacCPedi.Atencion AT ROW 3.42 COL 29 COLON-ALIGNED WIDGET-ID 4
          ...
          SIZE 13 BY .81
          BGCOLOR 11 FGCOLOR 9 
     FacCPedi.ordcmp AT ROW 3.42 COL 104 COLON-ALIGNED
          LABEL "O/Compra"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
          BGCOLOR 11 FGCOLOR 9 
     ...
     FacCPedi.Cmpbnte AT ROW 4.23 COL 104 COLON-ALIGNED
          LABEL "Comprobante"
     ...
     FacCPedi.LugEnt AT ROW 5.04 COL 9 COLON-ALIGNED WIDGET-ID 20
          ...
          VIEW-AS FILL-IN 
          SIZE 70 BY .81
     FacCPedi.CodMon AT ROW 5.04 COL 106 NO-LABEL
...
"US$", 2
     ...
     FacCPedi.Glosa AT ROW 5.85 COL 6.28 FORMAT "X(60)"
          ...
          SIZE 70 BY .81
          BGCOLOR 11 FGCOLOR 9 
     FacCPedi.FlgIgv AT ROW 5.85 COL 106 NO-LABEL WIDGET-ID 28
          VIEW-AS RADIO-SET HORIZONTAL
          RADIO-BUTTONS 
                    "Si", yes,
"No", no
...
DEFINE FRAME F-Main
     ...
     FacCPedi.NroRef AT ROW 9.08 COL 9 COLON-ALIGNED WIDGET-ID 10
          LABEL "Cotizacion"
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
          BGCOLOR 15 FGCOLOR 12 
     "Moneda:" VIEW-AS TEXT
          SIZE 7 BY .5 AT ROW 5.31 COL 99
     "Con IGV:" VIEW-AS TEXT
          SIZE 6.43 BY .81 AT ROW 5.85 COL 99 WIDGET-ID 26
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1 SCROLLABLE 
         FONT 4.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Facdpedi OF B-CPEDI NO-LOCK WHERE (Facdpedi.CanPed - Facdpedi.CanAte) > 0,
      ...
      /* BARREMOS LOS ALMACENES VALIDOS */
      ALMACENES:
      DO i = 1 TO NUM-ENTRIES(s-CodAlm):
          ...
          ELSE DO:    /* EMPAQUE OTROS */
              IF s-FlgEmpaque = YES AND Almmmatg.DEC__03 > 0 THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    chWorkSheet = chExcelApplication:Sheets:PEDI(1).
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH PEDI, FIRST Almmmatg OF PEDI NO-LOCK BY PEDI.NroItm: 
      ...
      IF s-StkDis < x-CanPed THEN DO:
          ...
          ELSE DO:    /* EMPAQUE OTROS */
              IF s-FlgEmpaque = YES AND Almmmatg.DEC__03 > 0 THEN DO:
                  PEDI.CanPed = (TRUNCATE((PEDI.CanPed / Almmmatg.DEC__03),0) * Almmmatg.DEC__03).
              END.
          END.
...
------------------------------------------------------------------------------*/
  ...
  MESSAGE 'Entrega DELIVERY?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE s-FlgEnv.
  s-codalm = "".
