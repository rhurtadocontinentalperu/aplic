...
/*     NO-LOCK NO-ERROR.                            */
  ...
  IF NOT AVAILABLE Faccpedi THEN DO:
    ...
    RETURN NO-APPLY.
  END.
  IF NOT (Faccpedi.flgest = 'P' AND Faccpedi.flgsit = 'X') THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Facdpedi NO-LOCK WHERE Facdpedi.codcia = s-codcia
    ...
    FIND C-Detalle WHERE C-Detalle.codmat = Facdpedi.codmat
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE C-Detalle THEN DO:
        MESSAGE 'No se ha registrado el artículo' SKIP 
            Facdpedi.codmat Almmmatg.desmat
            VIEW-AS ALERT-BOX ERROR.
        RETURN 'ADM-ERROR'.
    END.
    IF C-Detalle.CanPed <> Facdpedi.CanPed THEN DO:
        MESSAGE 'Diferencia de cantidades en el artículo' SKIP Facdpedi.codmat Almmmatg.desmat
            VIEW-AS ALERT-BOX ERROR.
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    IF Faccpedi.flgest = 'P' AND Faccpedi.flgsit = 'X'
    THEN DO:
