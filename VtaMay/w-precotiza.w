...
DEFINE VARIABLE h_b-precotiza AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-precotiza.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-precotiza ).
       RUN set-position IN h_b-precotiza ( 12.31 , 2.57 ) NO-ERROR.
       RUN set-size IN h_b-precotiza ( 10.65 , 130.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-precotiza. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido2x , 'Record':U , h_b-precotiza ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-precotiza ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-cotexp-2x-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
...
CASE L-Handle:
    ...
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
         ...
         RUN adm-open-query IN h_b-precotiza.
      END.
