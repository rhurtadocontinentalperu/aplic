...
DEFINE VARIABLE h_b-tmpdoc-3 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-factur-3 AS HANDLE NO-UNDO.
...
DO:
  ...
  /*RUN Imprimir IN h_v-factur (INPUT NO).*/
  IF p-Ok = 'YES'
  THEN RUN Imprimir IN h_v-factur-3 (INPUT YES).
  ELSE RUN Imprimir IN h_v-factur-3 (INPUT NO).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-factur-3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-factur-3 ).
       RUN set-position IN h_v-factur-3 ( 1.00 , 2.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-factur-3. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-factur-3 ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-factur-3 ).

       /* Links to SmartQuery h_q-docmto. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-docmto ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-factur-3 ,
             C-CodDoc:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-factur-3 , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/b-docmto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-docmto ).
       RUN set-position IN h_b-docmto ( 10.15 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-docmto ( 7.65 , 85.86 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_b-docmto ).

       /* Links to SmartViewer h_v-factur-1. */
       RUN add-link IN adm-broker-hdl ( h_v-factur-3 , 'Record':U , h_v-factur-1 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/b-tmpdoc-3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-tmpdoc-3 ).
       RUN set-position IN h_b-tmpdoc-3 ( 10.15 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-tmpdoc-3 ( 9.23 , 95.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-tmpdoc-3 ,
             Btn-Print:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          RUN dispatch IN h_b-tmpdoc-3 ('open-query':U).
      END.
