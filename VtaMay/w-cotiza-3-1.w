...
DEFINE NEW SHARED VARIABLE S-TPOPED LIKE Faccpedi.tpoped.
...
DEFINE VARIABLE h_q-pedido-3 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-cotiza-3-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-cotiza-3-1 AS HANDLE NO-UNDO.
...
DO:
   RUN Actualiza-datos-cliente IN h_v-cotiza-3-1.
END.
...
DO:
  RUN Genera-Excel2 IN h_v-cotiza-3-1.
END.
...
DO:
  ...
  RUN dispatch IN h_t-cotiza-3-1 ('open-query').
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/v-cotiza-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_v-cotiza-3-1 ).
       RUN set-position IN h_v-cotiza-3-1 ( 1.00 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/q-pedido-3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-pedido-3 ).
       RUN set-position IN h_q-pedido-3 ( 8.69 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 8.29 ) */

       /* Links to SmartObject h_v-cotiza-3-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv02 , 'TableIO':U , h_v-cotiza-3-1 ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido-3 , 'Record':U , h_v-cotiza-3-1 ).

       /* Links to SmartQuery h_q-pedido-3. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-pedido-3 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv02 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_q-pedido-3 ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_b-cotiza-2-1 ( 10.42 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-cotiza-2-1 ( 10.35 , 125.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotiza-2-1. */
       RUN add-link IN adm-broker-hdl ( h_q-pedido-3 , 'Record':U , h_b-cotiza-2-1 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtamay/t-cotiza-3-1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotiza-3-1 ).
       RUN set-position IN h_t-cotiza-3-1 ( 10.42 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-cotiza-3-1 ( 8.88 , 120.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-cotiza-3-1. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-cotiza-3-1 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-cotiza-3-1 ,
             B-CCTE:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_t-cotiza-3-1 , 'AFTER':U ).
    END. /* Page 2 */
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          ...
          IF h_t-cotiza-3-1 <> ? THEN RUN dispatch IN h_t-cotiza-3-1 ('open-query':U).
      END.
    ...
    WHEN "Recalculo" THEN DO WITH FRAME {&FRAME-NAME}:
        IF h_t-cotiza-3-1 <> ? THEN RUN Recalcular-Precios IN h_t-cotiza-3-1.
      END.
