...
------------------------------------------------------------------------------*/
  ...
  FIND b-faccpedi WHERE b-faccpedi.codcia = s-codcia
      ...
      AND b-faccpedi.coddoc = 'COT'
      AND b-faccpedi.nroped = s-nroped
      NO-LOCK.
  FIND FIRST B-PEDI NO-LOCK NO-ERROR.
  IF AVAILABLE B-PEDI AND b-faccpedi.fchped >= 01/06/2012 THEN DO:
      FIND B-Almmmatg OF B-PEDI NO-LOCK.
      IF LOOKUP(B-Almmmatg.codfam, '001,002,005') > 0 AND
          LOOKUP(Almmmatg.codfam, '001,002,005') = 0
          THEN DO:
          MESSAGE 'Solo se pueden registrar productos de la familia 001, 002 y 005'
              VIEW-AS ALERT-BOX ERROR.
          RETURN 'ADM-ERROR'.
      END.
      IF LOOKUP(B-Almmmatg.codfam, '001,002,005') = 0 AND
          LOOKUP(Almmmatg.codfam, '001,002,005') > 0
          THEN DO:
          MESSAGE 'NO se pueden registrar productos de la familia 001, 002 y 005'
              VIEW-AS ALERT-BOX ERROR.
          RETURN 'ADM-ERROR'.
      END.
  END.
