...
&Scoped-define OPEN-QUERY-br_table CASE COMBO-BOX-1:     WHEN 'Todos' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc /*          AND FacCPedi.Usuario = s-user-id */          AND FacCPedi.FlgEst = s-flgest NO-LOCK BY Faccpedi.FchPed DESC.     WHEN 'Inicien con' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc /*          AND FacCPedi.Usuario = s-user-id */          AND FacCPedi.FlgEst = s-flgest          AND FacCPedi.NomCli BEGINS x-NomCli NO-LOCK BY Faccpedi.FchPed DESC.     WHEN 'Contengan' THEN         OPEN QUERY {&SELF-NAME} FOR EACH FacCPedi WHERE ~{&KEY-PHRASE}               AND FacCPedi.CodCia = s-codcia          AND FacCPedi.CodDiv = s-coddiv          AND FacCPedi.CodDoc = s-coddoc /*          AND FacCPedi.Usuario = s-user-id */          AND FacCPedi.FlgEst = s-flgest          AND INDEX(FacCPedi.NomCli, ~
        ...
        IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(i) IN FRAME {&FRAME-NAME} THEN DO:
            ...
            FOR EACH Facdpedi OF Faccpedi:
                ...
                FIND B-DPEDI WHERE B-DPEDI.CodCia = Faccpedi.CodCia 
                    AND  B-DPEDI.CodDoc = B-CPEDI.CodRef 
