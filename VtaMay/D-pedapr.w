...
DEFINE  NEW SHARED VARIABLE S-TPOCMB   AS DECI.
DEFINE  NEW SHARED VARIABLE X-NRODEC AS INTEGER.
...
DEFINE FRAME D-Dialog
     B-Rechaza AT ROW 18.96 COL 22.72
     B-Aprobar AT ROW 19 COL 48.29
     B-Consulta AT ROW 18.88 COL 81.86
     SPACE(4.41) SKIP(0.34)
...
DO:
  MESSAGE 'Desea Aprobar Pedido' VIEW-AS ALERT-BOX 
...
DO:
  MESSAGE 'Desea Rechazar Pedido' VIEW-AS ALERT-BOX 
          QUESTION BUTTONS YES-NO UPDATE x-rpta AS LOGICAL.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/v-pedido.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedido ).
       RUN set-position IN h_v-pedido ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.31 , 91.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/b-pedido.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-pedido ).
       RUN set-position IN h_b-pedido ( 8.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.35 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtamay/q-pedcon.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-pedcon ).
       RUN set-position IN h_q-pedcon ( 18.88 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 15.86 ) */

       /* Links to SmartViewer h_v-pedido. */
       RUN add-link IN adm-broker-hdl ( h_q-pedcon , 'Record':U , h_v-pedido ).

       /* Links to SmartBrowser h_b-pedido. */
       RUN add-link IN adm-broker-hdl ( h_v-pedido , 'Record':U , h_b-pedido ).
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE B-CPEDI THEN DO:
    ...
    END.
     ...
     END.
     RUN dispatch IN h_v-pedido('display-fields':U).
  END.
