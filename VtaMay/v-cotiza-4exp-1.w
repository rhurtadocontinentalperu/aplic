...
DEFINE FRAME F-Main
     ...
     FacCPedi.FchPed AT ROW 1 COL 99 COLON-ALIGNED
          ...
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     BUTTON-Turno-Retrocede AT ROW 1.77 COL 74 WIDGET-ID 34
     BUTTON-Turno-Avanza AT ROW 1.77 COL 79 WIDGET-ID 32
     ...
     FacCPedi.TpoCmb AT ROW 1.81 COL 99 COLON-ALIGNED
          ...
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     FILL-IN-1 AT ROW 1.96 COL 82 COLON-ALIGNED NO-LABEL WIDGET-ID 36
...
/*       THEN s-cndvta-validos = s-cndvta-validos + ',401'. */
  ...
  IF LOOKUP('002', s-cndvta-validos) = 0 THEN s-cndvta-validos =  s-cndvta-validos + ',002'.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME} :
   ...
   END.
    ...
    /*Valida Cantidades Detalle Pedido*/

    FOR EACH PEDI OF FacCPedi WHERE TRIM(PEDI.UndVta) = 'UNI' NO-LOCK:
        IF (INT(PEDI.CanPed) - TRUNCATE(PEDI.CanPed,2)) <> 0 THEN DO:
            MESSAGE "Cantidad NO permitida en el Articulo " PEDI.CodMat
