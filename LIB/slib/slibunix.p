...
function unix_whoami returns char:

    define var retval as char no-undo.

    input through value( "who -m 2> /dev/null" ).
    import unformatted retval.
    input close.
        ...
        if retval <> "" then
           retval = entry( 1, retval, " " ).

    return retval.

end function. /* unix_whoami */
...
procedure unix_kill:
    ...
    define input param pcSignal as char no-undo.
    ...
    cCmd = "kill " + pcSignal + " " + string( piPid ).
...
end procedure. /* unix_kill */

function unix_getPid returns int:

    define var retval as int no-undo.

    run getpid( output retval ).
    return retval.

end function. /* getPid */

procedure getpid external {&libc} cdecl:
    define return param ppid as long no-undo.
end procedure.
...
procedure unix_createDir:
    ...
    do i = 2 to num-entries( pcDir, "/" ):
        ...
        if file-info:full-pathname = ? then do:
            ...
            if unix_iExitCode <> 0 then
                {slib/err_throw "'os_error'" "string( unix_iExitCode )"}.
            ***/
