/******************************************************************************
  ...
  02/10/07  T Judycki   Added support for national codepages in build-in
                        base 14 Type 1 fonts (for Poland only)
...
PROCEDURE pdf_font_diff_width:
  ...
  IF NOT CAN-FIND(FIRST TT_pdf_stream
                  WHERE TT_pdf_stream.obj_stream = pdfStream NO-LOCK)
  THEN DO:
    RUN pdf_error(pdfStream,"pdf_font_diff_width","Cannot find Stream!").
    RETURN .
  END.

  IF NOT AVAIL TT_pdf_font THEN
  FIND FIRST TT_pdf_font
    WHERE TT_pdf_font.obj_stream = pdfStream
      AND TT_pdf_font.font_name  = pdfFontName
    NO-LOCK NO-ERROR.

  IF NOT AVAILABLE TT_pdf_font
  THEN DO:
    RUN pdf_error(pdfStream,"pdf_font_diff_width","Cannot find Font Name = " + pdfFontName).
    RETURN .
  END.

  IF  TT_pdf_font.font_type = "VARIABLE" THEN DO:
    ...
    IF pdfRegCharNum >= L_FontWidthNumEntries THEN DO:
      RUN pdf_error(pdfStream,"pdf_font_diff_width","Font " + pdfFontName + " does not contain width definition for ASCII " + STRING(pdfRegCharNum) ).
      RETURN .
    END.
    ...
    END.
  END. /* TT_pdf_font.font_type = "VARIABLE" */
...
PROCEDURE pdf_set_base14_codepage :
  ...
  /* alonb ***/
  DEFINE BUFFER bTT_pdf_diff    FOR TT_pdf_diff.
  DEFINE BUFFER bTT_pdf_font    FOR TT_pdf_font.

  DEFINE VARIABLE cTtfFile      AS CHAR NO-UNDO.
  DEFINE VARIABLE cAfmFile      AS CHAR NO-UNDO.
  
  DEFINE VARIABLE cDiff         AS CHAR NO-UNDO.
  DEFINE VARIABLE iCharNum      AS INT NO-UNDO.

  DEFINE VARIABLE cWidth        AS CHAR NO-UNDO.
  DEFINE VARIABLE cWidthArr     AS CHAR NO-UNDO.
  DEFINE VARIABLE iWidthLen     AS INT NO-UNDO.

  DEFINE VARIABLE cFont         AS CHAR NO-UNDO.
  DEFINE VARIABLE iFirstChar    AS INT NO-UNDO.
  DEFINE VARIABLE iLastChar     AS INT NO-UNDO.
  ...
  IF NOT CAN-FIND(FIRST TT_pdf_stream
                  WHERE TT_pdf_stream.obj_stream = pdfStream NO-LOCK)
  THEN DO:
    RUN pdf_error(pdfStream,"pdf_set_base14_codepage","Cannot find Stream!").
    RETURN .
  END.
  ...
  CASE pdfCodePage :
    ...
    WHEN "iso8859-2" OR
    WHEN "ibm852" OR
    WHEN "mazovia" THEN DO:
      /* Variable length fonts in base 14 Type 1 fonts */
      DEFINE VARIABLE L_Base14PropFonts AS CHARACTER INITIAL
        "Times-Roman,Times-Bold,Times-Italic,Times-BoldItalic,Helvetica,Helvetica-Bold,Helvetica-Oblique,Helvetica-BoldOblique" NO-UNDO.
      /* 4 codepages */
      DEFINE VARIABLE L_CodePages AS CHARACTER INITIAL "1250,iso8859-2,ibm852,mazovia" NO-UNDO.
      /* 18 national characters */
      DEFINE VARIABLE L_PostScript AS CHARACTER INITIAL
        "Aogonek,aogonek,Zdotaccent,zdotaccent,Zacute,zacute,Sacute,sacute,Eogonek,eogonek,Cacute,cacute,Nacute,nacute,Oacute,oacute,Lslash,lslash" NO-UNDO.
      /* Characters with equivalent length to be used as a width pattern */
      DEFINE VARIABLE L_Regular    AS CHARACTER INITIAL
        "A,a,Z,z,Z,z,S,s,E,e,C,c,N,n,O,o,L,l" NO-UNDO.
      /* 4x18 codes */
      DEFINE VARIABLE L_CharNum    AS INTEGER EXTENT 72 INITIAL [
        165,185,175,191,143,159,140,156,202,234,198,230,209,241,211,243,163,179, /* 1250 */
        161,177,175,191,172,188,166,182,202,234,198,230,209,241,211,243,163,179, /* iso8859-2 */
        164,165,189,190,141,171,151,152,168,169,143,134,227,228,224,162,157,136, /* ibm852 */
        143,134,161,167,160,166,152,158,144,145,149,141,165,164,163,162,156,146  /* mazovia */
        ] NO-UNDO.
      ...
      L_CodePageNo = LOOKUP( pdfCodePage, "1250,iso8859-2,ibm852,mazovia" ) - 1.
      DO L_Count = 1 TO 18:
        /* create global substitution */
        CREATE TT_pdf_diff.
        ASSIGN TT_pdf_diff.obj_stream = pdfStream
               TT_pdf_diff.font_name  = "Base14WinAnsiEncoding"
               TT_pdf_diff.char_num   = L_CharNum[ 18 * L_CodePageNo + L_Count ]
               TT_pdf_diff.PS_Name    = "/" + ENTRY( L_Count, L_PostScript ).
        /* modify character width in each variable length font */
        DO L_FontNo = 1 TO NUM-ENTRIES(L_Base14PropFonts): 
          RUN pdf_font_diff_width(pdfStream,
                                  ENTRY(L_FontNo, L_Base14PropFonts),
                                  L_CharNum[ 18 * L_CodePageNo + L_Count ],
                                  ASC(ENTRY(L_Count,L_Regular))).
        END.
      END.
    END.
    ...
    WHEN "iso8859-8" THEN DO:
        ...
        ASSIGN
            cDiff = 
                ...
                "183:middot|" +
                "186:divide|" +
                "192:afii57799,afii57801,afii57800,afii57802,afii57793,afii57794,afii57795,afii57798,afii57797,afii57806,.notdef,afii57796,afii57807,afii57839,afii57645,afii57841,afii57842,afii57804,afii57803,afii57658,afii57716,afii57717,afii57718,gereshhebrew,gershayimhebrew,.notdef,.notdef,.notdef,.notdef,.notdef,.notdef,.notdef,afii57664,afii57665,afii57666,afii57667,afii57668,afii57669,afii57670,afii57671,afii57672,afii57673,afii57674,afii57675,afii57676,afii57677,afii57678,afii57679,afii57680,afii57681,afii57682,afii57683,afii57684,afii57685,afii57686,afii57687,afii57688,afii57689,afii57690,.notdef,.notdef,afii299,afii300,.notdef".

            cWidth = 
                "Times-Roman,224,250:443 422 296 380 422 253 253 443 422 232 401 401 401 465 443 253 275 422 422 422 401 443 422 422 422 549 465|" +

                "Times-Bold,181,255:527 485 232 295 274 506 464 675 675 675 464 654 654 654 654 654 654 907 654 612 612 612 612 359 359 359 359 654 654 696 696 696 696 696 274 696 654 654 654 654 654 549 464 485 443 338 422 464 274 295 464 485 274 422 401 401 464 485 295 295 443 464 443 422 506 464 443 422 633 464 506 506 464 506 464".

        DO i = 1 to NUM-ENTRIES( cDiff, "|" ):

            ASSIGN
                str         = ENTRY( i, cDiff, "|" )

                iCharNum    = INT( ENTRY( 1, str, ":" ) )
                str         =      ENTRY( 2, str, ":" ).

            DO j = 1 to NUM-ENTRIES( str ):

                CREATE TT_pdf_diff.
                ASSIGN TT_pdf_diff.obj_stream = pdfStream
                       TT_pdf_diff.font_name  = "Base14WinAnsiEncoding"
                       TT_pdf_diff.char_num   = iCharNum
                       TT_pdf_diff.PS_Name    = "/" + ENTRY( j, str ).
        ...
        DO i = 1 TO NUM-ENTRIES( cWidth, "|" ):
            ...
            ASSIGN
                str         = ENTRY( 1, ENTRY( i, cWidth, "|" ), ":" )

                cFont       =      ENTRY( 1, str )
                iFirstChar  = INT( ENTRY( 2, str ) )
                iLastChar   = INT( ENTRY( 3, str ) )

                str         = ENTRY( 2, ENTRY( i, cWidth, "|" ), ":" ).

            FIND FIRST tt_pdf_font
                 WHERE tt_pdf_font.font_file    = "PDFBASE14"
                   AND tt_pdf_font.font_name    = cFont
                   AND tt_pdf_font.font_width  <> ""
                 NO-LOCK NO-ERROR.

            IF AVAIL tt_pdf_font THEN DO:

                ASSIGN
                    cWidthArr   = tt_pdf_font.font_width
                    iWidthLen   = NUM-ENTRIES( cWidthArr, " " ).

                IF iWidthLen - 1 < iLastChar THEN

                DO j = iWidthLen - 1 TO iLastChar - 1:
                    cWidthArr = cWidthArr 
                        + ( IF cWidthArr <> "" THEN " " ELSE "" ) + "0".
                END.

                DO j = iFirstChar TO iLastChar:
                    ENTRY( j + 1, cWidthArr, " " ) = 
                        ENTRY( j - iFirstChar + 1, str, " " ).
                END.

                ASSIGN
                    tt_pdf_font.font_width  = cWidthArr
                    tt_pdf_font.afm_width   = tt_pdf_font.font_width.
        ...
        FILE-INFO:FILE-NAME = "slib/bin/times.ttf".

        /***
        IF FILE-INFO:FULL-PATHNAME = ? THEN
           FILE-INFO:FILE-NAME = "slib/times.afm".

        IF FILE-INFO:FULL-PATHNAME = ? THEN
           FILE-INFO:FILE-NAME = "times.afm".

        IF FILE-INFO:FULL-PATHNAME = ? THEN
           FILE-INFO:FILE-NAME = "xx/times.ttf".

        IF FILE-INFO:FULL-PATHNAME = ? THEN
           FILE-INFO:FILE-NAME = "us/xx/times.ttf".

        IF FILE-INFO:FULL-PATHNAME = ? THEN
           FILE-INFO:FILE-NAME = "xxsrc/times.ttf".

        IF FILE-INFO:FULL-PATHNAME = ? THEN
           FILE-INFO:FILE-NAME = "us/xxsrc/times.ttf".
        ***/

        cTtfFile = FILE-INFO:FULL-PATHNAME.



        FILE-INFO:FILE-NAME = "slib/bin/times.afm".
        ...
        IF FILE-INFO:FULL-PATHNAME = ? THEN
           FILE-INFO:FILE-NAME = "xx/times.afm".

        IF FILE-INFO:FULL-PATHNAME = ? THEN
           FILE-INFO:FILE-NAME = "us/xx/times.afm".

        IF FILE-INFO:FULL-PATHNAME = ? THEN
           FILE-INFO:FILE-NAME = "xxsrc/times.afm".

        IF FILE-INFO:FULL-PATHNAME = ? THEN
           FILE-INFO:FILE-NAME = "us/xxsrc/times.afm".
        ***/

        cAfmFile = FILE-INFO:FULL-PATHNAME.
 
        IF  cTtfFile <> ?
        AND cAfmFile <> ? THEN DO:

            RUN pdf_load_font( 
                INPUT "Spdf",
                INPUT "Times-Roman-Hebrew", 
                INPUT cTtfFile, 
                INPUT cAfmFile, 
                INPUT "" ).
            ...
            FOR EACH  TT_pdf_diff
                ...
                CREATE bTT_pdf_diff.
                BUFFER-COPY TT_pdf_diff TO bTT_pdf_diff
                    ASSIGN
                        bTT_pdf_diff.font_name = "Times-Roman-Hebrew".

            END. /* each TT_pdf_diff */ 



            FIND FIRST TT_pdf_font
                 WHERE TT_pdf_font.font_file = "PDFBASE14"
                   AND TT_pdf_font.font_name = "Times-Roman"
                 NO-ERROR.

            FIND FIRST bTT_pdf_font
                 WHERE bTT_pdf_font.font_name = "Times-Roman-Hebrew"
                 NO-ERROR.

            IF AVAIL TT_pdf_font AND AVAIL bTT_pdf_font THEN DO:
            
                ASSIGN
                    bTT_pdf_font.afm_FirstChar  = "0"
                    bTT_pdf_font.afm_LastChar   = "255"
                    bTT_pdf_font.afm_Widths     = TT_pdf_font.font_Width
                    bTT_pdf_font.font_Width     = bTT_pdf_font.afm_Widths.
    ...
    OTHERWISE DO:
      RUN pdf_error(pdfStream,"pdf_set_base14_codepage","Code page " + pdfCodePage + " not supported.").
      RETURN .
    END.
  END CASE.
...
/*** alonb ***/
PROCEDURE pdf_text_center_dec :
  DEFINE INPUT PARAMETER pdfStream   AS CHARACTER NO-UNDO.
  DEFINE INPUT PARAMETER pdfText     AS CHARACTER NO-UNDO.
  DEFINE INPUT PARAMETER pdfColumn   AS /* alonb *** INTEGER */ DECIMAL NO-UNDO.
  ...
  L_X = pdf_text_width(pdfStream,"E") * pdfColumn
      - pdf_text_width(pdfStream, L_orig_text) / 2.
...
END. /* pdf_text_center_dec */

PROCEDURE pdf_text_at_dec:
  DEFINE INPUT PARAMETER pdfStream   AS CHARACTER NO-UNDO.
  DEFINE INPUT PARAMETER pdfText     AS CHARACTER NO-UNDO.
  DEFINE INPUT PARAMETER pdfColumn   AS /* alonb *** INTEGER */ DECIMAL NO-UNDO.
...
PROCEDURE pdf_Encoding: /* PRIVATE */
  ...
  IF CAN-FIND( FIRST TT_pdf_diff 
    ...
    FOR EACH TT_pdf_diff
      WHERE TT_pdf_diff.obj_stream = pdfStream
        AND TT_pdf_diff.font_name  = "Base14WinAnsiEncoding" NO-LOCK:
      PUT STREAM S_pdf_inc UNFORMATTED
...
PROCEDURE pdf_set_tool_parameter :
  ...
  L_TableParams = "Outline,CellUnderline,"
                ...
                + "UseFields,StartY,Pages,ColumnVerticalPadding".
  ...
  OUTPUT STREAM S_pdf_out
    ...
    FOR EACH TT_pdf_FillTxt 
      ...
      FOR EACH TT_Widget 
        ...
        /* alonb ***/
        FOR FIRST tt_font
            WHERE tt_font.font_name = L_Font
              AND tt_font.font_base <> ""
              AND tt_font.font_base <> ?
            NO-LOCK,

            FIRST tt_pdf_font
            WHERE tt_pdf_font.font_name = tt_font.font_base
            NO-LOCK:

            ASSIGN
                L_Font = tt_pdf_font.font_name
                L_Disp = REPLACE( L_Disp, "/" + tt_font.font_name, "/" + tt_pdf_font.font_name )
                L_Disp = REPLACE( L_Disp,       tt_font.font_tag,        tt_pdf_font.font_tag ).
