...
function str_removeSpaces returns char ( pcStr as char ):
    ...
    return replace( replace( replace( replace( pcStr,
        "~n", "" ),
        "~r", "" ),
        "~t", "" ),
        " ", "" ).

end function. /* str_removeSpaces */
...
function str_escapeStr returns char ( pcStr as char ):
...
end function. /* str_escapeStr */

function str_unescapeStr returns char ( pcStr as char ):
...
function str_lookupList returns log ( pcEntryList as char, pcLookup as char ):

    define var len  as int no-undo.
    define var i    as int no-undo.

    len = num-entries( pcEntryList ).
    do i = 1 to len:

        if lookup( entry( i, pcEntryList ), pcLookup ) > 0 then
            return yes.
...
end function. /* str_lookupList */

function str_candoList returns log ( pcCanDo as char, pcEntryList as char ):
    ...
    do i = 1 to len:
        ...
        if can-do( pcCanDo, entry( i, pcEntryList ) ) then
            return yes.
...
function str_beautify returns char ( pcStr as char, plTitle as log ):

    define var cWords   as char no-undo.
    define var cWord    as char no-undo.
    define var cLast    as char no-undo.

    define var lSkip    as log no-undo.
    define var lFirst   as log no-undo.
    ...
    do iPos = 1 to iLen:
        ...
        case ch:
            ...
            when "~t" then do:
                ...
                if ( cWord <> "" or cLast <> " " ) and not lSkip then do:

                    if cWord = "" then
                       cWords = cWords
                            + ( if cWords <> "" then chr(1) else "" )
                            + ch.
                ...
                do while iPos <= iLen and index( " ~n~r~f~t", substr( pcStr, iPos + 1, 1 ) ) > 0:
                    iPos = iPos + 1.
                end.
            ...
            when "." then do:
                ...
                if not ( iPos - 2 >= 1
                     and substr( pcStr, iPos - 2, 1 ) <= "a"
                     and substr( pcStr, iPos - 2, 1 ) >= "z" )

                and iPos - 1 >= 1
                and substr( pcStr, iPos - 1, 1 ) >= "a"
                and substr( pcStr, iPos - 1, 1 ) <= "z"
                ...
                and not ( iPos + 2 <= iLen
                 ...
                 or iPos - 1 >= 1
                and substr( pcStr, iPos - 1, 1 ) >= "0"
                and substr( pcStr, iPos - 1, 1 ) <= "9"

                and iPos + 1 <= iLen
                and substr( pcStr, iPos + 1, 1 ) >= "0"
                and substr( pcStr, iPos + 1, 1 ) <= "9" then do:
    ...
    assign
      pcStr     = ""
       ...
       iLen = num-entries( cWords, chr(1) ).
    do iPos = 1 to iLen:

        cWord = entry( iPos, cWords, chr(1) ).
        ...
        case cWord:
            ...
            when "at" then do:
                ...
                if iPos = 1 then do:
                    ...
                    if  iPos + 1 <= iLen
                    and entry( iPos + 1, cWords, chr(1) ) = " " then
                        iPos = iPos + 1.
            ...
            otherwise do:
                ...
                and substr( cWord, 1, 1 ) <= "z"
                and substr( cWord, 2, 1 ) = "." then
                assign
                    cWord = caps( cWord ).

                else
                if lookup( cWord, {&str_xAcronym} ) > 0 then
                assign
                    cWord = caps( cWord ).
                ...
                assign
                    cWord = lc( cWord )
                    substr( cWord, 1, 1 ) = caps( substr( cWord, 1, 1 ) )
                        when plTitle or lFirst.

                if  substr( cWord, 1, 1 ) >= "a"
                and substr( cWord, 1, 1 ) <= "z" then
                    lFirst = no.
