...
&if defined( xStartSLib ) = 0 &then
    ...
    define new global shared var slib_hStartSLibHndl    as handle no-undo.
    define new global shared var slib_cStartSLibFile    as char no-undo.

    define var slib_hProcHndl       as handle no-undo.
    define var slib_lInitializeProc as log no-undo.

    if not valid-handle( slib_hStartSLibHndl ) then do:
    ...
    end. /* not valid-handle( slib_hStartSLibHndl ) */
