        ...
        case win_iPlatformId:
            ...
            win_cVersionName = ( if win_iMajorVersion =  3 then
                                 ...
                                 if win_iMajorVersion =  6 then
                                    ...
                                    ( if win_iMinorVersion = 0 then

                                        ( if win_iProductType = {&VER_NT_WORKSTATION} then
                                               "Vista"
                                          else "Windows 2008 Server" ) else

                                      if win_iMinorVersion = 1 then

                                        ( if win_iProductType = {&VER_NT_WORKSTATION} then
                                               "Windows 7"
                                          else "Windows 2008 Server R2" ) else

                                      if win_iMinorVersion = 2 then

                                        ( if win_iProductType = {&VER_NT_WORKSTATION} then
                                               "Windows 8"
                                          else "Windows 2012 Server" ) else

                                      if win_iMinorVersion = 3 then

                                        ( if win_iProductType = {&VER_NT_WORKSTATION} then
                                               "Windows 8.1"
                                          else "Windows 2012 Server R2" )
...
function win_getCurrentProcessId:
    ...
    define var retval as int no-undo.
    run GetCurrentProcessId( output retval ).

    return retval.

end function. /* win_getCurrentProcessId */
...
procedure win_terminateProcess:
    ...
    ***/
    ...
    if lTaskKill then do:
        ...
        cCmd = "taskkill.exe /PID " + string( piPid ) + " /F".
