...
define var cErrorParam      as char no-undo.
...
procedure fetchProErrors private:
    ...
    /***
        ...
        assign
            cError      = string( error-status:get-number(1) )
...
function err_throw returns log
    ...
    ( pcError       as char,
      pcErrorParam  as char,
      pcFileName    as char ):

    assign
        cError      = pcError
        cErrorMsg   = getErrorMsg( pcError, pcErrorParam, pcFileName )
        cErrorParam = pcErrorParam
        cStackTrace = getStackTrace( 2, yes ).
...
procedure err_throwLast:
    ...
    if err_catch(
        ...
        output cErrorMsg,
        output cErrorParams,
        output cStackTrace ) then

        err_throwExact( cErrorCode, cErrorMsg, cErrorParams, cStackTrace ).

end procedure. /* err_rethrowLast */
...
function getErrorMsg returns char private
    ...
    if str <> ? then do:
        ...
        if pcErrorParam <> ? then do:

            /* we don't use substitute( ) because the number of parameters is dynamic */

            j = min( num-entries( pcErrorParam, chr(1) ), 9 ). do i = 1 to j:

                str = replace( str, "&" + string(i), entry( i, pcErrorParam, chr(1) ) ).
