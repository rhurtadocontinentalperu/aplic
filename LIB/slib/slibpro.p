...
procedure pro_importFromStr:
    ...
    do iField = 1 to phBuffer:num-fields:
        ...
        repeat while iExtent <= hField:extent:
        
            case hField:data-type:
            
                when "character" then
                hField:buffer-value( iExtent ) = pro_importStr( pcStr, iCursor ).
                    
                when "integer" then
                hField:buffer-value( iExtent ) = pro_importInt( pcStr, iCursor ).
 
                when "decimal" then
                hField:buffer-value( iExtent ) = pro_importDec( pcStr, iCursor ).
 
                when "date" then
                hField:buffer-value( iExtent ) = pro_importDate( pcStr, iCursor ).
...
end procedure. /* pro_importFromStr */

function pro_importStr return char (

    input           pcStr       as char,
    input-output    piCursor    as int ):
    ...
    repeat:
        ...
        if piCursor > len then
            return "".
            
        if substr( pcStr, piCursor, 1 ) <> " " then
            leave.
    ...
    repeat:
        ...
        if cQuote <> ? and ch = cQuote then do:
        
            if substr( pcStr, piCursor, 1 ) = cQuote
            then piCursor = piCursor + 1.
            else leave.
...
end function. /* pro_importStr */

function pro_importInt return int (
...
end function. /* pro_importInt */

function pro_importDec returns dec (
...
end function. /* pro_importDec */

function pro_importDate returns date (
