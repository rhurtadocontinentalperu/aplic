...
/*REPEAT ON ERROR UNDO, RETURN ON STOP UNDO, RETURN:*/
REPEAT ON ERROR UNDO, THROW ON STOP UNDO, RETURN "ADM-ERROR":
    ...
    /*NOTAS:
        - Para bloqueo de preferencia usar: EXCLUSIVE-LOCK NO-WAIT, así en control estará en el CATCH
        - Si usas EXCLUSIVE-LOCK o EXCLUSIVE-LOCK NO-ERROR o EXCLUSIVE-LOCK NO-ERROR NO-WAIT
            el control del error pasa a FINALLY, lo cual no es lo ideal
    */
    ...
    FINALLY:
        ...
        IF TRUE <> ({&VarMessage} > "") AND NOT AVAILABLE {&TableName} THEN DO:
            {&VarMessage} = "(**)Error inesperado con la tabla: " + "{&TableName}".
            CASE ERROR-STATUS:NUM-MESSAGES:
                WHEN 0 THEN DO:
                    {&VarMessage} = {&VarMessage} + "~n~n" + "Mensaje de error: ".
                    {&VarMessage} = {&VarMessage} + "~n" + "Registro en uso por otro usuario".
                END.
                OTHERWISE DO:
