...
DEF OUTPUT PARAMETER pDirectorio AS CHAR.
...
&Scoped-Define ENABLED-OBJECTS RADIO-SET-FileType FILL-IN-archivo ~
Btn-archivos-2 Btn_OK Btn_Cancel 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-FileType RADIO-SET-Grid ~
RADIO-SET-Labels FILL-IN-archivo FILL-IN-directorio 
...
DEFINE VARIABLE FILL-IN-archivo AS CHARACTER FORMAT "X(15)":U 
...
DEFINE VARIABLE FILL-IN-directorio AS CHARACTER FORMAT "X(256)":U 
     LABEL "Directorio donde guardarlo" 
     VIEW-AS FILL-IN 
     SIZE 67 BY .81
     BGCOLOR 15 FGCOLOR 0 FONT 2 NO-UNDO.

DEFINE VARIABLE RADIO-SET-FileType AS CHARACTER 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "TXT", "TXT",
"XLS", "XLS"
     SIZE 10 BY 2.96 NO-UNDO.
...
DEFINE FRAME gDialog
     RADIO-SET-FileType AT ROW 1.27 COL 24 NO-LABEL WIDGET-ID 8
     RADIO-SET-Grid AT ROW 2.62 COL 57 NO-LABEL WIDGET-ID 14
     RADIO-SET-Labels AT ROW 3.42 COL 57 NO-LABEL WIDGET-ID 32
     FILL-IN-archivo AT ROW 4.65 COL 25 COLON-ALIGNED WIDGET-ID 40
     Btn-archivos AT ROW 4.65 COL 94 WIDGET-ID 38
     FILL-IN-directorio AT ROW 5.81 COL 25 COLON-ALIGNED WIDGET-ID 42
     Btn-archivos-2 AT ROW 5.81 COL 94 WIDGET-ID 44
...
/* SETTINGS FOR BUTTON Btn-archivos IN FRAME gDialog
   NO-ENABLE                                                            */
ASSIGN 
       Btn-archivos:HIDDEN IN FRAME gDialog           = TRUE.

/* SETTINGS FOR FILL-IN FILL-IN-directorio IN FRAME gDialog
   NO-ENABLE                                                            */
...
&Scoped-define SELF-NAME Btn-archivos-2
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn-archivos-2 gDialog
ON CHOOSE OF Btn-archivos-2 IN FRAME gDialog /* Arc... */
DO:
    ASSIGN RADIO-SET-FileType.
    DEF VAR OKpressed AS LOG.
    ...
    FILL-IN-directorio:SCREEN-VALUE = FILL-IN-directorio.
...
DO:
  ASSIGN
       RADIO-SET-FileType RADIO-SET-Grid RADIO-SET-Labels FILL-IN-archivo FILL-IN-directorio.
  IF FILL-IN-archivo = '' THEN DO:
      MESSAGE 'Debe definir el archivo de salida' VIEW-AS ALERT-BOX ERROR.
      RETURN NO-APPLY.
  END.
  IF FILL-IN-directorio = '' THEN DO:
      MESSAGE 'Debe definir el directorio de salida' VIEW-AS ALERT-BOX ERROR.
      RETURN NO-APPLY.
  END.
  ASSIGN
      ...
      pDirectorio = FILL-IN-directorio.
  IF SUBSTRING(pDirectorio, LENGTH(pDirectorio) , 1) <> "\" 
               THEN pDirectorio = TRIM(pDirectorio) + "\".
END.
...
ON ANY-PRINTABLE OF FILL-IN-archivo IN FRAME gDialog /* Nombre del Archivo */
DO:
...
/*         RETURN NO-APPLY.                                       */
/*     END.                                                       */
    IF R-INDEX( "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890" ,
        CAPS( CHR( LASTKEY ) ) ) = 0 THEN DO:  
        BELL.
        RETURN NO-APPLY.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-archivo gDialog
ON LEAVE OF FILL-IN-archivo IN FRAME gDialog /* Nombre del Archivo */
DO:
    IF SELF:SCREEN-VALUE = "" THEN RETURN.

    ASSIGN RADIO-SET-FileType FILL-IN-Archivo.
    DEF VAR OKpressed AS LOG.

    CASE RADIO-SET-FileType:
        WHEN "TXT" THEN FILL-IN-Archivo = TRIM(FILL-IN-archivo)+
            (IF INDEX(FILL-IN-archivo, ".TXT") = 0 THEN ".TXT" ELSE "").
        WHEN "XLS" THEN FILL-IN-Archivo = TRIM(FILL-IN-archivo)+
            (IF INDEX(FILL-IN-archivo, ".XLS") = 0 THEN ".XLS" ELSE "").
    END CASE.
...
&Scoped-define SELF-NAME FILL-IN-directorio
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-directorio gDialog
ON ANY-PRINTABLE OF FILL-IN-directorio IN FRAME gDialog /* Directorio donde guardarlo */
DO:
    IF R-INDEX( "abcdefghijklmn¤opqrstuvwxyz1234567890:./\\" ,
        CAPS( CHR( LASTKEY ) ) ) = 0 THEN DO:  
        BELL.
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-FileType RADIO-SET-Grid RADIO-SET-Labels FILL-IN-archivo 
          FILL-IN-directorio 
      WITH FRAME gDialog.
  ENABLE RADIO-SET-FileType FILL-IN-archivo Btn-archivos-2 Btn_OK Btn_Cancel 
      WITH FRAME gDialog.
  VIEW FRAME gDialog.
