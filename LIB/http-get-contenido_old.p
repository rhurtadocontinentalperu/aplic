...
/*DEFINE INPUT PARAMETER piTimeout AS INTEGER NO-UNDO.  /* Nuevo: timeout configurable */*/
...
ASSIGN
    ...
    iChunkSize = 8192  /* Leer en bloques de 8KB para mejor rendimiento */
...
requestString = "GET " + cURL + " HTTP/1.1" + {&HTTP-NEWLINE} +
                "Accept: */*" + {&HTTP-NEWLINE} +
                "Accept-Encoding: identity" + {&HTTP-NEWLINE} +  /* Evita compresión gzip */
...
DO WHILE vloop:
    ...
    IF vstarttime + iTimeout < ETIME THEN DO:
        vSocket:DISCONNECT().
        pRESULT = "0:Timeout (" + STRING(piTimeout) + "s)".
        DELETE OBJECT vSocket.
        RETURN.
    END.
END.
...
/* ============================================================
   Procesar Respuesta - MANEJO SEGURO DE LONGCHAR
   ============================================================ */
...
IF iHeaderEnd > 0 THEN DO:
    ...
    cBody = SUBSTRING(pRESPONSE, iHeaderEnd + LENGTH({&HTTP-NEWLINE} + {&HTTP-NEWLINE}), -1) NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    /* ============================================================
       Leer en bloques de 8KB para evitar error 9324
       ============================================================ */
    SET-SIZE(b) = bytesAvail + 1.
    vSocket:READ(b, 1, bytesAvail, 1).
    
    /* ? CONVERTIR A LONGCHAR SEGURO */
    cChunk = GET-STRING(b, 1, bytesAvail) NO-ERROR.
    IF cChunk <> ? THEN DO:
        /* Usar LONGCHAR para la concatenación */
        pRESPONSE = pRESPONSE + cChunk.
    END.
