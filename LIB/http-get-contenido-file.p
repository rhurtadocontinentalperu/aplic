...
------------------------------------------------------------------------------*/
    ...
    /* --------------------------------------------------------------
       ...
       -------------------------------------------------------------- */
    IF TRIM(pcWorkDir) = "" THEN
        pcWorkDir = SESSION:TEMP-DIRECTORY.

    cSep = "".
    IF pcWorkDir <> "" THEN DO:
        IF SUBSTRING(pcWorkDir,LENGTH(pcWorkDir),1) <> "\\"
           AND SUBSTRING(pcWorkDir,LENGTH(pcWorkDir),1) <> "/" THEN
            cSep = "\\".
    END.

    pcWorkDir = pcWorkDir + cSep.
    ...
    RUN UrlParser(INPUT pcURL,
                  OUTPUT cHOST,
                  OUTPUT cPORT,
                  OUTPUT cURL).

    IF cHOST = "" OR cURL = "" THEN DO:
        pcResult = "0:URL Invalida".
        RETURN.
    END.

    /* --------------------------------------------------------------
       Validar directorio de trabajo.
       FILE-INFO:FULL-PATHNAME permite saber que ruta esta viendo
       realmente Progress. Esto es especialmente importante en Graphon.
       -------------------------------------------------------------- */
    ...
    /* --------------------------------------------------------------
       ...
       Se crea el archivo antes del socket. Si Graphon no permite escribir
       en la ruta, el problema queda identificado inmediatamente.
       -------------------------------------------------------------- */
    OUTPUT TO VALUE(cTempFile).

    IF ERROR-STATUS:ERROR THEN DO:
        pcResult = "0:No se pudo crear archivo temporal: " +
                   cTempFile + " - " + GetLastError().
        RETURN.
    END.

    /* Escribir una marca inicial. Luego se cierra; los chunks se agregaran. */
    PUT UNFORMATTED "".
    ...
    IF ERROR-STATUS:ERROR THEN DO:
        ...
        pcResult = "0:No se pudo escribir archivo temporal: " + cErr.
        RETURN.
    END.

    OUTPUT CLOSE.

    /* Verificar que Progress realmente lo creo. */
    FILE-INFO:FILE-NAME = cTempFile.
    IF FILE-INFO:FULL-PATHNAME = ? THEN DO:
        pcResult = "0:El archivo temporal no fue creado: " + cTempFile.
        RETURN.
    END.
    ...
    /* --------------------------------------------------------------
       Crear socket.
       -------------------------------------------------------------- */
    CREATE SOCKET vSocket NO-ERROR.

    IF NOT VALID-HANDLE(vSocket) THEN DO:
        OS-DELETE VALUE(cTempFile) NO-ERROR.
        pcResult = "0:No se pudo crear SOCKET: " + GetLastError().
        RETURN.
    END.
    ...
    IF NOT wStatus THEN DO:
        pcResult = "0:No Socket - " + GetLastError().
    ...
    vSocket:WRITE(vBuffer,1,LENGTH(requestString)) NO-ERROR.

    IF ERROR-STATUS:ERROR THEN DO:
        pcResult = "0:Error enviando request - " + GetLastError().
        SET-SIZE(vBuffer) = 0.
        vSocket:DISCONNECT() NO-ERROR.
        DELETE OBJECT vSocket.
        OS-DELETE VALUE(cTempFile) NO-ERROR.
        RETURN.
    END.
    ...
    /* --------------------------------------------------------------
       Desconectar.
       -------------------------------------------------------------- */
    IF VALID-HANDLE(vSocket) THEN DO:
        IF vSocket:CONNECTED() THEN
            vSocket:DISCONNECT() NO-ERROR.
        DELETE OBJECT vSocket NO-ERROR.
    END.

    /* Si el timeout fue detectado, no seguir procesando. */
    IF pcResult = "0:Timeout (120s)" THEN DO:
        OS-DELETE VALUE(cTempFile) NO-ERROR.
        RETURN.
    END.
    ...
    IF FILE-INFO:FULL-PATHNAME = ? OR iFileSize <= 0 THEN DO:
        pcResult = "0:Archivo Temporal Vacio. Ruta=" + cTempFile.
        OS-DELETE VALUE(cTempFile) NO-ERROR.
        RETURN.
    END.

    /* --------------------------------------------------------------
       Crear archivo JSON en la misma ruta de trabajo.
       -------------------------------------------------------------- */
    cJsonFile = pcWorkDir +
                "json_" +
                STRING(ETIME) + "_" +
    ...
    /* El temporal HTTP ya no es necesario. */
    OS-DELETE VALUE(cTempFile) NO-ERROR.
