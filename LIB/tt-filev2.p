...
/* ******************************************************************** */
/* RHC 30/09/2014 Se va a crear en un directorio temporal y luego se va a copiar al usuario */
/* ******************************************************************** */
DEF VAR cOutputClientFile AS CHAR NO-UNDO.
run GetOutputFile ( output cOutputClientFile ).
if   ( cFileType = "" )
  or ( cFileType <> tt-browse and cOutputClientFile = "" ) then return.
/* RUN lib/getuniquefilename ("." + TRIM(cFileType), SESSION:TEMP-DIR, OUTPUT cOutputFile). */
cOutputFile = SESSION:TEMP-DIRECTORY + STRING(NEXT-VALUE(sec-arc,integral)) + "." + TRIM(cFileType).
/* ******************************************************************** */
...
cComando = "copy " + TRIM(cOutputFile) + ' ' + TRIM(cOutputClientFile).
OS-COMMAND 
    SILENT 
    VALUE(cComando).
