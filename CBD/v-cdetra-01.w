...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR lTipoRuc AS CHAR.
  DEFINE VAR lFiler35 AS CHAR.
 ...
 lRuc = "20100038146".
 IF INDEX(s-nomcia,'standford') > 0 THEN DO:
      lRuc = "20511358907".    
 END.
 
  /*x-Archivo = 'D' + '20100038146' + STRING(Cb-cdetr.nrolot, '999999') + '.txt'.*/
  x-Archivo = 'D' + lRuc + STRING(Cb-cdetr.nrolot, '999999') + '.txt'.
  ...
  FOR EACH Cb-ddetr OF Cb-cdetr NO-LOCK,
      ...
      IF LAST-OF(cb-ddetr.codpro) OR 
          ...
          LAST-OF(cb-ddetr.Provi_CodOpe) OR
          LAST-OF(cb-ddetr.CodDoc) OR
          LAST-OF(cb-ddetr.NroSer) OR
          LAST-OF(cb-ddetr.NroDoc) 
          THEN DO:
...
/*
  ...
  */
          lTipoRuc = '6'.
          lFiler35 = FILL(' ',35).
          lTipoDoc = if(cb-ddetr.coddoc = 'BOL') THEN '03' ELSE '01'.
          lSerDoc = TRIM(cb-ddetr.nroser).
          lSerDoc = FILL('0', 4 - LENGTH(lSerDoc)) + lSerDoc.
          lNroDoc = TRIM(cb-ddetr.nrodoc).
          lNroDoc = FILL('0', 8 - LENGTH(lNroDoc)) + lNroDoc.

          PUT 
              STRING(lTipoRuc, 'x(1)')        FORMAT 'x(1)'
              STRING(gn-prov.Ruc, 'x(11)')        FORMAT 'x(11)'
              STRING(lFiler35, 'x(35)')        FORMAT 'x(35)'
              ...
              STRING(lTipoDoc, 'x(2)') FORMAT 'x(2)'
              STRING(lSerDoc, 'x(4)') FORMAT 'x(4)'
              STRING(lNroDoc, 'x(8)') FORMAT 'x(8)'
              SKIP.
          x-Importe = 0.
      END.
  END.              
