...
&Scoped-define QUERY-STRING-Query-Main FOR EACH cb-cdetr WHERE ~{&KEY-PHRASE} ~
      AND cb-cdetr.CodCia = s-codcia ~
 AND cb-cdetr.Periodo = s-periodo NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH cb-cdetr WHERE ~{&KEY-PHRASE} ~
      AND cb-cdetr.CodCia = s-codcia ~
 AND cb-cdetr.Periodo = s-periodo NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main cb-cdetr
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main cb-cdetr
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "cb-cdetr.CodCia = s-codcia
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN  input-var-1 = STRING(s-codcia)
          input-var-2 = STRING(s-periodo)
          input-var-3 = ""
          output-var-1 = ?
          OK-WAIT-STATE = SESSION:SET-WAIT-STATE("GENERAL").

  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN lkup/c-detrac ("Detracciones").
