...
x-cta-fin x-Clasificacion x-auxiliar FILL-IN-Cco x-Impresora tg-ordcom ~
...
"Impresora Laser", 2
     SIZE 17 BY 1.62 NO-UNDO.

DEFINE VARIABLE tg-ordcom AS LOGICAL INITIAL no 
     LABEL "Incluir Orden Compra" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME gDialog
     ...
     tg-ordcom AT ROW 7.12 COL 4 WIDGET-ID 48
     Btn_OK AT ROW 8.23 COL 4 WIDGET-ID 4
     BUTTON-2 AT ROW 8.23 COL 12 WIDGET-ID 6
     Btn_Cancel AT ROW 8.23 COL 20 WIDGET-ID 2
     BUTTON-1 AT ROW 8.54 COL 29 WIDGET-ID 50
     SPACE(24.13) SKIP(0.49)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 4
         TITLE "Libro Mayor General" WIDGET-ID 100.
...
ON WINDOW-CLOSE OF FRAME gDialog /* Libro Mayor General */
DO:  
...
/*    RUN Excel.*/
    IF NOT tg-ordcom THEN RUN Excel.
    ELSE RUN Excel2.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Temporal-B gDialog 
PROCEDURE Carga-Temporal-B :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
        ...
        FOR EACH cb-dmov NO-LOCK WHERE cb-dmov.codcia  = s-codcia
            ...
            IF NOT (x-haber = 0 AND x-debe = 0) AND x-debe <> ? AND x-haber <> ? THEN DO:
                ...
                ASSIGN
                    ...
                    w-report.campo-c[11] = x-nroref
                    w-report.campo-c[12] = x-glodoc
                    w-report.campo-c[13] = cb-dmov.OrdCmp
                    w-report.llave-i = x-Orden.
                x-Orden = x-Orden + 1.
                IF x-debe <> 0 THEN DO:
                    w-report.campo-f[1] = x-debe.
...
------------------------------------------------------------------------------*/
  DISPLAY C-Moneda y-CodDiv C-Mes y-codope x-cta-ini x-cta-fin x-Clasificacion 
          x-auxiliar FILL-IN-Cco x-Impresora tg-ordcom 
      WITH FRAME gDialog.
  ENABLE C-Moneda y-CodDiv C-Mes y-codope x-cta-ini x-cta-fin x-Clasificacion 
         x-auxiliar FILL-IN-Cco x-Impresora tg-ordcom Btn_OK BUTTON-2 
         Btn_Cancel BUTTON-1 
      WITH FRAME gDialog.
  VIEW FRAME gDialog.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Excel2 gDialog 
PROCEDURE Excel2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
IF NOT tg-ordcom THEN
    RUN Carga-Temporal.
ELSE RUN Carga-Temporal-B.
    ...
    IF NOT tg-ordcom THEN
        IF x-Impresora = 1 
        THEN RB-REPORT-NAME = "Libro Mayor General Draft".
        ELSE RB-REPORT-NAME = "Libro Mayor General Laser".
    ELSE IF x-Impresora = 1 
        THEN RB-REPORT-NAME = "Libro Mayor General Draft-B".
        ELSE RB-REPORT-NAME = "Libro Mayor General Laser-B".
...
------------------------------------------------------------------------------*/
    ...
    x-Archivo = 'MayorGeneral' + STRING(s-Periodo, '9999') + STRING(s-NroMes, '99') + 's.txt'.
    ...
    IF NOT tg-ordcom THEN RUN Carga-Temporal.
    ELSE RUN Carga-Temporal-B.
    ...
    DEFINE FRAME f-cab
       w-report.campo-c[1] COLUMN-LABEL "CodCta|" FORMAT 'x(8)'
       '|'
       w-report.campo-c[2] COLUMN-LABEL "NomCta|" FORMAT 'x(30)'
       '|'
        w-report.campo-c[3] COLUMN-LABEL "CodDiv|" FORMAT 'x(5)'
         '|'
        w-report.campo-c[4] COLUMN-LABEL "Cco|" FORMAT 'x(2)'
         '|'
        w-report.campo-c[20] COLUMN-LABEL "FchAst|" FORMAT 'x(5)'
         '|'
        w-report.campo-c[5] COLUMN-LABEL "NroAst|" FORMAT 'x(6)'
         '|'
        w-report.campo-c[6] COLUMN-LABEL "CodOpe|" FORMAT 'x(3)'
         '|'
        w-report.campo-c[7] COLUMN-LABEL "ClfAux|" FORMAT 'x(4)'
         '|'
        w-report.campo-c[8] COLUMN-LABEL "CodAux|" FORMAT 'x(11)'
         '|'
        w-report.campo-c[21] COLUMN-LABEL "FchDoc|" FORMAT 'x(5)'
         '|'
        w-report.campo-c[9] COLUMN-LABEL "CodDoc|" FORMAT 'x(4)'
         '|'
        w-report.campo-c[10] COLUMN-LABEL "NroDoc|" FORMAT 'x(10)'
         '|'
        w-report.campo-c[11] COLUMN-LABEL "NroRef|" FORMAT 'x(10)'
         '|'
        w-report.campo-c[12] COLUMN-LABEL "Detalle|" FORMAT 'x(22)'
         '|'
        w-report.campo-f[1] COLUMN-LABEL "Cargo|" FORMAT '->>>,>>>,>>>,>>9.99'
         '|'
        w-report.campo-f[2] COLUMN-LABEL "Abono|" FORMAT '->>>,>>>,>>>,>>9.99'
         '|'
        w-report.campo-f[3] COLUMN-LABEL "ImpSol|" FORMAT '->>>,>>>,>>>,>>9.99'
         '|'
   ...
   FOR EACH w-report NO-LOCK WHERE w-report.task-no = s-task-no
       AND w-report.Llave-C = s-user-id
       BY w-report.Llave-i BY w-report.Campo-c[1]:
       /*DISPLAY STREAM REPORT*/
       PUT STREAM REPORT
           s-Periodo FORMAT '9999'
           s-NroMes FORMAT '99'
           w-report.campo-c[1] FORMAT 'x(8)'
           w-report.campo-c[2] FORMAT 'x(30)'
            w-report.campo-c[3] FORMAT 'x(5)'
            w-report.campo-c[4] FORMAT 'x(2)'
            w-report.campo-c[20] FORMAT 'x(5)'
            w-report.campo-c[5] FORMAT 'x(6)'
            w-report.campo-c[6] FORMAT 'x(3)'
            w-report.campo-c[7] FORMAT 'x(4)'
            w-report.campo-c[8] FORMAT 'x(11)'
            w-report.campo-c[21] FORMAT 'x(5)'
            w-report.campo-c[9] FORMAT 'x(4)'
            w-report.campo-c[10] FORMAT 'x(10)'
            w-report.campo-c[11] FORMAT 'x(10)'
            w-report.campo-c[12] FORMAT 'x(22)'
