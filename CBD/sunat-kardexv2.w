...
DEFINE FRAME F-Main
     ...
     "v1" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 1.19 COL 2 WIDGET-ID 58
     "Moneda:" VIEW-AS TEXT
...
------------------------------------------------------------------------------*/
    ...
    */
    ...
    PUT STREAM REPORT UNFORMATTED
        "KPERIODO|KANEXO|KCATALOGO|KTIPEXIST|KCODEXIST|KFECDOC|KTIPDOC|KSERDOC|KNUMDOC|"
        "KTIPOPE|KDESEXIST|KUNIMED|KMETVAL|KUNIING|KCOSING|KTOTING|"
        "KUNIRET|KCOSRET|KTOTRET|KSALFIN|KCOSFIN|KTOTFIN|KESTOPE|KINTDIAMAY|KINTVTACOM|KINTREG|Almac|TipDocto"
        SKIP.
    ...
    FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.CodCia = S-CODCIA 
        ...
        /*IF LOOKUP(almmmatg.codmat,'061642,039607,012118') = 0 THEN NEXT.*/
        ...
        IF AVAILABLE AlmStkGe THEN DO:
            ...
            /* RHC 15/06/2015 Solo se imprime en el mes de enero */
            IF COMBO-BOX-NroMes = 01 THEN
                /* RHC 15/06/2015 Solo se imprime en el mes de enero */
                    lPeriodo = STRING(s-Periodo,"9999") + string(COMBO-BOX-NroMes,"99") + "00".
                    PUT STREAM REPORT
                    lPeriodo FORMAT '99999999' '|'
                    lAnexo FORMAT '999999' '|'
                    '9|'
        ...
        FOR EACH Almdmov NO-LOCK USE-INDEX ALMD02 WHERE Almdmov.CodCia = Almmmatg.CodCia 
            ...
            IF Almdmov.codmov = 03 THEN DO:
                /* Transferenecias NO, indicacion de Luis "El GAY Urbano" */
                NEXT.
            END.

            pControlKardex = YES.
            lAnexo = gn-divi.campo-c[10]. /* almacen.coddiv. */
            lCodRef = "".

            IF COMBO-BOX-NroMes = 01 AND lxStockIni = YES THEN DO:
                /* RHC 15/06/2015 Solo se imprime en el mes de enero */
                /*
                    lPeriodo = STRING(s-Periodo,"9999") + string(COMBO-BOX-NroMes,"99") + "00".
                    PUT STREAM REPORT
            ...
            x-serie-doc = STRING(Almdmov.nroser,"999").
            x-numero-doc = STRING(Almdmov.nrodoc,"9999999999").
            ...
            IF AVAILABLE Almcmov THEN DO:
                ASSIGN
                    ...
                    lCodRef = almcmov.codref.

                IF almcmov.codmov = 90 AND LOOKUP(almcmov.tipmov,"I,S") > 0 THEN DO:
                    /* --- */
                    IF NOT (TRUE <> (almcmov.nrorf3 > "")) THEN DO:
                        /* Guia de Remision */
                        IF NUM-ENTRIES(almcmov.nrorf3,"-") > 1  THEN DO:
                            x-serie-doc = ENTRY(1,almcmov.nrorf3,"-").
                            x-numero-doc = ENTRY(2,almcmov.nrorf3,"-").
                        END.
                        ELSE DO:
                            IF LENGTH(TRIM(almcmov.nrorf3)) > 3 THEN DO:
                                x-serie-doc = SUBSTRING(TRIM(almcmov.nrorf3),1,3).
                                x-numero-doc = SUBSTRING(TRIM(almcmov.nrorf3),4).
                            END.
                            ELSE DO:                    
                                x-serie-doc = "000".
                                x-numero-doc = TRIM(almcmov.nrorf3).
                            END.
                        END.
                    END.
                    ELSE DO:
                        IF NOT (TRUE <> (almcmov.nrorf1 > "")) THEN DO:
                            /* O/C */
                            x-serie-doc = "000".
                            x-numero-doc = TRIM(almcmov.nrorf1).
                        END.
                    END.
                END.
            END.
            ...
            FIND Facdocum WHERE Facdocum.codcia = s-codcia
                AND Facdocum.coddoc = lCodRef
                NO-LOCK NO-ERROR.
            IF AVAILABLE Facdocum THEN KTIPDOC = FacDocum.CodCbd.

            IF KTIPDOC = '' OR KTIPDOC = ? THEN KTIPDOC = "00".
            ...
            /* Ic - 208Nov2019 - Luis "EL GAY" Urbano, devoluciones de Clientes (I09), el costo debe ser el promedio del Kardex */
            IF nCodmon = x-Codmon THEN DO:
                IF Almdmov.Tipmov = 'I' THEN DO:                    
                    IF Almdmov.codmov = 9 THEN DO:
                        F-PreIng  = Almdmov.vctomn1.
                    END.
                    ELSE DO:
...
/*             F-VALCTO = Almdmov.StkAct * Almdmov.VctoMn1. */
            ...
            x-undmed = Almmmatg.undstk.

            /*  
                Ic 27Nov2019, Luis Urbano lo dejo sin efecto            
                
            x-undmed = "ST".

            FIND FIRST unidades WHERE unidades.codunid = Almmmatg.undstk NO-LOCK NO-ERROR.
            IF AVAILABLE unidades THEN DO:
                IF unidades.codsunat <> ? AND unidades.codsunat <> '' THEN DO:
                    x-undmed = TRIM(unidades.codsunat).
                END.
            END.
            */
            ...
            ASSIGN
                KUNIING = F-Ingreso
                KCOSING = F-PreIng
                KTOTING = KUNIING * KCOSING /*F-TotIng*/ .
            IF Almdmov.tipmov = "S" 
                THEN ASSIGN
                KUNIRET = F-Salida * -1
                KCOSRET = F-PRECIO
                KTOTRET = KUNIRET * KCOSRET /*F-VALCTO*/.
                lPeriodo = STRING(s-Periodo,"9999") + string(COMBO-BOX-NroMes,"99") + "00".
            ...
            PUT STREAM REPORT
                ...
                KTIPDOC FORMAT 'x(2)' '|'
                x-serie-doc FORMAT 'x(3)' '|'
                x-numero-doc FORMAT 'x(10)' '|'
                KTIPOPE FORMAT 'x(2)' '|'
                Almmmatg.desmat FORMAT 'x(80)' '|'
                x-undmed FORMAT 'x(3)' '|'
                '1|'
