...
&Scoped-define QUERY-STRING-b-cb-cfgd FOR EACH cb-tbal ~
      WHERE cb-tbal.CodCia = 0 AND  ~
integral.cb-tbal.TpoBal = "2" NO-LOCK
...
x-Cco C-FORMATO c-moneda Btn_OK Btn_Cancel 
&Scoped-Define DISPLAYED-OBJECTS C-mes R-Tipo F-CodDiv x-Cco C-FORMATO ~
c-moneda 
...
DEFINE FRAME D-Dialog
     ...
     C-mes AT ROW 1.23 COL 61.57 COLON-ALIGNED
     R-Tipo AT ROW 2.69 COL 49.14 NO-LABEL
     F-CodDiv AT ROW 3.69 COL 53 COLON-ALIGNED
     x-Cco AT ROW 4.46 COL 53 COLON-ALIGNED
     C-FORMATO AT ROW 5.35 COL 6.72 COLON-ALIGNED
     c-moneda AT ROW 5.58 COL 54.86 NO-LABEL
     Btn_OK AT ROW 6.77 COL 3
     Btn_Cancel AT ROW 6.81 COL 15.86
     "Moneda" VIEW-AS TEXT
          SIZE 5.86 BY .69 AT ROW 5.58 COL 47.57
     RECT-1 AT ROW 2.46 COL 48.57
     RECT-2 AT ROW 5.42 COL 46
     SPACE(0.85) SKIP(3.18)
...
/* Query rebuild information for BROWSE b-cb-cfgd
     ...
     _Options          = "NO-LOCK"
     _Where[1]         = "integral.cb-tbal.CodCia = 0 AND 
integral.cb-tbal.TpoBal = ""2"""
     _FldNameList[1]   > integral.cb-tbal.CodBal
"cb-tbal.CodBal" ? "X(3)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ASSIGN C-FORMATO C-mes c-moneda R-tipo F-CodDiv x-Cco.
  RUN IMPRIMIR.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY C-mes R-Tipo F-CodDiv x-Cco C-FORMATO c-moneda 
      WITH FRAME D-Dialog.
  ENABLE RECT-1 RECT-2 b-cb-cfgd R-Tipo F-CodDiv x-Cco C-FORMATO c-moneda 
         Btn_OK Btn_Cancel 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
FOR EACH cb-nbal NO-LOCK WHERE cb-nbal.CodCia = 0        AND 
                               cb-nbal.TpoBal = "2"      AND
...
FOR EACH T-BAL :
    ...
    DISPLAY x-mensaje WITH FRAME f-mensaje.
    PAUSE 0.    
    a-Porcen = ROUND((a-import * 100) / a-total, 2).    
    p-Porcen = ROUND((p-import * 100) / p-total, 2).    
END.
...
------------------------------------------------------------------------------*/
  ...
  SPACE(34)
  x-Glosa  NO-LABEL FORMAT "X(40)"
  x-Import COLUMN-LABEL "AJUSTADO" FORMAT "X(16)"
  HEADER
  {&Prn7a} + s-NomCia + {&Prn7b} FORMAT "x(80)"
  "FECHA  : " AT 98 TODAY AT 108 SKIP
  "HORA   : " AT 98 STRING(TIME,"HH:MM:SS") AT 108 SKIP 
 ...
 FIND FIRST T-BAL NO-ERROR.
  ...
  FOR EACH T-BAL BREAK BY ORDEN :      
      ...
      CASE T-bal.Nota :
         ...
         WHEN "RD" THEN x-Import = FILL("=",16).
         WHEN "NE" THEN x-Glosa  = {&Prn6a} + x-Glosa + {&Prn6b}.
         OTHERWISE x-Import = IF T-BAL.p-Import = 0 THEN "" 
                                 ELSE STRING(T-BAL.p-Import,"(ZZZ,ZZZ,ZZ9.99)").                                 
      END CASE.
 ...
 FIND FIRST T-BAL NO-ERROR.
  ...
  FOR EACH T-BAL BREAK BY ORDEN :      
      ...
      CASE T-bal.Nota :
         ...
         WHEN "RD" THEN x-Import = FILL("=",16).
         WHEN "NE" THEN x-Glosa  = {&Prn6a} + x-Glosa + {&Prn6b}.
         OTHERWISE ASSIGN x-Import[1] = IF T-BAL.a-Import = 0 THEN "" 
                                        ELSE STRING(T-BAL.a-Import,"(ZZZ,ZZZ,ZZ9.99)")
                          x-Import[2] = IF T-BAL.P-Import = 0 THEN "" 
                                        ELSE STRING(T-BAL.P-Import,"(ZZZ,ZZZ,ZZ9.99)"). 
      END CASE.
             
      DISPLAY STREAM REPORT SPACE(24)
                            x-Glosa
                            x-Import[1]
                            x-Import[2]
                            WITH FRAME f-cab.                            
...
------------------------------------------------------------------------------*/
  ...
  x-Glosa  NO-LABEL FORMAT "X(40)"
  x-Import COLUMN-LABEL "HISTORICO" FORMAT "X(16)"
  x-Porcen COLUMN-LABEL "PORCENTAJE" FORMAT "X(10)"
  HEADER
  {&Prn7a} + s-NomCia + {&Prn7b} FORMAT "x(80)"
  "FECHA  : " AT 98 TODAY AT 108 SKIP
  "HORA   : " AT 98 STRING(TIME,"HH:MM:SS") AT 108 SKIP 
...
* ***/
 ...
 FIND FIRST T-BAL NO-ERROR.
  ...
  FOR EACH T-BAL BREAK BY ORDEN :      
      x-Glosa = T-BAL.Glosa. 
      x-porcen = IF T-BAL.a-Porcen = 0 THEN "" 
                    ELSE STRING(T-BAL.a-Porcen,"(ZZZ9.99)").
      CASE T-bal.Nota :
         WHEN "RS" THEN x-Import = FILL("-",16).
         WHEN "RD" THEN x-Import = FILL("=",16).
         WHEN "NE" THEN x-Glosa  = {&Prn6a} + x-Glosa + {&Prn6b}.
...
* ***/
 ...
 FIND FIRST T-BAL NO-ERROR.
  ...
  FOR EACH T-BAL BREAK BY ORDEN :      
      x-Glosa = T-BAL.Glosa.      
      x-porcen = IF T-BAL.p-Porcen = 0 THEN "" 
                    ELSE STRING(T-BAL.p-Porcen,"(ZZZ9.99)").
      CASE T-bal.Nota :
         WHEN "RS" THEN x-Import = FILL("-",16).
         WHEN "RD" THEN x-Import = FILL("=",16).
         WHEN "NE" THEN x-Glosa  = {&Prn6a} + x-Glosa + {&Prn6b}.
...
* ***/
 ...
 FIND FIRST T-BAL NO-ERROR.
  ...
  FOR EACH T-BAL BREAK BY ORDEN :      
      x-Glosa = T-BAL.Glosa.      
      x-porcen[1] = IF T-BAL.a-Porcen = 0 THEN "" 
                    ELSE STRING(T-BAL.a-Porcen,"(ZZZ9.99)").

      x-porcen[2] = IF T-BAL.p-Porcen = 0 THEN "" 
                    ELSE STRING(T-BAL.p-Porcen,"(ZZZ9.99)").
