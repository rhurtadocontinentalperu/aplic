...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH cb-oper WHERE ~{&KEY-PHRASE} ~
      AND cb-oper.CodCia = cb-codcia NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Query-Main cb-oper
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main cb-oper


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS RECT-2 F-CodOpe B-buscar 
&Scoped-Define DISPLAYED-OBJECTS F-CodOpe 
...
DEFINE FRAME FRAME-A
     F-CodOpe AT ROW 1.31 COL 1.86 NO-LABEL
     B-buscar AT ROW 1.15 COL 8.14
     RECT-2 AT ROW 1.08 COL 1
...
/* SETTINGS FOR FILL-IN F-CodOpe IN FRAME FRAME-A
   ALIGN-L                                                              */
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "integral.cb-oper.CodCia = cb-codcia"
     _Design-Parent    is WINDOW q-tables @ ( 1.27 , 2.14 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-buscar q-tables
ON CHOOSE OF B-buscar IN FRAME FRAME-A /* buscar */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-CodOpe q-tables
ON LEAVE OF F-CodOpe IN FRAME FRAME-A /* Fill 1 */
DO:
  ASSIGN F-CodOpe.
  RUN Busca-codigo.
...
FIND cb-oper WHERE cb-oper.codcia = cb-CodCia AND 
                   cb-oper.CodOpe = F-CodOpe NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    ...
    */
    RUN lkup\C-Libros ("Operaciones / Libros Contable").    
        IF OUTPUT-VAR-1 <> ? THEN DO:
         FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE
              ROWID({&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}}) = OUTPUT-VAR-1
              NO-LOCK NO-ERROR.
