...
DEFINE FRAME F-Main
     ...
     x-CodDiv AT ROW 3.12 COL 9 COLON-ALIGNED WIDGET-ID 8
     FILL-IN-desde AT ROW 4.08 COL 9.29 COLON-ALIGNED WIDGET-ID 18
     FILL-IN-hasta AT ROW 4.08 COL 29.29 COLON-ALIGNED WIDGET-ID 20
...
DO:
    ASSIGN /*CB-MES*/ x-coddiv fill-in-desde fill-in-hasta /*tg-acum x-periodo*/.
    /*
...
------------------------------------------------------------------------------*/
    ...
    c-xls-file = 'd:\xpciman\JuanSusana.xlsx'.
...
FIND FIRST detalle OF Almdmov EXCLUSIVE-LOCK NO-ERROR.
...
CREATE tdmov.
    ASSIGN
        ...
        tdmov.candes = Almdmov.CanDes * Almdmov.Factor.
    ...
    IF AVAILABLE Ccbddocu THEN DO:
        ...
        ELSE DO:
            ...
            IF ccbcdocu.codmon = 1 THEN DO:
                ...
                /*ASSIGN detalle.implin = detalle.implin + (detalle.candes * dPrecioUnitarioSinImpsto * x-factor).*/
            END.
            ELSE DO:
                FIND FIRST gn-tcmb WHERE gn-tcmb.fecha = Almcmov.fchdoc NO-LOCK NO-ERROR.
                IF AVAIL gn-tcmb 
                THEN detalle.implin = detalle.implin + ((Almdmov.CanDes * Almdmov.Factor) * dPrecioUnitarioSinImpsto) * gn-tcmb.venta * x-factor.
                ELSE detalle.implin = detalle.implin + ((Almdmov.CanDes * Almdmov.Factor) * dPrecioUnitarioSinImpsto) * ccbcdocu.tpocmb * x-factor.
                /*
                 FIND FIRST gn-tcmb WHERE gn-tcmb.fecha = Almcmov.fchdoc NO-LOCK NO-ERROR.
                 IF AVAIL gn-tcmb 
                 THEN detalle.implin = detalle.implin + (detalle.candes * dPrecioUnitarioSinImpsto) * gn-tcmb.venta * x-factor.
                 ELSE detalle.implin = detalle.implin + (detalle.candes * dPrecioUnitarioSinImpsto) * ccbcdocu.tpocmb * x-factor.
               */
            END.
        END.
    END.
...
FOR EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-CodCia AND (x-coddiv = 'Todas' OR gn-divi.coddiv = x-coddiv): 
    ...
    FOR EACH Almacen NO-LOCK WHERE Almacen.codcia = gn-divi.codcia AND Almacen.coddiv = gn-divi.coddiv,
        ...
        IF toggle-pi-pendientes = YES AND NOT AVAILABLE Ccbcdocu THEN DO:
            /* Parte de ingreso sin N/C, entonces buscamos la factura */
            sOrigenNotaCredito = NO.     
            FIND FIRST Ccbcdocu WHERE Ccbcdocu.codcia = s-codcia AND
                Ccbcdocu.coddoc = Almcmov.codref AND
