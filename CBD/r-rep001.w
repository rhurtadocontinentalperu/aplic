...
&Scoped-Define ENABLED-OBJECTS Btn_OK COMBO-BOX-Periodo COMBO-BOX-Mes ~
...
DEFINE FRAME D-Dialog
     ...
     COMBO-BOX-Periodo AT ROW 1.38 COL 23 COLON-ALIGNED
     COMBO-BOX-Mes AT ROW 2.35 COL 23 COLON-ALIGNED
     RADIO-SET-Moneda AT ROW 3.31 COL 25 NO-LABEL
     Btn_Cancel AT ROW 4.65 COL 25
     "Moneda:" VIEW-AS TEXT
          SIZE 6 BY .5 AT ROW 3.31 COL 19
     SPACE(40.13) SKIP(3.22)
...
DO:
  ASSIGN COMBO-BOX-Mes COMBO-BOX-Periodo RADIO-SET-Moneda.
  RUN Excel.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY COMBO-BOX-Periodo COMBO-BOX-Mes RADIO-SET-Moneda 
      WITH FRAME D-Dialog.
  ENABLE Btn_OK COMBO-BOX-Periodo COMBO-BOX-Mes RADIO-SET-Moneda Btn_Cancel 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
chWorkSheet:Range("B1"):Value = "GASTOS POR CENTRO DE COSTO MENSUAL - " + 
                            IF RADIO-SET-Moneda = 1 THEN 'NUEVOS SOLES' ELSE 'DOLARES AMERICANOS'.
chWorkSheet:Range("B2"):Value = "MES DE " + COMBO-BOX-Mes + " DE " + 
...
FOR EACH CB-AUXI NO-LOCK WHERE cb-auxi.codcia = cb-codcia
    ...
    chWorkSheet:Range(cRange):Value = "'" + cb-auxi.codaux.
    iCount = iCount + 1.
    IF iCount > 90 
    THEN ASSIGN
            iCount = ASC("A")
...
FOR EACH CB-CTAS NO-LOCK WHERE CB-CTAS.codcia = cb-codcia
    ...
    FOR EACH CB-DMOV NO-LOCK WHERE cb-dmov.codcia = s-codcia
            ...
            AND cb-dmov.nromes  = LOOKUP(COMBO-BOX-Mes, COMBO-BOX-MES:LIST-ITEMS IN FRAME {&FRAME-NAME})
            AND cb-dmov.codcta  = cb-ctas.codcta
            AND cb-dmov.cco     <> '':
        /*
        ASSIGN
            x-TotMn1 = x-TotMn1 + ABSOLUTE(cb-dmov.impmn1)
            x-TotMn2 = x-TotMn2 + ABSOLUTE(cb-dmov.impmn2).
        */
    ...
    /*IF x-TotMn1 + x-TotMn2 <= 0 THEN NEXT.*/
    /* *********************************** */
    t-column = t-column + 1.
    cColumn = STRING(t-Column).
    cRange = "A" + cColumn.
