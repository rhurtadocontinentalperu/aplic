...
&SCOPED-DEFINE l-lookup h_l-devopendientes
...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 Btn_OK Btn_Cancel 
...
DEFINE VARIABLE h_l-devopendientes AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     BUTTON-1 AT ROW 5.04 COL 84 WIDGET-ID 4
     Btn_OK AT ROW 7.77 COL 84.43
     Btn_Cancel AT ROW 8.96 COL 84.43
     SPACE(3.13) SKIP(2.33)
...
ON WINDOW-CLOSE OF FRAME C-Dialog /* Devoluciones pendientes */
DO:  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-1 C-Dialog
ON CHOOSE OF BUTTON-1 IN FRAME C-Dialog /* Excel */
DO:
  RUN enviar-excel IN h_l-devopendientes.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/lkup/l-devopendientes.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-devopendientes ).
       RUN set-position IN h_l-devopendientes ( 1.19 , 2.00 ) NO-ERROR.
       RUN set-size IN h_l-devopendientes ( 10.92 , 79.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.58 , 82.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-devopendientes. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-devopendientes ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-devopendientes ,
             BUTTON-1:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-devopendientes , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE BUTTON-1 Btn_OK Btn_Cancel 
      WITH FRAME C-Dialog.
  VIEW FRAME C-Dialog.
