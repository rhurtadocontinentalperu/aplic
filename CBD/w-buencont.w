...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-2 W-Win _STRUCTURED
  QUERY BROWSE-2 NO-LOCK DISPLAY
      ...
      PROV.NomPro FORMAT "x(60)":U WIDTH 48.43
      PROV.Libre_c02 COLUMN-LABEL "Buen Contribuyente" FORMAT "x(2)":U
            WIDTH 13.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "IMPORTAR PROVEEDORES BUENOS CONTRIBUYENTES"
         HEIGHT             = 17
         WIDTH              = 91.86
         MAX-HEIGHT         = 17.08
         MAX-WIDTH          = 123.43
...
"PROV.Ruc" ? ? "character" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.PROV.CodPro
"PROV.CodPro" ? ? "character" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.PROV.NomPro
"PROV.NomPro" ? "x(60)" "character" ? ? ? ? ? ? no ? no no "48.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.PROV.Libre_c02
"PROV.Libre_c02" "Buen Contribuyente" "x(2)" "character" ? ? ? ? ? ? no ? no no "13.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
&ANALYZE-RESUME
...
ON END-ERROR OF W-Win /* IMPORTAR PROVEEDORES BUENOS CONTRIBUYENTES */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* IMPORTAR PROVEEDORES BUENOS CONTRIBUYENTES */
DO:
...
FOR EACH gn-prov WHERE gn-prov.codcia = pv-codcia:
    ...
    IF AVAILABLE PROV THEN gn-prov.Libre_c02 = 'Si'.
...
DEF VAR x-Pedido AS CHAR INIT 'buecont_txt.txt'.
DEF VAR x-Ok AS LOG.
...
SYSTEM-DIALOG GET-FILE x-Pedido
    ...
    MUST-EXIST 
    RETURN-TO-START-DIR
    TITLE 'SELECCIONE EL ARCHIVO DE BUENOS CONTRIBUYENTES'
    USE-FILENAME
    UPDATE x-Ok.
