...
DEFINE VARIABLE x-debe     AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-" 
                           COLUMN-LABEL "M o v i m i!Cargos     " NO-UNDO. 
DEFINE VARIABLE x-haber    AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-" 
                           COLUMN-LABEL "e n t o s      !Abonos     " NO-UNDO.
DEFINE VARIABLE x-saldoi   AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-"
                           COLUMN-LABEL "Saldo     !Inicial    " NO-UNDO.
DEFINE VARIABLE y-saldoi   AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-"
                           COLUMN-LABEL "Saldo     !Inicial    " NO-UNDO.
DEFINE VARIABLE q-saldoi   AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-"
                           COLUMN-LABEL "Saldo     !Inicial    " NO-UNDO.

DEFINE VARIABLE x-saldof   AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-"
                           COLUMN-LABEL "S a l d o!Deudor    " NO-UNDO.
DEFINE VARIABLE x-deudor   AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-"
                           COLUMN-LABEL "S a l d o!Deudor    " NO-UNDO.
DEFINE VARIABLE x-acreedor AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-" 
                           COLUMN-LABEL "A c t u a l    !Acreedor   " NO-UNDO.

DEFINE VARIABLE f-debe     AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-" 
                           COLUMN-LABEL "M o v i m i!Cargos     " NO-UNDO. 
DEFINE VARIABLE f-haber    AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-" 
                           COLUMN-LABEL "e n t o s      !Abonos     " NO-UNDO.
DEFINE VARIABLE f-saldof   AS DECIMAL FORMAT "ZZ,ZZZ,ZZZ,ZZ9.99-"
                           COLUMN-LABEL "S a l d o!Deudor    " NO-UNDO.
...
DEFINE FRAME f-cab
        ...
        "L I B R O   C A J A   Y   B A N C O S" TO 105
        /*"FECHA : " TO 162 TODAY TO 172*/
        SKIP
        pinta-mes AT 67
        "PAGINA :" TO 162 c-pagina FORMAT "ZZZ9" TO 172 skip
        x-expres  AT 67
        "HORA   :" TO 162 STRING(TIME,"HH:MM AM") TO 172  SKIP(2)
...
&Scoped-Define ENABLED-OBJECTS RECT-4 RECT-6 RECT-5 C-Moneda X-CodDiv ~
codope C-Mes T-resumen x-Clasificacion x-auxiliar RADIO-SET-1 ~
RB-NUMBER-COPIES B-impresoras RB-BEGIN-PAGE RB-END-PAGE B-imprime B-cancela 
&Scoped-Define DISPLAYED-OBJECTS C-Moneda X-CodDiv codope C-Mes x-cta-ini ~
x-cta-fin T-resumen x-Clasificacion x-auxiliar RADIO-SET-1 RB-NUMBER-COPIES ~
...
DEFINE FRAME DIALOG-1
     ...
     codope AT ROW 2.08 COL 53.43 COLON-ALIGNED
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Libro Caja y Bancos Acumulado".
...
ON GO OF FRAME DIALOG-1 /* Libro Caja y Bancos Acumulado */
DO:
...
DO:
    ...
    ASSIGN RB-OUTPUT-FILE:SCREEN-VALUE = x-archivo.
...
------------------------------------------------------------------------------*/
  DISPLAY C-Moneda X-CodDiv codope C-Mes x-cta-ini x-cta-fin T-resumen 
          x-Clasificacion x-auxiliar RADIO-SET-1 RB-NUMBER-COPIES RB-BEGIN-PAGE 
          RB-END-PAGE 
      WITH FRAME DIALOG-1.
  ENABLE RECT-4 RECT-6 RECT-5 C-Moneda X-CodDiv codope C-Mes T-resumen 
         x-Clasificacion x-auxiliar RADIO-SET-1 RB-NUMBER-COPIES B-impresoras 
...
FOR EACH cb-dmov NO-LOCK WHERE cb-dmov.codcia  = s-codcia
                           ...
                           AND cb-dmov.codope BEGINS (codope)
    ...
    THEN DO:
        ...
        DISPLAY STREAM report x-nromes
                              x-Glodoc
                              y-saldoi when y-saldoi <> 0 @ x-saldoi 
                              x-debe
                              x-haber
                              x-SaldoF
              WITH FRAME f-cab.
...
IF X-INI > 0 THEN DO:
   ...
   DOWN STREAM report WITH FRAME f-cab.
   {&NEW-PAGE}.
   UNDERLINE STREAM report x-fchvto
                           x-nroref
                           x-glodoc
                           x-saldoi
                           x-debe   
   ...
   DOWN STREAM report WITH FRAME F-cab.
   ASSIGN
    f-Debe = f-Debe + x-Debe
    f-Haber = f-Haber + x-Haber.
END.
...
P-largo  = 88. /* En el caso de 88 lineas por página usando 8 lpi ( P-8lpi )
   ...
   DISPLAY STREAM report   
                           ...
                           f-debe @ x-debe
                           f-haber @ x-haber
                           f-Saldof @ x-saldof WITH FRAME f-cab.
   DOWN STREAM report WITH FRAME f-cab.
   {&NEW-PAGE}.
   UNDERLINE STREAM report x-fchvto
                           x-nroref
