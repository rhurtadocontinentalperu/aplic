...
&Scoped-Define ENABLED-OBJECTS FILL-IN-CodPer COMBO-BOX-Areas BUTTON-1 ~
BtnDone BROWSE-2 
...
DEFINE VARIABLE COMBO-BOX-Areas AS CHARACTER FORMAT "X(256)":U 
     LABEL "Areas" 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 96 BY 13.19
...
DEFINE FRAME F-Main
     FILL-IN-CodPer AT ROW 1.54 COL 19 COLON-ALIGNED WIDGET-ID 2
     FILL-IN-NomPer AT ROW 2.35 COL 19 COLON-ALIGNED WIDGET-ID 4
     COMBO-BOX-Areas AT ROW 3.15 COL 19 COLON-ALIGNED WIDGET-ID 6
     BUTTON-1 AT ROW 1.54 COL 82 WIDGET-ID 8
     BtnDone AT ROW 1.54 COL 90 WIDGET-ID 10
     BROWSE-2 AT ROW 4.23 COL 2 WIDGET-ID 200
...
"almtabla.Nombre" "Descripción" "x(20)" "character" ? ? ? ? ? ? no ? no no "29.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
&ANALYZE-RESUME
...
ON END-ERROR OF W-Win /* REGISTRO DEL PERSONAL */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* REGISTRO DEL PERSONAL */
DO:
...
DO:
  /* Consistencias */
    ...
    IF AVAILABLE cpetrased THEN DO:
        MESSAGE 'Trabajador ya registrado' VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    IF COMBO-BOX-Areas:SCREEN-VALUE = ? THEN DO:
        MESSAGE 'Seleccione el área' VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
    END.
    /* Grabación */
    CREATE cpetrased.
    ASSIGN
        cpetrased.CodArea = ENTRY(1, COMBO-BOX-Areas:SCREEN-VALUE, ' - ')
        cpetrased.CodCia = s-codcia
        cpetrased.CodDiv = s-coddiv
        cpetrased.CodPer = FILL-IN-CodPer:SCREEN-VALUE
        cpetrased.FechaReg = DATETIME(TODAY, MTIME)
        cpetrased.FlgEst = 'P'
        cpetrased.UsuarioReg = s-user-id.
    RELEASE cpetrased.
...
DO:
  ...
  IF AVAILABLE pl-pers THEN FILL-IN-NomPer:SCREEN-VALUE = fNomPer(SELF:SCREEN-VALUE).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-CodPer FILL-IN-NomPer COMBO-BOX-Areas 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE FILL-IN-CodPer COMBO-BOX-Areas BUTTON-1 BtnDone BROWSE-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      FOR EACH Almtabla NO-LOCK WHERE Almtabla.Tabla = 'AS':    /* Areas por sede */
          COMBO-BOX-Areas:ADD-LAST(almtabla.Codigo + ' - ' + almtabla.Nombre).
