...
DEF VAR s-Tabla AS CHAR INIT 'CPETAREA'.

DEFINE BUFFER Tabla FOR VtaTabla.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c1 COLUMN-LABEL "Codigo" FORMAT "x(3)":U
      VtaTabla.Llave_c2 COLUMN-LABEL "Descripcion" FORMAT "x(30)":U
            WIDTH 33.57
      VtaTabla.Libre_c01 COLUMN-LABEL "¿Control OD?" FORMAT "x(8)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 55 BY 14.27
         FONT 4.
...
"VtaTabla.Llave_c1" "Codigo" "x(3)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.VtaTabla.Llave_c2
"VtaTabla.Llave_c2" "Descripcion" "x(30)" "character" ? ? ? ? ? ? yes ? no no "33.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaTabla.Libre_c01
"VtaTabla.Libre_c01" "¿Control OD?" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," "Si,No" ? 2 no 0 no no
...
DO:
  ...
  IF RETURN-VALUE = 'YES' THEN VtaTabla.Libre_c01:SCREEN-VALUE IN BROWSE {&browse-name} = 'No'.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = 'YES' THEN DO:
    FIND FIRST Tabla WHERE Tabla.CodCia = s-codcia
        AND Tabla.Tabla = s-Tabla
        AND Tabla.Llave_c1 = VtaTabla.Llave_c1:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
        NO-LOCK NO-ERROR.
    IF AVAIL Tabla THEN DO:
        MESSAGE "Codigo ya registrado"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        RETURN "ADM-ERROR".
        APPLY "ENTRY" TO VtaTabla.Llave_c1.
    END.
  END.
RETURN "OK".
