...
COMBO-BOX-Ubicacion COMBO-BOX-CodDoc FILL-IN-NroPed BUTTON-3 BROWSE-7 ~
BROWSE-2 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-Situacion COMBO-BOX-Ubicacion ~
COMBO-BOX-CodDoc FILL-IN-NroPed 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 146 BY 11.31
         FONT 4
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-7 wWin _STRUCTURED
  QUERY BROWSE-7 NO-LOCK DISPLAY
      ...
      CPEDI.Libre_c04 COLUMN-LABEL "Zona" FORMAT "x(5)":U
      ...
      CPEDI.NomCli COLUMN-LABEL "Cliente" FORMAT "x(40)":U WIDTH 28.72
      CPEDI.Libre_d02 COLUMN-LABEL "Items" FORMAT ">>9":U
      CPEDI.ImpTot FORMAT "->>,>>>,>>9.99":U WIDTH 9.43
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 146 BY 9.42
         FONT 4
...
DEFINE FRAME fMain
     BUTTON-4 AT ROW 1 COL 88 WIDGET-ID 12
     ...
     COMBO-BOX-CodDoc AT ROW 1.27 COL 67 COLON-ALIGNED NO-LABEL WIDGET-ID 14
     FILL-IN-NroPed AT ROW 1.27 COL 74 COLON-ALIGNED NO-LABEL WIDGET-ID 10
...
"CPEDI.Libre_c04" "Zona" "x(5)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CPEDI.NomCli" "Cliente" "x(40)" "character" ? ? ? ? ? ? no ? no no "28.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > Temp-Tables.CPEDI.Libre_d02
"CPEDI.Libre_d02" "Items" ">>9" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[14]   > Temp-Tables.CPEDI.ImpTot
"CPEDI.ImpTot" ? ? "decimal" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-7 */
&ANALYZE-RESUME
...
/* CtrlFrame OCXINFO:CREATE-CONTROL from: {F0B88A90-F5DA-11CF-B545-0020AF6ED35A} type: PSTimer */
      CtrlFrame:MOVE-AFTER(FILL-IN-NroPed:HANDLE IN FRAME fMain).
...
DO:
  ...
  REPEAT WHILE AVAILABLE CPEDI:
      IF CPEDI.CodDoc = COMBO-BOX-CodDoc:SCREEN-VALUE 
          AND CPEDI.NroPed = FILL-IN-NroPed:SCREEN-VALUE THEN DO:
...
FOR EACH Faccpedi USE-INDEX Llave08 NO-LOCK WHERE Faccpedi.codcia = s-codcia
    ...
    /* RHC 26.01.2012 buscamos tareas asignadas  */
    FIND FIRST CpeTareas WHERE CpeTareas.codcia = s-codcia
        AND CpeTareas.coddiv = s-coddiv
    ...
    IF AVAILABLE CpeTareas THEN DO:
        FIND FIRST CpeTareas WHERE CpeTareas.codcia = s-codcia
            ...
            AND Cpetareas.codOD = Faccpedi.coddoc
            AND CpeTareas.nroOD = Faccpedi.nroped
            AND CpeTareas.flgest = 'P' NO-LOCK NO-ERROR.
        IF AVAILABLE CpeTareas THEN NEXT.
    END.
    /* **************************************** */
    FOR EACH facdpedi OF faccpedi NO-LOCK,
        FIRST Almmmate NO-LOCK WHERE Almmmate.codcia = Facdpedi.codcia
...
FOR EACH Faccpedi USE-INDEX Llave08 NO-LOCK WHERE Faccpedi.codcia = s-codcia
    ...
    FOR EACH facdpedi OF faccpedi NO-LOCK,
        ...
        FIRST Almtubic OF Almmmate NO-LOCK:
        t-Zonas = TRIM(Facdpedi.almdes) + ',' + TRIM(Almtubic.codzona).
        IF x-Zonas = '' THEN x-Zonas = t-Zonas.
        ELSE IF LOOKUP(t-Zonas, x-Zonas, '|') = 0 THEN x-Zonas = x-Zonas + '|' + t-Zonas.
    END.
    DO k = 1 TO NUM-ENTRIES(x-Zonas, '|'):
        CREATE CPEDI.
        BUFFER-COPY Faccpedi TO CPEDI.
...
END.
ELSE MESSAGE "wcon003.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  DISPLAY COMBO-BOX-Situacion COMBO-BOX-Ubicacion COMBO-BOX-CodDoc 
          FILL-IN-NroPed 
      WITH FRAME fMain IN WINDOW wWin.
  ENABLE BUTTON-4 COMBO-BOX-Situacion COMBO-BOX-Ubicacion COMBO-BOX-CodDoc 
         FILL-IN-NroPed BUTTON-3 BROWSE-7 BROWSE-2 
      WITH FRAME fMain IN WINDOW wWin.
