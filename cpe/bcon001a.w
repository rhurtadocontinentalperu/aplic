...
&Scoped-define INTERNAL-TABLES CpeTraSed PL-PERS

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table CpeTraSed.CodPer ~
fNomPer(cpetrased.CodPer) @ PL-PERS.nomper CpeTraSed.CodArea ~
CpeTraSed.FechaReg fEstado() @ x-Estado 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH CpeTraSed WHERE ~{&KEY-PHRASE} ~
      AND CpeTraSed.CodCia = s-codcia ~
 AND CpeTraSed.FlgEst = "P" ~
 AND CpeTraSed.CodDiv = s-coddiv NO-LOCK, ~
      EACH PL-PERS WHERE PL-PERS.codper = CpeTraSed.CodPer NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH CpeTraSed WHERE ~{&KEY-PHRASE} ~
      AND CpeTraSed.CodCia = s-codcia ~
 AND CpeTraSed.FlgEst = "P" ~
 AND CpeTraSed.CodDiv = s-coddiv NO-LOCK, ~
      EACH PL-PERS WHERE PL-PERS.codper = CpeTraSed.CodPer NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table CpeTraSed PL-PERS
&Scoped-define FIRST-TABLE-IN-QUERY-br_table CpeTraSed
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CpeTraSed.CodPer FORMAT "X(6)":U WIDTH 6.43
...
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 74 BY 6.69
         FONT 4
         TITLE "PERSONAL ACTIVO REGISTRADO EN EL DIA".
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.CpeTraSed,INTEGRAL.PL-PERS WHERE INTEGRAL.CpeTraSed ..."
...
"CpeTraSed.CodPer" ? ? "character" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"fNomPer(cpetrased.CodPer) @ PL-PERS.nomper" "Nombre" "x(40)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[4]   > INTEGRAL.CpeTraSed.FechaReg
"CpeTraSed.FechaReg" "Fecha-Hora de registro" "99/99/9999 HH:MM:SS" "datetime" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"fEstado() @ x-Estado" "Estado" "x(15)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* PERSONAL ACTIVO REGISTRADO EN EL DIA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* PERSONAL ACTIVO REGISTRADO EN EL DIA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* PERSONAL ACTIVO REGISTRADO EN EL DIA */
DO:
