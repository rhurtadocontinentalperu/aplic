...
DEF TEMP-TABLE Detalle LIKE CpeTareas
...
ON END-ERROR OF wWin /* DETALLE DE TAREAS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* DETALLE DE TAREAS */
DO:
...
FOR EACH CpeTraSed NO-LOCK WHERE Cpetrased.codcia = s-codcia
    ...
    FOR EACH CpeTareas NO-LOCK WHERE CpeTareas.CodCia = s-codcia
        ...
        CREATE Detalle.
        BUFFER-COPY CpeTareas TO Detalle.
...
chWorkSheet:Range("B1"):VALUE = "Area".
chWorkSheet:Range("C1"):VALUE = "N° Tarea".
chWorkSheet:Range("D1"):VALUE = "Inicio".
chWorkSheet:Range("E1"):VALUE = "Termino".
chWorkSheet:Range("F1"):VALUE = "Tiempo Trabajado".
chWorkSheet:Range("G1"):VALUE = "Tiempo otras tareas".
chWorkSheet:Range("H1"):VALUE = "Items".
chWorkSheet:Range("I1"):VALUE = "Importe".
chWorkSheet:Range("J1"):VALUE = "Orden".
chWorkSheet:Range("K1"):VALUE = "Numero".
chWorkSheet:Range("L1"):VALUE = "Almacén".
chWorkSheet:Range("M1"):VALUE = "Zona".
chWorkSheet:Range("N1"):VALUE = "Descripción de la tarea".
...
FOR EACH Detalle, FIRST pl-pers WHERE pl-pers.codper = Detalle.codper NO-LOCK:
    t-Row = t-Row + 1.
    t-Column = 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.codper + ' ' + TRIM(pl-pers.patper) + ' ' + 
                                        TRIM(pl-pers.matper) + ', ' + pl-pers.nomper.
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.codarea.
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.nrotarea.
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.FchInicio.
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.FchFin.
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = ROUND(Detalle.tiempotarea / 3600, 2).
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = ROUND(Detalle.tiempootros / 3600, 2).
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.totitems.
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.totimporte.
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.codod.
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.nrood.
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.codalm.
    t-column = t-column + 1.
    chWorkSheet:Cells(t-Row, t-Column) = Detalle.codzona.
    FIND VtaTabla WHERE VtaTabla.codcia = s-codcia
        AND VtaTabla.llave_c1 = Detalle.tpotarea
        AND vtaTabla.tabla = 'CPETAREA'
        NO-LOCK NO-ERROR.
    IF AVAILABLE VtaTabla THEN DO:
        t-column = t-column + 1.
        chWorkSheet:Cells(t-Row, t-Column) = VtaTabla.llave_c2.
    END.
END.
