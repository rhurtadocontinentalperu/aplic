...
&Scoped-Define ENABLED-OBJECTS br_table FILL-IN_Descrip BUTTON_Down ~
BUTTON_Up BUTTON_Exit 
...
DEFINE FRAME F-Main
     ...
     FILL-IN_Descrip AT ROW 12.85 COL 4 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     BUTTON_Down AT ROW 12.85 COL 29 WIDGET-ID 14
     BUTTON_Up AT ROW 12.85 COL 34 WIDGET-ID 16
     BUTTON_Exit AT ROW 12.85 COL 40 WIDGET-ID 18
...
DO:
    ...
    CREATE QUERY h-Query.
    h-Query:SET-BUFFERS(BUFFER b-almmmatg:HANDLE).
    h-Query:QUERY-PREPARE("FOR EACH b-almmmatg NO-LOCK BY b-almmmatg.Descrip DESC").
    h-Query:QUERY-OPEN.
    h-Query:REPOSITION-TO-ROWID(ROWID(almmmatg)).
    h-Query:GET-NEXT().
    h-Query:GET-NEXT().
