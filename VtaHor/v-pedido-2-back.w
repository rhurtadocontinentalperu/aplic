...
&Scoped-Define DISPLAYED-FIELDS FacCPedi.Atencion FacCPedi.CodCli ~
FacCPedi.NroRef FacCPedi.NroPed FacCPedi.FchPed FacCPedi.NomCli ~
...
DEFINE FRAME F-Main
     ...
     C-TpoVta AT ROW 4.27 COL 88 COLON-ALIGNED
...
------------------------------------------------------------------------------*/
 IF NOT AVAILABLE FacCPedi THEN DO:
    ...
    END.                                                        
 IF FaccPedi.FchVen < TODAY THEN DO:
       MESSAGE 'Pedido Vencido. NO disponible' VIEW-AS ALERT-BOX ERROR.
       UNDO, RETURN 'ADM-ERROR'.
 END.

 IF FacCPedi.FlgEst = "P" AND FacCPedi.FlgSit = "" 
 THEN DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
     MESSAGE 'Confirmar envío del Pedido a Chequeo' VIEW-AS ALERT-BOX QUESTION
         BUTTONS YES-NO UPDATE rpta AS LOG.
     IF rpta = NO THEN RETURN 'ADM-ERROR'.
     FIND CURRENT Faccpedi EXCLUSIVE-LOCK NO-ERROR.
     ASSIGN
         Faccpedi.FlgSit = 'X'.      /* Por chequear */
...
------------------------------------------------------------------------------*/
    ...
    IF S-CODDOC = "PED" AND FacCPedi.FlgEst <> "A" THEN DO:
    ...
    FOR EACH PEDI NO-LOCK BREAK BY ALMDES:
        ...
        IF LAST-OF(AlmDes) THEN DO:
            F-BOL = IF INTEGER(FacCPedi.CodMon:SCREEN-VALUE) = 1 
                    THEN F-TOT
                    ELSE F-Tot * DECIMAL(FacCPedi.TpoCmb:SCREEN-VALUE).
            IF C-TpoVta:SCREEN-VALUE = 'Boleta' AND F-BOL > 700 
