...
&SCOPED-DEFINE l-lookup h_l-cotpen-3
...
DEFINE VARIABLE h_l-cotpen-3 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtahor/l-cotpen-3.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-cotpen-3 ).
       RUN set-position IN h_l-cotpen-3 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_l-cotpen-3 ( 10.92 , 58.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.38 , 60.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-cotpen-3. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-cotpen-3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-cotpen-3 ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-cotpen-3 , 'AFTER':U ).
    END. /* Page 0 */
