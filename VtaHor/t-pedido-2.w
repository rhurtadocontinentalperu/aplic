...
------------------------------------------------------------------------------*/
  ...
  f-CanPed = 0.
  IF s-NroPed <> '?' THEN DO:
      FIND D-PEDI WHERE D-PEDI.codcia = s-codcia
          AND D-PEDI.coddoc = 'PED'
          AND D-PEDI.nroped = s-NroPed
          AND D-PEDI.codmat = PEDI.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
          NO-LOCK NO-ERROR.
      IF AVAILABLE D-PEDI THEN f-CanPed = D-PEDI.canped.
  END.
  FIND D-PEDI WHERE D-PEDI.codcia = s-codcia
    AND D-PEDI.coddoc = 'COT'
    AND D-PEDI.nroped = s-NroCot
    AND D-PEDI.codmat = PEDI.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
    NO-LOCK NO-ERROR.
  IF (D-PEDI.CanPed - D-PEDI.CanAte + f-CanPed) < DECIMAL(PEDI.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME})
  THEN DO:
    MESSAGE 'NO se puede pedir mas de ' STRING(D-PEDI.CanPed - D-PEDI.CanAte + f-CanPed)
        PEDI.UndVta SKIP
        VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO PEDI.AlmDes  IN BROWSE {&BROWSE-NAME}. 
    RETURN "ADM-ERROR".
