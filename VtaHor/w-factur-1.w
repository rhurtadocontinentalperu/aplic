...
DEFINE VARIABLE h_v-factur-2 AS HANDLE NO-UNDO.
...
DO:
  ...
  IF p-Ok = 'YES'
  THEN RUN Imprimir IN h_v-factur-2 (INPUT YES).
  ELSE RUN Imprimir IN h_v-factur-2 (INPUT NO).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vtahor/v-factur-2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-factur-2 ).
       RUN set-position IN h_v-factur-2 ( 1.00 , 2.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-factur-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_v-factur-2 ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-factur-2 ).

       /* Links to SmartQuery h_q-docmto. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_q-docmto ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-factur-2 ,
             C-CodDoc:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-factur-2 , 'AFTER':U ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-size IN h_b-tmpdoc ( 9.31 , 87.00 ) NO-ERROR.
