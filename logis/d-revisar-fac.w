...
&Scoped-Define ENABLED-OBJECTS RECT-70 Btn_Cancel 
...
DEFINE FRAME D-Dialog
     Btn_Cancel AT ROW 2.08 COL 122
     RECT-70 AT ROW 1.27 COL 2 WIDGET-ID 2
     SPACE(21.99) SKIP(15.02)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/logis/v-mantto-fact-credito.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-mantto-fact-credito ).
       RUN set-position IN h_v-mantto-fact-credito ( 1.54 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.27 , 117.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/logis/b-mantto-fact-credito.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-mantto-fact-credito ).
       RUN set-position IN h_b-mantto-fact-credito ( 9.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-mantto-fact-credito ( 14.81 , 140.00 ) NO-ERROR.
       ...
       RUN set-position IN h_q-comprobante-unico ( 3.96 , 126.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.50 , 6.86 ) */

       /* Links to SmartViewer h_v-mantto-fact-credito. */
       RUN add-link IN adm-broker-hdl ( h_q-comprobante-unico , 'Record':U , h_v-mantto-fact-credito ).

       /* Links to SmartBrowser h_b-mantto-fact-credito. */
       RUN add-link IN adm-broker-hdl ( h_q-comprobante-unico , 'Record':U , h_b-mantto-fact-credito ).
