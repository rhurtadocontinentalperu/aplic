...
&Scoped-define FIELDS-IN-QUERY-br_table DI-RutaC.CodRut DI-RutaC.FchDoc ~
DI-RutaC.NroDoc fEstado() @ x-Estado tt-DI-RutaC.Libre_d01 ~
...
DI-RutaC.Libre_f05 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table DI-RutaC.CodRut ~
DI-RutaC.Observ 
...
&Scoped-Define ENABLED-OBJECTS COMBO-BOX-cuales COMBO-BOX_CodPed ~
FILL-IN_NroPed FILL-IN-contenido br_table BUTTON-Aplicar FILL-IN-Desde ~
FILL-IN-Hasta RADIO-SET-FlgEst 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-cuales COMBO-BOX_CodPed ~
FILL-IN_NroPed FILL-IN-contenido FILL-IN-Desde FILL-IN-Hasta ~
RADIO-SET-FlgEst FILL-IN-Peso FILL-IN-Volumen FILL-IN-Importe ~
FILL-IN-Bultos FILL-IN-SKUs FILL-IN-SKUnochk 
...
DEFINE VARIABLE COMBO-BOX_CodPed AS CHARACTER FORMAT "X(256)":U INITIAL "O/D" 
     LABEL "Pedido" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "O/D","OTR" 
     DROP-DOWN-LIST
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      DI-RutaC.CodRut COLUMN-LABEL "PRIORIDAD" FORMAT "X(4)":U
            COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 11 COLUMN-FONT 6 VIEW-AS COMBO-BOX INNER-LINES 10
                      LIST-ITEMS "uno" 
                      DROP-DOWN-LIST 
...
DEFINE FRAME F-Main
     ...
     COMBO-BOX_CodPed AT ROW 1 COL 68 COLON-ALIGNED WIDGET-ID 34
     FILL-IN_NroPed AT ROW 1 COL 80 COLON-ALIGNED WIDGET-ID 14
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > INTEGRAL.DI-RutaC.CodRut
"DI-RutaC.CodRut" "PRIORIDAD" ? "character" 11 0 6 ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," "uno" ? 10 no 0 no no
...
"fEstado() @ x-Estado" "Estado" "x(20)" ? ? ? ? ? ? ? no ? no no "12.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.tt-DI-RutaC.Libre_d01
...
"tt-DI-RutaC.Libre_d06 @ x-Volumen" "Volumen" ">>>,>>9.99" ? ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.tt-DI-RutaC.Libre_d02
"tt-DI-RutaC.Libre_d02" "Importe" ">>>,>>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "10.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.tt-DI-RutaC.Libre_d04
...
"tt-DI-RutaC.Libre_d08 @ x-SKUnochk" "SKUs!No Cheq." ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > Temp-Tables.tt-DI-RutaC.Libre_d03
...
"tt-DI-RUTAC.libre_d07 @ x-ptos-entrega" "Ptos!Entrega" "->,>>9" ? ? ? ? ? ? ? no ? no no "5.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > INTEGRAL.DI-RutaC.Observ
...
DO:
  ASSIGN FILL-IN-Desde FILL-IN-Hasta RADIO-SET-FlgEst combo-box-cuales fill-in-contenido .
  ASSIGN COMBO-BOX_CodPed FILL-IN_NroPed.
...
x-NroPed = FILL-IN_NroPed.
x-CodPed = COMBO-BOX_CodPed.
...
DO k = 1 TO NUM-ENTRIES(RADIO-SET-FlgEst,'|'):
    ...
    FOR EACH DI-RutaC NO-LOCK WHERE DI-RutaC.CodCia = s-codcia AND
        ...
        IF DI-RutaC.FchDoc > FILL-IN-Hasta THEN NEXT.
        ...
        IF x-NroPed > "" THEN DO:
            ...
            FOR EACH x-di-rutaD WHERE x-di-rutaD.codcia = s-codcia AND
                x-di-rutaD.coddoc = s-coddoc AND
                x-di-rutaD.codref = x-codped AND
                x-di-rutaD.nroref = x-nroped NO-LOCK :
                IF x-di-rutaD.nrodoc = x-nro-phr THEN DO:
                    x-existe = YES.
                    LEAVE BUSCAR_OD.
                END.
            END.
            IF x-existe = NO THEN NEXT FORLOOP.
        END.
        /* Cliente */
        x-cuales = combo-box-cuales.
        x-existe = YES.    
        IF x-contenido > "" THEN DO:
            IF x-cuales > 1 THEN DO:
                ...
                FOR EACH x-di-rutaD WHERE x-di-rutaD.codcia = s-codcia AND
                    ...
                    IF AVAILABLE x-faccpedi THEN DO:
                        FIND FIRST tt-Clientes WHERE tt-Clientes.tcodcli = x-faccpedi.codcli NO-LOCK NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Temporal-x-probar B-table-Win 
PROCEDURE Carga-Temporal-x-probar :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
x-codped = COMBO-BOX_CodPed:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
x-nroped = FILL-IN_NroPed:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
RLOOP:
DO k = 1 TO NUM-ENTRIES(RADIO-SET-FlgEst,'|'):
    cFlgEst = ENTRY(k, RADIO-SET-FlgEst, '|').
    FOR EACH DI-RutaC NO-LOCK WHERE DI-RutaC.CodCia = s-codcia AND
        ...
        DI-RutaC.CodDoc = s-coddoc AND
        DI-RutaC.FlgEst = cFlgEst AND 
        DI-RutaC.FchDoc >= FILL-IN-Desde:

        FOR EACH x-di-rutaD OF DI-RutaC NO-LOCK,
            FIRST faccpedi WHERE faccpedi.codcia = s-codcia AND
                        faccpedi.coddoc = x-di-rutaD.codref AND
                        faccpedi.nroped = x-di-rutaD.nroref NO-LOCK :
    
            IF DI-RutaC.FchDoc > FILL-IN-Hasta THEN NEXT.
            /*
            FIND FIRST faccpedi WHERE faccpedi.codcia = s-codcia AND
                        faccpedi.coddoc = x-di-rutaD.codref AND
                        faccpedi.nroped = x-di-rutaD.nroref NO-LOCK NO-ERROR.
    
            IF NOT AVAILABLE faccpedi THEN NEXT.
            */
            IF NOT (TRUE <> (x-NroPed > "")) THEN DO:
                x-nro-phr = di-rutaC.nrodoc.
                x-existe = NO.
                IF x-di-rutaD.coddoc = "PHR" AND x-di-rutaD.codref = x-CodPed AND 
                    x-di-rutaD.nroref = x-NroPed AND x-di-rutaD.nrodoc = x-nro-phr THEN DO:
                    x-existe = YES.
                END.
                ELSE DO:
                    NEXT.
            ...
            IF NOT (TRUE <> (x-contenido > "")) THEN DO:
                IF x-cuales > 1 THEN DO:
                    ...
                    IF AVAILABLE tt-clientes THEN DO:
                        x-existe = YES.
                    END.
                    IF x-existe = NO THEN NEXT.
                END.
            END.
    
            FIND FIRST tt-DI-RutaC WHERE tt-DI-RutaC.codcia = DI-RutaC.codcia AND
                                            tt-DI-RutaC.coddiv = DI-RutaC.CodDiv AND
                                            tt-DI-RutaC.coddoc = DI-RutaC.CodDoc AND
                                            tt-DI-RutaC.nrodoc = DI-RutaC.NroDoc EXCLUSIVE-LOCK NO-ERROR.
            /* DATOS ADICIONALES */
            ...
            ELSE DO:
                ...
                IF AVAILABLE Gn-tccja THEN
                    ASSIGN tt-Di-RutaC.Libre_d02 = tt-Di-RutaC.Libre_d02 + (IF Faccpedi.codmon = 2 THEN Gn-tccja.venta * Faccpedi.ImpTot ELSE Faccpedi.ImpTot).
                ELSE 
                    ASSIGN tt-Di-RutaC.Libre_d02 = tt-Di-RutaC.Libre_d02 + (IF Faccpedi.codmon = 2 THEN Faccpedi.TpoCmb * Faccpedi.ImpTot ELSE Faccpedi.ImpTot).
            END.       
            /* ******************************* */
            /* Ic - 04Set2020 Puntos de entrega a pedido de Fernan segun meet avalado po Daniel Llican y Maz Ramos */         
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR cPrioridad AS CHAR NO-UNDO.
  DEF VAR cEvento AS CHAR INIT "PHR_PRIORITY" NO-UNDO.
  DEF VAR lGrabar AS LOG INIT NO NO-UNDO.

  RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
  IF RETURN-VALUE = "YES" 
      THEN ASSIGN cEstado = "CREATE" cPrioridad = "".
      ELSE ASSIGN cEstado = "UPDATE" cPrioridad = DI-RutaC.CodRut.
  ...
  lGrabar = NO.
  IF cEstado = "CREATE" AND DI-RutaC.CodRut > "" THEN DO:
      lGrabar = YES.
  END.
  ELSE DO:
      IF (cPrioridad > '' AND cPrioridad <> DI-RutaC.CodRut) OR 
          (TRUE <> (cPrioridad > '') AND DI-RutaC.CodRut > '') THEN DO:
          lGrabar = YES.
      END.
  END.
  IF lGrabar THEN DO:
      ...
      ASSIGN
          LogisLogControl.CodCia = DI-RutaC.CodCia 
          LogisLogControl.CodDiv = DI-RutaC.CodDiv 
          LogisLogControl.CodDoc = DI-RutaC.CodDoc 
          LogisLogControl.NroDoc = DI-RutaC.NroDoc
          LogisLogControl.Evento = cEvento
          LogisLogControl.Fecha  = TODAY
          LogisLogControl.Hora   = STRING(TIME,'HH:MM:SS')
          LogisLogControl.Usuario = s-user-id
          LogisLogControl.Libre_c01 = DI-RutaC.CodRut.
  END.
...
------------------------------------------------------------------------------*/
  ...
  /* Solo para algunas condiciones */
  IF LOOKUP(TRIM(DI-RutaC.FlgEst), 'P,PX') = 0 THEN DO:
      MESSAGE "No se puede anular la PHR" SKIP
          "Solo se pueden anular PHR con CHEQUEO OK o GENERADAS" SKIP
          "Acceso Denegado"
  ...
  IF CAN-FIND(FIRST Di-RutaD OF Di-RutaC NO-LOCK) THEN DO:
      MESSAGE "No se puede anular si tiene pedidos registrados" SKIP
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      /* 22/08/2025: Eliminamos también el detalle */
      FOR EACH DI-RutaD OF Di-RutaC EXCLUSIVE-LOCK ON ERROR UNDO, THROW:
          DELETE DI-RutaD.
      END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      /* 17/9/2025: Solo para PX, PK o PF se pude asignar/modificar prioridades */
      IF LOOKUP(TRIM(Di-RutaC.FlgEst), "PX,PK,PF") > 0 
          THEN DI-RutaC.CodRut:READ-ONLY IN BROWSE {&BROWSE-NAME} = NO.
      ELSE DI-RutaC.CodRut:READ-ONLY IN BROWSE {&BROWSE-NAME} = YES.
  END.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  DO WITH FRAME {&FRAME-NAME}:
      DEF VAR x-Numeros AS CHAR INIT " " NO-UNDO.
      DEF VAR i AS INTE NO-UNDO.
      DO i = 1 TO 100:
          x-Numeros = x-Numeros + "," + STRING(i).
      END.
      DI-RutaC.CodRut:DELETE(1) IN BROWSE {&BROWSE-NAME}.
      DI-RutaC.CodRut:ADD-LAST(x-Numeros) IN BROWSE {&BROWSE-NAME}.
  END.
