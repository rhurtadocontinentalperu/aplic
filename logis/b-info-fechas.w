...
&Scoped-define FIELDS-IN-QUERY-br_table t-report.Campo-D[1] ~
...
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 45 BY 9.69
         FONT 4
         TITLE "FECHA DE GENERACION HPK" FIT-LAST-COLUMN.
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > Temp-Tables.t-report.Campo-D[1]
"t-report.Campo-D[1]" "Fecha" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* FECHA DE GENERACION HPK */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* FECHA DE GENERACION HPK */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* FECHA DE GENERACION HPK */
DO:
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Vtacdocu NO-LOCK WHERE VtaCDocu.CodCia = s-codcia
      ...
      FIND t-report WHERE Campo-D[1] = VtaCDocu.FchPed NO-ERROR.
      IF NOT AVAILABLE t-report THEN CREATE t-report.
      ASSIGN
          Campo-D[1] = VtaCDocu.FchPed.
      ASSIGN
          Campo-F[3] = Campo-F[3] + Vtacdocu.Items.
  END.
  FOR EACH t-report EXCLUSIVE-LOCK,
      EACH Vtacdocu NO-LOCK WHERE VtaCDocu.CodCia = s-codcia
      AND VtaCDocu.CodDiv = s-coddiv
      AND VtaCDocu.CodPed = 'HPK'
      AND Vtacdocu.FlgEst = 'P'
      AND Vtacdocu.FchPed = t-report.Campo-D[1]
      AND LOOKUP(Vtacdocu.FlgSit, 'PC,C') > 0:
      ASSIGN
          Campo-F[2] = Campo-F[2] + Vtacdocu.Items.
  END.
  FOR EACH t-report EXCLUSIVE-LOCK,
      EACH Vtacdocu NO-LOCK WHERE VtaCDocu.CodCia = s-codcia
      AND VtaCDocu.CodDiv = s-coddiv
      ...
      ASSIGN
          Campo-F[1] = Campo-F[1] + Vtacdocu.Items.
  END.

  DO WITH FRAME {&FRAME-NAME}:
      FILL-IN-Total = 0.
      FOR EACH t-report NO-LOCK:
          FILL-IN-Total = FILL-IN-Total + t-report.Campo-F[3].
      END.
