...
IF NOT LOOKUP(DI-RutaC.FlgEst, 'PF,PX,PK') > 0 THEN DO:
    pMensaje = "Solo se puede Adicionar o Reprogramar las PHR que estén Pendientes de Almacén (PF, PX o PK)".
    RETURN.
END.
  ...
  CASE COMBO-BOX-CodDoc:
      ...
      WHEN "OTR" THEN DO:
          FIND FIRST Faccpedi WHERE Faccpedi.codcia = s-codcia AND
              ...
              Faccpedi.TpoPed = "XD" */ NO-LOCK NO-ERROR.
          IF NOT AVAILABLE Faccpedi THEN DO:
...
FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia AND
    ...
    /* 28/04/2026: Chirinos, se filtra las G/R por la O/D referenciada */
    Ccbcdocu.libre_c01 = COMBO-BOX-CodDoc AND   /* O/D */
    Ccbcdocu.libre_c02 = FILL-IN-NroDoc:
...
FIND FIRST Faccpedi WHERE FacCPedi.CodCia = s-codcia
    ...
    AND FacCPedi.FlgEst = "C" NO-LOCK NO-ERROR.
IF NOT AVAILABLE Faccpedi THEN DO:
    pMensaje = 'OTR no existe o no esta documentada'.
    RETURN 'ADM-ERROR'.
END.
*/
FIND FIRST Faccpedi WHERE Faccpedi.codcia = s-codcia AND
