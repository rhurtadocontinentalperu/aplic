...
DEFINE VARIABLE h_v-mantto-gr-cred-dbs AS HANDLE NO-UNDO.
...
DO:
   RUN Asigna-datos IN h_v-mantto-gr-cred-dbs.
END.
...
DO:
  RUN Imprimir-Transportista IN h_v-mantto-gr-cred-dbs.
END.
...
DO:
   ...
   RUN Copiar-Guia IN h_v-mantto-gr-cred-dbs (OUTPUT pParam).
...
DO:
   RUN dispatch IN h_v-mantto-gr-cred-dbs ('delete-record':U).
END.
...
DO:
  RUN dispatch IN h_v-mantto-gr-cred-dbs ('imprime':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'logis/v-mantto-gr-cred-dbs.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-mantto-gr-cred-dbs ).
       RUN set-position IN h_v-mantto-gr-cred-dbs ( 2.62 , 3.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-mantto-gr-cred-dbs. */
       RUN add-link IN adm-broker-hdl ( h_q-guias-remision , 'Record':U , h_v-mantto-gr-cred-dbs ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-mantto-gr-cred-dbs ,
             h_p-navico , 'AFTER':U ).
