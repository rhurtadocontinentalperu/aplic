...
&Scoped-Define ENABLED-OBJECTS RADIO-SET-1 TOGGLE-formato-bcp Btn_OK ~
Btn_Cancel 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-1 TOGGLE-formato-bcp 
...
DEFINE VARIABLE RADIO-SET-1 AS INTEGER INITIAL 1 
     ...
     RADIO-BUTTONS 
          "Formato Pre-impreso Normal", 1,
"Formato Pre-Impreso con Codigos EAN", 2,
"Papel en Blanco", 3
     SIZE 29 BY 3.31 NO-UNDO.

DEFINE VARIABLE TOGGLE-formato-bcp AS LOGICAL INITIAL no 
     LABEL "¿Usar formato BCP?" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME D-Dialog
     RADIO-SET-1 AT ROW 1.27 COL 9 NO-LABEL
     TOGGLE-formato-bcp AT ROW 4.96 COL 12 WIDGET-ID 4
     Btn_OK AT ROW 6.15 COL 9
     Btn_Cancel AT ROW 6.15 COL 23
     SPACE(9.71) SKIP(0.34)
...
DO:
  ASSIGN RADIO-SET-1 toggle-formato-bcp.

  DEFINE VAR x-docx AS CHAR.  

    x-docx = "G/R|NO".
    IF toggle-formato-bcp = YES THEN DO:
        x-docx = "G/R|SI".
    END.
  ...
  CASE TRUE:
      WHEN x-docx = "SI" THEN DO:
        RUN gn/p-impresion-gral-gr.r (INPUT x-docx,
                                      INPUT x-Rowid,
                                      INPUT 1).
      END.
      WHEN RADIO-SET-1 = 1 THEN DO:
          SYSTEM-DIALOG PRINTER-SETUP UPDATE pStatus.
          IF pStatus = NO THEN RETURN.  
          RUN logis/library-print-gr-manuales.p PERSISTENT SET hProc.
          RUN GRIMP_Rutina-Principal IN hProc (INPUT x-docx,
                                               INPUT x-Rowid,
                                               INPUT 1).
          DELETE PROCEDURE hProc.
      END.
      WHEN RADIO-SET-1 = 2 THEN DO:
          SYSTEM-DIALOG PRINTER-SETUP UPDATE pStatus.
          IF pStatus = NO THEN RETURN.  
          RUN logis/library-print-gr-manuales.p PERSISTENT SET hProc.
          RUN GRIMP_Rutina-Principal IN hProc (INPUT x-docx,
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-1 TOGGLE-formato-bcp 
      WITH FRAME D-Dialog.
  ENABLE RADIO-SET-1 TOGGLE-formato-bcp Btn_OK Btn_Cancel 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
