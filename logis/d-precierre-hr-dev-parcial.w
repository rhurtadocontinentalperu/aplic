...
RUN Carga-Browse.
IF NOT CAN-FIND(FIRST T-RutaDv NO-LOCK) THEN RETURN 'ADM-ERROR'.    /* ERROR.*/
...
DO:
  ...
  IF x-CanDev = 0
  THEN DO:
      MESSAGE "No hay devoluciones" VIEW-AS ALERT-BOX INFORMATION.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE Di-RutaDv THEN DO:      /* Cargamos la informacion */
  ...
  ELSE DO:      /* Creamos los registros */
      ...
      FOR EACH ccbddocu OF ccbcdocu NO-LOCK:
          FOR FIRST Almcmov NO-LOCK WHERE Almcmov.codcia = FACTURA.codcia AND
              ...
              Almcmov.flgest <> 'A',
              FIRST Almdmov OF Almcmov WHERE Almdmov.codmat = Ccbddocu.codmat NO-LOCK:
              CREATE T-RutaDv.
              BUFFER-COPY CcbDDocu TO T-RutaDv
                  ASSIGN
                      ...
                      T-RutaDv.CanDev = (Almdmov.candes * Almdmov.factor).
          END.
      END.
      /* CONSISTENCIA: Solo devoluciones parcial, NO TOTALES */
      ...
      IF x-Total-1 = x-Total-2 THEN DO:
          MESSAGE 'Se ha devuelto TOTALMENTE la' FACTURA.coddoc FACTURA.nrodoc VIEW-AS ALERT-BOX WARNING.
          EMPTY TEMP-TABLE T-RutaDv.
      END.
  END.
