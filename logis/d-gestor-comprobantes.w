...
TOGGLE-genera-pgre BROWSE-4 FILL-IN-tipo 
...
FILL-IN-Glosa TOGGLE-genera-pgre FILL-IN-tipo FILL-IN-msg 
...
DEFINE VARIABLE FILL-IN-msg AS CHARACTER FORMAT "X(256)":U INITIAL "Texto adicional..." 
      VIEW-AS TEXT 
     SIZE 101 BY .77
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-4 D-Dialog _STRUCTURED
  QUERY BROWSE-4 NO-LOCK DISPLAY
      ...
      PEDI.AlmDes COLUMN-LABEL "Almacén" FORMAT "x(8)":U WIDTH 17.43
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME D-Dialog
     ...
     FILL-IN-LugEnt AT ROW 4.77 COL 11 COLON-ALIGNED WIDGET-ID 30
     FILL-IN-Glosa AT ROW 5.58 COL 11 COLON-ALIGNED WIDGET-ID 28
     TOGGLE-genera-pgre AT ROW 6.69 COL 13.14 WIDGET-ID 44
     BROWSE-4 AT ROW 9.08 COL 2 WIDGET-ID 200
     FILL-IN-tipo AT ROW 3.27 COL 50.86 COLON-ALIGNED NO-LABEL WIDGET-ID 42
     FILL-IN-msg AT ROW 7.73 COL 11 COLON-ALIGNED NO-LABEL WIDGET-ID 52
     RECT-1 AT ROW 1.27 COL 2 WIDGET-ID 4
     SPACE(1.28) SKIP(15.60)
...
/* SETTINGS FOR DIALOG-BOX D-Dialog
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-4 TOGGLE-genera-pgre D-Dialog */
ASSIGN 
...
/* SETTINGS FOR FILL-IN FILL-IN-NroPed IN FRAME D-Dialog
   NO-ENABLE                                                            */
ASSIGN 
       TOGGLE-genera-pgre:MANUAL-HIGHLIGHT IN FRAME D-Dialog = TRUE.
...
"PEDI.AlmDes" "Almacén" "x(8)" "character" ? ? ? ? ? ? no ? no no "17.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-4 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-NroPed FILL-IN-Cliente FILL-IN-DirClie COMBO-NroSer 
          FILL-IN-NroDoc FILL-IN-items FILL-IN-LugEnt FILL-IN-Glosa 
          TOGGLE-genera-pgre FILL-IN-tipo FILL-IN-msg 
      WITH FRAME D-Dialog.
  ENABLE RECT-1 Btn_OK Btn_Cancel COMBO-NroSer TOGGLE-genera-pgre BROWSE-4 
         FILL-IN-tipo 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      fill-in-msg:SCREEN-VALUE = "El pedido logistico SI va generar GUIA DE REMISION".
      ...
      IF AVAILABLE b-faccpedi THEN DO:
          ...
          IF b-faccpedi.cliente_recoge = YES OR b-faccpedi.TipVta = 'si' THEN DO:
              fill-in-msg:SCREEN-VALUE = "El pedido logistico es CLIENTE RECOGE por lo tanto no genera GUIA DE REMISION".
              IF b-faccpedi.TipVta = 'si' THEN DO:
                  fill-in-msg:SCREEN-VALUE = "El pedido logistico es TRAMITE DOCUMENTARIO por lo tanto no genera GUIA DE REMISION".
              END.
...
FOR EACH Reporte NO-LOCK, FIRST Ccbcdocu OF Reporte NO-LOCK:
    ...
    IF RETURN-VALUE = "OK" OR RETURN-VALUE = "PLAN-B" THEN DO:
        ...
        IF toggle-genera-pgre = NO THEN DO:
            ASSIGN Gre_Cmpte.estado = "NO GENERA PGRE".
        END.
