...
DEFINE OUTPUT PARAMETER pNuevaMesa AS CHAR.
...
FIND FIRST rut-pers-turno WHERE rut-pers-turno.codcia = s-codcia AND
                                rut-pers-turno.coddiv = s-coddiv AND
                                rut-pers-turno.dni = pCodigo AND
                                rut-pers-turno.rol = "CHEQUEADOR" AND
                                rut-pers-turno.fchasigna = pFecha NO-LOCK NO-ERROR.
IF NOT AVAILABLE rut-pers-turno THEN DO:
    MESSAGE "Codigo (" + pCOdigo + ") no esta registrado" VIEW-AS ALERT-BOX INFORMATION.
    RETURN ERROR.
END.

IF NOT CAN-FIND(FIRST chkMesas WHERE chkMesas.codcia = s-codcia AND
                chkMesas.coddiv = s-coddiv AND
                chkmesas.flgest = 'L' NO-LOCK)
    THEN DO:
    MESSAGE 'NO hay mesas libres' SKIP 'Proceso abortado' VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
...
DEFINE VARIABLE COMBO-BOX-mesa AS CHARACTER FORMAT "X(10)":U 
     LABEL "Seleccione la nueva mesa" 
...
DEFINE FRAME D-Dialog
     ...
     COMBO-BOX-mesa AT ROW 4.46 COL 23.86 COLON-ALIGNED WIDGET-ID 6
     Btn_OK AT ROW 6.19 COL 31
     Btn_Cancel AT ROW 6.19 COL 49
     "Cambio de Mesa" VIEW-AS TEXT
          SIZE 30 BY .96 AT ROW 4.35 COL 44.57 WIDGET-ID 8
          BGCOLOR 15 FGCOLOR 9 FONT 9
     SPACE(1.99) SKIP(3.30)
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Datos del Chequeador */
DO:  
...
DO:
    ...
    IF COMBO-BOX-mesa:SCREEN-VALUE = ? OR COMBO-BOX-mesa:SCREEN-VALUE = "" THEN DO:                
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR x-nombre-pers AS CHAR.
  DEFINE VAR x-origen AS CHAR.

  RUN logis/p-busca-por-dni(INPUT pCodigo, OUTPUT x-nombre-pers, OUTPUT x-origen).
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      fill-in-trabajador:SCREEN-VALUE = x-nombre-pers.

      /*
      FIND FIRST x-pl-pers WHERE x-pl-pers.codper = pCodigo NO-LOCK NO-ERROR.

      IF AVAILABLE x-pl-pers THEN DO:
          fill-in-trabajador:SCREEN-VALUE = x-pl-pers.patper + " " + x-pl-pers.matper + " " + nomper.
      END.
      */
  END.
