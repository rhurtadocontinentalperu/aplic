...
(x-FchDoc-1 = ? OR Faccpedi.fchped >= x-FchDoc-1) AND ~
(x-FchDoc-2 = ? OR Faccpedi.fchped <= x-FchDoc-2) AND ~
(TRUE <> (x-CodCli > '') OR Faccpedi.codcli = x-codcli) AND ~
(TRUE <> (x-NomCli > '') OR INDEX(Faccpedi.nomcli, x-NomCli) > 0) AND ~
(TRUE <> (x-NroPed > '') OR Faccpedi.nroped = x-NroPed) AND ~
Faccpedi.flgest <> 'A' ~
)

&SCOPED-DEFINE Condicion2 ( ~
Vtacdocu.codped = 'HPK' AND ~
(TRUE <> (x-NroHpk > '') OR Vtacdocu.nroped = x-NroHpk) AND ~
Vtacdocu.flgest <> 'A' ~
)
...
&Scoped-define INTERNAL-TABLES FacCPedi VtaCDocu
...
VtaCDocu.CodPed VtaCDocu.NroPed FacCPedi.CodCli FacCPedi.NomCli ~
FacCPedi.FchEnt FacCPedi.FchPed 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion}  NO-LOCK, ~
      EACH VtaCDocu WHERE VtaCDocu.CodCia = FacCPedi.CodCia ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
  ...
  AND VtaCDocu.NroRef = FacCPedi.NroPed ~
      AND {&Condicion2} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table FacCPedi VtaCDocu
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      VtaCDocu.NroPed COLUMN-LABEL "Numero" FORMAT "X(12)":U WIDTH 12.14
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.FacCPedi,INTEGRAL.VtaCDocu WHERE INTEGRAL.FacCPedi ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = "USED, USED"
     _Where[1]         = "{&Condicion} "
     _JoinCode[2]      = "VtaCDocu.CodCia = FacCPedi.CodCia
  AND VtaCDocu.CodDiv = FacCPedi.DivDes
  AND VtaCDocu.CodRef = FacCPedi.CodDoc
  AND VtaCDocu.NroRef = FacCPedi.NroPed"
     _Where[2]         = "{&Condicion2}"
...
"FacCPedi.NroPed" "Numero" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = INTEGRAL.VtaCDocu.CodPed
     _FldNameList[4]   > INTEGRAL.VtaCDocu.NroPed
"VtaCDocu.NroPed" "Numero" ? "character" ? ? ? ? ? ? no ? no no "12.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.CodCli" ? ? "character" ? ? ? ? ? ? no ? no no "12.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[8]   > INTEGRAL.FacCPedi.FchPed
