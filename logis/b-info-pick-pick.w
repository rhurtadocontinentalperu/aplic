...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 114.72 BY 7
...
DEFINE FRAME F-Main
     ...
     FILL-IN-Pedidos AT ROW 9.62 COL 52.86 COLON-ALIGNED NO-LABEL WIDGET-ID 30
     FILL-IN-Producidos AT ROW 9.62 COL 61.86 COLON-ALIGNED NO-LABEL WIDGET-ID 14
     FILL-IN-Peso AT ROW 9.62 COL 71.72 COLON-ALIGNED NO-LABEL WIDGET-ID 16
     FILL-IN-Volumen AT ROW 9.62 COL 83.86 COLON-ALIGNED NO-LABEL WIDGET-ID 18
...
"t-report-2.Campo-F[5]" "Tiempo!(Horas)" ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no "8.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.t-report-2.Campo-F[6]
"t-report-2.Campo-F[6]" "Tramos" ">>>,>>9" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR x-Inicio-PK AS DATETIME NO-UNDO.
  DEF VAR x-Fin-PK AS DATETIME NO-UNDO.
  ...
  FOR EACH LogTrkDocs NO-LOCK WHERE LogTrkDocs.CodCia = s-CodCia AND 
      ...
      IF NOT (LogTrkDocs.CodDoc = "HPK" AND LogTrkDocs.Clave = 'TRCKHPK') THEN NEXT.
      ...
      IF ( VtaCDocu.Libre_c03 > '' AND NUM-ENTRIES(VtaCDocu.Libre_c03,'|') >= 3 )
          ...
          IF NOT AVAILABLE Detalle-2 THEN DO:
              FIND FIRST Detalle-2 WHERE Detalle-2.DNI = t-report-2.campo-c[1] AND
                  Detalle-2.CodDoc = VtaCDocu.CodPed AND
                  Detalle-2.NroDoc = VtaCDocu.NroPed EXCLUSIVE-LOCK NO-ERROR.
              IF NOT AVAILABLE Detalle-2 THEN DO:
                  CREATE Detalle-2.
                  ASSIGN
                  ...
                  FOR EACH b-LogTrkDocs NO-LOCK USE-INDEX IDX01 WHERE
                      ...
                      /* b-LogTrkDocs.Codigo
                        ...
                        PK_ERU          Picking productos no encontrado en su ubicacion
                        PK_OBS          Picking Observado
                        PK_COM          Picking Completado
                      */
                       /* Inicio de Picking */
                       IF b-LogTrkDocs.Codigo = "PK_INI" THEN DO:
                           x-Inicio-PK = b-LogTrkDocs.Fecha.
                       END.
                       /* Pausa de Picking (Productos sin ubicacion / Observados) */
                       IF b-LogTrkDocs.Codigo = "PK_ERU" OR b-LogTrkDocs.Codigo = "PK_OBS" THEN DO:
                           x-Fin-PK = b-LogTrkDocs.Fecha.
                           x-Tramos = x-Tramos + 1.
                           Detalle-2.Tiempo = Detalle-2.Tiempo + ROUND( (x-Fin-PK - x-Inicio-PK) / 1000 / 3600 , 2 ).
                       END.
                       /* Fin de Picking */
                       IF b-LogTrkDocs.Codigo = "PK_COM" THEN DO:
                           x-Fin-PK = b-LogTrkDocs.Fecha.
                           x-Tramos = x-Tramos + 1.
                           Detalle-2.Tiempo = Detalle-2.Tiempo + ROUND( (x-Fin-PK - x-Inicio-PK) / 1000 / 3600 , 2 ).
                           LEAVE.
