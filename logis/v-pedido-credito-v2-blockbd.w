      ...
      IF LocalVerificaStock = YES THEN DO:
          ...
          IF s-StkDis < x-CanPed THEN DO:
              ...
              IF x-VentaDelivery = YES THEN DO:
                  pMensajeFinal = 'Producto ' + PEDI.codmat + ' NO tiene Stock Disponible suficiente en el almacén ' + PEDI.AlmDes + CHR(10) +
...
------------------------------------------------------------------------------*/
  ...
  */
  ...
  IF RETURN-VALUE = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'.
  /* *************************************************************************************** */
  /* RHC 30/09/2022 Control de BD por aplicar ARITMETICA SUNAT - 002 CONTADO ANTICIPADO */
  /* *************************************************************************************** */
  IF Faccpedi.FmaPgo = "002" THEN DO:
      ...
      IF x-boleta-deposito > "" THEN DO:
          RUN valida-deposito(INPUT x-cod-boleta-deposito, 
                              ...
                              INPUT Faccpedi.ImpTot, 
                              OUTPUT pMensaje).
          IF RETURN-VALUE = "ADM-ERROR" THEN DO:
              IF TRUE <> (pMensaje > '') THEN pMensaje = 'Debe seleccionar el Doc. depósito'.
              pMensaje = pMensaje + CHR(10) + CHR(10) + 'Comunicarse con el Gestor Contraloria'.
              APPLY 'ENTRY':U TO FacCPedi.Libre_c03.
              UNDO, RETURN 'ADM-ERROR'.
          END.
          IF pMensaje > "" THEN DO:
              pMensajeFinal = pMensaje.
              pMensaje = "".
          END.
      END.
  END.
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    */
    ...
    IF AVAILABLE vtatabla THEN DO:
        /* ¿? */
    END.
    /* 30/07/2022 Susana León y Ana Huaman: bloquear grabación si no cubre el pedido */
