...
&Scoped-Define ENABLED-OBJECTS RECT-66 RECT-67 COMBO-BOX-doc 
...
"BOL", "BOL":U
     ...
     FacCPedi.FlgIgv AT ROW 4.5 COL 55.43 WIDGET-ID 108
          ...
          VIEW-AS TOGGLE-BOX
          SIZE 11.29 BY .77
     FacCPedi.Libre_c03 AT ROW 5.27 COL 31.86 COLON-ALIGNED WIDGET-ID 130
...
"Dolares", 2
          SIZE 15 BY .81
     COMBO-BOX-doc AT ROW 5.31 COL 12 COLON-ALIGNED WIDGET-ID 182
...
DO:
    ...
    IF AVAILABLE gn-clied THEN DO:
        ...
        FIND Gn-ClieD WHERE Gn-ClieD.CodCia = cl-codcia
            ...
            NO-LOCK NO-ERROR.
        IF AVAILABLE Gn-ClieD AND Gn-ClieD.Libre_c03 = "Si" THEN FacCPedi.EmpaqEspec:SCREEN-VALUE = 'YES'.
        /* *************************************************************************************** */
    END.
END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH PEDI EXCLUSIVE-LOCK, 
      ...
      IF pEvento = "UPDATE" THEN DO:
          ...
          IF AVAILABLE Facdpedi AND PEDI.CanPed > Facdpedi.CanPed THEN LocalVerificaStock = YES.
      END.
...
------------------------------------------------------------------------------*/
  ...
  */
  ...
  IF NOT(TRUE <> (x-boleta-deposito > "")) THEN DO:
    ...
    /* BD o A/R */
    ASSIGN faccpedi.usrchq = combo-box-doc:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
    ...
    IF LOCKED vtatabla THEN DO:    
        pMensaje = "Ocurrio un problema con la tabla VTATABLA" + CHR(10) +                    
                        "Esta siendo usado por otro usuario".
        UNDO, RETURN 'ADM-ERROR'.
    END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
      IF RETURN-VALUE = 'NO' THEN x-boleta-deposito = FacCPedi.Libre_c03.
...
/*     END.                                                              */
    ...
    IF s-FmaPgo = '002' /*AND LOOKUP(s-CodDiv, '00018,00024,00030') > 0 */ THEN DO:
        ...
        IF x-boleta-deposito <> "" THEN DO:
            ...
            RUN valida-deposito(INPUT x-cod-boleta-deposito, INPUT x-boleta-deposito, INPUT f-tot, OUTPUT x-ret-msg).

            IF RETURN-VALUE = "ADM-ERROR" THEN DO:
                MESSAGE x-ret-msg VIEW-AS ALERT-BOX ERROR.
    ...
    FOR EACH t-facdpedi NO-LOCK, FIRST Almmmatg OF t-facdpedi NO-LOCK:
        ...
        IF AVAILABLE PEDI THEN DO:
            IF PEDI.canped < (t-facdpedi.canped - t-facdpedi.canate) THEN DO:
                MESSAGE "Los items de la cotizacion deben despacharse en su totalidad" SKIP
        ...
        END.
        ELSE DO:
            MESSAGE "No debe quedarse ningun item de la Cotizacion pendiente de despacho" SKIP
                "Código: " t-facdpedi.codmat almmmatg.desmat
                VIEW-AS ALERT-BOX INFORMATION.
            x-validawsp = NO.
            LEAVE VALIDAWSP.
...
IF s-FmaPgo = '002' THEN DO:
    IF x-boleta-deposito = "" THEN DO:
        /*
        MESSAGE 'Seguro de grabar el pedido, SIN LA BOLETA DE DEPOSITO ?' VIEW-AS ALERT-BOX QUESTION
        ...
        END.
        */
        MESSAGE "1.- Seleccionar GUARDAR para la reserva de stock segun" SKIP
                "configuracion de cada division" SKIP
                "2.- Debe ASIGNAR el BD APROBADO para el PROCESO" SKIP
                "AUTOMATICO DEL PEDIDO" SKIP
                "3.- No procesara si cliente TIENE DEUDA, pedido debe ser" SKIP
                "aprobado por Creditos y Cobranzas"
