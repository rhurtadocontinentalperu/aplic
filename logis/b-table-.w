...
LOOKUP(DI-RutaC.FlgEst, "PX,PK") > 0 )
...
t-DI-RutaC.FchDoc t-DI-RutaC.Observ 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      t-DI-RutaC.CodDoc FORMAT "x(3)":U
      t-DI-RutaC.NroDoc FORMAT "X(9)":U WIDTH 9.57
      t-DI-RutaC.FchDoc FORMAT "99/99/9999":U
      t-DI-RutaC.Observ FORMAT "x(60)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"t-DI-RutaC.NroDoc" ? ? "character" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR x-Estado AS CHAR INIT 'PK,PX' NO-UNDO.
  DEF VAR x-FlgEst AS CHAR NO-UNDO.
  DEF VAR j AS INTE NO-UNDO.

  SESSION:SET-WAIT-STATE('GENERAL').
  DO j = 1 TO 2:
      x-FlgEst = ENTRY(j, x-Estado).
      FOR EACH Di-RutaC USE-INDEX Llave07 NO-LOCK  WHERE DI-RutaC.CodCia = s-CodCia AND
          DI-RutaC.CodDiv = s-CodDiv AND
          DI-RutaC.CodDoc = "PHR" AND 
          DI-RutaC.FlgEst = x-FlgEst:
          CREATE t-DI-RutaC.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Pasar-a-Fedateo B-table-Win 
PROCEDURE Pasar-a-Fedateo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
MESSAGE 'Pasamos la PHR a FEDATEAR?' VIEW-AS ALERT-BOX QUESTION
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    IF NOT {&Condicion} THEN DO:
        pMensaje = "La PHR ya no se puede enviar a Fedatear".
        UNDO, LEAVE.
    END.
    RUN PHR_Listo-para-Fedateo IN hProc (INPUT DI-RutaC.CodDoc,
                                         INPUT DI-RutaC.NroDoc,
                                         OUTPUT pMensaje).
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        IF TRUE <> (pMensaje > '') THEN pMensaje = 'NO se pudo actualizar la PHR'.
        UNDO, LEAVE.
    END.
END.
