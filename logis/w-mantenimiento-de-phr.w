...
DEFINE BUFFER COMPROBANTES FOR CcbCDocu.
...
DEF TEMP-TABLE Detalle
    ...
    FIELD CodRef AS CHAR FORMAT 'x(3)' LABEL 'Codigo1'
    FIELD NroRef AS CHAR FORMAT 'x(9)' LABEL 'Numero1'
    ...
    FIELD Libre_d021 AS DECI FORMAT '>>>,>>9.99' LABEL 'Importe Det'
    FIELD Libre_d011 AS DECI FORMAT '>>>,>>9.99' LABEL 'Peso Det'
    FIELD ImpCob AS DECI FORMAT '>>>,>>9.99' LABEL 'Volumen Det'

    FIELD Liquidacion AS CHAR   FORMAT 'x(40)'      LABEL 'Liquid. según I/C'
    FIELD TipoBCP   AS CHAR     FORMAT 'x(15)'      LABEL 'Tipo BCP'
    FIELD GlosaOD   AS CHAR     FORMAT 'x(50)'      LABEL 'Glosa OD'
    FIELD LugEnt    AS CHAR     FORMAT 'x(60)'      LABEL 'Lugar de Entrega'
    FIELD CodRef2   AS CHAR     FORMAT 'x(8)'       LABEL 'Cod. Refer.'
    FIELD NroRef2   AS CHAR     FORMAT 'x(15)'      LABEL 'Nro. Refer.'
    FIELD Transport AS CHAR     FORMAT 'x(80)'      LABEL 'Transportista'
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fLiquidacion W-Win 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fLugEnt W-Win 
...
DEFINE FRAME F-Main
     BUTTON-TEXTO AT ROW 1 COL 45.14 WIDGET-ID 30
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: COMPROBANTES B "?" ? INTEGRAL CcbCDocu
...
DO:
    ...
    FOR EACH T-RutaC NO-LOCK, FIRST Di-RutaC OF T-RutaC NO-LOCK:
        ...
        FOR EACH T-RutaD NO-LOCK WHERE T-RutaD.CodCia = T-RutaC.CodCia AND
            ...
            /* 10/07/2026 Harold Segura y Martin Chirinos */
            Detalle.Liquidacion = fLiquidacion().
            Detalle.TipoBCP = T-RUTAD.Libre_C02.
            Detalle.GlosaOD = Faccpedi.Glosa.
            Detalle.LugEnt = fLugEnt().
            Detalle.CodRef2 = Faccpedi.codref.
            Detalle.NroRef2 = Faccpedi.nroref.
            Detalle.Transport = T-RUTAD.Libre_c03.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'logis/b-mantenimiento-de-phr.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ,
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'logis/b-mantenimiento-de-phr-det.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fLiquidacion W-Win 
FUNCTION fLiquidacion RETURNS CHARACTER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fLugEnt W-Win 
FUNCTION fLugEnt RETURNS CHARACTER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH COMPROBANTES NO-LOCK WHERE COMPROBANTES.codcia = s-codcia AND
      ...
      COMPROBANTES.nroped = Faccpedi.nroref AND
      COMPROBANTES.flgest <> "A":
      IF LOOKUP(COMPROBANTES.coddoc, 'FAC,BOL') = 0  THEN NEXT.
