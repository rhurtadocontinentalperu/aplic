...
DO:
    ...
    DEFINE VAR x-msg AS CHAR NO-UNDO.
    ...
    IF AVAILABLE di-rutaC THEN DO:
        ...
        ELSE DO:
            ...
            END.
            ELSE DO:
                RUN Reasignar-Orden (OUTPUT x-Msg).
                IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                    MESSAGE 'NO se completó el proceso de reasignación' SKIP
                        x-Msg VIEW-AS ALERT-BOX WARNING.
                    RETURN NO-APPLY.
                END.
            END.
        END.
...
DEFINE BUFFER xdi-rutaD FOR di-rutaD.

GRABAR_DATOS:
DO TRANSACTION ON ERROR UNDO GRABAR_DATOS, RETURN 'ADM-ERROR' ON STOP UNDO GRABAR_DATOS, RETURN 'ADM-ERROR':
    /* Se barre cada O/D, si es que hay más de una */
    FOR EACH tr-RutaD NO-LOCK:
        /* 1ra. parte: Se reasignan las HPK relacionadas */
        FOR EACH Vtacdocu NO-LOCK WHERE Vtacdocu.codcia = s-codcia AND
            Vtacdocu.codref = tr-RutaD.codref AND       /* la O/D */
            Vtacdocu.nroref = tr-RutaD.nroref  AND
            Vtacdocu.codped = "HPK" AND
            Vtacdocu.flgest <> "A":
            FIND x-vtacdocu WHERE ROWID(x-vtacdocu) = ROWID(Vtacdocu) EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
            IF ERROR-STATUS:ERROR = YES THEN DO:
                {lib/mensaje-de-error.i &CuentaError="iCuenta" &MensajeError="pError"}
                UNDO GRABAR_DATOS, RETURN 'ADM-ERROR'.
            END.
            x-codori = x-vtacdocu.CodOri .
            x-nroori = x-vtacdocu.NroOri .
            ASSIGN 
                x-vtacdocu.codori = Di-RutaC.coddoc
                x-vtacdocu.nroori = Di-RutaC.nrodoc.        /* Nueva PHR */
        ...
        END.
        /* 2da. parte: Se reasigna la O/D */
        FOR EACH di-rutaD NO-LOCK WHERE di-rutaD.codcia = s-codcia AND
            di-rutad.coddiv = s-coddiv AND
            di-rutaD.coddoc = pCodPHR AND               /* PHR Origen */
            di-rutaD.nrodoc = pNroPHR AND
            di-rutaD.codref = tr-RutaD.codref AND     /* O/D */
            di-rutaD.nroref = tr-RutaD.nroref:
            FIND xdi-rutad WHERE ROWID(xdi-rutad) = ROWID(di-rutad) EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
            IF ERROR-STATUS:ERROR = YES THEN DO:
                {lib/mensaje-de-error.i &CuentaError="iCuenta" &MensajeError="pError"}
                UNDO GRABAR_DATOS, RETURN 'ADM-ERROR'.
            END.
            ...
            ASSIGN 
                xdi-rutaD.nrodoc = di-rutaC.nrodoc.     /* Nueva PHR */
            /* RHC Log de Control de REASIGNACION por HPK*/
            CREATE LogisLogControl.
            ASSIGN
                LogisLogControl.CodCia = s-CodCia
    ...
    END.
END.
IF AVAILABLE(x-vtacdocu) THEN RELEASE x-vtacdocu NO-ERROR.
IF AVAILABLE(xdi-rutad)  THEN RELEASE xdi-rutaD NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Reasignar-Orden-Old gDialog 
PROCEDURE Reasignar-Orden-Old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
