...
fPeso() @ x-Peso fVolumen() @ x-Volumen fImporte() @ x-Importe 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      T-CPEDI.NroPed COLUMN-LABEL "Numero" FORMAT "X(15)":U
      fPeso() @ x-Peso COLUMN-LABEL "Peso kg"
      fVolumen() @ x-Volumen COLUMN-LABEL "Volumen m3"
      fImporte() @ x-Importe COLUMN-LABEL "Importe"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 44 BY 6.69
         FONT 4.
...
"T-CPEDI.NroPed" "Numero" "X(15)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > "_<CALC>"
"fPeso() @ x-Peso" "Peso kg" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > "_<CALC>"
"fVolumen() @ x-Volumen" "Volumen m3" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"fImporte() @ x-Importe" "Importe" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
FOR EACH T-DPEDI OF T-CPEDI NO-LOCK, FIRST ALMMMATG OF T-DPEDI NO-LOCK:
    ASSIGN
        x-Importe = x-Importe + (T-DPEDI.CanPed * T-DPEDI.PreUni).
END.
RETURN x-Importe.
...
FOR EACH T-DPEDI OF T-CPEDI NO-LOCK, FIRST ALMMMATG OF T-DPEDI NO-LOCK:
    ASSIGN
        x-Importe = x-Importe + (T-DPEDI.CanPed * ALMMMATG.PesMat).
END.
RETURN x-Importe.
...
FOR EACH T-DPEDI OF T-CPEDI NO-LOCK, FIRST ALMMMATG OF T-DPEDI NO-LOCK:
    ASSIGN
        x-Volumen = x-Volumen + (T-DPEDI.CanPed * T-DPEDI.Factor * ALMMMATG.Libre_d02 / 1000000).
END.
RETURN x-Volumen.
