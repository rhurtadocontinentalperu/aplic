...
DEFINE FRAME F-Main
     ...
     FILL-IN-FchEnt-2 AT ROW 1.27 COL 37 COLON-ALIGNED WIDGET-ID 42
...
DO:
  ...
  ASSIGN
      ...
      COMBO-BOX-PHR  = "TODOS".
  IF FILL-IN-FchEnt-1 = ? THEN DO:
      MESSAGE 'Debe ingresar una fecha válida' VIEW-AS ALERT-BOX WARNING.
      APPLY 'ENTRY':U TO FILL-IN-FchEnt-1.
      RETURN NO-APPLY.
  END.
  IF FILL-IN-FchEnt-2 = ? THEN DO:
      MESSAGE 'Debe ingresar una fecha válida' VIEW-AS ALERT-BOX WARNING.
      APPLY 'ENTRY':U TO FILL-IN-FchEnt-2.
      RETURN NO-APPLY.
  END.
...
FOR EACH Vtacdocu NO-LOCK WHERE vtacdocu.codcia = s-codcia AND 
    ...
    Vtacdocu.FchPed >= FILL-IN-FchEnt-1 AND
    Vtacdocu.FchPed <= FILL-IN-FchEnt-2 AND
    vtacdocu.codped = 'HPK':
...
FOR EACH rut-pers-turno NO-LOCK WHERE rut-pers-turno.codcia = s-codcia AND
    rut-pers-turno.fchasignada = TODAY AND 
    rut-pers-turno.coddiv = s-coddiv AND
    rut-pers-turno.rol = 'PICADOR':
