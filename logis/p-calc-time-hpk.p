...
FIND FIRST Vtacdocu WHERE ROWID(Vtacdocu) = pRowid NO-LOCK NO-ERROR.
...
FOR EACH Vtaddocu OF Vtacdocu NO-LOCK, 
    ...
    FOR EACH VtaDTabla NO-LOCK WHERE VtaDTabla.CodCia = Vtacdocu.CodCia AND 
        ...
        IF NOT ( pRol = "" AND (LOOKUP(VtaDTabla.Libre_c04,"CHEQUEADOR,PICADOR") > 0 OR VtaDTabla.Libre_c04 = pRol)) THEN NEXT.
        /* Sector */
