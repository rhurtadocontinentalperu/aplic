...
/* 24/11/23: C.Tenazoa no bloquear vencidas */
...
&Scoped-define INTERNAL-TABLES t-FacCPedi FacCPedi gn-ConVt
...
FacCPedi.FchPed FacCPedi.ImpTot FacCPedi.fchven FacCPedi.FchEnt ~
FacCPedi.ordcmp FacCPedi.CodCli FacCPedi.NomCli FacCPedi.FmaPgo ~
gn-ConVt.Nombr 
...
&Scoped-define QUERY-STRING-br_table FOR EACH t-FacCPedi WHERE ~{&KEY-PHRASE} NO-LOCK, ~
    ...
    BY t-FacCPedi.FchPed DESCENDING ~
       BY t-FacCPedi.NroPed DESCENDING INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH t-FacCPedi WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      FIRST FacCPedi OF t-FacCPedi NO-LOCK, ~
      FIRST gn-ConVt WHERE gn-ConVt.Codig = t-FacCPedi.FmaPgo OUTER-JOIN NO-LOCK ~
    BY t-FacCPedi.FchPed DESCENDING ~
       BY t-FacCPedi.NroPed DESCENDING INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table t-FacCPedi FacCPedi gn-ConVt
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.NroPed COLUMN-LABEL "Numero" FORMAT "X(12)":U WIDTH 9
      FacCPedi.FchPed COLUMN-LABEL "Fecha de!Emisión" FORMAT "99/99/9999":U
      FacCPedi.ImpTot FORMAT "->>,>>>,>>9.99":U WIDTH 10.57
      ...
      FacCPedi.ordcmp FORMAT "X(12)":U WIDTH 10.72
      FacCPedi.CodCli COLUMN-LABEL "Cliente" FORMAT "x(11)":U WIDTH 11.14
      FacCPedi.NomCli FORMAT "x(100)":U WIDTH 48.14
      FacCPedi.FmaPgo COLUMN-LABEL "Cond.!Venta" FORMAT "X(5)":U
            WIDTH 5
      gn-ConVt.Nombr FORMAT "X(50)":U WIDTH 16.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 148 BY 19.65
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "Temp-Tables.t-FacCPedi,INTEGRAL.FacCPedi OF Temp-Tables.t-FacCPedi,INTEGRAL.gn-ConVt WHERE Temp-Tables.t-FacCPedi ..."
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE"
     _TblOptList       = "USED, FIRST, FIRST OUTER"
     _OrdList          = "Temp-Tables.t-FacCPedi.FchPed|no,Temp-Tables.t-FacCPedi.NroPed|no"
     _JoinCode[3]      = "INTEGRAL.gn-ConVt.Codig = Temp-Tables.t-FacCPedi.FmaPgo"
...
"FacCPedi.NroPed" "Numero" ? "character" ? ? ? ? ? ? no ? no no "9" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.ImpTot" ? ? "decimal" ? ? ? ? ? ? no ? no no "10.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.ordcmp" ? ? "character" ? ? ? ? ? ? no ? no no "10.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"FacCPedi.NomCli" ? ? "character" ? ? ? ? ? ? no ? no no "48.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > INTEGRAL.FacCPedi.FmaPgo
"FacCPedi.FmaPgo" "Cond.!Venta" "X(5)" "character" ? ? ? ? ? ? no ? no no "5" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > INTEGRAL.gn-ConVt.Nombr
"gn-ConVt.Nombr" ? ? "character" ? ? ? ? ? ? no ? no no "16.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF AVAILABLE Faccpedi THEN DO:
    ...
    */
    ...
    IF TRUE <> (FacCPedi.Lista_de_Precios > '') 
        THEN x-Lista_de_Precios = FacCPedi.Libre_c01.
    ELSE x-Lista_de_Precios = FacCPedi.Lista_de_Precios.
    
    FIND FIRST x-factabla WHERE x-factabla.codcia = s-codcia AND
                                x-factabla.tabla = 'EVENTOS_LISTAS' AND
                                x-factabla.codigo = x-Lista_de_Precios 
                                NO-LOCK NO-ERROR.
    IF AVAILABLE x-factabla THEN DO:
        MESSAGE 'Cotización' Faccpedi.nroped ' pertenece a evento, ' SKIP 
    ...
    IF AVAILABLE x-factabla THEN DO:
        ...
        IF x-FacTabla.campo-c[1] > '' AND x-FacTabla.campo-c[1] > '0000' THEN DO:
            ...
            IF STRING(TIME, 'HH:MM') > TimeLimit THEN DO:
                MESSAGE 'Cotización ' Faccpedi.nroped ' , ya pasaron las' TimeLimit 'horas' SKIP 
...
CASE TRUE:
    WHEN FILL-IN-CodCli > '' THEN DO:
        FOR EACH Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-codcia AND
            ...
            Faccpedi.flgest = s-flgest:
            IF txtOrdenCompra > '' AND NOT Faccpedi.ordcmp BEGINS txtOrdenCompra THEN NEXT.
            CREATE t-Faccpedi.
            BUFFER-COPY Faccpedi TO t-Faccpedi.
        END.
    END.
    WHEN FILL-IN-NomCli > '' THEN DO:
        FOR EACH Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-codcia AND
            Faccpedi.coddiv = s-coddiv AND
            Faccpedi.flgest = s-flgest AND
            INDEX(FacCPedi.NomCli, FILL-IN-NomCli) > 0 AND
    ...
    END.
    OTHERWISE DO:
        FOR EACH Faccpedi NO-LOCK USE-INDEX Llave10 WHERE Faccpedi.codcia = s-codcia AND
            Faccpedi.coddiv = s-coddiv AND
            Faccpedi.coddoc = s-coddoc AND
            Faccpedi.flgest = s-flgest:
            IF txtOrdenCompra > '' AND NOT Faccpedi.ordcmp BEGINS txtOrdenCompra THEN NEXT.
...
/*
EACH INTEGRAL.FacCPedi WHERE ~{&KEY-PHRASE}
      AND {&Condicion} NO-LOCK,
      FIRST INTEGRAL.gn-ConVt WHERE gn-ConVt.Codig = FacCPedi.FmaPgo OUTER-JOIN NO-LOCK
