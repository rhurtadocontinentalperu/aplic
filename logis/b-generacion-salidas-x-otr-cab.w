...
&Scoped-Define ENABLED-OBJECTS FILL-IN-od FILL-IN-contenido ~
COMBO-BOX-cuales br_table BUTTON-Aplicar FILL-IN-Desde FILL-IN-Hasta ~
RADIO-SET-FlgEst 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-od FILL-IN-contenido ~
COMBO-BOX-cuales FILL-IN-Desde FILL-IN-Hasta RADIO-SET-FlgEst FILL-IN-Peso ~
...
<SORTBY-OPTIONS>
Fecha|y||INTEGRAL.DI-RutaC.FchDoc|no
Glosa|||INTEGRAL.DI-RutaC.Observ|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
DEFINE FRAME F-Main
     ...
     COMBO-BOX-cuales AT ROW 1.08 COL 1.43 NO-LABEL WIDGET-ID 18
     ...
     FILL-IN-Importe AT ROW 9.88 COL 62 RIGHT-ALIGNED NO-LABEL WIDGET-ID 26
     ...
     FILL-IN-SKUnochk AT ROW 9.88 COL 79 RIGHT-ALIGNED NO-LABEL WIDGET-ID 32
...
/* BROWSE-TAB br_table COMBO-BOX-cuales F-Main */
ASSIGN 
    ...
    IF NOT AVAILABLE tt-di-rutaC THEN DO:
        ...
        FOR EACH Di-RutaD OF Di-RutaC NO-LOCK,
            FIRST Faccpedi NO-LOCK WHERE FacCPedi.CodCia = Di-RutaD.codcia
                AND FacCPedi.CodDoc = "OTR"        /*Di-RutaD.codref       /* O/D , OTR */*/
                AND FacCPedi.NroPed = Di-RutaD.nroref
            BREAK BY Faccpedi.CodCli:

            IF x-header-on = 0 THEN DO:
...
/*             END.                                                                                */
            ...
            CREATE tt-puntos-entrega2.
                ASSIGN   tt-puntos-entrega2.tnrophr = di-rutaC.nrodoc
...
/*                 CcbCBult.CodDiv = FacCPedi.coddiv AND                                   */
/*                 CcbCBult.CodDoc = FacCPedi.coddoc AND      /* O/D */                    */
/*                 CcbCBult.NroDoc = FacCPedi.nroped NO-LOCK:                              */
/*                                                                                         */
/*                 ASSIGN tt-di-rutaC.libre_d04 = tt-di-rutaC.libre_d04 + CcbCBult.Bultos. */
/*             END.                                                                        */

        END.    

        IF x-header-on = 0 OR (x-header-on > 0 AND tt-di-rutaC.libre_d05 <= 0) THEN RETURN ERROR.
    END.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Fecha':U THEN DO:
      &Scope SORTBY-PHRASE BY DI-RutaC.FchDoc DESCENDING
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    WHEN 'Glosa':U THEN DO:
      &Scope SORTBY-PHRASE BY DI-RutaC.Observ
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    OTHERWISE DO:
