...
DEFINE TEMP-TABLE tRotulos
    FIELD   cRotulo     AS  CHAR    FORMAT 'x(25)'.

DEFINE VAR lOrdenCerrada AS LOG INIT NO.
...
DEFINE FRAME F-Main
     ...
     br_table AT ROW 2.58 COL 1.43
     FILL-IN-Items AT ROW 19.31 COL 62.29 COLON-ALIGNED NO-LABEL WIDGET-ID 40
     TOGGLE-rotulo-automatico AT ROW 19.38 COL 84 WIDGET-ID 44
...
DO:
  ...
  IF toggle-rotulo-automatico = YES AND NOT (TRUE <> (x-ultimobulto > "")) THEN DO:
        ...
        EMPTY TEMP-TABLE tRotulos.
        CREATE tRotulos.
        ASSIGN tRotulos.crotulo = cUltimoRotuloCerrado.
        /**/
...
DO:  
    ...
    FIND FIRST b-Detalle WHERE b-Detalle.CodCia = s-CodCia
        ...
        AND b-Detalle.CodPed = x-CodDoc     /* hpk */
    ...
    RUN logis/d-chequeo-de-ordenes-rotulos-v2.r(INPUT x-CodDoc, /* O/D - OTR */
                                              INPUT x-NroDoc, 
                                              INPUT y-bultos, 
...
&Scoped-define SELF-NAME TOGGLE-rotulo-automatico
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL TOGGLE-rotulo-automatico B-table-Win
ON VALUE-CHANGED OF TOGGLE-rotulo-automatico IN FRAME F-Main /* Imprime rotulo automatico */
DO:
  ASSIGN {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  ...
  */
  ...
  /* Imprimir los Rotulos */

  lOrdenCerrada = YES.

  APPLY "CHOOSE" TO button-imprimir IN FRAME {&FRAME-NAME}.
...
DEFINE VAR cRotuloImprimir AS CHAR.
...
xFiler = 0.
FOR EACH tRotulos NO-LOCK:
    cRotuloImprimir = tRotulos.crotulo.
    FOR EACH b-Detalle NO-LOCK WHERE b-Detalle.CodCia = s-CodCia
        AND b-Detalle.CodDiv = s-CodDiv
        AND b-Detalle.CodPed = x-CodDoc
        AND b-Detalle.NroPed = x-NroDoc
        AND B-DETAlle.Etiqueta = cRotuloImprimir
        BREAK BY b-Detalle.Etiqueta: 
        IF FIRST-OF(b-Detalle.Etiqueta) THEN DO:
            xFiler = xFiler + 1.
...
------------------------------------------------------------------------------*/
  ...
  CASE TRUE:
      WHEN s-generacion-multiple = YES AND x-Factor >= 1 THEN DO:
          ...
          EMPTY TEMP-TABLE tRotulos.    /* Rotulos */
          ...
          DO k = 1 TO x-Cantidad:
              ...
              x-Bulto = x-Bulto + 1.
              /* Rotulos */
              IF toggle-rotulo-automatico = YES THEN DO:
                  CREATE tRotulos.
                  ASSIGN tRotulos.crotulo = logisdchequeo.Etiqueta.
              END.
          END.
          IF toggle-rotulo-automatico = YES THEN DO:
              RUN imprime-rotulo-automatico.
