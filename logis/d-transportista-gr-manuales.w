...
DEF INPUT PARAMETER TABLE FOR tgre_cmpte.       /* Todas las FAC o BOL seleccionadas */
DEF INPUT PARAMETER pCodCia AS INT NO-UNDO.
DEF INPUT PARAMETER pCodDiv AS CHAR NO-UNDO.
DEF INPUT PARAMETER pCodDoc AS CHAR NO-UNDO.    /* FAC o BOL */
...
FIND Ccbcdocu WHERE Ccbcdocu.codcia = pcodcia
    AND Ccbcdocu.coddiv = pCodDiv
    AND Ccbcdocu.coddoc = pCodDoc
    AND Ccbcdocu.nrodoc = pNroDoc
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE Ccbcdocu THEN DO:
    MESSAGE 'Documento' pcoddoc pnrodoc 'de la división' pcoddiv 'no registrado'
...
f-NroLicencia Btn_OK Btn_Cancel 
&Scoped-Define DISPLAYED-OBJECTS f-Placa f-Marca f-CodAge f-NomTra f-RucAge ~
f-Traslado f-NroLicencia f-Chofer f-Certificado F-CodTran F-Nombre ~
...
DEFINE FRAME D-Dialog
     ...
     f-Chofer AT ROW 6.92 COL 20 COLON-ALIGNED WIDGET-ID 56
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: T-CcbADocu T "?" ? INTEGRAL CcbADocu
...
/* SETTINGS FOR FILL-IN f-CodAge IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN F-CodTran IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN F-Contacto IN FRAME D-Dialog
   NO-ENABLE                                                            */
...
/* SETTINGS FOR FILL-IN F-horario IN FRAME D-Dialog
   NO-ENABLE                                                            */
...
/* SETTINGS FOR FILL-IN f-NomTra IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN F-Observ IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN F-ruc IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN f-RucAge IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN f-SedeTran IN FRAME D-Dialog
   NO-ENABLE                                                            */
...
DO:
  ...
  FIND VtaTabla WHERE VtaTabla.CodCia = s-codcia AND 
      ...
      VtaTabla.Llave_C8 = "SI" AND
      VtaTabla.Llave_C1 = SELF:SCREEN-VALUE NO-LOCK NO-ERROR.
  IF NOT AVAILABLE VtaTabla THEN DO:
      MESSAGE '# de Licencia no válida' VIEW-AS ALERT-BOX WARNING.
      SELF:SCREEN-VALUE = ''.
      RETURN NO-APPLY.
  END.
  f-Chofer:SCREEN-VALUE = VtaTabla.Libre_c03 + " " + VtaTabla.Libre_c01 + " " + VtaTabla.Libre_c02.
END.
...
OR F8 OF f-NroLicencia DO:
  input-var-1 = ''.
  input-var-2 = ''.
  input-var-3 = ''.
  output-var-1 = ?.
  RUN lkup/c-conductor-02.w ('Licencias').
  IF output-var-1 <> ? THEN SELF:SCREEN-VALUE = output-var-2.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY f-Placa f-Marca f-CodAge f-NomTra f-RucAge f-Traslado f-NroLicencia 
          f-Chofer f-Certificado F-CodTran F-Nombre f-SedeTran F-Direccion F-ruc 
          F-Lugar F-Contacto F-horario F-Observ 
      WITH FRAME D-Dialog.
  ENABLE RECT-66 RECT-67 RECT-21 f-Placa f-Traslado f-NroLicencia Btn_OK 
         Btn_Cancel 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION WITH FRAME {&FRAME-NAME} ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
      ...
      IF ERROR-STATUS:ERROR = YES THEN DO:
          MESSAGE ERROR-STATUS:GET-MESSAGE(1) VIEW-AS ALERT-BOX WARNING.
          UNDO, RETURN 'ADM-ERROR'.
      END.
      /* 25/06/2026 Martín Chirinos: replicar los datos a todas las FAC con la misma PHR */
      /* 1ro. Buscamos la PHR */
      DEF VAR cNroPHR AS CHAR NO-UNDO.

      FOR FIRST Di-RutaD NO-LOCK WHERE Di-RutaD.codcia = pcodcia
              AND Di-RutaD.codref = Ccbcdocu.Libre_c01       /* O/D */
          ...
          LEAVE.
      END.
      /* 2do. Copiar la información al resto de comprobantes relacionados */
      DEF BUFFER bT-CcbADocu FOR T-CcbADocu.
      DEF BUFFER bCcbcdocu FOR Ccbcdocu.

      IF cNroPHR > '' THEN DO:
          /* 1er. caso: Pertenecen a la misma PHR => Deben tener el mismo dato del transportista */
          FOR EACH tgre_cmpte NO-LOCK,
              FIRST bCcbcdocu NO-LOCK WHERE bCcbcdocu.codcia = pcodcia 
                AND bCcbcdocu.coddoc = tgre_cmpte.coddoc     /* FAC */
                AND bCcbcdocu.nrodoc = tgre_cmpte.nrodoc,
              FIRST Faccpedi NO-LOCK WHERE Faccpedi.codcia = pcodcia
                AND Faccpedi.coddoc = bCcbcdocu.libre_c01    /* O/D */
                AND Faccpedi.nroped = bCcbcdocu.libre_c02,
              FIRST Di-RutaD NO-LOCK WHERE Di-RutaD.codcia = pcodcia
                AND Di-RutaD.codref = Faccpedi.coddoc
                AND Di-RutaD.nroref = Faccpedi.nroped
                AND Di-RutaD.coddoc = "PHR" 
                AND Di-RutaD.nrodoc = cNroPHR:
              /* Filtro */
              IF bCcbcdocu.nrodoc = pnrodoc THEN NEXT.
              FIND FIRST bT-CcbADocu WHERE bT-CcbADocu.codcia = bCcbcdocu.codcia AND
                  bT-CcbADocu.coddiv = bCcbcdocu.CodDiv AND
                  bT-CcbADocu.coddoc = bCcbcdocu.CodDoc AND
                  bT-CcbADocu.nrodoc = bCcbcdocu.NroDoc
                  EXCLUSIVE-LOCK NO-ERROR. 
              IF NOT AVAILABLE bT-CcbADocu THEN DO:
                  CREATE bT-CcbADocu.
                  ASSIGN 
                      bT-Ccbadocu.codcia = bCcbcdocu.codcia
                      bT-Ccbadocu.coddiv = bCcbcdocu.coddiv
                      bT-Ccbadocu.coddoc = bCcbcdocu.CodDoc
                      bT-Ccbadocu.nrodoc = bCcbcdocu.NroDoc
                      .
              END.
              ASSIGN
                  bT-CcbADocu.Libre_C[1] = T-CcbADocu.Libre_C[1]        /* Placa */
                  bT-CcbADocu.Libre_C[2] = T-CcbADocu.Libre_C[2]        /* Vehic. Marca */
                  bT-CcbADocu.Libre_C[3] = T-CcbADocu.Libre_C[3]        /* Transportista */
                  bT-CcbADocu.Libre_C[4] = T-CcbADocu.Libre_C[4]        /* Nomb Transp */
                  bT-CcbADocu.Libre_C[5] = T-CcbADocu.Libre_C[5]        /* RUC Transp */
                  bT-CcbADocu.Libre_F[1] = T-CcbADocu.Libre_F[1]        /* Fecha traslado */
                  bT-CcbADocu.Libre_C[6] = T-CcbADocu.Libre_C[6]        /* # Licencia */
                  bT-CcbADocu.Libre_C[8] = T-CcbADocu.Libre_C[8]        /* # Licencia */
                  bT-CcbADocu.Libre_C[7] = T-CcbADocu.Libre_C[7]        /* Chofer */
                  bT-CcbADocu.Libre_C[17] = T-CcbADocu.Libre_C[17]      /* Certificado */
                  .
          END.
      END.
  END.
