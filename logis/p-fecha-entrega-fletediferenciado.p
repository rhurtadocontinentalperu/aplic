...
/* *************************************************************************** */
DEF VAR x-Minimo AS INT INIT 0 NO-UNDO.     /* Mínimo 0 días antes de la hora de corte */
                                            /* Servicio 24/7 */
DEF VAR x-HoraCorte AS CHAR INIT '17:00:00' NO-UNDO.
/* *************************************************************************** */
/* CONSISTENCIAS */
/* *************************************************************************** */
...
IF AVAILABLE FacCPedi AND FacCPedi.EmpaqEspec = YES THEN DO:
    FIND VtaTabla WHERE VtaTabla.CodCia = s-CodCia
        AND VtaTabla.Tabla = "CDSKU_DIV"        /* OJO */
...
/*MESSAGE 'inicia' ffchprog pfechabase.*/
PRINCIPAL:
REPEAT:
    ...
    IF NOT AVAILABLE VtaTabla THEN DO:
        ASSIGN
            ...
            .
        /*MESSAGE 'dos ok' s-codcia s-llave-canaldia ccoddiv pdivori WEEKDAY(fFchProg).*/
        NEXT.
    END.
    /* ************************************************************** */
    ...
    IF NOT AVAILABLE VtaTabla THEN DO:
        ASSIGN
            ...
            .
        /*MESSAGE 'tres ok' s-codcia s-Llave-Zona cCodDiv cZonaGHR cSubZonaGHR WEEKDAY(fFchProg).*/
    ...
    FOR EACH VtaDTabla NO-LOCK WHERE VtaDTabla.CodCia = s-codcia AND
        VtaDTabla.Tabla = "CANALHR_DIV" AND
        VtaDTabla.Llave = "CFG_FLGEST_DIV" AND
...
CASE s-CentroDistribucion:
    WHEN YES THEN DO:
        ...
        /* Si fuera sábado o domingo*/
        IF (WEEKDAY(pFechaBase) = 1 OR WEEKDAY(pFechaBase) = 7) AND 
            VtaTabla.Libre_c02 > '' THEN x-HoraCorte = VtaTabla.Libre_c02.
    END.
    WHEN NO THEN DO:
        FIND FIRST FacTabla WHERE FacTabla.codcia = s-CodCia
            AND FacTabla.tabla = 'CFG_CAL_LOG_DIV' 
...
REPEAT:
    ...
    FIND FacTabla WHERE FacTabla.CodCia = s-codcia
        AND FacTabla.Tabla = 'SZGHRFERIADO_DIV'
        AND FacTabla.Codigo = cCodDiv
        NO-LOCK NO-ERROR.
    /*MESSAGE ccoddiv pfchent x-dia.*/
    IF AVAILABLE FacTabla AND FacTabla.Campo-L[WEEKDAY(pFchEnt)] = YES THEN DO:
        Log-Edi-Trans.MsgProceso = Log-Edi-Trans.MsgProceso +  CHR(10) +
        "Saltamos Feriado = " + STRING(pfchent).
        /*MESSAGE 'saltaferiado'.*/
        NEXT.
    END.
    LEAVE RLOOP.
END.
