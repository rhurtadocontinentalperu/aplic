...
IF pFechaBase = ? AND pCodDoc > '' AND pNroPed > '' THEN DO:
    ...
    FIND FIRST DOCBASE WHERE DOCBASE.codcia = s-codcia
...
RELEASE Log-Edi-Trans NO-ERROR.
...
PROCEDURE CentroDistribucion PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF VAR dFchPed AS DATE NO-UNDO.
dFchPed = ADD-INTERVAL(TODAY, -1, 'month').
...
REPEAT:
    ...
    DO iItem = 1 TO NUM-ENTRIES(LocalFlgEst):
        FOR EACH Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-codcia
            AND Faccpedi.divdes = cCodDiv
            AND Faccpedi.coddoc = 'O/D'
            AND Faccpedi.flgest = ENTRY(iItem, LocalFlgEst)
            AND Faccpedi.fchped >= dFchPed
...
PROCEDURE Consistencia PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
