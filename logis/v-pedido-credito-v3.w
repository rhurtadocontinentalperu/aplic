...
&Scoped-define SELF-NAME COMBO-BOX-doc
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL COMBO-BOX-doc V-table-Win
ON VALUE-CHANGED OF COMBO-BOX-doc IN FRAME F-Main /* Cod. Doc. deposito */
DO:
  IF TRUE <> (SELF:SCREEN-VALUE > '') THEN FacCPedi.Libre_c03:SCREEN-VALUE = ''.
...
DO:
    ...
    /* 25/01/2022 De acuerdo a la condición de venta */
    FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia AND
        VtaTabla.Tabla = 'CONFIG-VTAS' AND
        VtaTabla.Llave_c1 = 'DOC-DEPOSITOS' AND
        VtaTabla.Llave_c2 = 'COND.VTA' AND
        VtaTabla.LLave_c3 = s-FmaPgo
        NO-LOCK NO-ERROR.
    COMBO-BOX-doc:DELETE(COMBO-BOX-doc:LIST-ITEMS).
    ...
    IF AVAILABLE VtaTabla THEN DO:
        ...
        FOR EACH VtaTabla NO-LOCK WHERE VtaTabla.CodCia = s-codcia AND
            ...
            VtaTabla.Llave_c2 = 'COND.VTA' AND
            VtaTabla.LLave_c3 = s-FmaPgo:
            COMBO-BOX-doc:ADD-LAST(VtaTabla.LLave_c4).
        END.
    END.
    ELSE DO:
...
/* ******************************************************************************* */
/* Nueva validación 25/01/2022 */
/* ******************************************************************************* */
FIND FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia AND
        VtaTabla.Tabla = 'CONFIG-VTAS' AND
        VtaTabla.Llave_c1 = 'DOC-DEPOSITOS' AND
        VtaTabla.Llave_c2 = 'COND.VTA' AND
        VtaTabla.LLave_c3 = FacCPedi.FmaPgo
        NO-LOCK NO-ERROR.
IF AVAILABLE VTaTabla THEN DO:
    IF TRUE <> (x-boleta-deposito > "") THEN DO:
        MESSAGE "Imposible realizar despacho" SKIP
                "Debe asignar" SKIP
                "la BD APROBADA " VIEW-AS ALERT-BOX INFORMATION.
        RETURN.
    END.
END.
/* ******************************************************************************* */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      APPLY 'LEAVE':U TO FacCPedi.FmaPgo.   /* Aquí se actualiza COMBO-BOX-doc */
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE Faccpedi THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      /* Cargamos Documentos de Abono por defecto */
      COMBO-BOX-doc:DELETE(COMBO-BOX-doc:LIST-ITEMS).
      COMBO-BOX-doc:ADD-LAST(' ').
      FOR EACH FacDocum NO-LOCK WHERE FacDocum.CodCia = s-codcia AND
          FacDocum.TpoDoc = NO:
          COMBO-BOX-doc:ADD-LAST(FacDocum.CodDoc).
      END.
      combo-box-doc = " ".
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    /* ******************************************************************************** */
    /* Nueva funcionalidad 25/01/2022 */
    ...
    IF x-boleta-deposito > "" AND TRUE <> (x-cod-boleta-deposito > '') THEN DO:
        MESSAGE 'Ingrese el Cod. Doc. depósito'
            VIEW-AS ALERT-BOX INFORMATION
            TITLE 'VERIFICACION DOC DEPOSITO (1)'.
        APPLY 'ENTRY':U TO COMBO-BOX-doc.
        RETURN "ADM-ERROR".
    END.
...
/* Ic - 23Jun2020  */
/* ***************************************************************************** */
/* 25/01/2021 Nueva validación: por defecto COMBO-BOX-doc tiene 1 item en blanco */
IF COMBO-BOX-doc:NUM-ITEMS IN FRAME {&FRAME-NAME} > 1 THEN DO:
    IF TRUE <> (x-boleta-deposito > "") THEN DO:
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    IF NOT AVAILABLE Ccbcdocu OR Ccbcdocu.flgest <> 'P' OR Ccbcdocu.SdoAct <= 0 THEN DO:
        ...
        RETURN 'ADM-ERROR'.
    END.
    IF Ccbcdocu.FmaPgo > '' AND Ccbcdocu.FmaPgo <> s-FmaPgo THEN DO:
    ...
    IF INPUT FacCPedi.CodMon = Ccbcdocu.CodMon THEN DO:
        IF Ccbcdocu.sdoact < pTot THEN DO:
            pMsgRet = "El saldo del " + pCodDeposito + " NO cubre el monto total" + CHR(13) + CHR(10) +
                        "El pedido va ir obligatoriamente a evaluacion de créditos y cobranzas".
        END.
    END.
    ELSE DO:
...
APPLY 'LEAVE':U TO FacCPedi.FmaPgo IN FRAME {&FRAME-NAME}.
COMBO-BOX-doc:SCREEN-VALUE IN FRAME {&FRAME-NAME} = faccpedi.usrchq.
