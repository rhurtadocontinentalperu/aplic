...
DEF BUFFER BX-RutaC FOR Di-RutaC.
...
DEFINE TEMP-TABLE ttCondVtas
    FIELD codcondvta AS CHAR    FORMAT 'x(5)'.
...
DEFINE FRAME F-Main
     COMBO-BOX_FmaPgo AT ROW 1.08 COL 19 COLON-ALIGNED WIDGET-ID 24
...
EMPTY TEMP-TABLE ttCondVtas.

FIND FIRST B-RutaC WHERE ROWID(B-RUtaC) = pRowid NO-LOCK NO-ERROR.
IF NOT AVAILABLE B-RutaC THEN RETURN.
    
FIND FIRST x-di-rutac WHERE ROWID(x-di-rutac) = pRowid NO-LOCK NO-ERROR.
/**/
DEFINE VAR hProc AS HANDLE NO-UNDO.             /* Handle Libreria */
DEFINE VAR hProc1 AS HANDLE NO-UNDO.             /* Handle Libreria */
...
RUN logis\logis-library.p PERSISTENT SET hProc1.
...
FOR EACH DI-RutaD OF B-RutaC NO-LOCK, 
    ...
    END.
    /**/
    RUN ffFlgSitPedido IN hProc1 (Faccpedi.CodDoc, Faccpedi.FlgSit, OUTPUT cEstado).
    IF Faccpedi.FlgEst = "C" THEN cEstado = "DOCUMENTADO".
    IF TRUE <> (cEstado > '') THEN cEstado = 'APROBADO'.
    ASSIGN T-RUTAD.Estado = cEstado.

    /**/
    FIND FIRST ttCondVtas WHERE ttCondVtas.codcondvta = Faccpedi.fmapgo EXCLUSIVE-LOCK NO-ERROR.
    IF NOT AVAILABLE ttCondVtas THEN DO:
        CREATE ttCondVtas.
            ASSIGN ttCondVtas.codcondvta = Faccpedi.fmapgo.
    END.
    /* Totales */
        FILL-IN-Peso = FILL-IN-Peso + T-RUTAD.Libre_d01.
        FILL-IN-Volumen = FILL-IN-Volumen + T-RUTAD.ImpCob.
        FILL-IN-Importe = FILL-IN-Importe + T-RUTAD.Libre_d02.
        FILL-IN-Bultos = FILL-IN-Bultos + INTEGER(T-RUTAD.Libre_c01).
        FILL-IN-SKUs = FILL-IN-SKUs + T-RUTAD.SKU.
END.
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    FOR EACH ttCondVtas NO-LOCK, FIRST gn-ConVt NO-LOCK WHERE gn-ConVt.Codig = ttCondVtas.codcondvta:
...
/* 1. Solo se pude REASIGNAR si la PHR aún no se ha CERRADO */
IF NOT AVAILABLE T-RUTAD THEN RETURN.
FIND B-RutaC OF T-RUTAD NO-LOCK NO-ERROR.
IF NOT AVAILABLE B-RutaC OR NOT B-RutaC.FlgEst BEGINS "P" THEN DO:
    MESSAGE "La PHR no debe estar cerrada o anulada" VIEW-AS ALERT-BOX WARNING.
    RETURN.
END.
/* 2. Reasignamos las seleccionadas */
