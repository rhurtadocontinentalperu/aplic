...
tFacCPedi.FchPed fGetCurr(FacCPedi.CodMon) @ cCurrency tFacCPedi.Peso ~
tFacCPedi.Libre_c01 fGetODOrigen() @ x-NroRef 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      tFacCPedi.Peso COLUMN-LABEL "Peso!Kgrs" FORMAT "->>,>>,>>9.99":U
            COLUMN-BGCOLOR 14 COLUMN-FONT 6
...
DEFINE FRAME F-Main
     ...
     FILL-IN_Info AT ROW 9.38 COL 9 COLON-ALIGNED NO-LABEL WIDGET-ID 6
...
"fGetCurr(FacCPedi.CodMon) @ cCurrency" "Moneda" "x(3)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > Temp-Tables.tFacCPedi.Peso
"tFacCPedi.Peso" "Peso!Kgrs" "->>,>>,>>9.99" "decimal" 14 ? 6 ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/*     END.                                                     */
    ...
    IF USERID("DICTDB") = 'admin' THEN DO:
        RUN logis/d-distribucion-fraccionamiento-ordenes.r ( ROWID(Faccpedi), OUTPUT pNroBultos, OUTPUT rpta ).
    END.
    ELSE DO:
