...
gn-prov.FaxPro gn-prov.CndCmp gn-prov.Libre_c05 gn-prov.Libre_d01 
...
DEFINE FRAME F-Main
     ...
     gn-prov.Libre_c05 AT ROW 6.92 COL 61 COLON-ALIGNED WIDGET-ID 56
          ...
          VIEW-AS FILL-IN 
          SIZE 25 BY .81
     gn-prov.Libre_d01 AT ROW 8 COL 17.43 NO-LABEL WIDGET-ID 128
          VIEW-AS RADIO-SET HORIZONTAL
          RADIO-BUTTONS 
                    "Ninguno", 0,
"Publico", 1,
"Privado", 2
     ...
     "Tipo transporte" VIEW-AS TEXT
          SIZE 11 BY .81 AT ROW 8 COL 6 WIDGET-ID 132
     "(privado = propio, publico = tercero)" VIEW-AS TEXT
          SIZE 29.86 BY .81 AT ROW 8 COL 46.14 WIDGET-ID 134
          FGCOLOR 9 FONT 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1 SCROLLABLE 
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE gn-prov THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      DISPLAY F-DesCnd.
      IF TRUE <> (gn-prov.Girpro > '') THEN DO:
          gn-prov.Girpro:SCREEN-VALUE = ' '.
      END.
  END.
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        /* 17/02/2022 Obligado el Giro de la empresa */
        IF TRUE <> (gn-prov.Girpro:SCREEN-VALUE > '') THEN DO:
            MESSAGE 'Es obligatorio el Giro de la Empresa' VIEW-AS ALERT-BOX ERROR.
            APPLY 'ENTRY':U TO gn-prov.Girpro.
            RETURN 'ADM-ERROR'.
        END.
        IF DEC(libre_d01:SCREEN-VALUE) < 1 OR DEC(libre_d01:SCREEN-VALUE) > 2 THEN DO:
            MESSAGE 'Elija el tipo de transporte correcto (Publico o Privado)' VIEW-AS ALERT-BOX ERROR.
            APPLY 'ENTRY':U TO gn-prov.libre_d01.
            RETURN 'ADM-ERROR'.
        END.
    END.
    RETURN "OK".
