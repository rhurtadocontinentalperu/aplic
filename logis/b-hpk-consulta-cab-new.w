...
FOR EACH Di-RutaC NO-LOCK WHERE DI-RutaC.CodCia = s-CodCia AND 
    DI-RutaC.CodDoc = "PHR" AND 
    (DI-RutaC.FchDoc >= x-FchDoc-1 AND DI-RutaC.FchDoc <= x-FchDoc-2) AND
    ...
    x-Ok = NO.
    FOR EACH Di-RutaD OF Di-RutaC NO-LOCK,
        EACH Vtacdocu WHERE Vtacdocu.CodCia = s-codcia
            AND Vtacdocu.CodDiv = Di-RutaC.CodDiv
            AND Vtacdocu.CodPed = "HPK"
            AND Vtacdocu.CodRef = Di-RutaD.CodRef
            AND Vtacdocu.NroRef = Di-RutaD.NroRef.
        IF Vtacdocu.FlgEst = "A" THEN NEXT.
        IF Vtacdocu.FlgSit = "C" THEN NEXT.
        x-Ok = YES.
    END.
    IF x-Ok = NO THEN NEXT.

    /* Barremos los detalles para aplicar los filtros */
    /* Basta con que se encuentre una ocurrencia */
    IF x-NroPed > "" OR x-NomCli > "" OR x-NroHpk > "" OR x-FchEnt-1 <> ? OR x-FchEnt-2 <> ? THEN DO:
        x-Ok = NO.
        RLOOPEACH:
        FOR EACH Di-RutaD OF Di-RutaC NO-LOCK,
            FIRST Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-codcia
                AND Faccpedi.coddoc = Di-RutaD.codref       /* O/D u OTR */
                AND Faccpedi.nroped = Di-RutaD.nroref:
            ...
            IF x-NomCli > "" AND INDEX(Faccpedi.nomcli, x-NomCli) > 0 THEN DO:
                ...
                LEAVE.
            END.
            IF (x-FchEnt-1 <> ? AND Faccpedi.FchEnt >= x-FchEnt-1) AND
                (x-FchEnt-2 <> ? AND Faccpedi.FchEnt <= x-FchEnt-2)
                THEN DO:
                x-Ok = YES.
                LEAVE.
            END.
            ...
            IF x-NroHpk > "" THEN DO:
                ...
                FOR EACH Vtacdocu NO-LOCK WHERE Vtacdocu.codcia = s-codcia
                    ...
                    IF Vtacdocu.FlgEst = "A" THEN NEXT.
                    IF Vtacdocu.FlgSit = "C" THEN NEXT.
                    IF Vtacdocu.nroped = x-NroHpk THEN DO:
                        x-Ok = YES.
                        LEAVE RLOOPEACH.
                    END.
                END.
            END.
        END.
        IF x-Ok = NO THEN NEXT. /* Pasamos al siguiente */
    END.
    /* ******************** */
    CREATE T-Di-RutaC.
