...
logisdchequeo.CanPed logisdchequeo.Etiqueta 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      logisdchequeo.Etiqueta COLUMN-LABEL "Número de Etiqueta" FORMAT "x(30)":U
            WIDTH 21.14
      logisdchequeo.FechaHora FORMAT "99/99/9999 HH:MM:SS":U WIDTH 17.14
  ENABLE
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 160 BY 16.42
...
DEFINE FRAME F-Main
     ...
     BUTTON-Renumera AT ROW 1.27 COL 113 WIDGET-ID 34
...
"logisdchequeo.FechaHora" ? "99/99/9999 HH:MM:SS" "datetime" ? ? ? ? ? ? no ? no no "17.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  FOR EACH B-DETA WHERE B-DETA.CodCia = s-CodCia
      ...
      ASSIGN
          B-DETA.Etiqueta = x-Llave + STRING(x-bulto,"999").
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST t-Detalle WHERE t-Detalle.CanPed <> t-Detalle.CanChk) THEN DO:
      ...
      IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
          ...
          RETURN 'ADM-ERROR'.
      END.
/*       RUN grabar-chequeo-hpk-observado (OUTPUT pMensaje). */
/*       IF pMensaje > "" THEN DO:     /* Error al grabar */ */
/*           MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.       */
/*           RETURN 'ADM-ERROR'.                             */
/*       END.                                                */
  END.
  ELSE DO:
      ...
      pMensaje = "".
      /*RUN grabar-chequeo-hpk-resto (OUTPUT pMensaje).*/
      RUN logis/p-cierre-de-chequeo (INPUT x-CodDoc,
                                     INPUT x-NroDoc,
                                     INPUT x-Embalado,
                                     INPUT x-CodPer,
...
/* ******************************************* */
ASSIGN
    logisdchequeo.Etiqueta = x-Llave + STRING(x-bulto,"999").
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  ENABLE BUTTON-CerrarBulto BUTTON-imprimir TOGGLE-Multiple /*BUTTON-Renumera*/ WITH FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
  ...
  END.
  DISABLE BUTTON-CerrarBulto BUTTON-imprimir TOGGLE-Multiple /*BUTTON-Renumera*/ WITH FRAME {&FRAME-NAME}.
