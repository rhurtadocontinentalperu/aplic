...
DEFINE SHARED TEMP-TABLE T-CFAIS LIKE CcbCDocu.
DEFINE SHARED TEMP-TABLE T-CGUIAS LIKE CcbCDocu.
...
&Scoped-define FIELDS-IN-QUERY-br_table CcbCDocu.CodDoc CcbCDocu.NroDoc CcbCDocu.FchDoc CcbCDocu.CodCli CcbCDocu.NomCli   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define SELF-NAME br_table
&Scoped-define QUERY-STRING-br_table FOR EACH CcbCDocu WHERE ~{&KEY-PHRASE}       AND {&Condicion} NO-LOCK, ~
             FIRST FAIS WHERE {&Condicion2} NO-LOCK       BY CcbCDocu.FchDoc DESC BY CcbCDocu.NroDoc DESC
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH CcbCDocu WHERE ~{&KEY-PHRASE}       AND {&Condicion} NO-LOCK, ~
             FIRST FAIS WHERE {&Condicion2} NO-LOCK       BY CcbCDocu.FchDoc DESC BY CcbCDocu.NroDoc DESC.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: T-CFAIS T "SHARED" ? INTEGRAL CcbCDocu
      TABLE: T-CGUIAS T "SHARED" ? INTEGRAL CcbCDocu
   END-TABLES.
 */
...
OPEN QUERY {&SELF-NAME} FOR EACH CcbCDocu WHERE ~{&KEY-PHRASE}
      ...
      FIRST FAIS WHERE {&Condicion2} NO-LOCK
...
DO:
  ...
  RUN Procesa-handle IN lh_handle ('Pinta-Browses').
END.
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    FOR EACH T-CFAIS NO-LOCK:
        FIND FIRST FAIS OF T-CFAIS EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    REPEAT:
        ...
        IF NOT {&Condicion2} THEN DO:
            pMensaje = 'la FAI ' + FAIS.NroDoc + ' ha sido modificada por otro usuario' + 
                CHR(10) + 'Proceso Abortado'.
            UNDO RLOOP, LEAVE RLOOP.
        END.
        /* Limpiamos Controles */
        ASSIGN
...
/*    RUN ccb/d-motanu (ccbcdocu.codcia,         */
/*                      ccbcdocu.coddoc,         */
/*                      ccbcdocu.nrodoc,         */
/*                      s-user-id,               */
/*                      OUTPUT cReturnValue).    */
/*    IF cReturnValue = 'ADM-ERROR' THEN RETURN. */
   ...
   IF pHojRut > '' AND pFlgEst-1 <> 'A' THEN DO:
       MESSAGE "NO se puede anular" SKIP "Revisar la Hoja de Ruta:" pHojRut SKIP
   ...
   DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
       /* 1ro. Recargamos temporales: T-CGUIAS y T-CFAIS */
       RUN Carga-Temporales.

       /* 2do. Anulamos la GR relacionadas */
       RUN Anular-GR-Relacionadas (OUTPUT pMensaje).
       IF pMensaje > '' THEN UNDO, LEAVE.

       /* 3ro. Anulamos FAIs relacionadas */
       RUN Anular-FAIS (OUTPUT pMensaje).
       IF pMensaje > '' THEN UNDO, LEAVE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Anular-GR-Relacionadas B-table-Win 
PROCEDURE Anular-GR-Relacionadas :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   ...
   DO TRANSACTION ON ERROR UNDO RLOOP, LEAVE RLOOP ON STOP UNDO RLOOP, LEAVE RLOOP:
       FOR EACH T-CGUIAS NO-LOCK:
           FIND FIRST GUIA OF T-CGUIAS EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
           IF ERROR-STATUS:ERROR = YES THEN DO:
               {lib/mensaje-de-error.i &MensajeError="pMensaje"}
               UNDO RLOOP, LEAVE RLOOP.
           END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Temporales B-table-Win 
...
EMPTY TEMP-TABLE T-CGUIAS.
EMPTY TEMP-TABLE T-CFAIS.

FOR EACH FAIS NO-LOCK WHERE {&Condicion2}:
    CREATE T-CFAIS.
    BUFFER-COPY FAIS TO T-CFAIS.
    /* Referencia cruzada */
    FOR EACH Vtatabla NO-LOCK WHERE Vtatabla.codcia = FAIS.codcia 
        AND Vtatabla.Tabla = "CRUCE-FAI-GR"
        AND ENTRY(1,Vtatabla.Llave_c1,'-') = 'FAI'
        AND ENTRY(2,Vtatabla.Llave_c1,'-') = FAIS.NroDoc:
        FIND FIRST GUIA WHERE GUIA.codcia = Vtatabla.codcia
            AND GUIA.coddoc = ENTRY(1,Vtatabla.llave_c2,'-')
            AND GUIA.nrodoc = ENTRY(2,Vtatabla.llave_c2,'-')
            AND GUIA.flgest = "P"
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE GUIA THEN NEXT.
        FIND FIRST T-CGUIAS OF GUIA NO-LOCK NO-ERROR.
        IF NOT AVAILABLE T-CGUIAS THEN DO:
            CREATE T-CGUIAS.
            BUFFER-COPY GUIA TO T-CGUIAS.
        END.
    END.
END.
