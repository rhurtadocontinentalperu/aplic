...
&Scoped-Define ENABLED-OBJECTS br_table txtOTR BtnAddOtr 
&Scoped-Define DISPLAYED-OBJECTS txtOTR 
...
DEFINE BUTTON BtnAddOtr 
     LABEL "Add OTR" 
     SIZE 13 BY .96.

DEFINE VARIABLE txtOTR AS CHARACTER FORMAT "X(15)":U 
     LABEL "Orden de Transferencia (OTR)" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME F-Main
     ...
     txtOTR AT ROW 8.5 COL 26 COLON-ALIGNED WIDGET-ID 2
     BtnAddOtr AT ROW 8.5 COL 45 WIDGET-ID 4
...
&Scoped-define SELF-NAME BtnAddOtr
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BtnAddOtr B-table-Win
ON CHOOSE OF BtnAddOtr IN FRAME F-Main /* Add OTR */
DO:
  ASSIGN txtOTR.
...
DO iCounter = 1 TO hBrowse:NUM-COLUMNS:
    ...
    IF LOOKUP(hColumn:NAME, 'SerRef,NroRef') > 0 THEN hColumn:READ-ONLY = TRUE.
END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      BtnAddOtr:SENSITIVE = YES.
      txtOTR:SENSITIVE = YES.
  END.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  DO WITH FRAME {&FRAME-NAME}:
      IF s-HR-Manual = NO THEN 
          ASSIGN
          btnAddOTR:VISIBLE = NO
          txtOTR:VISIBLE = NO.
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ue-add-otr B-table-Win 
PROCEDURE ue-add-otr :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE VAR lAlertMsg AS CHAR.

lAlertMsg = "No existe OTR".

/* Busco las G/R de la OTR y que no esten anulada */
FOR EACH b-almcmov USE-INDEX almc07 WHERE b-almcmov.codcia = s-codcia
    AND b-almcmov.codref = 'OTR'
    AND b-almcmov.nroref = txtOTR
    AND b-almcmov.tipmov = s-tipmov 
    AND b-almcmov.codmov = s-codmov
    AND b-Almcmov.flgest <> "A" NO-LOCK:
    /* ************************************ */
    /* 04/10/2023: F.Oblitas chequeo vs GRE */
    /* ************************************ */
    IF lGREONLINE THEN DO:
        FIND gre_header WHERE gre_header.serieguia = b-almcmov.nroser AND 
            gre_header.numeroguia = b-almcmov.nrodoc AND
            gre_header.m_rspta_sunat = "ACEPTADO POR SUNAT"
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE gre_header THEN NEXT.
    END.
    /* ************************************ */
    lAlertMsg = "".
    /* Verifico que la OTR este Rotulada */    
    FIND FIRST CcbCBult USE-INDEX llave03 WHERE CcbCBult.CodCia = s-codcia
        AND CcbCBult.CodDoc = b-Almcmov.CodRef
        AND CcbCBult.NroDoc = b-Almcmov.NroRef
        AND CcbCBult.CHR_01 = "P"
        NO-LOCK NO-ERROR.
    IF AVAILABLE Ccbcbult THEN DO:
        /* Si aun no esta en la hoja de Ruta Activa */
        IF NOT CAN-FIND(FIRST DI-RutaG OF DI-RutaC WHERE DI-RutaG.serref = b-almcmov.nroser 
                    AND DI-RutaG.nroref = b-almcmov.nrodoc NO-LOCK) THEN DO:

            /* Si no esta en otra H/R */
            lMsg = "".
            FOR EACH b-rutad USE-INDEX llave02 NO-LOCK WHERE b-rutad.codcia = s-codcia
                AND b-rutad.coddoc = s-coddoc
                AND b-rutad.serref = b-almcmov.nroser
                AND b-rutad.nroref = b-almcmov.nrodoc,
                FIRST b-rutac OF b-rutad NO-LOCK:
                IF b-rutac.flgest = "E" OR b-rutac.flgest = "P" THEN DO:
                    lmsg = "Documento ya se encuentra registrado en la H.R.:" + b-rutac.nrodoc.
                    lAlertMsg = "Documento ya se encuentra registrado en la H.R.:" + b-rutac.nrodoc.
                END.
                IF b-rutac.flgest = "C"  AND b-rutad.flgest = "C" THEN DO:
                    lMsg = "Documento ya se encuentra registrado en la H.R.:" + b-rutac.nrodoc.
                    "Documento ya se encuentra registrado en la H.R.:" + b-rutac.nrodoc.
                END.
            END.
            IF lMsg = "" THEN DO:
                CREATE di-rutaG.
                    ASSIGN di-rutaG.serref = b-almcmov.nroser
                            di-rutaG.nroref = b-almcmov.nrodoc
                            di-rutaG.codalm = b-almcmov.codalm.
        ...
        END.
        ELSE DO:
            /*lAlertMsg = "Ya se encuenta en la H/R".*/
        END.
    END.
    ELSE DO:
        lAlertMsg = "No esta Rotulado "  + STRING(Almcmov.CodRef,"999") + "-" + STRING(Almcmov.NroRef,"999999").
    END.
END.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = "YES" THEN DO:
      /* RHC 10/04/2019 SOlo por una semana saltar los UTILEX (MIN) */
      IF CAN-FIND(FIRST almacen WHERE almacen.codcia = s-codcia AND
                  almacen.codalm = Di-RutaG.CodAlm:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} AND
                  CAN-FIND(FIRST gn-divi WHERE gn-divi.codcia = s-codcia AND
                           gn-divi.coddiv = almacen.coddiv AND
                           GN-DIVI.CanalVenta = "MIN" NO-LOCK)
                  NO-LOCK) 
          AND (TODAY >= DATE(04/10/2019) AND TODAY <= DATE(04,17,2019) ) THEN DO:
      END.
      ELSE DO:
