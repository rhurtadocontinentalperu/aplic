...
DEFINE INPUT PARAMETER ppCodDoc  AS CHAR NO-UNDO.       /* Ojo pp  HPK */
...
DEFINE INPUT PARAMETER pOrdenCerrada AS LOG NO-UNDO.
...
FIND FIRST vtacdocu WHERE vtacdocu.codcia = s-codcia AND
                            ...
                            vtacdocu.codped = ppCodDoc AND       /* HPK */
                            vtacdocu.nroped = ppNroDoc NO-LOCK NO-ERROR.
...
FILL-IN-oc FILL-IN-sap RADIO-SET-tipo-rotulo Btn_OK-2 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 48 BY 17.12
...
DEFINE FRAME D-Dialog
     ...
     RADIO-SET-tipo-rotulo AT ROW 22.38 COL 24.86 NO-LABEL WIDGET-ID 8
...
ASSIGN 
       tt-w-report.Campo-L[1]:VISIBLE IN BROWSE BROWSE-14 = FALSE.
...
"tt-w-report.Campo-L[1]" "Sele" ? "logical" ? ? ? ? ? ? no ? no no "4.43" no no no "U" "" "" "TOGGLE-BOX" "," ? ? 5 no 0 no no
...
DO:
  ...
  IF radio-set-tipo-rotulo = 1 THEN DO:
      ...
      DEFINE VAR cHito AS CHAR.

      cHito = "CHEQUEO ROTULO".
      IF pOrdenCerrada = YES THEN cHito = "ROTULOS DESPACHO".
...
DO:
  ...
  IF browse-14:NUM-SELECTED-ROWS > 0 THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE vtacdocu THEN DO:
        FIND FIRST faccpedi WHERE faccpedi.codcia = s-codcia AND faccpedi.coddoc = vtacdocu.codref AND
                                    faccpedi.nroped = vtacdocu.nroref NO-LOCK NO-ERROR.
        IF AVAILABLE faccpedi THEN dCliente = faccpedi.nomcli.
    END.
    /*           
    /* Rotulos marcados */
    FOR EACH tt-w-report WHERE tt-w-report.campo-l[1] = YES :
    ...
    */
    ...
    DO iConteo = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
        IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(iConteo) THEN DO:
            CREATE rotulo-w-report.
                ASSIGN rotulo-w-report.Campo-C[1] = IF (AVAILABLE vtacdocu) THEN vtacdocu.codref ELSE "ERR"
                        rotulo-w-report.Campo-C[2] = IF (AVAILABLE vtacdocu) THEN vtacdocu.nroref ELSE "HPK NO EXI"
                        rotulo-w-report.Campo-C[3] = tt-w-report.campo-c[1].
                IF pOrdenCerrada = NO THEN DO:
                    ASSIGN rotulo-w-report.Campo-C[4] = ""
                            rotulo-w-report.Campo-C[30] = x-codper
                            rotulo-w-report.Campo-L[1] = YES.

                END.
                ELSE DO:
                    ASSIGN rotulo-w-report.Campo-C[4] = dCliente.
                END.
        END.
    END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BROWSE-14 BUTTON_Marcar BUTTON_Desmarcar FILL-IN-oc FILL-IN-sap 
         RADIO-SET-tipo-rotulo Btn_OK-2 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
