...
/* CASE TRUE:                                                                                    */
/*     WHEN x-Sloting = YES AND x-BarrCode = YES THEN RUN alm/w-chk-ingreso-dev-cli-v2.w ("BC"). */
/*     WHEN x-Sloting = YES AND x-BarrCode = NO  THEN RUN alm/w-chk-ingreso-dev-cli-v2.w ("M").  */
/*     WHEN x-Sloting = NO  AND x-BarrCode = YES THEN RUN alm/w-chk-ingreso-dev-cli.w ("BC").    */
/*     WHEN x-Sloting = NO  AND x-BarrCode = NO  THEN RUN alm/w-chk-ingreso-dev-cli.w ("M").     */
/* END CASE.                                                                                     */

RUN alm/w-chk-ingreso-dev-cli (INPUT x-Sloting, INPUT x-BarrCode).
