...
ELSE DO:
    FOR EACH VtaAlmDiv NO-LOCK WHERE Vtaalmdiv.codcia = s-codcia
        AND Vtaalmdiv.coddiv = B-CPEDI.Lista_de_Precios,
        FIRST Almacen OF Vtaalmdiv NO-LOCK WHERE Almacen.Campo-C[3] = 'Si'     /* SI Remates */
        BY VtaAlmDiv.Orden:
        j = j + 1.
        IF j > 5 THEN LEAVE.
        IF TRUE <> (s-CodAlm > "") THEN s-CodAlm = TRIM(VtaAlmDiv.CodAlm).
...
IF TRUE <> (s-CodAlm > '') THEN DO:
    ...
    ELSE DO:
        FOR EACH VtaAlmDiv NO-LOCK WHERE Vtaalmdiv.codcia = s-codcia
            AND Vtaalmdiv.coddiv = s-CodDiv,
            FIRST Almacen OF Vtaalmdiv NO-LOCK WHERE Almacen.Campo-C[3] = 'Si'     /* Remates */
            BY VtaAlmDiv.Orden:
            j = j + 1.
            IF j > 5 THEN LEAVE.
            IF TRUE <> (s-CodAlm > "") THEN s-CodAlm = TRIM(VtaAlmDiv.CodAlm).
...
/*                 NO-LOCK NO-ERROR.                                    */
    ...
    /*IF facdpedi.codmat = '094778' THEN MESSAGE FacDPedi.CanPed FacDPedi.CanAte.*/
    /* FILTROS */
...
/*     END.                                                                  */
    ...
    IF x-StkAct > 0 AND x-articulo-ICBPER <> Facdpedi.CodMat THEN DO:
        ...
        IF AVAILABLE FacTabla AND s-StkComprometido > 0 THEN DO:
            ...
            /*MESSAGE 'uno' s-StkComprometido Facdpedi.CanPed.*/
            s-StkComprometido = s-StkComprometido - (Facdpedi.Factor * (Facdpedi.CanPed - Facdpedi.CanAte)).
            /*IF s-StkComprometido <= 0 THEN s-StkComprometido = 0.*/
        END.
        /*IF facdpedi.codmat = '053593' THEN MESSAGE Facdpedi.CanPed x-articulo-ICBPER s-stkcomprometido B-CPEDI.CodDiv.*/
        /* ************************************* */
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-1 FILL-IN-2 FILL-IN-3 FILL-IN-4 FILL-IN-5 RADIO-SET-1 
      WITH FRAME D-Dialog.
