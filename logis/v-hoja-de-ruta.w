...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-Estado FILL-IN-Marca ~
FILL-IN_Registrado txtHora txtMinuto FILL-IN_Modificado FILL-IN-Responsable ~
txtCodPro txtDTrans FILL-IN-Ayudante-1 txtSerie txtNro FILL-IN-Ayudante-2 ~
txtConductor FILL-IN-Ayudante-3 FILL-IN-Ayudante-4 FILL-IN-Ayudante-5 ~
FILL-IN-Ayudante-6 FILL-IN-Ayudante-7 txtCargaMaxima txtPeso txtNroClientes ~
...
DEFINE VARIABLE FILL-IN_Modificado AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 20 BY .81 NO-UNDO.

DEFINE VARIABLE FILL-IN_Registrado AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 20 BY .81 NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     DI-RutaC.FchDoc AT ROW 1 COL 73 COLON-ALIGNED
          ...
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     FILL-IN-Estado AT ROW 1 COL 93 COLON-ALIGNED
...
"Externo", "02":U
          SIZE 16 BY .77
     DI-RutaC.usuario AT ROW 1.81 COL 93 COLON-ALIGNED
          LABEL "Usuario"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
          BGCOLOR 15 FGCOLOR 12 
     FILL-IN_Registrado AT ROW 1.81 COL 103 COLON-ALIGNED NO-LABEL WIDGET-ID 118
     ...
     DI-RutaC.Libre_c05 AT ROW 2.62 COL 93 COLON-ALIGNED WIDGET-ID 70
          LABEL "Modificado por" FORMAT "x(10)"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     FILL-IN_Modificado AT ROW 2.62 COL 103 COLON-ALIGNED NO-LABEL WIDGET-ID 116
     DI-RutaC.Libre_f05 AT ROW 2.62 COL 127 COLON-ALIGNED WIDGET-ID 72
          LABEL "Día"
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
...
/* SETTINGS FOR FILL-IN FILL-IN-Responsable IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FILL-IN_Modificado IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FILL-IN_Registrado IN FRAME F-Main
   NO-ENABLE                                                            */
...
------------------------------------------------------------------------------*/
  ...
  /* ***************************************** */
  /* 17/08/2022 Grabamos si es supervisor o no */
  /* ***************************************** */
  ASSIGN
      DI-RutaC.Libre_l05 = s-acceso-total.  /* (YES: supervisor   NO: digitador) */
  /* ***************************************** */
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
  /* Solo se anula siempre y cuando esté pendiente */
  IF DI-RutaC.FlgEst <> "P" THEN DO:
      MESSAGE 'Acceso Denegado' VIEW-AS ALERT-BOX ERROR.
      RETURN 'ADM-ERROR'.
  END.
  /* Si está CERRADO solo lo puede anular el SUPERVISOR */
  IF DI-RutaC.Libre_l01 = YES AND s-Acceso-Total = NO THEN DO:
      MESSAGE 'Acceso Denegado' VIEW-AS ALERT-BOX ERROR.
      RETURN 'ADM-ERROR'.
  END.
  ...
  IF AVAILABLE DI-RUTAC THEN DO WITH FRAME {&FRAME-NAME}:
    ...
    FOR EACH di-rutaD WHERE di-rutac.codcia = di-rutad.codcia
        ...
        IF AVAILABLE ccbcdocu THEN DO:
            ...
            /* 28/02/2023 */
            ASSIGN
                txtPeso = txtPeso + ccbcdocu.libre_d01
                txtVol = txtVol + ccbcdocu.libre_d02
                .
        END.
...
/*                     NO-ERROR.                                                                            */
            ...
            /* 28/02/2023 */
            ASSIGN
                txtPeso = txtPeso + almcmov.libre_d01
                txtVol = txtVol + almcmov.libre_d02
                .
        END.
    END.
    /* 23/05/2023: Peso Guías "Itinerantes" */
    FOR EACH Di-RutaDG OF Di-RutaC NO-LOCK:
        /* Buscamos el GRE_HEADER */
        FIND gre_header WHERE gre_header.m_divorigen = Di-RutaC.coddiv AND
            ...
            NO-LOCK NO-ERROR.
        IF AVAILABLE gre_header THEN txtPeso = txtPeso + gre_header.pesoBrutoTotalBienes.
    END.
    ...
    DEF VAR pDisabled AS LOG  NO-UNDO.
    RUN lib/_user-information (DI-RutaC.usuario,
                               OUTPUT FILL-IN_Registrado,
                               OUTPUT pPassword,
                               OUTPUT pDisabled).
    DISPLAY FILL-IN_Registrado WITH FRAME {&FRAME-NAME}.
    RUN lib/_user-information (DI-RutaC.Libre_c05,
                               OUTPUT FILL-IN_Modificado,
                               OUTPUT pPassword,
                               OUTPUT pDisabled).
    DISPLAY FILL-IN_Modificado WITH FRAME {&FRAME-NAME}.
    /* ************************************************************************ */
    /* ************************************************************************ */
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      */
      ...
      /* ************************************************ */
      RUN Validacion-Estandar.
      IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN 'ADM-ERROR'.
      /* ************************************************ */
      /* 17/08/2022: Control de CONTRA-ENTREGA (001): NO para supervisores */
      DEF VAR lContraEntrega AS LOG INIT NO NO-UNDO.
      IF LOGICAL(DI-RutaC.Libre_l01:SCREEN-VALUE) = YES THEN DO:
          /* Buscamos contra-entrega que no estén cancelados */
          FOR EACH di-rutaD WHERE di-rutad.codcia = di-rutac.codcia AND
              ...
              IF FACTURA.FlgEst <> "C" THEN DO:
/*                   MESSAGE 'NO se puede CONFIRMAR la Hoja de Ruta porque' SKIP   */
/*                       'el comprobante' FACTURA.coddoc FACTURA.nrodoc SKIP       */
/*                       'relacionado con la' di-rutad.codref di-rutad.nroref SKIP */
...
/*                   RETURN 'ADM-ERROR'.                                           */
              ...
              lContraEntrega = YES.
          END.
          /* Verificamos el cobrador */
          IF lContraEntrega = YES THEN DO:
              FOR EACH PL-PERS NO-LOCK WHERE PL-PERS.NroDocId = DI-RutaC.responsable:SCREEN-VALUE AND 
                  ...
                  IF NOT AVAILABLE PL-FLG-MES OR INDEX(PL-FLG-MES.cargos, 'COBRADOR') = 0 OR PL-FLG-MES.vcontr <> ? THEN DO:
                      CASE s-acceso-total:
                          WHEN NO THEN DO:      /* Digitador */
                              MESSAGE 'H/R tiene pedidos contraentrega' SKIP
                                  'Asignar un cobrador' VIEW-AS ALERT-BOX ERROR.
                              RETURN 'ADM-ERROR'.
                          END.
                          WHEN YES THEN DO:     /* Supervisor */
                              MESSAGE 'H/R tiene pedidos contraentrega' SKIP
                                  'El responsable NO es un cobrador' SKIP
      ...
      END.
      /* ***************************************************************************************** */
      /* 10/07/2023: Control GRE */
      /* ***************************************************************************************** */
      /*
      IF LOGICAL(DI-RutaC.Libre_l01:SCREEN-VALUE) = YES THEN DO:
          /* por GRE por ventas */
          FOR EACH Di-RutaD OF Di-RutaC NO-LOCK,
              FIRST gre_header NO-LOCK WHERE gre_header.serieGuia = INTEGER(SUBSTRING(DI-RutaD.NroRef,1,3)) AND
              gre_header.numeroGuia = INTEGER(SUBSTRING(DI-RutaD.NroRef,4)):
              IF gre_header.fechaInicioTraslado <> DATE(DI-RutaC.FchSal:SCREEN-VALUE) THEN DO:
                  MESSAGE 'La Fecha de Salida del Vehículo de la H/R no coincide con la Fecha de Inicio de Traslado de la GRE' SKIP
                      'Revisar la GRE:' STRING(gre_header.serieGuia, '999') + STRING(gre_header.numeroGuia, '9999999999') 
                      VIEW-AS ALERT-BOX WARNING.
                  RETURN 'ADM-ERROR'.
              END.
          END.
          /* por GRE por transferencias */
          FOR EACH Di-RutaG OF Di-RutaC NO-LOCK,
              FIRST gre_header NO-LOCK WHERE gre_header.serieGuia = DI-RutaG.SerRef AND
              gre_header.numeroGuia = DI-RutaG.NroRef:
              IF gre_header.fechaInicioTraslado <> DATE(DI-RutaC.FchSal:SCREEN-VALUE) THEN DO:
                  MESSAGE 'La Fecha de Salida del Vehículo de la H/R no coincide con la Fecha de Inicio de Traslado de la GRE' SKIP
                      'Revisar la GRE:' STRING(gre_header.serieGuia, '999') + STRING(gre_header.numeroGuia, '9999999999') 
                      VIEW-AS ALERT-BOX WARNING.
          ...
          /* por GRE por otros */
          FOR EACH Di-RutaDG OF Di-RutaC NO-LOCK,
              FIRST gre_header NO-LOCK WHERE gre_header.serieGuia = INTEGER(SUBSTRING(DI-RutaDG.NroRef,1,3)) AND
              gre_header.numeroGuia = INTEGER(SUBSTRING(DI-RutaDG.NroRef,4)):
              IF gre_header.fechaInicioTraslado <> DATE(DI-RutaC.FchSal:SCREEN-VALUE) THEN DO:
                  MESSAGE 'La Fecha de Salida del Vehículo de la H/R no coincide con la Fecha de Inicio de Traslado de la GRE' SKIP
                      'Revisar la GRE:' STRING(gre_header.serieGuia, '999') + STRING(gre_header.numeroGuia, '9999999999') 
                      VIEW-AS ALERT-BOX WARNING.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Validacion-Estandar V-table-Win 
PROCEDURE Validacion-Estandar :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    /* 01/10/22 Fernan Oblitas: para todos */
    IF DI-RutaC.Responsable:SCREEN-VALUE > '' THEN DO:
        RUN Valida-Personal (INPUT DI-RutaC.Responsable:SCREEN-VALUE,
                             OUTPUT pNombre,
                             OUTPUT pOrigen).
        IF pOrigen = "ERROR" THEN DO:
            MESSAGE 'Responsable NO registrado ni por TERCERO ni PROPIO' VIEW-AS ALERT-BOX WARNING.
    ...
    CASE s-HR-Manual:
      WHEN NO THEN DO:  /* CD's */
          ...
          IF RETURN-VALUE = "YES" THEN DO:
              ...
              END.
          END.
          /* 11/04/2025 Martin Chirinos: Digitar ESTIBADORES */
/*           /* RHC 07/09/2019 SOlo aceptar personal propio */                         */
/*           DO iIndice = 1 TO gn-vehic.NroEstibadores:                                            */
/*               CASE iIndice:                                                                     */
/*                   WHEN 1 THEN DO:                                                               */
