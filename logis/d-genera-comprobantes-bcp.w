...
FOR EACH FacCorre NO-LOCK WHERE 
    ...
    END.
    ELSE cOk = YES.
    cListItems = cListItems + (IF TRUE <> (cListItems > '') THEN '' ELSE ',') + STRING(FacCorre.NroSer,'999').
END.
...
Btn_Cancel TOGGLE-genera-pgre RECT-1 
...
FILL-IN-NroDoc-GR TOGGLE-genera-pgre 
...
DEFINE FRAME bgDialog
     BROWSE-2 AT ROW 9.23 COL 2 WIDGET-ID 200
     ...
     TOGGLE-genera-pgre AT ROW 7.35 COL 13.14 WIDGET-ID 44
...
/*                   MESSAGE "La O/D pertenece a pedido PLANIFICADO, la FAI debe generarse sin G/R" */
/*                       VIEW-AS ALERT-BOX ERROR.                                                   */
/*                   RETURN NO-APPLY.                                                               */
/*               END.                                                                               */
/*           END.                                                                                   */
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-tipo COMBO-BOX-Guias COMBO-NroSer-Guia FILL-IN-NroPed 
          ...
          TOGGLE-genera-pgre 
      WITH FRAME bgDialog.
  ENABLE BROWSE-2 FILL-IN-tipo COMBO-NroSer Btn_OK Btn_Cancel 
         TOGGLE-genera-pgre RECT-1 
      WITH FRAME bgDialog.
  VIEW FRAME bgDialog.
    ...
    FOR EACH T-CDOCU NO-LOCK:
        ...
        IF COMBO-BOX-Guias = "SI" THEN DO:
            ...
            ASSIGN
                ...
                CcbDCaja.CodDoc = Ccbcdocu.coddoc
                CcbDCaja.CodMon = Ccbcdocu.codmon
                CcbDCaja.CodRef = Ccbcdocu.coddoc
                CcbDCaja.FchCbd = TODAY
                CcbDCaja.FchDoc = TODAY
                CcbDCaja.ImpTot = Ccbcdocu.sdoact
        ...
        /* *********************************************************************** */
        RUN vtagn/p-actualiza-almacen.p ( INPUT ROWID(CcbCDocu), OUTPUT pMensaje ).
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
...
/*   DEFINE VARIABLE cListItems AS CHARACTER NO-UNDO. */
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      /* *********************** */
      CASE cCodDoc:
          WHEN "FAC" THEN ASSIGN FILL-IN-items = FacCfgGn.Items_Factura COMBO-NroSer:LABEL = 'FACTURA'.
          WHEN "BOL" THEN ASSIGN FILL-IN-items = FacCfgGn.Items_Boleta  COMBO-NroSer:LABEL = 'BOLETAS'.
          WHEN "FAI" THEN ASSIGN FILL-IN-items = FacCfgGn.Items_Boleta  COMBO-NroSer:LABEL = 'FAIs'.
      END CASE.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    BUFFER-COPY FacCPedi 
        ...
        T-CDOCU.CodMov = cCodMov                    /* OJO: Se envía el CodMov para almacén */
...
FOR EACH Reporte NO-LOCK, FIRST Ccbcdocu OF Reporte NO-LOCK:
    ...
    IF RETURN-VALUE = "OK" OR RETURN-VALUE = "PLAN-B" THEN DO:
        ...
        IF toggle-genera-pgre = NO THEN DO:
            b-gre_cmpte.estado = "NO GENERA PGRE".
        END.
