...
DO:
  ...
  REPEAT WHILE NOT QUERY-OFF-END("BROWSE_Destino"):
      IF AVAILABLE(Ccbcdocu) AND NOT CAN-FIND(FIRST t-Ccbadocu WHERE t-Ccbadocu.codcia = Ccbcdocu.codcia AND
                                              t-Ccbadocu.coddiv = Ccbcdocu.coddiv AND
                                              t-Ccbadocu.coddoc = Ccbcdocu.coddoc AND
                                              t-Ccbadocu.nrodoc = ccbcdocu.nrodoc NO-LOCK) THEN DO:
  ...
  DO iRegistro = 1 TO BROWSE_Origen:NUM-SELECTED-ROWS:
      IF BROWSE_Origen:FETCH-SELECTED-ROW(iRegistro) THEN DO:
          IF CAN-FIND(bCcbcdocu WHERE bCcbcdocu.codcia = s-codcia 
              ...
              NEXT.
          END.
          /* 02/06/2026: Pasamos datos del transportista por Comprobante */
          IF AVAILABLE Ccbcdocu THEN DO:
              FIND FIRST CcbADocu WHERE CcbADocu.codcia = s-codcia AND
                  CcbADocu.coddiv = Ccbcdocu.coddiv AND
                  CcbADocu.coddoc = Ccbcdocu.coddoc AND
                  CcbADocu.nrodoc = Ccbcdocu.nrodoc NO-LOCK NO-ERROR.
              IF AVAILABLE CcbADocu THEN DO:
                  FIND t-CcbADocu WHERE t-CcbADocu.codcia = s-codcia AND
                      t-CcbADocu.coddiv = Ccbcdocu.coddiv AND
                      t-CcbADocu.coddoc = Ccbcdocu.coddoc AND
                      t-CcbADocu.nrodoc = Ccbcdocu.nrodoc NO-LOCK NO-ERROR.
...
DO:
  IF NOT AVAILABLE(tgre_cmpte_2) THEN RETURN NO-APPLY.
  ...
  FIND bCcbcdocu WHERE bCcbcdocu.codcia = s-codcia 
      AND bCcbcdocu.coddoc = tgre_cmpte_2.coddoc
      AND bCcbCDocu.NroDoc = tgre_cmpte_2.nrodoc
      NO-LOCK NO-ERROR.
  IF NOT AVAILABLE bCcbcdocu THEN RETURN NO-APPLY.
  FIND FIRST FacCPedi WHERE FacCPedi.CodCia = bCcbCDocu.CodCia
      AND FacCPedi.CodDoc = bCcbCDocu.Libre_c01
  ...
  ASSIGN
      pcCodDiv = Ccbcdocu.coddiv
      pcCodDoc = Ccbcdocu.coddoc    /* FAC o BOL */
      pcNroDoc = Ccbcdocu.nrodoc.
    ...
    DO TRANSACTION ON ERROR UNDO trloop, RETURN 'ADM-ERROR' ON STOP UNDO trloop, RETURN 'ADM-ERROR':
        ...
        FOR EACH T-DDOCU NO-LOCK, FIRST Almmmatg OF T-DDOCU NO-LOCK,
            ...
            IF lCreaHeader THEN DO:
                ...
                FIND FIRST T-CcbADocu WHERE T-CcbADocu.codcia = B-CDOCU.codcia AND
                    T-CcbADocu.coddiv = B-CDOCU.coddiv AND
                    T-CcbADocu.coddoc = B-CDOCU.coddoc AND
                    T-CcbADocu.nrodoc = B-CDOCU.nrodoc
                    NO-LOCK NO-ERROR.
