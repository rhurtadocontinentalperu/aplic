...
VtaDTabla.Libre_c04 VtaDTabla.Libre_c01 Almtfami.desfam VtaDTabla.Libre_c02 ~
AlmSFami.dessub VtaDTabla.Libre_c03 Almmmatg.DesMat VtaDTabla.Libre_l01 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table VtaDTabla.Libre_d01 ~
VtaDTabla.Tipo VtaDTabla.Libre_c04 VtaDTabla.Libre_c01 VtaDTabla.Libre_c02 ~
VtaDTabla.Libre_c03 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      VtaDTabla.Tipo FORMAT "x(15)":U WIDTH 14.57 VIEW-AS COMBO-BOX INNER-LINES 5
                      LIST-ITEMS "RACK" 
                      DROP-DOWN-LIST 
      VtaDTabla.Libre_c04 COLUMN-LABEL "Sector" FORMAT "x(8)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 144 BY 15.35
...
"VtaDTabla.Tipo" ? "x(15)" "character" ? ? ? ? ? ? yes ? no no "14.57" yes no no "U" "" "" "DROP-DOWN-LIST" "," "RACK" ? 5 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaDTabla.Libre_c04
"VtaDTabla.Libre_c04" "Sector" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&Scoped-define SELF-NAME VtaDTabla.Libre_c04
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL VtaDTabla.Libre_c04 br_table _BROWSE-COLUMN B-table-Win
ON LEFT-MOUSE-DBLCLICK OF VtaDTabla.Libre_c04 IN BROWSE br_table /* Sector */
OR f8 OF VtaDTabla.Libre_c04 DO:
  ASSIGN
      input-var-1 = ''
      input-var-2 = ''
      input-var-3 = ''
      output-var-1 = ?.
  FIND FIRST Almacen WHERE Almacen.CodDiv = gn-divi.coddiv AND
      Almacen.CodCia = gn-divi.codcia AND
      Almacen.AlmPrincipal = YES
      NO-LOCK NO-ERROR.
  IF AVAILABLE Almacen THEN DO:
      input-var-1 = Almacen.CodAlm.
      RUN lkup/c-sectores ('Sectores').
...
IF VtaDTabla.Libre_c03:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > '' THEN DO:
    ...
    END.
END.
IF VtaDTabla.Libre_c04:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} > '' THEN DO:
    FIND FIRST Almacen WHERE Almacen.CodDiv = gn-divi.coddiv AND
        Almacen.CodCia = gn-divi.codcia AND
        Almacen.AlmPrincipal = YES
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almacen THEN DO:
        MESSAGE 'NO está definido el almacén principal para esta división' VIEW-AS ALERT-BOX ERROR.
        VtaDTabla.Libre_c04:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = ''.
        APPLY 'ENTRY':U TO VtaDTabla.Libre_c04.
        RETURN 'ADM-ERROR'.
    END.
    FIND AlmtZona WHERE AlmtZona.CodCia = Almacen.CodCia AND
        AlmtZona.CodAlm = Almacen.CodAlm AND
        AlmtZona.CodZona = VtaDTabla.Libre_c04:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE AlmtZona THEN DO:
        MESSAGE 'Zona NO definida en el almacén principal' Almacen.codalm VIEW-AS ALERT-BOX ERROR.
        VtaDTabla.Libre_c04:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = ''.
        APPLY 'ENTRY':U TO VtaDTabla.Libre_c04.
        RETURN 'ADM-ERROR'.
    END.
