...
&Scoped-define QUERY-STRING-Query-Main FOR EACH gn-provd OF gn-prov WHERE ~{&KEY-PHRASE} ~
      AND gn-provd.Sede = "@@@" NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH gn-provd OF gn-prov WHERE ~{&KEY-PHRASE} ~
      AND gn-provd.Sede = "@@@" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main gn-provd
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main gn-provd
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.gn-provd OF INTEGRAL.gn-prov"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "gn-provd.Sede = ""@@@"""
...
------------------------------------------------------------------------------*/
   ...
   DEF BUFFER B-PROVD FOR gn-provd.
   IF NOT CAN-FIND(FIRST B-PROVD WHERE B-PROVD.CodCia = gn-prov.CodCia AND
                   B-PROVD.CodPro = B-PROVD.CodPro AND
                   B-PROVD.Sede = "@@@" NO-LOCK)
       THEN DO:
       /* RHC 31/10/18 Se crea un refgistro en gn-provd automaticamente */
       CREATE B-PROVD.
       BUFFER-COPY gn-prov TO B-PROVD
           ASSIGN 
           B-PROVD.DomFiscal = YES
           B-PROVD.Sede = "@@@".   /* Valor que no se puede anular */
       RELEASE B-PROVD.
   END.
