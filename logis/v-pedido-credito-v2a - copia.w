...
------------------------------------------------------------------------------*/
  ...
  FOR EACH PEDI, FIRST Almmmatg OF PEDI NO-LOCK BY PEDI.NroItm:
      ...
      FIND Almmmate WHERE Almmmate.codcia = s-codcia
          ...
          NO-LOCK NO-ERROR.
      IF NOT AVAILABLE Almmmate THEN DO:
          pMensaje = "NO está asignada la bonificación " + PEDI.CodMat + " en el almacén " + PEDI.AlmDes.
          RETURN 'ADM-ERROR'.
      END.
      x-StkAct = Almmmate.StkAct.
      ...
      s-StkDis = x-StkAct - s-StkComprometido.
      IF s-StkDis <= 0 THEN DO:
          pMensaje = 'El STOCK esta en CERO para la bonificación ' + PEDI.codmat +
              'en el almacén ' + PEDI.AlmDes.
          /* OJO: NO DESPACHAR */
          RETURN 'ADM-ERROR'.
      END.
      /* **************************************************************************************** */
      ...
      IF s-StkDis < x-CanPed THEN DO:
          pMensaje = 'El STOCK no cubre la bonificación ' + PEDI.codmat +
              'en el almacén ' + PEDI.AlmDes.
          /* OJO: NO DESPACHAR */
          RETURN 'ADM-ERROR'.
      END.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH Facdpedi OF Faccpedi NO-LOCK:
        I-NPEDI = I-NPEDI + 1.
    END.
    FOR EACH BONIFICACION WHERE BONIFICACION.Por_Dsctos[2] = 0:
        CREATE Facdpedi.
        BUFFER-COPY BONIFICACION
            EXCEPT BONIFICACION.TipVta    /* Campo con valor A, B, C o D */
            TO Facdpedi
            ASSIGN
                Facdpedi.CodCia = Faccpedi.CodCia
                Facdpedi.CodDiv = Faccpedi.CodDiv
    ...
    IF pParam = "UPDATE" THEN DO:
        FIND FIRST PEDIDO WHERE PEDIDO.codcia = Faccpedi.codcia AND
            ...
            PEDIDO.nroref = Faccpedi.nroref EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
        IF ERROR-STATUS:ERROR = YES THEN DO:
            {lib/mensaje-de-error.i &MensajeError="pMensaje" &CuentaError="i-Cuenta"}
            UNDO, RETURN 'ADM-ERROR'.
        END.
