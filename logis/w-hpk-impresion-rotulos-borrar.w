...
(ChkTareas.FlgEst = 'T' or ChkTareas.FlgEst = 'C') NO-LOCK, ~
 ...
 VtaCDocu.NroPed =  ChkTareas.NroPed ~
      AND (fill-in-codtrab = "" or entry(1,vtacdocu.libre_c04,"|") = fill-in-codtrab) and ~
...
BUTTON-6 BUTTON-5 BUTTON-4 FILL-IN-desde FILL-IN-hasta BUTTON-1 BUTTON-2 ~
BUTTON-3 BROWSE-16 FILL-IN-phr 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-codtrab FILL-IN-copias ~
FILL-IN-desde FILL-IN-hasta FILL-IN-phr 
...
DEFINE FRAME F-Main
     ...
     FILL-IN-codtrab AT ROW 1.12 COL 12.14 COLON-ALIGNED WIDGET-ID 30
     ...
     FILL-IN-desde AT ROW 1.08 COL 83.57 COLON-ALIGNED WIDGET-ID 12
     FILL-IN-hasta AT ROW 1.08 COL 108.29 COLON-ALIGNED WIDGET-ID 14
     BUTTON-1 AT ROW 17.15 COL 96 WIDGET-ID 8
     BUTTON-2 AT ROW 18.58 COL 96 WIDGET-ID 10
     BUTTON-3 AT ROW 1.12 COL 128.29 WIDGET-ID 16
     BROWSE-16 AT ROW 15.58 COL 2 WIDGET-ID 300
     FILL-IN-phr AT ROW 1.12 COL 36.86 COLON-ALIGNED WIDGET-ID 32
     ...
     "Ordenes terminadas" VIEW-AS TEXT
          SIZE 20.57 BY .62 AT ROW 1.23 COL 56.43 WIDGET-ID 18
          FGCOLOR 9 FONT 11
...
(ChkTareas.FlgEst = 'T' or ChkTareas.FlgEst = 'C')"
 ...
 INTEGRAL.VtaCDocu.NroPed =  ChkTareas.NroPed"
     _Where[2]         = "(fill-in-codtrab = """" or entry(1,vtacdocu.libre_c04,""|"") = fill-in-codtrab) and
(fill-in-phr = """" or vtacdocu.nroori = fill-in-phr)"
...
"INTEGRAL.ChkTareas.CodDoc" "Cod." ? "character" ? ? ? ? ? 0 no ? no no "3.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.ChkTareas.NroPed
"INTEGRAL.ChkTareas.NroPed" "Nro.Orden" ? "character" ? ? ? ? ? 0 no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"INTEGRAL.ChkTareas.Embalaje" "Embalado" ? "logical" ? ? ? ? ? ? no ? no no "9.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > INTEGRAL.ChkTareas.Mesa
"INTEGRAL.ChkTareas.Mesa" ? ? "character" ? ? ? ? ? ? no ? no no "9.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    ASSIGN fill-in-codtrab fill-in-phr.
...
FOR EACH tt-w-report WHERE tt-w-report.campo-l[1] = YES 
    ...
    ASSIGN         
        ...
        rr-w-report.Campo-F[1] = tt-w-report.campo-f[1]
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BROWSE-14 FILL-IN-codtrab FILL-IN-copias BUTTON-6 BUTTON-5 BUTTON-4 
...
IF chktareas.coddoc = 'HPK' THEN DO:
    ...
    IF AVAILABLE vtacdocu THEN DO:
        ...
        FOR EACH vtacdocu WHERE vtacdocu.codcia = s-codcia AND
                                    ...
                                    vtacdocu.nroref = x-nrodoc NO-LOCK:
            IF vtacdocu.flgsit <> 'PC' THEN x-phr-esta-cerrada = NO.
        END.
        /**/
        IF x-phr-esta-cerrada = NO THEN DO:
            MESSAGE "La " + x-coddoc + "-" + x-nrodoc + " aun tiene HPK pendientes por Chequear"
