...
DO:
    ...
    IF NOT AVAILABLE vtacdocu THEN DO:
        ...
        RETURN NO-APPLY.
    END.
    IF NOT (vtacdocu.FlgEst = "P" AND vtacdocu.FlgSit = "TI" AND vtacdocu.FlgImpOd = YES) THEN DO:
        MESSAGE 'La HPK debe estar asignada a piqueador e impresa' VIEW-AS ALERT-BOX ERROR.
        SELF:SCREEN-VALUE = ''.
        RETURN NO-APPLY.
    END.
    /* Piqueador */
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE WITH FRAME {&FRAME-NAME}:
    ...
    IF NOT (vtacdocu.FlgEst = "P" AND vtacdocu.FlgSit = "TI") THEN DO:
        pMensaje = 'Orden ya fue ingresada'.
        UNDO, LEAVE.
    END.
    /* ********************************************* */
    /* RHC 16/09/2019 Verificamos tareas del picador */
    /* Bloqueado el 07/10/2019 */
    /* ********************************************* */
/*     DEF BUFFER B-CDOCU FOR Vtacdocu.                                        */
/*     FIND FIRST B-CDOCU WHERE B-CDOCU.codcia = Vtacdocu.codcia AND           */
/*         B-CDOCU.coddiv = Vtacdocu.coddiv AND                                */
/*         B-CDOCU.codped = Vtacdocu.codped AND                                */
/*         B-CDOCU.nroped <> Vtacdocu.nroped AND                               */
/*         B-CDOCU.UsrSac = Vtacdocu.usrsac AND                                */
/*         B-CDOCU.FlgEst = "P" AND                                            */
/*         B-CDOCU.FlgSit = "TP" NO-LOCK NO-ERROR.                             */
/*     IF AVAILABLE B-CDOCU THEN DO:                                           */
/*         pMensaje = "El piqueador ya tiene un piqueo INICIADO: " + CHR(10) + */
/*             B-CDOCU.CodPed + " " + B-CDOCU.NroPed.                          */
/*         UNDO, LEAVE.                                                        */
/*     END.                                                                    */
    /* ********************************************* */
    ASSIGN
