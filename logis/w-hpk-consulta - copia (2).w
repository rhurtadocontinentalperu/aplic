...
DEFINE FRAME F-Main
     ...
     BUTTON-TEXTO AT ROW 5.31 COL 92 WIDGET-ID 30
    ...
    FOR EACH T-DI-RutaC NO-LOCK, FIRST DI-RutaC OF T-DI-RutaC NO-LOCK:
        ...
        FOR EACH T-VtaCDocu WHERE T-VtaCDocu.CodCia = DI-RutaC.CodCia
            ...
            BUFFER-COPY T-VtaCDocu EXCEPT T-VtaCDocu.Bultos
                TO DETALLE 
                ASSIGN DETALLE.BultosDetalle = T-VtaCDocu.Bultos.
        END.
    END.
    SESSION:SET-WAIT-STATE('').
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_b-hpk-consulta-cab ( 6.69 , 78.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-size IN h_b-hpk-consulta-det ( 14.54 , 122.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-size IN h_b-phr-consulta-doc ( 14.00 , 124.00 ) NO-ERROR.
