...
/* Filtros dinámicos */
DEF VAR x-Sector AS CHAR INIT 'TODOS' NO-UNDO.
DEF VAR x-PHR AS CHAR INIT 'TODOS' NO-UNDO.

DEF VAR x-CodRef AS CHAR NO-UNDO.
DEF VAR x-NroRef AS CHAR NO-UNDO.

/* para la 2da ubicacion */ 
DEFINE VAR x-empaques AS CHAR.

DEFINE VAR x-master AS CHAR INIT " ".
DEFINE VAR x-inner AS CHAR INIT " ".
DEFINE VAR x-unidades AS CHAR INIT " ".
DEFINE VAR x-qmaster AS INT INIT 0.
DEFINE VAR x-qinner AS INT INIT 0.
DEFINE VAR x-qunidades AS INT INIT 0.
...
DEFINE VAR x-que-ubicacion AS INT.
...
&Scoped-define QUERY-STRING-br_table FOR EACH T-CDocu WHERE ~{&KEY-PHRASE} ~
      AND (x-Sector = 'TODOS' OR T-CDocu.ZonaPickeo = x-Sector) ~
 AND (x-PHR = 'TODOS' OR T-CDocu.NroOri = x-PHR) NO-LOCK, ~
      FIRST VtaCDocu OF T-CDocu NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH T-CDocu WHERE ~{&KEY-PHRASE} ~
      AND (x-Sector = 'TODOS' OR T-CDocu.ZonaPickeo = x-Sector) ~
 AND (x-PHR = 'TODOS' OR T-CDocu.NroOri = x-PHR) NO-LOCK, ~
      FIRST VtaCDocu OF T-CDocu NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 149 BY 8.08
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "(x-Sector = 'TODOS' OR T-CDocu.ZonaPickeo = x-Sector)
 AND (x-PHR = 'TODOS' OR T-CDocu.NroOri = x-PHR)"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Actualiza-PHR B-table-Win 
PROCEDURE Actualiza-PHR :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF OUTPUT PARAMETER pPHR AS CHAR.

    pPHR = ''.
    FOR EACH T-CDOCU NO-LOCK BREAK BY T-CDOCU.NroOri:
        IF FIRST-OF(T-CDOCU.NroOri) THEN pPHR = pPHR +
            (IF TRUE <> (pPHR > '') THEN '' ELSE ',') +
            T-CDOCU.NroOri.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Actualiza-Sectores B-table-Win 
PROCEDURE Actualiza-Sectores :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF OUTPUT PARAMETER pSector AS CHAR.
    ...
    pSector = ''.
    FOR EACH T-CDOCU NO-LOCK BREAK BY T-CDOCU.ZonaPickeo:
        IF FIRST-OF(T-CDOCU.ZonaPickeo) THEN pSector = pSector +
            (IF TRUE <> (pSector > '') THEN '' ELSE ',') +
            T-CDOCU.ZonaPickeo.
    END.
...
DEF INPUT PARAMETER pPHR AS CHAR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Captura-Sector B-table-Win 
PROCEDURE Captura-Sector :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF INPUT PARAMETER pSector AS CHAR.
...
/*MESSAGE "x-que-ubicacion " x-que-ubicacion.*/
...
DO k = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
    IF {&BROWSE-NAME}:FETCH-SELECTED-ROW(k) THEN DO:
        ...
        FOR EACH VtaDDocu OF VtaCDocu NO-LOCK,
            ...
            x-empaques = fGen-Empaques( VtaDDocu.CanPed * VtaDDocu.Factor ).

            FIND Almmmate OF Almmmatg WHERE Almmmate.codalm = VtaDDocu.AlmDes NO-LOCK NO-ERROR.
            x-ubicacion = (IF AVAILABLE Almmmate THEN Almmmate.CodUbi ELSE VtaDDocu.CodUbi).

            IF x-que-ubicacion > 0 THEN DO:            
                /* Verificamos si tiene segunda ubicacion */
                FIND FIRST x-vtatabla WHERE x-vtatabla.codcia = s-codcia AND
                                            x-vtatabla.tabla = "ART-UBIC" AND
                                            x-vtatabla.llave_c1 = vtacdocu.codalm AND
                                            x-vtatabla.llave_c2 = vtaddocu.codmat NO-LOCK NO-ERROR.
                IF AVAILABLE x-vtatabla THEN DO:
                    /* Tiene doble ubicacion */        
                    IF x-que-ubicacion = 1 AND TRUE <> (x-master > "")  THEN DO:
                        /* Se esta imprimiendo la ubicacion principal y no hay master */
                        NEXT.
                    END.
                    IF x-que-ubicacion = 2 AND TRUE <> (x-inner > "") AND TRUE <> (x-unidades > "") THEN DO:
                        /* Se esta imprimiendo la 2da ubicacion y no hay inner ni unidades */
                        NEXT.
                    END.
                    IF x-que-ubicacion > 1 THEN DO:
                        x-ubicacion = x-vtatabla.llave_c3.
                        x-empaques = x-inner + x-unidades.
                        x-cantidad = x-qinner + x-qunidades.
                    END.
                    ELSE DO:
                        x-empaques = x-master.
                        x-cantidad = x-qmaster.
                    END.
                END.
                ELSE DO:
                    IF x-que-ubicacion > 1 THEN DO:
            ...
            ASSIGN 
                ...
                Reporte.CodDoc = VtaCDocu.CodPed
                Reporte.NroPed = VtaCDocu.NroPed
                Reporte.CodRef = VtaCDocu.CodRef    /* O/D OTR */
                Reporte.NroRef = VtaCDocu.NroRef
                Reporte.CodMat = VtaDDocu.CodMat
                Reporte.DesMat = Almmmatg.DesMat
                Reporte.DesMar = Almmmatg.DesMar
                ...
                Reporte.Agencia = (IF Vtacdocu.ZonaPickeo = "VA" THEN YES ELSE NO)
                Reporte.CodZona = "G-0"
                Reporte.x-peso = almmmatg.pesmat
                /*Reporte.x-empaque = fGen-Empaques(Reporte.CanPed)*/
                ...
                .
            /* 06/05/2024: C.Tenazoa ORIGEN: RIQRA HORIZONTAL */
            FIND ORDEN WHERE ORDEN.codcia = s-codcia AND
                ORDEN.coddoc = VtaCDocu.CodRef AND
                ORDEN.nroped = VtaCDocu.NroRef
                NO-LOCK NO-ERROR.
            IF AVAILABLE ORDEN THEN DO:
                ASSIGN
                    Reporte.CodOrigen = ORDEN.CodOrigen
                    Reporte.NroOrigen = ORDEN.NroOrigen.
            END.

            /* RHC 27/02/2020 Control de INCIDENCIA */
            FIND Faccpedi WHERE Faccpedi.codcia = s-codcia AND
                Faccpedi.coddoc = Vtacdocu.codref AND
                Faccpedi.nroped = Vtacdocu.nroref NO-LOCK NO-ERROR.
            IF AVAILABLE Faccpedi THEN IF Faccpedi.TpoPed = "INC" THEN Reporte.Mensaje = "INCIDENCIA".
            /* ************************************ */
            FIND Almtubic WHERE Almtubic.codcia = s-codcia
                AND Almtubic.codubi = Reporte.codubi
                AND Almtubic.codalm = Reporte.codalm
...
DEF INPUT PARAM pNroPed   AS CHAR NO-UNDO.
DEF INPUT PARAM pCodUbi   AS CHAR NO-UNDO.
DEF INPUT PARAM pCodRef   AS CHAR NO-UNDO.
DEF INPUT PARAM pNroRef   AS CHAR NO-UNDO.
...
IF TRUE <> (x-NroRef > '') THEN x-CodRef = 'TODOS'.
...
CASE TRUE:
    ...
    OTHERWISE DO:
        FOR EACH Vtacdocu NO-LOCK WHERE {&Condicion}:
            ...
            END.
            ELSE DO:
                IF Vtacdocu.FlgImpOD <> YES OR Vtacdocu.FlgImpOD = ? THEN NEXT.
            END.
            IF pNroPed > '' AND VtaCDocu.NroPed <> pNroPed THEN NEXT.
            IF pCodUbi > '' AND VtaCDocu.ZonaPickeo <> pCodUbi THEN NEXT.

            IF x-CodRef <> 'TODOS' THEN DO:
                IF NOT (Vtacdocu.codref = x-CodRef AND Vtacdocu.nroref = x-NroRef) THEN NEXT.
            END.
            CREATE T-CDOCU.
            BUFFER-COPY Vtacdocu TO T-CDOCU.
        END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setDobleUbicacion B-table-Win 
PROCEDURE setDobleUbicacion :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE INPUT PARAMETER pSetDobleUbicacion AS INT.

x-que-ubicacion = pSetDobleUbicacion.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ue-anular-w-report B-table-Win 
PROCEDURE ue-anular-w-report :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
lSeqSector = 0.
s-Task-No = 0.
FOR EACH Reporte NO-LOCK BREAK BY reporte.nroped BY Reporte.codubi:
    ...
    ASSIGN
        ...
        w-report.Llave-I = (IF Reporte.Agencia = YES THEN 1 ELSE 0)
        .
        IF x-que-ubicacion > 0 THEN DO:
            ASSIGN w-report.campo-c[21] = Reporte.Tipo + IF(x-que-ubicacion = 1) THEN " UBIC.PRINCIPAL " ELSE " 2da UBICACION ".
        END.
    /* Cabecera */
    IF NUM-ENTRIES(x-DestinoFinal, '|') > 1
        THEN x-DestinoFinal = ENTRY(2,x-DestinoFinal,'|').
    ASSIGN  
    ...
    IF Vtacdocu.CodRef = "OTR" THEN DO:
        ASSIGN
            w-report.campo-c[8] = "ORIGEN : " + Vtacdocu.codalm.
        FIND Almacen WHERE Almacen.codcia = s-codcia AND Almacen.codalm = Vtacdocu.codalm NO-LOCK NO-ERROR.
        IF AVAILABLE Almacen THEN w-report.campo-c[8] = w-report.campo-c[8] + " - " + Almacen.Descripcion.
    END.
    ASSIGN
        ...
        w-report.campo-c[16] = x-DestinoFinal
        w-report.campo-c[25] = VtaCDocu.CodOri + " " + VtaCDocu.NroOri.
    FIND DI-RutaC WHERE DI-RutaC.CodCia = Vtacdocu.codcia
        AND DI-RutaC.CodDiv = Vtacdocu.coddiv
        AND DI-RutaC.CodDoc = VtaCDocu.CodOri
        AND DI-RutaC.NroDoc = VtaCDocu.NroOri
        NO-LOCK NO-ERROR.
    IF AVAILABLE DI-RutaC THEN w-report.campo-c[25] = w-report.campo-c[25] + " " + DI-RutaC.Observ.
    
    ASSIGN 
        w-report.campo-l[1] = x-CrossDocking.
    /* Detalle */
    ASSIGN  
    ...
    ASSIGN
        w-report.Campo-C[30] = Reporte.Mensaje.
    /* Direccion de entrega */
    FIND Faccpedi WHERE Faccpedi.codcia = s-codcia AND
        Faccpedi.coddoc = Reporte.codref AND                /* O/D OTR */
        Faccpedi.nroped = Reporte.nroref NO-LOCK NO-ERROR.
    IF AVAILABLE Faccpedi THEN DO:
        ...
        IF AVAILABLE gn-clied THEN DO:
            w-report.campo-c[17] = Gn-ClieD.DirCli .
            IF NUM-ENTRIES(Gn-ClieD.DirCli, '|') > 1 THEN
                w-report.campo-c[17] = ENTRY(2,Gn-ClieD.DirCli,'|').
        END.
        /* 11/09/2020 Nro TOPAZ */
        /* Solo para MI BANCO y BCP */
        IF LOOKUP(Faccpedi.CodCli, '20382036655,20100047218') > 0 THEN DO:
            ...
            IF AVAILABLE PEDIDO THEN DO:
                ...
                IF AVAILABLE COTIZACION THEN DO:
                    IF COTIZACION.CodCli = '20382036655' THEN w-report.campo-c[29] = COTIZACION.OfficeCustomer.
                    IF COTIZACION.CodCli = '20100047218' THEN w-report.campo-c[29] = COTIZACION.InvoiceCustomerGroup.
                END.
            END.
        END.
    END.
    /* EMbalado Especial */
    ASSIGN
        w-report.campo-L[2] = Reporte.EmpaqEspec.
    /* 06/05/2024: C.Tenazoa RIQRA HORIZONTAL */
    ASSIGN
        w-report.Campo-C[28] = TRIM(Reporte.CodOrigen) + " " + TRIM(Reporte.NroOrigen).
END.
...
------------------------------------------------------------------------------*/
    ...
    IF lSaldo > 0 AND almmmatg.canemp > 0 THEN DO:
        ...
        IF lValor > 0 THEN DO:
            ...
            x-master = "M(" + STRING(lValor) + "/" + STRING(almmmatg.canemp) + " )".
        END.
        lSaldo = lSaldo - (lValor * almmmatg.canemp).
        x-qmaster = x-qmaster + (lValor * almmmatg.canemp).
    END.
    IF lSaldo > 0 AND almmmatg.StkRep > 0 THEN DO:
        ...
        IF lValor > 0 THEN DO:
            ...
            x-inner = "I(" + STRING(lValor) + "/" + STRING(almmmatg.StkRep) + ")".
        END.
        lSaldo = lSaldo - (lValor * almmmatg.StkRep).
        x-qinner = x-qinner + (lValor * almmmatg.StkRep).
    END.
    IF lSaldo > 0 THEN DO:
        lRetVal = lRetVal + IF(lRetVal = "") THEN "" ELSE " ".
        lRetVal = lRetVal + "U(" + STRING(lSaldo) + ")".
        x-unidades = "U(" + STRING(lSaldo) + ")".
        x-qunidades = x-qunidades + lSaldo.
    END.
