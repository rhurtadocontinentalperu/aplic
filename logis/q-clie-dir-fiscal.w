...
&Scoped-define QUERY-STRING-Query-Main FOR EACH Gn-ClieD OF gn-clie WHERE ~{&KEY-PHRASE} ~
      AND Gn-ClieD.Sede = "@@@" NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Gn-ClieD OF gn-clie WHERE ~{&KEY-PHRASE} ~
      AND Gn-ClieD.Sede = "@@@" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main Gn-ClieD
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main Gn-ClieD
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "Gn-ClieD.Sede = ""@@@"""
     _Design-Parent    is WINDOW q-tables @ ( 1.15 , 10.86 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Crea-Registro q-tables 
...
------------------------------------------------------------------------------*/
   ...
   DEF BUFFER B-CLIED FOR gn-clied.
   IF NOT CAN-FIND(FIRST B-CLIED WHERE B-CLIED.CodCia = gn-clie.CodCia AND
                   B-CLIED.CodCli = B-CLIED.CodCli AND
                   B-CLIED.Sede = "@@@" NO-LOCK)
       THEN DO:
       /* RHC 31/10/18 Se crea un refgistro en gn-clied automaticamente */
       CREATE B-CLIED.
       BUFFER-COPY gn-clie TO B-CLIED
           ASSIGN 
           B-CLIED.SwSedeSunat = "M"    /* MANUAL */
           B-CLIED.DomFiscal = YES
           B-CLIED.Sede = "@@@".   /* Valor que no se puede anular */
       RELEASE B-CLIED.
   END.
