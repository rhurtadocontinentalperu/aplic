...
/* ************************************************************************************ */
/* RHC: Definimos la forma de trabajo 
    Sintaxis de pParametro:
        FER: Solo para EVENTOS, capturamos los PCO's 
*/    
/* ************************************************************************************ */
DEF INPUT PARAMETER pParametro AS CHAR.
...
DEFINE VARIABLE h_v-pedido-credito AS HANDLE NO-UNDO.
...
DO:
   RUN Asigna-datos IN h_v-pedido-credito.
END.
...
DO:
  ...
  RUN Excel IN h_v-pedido-credito.
...
DO:
  RUN Margen IN h_v-pedido-credito.
END.
...
DO:
  RUN Despachar-Pedido IN h_v-pedido-credito.
END.
...
DO:
  ...
  RETURN NO-APPLY.
  RUN Redondeo IN h_v-pedido-credito.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'logis/v-pedido-credito.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-pedido-credito ).
       RUN set-position IN h_v-pedido-credito ( 2.35 , 1.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-pedido-credito. */
       RUN add-link IN adm-broker-hdl ( h_p-updv04 , 'TableIO':U , h_v-pedido-credito ).
       RUN add-link IN adm-broker-hdl ( h_q-pedido , 'Record':U , h_v-pedido-credito ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-pedido-credito ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_v-pedido-credito , 'AFTER':U ).
...
CASE L-Handle:
 ...
 WHEN "Disable-Head" THEN DO:
     ...
     RUN dispatch IN h_v-pedido-credito ('disable-fields').
   END.
 WHEN "Enable-Head" THEN DO:
     RUN dispatch IN h_p-updv04 ('enable':U).
     RUN dispatch IN h_v-pedido-credito ('enable-fields').
   END.
END CASE.
