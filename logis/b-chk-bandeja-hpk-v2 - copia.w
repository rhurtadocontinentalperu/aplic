...
logisdchequeo.CanPed logisdchequeo.Etiqueta logisdchequeo.FechaHora 
...
logisdchequeo.Factor logisdchequeo.CanPed logisdchequeo.Etiqueta 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      logisdchequeo.CanChk COLUMN-LABEL "Cantidad" FORMAT ">,>>>,>>9.9999":U
            WIDTH 8.57
      logisdchequeo.Factor FORMAT ">>,>>9.9999":U WIDTH 5.14
...
DEFINE FRAME F-Main
     FILL-IN-Bultos AT ROW 1.23 COL 101 COLON-ALIGNED NO-LABEL WIDGET-ID 30
     TOGGLE-Multiple AT ROW 1.27 COL 5 WIDGET-ID 28
     BUTTON-CerrarBulto AT ROW 1.27 COL 39 WIDGET-ID 8
     BUTTON-imprimir AT ROW 1.27 COL 53 WIDGET-ID 10
     BUTTON-Extorna-CerrarBulto AT ROW 1.27 COL 68 WIDGET-ID 36
     BUTTON-Renumera AT ROW 1.27 COL 125 WIDGET-ID 34
     br_table AT ROW 2.62 COL 1
     "BULTOS:" VIEW-AS TEXT
          SIZE 15 BY 1.08 AT ROW 1.27 COL 88 WIDGET-ID 32
...
/* BROWSE-TAB br_table BUTTON-Renumera F-Main */
ASSIGN 
...
"logisdchequeo.CanChk" "Cantidad" ? "decimal" ? ? ? ? ? ? yes ? no no "8.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.logisdchequeo.Factor
"logisdchequeo.Factor" ? ? "decimal" ? ? ? ? ? ? yes ? no no "5.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"logisdchequeo.Etiqueta" "Número de Etiqueta" ? "character" ? ? ? ? ? ? yes ? no no "21.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    /* RHC 01/03/2021 por ahora o validar para OTR */
    FIND b-vtacdocu WHERE b-vtacdocu.codcia = s-codcia
        ...
        NO-LOCK NO-ERROR.
    IF b-vtacdocu.codref = "O/D" AND Almmmatg.RequiereSerialNr = "no" THEN DO:
        SELF:SCREEN-VALUE = ''.
    END.
END.
...
DO:  
    ...
    FOR EACH b-Detalle NO-LOCK WHERE b-Detalle.CodCia = s-CodCia
        ...
        END.
    END.
    RUN logis/d-chequeo-de-ordenes-rotulos-v2(INPUT x-CodDoc, 
                                              INPUT x-NroDoc, 
                                              INPUT y-bultos, 
...
------------------------------------------------------------------------------*/
  ...
  FIND b-vtacdocu WHERE b-vtacdocu.codcia = s-codcia
      ...
      AND b-vtacdocu.nroped = x-nrodoc
      NO-LOCK NO-ERROR.
  IF b-vtacdocu.codref = "O/D" AND Almmmatg.RequiereSerialNr = "si" 
      THEN DO:
      ...
      FIND FIRST fifommate WHERE fifommate.CodCia = s-CodCia
          ...
          AND fifommate.SerialNumber = logisdchequeo.SerialNumber:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
          NO-LOCK NO-ERROR.
