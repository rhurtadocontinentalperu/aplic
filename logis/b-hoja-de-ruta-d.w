...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.TotalPrecioVenta COLUMN-LABEL "Importe" FORMAT "->>,>>>,>>9.99":U
  ENABLE
      DI-RutaD.CodRef
      DI-RutaD.NroRef
/* _UIB-CODE-BLOCK-END */
...
"IF CcbCDocu.CodMon = 1 THEN 'S/.' ELSE 'US$' @ x-moneda" "Mon." "x(3)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > INTEGRAL.CcbCDocu.TotalPrecioVenta
"CcbCDocu.TotalPrecioVenta" "Importe" "->>,>>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO iCounter = 1 TO hBrowse:NUM-COLUMNS:
    ...
    IF LOOKUP(hColumn:NAME, 'CodRef,NroRef') > 0 THEN hColumn:READ-ONLY = TRUE.
END.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  ENABLE btnAddOD txtOrdDesp WITH FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  DISABLE btnAddOD txtOrdDesp WITH FRAME {&FRAME-NAME}.
...
IF AVAILABLE faccpedi THEN DO:
    ...
    END.
    ELSE DO:
        MESSAGE "O/D no esta Rotulado o ya fue cargada a una H/R o No existe".
        RETURN "ADM-ERROR". 
    END.
END.
ELSE DO:
