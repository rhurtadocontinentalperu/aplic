...
&Scoped-define QUERY-STRING-br_table FOR EACH DI-RutaC WHERE ~{&KEY-PHRASE} ~
 ...
 AND (TRUE <> (DI-RutaC.FlgCie > '')) NO-LOCK ~
    BY DI-RutaC.Nomtra ~
...
DO:
    IF AVAILABLE Di-RutaC THEN DO:
        RLOOP:
        DO ON ERROR UNDO, RETURN NO-APPLY ON STOP UNDO, RETURN NO-APPLY:
...
/* ******************************************************************************* */
/* 1ro. Guias de Remisión por Ventas */
/* ******************************************************************************* */
RUN HR_Verifica-RutaD IN hProc (INPUT ROWID(DI-RutaC),   /* La H/R */
                                INPUT YES).              /* Mostrar el error */
IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN.
/* ******************************************************************************* */
/* 2do. Guias de Transferencia */
/* ******************************************************************************* */
RUN HR_Verifica-RutaG IN hProc (INPUT ROWID(DI-RutaC),   /* La H/R */
                                INPUT YES).              /* Mostrar el error */
IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN.
/* ******************************************************************************* */
/* 3ro. Guias Itinerantes */
/* ******************************************************************************* */
RUN HR_Verifica-RutaDG IN hProc (INPUT ROWID(DI-RutaC),   /* La H/R */
                                 INPUT YES).              /* Mostrar el error */
IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN.
...
/* CIERRE DE ACUERDO AL ALMACEN */
RUN logis/d-precierre-hr-gen.w (DI-RutaC.NroDoc, s-FlgEst).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Cierre-Hoja-Old B-table-Win 
PROCEDURE Cierre-Hoja-Old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
