...
DEFINE TEMP-TABLE tt-seguimiento NO-UNDO
        ...
        FIELD  tt-SitPedido AS CHAR                 FORMAT 'x(30)'                      LABEL 'Estado de la Orden'
        ...
        /* */
        FIELDS tt-situacion AS CHAR                 LABEL "Situacion Pedido" FORMAT 'x(50)'
        /* */
        FIELD tt-motivo     AS CHAR                 LABEL 'Motivo no entregado' FORMAT 'x(60)'
        FIELD tt-reprogram  AS CHAR                 LABEL 'Situacion despacho'  FORMAT 'x(20)'
    INDEX Llave01 AS PRIMARY tt-codped tt-nroped
    .
...
txtHasta ChkbxGenTxt RADIO-SET-tipofile ChkbxResumen Fi-Mensaje 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fTipo W-Win 
...
DEFINE FRAME F-Main
     ...
     Fi-Mensaje AT ROW 10.15 COL 9 COLON-ALIGNED NO-LABEL WIDGET-ID 44
     "v3" VIEW-AS TEXT
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: cot_faccpedi B "?" ? INTEGRAL FacCPedi
   END-TABLES.
 */
...
/* SETTINGS FOR FILL-IN Fi-Mensaje IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR RADIO-SET RADIO-SET-tipofile IN FRAME F-Main
   NO-ENABLE                                                            */
...
ON END-ERROR OF W-Win /* RESUMEN SEGUIMIENTO DE PED, R/A Y TRA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* RESUMEN SEGUIMIENTO DE PED, R/A Y TRA */
DO:
...
DO:
  ...
  IF iDias > 31 AND NOT (s-user-id = 'ADMIN' OR s-user-id = 'rohc-00') THEN DO:
      MESSAGE 'Maximo de rango de fechas en 31 días' VIEW-AS ALERT-BOX WARNING.
      RETURN NO-APPLY.
  END.
  IF iDias > 31 AND (s-user-id = 'ADMIN' OR s-user-id = 'rohc-00') THEN DO:
      MESSAGE 'ADMIN, ten cuidado: Rango de fechas supera 31 días' VIEW-AS ALERT-BOX INFORMATION.
  END.
...
------------------------------------------------------------------------------*/
  DISPLAY txtCoddivi txtDesDivi txtFile txtDesde txtHasta ChkbxGenTxt 
          RADIO-SET-tipofile ChkbxResumen Fi-Mensaje 
...
------------------------------------------------------------------------------*/
    ...
    END.
    ELSE DO:
        SYSTEM-DIALOG GET-DIR cArchivo RETURN-TO-START-DIR TITLE 'Directorio Files'.
    ...
    IF radio-set-tipofile = 2 THEN DO:
        FI-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "PROCESANDO EXCEL".
    ...
    END.
    ELSE DO:
        FI-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "PROCESANDO TEXTO".
    ...
    /* ******************************************************* */
    END.
    FI-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
CASE p-Fase:
    WHEN "PEDIDO" THEN DO:
        ...
        FIND FIRST almtabla WHERE almtabla.tabla = 'CP' AND 
            ...
            NO-LOCK NO-ERROR.
        IF AVAILABLE almtabla THEN ASSIGN tt-seguimiento.tt-despos = almtabla.nombre.
        ...
        FIND FIRST cot_faccpedi WHERE cot_faccpedi.codcia = s-codcia AND 
            ...
            NO-LOCK NO-ERROR.
        IF AVAILABLE cot_faccpedi THEN ASSIGN tt-seguimiento.tt-lprecio = cot_faccpedi.libre_c01.

        lSituacion = "EMISION PEDIDO".

        /* RHC 18/08/18 Ingresó a Créditos para su aprobación */
        FIND LAST vtadtrkped USE-INDEX Indice02 WHERE vtadtrkped.CodCia = s-CodCia AND 
    ...
    WHEN "ORDEN" THEN DO:
        ...
        /* 09/04/2026: Harold Segura, situación de Pedidos */
        FOR EACH LogTrkDocs NO-LOCK WHERE LogTrkDocs.CodCia = s-codcia AND
            LogTrkDocs.CodDoc = od_faccpedi.coddoc AND
            LogTrkDocs.NroDoc = od_faccpedi.nroped AND
            ...
            tt-SitPedido = TabTrkDocs.NomCorto.
            LEAVE.
        END.
    ...
    WHEN "DOCUMENTO" THEN DO:
        /* Generacion del Comprobante */
        ASSIGN 
            tt-seguimiento.tt-cdoc  = ccbcdocu.coddoc
        ...
        ASSIGN  
            tt-seguimiento.tt-lugent    = IF NOT (TRUE <> (ccbcdocu.lugent > "") ) THEN ccbcdocu.lugent ELSE tt-seguimiento.tt-lugent
        ...
        FOR EACH Almcmov NO-LOCK WHERE Almcmov.codcia = s-codcia AND
                ...
                Almcmov.flgest <> 'A':
            IF almcmov.tipmov = 'I' AND almcmov.codmov = 09 THEN DO:
                ASSIGN 
    ...
    WHEN "HRUTA" THEN DO:
        ...
        ASSIGN 
            tt-seguimiento.tt-hruta    = di-rutac.nrodoc
    ...
    WHEN "VIGISALIDA" THEN DO:
        ...
        FIND FIRST TraIngSal WHERE TraIngSal.codcia = 1 AND 
            TraIngSal.coddiv = tt-seguimiento.tt-divori AND
            TraIngSal.placa = tt-seguimiento.tt-placa AND 
            TraIngSal.nrodoc = tt-seguimiento.tt-hruta NO-LOCK NO-ERROR.
...
FOR EACH almcrepo NO-LOCK WHERE almcrepo.codcia = s-codcia AND 
    ...
    /* Destino */
    IF lProv > '' THEN DO:
        IF lProv = 'LIMA' THEN lDestino = lDist.
        ELSE DO:
            IF lProv = 'CALLAO' THEN lDestino = "CALLAO".
            ELSE lDestino = "PROVINCIAS".
        END.
    END.
    lTieneODs = NO.
...
/*     FI-Mensaje = "REPOSICIONES: Tipo " + Almcrepo.tipmov + " " + STRING(almcrepo.nroser,"999") + " " +  STRING(almcrepo.nrodoc,"999999"). */
/*     DISPLAY FI-Mensaje WITH FRAME F-Proceso.                                                                                              */


    /* *************************************************************************************** */
    /* SIEMPRE GRABAMOS LA CABECERA ASÍ NO HAYA TENIDO MOVIMIENTOS */
    /* *************************************************************************************** */
    ...
    FOR EACH od_faccpedi NO-LOCK WHERE od_faccpedi.codcia = almcrepo.codcia AND 
        ...
        FI-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 
            "REPOSICIONES: Tipo " + Almcrepo.tipmov + " " + STRING(almcrepo.fchdoc,'99/99/9999') + " " + STRING(almcrepo.nroser,"999") + " " +  STRING(almcrepo.nrodoc,"999999") + " " +
        ...
        FOR EACH almcmov NO-LOCK WHERE almcmov.codcia = s-codcia AND 
            ...
            FOR EACH di-RutaG NO-LOCK WHERE di-RutaG.codcia = s-codcia AND 
                ...
                IF AVAILABLE gn-vehic THEN lCodPro     = gn-vehic.codpro.
...
END.
/* 31/03/2026: Se limitan los estados  */
cFlags = "G,X,W,WX,WC,P,F,R,C,O".
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Se genera un registro por cada G/R
------------------------------------------------------------------------------*/
...
FOR EACH x-gn-divi NO-LOCK WHERE x-gn-divi.codcia = s-codcia AND 
    ...
    FOR EACH faccpedi NO-LOCK USE-INDEX Llave08 WHERE faccpedi.codcia = s-codcia AND 
        ...
        END.    
        FI-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 
            "[VENTAS] DIVISION: " + Faccpedi.divdes + " " + STRING(Faccpedi.fchped,'99/99/9999') + " " + Faccpedi.coddoc + " " + Faccpedi.nroped .
        /* */
        lDcli = "".
        lDpto = "".
        lProv = "".
        ...
        FIND FIRST tabdepto WHERE tabdepto.coddepto = gn-clie.coddept NO-LOCK NO-ERROR NO-WAIT.
        IF AVAILABLE tabdepto THEN lDpto = tabdepto.nomdepto.
        ...
        IF lProv > '' THEN DO:
            ...
            END.
        END.
        lTieneODs = NO.     /* Control de O/Ds por cada PED */
        /* BARREMOS TODAS LAS O/Ds RELACIONADAS, INCLUSIVE LAS QUE HAN SIDO FRACCIONADAS */
        FOR EACH od_faccpedi NO-LOCK WHERE od_faccpedi.codcia = faccpedi.codcia AND 
            ...
            IF ChkbxResumen = NO THEN DO:
                ...
                lqPeso = od_faccpedi.peso.
                lqVolumen = od_faccpedi.volumen.
                IF lqItems = 0 OR lqPeso = 0 THEN DO:
            ...
            FOR EACH ccbcdocu NO-LOCK WHERE ccbcdocu.codcia = s-codcia AND 
                ...
                ccbcdocu.libre_c02 = od_faccpedi.nroped AND
                ccbcdocu.flgest <> 'A':
                cDoctos = "FAI".    /* Solo FAIs */
                LEAVE.
            END.
            /* Buscar los documentos (FAC,BOL) emitidos por cada O/D */
            lTieneGRs = NO.     /* Control de G/R por cada O/D */
            FOR EACH ccbcdocu NO-LOCK WHERE ccbcdocu.codcia = s-codcia AND 
                    ccbcdocu.codped = faccpedi.coddoc AND
                    ccbcdocu.nroped = faccpedi.nroped AND
                    ccbcdocu.flgest <> 'A':
                IF NOT (LOOKUP(ccbcdocu.coddoc,cDoctos) > 0 AND 
                        ccbcdocu.libre_c01 = od_faccpedi.coddoc AND
                        ccbcdocu.libre_c02 = od_faccpedi.nroped
                        )
                    THEN NEXT.
                ...
                FOR EACH gr_ccbcdocu NO-LOCK WHERE gr_ccbcdocu.codcia = s-codcia AND
                        ...
                        gr_ccbcdocu.coddoc = 'G/R' AND
                        gr_ccbcdocu.flgest <> 'A':
                    /* */
                    lTieneGRs = YES.
                    lTieneHRs = NO.     /* Control de H/R por cada G/R */
                    /* ----- */
                    RUN ue-ventas-hoja-de-ruta(INPUT gr_ccbcdocu.coddoc, INPUT gr_ccbcdocu.nrodoc, OUTPUT lTieneHRs).
                    /* La G/R No tiene HR */
                    IF lTieneHRs = NO THEN DO:
                        /* Adiciono registro por cada G/R */
                        CREATE tt-seguimiento.
                        RUN ue-grabar-mov(INPUT "PEDIDO").
                        RUN ue-grabar-mov(INPUT "ORDEN").
                        RUN ue-grabar-mov(INPUT "DOCUMENTO").
                    ...
                    END.
                END.
                /* El comprobante FAC/BOL/TCK no tiene G/R */
                IF lTieneGRs = NO THEN DO:
                    /**/
                    lqItems = 0.
                    lqPeso = 0.
...
FI-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "PROCESANDO VENTAS".
RUN ue-ventas.
HIDE FRAME F-Proceso.

IF TRUE <> (txtFile > "") THEN DO:
    FI-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "PROCESANDO ORDENES".
    RUN ue-ordenes-transferencias.
END.

FI-Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "LIMPIANDO TEXTO".
RUN Limpiar-Texto.
lTiempoHasta = NOW.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fTipo W-Win 
FUNCTION fTipo RETURNS CHARACTER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
