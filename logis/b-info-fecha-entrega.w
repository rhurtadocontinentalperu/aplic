...
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 45 BY 9.69
         FONT 4
         TITLE "FECHA DE ENTREGA" FIT-LAST-COLUMN.
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* FECHA DE ENTREGA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* FECHA DE ENTREGA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* FECHA DE ENTREGA */
DO:
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Vtacdocu NO-LOCK WHERE VtaCDocu.CodCia = s-codcia
      ...
      FIND t-report WHERE Campo-D[1] = VtaCDocu.FchEnt NO-ERROR.
      IF NOT AVAILABLE t-report THEN CREATE t-report.
      ASSIGN
          Campo-D[1] = VtaCDocu.FchEnt.
      ASSIGN
  ...
  FOR EACH t-report EXCLUSIVE-LOCK,
      ...
      AND VtaCDocu.CodPed = 'HPK'
      AND Vtacdocu.FlgEst = 'P'
      AND VtaCDocu.FchEnt = t-report.Campo-D[1]
