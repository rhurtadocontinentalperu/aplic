...
(IF VtaCDocu.FlgSit = "TX" THEN "FALTANTE" ELSE "COMPLETADO") @ VtaCDocu.Libre_c05 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-2 W-Win _STRUCTURED
  QUERY BROWSE-2 NO-LOCK DISPLAY
      ...
      (IF NUM-ENTRIES(Vtacdocu.Libre_c03, '|') > 1 THEN DATETIME(ENTRY(2, Vtacdocu.Libre_c03, '|')) ELSE ?) @ x-FchImpOD COLUMN-LABEL "Fecha y hora de Pickeo" FORMAT "99/99/9999 HH:MM":U
            WIDTH 29.86
      (IF VtaCDocu.FlgSit = "TX" THEN "FALTANTE" ELSE "COMPLETADO") @ VtaCDocu.Libre_c05 COLUMN-LABEL "SITUACION" FORMAT "x(10)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-2 BUTTON-Refrescar F-Main */
...
"(IF VtaCDocu.FlgSit = ""TX"" THEN ""FALTANTE"" ELSE ""COMPLETADO"") @ VtaCDocu.Libre_c05" "SITUACION" "x(10)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
&ANALYZE-RESUME
...
DO:
    CASE Vtacdocu.FlgSit:
        WHEN "TX" THEN vtacdocu.libre_c05:BGCOLOR IN BROWSE {&BROWSE-NAME} = 12.
        OTHERWISE vtacdocu.libre_c05:BGCOLOR IN BROWSE {&BROWSE-NAME} = 2.
    END CASE.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
      /* 01/03/2022: Pregunta si todos los artículos están en su ubicación */
      RUN logis/d-cierre-hpk-ubicacion (OUTPUT pOk).
      IF pOk = ? THEN RETURN.
      EMPTY TEMP-TABLE t-Vtaddocu.
      IF pOk = NO THEN DO:
          /* Solicita los productos a marcar */
          RUN logis/d-cierre-hpk-noubicados (INPUT x-Rowid, OUTPUT pOk, OUTPUT TABLE t-Vtaddocu).
      ...
      END.
      /* ********************************************************************************* */
      /* 03/03/3020 Si existe código no encontrados en su ubicación => Control de tracking */
      /* ********************************************************************************* */
      FIND FIRST t-Vtaddocu NO-LOCK NO-ERROR.
      ...
      /* Log de Artículo NO Ubicados en su lugar */
      FOR EACH t-Vtaddocu NO-LOCK:
          CREATE LogHpkNoUbic.
          ASSIGN
              LogHpkNoUbic.CodCia = s-CodCia
              LogHpkNoUbic.CodDiv = s-CodDiv
              LogHpkNoUbic.CodPed = Vtacdocu.codped
              LogHpkNoUbic.NroPed = Vtacdocu.nroped
              LogHpkNoUbic.CodRef = Vtacdocu.codref
              LogHpkNoUbic.NroRef = Vtacdocu.nroref
              LogHpkNoUbic.AlmDes = t-Vtaddocu.almdes
              LogHpkNoUbic.CanPed = t-Vtaddocu.canped
              LogHpkNoUbic.CodMat = t-Vtaddocu.codmat
              LogHpkNoUbic.Factor = t-Vtaddocu.factor
              LogHpkNoUbic.NroItm = t-Vtaddocu.nroitm
              LogHpkNoUbic.UndVta = t-Vtaddocu.undvta
              LogHpkNoUbic.Picador = VtaCDocu.UsrSac
              LogHpkNoUbic.NomPicador = fPicador(VtaCDocu.UsrSac)
              LogHpkNoUbic.Fecha = TODAY
              LogHpkNoUbic.Hora = STRING(TIME, 'HH:MM:SS')
              LogHpkNoUbic.Usuario = s-user-id.
          FIND Almmmate WHERE Almmmate.codcia = s-codcia
              AND Almmmate.codalm = t-Vtaddocu.almdes
              AND Almmmate.codmat = t-Vtaddocu.codmat
              NO-LOCK NO-ERROR.
          IF AVAILABLE Almmmate THEN DO:
              FIND FIRST Almtubic WHERE almtubic.CodCia = Almmmate.codcia
                  AND almtubic.CodAlm = Almmmate.codalm
                  AND almtubic.CodUbi = Almmmate.codubi
                  NO-LOCK NO-ERROR.
              IF AVAILABLE almtubic THEN 
                  ASSIGN
                  LogHpkNoUbic.CodUbi  = almtubic.CodUbi
                  LogHpkNoUbic.CodZona = almtubic.CodZona.
          END.
          RELEASE LogHpkNoUbic.
      END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RADIO-SET-CodDoc FILL-IN-NroPed BUTTON-Refrescar BROWSE-2 
      WITH FRAME F-Main IN WINDOW W-Win.
