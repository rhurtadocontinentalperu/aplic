...
PL-PERS.matper PL-PERS.nomper T-FLG-MES.vcontr 
...
DEFINE BUTTON BUTTON-2 
     LABEL "ACTUALIZAR FECHA DE CESE" 
     SIZE 25 BY 1.35.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-3 wWin _STRUCTURED
  QUERY BROWSE-3 NO-LOCK DISPLAY
      ...
      PL-PERS.nomper FORMAT "X(20)":U WIDTH 32
      T-FLG-MES.vcontr COLUMN-LABEL "Fecha de Cese" FORMAT "99/99/9999":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-7 wWin _STRUCTURED
  QUERY BROWSE-7 NO-LOCK DISPLAY
      ...
      T-FLG-MES-2.FchIniCont COLUMN-LABEL "Fecha de Cese" FORMAT "99/99/9999":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 21 BY 3.35
         FONT 4
...
DEFINE FRAME fMain
     ...
     BtnDone AT ROW 1.27 COL 47 WIDGET-ID 98
...
"PL-PERS.nomper" ? "X(20)" "character" ? ? ? ? ? ? no ? no no "32" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.T-FLG-MES.vcontr
"T-FLG-MES.vcontr" "Fecha de Cese" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-3 */
&ANALYZE-RESUME
...
"T-FLG-MES-2.FchIniCont" "Fecha de Cese" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-7 */
&ANALYZE-RESUME
...
ON END-ERROR OF wWin /* CARGA DE CESE */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* CARGA DE CESE */
DO:
...
ON CHOOSE OF BUTTON-2 IN FRAME fMain /* ACTUALIZAR FECHA DE CESE */
DO:
   MESSAGE 'Procedemos a actualizar las fechas de cese'
...
FOR EACH T-FLG-MES TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    ASSIGN
        PL-FLG-MES.VContr = T-FLG-MES.VContr
        PL-FLG-MES.SitAct = "Inactivo"
        PL-FLG-MES.CnPago = "CHEQUE".
    DELETE T-FLG-MES.
END.
