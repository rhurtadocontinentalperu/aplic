...
PL-PERS.matper PL-PERS.nomper T-FLG-MES.CTS T-FLG-MES.NroDpt-CTS 
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-9 T-FLG-MES-2.codper T-FLG-MES-2.CTS ~
T-FLG-MES-2.NroDpt-CTS 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-9 
&Scoped-define QUERY-STRING-BROWSE-9 FOR EACH T-FLG-MES-2 NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-9 OPEN QUERY BROWSE-9 FOR EACH T-FLG-MES-2 NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-9 T-FLG-MES-2
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-9 T-FLG-MES-2
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-3 wWin _STRUCTURED
  QUERY BROWSE-3 NO-LOCK DISPLAY
      ...
      T-FLG-MES.CTS FORMAT "x(30)":U
      T-FLG-MES.NroDpt-CTS FORMAT "X(20)":U WIDTH 10.86
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 117 BY 15
         FONT 4 FIT-LAST-COLUMN.

DEFINE BROWSE BROWSE-9
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-9 wWin _STRUCTURED
  QUERY BROWSE-9 NO-LOCK DISPLAY
      T-FLG-MES-2.codper FORMAT "X(6)":U WIDTH 8.43
      T-FLG-MES-2.CTS COLUMN-LABEL "AHORROS" FORMAT "x(40)":U WIDTH 19.43
      T-FLG-MES-2.NroDpt-CTS FORMAT "X(20)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 56 BY 3.46
         FONT 4
...
DEFINE FRAME fMain
     ...
     BROWSE-3 AT ROW 2.73 COL 2 WIDGET-ID 200
     BROWSE-9 AT ROW 18.69 COL 4 WIDGET-ID 300
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "CARGA DE CUENTAS DE AHORROS"
         HEIGHT             = 21.69
         WIDTH              = 120.14
         MAX-HEIGHT         = 28.81
         MAX-WIDTH          = 146.14
...
/* BROWSE-TAB BROWSE-9 BROWSE-3 fMain */
...
"PL-PERS.nomper" ? "X(20)" "character" ? ? ? ? ? ? no ? no no "21.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.T-FLG-MES.CTS
"T-FLG-MES.CTS" ? "x(30)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.T-FLG-MES.NroDpt-CTS
"T-FLG-MES.NroDpt-CTS" ? ? "character" ? ? ? ? ? ? no ? no no "10.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-3 */
&ANALYZE-RESUME
...
"codper" ? ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.T-FLG-MES-2.CTS
"CTS" "AHORROS" ? "character" ? ? ? ? ? ? no ? no no "19.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = Temp-Tables.T-FLG-MES-2.NroDpt-CTS
     _Query            is OPENED
*/  /* BROWSE BROWSE-9 */
&ANALYZE-RESUME
...
ON END-ERROR OF wWin /* CARGA DE CUENTAS DE AHORROS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* CARGA DE CUENTAS DE AHORROS */
DO:
...
ON CHOOSE OF BUTTON-2 IN FRAME fMain /* ACEPTAR AHORROS */
DO:
   MESSAGE 'Procedemos a actualizar los vencimientos de contrato?'
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Aceptar-Ahorros wWin 
PROCEDURE Aceptar-Ahorros :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH T-FLG-MES TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    ASSIGN
        PL-FLG-MES.CnPago = T-FLG-MES.CnPago
        PL-FLG-MES.NroDpt = T-FLG-MES.NroDpt.
    DELETE T-FLG-MES.
END.
...
------------------------------------------------------------------------------*/
  ENABLE RECT-1 BUTTON-Excel BUTTON-2 BtnDone BROWSE-3 BROWSE-9 
...
SYSTEM-DIALOG GET-FILE FILL-IN-file
    ...
    TITLE "IMPORTAR CUENTAS DE AHORROS"
    MUST-EXIST
    USE-FILENAME
    UPDATE OKpressed.
...
REPEAT:
    ...
    cValue = chWorkSheet:Range(cRange):VALUE.
    ASSIGN
        T-FLG-MES.CnPago = cValue.

    /* NUMERO */
    cRange = "C" + TRIM(STRING(iCountLine)).
    cValue = chWorkSheet:Range(cRange):VALUE.
    ASSIGN
        T-FLG-MES.NroDpt = cValue.
END.
...
FOR EACH T-FLG-MES:
    /* AHORRROS */
    FIND pl-pago WHERE PL-PAGO.cnpago = T-FLG-MES.CnPago NO-LOCK NO-ERROR.
    IF NOT AVAILABLE pl-pago THEN DELETE T-FLG-MES.
END.
