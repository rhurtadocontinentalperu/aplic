...
DEF VAR ANOS-TRAB AS INT.
DEF VAR MESES-TRAB AS INT.
DEF VAR DIAS-TRAB AS INT.
DEF VAR FECHA-INGRESO AS DATE.
DEF VAR FECHA-CESE AS DATE.
DEF VAR FECHA-CONTROL AS DATE.

FOR EACH PL-FLG-MES WHERE /*codper = '000731' AND*/
    ...
    /* RHC 14.06.2011 CONTROL DE VACACIONES */
    FIND pl-ctrvac WHERE pl-ctrvac.codcia = pl-flg-mes.codcia
        AND pl-ctrvac.codper = pl-flg-mes.codper
        AND pl-ctrvac.periodo = pl-flg-mes.periodo
        AND pl-ctrvac.nromes = pl-flg-mes.nromes
        EXCLUSIVE-LOCK NO-ERROR.
    IF AVAILABLE pl-ctrvac THEN DO:
        x-Dias = pl-ctrvac.dias.
        DELETE pl-ctrvac.
        /* Extorno */
        IF pl-ctrvac.tipo = 'S' THEN DO:
            FIND LAST pl-ctrvac WHERE pl-ctrvac.codcia = pl-flg-mes.codcia
                AND pl-ctrvac.codper = pl-flg-mes.codper
                AND pl-ctrvac.tipo = 'I'
                EXCLUSIVE-LOCK NO-ERROR.
            REPEAT WHILE AVAILABLE pl-ctrvac:
                x-Saldo = pl-ctrvac.dias - pl-ctrvac.saldo.
                pl-ctrvac.saldo = pl-ctrvac.saldo + MINIMUM(x-Saldo,x-Dias).
                x-Dias = x-Dias - MINIMUM(x-Saldo,x-Dias).
                IF x-Dias <= 0 THEN LEAVE.
                FIND PREV pl-ctrvac WHERE pl-ctrvac.codcia = pl-flg-mes.codcia
                    AND pl-ctrvac.codper = pl-flg-mes.codper
                    AND pl-ctrvac.tipo = 'I'
                    EXCLUSIVE-LOCK NO-ERROR.
            END.
        END.
    END.
    /* buscamos vacaciones concepto 023 */
    FIND pl-mov-mes WHERE INTEGRAL.PL-MOV-MES.CodCia = s-codcia
        AND INTEGRAL.PL-MOV-MES.codper = pl-flg-mes.codper
        AND INTEGRAL.PL-MOV-MES.Periodo = pl-flg-mes.periodo
        AND INTEGRAL.PL-MOV-MES.NroMes = pl-flg-mes.nromes
        AND INTEGRAL.PL-MOV-MES.CodMov = 023
        AND INTEGRAL.PL-MOV-MES.codcal = 000    /*001*/
        AND INTEGRAL.PL-MOV-MES.codpln = 01
        NO-LOCK NO-ERROR.
    IF AVAILABLE INTEGRAL.PL-MOV-MES THEN DO:
        CREATE pl-ctrvac.
        ASSIGN
            INTEGRAL.pl-ctrvac.CodCia = pl-flg-mes.codcia
            INTEGRAL.pl-ctrvac.codper = pl-flg-mes.codper
            INTEGRAL.pl-ctrvac.Fecha = DATETIME(TODAY, MTIME)
            INTEGRAL.pl-ctrvac.NroMes = pl-flg-mes.nromes
            INTEGRAL.pl-ctrvac.Periodo = pl-flg-mes.periodo
            INTEGRAL.pl-ctrvac.Usuario = s-user-id
            INTEGRAL.pl-ctrvac.Desde = PL-FLG-MES.inivac
            INTEGRAL.pl-ctrvac.Dias = INTEGRAL.PL-MOV-MES.valcal-mes
            INTEGRAL.pl-ctrvac.Hasta = PL-FLG-MES.inivac + INTEGRAL.PL-MOV-MES.valcal-mes - 1
            INTEGRAL.pl-ctrvac.Tipo = 'S'.
        /* actualizamos saldos */
        x-Dias = INTEGRAL.pl-ctrvac.Dias.
        FIND LAST pl-ctrvac WHERE pl-ctrvac.codcia = pl-flg-mes.codcia
            AND pl-ctrvac.codper = pl-flg-mes.codper
            AND pl-ctrvac.tipo = 'I'
            EXCLUSIVE-LOCK NO-ERROR.
        REPEAT WHILE AVAILABLE pl-ctrvac:
            x-Saldo = pl-ctrvac.saldo.
            pl-ctrvac.saldo = pl-ctrvac.saldo - MINIMUM(x-Saldo,x-Dias).
            x-Dias = x-Dias - MINIMUM(x-Saldo,x-Dias).
            IF x-Dias <= 0 THEN LEAVE.
            FIND PREV pl-ctrvac WHERE pl-ctrvac.codcia = pl-flg-mes.codcia
    ...
    ELSE DO:
        ...
        RUN src/bin/_dateif (pl-flg-mes.nromes, pl-flg-mes.periodo,
                             OUTPUT FECHA-INGRESO, OUTPUT FECHA-CESE).

        ASSIGN
            ANOS-TRAB = 0
            MESES-TRAB = 0
            DIAS-TRAB = 0
            FECHA-INGRESO = INTEGRAL.PL-FLG-MES.fecing.

        IF FECHA-INGRESO <> ? THEN RUN PLN/P-TSERV02 (FECHA-INGRESO - 1, FECHA-CESE,
                                                      OUTPUT ANOS-TRAB, OUTPUT MESES-TRAB, OUTPUT DIAS-TRAB).
        ELSE DO:
            MESSAGE 'El personal con código' PL-FLG-MES.CodPer SKIP
                'NO tiene registrada su fecha de inicio de labores'
                VIEW-AS ALERT-BOX WARNING.
        END.
        /*MESSAGE ANOS-TRAB FECHA-INGRESO FECHA-CESE.*/
        IF ANOS-TRAB >= 1 THEN DO:
            REPEAT:
                FECHA-CONTROL = FECHA-INGRESO.
                FECHA-INGRESO = DATE(MONTH(FECHA-INGRESO), DAY(FECHA-INGRESO), YEAR(FECHA-INGRESO) + 1).
                IF FECHA-INGRESO > FECHA-CESE THEN LEAVE.
            END.
            /* buscamos si ya existe un control anterior */
            FIND LAST pl-ctrvac WHERE pl-ctrvac.codcia = pl-flg-mes.codcia
                AND pl-ctrvac.codper = pl-flg-mes.codper
                AND pl-ctrvac.tipo = 'I'
                AND pl-ctrvac.periodo = YEAR(FECHA-CONTROL)
                AND pl-ctrvac.nromes = MONTH(FECHA-CONTROL)
                NO-LOCK NO-ERROR.
            IF NOT AVAILABLE pl-ctrvac THEN DO:
                CREATE pl-ctrvac.
                ASSIGN
                    INTEGRAL.pl-ctrvac.CodCia = pl-flg-mes.codcia
                    INTEGRAL.pl-ctrvac.codper = pl-flg-mes.codper
                    INTEGRAL.pl-ctrvac.Fecha = DATETIME(TODAY, MTIME)
                    INTEGRAL.pl-ctrvac.NroMes = MONTH(FECHA-CONTROL)
                    INTEGRAL.pl-ctrvac.Periodo = YEAR(FECHA-CONTROL)
                    INTEGRAL.pl-ctrvac.Usuario = s-user-id
                    INTEGRAL.pl-ctrvac.Dias = 30
                    INTEGRAL.pl-ctrvac.Saldo = 30
                    INTEGRAL.pl-ctrvac.Tipo = 'I'
                    INTEGRAL.pl-ctrvac.Desde = DATE (MONTH (FECHA-CONTROL), DAY (FECHA-CONTROL), YEAR (FECHA-CONTROL) - 1)
                    INTEGRAL.pl-ctrvac.Hasta = FECHA-CONTROL.
            END.
        END.
    END.
