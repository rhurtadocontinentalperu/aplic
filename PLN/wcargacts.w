...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-9 wWin _STRUCTURED
  QUERY BROWSE-9 NO-LOCK DISPLAY
      ...
      T-FLG-MES-2.CTS FORMAT "x(40)":U WIDTH 19.43
...
"T-FLG-MES-2.codper" ? ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.T-FLG-MES-2.CTS
"T-FLG-MES-2.CTS" ? ? "character" ? ? ? ? ? ? no ? no no "19.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON END-ERROR OF wWin /* CARGA DE CTS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* CARGA DE CTS */
DO:
...
ON CHOOSE OF BUTTON-2 IN FRAME fMain /* ACEPTAR CTS */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Aceptar-CTS wWin 
PROCEDURE Aceptar-CTS :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH T-FLG-MES TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    ASSIGN
        PL-FLG-MES.CTS = T-FLG-MES.CTS
        PL-FLG-MES.NroDpt-CTS = T-FLG-MES.NroDpt-CTS.
    DELETE T-FLG-MES.
END.
...
REPEAT:
    ...
    cValue = chWorkSheet:Range(cRange):VALUE.
    ASSIGN
        T-FLG-MES.CTS = cValue.

    /* NUMERO */
    cRange = "C" + TRIM(STRING(iCountLine)).
    cValue = chWorkSheet:Range(cRange):VALUE.
    ASSIGN
        T-FLG-MES.NroDpt-CTS = cValue.
END.
...
FOR EACH T-FLG-MES:
    /* CTS */
    FIND pl-cts WHERE pl-cts.cts = T-FLG-MES.CTS NO-LOCK NO-ERROR.
    IF NOT AVAILABLE pl-cts THEN DELETE T-FLG-MES.
END.
