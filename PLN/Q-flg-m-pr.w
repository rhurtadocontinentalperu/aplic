...
/*DEFINE VAR X-SECCION AS CHAR INIT "0010-FABRICA".*/
DEFINE VAR X-SECCION AS CHAR INIT "FABRICA".
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH integral.PL-FLG-MES ~
 ...
 AND PL-FLG-MES.Seccion = X-Seccion NO-LOCK, ~
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH integral.PL-FLG-MES ~
      WHERE PL-FLG-MES.CodCia = s-CodCia ~
 AND PL-FLG-MES.Periodo = s-Periodo ~
 AND PL-FLG-MES.NroMes = s-NroMes ~
 AND PL-FLG-MES.codpln = PL-PLAN.CodPln ~
    ...
    BY integral.PL-FLG-MES.Proyecto ~
       BY integral.PL-FLG-MES.seccion ~
        BY integral.PL-PERS.patper ~
         BY integral.PL-PERS.matper ~
          BY integral.PL-PERS.nomper.
&Scoped-define TABLES-IN-QUERY-Query-Main integral.PL-FLG-MES ~
integral.PL-PERS
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main integral.PL-FLG-MES
&Scoped-define SECOND-TABLE-IN-QUERY-Query-Main integral.PL-PERS
...
/* Query rebuild information for QUERY Query-Main
 ...
 AND PL-FLG-MES.Seccion = X-Seccion"
     _Design-Parent    is WINDOW q-tables @ ( 1.42 , 4 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
DO:
    ...
    RUN PLN/H-FLG-M-PR.W (PL-PLAN.CodPln, OUTPUT reg-act).
    IF reg-act <> ? THEN DO:
...
DO:
    ...
    ELSE DO:
        FIND PL-FLG-MES WHERE
            ...
            PL-FLG-MES.codpln = PL-PLAN.codpln AND
            PL-FLG-MES.NroMes = s-nromes AND
            PL-FLG-MES.codper = FILL-IN-codper:SCREEN-VALUE AND
            PL-FLG-MES.Seccion = X-Seccion NO-LOCK NO-ERROR.
