...
DEFINE TEMP-TABLE tt-datos
    ...
    FIELD  t-vcontr LIKE pl-flg-mes.vcont
    .
...
------------------------------------------------------------------------------*/
    ...
    /* 15/01/2025 Alex Quiroz datos adicionales */
    cRange = "BN" + cColumn.
    chWorkSheet:Range(cRange):Value = "FECHA DE CESE".
    cRange = "BO" + cColumn.
    chWorkSheet:Range(cRange):Value = "MINUTOS DE TARDANZA".
    cRange = "BP" + cColumn.
    chWorkSheet:Range(cRange):Value = "DIAS DE INASISTENCIA".
    chWorkSheet:Columns("BN"):NumberFormat = "dd/MM/yyyy".
    chWorkSheet:Columns("BO"):NumberFormat = "@".
    chWorkSheet:Columns("BP"):NumberFormat = "@".
    ...
    FOR EACH pl-mov-mes WHERE pl-mov-mes.codcia = s-codcia
        ...
        IF NOT AVAIL tt-datos THEN DO:
            FIND FIRST pl-flg-mes WHERE pl-flg-mes.codcia = s-codcia
                AND pl-flg-mes.codper = TRIM(pl-pers.codper)
    ...
    FOR EACH TT-DATOS  NO-LOCK BREAK BY t-ccosto BY T-CODPER:
        ...
        /* 15/01/2025 Alex Quiroz, datos adicionales */
        cRange = "BN" + cColumn.
        chWorkSheet:Range(cRange):Value = t-vcont.
        FIND FIRST B-MOV-MES WHERE B-MOV-MES.codcia = s-codcia
                    AND B-MOV-MES.codper = t-codper
                    AND B-MOV-MES.periodo = x-periodo
                    AND B-MOV-MES.nromes  = cb-mes
                    AND B-MOV-MES.codpln  = 01
                    AND B-MOV-MES.codcal  = 000
                    AND B-MOV-MES.codmov = 507 NO-LOCK NO-ERROR.
        IF AVAILABLE B-MOV-MES THEN DO:
            cRange = "BO" + cColumn.
            chWorkSheet:Range(cRange):Value = B-MOV-MES.valcal-mes.
        END.
        cRange = "BP" + cColumn.
        chWorkSheet:Range(cRange):VALUE = t-totvvv[37].

        DISPLAY t-codper + "-" + t-nombre @ x-mensaje WITH FRAME {&FRAME-NAME}.        
        PAUSE 0.
    END.
