...
&Scoped-define QUERY-STRING-Query-Main FOR EACH INTEGRAL.PL-CALC OF INTEGRAL.PL-PLAN ~
      WHERE PL-CALC.codcal = 6 NO-LOCK
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH INTEGRAL.PL-CALC OF INTEGRAL.PL-PLAN ~
      WHERE PL-CALC.codcal = 6 NO-LOCK.
&Scoped-define TABLES-IN-QUERY-Query-Main INTEGRAL.PL-CALC
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main INTEGRAL.PL-CALC
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.PL-CALC OF INTEGRAL.PL-PLAN"
     _Options          = "NO-LOCK"
     _Where[1]         = "PL-CALC.codcal = 6"
     _Design-Parent    is WINDOW q-tables @ ( 1 , 1 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Reposition-Calc q-tables 
PROCEDURE Reposition-Calc :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAMETER pCodCal LIKE INTEGRAL.PL-CALC.codcal.

DEF BUFFER B-PL-CALC FOR INTEGRAL.PL-CALC.

FIND FIRST B-PL-CALC WHERE B-PL-CALC.codpln = INTEGRAL.PL-PLAN.codpln
    AND B-PL-CALC.codcal = pCodCal NO-LOCK NO-ERROR.
IF AVAILABLE B-PL-CALC THEN DO:
    REPOSITION {&QUERY-NAME} TO ROWID ROWID(B-PL-CALC) NO-ERROR.
