...
DEFINE STREAM lLinea .

lListaTest = "000175,000665,000894,000951,000987".
...
DEFINE FRAME F-Main
     ...
     B-aceptar AT ROW 6.58 COL 6.57
...
DO:
    ...
    /* Si no es BBVA lista de prueba blanquear */
    IF COMBO-canal <> 'BBVA' THEN lListaTest = "".
...
IF NOT CAN-FIND(FIRST PL-MOV-MES WHERE
    ...
    PL-MOV-MES.CodPer  = PL-FLG-MES.CodPer) THEN RETURN. /* Si no tiene calculo */
...
FOR EACH PL-MOV-MES WHERE
    ...
    IF PL-MOV-MES.ValCal-Mes <> 0 AND
        (lListaTest <> "" OR TRIM(PL-FLG-Mes.cnpago) = COMBO-canal) AND
...
/*           "   0" + '|'.                     */
    CASE pl-pers.TpoDocId:
        ...
        OTHERWISE X = X + "DNI".
    END CASE.
    X = X + string(PL-PERS.NroDocId,"x(9)") + "   0" + '|'.
    x = REPLACE ( x, '|', CHR(9) ).
    PUT x SKIP.
  end.  
end.  
...
CASE COMBO-canal:
    WHEN "BIF" THEN RUN procesa_bif.
    WHEN "BANCO DE CREDITO" THEN RUN procesa_bcp.
    WHEN "BBVA" THEN RUN procesa_bbva.
END CASE.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH pl-pago NO-LOCK WHERE LOOKUP(pl-pago.cnpago, "CHEQUE,EFECTIVO") = 0:
      COMBO-canal:ADD-LAST(pl-pago.cnpago) IN FRAME {&FRAME-NAME}.
  END.
...
lCTaCargo   = "00110130010000000455".        /* 20 Digitos */
...
for each tempo:
    ...
    find pl-flg-mes where pl-flg-mes.codcia  = tempo.codcia  
        ...
        and pl-flg-mes.nromes  = tempo.nromes  
        and pl-flg-mes.codper  = tempo.codper  no-lock no-error.
    if available pl-flg-mes and (pl-flg-mes.cnpago = "EFECTIVO" OR pl-flg-mes.cnpago = "") THEN NEXT .
    if available pl-flg-mes and nrodpt <> "" then do: 
        x-in = 0.
        x-in = (tempo.valcal-mes  ) * 100.
        x-tot = x-tot + x-in.
