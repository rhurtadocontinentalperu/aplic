...
COMBO-BOX-NroMes-2 FILL-IN-CCosto BUTTON-1 BUTTON-2 BUTTON-3 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-Periodo COMBO-BOX-NroMes ~
COMBO-BOX-NroMes-2 FILL-IN-CCosto FILL-IN-NomAux 
...
DEFINE VARIABLE COMBO-BOX-NroMes-2 AS INTEGER FORMAT "99":U INITIAL 12 
     LABEL "Hasta" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "01","02","03","04","05","06","07","08","09","10","11","12" 
     DROP-DOWN-LIST
...
DEFINE FRAME F-Main
     ...
     COMBO-BOX-NroMes-2 AT ROW 3.54 COL 19 COLON-ALIGNED WIDGET-ID 2
     FILL-IN-CCosto AT ROW 4.62 COL 19 COLON-ALIGNED
     FILL-IN-NomAux AT ROW 4.62 COL 27 COLON-ALIGNED NO-LABEL
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "DETALLADO CONSOLIDADO DE PLANILLAS POR CENTRO DE COSTO"
         HEIGHT             = 7.62
         WIDTH              = 71.86
         MAX-HEIGHT         = 7.62
         MAX-WIDTH          = 71.86
...
ON END-ERROR OF W-Win /* DETALLADO CONSOLIDADO DE PLANILLAS POR CENTRO DE COSTO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* DETALLADO CONSOLIDADO DE PLANILLAS POR CENTRO DE COSTO */
DO:
...
DO:
  ASSIGN COMBO-BOX-Periodo FILL-IN-CCosto COMBO-BOX-NroMes COMBO-BOX-NroMes-2.
  IF FILL-IN-CCosto <> ''
  THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH PL-MOV-MES NO-LOCK WHERE codcia = s-codcia
        ...
        AND nromes  >= COMBO-BOX-NroMes
        AND nromes  <= COMBO-BOX-NroMes-2        
        AND codpln  = 01,
        FIRST PL-FLG-MES OF PL-MOV-MES NO-LOCK WHERE PL-FLG-MES.ccosto BEGINS FILL-IN-CCosto,
    ...
    FIND w-report WHERE w-report.task-no = s-task-no 
        ...
        AND w-report.Campo-c[1] = PL-FLG-MES.codper
    ...
    ASSIGN
        ...
        w-report.Campo-c[1] = PL-FLG-MES.codper
        w-report.Campo-c[2] = TRIM(PL-PERS.patper) + ' ' + TRIM(PL-PERS.matper) + ', ' + TRIM(PL-PERS.nomper).
        w-report.Campo-c[3] = PL-FLG-MES.cargos.
...
------------------------------------------------------------------------------*/
  DISPLAY COMBO-BOX-Periodo COMBO-BOX-NroMes COMBO-BOX-NroMes-2 FILL-IN-CCosto 
          FILL-IN-NomAux 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE COMBO-BOX-Periodo COMBO-BOX-NroMes COMBO-BOX-NroMes-2 FILL-IN-CCosto 
...
------------------------------------------------------------------------------*/
    ...
    THEN DO:
        ...
        RETURN.
    END.
    s-Titulo = 'MES DE ' + TRIM(ENTRY(COMBO-BOX-NroMes, x-Meses)) + ' DE ' +
    ...
    cRange = "B" + cColumn.
    chWorkSheet:Range(cRange):Value = "DETALLE CONSOLIDADO DE PLANILLAS POR CENTRO DE COSTO EMPLEADOS".
    iCount = iCount + 1.
    cColumn = STRING(iCount).
    cRange = "B" + cColumn.
    ...
    cRange = "D" + cColumn.
    chWorkSheet:Range(cRange):Value = "101 y 103".
    cRange = "E" + cColumn.
    chWorkSheet:Range(cRange):Value = "125,126,127".
    cRange = "F" + cColumn.
    chWorkSheet:Range(cRange):Value = "209".
    cRange = "G" + cColumn.
    ...
    FOR EACH w-report WHERE w-report.task-no = s-task-no BREAK BY w-report.task-no BY w-report.llave-c BY w-report.llave-i:      
      IF FIRST-OF(w-report.llave-c) THEN DO:
          FIND cb-auxi WHERE cb-auxi.codcia = cb-codcia
              AND cb-auxi.clfaux = 'CCO'
              AND cb-auxi.codaux = w-report.llave-c NO-LOCK NO-ERROR.
          IF NOT AVAILABLE cb-auxi THEN FIND cb-auxi WHERE cb-auxi.codcia = s-codcia
              AND cb-auxi.clfaux = 'CCO'
              AND cb-auxi.codaux = w-report.llave-c NO-LOCK NO-ERROR.
          IF AVAILABLE cb-auxi THEN x-Descrip = cb-auxi.nomaux.
          ELSE x-Descrip = ''.
          iCount = iCount + 1.
          cColumn = STRING(iCount).
          cRange = "B" + cColumn.
      ...
      /*RD01-Calculando Concepto Otros*/
      ASSIGN w-report.campo-f[14] = w-report.campo-f[1] - (w-report.campo-f[11] + w-report.campo-f[12] + 
                                    w-report.campo-f[13] + w-report.campo-f[15]).
      ...
      ACCUMULATE w-report.campo-f[14] (TOTAL BY w-report.task-no BY w-report.llave-c).
      ...
      cRange = "F" + cColumn.
      chWorkSheet:Range(cRange):Value = w-report.campo-f[13].
      cRange = "G" + cColumn.
      chWorkSheet:Range(cRange):Value = w-report.campo-f[14].
      cRange = "H" + cColumn.
      ...
      IF LAST-OF(w-report.llave-c) THEN DO:
          ...
          cRange = "F" + cColumn.
          chWorkSheet:Range(cRange):Value = ACCUM TOTAL BY w-report.llave-c w-report.campo-f[13] .
          cRange = "G" + cColumn.
          chWorkSheet:Range(cRange):Value = ACCUM TOTAL BY w-report.llave-c w-report.campo-f[14] .
          cRange = "H" + cColumn.
      ...
      IF LAST-OF(w-report.task-no) THEN DO:
          ...
          cRange = "F" + cColumn.
          chWorkSheet:Range(cRange):Value = ACCUM TOTAL BY w-report.task-no w-report.campo-f[13] .
          cRange = "G" + cColumn.
          chWorkSheet:Range(cRange):Value = ACCUM TOTAL BY w-report.task-no w-report.campo-f[14] .
          cRange = "H" + cColumn.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-REPORTE
    w-report.campo-c[1] FORMAT 'x(6)'
    w-report.campo-c[2] FORMAT 'x(35)'
    w-report.campo-c[3] FORMAT 'x(20)'

    w-report.campo-f[11] FORMAT '>>>>,>>9.99' 
    w-report.campo-f[12] FORMAT '>>>>,>>9.99' 
    w-report.campo-f[13] FORMAT '>>>>,>>9.99' 
    w-report.campo-f[14] FORMAT '>>>>,>>9.99' 
    
    w-report.campo-f[1] FORMAT '>>>>,>>9.99' 
    w-report.campo-f[15] FORMAT '>>>>,>>9.99' 
...
/*RD01-Cambio de formato de Cabecera******** 
...
/*RD01- Cabecera detalla sueldo*/
  DEFINE FRAME F-HEADER       
    HEADER
        ...
        "Hora  : " AT 80 STRING(TIME,"HH:MM:SS") SKIP
        s-Titulo FORMAT 'x(50)' SKIP
       "                                                                                  125                                                         Total       Total                          " SKIP
       "                                                                      101         126          209         Otros      Total       301         Total   Vacaciones   Gratific.      Total  " SKIP
       "Codigo Apellidos y Nombres                       Cargos               103         127                                Sueldo                     CTS      Truncas     Truncas    General  " SKIP
       "-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" SKIP
/*
...
*/
  ...
  FOR EACH w-report WHERE w-report.task-no = s-task-no BREAK BY w-report.task-no BY w-report.llave-c BY w-report.llave-i:
    ...
    /*RD01-Calculando Concepto Otros*/
    ASSIGN w-report.campo-f[14] = w-report.campo-f[1] - (w-report.campo-f[11] + w-report.campo-f[12] + w-report.campo-f[13]).
    ...
    THEN DO:
        ...
        DISPLAY STREAM REPORT
            ...
            ACCUM TOTAL BY w-report.llave-c w-report.campo-f[14] @ w-report.campo-f[14]            
