...
/* 03/03/2026: Datos necesarios para SAP */
DEF VAR cSwCargaSunat AS CHAR NO-UNDO.
DEF VAR cDirCli AS CHAR NO-UNDO.
DEF VAR cCodDept AS CHAR NO-UNDO.
DEF VAR cCodProv AS CHAR NO-UNDO.
DEF VAR cCodDist AS CHAR NO-UNDO.
DEF VAR dLibre_F01 AS DATE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  IF RETURN-VALUE = "YES" THEN DO:
     ...
     IF s-CodDiv = "00024" THEN DO:
         ...
         RELEASE LogTabla.
     END.
     /* 03/03/2026: Información necesaria para SAP */
     ASSIGN
         gn-clie.SwCargaSunat = cSwCargaSunat     /* SUNAT */
         gn-clie.DirCli = cDirCli
         gn-clie.CodDept = cCodDept
         gn-clie.CodProv = cCodProv
         gn-clie.CodDist = cCodDist
         gn-clie.Libre_F01 = dLibre_F01
         .
...
END.
  ...
  /* 03/03/2026: Datos obligatorios para SAP */
  /* Información de SUNAT */
  /* Verificamos Información SUNAT */
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      IF RETURN-VALUE = 'YES' THEN DO:
          ...
          IF gn-clie.Ruc:SCREEN-VALUE > "" THEN DO:
              ...
              IF TRUE <> (pError > "") THEN DO:
                  ASSIGN
                      cSwCargaSunat = "S"     /* SUNAT */
                      cDirCli = pAddress
                      cCodDept = SUBSTRING(pUbigeo,1,2)
                      cCodProv = SUBSTRING(pUbigeo,3,2)
                      cCodDist = SUBSTRING(pUbigeo,5,2)
                      dLibre_F01 = pDateInscription.
              END.
          END.
          RUN SAP/d-cliente-ubigeo.w (INPUT-OUTPUT cDirCli,
                                      INPUT-OUTPUT cCodDept,
                                      INPUT-OUTPUT cCodProv,
                                      INPUT-OUTPUT cCodDist,
                                      OUTPUT pError).
          IF pError > '' THEN DO:
