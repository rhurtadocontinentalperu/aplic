...
&IF DEFINED(EXCLUDE-api_eecc_pendientes) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE api_eecc_pendientes Procedure 
PROCEDURE api_eecc_pendientes :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* Sintaxis
http://192.168.0.232:5500/continental/sap/ventas/cliente/estadodecuenta
?listaclientes=20100038146%2C10081846770
*/
/* Formato de Retorno:
    {
        "ClienteCodigo": "CL20100038146",
        "ClienteRazonSocial": "CONTINENTAL SAC",
        "DocumentoFechaEmision": "2026-06-02",
        "DocumentoFechaVencimiento": "2026-06-02",
        "DocumentoTipo": "FACTURA",
        "DocumentoNumero": "F253-00030000",
        "DocumentoMoneda": "SOL",
        "DocumentoTotal": 28.49,
        "DocumentoSaldo": 4.33,
        "DocumentoDivision": "TIENDA UCAYALI",
        "DocumentoCondicionPago": "CONTADO",
        "LetraSituacion": "",
        "LetraBanco": "",
        "LetraCuentaBancaria": "",
        "LetraEstado": "",
        "ObjetoSAPCodigo": "13",
        "DocumentoAsiento": 265,
        "DocumentoClave": 95,
        "coddoc": "",
        "nrodoc": "",
        "codclie": "20100038146"
...
DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    IF xUrl = "" THEN DO:
        ...
        LEAVE.
    END.
    xUrl = xUrl + "?listaclientes=" + pcCodCli.
    /*
    FIND FIRST VtaTabla WHERE VtaTabla.CodCia   = s-CodCia
           AND VtaTabla.Tabla    = "CONFIG-WEB-SAT"
           AND VtaTabla.Llave_c1 = "ESTADODECUENTA"
    ...
    */
    ...
    IF INDEX(pResult,"0:No Socket") > 0 THEN DO:
        pcError = "No fue posible conectarse al servicio." + CHR(10)
            + "URL: " + xUrl.
        LEAVE.
    END.
...
&IF DEFINED(EXCLUDE-api_eecc_pendientes_old) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE api_eecc_pendientes_old Procedure 
PROCEDURE api_eecc_pendientes_old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    IF INDEX(pResult,"0:No Socket") > 0 THEN DO:
        pcError =
            "No fue posible conectarse al servicio." + CHR(10)
          + "URL: " + xUrl.
        LEAVE.
    END.

    IF INDEX(pContent,"[]") > 0 THEN
        LEAVE.
    ...
    IF NOT VALID-HANDLE(hJson) THEN DO:
        pcError = "La librería JSON no está inicializada.".
        LEAVE.
    END.

    RUN iniJson IN hJson (INPUT pContent) NO-ERROR.

    IF ERROR-STATUS:ERROR THEN DO:
        pcError = "No fue posible inicializar el parser JSON.".
        LEAVE.
    END.

    raiz = DYNAMIC-FUNCTION(
                "parseVal" IN hJson,
                0,
                "") NO-ERROR.
    ...
    OR raiz <= 0 THEN DO:
        ...
        pcError = "El JSON recibido no tiene un formato válido.".
        LEAVE.
    ...
    TotalItems = DYNAMIC-FUNCTION(
                    "arrayCount" IN hJson,
                    arrayId) NO-ERROR.

    IF ERROR-STATUS:ERROR THEN DO:

        pcError = "No fue posible obtener la cantidad de elementos del JSON.".
        LEAVE.
    ...
    DO iRegistro = 1 TO TotalItems:
        ...
        DO ON ERROR UNDO, NEXT:
            ...
            itemId = DYNAMIC-FUNCTION(
                        "getArrayItem" IN hJson,
                        arrayId,
                        iRegistro) NO-ERROR.
            ...
            cCodCli    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"codclie").
            cFchDoc    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"DocumentoFechaEmision").
            cFchVto    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"DocumentoFechaVencimiento").
            cCodDoc    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"DocumentoTipo").
            cNroDoc    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"DocumentoNumero").
            cMoneda    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"DocumentoMoneda").
            cDivision  = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"DocumentoDivision").
            cFmaPgo    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"DocumentoCondicionPago").
            cLetSit    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"LetraSituacion").
            cLetBco    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"LetraBanco").
            cLetCta    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"LetraCuentaBancaria").
            cLetStatus = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"LetraEstado").
            cCodigo    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"coddoc").
            cNumero    = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"nrodoc").

            iCodMon = IF CAPS(cMoneda) = "SOL" THEN 1 ELSE 2.

            fImpTot = 0.
            cValor = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"DocumentoTotal").
            IF TRIM(cValor) > "" THEN
                fImpTot = DECIMAL(cValor) NO-ERROR.

            IF ERROR-STATUS:ERROR THEN
                fImpTot = 0.

            fSdoAct = 0.
            cValor = DYNAMIC-FUNCTION("getVal" IN hJson,itemId,"DocumentoSaldo").
            IF TRIM(cValor) > "" THEN
                fSdoAct = DECIMAL(cValor) NO-ERROR.
...
DO ON ERROR UNDO, LEAVE ON STOP  UNDO, LEAVE:
    ...
    IF ERROR-STATUS:ERROR THEN DO:
        ...
        LEAVE.
    END.
    IF NOT (pcAgrupados = YES AND TRIM(pcRelacionados) > "") THEN pcRelacionados = pcCodCli.

    /*--------------------------------------------------------------*/
    /* Invoca API EECC                                              */
    /*--------------------------------------------------------------*/
    RUN api_eecc_pendientes
    (
    ...
    NO-ERROR.
    IF ERROR-STATUS:ERROR THEN DO:
        pcError = "Error ejecutando api_eecc_pendientes." + CHR(10) + RETURN-VALUE.
        DO iError = 1 TO ERROR-STATUS:NUM-MESSAGES:
            pcError = pcError + CHR(10) + ERROR-STATUS:GET-MESSAGE(iError).
        END.
        LEAVE.
    ...
    DO iLinea = 1 TO NUM-ENTRIES(pcReturn,","):
        ...
        IF iCampos < 16 THEN DO:
            pcError = pcError
                    ...
                    + STRING(iLinea)
                    + " descartado. "
                    + "Se esperaban 16 columnas y llegaron "
                    + STRING(iCampos).
            NEXT.
        END.
        /*----------------------------------------------------------*/
        /* A partir de aquí comienza la carga segura del temporal   */
        /*----------------------------------------------------------*/
        DO ON ERROR UNDO,NEXT:
            CREATE btw-report.
            ASSIGN
            ...
            /*------------------------------------------------------*/
            DO iError = 1 TO 16:
                IF TRIM(LC(btw-report.campo-c[iError])) = "null" THEN btw-report.campo-c[iError] = "".
            END.
            /*------------------------------------------------------*/
            /* Fecha Documento                                      */
            /*------------------------------------------------------*/
            ...
            END.
/*             IF btw-report.campo-c[2] > "" AND NUM-ENTRIES(btw-report.campo-c[2],"-") = 3 THEN DO: */
/*                 btw-report.campo-d[1] =                                                           */
/*                     DATE                                                                          */
/*                     (                                                                             */
/*                         INTEGER(ENTRY(2,btw-report.campo-c[2],"-")),                              */
/*                         INTEGER(ENTRY(3,btw-report.campo-c[2],"-")),                              */
/*                         INTEGER(ENTRY(1,btw-report.campo-c[2],"-"))                               */
/*                     )                                                                             */
/*                     NO-ERROR.                                                                     */
/*                 IF ERROR-STATUS:ERROR THEN                                                        */
/*                     ASSIGN                                                                        */
            ...
            IF btw-report.campo-c[3] > "" THEN DO:
                ...
                IF ERROR-STATUS:ERROR THEN
                    ASSIGN
                        ...
                        btw-report.campo-d[1] = ?.
            END.
/*             IF btw-report.campo-c[3] > "" AND NUM-ENTRIES(btw-report.campo-c[3],"-") = 3 THEN DO: */
/*                 btw-report.campo-d[2] =                                                           */
/*                     DATE                                                                          */
/*                     (                                                                             */
/*                         INTEGER(ENTRY(2,btw-report.campo-c[3],"-")),                              */
/*                         INTEGER(ENTRY(3,btw-report.campo-c[3],"-")),                              */
/*                         INTEGER(ENTRY(1,btw-report.campo-c[3],"-"))                               */
/*                     )                                                                             */
/*                     NO-ERROR.                                                                     */
/*                 IF ERROR-STATUS:ERROR THEN                                                        */
/*                     ASSIGN                                                                        */
