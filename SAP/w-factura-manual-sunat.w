...
DEFINE NEW SHARED VARIABLE S-FLGSIT   AS CHAR INIT "YES".       /* Con restricciones */
...
/* ************************************* */
/* 13/06/2023: Observación de Julia Vera */
...
DEFINE VARIABLE h_b-facman AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv05 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-docmto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_t-facman AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-facman AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     C-CodDoc AT ROW 1.31 COL 74 NO-LABEL
     C-NroSer AT ROW 1.31 COL 81.57 NO-LABEL
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_p-navico ( 1.27 , 17.29 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv05 ).
       RUN set-position IN h_p-updv05 ( 1.00 , 19.57 ) NO-ERROR.
       RUN set-size IN h_p-updv05 ( 1.31 , 54.29 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'SAP/v-factura-manual-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-facman ).
       RUN set-position IN h_v-facman ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.46 , 130.29 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'SAP/q-factura-manual-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q-docmto ).
       RUN set-position IN h_q-docmto ( 1.19 , 88.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.04 , 11.00 ) */

       /* Links to SmartViewer h_v-facman. */
       RUN add-link IN adm-broker-hdl ( h_p-updv05 , 'TableIO':U , h_v-facman ).
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_v-facman ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv05 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-facman ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-facman ).
       RUN set-position IN h_b-facman ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-facman ( 12.85 , 137.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-facman. */
       RUN add-link IN adm-broker-hdl ( h_q-docmto , 'Record':U , h_b-facman ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-facman ,
             h_v-facman , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'SAP/t-factura-manual-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-facman ).
       RUN set-position IN h_t-facman ( 9.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_t-facman ( 11.50 , 127.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_t-facman. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_t-facman ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-facman ,
             h_v-facman , 'AFTER':U ).
...
CASE L-Handle:
    WHEN "browse" THEN DO:
          RUN dispatch IN h_b-facman ('open-query':U).
          RUN dispatch IN h_t-facman ('open-query':U).
      END.
    ...
    WHEN "Importe-de-control" THEN DO:
        RETURN 'OK'.
/*         RUN Importe-de-control IN h_v-facman.                  */
