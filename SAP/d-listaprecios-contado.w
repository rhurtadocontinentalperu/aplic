...
COMBO-BOX-CodAlm RADIO-SET-Stock FILL-IN-codigo FILL-IN_Empaque ~
FILL-IN_UndStk FILL-IN_FacEqu 
...
DEFINE VARIABLE FILL-IN_Empaque AS DECIMAL FORMAT "->>,>>9.99":U INITIAL 0 
     LABEL "Empaque" 
     VIEW-AS FILL-IN 
     SIZE 9 BY .81
     BGCOLOR 14 FGCOLOR 0  NO-UNDO.

DEFINE VARIABLE FILL-IN_FacEqu AS DECIMAL FORMAT ">>>,>>>,>>9.99":U INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .81
     BGCOLOR 14 FGCOLOR 0  NO-UNDO.

DEFINE VARIABLE FILL-IN_UndStk AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 7 BY .81
     BGCOLOR 14 FGCOLOR 0  NO-UNDO.
...
DEFINE FRAME D-Dialog
     ...
     FILL-IN_Empaque AT ROW 23.88 COL 15 COLON-ALIGNED WIDGET-ID 28
     FILL-IN_UndStk AT ROW 23.88 COL 24 COLON-ALIGNED NO-LABEL WIDGET-ID 30
     FILL-IN_FacEqu AT ROW 23.88 COL 32 COLON-ALIGNED NO-LABEL WIDGET-ID 32
...
/* SETTINGS FOR FILL-IN FILL-IN-filtro IN FRAME D-Dialog
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN FILL-IN_Empaque IN FRAME D-Dialog
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FILL-IN_FacEqu IN FRAME D-Dialog
   NO-ENABLE                                                            */
...
"t-Almmmatg.StkMax" "Stock!Disponible" "-ZZ,ZZZ,ZZ9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Lista de Precios */
DO:  
...
DO:
  ASSIGN 
      CMB-filtro COMBO-BOX-CodAlm FILL-IN-codigo FILL-IN-filtro f-CodFam
      F-marca RADIO-SET-Stock.
  IF TRUE <> (FILL-IN-codigo > '') AND
      TRUE <> (F-marca > '') AND
      CMB-filtro = 'Todos'  AND
...
FOR EACH t-Almmmatg EXCLUSIVE-LOCK:
    /* NO stock comprometido */
    RUN SAP/stock-comprometido (INPUT t-Almmmatg.CodMat,
                                INPUT COMBO-BOX-CodAlm,
                                INPUT TRUE,
                                OUTPUT pComprometido).
    ASSIGN t-Almmmatg.StkMax = t-Almmmatg.StkMax - pComprometido.
    IF (RADIO-SET-Stock = 'C' AND t-Almmmatg.StkMax <= 0) OR t-Almmmatg.StkMax < 0 THEN DO:
        DELETE t-Almmmatg.
        NEXT.
    END.
    ASSIGN
...
FOR EACH t-Almmmatg EXCLUSIVE-LOCK WHERE t-Almmmatg.Prevta[1] <= 0:
    DELETE t-Almmmatg.
END.
...
&SCOPED-DEFINE CONDICION2 ( Inventory_SAP.WarehouseCode = COMBO-BOX-CodAlm AND ~
Inventory_SAP.ItemCode = Almmmatg.codmat )

&SCOPED-DEFINE FILTRO1 ( Almmmatg.CodCia = S-CODCIA  AND Almmmatg.DesMat BEGINS FILL-IN-filtro )
...
CASE TRUE:
    WHEN FILL-IN-codigo > '' THEN DO:
        FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND Almmmatg.codmat = FILL-IN-codigo:
            ...
            FOR EACH Inventory_SAP NO-LOCK WHERE {&CONDICION2}:
                CREATE t-Almmmatg.
...
------------------------------------------------------------------------------*/
  DISPLAY F-marca f-CodFam CMB-filtro FILL-IN-filtro COMBO-BOX-CodAlm 
          RADIO-SET-Stock FILL-IN-codigo FILL-IN_Empaque FILL-IN_UndStk 
          FILL-IN_FacEqu 
      WITH FRAME D-Dialog.
