...
&Scoped-define INTERNAL-TABLES Almmmatg Almtfami Inventory_SAP
...
Almmmatg.codmat Almmmatg.DesMat Inventory_SAP.Quantity 
...
&Scoped-define QUERY-STRING-br_table FOR EACH Almmmatg WHERE ~{&KEY-PHRASE} ~
      ...
      FIRST Inventory_SAP WHERE Inventory_SAP.ItemCode = Almmmatg.codmat ~
      AND Inventory_SAP.WarehouseCode = x-codalm ~
 AND (RADIO-SET-Stock = "T" OR Inventory_SAP.Quantity > 0) NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almmmatg WHERE ~{&KEY-PHRASE} ~
 ...
 AND (RADIO-SET-Stock = "T" OR Inventory_SAP.Quantity > 0) NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Almmmatg Almtfami Inventory_SAP
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Almmmatg
&Scoped-define SECOND-TABLE-IN-QUERY-br_table Almtfami
&Scoped-define THIRD-TABLE-IN-QUERY-br_table Inventory_SAP
...
DEFINE QUERY br_table FOR 
    ...
    FIELDS(), 
      Inventory_SAP
    FIELDS(Inventory_SAP.Quantity) SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almmmatg.codmat COLUMN-LABEL "Codigo!Articulo" FORMAT "X(6)":U
            WIDTH 7.43
      Almmmatg.DesMat FORMAT "X(80)":U WIDTH 81.43
      Inventory_SAP.Quantity COLUMN-LABEL "Stock actual" FORMAT "->>>,>>>,>>9.99":U
            WIDTH 12.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Almmmatg,INTEGRAL.Almtfami OF INTEGRAL.Almmmatg,INTEGRAL.Inventory_SAP WHERE INTEGRAL.Almmmatg ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST USED, FIRST USED"
     _Where[1]         = "{&Condicion}"
     _Where[2]         = "Almtfami.SwComercial = TRUE"
     _JoinCode[3]      = "Inventory_SAP.ItemCode = Almmmatg.codmat"
     _Where[3]         = "Inventory_SAP.WarehouseCode = x-codalm
 AND (RADIO-SET-Stock = ""T"" OR Inventory_SAP.Quantity > 0)"
...
"Almmmatg.DesMat" ? "X(80)" "character" ? ? ? ? ? ? no ? no no "81.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.Inventory_SAP.Quantity
"Inventory_SAP.Quantity" "Stock actual" ? "decimal" ? ? ? ? ? ? no ? no no "12.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    /* 16/2/2026 Preparamos un query dinámico */
    /* 1. Definición de variables necesarias */
    DEFINE VARIABLE hQuery       AS HANDLE    NO-UNDO.
    DEFINE VARIABLE cBaseQuery   AS CHARACTER NO-UNDO.
    DEFINE VARIABLE cWhereSAP    AS CHARACTER NO-UNDO.

    /* Supongamos que tu browse se llama br_mats. 
    Si ya tiene un query asignado, lo obtenemos; si no, lo creamos. */
    hQuery = BROWSE br_table:QUERY.

    /* 2. Limpiar el query actual si ya está abierto */
    IF hQuery:IS-OPEN THEN hQuery:QUERY-CLOSE().

    /* 3. Construir la base del query (Tablas Almmmatg y Almtfami) */
/*
&SCOPED-DEFINE CONDICION ( Almmmatg.CodCia = S-CODCIA  ~
AND (F-Codfam = "" OR Almmmatg.Codfam = F-Codfam) ~
AND (F-Subfam = "" OR Almmmatg.Subfam = F-Subfam) ~
...
*/
    ...
    cBaseQuery = "FOR EACH Almmmatg WHERE {&KEY-PHRASE} AND ".
    cBaseQuery = cBaseQuery + "( Almmmatg.codcia = " + STRING(s-codcia).
    IF f-CodFam >'' THEN cBaseQuery = cBaseQuery + " AND Almmmatg.codfam = '" + f-CodFam + "'".
    IF f-SubFam > '' THEN cBaseQuery = cBaseQuery + " AND Almmmatg.subfam = '" + f-SubFam + "'".
    IF f-Marca > '' THEN cBaseQuery = cBaseQuery + " AND INDEX(Almmmatg.desmar, '" + f-SubFam + "') > 0".
    cBaseQuery = cBaseQuery + " AND Almmmatg.TpoArt = 'A'".
    cBaseQuery = cBaseQuery + " AND Almmmatg.TpoMrg <> '2'".
    CASE CMB-filtro:
        WHEN "Nombres que inicien con" THEN DO:
            cBaseQuery = cBaseQuery + " AND Almmmatg.DesMat BEGINS '" + FILL-IN-filtro + "'".
        END.
        WHEN "Nombres que contengan" THEN DO:
            cBaseQuery = cBaseQuery + " AND INDEX(Almmmatg.DesMat , '" + FILL-IN-filtro + "' ) > 0".
        END.
    END CASE.
    cBaseQuery = cBaseQuery + " )".
    cBaseQuery = cBaseQuery + ' NO-LOCK, '.
    cBaseQuery = cBaseQuery + "FIRST Almtfami OF Almmmatg WHERE Almtfami.SwComercial = TRUE NO-LOCK, ".


    /* 4. Lógica dinámica según el RADIO-SET */
    IF RADIO-SET-Stock:SCREEN-VALUE = "T" THEN DO:
        /* Si es "T", usamos OUTER-JOIN para no perder el registro de Almmmatg 
        aunque no exista en Inventory_SAP */
        cWhereSAP = "FIRST Inventory_SAP WHERE Inventory_SAP.ItemCode = Almmmatg.codmat " +
            "AND Inventory_SAP.WarehouseCode = " + QUOTER(x-codalm) + " OUTER-JOIN NO-LOCK".
    END.
    ELSE DO:
        /* Si no es "T", forzamos a que tenga stock (Join normal) */
        cWhereSAP = "FIRST Inventory_SAP WHERE Inventory_SAP.ItemCode = Almmmatg.codmat " +
            "AND Inventory_SAP.WarehouseCode = " + QUOTER(x-codalm) + 
            " AND Inventory_SAP.Quantity > 0 NO-LOCK".
    END.

    /* 5. Unir todo y preparar */
    cBaseQuery = cBaseQuery + cWhereSAP + " {&SORTBY-PHRASE}".

    /*MESSAGE cBaseQuery VIEW-AS ALERT-BOX WARNING.*/

    /* Aplicar el query al handle */
...
/*
FOR EACH Almmmatg WHERE ~{&KEY-PHRASE}
      AND {&Condicion} NO-LOCK,
      FIRST Almtfami OF Almmmatg
      WHERE Almtfami.SwComercial = TRUE NO-LOCK,
      FIRST Inventory_SAP WHERE Inventory_SAP.ItemCode = Almmmatg.codmat
...
FOR EACH Almmmatg WHERE ~{&KEY-PHRASE}
      ...
      FIRST Inventory_SAP WHERE Inventory_SAP.ItemCode = Almmmatg.codmat
      AND Inventory_SAP.WarehouseCode = x-codalm
 AND (RADIO-SET-Stock = "T" OR Inventory_SAP.Quantity > 0) OUTER-JOIN NO-LOCK
    ~{&SORTBY-PHRASE}:
*/
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "Inventory_SAP"}
