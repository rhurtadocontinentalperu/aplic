...
DO:
    RUN SAP/d-ped-may-mostr-lista-precios.w.
...
DO:
    ...
    RUN SAP/d-uniabc (ITEM.codmat:SCREEN-VALUE IN BROWSE {&browse-name},
                      s-codalm,
                      s-coddiv,
                      s-codcli,
                      s-tpocmb,
...
DO:
  ...
  RUN SAP/d-ped-may-mostr-codigos.w (INPUT-OUTPUT TABLE ITEM, OUTPUT pOk).
...
FOR EACH B-ITEM:
    ...
    /* 27/04/2026 Susana León */
    pCodAlm = B-ITEM.AlmDes.
END.
...
FOR EACH ITEM WHERE ITEM.Libre_c05 <> "OF",     /* NO promociones */
    ...
    /* 10/03/2025: Corrección del bug, no grababa CONTRATO */
    ASSIGN
        ITEM.Libre_c04 = x-TipDto.
    ASSIGN 
        ITEM.Libre_d02 = f-FleteUnitario. 
...
------------------------------------------------------------------------------*/
  ...
  FIND Inventory_SAP WHERE Inventory_SAP.WarehouseCode = ITEM.AlmDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} AND
...
/*   END.                                                                                                               */
  ...
  /* 27/04/2026: No se puede hacer un pedido con diversos almacenes */
  IF CAN-FIND(FIRST bITEM WHERE bITEM.codmat <> ITEM.codmat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
              AND bITEM.almdes <> ITEM.AlmDes:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} NO-LOCK)
      THEN DO:
      MESSAGE 'NO se puede hacer un pedido de varios almacenes' SKIP 'Solo se permite ventas de un solo almacén'
          VIEW-AS ALERT-BOX WARNING.
      APPLY 'ENTRY' TO ITEM.AlmDes.
      RETURN 'ADM-ERROR'.
  END.
