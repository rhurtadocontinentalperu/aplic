...
&SCOPED-DEFINE Condicion (~
    ...
    ( TRUE <> (x-DesMar > '') OR INDEX(Almmmatg.desmar, x-DesMar) > 0) AND ~
    Almmmatg.TpoArt = "A" ~
    )
...
&Scoped-define INTERNAL-TABLES Almmmatg T-MATG Inventory_SAP
...
Almmmatg.DesMar Almmmatg.UndBas Inventory_SAP.Quantity T-MATG.Libre_d01 
...
&Scoped-define QUERY-STRING-br_table FOR EACH Almmmatg WHERE ~{&KEY-PHRASE} ~
      ...
      EACH Inventory_SAP WHERE Inventory_SAP.ItemCode = Almmmatg.codmat ~
      AND Inventory_SAP.WarehouseCode = x-codalm NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almmmatg WHERE ~{&KEY-PHRASE} ~
      ...
      AND Inventory_SAP.WarehouseCode = x-codalm NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Almmmatg T-MATG Inventory_SAP
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Almmmatg
&Scoped-define SECOND-TABLE-IN-QUERY-br_table T-MATG
...
DEFINE QUERY br_table FOR 
      Almmmatg, 
      T-MATG
    FIELDS(T-MATG.Libre_d01), 
      Inventory_SAP SCROLLING.
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Almmmatg,Temp-Tables.T-MATG OF INTEGRAL.Almmmatg,INTEGRAL.Inventory_SAP WHERE INTEGRAL.Almmmatg ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST OUTER USED,"
     _Where[1]         = "{&Condicion}"
     _JoinCode[3]      = "INTEGRAL.Inventory_SAP.ItemCode = INTEGRAL.Almmmatg.codmat"
     _Where[3]         = "INTEGRAL.Inventory_SAP.WarehouseCode = x-codalm"
...
"INTEGRAL.Almmmatg.DesMat" ? "X(80)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"INTEGRAL.Almmmatg.UndBas" "Unidad" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.Inventory_SAP.Quantity
"INTEGRAL.Inventory_SAP.Quantity" "Stock actual" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.T-MATG.Libre_d01
"Temp-Tables.T-MATG.Libre_d01" "Cantidad" ">>>,>>>,>>9.99" "decimal" 11 0 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
LocalCadena = "".
/* DO LocalOrden = 1 TO NUM-ENTRIES(x-DesMat, " "):                                   */
/*     LocalCadena = LocalCadena + (IF TRUE <> (LocalCadena > '') THEN "" ELSE " ") + */
/*         TRIM(ENTRY(LocalOrden,x-DesMat, " ")) + "*".                               */
/* END.                                                                               */
DO LocalOrden = 1 TO NUM-ENTRIES(x-DesMat, " "):
    LocalCadena = LocalCadena + (IF TRUE <> (LocalCadena > '') THEN "" ELSE " ") + 
        TRIM(ENTRY(LocalOrden,x-DesMat, " ")) + "*".
