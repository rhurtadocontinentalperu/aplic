...
&SCOPED-DEFINE precio-venta-general sap/pri-precio-mayorista-mostrador.p
...
DEF SHARED VAR lh_handle_01 AS HANDLE.
...
FOR EACH t-Almmmatg EXCLUSIVE-LOCK:
    ...
    t-Almmmatg.StkMax = 0.
    FIND FIRST Inventory_SAP WHERE Inventory_SAP.WarehouseCode = pCodAlm AND
        Inventory_SAP.ItemCode = t-Almmmatg.codmat
        NO-LOCK NO-ERROR.
    IF AVAILABLE Inventory_SAP THEN DO:
        IF Inventory_SAP.Quantity <= 0 THEN DO:
            DELETE t-Almmmatg.
            NEXT.
        END.
        ASSIGN t-Almmmatg.StkMax = Inventory_SAP.Quantity.
        /* 22/05/2023 */
...
CASE TRUE:
    WHEN pCodMat > '' THEN DO:
        ...
        FOR FIRST Inventory_SAP NO-LOCK WHERE Inventory_SAP.WarehouseCode = pCodAlm 
                AND Inventory_SAP.ItemCode = pCodMat
                AND Inventory_SAP.Quantity > 0,
            FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND Almmmatg.codmat = Inventory_SAP.ItemCode AND
                Almmmatg.tpoart <> "D",
    ...
    WHEN CMB-filtro = 'Nombres que inicien con' AND pFiltro > '' THEN DO:
        /* Vamos por la descripción */
        FOR EACH Inventory_SAP WHERE Inventory_SAP.WarehouseCode = pCodAlm AND Inventory_SAP.Quantity > 0,
            FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND Almmmatg.codmat = Inventory_SAP.ItemCode AND
                Almmmatg.desmat BEGINS pFiltro AND Almmmatg.tpoart <> "D",
            FIRST Almtfami OF Almmmatg NO-LOCK WHERE Almtfami.swcomercial = YES:
            /* Filtros */
            IF pMarca > '' AND INDEX(Almmmatg.desmar, pMarca) = 0 THEN NEXT.
            IF pCodFam <> 'Todos' AND Almmmatg.codfam <> pCodFam THEN NEXT.
            /* */
            IF NOT CAN-FIND(FIRST Inventory_SAP WHERE Inventory_SAP.WarehouseCode = pCodAlm AND 
                            Inventory_SAP.ItemCode = Almmmatg.codmat AND
                            Inventory_SAP.Quantity > 0 NO-LOCK)
                THEN NEXT.
    ...
    */
    WHEN CMB-filtro = 'Nombres que contengan' AND pFiltro > '' THEN DO:
        ...
        END.
        LocalCadena = REPLACE(CAPS(LocalCadena),"Z*","*").
        /* Vamos por la descripción */
        FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.desmat CONTAINS LocalCadena AND 
            Almmmatg.codcia = s-codcia AND
            Almmmatg.tpoart <> "D":
            /* Filtros */
            IF pMarca > '' AND NOT Almmmatg.desmar BEGINS pMarca THEN NEXT.
            IF pCodFam <> 'Todos' AND Almmmatg.codfam <> pCodFam THEN NEXT.
            FOR EACH Almtfami OF Almmmatg NO-LOCK WHERE Almtfami.swcomercial = YES:
                FOR FIRST Inventory_SAP NO-LOCK WHERE Inventory_SAP.WarehouseCode = pCodAlm AND
                    Inventory_SAP.ItemCode = Almmmatg.codmat AND 
                    Inventory_SAP.Quantity > 0:
                    CREATE t-Matg.
                    t-Matg.codmat = Almmmatg.codmat.
                END.
            END.
        END.
    END.
    WHEN pCodFam <> 'Todos' THEN DO:
        FOR EACH Almmmatg FIELD(codcia codmat codfam tpoart desmar)
            WHERE Almmmatg.codcia = s-codcia AND
                Almmmatg.codfam = pCodFam AND
    ...
    WHEN pMarca > '' THEN DO:
        FOR EACH Almmmatg FIELD(codcia codmat codfam tpoart desmar)
            WHERE Almmmatg.codcia = s-codcia AND
                Almmmatg.desmar BEGINS pMarca AND
                Almmmatg.tpoart <> "D" NO-LOCK:
            FOR FIRST Inventory_SAP NO-LOCK WHERE Inventory_SAP.WarehouseCode = pCodAlm AND
                    Inventory_SAP.ItemCode = Almmmatg.codmat AND
                    Inventory_SAP.Quantity > 0:
    ...
    OTHERWISE DO:
        /* Vamos de menor a mayor */
        FOR EACH Inventory_SAP NO-LOCK WHERE Inventory_SAP.WarehouseCode = pCodAlm AND Inventory_SAP.Quantity > 0,
            FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND Almmmatg.codmat = Inventory_SAP.ItemCode AND
                Almmmatg.tpoart <> "D",
            FIRST Almtfami OF Almmmatg NO-LOCK WHERE Almtfami.swcomercial = YES:
            /* Filtros */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Temporal-con-SELECT B-table-Win 
PROCEDURE Carga-Temporal-con-SELECT :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
CASE TRUE:
    WHEN pCodMat > '' THEN DO:
        ...
        SELECT Almmmatg.codmat 
            FROM Almmmatg, Almtfami, Inventory_SAP
            WHERE (Almmmatg.codcia = s-codcia AND 
            ...
            (Inventory_SAP.WarehouseCode = pCodAlm AND 
            Inventory_SAP.ItemCode = Almmmatg.codmat AND
            Inventory_SAP.Quantity > 0)
            .
        OPEN cur-CodMat-01.
...
/*   ELSE ITEM.ImpIgv = 0.                                                                                                                           */
  ...
  RUN "Disable_Almacenes" IN lh_handle_01.
