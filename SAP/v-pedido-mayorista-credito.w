...
&SCOPED-DEFINE Promocion SAP/promocion-general-flash.p
...
DEF VAR hProcSapLibrary AS HANDLE NO-UNDO.

RUN SAP\sap-library PERSISTENT SET hProcSapLibrary.
...
"Si", "Si":U
     ...
     FacCPedi.Libre_c04 AT ROW 11.77 COL 14 COLON-ALIGNED WIDGET-ID 186
...
ON LEFT-MOUSE-DBLCLICK OF FacCPedi.Libre_c03 IN FRAME F-Main /* Nro. Depósito */
DO:
  /* Solicitamos la información */
  DEF VAR pcReturn AS LONGCHAR NO-UNDO.
  DEF VAR pcError  AS LONGCHAR NO-UNDO.

  RUN Carga-PR-Pendientes IN hProcSapLibrary (INPUT COMBO-BOX-doc:SCREEN-VALUE, 
                                              INPUT s-codcli,
                                              INPUT FacCPedi.FmaPgo:SCREEN-VALUE,
                                              BUFFER tw-report,
                                              OUTPUT pcReturn, 
                                              OUTPUT pcError).
  /*RUN Carga-BD-Pendientes (INPUT COMBO-BOX-doc:SCREEN-VALUE, OUTPUT pcReturn, OUTPUT pcError).*/
  IF pcError > '' THEN DO:
      MESSAGE STRING(pcError) VIEW-AS ALERT-BOX WARNING.
      SELF:SCREEN-VALUE = "".
      RETURN NO-APPLY.
  END.
/*   IF TRUE <> (pcReturn > '') THEN DO:                                                          */
/*       MESSAGE 'NO hay' (COMBO-BOX-doc:SCREEN-VALUE + "'s pendientes") VIEW-AS ALERT-BOX ERROR. */
/*       SELF:SCREEN-VALUE = "".                                                                  */
/*       RETURN NO-APPLY.                                                                         */
/*   END.                                                                                         */

  /* Lo presentamos en un BROWSE genérico */
  DEF VAR pcTitulo AS CHAR NO-UNDO.
  DEF VAR pcNroDoc AS CHAR NO-UNDO.

  CASE COMBO-BOX-doc:SCREEN-VALUE:
      WHEN "BD"  THEN pcTitulo = "BOLETAS DE DEPOSITO PENDIENTES".
      WHEN "A/R" THEN pcTitulo = "ANTICIPOS RECIBIDOS PENDIENTES".
  END CASE.
  RUN SAP/c-documentos-pendientes  (INPUT pcTitulo,
                                    INPUT TABLE tw-report,
                                    OUTPUT pcNroDoc).
  IF pcNroDoc > '' THEN SELF:SCREEN-VALUE = pcNroDoc.
END.
...
------------------------------------------------------------------------------*/
  ...
  RUN SAP/ventas-library PERSISTENT SET hProc.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    ASSIGN 
        ...
        FacCPedi.porcent[1] = COTIZACION.porcent[1]     /* % Dscto Global */
        FacCPedi.porcent[2] = COTIZACION.porcent[2]     /* Importe Dscto Global */
        NO-ERROR
        .
...
PROCEDURE Delete-Items-Pbo PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF VAR pMensaje AS CHAR NO-UNDO.                
DEF VAR pAviso AS CHAR NO-UNDO.
...
RUN SAP/genera-od-otr-hpk-phr PERSISTENT SET hProc.
RUN PED_Despachar_Rutina_Master IN hProc (INPUT x-Rowid,
                                          INPUT NO,
                                          OUTPUT pMensaje,
                                          OUTPUT pAviso).
DELETE PROCEDURE hProc.
...
ELSE DO:
    IF pAviso > '' THEN DO:
        MESSAGE pAviso VIEW-AS ALERT-BOX WARNING.
    END.
END.
...
RUN Saldo-AC-PED-OD-Disponible IN hProcSapLibrary (BUFFER Faccpedi,
                                                   OUTPUT pSdoAct,
                                                   OUTPUT pMensaje).
...
PROCEDURE Fecha-Entrega PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH PEDI EXCLUSIVE-LOCK, FIRST Almmmatg OF PEDI NO-LOCK, FIRST Almtfami OF Almmmatg NO-LOCK BY PEDI.NroItm:
      ...
      /* **************************************************************************************** */
      FIND Inventory_SAP WHERE Inventory_SAP.WarehouseCode = PEDI.AlmDes AND
          Inventory_SAP.ItemCode = PEDI.CodMat
      ...
      IF LocalVerificaStock = YES THEN DO:
          ...
          IF x-StkAct > 0 THEN DO:
              RUN SAP/stock-comprometido.p (INPUT PEDI.CodMat, 
                                            INPUT PEDI.AlmDes, 
                                            INPUT YES, 
...
PROCEDURE Graba-Totales PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
    ...
    MESSAGE 'Anulación Exitosa' VIEW-AS ALERT-BOX INFORMATION.
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    /* 
        Ic - 23Abr2020, ventas por whatsapp no validar cliente nuevo ya que esta venta viene como tpoped = 'E'
    */
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    ***************************************************************************** */
    ...
    FIND FIRST gn-convt WHERE gn-ConVt.Codig = s-FmaPgo NO-LOCK NO-ERROR.   /* Condicion de venta */
    FIND FIRST B-CPEDI WHERE B-CPEDI.codcia = s-codcia
        ...
        NO-LOCK.
    /*** No valida LC para Contratos Marco ***/
    /* 25/04/2025 Susana León: debe verificar el switch (gn-ConVt.Libre_c03) 
        "Sin Control de Línea de Crédito" */
    CASE TRUE:
        WHEN B-CPEDI.TpoPed = 'M' OR (AVAILABLE gn-convt AND gn-ConVt.Libre_c03 = "SI") THEN DO:
            /* NO se verifica línea de crédito */
        END.
        OTHERWISE DO:
            /* Verifica LC siempre y cuando no sea Anticipo de Campaña (gn-ConVt.Libre_l03) (A/C) */      
            IF NOT AVAILABLE gn-convt OR gn-ConVt.Libre_l03 = NO THEN DO:
                RUN verificar-linea-de-credito (INPUT f-Importe, OUTPUT pMensaje).
                IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                    MESSAGE pMensaje VIEW-AS ALERT-BOX WARNING.
                    APPLY "ENTRY" TO FacCPedi.Glosa.
                    RETURN "ADM-ERROR".
    ...
    IF s-FmaPgo = '002' THEN DO:
        IF TRUE <> (x-boleta-deposito > "") THEN DO:
            MESSAGE "1.- Seleccionar GUARDAR para la reserva de stock según" SKIP
                ...
                VIEW-AS ALERT-BOX INFORMATION TITLE "NOTA IMPORTANTE :".
        END.
    END.
...
------------------------------------------------------------------------------*/
  ...
  /* RHC 09/02/17 Luis Urbano solo para condiciones de pago 403 */
  DO WITH FRAME {&FRAME-NAME}:
      pTpoLic = NO.       /* Valor por defecto */
      
      FIND FIRST gn-convt WHERE gn-ConVt.Codig = s-FmaPgo NO-LOCK NO-ERROR.
      IF NOT AVAILABLE gn-convt OR gn-ConVt.Libre_l03 = NO THEN RETURN 'OK'.    /* OJO */
      ...
      /* ****************************************************************************** */
      RUN Saldo-AC-Pendientes IN hProcSapLibrary (INPUT Faccpedi.codcli,
      ...
      /* 14/08/2025 */
      FOR EACH ORDEN NO-LOCK WHERE ORDEN.codcia = s-codcia AND
          ORDEN.codcli = s-codcli AND
          ORDEN.coddoc = "O/D" AND
          ORDEN.flgest = "P":
          IF ORDEN.codmon <> s-codmon THEN NEXT.
...
PROCEDURE valida-deposito PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       NO PASA si no cubre el pedido logístico
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER pCodDeposito     AS CHAR NO-UNDO.
DEFINE INPUT PARAMETER pNumeroDeposito  AS CHAR NO-UNDO.
DEFINE INPUT PARAMETER pTot             AS DECI NO-UNDO.
...
DO WITH FRAME {&FRAME-NAME}:
    /* Recargamos información */
    /* Solicitamos la información */
    DEF VAR pcReturn AS LONGCHAR NO-UNDO.
    DEF VAR pcError  AS LONGCHAR NO-UNDO.

    RUN Carga-PR-Pendientes IN hProcSapLibrary (INPUT pCodDeposito, 
                                                INPUT s-codcli,
                                                INPUT FacCPedi.FmaPgo:SCREEN-VALUE,
                                                BUFFER tw-report,
                                                OUTPUT pcReturn, 
                                                OUTPUT pcError).
    IF pcError > '' THEN DO: 
        pMsgRet = STRING(pcError).
        RETURN 'ADM-ERROR'.
    END.

    FIND tw-report WHERE tw-report.campo-C[1] = pCodDeposito
        AND tw-report.campo-c[2] = pNumeroDeposito
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE tw-report THEN DO:
        pMsgRet = "El " + pCodDeposito + " " + pNumeroDeposito + " NO REGISTRADO o NO ESTA PENDIENTE o NO es CONTADO ANTICIPADO".
        RETURN 'ADM-ERROR'.
    END.
    ...
    */
    ...
    IF s-CodMon = tw-report.Campo-I[1] THEN DO:
        IF tw-report.Campo-F[2] < pTot THEN DO:
            pMsgRet = "El saldo del " + pCodDeposito + " NO cubre el monto total".
        END.
    END.
    ELSE DO:
        IF tw-report.Campo-I[1] = 1 THEN DO:
            IF tw-report.Campo-F[2] < (pTot * s-Tpocmb) THEN DO:
                pMsgRet = "El saldo del " + pCodDeposito + " NO cubre el monto total".
            END.
        END.
        ELSE DO:
            IF tw-report.Campo-F[2] < (pTot / FaccPedi.Tpocmb) THEN DO:
                pMsgRet = "El saldo del " + pCodDeposito + " NO cubre el monto total".
            END.
        END.
    END.
...
PROCEDURE VALIDATE-DETAIL PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  FOR EACH PEDI EXCLUSIVE-LOCK, FIRST Almmmatg OF PEDI NO-LOCK, FIRST Almtfami OF Almmmatg NO-LOCK:
      ...
      /* VERIFICAMOS STOCK DISPONIBLE DE ALMACEN */
      /* **************************************************************************************** */
      FIND FIRST Inventory_SAP WHERE Inventory_SAP.ItemCode = PEDI.CodMat AND
          Inventory_SAP.WarehouseCode = PEDI.AlmDes
          NO-LOCK NO-ERROR NO-WAIT.
      /* RHC 15/03/2021 Decidimos cuándo verificamos el stock */
      LocalVerificaStock = NO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE verificar-linea-de-credito V-table-Win 
PROCEDURE verificar-linea-de-credito PRIVATE :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE INPUT PARAMETER fImporte AS DEC NO-UNDO.
  DEFINE OUTPUT PARAMETER pcError AS CHAR NO-UNDO.

  /* Si la venta es al contado NO se verifica la línea de crédito */
  FIND FIRST gn-convt WHERE gn-ConVt.Codig = s-FmaPgo NO-LOCK NO-ERROR.
  IF AVAILABLE gn-convt AND gn-convt.tipvta = "1" THEN RETURN 'OK'.

  /* 28/11/2024 Gina Condor */
  RUN Verifica-Linea-de-Credito IN hProcSapLibrary (INPUT s-CodDiv,
                                                    INPUT s-CodCli,
                                                    INPUT s-CodMon,
                                                    INPUT fImporte,
                                                    OUTPUT pcError).
  IF pcError <> "" THEN RETURN "ADM-ERROR".
/*   RUN Verifica-Linea-de-Credito IN hProcSapLibrary (INPUT s-CodDiv, */
/*                                                     INPUT s-CodCli, */
/*                                                     INPUT s-CodMon, */
/*                                                     INPUT fImporte, */
