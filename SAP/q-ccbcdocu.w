...
&Scoped-define QUERY-STRING-Query-Main FOR EACH CcbCDocu WHERE CcbCDocu.CodCia = GN-DIVI.CodCia ~
  AND CcbCDocu.CodDiv = GN-DIVI.CodDiv ~
      AND CcbCDocu.CodDoc = s-coddoc USE-INDEX Llave10 NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH CcbCDocu WHERE CcbCDocu.CodCia = GN-DIVI.CodCia ~
  AND CcbCDocu.CodDiv = GN-DIVI.CodDiv ~
      AND CcbCDocu.CodDoc = s-coddoc USE-INDEX Llave10 NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 FILL-IN-1 RECT-3 
...
DEFINE VARIABLE FILL-IN-1 AS CHARACTER FORMAT "X(9)":U 
     LABEL "Buscar" 
     VIEW-AS FILL-IN 
     SIZE 13 BY .73
...
DEFINE FRAME FRAME-A
     BUTTON-1 AT ROW 1 COL 23 WIDGET-ID 2
     FILL-IN-1 AT ROW 1.27 COL 3.43 WIDGET-ID 4
     RECT-3 AT ROW 1 COL 1 WIDGET-ID 6
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "INTEGRAL.CcbCDocu WHERE INTEGRAL.GN-DIVI ..."
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[1]      = "CcbCDocu.CodCia = GN-DIVI.CodCia
  AND CcbCDocu.CodDiv = GN-DIVI.CodDiv"
     _Where[1]         = "CcbCDocu.CodDoc = s-coddoc USE-INDEX Llave10"
     _Design-Parent    is WINDOW q-tables @ ( 1.08 , 29.57 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
ON LEAVE OF FILL-IN-1 IN FRAME FRAME-A /* Buscar */
OR "RETURN":U OF FILL-IN-1 DO:
  IF TRUE <> (SELF:SCREEN-VALUE > '') THEN RETURN.

  ASSIGN 
      SELF:SCREEN-VALUE = STRING(INTEGER(SELF:SCREEN-VALUE), "999999999") NO-ERROR.
  RUN Ubica-Registro.
  SELF:SCREEN-VALUE = "".
END.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN sap/c-bol-deposito ("Boletas de Depositos").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE set_filters q-tables 
...
FIND FIRST Ccbcdocu WHERE CcbCDocu.CodCia = gn-divi.codcia AND
    CcbCDocu.CodDiv = gn-divi.coddiv AND 
    CcbCDocu.CodDoc = s-coddoc AND
    Ccbcdocu.NroDoc = FILL-IN-1:SCREEN-VALUE IN FRAME {&FRAME-NAME}
    NO-LOCK NO-ERROR.
