...
PEDI.UndVta PEDI.CanPed PEDI.Por_Dsctos[1] PEDI.Por_Dsctos[2] ~
PEDI.Por_Dsctos[3] PEDI.PreUni 
...
"PEDI.UndVta" "Unidad" "x(10)" "character" ? ? ? ? ? ? yes ? no no "5.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    RUN SAP/d-uniabc (PEDI.codmat:SCREEN-VALUE IN BROWSE {&browse-name},
                      s-codalm,
                      s-coddiv,
                      s-codcli,
                      s-tpocmb,
...
DO:
    ...
    RUN web/PrecioFinalContadoMayorista.p (INPUT s-CodCia,
                                           INPUT s-CodDiv,
                                           INPUT s-CodCli,
                                           INPUT s-CodMon,
                                           INPUT s-TpoCmb,
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  /* 31/10/2022: Determinamos el factor de equivalencia */
  f-Factor = 1.
  FIND Almtconv WHERE Almtconv.CodUnid = Almmmatg.UndBas AND  
       Almtconv.Codalter = PEDI.UndVta NO-LOCK NO-ERROR.
  IF AVAILABLE Almtconv THEN f-Factor = Almtconv.Equival.
  /* ************************************************** */
...
------------------------------------------------------------------------------*/
  ...
  FIND Almtconv WHERE Almtconv.CodUnid  = Almmmatg.UndBas                      
      ...
      NO-LOCK NO-ERROR.
  IF NOT AVAILABLE ALmtconv THEN DO:
      MESSAGE 'Equivalencia NO registrada' SKIP
          ' Unidad Base:' Almmmatg.UndBas SKIP
          'Unidad Venta:' PEDI.UndVta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
          VIEW-AS ALERT-BOX ERROR.
      APPLY 'ENTRY':U TO PEDI.CodMat.
      RETURN "ADM-ERROR".
  END.
