...
/*     IF (DETA.Por_Dsctos[1] + DETA.Por_Dsctos[2] + DETA.Por_Dsctos[3]) = 100 */
/*         THEN ASSIGN                                                         */
...
/*         CcbDDocu.cTipoAfectacion = "GRATUITA"                               */
/*         .                                                                   */
...
&IF {&ARITMETICA-SUNAT} &THEN
    ...
    FOR EACH Ccbddocu OF Ccbcdocu EXCLUSIVE-LOCK WHERE (CcbDDocu.Por_Dsctos[1] + CcbDDocu.Por_Dsctos[2] + CcbDDocu.Por_Dsctos[3]) <> 100,
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    FOR EACH Ccbddocu OF B-CDOCU NO-LOCK WHERE Ccbddocu.candes - Ccbddocu.candev > 0,
        ...
        /* 10/06/2026: Caso 100% de descuento */
/*         CREATE DETA.                                                                                                            */
/*         BUFFER-COPY Ccbddocu TO DETA.                                                                                           */
/*         IF (CcbDDocu.Por_Dsctos[1] + CcbDDocu.Por_Dsctos[2] + CcbDDocu.Por_Dsctos[3]) = 100                                     */
/*             THEN DO:                                                                                                            */
/*             ASSIGN                                                                                                              */
...
/*         IF DETA.AftIsc                                                                                                          */
/*             THEN DETA.ImpIsc = ROUND(DETA.PreBas * DETA.CanDes * (Almmmatg.PorIsc / 100),4).                                    */
/*         ELSE DETA.ImpIsc = 0.                                                                                                   */
/*         IF DETA.AftIgv                                                                                                          */
/*             THEN DETA.ImpIgv = DETA.ImpLin - ROUND(DETA.ImpLin  / (1 + (s-PorIgv / 100)),4).                                    */
/*         ELSE DETA.ImpIgv = 0.                                                                                                   */
...
DO WITH FRAME {&FRAME-NAME} :
    ...
    END.
    /*MESSAGE x-impte x-impte-devolucion  B-CDocu.imptot.*/
