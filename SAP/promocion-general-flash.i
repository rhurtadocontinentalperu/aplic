    ...
    IF s-CodDoc <> "COT" THEN DO:
        ...
        CASE TRUE:
            WHEN Vtactabla.Libre_L05 = YES AND Vtactabla.Libre_c04 = "OR" THEN DO:
                ...
                FOR EACH Promocion:
                    x-StkAct = 0.
                    FIND Inventory_SAP WHERE Inventory_SAP.WarehouseCode = pCodAlm
                        AND Inventory_SAP.ItemCode = Promocion.LlaveDetalle
                        NO-LOCK NO-ERROR NO-WAIT.
                    IF AVAILABLE Inventory_SAP THEN x-StkAct = Inventory_SAP.Quantity.
                    IF x-StkAct > 0 THEN DO:
                        RUN SAP/Stock-Comprometido (INPUT Promocion.LlaveDetalle, 
                                                    INPUT pCodAlm, 
                                                    INPUT YES, 
