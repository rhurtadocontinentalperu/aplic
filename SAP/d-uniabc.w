...
IF AVAIL B-Almmmatg THEN DO:
    ...
    FIND Inventory_SAP WHERE Inventory_SAP.WarehouseCode = ENTRY(1, s-codalm)
        AND Inventory_SAP.ItemCode = B-Almmmatg.codmat
        NO-LOCK NO-ERROR.
    IF AVAILABLE Inventory_SAP THEN F-totstk = f-TotStk + Inventory_SAP.Quantity.
    /* Solo calculamos comprometido si hubiera stock */
    IF f-TotStk > 0 THEN RUN SAP/stock-comprometido (s-codmat, ENTRY(1, s-codalm), YES, OUTPUT x-CanPed).
    /* ********************************************* */
    ASSIGN 
    ...
    RUN Precio-Unitario-New (1, B-Almmmatg.UndBas, OUTPUT x-PreUniNac).
    RUN Precio-Unitario-New (2, B-Almmmatg.UndBas, OUTPUT x-PreUniUsa).
...
/* ************************ */
    ...
    DEFINE VAR pcReturn AS LONGCHAR NO-UNDO.
    DEFINE VAR pcError AS LONGCHAR NO-UNDO.

    RUN web/web-library.p PERSISTENT SET hProc.

    RUN web_api_unit_price IN hProc (INPUT s-CodMat,
                                     INPUT "C",
                                     INPUT "000",
                                     INPUT s-CodCli,
                                     INPUT s-CodDiv,
