...
/* 14-8-2023: Regalos por Escalas */
FOR EACH Vtactabla NO-LOCK WHERE Vtactabla.codcia = s-codcia
    AND VtaCTabla.Tabla = "REGALOS_POR_ESCALA"      /* OJO */
    AND VtaCTabla.Estado = "A"      /* Activa */
    ...
    RUN Carga-con-filtros.

    RUN Graba-regalos-por-escala.
    IF RETURN-VALUE = "ADM-ERROR" THEN RETURN "ADM-ERROR".
END.
...
FOR EACH Vtadtabla NO-LOCK WHERE Vtadtabla.codcia = Vtactabla.codcia
    AND Vtadtabla.tabla = Vtactabla.tabla 
    AND Vtadtabla.llave = Vtactabla.llave
    AND Vtadtabla.Tipo = "R":
    /* Acumulamos por Línea y/o Sublinea */
    x-ImpLin = 0.
    x-Cantidad = 0.
    /*FOR EACH DETALLE NO-LOCK, FIRST Almmmatg OF DETALLE NO-LOCK WHERE Almmmatg.codmar = Vtadtabla.LlaveDetalle :*/
    FOR EACH DETALLE NO-LOCK, FIRST Almmmatg WHERE Almmmatg.codcia = s-codcia
            AND Almmmatg.codmat = DETALLE.codmat NO-LOCK:
        IF TRIM(Almmmatg.codmar) <> Vtadtabla.LlaveDetalle THEN NEXT.
        /* Buscamos si es una excepción */
        RUN Excepcion-Linea (OUTPUT pExcepcion).
        IF pExcepcion = YES THEN NEXT.
        /* Fin de excepciones */
    ...
    /* Verificamos el "Operador" */
    CASE TRUE:
        WHEN Vtactabla.Libre_c06 = "OR" THEN DO:    /* Por la compra de cualquier de los productos */
            /* Verificamos si tiene mínimo */
            IF Vtadtabla.Libre_d01 > 0 THEN DO:      /* Debe comprar como mínimo */
                IF Vtadtabla.Libre_c02 = "Importe" THEN IF Vtadtabla.Libre_d01 > x-ImpLin THEN NEXT rloop.
                IF Vtadtabla.Libre_c02 = "Cantidad" THEN IF Vtadtabla.Libre_d01 > x-Cantidad THEN NEXT rloop.
            ...
            ASSIGN
                ...
                x-Cantidades-04 = x-Cantidades-04 + x-Cantidad.
        END.
        WHEN Vtactabla.Libre_c06 = "AND" THEN DO:    /* Debe comprar todos los productos */
            /* Debe comprarlo */
        ...
        END.
        OTHERWISE DO:
            MESSAGE 'Promoción' Vtactabla.llave 'hay un problema con el OPERADOR de MARCAS' SKIP
                'Debe ser SI o NO, pero está vacio' SKIP
                'Comunicarse con los Jefes de Línea' VIEW-AS ALERT-BOX WARNING.
            LEAVE rloop.
        END.
    END CASE.
END.
...
FIND FIRST B-DTABLA4 OF Vtactabla WHERE B-DTABLA4.Tipo = "R" NO-LOCK NO-ERROR.  /* Por Marcas */

IF NOT AVAILABLE B-DTABLA AND NOT AVAILABLE B-DTABLA2 AND NOT AVAILABLE B-DTABLA3 AND NOT AVAILABLE B-DTABLA4
...
FOR EACH Vtadtabla OF Vtactabla NO-LOCK WHERE Vtadtabla.Tipo = "OF" BY Vtadtabla.Libre_d04 DESC:
    ...
    /* *********************************** */
    CREATE Promocion.
    BUFFER-COPY Vtadtabla  
        EXCEPT Vtadtabla.Libre_d04 Vtadtabla.Libre_d05 
        TO Promocion
        ASSIGN Promocion.llave = Vtactabla.Llave
        .
    IF VtaCTabla.Libre_d01 > 0 THEN DO:
        IF Vtactabla.Libre_c01 = "Soles" THEN 
            Promocion.Libre_d05 = TRUNCATE(x-AcumulaImportes / VtaCTabla.Libre_d01, 0) * Vtadtabla.Libre_d01.
...
&IF DEFINED(EXCLUDE-Graba-regalos-por-escala) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Graba-regalos-por-escala Procedure 
PROCEDURE Graba-regalos-por-escala :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* Determinamos la escala válida */
DEF VAR f-Factor-Escala AS DECI NO-UNDO.
FOR EACH Vtadscale OF Vtactabla NO-LOCK BY VtadScale.Scale:
    IF VtadScale.Qty <= 0 THEN LEAVE.   /* Fin del loop */
    IF NOT CAN-FIND(FIRST VtadScaleGift WHERE VtadScaleGift.CodCia = VtadScale.CodCia
        AND VtadScaleGift.Tabla = VtadScale.Tabla
        AND VtadScaleGift.Llave = VtadScale.Llave NO-LOCK)
        THEN NEXT.
    /* Filtros */
    IF Vtactabla.Libre_c01 = "Soles" AND x-AcumulaImportes < VtadScale.Qty THEN NEXT.
    IF Vtactabla.Libre_c01 = "Unidades" AND x-AcumulaCantidades < VtadScale.Qty THEN NEXT.

    /* **************************************** */
    /* Siempre cargamos la Promocion como nueva */
    /* **************************************** */
    /*f-Factor-Escala = TRUNCATE(x-AcumulaImportes / VtaCTabla.Libre_d01, 0).*/
    CASE Vtactabla.Libre_c01:
        WHEN "Soles" THEN f-Factor-Escala = TRUNCATE(x-AcumulaImportes / VtadScale.Qty, 0).
        WHEN "Unidades" THEN f-Factor-Escala = TRUNCATE(x-AcumulaCantidades / VtadScale.Qty, 0).
    END CASE.
    /* **************************************** */
    IF VtadScale.MAXIMUM > 0 THEN f-Factor-Escala = MINIMUM(f-Factor-Escala, VtadScale.MAXIMUM).
    EMPTY TEMP-TABLE Promocion.
    FOR EACH VtadScaleGift WHERE VtadScaleGift.CodCia = VtadScale.CodCia
        AND VtadScaleGift.Tabla = VtadScale.Tabla
        ...
        ASSIGN
            ...
            Promocion.LlaveDetalle = VtadScaleGift.CodMat
            Promocion.Libre_d01 = VtadScaleGift.Qty     /* En unidades base */
            Promocion.Libre_d02 = VtadScale.Maximum.    /* Tope */
        IF Vtactabla.Libre_c01 = "Soles" THEN 
            Promocion.Libre_d05 = f-Factor-Escala * VtadScaleGift.Qty.
        IF Vtactabla.Libre_c01 = "Unidades" THEN 
            Promocion.Libre_d05 = f-Factor-Escala * VtadScaleGift.Qty.
    END.
END.
