...
T-MATE.UndVta T-MATE.StkAct T-MATE.StkRep T-MATE.StkMax 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-5 D-Dialog _STRUCTURED
  QUERY BROWSE-5 NO-LOCK DISPLAY
      ...
      T-MATE.StkMax COLUMN-LABEL "Disponible" FORMAT "(ZZZ,ZZZ,ZZ9.99)":U
            WIDTH 11 COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 10
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"Almacen.Descripcion" ? "X(60)" "character" ? ? ? ? ? ? no ? no no "59.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"T-MATE.StkAct" "Stock Actual" ? "decimal" 11 ? ? ? ? ? no ? no no "12.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"T-MATE.StkMax" "Disponible" "(ZZZ,ZZZ,ZZ9.99)" "decimal" 10 0 ? ? ? ? no ? no no "11" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-5 */
&ANALYZE-RESUME
...
FOR EACH VtaAlmDiv NO-LOCK WHERE VtaAlmDiv.CodCia = s-codcia
    ...
    BY VtaAlmDiv.Orden:
    FIND Inventory_SAP WHERE Inventory_SAP.WarehouseCode = Vtaalmdiv.codalm AND
        Inventory_SAP.ItemCode = input-var-1
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Inventory_SAP THEN NEXT.

    FIND Almmmatg WHERE Almmmatg.codcia = s-codcia AND
        Almmmatg.codmat = input-var-1 NO-LOCK NO-ERROR.
    CREATE T-MATE.
    ASSIGN
        ...
        T-MATE.stkact = Inventory_SAP.Quantity 
        pComprometido = 0.
    RUN SAP/stock-comprometido (Almmmatg.codmat, Vtaalmdiv.codalm, YES, OUTPUT pComprometido).
    ASSIGN
...
SESSION:SET-WAIT-STATE('GENERAL').
EMPTY TEMP-TABLE T-MATE.
FOR EACH Inventory_SAP NO-LOCK WHERE Inventory_SAP.ItemCode = input-var-1
        AND Inventory_SAP.Quantity > 0,
    FIRST Almacen NO-LOCK WHERE Almacen.codcia = s-codcia and
        Almacen.codalm = Inventory_SAP.WarehouseCode AND 
    ...
    FIRST Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND
        Almmmatg.codmat = Inventory_SAP.ItemCode:
    CREATE T-MATE.
    ASSIGN
        T-MATE.codcia = s-codcia
        T-MATE.codalm = Inventory_SAP.WarehouseCode
        T-MATE.codmat = Inventory_SAP.ItemCode
        T-MATE.undvta = Almmmatg.undbas
        T-MATE.stkact = Inventory_SAP.Quantity
        pComprometido = 0.
    RUN SAP/stock-comprometido (Inventory_SAP.ItemCode, Inventory_SAP.WarehouseCode, YES, OUTPUT pComprometido).
    ASSIGN
        T-MATE.StkRep = pComprometido
        T-MATE.StkMax = T-MATE.StkAct - T-MATE.StkRep.
END.
