...
&Scoped-define QUERY-STRING-Query-Main FOR EACH gn-clie WHERE ~{&KEY-PHRASE} NO-LOCK ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Position-Register q-tables 
PROCEDURE Position-Register :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* 1. Buscar directamente en la tabla física (Cambia 'gn-clie' por tu tabla real si se llama distinto) */
FIND FIRST gn-clie WHERE gn-clie.codcia = cl-codcia AND gn-clie.codcli = pcCodCli NO-LOCK NO-ERROR.

/* 2. Validar que exista */
IF NOT AVAILABLE gn-clie THEN RETURN.

/* 3. Obtener el ROWID */
rRowid = ROWID(gn-clie).

/* 4. Reposicionar el Query principal (Cambia 'Query-Main' por el nombre real de tu query) */
REPOSITION Query-Main TO ROWID rRowid NO-ERROR.

/* 5. Forzar la actualización del SmartViewer en ADM1 */
