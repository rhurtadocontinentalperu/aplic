...
/* NO anular O/Ds con más de 30 días */
DEF VAR dFechaLimite AS DATE NO-UNDO.
dFechaLimite = ADD-INTERVAL(TODAY, -1, 'month').
...
&Scoped-define QUERY-STRING-br_table FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
 ...
 AND FacCPedi.FchPed >= dFechaLimite ~
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: bFacCPedi B "?" ? INTEGRAL FacCPedi
...
/* FILTRO DE CONTROL */
/* Reposicionamos la O/D */
FIND bFaccpedi WHERE ROWID(bFaccpedi) = ROWID(Faccpedi) NO-LOCK.
IF bFacCPedi.FlgEst <> "P" THEN DO:
    MESSAGE 'La Orden NO está pendiente' SKIP 'Acceso Denegado' VIEW-AS ALERT-BOX WARNING.
    RETURN "ADM-ERROR".
END.
/* 2 CASOS:
    1. Despacho por Tienda
    2. Despacho por CD
*/
FIND gn-divi WHERE gn-divi.codcia = s-codcia AND gn-divi.coddiv = bFaccpedi.divdes NO-LOCK.
CASE gn-divi.Campo-Char[6]:
    WHEN "PR" THEN DO:      /* Picking por Rutas => WMS */
        /* Aquí debe ir la llamda al API */
    END.
    OTHERWISE DO:           /* Tiendas y Sectores */
        /* Se puede anular hasta antes de cerrar el Pre-Picking */
        IF bFacCPedi.FlgSit <> "T" THEN DO:
            MESSAGE "Pre-Picking concluido, no se puede anular la orden" VIEW-AS ALERT-BOX WARNING.
            RETURN 'ADM-ERROR'.
        END.
        FIND FIRST bFacDPedi OF bFacCPedi WHERE bFacDPedi.canate > 0 NO-LOCK NO-ERROR.
        IF AVAILABLE bFacDPedi THEN DO:
...
IF bFaccpedi.CodRef = "PED" AND bFaccpedi.CrossDocking = NO AND bFaccpedi.TpoPed = "XD" THEN DO:
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    {lib/lock-genericov3.i
        &Tabla="bFacCPedi"
        &Condicion="ROWID(bFacCPedi) = ROWID(FacCPedi)"
        &Bloqueo="EXCLUSIVE-LOCK NO-ERROR"
        &Accion="RETRY"
        &Mensaje="NO"
        &txtMensaje="pMensaje"
