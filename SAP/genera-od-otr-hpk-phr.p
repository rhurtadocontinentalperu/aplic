...
/* RUTINA ESTANDAR */
/* *********************************************************************** */
RUN CCB_Verifica_Cliente_FmaPgo (OUTPUT pAviso, OUTPUT pMensaje).
...
IF PEDIDO.fmapgo = '002' AND PEDIDO.FlgEst = "P" THEN DO:
    /* Solicitamos información al SAT */
    ...
    RUN Carga-PR-Pendientes IN hProcSapLibrary (INPUT x-doc-deposito, 
                                                INPUT PEDIDO.CodCli,
                                                INPUT PEDIDO.FmaPgo,
                                                BUFFER tw-report,
                                                OUTPUT pcReturn, 
                                                OUTPUT pcError).
    DELETE PROCEDURE hProcSapLibrary.
    IF pcError > '' THEN DO:
        pMensaje = STRING(pcError).
        UNDO, RETURN 'ADM-ERROR'.
    END.
    IF TRUE <> (pcReturn > '') THEN DO:
        pMensaje = 'NO hay ' + x-doc-deposito + "'s pendientes".
        UNDO, RETURN 'ADM-ERROR'.
    END.
    FIND tw-report WHERE tw-report.campo-C[1] = x-doc-deposito
        AND tw-report.campo-c[2] = PEDIDO.Libre_c03
        NO-LOCK NO-ERROR.
    ...
    /*
        01Oct2019, Susana Leon converso con Daniel Llican y en coordinacion
                    ...
                    al monto del pedido se debe derivar a la bandeja de CyC
    */            
    IF PEDIDO.CodMon = tw-report.Campo-I[1] THEN DO:
        IF tw-report.Campo-F[2] < PEDIDO.imptot THEN DO:
            ASSIGN 
                PEDIDO.flgest = 'X'
    ...
    ELSE DO:
        IF tw-report.Campo-I[1] = 1 THEN DO:
            IF tw-report.Campo-F[2] < (PEDIDO.imptot * PEDIDO.Tpocmb) THEN DO:
                ASSIGN 
                    PEDIDO.flgest = 'X'
                    PEDIDO.Libre_c05 = 'EL MONTO DEL " + PEDIDO.usrchq + " " + PEDIDO.Libre_c03 + " ES INFERIOR AL MONTO DEL PEDIDO'
                    PEDIDO.Libre_c04 = '- PASA POR EVALUACION DE CREDITO'.
...
/* ************************************************************ */
IF LOOKUP(TRIM(PEDIDO.fmapgo),"001,002") = 0 THEN DO:
...
/* ********************************************************************************* */
FIND FIRST gn-convt WHERE gn-ConVt.Codig = PEDIDO.FmaPgo NO-LOCK NO-ERROR.
/* 10/12/2024: Carla Tenazoa, condición de venta 403 NO debería verificar línea de crédito */
/* Solo verificamos 0 */
CONTADOS:
DO:
    IF AVAILABLE gn-convt AND gn-convt.tipvta = "1" THEN DO:
        IF gn-convt.libre_l01 = YES THEN LEAVE CONTADOS.    /* Acepta FAI */
        IF gn-convt.libre_l02 = YES THEN LEAVE CONTADOS.    /* Venta única campaña */
        PEDIDO.Flgest = 'P'.
        RETURN "OK".    /* NO controla línea de crédito */
    END.
END.
...
RUN Verifica-Linea-de-Credito IN hProcSapLibrary (INPUT PEDIDO.CodDiv,
                                                  INPUT PEDIDO.CodCli,
                                                  INPUT PEDIDO.CodMon,
                                                  INPUT 0,
                                                  OUTPUT pMensaje).
...
RUN Carga-EECC-Pendientes IN hProcSapLibrary (INPUT PEDIDO.CodCli,
                                              BUFFER tw-report,
                                              OUTPUT pcReturn,
                                              OUTPUT pcError).
DELETE PROCEDURE hProcSapLibrary.
...
FOR EACH factabla WHERE factabla.codcia = s-codcia AND factabla.tabla = x-tabla-cyc NO-LOCK:
    ...
    FIND FIRST tw-report WHERE tw-report.Campo-C[4] = Factabla.Codigo AND
        (tw-report.Campo-D[2] + x-dias-tolerables ) < TODAY 
        NO-LOCK NO-ERROR.
    IF AVAIL CcbCDocu THEN DO:
        ASSIGN
            PEDIDO.Flgest = 'X'
            PEDIDO.Glosa  = TRIM (PEDIDO.Glosa) + '//Doc. Venc.'
            PEDIDO.Libre_c05 = 'DEUDA VENCIDA: ' + tw-report.Campo-C[4] + ' ' + tw-report.Campo-C[5].
        pAviso = "El cliente/grupo tiene una deuda atrazada:" + CHR(13) + CHR(10) +
            "Cliente : " + TRIM(PEDIDO.nomcli) + CHR(13) + CHR(10) +
            "Documento : " + TRIM(tw-report.Campo-C[4]) + " " + TRIM(tw-report.Campo-C[5]) + CHR(13) + CHR(10) +
            "Vencimiento : " + STRING(tw-report.Campo-D[2],"99/99/9999").
        RETURN.
    END.
END.        
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* 08/05/2026: OJO, como todo está en SAP entonces TODO va a ser ubicación genérica */
    lSector = "G0".
    lSectorOK = NO.
    FOR EACH Facdpedi OF bFaccpedi NO-LOCK:
        IF lSectorG0 = NO THEN DO:
            CREATE vtacdocu.
            BUFFER-COPY bFaccpedi TO vtacdocu
                ...
                NO-ERROR.
            IF ERROR-STATUS:ERROR = YES THEN DO:
                pMensaje = "Error al grabar la suborden " + bFaccpedi.nroped + '-' + lSector.
                UNDO RLOOP, RETURN 'ADM-ERROR'.
            END.
            ASSIGN
            ...
            lSectorG0 = YES.
        END.
/*         FIND FIRST almmmate NO-LOCK WHERE Almmmate.CodCia = facdpedi.codcia                                              */
