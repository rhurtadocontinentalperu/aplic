...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    ASSIGN 
        ...
        /* 10/08/2026: César Iman, solo el 1er almacén */
        FacCPedi.CodAlm = ENTRY(1,s-CodAlm)    /* Lista de Almacenes Válidos de Venta */
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ITEM EXCLUSIVE-LOCK WHERE ITEM.codmat <> x-articulo-ICBPER, 
      ...
      x-StkAct = 0.
      FIND Inventory_SAP WHERE Inventory_SAP.WarehouseCode = ITEM.AlmDes AND Inventory_SAP.ItemCode = ITEM.CodMat NO-LOCK NO-ERROR.
...
/*   IF RETURN-VALUE = "ADM-ERROR" THEN UNDO, RETURN "ADM-ERROR".                     */
  ...
  */                
  ...
  FOR EACH ITEM NO-LOCK WHERE ITEM.Libre_c05 = "OF", FIRST Almmmatg OF ITEM NO-LOCK, FIRST Almtfami OF Almmmatg NO-LOCK:
      ...
      x-StkAct = 0.
      IF AVAILABLE Inventory_SAP THEN x-StkAct = Inventory_SAP.Quantity.
      RUN SAP/stock-comprometido.p (ITEM.CodMat, ITEM.AlmDes, YES, OUTPUT s-StkComprometido).
      /* **************************************************************** */
      /* OJO: Cuando se modifica hay que actualizar el STOCK COMPROMETIDO */
      /* **************************************************************** */
      IF pEvento = "NO" THEN DO:
