...
&Scoped-Define ENABLED-OBJECTS RADIO-SET-option FILL-IN-Codpro ~
FILL-IN-fecdoc BUTTON-print BUTTON-exit FILL-IN-Codpro-2 RECT-2 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-option FILL-IN-Codpro ~
FILL-IN-fecdoc FILL-IN-Codpro-2 
...
DEFINE VARIABLE FILL-IN-Codpro-2 AS CHARACTER FORMAT "X(256)":U 
     LABEL "Hasta" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .81 NO-UNDO.
...
DEFINE FRAME F-Main
     RADIO-SET-option AT ROW 2.88 COL 42 NO-LABEL WIDGET-ID 8
     FILL-IN-Codpro AT ROW 1.81 COL 11 COLON-ALIGNED WIDGET-ID 4
     FILL-IN-fecdoc AT ROW 2.88 COL 11 COLON-ALIGNED
     BUTTON-print AT ROW 5.85 COL 34
     BUTTON-exit AT ROW 5.85 COL 50
     FILL-IN-Codpro-2 AT ROW 1.81 COL 36 COLON-ALIGNED WIDGET-ID 14
     " Imprimir:" VIEW-AS TEXT
          SIZE 6 BY .5 AT ROW 3.69 COL 35 WIDGET-ID 12
...
ON END-ERROR OF W-Win /* Carta a Proveedores */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Carta a Proveedores */
DO:
...
DO:
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        IF FILL-IN-Codpro-2 = "" THEN
            FILL-IN-Codpro-2 = "99999999999".
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-option FILL-IN-Codpro FILL-IN-fecdoc FILL-IN-Codpro-2 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RADIO-SET-option FILL-IN-Codpro FILL-IN-fecdoc BUTTON-print 
         BUTTON-exit FILL-IN-Codpro-2 RECT-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH gn-prov
        FIELDS (gn-prov.codcia gn-prov.codpro gn-prov.nompro gn-prov.dirpro gn-prov.ruc
            gn-prov.flgsit gn-prov.rucold gn-prov.CodDept gn-prov.codprov gn-prov.codprov
            gn-prov.coddist)
        WHERE gn-prov.codcia = pv-codcia
        AND gn-prov.codpro >= FILL-IN-Codpro
        AND gn-prov.codpro <= FILL-IN-Codpro-2
        AND gn-prov.flgsit = "A"
        AND LENGTH(gn-prov.ruc) = 11
        AND (gn-prov.ruc BEGINS "10" OR gn-prov.ruc BEGINS "17" OR gn-prov.ruc BEGINS "20")
        NO-LOCK:
        DISPLAY
            gn-prov.codpro @ Fi-Mensaje LABEL "  Proveedor" FORMAT "X(14)"
            WITH FRAME F-Proceso.

        FIND Tabprovi
            WHERE Tabprovi.CodDepto = gn-prov.CodDept
            AND Tabprovi.Codprovi = gn-prov.codprov
            NO-LOCK NO-ERROR.
        ...
        /* Captura Distrito */
        FIND Tabdistr
            WHERE Tabdistr.CodDepto = gn-prov.CodDept
        ...
        CASE RADIO-SET-option:
            WHEN 1 THEN RUN proc_print_letter(gn-prov.nompro,gn-prov.dirpro,cProvincia).
            WHEN 2 THEN RUN proc_print_sobre(gn-prov.nompro,gn-prov.dirpro,cProvincia).
        END CASE.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT PARAMETER para_nompro AS CHARACTER NO-UNDO.
    DEFINE INPUT PARAMETER para_dirpro AS CHARACTER NO-UNDO.
    DEFINE INPUT PARAMETER para_distri AS CHARACTER NO-UNDO.

    DEFINE VARIABLE cFile AS CHARACTER NO-UNDO.

    CREATE "Word.Application" Word.
    Word:Documents:ADD("cartaproveedor01").
...
------------------------------------------------------------------------------*/
    ...
    CREATE "Word.Application" Word.
    Word:Documents:ADD("sobreproveedor01").
