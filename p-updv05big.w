...
Btn-Cancel Btn-Imprimir Btn-Salir 
...
DEFINE BUTTON Btn-Save 
     IMAGE-UP FILE "aplic/snowe2.1 icon 19.ico":U
     LABEL "&Grabar" 
     SIZE 8 BY 1.92
     FONT 4.
...
DEFINE FRAME Panel-Frame
     ...
     Btn-Reset AT ROW 1.19 COL 10
     Btn-Add AT ROW 1.19 COL 18
     Btn-Delete AT ROW 1.19 COL 25.86
     Btn-Cancel AT ROW 1.19 COL 33.86
     Btn-Imprimir AT ROW 1.19 COL 41.86
     Btn-Salir AT ROW 1.19 COL 49.86
...
ON CHOOSE OF Btn-Imprimir IN FRAME Panel-Frame /* Imprimir */
DO:
   RUN notify ('imprime':U).
END.
...
ON CHOOSE OF Btn-Reset IN FRAME Panel-Frame /* Reiniciar */
DO:
...
&ENDIF
&IF LOOKUP("Btn-Buscar":U, "{&ENABLED-OBJECTS}":U," ":U) NE 0 &THEN
             Btn-Buscar:SENSITIVE = NO.
&ENDIF
&IF LOOKUP("Btn-Imprimir":U, "{&ENABLED-OBJECTS}":U," ":U) NE 0 &THEN
             Btn-Imprimir:SENSITIVE = NO.
&ENDIF
...
&ENDIF
&IF LOOKUP("Btn-buscar":U, "{&ENABLED-OBJECTS}":U," ":U) NE 0 &THEN
             Btn-buscar:SENSITIVE = YES.
&ENDIF
&IF LOOKUP("Btn-Imprimir":U, "{&ENABLED-OBJECTS}":U," ":U) NE 0 &THEN
             Btn-Imprimir:SENSITIVE = YES.
&ENDIF
