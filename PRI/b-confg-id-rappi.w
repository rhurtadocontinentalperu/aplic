...
DEF VAR s-Tabla AS CHAR INIT 'IDRAPPI' NO-UNDO.
...
RUN PRI_Divisiones-Validas IN hProc (INPUT 2,
                                     OUTPUT x-Divisiones).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacTabla.Codigo COLUMN-LABEL "Tienda" FORMAT "x(8)":U
      GN-DIVI.DesDiv FORMAT "X(40)":U WIDTH 41.86
      FacTabla.Campo-C[1] COLUMN-LABEL "Tienda ID" FORMAT "x(8)":U
            WIDTH 9.72
  ENABLE
      FacTabla.Codigo
      FacTabla.Campo-C[1]
...
"FacTabla.Codigo" "Tienda" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.GN-DIVI.DesDiv
"GN-DIVI.DesDiv" ? ? "character" ? ? ? ? ? ? no ? no no "41.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.FacTabla.Campo-C[1]
"FacTabla.Campo-C[1]" "Tienda ID" ? "character" ? ? ? ? ? ? yes ? no no "9.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF FacTabla.Codigo IN BROWSE br_table /* Tienda */
DO:
...
IF LOOKUP(FacTabla.Codigo:SCREEN-VALUE IN BROWSE {&browse-name}, x-Divisiones) = 0
    THEN DO:
...
IF NOT CAN-FIND(FIRST gn-divi WHERE gn-divi.CodCia = s-codcia
            AND gn-div.coddiv = FacTabla.Codigo:SCREEN-VALUE IN BROWSE {&browse-name}
            NO-LOCK)
    THEN DO:
    MESSAGE 'División errada' VIEW-AS ALERT-BOX ERROR.
    APPLY 'ENTRY':U TO FacTabla.Codigo.
    RETURN 'ADM-ERROR'.
END.
/* NO hay modificación del CODIGO */
RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
