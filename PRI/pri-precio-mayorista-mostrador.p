...
RUN web_api_unit_price IN hProcPricing (INPUT s-CodMat,
                                        ...
                                        OUTPUT pcReturn,
                                        OUTPUT pcError).
/*MESSAGE "pcReturn: " STRING(pcReturn) SKIP "pcError:" STRING(pcError).*/
/* RETURN 'ADM-ERROR'.                                                    */
...
IF ENTRY(7,pcReturn,":") = "Y" THEN pPrecioContrato = DECIMAL(ENTRY(2,pcReturn,":")) NO-ERROR.
    ...
    WHEN pPrecioContrato <= 0 THEN DO:
        ...
        CASE s-FlgSit:
            WHEN 'T' THEN DO:       /* PAGO CON TARJETA DE CREDITO */
                ...
                RUN web_api-unit-cost-price IN hProc (INPUT s-CodMat,
                                                      INPUT "C",
                                                      INPUT "000",
                                                      INPUT s-CodCli,
                                                      INPUT s-CodDiv,
...
*/
                IF pcError > '' AND TRUE <> (pcReturn > '') THEN DO:
                    ...
                    RETURN 'ADM-ERROR'.
                END.
                LocalMonVta = s-CodMon.
                LocalTpoCmb = s-TpoCmb.
                x-ImporteCosto = 0.
                ASSIGN LocalMonVta      = INTEGER(ENTRY(2,pcReturn,':')) NO-ERROR.
                ASSIGN LocalTpoCmb      = DECIMAL(ENTRY(3,pcReturn,':')) NO-ERROR.
                ASSIGN x-ImporteCosto   = DECIMAL(ENTRY(5,pcReturn,':')) NO-ERROR.
