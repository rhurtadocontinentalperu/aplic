...
DEF VAR cFchIni AS CHAR.
DEF VAR cFchFin AS CHAR.
...
FOR EACH Detalle EXCLUSIVE-LOCK BREAK BY Detalle.Sku:
    IF FIRST-OF(Detalle.sku) THEN DO:
        ...
        FOR EACH VtaDctoProm NO-LOCK WHERE VtaDctoProm.CodCia = s-CodCia
            ...
            AND VtaDctoProm.CodMat = cCodMat
            ...
            IF VtaDctoProm.Descuento >= x-DctoPromocional THEN DO:
    ...
    IF x-DctoPromocional > 0 THEN DO:
        ASSIGN
            ...
            Detalle.Descuento = STRING(x-DctoPromocional).
    END.
    /**/
    IF LAST-OF(Detalle.sku) THEN DO:
        x-Old-Descuento = 0. 
        x-DctoPromocional = 0.
        cFchIni = "".
        cFchFin = "".
...
FOR EACH FacTabla NO-LOCK WHERE FacTabla.CodCia = s-codcia AND FacTabla.Tabla = 'IDRAPPI',
    ...
    RUN Carga-Temporal (INPUT Almacen.CodAlm, INPUT pPorStock, INPUT pStock, INPUT pMinimo).
END.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH Almmmate NO-LOCK WHERE Almmmate.codcia = s-codcia AND
        ...
        Almmmate.stkact > 0,
        FIRST Almmmatg OF Almmmate NO-LOCK WHERE Almmmatg.TpoArt = "A",
        FIRST Almtfami OF Almmmatg NO-LOCK WHERE Almtfami.SwComercial = YES,
        FIRST Almsfami OF Almmmatg NO-LOCK:
        x-StkAct = Almmmate.StkAct * pPorStock / 100.
        ...
        /* Pintamos cada 100 productos */
        IF x-Cuenta MODULO 100 = 0 THEN
            FILL-IN_DesMat:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "STOCK >> ALMACEN " + pCodAlm + " " + Almmmatg.codmat + " - " + Almmmatg.desmat.
...
RUN Graba-Texto (INPUT cCarpeta).

/* *************************************************************************************** */
/* *************************************************************************************** */
OUTPUT STREAM txt-trace CLOSE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Graba-Texto B-table-Win 
PROCEDURE Graba-Texto :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF INPUT PARAMETER cCarpeta AS CHAR.
