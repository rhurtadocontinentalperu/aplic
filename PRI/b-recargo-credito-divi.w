...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      FacTabla.Codigo COLUMN-LABEL "División" FORMAT "x(8)":U
      GN-DIVI.DesDiv FORMAT "X(40)":U
      FacTabla.Valor[1] COLUMN-LABEL "Recargo!S/ con IGV" FORMAT ">>9.9999":U
...
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 78 BY 6.69
         FONT 4
         TITLE "RECARGO POR DIVISIONES - VALOR POR DEFECTO".
...
"FacTabla.Codigo" "División" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.GN-DIVI.DesDiv
     _FldNameList[3]   > INTEGRAL.FacTabla.Valor[1]
"FacTabla.Valor[1]" "Recargo!S/ con IGV" ">>9.9999" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* RECARGO POR DIVISIONES - VALOR POR DEFECTO */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* RECARGO POR DIVISIONES - VALOR POR DEFECTO */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* RECARGO POR DIVISIONES - VALOR POR DEFECTO */
DO:
...
ON LEFT-MOUSE-DBLCLICK OF FacTabla.Codigo IN BROWSE br_table /* División */
OR F8 OF FacTabla.Codigo DO:
    ASSIGN
        input-var-1 = ''
        input-var-2 = ''
        input-var-3 = ''
...
------------------------------------------------------------------------------*/
  ...
  RUN lib/logtabla ("FacTabla",
                    FacTabla.Tabla + "|" + FacTabla.Codigo + "|" + STRING(FacTabla.Valor[1]),
                    pEvento).
...
------------------------------------------------------------------------------*/
  ...
  RUN lib/logtabla ("FacTabla",
                    FacTabla.Tabla + "|" + FacTabla.Codigo + "|" + STRING(FacTabla.Valor[1]),
                    "DELETE").
  FOR EACH Vtadtabla EXCLUSIVE-LOCK WHERE VtaDTabla.CodCia = FacTabla.codcia AND
      VtaDTabla.Tabla = FacTabla.tabla AND
      VtaDTabla.Llave = FacTabla.codigo:
...
IF RETURN-VALUE = "YES" THEN DO:
    ...
    IF CAN-FIND(FIRST b-FacTabla WHERE b-FacTabla.codcia = s-codcia AND
                      b-FacTabla.tabla = s-tabla AND
                      b-FacTabla.Codigo = FacTabla.Codigo:SCREEN-VALUE IN BROWSE {&browse-name}
                      NO-LOCK)
        THEN DO:
    ...
    FIND gn-divi WHERE gn-divi.codcia = s-codcia AND
        gn-divi.coddiv = FacTabla.Codigo:SCREEN-VALUE IN BROWSE {&browse-name}
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE gn-divi OR gn-divi.campo-log[1] = YES THEN DO:
        MESSAGE 'División no válida' VIEW-AS ALERT-BOX ERROR.
        APPLY 'ENTRY':U TO FacTabla.Codigo.
        RETURN 'ADM-ERROR'.
    END.
    DISPLAY GN-DIVI.DesDiv WITH BROWSE {&browse-name}.
END.
IF DECIMAL(FacTabla.Valor[1]:SCREEN-VALUE IN BROWSE {&Browse-name}) = 0
    THEN DO:
    MESSAGE 'El recargo lo está grabando con valor 0 (cero)' SKIP
        'Está seguro?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
        UPDATE rpta AS LOG.
    IF rpta = NO THEN DO:
...
------------------------------------------------------------------------------*/
  FIND LAST logtabla USE-INDEX Llave01 WHERE logtabla.codcia = s-codcia AND
      logtabla.tabla = "FacTabla" AND
      logtabla.valorllave BEGINS s-tabla + "|" + FacTabla.Codigo
      NO-LOCK NO-ERROR.
