...
/* BUSCAMOS EL PRECIO UNITARIO EN LA TABLA DE PRICING: RELACIONADO CON EL GRUPO-DIVISION */
FIND prigrupodivision WHERE prigrupodivision.CodCia = s-CodCia AND
    prigrupodivision.CodDiv = pCodDiv
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE prigrupodivision THEN DO:
    IF pError = YES THEN MESSAGE 'NO está configurada la Lista/División en el PRICING' VIEW-AS ALERT-BOX ERROR.
    RETURN "ADM-ERROR".
END.
FIND pricanalgrupo WHERE pricanalgrupo.CodCia = s-codcia AND 
    pricanalgrupo.Grupo = prigrupodivision.Grupo
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE pricanalgrupo THEN DO:
    IF pError = YES THEN MESSAGE 'NO está configurada la Lista/División en el PRICING' VIEW-AS ALERT-BOX ERROR.
    RETURN "ADM-ERROR".
END.
FIND prilistaprecios WHERE prilistaprecios.CodCia = s-codcia AND 
    prilistaprecios.Canal = pricanalgrupo.Canal AND
    prilistaprecios.Grupo = pricanalgrupo.Grupo AND
    prilistaprecios.CodMat = s-CodMat
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE prilistaprecios THEN DO:
    IF pError = YES THEN MESSAGE 'NO está configuradel precio en el PRICING' VIEW-AS ALERT-BOX ERROR.
    RETURN "ADM-ERROR".
END.
/* ********************************************************************************** */
...
ASSIGN
    f-Factor = 1
    F-PreBas = prilistaprecios.PreUni.
/* ********************************************************************************** */
...
IF prilistaprecios.UndBas > '' THEN ASSIGN s-UndVta = prilistaprecios.UndBas.
...
END CASE.
/* ********************************************** */
{pri/precio-de-venta.i &Tabla=VtaListaMay ~
    &PreVta=VtaListaMay.PreOfi ~
    &Promocional="~
