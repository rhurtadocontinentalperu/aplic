...
DEFINE VAR x-iniciando AS LOG INIT YES.
DEFINE VAR x-current-page AS INT INIT 0.
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'LABEL-FONT = 4,
                     ...
                     LABELS = Mayorista|Consulta de Precios|Eventos|Por División|Contrato Marco|Remates|UTILEX':U ,
             OUTPUT h_tab95 ).
...
------------------------------------------------------------------------------*/
  ...
  IF adm-current-page = 5 OR adm-current-page = 6 THEN DO:
      /* Dispatch standard ADM method.                             */
      RUN dispatch IN THIS-PROCEDURE ( INPUT 'change-page':U ) .
      x-current-page = adm-current-page.
  END.
  ELSE DO:
      IF x-iniciando = NO THEN DO:
          MESSAGE "Imposible entrar a dicha pestaña, pasara a ser obsoleta"
              VIEW-AS ALERT-BOX INFORMATION.
      END.
        IF x-current-page = 5 OR x-current-page = 6 THEN DO:
            RUN select-page IN THIS-PROCEDURE (x-current-page).
        END.                
      RETURN "ADM-ERROR".
  END.
