...
CASE TRUE:
    WHEN pPrecioContrato <= 0 THEN DO:      /* NO ES CONTRATO */
        ...
        /*
        MESSAGE "pCodDiv " pCodDiv SKIP
                "LogParteEscalera " LogParteEscalera SKIP
                "pSalesChannel " pSalesChannel SKIP
                "pMensaje " pMensaje.
        */
        IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN 'ADM-ERROR'.
...
DELETE PROCEDURE hProc.
/*
MESSAGE "s-CodMat " s-CodMat SKIP
        "pSalesChannel " pSalesChannel SKIP
        "clfclie " (IF Almmmatg.CHR__02 = "P" THEN x-ClfCli ELSE x-ClfCli2) SKIP
        "s-CndVta " s-CndVta SKIP
        "x-MonVta " x-MonVta SKIP
        "s-TpoCmb " s-TpoCmb SKIP
...
------------------------------------------------------------------------------*/
    ...
    RUN web_api-pricing-preuni IN hProc (INPUT s-CodMat,
                                         ...
                                         OUTPUT pMensaje).
    DELETE PROCEDURE hProc.
/* IF S-CODMAT = "045575" THEN                                                                 */
/*     MESSAGE PSALESCHANNEL (IF Almmmatg.CHR__02 = "P" THEN x-ClfCli ELSE x-ClfCli2) S-CNDVTA */
/*     F-PREVTA.                                                                               */
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        RETURN 'ADM-ERROR'.
    END.
