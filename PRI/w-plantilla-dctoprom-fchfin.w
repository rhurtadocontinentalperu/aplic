...
DEFINE NEW SHARED VARIABLE s-Tipo-Lista AS INTE.

ASSIGN
    s-Tipo-Lista = 1.   /* Listas Mayorista Continental */
...
/*                                                                               */
/* RADIO-SET-1 = s-Tipo-Lista.                                                   */
/* RUN PRI_Divisiones-Validas IN hProc (INPUT RADIO-SET-1, OUTPUT f-Divisiones). */
/*                                                                               */
...
COMBO-BOX-SubFam x-CodPro FILL-IN-NomPro x-DesMar 
...
DEFINE VARIABLE h_b-div-valida-xlista AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     RADIO-SET-1 AT ROW 3.15 COL 71 NO-LABEL WIDGET-ID 24
     ...
     BROWSE-2 AT ROW 13.65 COL 3 WIDGET-ID 200
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 109.86 BY 26.15
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "GENERACION DE PLANTILLA CAMBIO DE VENCIMIENTO"
         HEIGHT             = 26.15
         WIDTH              = 109.86
         MAX-HEIGHT         = 26.15
         MAX-WIDTH          = 191.29
...
ON END-ERROR OF W-Win /* GENERACION DE PLANTILLA CAMBIO DE VENCIMIENTO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* GENERACION DE PLANTILLA CAMBIO DE VENCIMIENTO */
DO:
...
DO:
    ASSIGN RADIO-SET-1.
    /* Solo interesa las divisiones en caso de RADIO-SET-1 > 1 */
    RUN Devuelve-Lista IN h_b-div-valida-xlista ( OUTPUT s-CodDiv /* CHARACTER */).
    IF RADIO-SET-1 > 1 AND TRUE <> (s-CodDiv > '') THEN RETURN NO-APPLY.
...
DO:
    ...
    IF TRUE <> (f-Divisiones > '') THEN DO:
        ...
        RETURN NO-APPLY.
    END.
    RUN Carga-Divisiones IN h_b-div-valida-xlista.
    RUN dispatch IN h_b-div-valida-xlista ('open-query':U).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pri/b-div-valida-xlista.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-div-valida-xlista ).
       RUN set-position IN h_b-div-valida-xlista ( 1.54 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-div-valida-xlista ( 6.69 , 66.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-div-valida-xlista ,
             RADIO-SET-1:HANDLE IN FRAME F-Main , 'BEFORE':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RADIO-SET-1 COMBO-BOX-CodFam COMBO-BOX-SubFam x-CodPro x-DesMar 
         BUTTON-1 BROWSE-2 
      WITH FRAME F-Main IN WINDOW W-Win.
...
ASSIGN
    chWorkSheet:Range("A1"):Value = "DCTO PROMOCIONAL - CAMBIO FECHA FIN"
    chWorkSheet:Range("A2"):Value = "DIVISION"
    chWorkSheet:Range("B2"):Value = "CODIGO"
    chWorkSheet:Columns("A"):NumberFormat = "@"
    chWorkSheet:Columns("B"):NumberFormat = "@"
    chWorkSheet:Range("C2"):Value = "DESCRIPCION"
    chWorkSheet:Range("D2"):Value = "LINEA"
    chWorkSheet:Columns("D"):NumberFormat = "@"
    chWorkSheet:Range("E2") = "DESDE"
    chWorkSheet:COLUMNS("E"):NumberFormat = "dd/MM/yyyy"
    chWorkSheet:Range("F1") = "ANTES"
    chWorkSheet:Range("F2") = "HASTA"
    chWorkSheet:COLUMNS("F"):NumberFormat = "dd/MM/yyyy"
    chWorkSheet:Range("G1") = "DESPUES"
    chWorkSheet:Range("G2") = "HASTA"
    chWorkSheet:COLUMNS("G"):NumberFormat = "dd/MM/yyyy"
    .
CASE RADIO-SET-1:
    WHEN 2 THEN chWorkSheet:Range("A1"):VALUE = chWorkSheet:Range("A1"):VALUE + " - MINORISTA".
    OTHERWISE chWorkSheet:Range("A1"):VALUE = chWorkSheet:Range("A1"):VALUE + " - MAYORISTA".
END CASE.
...
FOR EACH Almmmatg NO-LOCK WHERE {&Condicion}:
    ...
    /* VtaDctProm o VtaDctoPromMin */
    CASE RADIO-SET-1:
        WHEN 2 THEN DO:
            FOR EACH VtaDctoPromMin NO-LOCK WHERE VtaDctoPromMin.CodCia = s-CodCia AND
                LOOKUP(VtaDctoPromMin.CodDiv, f-Divisiones) > 0 AND
                VtaDctoPromMin.CodMat = Almmmatg.codmat AND
                (TODAY >= VtaDctoPromMin.FchIni AND TODAY <= VtaDctoPromMin.FchFin):
                FIND FIRST T-DPROM WHERE T-DPROM.codcia = VtaDctoPromMin.codcia
                    AND T-DPROM.coddiv = VtaDctoPromMin.coddiv
                    AND T-DPROM.codmat = VtaDctoPromMin.codmat
                    AND T-DPROM.fchini = VtaDctoPromMin.fchini
                    AND T-DPROM.fchfin = VtaDctoPromMin.fchfin
                    NO-ERROR.
                IF NOT AVAILABLE T-DPROM THEN DO:
                    CREATE T-DPROM.
                    BUFFER-COPY VtaDctoPromMin TO T-DPROM.
                END.
            END.
            FOR EACH VtaDctoPromMin NO-LOCK WHERE VtaDctoPromMin.CodCia = s-CodCia AND
                LOOKUP(VtaDctoPromMin.CodDiv, f-Divisiones) > 0 AND
                VtaDctoPromMin.CodMat = Almmmatg.codmat AND
                VtaDctoPromMin.FchIni > TODAY:
                FIND FIRST T-DPROM WHERE T-DPROM.codcia = VtaDctoPromMin.codcia
                    AND T-DPROM.coddiv = VtaDctoPromMin.coddiv
                    AND T-DPROM.codmat = VtaDctoPromMin.codmat
                    AND T-DPROM.fchini = VtaDctoPromMin.fchini
        ...
        OTHERWISE DO:
            FOR EACH VtaDctoProm NO-LOCK WHERE VtaDctoProm.CodCia = s-CodCia AND
                LOOKUP(VtaDctoProm.CodDiv, f-Divisiones) > 0 AND
                VtaDctoProm.CodMat = Almmmatg.codmat AND
                (TODAY >= VtaDctoProm.FchIni AND TODAY <= VtaDctoProm.FchFin):
                FIND FIRST T-DPROM WHERE T-DPROM.codcia = VtaDctoProm.codcia
                    AND T-DPROM.coddiv = VtaDctoProm.coddiv
                    AND T-DPROM.codmat = VtaDctoProm.codmat
                    AND T-DPROM.fchini = VtaDctoProm.fchini
                    AND T-DPROM.fchfin = VtaDctoProm.fchfin
                    NO-ERROR.
                IF NOT AVAILABLE T-DPROM THEN DO:
                    CREATE T-DPROM.
                    BUFFER-COPY VtaDctoProm TO T-DPROM.
                END.
            END.
            FOR EACH VtaDctoProm NO-LOCK WHERE VtaDctoProm.CodCia = s-CodCia AND
                LOOKUP(VtaDctoProm.CodDiv, f-Divisiones) > 0 AND
                VtaDctoProm.CodMat = Almmmatg.codmat AND
                VtaDctoProm.FchIni > TODAY:
                FIND FIRST T-DPROM WHERE T-DPROM.codcia = VtaDctoProm.codcia
                    AND T-DPROM.coddiv = VtaDctoProm.coddiv
                    AND T-DPROM.codmat = VtaDctoProm.codmat
                    AND T-DPROM.fchini = VtaDctoProm.fchini
        ...
        END.
    END CASE.
    FOR EACH T-DPROM NO-LOCK BREAK BY T-DPROM.FChIni BY T-DPROM.FchFin:
        ASSIGN
            t-Column = 0
            t-Row    = t-Row + 1.
        /* LISTA DE PRECIO */
        ASSIGN
            t-Column = t-Column + 1
            chWorkSheet:Cells(t-Row, t-Column):VALUE = T-DPROM.CodDiv.
        ASSIGN
            t-Column = t-Column + 1
        ...
        ASSIGN
            t-Column = t-Column + 1
            chWorkSheet:Cells(t-Row, t-Column):VALUE = T-DPROM.FchIni.
        ASSIGN
            t-Column = t-Column + 1
            chWorkSheet:Cells(t-Row, t-Column):VALUE = T-DPROM.FchFin.
        ASSIGN
            t-Column = t-Column + 1
            chWorkSheet:Cells(t-Row, t-Column):VALUE = T-DPROM.FchFin.
    END.
