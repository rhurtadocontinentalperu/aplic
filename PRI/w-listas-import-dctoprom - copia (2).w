...
&Scoped-Define DISPLAYED-OBJECTS txtCodMat 
...
DEFINE VARIABLE h_t-listas-import-dctoprom AS HANDLE NO-UNDO.
...
DO:
    ...
    IF h_t-listas-import-dctoprom <> ? THEN RUN dispatch IN h_t-listas-import-dctoprom ('open-query':U).
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'PRI/t-listas-import-dctoprom.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-listas-import-dctoprom ).
       RUN set-position IN h_t-listas-import-dctoprom ( 2.92 , 1.00 ) NO-ERROR.
       RUN set-size IN h_t-listas-import-dctoprom ( 19.04 , 86.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'PRI/t-listas-import-dctoprom-det.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
       ...
       RUN add-link IN adm-broker-hdl ( h_t-listas-import-dctoprom , 'Record':U , h_t-listas-import-dctoprom-det ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-listas-import-dctoprom ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_t-listas-import-dctoprom-det ,
             h_t-listas-import-dctoprom , 'AFTER':U ).
    END. /* Page 1 */
...
CASE RADIO-SET-1:
    ...
    WHEN 2 THEN DO:
        RUN vta2/dutilexdctoprom-02 (INPUT-OUTPUT x-Division, OUTPUT x-MetodoActualizacion).
        IF x-Division = '' THEN RETURN.
        RUN Pinta-Titulo IN h_t-listas-import-dctoprom ( "UTILEX - DESCUENTOS PROMOCIONALES - DIVISION ESPECIFICA" ).
    END.
END CASE.
/* ********************************** */
...
RUN dispatch IN h_t-listas-import-dctoprom ('open-query':U).
...
FOR EACH T-MATG NO-LOCK:
    FOR EACH T-DPROM WHERE T-DPROM.CodMat = T-MATG.CodMat
        AND T-DPROM.Descuento <> 0:
        RUN vtagn/p-margen-utilidad (
            T-MATG.CodMat,
            T-MATG.PreVta[1] * ( 1 - (T-DPROM.Descuento / 100) ),
            T-MATG.CHR__01,
            1,                      /* Moneda */
            T-MATG.TpoCmb,
            NO,                     /* Muestra error? */
...
FOR EACH T-MATG ON STOP UNDO, RETURN ON ERROR UNDO, RETURN:
    CASE x-MetodoActualizacion:
        WHEN 2 THEN DO:     /* Una división específica */
            ...
            FOR EACH T-DPROM NO-LOCK WHERE T-DPROM.CodCia = T-MATG.CodCia AND
                T-DPROM.CodDiv = f-Division AND 
                T-DPROM.CodMat = T-MATG.CodMat:
                FOR EACH VtaDctoProm EXCLUSIVE-LOCK WHERE VtaDctoProm.CodCia = T-DPROM.CodCia AND
                ...
                END.
            END.
/*             FOR EACH VtaDctoProm EXCLUSIVE-LOCK WHERE VtaDctoProm.CodCia = T-MATG.CodCia AND */
/*                 VtaDctoProm.CodDiv = f-Division AND                                          */
/*                 VtaDctoProm.CodMat = T-MATG.CodMat AND                                       */
...
/*             END.                                                                             */
            ...
            FOR EACH T-DPROM WHERE T-DPROM.CodMat = T-MATG.CodMat
                ...
                BUFFER-COPY T-DPROM
                    ...
                    VtaDctoProm.Precio = ROUND(T-MATG.PreVta[1] * (1 - (T-DPROM.Descuento / 100)), 4)
                    VtaDctoProm.UsrCreacion = s-user-id.
            END.
        END.
        OTHERWISE DO:       /* Todas las divisiones */
            /* Limpiamos informacion ENTRE FECHAS ACTIVAS */
            FOR EACH T-DPROM NO-LOCK WHERE T-DPROM.CodCia = T-MATG.CodCia AND
                T-DPROM.CodMat = T-MATG.CodMat:
                FOR EACH VtaDctoProm EXCLUSIVE-LOCK WHERE VtaDctoProm.CodCia = T-DPROM.CodCia AND
                    VtaDctoProm.CodDiv = T-DPROM.CodDiv AND
            ...
            END.
/*             FOR EACH T-DPROM WHERE T-DPROM.CodMat = T-MATG.codmat:                    */
/*                 FOR EACH VtaDctoProm WHERE VtaDctoProm.codcia = s-codcia              */
/*                     AND VtaDctoProm.CodDiv = T-DPROM.CodDiv                           */
/*                     AND VtaDctoProm.CodMat = T-DPROM.CodMat                           */
