...
DEFINE NEW SHARED TEMP-TABLE T-DVOL LIKE VtaDctoVol.
...
DEFINE VARIABLE h_b-eventos-import-dctovol AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv97-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-eventos-import-dctovol AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 150.86 BY 21.42 WIDGET-ID 100.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      ADDITIONAL-FIELDS:
          INDEX Idx00 AS PRIMARY CodMat FchPrmD
      END-FIELDS.
      TABLE: T-DVOL T "NEW SHARED" ? INTEGRAL VtaDctoVol
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CARGAR DESCUENTOS POR VOLUMEN - SOLO EVENTOS"
         HEIGHT             = 21.42
         WIDTH              = 150.86
         MAX-HEIGHT         = 27.15
         MAX-WIDTH          = 194.86
...
ON END-ERROR OF W-Win /* CARGAR DESCUENTOS POR VOLUMEN - SOLO EVENTOS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* CARGAR DESCUENTOS POR VOLUMEN - SOLO EVENTOS */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'pri/b-listas-import-dctovol.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-eventos-import-dctovol ).
       RUN set-position IN h_b-eventos-import-dctovol ( 3.15 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-eventos-import-dctovol ( 19.04 , 108.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'pri/v-listas-import-dctovol.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-eventos-import-dctovol ).
       RUN set-position IN h_v-eventos-import-dctovol ( 3.15 , 109.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv97-2 ).
       RUN set-position IN h_p-updv97-2 ( 14.19 , 110.00 ) NO-ERROR.
       RUN set-size IN h_p-updv97-2 ( 1.54 , 34.00 ) NO-ERROR.

       /* Links to SmartViewer h_v-eventos-import-dctovol. */
       RUN add-link IN adm-broker-hdl ( h_b-eventos-import-dctovol , 'Record':U , h_v-eventos-import-dctovol ).
       RUN add-link IN adm-broker-hdl ( h_p-updv97-2 , 'TableIO':U , h_v-eventos-import-dctovol ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-eventos-import-dctovol ,
             BUTTON-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-eventos-import-dctovol ,
             h_b-eventos-import-dctovol , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv97-2 ,
             h_v-eventos-import-dctovol , 'AFTER':U ).
    END. /* Page 0 */
...
IF NOT cValue BEGINS (COMBO-BOX-Division + " - DESCUENTOS POR VOLUMEN") THEN DO:
    pMensaje = 'Formato del archivo Excel errado'.
    RETURN.
END.
cNombreLista = cValue.
...
CASE TRUE:
    ...
    OTHERWISE RETURN.
END CASE.
RUN Pinta-Titulo IN h_b-eventos-import-dctovol ( INPUT cNombreLista /* CHARACTER */).
...
RUN dispatch IN h_b-eventos-import-dctovol ('open-query':U).
...
/* *************************************************************************** */
/* *************************************************************************** */
FOR EACH T-MATG, EACH T-DVOL NO-LOCK WHERE T-DVOL.CodMat = T-MATG.CodMat
    ON STOP UNDO, RETURN ON ERROR UNDO, RETURN:
    /* Limpiamos informacion */
    FOR EACH VtaDctoVol EXCLUSIVE-LOCK WHERE VtaDctoVol.CodCia = s-CodCia AND
        VtaDctoVol.CodDiv = T-DVOL.CodDiv AND
        VtaDctoVol.CodMat = T-DVOL.CodMat AND
...
FOR EACH T-MATG, EACH T-DVOL NO-LOCK WHERE T-DVOL.CodMat = T-MATG.CodMat
    ...
    CREATE VtaDctoVol.
    ASSIGN
        VtaDctoVol.CodCia = T-MATG.CodCia
        VtaDctoVol.CodMat = T-MATG.CodMat
        VtaDctoVol.CodDiv = T-DVOL.CodDiv
        VtaDctoVol.FchIni = T-DVOL.FchIni
        VtaDctoVol.FchFin = T-DVOL.FchFin
        VtaDctoVol.FchCreacion = TODAY
        VtaDctoVol.UsrCreacion = s-user-id
        VtaDctoVol.HoraCreacion = STRING(TIME, 'HH:MM:SS').
    /* TODO EN SOLES */
    x-PreUni = T-MATG.PreVta[1].
    IF T-MATG.MonVta = 2 THEN x-PreUni = x-PreUni * T-MATG.TpoCmb.
    DO k = 1 TO 10:
        ASSIGN
            VtaDctoVol.DtoVolR[k] = 0
            VtaDctoVol.DtoVolD[k] = 0
            VtaDctoVol.DtoVolP[k] = 0.
    END.
    /* Cargamos informacion */
    DO k = 1 TO 10:
        ASSIGN
            VtaDctoVol.DtoVolR[k] = T-DVOL.DtoVolR[k]
            VtaDctoVol.DtoVolD[k] = T-DVOL.DtoVolD[k]
            VtaDctoVol.DtoVolP[k] = ROUND(x-PreUni * (1 - (VtaDctoVol.DtoVolD[k] / 100)), 4).
    END.
    DO k = 1 TO 10:
        IF VtaDctoVol.DtoVolR[k] = 0 OR VtaDctoVol.DtoVolD[k] = 0
        THEN ASSIGN
                VtaDctoVol.DtoVolR[k] = 0
                VtaDctoVol.DtoVolD[k] = 0
                VtaDctoVol.DtoVolP[k] = 0.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      FOR EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-codcia AND
          ...
          GN-DIVI.VentaMayorista = 2:   /* Lista por División */
