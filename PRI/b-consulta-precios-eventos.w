...
Almmmatg.DesMar (IF Almmmatg.MonVta = 1 THEN 'S/' ELSE 'US$') @ x-Moneda ~
VtaListaMay.PreOfi Almmmatg.Chr__01 
...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaListaMay OF GN-DIVI WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      ...
      WHERE ( TRUE <> (x-Lineas > "") OR LOOKUP(Almmmatg.codfam, x-Lineas) > 0 ) NO-LOCK ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      (IF Almmmatg.MonVta = 1 THEN 'S/' ELSE 'US$') @ x-Moneda COLUMN-LABEL "Moneda"
      VtaListaMay.PreOfi COLUMN-LABEL "Precio Unitaro" FORMAT ">,>>>,>>9.9999":U
      Almmmatg.Chr__01 COLUMN-LABEL "Unidad" FORMAT "X(8)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 94 BY 15.08
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[2]         = "( TRUE <> (x-Lineas > """") OR LOOKUP(Almmmatg.codfam, x-Lineas) > 0 )"
     _FldNameList[1]   > INTEGRAL.VtaListaMay.codmat
"VtaListaMay.codmat" "Articulo" ? "character" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"(IF Almmmatg.MonVta = 1 THEN 'S/' ELSE 'US$') @ x-Moneda" "Moneda" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > INTEGRAL.VtaListaMay.PreOfi
"VtaListaMay.PreOfi" "Precio Unitaro" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
