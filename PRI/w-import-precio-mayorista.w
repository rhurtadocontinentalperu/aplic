...
DEF VAR x-radio-set-1 AS CHAR NO-UNDO.
...
DEFINE VAR z-tabla AS CHAR INIT "MMLX".
...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-Division FILL-IN-codmat 
...
DEFINE VARIABLE FILL-IN-codmat AS CHARACTER FORMAT "X(60)":U 
     LABEL "Articulo" 
      VIEW-AS TEXT 
...
DEFINE FRAME F-Main
     ...
     FILL-IN-codmat AT ROW 19.19 COL 78 COLON-ALIGNED WIDGET-ID 12
     RECT-2 AT ROW 1 COL 1 WIDGET-ID 10
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 142.43 BY 19.19 WIDGET-ID 100.
...
DO:
    MESSAGE 'Procedemos a grabar los precios ' + x-radio-set-1 + '?'
    ...
    CASE RADIO-SET-1:
        WHEN 1 THEN DO:
            SESSION:SET-WAIT-STATE('GENERAL').
            RUN Grabar-Precios-Conti (OUTPUT pMensaje).
            SESSION:SET-WAIT-STATE('').
        ...
        WHEN 2 THEN DO:
            SESSION:SET-WAIT-STATE('GENERAL').
            RUN Grabar-Precios-Utilex (OUTPUT pMensaje).
            SESSION:SET-WAIT-STATE('').
            IF pMensaje > '' THEN MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.
        ...
        WHEN 3 THEN DO:
            SESSION:SET-WAIT-STATE('GENERAL').
            RUN Grabar-Precios-Division (OUTPUT pMensaje).
            SESSION:SET-WAIT-STATE('').
            IF pMensaje > '' THEN MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.
        ...
        WHEN 4 THEN DO:
            SESSION:SET-WAIT-STATE('GENERAL').
            RUN Grabar-Precios-Marco (OUTPUT pMensaje).
            SESSION:SET-WAIT-STATE('').
            IF pMensaje > '' THEN MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.
        ...
        WHEN 5 THEN DO:
            SESSION:SET-WAIT-STATE('GENERAL').
            RUN Grabar-Precios-Remate (OUTPUT pMensaje).
            SESSION:SET-WAIT-STATE('').
            IF pMensaje > '' THEN MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Incosistencias W-Win 
PROCEDURE Carga-Incosistencias :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH T-MATG EXCLUSIVE-LOCK:
        ...
        /*
            Ic - 25Mar2022, validar que no este en la tabla de excepciones
        */

        FIND FIRST x-vtatabla WHERE x-vtatabla.codcia = s-codcia AND 
                                    x-vtatabla.tabla = z-tabla AND 
                                    x-vtatabla.llave_c1 = Almmmatg.codmat NO-LOCK NO-ERROR.
        ...
        IF T-MATG.MrgUti-C <> 0 AND T-MATG.MrgUti-C > T-MATG.MrgUti-B THEN DO:
            CREATE E-MATG.
            BUFFER-COPY T-MATG 
                TO E-MATG
            ...
            NEXT.
        END.
        IF T-MATG.MrgUti-B <> 0 AND T-MATG.MrgUti-B > T-MATG.MrgUti-A THEN DO:
            CREATE E-MATG.
            BUFFER-COPY T-MATG 
                TO E-MATG
                ASSIGN E-MATG.Libre_c01 = "Margen de util. B " + string(T-MATG.MrgUti-B) + " as mayor que el margen de util. A " + STRING(T-MATG.MrgUti-A).
            DELETE T-MATG.
            NEXT.
        END.
    END.
...
CASE RADIO-SET-1:
    WHEN 1 THEN x-radio-set-1 = "Carga-Temporal-Conti".
    WHEN 2 THEN x-radio-set-1 = "Carga-Temporal-Utilex".
    WHEN 3 THEN x-radio-set-1 = "Carga-Temporal-Division".
    WHEN 4 THEN x-radio-set-1 = "Carga-Temporal-Marco".
    WHEN 5 THEN x-radio-set-1 = "Carga-Temporal-Remate".
END CASE.
...
FOR EACH T-MATG, 
    FIRST Almmmatg WHERE Almmmatg.codcia = s-codcia AND Almmmatg.codmat = T-MATG.codmat NO-LOCK:
    ASSIGN
    ...
    /* Revisa como COSTOS */
    IF Almmmatg.MonVta = T-MATG.MonVta THEN
        ASSIGN T-MATG.CtoLis = Almmmatg.CtoLis T-MATG.CtoTot = Almmmatg.CtoTot.
    ELSE IF T-MATG.MonVta = 2 THEN ASSIGN T-MATG.CtoLis = Almmmatg.CtoLis / Almmmatg.TpoCmb T-MATG.CtoTot = Almmmatg.CtoTot / Almmmatg.TpoCmb.
    ELSE ASSIGN T-MATG.CtoLis = Almmmatg.CtoLis * Almmmatg.TpoCmb T-MATG.CtoTot = Almmmatg.CtoTot * Almmmatg.TpoCmb.
END.

/* 28/04/2022 Consistencia final */
FOR EACH T-MATG, FIRST Almmmatg OF T-MATG NO-LOCK:
...
END.
FOR EACH T-MATG:
    IF NOT CAN-FIND(FIRST Almmmatg OF T-MATG NO-LOCK) THEN DELETE T-MATG.
END.
...
FOR EACH T-MATG, FIRST Almmmatg OF T-MATG NO-LOCK:
    FIND Almtconv WHERE Almtconv.CodUnid  = Almmmatg.UndBas 
        AND Almtconv.Codalter = T-MATG.UndAlt[1]
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almtconv OR Almtconv.Equival = 0 THEN DELETE T-MATG.
END.
FOR EACH T-MATG:
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-Division FILL-IN-codmat 
...
/* Limitamos Transacción */
/* ****************************************************************************************** */
RUN Carga-Incosistencias.
/* ****************************************************************************************** */
...
FOR EACH T-MATG EXCLUSIVE-LOCK ON STOP UNDO, RETURN ON ERROR UNDO, RETURN:
    ...
    IF Almmmatg.MonVta = Almmmatg.DsctoProm[1] THEN     /* MOneda LP vs Moneda de Compra */
        ASSIGN Almmmatg.CtoTot = Almmmatg.DsctoProm[2].
    ELSE IF Almmmatg.MonVta = 1 THEN
        ASSIGN Almmmatg.CtoTot = Almmmatg.DsctoProm[2] * Almmmatg.TpoCmb.
    ELSE ASSIGN Almmmatg.CtoTot = Almmmatg.DsctoProm[2] / Almmmatg.TpoCmb.
    ASSIGN 
    ...
    /* ****************************************************************************************************** */
    IF Almmmatg.Prevta[1] > 0 THEN DO:
        F-Factor = 1.
        FIND Almtconv WHERE Almtconv.CodUnid = Almmmatg.UndBas AND Almtconv.Codalter = Almmmatg.UndStk NO-LOCK NO-ERROR.
        IF AVAILABLE Almtconv THEN F-FACTOR = Almtconv.Equival.
        x-PreUni = Almmmatg.PreVta[1] / f-Factor.
        RUN Verifica-Margen (INPUT x-PreUni, OUTPUT pError).
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            CREATE E-MATG.
            BUFFER-COPY Almmmatg TO E-MATG
                ASSIGN E-MATG.Libre_c01 = 'Precio de Lista: ' + pError.
            UNDO RLOOP, NEXT RLOOP.
        END.
        IF pError > '' THEN DO:
            CREATE A-MATG.
            BUFFER-COPY Almmmatg TO A-MATG
    ...
    IF Almmmatg.Prevta[2] > 0 THEN DO:
        ...
        IF AVAILABLE Almtconv THEN F-FACTOR = Almtconv.Equival.
        x-PreUni = Almmmatg.PreVta[2] / f-Factor.
        RUN Verifica-Margen (INPUT x-PreUni, OUTPUT pError).
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            CREATE E-MATG.
            BUFFER-COPY Almmmatg TO E-MATG
    ...
    IF Almmmatg.Prevta[3] > 0 THEN DO:
        F-Factor = 1.
        FIND Almtconv WHERE Almtconv.CodUnid = Almmmatg.UndBas AND Almtconv.Codalter = Almmmatg.UndB NO-LOCK NO-ERROR.
        IF AVAILABLE Almtconv THEN F-FACTOR = Almtconv.Equival.
        x-PreUni = Almmmatg.PreVta[3] / f-Factor.
        RUN Verifica-Margen (INPUT x-PreUni, OUTPUT pError).
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            CREATE E-MATG.
            BUFFER-COPY Almmmatg TO E-MATG
    ...
    IF Almmmatg.Prevta[4] > 0 THEN DO:
        F-Factor = 1.
        FIND Almtconv WHERE Almtconv.CodUnid = Almmmatg.UndBas AND Almtconv.Codalter = Almmmatg.UndC NO-LOCK NO-ERROR.
        IF AVAILABLE Almtconv THEN F-FACTOR = Almtconv.Equival.
        x-PreUni = Almmmatg.PreVta[4] / f-Factor.
        RUN Verifica-Margen (INPUT x-PreUni, OUTPUT pError).
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            CREATE E-MATG.
            BUFFER-COPY Almmmatg TO E-MATG
...
/* 1ro. Calculamos el margen de utilidad */
DEFINE VAR hProc AS HANDLE NO-UNDO.
RUN pri/pri-librerias.r PERSISTENT SET hProc.

FOR EACH T-MATG EXCLUSIVE-LOCK, 
    FIRST Almmmatg OF T-MATG NO-LOCK ON ERROR UNDO, RETURN ON STOP UNDO, RETURN:
    /* Por Precio de Lista */
    fill-in-codmat:SCREEN-VALUE IN FRAME {&FRAME-NAME}= T-MATG.CodMat + " INI".
    ...
    x-conteo = x-conteo + 1.
    fill-in-codmat:SCREEN-VALUE IN FRAME {&FRAME-NAME}= string(x-conteo) + ".- " + T-MATG.CodMat + " OK".
...
------------------------------------------------------------------------------*/
    ...
    IF pError > '' THEN DO:
        ...
        /* 2do. Verificamos si solo es una ALERTA, definido por GG */
        DEF VAR pAlerta AS LOG NO-UNDO.
        RUN PRI_Alerta-de-Margen IN hProc (INPUT Almmmatg.codmat, OUTPUT pAlerta).
