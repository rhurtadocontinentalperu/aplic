...
DEF NEW SHARED VAR s-Tabla AS CHAR INIT "REGALOS_POR_ESCALA".
...
DEFINE VARIABLE h_b-scale AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-scale-gift AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_f-scale-gift-import-excel AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-gift-for-scale-question AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CONFIGURACION GENERAL DE REGALOS POR ESCALA"
         HEIGHT             = 26.69
         WIDTH              = 152
         MAX-HEIGHT         = 27.23
         MAX-WIDTH          = 152
...
ON END-ERROR OF W-Win /* CONFIGURACION GENERAL DE REGALOS POR ESCALA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* CONFIGURACION GENERAL DE REGALOS POR ESCALA */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Condición|Tiendas|Regalos|Excepciones' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pri/b-scale.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-scale ).
       RUN set-position IN h_b-scale ( 7.19 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-scale ( 6.69 , 30.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pri/b-scale-gift.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-scale-gift ).
       RUN set-position IN h_b-scale-gift ( 7.19 , 35.00 ) NO-ERROR.
       RUN set-size IN h_b-scale-gift ( 14.00 , 82.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pri/v-gift-for-scale-question.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-gift-for-scale-question ).
       RUN set-position IN h_v-gift-for-scale-question ( 7.73 , 124.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv96-3 ).
       RUN set-position IN h_p-updv96-3 ( 10.15 , 122.00 ) NO-ERROR.
       RUN set-size IN h_p-updv96-3 ( 1.54 , 26.14 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pri/f-scale-gift-import-excel.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_f-scale-gift-import-excel ).
       RUN set-position IN h_f-scale-gift-import-excel ( 13.38 , 130.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.92 , 8.14 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv100 ).
       RUN set-position IN h_p-updv100 ( 13.92 , 4.00 ) NO-ERROR.
       RUN set-size IN h_p-updv100 ( 1.42 , 30.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv10 ).
       RUN set-position IN h_p-updv10 ( 21.19 , 35.00 ) NO-ERROR.
       RUN set-size IN h_p-updv10 ( 1.42 , 41.72 ) NO-ERROR.

       /* Links to SmartBrowser h_b-scale. */
       RUN add-link IN adm-broker-hdl ( h_p-updv100 , 'TableIO':U , h_b-scale ).
       RUN add-link IN adm-broker-hdl ( h_qvtactabla , 'Record':U , h_b-scale ).

       /* Links to SmartBrowser h_b-scale-gift. */
       RUN add-link IN adm-broker-hdl ( h_b-scale , 'Record':U , h_b-scale-gift ).
       RUN add-link IN adm-broker-hdl ( h_p-updv10 , 'TableIO':U , h_b-scale-gift ).

       /* Links to SmartViewer h_v-gift-for-scale-question. */
       RUN add-link IN adm-broker-hdl ( h_p-updv96-3 , 'TableIO':U , h_v-gift-for-scale-question ).
       RUN add-link IN adm-broker-hdl ( h_qvtactabla , 'Record':U , h_v-gift-for-scale-question ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-scale ,
             BUTTON-Selecciona-Tdas:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-scale-gift ,
             h_b-scale , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-gift-for-scale-question ,
             h_b-scale-gift , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv96-3 ,
             h_v-gift-for-scale-question , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-scale-gift-import-excel ,
             h_p-updv96-3 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv100 ,
             h_f-scale-gift-import-excel , 'AFTER':U ).
...
CASE pParam:
    WHEN "Import-Excel-Regalos" THEN DO:
        RUN Import-Excel-Regalos IN h_b-scale-gift.
    END.
END CASE.
