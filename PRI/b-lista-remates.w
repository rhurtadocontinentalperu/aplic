...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaTabla WHERE ~{&KEY-PHRASE} ~
 ...
 AND ( TRUE <> (FILL-IN-CodMat> '') OR VtaTabla.Llave_c1 = FILL-IN-CodMat ) NO-LOCK, ~
...
&Scoped-Define ENABLED-OBJECTS BUTTON-Filtrar BUTTON-8 COMBO-BOX-Linea ~
COMBO-BOX-Sublinea BUTTON-NoFiltrar FILL-IN-CodMat FILL-IN-CodPro br_table 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-Linea COMBO-BOX-Sublinea ~
FILL-IN-CodMat FILL-IN-CodPro FILL-IN-NomPro 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 120 BY 18.85
...
DEFINE FRAME F-Main
     ...
     COMBO-BOX-Linea AT ROW 1.27 COL 12 COLON-ALIGNED WIDGET-ID 22
     COMBO-BOX-Sublinea AT ROW 2.23 COL 12 COLON-ALIGNED WIDGET-ID 24
     BUTTON-NoFiltrar AT ROW 2.35 COL 94 WIDGET-ID 12
     FILL-IN-CodMat AT ROW 3.15 COL 12 COLON-ALIGNED WIDGET-ID 18
...
/* Query rebuild information for BROWSE br_table
 ...
 AND ( TRUE <> (FILL-IN-CodMat> '') OR VtaTabla.Llave_c1 = FILL-IN-CodMat )"
...
"fMoneda() @ x-Moneda" "Moneda!LP" "x(3)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaTabla.Valor[1]" "Precio Unitario" ">>>,>>9.999999" "decimal" 11 9 ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  IF FILL-IN-CodMat > '' 
      AND CAN-FIND(FIRST Almmmatg WHERE Almmmatg.codcia = s-codcia
                   AND Almmmatg.codmat = FILL-IN-CodMat NO-LOCK)
      AND CAN-FIND(FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia
                   AND VtaTabla.Tabla = s-Tabla
                   AND VtaTabla.Llave_c1 = FILL-IN-CodMat NO-LOCK)
      THEN DO:
      ...
      ASSIGN
          ...
          FILL-IN-NomPro = ''.
      DISPLAY
          COMBO-BOX-Linea COMBO-BOX-Sublinea FILL-IN-CodPro FILL-IN-NomPro
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      FOR EACH B-TABLA NO-LOCK WHERE B-TABLA.codcia = s-codcia
          ...
          FIRST Almtfami NO-LOCK WHERE Almtfami.codcia = s-codcia
          AND Almtfami.codfam = B-TABLA.llave_c2:
