...
DEF VAR s-Tabla AS CHAR INIT 'FLETEXARTICULO' NO-UNDO.
...
Almmmatg.UndBas VtaTabla.Valor[1] 
...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaTabla WHERE VtaTabla.CodCia = GN-DIVI.CodCia ~
  ...
  AND Almmmatg.codmat = VtaTabla.Llave_c2 ~
      AND LOOKUP(Almmmatg.codfam, s-Lineas-Validas) > 0 NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c2 COLUMN-LABEL "Artículo" FORMAT "x(8)":U
      Almmmatg.DesMat FORMAT "X(60)":U
      Almmmatg.UndBas COLUMN-LABEL "Unidad" FORMAT "X(8)":U
      VtaTabla.Valor[1] COLUMN-LABEL "Flete Unitario (S/)" FORMAT ">>>,>>>,>>9.999999":U
  ENABLE
      VtaTabla.Llave_c2
      VtaTabla.Valor[1]
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 76 BY 15.88
         FONT 4.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaTabla WHERE INTEGRAL.GN-DIVI <external> ...,INTEGRAL.Almmmatg WHERE INTEGRAL.VtaTabla ..."
  ...
  AND Almmmatg.codmat = VtaTabla.Llave_c2"
     _Where[2]         = "LOOKUP(Almmmatg.codfam, s-Lineas-Validas) > 0"
     _FldNameList[1]   > INTEGRAL.VtaTabla.Llave_c2
"VtaTabla.Llave_c2" "Artículo" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaTabla.Valor[1]" "Flete Unitario (S/)" ">>>,>>>,>>9.999999" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  FIND B-MATG WHERE B-MATG.CodCia = s-CodCia
      AND B-MATG.CodMat = VtaTabla.Llave_c2:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
      NO-LOCK NO-ERROR.
  IF NOT AVAILABLE B-MATG THEN DO:
      ...
      RETURN 'ADM-ERROR'.
  END.
  IF LOOKUP(B-MATG.CodFam, s-Lineas-Validas) = 0 THEN DO:
      MESSAGE 'El artículo pertenece a la línea' B-MATG.CodFam SKIP
          'Usted NO tiene autorización a usar esta línea de producto'
