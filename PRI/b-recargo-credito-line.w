...
&Scoped-define QUERY-STRING-br_table FOR EACH VtaDTabla WHERE VtaDTabla.CodCia = FacTabla.CodCia ~
  AND VtaDTabla.Tabla = FacTabla.Tabla ~
  AND VtaDTabla.Llave = FacTabla.Codigo NO-LOCK, ~
  ...
  AND AlmSFami.subfam = VtaDTabla.Libre_c01 OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH VtaDTabla WHERE VtaDTabla.CodCia = FacTabla.CodCia ~
  AND VtaDTabla.Tabla = FacTabla.Tabla ~
  AND VtaDTabla.Llave = FacTabla.Codigo NO-LOCK, ~
      FIRST Almtfami WHERE Almtfami.CodCia = VtaDTabla.CodCia ~
  AND Almtfami.codfam = VtaDTabla.LlaveDetalle NO-LOCK, ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      Almtfami.desfam FORMAT "X(30)":U WIDTH 26.86
      VtaDTabla.Libre_c01 COLUMN-LABEL "Sub-Línea" FORMAT "x(8)":U
      AlmSFami.dessub FORMAT "X(30)":U WIDTH 31
      VtaDTabla.Libre_d01 COLUMN-LABEL "Recargo!S/ con IGV" FORMAT ">>9.9999":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 115 BY 6.69
         FONT 4
         TITLE "EXCLUSIONES POR LINEA/SUB-LINEA".
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.VtaDTabla WHERE INTEGRAL.FacTabla <external> ...,INTEGRAL.Almtfami WHERE INTEGRAL.VtaDTabla ...,INTEGRAL.AlmSFami WHERE INTEGRAL.VtaDTabla ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST USED, FIRST OUTER USED,"
     _JoinCode[1]      = "VtaDTabla.CodCia = FacTabla.CodCia
  AND VtaDTabla.Tabla = FacTabla.Tabla
  AND VtaDTabla.Llave = FacTabla.Codigo"
...
"Almtfami.desfam" ? ? "character" ? ? ? ? ? ? no ? no no "26.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > INTEGRAL.VtaDTabla.Libre_c01
"VtaDTabla.Libre_c01" "Sub-Línea" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"VtaDTabla.Libre_d01" "Recargo!S/ con IGV" ">>9.9999" "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* EXCLUSIONES POR LINEA/SUB-LINEA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* EXCLUSIONES POR LINEA/SUB-LINEA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* EXCLUSIONES POR LINEA/SUB-LINEA */
DO:
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  ASSIGN
      VtaDTabla.CodCia =  FacTabla.CodCia
      VtaDTabla.Tabla = FacTabla.Tabla
...
IF RETURN-VALUE = "YES" THEN DO:
    ...
    IF CAN-FIND(FIRST b-vtadtabla WHERE b-vtadtabla.codcia = s-codcia AND
                b-vtadtabla.tabla = FacTabla.tabla AND
                b-vtadtabla.llave = FacTabla.Codigo AND
