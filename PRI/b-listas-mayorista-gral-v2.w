...
T-MATG.UndStk T-MATG.DesMar T-MATG.dsctoprom[2] ~
( IF T-MATG.DsctoProm[1] = 1 THEN  'Soles'  ELSE 'Dolares' ) @ x-MonCmp ~
...
T-MATG.Prevta[3] T-MATG.MrgUti-C T-MATG.Prevta[4] 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      T-MATG.dsctoprom[2] COLUMN-LABEL "Costo" FORMAT "->>>,>>9.9999":U
            COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 8
      ( IF T-MATG.DsctoProm[1] = 1 THEN  'Soles'  ELSE 'Dolares' ) @ x-MonCmp COLUMN-LABEL "Moneda!Compra"
            COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 8
...
"T-MATG.DesMar" "Marca" "X(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.T-MATG.dsctoprom[2]
"T-MATG.dsctoprom[2]" "Costo" "->>>,>>9.9999" "decimal" 8 0 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"( IF T-MATG.DsctoProm[1] = 1 THEN  'Soles'  ELSE 'Dolares' ) @ x-MonCmp" "Moneda!Compra" ? ? 8 0 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  /* RHC 21/07/2021 Rutina en caso cambie la moneda de la lista de precios */
  IF INTEGER(T-MATG.MonVta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) <> Almmmatg.MonVta THEN DO:
      MESSAGE 'Se pretende cambiar la moneda de la lista de precios' SKIP
          'Continuamos?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE rpta AS LOG.
      IF rpta = NO THEN DO:
          T-MATG.MonVta:SCREEN-VALUE = STRING(Almmmatg.MonVta).
          RETURN NO-APPLY.
      END.
      /* Al cambiar la moneda de venta cambian los siguientes valores */
      IF INTEGER(T-MATG.MonVta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}) = 1 THEN DO:
          /* SOLES */
          DISPLAY 
              Almmmatg.CtoTot * Almmmatg.TpoCmb @ T-MATG.CtoTot 
              Almmmatg.CtoTotMarco * Almmmatg.TpoCmb @ T-MATG.CtoTotMarco
              Almmmatg.PreVta[1] * Almmmatg.TpoCmb @ T-MATG.Prevta[1]
              Almmmatg.PreVta[2] * Almmmatg.TpoCmb @ T-MATG.Prevta[2]
              Almmmatg.PreVta[3] * Almmmatg.TpoCmb @ T-MATG.Prevta[3]
              Almmmatg.PreVta[4] * Almmmatg.TpoCmb @ T-MATG.Prevta[4]
      ...
      ELSE DO:
          ...
          DISPLAY 
              ...
              WITH BROWSE {&BROWSE-NAME}.
      END.
      APPLY 'LEAVE':U TO T-MATG.Prevta[1].
      APPLY 'LEAVE':U TO T-MATG.Prevta[2].
      APPLY 'LEAVE':U TO T-MATG.Prevta[3].
      APPLY 'LEAVE':U TO T-MATG.Prevta[4].
      T-MATG.MOnVta:READ-ONLY = YES.
  END.
END.
...
DO:
    ASSIGN
     ...
     IF T-MATG.UndA <> "" THEN DO:
         ...
         /*MESSAGE t-matg.codmat x-ctound f-mrguti-a f-factor.*/
     END.
...
DEF BUFFER BT-MATG FOR T-MATG.

FOR EACH T-MATG NO-LOCK, FIRST BT-MATG OF T-MATG EXCLUSIVE-LOCK,
    FIRST Almmmatg OF T-MATG NO-LOCK, 
    ...
    REPEAT WHILE LocalCuenta <= 100:
        ...
        IF AVAILABLE B-MATG THEN DO:
            ...
            RELEASE B-MATG.
            FIND B-MATG WHERE ROWID(B-MATG) = ROWID(Almmmatg) NO-LOCK NO-ERROR.
            BUFFER-COPY B-MATG TO BT-MATG.
            LEAVE.
        END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      ...
      T-MATG.CtoTotMarco = DECIMAL(T-MATG.CtoTotMarco:SCREEN-VALUE IN BROWSE {&browse-name})
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      ASSIGN
          T-MATG.MonVta:READ-ONLY IN  BROWSE {&browse-name} = NO.
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST AlmmmatgExt WHERE AlmmmatgExt.CodCia = s-codcia
              AND AlmmmatgExt.CodMat = T-MATG.CodMat:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
