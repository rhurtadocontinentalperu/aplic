...
DEFINE MENU POPUP-MENU-br_table 
       ...
       MENU-ITEM m_VIP_HUANCAYO_-_Descuento_po LABEL "VIP_HUANCAYO - Descuento por Volumen".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      ( IF AlmmmatgExt.MonCmp = 1 THEN  'Soles'  ELSE 'Dolares' ) @ x-MonCmp COLUMN-LABEL "Moneda!Compra"
            COLUMN-FGCOLOR 0 COLUMN-BGCOLOR 8
      T-MATG.MonVta COLUMN-LABEL "Moneda!Venta" FORMAT "9":U WIDTH 7.57
...
"T-MATG.MonVta" "Moneda!Venta" ? "integer" 10 0 ? ? ? ? yes ? no no "7.57" yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "Soles,1,Dolares,2" 5 no 0 no no
...
"T-MATG.CtoTotMarco" "Costo MARCO!S/." ">>>,>>9.9999" "decimal" 7 15 ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF T-MATG.MonVta IN BROWSE br_table /* Moneda!Venta */
DO:
...
DO:
    IF AVAILABLE Almmmatg THEN RUN Vta2/D-Lima-DtoVol.r (Almmmatg.codmat).
    /*IF AVAILABLE Almmmatg THEN RUN pri/D-Lima-DctoVol (Almmmatg.codmat).*/
END.
...
DO:
    IF AVAILABLE Almmmatg THEN RUN PRI/D-Lima-DctoVol-v2.r (Almmmatg.codmat).
END.
...
DO:
  ...
  IF AVAILABLE Almmmatg THEN RUN pri/D-Lima-DctoProm.r (Almmmatg.codmat).
END.
...
DO:
    ...
    IF AVAILABLE VtaListaMinGn THEN RUN Vta2/D-Utilex-DtoVol.r (T-MATG.codmat).
    /*IF AVAILABLE Almmmatg THEN RUN pri/D-Utilex-DctoVol (Almmmatg.codmat).*/
    ELSE DO:
...
DO:
    FIND VtaListaMinGn OF T-MATG NO-LOCK NO-ERROR.
    /*IF AVAILABLE VtaListaMinGn THEN RUN Vta2/D-Utilex-DtoPromv2 (T-MATG.codmat).*/
    IF AVAILABLE VtaListaMinGn THEN RUN Pri/D-Utilex-DctoProm.r (T-MATG.codmat).
    ELSE DO:
        MESSAGE 'Primero debe asignarle un PRECIO DE OFICINA UTILEX'
            VIEW-AS ALERT-BOX WARNING.
    END.
...
&Scoped-define SELF-NAME m_VIP_HUANCAYO_-_Descuento_po
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL m_VIP_HUANCAYO_-_Descuento_po B-table-Win
ON CHOOSE OF MENU-ITEM m_VIP_HUANCAYO_-_Descuento_po /* VIP_HUANCAYO - Descuento por Volumen */
DO:
    IF AVAILABLE Almmmatg THEN RUN PRI/D-Huancayo-DctoVol.r (Almmmatg.codmat).
END.
...
END.
/* ************************************** */
/* TODOS LOS PRECIOS EN SOLES A LA MONEDA DE VENTA */
...
------------------------------------------------------------------------------*/
  ...
  /* Todos estos valores están en SOLES */
  ASSIGN
  ...
  IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
/*       T-MATG.PreAlt[1]:SCREEN-VALUE = STRING(x-PreUtilex). */
      UNDO, RETURN 'ADM-ERROR'.
  END.
