...
DEFINE VARIABLE h_b-aprobar-cot-evento AS HANDLE NO-UNDO.
...
DO:
  RUN Aprobar IN h_b-aprobar-cot-evento.
END.
...
DO:
  RUN Rechazar IN h_b-aprobar-cot-evento.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'PRI/b-aprobar-cot-evento.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-aprobar-cot-evento ).
       RUN set-position IN h_b-aprobar-cot-evento ( 2.62 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-aprobar-cot-evento ( 6.69 , 111.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-aprobar-cot-evento ,
             BUTTON-5:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_tab95 ,
             h_b-aprobar-cot-evento , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN add-link IN adm-broker-hdl ( h_b-aprobar-cot-evento , 'Record':U , h_v-cotizacion-unificada ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-size IN h_b-cotizacion-general-v2 ( 13.19 , 156.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-cotizacion-general-v2. */
       RUN add-link IN adm-broker-hdl ( h_b-aprobar-cot-evento , 'Record':U , h_b-cotizacion-general-v2 ).
