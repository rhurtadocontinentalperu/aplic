...
DEFINE VARIABLE h_b-eventos-import-precios AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'PRI/b-eventos-import-precios.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-eventos-import-precios ).
       RUN set-position IN h_b-eventos-import-precios ( 8.27 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-eventos-import-precios ( 16.35 , 139.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-eventos-import-precios. */
       RUN add-link IN adm-broker-hdl ( h_p-updv97 , 'TableIO':U , h_b-eventos-import-precios ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-eventos-import-precios ,
             FILL-IN-Mensaje:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv97 ,
             h_b-eventos-import-precios , 'AFTER':U ).
    END. /* Page 1 */
...
RUN Recalcular IN h_b-eventos-import-precios.
RUN dispatch IN h_b-eventos-import-precios ('open-query':U).
...
FOR EACH T-MATG, FIRST Almmmatg OF T-MATG NO-LOCK ON ERROR UNDO, RETURN ON STOP UNDO, RETURN:
    ...
    FIND FIRST VtaListaMay WHERE VtaListaMay.CodCia = T-MATG.CodCia AND
        VtaListaMay.codmat = T-MATG.CodMat AND
        VtaListaMay.CodDiv = s-CodDiv EXCLUSIVE-LOCK NO-ERROR.
