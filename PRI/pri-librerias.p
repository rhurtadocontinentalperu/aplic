...
RUN web_api-unit-cost-price IN hProc (INPUT pCodMat,
                                      INPUT "C",
                                      INPUT "000",
                                      INPUT "",     /* Sin Cliente */
                                      INPUT pCodDiv,
...
IF pcError > '' AND TRUE <> (pcReturn > '') THEN DO:
    pError = STRING(pcError).
    RETURN 'ADM-ERROR'.
END.
/* Sintaxis pcReturn
<cCodMat>:<cCodMon>:<fTpoCmb>:<fPreUni>:<fRepositionCostInCurrencySale>:<cContrato>[,...].
*/
priMonVta = INTEGER(ENTRY(2,pcReturn,':')) NO-ERROR.
priTpoCmb = DECIMAL(ENTRY(3,pcReturn,':')) NO-ERROR.
x-Costo   = DECIMAL(ENTRY(5,pcReturn,':')) NO-ERROR.
...
/* 29/01/2026: Cesar Camus, código duro VENTAS OUTLET */
IF pCodDiv = "00528" OR pCodDiv = "00012" THEN RETURN "OK".
...
&IF DEFINED(EXCLUDE-PRI_Margen-Utilidad-Old) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PRI_Margen-Utilidad-Old Procedure 
PROCEDURE PRI_Margen-Utilidad-Old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Rutina que solo calcula el margen de utilidad en base al costo de reposición
...
RUN web_api-unit-cost-price IN hWebLibrary (INPUT pCodMat,
                                            INPUT "C",
                                            INPUT "000",
                                            INPUT "",     /* Sin Cliente */
                                            INPUT pCodDiv,
...
&IF DEFINED(EXCLUDE-PRI_Margen-Utilidad-Ventas-Old) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PRI_Margen-Utilidad-Ventas-Old Procedure 
PROCEDURE PRI_Margen-Utilidad-Ventas-Old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Rutina que solo calcula el margen de utilidad en base al costo de reposición
