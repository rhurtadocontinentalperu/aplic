...
&Scoped-define INTERNAL-TABLES PriDDtoVolAcu GN-DIVI
...
&Scoped-define QUERY-STRING-br_table FOR EACH PriDDtoVolAcu OF PriCDtoVolAcu WHERE ~{&KEY-PHRASE} ~
      ...
      FIRST GN-DIVI WHERE GN-DIVI.CodCia = PriDDtoVolAcu.CodCia ~
  AND GN-DIVI.CodDiv = PriDDtoVolAcu.Llave_c1 NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH PriDDtoVolAcu OF PriCDtoVolAcu WHERE ~{&KEY-PHRASE} ~
  ...
  AND GN-DIVI.CodDiv = PriDDtoVolAcu.Llave_c1 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table PriDDtoVolAcu GN-DIVI
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      PriDDtoVolAcu.Llave_c1 COLUMN-LABEL "División" FORMAT "x(15)":U
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.PriDDtoVolAcu OF INTEGRAL.PriCDtoVolAcu,INTEGRAL.GN-DIVI WHERE INTEGRAL.PriDDtoVolAcu ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST USED"
     _Where[1]         = "PriDDtoVolAcu.Tabla = s-Tabla"
     _JoinCode[2]      = "GN-DIVI.CodCia = PriDDtoVolAcu.CodCia
  AND GN-DIVI.CodDiv = PriDDtoVolAcu.Llave_c1"
     _FldNameList[1]   > INTEGRAL.PriDDtoVolAcu.Llave_c1
"PriDDtoVolAcu.Llave_c1" "División" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.GN-DIVI.DesDiv
"GN-DIVI.DesDiv" "Descripción" ? "character" ? ? ? ? ? ? no ? no no "75.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON LEAVE OF PriDDtoVolAcu.Llave_c1 IN BROWSE br_table /* División */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL PriDDtoVolAcu.Llave_c1 br_table _BROWSE-COLUMN B-table-Win
ON LEFT-MOUSE-DBLCLICK OF PriDDtoVolAcu.Llave_c1 IN BROWSE br_table /* División */
DO:
    ASSIGN
        input-var-1 = ''
        input-var-2 = ''
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST gn-divi WHERE gn-divi.codcia = s-codcia
        AND gn-divi.coddiv = PriDDtoVolAcu.Llave_c1:SCREEN-VALUE IN BROWSE {&browse-name}
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE gn-divi THEN DO:
        MESSAGE 'División NO registrado' VIEW-AS ALERT-BOX ERROR.
