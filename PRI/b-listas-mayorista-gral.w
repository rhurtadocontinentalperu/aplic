...
T-MATG.UndStk T-MATG.DesMar T-MATG.MonVta T-MATG.TpoCmb T-MATG.CtoTot ~
...
"T-MATG.DesMar" "Marca" "X(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.T-MATG.MonVta
"T-MATG.MonVta" "Moneda" ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "Soles,1,Dolares,2" 5 no 0 no no
     _FldNameList[6]   > Temp-Tables.T-MATG.TpoCmb
"T-MATG.TpoCmb" "TC!Venta" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      END CASE.
  END.
  /*IF s-user-id = 'admin' THEN s-lineasvalidas = '011'.*/
...
------------------------------------------------------------------------------*/
   ...
   CASE T-MATG.Chr__02 :
    ...
    /* Control Margen de Utilidad */
    /* ****************************************************************************************************** */
    /* RHC 14/07/20201 Solución temporal: si la MonVta = 0 grabamos inmediatamente el valor de pantalla */
    IF Almmmatg.MonVta = 0 THEN DO:
        ...
        END.
        ASSIGN
            B-MATG.MonVta = INTEGER(T-MATG.MonVta:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}).
        RELEASE B-MATG.
    END.
