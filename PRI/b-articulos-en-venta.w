...
DEF VAR x-PreUtilex AS DEC NO-UNDO.
DEF VAR x-PreLima AS DEC NO-UNDO.
...
AND (TRUE <> (x-SubFam > '') OR Almmmatg.subfam = x-subfam) ~
...
&Scoped-define INTERNAL-TABLES Almmmatg Almtfami VtaListaMinGn
...
Almmmatg.codfam Almmmatg.subfam ~
(IF Almmmatg.MonVta = 2 THEN Almmmatg.TpoCmb ELSE 1) * Almmmatg.CtoTot @ x-CtoTot ~
(IF Almmmatg.MonVta = 2 THEN Almmmatg.TpoCmb ELSE 1) * Almmmatg.PreOfi @ x-PreLima ~
Almmmatg.Chr__01 ~
(IF Almmmatg.MonVta = 2 THEN Almmmatg.TpoCmb ELSE 1) * VtaListaMinGn.PreOfi @ x-PreUtilex ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Almmmatg WHERE ~{&KEY-PHRASE} ~
      ...
      FIRST VtaListaMinGn OF Almmmatg OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table Almmmatg Almtfami VtaListaMinGn
...
DEFINE QUERY br_table FOR 
    ...
    FIELDS(), 
      VtaListaMinGn
    FIELDS(VtaListaMinGn.PreOfi
      VtaListaMinGn.Chr__01) SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      (IF Almmmatg.MonVta = 2 THEN Almmmatg.TpoCmb ELSE 1) * Almmmatg.CtoTot @ x-CtoTot COLUMN-LABEL "Costo en S/."
      (IF Almmmatg.MonVta = 2 THEN Almmmatg.TpoCmb ELSE 1) * Almmmatg.PreOfi @ x-PreLima COLUMN-LABEL "Mayorista S/."
      Almmmatg.Chr__01 COLUMN-LABEL "Unidad" FORMAT "X(8)":U
      (IF Almmmatg.MonVta = 2 THEN Almmmatg.TpoCmb ELSE 1) * VtaListaMinGn.PreOfi @ x-PreUtilex COLUMN-LABEL "Minorista S/."
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "INTEGRAL.Almmmatg,INTEGRAL.Almtfami OF INTEGRAL.Almmmatg,INTEGRAL.VtaListaMinGn OF INTEGRAL.Almmmatg"
...
"Almmmatg.subfam" "Sub-Línea" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"(IF Almmmatg.MonVta = 2 THEN Almmmatg.TpoCmb ELSE 1) * Almmmatg.CtoTot @ x-CtoTot" "Costo en S/." ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > "_<CALC>"
"(IF Almmmatg.MonVta = 2 THEN Almmmatg.TpoCmb ELSE 1) * Almmmatg.PreOfi @ x-PreLima" "Mayorista S/." ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > INTEGRAL.Almmmatg.Chr__01
"Almmmatg.Chr__01" "Unidad" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > "_<CALC>"
"(IF Almmmatg.MonVta = 2 THEN Almmmatg.TpoCmb ELSE 1) * VtaListaMinGn.PreOfi @ x-PreUtilex" "Minorista S/." ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Captura-Articulos B-table-Win 
PROCEDURE Captura-Articulos :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
x-CodFam = (IF pCodFam = "TODAS" THEN "" ELSE pCodFam).
x-SubFam = (IF pSubFam = "TODAS" THEN "" ELSE pSubFam).
