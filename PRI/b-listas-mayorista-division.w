...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 155 BY 19.62
...
"Temp-Tables.T-MATG.PreBas" "Precio Oficina!US$" ">>>,>>9.9999" "decimal" ? ? ? ? ? ? no ? no no "12.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > Temp-Tables.T-MATG.Orden
"Temp-Tables.T-MATG.Orden" "Config. de Desctos." "9" "integer" ? ? 4 ? ? ? yes ? no no "13.72" yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "Normal,0,Sin Descuentos,1" 5 no 0 no no
...
DO:
    IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN DO:
        /*RUN vta2/d-xdivision-dtoprom ({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codmat, COMBO-BOX-Division).*/
        RUN pri/d-xlista-dctoprom ({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codmat, COMBO-BOX-Division).
   END.
END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      ...
      T-MATG.CHR__01 = VtaListaMay.CHR__01.
  ASSIGN
      T-MATG.PreBas = (IF Almmmatg.MonVta = 2 THEN VtaListaMay.PreOfi ELSE VtaListaMay.PreOfi / Almmmatg.TpoCmb).
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      FOR EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-codcia AND 
          GN-DIVI.Campo-Log[1] = NO AND     /* Activo */
          LOOKUP(GN-DIVI.Campo-Char[1],'A,L') > 0 AND   /* LIsta de Precios o Ambos */
          GN-DIVI.VentaMayorista = 2 AND    /* LIsta x División */
...
x-PreUni = DECIMAL(T-MATG.PreOfi:SCREEN-VALUE IN BROWSE {&Browse-name}).
...
IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
    ...
    APPLY 'ENTRY':U TO T-MATG.PreOfi IN BROWSE {&browse-name}.
    RETURN 'ADM-ERROR'.
END.
...
/* ****************************************************************************************************** */
/* /* RHC 16/11/2020 Margen de Utilidad */                                                */
/* DEF VAR x-PreUni AS DEC NO-UNDO.                                                       */
...
/*                                                                                        */
/* x-PreUni = DECIMAL (T-MATG.PreOfi:SCREEN-VALUE IN BROWSE {&Browse-name}).              */
/* RUN vtagn/p-margen-utilidad-v11 (COMBO-BOX-Division,                                   */
