...
DEFINE FRAME F-Main
     ...
     RADIO-SET-2 AT ROW 12.23 COL 79 NO-LABEL WIDGET-ID 32
     BROWSE-2 AT ROW 13.58 COL 2 WIDGET-ID 200
     "Valores en:" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 12.42 COL 70 WIDGET-ID 36
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 107.43 BY 25.62
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "PREPARAR PLANTILLA DESCUENTOS POR VOLUMEN  - SOLO EVENTOS"
         HEIGHT             = 25.62
         WIDTH              = 107.43
         MAX-HEIGHT         = 25.62
         MAX-WIDTH          = 111.14
...
ON END-ERROR OF W-Win /* PREPARAR PLANTILLA DESCUENTOS POR VOLUMEN  - SOLO EVENTOS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* PREPARAR PLANTILLA DESCUENTOS POR VOLUMEN  - SOLO EVENTOS */
DO:
...
ASSIGN
    ...
    chWorkSheet:Range("I2"):Value = "DIVISION"
    chWorkSheet:Columns("I"):NumberFormat = "@"

    chWorkSheet:Range("J2"):Value = "DESDE"
    chWorkSheet:Columns("J"):NumberFormat = "dd/MM/yyyy"
    chWorkSheet:Range("K2"):Value = "HASTA"
    chWorkSheet:Columns("K"):NumberFormat = "dd/MM/yyyy"
    chWorkSheet:Range("L2"):Value = "MINIMO 1"
    chWorkSheet:Range("M2"):Value = "DESCUENTO 1"
    chWorkSheet:Range("N2"):Value = "MINIMO 2"
    chWorkSheet:Range("O2"):Value = "DESCUENTO 2"
    chWorkSheet:Range("P2"):Value = "MINIMO 3"
    chWorkSheet:Range("Q2"):Value = "DESCUENTO 3"
    chWorkSheet:Range("R2"):Value = "MINIMO 4"
    chWorkSheet:Range("S2"):Value = "DESCUENTO 4"
    chWorkSheet:Range("T2"):Value = "MINIMO 5"
    chWorkSheet:Range("U2"):Value = "DESCUENTO 5".
...
FOR EACH Almmmatg NO-LOCK WHERE {&Condicion},
    ...
    FIND FIRST VtaDctoVol WHERE VtaDctoVol.CodCia = VtaListaMay.CodCia AND
        VtaDctoVol.CodDiv = VtaListaMay.CodDiv AND
        VtaDctoVol.CodMat = VtaListaMay.codmat AND
        (TODAY >= VtaDctoVol.FchIni OR TODAY <= VtaDctoVol.FchFin)
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE VtaDctoVol THEN DO:
        ASSIGN
            t-Column = 0
            t-Row    = t-Row + 1.
        RUN Registro-Comun (INPUT-OUTPUT t-Column, INPUT-OUTPUT t-Row, INPUT-OUTPUT x-PreOfi).
    END.
    ELSE DO:
        REPEAT WHILE AVAILABLE VtaDctoVol:
            ASSIGN
                t-Column = 0
                t-Row    = t-Row + 1.
            RUN Registro-Comun (INPUT-OUTPUT t-Column, INPUT-OUTPUT t-Row, INPUT-OUTPUT x-PreOfi).
            ...
            ASSIGN
                t-Column = t-Column + 1
                chWorkSheet:Cells(t-Row, t-Column):VALUE = VtaDctoVol.FchIni.
            /* HASTA */
            ASSIGN
                t-Column = t-Column + 1
                chWorkSheet:Cells(t-Row, t-Column):VALUE = VtaDctoVol.FchFin.
            /* DESCUENTOS */
            DO k = 1 TO 5:
                IF VtaDctoVol.DtoVolD[k] <= 0 THEN NEXT.
                ASSIGN
                    t-Column = t-Column + 1
                    chWorkSheet:Cells(t-Row, t-Column):VALUE = VtaDctoVol.DtoVolR[k].
                ASSIGN
                    t-Column = t-Column + 1
                    chWorkSheet:Cells(t-Row, t-Column):VALUE = (IF RADIO-SET-2 = 1 THEN VtaDctoVol.DtoVolD[k]
                        ELSE ROUND(x-PreOfi * ( 1 - ( VtaDctoVol.DtoVolD[k] / 100 ) ),4) ).
            END.
            FIND NEXT VtaDctoVol WHERE VtaDctoVol.CodCia = VtaListaMay.CodCia AND
                VtaDctoVol.CodDiv = VtaListaMay.CodDiv AND
                VtaDctoVol.CodMat = VtaListaMay.codmat AND
                (TODAY >= VtaDctoVol.FchIni OR TODAY <= VtaDctoVol.FchFin)
                NO-LOCK NO-ERROR.
...
PROCEDURE Registro-Comun:
/* ********************** */

DEF INPUT-OUTPUT PARAMETER t-Column AS INT.
DEF INPUT-OUTPUT PARAMETER t-Row    AS INT.
DEF INPUT-OUTPUT PARAMETER x-PreOfi AS DEC.
...
ASSIGN
    t-Column = t-Column + 1
    chWorkSheet:Cells(t-Row, t-Column):VALUE = s-CodDiv.
