...
/* Barremos por cada precio registrado en la lista de precios y
    por cada ID Rappi registrado
*/
FOR EACH VtaListaMay NO-LOCK WHERE VtaListaMay.CodCia = gn-divi.codcia AND 
    VtaListaMay.CodDiv = gn-divi.coddiv,
    FIRST Almmmatg OF VtaListaMay NO-LOCK,
    FIRST Almtfami OF Almmmatg NO-LOCK,
    FIRST Almsfami OF Almmmatg NO-LOCK:
    ...
    IF pPrecio > 1 THEN DO:
        ...
        IF pPrecio = 3 AND VtaListaMay.PreOfi > 0 THEN NEXT.
    END.
    /* 2do. barremos por tiendas RAPPI: Solo el almacén principal */
    FOR EACH FacTabla NO-LOCK WHERE FacTabla.CodCia = s-codcia 
        ...
        ASSIGN
            ...
            /*Detalle.Descripcion = Almmmatg.DesMat     NO VA */
            Detalle.Marca = Almmmatg.DesMar
            Detalle.Stock = x-StkAct
            Detalle.Tienda = FacTabla.Campo-C[1] 
            Detalle.Precio_Por_Tienda = ROUND(VtaListaMay.PreOfi * (IF Almmmatg.MonVta = 2 THEN Almmmatg.TpoCmb ELSE 1),2)
...
FOR EACH VtaListaMay NO-LOCK WHERE VtaListaMay.CodCia = gn-divi.codcia AND VtaListaMay.CodDiv = gn-divi.coddiv,
    ...
    FIRST Almtfami OF Almmmatg NO-LOCK,
    FIRST Almsfami OF Almmmatg NO-LOCK,
    EACH FacTabla NO-LOCK WHERE FacTabla.CodCia = s-codcia AND FacTabla.Tabla = 'IDRAPPI',
    FIRST TIENDAS NO-LOCK WHERE TIENDAS.codcia = s-codcia AND TIENDAS.coddiv = FacTabla.Codigo:
    IF pPrecio > 1 THEN DO:
        IF pPrecio = 2 AND VtaListaMay.PreOfi = 0 THEN NEXT.
        IF pPrecio = 3 AND VtaListaMay.PreOfi > 0 THEN NEXT.
    END.
    ...
    FOR EACH VtaDctoProm NO-LOCK WHERE VtaDctoProm.CodCia = s-CodCia
        ...
        cFchFin = STRING(DAY(VtaDctoProm.FchFin),'99') + '-' +
                    ...
                    STRING(YEAR(VtaDctoProm.FchFin),'9999').
        ASSIGN
            Detalle.Descuento = STRING(VtaDctoProm.Descuento, '->>>9.99999999')
            /*Detalle.Precio_Con_Descuento = STRING( ROUND( Detalle.Precio_Por_Tienda * (1 - ( VtaDctoProm.Descuento / 100 )), 4) )*/
            /*Detalle.Fecha_Inicio_Descuento = STRING(VtaDctoProm.FchIni,'99-99-9999')*/
            /*Detalle.Fecha_Fin_Descuento = STRING(VtaDctoProm.FchFin,'99-99-9999')*/
            Detalle.Fecha_Inicio_Descuento = cFchIni
