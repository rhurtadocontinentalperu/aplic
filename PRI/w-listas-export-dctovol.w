...
DO:
    ASSIGN RADIO-SET-1.
    CASE RADIO-SET-1:
        WHEN 1 THEN ASSIGN s-CanalVenta = "ATL,HOR,INS,PRO,TDA,MOD".
...
CASE RADIO-SET-1:
    ...
    WHEN 3 THEN RUN PRI_Divisiones-Validas IN hProc (INPUT 3, OUTPUT f-Divisiones).
END CASE.
...
/*             AND LOOKUP(GN-DIVI.CanalVenta, s-CanalVenta) > 0                                                  */
/*             AND GN-DIVI.Campo-Log[1] = NO      /* ¿INACTIVO? */                                               */
/*             AND GN-DIVI.VentaMayorista = 1:      /* LISTA DE PRECIOS GENERAL */                               */
...
/*             AND LOOKUP(GN-DIVI.CanalVenta, s-CanalVenta) > 0                                                  */
/*             AND GN-DIVI.Campo-Log[1] = NO      /* ¿INACTIVO? */                                               */
/*             AND GN-DIVI.VentaMayorista = 2:      /* LISTA POR DIVISION */                                     */
/*             f-Divisiones = f-Divisiones + (IF TRUE <> (f-Divisiones > "") THEN '' ELSE ',') + gn-divi.coddiv. */
/*         END.                                                                                                  */
/*     END.                                                                                                      */
/* END CASE.                                                                                                     */
...
CASE RADIO-SET-1:
    ...
    WHEN 3 THEN chWorkSheet:Range("A1"):Value = "POR DIVISION - DESCUENTOS POR VOLUMEN".
END CASE.
CASE RADIO-SET-2:
...
FOR EACH Almmmatg NO-LOCK WHERE {&Condicion},
    EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-codcia AND
    LOOKUP(gn-divi.coddiv, f-Divisiones) > 0:
    ...
    /* PRECIO LISTA */
    IF RADIO-SET-1 = 1 THEN x-PreVta = Almmmatg.PreOfi.
    IF RADIO-SET-1 = 2 THEN x-PreVta = Vtalistamingn.preofi.
    IF RADIO-SET-1 = 3 THEN x-PreVta = VtaListaMay.preofi.
    ...
    ASSIGN
        t-Column = t-Column + 1
        chWorkSheet:Cells(t-Row, t-Column):VALUE = GN-DIVI.CodDiv.
    /* VIGENCIA */
    FIND FIRST VtaDctoVol WHERE VtaDctoVol.CodCia = s-CodCia AND
        VtaDctoVol.CodDiv = GN-DIVI.CodDiv AND
        VtaDctoVol.CodMat = Almmmatg.codmat AND
        (TODAY >= VtaDctoVol.FchIni AND TODAY <= VtaDctoVol.FChFin)
        NO-LOCK NO-ERROR.
    IF AVAILABLE VtaDctoVol THEN DO:
        ASSIGN
            t-Column = t-Column + 1
        ...
        /* DESCUENTOS */
        DO k = 1 TO 5:
            IF VtaDctoVol.DtoVolR[k] <= 0 THEN NEXT.
            ASSIGN
                t-Column = t-Column + 1
            ...
            ASSIGN
                ...
                chWorkSheet:Cells(t-Row, t-Column):VALUE = (IF RADIO-SET-2 = 1 THEN VtaDctoVol.DtoVolD[k]
                    ELSE ROUND(x-PreVta * ( 1 - ( VtaDctoVol.DtoVolD[k] / 100 ) ),4) ).
    ...
    CASE RADIO-SET-1:
        WHEN 1 OR WHEN 2 OR WHEN 3 THEN DO:
            ...
            ASSIGN
                ...
                chWorkSheet:Cells(t-Row, t-Column):VALUE = x-CtoTot.
            /* PRECIO LISTA */
            IF RADIO-SET-1 = 1 THEN x-PreVta = Almmmatg.prevta[1].
            IF RADIO-SET-1 = 2 THEN x-PreVta = Vtalistamingn.preofi.
            IF RADIO-SET-1 = 3 THEN x-PreVta = VtaListaMay.preofi.
