...
DEF TEMP-TABLE Detalle NO-UNDO
    ...
    INDEX Llave01 AS PRIMARY Referencia_Aliado Sku
    .
...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN_Mensaje 
...
DEFINE FRAME F-Main
     ...
     FILL-IN_Mensaje AT ROW 17.15 COL 12 COLON-ALIGNED NO-LABEL WIDGET-ID 4
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: t-Almmmatg T "?" ? INTEGRAL Almmmatg
   END-TABLES.
 */
...
FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND Almmmatg.tpoart <> "D",
    ...
    /* 1ro. ver si el producto es válido para Rappi */  
    ...
    FILL-IN_Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = Almmmatg.codmat + ' ' + Almmmatg.desmat.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Envio-a-Texto B-table-Win 
PROCEDURE Envio-a-Texto :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
RUN Carga-Temporal (INPUT pPrecio).
...
FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia AND Almmmatg.tpoart <> "D",
    ...
    END.
    FILL-IN_Mensaje:SCREEN-VALUE IN FRAME {&FRAME-NAME} = Almmmatg.codmat + ' ' + Almmmatg.desmat.
   /* 3ro. barremos por tiendas RAPPI: Solo el almacén principal */
...
RUN Envio-a-Texto (INPUT cCarpeta).
