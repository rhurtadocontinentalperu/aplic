...
FOR EACH Almmmatg NO-LOCK WHERE {&Condicion},
    ...
    /* VIGENCIA */
    FIND FIRST PRIDctoVol WHERE PRIDctoVol.CodCia = s-CodCia AND
        PRIDctoVol.CodDiv = GN-DIVI.CodDiv AND
        PRIDctoVol.CodMat = Almmmatg.codmat AND
...
/*         PriDctoVol.Grupo = GN-DIVI.Grupo_Divi_GG AND */
        ...
        NO-LOCK NO-ERROR.
    IF AVAILABLE PRIDctoVol THEN DO:
        ASSIGN
            t-Column = t-Column + 1
            chWorkSheet:Cells(t-Row, t-Column):VALUE = PRIDctoVol.FchIni.
        ASSIGN
            t-Column = t-Column + 1
            chWorkSheet:Cells(t-Row, t-Column):VALUE = PRIDctoVol.FchFin.
        /* DESCUENTOS */
        DO k = 1 TO 5:
            IF PRIDctoVol.DtoVolR[k] <= 0 THEN NEXT.
            ASSIGN
                t-Column = t-Column + 1
                chWorkSheet:Cells(t-Row, t-Column):VALUE = PRIDctoVol.DtoVolR[k].
            ASSIGN
                t-Column = t-Column + 1
                chWorkSheet:Cells(t-Row, t-Column):VALUE = (IF RADIO-SET-2 = 1 THEN PRIDctoVol.DtoVolD[k]
                    ELSE ROUND(x-PreVta * ( 1 - ( PRIDctoVol.DtoVolD[k] / 100 ) ),4) ).
