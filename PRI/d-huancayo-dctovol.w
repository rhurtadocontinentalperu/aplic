...
/*     gn-divi.Grupo_Divi_GG = '03':   /* Tiendas Provincia */                                 */
/*     s-Divisiones = s-Divisiones + (IF s-Divisiones > '' THEN ',' ELSE '') + gn-divi.coddiv. */
/* END.                                                                                        */

s-Divisiones = '00072'.     /* Exclusivamente Huancayo */
...
DEFINE VARIABLE h_b-huancayo-dctovol AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv96 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-lima-dtopromv2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-huancayo-dctovol AS HANDLE NO-UNDO.
...
DEFINE FRAME D-Dialog
     ...
     SPACE(85.42) SKIP(13.41)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "VIP HUANCAYO - DESCUENTO POR VOLUMEN" WIDGET-ID 100.
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* VIP HUANCAYO - DESCUENTO POR VOLUMEN */
DO:  
...
DO:
    ...
    RUN Replica-Plantilla IN h_b-huancayo-dctovol ( INPUT pDivisiones /* CHARACTER */).
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pri/b-huancayo-dctovol.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-huancayo-dctovol ).
       RUN set-position IN h_b-huancayo-dctovol ( 2.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-huancayo-dctovol ( 12.12 , 71.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/pri/v-huancayo-dctovol.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-huancayo-dctovol ).
       RUN set-position IN h_v-huancayo-dctovol ( 2.62 , 77.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updv96 ).
       RUN set-position IN h_p-updv96 ( 13.92 , 78.00 ) NO-ERROR.
       ...
       /* Links to SmartBrowser h_b-huancayo-dctovol. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_b-huancayo-dctovol ).
       RUN add-link IN adm-broker-hdl ( h_q-lima-dtopromv2 , 'Record':U , h_b-huancayo-dctovol ).

       /* Links to SmartViewer h_v-huancayo-dctovol. */
       RUN add-link IN adm-broker-hdl ( h_b-huancayo-dctovol , 'Record':U , h_v-huancayo-dctovol ).
       RUN add-link IN adm-broker-hdl ( h_p-updv96 , 'TableIO':U , h_v-huancayo-dctovol ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             Btn_OK:HANDLE , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-huancayo-dctovol ,
             BUTTON-2:HANDLE , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-huancayo-dctovol ,
             h_b-huancayo-dctovol , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv96 ,
             h_v-huancayo-dctovol , 'AFTER':U ).
    END. /* Page 0 */
