...
FOR EACH T-MATG EXCLUSIVE-LOCK ON STOP UNDO, RETURN ON ERROR UNDO, RETURN:
    ...
    IF NOT AVAILABLE AlmmmatgExt THEN DO:
        ...
        UNDO, RETURN.
    END.
    FIND lcc_mayorista OF T-MATG WHERE lcc_mayorista.FlgEst = "P" NO-LOCK NO-ERROR.
    IF AVAILABLE lcc_mayorista THEN DO:
        FIND CURRENT lcc_mayorista EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
        IF NOT AVAILABLE lcc_mayorista THEN DO:
            {lib/mensaje-de-error.i &CuentaError="pCuenta" &MensajeError="pMensaje"}
            UNDO, RETURN.
        END.
    END.
    ELSE DO:
        CREATE lcc_mayorista.
        ASSIGN
            lcc_mayorista.CodCia = s-CodCia
            lcc_mayorista.codmat = T-MATG.CodMat.
    END.
    /* SE GRABA TODO EN MONEDA LP */
    ASSIGN
        lcc_mayorista.usuario   = s-User-Id
        lcc_mayorista.MonVta    = T-MATG.MonVta
        lcc_mayorista.PreVta[1] = T-MATG.PreVta[1]
        lcc_mayorista.MrgUti-A  = T-MATG.MrgUti-A
        lcc_mayorista.Prevta[2] = T-MATG.Prevta[2]
        lcc_mayorista.MrgUti-B  = T-MATG.MrgUti-B
        lcc_mayorista.PreVta[3] = T-MATG.Prevta[3]
        lcc_mayorista.MrgUti-C  = T-MATG.MrgUti-C
        lcc_mayorista.PreVta[4] = T-MATG.Prevta[4]
        lcc_mayorista.Dec__01   = T-MATG.Dec__01
        lcc_mayorista.PreOfi    = T-MATG.PreOfi.
    /* ****************************************************************************************************** */
    /* Control Margen de Utilidad */
    /* ****************************************************************************************************** */
    IF lcc_mayorista.Prevta[1] > 0 THEN DO:
        F-Factor = 1.
    ...
    IF lcc_mayorista.Prevta[2] > 0 THEN DO:
        ...
        IF AVAILABLE Almtconv THEN F-FACTOR = Almtconv.Equival.
        x-PreUni = lcc_mayorista.PreVta[2] / f-Factor.
        RUN Verifica-Margen (INPUT x-PreUni, OUTPUT pError).
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            CREATE E-MATG.
            BUFFER-COPY lcc_mayorista TO E-MATG
    ...
    IF lcc_mayorista.Prevta[3] > 0 THEN DO:
        ...
        IF AVAILABLE Almtconv THEN F-FACTOR = Almtconv.Equival.
        x-PreUni = lcc_mayorista.PreVta[3] / f-Factor.
        RUN Verifica-Margen (INPUT x-PreUni, OUTPUT pError).
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            CREATE E-MATG.
            BUFFER-COPY lcc_mayorista TO E-MATG
    ...
    IF lcc_mayorista.Prevta[4] > 0 THEN DO:
        ...
        IF AVAILABLE Almtconv THEN F-FACTOR = Almtconv.Equival.
        x-PreUni = lcc_mayorista.PreVta[4] / f-Factor.
        RUN Verifica-Margen (INPUT x-PreUni, OUTPUT pError).
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            CREATE E-MATG.
            BUFFER-COPY lcc_mayorista TO E-MATG
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Trabajamos con la tabla lcc_mayorista
------------------------------------------------------------------------------*/
