...
&Scoped-Define ENABLED-OBJECTS BtnDone RADIO-SET-Reemplazar BUTTON-Capturar 
...
DEFINE VARIABLE h_b-eventos-import-dctoprom-v2 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BtnDone AT ROW 1 COL 138 WIDGET-ID 6
     RADIO-SET-Reemplazar AT ROW 1.27 COL 69 NO-LABEL WIDGET-ID 40
     BUTTON-Capturar AT ROW 4.5 COL 69 WIDGET-ID 2
     BUTTON-Vista AT ROW 4.5 COL 94 WIDGET-ID 38
     BUTTON-Grabar AT ROW 5.58 COL 69 WIDGET-ID 4
     BUTTON-Limpiar AT ROW 5.58 COL 94 WIDGET-ID 44
...
DO:
    ...
    RUN dispatch IN h_b-eventos-import-dctoprom-v2 ('open-query':U).
END.
...
DO:
  ...
  RUN dispatch IN h_b-eventos-import-dctoprom-v2 ('open-query':U).
  RUN dispatch IN h_b-div-expo ('enable':U).
END.
...
DO:
    ...
    FOR EACH T-MATG NO-LOCK:
        ...
        ASSIGN
            ...
            T-DPROM.CodDiv = s-CodDiv
            T-DPROM.CodMat = T-MATG.CodMat
            T-DPROM.Descuento = T-MATG.Libre_d03
            T-DPROM.FchFin = T-MATG.FchPrmH
            T-DPROM.FchIni = T-MATG.FchPrmD.
    END.
    RUN pri/d-listas-import-dctoprom-v2.w (INPUT 3,
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'PRI/b-eventos-import-dctoprom-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-eventos-import-dctoprom-v2 ).
       RUN set-position IN h_b-eventos-import-dctoprom-v2 ( 8.00 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-eventos-import-dctoprom-v2 ( 18.31 , 141.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-div-expo ,
             BtnDone:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-eventos-import-dctoprom-v2 ,
...
REPEAT:
    ...
    IF ERROR-STATUS:ERROR THEN DO:
        pMensaje = "Error en la linea " + TRIM(STRING(t-Row, '>>>9')) + ":"  + CHR(10) 
            + "Artículo " + cValue + " no registrado an la lista de precios " + s-CodDiv.
        LEAVE.
    END.
    ...
    cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
    ASSIGN
        T-MATG.desmat = (IF TRUE <> (cValue > '') THEN Almmmatg.DesMat ELSE cValue).
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH T-MATG NO-LOCK,
        ...
        VtaListaMay.CodDiv = s-coddiv NO-LOCK:
        x-PreUni = VtaListaMay.PreOfi.
        FIND Almmmatg OF VtaListaMay NO-LOCK NO-ERROR.
        IF T-MATG.Libre_d01 > 0 THEN DO:
            pPreUni = ROUND(x-PreUni * (1 - (T-MATG.Libre_d01 / 100)), 4).
            RUN Verifica-Margen (INPUT pPreUni, INPUT 'VIP').
            IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                DELETE T-MATG.
                NEXT trloop.
            END.
        ...
        IF T-MATG.Libre_d02 > 0 THEN DO:
            pPreUni = ROUND(x-PreUni * (1 - (T-MATG.Libre_d02 / 100)), 4).
            RUN Verifica-Margen (INPUT pPreUni, INPUT 'MR').
            IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                DELETE T-MATG.
                NEXT trloop.
            END.
        ...
        IF T-MATG.Libre_d03 > 0 THEN DO:
            pPreUni = ROUND(x-PreUni * (1 - (T-MATG.Libre_d03 / 100)), 4).
            RUN Verifica-Margen (INPUT pPreUni, INPUT 'VARIOS').
            IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                DELETE T-MATG.
                NEXT trloop.
            END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BtnDone RADIO-SET-Reemplazar BUTTON-Capturar 
      WITH FRAME F-Main IN WINDOW W-Win.
...
FOR EACH T-MATG EXCLUSIVE-LOCK,
    ...
    RUN PRI_Graba-Dscto-Prom-Divi IN hProc (INPUT RADIO-SET-Reemplazar,
                                            INPUT VtaListaMay.CodDiv,
                                            INPUT VtaListaMay.CodMat,
                                            INPUT VtaListaMay.PreOfi,
                                            INPUT T-MATG.FchPrmD,
                                            INPUT T-MATG.FchPrmH,
                                            INPUT T-MATG.Libre_d01,
                                            INPUT T-MATG.Libre_d02,
                                            INPUT T-MATG.Libre_d03,
                                            OUTPUT pMensaje).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Verifica-Margen W-Win 
...
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT PARAMETER x-PreUni AS DECI.
    ...
    IF pError > '' THEN DO:
        ...
        IF pAlerta = YES THEN DO:
            ...
            BUFFER-COPY T-MATG TO A-MATG
                ASSIGN
                    A-MATG.Libre_c01 = "Margen de utilidad " + x-Tipo + " NO debe ser menor a " +
        ...
        ELSE DO:
            ...
            BUFFER-COPY T-MATG TO E-MATG
                ASSIGN
                    E-MATG.Libre_c01 = "Margen de utilidad " + x-Tipo + " NO debe ser menor a " +
                                        TRIM(STRING(x-Limite, ">>>,>>9.99")).
            RETURN 'ADM-ERROR'.
        END.
    END.
