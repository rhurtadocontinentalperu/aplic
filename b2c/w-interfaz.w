...
"T-INTERFASE.Pedido_Cliente" "Pedido Cliente" ? "character" ? ? ? ? ? ? no ? no no "15.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
&ANALYZE-RESUME
...
DEFINE VAR x-total-registros AS INT.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    FOR EACH T-INTERFASE EXCLUSIVE-LOCK WHERE T-INTERFASE.CodCia = s-CodCia:
        ...
        x-total-registros = x-total-registros + 1.
...
MESSAGE "Se GRABARON " + STRING(x-registros-validos) + " Registro(s) " SKIP
        "de un total de " + STRING(x-total-registros) + " Registro(s)"
...
REPEAT:
    ...
    /* ***************************************************************************** */
    cRange = "H" + STRING(t-row). 
    cValue = CAPS(TRIM(chWorkSheet:Range(cRange):TEXT)).
    IF cValue <> "LIQUIDADA" THEN NEXT.
    /* ***************************************************************************** */
    /* ***************************************************************************** */
    ...
    /* Fecha de Proceso */
    cRange = "O" + STRING(t-row). 
    cValue = TRIM(chWorkSheet:Range(cRange):TEXT).
    ASSIGN
        T-INTERFASE.Fecha_de_Proceso = DATE(cValue) NO-ERROR.

    /* Tipo de Operación  */
    cRange = "I" + STRING(t-row). 
    cValue = TRIM(chWorkSheet:Range(cRange):TEXT).
    ASSIGN 
        T-INTERFASE.Tipo_Operacion = cValue.

    /* Tarjeta  */
    cRange = "P" + STRING(t-row). 
    cValue = TRIM(chWorkSheet:Range(cRange):TEXT).
    ASSIGN 
        T-INTERFASE.Tarjeta = cValue.

    /* Tipo de Tarjeta */
    cRange = "E" + STRING(t-row). 
    cValue = TRIM(chWorkSheet:Range(cRange):TEXT).
    ASSIGN 
        T-INTERFASE.Tipo_Tarjeta = cValue.

    /* Moneda  */
    cRange = "C" + STRING(t-row). 
    cValue = TRIM(chWorkSheet:Range(cRange):TEXT).
    ASSIGN 
        T-INTERFASE.Moneda = cValue.

    /* Importe */
    cRange = "D" + STRING(t-row). 
    cValue = TRIM(chWorkSheet:Range(cRange):TEXT).
    ASSIGN 
    ...
    /* Autorizacion */
    cRange = "R" + STRING(t-row). 
    cValue = TRIM(chWorkSheet:Range(cRange):TEXT).
    ASSIGN 
        T-INTERFASE.Autorizacion = cValue.

    /* ID Unico */
    cRange = "N" + STRING(t-row). 
    cValue = TRIM(chWorkSheet:Range(cRange):TEXT).
    ASSIGN 
    ...
    ASSIGN
        ...
        T-INTERFASE.CodMon = (IF T-INTERFASE.Moneda <> "PEN" THEN 2 ELSE 1).

END.
SESSION:SET-WAIT-STATE('').
{lib/excel-close-file.i}

MESSAGE "Se cargaron " + STRING(x-total-registros) + " Registro(s)"
    VIEW-AS ALERT-BOX INFORMATION.
