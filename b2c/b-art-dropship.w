...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      VtaTabla.Llave_c1 COLUMN-LABEL "Artículo" FORMAT "x(8)":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 93 BY 23.15
         FONT 4.
...
"VtaTabla.Llave_c1" "Artículo" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
IF NOT CAN-FIND(FIRST Almmmatg WHERE Almmmatg.codcia = s-codcia AND
                Almmmatg.codmat = VtaTabla.Llave_c1:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
                NO-LOCK) THEN DO:
