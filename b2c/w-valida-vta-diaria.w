...
T-INTERFASE.Estado T-INTERFASE.Id_Unico T-CPEDI.NroPed T-CPEDI.NomCli 
...
BUTTON-Procesar BUTTON-Aprobar BUTTON-buscar RADIO-SET-cuales ~
FILL-IN-pedido FILL-IN-FchDoc-2 BROWSE-2 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-cuales FILL-IN-pedido ~
FILL-IN-FchDoc FILL-IN-FchDoc-2 FILL-IN-1 
...
DEFINE VARIABLE FILL-IN-1 AS CHARACTER FORMAT "X(100)":U INITIAL "Fecha de emision de las cotizaciones CONTINENTAL" 
      VIEW-AS TEXT 
     SIZE 46 BY .5
...
DEFINE FRAME F-Main
     BUTTON-filtrar AT ROW 1.42 COL 129 WIDGET-ID 16
     BUTTON-Procesar AT ROW 1.54 COL 55 WIDGET-ID 4
     BUTTON-Aprobar AT ROW 1.54 COL 76 WIDGET-ID 6
     BUTTON-buscar AT ROW 1.54 COL 173.72 WIDGET-ID 18
     RADIO-SET-cuales AT ROW 1.62 COL 95 NO-LABEL WIDGET-ID 10
     FILL-IN-pedido AT ROW 1.62 COL 159.14 COLON-ALIGNED WIDGET-ID 14
     FILL-IN-FchDoc AT ROW 1.88 COL 19 COLON-ALIGNED WIDGET-ID 2
     FILL-IN-FchDoc-2 AT ROW 1.88 COL 37 COLON-ALIGNED WIDGET-ID 8
     BROWSE-2 AT ROW 3.15 COL 2 WIDGET-ID 200
     FILL-IN-1 AT ROW 1.23 COL 3 COLON-ALIGNED NO-LABEL WIDGET-ID 24
     RECT-34 AT ROW 1.35 COL 93.57 WIDGET-ID 20
     RECT-35 AT ROW 1.38 COL 149.72 WIDGET-ID 22
...
"T-INTERFASE.Importe" "Importe!Transaccion" ? "decimal" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   = Temp-Tables.T-INTERFASE.Estado
     _FldNameList[12]   = Temp-Tables.T-INTERFASE.Id_Unico
     _FldNameList[13]   > Temp-Tables.T-CPEDI.NroPed
"T-CPEDI.NroPed" "Cotización" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[14]   > Temp-Tables.T-CPEDI.NomCli
...
DO:
    ...
    /*FILL-IN-FchDoc:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(fill-in-fchdoc + x-rango,"99/99/9999").*/
    ...
    FOR EACH T-CPEDI:
        ...
        IF AVAILABLE b2c-interfase THEN DO:
            ...
            ASSIGN
                ...
                b2c-interfase.libre_char[3] = T-CPEDI.Libre_c01         /* FAC/BOL */
                b2c-interfase.libre_char[4] = T-CPEDI.Libre_c02         /* numero */
                .
        END.
    END.
    EMPTY TEMP-TABLE T-CPEDI.
...
FOR EACH Faccpedi NO-LOCK WHERE Faccpedi.codcia = s-codcia AND
    ...
    x-NroOrd = STRING(Faccpedi.OrdCmp).
    /*
    ASSIGN
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-cuales FILL-IN-pedido FILL-IN-FchDoc FILL-IN-FchDoc-2 
          FILL-IN-1 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-34 RECT-35 BUTTON-filtrar BUTTON-Procesar BUTTON-Aprobar 
         BUTTON-buscar RADIO-SET-cuales FILL-IN-pedido FILL-IN-FchDoc-2 
         BROWSE-2 
      WITH FRAME F-Main IN WINDOW W-Win.
